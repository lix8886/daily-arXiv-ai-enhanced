...
> This paper introduces two hybrid algorithms for efficient and verifiable multi-hop question answering over knowledge graphs, one guided by LLMs and the other by fused text and graph embeddings.

<details>
  <summary>Details</summary>

**Motivation:** To address the computational challenges of multi-hop question answering over knowledge graphs, improve efficiency, and ensure verifiable grounding without relying solely on expensive LLM inference.

**Method:** Multi-hop question answering over knowledge graphs using two hybrid algorithms: LLM-Guided Planning and Embedding-Guided Neural Search. LLM-Guided Planning predicts relation sequences via a single LLM call with BFS, while Embedding-Guided Neural Search fuses text and graph embeddings with a lightweight model.

**Result:** LLM-Guided Planning achieves near-perfect accuracy with grounded answers, and Embedding-Guided Neural Search is over 100x faster with competitive accuracy. Knowledge distillation compresses planning into a 4B-parameter model, matching large-model performance without API costs.

**Conclusion:** Verifiable multi-hop reasoning can be efficiently achieved with the right architectural inductive biases that combine symbolic structure with learned representations, rather than large models at inference time.
...
> 研究评估了LLMs在波斯语情绪分类中的解释准确性，发现尽管分类性能优良，解释却往往偏离准确推理，强调了改进解释方法的必要性。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型（LLMs）越来越常用来自动生成与其预测相关的解释。然而，这种做法在较低资源语言中的解释准确性引起了关注。这项研究旨在评估LLMs生成的解释在低资源语言波斯语情绪分类中的准确性。

**Method:** 本研究评估了LLM在波斯语情绪分类中生成解释的准确性。通过比较模型识别的重要词与人工注释者识别的词来评估准确性。采用基于token级对数概率的信心得分进行评估。研究测试了两种提示策略——预测后再解释和先解释后预测——对解释准确性的影响。

**Result:** 研究结果表明，尽管LLM在情绪分类上性能良好，但生成的解释往往偏离了准确推理，更符合模型之间的认知，而不是人类的判断。

**Conclusion:** 研究结果突显了当前解释方法和度量的局限性，强调在多语种和低资源环境中需要更稳健的方法来确保LLM的可靠性。
...
> 研究比较了10种针对心脏病学调整的变压器模型，发现BioLinkBERT在领域内表现最好且计算资源需求少。

<details>
  <summary>Details</summary>

**Motivation:** 为了评估在临床自然语言处理中的领域特定文本嵌入性能，尤其是心脏病学领域，并对比不同模型架构的表现。

**Method:** 使用了Low-Rank Adaptation (LoRA) 对十种基于变压器的嵌入模型进行了调整，并在来自权威医学教科书的106,535个心脏病学文本对上进行了微调。

**Result:** 研究结果表明，编码器仅的架构，特别是BioLinkBERT，在领域特定性能上表现出色（分离分数：0.510），相比之下，较大的解码器架构模型性能较差，但计算资源需求显著较少。

**Conclusion:** 研究结果挑战了大语言模型在生成更好的领域特定嵌入方面的假设，并为临床NLP系统的开发提供了实用的指导。
...
> 语言的理解需要大脑将语言信息转移到其他区域进行深入处理，认知神经科学的进步提供直接测试这一理论的方法。

<details>
  <summary>Details</summary>

**Motivation:** 解释语言理解不仅仅是提取语言输入的表面意义，还包括构建其描述情景的丰富心理模型。

**Method:** 提出大脑核心语言系统本身是有限的，语言的深刻理解需要将信息从语言系统转移到其他脑区，这些脑区负责感知和运动表征、构建心理模型以及存储世界知识和自传体记忆。

**Result:** 审查了支持这一假设的现有证据，并论证了认知神经科学的近期进展提供了直接测试该假设的概念基础和方法。

**Conclusion:** 开辟了一种新的策略，揭示从认知和神经层面理解语言的意义。
...
> 研究探讨了大语言模型在情感理解方面是否存在性别偏见，并提出几种去偏策略，表明仅靠推理时的提示策略并不能显著减少偏见，需要依赖训练时的干预措施。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型在日常生活中的应用越来越多，确保其公平性变得愈发重要。本文旨在考察这些模型在情感理解方面的性别偏见。

**Method:** 研究分析了大语言模型在面对人物及环境描述，并询问该人物感受时是否存在性别偏见，同时评估了几种去偏策略的效果。

**Result:** 结果显示，仅靠推理阶段的策略如提示工程并不能显著减少偏见，需要通过训练过程的干预来实现更有效的减偏效果。

**Conclusion:** 结论指出，大语言模型的去偏策略应当重视训练阶段的干预措施，而不仅仅是推理时的简单方法调整。
...
> 研究扩展了NRC VAD词典，增加了对多词表达情感维度的评分，涵盖了10000个多词表达和25000个单词，增强了词典数据的可靠性和适用范围。

<details>
  <summary>Details</summary>

**Motivation:** 现有的VAD词典（如2018年的NRC VAD词典）缺乏对多词短语的情感描述，此项研究的动机在于补充和扩展VAD词典，为这些短语提供情感维度的评分。

**Method:** 该研究通过人类评分的方法，为10000个英文短语表达（MWEs）及其组成单词提供了情绪维度（VAD）的评分，增加了单字词的覆盖率，特别是自2018年以来使用更频繁的单词。

**Result:** 新版NRC VAD词典v2现在包含了10000个短语和25000个单词的条目，除了一版已有的条目。结果表明这些情绪关联性评分具有很高的可靠性，并揭示了短语在情感强度和情感组合性上的特征。

**Conclusion:** 更新后的词典能够支持自然语言处理、心理学、公共卫生、数字人文和社会科学等多个领域的广泛研究。新版词典v2是免费提供的。
...
> 本文介绍了一种利用表情符号和词汇实现自动情绪标注的独立语言方法，尤其适用于低资源语言，能有效减少手动标注的工作。

<details>
  <summary>Details</summary>

**Motivation:** 由于非洲语言如Sepedi和Setswana属于低资源语言，缺乏数字语言资源，手动标注文本数据耗时且昂贵，因此需要自动快速的方法减少人工标注的负担。

**Method:** 本文提出了一种基于情绪表情符号和词汇信息的自动语言独立情绪标签方法。

**Result:** 实验结果显示，该方法能够以66%的准确率给英语推文标注情绪标签，以69%的准确率给Sepedi推文标注情绪标签，以及以63%的准确率给Setswana推文标注情绪标签。总体来说，只需手动更正大约34%的自动标注标签。

**Conclusion:** 本文的方法有助于减少手动标注情绪标签的工作，对于低资源语言的情绪分析特别有帮助。
...
> 本文提出了一种名为PersonaPulse的框架，用于动态优化大语言模型中的人格表现，实验表明，该框架比以前的方法更有效，并对人格特质的唤起提供了新的见解。

<details>
  <summary>Details</summary>

**Motivation:** 尽管之前的研究探索了使用提示来激发大语言模型中的特定人格特质，但它们并未优化这些提示以最大化人格表达。为解决这一局限性，提出了PersonaPulse框架。

**Method:** PersonaPulse是一种动态档案优化框架，旨在通过迭代增强角色扮演提示并结合情境响应基准作为评分工具，以优化大语言模型中的人格表现。

**Result:** 定量评估表明，PersonaPulse生成的提示优于基于心理学研究设计的先前工作中的提示。实验还揭示了模型大小和人格建模之间的关系，并且对于某些人格特质，通过暂停优化过程可以部分控制人格的唤起程度。

**Conclusion:** 这些发现强调了提示优化在塑造大语言模型中人格表达的重要性，并为适应性人工智能交互的未来研究提供了宝贵的见解。
...
> 研究评估了零样本提示、单静态随机示例提示（SPR）和检索增强动力提示法（RDP）在医学错误处理任务中的表现，结果表明RDP策略在几乎所有方面都优于其他两种策略。

<details>
  <summary>Details</summary>

**Motivation:** 医学文档中存在可能会威胁患者安全的事实，诊断和管理错误。大型语言模型（LLMs）可能有助于检测和纠正这些错误，但其在不同的提示策略下的表现仍有待明确。

**Method:** 该研究使用了MEDEC数据集来评估九种指令调整后的大型语言模型（包括GPT，Claude，Gemini和OpenAI的o系列模型）在医学错误处理三个子任务中的表现：错误标记检测，错误句子检测和错误纠正。使用准确率，召回率，误检率（FPR）以及ROUGE-1，BLEURT和BERTScore的综合得分来衡量错误纠正任务的表现，并对示例输出进行分析以识别失败模式和LLM与临床医生推理之间的差异。

**Result:** 零样本提示在两个检测任务中的召回率低，常遗漏缩写密集或非典型错误。与零样本提示相比，静态随机示例提示（SPR）改善了召回率但增加了FPR。在所有九个LLM中，检索增强动力提示法（RDP）将FPR降低了约15％，在错误句子检测中的召回率提高了5到10％，并生成了更符合语境的纠正结果。

**Conclusion:** 跨不同模型，RDP优于零样本提示和SPR提示，提高了检测准确性，减少了误检并增强了对医学错误纠正的可靠性。
...
> 本文介绍了AppSelectBench，一个全面的基准测试程序，用于评估计算机使用代理(CUAs)中的应用程序选择能力，它生成大量真实、多样且语义相关的用户任务，优化跨应用程序的推理和选择。

<details>
  <summary>Details</summary>

**Motivation:** 动机在于现有的基准主要评估细粒度API的选择，无法了解模型在不同应用程序之间进行选择和推理的能力。因此，AppSelectBench旨在填补这一研究空白。

**Method:** 介绍了AppSelectBench，这是一个用于评估计算机使用代理(CUAs)中应用程序选择的全面基准。它包括一个新颖的用户任务生成管道，可生成大量的真实、多样且语义相关的用户意图，并涵盖随机选择、启发式选择、零样本、少样本和检索增强设定的统一评估协议。

**Result:** 实验表明，即使是最能力强的模型，在应用程序选择上也难以保持一致性，这表明AppSelectBench可以作为评估模型应用程序级推理的关键工具。

**Conclusion:** AppSelectBench确立了作为研究和提升CUAs中应用程序级推理能力的基础地位，这是一个智能CUAs中必需但又鲜少被探索的能力。
...
> 我们提出了R2R框架，以解决通用模型在特定领域中由于表面形式过拟合和灾难性遗忘而失去细微差别的问题。R2R通过动态专家路由和实体抽象化以泛化策略，展示了跨领域的强大鲁棒性，超越了一般和单一领域的基线。

<details>
  <summary>Details</summary>

**Motivation:** 通用模型错过了像金融和法律这些高风险领域的特定领域细微差别，而简单的微调会导致表面形式过拟合和灾难性遗忘。为了应对这一挑战，我们提出了R2R。

**Method:** R2R是一个结合动态专家路由和两阶段训练策略（实体抽象化以泛化，EAG）的领域感知框架。EAG通过屏蔽最具预测性的表面线索，迫使重新排序器学习领域不变的相关性模式，而不是记忆特定于数据集的实体。为了高效激活领域专家，R2R使用了一个轻量级的潜在语义路由，它从冻结的基础解码器的内部表示中探查，以选择每个查询的最佳LoRA专家。

**Result:** 在不同重新排序器基础模型和多样化领域（法律、医疗和金融）的广泛实验中，结果表明，R2R始终超越通用和单一领域微调的基线模型。

**Conclusion:** R2R是一种模型无关且模块化的方法，用于领域专门化，并具有强大的跨领域鲁棒性。
...
> 研究揭示了因果Transformer训练中内在的方向性摩擦，即使去除语言学先验、标记频率和语料库级别的时间不对称性，这种方向性摩擦依然存在。

<details>
  <summary>Details</summary>

**Motivation:** 探讨语言模型中的方向性失效是由于语言学统计特性还是模型架构本身引起的。

**Method:** 使用了一个完全合成的、控制了熵的基准测试，旨在成为一个干净的房间压力测试，用于方向性学习。通过调节分支因子K，构造了前向任务和逆向任务。

**Result:** 即使是未经过预训练的GPT-2模型也会表现出强而可重复的方向性优化差距，这比在相同数据上训练的MLP模型更大。预训练初始化会改变优化行为，但不会消除这种差距。

**Conclusion:** 实验结果突出了在现代序列模型中的方向性偏差，并强调了需要更深入地探讨为什么对Transformer而言，反转从根本上更难。
...
> 本文提出了一种统一的多类分类框架来检测社交媒体中的心理健康和网络欺凌类别，展示了端到端微调方法优于其他模型，并提出了一个作为人类辅助筛选工具的原型仪表板。

<details>
  <summary>Details</summary>

**Motivation:** 心理健康问题和网络欺凌在数字空间中日益普遍，需要可扩展且可解释的检测系统。

**Method:** 介绍了一种用于检测来自社交媒体数据的十种不同心理健康和网络欺凌类别的统一多类分类框架。数据集来自Twitter和Reddit，采用严格的“分割后平衡”管道进行训练和评估。评估了传统的基于词典的模型、混合方法和几种端到端微调的变压器模型。

**Result:** 实验结果显示端到端微调对于性能至关重要，具体而言，领域自适应的MentalBERT在准确率(0.92)和Macro F1值(0.76)方面表现最佳，超过了其通用版本和零样本大语言模型。

**Conclusion:** 提出的工作提供了一个有力的基线，并强调了未来对多标签、临床验证数据集的需求，特别是在在线安全和计算心理健康的关键交汇点。
...
> 本文提出了Online-PVLM框架，利用双曲表示技术实现个性化视觉语言模型的在线概念学习，解决其实时适应新概念的难题，并提供了大规模评估基准OP-Eval。

<details>
  <summary>Details</summary>

**Motivation:** 现有的个性化视觉语言模型难以实现实时适应新概念，尤其在大规模场景下难以高效检索概念嵌入，而我们的方法旨在解决这一问题。

**Method:** 我们提出了一种名为Online-PVLM的框架，通过利用双曲表示在线学习概念，实现了在测试阶段无训练生成概念嵌入，提高了个性化视觉语言模型的可扩展性和效率。

**Result:** 通过实验验证，我们的框架显示出了最先进的性能。

**Conclusion:** 我们的方法实现了高效、可扩展的个性化视觉语言模型，并通过大规模实验验证了其有效性。
...
> 本文提出了一种名为MTA的新框架，旨在通过共享元LoRA Bank、自适应LoRA融合和少量数据学习LoRA堆叠，改善个人化大型语言模型的可扩展性和泛化能力，实验结果表明该方法有效。

<details>
  <summary>Details</summary>

**Motivation:** 传统的为每位用户单独微调的方法面临存储成本高和对于数据稀疏用户泛化性能不佳的问题。为解决这些问题，我们提出了MTA框架，以实现更高效和灵活的个人化语言模型。

**Method:** 我们提出了MTA，一种合并后适应（Merge-then-Adapt）框架，用于个人化大型语言模型。MTA包含三个关键阶段。首先，通过选取锚点用户并预训练元泛化特征来构建共享的Meta-LoRA Bank。其次，引入自适应LoRA融合阶段，以确保可扩展性和动态泛化组合，该阶段从锚点元LoRA中检索并动态合并最相关的元LoRA来合成用户特定的LoRA。最后，提出了一种适用于少量样本场景的LoRA堆叠泛化阶段，通过在合并LoRA之上额外应用一个轻量级低秩LoRA模块，并对其进行微调以实现有效泛化。

**Result:** 在LaMP基准上的广泛实验显示，我们的方法在多个任务上都优于现有的最先进方法。

**Conclusion:** 实验结果表明，我们的方法在LaMP基准测试中的多个任务上优于现有的最先进方法。
...
> 提出了BiasPrompting，该框架改进了大语言模型在多选题任务中的表现，通过生成和支持性推理以及综合选择最佳答案，显著提高了五个多选题问答基准测试的表现。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型（LLMs）的发展，它们在多选题（MCQ）任务上的表现有了显著的提高。然而，现有方法存在关键限制：答案选项通常没有上下文或解释就呈现给LLMs。这种缺乏上下文的情况可能导致不能充分探索所有可能的答案，最终降低模型的推理能力。

**Method:** 提出了一种名为BiasPrompting的新推理框架，该框架指导大语言模型（LLMs）生成并批判性评价所有可能答案的推理，再通过生成的推理综合选择最合理的答案。它包含两个组件：一个推理生成阶段，模型被提示为每个答案选项生成支持性推理；然后是一个推理引导的同意阶段，综合生成的推理以选择最可能的答案。

**Result:** 通过全面的评估，BiasPrompting在五个广泛使用的多选题问答基准测试中表现出显著的改进，提升了大语言模型的推理能力。

**Conclusion:** 实验结果显示，BiasPrompting显著提高了五个多选题问答基准测试的表现，增强了大语言模型的推理能力，并为处理复杂且具有挑战性的问题提供了一个坚实的基础。
...
> The paper addresses the paradox in sparse attention mechanisms and proposes SSA, a training framework that improves both the sparsity and performance in large language models.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind the research is to address the issue of poor performance in sparse attention mechanisms, especially when training is not involved. The research aims to improve the sparsity of attention without degrading performance.

**Method:** The paper proposes SSA (Sparse Sparse Attention), a training framework that unifies sparse and full attention mechanisms. This framework ensures bidirectional alignment at every layer, preserving gradient flow to all tokens and encouraging sparse attention to approximate full attention accurately.

**Result:** SSA achieves state-of-the-art performance across multiple benchmarks, adapts smoothly to varying sparsity budgets, and demonstrates strong long-context extrapolation capability.

**Conclusion:** The findings indicate that the SSA model effectively balances the trade-off between computational efficiency and performance, contributing to the development of efficient long-context processing in large language models (LLMs).
...
> 该论文介绍了一种为混合情绪识别构建的新的多语言语音语料库EM2LDL，该语料库可捕捉多语言环境中情感的复杂动态。

<details>
  <summary>Details</summary>

**Motivation:** 为了克服现有情绪语料库主要以单语言和单情绪标签为主，限制了语言多样性，并且无法模拟混合情绪，同时缺失生态效度的问题；同时捕捉在香港和澳门等多语言地区普通话、粤语、以及英语的频繁使用情况。

**Method:** 该研究引入了EM2LDL，一种为通过标记分布学习推动混合情绪识别的新型多语言语音语料库。EM2LDL包括英语、普通话和粤语的表达用语，捕捉在香港和澳门等多语言地区常见的句内代码切换行为，并包含了来自在线平台的自发情绪表达，精细标注了32类情绪分布。

**Result:** 

**Conclusion:** 通过将语言多样性和生态效度整合，EM2LDL允许探索多语言环境中复杂的感情动态。这项研究为开发用于情感计算应用（如心理健康管理及跨文化沟通）的自适应、同理心系统的测试平台提供了可能性。
...
> 提出了一种无需训练的误读检测和诊断框架，利用检索技术和预训练的自动语音识别模型，达到了69.60%的F1分数。

<details>
  <summary>Details</summary>

**Motivation:** 传统的误读检测需要评分模型或训练发音模型，这种方法复杂且需要大量资源。

**Method:** 采用了一种无需训练的框架，利用检索技术结合预训练的自动语音识别模型来检测和诊断发音错误。

**Result:** 在L2-ARCTIC数据集上的实验表明，该方法实现了69.60%的F1评分。

**Conclusion:** 新方法达到了较高的准确率，同时避免了传统方法中复杂模型训练的问题。
...
> 利用先进的语言模型和少量样本策略改进印度语族语言的语法错误纠正技术，并展示了大型语言模型在多语言任务中的有效性。

<details>
  <summary>Details</summary>

**Motivation:** 提升低资源印度语言的语法错误纠正能力，解决这些语言资源受限、语言多样性和形态复杂所带来的挑战。

**Method:** 本研究采用基于提示的策略，使用了诸如GPT-4.1、Gemini-2.5和LLaMA-4等最先进的大型语言模型，并结合少量样本方法来适应低资源环境。

**Result:** 研究表明，即使是基本的提示策略，例如零样本和少量样本方法，也能使这些大型语言模型在多种印度语言的语法错误修正任务上大幅超越微调过的印度语言模型Sarvam-22B。实验在Tamil（GLEU：91.57）、Hindi（GLEU：85.69）、Telugu、Bangla和Malayalam等语言上获得了领先的结果。

**Conclusion:** 研究结果表明，基于提示的技术在语法错误纠正任务中的有效性，以及大规模语言模型在多语言语法错误纠正中减轻资源缺口的潜力。
...
> An integration of image data augmentation techniques into grid-based models improves the recognition of discontinuous entities in NER.

<details>
  <summary>Details</summary>

**Motivation:** The aim is to address the segmentation and omission issues of discontinuous entities in NER, which are typically missed or missegmented by traditional methods.

**Method:** Our approach integrates image data augmentation techniques, such as cropping, scaling, and padding, into grid-based models to enhance their ability to recognize discontinuous entities.

**Result:** The augmented grid models achieve notable improvements over traditional methods with F1 score gains of 1-2.5% overall and 3.7-8.4% specifically for discontinuous entities on CADEC, ShARe13, and ShARe14 datasets.

**Conclusion:** The effectiveness of augmented grid models is confirmed, showing improvements in the recognition of discontinuous entities with significant F1 score gains over traditional methods.
...
> 提出KyrgyzBERT作为首个公开的吉尔吉斯语BERT模型，显示出良好的情感分析性能，与更大模型性能相当。

<details>
  <summary>Details</summary>

**Motivation:** 吉尔吉斯语是一种低资源语言，基础NLP工具匮乏。为了填补这一空白，本文引入了KyrgyzBERT模型。

**Method:** 引入了KyrgyzBERT，这是首个公开的针对吉尔吉斯语的单语BERT模型。该模型拥有3590万参数，并使用了一种专门设计以适应该语言词形结构的自定义分词器。

**Result:** 在基于Stanford情感树库手动标注的吉尔吉斯语情感分析基准测试集中，KyrgyzBERT微调后的F1评分为0.8280，与其相比，一个体积大五倍的微调mBERT模型表现相当。

**Conclusion:** 所有模型、数据和代码均公开发布，以支持未来吉尔吉斯语NLP研究。
...
> 文章提出了一种名为REFLEX的新框架，旨在通过利用基础模型内部的知识来提高事实核查的准确性和解释质量，从而解决基于大型语言模型的事实核查系统中的实时性、可靠性和可解释性问题。实验表明，该方法性能优越，尤其是在使用较少自修正训练样本的情况下，表现尤为突出，达到了最先进的水平。此外，具有解释性目标的模型可以指导没有这些目标的模型，提高其性能。

<details>
  <summary>Details</summary>

**Motivation:** 提出REFLEX的动机在于解决现有基于大型语言模型的事实核查系统过度依赖外部知识源所带来的延迟、不可靠性、不可解释性和不及时性问题。

**Method:** REFLEX通过将事实核查重新表述为角色扮演对话，并联合训练判决预测和解释生成。它从基础模型及其微调变体中自适应提取对比激活对，构建导向矢量，解耦事实的本质与风格，以引导推理并抑制噪声解释。

**Result:** 实验表明，REFLEX显著优于其他事实核查方法，特别是在使用较少训练样本的情况下表现出色。具有解释性目标的模型可以提高没有解释性目标的模型性能，显示出内部解释信号在解释和事实推理这两方面的双重作用。

**Conclusion:** REFLEX框架通过利用模型内部信号，改善了事实核查的准确性与可解释性，并在较少训练样本情况下达到顶尖性能，验证了内部解释信号在提高模型性能方面的有效性。
...
> The paper introduces SpecFormer, a novel architecture for speculative decoding of large language models, integrating unidirectional and bidirectional attention mechanisms to achieve efficient and consistent acceleration with lower computational costs.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this work is to develop a method for speculative decoding that requires fewer resources for verification and incurs lower scheduling costs, using more capable models that allow for parallel generation on draft sequences.

**Method:** We propose SpecFormer, an architecture integrating unidirectional and bidirectional attention mechanisms, which combines the capabilities of autoregressive and non-autoregressive models to achieve consistent acceleration across different batch sizes.

**Result:** Through lossless speculative decoding experiments with models of various sizes, SpecFormer demonstrates improved efficiency in terms of training demands and computation costs.

**Conclusion:** SpecFormer establishes a new benchmark for large language model inference by reducing the need for large prefix trees, lowering training demands, and decreasing computational costs while providing consistent acceleration across different batch sizes.
...
> 通过类比推理分析LLMs在处理高阶关系概念时的能力和局限性。

<details>
  <summary>Details</summary>

**Motivation:** 研究LLMs是否能够编码并应用高阶关系概念到新情况中。

**Method:** 通过比例类比和故事类比来探讨LLMs在高阶关系概念编码和应用方面的能力。

**Result:** LLMs能够有效编码类似实体之间的关系，但在应用关系信息到新实体时有困难，且成功的类比推理需要类比情况之间有强结构一致。

**Conclusion:** LLMs在编码和应用高阶关系概念方面表现出有限的新能力，与人类认知既有平行之处也有差距。
...
> BengaliFig数据集针对孟加拉语中的比喻性和文化内涵推理弱点进行了采集和标注，评估了八种前沿的LLM模型，揭示了在低资源文化背景下的推理能力不足。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在多语言基准测试中表现出色，但在比喻性和文化内涵推理方面，尤其是在低资源语境下的评估仍有待完成。

**Method:** 提出了BengaliFig数据集，包含435个独特的谜语，从五个不同的维度进行标注，包括推理类型、陷阱类型、文化深度、答案分类和难度。通过约束感知和AI辅助的管道自动转换为多项选择题格式。

**Result:** 在零样本和少量样本思维链提示下的评估结果显示在比喻性和文化特定推理方面存在持续的弱点。

**Conclusion:** BengaliFig为在低资源文化背景下评估大型语言模型的鲁棒性提供了一个诊断探针，并向着包容性更强和尊重文化传统的自然语言处理评估迈出了重要一步。
...
> 本文提出了一种新的任务导向的词干提取方法评估框架，考虑有效性、下游任务表现及其与原词的语义相似度，并通过比较孟加拉语和英语的两种词干提取器验证了该框架的有效性，揭示了过度词干化对语义的影响。

<details>
  <summary>Details</summary>

**Motivation:** 现有的词干提取方法评估手段有限，不能充分反映过度词干化的负面影响。因此，有必要开发新的评估方法。

**Method:** 提出了一种新的任务导向评估框架，包括词干提取的有效性评分（SES），下游任务性能差异（MPD）和词干化前后语义相似度（ANLD）。

**Result:** 通过应用新框架评估了两种词干提取器：孟加拉语BNLTK词干提取器取得高SES（1.67）但ANLD高（0.26），表明过度词干化；英语Snowball词干提取器取得中等SES（1.31）但ANLD低（0.14），表明相对安全。

**Conclusion:** 研究提供了一种评估工具，可以区分词干提取方法的有效性和语义保持能力，为更可靠的词干提取器评估提供重要参考。
...
> 本文提出了一种框架，该框架利用大型语言模型生成和评估19世纪小说家风格的句子，解决如何在没有配对数据的情况下生成风格文本以及如何不依赖人类判断对其评估的问题。

<details>
  <summary>Details</summary>

**Motivation:** 动机在于推进文体学研究，通过大型语言模型模仿19世纪作家的写作风格，并开发基于人工智能的评估方法来评估生成文本的风格真实性。

**Method:** Structure

**Result:** {
  "tldr": "The paper presents a framework that leverages large language models to generate and evaluate sentences in the style of 19th-century novelists, addressing the challenges of generating stylistic text without paired data and evaluating it without solely relying on human judgment.", 
  "motivation": "The motivation is to advance the field of stylometry by using large language models to imitate the writing styles of 19th-century authors and to develop AI-based evaluation methods that assess the stylistic authenticity of generated text.", 
  "method": "The method includes fine-tuning large language models with minimal prompts to generate text in the style of targeted authors. The evaluation leverages a transformer-based model trained on authentic sentences to classify and explain stylistic texts, along with syntactic comparisons and explainable AI techniques.", 
  "result": "The results show that the generated sentences reflect the characteristic patterns of the targeted 19th-century authors, and AI-based evaluation provides a reliable alternative to human judgment.", 
  "conclusion": "The conclusion is that the framework can effectively generate and evaluate stylistic text in the voices of 19th-century novelists, showcasing the potential of AI in the study of historical written styles. The findings support the use of large language models and AI evaluation tools as valuable resources in the field of stylometry. "
}

**Conclusion:** 本文得出的结论是，该框架可以有效地生成并评估出19世纪小说家风格的文本，展示了人工智能在研究历史书面风格上的潜力。该研究成果支持大型语言模型和基于人工智能的评估工具作为该领域内有价值的资源。
...
> 本文介绍了一种新的威胁——对抗性困惑攻击，可以通过一个对抗性图像在多个开源及专有模型中引起混乱，即使使用的是基本的对抗技术也能实现跨模型干扰。

<details>
  <summary>Details</summary>

**Motivation:** 该研究的动机在于探索和展现多模态大型语言模型潜在的安全威胁，开发能够跨模型干扰的攻击方法，以提高对这些系统的理解和防御策略。

**Method:** 使用小规模开源多模态大型语言模型的集合，攻击通过最大化下一个标记的熵来进行。在白盒设置下，一个对抗性图像能扰乱集合中的所有模型，不论是全图像还是对抗性验证码设置。

**Result:** 通过引入对抗性困惑攻击，研究团队揭示了一种针对多模态大型语言模型的新威胁。与越狱或目标误分类不同，这种攻击旨在系统性地扰乱模型，使其生成不连贯或自信地错误输出。实验表明，单一的对抗性图像能够同时破坏多个模型，并且这种攻击即使依赖基本的对抗技术（如PGD），也能迁移至未见过的开源和专有模型中。

**Conclusion:** 研究结果表明，即便是简单的对抗技术也能生成能够跨模态模型转移的干扰，这对保护多模态大型语言模型的稳定性和可靠性具有重要意义。
...
> 本文介绍了文本失语症测试（TAB），这是为评估大语言模型中类似失语症的缺陷而设计的，提供了一个自动化评估协议，该协议在可靠性方面与人类专家相当，TAB被视为分析人工系统中语言缺陷的临床依据和可扩展框架。

<details>
  <summary>Details</summary>

**Motivation:** 鉴于传统的临床评估方法并不适用于大语言模型，因为这些评估假设了人为的语用压力和认知过程，这些问题并不存在于人工架构中。因此，需要一种适用于评估大语言模型中类似失语症缺陷的新方法。

**Method:** 介绍了一种称为文本失语症测试（TAB）的新的基准测试，该测试从快速失语症测试（QAB）改编而来，旨在评估大语言模型（LLMs）中类似失语症的缺陷。TAB包括四个子测试：连贯文本、单词理解、句子理解和重复。此外，为大规模使用提供了一个自动化评估协议，该协议使用Gemini 2.5 Flash进行验证，达到了与专家人工评估相当的可靠性。

**Result:** 验证了一个自动化评估协议，使用Gemini 2.5 Flash实现了与专家人工评估相当的可靠性（加权Cohen's kappa分别为模型与共识协议的0.255和人类与人类评估的0.286）。

**Conclusion:** 作为一种以临床为基础、可扩展的框架，TAB可以用于分析大语言模型中的语言缺陷，这为理解人类类似的语言障碍提供了一种新的计算基础。
...
> 论文介绍了一种新的数据增强技术，解决了低资源语言在自动语音识别中的性能差距问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的机器学习模型在英语和其他资源丰富的语言上表现出色，但在低资源语言上性能较差，因为数据收集困难且成本高。

**Method:** 引入了一种新的语音语料库数据增强技术，旨在缩小低资源语言在自动语音识别系统性能上的差距。

**Result:** 实验表明，该方法显著提升了低资源语言在自动语音识别系统上的性能，并优于现有数据增强策略。

**Conclusion:** 该方法为改善代表性不足的语言社区的语音技术提供了一种实用的解决方案。
...
> 本研究提出一个标注的教育对话数据集，使用GPT-3.5和Llama-3.1预测学生对话中的知识构建和任务生产话语特征。实验表明这些模型在任务上表现欠佳。

<details>
  <summary>Details</summary>

**Motivation:** 自动检测学生对话中的语用特征对于教育研究者识别影响学生知识构建的课程和教学变量具有重要意义。然而，手动分析这种对话既耗时又耗力。现有的自然语言处理(NLP)研究通常不涉及教育数据。

**Method:** 本研究通过引入一个标注的教育对话数据集，该数据集包含学生对话中知识构建和任务生产的话语特征。同时，使用预训练的大规模语言模型GPT-3.5和Llama-3.1来建立基线模型，预测对话中每一转折点的话语性质。

**Result:** 实验结果表明，这些最先进的模型在这个任务上的表现低于预期，显示出未来研究的潜力。

**Conclusion:** 研究显示，虽然当前的预训练模型在自动检测教育对话中的语用特征方面表现不佳，但为未来该领域的研究奠定了基础。
...
> 研究了大语言模型的生成与评估能力之间的关系，提出了一种新的基准测试方法AlignEval，它可以有效评估模型与人类偏好对齐的程度，无需直接评估模型生成的输出。

<details>
  <summary>Details</summary>

**Motivation:** 研究大语言模型生成和评估能力之间的关系，旨在开发一种能够评估大语言模型与人类偏好对齐程度的新基准测试方法。

**Method:** 通过综合分析各种大语言模型的生成-评估一致性，研究了大语言模型在符合人类偏好方面的生成与评估能力之间的关系。利用这一发现，提出了一种新的基准测试范式，该范式无需直接评估模型生成的输出，而是在模型作为评估者的角色中进行评估。

**Result:** 研究表明，所提出的基准测试AlignEval在捕捉人类偏好以及对大语言模型进行排名方面，与广泛使用的自动评估基准（如AlpacaEval和Arena-Hard）相当或更优。

**Conclusion:** 研究提供了关于大语言模型生成与评估能力之间联系的重要见解，引入了一种无需直接评估模型生成输出就能评估对齐性的基准测试方法。
...
> 论文提出了一个无训练需求的多智能体系统框架——LatentMAS，允许模型在连续潜在空间中直接合作，相比现有方法它具有更高的表达能力、信息保真度与效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型（LLMs）依赖于基于文本的中介进行推理和沟通，而LatentMAS提出了一种无训练、纯潜在空间合作框架，允许模型在连续潜在空间中直接协作，以实现更高效的系统级智能。

**Method:** LatentMAS框架内，每个代理首先通过自回归潜在思想生成，利用最后一层隐藏嵌入进行操作。然后，通过一个共享的潜在工作记忆来保存和传递每个代理的内部表征，确保无损信息交换。

**Result:** 理论分析显示，LatentMAS在表达能力和信息保真度上优于传统文本介导的多智能体系统，且复杂度更低。实证评估显示，LatentMAS超越了单模型和文本介导的MAS基线，实现了14.6%的准确率提升，减少了70.8%-83.7%的输出标记使用，并且推理速度加快了4到4.3倍。

**Conclusion:** LatentMAS不仅提高了系统级推理质量，还实现了显著的效率增益，而不需任何额外的训练。
...
> PuzzlePole作为新型识别标记，提供360°视角的可靠识别与姿态估计，适用于多种自主系统场景。

<details>
  <summary>Details</summary>

**Motivation:** 环境的可靠感知是自主系统的关键，校准和定位任务经常依赖于稳健的视觉标记。

**Method:** 引入了PuzzlePole，这是一种新型的源自PuzzleBoard校准模式的识别标记。PuzzlePole是一个圆柱形标记，可以从360°视角进行可靠识别与姿态估计。通过利用其独特的组合结构，PuzzlePole在定位和定向上提供高精度且具有抗遮挡性。

**Result:** 实现了高精度定位和定向，具有抗遮挡性。

**Conclusion:** 设计为多种自主系统场景提供了灵活性，包括机器人导航和SLAM到有形接口。
...
> PIGReward is a personalized reward model for text-to-image generation, which dynamically evaluates images based on user preferences using CoT reasoning and a self-bootstrapping strategy.

<details>
  <summary>Details</summary>

**Motivation:** To address the challenge of evaluating the alignment of generated images with user preferences using conventional methods.

**Method:** PIGReward uses a self-bootstrapping strategy to reason over limited reference data to construct user contexts, and it provides personalized feedback that can be used to optimize prompts.

**Result:** Experiments show that PIGReward surpasses existing methods in both accuracy and interpretability, with the help of PIGBench, a per-user preference benchmark.

**Conclusion:** PIGReward establishes a scalable reasoning-based foundation for personalized text-to-image generation evaluation and optimization.
...
> 研究团队提出了SG-OIF框架，有效解决了深度学习视觉模型中影响估计的问题，通过实时稳定性控制，充分利用轻量级计算和多模块方法，改善了噪音标签和分布外检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 提升深度学习视觉模型中对训练样本对测试预测的影响的近似，这对于查找噪音数据至关重要，但现有的影响函数方法因逆曲率计算昂贵及训练非平稳问题而导致挑战。

**Method:** 引入了稳定性引导在线影响框架（SG-OIF），这是一个将算法稳定性作为实时控制器的框架，它（i）通过随机Richardson和预处理Neumann保持轻量级锚IHVPs；（ii）提出了模块化曲率后端，使用稳定性指导的残差阈值、异常门控和信心来调节每个示例的影响分数。

**Result:** 实验结果显示，在多个数据集和不同类型的损坏下，SG-OIF在噪声标签和分布外检测任务上达到了最先进（SOTA）的效果。

**Conclusion:** SG-OIF框架在嘈杂和异常数据检测任务中展示了卓越的性能，尤其在CIFAR-10和MNIST数据集上达到了高准确性和AUPR分数，证明其作为在线影响估计算法的实用性。
...
> 介绍了Pistachio，一个新的视频异常检测和理解基准，它通过生成模型克服了现有基准的限制，展现了新的研究挑战。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频异常检测基准缺乏场景多样性、异常平衡覆盖和时间复杂度，无法可靠地评估现实世界性能，而视频异常理解需要更深层次的语义和因果推理，但其基准测试难度大，因为需要大量的手动标注工作。

**Method:** 通过控制的、基于生成的管道构建了一个新的视频异常检测和理解基准（Pistachio），利用近期的视频生成模型进步，实现了对场景、异常类型和时间叙事的精确控制。

**Result:** 实验展示了Pistachio的规模、多样性和复杂性，揭示了现有方法面临的新挑战。

**Conclusion:** Pistachio的引入揭示了现有视频异常检测和理解方法的新挑战，并激励未来在这两个领域的研究。
...
> This paper introduces SATA, a universal framework for tracking and segmentation tasks that employs a Decoupled Mixture-of-Expert mechanism and Task-aware Multi-object Tracking pipeline to address cross-modal and cross-task challenges, offering superior performance on multiple benchmarks.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind the paper is to tackle the limitations presented by existing specialized architectures for tracking and segmentation tasks, which often struggle with generalization and scalability due to the distributional and feature representation gaps across different tasks and modalities.

**Method:** The method consists of a Decoupled Mixture-of-Expert (DeMoE) mechanism that segments unified representation learning into shared cross-modal knowledge and specific information. A Task-aware Multi-object Tracking (TaMOT) pipeline then unifies task outputs, maintaining calibrated ID information and alleviating knowledge degradation during multi-task training.

**Result:** The proposed SATA framework demonstrated superior performance across 18 challenging benchmarks in tracking and segmentation, indicating its effectiveness in addressing cross-modal and cross-task challenges.

**Conclusion:** SATA, a unified tracking and segmentation framework, effectively addresses cross-modal and cross-task challenges through a Decoupled Mixture-of-Expert (DeMoE) mechanism and Task-aware Multi-object Tracking (TaMOT) pipeline, showcasing superior performance on various benchmarks and proposing a new direction for generalizable video understanding systems.
...
> 本文采用DRaM方法解决准确的EnP和OnP问题，提供了解决有噪声数据的姿态估计问题的方案，并且分析了该方法在3D和2D正射投影姿态估计问题中的行为。

<details>
  <summary>Details</summary>

**Motivation:** 本文旨在通过解决误差无和有噪情况下的EnP及OnP问题来提供新的解决方案，并通过DRaM方法深化对这些问题的理解。

**Method:** 本文采用DRaM方法解决3D和2D正射投影姿态估计问题，该方法解决了准确的EnP和OnP问题，并展示了这些方案在处理有噪声数据时的校正旋转方案。

**Result:** 本文提出了DRaM方法来解决准确的EnP和OnP问题，并在面对有噪声数据时提供了一种直接的旋转校正方法，强化了一种普适性的解决方案。

**Conclusion:** 本文不仅提供了新的3D和2D正射投影姿态估计问题解决方案，还加深了对这些姿态估计问题的理解，并指出类似高斯时代的DRaM解决方案可以解决各种N维欧式姿态估计问题。
...
> LatentDreamer, a novel framework, efficiently generates high-fidelity 3D objects from single images using a pre-trained variational autoencoder, achieving high detail and speed.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to address the challenge faced by existing automatic 3D generation methods, which often cannot achieve speed, detail, and high fidelity simultaneously.

**Method:** The key method is LatentDreamer, a framework that uses a pre-trained variational autoencoder to map 3D geometries to latent features, enabling the generation of 3D objects from single images in a fast and detailed manner.

**Result:** With only a small amount of training, LatentDreamer shows competitive performance compared to other contemporary approaches.

**Conclusion:** The proposed LatentDreamer method successfully demonstrates the capability to generate 3D objects with high fidelity and at high speed from single images.
...
> 研究模型容量与图像语义标记数之间的关系，提出正交过滤模块以提高效率。

<details>
  <summary>Details</summary>

**Motivation:** 探究模型容量与保持图像语义所需的最小视觉标记数量之间的基本关系。

**Method:** 通过重新解释图像标记为视觉语义空间中的向量，并定义图像的语义复杂度为所需最小基向量集以跨越该空间。基于这一视角，我们提出一种轻量级模块正交过滤，适应性地将冗余标记聚类成一组正交基。

**Result:** 通过在一系列ViT模型上的广泛实验，揭示了一个一致的标记、模型扩展法则：更大的模型需要显著较少的标记来跨越视觉语义空间。此外，我们还贡献了一个视觉长上下文数据集。

**Conclusion:** 研究了模型大小与视觉标记之间关系，并展示了正交过滤模块在降低需要处理的标记数量上的有效性。
...
> GroundingAgent 是一种新颖的无任务特定微调的视觉定位框架，依靠预训练模型和迭代推理机制，在多个基准上实现了65.1%的零样本定位准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法依赖大量的任务特定注释和精细调整，限制了它们在新型或分布外场景中的泛化能力。为了克服这些限制，研究提出了GroundingAgent框架。

**Method:** GroundingAgent 通过结合预训练的开放词汇对象检测器、多模态大型语言模型（MLLMs）和大型语言模型（LLMs），采用结构化、迭代推理机制，逐步通过语义和空间分析改进候选区域。

**Result:** GroundingAgent 在 RefCOCO、RefCOCO+ 和 RefCOCOg 等基准上实现了65.1%的零样本定位准确率，且通过将 MLLM 生成的标题替换为原始查询文本，在选择阶段的准确率达到了约90%。

**Conclusion:** GroundingAgent 在无需精细调整的情况下展示了极好的零样本定位能力，并提供了强解释性。
...
> 本文提出了一种信息理论框架InfoPrune，用于自适应地压缩视觉语言模型，既能减少模型的计算量，又不影响性能。

<details>
  <summary>Details</summary>

**Motivation:** 虽然视觉语言模型（VLMs）在多模态任务中表现出色，但其规模不断增长带来了部署和效率方面的严峻挑战。现有压缩方法依赖于启发式重要性指标或经验性剪枝规则，缺乏关于信息保留的理论保证。

**Method:** 我们提出了一种基于信息论的框架InfoPrune，用于视觉语言模型（VLMs）的自适应结构压缩。该框架引入了熵基有效排名（eRank）以及K-S距离来衡量剪枝后结构的差异。InfoPrune设计了两种互补方案，一种是基于训练的头部剪枝，另一种是无需训练的FFN压缩。

**Result:** 实验表明InfoPrune可实现高达3.2倍的FLOP减少和1.8倍的加速，且性能下降几乎不显著。

**Conclusion:** 实验结果表明，InfoPrune在多个数据集上取得了显著的FLOP减少以及加速效果，证明了其在高效多模态大规模模型中的理论奠基性和实用性。
...
> 我们提出了一种新的可重复性眼睑角度（ELA）指标，该指标能有效检测驾驶员疲劳状态，提供稳定和鲁棒的眨眼检测，并能生成逼真的眨眼动态的合成数据集。该方法在公共数据集上验证了其有效性和多样性扩展能力。

<details>
  <summary>Details</summary>

**Motivation:** 我们的动机在于提供一种稳定且鲁棒的几何描述方法来检测驾驶员的疲劳状态，该方法能够抵御摄像头角度变化的影响，并且可以生成可控的、多样化的训练数据以支持驾驶员状态监控。

**Method:** 我们引入了一种新的可重复性眼睑角度（Eyelid Angle, ELA）指标，该指标基于3D面部特征点提取眼睛张开度。该方法不仅包括眨眼检测框架的设计，还包括使用ELA信号在Blender 3D中创建带有逼真眨眼动态的合成数据集。

**Result:** 实验结果表明，ELA在公共驾驶员监控数据集上的表现显示出较低的角度变化方差，并且实现了精确的眨眼检测，同时合成数据集扩展了疲劳识别训练数据的多样性。

**Conclusion:** 我们提出的ELA作为一种可靠的生物特征度量和生成能够满足驾驶员状态监控需求的可扩展数据集的强大工具，被证明是有效的。
...
> VideoChat-M1是一个通过多代理系统处理视频理解的框架，采用协作策略规划（CPP）模式，提升了复杂视频的感知和推理能力。通过多代理强化学习方法优化策略代理的协作，实现了在多个基准测试中的优异表现，特别是在LongVideoBench上显著优于现有模型。

<details>
  <summary>Details</summary>

**Motivation:** 传统的方法在视频理解中采用静态和非学习性的工具调用机制，阻碍了对复杂视频多种线索的探索。因此，提出了VideoChat-M1框架，旨在提升对复杂视频的感知和推理能力，并优化多代理系统的协作策略。

**Method:** VideoChat-M1框架包含三个核心步骤：策略生成、策略执行以及策略沟通。每个代理根据用户查询生成其独特的策略，顺序执行策略并探索视频内容，并在执行过程中互相沟通以更新各自的策略。使用了多代理强化学习方法优化策略代理，以提高总体性能。

**Result:** 实验结果表明，VideoChat-M1在四个任务的八个基准测试中实现了顶尖性能。特别是在LongVideoBench上，其性能分别比SOTA模型Gemini 2.5 pro高3.6%，比GPT-4o高15.6%。

**Conclusion:** VideoChat-M1通过高效的协作策略规划和强化学习，展示了其在视频理解领域的强大能力，并在复杂的多代理系统中显著提高了性能。
...
> 此论文提出了感知分类法，一种评估模型进行结构化视觉推理的基准测试方法，展示了当前模型在这方面的不足，并指出通过模拟场景提供推理示例可提高模型性能。


<details>
  <summary>Details</summary>

**Motivation:** 当前的视觉-语言基准测试缺乏对物理导向视觉推理能力的全面评估，主要集中在表面级别的识别或图像-文本对齐上。为了填补这一空白，研究引入了感知分类法，一个物理导向视觉推理的基准测试。


**Method:** 提出了一种结构化的场景理解过程——感知分类法，该方法首先识别物体及其空间配置，然后推断任务相关的属性，如材料、功能、物理属性等，以支持目标导向的推理。为了填补当前视觉-语言基准测试在这方面的不足，研究者创建了一个新的基准测试。


**Result:** 实验结果表明，领先的视觉-语言模型在识别任务上表现出色，但在属性驱动的问题上表现下降10-20%，特别是那些需要对结构化属性进行多步骤推理的问题上。研究还发现，通过模拟场景提供推理示例可以提升对真实场景和专家设计问题的表现。


**Conclusion:** 这表明，在结构化的视觉理解方面存在持续的差异，并突显了依赖模式匹配的现有模型的局限性。提供基于感知分类法的上下文推理示例能有效提升模型性能。
...
> MapRF是一种弱监督框架，它使用仅有2D图像标签来构建3D地图，提出了一种自我训练机制以改进地图网络，并提出了一种策略来减少错误积累。实验结果表明这种框架在自主驾驶的高清地图构建中具有潜力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的用于在线构建高清地图的方法通常依赖于昂贵的3D地图标注来训练，这限制了它们在不同驾驶环境中的泛化和可扩展性。

**Method:** MapRF方法使用仅有2D图像标签的弱监督框架来学习构建3D地图。它引入了一个基于神经辐射场（NeRF）的模块，该模块可以根据地图预测重构视图一致的3D几何形状和语义，以生成高质量的伪标签。这些伪标签被迭代使用以自我训练方式改进地图网络，同时提出了一种Map-to-Ray匹配策略来减少自我训练过程中的错误积累。

**Result:** 在Argoverse 2和nuScenes数据集上的广泛实验表明，MapRF达到了与完全监督方法相当的表现，达到了基准性能的约75%，并且优于仅使用2D标签的几种方法。

**Conclusion:** 实验结果展示了MapRF在为自主驾驶提供可扩展且成本效益高的在线高清地图构建方面的潜力。
...
> Vidi2模型提升了视频理解能力，支持细粒度时空定位和视频问答，引入了新的评估基准VUE-STG并在多个基准上超越了现有系统。

<details>
  <summary>Details</summary>

**Motivation:** 满足互联网对高质量视频制作需求，提升视频创作和理解能力。

**Method:** Vidi2模型通过细粒度时空定位和视频问答扩展其功能，支持复杂编辑场景的应用。

**Result:** 在新引入的VUE-STG基准上超越了Gemini 3 Pro和GPT-5等领先系统，在视频问答基准上也取得了有竞争力的结果。

**Conclusion:** Vidi2模型通过引入新的评估基准和功能改进，大幅度提升了视频理解和处理效果。
...
> 研究发现，利用多模态大语言模型可以更好地进行跨区域光伏评估，优于传统方法。

<details>
  <summary>Details</summary>

**Motivation:** 随着分布式光伏系统的迅速扩张，未被记录的安装对电网管理提出了挑战。传统的计算机视觉模型需要大量的标注数据，且无法跨区域泛化。

**Method:** 本研究通过利用结构化提示和微调，将检测、定位和量化整合在一个统一的框架中，来研究多模态大语言模型在全球光伏评估中的跨领域泛化能力。

**Result:** 跨区域评估结果显示，所提出的模型在未见过的区域中的性能下降最小，优于传统的计算机视觉和变压器基准模型。

**Conclusion:** 这些结果突显了多模态大语言模型在领域转移下的鲁棒性，以及在全球光伏地图制作中的可扩展性、可转移性和解释性潜力。
...
> This paper explores cartographic heritage using a large dataset of maps, revealing cultural and historical insights into map design and content over six centuries.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to investigate cartographic heritage from a large-scale and cultural perspective, addressing the gap between automated map analysis and the historical and cultural contexts of maps.

**Method:** This research employs a diverse dataset of 771,561 map records and 99,715 digitized images from 38 digital catalogs. It applies semantic segmentation techniques and object detection models to recognize land classes and cartographic signs, trained on annotated and synthetic data.

**Result:** The research reveals the geographic and chronological distribution of map publications, their relation to political and military dynamics, and the evolution of cartographic styles and signs.

**Conclusion:** The conclusion highlights the importance of understanding cartography not just technically but culturally, showing how maps are designed to reflect and reinforce political, epistemic, and geographic expectations.
...
> 本文提出了ThiFAN-VQA框架，它结合链式思考、上下文学习和推理引导的回答选择，解决了AI框架在应对灾害评估时的数据成本高、灵活性差的问题，并在实验中表现出优越的准确性、解释性和适应性。

<details>
  <summary>Details</summary>

**Motivation:** 自然灾害发生后，及时准确地评估损失对于应急响应至关重要。尽管基于AI的框架可以快速处理大量无人机采集的航拍图像，但创建和标注模型训练数据成本高、耗时长、且复杂多样。现有的基于分类框架的方法限制了模型提供新信息的能力，而生成模型虽然具有高度灵活的开放答案空间，但往往产生不相关的泛化输出。

**Method:** 本文提出了ThiFAN-VQA，这是一种在灾难场景下的视觉问题回答的两阶段推理框架。它首先通过链式思考和上下文学习生成有结构的推理链，来实现有限监督下的可解释推理。随后通过回答选择模块评价生成的回答并选择最一致且语境相关的答案，进而有效提高模型性能。

**Result:** 实验结果表明，与现有的零样本和监督方法相比，ThiFAN-VQA能够在灵活性和一致性上取得更好的平衡，在FloodNet和RescueNet-VQA数据集上取得了更好的表现。

**Conclusion:** 通过实验，ThiFAN-VQA在洪水和飓风破坏地区的无人机数据集上实现了优秀的准确性、解释性和适应性，证明了其在处理现实世界灾害后损害评估任务中的有效性。
...
> HunyuanOCR是一款商业级、开源、轻量级Vision-Language Model，适用于OCR任务，实现了多功能性和效率的统一，并在多个评测中取得优异成绩。

<details>
  <summary>Details</summary>

**Motivation:** 开发HunyuanOCR的动机在于解决当前“OCR专家模型”多功能性局限和“通用VLM”低效率的问题，同时为了推进OCR任务的前沿研究和实际应用。

**Method:** 此论文介绍了HunyuanOCR，一种商业级的、开源的、轻量级的Vision-Language Model (VLM)，专门用于OCR任务。该模型架构包括原生视觉变压器（ViT）和轻量级语言模型，二者通过MLP适配器连接。HunyuanOCR通过统一多功能性和效率、采用纯端到端架构以及数据驱动和强化学习策略来实现其突破。

**Result:** HunyuanOCR在感知任务（文本检测、解析）和语义任务（信息提取、文本图像翻译）上表现优越，特别是在ICDAR 2025 DIMT Challenge（小型模型分组）中获得冠军，并且在具有少于3B参数的VLM中达到了SOTA结果。

**Conclusion:** HunyuanOCR实现了多功能性和效率的统一，采用了纯端到端架构以减少误差传播，通过高质量数据和强化学习策略获得了显著的性能提升。该模型已在HuggingFace上开源，并提供了基于vLLM的高性能部署方案，为前沿研究和工业应用提供了坚实的基础。
...
> 本文介绍了一种基于GANs的半监督分割方法，用于准确分割早期缺血性中风区域，通过对抗学习和多种损失函数提高早期缺血性改变的检测能力，显示出提高中风诊断和护理效率的潜力。

<details>
  <summary>Details</summary>

**Motivation:** 缺血性中风是一种时间紧迫的医疗紧急情况，快速诊断对于改善患者预后至关重要。然而，无对比剂计算机断层扫描（NCCT）作为一线成像工具，在超急性期往往无法揭示早期的缺血性改变，这可能导致关键干预措施的延误。

**Method:** 本研究提出了一种基于生成对抗网络（GANs）的半监督分割方法，用于准确区分早期缺血性中风区域。该方法利用对抗框架从有限的标注NCCT扫描中有效学习，同时利用大量的未标注扫描。通过采用Dice损失、交叉熵损失、特征匹配损失和自训练损失，模型能够识别和分割即使是模糊或体积较小的早期梗死区域。

**Result:** 在公共可获取的急性缺血性中风数据集（AISD）上进行的实验表明，所提出的方法有潜力提高诊断能力，减少手动标注的工作量，并支持中风护理中更高效的临床决策。

**Conclusion:** 研究展示了一种有效的半监督方法，利用GANs可以早期准确识别缺血性中风，支持及时的临床干预。这项技术可以提高早期诊断的效率，改善中风患者的护理和转归。
...
> 通过一种多尺度向量量化变分自编码器（MSVQ-VAE）模型，研究提出了一种生成合成无线胶囊内窥镜（WCE）图像的新方法，实现了正常图像中异常情况的引入，增强了训练CDS系统的数据多样性和可用性。

<details>
  <summary>Details</summary>

**Motivation:** 为解决由于隐私限制和标记成本导致的医疗图像数据稀缺问题，并提高基于深度学习的临床决策支持（CDS）系统的性能。

**Method:** 提出一种名为MSVQ-VAE的多尺度向量量化变分自编码器模型，用于生成包含多种异常情况的WCE图像。

**Result:** 采用生成的合成图像进行分类实验的结果与仅使用真实数据训练的模型相当，证明了该模型的有效性。

**Conclusion:** 该方法为医疗多媒体领域的各种数据集生成提供了通用性较强的解决方案，具有广泛的应用前景。
...
> 介绍SkillSight系统，通过结合视频和注视数据在功耗显著降低的情况下进行技能评估，并在多个数据集上验证了其有效性。

<details>
  <summary>Details</summary>

**Motivation:** 目标是使用智能眼镜中的自中心感知来改进在现实世界中学习新技能的方式，但自动技能评估仍然是一个基本的技术挑战。

**Method:** 介绍了一个名为SkillSight的系统，用于从第一人称视角数据进行高效的技能评估。该系统有两个阶段：首先联合建模注视和以自我为中心的视频来预测技能水平；其次，蒸馏出一个仅基于注视的学生模型，以在推理过程中仅依赖注视数据，从而显著减少功耗。

**Result:** 在烹饪、音乐和体育三个数据集上的实验表明，注视在理解各种现实世界环境中的技能方面具有重要作用。SkillSight教师模型达到最新技术水平，而仅基于注视的学生模型变体则使用比竞争方法低73倍的功耗保持高精度。

**Conclusion:** 研究结果显示基于注视的技能评估模型不仅可以维持高精度，还可以显著降低功耗，为进一步在现实世界中建立AI支持的技能学习铺平了道路。
...
> The paper proposes a method using a vision-language model to enhance undersampled MRI reconstructions by adding high-level semantic information. It outperforms traditional methods in preserving fine anatomical details and perceptual quality.

<details>
  <summary>Details</summary>

**Motivation:** The purpose was to investigate if a vision-language foundation model could enhance undersampled MRI reconstruction by providing high-level contextual information beyond conventional priors.

**Method:** We proposed a semantic distribution-guided reconstruction framework using a pre-trained vision-language foundation model to encode reconstructed images and auxiliary information into high-level semantic features. A contrastive objective was used to align the reconstructed representation with the target semantic distribution, ensuring consistency with high-level perceptual cues.

**Result:** Experiments on knee and brain datasets showed that semantic priors from images preserved fine anatomical structures and achieved superior perceptual quality compared with conventional regularization. The image-language information expanded the semantic distribution and enabled high-level control over reconstruction attributes.

**Conclusion:** The study concluded that vision-language foundation models could improve undersampled MRI reconstruction through semantic-space optimization.
...
> GIANT framework allows LMMs to navigate pathology images with high accuracy, demonstrating the potential for LMMs in medical diagnosis.

<details>
  <summary>Details</summary>

**Motivation:** To evaluate whether LMMs can be adapted to reason coherently and accurately in the evaluation of pathology images, addressing previous limitations of using low-resolution thumbnails or random patches.

**Method:** GIANT framework introduced to navigate whole-slide images, MultiPathQA benchmark created with 934 WSI-level questions.

**Result:** GIANT with GPT-5 achieves 62.5% accuracy on pathologist-authored questions, outperforming TITAN (43.8%) and SlideChat (37.5%).

**Conclusion:** Current LMMs can achieve significant performance improvements in pathology with tailored systems like GIANT, but there remains room for further development.
...
> The paper identifies a lack of faithfulness in tool usage among agentic vision-language models and proposes CodeV, a visual agent trained with TAPO, to address this issue by focusing on faithful tool use through process-level reinforcement learning techniques.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to increase the faithfulness of visual reasoning in agentic vision-language models. The authors observed that while these models may achieve high final-answer accuracy, they often use tools unfaithfully, such as invoking tools on irrelevant regions or ignoring tool outputs.

**Method:** CodeV is a code-based visual agent trained with Tool-Aware Policy Optimization (TAPO), a process-level reinforcement learning framework. TAPO provides more direct and verifiable supervision by assigning rewards based on visual tool inputs and outputs, rather than on textual chain-of-thought tokens.

**Result:** CodeV achieves competitive or better accuracy on visual search tasks while substantially increasing rates of faithful tool use. It also demonstrates strong performance on multimodal reasoning and math benchmarks.

**Conclusion:** The conclusion is that for the development of trustworthy visual reasoning systems, it is crucial to explicitly supervise intermediate tool behavior by assigning rewards based on visual tool outputs, which leads to more authentic and reliable performance.
...
> OncoVision，一种基于AI的多模态乳腺癌诊断工具，能够结合乳腺X光影像和临床数据，提高诊断精度并减少观察者差异，实现早期乳腺癌检测。

<details>
  <summary>Details</summary>

**Motivation:** 旨在开发一种基于AI的多模态乳腺癌诊断工具，提高诊断准确性，并减少观察者之间的差异，以便更好地进行早期乳腺癌检测与治疗。同时，为医护人员提供实时诊断支持，并提高医疗教学效率。

**Method:** 使用注意力机制的编码-解码模型作为框架，结合乳腺X光影像和临床数据进行乳腺癌诊断。模型可以分割四个感兴趣区域（如肿块、钙化点等），并预测十种临床特征。通过两种晚期融合策略将成像和临床见解融合，以提高诊断精度和减少观察者之间的差异。

**Result:** OncoVision在乳腺癌诊断方面表现出了高准确性和鲁棒性，并通过两种晚期融合策略提高了诊断精度，减少了观察者之间的差异。

**Conclusion:** OncoVision作为一个安全易用的网络应用，能够生成结构化的报告与注意力加权的可视化支持，以提高临床医生的信任，并有助于医疗教学。作为一个可扩展且公平的解决方案，OncoVision减少了全世界贫困地区的乳腺癌筛查壁垒。
...
> INTERLACE通过采用微调-冻结设计的交替模式，能够快速收敛并保持高性能，即便是在大大减少网络规模的情况下。

<details>
  <summary>Details</summary>

**Motivation:** 现有的层修剪方法在应用于视觉语言模型时会导致显著的性能下降。通过提出新的修剪方法INTERLACE，旨在解决这一问题。

**Method:** 我们提出了一种新的框架INTERLACE，用于在视觉语言模型（VLM）中修剪冗余层并保持性能，通过样本高效微调实现。我们通过分析连续三层来识别局部冗余，移除前两层中冗余度最高的那一层，微调剩余一层以补偿失去的能力，并冻结第三层作为稳定锚点进行微调。

**Result:** 通过仅在一个epoch的FineVision数据集的1%子集上微调部分层，INTERLACE在减少网络25%的情况下，实现了88.9%的性能保留，并达到SOTA性能。

**Conclusion:** INTERLACE证明了其在视觉语言模型中减少冗余层的同时保持高效率和高性能的能力。
...
> 介绍了针对工业任务协作工作的多模式自我中心和对象中心数据集IndEgo，该数据集提供了多种数据和注释，以促进任务理解和错误检测的研究，展示了对现有模型的挑战。

<details>
  <summary>Details</summary>

**Motivation:** 该研究的动机是创建一个详细的、多样的数据集，用于促进工业任务中多模态数据分析的研究，特别关注工人的协作性任务中的详细信息和挑战。这可以为未来的任务理解和错误检测提供基准。

**Method:** 该方法包括收集和处理多重来源的数据，如自我中心和对象中心摄像机捕捉的数据、声音和眼睛注视的数据，提供丰富的行动、错误检测和协作任务理解的详细注释和基准。

**Result:** 该论文介绍了IndEgo数据集，这是一个针对常见工业任务（如组装/拆卸、物流与组织、检查与维修、木工等）的多模式自我中心和对象中心数据集，包括3460段自我中心记录（约197小时）和1092段对象中心记录（约97小时）。该数据集的重点是协作性工作，在这期间两名工人共同完成认知和物理上都较为密集的任务。自我中心的记录包含了丰富的多模式数据和通过眼睛注视、叙述、声波、运动等增加的数据。数据集提供了详尽的标注（行动、总结、错误标注、叙述）、元数据、处理后的输出（眼睛注视、手势、半密集点云），以及针对程序性和非程序性任务理解、错误检测和基于推理的问题回答的基准测试。基于错误检测、问题回答和协作任务理解的基线评估显示，该数据集对最先进的多模式模型提出了挑战。该数据集可以在https://huggingface.co/datasets/FraunhoferIPK/IndEgo下载。

**Conclusion:** 数据集的评估表明，尽管该数据集可以应用于多种任务挑战，但现有的多模态模型面临挑战，这表明未来研究需要更多的探索和创新以应对这样的复杂数据和任务需求。
...
> 采用原型层密度估计网络实现细胞计数，增加深度学习模型在生物医学成像中的可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 细胞计数在生物医学成像中至关重要，但深度学习模型在这一领域的可解释性尚存挑战。

**Method:** 研究提出了一种基于原型的可解释细胞计数方法，通过密度图估计实现。该方法将原型层整合到密度估计网络中，使模型能够学习细胞和背景伪迹的代表性视觉模式。

**Result:** 通过生物学家的调查验证了所学原型的相关性，进一步验证了模型的可解释性。实验表明该方法在不牺牲计数效果的情况下实现了可解释性。

**Conclusion:** 该工作为研究人员和临床医生提供了一种透明可靠的细胞计数工具，可能增强对深度学习在关键时刻用中的信任并加速其采纳。
...
> RADSeg方法优化了基于RADIO模型的零样本开放式词汇语义分割性能，实现了更高的mIoU、更快的速度及更少的参数需求，优于之前的大规模视觉模型。

<details>
  <summary>Details</summary>

**Motivation:** 这项研究旨在解决当前在开放式词汇语义分割方面存在的问题，包括依赖有限的分割训练数据或应用零样本启发式视觉-语言模型，导致的难以获得高性能的挑战。

**Method:** 研究利用RADIO模型作为基础，通过自相关递归注意力、自相关全局聚合和高效的掩模细化增强性能，形成了名为RADSeg的新方法。

**Result:** 该论文提出了一种名为RADSeg的方法，用于提高零样本开放式词汇语义分割（OVSS）的性能，方法基于RADIO模型。研究通过引入自相关递归注意力、自相关全局聚合和计算效率高的掩模细化来增强RADIO的性能，实现了6-30%的mIoU提升，并且在速度和参数效率方面也有显著提升，较之现有的大型视觉模型，其在保持准确率的同时极大地降低了计算和内存需求。

**Conclusion:** RADSeg方法在零样本语义分割任务中实现了显著的优势，展示了自相关递归注意力和全局聚合的有效性，同时也证明了在不影响准确性的同时减少计算和内存负担的可行性。
...
> 本研究提出了一种用于训练阶段的分支式结构重参数化技术，能够显著减少ViT的层数同时维持高准确性和加速推理速度。

<details>
  <summary>Details</summary>

**Motivation:** 现有加速策略主要集中在算法级别的优化上，如token修剪和注意力加速，但这些策略没有解决Vision Transformer在实践中由于其深度架构导致的计算开销问题。研究动机是探讨是否可以在保持可比性的表示能力的同时降低堆叠的Transformer层数。

**Method:** 本研究提出了一种基于分支的结构重参数化技术，该技术在训练阶段操作，通过在Transformer块内使用并行分支，并系统地合并这些分支以生成适合推理部署的单一路径模型。合并机制通过逐步合并非线性组件入口点处的分支，使得前馈网络（FFN）和多头自注意力（MHSA）模块能够在测试阶段进行精确数学重参数化而不引入近似错误。

**Result:** 应用该框架到ViT-Tiny时，成功地将原始的12层架构减少到6层，甚至4层或仅3层，同时保持ImageNet-1K上的分类准确性。得到的压缩模型在移动CPU平台上实现了高达37%的推理加速。

**Conclusion:** 研究结果表明，偏好极深的Transformer堆栈的常规观点可能是不必要的限制性，为构造高效视觉Transformer提供了新的机会。
...
> 文章针对文本到图像生成模型的生成多样性问题提出TPSO方法，通过引入可学习参数优化token嵌入空间来减少生成重复图像的趋势，实验表明该方法有效提高了多样性，同时保持图像质量。

<details>
  <summary>Details</summary>

**Motivation:** 文章旨在解决当前文本到图像生成模型普遍存在的图像多样性不足的问题，现有方法如噪声重采样、提示重写或引导机制等依旧难以避免生成质量较高的模式，并且可能引起图像失真，损害图像质量。TPSO的动机在于提出一种新方法，有效地提高生成的多样性，同时保持图像的高保真度。

**Method:** TPSO方法引入可学习参数，以探索token嵌入空间中较少表示的区域，这减少了模型产生强烈模式样本的倾向。此过程在不牺牲图像质量的前提下，实现了生成多样性的显著提升。

**Result:** 该研究提出了一种名为Token-Prompt嵌入空间优化（TPSO）的方法，旨在解决文本到图像生成模型中的图像多样性问题。TPSO是一种无需训练且模型无关的模块，通过在token嵌入空间中引入可学习参数，帮助探索较少表示的区域，从而减少模型倾向于生成强烈模式样本的倾向。同时，提示层次的空间提供了全局语义约束，有助于防止质量退化并保持高保真度。实验结果显示，TPSO显著提高了生成多样性，基线性能提升了1.10到4.18分，并且不会降低图像质量。

**Conclusion:** 通过在MS-COCO数据集和三种生成模型上的广泛实验表明，TPSO方法在不降低图像质量的情况下，显著提高了生成多样性。具体地，与基线模型相比，性能提升了约3.08分。
...
> 我们提出了一种高度感知的动态图像分块方法，用于增强无人机在搜索和救援任务中对小目标的检测能力，显著提升了检测准确性和推理速度。

<details>
  <summary>Details</summary>

**Motivation:** 由于无人机（UAVs）在搜索和救援（SAR）任务中能够监测广阔的海域，但小目标由于低的目标与背景像素比，在高海拔作业时很难被检测到。

**Method:** 我们提出了一种基于高度感知的动态分块方法，该方法通过根据高度调整图像分块的大小，增强了对小目标的检测能力。该方法在减少不必要的计算的同时保持了检测性能。

**Result:** 与基线对比，该方法在“SeaDronesSee”数据集上使用YOLOv5和SAHI框架进行测试，对于小目标的mAP提升了38%，并且相较于静态分块，推理速度翻倍。

**Conclusion:** 该方法能够在不同的条件下使基于无人机的搜索和救援操作更加高效和准确。
...
> CropVLM是一种通过动态聚焦图像相关区域来提升视觉语言模型性能的外部低成本方法。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型在需要细致图像理解的任务上表现不佳，如场景文字识别或文档分析。因此，开发了一种新的方法来解决这些挑战。

**Method:** CropVLM作为一种外部低成本方法，通过强化学习训练，使视觉语言模型（VLM）能够动态地“放大”图像的相关区域，以增强其捕捉细节的能力。该方法无需使用人工标注的边界框作为监督信号，也无需昂贵的合成评估。

**Result:** 实验结果显示，CropVLM在需要高分辨率图像理解的任务上表现显著提高，尤其在目标VLM的领域外基准测试中。

**Conclusion:** 这种方法证明了其在无需修改或微调VLM的情况下，能够有效提升VLM的性能，避免了灾难性遗忘的问题。
...
> The paper studies the transferability of optimized slicers within the min-STP framework, showing that these slicers remain effective under slight data distribution perturbations, with a minibatch formulation introduced to enhance scalability.

<details>
  <summary>Details</summary>

**Motivation:** The research is motivated by the high computational cost of OT and the need for scalable methods that can be effectively transferred to new distribution pairs under distribution shifts.

**Method:** Slice-based transport plans are used to reduce computational cost by utilizing 1D OT solutions. The paper studies the min-STP framework which aims to investigate the transferability of optimized slicers.

**Result:** Theoretical analysis shows slicers are robust to data distribution changes, and empirical results demonstrate strong one-shot matching performance and utility in amortized training for point cloud alignment and generative modeling.

**Conclusion:** Min-STP with optimal slicers can efficiently transfer to new distribution pairs, and its minibatch formulation offers a scalable solution for OT applications.
...
> MAPS是一个细调框架，它通过逐步放松模块之间的接近约束，保持视觉编码器与预训练先验的接近，而使动作导向的语言层更自由地适应，从而提高VLA模型的泛化性能。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言动作（VLA）模型继承了预训练视觉语言模型的强先验，但简单的微调往往干扰这些表示，损害泛化能力。现有的解决方案要么过度约束适应性，要么忽视VLA组件的不同作用。

**Method:** MAPS (Module-Wise Proximity Scheduling) 是首个稳健的视觉语言动作模型细调框架。通过系统分析，它揭示了一个实证顺序，按照该顺序，逐渐放松接近约束可以平衡稳定性和灵活性。

**Result:** 在MiniVLA-VQ、MiniVLA-OFT、OpenVLA-OFT等数据集以及SimplerEnv、CALVIN、LIBERO等挑战性基准上，MAPS一致地提高了在分布内和分布外的性能（最高提升可达+30%）。

**Conclusion:** 实证导向地接近预训练的VLM们是一个简单却强大的原则，可以保持从VLM到VLA转移中的广泛建模。
...
> 研究介绍了一种轻量级Python包PathFMTools，用于高效处理病理学基础模型，并在皮肤鳞状细胞癌组织学分级任务上进行了测试，验证了基础模型应用于临床实践的潜力。

<details>
  <summary>Details</summary>

**Motivation:** 尽管计算病理学基础模型具有潜在的优势，但将它们应用于特定临床任务仍然面临挑战，如全玻片图像处理的复杂性、学习特征的不可见性以及广泛的潜在适应策略。本研究旨在解决这些问题。

**Method:** 介绍PathFMTools，这是一种轻量级、可扩展的Python包，用于高效执行、分析和可视化病理学基础模型。使用此工具，研究人员可以与CONCH和MUSK这两种最先进的视觉-语言基础模型进行交互，并在皮肤鳞状细胞癌(cSCC)的组织学分级任务上进行评估。

**Result:** 利用包含440个cSCC H&E全玻片图像的队列，对多种适应策略进行了基准测试，展示了预测方法之间的权衡，并验证了使用基础模型嵌入来训练小型专用模型的潜力。

**Conclusion:** 这些发现强调了病理学基础模型对于实际临床应用的前景，并且通过PathFMTools的使用，能够实现高效分析和验证。
...
> The paper addresses the underexplored capability of Vision Language Models in counterfactual reasoning for video understanding, introducing CounterVQA, a new benchmark for evaluating this skill, and develops CFGPT, a method to enhance models' counterfactual reasoning ability.

<details>
  <summary>Details</summary>

**Motivation:** Counterfactual reasoning in videos is essential for robust understanding beyond simple pattern recognition; however, this capability is not well-developed in current Vision Language Models.

**Method:** Structure

**Result:** {
  "tldr": "The paper addresses the underexplored capability of Vision Language Models in counterfactual reasoning for video understanding, introducing CounterVQA, a new benchmark for evaluating this skill, and develops CFGPT, a method to enhance models' counterfactual reasoning ability.", 
  "motivation": "Counterfactual reasoning in videos is essential for robust understanding beyond simple pattern recognition; however, this capability is not well-developed in current Vision Language Models.", 
  "method": "Developed CounterVQA, a benchmark for assessing counterfactual reasoning, and CFGPT, a post-training method to improve visual counterfactual reasoning by distilling reasoning capability from language modality.", 
  "result": "State-of-the-art models performed reasonably on simple counterfactual questions but poorly on complex multi-hop causal chains, with CFGPT showing consistent improvements across all difficulty levels of CounterVQA.", 
  "conclusion": "CFGPT effectively enhances the counterfactual reasoning capability of Vision Language Models, suggesting a promising direction for future research in video understanding.")
...
> 本研究通过扩展原型网络（ProtoPNets）来解决多模态神经网络在物种检测中的不可解释性和高成本问题，提出一套成本意识的多模态方法，以提高决策的透明度并减少对遗传数据的需求。

<details>
  <summary>Details</summary>

**Motivation:** 研究旨在改进多模态神经网络在物种检测中的应用，特别是解决其黑箱决策过程和收集遗传数据成本高昂的问题。

**Method:** 通过集成多种模式的原型网络，并引入相应的权重来决定模型在做出预测时对各模态的依赖程度，同时开发方法识别无需昂贵的遗传信息就能做出自信预测的情况。

**Result:** 本方法可以智能分配昂贵的遗传数据来进行细微区分，同时利用丰富的图像数据进行分类，达到与持续使用两种模态信息的模型相媲美的准确性。

**Conclusion:** 提出的成本意识的多模态方法能够提高物种检测模型的透明性，并在降低成本的同时保持良好的预测准确度。
...
> 研究了专业设计师间的UI设计偏好分歧，提出了个性化模型训练策略，证明了个性化模型在预测偏好上的优势。

<details>
  <summary>Details</summary>

**Motivation:** 由于视觉设计具有主观性和高度个性化的特点，人类标注的设计偏好在广泛人群之间差异很大。本研究旨在通过专业设计师的标注数据，研究他们之间的偏好分歧及其来源。

**Method:** 引入了DesignPref数据集，包含20位专业设计师标注的12k个UI设计生成的成对比较，以及多个级别的偏好评分。研究了经过训练的设计师之间的显著分歧（Krippendorff's alpha = 0.25对于二元偏好判断）。通过设计师提供的自然语言理由，发现了分歧源于对不同设计要素重要性的感知差异和个体偏好。

**Result:** 发现传统的多数投票方法训练的聚合裁判模型往往不能准确反映个体偏好。个性化策略，特别是微调或在RAG管道中结合设计师特定的注释，可以使个性化模型在预测个体设计师的偏好时，即使使用20倍少的例子，也能超越聚合基线模型。

**Conclusion:** 提供了首个研究个性化视觉设计评估的数据集，支持未来研究如何建模个体设计品味。
...
> VESSA integrates vision-language models into semi-supervised medical image segmentation, improving accuracy with limited annotations.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to integrate the strong generalization and few-shot capabilities of VLMs into semi-supervised medical image segmentation to achieve high accuracy with minimal expert annotations.

**Method:** Our approach consists of two stages. In Stage 1, the VLM-enhanced segmentation foundation model VESSA is trained using a template bank containing gold-standard exemplars. In Stage 2, VESSA is integrated into a state-of-the-art SSL framework, interacting dynamically with the student model to generate higher-quality pseudo-labels.

**Result:** Experiments demonstrate that VESSA significantly improves segmentation accuracy in multiple datasets and domains, even when faced with extremely limited annotation conditions.

**Conclusion:** VESSA-augmented SSL significantly enhances segmentation accuracy, outperforming state-of-the-art baselines under extreme annotation scarcity.
...
> 通过UniSandbox框架评估统一多模态模型，发现理解与生成间的显著差距，并提出通过链式思考及自我训练方法改善该差距。

<details>
  <summary>Details</summary>

**Motivation:** 探索在统一多模态模型中，理解如何影响生成的过程。

**Method:** 介绍 UniSandbox，一个将评估框架与受控的合成数据集结合的解耦框架，用于研究理解是否真正影响生成的问题。

**Result:** 研究结果表明，链式思考能显著提升推理生成任务和知识转移任务的表现，同时自我训练方法也能帮助隐式推理生成。

**Conclusion:** UniSandbox提供了一个新的视角来设计未来的统一架构和训练策略，以弥合理解与生成之间的差距。
...
> A relative angle feature is proposed to effectively represent directionality in point cloud data for surface defect reconstruction, achieving similar performance to normal vectors but with reduced storage requirements and input channel compression.

<details>
  <summary>Details</summary>

**Motivation:** To address the limitations of image-based methods which are easily affected by background noise and the high volume of 3D data in point cloud methods, a new feature is introduced to reduce data volume while maintaining the ability to represent surface defect characteristics.

**Method:** This study proposes a new feature, relative angle, which is the angle between the normal vector of a point and the average normal vector of its parent point cloud, to represent directionality information. It uses entropy-based feature evaluation to filter out redundant information and retains effective information in damaged sections. Models are trained and tested with PointNet++.

**Result:** Models based on the relative angles achieved similar performance to models based on normal vectors, realizing a 27.6% decrease in storage requirements and 83% input channel compression.

**Conclusion:** The relative angle feature can enable larger-batch execution on hardware with limited resources without the need for architectural changes to models, making it a promising approach.
...
> CrispFormer introduces a refined SegFormer decoder for better handling in weakly supervised semantic segmentation, achieving clear boundaries and improved segmentation fidelity with minimal compute overhead.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this paper is to improve the performance of weakly supervised semantic segmentation (WSSS) without modifying the backbone or relying heavily on post-processing techniques.

**Method:** Our method, named CrispFormer, enhances the SegFormer decoder with three key components: a boundary branch for clearer object contours, an uncertainty-guided refiner for better handling label noise, and a dynamic multi-scale fusion layer for more adaptive feature integration.

**Result:** The result is a single-pass model that provides higher quality masks, particularly improving metrics like boundary F-score, small-object recall, and mIoU over the baseline.

**Conclusion:** The paper concludes that by focusing on the decoder with simple yet effective modifications, WSSS models can achieve higher fidelity in creating dense masks from noisy supervision.
...
> 研究提出Prune-Then-Plan框架来解决大规模视觉语言模型在具身问答中探索不稳定问题，实现了更高的场景覆盖和更好的答案质量。

<details>
  <summary>Details</summary>

**Motivation:** 虽然大型视觉语言模型（VLMs）改善了具身问答（EQA）代理的开放词汇推理，但直接用于步骤级探索时，往往会因为过自信和校准不当导致前线震荡，造成探索不稳定和答案质量下降。本研究旨在解决此问题。

**Method:** Prune-Then-Plan框架通过步骤级校准来稳定探索。该方法不完全依赖于原始的VLM分数，而是使用Holm-Bonferroni启发的修剪过程来删除不可信的前沿选择，然后将最终决策委托给基于覆盖的规划器。

**Result:** 该方法在3D-Mem EQA框架中，分别在视觉相关的SPL和LLM-Match度量上比基线提高了49%和33%。总体上，在OpenEQA和EXPRESS-Bench数据集上的同等探索预算下，实现了更好的场景覆盖。

**Conclusion:** Prune-Then-Plan方法能够将过自信的预测转换为保守且可解释的动作，通过依赖人类层面的判断来校准VLM的步骤级行为，提高了具身问答的性能和探索效率。
...
> 本文发现了一种在使用旋转位置嵌入（RoPE）进行混合分辨率去噪时出现的一种核心故障模式，提出了一种解决方案：跨分辨率相位对齐注意力（CRPA），此方法无需重新训练，恢复了精确的相位模式，提高了图像与视频的生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 该研究的动机在于解决动态图像噪声处理时遇到的一种核心故障模式，即线性插值方法在不同空间网格的令牌混合时会导致注意力机制崩溃。

**Method:** 我们提出了跨分辨率相位对齐注意力机制（CRPA），这是一种无需重新训练的解决方案，用于解决在混合分辨率下进行去噪时使用旋转位置嵌入（RoPE）时遇到的注意力机制崩溃问题。CRPA仅修改每个注意力调用中的RoPE索引图，使得所有查询/键位置都以查询的步幅表达，从而恢复精确的相位模式。

**Result:** 实验结果表明，CRPA实现了高保真和高效的混合分辨率生成，优于之前的方法，在图像和视频生成方面表现出色。

**Conclusion:** CRPA是一个无需重新训练的解决方案，能够使预训练的DiTs稳定工作，恢复了精确的相位模式，适用于多种分辨率的生成任务。
...
> 本文研究了在场景文本视觉问答任务中，视觉语言模型（VLMs）如何在面对不确定性时选择放弃回答的问题。通过提出潜在表示探测方法（LRP），改进了放弃判断的准确性，为构建可靠的AI系统提供了新思路。

<details>
  <summary>Details</summary>

**Motivation:** 动机源于现有VLMs在面临不确定性时采取消极态度，主要依赖于不准确的输出概率或不适合OCR任务的语义一致性方法。这些失败现象提示我们可能需要从模型的内部表示中寻找不确定性信号。

**Method:** 本文提出了潜在表示探测（Latent Representation Probing, LRP）方法，具体包括三种探测器设计：跨所有层连接表示、聚合视觉标记上的注意力以及通过多数投票集成单层探测器。

**Result:** 实验结果显示，LRP方法在四个基准测试中的放弃判断准确性比最佳基线提高了7.6%。

**Conclusion:** 研究建立了通过检测内部状态中的置信信号来构建部署准备型AI系统的原理框架，而非依赖不可靠的输出。
...
> ReDirector通过改进RoPE和引入RoCE来改进视频重拍生成质量，相比之前的方法有显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 本研究的动机是改善视频重拍生成的质量，解决之前方法中相机路径和视频长度泛化能力不足的问题。

**Method:** 我们提出了ReDirector，这是一种由相机控制的视频重拍生成方法。它修复了先前研究中对RoPE的常见误用，并通过引入Rotary Camera Encoding（RoCE）来捕捉输入视频和目标视频之间的多视图关系。

**Result:** 实验结果显示，我们的方法在相机控制、几何一致性和视频质量方面都有显著改进。

**Conclusion:** 通过整合相机条件到RoPE中，我们的方法能够更好处理不同分布的相机轨迹和视频长度，提升了动态物体定位和静态背景保存的效果。
...
> 本文提出了一种新框架BHD-RAG，用于通过CT图像改善Birt-Hogg-Dube综合症的诊断，整合了DCLD特定的专业知识，经过四种类型囊性肺疾病的验证，效果优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 深学习方法在推进通过CT成像诊断Birt-Hogg-Dube综合症方面面临临床样本量有限以及囊性肺疾病之间的低类间区分度的双重挑战。尽管多模态大型语言模型显示了对这类罕见疾病的诊断潜力，但缺乏领域特定知识和可参考的放射学特征增加了幻觉风险。

**Method:** 本文提出了一种名为BHD-RAG的多模态检索增强生成框架，用于提高通过CT成像进行Birt-Hogg-Dube综合症（BHD）诊断的准确性。该框架包括：1）一个专门的代理生成CT图像的影像表现描述，构建多模态囊性肺疾病（DCLDs）案例语料库。2）基于余弦相似性的检索器，定位与查询图像相关的影像描述对。3）多模态大型语言模型将检索到的证据与成像数据结合进行诊断。

**Result:** 在涉及四种类型的DCLDs数据集上验证了BHD-RAG，结果显示了优越的准确性，并生成了与专家见解密切相关的基于证据的描述。

**Conclusion:** BHD-RAG框架通过结合DCLD特定的专业知识和临床案例与MLLM，改善了BHD诊断的准确性，生成的基于证据的描述与专家见解紧密关联。
...
> The paper introduces Rectified SpaAttn to solve problems of attention sparsity in Diffusion Transformers for video generation, achieving significant speedups while preserving quality.

<details>
  <summary>Details</summary>

**Motivation:** To tackle the high computational costs and performance degradation due to attention sparsity in Diffusion Transformers for video generation, the authors aim to refine attention allocation.

**Method:** The proposed solution is Rectified SpaAttn, which corrects attention allocation by integrating implicit full attention references. It includes Isolated-Pooling Attention Reallocation for critical tokens and Gain-Aware Pooling Rectification for non-critical tokens.

**Result:** The Rectified SpaAttn method leads to up to 3.33 and 2.08 times speedups on HunyuanVideo and Wan 2.1 datasets, respectively, without sacrificing generation quality.

**Conclusion:** Rectified SpaAttn effectively addresses the issues related to attention sparsity in video generation, providing a more efficient and accurate mechanism for attention allocation.
...
> 本文介绍了4DWorldBench，一个评估世界生成模型的基准测试，包括四个维度的评估，并创新性地引入了自适应条件设置，旨在推动世界生成技术的发展。

<details>
  <summary>Details</summary>

**Motivation:** 现有的评估基准在评估维度上有所侧重，缺少对世界真实感能力的统一评估。因此，本文旨在提供一个系统的框架来全面评估世界模型的能力。

**Method:** 提出了4DWorldBench基准测试，从感知质量、条件4D一致性、物理真实性和4D一致性四个关键维度系统评估世界模型。该基准涵盖了诸如图像到3D/4D、视频到4D、文本到3D/4D等任务，并且创新性地引入了多模态下的自适应条件设置，将不同模态条件映射到统一的文本空间，并结合LLM-as-judge、MLLM-as-judge以及传统的基于网络的方法进行评估。

**Result:** 初步的人类研究表明，自适应工具选择方法在一致性上更接近于主观的人类判断。

**Conclusion:** 该基准测试为世界模型提供了客观比较和改进的基础，加速了从“可视生成”向“世界生成”的转变。
...
> 研究表明，通过IMIC方法，可以有效实现在一个模型上完成多个任务，同时避免灾难性遗忘，并且模型在多个任务上表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 现有的微调模型存在灾难性遗忘的问题，我们希望通过单一嵌入空间执行多个任务（物体识别、高/低质量图像中的人脸识别、全身图像中的人物识别），并且不会显著增加灾难性遗忘。

**Method:** 我们引入了两种变体的交错多领域身份课程（IMIC）方法，这是一种耦合梯度的交错训练方案，可以同时对基础模型的所有四个任务进行微调。基础模型包括DINOv3、CLIP和EVA-02。

**Result:** IMIC方法在三个基础模型上都证明有效，其中EVA-02和CLIP在四个任务中表现与领域专家相当，并且在多任务处理上比人类更准确。这种方法在处理分布外数据上的泛化能力也没有受到严重影响。

**Conclusion:** 最准确的模型变体（EVA-02 + IMIC A和B）展示了四个任务在统一嵌入空间中的线性可分表示，并且任务之间存在显著特征共享。从任一任务计算出的不到100个主成分可以几乎不损失性能的情况下执行所有其他任务。
...
> 本文提出了Bézier Graph来自动从航拍图像中提取道路网络。该方法通过DOGE框架使Bézier Graph的全局优化成为可能，达到了SpceNet和CityScale基准测试中的新高水平，并提供了一个生成高保真度道路网络矢量图的方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法依赖于多边形线，这些线在模拟曲线几何形状时存在困难。我们认为道路几何本质上是基于曲线的。通过引入Bézier Graph，我们旨在解决过去方法中多边形线在建模曲线道路几何时的不足。

**Method:** 我们引入了Bézier Graph，这是一种基于可微分参数曲线的表示方法。通过重新定义任务为Bézier Graph上的全局优化问题，我们绕过了获得难以构建的矢量地面实况（GT）的障碍。我们的框架DOGE直接从分割掩模中学习参数化Bézier Graph，从而消除对曲线GT的需求。DOGE通过交替使用两个互补模块：DiffAlign通过可微渲染连续优化几何形状，而TopoAdapt使用离散操作来优化拓扑结构，从而整体优化图。

**Result:** 我们在大规模的空间网络和CityScale基准测试中建立了新的最先进的水平，展示了一种生成道路网络高保真矢量图的新范式。

**Conclusion:** 我们的方法展示了通过直接有效地优化Bézier Graph，无需曲面真实性验证即可获得更精确的道路网络表示是可能的，达到了在SpceNet和CityScale上新的状态水平。这打开了自动从航拍图像中提取高保真度道路网络的新方法。
...
> STAvatar is introduced to improve the flexibility and expressiveness of 3D head avatar reconstruction from monocular videos, particularly in complex regions such as the mouth and eyes.

<details>
  <summary>Details</summary>

**Motivation:** The need to address the limitations of existing 3D Gaussian Splatting methods by proposing a more flexible and adaptive approach to generate animatable 3D head avatars from monocular videos.

**Method:** Our method, STAvatar, introduces two key components: a UV-Adaptive Soft Binding framework that utilizes both image-based and geometric priors to improve Gaussian feature offset learning within UV space, and a Temporal ADC strategy that clusters similar frames and introduces a fused perceptual error to enhance detail in occluded regions.

**Result:** Experiments on four benchmark datasets show that STAvatar outperforms existing methods in reconstructing fine-grained details and handling occluded regions.

**Conclusion:** STAvatar provides state-of-the-art performance in 3D head avatar reconstruction from monocular videos, improving on existing methods by handling occluded regions more effectively and providing finer detail.
...
> 本文提出了TimeArtist框架，填补了时序数据和图像生成间语义级别对齐的空白，实现了高质量、多样化的直接从时序数据生成图像的功能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的方法无法在语义级别上对时序数据和图像生成进行有效对齐。TimeArtist旨在填补非视觉连续序列作为条件信号在高保真图像生成中的空白。

**Method:** TimeArtist采用“预热对齐”范式，首先使用自监督学习训练双自动编码器和共享量化器学习跨模态共享表示，然后冻结编码器和量化器，并引入投影层在表示层面对齐时序和视觉样本。

**Result:** 实验结果表明，TimeArtist在图像生成指标上表现良好，并且在零样本的时序任务中取得了优越的结果。

**Conclusion:** 该研究建立了一种新的跨模态生成范式，打通了时间动态和视觉语义间的鸿沟。
...
> GigaWorld-0 is a unified world model framework for Vision-Language-Action (VLA) learning, integrating large-scale video generation and 3D generative modeling to produce high-quality, diverse, and controllable embodied interaction data. This results in VLA models that perform well on physical robots without real-world training.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to develop a scalable, data-efficient embodied AI framework capable of generating rich, realistic, and instruction-aligned embodied interaction data for training models that can perform well in real-world scenarios without actual training in the real environment.

**Method:** The method involves two components of GigaWorld-0: GigaWorld-0-Video for video generation and GigaWorld-0-3D for 3D modeling and physical simulation. Both components are optimized together to ensure the data is geometrically consistent and physically plausible.

**Result:** The result is a comprehensive evaluation showing that GigaWorld-0 can generate high-quality, diverse, and controllable data. This data is effective for training VLA models, which then exhibit strong real-world performance, particularly in terms of generalization and task success on physical robots.

**Conclusion:** The conclusion is that GigaWorld-0 represents a significant step forward in embodied AI by providing a way to efficiently generate synthetic data that, when used for training, leads to models capable of strong performance on real physical robots without the need for real-world training data.
...
> ChessMamba is a novel structure-aware framework for change detection in multitemporal remote sensing imagery, achieving better accuracy and reliable change localization.

<details>
  <summary>Details</summary>

**Motivation:** Existing CD methodologies disrupt local structural consistency during temporal serialization, obscuring discriminative cues and hindering the reliable localization of changes. This study aims to develop a method that can better preserve structural consistency and enable more effective change localization.

**Method:** ChessMamba, a structure-aware framework, is proposed to improve fine-grained change detection in multitemporal remote sensing imagery. It includes a SpatialMamba encoder and a lightweight cross-source interaction module, featuring chessboard interleaving and structure-aware fusion via multi-dilated convolutions.

**Result:** The proposed approach, ChessMamba, achieves significant improvements in accuracy compared to existing state-of-the-art methods on three CD tasks: binary CD, semantic CD, and multimodal building damage assessment.

**Conclusion:** ChessMamba effectively fuses heterogeneous features through its unique structure-aware design, providing a robust solution for change detection in multitemporal remote sensing images.
...
> 本文提出了一种新的跨模态知识蒸馏技术，通过频域特性处理不同频率的特征，并采用特定的损失函数与共享分类器以优化模型性能，其实验结果显著优于传统方法和现有跨模态KD技术。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机在于解决跨模态知识蒸馏场景下的知识转移难题，特别是在视觉到语言等场景中由于表示不一致导致的传统知识蒸馏效果不佳的问题。

**Method:** 本文提出了基于频率解耦的跨模态知识蒸馏方法，通过利用频域特征来平衡不同模态间的知识传递。该方法引入了不同的损失函数处理低频和高频特征，并提出尺度一致性损失来解决模态间的分布变化问题，同时使用共享分类器统一特征空间。

**Result:** 基于多个基准数据集的广泛实验显示，本方法在跨模态知识蒸馏方面显著优于传统KD和现有先进技术。

**Conclusion:** 研究结果表明，基于频率的解耦方法能够有效提升跨模态知识蒸馏的效能，解决了模态间知识转移的一致性问题。
...
> A multi-task liver dataset (LiMT) is constructed for multiple liver lesion tasks, potentially addressing limitations in existing computer-aided diagnosis datasets and providing value for medical imaging research.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to address the limitations of existing CAD technology datasets that are typically limited to single tasks and potentially to explore the correlation between tasks.

**Method:** In this paper, a multi-task liver dataset (LiMT) is constructed to support liver and tumor segmentation, multi-label lesion classification, and lesion detection based on arterial phase-enhanced computed tomography (CT).

**Result:** The dataset includes CT volumes from 150 cases, with four types of liver diseases and normal cases, each carefully annotated and calibrated by medical professionals.

**Conclusion:** The paper concludes by providing relevant baseline experimental results, a review of existing liver-related datasets and methods, and the LiMT dataset being available for the medical imaging research community.
...
> 提出生成-验证框架来生成更高质量的科学视觉问答数据集VeriSciQA，并展示了其对提升现有开源模型性能的有效性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的开源模型在科学视觉问答（SVQA）任务中遇到挑战，原因在于缺乏大型、高质量的SVQA数据集。解决生成数据中的系统性错误，提高生成数据的质量。

**Method:** 提出了一种以验证为中心的生成-验证框架，该框架首先利用与图关联的文本上下文生成问答对，然后通过跨模态一致性检查以及辅助过滤器消除错误问答对。

**Result:** 实现了包含20,351个问答对的VeriSciQA数据集，该数据集跨越20个科学领域和12种图类型。该数据集为开源模型提出了一个具有挑战性的基准测试，开放源代码模型的准确率从64%到专有模型的82%之间有显著差距。

**Conclusion:** 通过可扩展框架持续的数据扩展能够进一步提升SVQA能力。
...
> 本文提出了Agent0-VL，一种结合工具集成推理的自我进化的视觉语言代理，通过工具使用来实现自我评估和自我修复，解决了纯文本自我评估在复杂视觉推理中的问题，并实现了持续改进。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言代理在多模态推理任务中取得了显著进展，但依赖于人类注释监督的限制影响其学习效果。同时，纯文本自我评估在复杂视觉推理验证中存在问题，因此提出结合工具集成推理来改进代理的自我评估和自我修复能力。

**Method:** 通过在LVLM中集成了解答者和验证者两个角色，解答者进行多回合工具集成推理，验证者通过工具基础的批评生成结构化的反馈和精细的自我奖励，形成自我演化的推理循环。

**Result:** 实验结果表明，Agent0-VL在几何问题求解和视觉科学分析任务上，相对于基础模型性能提升了12.5%。

**Conclusion:** Agent0-VL通过工具集成推理和自我演化推理循环，实现了从零外部奖励条件下的持续自我改进，不需要任何人类注释或外部奖励模型。
...
> 本研究提出了一种多模态连续手语识别方法，通过融合3D骨架特征和手形分类器进行边界检测，并在ASLLRP语料库上获得了改进的识别效果。

<details>
  <summary>Details</summary>

**Motivation:** 本研究关注的是改进连续手语识别的鲁棒性，提出了一种新的多模态方法，具体包括视频分割，手形识别，以及这两种信息的融合，以实现更好的连续手语识别效果。

**Method:** 本研究提出了一种多模态的连续手语识别方法，首先使用机器学习算法检测视频中美国手语（ASL）句子的起止帧，然后对分割的符号进行识别。为了提高鲁棒性，我们使用从手语视频中提取的3D骨架特征来捕捉符号属性及其动态变化的汇聚点，这些变化往往在符号边界处聚集。研究还着重于将3D手形信息纳入边界检测。为了检测通常在符号开始和结束时出现的手形，我们使用一个由几个现有数据集综合和标准化创建的新数据集，预训练了一个用于87个语言定义的典型手形类别的手形分类器。然后使用一个多模态融合模块将预训练的符号视频分割框架和手形分类模型统一起来。最后，使用估计出的边界进行符号识别，其中识别模型是通过一个包含both citation-form isolated signs及基于手动注释从连续收集中分段符号的大型数据库训练的。

**Result:** 我们在ASLLRP语料库上评估了该方法，并证明它比之前的工作有显著的改进。

**Conclusion:** 我们展示了新方法在连续手语识别尤其是结合3D骨架特征和手形信息后的有效性，说明了本方法较之前研究的显著改进。
...
> Motion Marionette 允许从单目源视频向单视图目标图像进行零样本刚体运动转移，解决了现有方法的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的方法通常依靠几何、生成或模拟先验来引导转移过程，但这些外部先验引入了辅助约束，导致通用性与时间连贯性之间的权衡。

**Method:** 通过将源视频和目标图像提升到一个统一的3D表示空间中，从源视频中提取运动轨迹来构建一个空间时间(SpaT)先验，该先验独立于物体几何和语义。然后，这个先验与目标对象结合生成一个可控的速度场，并使用基于位置的动力学(Position-Based Dynamics)来消除伪影并增强视觉连贯性。

**Result:** 实验结果表明，Motion Marionette 能够跨多种物体进行推广，产生与源运动良好对齐且表现时间连贯的视频，并支持可控的视频生成。

**Conclusion:** Motion Marionette 提供了一种无需额外几何、生成或仿真先验的零样本框架，能够在单目源视频和单视图目标图像之间进行刚体运动转移。
...
> Reasoning-VLA is a new autonomous driving model that improves upon existing Vision-Language-Action frameworks by providing better generalization and faster inference.

<details>
  <summary>Details</summary>

**Motivation:** To address the inefficiency in inference and poor generalization of existing Vision-Language-Action models in autonomous driving, while aiming to develop a faster and more versatile model.

**Method:** The proposed Reasoning-VLA framework employs learnable action queries initialized via Gaussian sampling from ground-truth trajectories. It interacts with enhanced vision-language features and uses both supervised learning and reinforcement learning for training.

**Result:** Extensive evaluations on multiple benchmarks reveal that the Reasoning-VLA model attains superior generalization and state-of-the-art performance with high inference speed.

**Conclusion:** The proposed Reasoning-VLA framework effectively improves decision-making capabilities in autonomous driving by enhancing generalization and inference efficiency.
...
> 文章介绍了一个新颖的框架C-PGA，它可以预测复杂3D打印物体的光化学转换，解决了从3D视觉数据中预测非视觉物理属性的难题。

<details>
  <summary>Details</summary>

**Motivation:** 该研究旨在解决预测复杂三维打印对象中的光化学转换这一具有挑战性的新计算机视觉任务，建立了从3D视觉数据中预测密集的非视觉体积物理属性的框架。

**Method:** 我们提出了一个名为耦合物理门控适应(C-PGA)的新多模态融合架构。C-PGA通过使用稀疏的几何和过程参数（如表面传输、打印层高度）作为查询，动态地门控和适应密集的视觉特征，从而显式建模物理耦合。这种方法通过平行的3D-CNN处理原始投影堆栈及其扩散-衍射校正的对应物，为空间调制双重3D视觉流提供了可能，使模型能够基于物理上下文重新校准其视觉感知。

**Result:** 该方法能够虚拟地进行化学表征，消除传统打印后的测量需求，实现了对化学转化状态的精准控制。

**Conclusion:** C-PGA架构在虚拟化学表征上的表现证明了其在理解和预测材料化学状态方面的能力，为化学转换状态的精确控制提供了新的可能性。
...
> 本文提出了一种无需训练的局部测试时间缩放框架LoTTS，其在无需整体图像重计算的情况下改善了图像中的局部缺陷，显著提升了图像局部质量和整体保真度，并减少了GPU成本。

<details>
  <summary>Details</summary>

**Motivation:** 现有测试时间缩放方法全图处理忽略了图像质量的空间异质性，导致在已经满意的区域进行不必要的计算和在局部缺陷区域矫正不足。本文动机在于探索一种新的方向，即局部测试时间缩放，该方法可以自适应地重新采样缺陷区域，同时保持高质量区域，从而大幅度减少搜索空间。

**Method:** 本文提出了一种新的方法LoTTS，这是一种完全不需要训练的局部测试时间缩放框架。LoTTS通过对比质量感知提示下的交叉注意力和自我注意力信号来定位缺陷区域，并将其细化为连贯的掩码。为了保持全局一致性，LoTTS只扰动缺陷区域，并对其进行局部去噪处理，确保对缺陷的纠正限制在局部区域，而图像的其他部分保持不变。

**Result:** 实验表明，LoTTS在SD2.1，SDXL和FLUX上取得了最先进的性能，不仅在局部质量上有所改善，还在全局一致性上有所提高，与Best-of-N采样相比，GPU成本降低了2-4倍。

**Conclusion:** 这些发现确立了局部测试时间缩放作为一个有前途的新方向，旨在在推理过程中对扩散模型进行有效缩放。
...
> HybriDLA is a novel generative framework for document layout analysis which combines diffusion and autoregressive decoding to improve performance on complex modern documents.

<details>
  <summary>Details</summary>

**Motivation:** Traditional methods for document layout analysis are insufficient for modern documents due to their diverse and complex layouts with a varying number of elements.

**Method:** HybriDLA employs a multi-scale feature-fusion encoder and unifies diffusion and autoregressive decoding to improve region prediction.

**Result:** The proposed method achieves an 83.5% mean Average Precision (mAP) and sets state-of-the-art performance on DocLayNet and M$^6$Doc benchmarks.

**Conclusion:** HybriDLA is an effective solution for the challenges posed by the growing complexity and variety of modern documents.
...
> 本文提出了一种新的图像检索框架DetVLM，结合目标检测和视觉大型模型，实现了高效率和高准确率的细粒度图像检索。

<details>
  <summary>Details</summary>

**Motivation:** 现有的细粒度图像检索方法存在不足，手动特征缺乏鲁棒性；基于深度学习的检测器无法执行状态特定检索或零样本搜索；视觉大型模型虽能提供语义和零样本能力，但空间定位不佳且计算成本高。

**Method:** DetVLM融合目标检测与视觉大型模型（VLMs），采用两阶段管道提升检索性能。第一阶段使用YOLO检测器进行高效、高召回率的组件级筛选；第二阶段使用VLM进行二次验证，增强召回能力。

**Result:** 实验结果表明，DetVLM在车辆部件数据集上的整体检索准确率为94.82%，优于仅使用检测器的基线方法。其在零样本搜索中检测驾驶员佩戴口罩的准确率为94.95%，在状态搜索任务中的平均准确率超过90%。

**Conclusion:** DetVLM通过结合检测器的高效能和VLM的语义和零样本能力，提供了一种新的智能图像搜索框架，提升了细粒度图像检索的准确性和效率。
...
> CPDATrack, a novel tracking framework, suppresses background and distractor tokens while maintaining contextual information, achieving state-of-the-art performance in tracking benchmarks.

<details>
  <summary>Details</summary>

**Motivation:** To address the issues of weakened discriminative capability due to excessive background token interference and potential loss of contextual information from existing token pruning methods.

**Method:** One-stream Transformer-based trackers concatenate template and search region tokens for joint attention. CPDATrack introduces a learnable module to estimate target association probabilities and a selective attention mechanism to reduce background and distractor interference while maintaining contextual cues.

**Result:** CPDATrack achieves state-of-the-art performance, notably an overlap of 75.1 percent on the GOT-10k benchmark.

**Conclusion:** The integration of a probability estimation method and selective attention mechanism effectively suppresses background and distractor tokens, thus improving overall tracking performance.
...
> 本文提出DRIFT框架，利用图像扩散模型的自注意力图作为语义标签传播核，结合测试时优化策略，实现零样本视频对象跟踪，达到先进性能。

<details>
  <summary>Details</summary>

**Motivation:** 虽然图像扩散模型最初是为图像生成开发的，但其捕捉的丰富语义结构可以用于诸如识别和定位等任务之外的工作。本文探索这些模型在视频对象跟踪中的应用潜力。

**Method:** 通过重新解释图像扩散模型的自注意力图作为语义标签传播核，提供像素级对应关系，并扩展到帧间传播，以实现零样本对象跟踪。还探讨了测试时优化策略，包括DDIM反转，文本反转和自适应头部加权，以适应扩散特征使之进行鲁棒和一致的标签传播。

**Result:** 提出了一种零样本对象视频分割框架DRIFT，利用预训练的图像扩散模型结合SAM指导的掩码细化，在标准视频对象分割基准上达到最先进性能。

**Conclusion:** 图像扩散模型的自注意力图可以作为语义标签传播核应用于零样本视频对象跟踪，通过测试时优化策略，DRIFT框架在视频对象分割任务上表现出色。
...
> The paper addresses high-resolution image editing limits with a patch-wise optimization framework, achieving high-quality edits and paving the way for better high-resolution content creation.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to address the challenge of high-resolution image editing, as existing methods are limited to low-resolution settings, typically supporting only up to 1K resolution.

**Method:** Our method performs patch-wise optimization on high-resolution source images, followed by a fine-grained detail transfer module and a novel synchronization strategy to maintain consistency across patches.

**Result:** Extensive experiments show that the method produces high-quality edits for high-resolution images.

**Conclusion:** The work introduces a test-time optimization framework for high-resolution image editing, advancing the field towards effective, controllable high-resolution content creation.
...
> 本文提出了一种名为SPROUT的无训练及无注释的分割框架，用于病理学中的核实例分割。通过组织学导向的先验知识指导，该方法无需参数更新即可实现精确分割，展示了无训练方法在病理学应用上的潜力。

<details>
  <summary>Details</summary>

**Motivation:** 研究表明，大规模视觉基础模型在零样本生物医学分段中表现出潜力，但大多数现有方法仍然依赖于密集的监督和计算密集的微调。因此，无训练方法是一个有吸引力的研究方向，但尚未得到充分研究。

**Method:** 本研究介绍了一种名为SPROUT的无训练、零注释的提示框架，用于核实例分割。该方法利用组织学相关的先验知识构建特定于幻灯片的参考原型，以缓解领域差异。通过部分最优传输方案逐步引导特征对齐，将生成的前景和背景特征转化为正负点提示，使分段任何模型（SAM）能够无需参数更新产生精确的核分割。

**Result:** 在多个组织病理学基准上的广泛实验表明，SPROUT在没有监督或重新训练的情况下，实现了有竞争力的性能，确立了用于病理学中可扩展、无训练的核实例分段的新范式。

**Conclusion:** SPROUT是首个完全无训练和无注释的方法，在核实例分割任务中表现出了显著的效果，为计算病理学提供了新的研究方向和应用可能性。
...
> 提出GFT-GCN，一种结合谱图学习和扩散机制保护模板的3D人脸识别框架，确保隐私保护的同时提供高识别精度和强抗重建攻击能力。

<details>
  <summary>Details</summary>

**Motivation:** 3D人脸识别因其捕获面部几何形状，对光照变化、姿态变化和呈现攻击具有抗性，所以可作为鲁棒的生物识别解决方案。然而，存储的生物识别模板的安全保护仍然是至关重要的。GFT-GCN框架旨在保护隐私的同时实现高性能。

**Method:** 结合谱图学习和扩散机制保护模板的3D人脸识别框架GFT-GCN，利用Graph Fourier Transform (GFT)和Graph Convolutional Networks (GCN)从3D人脸网格中提取紧凑且具有区分性的谱特征，并引入谱扩散机制生成不可逆、可再生且不可链接的模板。

**Result:** 实验基于BU-3DFE和FaceScape数据集，结果显示GFT-GCN具有高的识别精度和强的抗重建攻击性。

**Conclusion:** GFT-GCN框架有效平衡了隐私保护和性能之间的关系，对安全的3D人脸识别提供了一种实际解决方案。
...
> 本文提出了一种名为MambaEye的新视觉编码器，它能够处理任意图像分辨率，尤其是在高分辨率图像处理上表现出色，同时保持与patch数量线性的时间和内存复杂度。

<details>
  <summary>Details</summary>

**Motivation:** 尽管在视觉编码领域取得了数十年的进步，但一个真正与输入大小无关的视觉编码器——这是人类视觉的基本特征——仍然是遥不可及的。本文旨在解决这一问题。

**Method:** 提出了一种名为MambaEye的新颖因果顺序编码器，它基于纯Mamba2骨干网络，该网络具有低复杂度和基于因果过程的特点。与之前的Mamba视觉编码器不同，这种新型编码器采用严格的单向处理方式，保留了状态空间模型内部固有的因果性，能够在输入序列中的任意点生成预测。其核心创新在于使用相对移动编码，这为平移不变性提供了强烈的归纳偏置，使模型能够自然适应任意图像分辨率和扫描模式。为了达到这个目的，引入了一种新颖的扩散启发式损失函数，它提供了密集的逐步监督，使得模型随着收集更多视觉证据而建立信心。

**Result:** 实验结果表明，MambaEye在不同分辨率的图像上表现出稳健的性能，尤其是在高分辨率图像（例如1536²）的ImageNet-1K分类任务中表现突出。

**Conclusion:** 实验表明，MambaEye在ImageNet-1K分类任务上对于1536²的高分辨率图像具有稳健的性能，同时保持了与patch数量线性的时间和内存复杂度。
...
> 提出名为HiCoGen的新框架，解决复杂提示下扩散模型的生成问题，通过LLM分解提示，迭代合成单元，引入RL框架和衰减随机性调度，实验表明其在概念覆盖率和组合准确性上优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 当前的扩散模型在处理涉及多个对象和层次结构的复杂提示时遇到了困难，导致概念遗漏、混淆和组成性差的问题。该研究旨在解决这些局限性。

**Method:** 提出了一种名为HiCoGen的分层组合生成框架，该框架基于合成链(CoS)的新范式。HiCoGen首先使用大语言模型(LLM)将复杂的提示分解为最小的语义单元，然后在每个步骤中迭代地合成这些单元，以便于逐步构建所有文本概念到最终场景。为了进一步优化这一过程，我们引入了一个强化学习(RL)框架。我们发现传统扩散采样器的有限探索限制了有效RL，因此我们提出了一种称为衰减随机性调度的新采样方法来增强探索。此外，我们设计了一个分层奖励机制，用于全局、主体和关系三个层次上的图像评估。我们还构建了一个新的文本到图像基准测试HiCoPrompt，具有分层提示以进行严格的评估。

**Result:** 实验显示，我们方法在概念覆盖率和组合准确性方面显著优于现有方法。

**Conclusion:** 通过引入新的框架和方法，我们的研究在解决复杂图像生成的挑战方面取得了显著的进步，特别是在处理复杂提示时的概念覆盖率和组合准确性方面。
...
> 本文提出了VGGT4D，一个无需训练的框架，该框架扩展了3D基础模型VGGT，用于稳健的4D场景重建。通过挖掘和放大全球动态暗示，结合投影梯度驱动的细化策略，VGGT4D在动态对象分割、相机姿态估计和密集重建方面表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 现有的4D方法依赖于外部先验、繁重的后优化或需要在4D数据集上微调，而3D基础模型如VGGT在动态对象占主导时性能下降。因此，为了提高4D场景重建的鲁棒性，提出了VGGT4D。

**Method:** VGGT4D设计了一个策略，通过全局动态提示的挖掘和放大，结合在时间窗口内的聚合，以及投影梯度驱动的边界细化技术，提升了动态和静态元素的分离能力。

**Result:** 在六个数据集中，VGGT4D在动态对象分割、相机姿态估计和密集重建方面均优于现有方法，并支持对超过500帧序列进行单次推理。

**Conclusion:** VGGT4D框架展示了显著的动态4D场景重建能力，无需训练且能适应长时间序列的处理。这表明它是一个有效的解决方案，在各种4D应用中具有潜力。
...
> 研究探索强化学习与可验证奖励（RLVR）对大型多模态模型（LMMs）的输入激活影响，发现对低熵激活有调优的空间。提出一种无需额外训练的激活回放（Activation Replay）技术，这可提高LMMs的推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 动机在于探索RLVR如何影响大模态模型中的输入激活，发现RLVR对低熵激活影响超出预期，而对高熵激活的影响较少，这与LMM的推理能力相关，提出通过控制低熵激活来优化模型行为。

**Method:** 本文提出了一种名为Activation Replay的新方法，这是一种无需训练的简单而有效的方法，用于在多模态大模型（LMMs）的后训练阶段提升其多模态推理能力。该方法在测试时通过重放基础LMMs输入上下文中低熵激活来调节RLVR（强化学习与可验证奖励）增强的模型行为。

**Result:** 实验结果表明，Activation Replay可以在包括数学、o3-like视觉代理和视频推理等多种情况下提高LMMs的推理表现，同时提升Pass@K指标，并缓解RLVR覆盖范围狭窄的问题。

**Conclusion:** 结论是，通过测试时对视觉标记的输入进行操纵，激活重播激活的方法在提升推理能力方面表现出了优越性，超过了其他替代方案的性能。
...
> A novel EmoFeedback2 approach is introduced for generating images with continuous emotions through reinforcement learning and feedback mechanism, significantly improving emotional fidelity and continuity over existing C-EICG methods.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to address the existing gaps in continuous emotional image generation, namely the lack of emotional feedback from generated images and the inability to adjust emotional prompts according to the image content adaptively.

**Method:** The paper proposes a novel EmoFeedback2 paradigm that integrates a fine-tuned large vision-language model (LVLM) for reinforcement learning in continuous emotional image generation (C-EICG). This involves an emotion-aware reward feedback and a self-promotion textual feedback framework to improve emotional continuity and fidelity.

**Result:** Experimental results show that the proposed EmoFeedback2 approach enhances emotional continuity and fidelity, generating higher quality images compared to existing methods, as demonstrated on a custom dataset.

**Conclusion:** The proposed EmoFeedback2 method successfully achieves superior emotional image generation over existing state-of-the-art methods, with plans to share code and datasets for further research.
...
> A new training-free inpainting method that optimizes initial seed noise to enhance the performance of off-the-shelf text-to-image models, outperforming existing approaches.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to improve the effectiveness of training-free methods for inpainting, as existing guidance-based methods are limited and require specialized models.

**Method:** We propose a novel training-free method for inpainting by optimizing the initial seed noise to match the unmasked parts of the data, using linear approximation to relate initial noise and the output, and optimizing in the spectral domain to stabilize the process.

**Result:** The proposed method is demonstrated to be effective on various inpainting tasks and outperforms the state of the art.

**Conclusion:** This work shows that optimizing the initial seed noise can enable the use of generic models for inpainting without the need for specialized training.
...
> 本文提出了一种细节感知的改进框架，通过两个连续的参考驱动校正阶段，显著提升了像素级一致性，并在具有挑战性的参考引导修复基准上超越了开源和商用模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的扩散模型在使用参考图像进行图像生成时难以保留精细的视觉细节，原因是VAE基础的潜在压缩会丢弃细微纹理信息，并导致局部编辑不一致。

**Method:** 首先将单图像扩散编辑器进行微调，使其能同时处理草图图像和参考图像以实现全局一致的改进；然后用强化学习增强局部编辑能力，明确优化细节准确性和语义一致性。

**Result:** 实验结果表明，本方法显著改善了参考对齐能力并保留了精细细节，生成的编辑结果在视觉上与其他模型相比更一致、更真实。

**Conclusion:** 本文提出的方法通过两个连续阶段的参考驱动校正框架，超越了开源和商用模型，尤其在挑战性更强的参考引导恢复基准上表现更为优异。
...
> 本文提出了一个名为CREward的创意奖励模型，旨在通过几何、材质和纹理三个维度更加精确地评估创意图像。该模型基于人类评估和LVLMs的预测来训练，并可用于创意图像的评估、解释和引导生成。

<details>
  <summary>Details</summary>

**Motivation:** 当涉及到表示和评估创意时，将其视为单一且未细分的量似乎是幼稚和不全面的。

**Method:** 本文提出了一种名为CREward的创意奖励模型，这是一个首次尝试的特定类型创意奖励模型。该模型涵盖了三个创意维度：几何、材质和纹理，以便从图像形成管道的角度来观察创意。

**Result:** 作者通过人类基准评估捕捉了人类对不同类型创意图像的感知，并分析了人类判断与大型视觉语言模型（LVLMs）预测之间的相关性。发现LVLMs与人类感知有很强的一致性。基于这些观察结论，作者收集了LVLM生成的标签来训练适用于创意图像评估和生成的CREward模型。

**Conclusion:** CREward模型在创意评估、可解释的创意分析以及获取用于人类设计灵感和引导创意生成的创意样本方面都有应用价值，特别是在低秩适应引导创意生成方面。
...
> SAUPs, a method to manipulate model outputs in real-world scenarios with a single perturbation, are introduced and tested, revealing significant vulnerabilities in large language models.

<details>
  <summary>Details</summary>

**Motivation:** To reveal the threat of single perturbations capable of hijacking a sequence of decisions in neural networks, which can lead to severe risks.

**Method:** Semantic-Aware Universal Perturbations (SAUPs) are introduced to manipulate the outputs of a model towards multiple predefined outcomes, overcoming optimization challenges with a new algorithm.

**Result:** Experiments on three multimodal large language models show a 70% attack success rate, demonstrating the vulnerability to SAUPs even when controlling five distinct targets with just an adversarial frame.

**Conclusion:** The paper highlights the severe threat posed by SAUPs to models operating in a sequence of decisions and underscores the need for defenses against such perturbations.
...
> 本文提出了一种多模态融合网络，用于预测城市环境中行人穿越意图，有效降低了自动驾驶车辆与行人碰撞的风险。

<details>
  <summary>Details</summary>

**Motivation:** 行人的穿越意图预测对于在城市环境中部署自动驾驶车辆至关重要。理想化的预测为AVs提供了关键的环境线索，从而降低了与行人的碰撞风险。但是，该预测任务由于行人行为多样性及其对多种情境因素的依赖性而极具挑战性。

**Method:** 本文提出了一种多模态融合网络，利用视觉和运动分支的七个模态特征，旨在有效提取和整合不同模态的互补线索。具体来说，使用多个基于Transformer的提取模块从原始输入中提取运动和视觉特征。深度引导注意力模块利用深度信息通过全面的空间特征交互来指导对另一个模态的注意力。为了考虑不同模态和帧的重要性，设计了模态注意力和时间注意力以选择性地强调信息性较强的模态并有效捕捉时间依赖性。

**Result:** 在JAAD数据集上的广泛实验验证了所提网络的有效性，与基线方法相比，其表现更优。

**Conclusion:** 所提出的多模态融合网络在处理行人穿越意图预测任务的表现优于传统方法，为自动驾驶车辆在城市环境中安全驾驶提供了关键技术支持。
...
> 本文提出了一种新的多上下文融合变换器（MFT），能够更准确地预测行人在城市环境中的过马路意图，具有提升行人安全和减少交通事故的潜力。

<details>
  <summary>Details</summary>

**Motivation:** 为了解决城市环境中精确预测行人意图的难题，作者提出了一种多上下文融合变换器(MFT)，旨在提高自动驾驶车辆中的行人安全和减少交通事故。

**Method:** MFT采用多上下文融合策略，包括互内上下文注意力和互跨上下文注意力机制，以融合来自四个关键维度（行人行为、环境、行人定位和车辆运动）的数据，最终通过指导性跨上下文注意力增强全局CLS标记，实现更深层次的融合。

**Result:** 实验结果表明，MFT相较于现有方法具有显著优势，在JAADbeh、JAADall和PIE数据集上分别达到了73%、93%和90%的准确率。

**Conclusion:** MFT模型通过融合多维度的信息实现了精确的行人过马路意图预测，展示了在城市环境中提高行人安全和减少交通事故的潜力。
...
> 该论文提出了一种基于注意力机制的跨模态交互Transformer（ACIT），用于行人过马路意图预测，并展示了其在两个数据集上的优越性能。

<details>
  <summary>Details</summary>

**Motivation:** 行人过马路意图的准确预测对于防止自动驾驶车辆发生行人相关的碰撞至关重要，但如何有效提取和整合来自不同类型数据的互补线索是一个重大挑战。

**Method:** ACIT利用六个视觉和运动模态，并分为三组交互对。在每个视觉交互对中使用双路径注意力机制，在运动交互对中使用跨模态注意力机制。此外，还包括多模态特征融合模块和基于Transformer的时间特征聚合模块。

**Result:** 实验结果显示ACIT在JAADbeh和JAADall数据集上的准确率分别为70%和89%，优于现有方法。并进行了广泛的消融研究以探究ACIT不同模块的贡献。

**Conclusion:** ACIT作为一种融合多模态信息的有效框架，在行人过马路意图预测任务上取得了显著的成功。
...
> 我们定义了Safety-Critical Reasoning任务，使用多视角输入解决驾驶安全的关键推理问题。通过WaymoQA数据集，使模型更擅长处理复杂的高风险驾驶场景。

<details>
  <summary>Details</summary>

**Motivation:** 当前的多模态大型语言模型虽然对驾驶场景理解能力强，但面对高风险的自动驾场景时在高级推理方面仍面临挑战，尤其是在处理一种交通风险的同时可能产生另一种风险的情况。为了解决这一问题，我们提出了一个广泛视野下的多视角输入解决方案。

**Method:** 我们提出了Safety-Critical Reasoning任务，将其分为两阶段：首先解决即时风险，然后减少因决策产生的后续风险。为了支持这一任务，我们引入了WaymoQA数据集，包含35,000个人工标注的问题答案对，覆盖复杂的高风险驾驶场景。

**Result:** 实验结果显示，现有的MLLM模型在处理安全关键场景时的表现不如在普通驾驶场景下，但通过使用WaymoQA数据集进行微调后，这些模型的推理性能有了显著的提高。

**Conclusion:** 实验表明，现有的MLLMs在关键安全场景中的表现不如普通场景，但在使用WaymoQA进行微调后，其推理能力显著提升，证明了该数据集在开发更为安全、更具推理能力的驾驶代理上的有效性。
...
> 本文介绍了SAM-MI框架，通过引入文本引导稀疏点提示器、浅层掩码聚合和分离掩码注入技术有效整合SAM与OVSS模型，解决SAM过度分割问题，显著提高分割准确率和速度。

<details>
  <summary>Details</summary>

**Motivation:** 虽然SAM展示了出色的全局分割能力，但仍有过度分割和固定掩码与标签难组合的问题。本文旨在利用SAM增强OVSS模型，同时解决这些问题。

**Method:** SAM-MI框架包括文本引导稀疏点提示器进行稀疏提示采样，浅层掩码聚合缓解过度分割，以及分离掩码注入技术将SAM生成的掩码分别在低频和高频处进行指导。

**Result:** 在多个基准测试中，SAM-MI表现优异，在MESS基准上的mIoU相对提高16.7%，速度提升1.6倍。

**Conclusion:** SAM-MI框架通过其创新技术整合了SAM与OVSS模型，显著提高了分割效果和处理速度，有望成为连接SAM与OVSS的有效工具。
...
> 我们提出了无训练的Vision-Guided Attention (VGA) 方法，通过利用视觉标记的语义内容来改善MMLM的视觉注意力机制。VGA大大减少了幻觉现象，并且对多模态大语言模型的性能有显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 多模态大语言模型在解释视觉信息时，其视觉注意力机制的局限性常导致幻觉现象。尽管多模态大语言模型可以准确地从视觉标记中提取视觉语义，但在后续推理中未能充分利用这一优势。为了解决这个问题，我们提出了Vision-Guided Attention (VGA) 方法。

**Method:** 我们提出了一种名为Vision-Guided Attention (VGA) 的无训练方法。首先，通过利用视觉标记的语义内容构建精确的视觉对齐。然后，使用这种对齐来指导模型关注相关的视觉区域。在图像描述任务中，VGA 还通过抑制已经被描述过的区域，动态地改进这种引导。每个标记只需进行一次前向传递，引入的延迟开销仅为4.36％。此外，VGA 完全兼容高效的注意力实现方式，如FlashAttention。

**Result:** 在不同的多模态大语言模型（MLLMs）以及多个幻觉基准测试上进行的广泛实验表明，VGA 达到了最先进的去幻觉性能。

**Conclusion:** 进一步的分析证实，明确的视觉引导在增强多模态大语言模型的视觉理解能力方面发挥着关键作用。
...
> COVec提出了一种基于光照感知的矢量化方法，实现了图像的高质量矢量化，改进了现有方法在复杂图像处理上的表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有的矢量化方法在处理复杂的现实图像时常常会产生碎片化的形状，牺牲了语义简洁性。为了应对这一挑战，提出了COVec，旨在提高矢量图像的视觉保真度和可编辑性。

**Method:** COVec采用基于明暗对比原理的光照感知矢量化框架，首次在矢量域中引入固有图像分解，将图像分解为反照率、阴影和光线层。通过语义引导初始化和两阶段优化，利用可微渲染技术对这些图层进行优化。

**Result:** 实验结果表明，COVec在各种数据集上达到了更高的视觉保真度和显著改善的可编辑性。

**Conclusion:** COVec展示了其在矢量化技术中独特的创新潜力，展示了在视觉保真度和可编辑性上的显著提升。
...
> 本研究提出MFM-Point框架，显著提升点云生成方法的可扩展性和性能，同时保持简单性，特别在多类别和高分辨率生成任务上获得了优良战绩。

<details>
  <summary>Details</summary>

**Motivation:** 尽管基于点的方法因其训练成本低和算法简单而颇具吸引力，但它们的表现通常不及基于表示的方法。本研究旨在提升基于点的方法的可扩展性和性能，使其具有竞争力。

**Method:** 本论文提出了MFM-Point，一种用于点云生成的多尺度流匹配框架。该方法采用从粗到细的生成范式以提高生成质量和可扩展性，同时不增加训练或推断的开销。为了保持点云的几何结构和不同分辨率下的分布平滑过渡，我们引入了结构化的降采样和上采样策略，确保粗细分辨率之间的一致性。

**Result:** 实验结果显示，MFM-Point在基于点的方法中达到了最佳性能，甚至对某些表现优于基于表示的方法，尤其在多类别和高分辨率生成任务中表现出色。

**Conclusion:** 总的来说，MFM-Point框架提升了点云生成任务的性能水平，并展现了在多类别和高分辨率任务中的卓越表现。
...
> 提出HACBSR方法，解决行星遥感图像中盲超分辨率问题，无需地面真值图像。

<details>
  <summary>Details</summary>

**Motivation:** 行星遥感图像受成像环境和硬件限制的影响，存在多样化且未知的退化现象，这限制了图像质量并阻碍了监督盲超分辨率的发展。

**Method:** HACBSR采用对比采样机制和历史增强对比学习两种组件，无需地面真值图像和外部内核先验，实现了盲超分辨率。

**Result:** 实验表明，HACBSR在多个放大倍数下与最先进的无监督方法相比具有竞争力。

**Conclusion:** HACBSR作为一个无监督框架，无需地面真值图像，实现了盲超分辨率，在行星应用中展现了良好的性能。
...
> Introduces DeLight-Mono for self-supervised monocular depth estimation under uneven illumination, featuring illumination decoupling and verified by extensive evaluations.

<details>
  <summary>Details</summary>

**Motivation:** To improve depth estimation in endoscopic images where uneven illumination, especially low-light issues, degrades performance and existing methods are ineffective or unsuitable.

**Method:** Self-supervised monocular depth estimation framework with illumination decoupling called DeLight-Mono, involves image decomposition into illumination-reflectance-depth using auxiliary networks.

**Result:** The proposed method effectively mitigates uneven illumination impact on depth estimation, as proven by rigorous comparisons and an ablation study on two public datasets.

**Conclusion:** DeLight-Mono is effective in handling illumination-related challenges in depth estimation for endoscopic navigation systems, outperforming existing methods through rigorous validation.
...
> FLaTEC is a frequency-aware point cloud compression method that decouples frequency components and uses triplane representations to achieve high compression ratios with reduced computational costs and storage requirements.

<details>
  <summary>Details</summary>

**Motivation:** To address the challenge of balancing compression ratio and reconstruction quality, particularly regarding the differential contributions of low-frequency and high-frequency components.

**Method:** The paper proposes FLaTEC, a frequency-aware point cloud compression method. It decouples low-frequency structures and high-frequency textures, using triplane representations to reduce sparsity and computational costs. A frequency-disentangling technique extracts compact low-frequency content while preserving high-frequency details, which are stored in binary format. During decoding, a modulation block recovers full-spectrum signals, and a frequency-based attention mechanism ensures local connectivity.

**Result:** The proposed method achieves state-of-the-art rate-distortion performance, outperforming standard codecs by 78% and 94% in BD-rate on SemanticKITTI and Ford datasets.

**Conclusion:** FLaTEC effectively improves compression performance for point clouds by decoupling frequency components and using triplane representations, resulting in both high compression ratios and preserved reconstruction quality.
...
> 本文提出了PRADA方法，用于检测和归因自回归生成的图像，该方法基于概率比率分析效果显著。

<details>
  <summary>Details</summary>

**Motivation:** 到目前为止，还没有专门针对自回归生成图像检测的工作，因此本文旨在填补这一空白，提供一种简单且易于解释的方法来检测自回归生成的图像。

**Method:** 该论文提出了一种名为PRADA的方法，用于可靠地检测自回归生成的图像并将其归因于相应的生成模型。PRADA方法基于模型条件概率与无条件概率之间比率的检查来判断图像是否由特定自回归生成模型生成。通过校准一个简单的、模型特定的评分函数来利用这些比率的独特特征，进行阈值归因和检测。

**Result:** 实验评估表明，PRADA方法在八种类到图像和四种文本到图像的模型上都非常有效。

**Conclusion:** PRADA方法通过利用图像生成模型特有的概率比率特性，能够有效地检测和归因来自不同自回归生成模型的图像，表现出较高的检测准确率。
...
> 研究提出了一种新的TSS框架，并采用渐进预训练策略，通过引入状态概念强制视频表示学习模型在状态、步骤和任务之间形成关联，从而提高了任务识别、步骤识别和下一步预测的性能。

<details>
  <summary>Details</summary>

**Motivation:** 尽管现有方法通过将视觉内容与文本描述（任务和步骤级别）对齐来为视频表示注入程序语义，但由于这两者的高度抽象性，未能与视觉数据中具体的可观察细节形成稳健的对齐。因此，需要一种新的方法来解决这一问题。

**Method:** 引入“状态”概念，即将对象配置的文本快照作为视觉基础的语义层，以在抽象过程与模型实际可视的细节之间建立更稳定的连接。在此基础上，提出了任务-步骤-状态（TSS）框架，并采用渐进的预训练策略强制模型在状态、步骤和高层次任务之间建立关联。

**Result:** 在COIN和CrossTask数据集上的大量实验表明，提出的方法在任务识别、步骤识别和下一步预测等多种下游任务上优于基线模型。此外，引入状态监督是所有任务中性能提升的关键驱动因素，渐进预训练策略比标准联合训练更有效。

**Conclusion:** 该研究证明了在视频表示学习中纳入状态监督的重要性，以及渐进预训练策略在强制模型识别直观结构方面的有效性。这种方法有助于构建能够思考并执行复杂任务的代理。
...
> 本文提出了一种新的盲自适应本地降噪(BALD)方法来解决在CEST MRI数据中遇到的空间变化噪声和复杂成像协议的异质性问题，实验验证显示，该方法在降噪指标和分子浓度映射估计等下游任务中优于现有的CEST降噪方法。

<details>
  <summary>Details</summary>

**Motivation:** CEST MRI在临床上的转化受到硬件限制引起的空间变化噪声和复杂成像协议造成的数据异质性的影响，这扰乱了定量对比度映射的准确性，如酰胺质子转移成像。传统的降噪方法无法应对这种复杂的噪声，并且可能会影响生物医学分析所必需的潜在信息。为了克服这些限制，提出了一个新的盲自适应本地降噪(BALD)方法。

**Method:** BALD方法通过利用CEST数据的自相似性，推导出一种自适应的方差稳定变换，该变换可以在不知道噪声特性的前提下，平衡CEST像素之间的噪声分布。BALD通过在数据的线性变换上进行两阶段降噪，来区分分子信号和噪声。并且采用局部SVD分解作为线性变换，以防止空间和谱降噪伪影。

**Result:** 在多个模型和体内CEST扫描的广泛验证实验中，BALD方法在降噪指标以及包括分子浓度图估计和癌症检测在内的下游任务中，均一直优于现有的最先进的CEST降噪方法。

**Conclusion:** 研究表明，提出的BALD方法能够有效地解决CEST数据中的降噪问题，同时保持分子信号的完整性，对于提高CEST MRI在临床应用中的准确性和可靠性具有重要意义。
...
> 本文提出了CONVAD方法，通过扩展CBMs至VAD领域，生成了更丰富、基于概念的解释，提高了异常检测的可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的VAD方法虽然可以提供一些视觉解释，但缺乏直接且语义上有意义的解释。我们希望通过提出CONVAD方法来解决这一问题，增强VAD系统的解释性和用户的理解及信任。

**Method:** 通过扩展Concept Bottleneck Models (CBMs) 至VAD领域，我们开发了一种方法来生成基于概念和视觉的解释，以此来提高异常检测的可解释性。我们还引入了一个合成人工异常的流水线，保持了VAD减少对罕见异常样本依赖的特点。

**Result:** 我们的方法，Concept-Aware Visual Anomaly Detection (CONVAD)，不仅提供了丰富的基于概念的解释，以增强用户对VAD系统的信任，同时其性能与传统的VAD方法相当。

**Conclusion:** CONVAD提供了一种新颖且更具洞察力的方式来解释图像中的异常，增强了VAD系统的可解释性和用户的信任。
...
> 本文提出了一种名为WPT的世界到策略转移训练范式，可以通过端到端世界模型进行在线蒸馏，提升了策略规划的性能并保持实时部署能力，实验表明该方法在准确性和安全性方面超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 克服现有方法存在的实时紧密耦合或依赖离线奖励信号的限制，例如，导致显著的推理开销或阻碍端到端优化。

**Method:** WPT，一种World-to-Policy Transfer训练范式，它能够在端到端世界模型的指导下进行在线蒸馏。通过可训练的奖励模型，该方法将世界知识融入到教师策略中，对候选轨迹与世界模型预测的未来动态进行对齐。此外，提出了策略蒸馏和世界奖励蒸馏，将教师的推理能力转移到轻量级的学生策略中，以提升规划性能并保持实时性能。

**Result:** 在开环和闭环基准测试中，WPT实现了最先进的性能：开环下碰撞率为0.11，闭环下驾驶得分为79.23，超越了基于世界模型和模仿学习的方法在准确性和安全性方面的表现。此外，学生策略在保持大部分性能提升的同时，推理速度提高了4.9倍。

**Conclusion:** 本文提出的世界到策略转移（WPT）范式，通过引入可训练的奖励模型，能够更有效地将世界知识融合进策略学习中，同时实现了高性能和实时部署的结合。
...
> 本文探索了一种用于检测森林火灾的早期预警系统，提出了一个新的数据集并通过比较不同的检测方法来改进森林火灾的早期检测。

<details>
  <summary>Details</summary>

**Motivation:** 由于缺乏规模较大的数据集和针对此任务调优的模型，现有的方法在检测森林火灾方面存在误检问题。因此，我们致力于解决这一问题，提出了早期预警系统用于检测森林火灾。

**Method:** 我们首先提出了一种通过视觉分析早期识别森林火灾的数据集。该数据集不同于现有包含广泛火灾图像的数据集，我们的数据集包含多个烟雾和火灾的实例，这些实例表明火灾的初始阶段。我们通过使用游戏模拟器如Red Dead Redemption 2合成获得此数据集，并将其与已发布的图像相结合，以获得更全面的数据集。最后，我们在提出的数据集上比较了图像分类和定位方法，特别是YOLOv7（You Only Look Once）和不同检测变压器模型。

**Result:** 在我们提出的合成数据集上，通过比较YOLOv7和不同检测变压器模型的性能，提供了森林火灾早期识别的有效方法。

**Conclusion:** 通过使用合成数据集和现有的图像，我们能够更好地训练模型，以提高对森林火灾早期识别的准确性。比较了YOLOv7和不同检测变压器模型在该数据集上的性能，为森林火灾的早期检测提供了新的视角和潜在解决方案。
...
> 本文介绍了一种结合深度学习工具和注意力机制的整合方法，用于通过X光图像自动检测心脏扩大（cardiomegaly）。该方法采用Inception V3模型和多头注意力机制，以提高检测心脏扩大的准确性。最终模型达到较高的准确率、精确率、召回率等评价指标。

<details>
  <summary>Details</summary>

**Motivation:** 心血管疾病的诊断以及心脏结构异常如心脏扩大的可视化促进了新型成像技术在医疗行业的应用。本研究致力于提出一种更为有效的检测心脏扩大的方法。

**Method:** 研究中采用了深度学习的CNN配置，具体使用Inception V3模型，同时加入了多层注意力机制来增强模型的性能。其中，多头注意力机制是该方法的关键特征之一，能够自动学习特征并实现精确检测。

**Result:** 在评估模型时采用了严格的评价体系，最终模型达到95.6%的准确率，95.2%的精确率，96.2%的召回率，95.7%的敏感度，96.1%的特异度，以及96.0%的AUC值。

**Conclusion:** 该方法将深度学习与注意力机制相结合，成功提高了心脏扩大检测的准确性和临床应用价值。
...
> 提出LungEvaty模型，通过单一低剂量CT扫描预测肺癌风险，性能媲美现有顶尖方法，且使用大量CT样本训练。

<details>
  <summary>Details</summary>

**Motivation:** 随着越来越多的国家引入基于低剂量CT的全民筛查项目，肺癌风险评估的重要性日益增加。现有方法要么过于依赖像素级标注，限制了可扩展性，要么以碎片方式分析肺部，从而削弱性能。因此，需要一种能够高效处理整个肺部体积的可扩展方法。

**Method:** LungEvaty ，一种完全基于变压器的框架，用于从单次低剂量CT扫描预测1-6年的肺癌风险。该模型可处理整个肺部输入，直接从大规模筛查数据中学习以捕捉与恶性肿瘤风险相关的全面解剖和病理线索。无需任何区域监督，仅使用影像数据。

**Result:** LungEvaty 无需区域监督，只使用影像数据即可达到顶尖性能。加上可选的基于解剖信息的关注引导（AIAG）损失后，性能更佳。该模型已在超过90,000个CT扫描中进行训练，其中包括超过28,000个用于微调，6,000个用于评估。

**Conclusion:** 这项研究提供了一种简单、数据高效且完全开源的解决方案，为未来在纵向和多模式肺癌风险预测方面的研究提供了一个可扩展的基础。
...
> UltraViCo is a new method that improves the extrapolation ability of video diffusion transformers by suppressing attention dispersion beyond the training length, achieving significant performance gains in video synthesis and editing.

<details>
  <summary>Details</summary>

**Motivation:** To address the challenge of video length extrapolation, where video diffusion transformers struggle to generalize beyond their training length, leading to quality degradation and periodic content repetition.

**Method:** UltraViCo, a training-free, plug-and-play method that suppresses attention for tokens beyond the training window via a constant decay factor.

**Result:** The method outperforms a broad set of baselines across models and extrapolation ratios. It pushes the extrapolation limit from 2x to 4x, improving Dynamic Degree and Imaging Quality by 233% and 40.5% respectively over the previous best method.

**Conclusion:** UltraViCo effectively addresses the issues of attention dispersion and periodic repetition in video diffusion transformers, enhancing generalization across various models and tasks.
...
> 研究提出PETRG-3D框架以改善自动化PET/CT报告生成，强调在处理功能PET成像时的挑战，引入PETRG-Score评估临床效用，并展示在自然语言和临床效能度量上的改进。

<details>
  <summary>Details</summary>

**Motivation:** 随着PET/CT扫描仪的迅速增加，训练有素的专业人员数量不足，因此自动化PET/CT报告生成变得日益重要。

**Method:** 提出PETRG-3D，这是一个端到端的3D双分支框架，分别对PET和CT体积进行编码，并结合风格自适应提示来缓解因医院间报告实践差异带来问题。

**Result:** 实验显示，PETRG-3D在自然语言度量和临床效能度量上显著优于现有方法。

**Conclusion:** 这项工作为未来的PET/CT特定模型奠定了基础，强调疾病感知推理和临床可靠的评估。
...
> 提出HCFSSNet，结合CNN和频率状态空间网络进行学习图像压缩，使用更少参数实现高性能。

<details>
  <summary>Details</summary>

**Motivation:** 尽管基于Transformer和状态空间模型的架构在学习图像压缩方面表现出色，但仍存在结构信息丢失或忽略对压缩至关重要的频率特征的劣势。

**Method:** HCFSSNet使用CNN提取局部高频结构，并引入Vision Frequency State Space (VFSS)模块来建模长距离低频信息。VFSS模块结合VONSS（横向、纵向和对角线扫描特征）和AFMM（离散余弦变换频率组件的自适应加权）以实现更有效的比特分配。为了减少熵模型中的冗余，将AFMM与Swin Transformer集成，形成频率感知的辅助信息建模模块FSTAM。

**Result:** 实验结果表明，HCFSSNet在Kodak，Tecnick和CLIC数据集上相较于近期的SSM编码器如MambaIC，使用更少的参数，实现了有竞争力的率失真性能。

**Conclusion:** HCFSSNet是学习图像压缩的一个高效且可解释的混合架构，提供了一种优化未来压缩系统的新方法。
...
> 提出Restora-Flow，一种无需训练的图像恢复方法，它使用退化掩码指导流匹配采样并进行轨迹修正，以改善感知质量和降低处理时间。

<details>
  <summary>Details</summary>

**Motivation:** 当前的方法虽然使用流模型在图像恢复上取得了有希望的结果，但仍存在处理时间过长或结果过度模糊的问题。

**Method:** Flow matching作为生成模型的一个新兴方法，改进了扩散模型长时间采样的问题，同时在图像生成方面保持高质量。Restora-Flow作为训练无关的方法，通过退化掩码来指导流匹配采样，并采用轨迹修正机制以确保与退化输入的一致性。

**Result:** 在涉及基于掩码退化的自然和医学数据集上进行图像恢复任务（如图像修补、超分辨率、去噪）的评估表明，Restora-Flow在感知质量和处理时间上优于扩散模型和其他基于流匹配的方法。

**Conclusion:** Restora-Flow作为训练无关的方法，在图像恢复任务中通过使用退化掩码和轨迹修正机制提供了更高的感知质量和更短的处理时间。
...
> 本研究提出了一种名为Riemannian Manifold Mapping的框架，其包含时间感知的神经常微分方程和基于注意力的黎曼几何门控递归单元，用于从不规则采样的纵向sMRI数据中预测阿尔茨海默病的进展。实验结果显示该方法在疾病状态预测和认知得分回归任务中优于现有模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的疾病预测模型大多在欧几里得空间进行，无法充分捕捉不规则采样纵向影像的内在连续性和非线性几何结构，因此研究团队提出了一种新的框架来解决这一问题。

**Method:** 研究采用了一种Riemannian manifold mapping框架，该框架将特征从高维sMRI投影到流形空间，然后使用时间感知的神经常微分方程描述潜变量在观测之间的连续演化，并且采用基于注意力的黎曼几何门控递归单元自适应整合历史和当前的信息。

**Result:** 实验结果显示该方法在疾病状态预测和认知得分回归任务中优于当前最先进的模型。消融实验验证了各模块对预测精度的贡献，证明了模型在不同序列长度和数据缺失率下表现稳定，展现了其良好的时间泛化能力。

**Conclusion:** 该研究证明了所提出的框架在处理不规则采样纵向sMRI数据预测阿尔茨海默病进展方面具有高精度和时间泛化性，具有广泛应用的潜力。
...
> Introduction of MAP-World, a prior-free multi-modal planning framework that predicts diverse, temporally consistent driving paths without using anchor libraries or teacher policies, achieving state-of-the-art performance on NAVSIM while maintaining real-time efficiency.

<details>
  <summary>Details</summary>

**Motivation:** To handle multiple plausible futures in motion planning for autonomous driving while maintaining computational efficiency, without the need for handcrafted anchors or reinforcement learning to select a single best mode, which discards information about alternative futures and complicates optimization.

**Method:** Motion planning for autonomous driving using MAP-World, combining masked action planning with a path-weighted world model to predict diverse, temporally consistent modes without relying on anchor libraries or teacher policies. A lightweight world model rolls out future BEV semantics for each candidate trajectory, learning from the full distribution of plausible futures during training without selection bias.

**Result:** On NAVSIM, the method matches the performance of anchor-based approaches and achieves state-of-the-art results among world-model-based methods, while avoiding the need for reinforcement learning and maintaining real-time inference latency.

**Conclusion:** The proposed MAP-World framework effectively incorporates multiple plausible futures into motion planning for autonomous driving, providing diverse, temporally consistent trajectory predictions while avoiding the limitations of existing methods that rely on handcrafted anchors or reinforcement learning.
...
> 本文提出了SKEL-CF，一种基于粗细框架的SKEL参数估计算法，解决了由于训练数据不足、视角模糊和人体关节复杂性等问题，显著提升了人体运动分析的准确性和生物力学真实性。

<details>
  <summary>Details</summary>

**Motivation:** 解决现有3D人体模型（如SMPL）在生物力学真实性上的不足，并克服SKEL模型参数直接估计的挑战。

**Method:** 通过粗细框架方法，采用编码-解码器结构直接预测本体参数，并利用重新标定的4DHuman-SKEL数据库和相机建模技术提高模型精度。

**Result:** 在MOYO数据集上，SKEL-CF显著优于先前的最先进方法HSMR，达到了85.0 MPJPE / 51.4 PA-MPJPE。

**Conclusion:** SKEL-CF框架作为一个人体运动分析的高精度和生物力学准确性的工具，连接了计算机视觉与生物力学。
...
> 提出Harmonious Parameter Adaptation (HPA)，一种后训练框架，用于解决安全对齐的多模态语言模型在持续视觉指令调优中出现的任务遗忘和安全退化问题。

<details>
  <summary>Details</summary>

**Motivation:** 尽管持续视觉指令调优(CVIT)在调整多模态大型语言模型(MMLM)方面展示了潜力，但现有研究主要集中在无安全对齐的模型上，忽视了模型中安全机制的重要性。此工作关注安全对齐的MMLM的CVIT，发现模型在持续适应过程中不仅会遇到任务遗忘，还会影响其安全性。

**Method:** HPA框架，包含基于关注的参数划分、和谐平衡的参数选择和正交参数调整。具体地，HPA将参数划分为侧重安全和任务性能的两类，并从平衡的角度选择关注的参数。此外，HPA对参数更新施加正交约束以进一步缓解灾难性遗忘。

**Result:** 实验结果显示，相比现有基线，HPA在CVIT基准和安全性评估数据集上更好地维持了高安全性和降低了忘记现象。

**Conclusion:** 该研究通过HPA框架成功解决了在持续视觉指令调优过程中，安全对齐的多模态大型语言模型所面临的任务遗忘与安全退化问题。
...
> 研究探索了大型多模态模型（LMMs）在识别视频中物体互动开始或结束的准确性的能力，并发现这些模型存在一定的不足，即它们无法准确判定和定位事件的发生时刻和位置。

<details>
  <summary>Details</summary>

**Motivation:** 这项研究旨在探索大型多模态模型在视频分析中对视觉输入的理解程度，特别是模型是否能在视觉场景中准确推理并定位物体之间的接触和释放。

**Method:** 研究使用了超过20K个标注交互行为的大型数据集，该数据集来自Something-Something-V2，用于评估两个大型多模态模型（Qwen-2.5VL和GPT-4o）在视频中识别互动开始或结束时间及其位置的能力。

**Result:** 研究结果显示，尽管这些模型可以可靠地识别目标对象和动作并提供连贯的推理，但它们不能准确地找出互动开始或结束的帧，也无法在场景中定位事件。

**Conclusion:** 研究发现，这些模型在定义互动的时刻和地点上定位物理接触的能力较弱，这表明它们缺乏对动态场景进行深入理解所需的感知基础。
...
> 提出了ADNet基准和Dinomaly-m模型，以应对现有异常检测方法在多类别大规模场景下的性能下降问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的异常检测基准覆盖的类别范围有限，限制了跨上下文泛化和扩展性的评估。为了解决这个问题，我们开发了ADNet，旨在提供一个标准化和可扩展的基准，支持社区扩展多样化的异常检测数据集。

**Method:** 我们提出了ADNet，一个大规模、多领域的异常检测基准，包含来自不同领域的49个公开数据集的380个类别。此外，我们提出了Dinomaly-m，这是一个基于Dinomaly的上下文引导的专家混合扩展，它在不增加推理成本的情况下扩展了解码器容量。

**Result:** 实验显示，现有的最先进的方法在一个对一的设置中达到了90.6%的I-AUROC，但在包含所有380类别的多类设置中降至78.5%。我们的Dinomaly-m达到了83.2%的I-AUROC和93.1%的P-AUROC，优于现有方法。

**Conclusion:** ADNet为未来的异常检测奠定了标准化和可扩展性的基础。Dinomaly-m通过不增加推理成本扩展解码器容量，展示了在大规模异常检测中的优越性能。
...
> 本文提出了一种基于电池供电的可穿戴瞳孔中心跟踪系统，该系统集成了事件驱动的感知和神经形态处理，并在商用Speck2f系统芯片上实现了轻量级坐标解码，实现了超低功耗和高频率的实时瞳孔跟踪。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可穿戴平台难以实现高频率且功耗极低的眼睛跟踪，而基于事件的视觉传感器虽然可以提供微秒级分辨率和稀疏数据流，但仍缺乏低功耗的全集成处理解决方案。

**Method:** 该系统结合事件驱动的感知和神经形态处理，并采用了一种新的加速度量化的脉冲神经网络，优化了严格的内存和带宽限制，并通过系统的部署机制解决了现实差距。

**Result:** 在新发布的多用户数据集上验证系统，并展示了具有双神经形态设备的可穿戴原型，实现了双目瞳孔每秒100次跟踪，平均单眼功耗不足5毫瓦。

**Conclusion:** 该研究证明了端到端神经形态计算使得实现下一代高能效可穿戴系统中的实际、持续眼睛跟踪成为可能。
...
> The paper introduces Exo2EgoSyn, an adaptation of WAN 2.2, which enables Exocentric-to-Egocentric cross-view video synthesis using a framework with three key modules.

<details>
  <summary>Details</summary>

**Motivation:** To unlock Exocentric-to-Egocentric (Exo2Ego) cross-view video synthesis by adapting WAN 2.2.

**Method:** Our framework consists of three key modules: Ego-Exo View Alignment (EgoExo-Align), Multi-view Exocentric Video Conditioning (MultiExoCon), and Pose-Aware Latent Injection (PoseInj).

**Result:** Experiments on ExoEgo4D validate that Exo2EgoSyn significantly improves Ego2Exo synthesis, enabling high-fidelity ego view video generation from third-person observations without retraining from scratch.

**Conclusion:** The paper demonstrates that the proposed Exo2EgoSyn significantly enhances the cross-view video synthesis capabilities from exocentric to egocentric viewpoints, opening up new possibilities for scalable video generation with foundation models.
...
> 本文提出SFA框架，用于解决视频文本视觉问答的挑战，如尺度、方向和清晰度不同的文本，以及有效整合时间与语义上下文。SFA方法自适应地扫描视频帧，集中注意力于关键区域，并放大这些区域，最终在多个数据集上表现出比已有方法更优的效果。

<details>
  <summary>Details</summary>

**Motivation:** 受到人类回答问题过程的启发，以应对精确感知和理解场景文本的需求，这些文本在尺度、方向和清晰度上可能有所不同，并且模型需要有效地整合时间与语义上下文以生成正确答案。

**Method:** SFA, 一种无需训练的框架，也是首个专门为视频文本视觉问答设计的基于Video-LLM的方法。该方法通过自适应扫描视频帧，选择性地聚拢到关键区域，并放大这些区域，从而有效地引导Video-LLM的注意力关注到重要的线索，以便生成更加准确的答案。

**Result:** SFA 在多个公开的视频文本视觉问答数据集上达到了新的最佳性能，并且比以前的方法高出显著的幅度，证明了其有效性和普遍适用性。

**Conclusion:** SFA 方法无需训练的特点及其基于Video-LLM的核心理念，使它能自适应地解决视频文本视觉问答中的具体挑战，证明了其在该领域的有效性和优越的性能。
...
> GHR-VQA通过使用场景图和图神经网络改进了视频问题回答的能力，特别是在理解和推理视频中的人-物交互方面表现突出。

<details>
  <summary>Details</summary>

**Motivation:** 该方法旨在改进传统的基于像素的方法，通过显式地以人类为中心的结构，提高对视频内容的可解释性和理解深度。

**Method:** 我们提出了一种名为GHR-VQA的框架，该框架利用场景图捕捉视频序列中的人-物交互，并将每个帧表示为场景图，形成视频级别的图结构以实现跨帧推理。这些视频级别的图结构通过图神经网络转化为丰富的上下文感知嵌入，并与问题特征在一个分层网络中集成，以提高对视频内容的理解。

**Result:** 在Action Genome Question Answering数据集上，我们的方法在对象关系推理上比现有方法提高了7.3%的性能。

**Conclusion:** 结果表明，我们的方法能够更好地理解视频内容中的时空动态，尤其是在人与物之间的交互方面。
...
> A U-Net architecture with random masking augmentation for brain tumor MRI inpainting achieved top scores in the 2025 BraTS-Inpainting Challenge, surpassing previous years' winners.

<details>
  <summary>Details</summary>

**Motivation:** The goal is to improve the robustness of deep learning models in analyzing brain tumor MRIs by mitigating biases related to dataset composition and improving the synthesis of healthy tissue in scans.

**Method:** Our method centers around a U-Net architecture trained for inpainting tasks. It uses a random masking augmentation technique to generalize well and handle variations in brain tumor MRI data.

**Result:** Our submission achieved strong scores: SSIM of 0.919, PSNR of 26.932, and RMSE of 0.052 on the final online test set. This performance was superior to previous winners.

**Conclusion:** The paper concludes that its U-Net-based inpainting framework, enhanced by random masking, effectively synthesizes healthy tissue for improved quantitative analysis of brain tumor MRIs.
...
> OmniAlpha是一个新的统一多任务生成框架，用于处理RGBA图像。它使用了一种新颖的RoPE方法，并在新的AlphaLayers数据集上进行了联合训练，该数据集中包括1000个高质量的多层三元组。在各种任务上，OmniAlpha的表现优于专门化的基准模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的生成模型在RGB合成方面表现出色，但现实世界的应用需要RGBA操作。由于专门针对某个任务的模型缺乏通用性，而统一的多任务框架仅限于RGB领域，因此我们提出了OmniAlpha来填补这一关键空白。

**Method:** 提出OmniAlpha，这是一个用于序列到序列RGBA图像生成和编辑的统一多任务生成框架。其架构包括一个新颖的RoPE方法MSRoPE-BiL，支持扩散变压器(DiT)基础结构的双向扩展层轴，从而可以实现多输入和目标RGBA层的并发处理。

**Result:** OmniAlpha在21种不同任务上进行了广泛实验，相比于专业的且专门化的基准模型，该模型的表现明显更好。特别是在AIM-500数据集上的无掩模抠图任务中，SAD比基础模型减少了84.8%的相对误差，并在90%以上的人类偏好中胜出。

**Conclusion:** 研究证明，统一的多任务模型可以学习到RGBA的优越共享表示，为更强大的、感知层次的生成系统铺平了道路。
...
> Researchers propose CT-CIG to generate more natural and logically plausible camouflage images using a CRDM for text annotation, a FIRM for texture feature refinement, and Stable Diffusion with a lightweight controller.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this method is to improve the naturalness and logical plausibility of created camouflage images, addressing the deficiency of current methods which often fail to capture the logical relationship between camouflaged objects and backgrounds.

**Method:** CT-CIG, a Controllable Text-guided Camouflage Images Generation method, is proposed. It uses a Camouflage-Revealing Dialogue Mechanism (CRDM) for annotating camouflage datasets with text and a Frequency Interaction Refinement Module (FIRM) for better texture feature extraction, building on Stable Diffusion and incorporating a lightweight controller.

**Result:** Experiments using CLIPScore evaluation and assessments of camouflage effectiveness show that the generated images align well semantically and are photorealistically camouflaged.

**Conclusion:** The proposed method succeeds in producing more natural and logically consistent camouflage images through the use of advanced text guidance and texture refinement, suggesting a promising direction for future research in CIG.
...
> 研究团队使用一种深度学习方法，特别是通过微调视觉变换器（ViT）并在BraTS-Path 2025竞赛中取得第二名的成绩，为胶质母细胞瘤的自动化病理图像分析奠定了基础。

<details>
  <summary>Details</summary>

**Motivation:** 胶质母细胞瘤是一种侵袭性脑肿瘤，其分子和病理异质性使得诊断和患者分层变得复杂。传统的组织病理学评估仍然是标准，但深度学习提供了自动分析全幻灯片图像的有希望的解决方案。

**Method:** 作者开发了一种方法，该方法以BraTS-Path 2025挑战赛的官方训练数据集为基础，对预训练的视觉变换器（ViT）编码器进行了微调，添加了一种专用的分类头。

**Result:** 该研究针对胶质母细胞瘤的分子和病理异质性问题，利用深度学习方法，特别是对预训练的视觉变换器（ViT）进行微调，配合专用分类头，在BraTS-Path 2025竞赛中的在线验证集上取得了Matthews相关系数（MCC）0.7064和F1评分为0.7676的良好成绩。在最终测试集上，模型达到了MCC为0.6509和F1评分为0.5330的结果，为团队在比赛中赢得第二名。研究为基于ViT的组织病理学分析奠定了坚实的基础，并为未来的研究指明了方向，即缩小在未见过的数据上的性能差距。

**Conclusion:** 该研究为基于ViT的组织病理学分析奠定了坚实的基础，未来将致力于缩小在未见过的数据上的性能差距。
...
> The paper introduces V-Attack, a novel method for precise local semantic attacks on Large Vision-Language Models (LVLMs) by targeting value features in the transformer attention block, achieving a significant increase in attack success rate over existing methods.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this paper is to address the limitations of existing adversarial attack methods that struggle with controllable and precise manipulation of image semantics. The authors aim to provide a more effective approach to semantic control in LVLMs by leveraging value features, which retain high-entropy, disentangled local semantic information.

**Method:** Our method, V-Attack, targets value features in the transformer attention block of Large Vision-Language Models (LVLMs) to achieve precise local semantic manipulation. It introduces a Self-Value Enhancement module to refine the intrinsic semantic richness of value features and a Text-Guided Value Manipulation module that uses text prompts to guide the semantic attack from a source to a target concept.

**Result:** Extensive experiments across various LVLMs proved that V-Attack improves the attack success rate by an average of 36% compared to state-of-the-art methods, highlighting the vulnerabilities in visual-language understanding systems.

**Conclusion:** The conclusion of the paper is that V-Attack is effective in precisely manipulating local semantics of images through the use of value features in transformers, uncovering significant vulnerabilities in LVLMs and providing a higher success rate in adversarial attacks compared to current methods.
...
> 本文提出HistoSpeckle-Net，一种用于从多模光纤（MMF）散斑中重建结构性丰富的医学图像的深度学习架构。通过引入分布感知的训练策略和独特的三尺度特征细化模块，HistoSpeckle-Net在复杂的OrganAMNIST数据集上比基线模型（如U-Net和Pix2Pix）表现更佳，尤其是在训练样本有限和光纤扰动的情况下。

<details>
  <summary>Details</summary>

**Motivation:** 现有深度学习方法在处理复杂的多模光纤成像任务时面临数据依赖性问题。本文旨在开发一种能够在有限的数据集上表现出色，并且能够有效应对光纤弯曲影响的深度学习架构，以使其更适用于临床环境。

**Method:** HistoSpeckle-Net采用了基于直方图的互信息损失来提高模型的鲁棒性并减少对大量数据的依赖，并且通过引入三尺度特征细化模块，对应于多尺度结构相似性指数度量（SSIM）损失的计算。两者结合，提升了重建图像的结构保真度和统计一致性。

**Result:** 实验表明该模型在复杂数据集上的表现优于其他模型，尤其是在训练样本数量有限的情况下及各种光纤弯曲条件下，依然能更准确地重建医学图像。

**Conclusion:** 通过引入分布感知学习策略和三尺度特征细化模块，HistoSpeckle-Net提高了多模光纤成像的鲁棒性和结构保真度，尤其是在数据不足和光线扰动的条件下表现优异，使其更接近于实际临床应用环境。
...
> 为了应对现有方法在处理现实世界单目视频中乒乓球的精确3D运动时的挑战，该研究提出了一种两阶段的方法，通过使用新的2D监督数据集和物理正确的合成数据来提升模型，使其具有更高的鲁棒性和实用性。

<details>
  <summary>Details</summary>

**Motivation:** 目标是从标准单目视频中获得乒乓球的精确3D运动。现有的基于合成数据训练的方法在处理现实世界的噪声和不完美的乒乓球和桌面检测方面表现不佳。这主要是由于缺乏现实世界视频中的3D地面真实轨迹和旋转标注。

**Method:** 该研究提出了一种新颖的两阶段管道，将问题分为前端感知任务和后端2D到3D提升任务。前端组件使用新创建的TTHQ数据集提供的2D监督进行训练，而后端提升网络则专门使用物理正确的合成数据进行训练。此外，研究还对提升模型进行了重新设计，以适应真实世界的常见缺陷，如缺失检测和变化的帧率，并集成了球检测器和桌子关键点检测器。

**Result:** 尚无具体的实验结果展示。该方法被描述为一种实用、鲁棒且高性能的完整应用，用于3D乒乓球轨迹和旋转分析。

**Conclusion:** 该研究通过重新设计提升模型，整合球检测器和桌子关键点检测器，将一个初步的提升概念转化为一个实用、鲁棒和高性能的2D到3D提升系统，适用于3D乒乓球轨迹和旋转分析。
...
> 研究揭示了当前最先进的文本到图像生成模型在处理长提示时多样性下降的问题，并提出一种无训练方法PromptMoG来改善这个问题。

<details>
  <summary>Details</summary>

**Motivation:** 研究长提示在文本到图像生成中的影响，探讨如何平衡生成的保真度与多样性。

**Method:** 我们提出了一种无训练方法PromptMoG，该方法通过从嵌入空间的高斯混合中采样提示嵌入来提高多样性和保持语义。

**Result:** 在四个最先进的模型上进行实验，结果显示PromptMoG能够显著提高长提示生成的多样性，而不会导致语义偏离。

**Conclusion:** 实验表明，PromptMoG可以有效改善长提示生成的多样性，同时保持图像的语义不变。
...
> Zoo3D 是一种无需训练的 3D 物体检测框架，它通过图聚类和开放词表模块对物体进行检测和分类，在两项基准测试中取得了最先进的结果，尤其是零样本模式在无训练情况下表现优于现有的自监督方法。

<details>
  <summary>Details</summary>

**Motivation:** 挑战现有的依赖训练场景的 3D 物体检测方法，开发基于开放词汇表的无需训练的方法，以提升面对未见过对象的检测能力。

**Method:** Zoo3D 使用2D实例掩码的图聚类构建3D边界框，并使用开放词汇表模块通过最佳视图选择和视图一致性掩码生成进行语义标签分配，并且提供了无需训练的零样本模式和通过弱监督学习改进的模式。

**Result:** 在 ScanNet200 和 ARKitScenes 基准测试中，Zoo3D$_0$ 和 Zoo3D$_1$ 都达到了最先进的开放词汇表3D物体检测结果，特别是零样本模式的表现优异。

**Conclusion:** 无需训练的 3D 物体检测方法展示了其在真实世界应用中的强大适应性和优势，特别是在零样本情形下，无需任何训练即可实现高效的物体检测。
...
> 研究开发了一种使用卷积神经网络来检测和定位DaVinci Xi手术系统中摄像机激活状态的方法，能够在70,000帧的公开数据集上实现近乎完美的准确率和定位精度，为后续自动化手术数据分析提供基础。

<details>
  <summary>Details</summary>

**Motivation:** 腹腔镜手术中，机器人辅助微创手术依赖于腹腔镜视频作为唯一术中视觉反馈。识别摄像臂的激活状态提供了宝贵的元数据，如摄像机运动信息，这些信息能够支持下游的外科数据科学任务，包括工具跟踪、技能评估或摄像机控制自动化。

**Method:** 我们开发了一个基于ResNet18卷积神经网络的轻量级管道，用于自动识别DaVinci Xi UI中的摄像机位置及激活状态。模型在SurgToolLoc数据集的手动注释数据上进行了微调，并在包含超过70,000帧的三个公开数据集上进行了评估。

**Result:** 模型在摄像机的二元检测中达到了0.993到1.000的F1分数，并且在所有情况下都能正确定位摄像机，没有出现多摄像机检测错误。

**Conclusion:** 所提出的管道能够从外科视频中可靠地实时提取摄像机激活的元数据，从而为各种下游应用的自动化预处理和分析提供便利。所有代码、训练模型和注释都是公开的。
...
> Adv-GRPO, a new RL framework, utilizes an adversarial reward model trained alongside the generator with visual feedback from reference images, resulting in higher-quality images and better aesthetics, outperforming existing methods in human evaluations.

<details>
  <summary>Details</summary>

**Motivation:** To develop a more reliable reward function for RL in image generation that avoids reward hacking and leads to high-quality images with better visual aesthetics.

**Method:** A reliable reward function for reinforcement learning (RL) in image generation is explored, introducing Adv-GRPO, an RL framework with an adversarial reward model that is jointly trained with the generator using reference images as positive samples to avoid reward hacking and guide the generator via visual outputs.

**Result:** Adv-GRPO outperforms other approaches, achieving win rates of 70.0% and 72.4% in image quality and aesthetics, respectively, in human evaluations.

**Conclusion:** Adv-GRPO effectively improves the quality and aesthetics of generated images without suffering from reward hacking issues, demonstrating consistent gains across various metrics.
...
> 本文提出了一种新的协作蒸馏框架MBCD，它解决了权重平均技术在多模态领域泛化中遇到的问题，并且在实验中表现出优于现有方法的性能。

<details>
  <summary>Details</summary>

**Motivation:** 针对多模态领域泛化的挑战，即模态间优化速度不同导致权重平均方法在早期阶段会出现对速度快的模态过度拟合的问题，进而抑制慢但互补的模态的贡献，影响模态融合的效果。

**Method:** 提出了一种名为MBCD的统一协作蒸馏框架，该框架从学生模型中采用适应性模态dropout策略来缓解早期阶段对主导模态的偏倚，接着通过梯度一致性约束来协调单一模态分支与融合表示之间的学习信号，最后使用基于权重平均的教师模型跨模态蒸馏传播融合知识。

**Result:** {

**Conclusion:** 实验结果显示，MBCD方法可以在不同的未见领域中实现更高的精度和更强的鲁棒性。
...
> 论文引入了DiDiCM来增强图像分类在高不确定性情形（如输入图像受损或训练数据有限）下的性能，实验证明其在ImageNet上的精度优于传统分类器。

<details>
  <summary>Details</summary>

**Motivation:** 传统的分类方法通常直接从输入图像中训练模型以预测类标签，但在高不确定性情况下如输入图像受损或训练数据有限时，其性能可能不佳。

**Method:** 该论文提出了一种新的框架——Discrete Diffusion Classification Modeling (DiDiCM)，该框架利用扩散过程建模输入图像的条件类标签后验分布，以解决高不确定性条件下的图像分类任务。DiDiCM支持基于扩散的过程预测类概率或离散类标签，提供计算和内存权衡的灵活性。

**Result:** 通过一项全面的实证研究，DiDiCM在ImageNet数据集上展示了超过标准分类器的优越性能，在任务变得更加具有挑战性时，精度提高得更明显。

**Conclusion:** DiDiCM通过扩散过程改善了图像分类在高不确定性条件下的准确性，尤其在训练数据有限或输入图像受损的情况下表现优异。
...
> A semi-supervised deep reinforcement learning framework for industrial visual inspection anomaly detection is developed, significantly improving accuracy and anomaly localization over existing state-of-the-art methods on the MVTec AD dataset.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to address the challenges in anomaly detection in industrial visual inspection, such as the scarcity of defective samples and poor detection of subtle defects by existing unsupervised methods.

**Method:** We propose a semi-supervised deep reinforcement learning framework that integrates a neural batch sampler, an autoencoder, and a predictor. The RL-based sampler adaptively selects informative patches by balancing exploration and exploitation through a composite reward. The autoencoder generates loss profiles highlighting abnormal regions, while the predictor performs segmentation in the loss-profile space.

**Result:** Experiments on the MVTec AD dataset show that the proposed method achieves higher accuracy and better localization of subtle anomalies than recent state-of-the-art approaches with an average improvement of 0.15 in F1_max and 0.06 in AUC, with a maximum gain of 0.37 in F1_max in the best case.

**Conclusion:** The integration of a neural batch sampler, autoencoder, and predictor in a deep reinforcement learning framework enables effective detection of anomalies with limited labeled data and shows better performance in accuracy and anomaly localization compared to existing methods.
...
> The paper introduces VKnowU, a benchmark for evaluating MLLMs' understanding of visual knowledge, and VideoKnow+, a model that incorporates visual knowledge with reinforcement learning, improving performance by 3.7%.

<details>
  <summary>Details</summary>

**Motivation:** To address the gap in MLLMs' ability to understand visual knowledge, which encompasses the physical and social principles of the world.

**Method:** Developed VKnowU to evaluate MLLMs' performance on visual knowledge and created VKnowQA and VideoKnow+ to explicitly incorporate visual knowledge improvement.

**Result:** VideoKnow+ achieved a 3.7% improvement on VKnowU and consistent gains on other benchmarks.

**Conclusion:** Visual knowledge is essential for MLLMs to have a more comprehensive understanding of the physical and social world.
...
> ScenarioCLIP模型通过接受输入文本、关联关系和图像，以及突出显示关系的聚焦区域，来改进场景分析中的跨模态检索和细粒度视觉理解任务。该模型是在预先整理的数据集上进行预训练，并在其基础上针对特定下游任务进行微调。

<details>
  <summary>Details</summary>

**Motivation:** 尽管CLIP型基础模型在短描述检索或场景中的单个对象分类任务上有所发展，但它们并未显式建模物体间的关联性或复合结构。研究旨在填补这一空白，提高对复杂场景中多对象和行动关系的理解。

**Method:** ScenarioCLIP模型通过对预先整理的场景数据集进行预训练，并针对特定下游任务进行微调，该模型不仅能处理跨模态检索任务，还能实现细粒度的视觉理解。通过现有语言模型生成图像文本对扩展了原有多样化场景数据集。

**Result:** 通过广泛的基准测试，ScenarioCLIP在多种特定领域的任务上展示了强大的零样本和微调性能。

**Conclusion:** ScenarioCLIP的提出和发展标志着在处理复杂场景中的多对象和行动关系上取得重要进展，为未来的工作提供了有力的工具和方法。
...
> This paper introduces DAPointMamba, a novel method for DA PCC that uses State Space Models to improve adaptability and efficiency, offering better performance than state-of-the-art methods with less computational demand.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this work is to improve the adaptability between labeled source and unlabeled target domains in point cloud completion by overcoming the limitations of existing methods, such as limited receptive fields and high complexity.

**Method:** The paper proposes a novel framework, DAPointMamba, for Domain Adaptive Point Cloud Completion (DA PCC), addressing the current limitations of CNNs and vision Transformers with modules that use State Space Models (SSMs) to improve adaptability. It includes the Cross-Domain Patch-Level Scanning for local alignment, Cross-Domain Spatial SSM Alignment to strengthen spatial consistency, and Cross-Domain Channel SSM Alignment to address global semantic gaps.

**Result:** The proposed DAPointMamba outperforms state-of-the-art methods on both synthetic and real-world benchmarks, showing improved adaptability and reduced computational complexity.

**Conclusion:** DAPointMamba demonstrates strong adaptability and efficiency in DA PCC, providing a promising solution that can handle complex and diverse point cloud data effectively with reduced latency.
...
> SelfMOTR展示了MOTR样模型的隐藏检测能力，并利用自我生成的检测先验进行改进，从而在跟踪任务中取得良好性能。

<details>
  <summary>Details</summary>

**Motivation:** 论文旨在解决现有端到端跟踪方法中检测性能不佳及检测与关联之间的冲突问题，重点在于如何有效利用检测先验来改善这些问题。

**Method:** SelfMOTR是一种新颖的跟踪变压器，其核心在于从原动机模型（如MOTR）中挖掘隐藏的强大检测能力，并通过自我生成的检测先验进行改进。

**Result:** 该论文提出了一种新颖的跟踪变压器SelfMOTR，它依赖于自我生成的检测先验。作者展示了MOTR样模型隐藏的强大检测能力，并提供了一系列工具有效利用这些能力。实验结果表明，SelfMOTR在DanceTrack数据集上表现强劲，与最近端到端跟踪方法的最先进水平相当。

**Conclusion:** SelfMOTR通过揭示并利用MOTR模型中隐藏的检测优势，在DanceTrack数据集上表现出类端到端跟踪的顶级性能，展示了通过自我生成检测先验改善跟踪性能的可能性。
...
> 本文提出了一种无需训练且即插即用的迭代自我优化框架，以提高视频生成的物理一致性。

<details>
  <summary>Details</summary>

**Motivation:** 尽管视频生成的视觉质量有所提高，但现有模型在生成符合现实世界物理原理的视频方面仍面临挑战。

**Method:** 提出了一种基于大规模语言模型和视觉语言模型的迭代自我优化框架，该框架通过多模态思想链（MM-CoT）过程，根据物理不一致性反馈逐步优化提示，从而提高生成质量。

**Result:** 在PhyIQ基准上的实验结果表明，该方法将Physics-IQ得分从56.31提高到了62.38。

**Conclusion:** 该工作作为物理一致的视频生成的初步探索，为未来的研究提供了潜在的见解。
...
> The paper introduces Back To The Feature (BTTF), an optimization framework that produces temporally coherent, smooth motion trajectory, and physically plausible counterfactual videos, addressing the gap in the field of video classifier explanation methods.

<details>
  <summary>Details</summary>

**Motivation:** The generation of CFEs for video classifiers is underexplored, and existing CFE image-based methods can't generate temporally coherent, smooth and physically plausible video CFEs. This motivates the proposal of Back To The Feature (BTTF) method.

**Method:** Our method introduces two novel features: an optimization scheme to retrieve the initial latent noise conditioned by the first frame of the input video, and a two-stage optimization strategy to enable the search for counterfactual videos in the vicinity of the input video. For acceleration, a progressive optimization strategy is used.

**Result:** Extensive experiments on video datasets such as Shape-Moving (motion classification), MEAD (emotion classification), and NTU RGB+D (action classification) show that our BTTF effectively generates valid, visually similar and realistic counterfactual videos.

**Conclusion:** BTTF method can generate valid, physically plausible, temporally coherent, smooth motion trajectory, and realistic counterfactual videos, providing concrete insights into the classifier's decision-making mechanism.
...
> 本文提出了CrossEarth-Gate方法，通过建立遥感模块工具箱和Fisher信息引导的自适应选择机制，来解决遥感中多方面领域差距问题，实现了跨域遥感分割的前沿性能。

<details>
  <summary>Details</summary>

**Motivation:** 在遥感领域，现有的专用参数高效微调方法在大幅地球观测任务中往往表现不佳，无法完全解决数据中存在的多方面和不可预知的领域差距。本文旨在提出一种解决方案来克服这一局限性。

**Method:** 本文提出了CrossEarth-Gate方法，包括两个主要贡献：第一，建立了一个全面的遥感模块工具箱，包括空间、语义和频率模块以解决多方面领域差距的问题；第二，开发了一种基于Fisher信息的自适应选择机制，该机制根据各个模块对任务特定梯度流的贡献来动态激活关键模块。

**Result:** CrossEarth-Gate在遥感语义分割的16个跨域基准测试中展现了优秀的性能，达到了最先进的水平，证明了方法的有效性和广泛适用性。

**Conclusion:** 实验结果表明本文所提CrossEarth-Gate方法的效率和通用性，在16个遥感语义分割跨域基准测试中达到了最先进的性能。代码将被发布。
...
> TReFT方法通过直接使用预训练模型输出速度优化了RF模型的收敛性，实现了单步实时图像转换。

<details>
  <summary>Details</summary>

**Motivation:** 本论文的动机在于解决RF模型在图像到图像转换中依赖多步降噪的问题，实现单步实时推断，通过解决对抗训练中的收敛问题来提高模型效率。

**Method:** TReFT使用预训练DiT或UNet预测的速度作为输出，引入内存高效的隐含循环一致性损失及身份损失，并对架构进行简化，实现了对抗训练中的收敛问题解决与单步推断的结合。

**Result:** Rectified Flow (RF) 模型通过最优传输理论在高质图像和视频合成方面取得了进展，但在图像到图像的转换中仍依赖于耗时的多步降噪，阻碍了实时应用。虽然最近的对抗训练范式CycleGAN-Turbo在预训练扩散模型中实现了单步图像转换，但将其直接应用于RF模型会遇到严重的收敛问题。本论文中，我们分析了这些挑战并提出TReFT，一种新的方法来驯服RF模型实现单步图像转换。TReFT使用预训练DiT或UNet预测的速度作为输出，解决了对抗训练中的收敛问题，并实现单步推断。这一设计是基于一个新的观察：在降噪过程的末尾，预训练RF模型预测的速度趋近于从原点到最终清晰图像的向量，这一点通过理论分析进一步验证。当TReFT应用于大型预训练RF模型如SD3.5和FLUX时，我们引入了高效内存的隐含循环一致性损失和身份损失，并简化架构以加速推断。经过TReFT微调的预训练RF模型在多个图像转换数据集上实现了性能与最先进方法相当，同时支持实时推断。

**Conclusion:** 经过TReFT微调的预训练RF模型在多个数据集上实现高性能的同时支持实时推断，证明了TReFT方法的有效性。
...
> 提出了IrisNet，这是一种新元学习方法，用于增强红外小目标检测的动态适应性和鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于深度学习的编解码框架在处理红外小目标检测（IRSTD）时，由于其静态学习模式，难以适应不同场景（例如，昼夜变化，天空/海面/地面区域）的模式漂移问题，导致鲁棒性有限。而IrisNet设计旨在解决这些问题以提高检测性能和鲁棒性。

**Method:** 提出了一种名为IrisNet的新元学习框架，该框架能够根据不同输入的红外图像状态动态调整检测策略。通过图像到解码器的Transformer建立了红外图像特征与整个解码器参数之间的动态映射。具体来说，将参数化的解码器表示为保留层次结构层相关性的结构化2D张量，并通过自我注意机制在Transformer中建模层间依赖关系，通过交叉注意机制生成自适应解码模式。此外，通过整合高频成分来增强红外图像的感知能力，补充目标位置和场景边缘信息。

**Result:** 实验研究表明，IrisNet在NUDT-SIRST、NUAA-SIRST和IRSTD-1K数据集上实现了最先进的性能。

**Conclusion:** 实验结果验证了IrisNet的有效性，证明它能显著提高红外小目标检测的性能。
...
> 提出了一种改进的世界模型框架，能够更准确地预测危险情境，通过强化学习框架集成，实现了在复杂场景中减少安全违规行为。

<details>
  <summary>Details</summary>

**Motivation:** 针对强化学习在自动驾驶领域中存在的对世界模型的乐观偏见问题，提出了解决方案以提高自动驾驶的安全性和处理长尾事件的能力。

**Method:** 提出了一种框架，用于生成一个不偏不倚的世界模型，该模型通过反事实合成管道来系统生成潜在的碰撞和出道路事件，从而变得诚实对待危险情况。这种方法将模型从被动场景完成者转变为忠实于因果关系的验证预报者。

**Result:** 通过广泛的实验，包括在新的风险预见基准上的测试，证明了模型在预测故障方面显著优于基线模型，并且作为批评者，极大地减少了具有挑战性的仿真中的安全违规行为。

**Conclusion:** 教授模型预测危险是一个关键步骤，它可以创造出更加安全和智能的自动驾驶代理。这种方法有效提升了自动驾驶车辆在复杂环境中的安全性能。
...
> 本文设计了一个小型的PID卷积神经网络用来实时感知双目视觉目标的三维运动信息，通过使用模拟的随机移动球数据集进行训练和测试，达到了输入图像分辨率能表示的预测准确度上限，并分析了实验结果中的错误、不足及改进方向，讨论了高维卷积及PID信息在提升计算效率和实现记忆与注意力机制方面的潜在优势。

<details>
  <summary>Details</summary>

**Motivation:** 理解神经网络拟合非线性问题的能力，从PID控制的角度出发，探索神经网络在处理空间-时间感知任务上的潜力。

**Method:** 设计了一个包含17层、413K参数的小型PID卷积神经网络，采用特征复用方法通过连接和池化实现，同时分析了网络设计的一些参考原则。

**Result:** 利用模拟的随机移动球数据集进行了训练和测试，网络的预测准确度达到了输入图像分辨率的极限。

**Conclusion:** 讨论了高维卷积的优点以及PID信息在增强计算效率和在记忆与注意力机制中的潜在应用前景。同时指出了研究中存在的不足，并提出了未来可能的改进方向。
...
> A deep learning approach for homography estimation from a single image uses a ConvNeXt-based model and novel data augmentation, achieving superior results with fast inference.

<details>
  <summary>Details</summary>

**Motivation:** The task of estimating homography from a single image is important in practical applications such as retail shelf monitoring but is limited by data scarcity and complexity.

**Method:** Our model uses a ConvNeXt-based backbone for feature extraction and normalized coordinate regression to predict a 4-point parameterized homography matrix. A novel augmentation strategy of modeling and sampling synthetic homographies is introduced to overcome data scarcity and improve generalization.

**Result:** The proposed method achieves a mean corner error of 1.298 pixels on the test set, showing competitive performance against classical and deep learning approaches in both accuracy and speed.

**Conclusion:** This deep learning framework is a robust and efficient solution for single-view image rectification, with the added benefit of a publicly available dataset to facilitate further research.
...
> 研究提出了AMB3R，一种用于多任务3D视觉的前端模型，具备出色的密集3D重建性能，在常见基准测试中表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 这个研究的动机是提出一种能够处理多种3D视觉任务的模型，且不需针对特定任务进行微调或进行测试时优化。

**Method:** AMB3R采用多视图前馈模型进行密集的3D重建，使用稀疏但紧凑的体积场景表示来执行几何推理。

**Result:** 相比之前的基于点图模型，AMB3R在相机姿态、深度和尺度估计以及3D重建方面达到了最先进的性能，并在常见基准测试中超过了基于优化的SLAM和SfM方法。

**Conclusion:** AMB3R表明了其在密集3D重建上的优越性能，且无需针对任务进行特定优化，展示了其灵活性和鲁棒性。
...
> 本文提出了一种完全基于相机的三维重建和传感仿真方法，使用3D高斯溅射技术，结合视觉模型，实现了与激光雷达-相机融合类似的效果，而无需复杂的硬件和校准。

<details>
  <summary>Details</summary>

**Motivation:** 传统的激光雷达-相机融合方法需要复杂的校准，而在某些材料（如玻璃）上的表现仍然不尽如人意。本文的动机是开发一种仅使用相机来实现三维重建的方法，以此来克服传统方法中存在的问题，并提供与激光雷达-相机融合方法相当的传感器仿真精度，同时减少硬件复杂性和校准要求。

**Method:** 本文的方法是通过多视角图像进行3D高斯溅射重建，提取语义材质掩模，然后使用视觉模型将高斯表面转换为带有投影材质标签的网格表面，并分配物理基础材质属性。该流程完全依赖相机数据完成，不需要激光雷达的参与。

**Result:** 本文提出了一种仅使用相机的三维重建方法，通过多视角图像实现3D高斯溅射重建场景，提取语义材质掩模，并使用视觉模型将高斯表示转换成带有投影材质标签的网格表面，同时分配物理基础材质属性，以实现现代图形引擎和模拟器中的精确传感器仿真。通过这种方式，该方法结合了照片级重建和物理基础材质的分配，与激光雷达-相机融合相比，在减少硬件复杂性和校准要求的情况下，提供了相同的传感器仿真精度。我们利用来自配备测试车辆的内部数据集验证了这一仅基于相机的方法，其中激光雷达作为反射率验证的地面真值，与其他图像相似性度量一起使用。

**Conclusion:** 实验表明，通过使用配备有测试车辆的内部数据集，新方法在反射率和图像相似性方面表现良好，能够达到与激光雷达-相机融合方法相当的传感器仿真精度，并且简化了硬件复杂度和校准过程。
...
> 研究提出了一种三维环境下的虚拟人类视觉搜索模型，并构建了新的基准测试H* Bench，以评估对象和路径搜索性能。结果显示现有模型仍然有很大的改进空间。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉搜索方法局限于静态图像，忽略了实体与三维世界的交互。研究动机是开发能够绕过实际硬件限制，如同人类一样高效的自主视觉搜索代理。

**Method:** 本研究提出了一个基于三维环境的虚拟人类视觉搜索模型，通过旋转头部来搜索360°全景图像中的对象或路径。为了研究在复杂现实环境中的视觉搜索问题，研究者构建了一个新的基准H* Bench，涵盖了交通集散地、大型零售空间、城市街道和公共机构等场景，需要高级的空间视觉推理能力。

**Result:** 实验表明，顶级专有模型在对象和路径搜索上的成功率仅为约30%。使用后训练技术后的开源模型Qwen2.5-VL在对象搜索和路径搜索上的成功率提高了三倍以上，但路径搜索的成功率仍然较低，表明此任务的难度较大。

**Conclusion:** 研究结果表明，构建能够无缝集成入日常生活的多模态大语言模型代理还有巨大的挑战，但也为其未来发展指明了方向。
...
> 本文提出了GS-Checker，一种用于识别3DGS模型中篡改区域的技术，通过引入3D篡改属性、3D对比机制和循环优化策略实现这一目标。

<details>
  <summary>Details</summary>

**Motivation:** 尽管3D Gaussian Splatting编辑技术的进步简化了对3D场景的操控，但它们也引发了对潜在恶意操控3D内容的担忧。为避免这种情况，准确定位篡改区域变得尤为重要。

**Method:** 我们提出了GS-Checker，一种用于定位3DGS模型中篡改区域的新方法。该方法将3D篡改属性整合到3D高斯参数中，用以表明高斯是否被篡改。同时，设计了一种3D对比机制，通过比较3D高斯关键属性的相似性来寻找篡改线索。此外，引入了循环优化策略来细化3D篡改属性，使得篡改定位更加准确。

**Result:** 我们的方法不需要昂贵的3D标签进行监督，且实验证明，该方法可以有效地定位被篡改的3DGS区域。

**Conclusion:** 通过结合3D篡改属性、3D对比机制和循环优化策略，研究提供了一种有效的方法来识别3DGS模型中的篡改区域，而无需依赖于3D标签。
...
> BoxPromptIML框架通过融合轻量级模型设计与知识蒸馏技术，解决了图像操作定位中的标注成本与精细定位之间的矛盾，同时实现了高效和准确的定位性能。

<details>
  <summary>Details</summary>

**Motivation:** 解决图像操作定位（IML）中标注成本与细粒度定位精度之间的矛盾。现有全监督IML方法需要密集的像素级标注，局限了大规模数据集的应用和实际部署；而弱监督IML方法虽然通过图像级标签大降低了标注成本，但通常缺乏精准的空间定位。

**Method:** 提出了一种新的弱监督图像操作定位框架BoxPromptIML，采用了粗略区域标注策略，生成相对准确的操作蒙版，同时降低了标注成本。此外，设计了一个高效的轻量级学生模型，通过知识蒸馏从固定教师模型（基于Segment Anything Model，SAM）学习执行细粒度定位。特征融合模块通过双重引导策略增强，该策略模仿人类潜意识记忆机制，动态结合长期记忆与实时观察线索，以提高定位准确性和鲁棒性。

**Result:** BoxPromptIML在多种数据集上的测试结果表明，该方法不仅能实现精确的细粒度定位，而且优于或匹敌全监督方法，并且能够保持低标注成本和高效部署的特点。

**Conclusion:** 实验结果表明，BoxPromptIML在图像内分布和分布外数据集上都表现出色，与全监督模型相比具有竞争力，并保持了强大的泛化能力、低标注成本和部署效率。
...
> 本文介绍了一种自动方法VGGTFace，用于从未经处理的多视角图像中重构具有拓扑一致性的面部几何结构，方法结合了3D基础模型VGGT和Pixel3DMM，实验验证了其在准确性和泛化能力上的优越性。

<details>
  <summary>Details</summary>

**Motivation:** 尽管现有方法可以重构面部几何结构，但它们往往需要繁琐的人工干预，或在泛化能力或3D可变形模型的表达力方面存在限制。针对这些局限性，研究者提出了一种无需手动输入便可处理真实世界数据的自动化方法。

**Method:** VGGTFace方法通过结合3D基础模型VGGT和Pixel3DMM，实现从日常生活用户拍摄的多视角图像中自动重构具有拓扑一致性的面部几何结构。VGGT模型由于大规模训练具备了强大的泛化能力和表达能力，但缺乏拓扑信息，为此通过Pixel3DMM注入像素对齐的UV值来获取拓扑信息，将VGGT生成的像素对齐点云转换为具有拓扑特性的点云，最后通过专门设计的拓扑感知捆绑调整策略来融合点云，以达到高质量的几何重构。

**Result:** 实验结果表明，VGGTFace方法在基准测试中取得了最先进的效果，并且在日常生活数据上展示了优异的泛化能力。

**Conclusion:** VGGTFace成功解决了现有方法中存在的问题，实现了从多视角图像中自动重构具有拓扑一致性的高质量面部几何形状，并在多个方面超越了现有的方法。
...
> Introduces FREE, a framework that accelerates Diffusion Transformers (DiTs) up to $2.25 	imes$ with lossless sampling through feature-level autoregression and parallel verification, and uncertainty-guided relaxation.

<details>
  <summary>Details</summary>

**Motivation:** To address the limitation of speculative inference methods on DiTs by enhancing draft accuracy and to reduce high inference latency without loss of generation quality.

**Method:** Discovers the strong temporal consistency and rich semantic abstraction in the top-block features of DiTs, proposing the FREE framework for lossless acceleration through feature-level autoregression with parallel verification. Further introduces an uncertainty-guided relaxation strategy, FREE (relax), to dynamically adjust the acceptance probability based on uncertainty levels for improved acceleration while maintaining generation quality.

**Result:** Experiments on ImageNet-$512^2$ demonstrate that FREE accelerates DiTs by $1.86 	imes$ and FREE (relax) by $2.25 	imes$ while preserving high-fidelity generation quality.

**Conclusion:** The proposed FREE framework and its extension, FREE (relax), successfully accelerate Diffusion Transformers with theoretical and empirical evidence, showing promising results in reducing inference latency without sacrificing generation quality.
...
> 本文提出了MultiID，一种无需训练的方法来解决多ID定制的难题。通过ID解耦的交叉注意力机制和三种关键策略来改善生成质量和文本控制能力，实验结果证明MultiID在解决多ID定制挑战方面表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 多ID定制是一个有趣且备受关注的计算机视觉话题。该方法的目标是在保留各自身份的同时，生成一个将多个个体无缝整合的定制图像。相较于单ID定制，多ID定制更为复杂，主要面临两个挑战：1) 模型在推理时容易出现“粘贴”问题，降低了图像质量；2) 文本控制能力较差。我们的工作动机是解决这些挑战。

**Method:** 我们提出了MultiID来解决多ID定制的挑战，这是一种无需训练的方法。具体来说，我们采用了ID解耦的交叉注意力机制，将不同的ID嵌入到相应的图像区域，从而生成多ID的输出结果。为了提高生成的可控性，我们提出了三种关键策略：局部提示、基于深度的空间控制和扩展的自注意力机制，使生成结果更加符合文本提示和ID图像。

**Result:** 我们构建了名为IDBench的基准测试集，并通过广泛的定性和定量实验结果展示了MultiID在解决上述两个挑战上的有效性，其性能与基于训练的多ID定制方法相当或更好。

**Conclusion:** 研究结论指出，提出的MultiID方法能够在没有训练的情况下有效克服多ID定制中的“粘贴”问题和文本控制不佳的问题，它通过改进生成技术和使用一个精心构建的测试基准显示出对挑战的有效应对，性能与训练方法相比不相上下或更优。
...
> TBCM提出了一种新的时间反向一致性模型方法，通过直接从教师模型的生成轨迹中提取潜在表示并摒弃对外部训练数据的依赖，提高了生成模型的效率和简单性。实验结果表明TBCM在减少训练时间、节省GPU内存的同时保持了高质量的生成结果。

<details>
  <summary>Details</summary>

**Motivation:** 改进现有连续时间一致性蒸馏方法，减少对外部训练数据的依赖，提高效率和适用性。

**Method:** 提出Trajectory-Backward Consistency Model (TBCM)方法，不再需要变分编码和大规模数据集，能够直接从教师模型的生成轨迹中提取潜在表示。

**Result:** TBCM在MJHQ-30k数据集上进行了实验，一元步生成中的FID为6.52，CLIP分数为28.08，同时减少了40%的训练时间和大量的GPU内存使用。

**Conclusion:** 实验表明TBCM相比于现有方法能够提高生成模型的效率，同时保证质量不受损害。通过对连续时间一致性蒸馏中的扩散生成空间差异性的分析，为未来的研究提供了有价值的见解。
...
> 提出了 MajutsuCity 用于生成 3D 城市场景，能够提供结构一致性、风格多样性、细粒度控制和创造性灵活性。实验结果表明它的性能领先。

<details>
  <summary>Details</summary>

**Motivation:** 由于现有方法难以平衡基于文本生成提供的创造性灵活性和显式结构表示支持的对象级别可编辑性，因此引入了 MajutsuCity，旨在生成同时具有风格多样性、细粒度控制以及一定创造性的 3D 城市场景。

**Method:** MajutsuCity 作为一种自然语言驱动、且具有美学适应性的框架，用于合成结构一致且风格多样的 3D 城市场景。这个框架通过四阶段的流水线进行运作，并引入了 MajutsuAgent 交互式语言引导编辑代理，支持五种对象级别的操作。此外，构建了 MajutsuDataset 数据集，包含 2D 语义布局和高度图，多样化的 3D 建筑资产，以及精选的 PBR 材质和环境背景。同时，还开发了一套实用的评估指标，涵盖结构一致性、场景复杂度、材质保真度和光照气氛等关键维度。

**Result:** 广泛的实验表明，MajutsuCity 与 CityDreamer 相比降低了 83.7% 的布局 FID，与 CityCraft 相比降低了 20.1%。在 AQS 和 RDR 评分上，我们的方法排名第一，超过了现有的方法。

**Conclusion:** 这些结果验证了 MajutsuCity 在 3D 城市生成领域的几何精确性、风格适应性和语义控制能力方面达到了新的先进水平，并作为了该领域的新型基准模型。期待这个框架能够激发新的 3D 城市生成研究方向。
...
> StableTrack stabilizes the quality of tracking on low-frequency detections, achieving a significant HOTA improvement on MOT17-val and performing well on full-frequency detection benchmarks.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to address the problem of running the model under conditions of limited computing resources, which is due to the current approaches that mainly focus on tracking objects in each frame of a video stream.

**Method:** Our method introduces a new two-stage matching strategy to improve the cross-frame association between low-frequency detections and proposes a Bbox-Based Distance instead of the conventional Mahalanobis distance to match objects using the Re-ID model. It also integrates visual tracking into the Kalman Filter and the overall tracking pipeline.

**Result:** The method outperforms current state-of-the-art trackers, achieving an 11.6% HOTA improvement at 1 Hz on MOT17-val, while keeping up with the best approaches on the standard MOT17, MOT20, and DanceTrack benchmarks with full-frequency detections.

**Conclusion:** StableTrack introduces a method that improves the performance of multi-object tracking in the scenario of low-frequency detections, while remaining competitive on benchmarks with full-frequency detections.
...
> 块级联技术显著改善了视频生成过程中速度与质量之间的权衡问题，通过利用同步去噪和并行计算，实现了更高的推理速度，同时保持生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 视频块因果生成面临着速度与质量之间的显著权衡：小规模的1.3B模型只能达到16 FPS，而大规模的14B模型则只有4.5 FPS，这使得用户必须在响应性和质量之间做出选择。

**Method:** Structure

**Result:** {"motivation":"视频块因果生成面临着速度与质量之间的显著权衡：小规模的1.3B模型只能达到16 FPS，而大规模的14B模型则只有4.5 FPS，这使得用户必须在响应性和质量之间做出选择。","method":"块级级联技术免训练并行化显著减缓了这一权衡。该技术的关键见解在于，未来的视频块不需要完全去噪的当前块就可以开始生成。通过从之前的视频块中获得部分去噪的上下文开始生成，将顺序管道转换为并行级联，从而实现多块同时去噪。使用5个GPU并利用时间上的并行化，可以实现几乎所有模型规模的加速。","result":"通过使用块级联技术，1.3B模型的加速从16 FPS提升到30 FPS，14B模型的加速从4.5 FPS提升到12.5 FPS。同时，该技术还消除了在上下文切换期间交互生成的KV缓存重新加载的开销（约200ms）。","conclusion":"广泛的评估证实了切换到块级联管道进行推理时在生成质量上的无明显损失。"}

**Conclusion:** 广泛的评估证实了切换到块级联管道进行推理时在生成质量上的无明显损失。
...
> BRIC is a TTA framework that combines diffusion models and reinforcement learning to generate realistic long-term human motion by correcting execution discrepancies.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind BRIC is to resolve execution discrepancies between diffusion models and physics controllers for generating more realistic and stable human motion over extended periods.

**Method:** We propose BRIC, a framework that combines diffusion-based kinematic motion planners with reinforcement learning-based physics controllers to enable long-term human motion generation. It dynamically adapts the physics controller to correct for noisy motion plans and introduces a lightweight guidance mechanism for the diffusion model.

**Result:** BRIC ensures consistent and physically plausible long-term executions across various environments and tasks, achieving state-of-the-art performance in motion composition, obstacle avoidance, and human-scene interaction.

**Conclusion:** BRIC demonstrates its effectiveness across different long-term tasks, showcasing its ability to produce state-of-the-art performance in generating physically plausible human motion in diverse environments.
...
> 提出了一种名为OC-VTP的新方法，可以直接并有保障地剪枝视觉标记，以提高视觉语言模型的推理效率，同时保持高精度，无需额外的模型微调。

<details>
  <summary>Details</summary>

**Motivation:** 在视觉语言模型中，视觉标记相较于语言标记数量多但信息分散，消耗过多计算资源。虽然已经有一些研究专注于剪枝冗余视觉标记以提高模型推理效率，但这些方法都是间接的，且效果无法保证。

**Method:** 我们提出了一种称为OC-VTP的方法，它通过预先训练一个轻量级的小型基于对象的视觉标记剪枝器来直接且有保证地选取最具代表性的视觉标记。这种方法能够插入到现有的视觉语言模型中使用，而无需对任何模型进行微调。

**Result:** 我们的方法无论视觉剪枝比例如何，都能够帮助主流视觉语言模型保持最高的推理准确性。此外，剪枝结果还显示出了有趣的可解释性。

**Conclusion:** OC-VTP通过轻量级的预训练确保能够选取最有代表性的视觉标记，不需对模型进行任何数据集上的微调，可以在不降低模型准确性的前提下提高模型的推理效率。
...
> 本文提出了一种新型研究问题HHOIs，即人-人-物互动，并开发了一个新的数据集和合成数据的方法，并引入了一个统一生成框架来合成具备文本描述条件的现实的HHOIs。

<details>
  <summary>Details</summary>

**Motivation:** 为了使机器能够理解复杂的、上下文依赖的行为模式，需要在考虑周围场景的情况下对多人进行建模，从而解决一个新问题：建模两人围绕共同物体的交互。

**Method:** HHOIs 的生成通过结合文本到 HOI 和文本到 HHI 的模型来实现，这两个模型是使用得分扩散模型训练的，最终整合为一个统一的生成框架，通过高级采样过程实现HHOI的合成，支持多个人物的交互场景。

**Result:** 实验结果显示，该方法在生成基于文本描述的HHOIs方面优于仅关注单人HOI的方法，可以生成真实且多样化的场景。

**Conclusion:** 该研究旨在通过引入HHOIs的概念和相应的生成框架来改进交互场景的理解和生成，以满足多人体互动的复杂需求。
...
> ZoomSearch: 无需训练、即插即用的超超高分辨率遥感图像视觉问答解耦方法，通过增强定位与重组拼图策略提升准确性和高效性。

<details>
  <summary>Details</summary>

**Motivation:** 随着卫星星座、传感器技术及成像流程的进步，超超高分辨率遥感图像越来越常见。然而，现有的遥感基础模型无法很好地处理这些输入，因此如何指导模型在预测前关注重要的地方变得至关重要。

**Method:** ZoomSearch 是一个无需训练、即插即用的流水线，将“在哪里看”与“如何回答”解耦。它结合了自适应多分支缩放搜索（对图像块执行层次搜索，以定位与查询相关的区域）和布局感知的拼图重组（将选定的拼图块重新组织成紧凑且忠实于布局的画布）。

**Result:** 在Ultra-HR RS-VQA基准测试MME-RealWorld-RS和LRS-VQA上，ZoomSearch相对于现有模型（包括强大的通用基础模型、遥感基础模型、Ultra-HR RS-VQA方法以及搜索式VQA方法）显著提高了准确性，特别是在LLaVA-ov上提升了26.3%的准确性(LRS-VQA)和114.8%的准确性(MME-RealWorld-RS)，同时提高了推理效率，比先前的搜索式方法快20%~44%。

**Conclusion:** ZoomSearch通过结合自适应多分支搜索与拼图重组提供了一种有效的方法，用于处理超超高分辨率遥感图像中的视觉问答任务，并且在多个实验中证明了其高效性和优越性。
...
> STARFlow-V is a video generation model based on normalizing flows that showcases improved causal prediction and robustness compared to diffusion models, with applications in various generative tasks like text-to-video, image-to-video, and video-to-video generation.

<details>
  <summary>Details</summary>

**Motivation:** To explore normalizing flows (NFs) for video generation, which is an area dominated by diffusion models due to the high spatiotemporal complexity and computational costs associated.

**Method:** Developed STARFlow-V, a NFs-based video generator using a global-local architecture in the spatiotemporal latent space, flow-score matching for consistency, and a video-aware Jacobi iteration scheme to maintain causal error accumulation control and sampling efficiency.

**Result:** STARFlow-V demonstrates competitive visual fidelity and temporal consistency compared to diffusion-based models, supporting multiple generation tasks natively.

**Conclusion:** The study confirms the potential of normalizing flows for high-quality autoregressive video generation, suggesting a promising new direction for modeling complex spatiotemporal data.
...
> 本文介绍了一个轻量级的舞蹈风格分类框架，利用基于Laban运动分析的时间空间描述符和快速傅里叶变换处理视频中的姿态数据，能够有效地识别和区分多种舞蹈风格。

<details>
  <summary>Details</summary>

**Motivation:** 舞蹈是人类文化的重要组成部分，能够传递情感和讲故事。根据运动数据识别和区分舞蹈风格对于人体动作识别是一个复杂的问题，因为各种舞蹈风格共享相似的姿态、手势和时间运动模式。因此，研究一个轻量级的，能够有效捕获并表现舞蹈特征的方法是具重要意义的。

**Method:** 本文提出了一个轻量级的框架，依据视频中提取的姿态估计数据来分类舞蹈风格。采用基于La班运动分析的时间-空间描述符捕捉上身局部关节的动力，包括速度、加速度和角运动。为了进一步诠释运动的节奏和周期特征，作者引入了快速傅里叶变换特征，捕捉频率域中的运动模式。

**Result:** 本文提出了一种轻量级的框架，用于根据视频中提取的姿态估计数据来分类舞蹈风格。通过引入基于La班运动分析的时间-空间描述符，并结合快速傅里叶变换特征以捕捉运动的节奏性和周期性，本文构建了能够有效区分舞蹈风格的结构化空间协调表示方法。该方法能够在避免使用复杂模型结构的情况下实现稳健的分类，并证明可解释的动作表示可以有效地捕获风格上的细微差别。

**Conclusion:** 本研究提出的轻量级框架能够在不过度依赖复杂模型结构的情况下实现舞蹈风格的稳健分类。这表明，通过使用可解释的动作表示，能够有效捕捉舞蹈风格之间的细微差异。
...
> The research benchmarks three independent sensing modules for visual and auditory perception tasks with high accuracies, suggesting feasibility for real-time, multimodal integration in assistive devices.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to develop comprehensive assistive technologies by seamlessly integrating visual and auditory perception through a modular architecture.

**Method:** The method involves proposing and benchmarking three independent sensing modules: a CNN for eye state detection, a deep CNN for facial expression recognition, and an LSTM network for voice-based speaker identification.

**Result:** The models achieved accuracies of 93.0%, 97.8%, and 96.89% respectively using specific datasets.

**Conclusion:** The conclusion is that lightweight, domain-specific models can achieve high fidelity on discrete tasks, providing a foundation for future multimodal integration in real-time assistive devices.
...
> 该研究提出了一种新的基于物理学的损失函数（PIL），它通过模拟弹性过程来改进连续血管边界模型的准确性和鲁棒性。PIL在几个不同的分割架构上进行了测试，并表明它在提高血管分割性能方面优于传统方法。

<details>
  <summary>Details</summary>

**Motivation:** 传统的损失函数依赖于像素级别的重叠，忽视了血管边界在几何和物理上的连贯性，导致碎片化或不稳定血管预测的问题。

**Method:** 提出了一种基于物理学的损失函数（Physics-Informed Loss, PIL），该方法借鉴材料物理中的位错理论，将预测边界与真实边界之间的相互作用建模为一个弹性过程，引入了基于物理的正则项，以确保轮廓进化平滑且结构一致，从而更好地捕捉精细的血管几何形状。PIL被集成到U-Net、U-Net++、SegFormer和MedFormer等分割架构中，并在一个公开基准数据集上进行了评估。

**Result:** 该方法在DIAS和DSCA两个公开基准上进行了评估，结果表明该方法在多个评价指标上均优于传统方法。

**Conclusion:** 实验结果表明，PIL在灵敏度、F1分数和边界一致性方面均优于传统的损失函数，如交叉熵、Dice、主动轮廓和表面损失。这表明在深度神经网络中加入基于物理的边界交互能够改善动态血管造影成像的血管分割的精确性和鲁棒性。
