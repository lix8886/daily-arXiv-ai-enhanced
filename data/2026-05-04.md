...
> 研究发现仅占数据量0.3%的50个样本子集就能高效评估大音频模型，相比完整基准测试可大幅降低成本，且通过回归模型训练后的子集甚至能超越完整基准的性能预测能力。

<details>
  <summary>Details</summary>

**Motivation:** 大音频模型(LAMs)快速发展需要有效的模型评估方法，但全面基准测试成本高昂，亟需寻找既能保证评估可靠性又能降低成本的解决方案。

**Method:** 分析了10种子集选择方法，在18个音频模型的40个任务上进行评估，收集776个人类偏好评分，训练回归模型预测用户满意度，开发HUMANS基准测试。

**Result:** 50个样本的子集与完整基准测试达到0.93皮尔逊相关性，两者的用户满意度预测均为0.85相关性，而基于精选子集训练的回归模型达到0.98相关性，优于完整基准。

**Conclusion:** 精心设计的子集在回归建模中能够超越完整基准的预测能力，证明了质量胜于数量，开源的HUMANS基准可作为高效的LAM评估代理。
...
> NorBERTo是基于ModernBERT架构的现代葡萄牙语编码器模型，使用3310亿Tokens的新语料库Aurora-PT训练，在多项葡萄牙语NLP任务上达到业界领先水平。

<details>
  <summary>Details</summary>

**Motivation:** 葡萄牙语NLP发展缺乏高质量大规模语料库和现代编码器模型，现有的BERTimbau和Albertina等模型在架构和性能上需要更新升级。

**Method:** 构建Aurora-PT语料库（3310亿GPT-2 Tokens），基于ModernBERT架构训练NorBERTo模型，采用长上下文支持和高效注意力机制，在ASSIN 2和PLUE等标准数据集上进行系统性评估。

**Result:** NorBERTo-large在PLUE数据集上表现最佳（MRPC F1=0.9191，RTE准确率=0.7689），在ASSIN 2上获得最高的文本蕴含F1(~0.904)。Aurora-PT是当前最大的开源葡萄牙语单语语料库。

**Conclusion:** NorBERTo提供了一个适合实际部署的现代中等规模编码器，易于微调、服务高效，可作为检索增强生成等下游葡萄牙语NLP系统的骨干模型，显著推进了葡萄牙语NLP技术的发展。
...
> 研究发现大型语言模型在神经多样化(ND)语境下会调整输出结构，但仅凭身份声明无法减少有害倾向，需要明确指令才能有效改善。

<details>
  <summary>Details</summary>

**Motivation:** 研究旨在评估前沿对话式大型语言模型是否能够根据神经多样化(Neurodivergent, ND)的系统提示上下文调整其输出，并描述这种调整的性质，以促进LLM对神经多样群体的包容性和适应性。

**Method:** 提出NDBench基准测试框架，包含576个输出样本，涉及2个前沿模型、3种系统提示类型（基线、ND身份声明、带明确调整指令的ND身份声明）、4个典型ND档案、24个涵盖4类主题的提示（其中包含对抗性掩饰策略）。

**Result:** LLM在ND语境下表现出显著适应性：输出更长更结构化（令牌数更多、标题更多、步骤更详尽）；调整主要体现在结构层面；仅身份声明无法抑制有害倾向（掩饰强化仅减少4-6%），明确指令可减少36-44%；可靠性分析显示6个维度中仅2个（掩饰强化、验证质量）满足评分者间一致性标准。

**Conclusion:** NDBench为审核未来LLM对神经多样意识的适应性提供了可复现框架；研究表明需要明确的系统性指令而非仅身份认知来改善LLM对神经多样群体的输出质量。
...
> 本文推出了首个越南法律领域的大规模自然语言推理数据集ViLegalNLI，包含42,012条标注对，并提出半自动生成框架，为越南法律推理建立了基础基准。

<details>
  <summary>Details</summary>

**Motivation:** 缺乏专门针对越南法律领域的自然语言推理数据集，限制了法律AI系统的发展。法律文本具有结构化逻辑、条件从句和领域术语等特征，需要专门的推理基准。

**Method:** 提出半自动数据生成框架，结合大语言模型进行受控假设生成和系统质量验证，包含artifact缓解策略和跨模型验证，确保法律一致性和标注可靠性。

**Result:** 构建了包含42,012条前提-假设对的数据集，涵盖多个法律领域。实验表明少样本LLM配置表现最佳，假设长度、词汇重叠和推理复杂度显著影响性能，跨域评估揭示了法律推理泛化的挑战。

**Conclusion:** ViLegalNLI为越南法律NLP研究建立了基础基准，支持法律推理、法规文本理解和可靠AI法律分析系统的开发，数据集已公开发布。
...
> 该研究提出了ArabCulture-Dialogue数据集，涵盖13个阿拉伯国家的现代标准语和方言对话，用于评估大语言模型在阿拉伯文化推理方面的表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有阿拉伯语基准测试主要关注现代标准阿拉伯语的短文本，忽略了对话中自然产生的文化细微差异和方言语境，导致评估LLM文化推理能力存在显著空白。

**Method:** 构建了包含13个阿拉伯国家的ArabCulture-Dialogue数据集，涵盖现代标准语和各国方言，涉及12个日常生活话题和54个细分子话题。基于该数据集设计了三个基准任务：多项选择文化推理、现代标准语与方言间的机器翻译、方言引导生成。

**Result:** 实验表明，现代标准语和阿拉伯方言之间 still存在性能差距，模型在所有三个任务的方言设置中都表现较差，相比现代标准语设置。

**Conclusion:** 该研究填补了阿拉伯语文化对话数据集的空白，揭示了LLM在处理阿拉伯方言时的不足，为改进模型的文化推理能力提供了重要的评估基准。
...
> 研究发现，在单口喜剧表演中，时间结构比语义内容更影响观众的快乐感受，高峰语义违反前的停顿长度是成功表演的关键。

<details>
  <summary>Details</summary>

**Motivation:** 传统幽默理论强调语义不一致性，但忽视了时间动态因素，尽管喜剧演员认为'时机就是一切'。时间结构对幽默欣赏的贡献程度及其与语义内容的相互作用尚不清楚。

**Method:** 提出双重预测违反(DPV)框架，通过分析828场专业中文单口喜剧表演，研究时间特征与语义不一致性对观众欣赏的预测作用。

**Result:** 时间特征在预测观众欣赏方面显著优于语义不一致性；高峰语义违反比平均不一致更重要；高惊讶笑点前停顿系统性地延长，这是区分成功与失败表演的策略性耦合。

**Conclusion:** 幽默应被重新定义为时间脚手架式的，时机与语义内容进行策略性协调而非独立运作。DPV框架连接了幽默理论与预测处理，证明了时间结构在自然幽默欣赏中的核心作用。
...
> RSAT方法通过逐步推理训练小型语言模型生成带单元格级引用的表格问题回答，显著提升了回答的忠实度。

<details>
  <summary>Details</summary>

**Motivation:** 用户在语言模型回答表格问题时无法验证哪些单元格支撑了推理过程，缺乏可解释性和可信度。

**Method:** 采用两阶段训练：Phase 1通过监督微调学习结构化JSON输出格式；Phase 2使用GRPO优化组合奖励函数，重点优化基于NLI的忠实度，并结合引用有效性和简洁性。

**Result:** 在Qwen 2.5和Llama 3两个系列的六种模型上，RSAT使忠实度提升3.7倍，引用有效性达到0.992；后期归因方法格式成功率低于13%，证明归因必须集成到推理中；消融实验表明忠实度奖励至关重要。

**Conclusion:** RSAT成功实现了表格问答的细粒度可解释性，证明将引用机制直接集成到训练过程中的必要性，为小型语言模型的可信推理提供了有效解决方案。
...
> 本文提出了一种混合置信度框架，结合LLM模型置信度信号和数据集衍生的不确定性估计，用于提高自动简答题评分的置信度估计可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 尽管基于LLM的自动简答题评分表现出色，但其评分仍不完美，需要可靠的置信度估计来支持安全有效的人机协作教育决策。

**Method:** 比较三种模型置信度估计策略（口头化、潜在特征、一致性），并提出了混合框架，将模型置信度与通过聚类学生响应量化的数据集衍生偶然不确定性相结合。

**Result:** 混合置信度措施相比单源方法能产生更可靠的置信度估计，并提高了选择性评分性能。

**Conclusion:** 这项工作推进了基于置信度的LLM评分技术，为人在回路评估系统提供了更可信的AI辅助教育评估支持。
...
> 研究发现大型语言模型在不完全信息游戏决策中存在两个关键缺陷：观察-信念差距（内部信念比口头报告更准确但易碎）和信念-行动差距（内部信念到行动的转换效果较差）。

<details>
  <summary>Details</summary>

**Motivation:** LLM被越来越多地用于谈判和政策制定等不完全信息下的战略决策，但它们在这些任务中会以难以理解的方式失败，需要深入理解其失败机制。

**Method:** 通过对Llama 3.1、Qwen3和gpt-oss等开源模型进行实验，分析LLM在不完全信息游戏中的内部决策机制。

**Result:** 发现了观察-信念差距（内部信念更准确但脆弱，存在多步推理退化、首尾近因效应偏差和贝叶斯一致性漂移）和信念-行动差距（内部信念比外部化信念对行动影响更弱，且都不能稳定获得更高收益）。

**Conclusion:** LLM的内部过程分析揭示了其系统性漏洞，在战略领域部署LLM前需要强大的防护措施，需要谨慎对待。
...
> 该研究开发了一个评估AI伴侣应用安全性的端到端框架，通过构建高风险人群画像进行模拟对话，发现Replika应用存在情感表达狭隘和传播有害内容的安全隐患。

<details>
  <summary>Details</summary>

**Motivation:** 现有AI伴侣应用的安全性评估主要依赖用户自述数据或访谈，无法深入分析实时交互动态，亟需建立一个可控的模拟评估框架来系统识别安全风险。

**Method:** 开发了包含四个核心组件的框架：经过临床和心理学验证的画像构建、画像特定的场景生成、带对话保真度优化模块的多轮模拟交互、以及伤害评估。针对Replika应用构建了9个高风险人群（抑郁、焦虑、PTSD、饮食失调、incel等）画像，收集25个高风险场景下的1,674组对话。

**Result:** Replika表现出狭隘的情感范围（以好奇和关怀为主），频繁反映或正常化自残、饮食失调、暴力幻想等不安全内容，存在安全隐患。

**Conclusion:** 受控的画像模拟可作为评估AI伴侣安全风险的可扩展测试平台，为AI安全性研究提供了新的评估方法。
...
> 研究提出基于项目反应理论(IRT)的评估框架，用于分析大语言模型在自动简答题评分中不同难度学生回答的表现差异，揭示了传统聚合指标无法发现的评估鲁棒性问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LLM自动简答题评分主要使用宏F1和Cohen's kappa等聚合指标，但不能充分展示评分性能在不同难度学生回答上的变化情况，缺乏对评分器能力和回答难度的深入分析。

**Method:** 引入基于项目反应理论的评估框架，将评分正确性建模为潜在评分器能力和回答评分难度的函数，进行响应级别分析，应用于17个开源LLM在SciEntsBank和Beetle基准测试上。

**Result:** 研究发现即使整体性能相似的模型，在回答难度增加时评分准确率下降幅度存在显著差异；困难回答的错误过度集中在'partially_correct_incomplete'标签上；较高难度与参考答案语义对齐较弱、矛盾信号更强、嵌入空间语义隔离度更大相关。

**Conclusion:** 项目反应理论为评估基于LLM的自动简答题评分提供了超越聚合性能指标的有用框架，能够揭示评分器在不同难度条件下的行为模式。
...
> 研究发现Mamba模型的循环状态无法直接提取出有效的句子表示，存在严重的各向异性和表示坍塌问题。

<details>
  <summary>Details</summary>

**Motivation:** 基于Mamba模型的循环状态h_t本质上是所有已见token的压缩摘要，研究者假设可以通过固定补丁边界提取token级输出获得免费的语义句子摘要，无需池化头、微调或[CLS]标记。

**Method:** 在严格的冻结特征探查协议下，使用预训练的Mamba-130M作为骨干网络，在五个基准数据集(SST-2, CoLA, MRPC, STS-B, IMDb)上比较四种提取冻结句子表示的策略，使用多个随机种子进行验证。

**Result:** 结果不支持原始假设：补丁边界读取不如简单的平均池化性能稳定。发现了两个结构性问题：严重各向异性(平均余弦相似度0.9999)和原始最终SSM状态的表示坍塌(CoLA数据集MCC=0.000)。

**Conclusion:** Mamba模型的循环状态无法直接生成高质量的句子表示，需要改进方法。研究者提出了正交注入约束新信息的修改循环机制来解决问题。
...
> CA-ThinkFlow是一个参数高效的RAG框架，专门针对印度特许会计任务，使用14B量化模型，性能达到GPT-4o等大模型的68.75%

<details>
  <summary>Details</summary>

**Motivation:** 现有大型语言模型在复杂的、特定司法管辖区的任务（如印度特许会计）中可靠性有限，难以处理多步数值计算，需要法律法规知识，且在资源受限环境中扩展成本高昂

**Method:** 采用参数高效的检索增强生成框架，结合14B位4比特量化的DeepSeek-R1推理模型和布局感知的Docling提取系统，使用基础RAG方法自动添加检索信息，依赖模型内置的CoT功能

**Result:** 在多级CA-Ben基准测试中，其Scholastic Reliability Coefficient达到GPT-4o和Claude 3.5 Sonnet的68.75%，展现了高效率和参数处理能力

**Conclusion:** 框架在效率和参数处理方面表现优异，但在处理税务等复杂法规文本的推理能力仍存在不足
...
> 现有LLM白盒OOD检测方法被序列长度混淆，作者提出双路径框架解混淆：嵌入路径检测主题偏移，处理轨迹路径检测隐蔽意图，性能达0.721 AUROC。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM的OOD检测方法在长度匹配评估下效果接近随机猜测，存在严重的序列长度混淆问题，需要寻找真正有效的OOD信号。

**Method:** 提出双路径框架：1)嵌入路径捕获文本主题信息；2)处理轨迹路径分析隐藏状态跨层演化。通过k-NN和轨迹评分的组合，分别处理不同类型的OOD输入。

**Result:** 在长度匹配评估下，传统方法性能从0.759降至0.389，而新方法达到0.721平均AUROC，越狱检测达0.850。电路归因分析证实对抗任务更多依赖注意力电路。

**Conclusion:** OOD检测需要区分词汇透明性：嵌入方法适合词汇显性偏移，轨迹方法适合隐蔽意图。处理轨迹能从接近随机的嵌入中构建真正的OOD信号。
...
> 提出一种神经符号聚合框架，通过MaxSAT解决自然语言判断聚合的逻辑一致性问题，在Reddit道德争议数据上取得86%人工评估一致率。

<details>
  <summary>Details</summary>

**Motivation:** 传统聚合方法(如多数投票)在高冲突领域无法产生逻辑一致的结果，将不同意见视为噪声，无法有效处理大规模道德争议。

**Method:** 构建神经符号聚合管道：使用语言模型将非结构化解释映射为可解释逻辑谓词和置信度权重，然后将这些组件编码为Z3求解器中的软约束，将聚合问题转化为寻求最大一致性的优化任务。

**Result:** 在Reddit r/AmItheAsshole论坛的道德争议案例中，系统生成逻辑一致的判决，62%的时间与基于流行度的标签不同，获得86%的独立人工评估者一致率。

**Conclusion:** 研究证明了将神经语义提取与形式求解器相结合的有效性，能够在噪声人类推理的聚合中强制执行逻辑合理性和可解释性。
...
> 本研究提出了一种结合定量性能分析和LLM评估的系统方法，用于识别和描述在线学习环境中学生的错误概念，通过分析9个课程周期的3802名学生数据，证实了该方法的可行性和实用性。

<details>
  <summary>Details</summary>

**Motivation:** 在在线学习环境中，如何系统性地识别和理解学生的错误概念是一个重要挑战。传统的性能分析只能发现学生表现不佳的领域，但无法深入理解背后的具体错误认知。随着大规模在线课程的普及，需要一种可扩展的方法来深入了解学生的困难，以便提供更有针对性的教学干预。

**Method:** 采用两阶段方法论：第一阶段通过测验级别的性能指标识别具有挑战性的核心主题；第二阶段利用大语言模型(LLM)分析三个数据源（测验题目内容、学生答题模式、讲座转录文本）来描述高优先级领域中的潜在错误概念。研究者分析了40-50个主题导向测验中学生的首次尝试表现。

**Result:** 研究发现了一些始终具有挑战性的主题，这些主题也是课程目标的核心。LLM识别的错误概念质量被学科专家评为优秀。教师访谈显示，数据驱动的主题识别方法具有价值性，并验证了他们的课堂观察。该方法揭示了仅从性能数据本身无法获得的关于学生错误概念的可行见解。

**Conclusion:** 这种结合定量分析和LLM评估的方法论为在使用测验的学习环境中描述学生困难提供了可扩展的解决方案。研究结果展示了在未来课程迭代中进行有针对性且潜在个性化干预的潜力，为通过后续测验表现衡量干预效果提供了明确的路径。
...
> 提出结构感知表格分块框架STC，通过构建行树表示和键值块编码，在保留表格结构的同时显著提升检索性能和效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的RAG分块策略主要针对非结构化文本设计，无法有效处理CSV、Excel等表格数据的结构特征，导致检索性能低下和资源浪费。

**Method:** STC框架构建分层行树表示，将每行编码为键值块，执行受令牌限制的结构边界对齐分割，并应用无重叠贪婪合并生成密集非重叠块。

**Result:** 在MAUD数据集上，STC相比递归基线减少40%块数，相比键值基线减少56%；混合检索MRR从0.3576提升至0.5945，BM25检索的Recall@1从0.366提升至0.754。

**Conclusion:** 保持表格结构的分块方法能显著改善检索性能，证明了结构感知分块对于表格数据RAG的重要性。
...
> 研究发现零样本视觉语言模型的安全分类器在语义等价的提示词下会产生不可靠的安全概率，而通过无训练的提示词平均集成方法可以显著提升模型的可靠性和校准性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的零样本VLM安全分类器使用单提示词的首词概率作为决策分数，但发现即使在二分类标签固定的情况下，语义等价的提示词重构仍会导致同一样本产生明显不同的不安全概率，这种提示词敏感性影响模型可靠性。

**Method:** 提出无训练的均值集成方法，将多个语义等价提示词的首词概率进行平均；对比了温度缩放、Platt缩放和等温回归等传统校准方法；在14个数据集-模型对上评估NLL、ECE、AUROC和AUPRC指标。

**Result:** 均值集成在所有14个评估对上改善了NLL，在12/14对上改善了ECE；在排序性能上，AUROC和AUPRC相对训练选择的单提示词基线都有提升；有标签校准可进一步带来增益。

**Conclusion:** 提示词平均是一个强大的无标签预处理步骤，而非校准的替代品；建议将提示词族评估与均值聚合作为标准的无标签可靠性基线，用于零样本VLM安全分数的可靠性压力测试。
...
> 该论文提出了一种在严格token预算下选择临床文档子集的方法RCD，通过平衡相关性、覆盖率和多样性的子模优化目标，有效降低大语言模型在临床任务上的部署成本。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在临床应用中面临高昂的token成本和部署成本，临床输入文档通常冗长且冗余，而下游任务简短且重要，需要在固定成本和延迟约束下选择最相关的文档子集。

**Method:** 将问题建模为背包约束的子集选择问题，提出RCD单调子模目标函数平衡相关性、覆盖率和多样性，比较了句子、章节、窗口和聚类等单元化方案，并引入适应预算范围的路由启发式算法。

**Result:** 实验表明最优策略取决于评估设置：低预算时位置启发式在抽取任务中表现最佳，而基于多样性的MMR等方法在LLM生成中效果更好；选择器选择比单元化更重要，聚类分组会降低性能；ROUGE在LLM摘要中趋于饱和，BERTScore能更好反映质量差异。

**Conclusion:** 预算化上下文选择是一个具有挑战性的优化问题，RCD方法为临床文档的token成本控制提供了有效解决方案，选择器策略和评估指标的选择对结果影响显著。
...
> EVICT是一种训练自由、超参数自由且无损的自适应验证方法，专门用于加速MoE模型的推测解码，通过在验证前截断草稿树并保留性价比高的前缀，实现显著加速并减少不必要的专家激活。

<details>
  <summary>Details</summary>

**Motivation:** 基于树的推测解码虽然能并行验证多个草稿候选来加速自回归生成，但这一优势在稀疏MoE模型中会明显减弱。当草稿树增长时，不同分支会激活不同的专家，导致激活专家的并集扩大，从而显著增加目标端验证成本。

**Method:** EVICT方法通过在目标验证前截断草稿树，仅保留性价比高的前缀；利用细粒度的草稿信号来估计候选效益，结合离线分析的验证成本；与高性能基于图的服务框架SGLang高度兼容，实现训练自由、超参数自由的自适应验证。

**Result:** 实验表明，EVICT在各种MoE主干网络和基准测试上相比自回归解码实现最高2.35倍加速，相比最先进基线EAGLE-3平均实现1.21倍加速，同时在验证过程中显著减少不必要的专家激活。

**Conclusion:** EVICT成功解决了MoE模型中推测解码的效率瓶颈问题，通过智能的草稿树截断策略在不损失生成质量的前提下显著提升推理速度，为MoE模型的高效部署提供了实用解决方案。
...
> MemRouter是一种基于嵌入的内存路由器，用轻量级分类头替代LLM的每轮内存管理解码，显著降低延迟并提升性能

<details>
  <summary>Details</summary>

**Motivation:** 长期对话代理需要决定哪些对话轮次存储到外部内存中，但现有系统每轮都依赖自回归LLM生成来做决策，效率低下

**Method:** MemRouter将每个轮次与最近上下文一起编码，通过冻结的LLM主干投影嵌入，使用仅需训练12M参数的轻量级分类头预测是否存储

**Result:** 在LoCoMo数据集上，MemRouter在所有问题类别上均优于基于LLM的内存管理器（F1：52.0 vs 45.6），内存管理延迟从970ms降至58ms

**Conclusion:** 写端内存准入机制可通过小型监督路由器学习，答案生成长期对话QA中仍保持为独立的下游组件
...
> 本文提出了一种新的LLM参数编辑方法，用前向传播替代传统的反向传播，通过在第一层优化锚点并向前传播来获得更准确的层间目标状态，具有相同计算复杂度但精度更高。

<details>
  <summary>Details</summary>

**Motivation:** 传统的LLM参数编辑方法依赖反向传播技术，但其底层原理缺乏系统研究，存在能力边界不清、实际考虑不足和潜在失败模式等问题，需要更深入的分析和改进。

**Method:** 首先系统研究了传统反向传播方法的基础，明确了其能力边界和潜在问题。然后提出前向传播替代方案：在第一编辑层优化锚点，通过前向传播为所有后续编辑层生成准确且相互兼容的目标隐藏状态。

**Result:** 新方法在保持与现有方法相同计算复杂度的同时，产生了更精确的层间目标。方法简单优雅，不干扰其他编辑组件，可广泛应用于各种LLM参数编辑方法。

**Conclusion:** 通过系统研究传统方法的基础并创新性地采用前向传播机制，本方法为LLM参数编辑提供了更可靠、更高效的解决方案，具有重要的理论意义和实用价值。
...
> TokenUnlearn 是一种标记级别的机器反学习框架，通过精确识别和定位关键标记来改进大型语言模型的反学习效果，在保持模型效用的同时显著提升 forgetting 性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的机器反学习方法在序列级别操作，对所有标记统一更新，但实际只有部分标记编码了需要删除的知识，这种方法引入梯度噪声、降低模型效用，导致反学习效果次优。

**Method:** 提出标记级别的归因框架 TokenUnlearn，结合知识感知（通过掩码）和熵感知信号计算标记重要性分数，并开发两种策略：硬选择（仅对高重要性标记反学习）和软权重（根据重要性分数调节梯度贡献）。

**Result:** 在 TOFU 和 WMDP 基准测试上，三种模型架构的实验表明，TokenUnlearn 在遗忘效果和效用保持方面均优于序列级别的基线方法。

**Conclusion:** 标记级别的选择能够改善梯度信噪比，理论分析和实验结果证明了 TokenUnlearn 框架的有效性，为隐私、安全和监管需求下的 LLM 反学习提供了更精确的解决方案。
...
> 本研究为2025年大阪世博会开发了多语言翻译技术，实现了高质量、低延迟的同声传译系统，并在实际场景中得到应用部署。

<details>
  <summary>Details</summary>

**Motivation:** 为了配合《全球交流计划2025》，消除2025年大阪世博会的语言障碍，为来自世界各地的参观者提供无缝的交流体验。

**Method:** 开发了基于分块的输入分割技术、上下文感知翻译技术和多引擎机器翻译技术，构建了高质量、低延迟的同声传译系统。

**Result:** 通过演示部署和与私营企业的合作，相关技术已成功应用于实际场景，多个服务和系统在2025年大阪世博会上得到展示。

**Conclusion:** 研究成功实现了世博会环境下的无语言障碍交流体验，证明了多语言翻译技术在国际大型活动中的实用价值和推广前景。
...
> 本研究构建了一个基于智能代理的AI网络应用，通过整合缉毒局记录和实时文献检索，提供透明的情境化药物使用教育，专家评估显示系统在准确性、引用质量等方面表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 传统物质教育面临可扩展性、个性化和信息时效性等挑战，而AI在提供实时、权威的物质使用教育方面的应用尚未充分探索，需要开发新的解决方案。

**Method:** 开发了基于检索增强生成的AI代理系统，整合102篇经过筛选的文献和动态PubMed查询，采用语义分块和向量存储技术，通过专家评估研究验证系统效果。

**Result:** 专家评估显示系统在四个评价标准上的平均评分均在4.18-4.35之间，评价员间一致性程度高(Cohen's kappa = 0.78)，表明系统表现优异。

**Conclusion:** 基于智能代理的AI架构整合权威监管源和实时科学文献是医疗教育的一个有前景方向，值得通过更多的纵向用户研究进一步评估。
...
> Agent Capsules是一种自适应执行运行时，通过优化多智能体管道的执行策略，实现显著的token节省，同时保持或提升输出质量。

<details>
  <summary>Details</summary>

**Motivation:** 传统多智能体管道每个智能体需要单独进行LLM调用，成本高昂。将多个智能体合并调用虽可节省token，但会因工具丢失和提示压缩导致质量下降，需要智能的解决方案来平衡效率与质量。

**Method:** 提出Agent Capsules框架，将多智能体管道执行视为带质量约束的优化问题。运行时测量每个组的协调开销，评估组合机会，在三种复合执行策略间选择，并通过滚动平均输出质量控制模式切换。采用渐进式升级策略：标准→两阶段→顺序执行。

**Result:** 在14个智能体的竞争情报管道上减少51%的细粒度模式输入token和42%的复合模式输入token，质量分别提升0.020和0.017。在5个智能体的尽职调查管道上比未编译的DSPy节省19% token，质量相当；比MIPROv2节省68% token，质量提升0.052。

**Conclusion:** Agent Capsicles通过自适应执行控制和策略选择，实现了多智能体系统的token效率优化，无需训练数据或按管道工程即可匹配手工调优和编译时基线的性能。其渐进式质量控制机制确保了在追求token节省的同时不会牺牲输出质量。
...
> 研究表明，通过LoRA微调的小型语言模型（Qwen2.5-3B和Qwen3-4B）可以在消费级CPU上实现高性能的多任务放射学AI助手功能。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在放射学领域表现出色，但计算需求过高限制了其在资源受限的临床环境中的部署应用。

**Method:** 使用LoRA微调技术训练Qwen2.5-3B-Instruct和Qwen3-4B模型，基于12个公共数据集构建的162K样本，涵盖9个放射学任务，并在每个任务的500个测试样本上评估性能。

**Result:** LoRA微调显著提升性能（RADS准确率+53%，NLI+60%，N分期+89%），两模型各具优势，集成模型达到最佳效果，量化后可在消费级硬件上以4-8tokens/秒速度运行。

**Conclusion:** 名为RadLite的小型高效微调模型可以作为实用的多任务放射学AI助手，完全部署在消费级硬件上，无需GPU支持。
...
> 本文将文本生成中的模式崩溃问题重新解释为几何崩溃现象，并提出了强化模式调节(RMR)方法来解决这个问题。

<details>
  <summary>Details</summary>

**Motivation:** 模式崩溃是生成建模中的持续挑战，在自回归文本生成中表现为从显式循环到逐渐失去多样性和过早轨迹收敛等行为。现有的符号约束或纯概率解码启发式方法无法可靠解决这一问题。

**Method:** 作者从动力学系统角度出发，将模式崩溃重新解释为几何崩溃——模型内部轨迹在其表示空间中被限制在低维区域。基于这个观点，提出了强化模式调节(RMR)，这是一种轻量级的在线状态空间干预方法，通过在Transformer值缓存中调节主导自增强方向（实现为低秩阻尼）来防止几何崩溃。

**Result:** 在多个大型语言模型上，RMR显著减少了模式崩溃，能够在极低熵率（低至0.8 nats/步）下实现稳定、高质量的生成，而标准解码通常在2.0 nats/步附近崩溃。

**Conclusion:** 该研究表明模式崩溃不仅仅是符号层面的现象，而是几何结构问题，RMR方法为解决这一问题提供了有效的新思路，能够在保持高质量的同时实现更稳定的文本生成。
...
> 本文通过囚徒困境实验发现，LLMs对任务表述方式敏感，不当表述会导致模型产生预设偏见，而中性表述能促进逻辑推理。

<details>
  <summary>Details</summary>

**Motivation:** 出于对大型语言模型在不可预测的真实世界应用中的安全性和可靠性的担忧，研究任务表述如何导致LLMs产生预设偏见并影响其适应性。

**Method:** 以迭代囚徒困境为案例研究，通过实验对比不同任务表述方式下LLMs的决策表现，包括有推理步骤和无推理步骤的情况。

**Result:** 实验表明，即使有推理步骤，LLMs在决策时仍容易受到表述方式的影响而产生预设偏见；当使用中性任务表述时，模型能够进行较少偏见的逻辑推理。

**Conclusion:** 研究发现强调了恰当地表述任务对于降低LLMs中预设偏见风险的重要性，为提升模型安全性和可靠性提供了指导。
...
> 该研究创建了一个包含300个参与者-PLS(通俗语言摘要)对的ReLay数据集，用于研究LLM个性化摘要的效果，发现个性化能提高理解度和质量感知，但也存在强化偏见和安全风险。

<details>
  <summary>Details</summary>

**Motivation:** 传统PLS采用一刀切的写作风格，忽略了读者的信息需求和理解能力差异，在健康领域这种局限性尤其重要，因为误解科学信息可能影响现实世界的决策。

**Method:** 构建ReLay数据集，包含50名外行参与者在静态(专家撰写)和交互式(LLM个性化)设置中的300个参与者-PLS对，收集用户特征、健康信息需求、搜索行为、理解结果、交互日志和质量评级，并评估5个LLM在两种个性化方法上的表现。

**Result:** 个性化和确实提高了理解度和感知质量，但同时也存在强化用户偏见和产生幻觉的风险，揭示了个性化与安全性之间的权衡关系。

**Conclusion:** 研究强调了需要开发既有效又可信的个性化方法，以服务于多样化的外行受众，平衡个性化效果与安全性的考量。
...
> 该研究将话语产生建模为基于信息论成本的情境替代选择概率模型，发现相对于目标导向替代的选择惊奇最小化能最有效地预测自然对话中的语言产生。

<details>
  <summary>Details</summary>

**Motivation:** 传统语言产生研究往往难以区分说话者和听话者的不同压力约束。本研究旨在建立一个统一框架，通过区分目标导向和目标无关的替代选项，来更好地理解和量化语言产生中不同因素的作用。

**Method:** 提出基于信息论的成本敏感选择模型，使用语言模型生成两种类型（目标导向和目标无关）的替代选项集合，在开放式对话中分析确定性和概率性成本最小化下的产生选择。

**Result:** 实验发现，相对于目标导向替代的惊奇最小化在两种分析下都具有最强的预测能力，而统一信息密度和基于长度的成本在不同条件下表现出较弱且不一致的预测力。

**Conclusion:** 研究表明，基于语言模型生成替代选项的条件优化框架为研究自然语言产生中的说话者和听话者压力提供了原则性方法，惊奇最小化是解释话语选择的关键机制。
...
> ControBench是一个结合了异质社交交互图和丰富文本语义的争议性话语分析基准，包含Reddit三大争议话题的用户互动数据。

<details>
  <summary>Details</summary>

**Motivation:** 现有数据集在分析跨意识形态网络争论时存在不足：有的只保留文本但忽略交互结构，有的建模结构但缺乏丰富语义，有的对话表示缺少稳定的用户级意识形态身份。

**Method:** 从Reddit关于特朗普、堕胎和宗教的讨论中构建数据集，包含7,370个用户、1,783个帖子和26,525次交互。图中包含用户和帖子节点，边富含语义信息，特别是用户-评论-用户边编码了回复和被回复的父评论。用户标签来源于自声明的Reddit风格，提供意识形态身份的可扩展代理。

**Result:** 数据集表现出低或负的调整同配性（特朗普：-0.77，堕胎：0.06，宗教：0.04），反映了现实世界争论的交叉结构。不同模型在ControBench上表现出不同的性能模式，特别是在意识形态边界模糊时。

**Conclusion:** ControBench为争议性话语分析提供了一个具有挑战性和现实性的基准，能够有效评估各种模型在跨意识形态话语分析中的表现。
...
> AGoQ是一种通过层感知激活量化和8-bit梯度量化技术，显著降低大语言模型训练内存需求并提升训练速度的新方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的量化方法在处理4-bit激活和8-bit梯度时效果不佳，容易导致收敛缓慢或精度损失，无法有效满足大语言模型训练对GPU内存的优化需求。

**Method:** 提出AGoQ框架，包含两项核心技术：1）层感知激活量化算法，根据层类型和流水线阶段为不同层分配适当比特宽度的激活值；2）梯度量化算法，采用8-bit梯度存储和精度保持的8-bit All-Reduce通信来减少内存使用并缩短通信时间。

**Result:** 在两个GPU集群（最多64个GPU）上对不同规模LLM的实验表明，与Megatron-LM、COAT和DeepSpeed等先进训练系统相比，AGoQ可将内存使用减少高达52%，训练速度提升1.34倍，同时在预训练收敛损失和下游任务精度上保持与LLaMA架构相当的性能。

**Conclusion:** AGoQ通过创新的激活和梯度量化策略，有效解决了现有量化方法在4-bit激活和8-bit梯度场景下的局限性，为大语言模型的高效内存优化和加速训练提供了新的解决方案。
...
> A11y-Compressor框架将线性的可访问性树压缩为结构化表示，在减少78%输入token的同时将任务成功率提升5.1%。

<details>
  <summary>Details</summary>

**Motivation:** 当前AI代理 GUI交互中使用的可访问性树存在冗余信息和缺乏结构化信息的缺陷，如元素间的空间关系，这影响了交互的有效性和可靠性。

**Method:** 提出A11y-Compressor框架，实现Compressed-a11y工具，通过轻量级结构化处理管道进行模态检测、冗余减少和语义结构化来转换线性化可访问性树。

**Result:** 在OSWorld基准测试中，Compressed-a11y成功将输入tokens减少至原数据的22%，平均任务成功率提升5.1个百分点。

**Conclusion:** 该方法证明了结构化和压缩的可访问性表示能够有效提升AI GUI代理的观察质量和交互性能，为GUI自动化任务提供了更高效的解决方案。
...
> SCISENSE是一个基于认知科学的IDEATION框架，通过构建10万级研究轨迹数据集训练了不同规模的语言模型，发现目标导向训练比推断训练能产生更高质量、新颖和多样化的研究轨迹。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法将科学发现中的构思阶段视为简短的前言，但这一阶段实际上在研究中处于核心地位。需要将IDEATION过程结构化并开发相应的工具来增强AI驱动的研究工作流。

**Method:** 基于Pirolli & Card的认知阶段理论构建SCISENSE框架；创建了SCISENSE-Traj数据集，包含目标重建和推断创新两种模式的10万级研究轨迹；训练了从3B到70B的SCISENSE-LM模型系列。

**Result:** 目标导向训练的模型在轨迹质量上比推断训练提高2.0%，且产出更具新颖性和多样性；下游编程代理使用目标轨迹产生的产出具有更高的可执行性和质量。

**Conclusion:** 目标导向的IDEATION能够减少下游代理的认知负担，释放其探索创造力；SCISENSE既为增强AI研究工作流提供了实用工具，也为研究规划如何影响科学发现提供了原则性平台。
...
> 论文提出了一种新的编码探针方法，通过可解释特征重构模型内部表示，解决了传统解码探针无法直接比较特征贡献和特征相关性影响的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的解码探针方法存在两个主要限制：无法直接比较不同特征对模型表示的贡献，以及特征之间的相关性会影响探测结果的准确性。

**Method:** 提出了编码探针方法，逆转了传统解码方向，使用可解释特征集来重构模型的内部表示，并在文本和语音Transformer模型上进行了评估，涵盖声学、语音学、语法、词汇和说话人身份等特征。

**Result:** 研究结果表明，说话人相关效应在不同的训练目标和数据集之间存在强烈变化，而语法和词汇特征对重构的贡献是相互独立的。

**Conclusion:** 编码探针为解释模型表示提供了可解码性之外的补充视角，为理解语言模型内部机制提供了新的方法论。
...
> 本研究开发了评估机器翻译对段落嵌入余弦相似度不变性的框架，通过分析28种语言政治宣言语料，发现10种语言翻译后语义结构得以保持，4种语言出现可检测的语义扭曲。

<details>
  <summary>Details</summary>

**Motivation:** 随着机器翻译技术的广泛应用，需要评估翻译过程对文本嵌入表示语义结构的影响，特别是在跨语言信息检索、文本分析等应用中，确保翻译不会破坏原文的语义关系至关重要。

**Method:** 研究人员设计了一个非劣效性测试框架，通过比较原始语言和翻译后文本的嵌入模型相似度关系稳定性，使用模型间分歧度作为校准阈值，来评估翻译对语义结构的影响。

**Result:** 在包含2,800多个政党宣言的多语言语料上，该框架成功识别出10种语言表现出翻译不变性，4种语言存在可检测的语义扭曲，其余语言的证据不足以做出判断。

**Conclusion:** 该框架为评估机器翻译对语义结构保持的影响提供了系统化方法，具有语料库和处理流水线无关性，可自然扩展到下游任务，为翻译质量评估和新工具开发提供了重要参考。
...
> 提出SC-Taxo框架，利用大语言模型通过层次感知的精炼阶段生成语义一致的科学分类法，解决了现有方法在结构一致性和语义对齐方面的问题。

<details>
  <summary>Details</summary>

**Motivation:** 科学文献激增使得领域知识的组织访问面临挑战，现有分类法生成方法存在结构不一致和层次间语义错位问题，主要源于未能充分建模层次语义一致性。

**Method:** SC-Taxo框架引入双向标题生成机制，同时执行自下而上的抽象和自上而下的语义约束，并捕捉同级语义依赖以增强水平一致性，确保生成的分类法语义一致。

**Result:** 在多个基准数据集上的实验表明，该方法在层次对齐和标题质量方面取得一致改进，中文科学文献的额外评估验证了其强大的跨语言泛化能力。

**Conclusion:** 通过有效建模层次语义一致性，SC-Taxo框架能够生成高质量的科学分类法，为文献探索、主题导航及下游应用提供更好的支持。
...
> H-RAG是一个用于多轮对话检索增强生成的分层父-子RAG方法，通过细粒度子块检索和父级上下文重建，在SemEval-2026任务中取得了良好效果。

<details>
  <summary>Details</summary>

**Motivation:** 解决多轮对话环境中的检索增强生成问题，需要在保持检索准确性的同时，确保生成的答案既准确又忠实于检索到的证据。

**Method:** 实现了分层父子RAG管线：将文档分割为重叠的句子级子块进行细粒度检索，同时保留完整文档作为父级单元提供连贯上下文；采用混合稠密-稀疏搜索、可调权重和基于嵌入的相似度重排序；在父级聚合检索证据并输入指令调优语言模型生成响应。

**Result:** 在任务A上获得0.4271的nDCG@5分数，在任务C上获得0.3241的调和平均分数（RB_agg: 0.2488, RL_F: 0.2703, RB_llm: 0.6508），验证了检索配置和父级聚合在多轮RAG性能中的重要性。

**Conclusion:** 分层检索方法有效平衡了检索准确性和上下文连贯性，为多轮对话RAG系统提供了有价值的解决方案。
...
> MathArena从静态基准测试扩展为持续维护的数学推理评估平台，覆盖范围更广，显示LLMs在数学领域已达到极高水平。

<details>
  <summary>Details</summary>

**Motivation:** 传统的静态数学基准测试已经无法有效评估LLMs的快速进展，因为它们范围狭窄、容易被突破且很少更新，导致难以可靠地比较模型和追踪发展。

**Method:** 大幅扩展MathArena平台，从仅限奥赛最终答案问题扩展到包括基于证明的竞赛、研究级arXiv问题、Lean形式化证明生成等更广泛的数学任务，并建立统一的评估协议，定期设计新基准。

**Result:** 最强模型GPT-5.5在2026年美国数学奥赛中达到98%准确率，在研究级问题上达到74%，表明前沿模型已能解决极其困难的数学问题。

**Conclusion:** 像MathArena这样的持续维护评估平台对于追踪LLMs在数学推理方面的快速进展至关重要，传统的静态基准测试已经不再适用。
...
> 提出了ML-Bench多语言安全基准和ML-Guard防护模型，基于各地区法规构建，支持14种语言的安全评估和合规性检查。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型在跨语言环境中的部署，确保安全成为关键挑战。现有的多语言基准主要依赖通用风险分类和机器翻译，限制了防护模型适应特定地区法规和文化差异的能力。

**Method:** 1) 基于地区法规构建ML-Bench基准，涵盖了14种语言；2) 开发基于扩散大语言模型的ML-Guard防护模型，提供1.5B轻量级和7B增强版两个变体；3) 支持快速安全检查和自定义合规性评估。

**Result:** 在6个现有多语言安全基准和ML-Bench上，ML-Guard相比11个强基线方法表现优异，证明了其有效性和优越性。

**Conclusion:** ML-Bench和ML-Guard有助于推动法规感知和文化对齐的多语言防护系统发展，为构建更安全的多语言AI应用提供了重要工具。
...
> MemCoE是一个认知启发的两阶段优化框架，通过记忆指导归纳和指导对齐的策略优化，解决LLM代理在长期个性化交互中的记忆管理问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型代理因上下文窗口限制而难以长期跟踪用户偏好的演变，传统的静态记忆更新规则效果有限，而基于强化学习的方法由于稀疏奖励监督弱导致长时程优化不稳定。

**Method:** 提出MemCoE两阶段框架：第一阶段通过对比反馈优化全局记忆指导原则并解释为文本梯度；第二阶段使用诱导的指导原则定义结构化过程奖励，进行多轮强化学习学习遵循指导的记忆演化策略。

**Result:** 在三个个性化记忆基准测试上，包括显式/隐式偏好以及不同规模和噪声的数据，均观察到相比强基线方法的持续改进，具有良好的鲁棒性、可迁移性和效率。

**Conclusion:** MemCoE框架成功地将认知科学中的记忆图式理论应用于LLM代理的记忆管理，通过两阶段优化实现了更稳定有效的长期个性化记忆系统。
...
> FinSafetyBench是一个用于评估金融领域大语言模型安全的双语基准测试，揭示了现有模型在金融合规方面的脆弱性。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在金融场景应用日益增多，但可能产生有害输出，如协助非法活动或不道德行为，存在严重的合规风险。需要系统评估金融领域LLM的安全性。

**Method:** 提出了FinSafetyBench，一个基于真实金融犯罪案例和道德标准的英汉双语红队测试基准，包含14个金融犯罪和道德违规子类别。在通用和金融专业LLM上进行了三种代表性攻击设置的广泛实验。

**Result:** 发现了关键漏洞，使对抗性提示能够绕过合规保护措施。分析表明中文环境下的模型更易受损，并且提示级防御策略在应对复杂或隐式操纵策略时存在局限性。

**Conclusion:** 金融领域的大语言模型面临严重的安全挑战，需要更强的防护机制。特别是在中文环境下，模型更容易受到攻击，现有的提示级防御策略不足以应对复杂的威胁。
...
> 本文通过形式化分析揭示了Transformer局部注意力机制为何能提升模型质量——它增加了识别器的表达能力，使得模型能识别更丰富的正则语言类别。

<details>
  <summary>Details</summary>

**Motivation:** Transformer的全局注意力机制虽然有效但计算成本高。实践中发现局部注意力不仅提高了效率，还能改善模型质量，但这一现象缺乏理论解释。

**Method:** 通过形式化语言理论分析，证明全局注意力对应包含单个过去算子的线性时序逻辑片段，而局部注意力引入了第二个时序算子，严格扩展了可识别正则语言的类别。

**Result:** 证明了全局和局部注意力在表达能力上是互补的，两者结合能识别最丰富的语言片段。实验验证了混合全局-局部Transformer在形式语言识别和自然语言建模上优于仅使用全局注意力的模型。

**Conclusion:** 局部注意力机制的优越性源于其增强了模型的表达能力，为Transformer架构的设计提供了理论基础，支持了混合注意力策略的合理性。
...
> DSR方法通过基于transformer的模型对识别文本中情感目标并沿社会情感维度进行多维度评分，解决了传统情感分析无法处理情感共存的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 现有NLP情感分析工具只能将文本整体划分为积极、中性或消极，无法识别同一消息中针对不同话题同时存在的亲社会与反社会情感，也无法确定情感的具体目标。

**Method:** 提出DSR（Directed Social Regard）方法，包含两个transformer模型：首先检测消息中span级情感目标，然后在消息语境中沿三个(-1,1)关注维度对所有span进行评分，基于道德脱离和道德框架的社会科学理论构建数据集和架构。

**Result:** 在六个第三方在线媒体数据集上验证了DSR模型，其输出与社会科学数据集的标签和主题之间存在显著相关性，验证了方法的有效性。

**Conclusion:** DSR成功实现了文本中情感目标识别和多维度情感评分，为分析在线平台、影响行动和政治言论中复杂的情感表达提供了新的技术手段。
...
> 研究发现大型语言模型在执行分步算法时，随着步骤增加，准确率从61%骤降至20%，暴露了程序执行的根本缺陷。

<details>
  <summary>Details</summary>

**Motivation:** 仅凭最终答案准确率无法评估LLM是否真正按照指定程序执行推理过程，需要专门诊断模型对程序性指令的忠实执行能力。

**Method:** 构建了一个程序执行诊断基准，使用简单算术操作，但通过增加算法长度和中间变量的回溯依赖来提升复杂性，测试14个模型在55个数据集上的表现。

**Result:** 模型在5步骤程序上平均准确率61%，但在95步骤程序上降至20%；分析发现失败原因包括缺失答案、过早回答、错误后自我纠正、执行轨迹不完整和幻觉额外步骤。

**Conclusion:** 表面的推理能力可能掩盖了对指令忠实执行的严重不足，LLM在程序化推理执行方面存在根本性缺陷。
...
### [47] [Learning from the Unseen: Generative Data Augmentation for Geometric-Semantic Accident Anticipation](https://arxiv.org/abs/2605.00051)
*Yanchen Guan,Haicheng Liao,Chengyue Wang,Xingcheng Liu,Jiaxun Zhang,Keqiang Li,Zhenning Li*

Main category: cs.CV

> 本文提出了一个双路径框架来解决自动驾驶中的交通事故预测问题，通过视频合成技术生成高质量合成驾驶场景，并采用融合语义的图神经网络进行动态关系推理，同时发布了一个大规模基准数据集。

<details>
  <summary>Details</summary>

**Motivation:** 自动驾驶中的事故预测是一个关键但未解决的问题，主要受限于道路使用者间交互建模的复杂性和多样化大规模数据集的缺乏。

**Method:** 双路径方法：1）基于结构化提示的视频合成管道，从现有语料库提取特征分布并生成符合真实数据统计模式的高保真合成驾驶场景；2）融合语义线索的图神经网络，可对参与者的空间和语义关系进行动态推理。

**Result:** 在现有数据集和新的基准数据集上的评估表明，该方法在预测准确性和预测提前时间上都有显著提升，有效缓解了数据瓶颈问题。

**Conclusion:** 所提出的框架通过合成数据和改进的关系推理模型，显著增强了自动驾驶系统的可靠性，为解决交通事故预测这一难题提供了有效方案。
...
### [48] [Two-View Accumulation as the Primary Training Lever for Hybrid-Capture Gaussian Splatting: A Variance-Decomposition View of When Gradient Surgery Helps](https://arxiv.org/abs/2605.00052)
*Sungjun Cho*

Main category: cs.CV

> 混合捕获新视角合成中，通过将每次优化步骤渲染两个视图而不是一个，可以显著缩小不同距离视角间的性能差距，而不需要复杂的配对规则或其他技术手段。

<details>
  <summary>Details</summary>

**Motivation:** 标准3D高斯散射(3DGS)在混合捕获场景（如无人机和地面视角）中训练时，存在少数视角欠拟合1-3dB的问题，需要找到简单有效的解决方案。

**Method:** 系统比较了多种计算匹配的改进方案，包括增加迭代次数、梯度修正、方向感知梯度手术、投影预处理、置信门控采样以及随机双视图控制，发现最简单的结构改变——每步渲染两个视图——效果最好。

**Result:** 在五个混合捕获基准测试上，双视图方案显著缩小了性能差距。方差分解框架解释了这一现象：在双峰相机制度下，制度间梯度方差相对较小，因此结构化和随机配对在期望上是方差等价的，而双视图累积的方差减半是主导效应。

**Conclusion:** 为混合捕获3DGS提供了诚实的结果分析，明确了哪些训练方法能或不能提升PSNR，并通过理论框架解释了原因。双视图方法在Scaffold-GS和Pixel-GS骨干网络上同样有效。
...
### [49] [AIDA-ReID: Adaptive Intermediate Domain Adaptation for Generalizable and Source-Free Person Re-Identification](https://arxiv.org/abs/2605.00111)
*Sundas Iqbal,Qing Tian,Danish Ali,Jianping Gou,Weihua Oue*

Main category: cs.CV

> AIDA/SF-MIDA是一种自适应中间域适应方法，通过动态调节特征混合和正则化强度来处理无监督跨域行人重识别任务。

<details>
  <summary>Details</summary>

**Motivation:** 现有中间域方法存在混合策略固定、需要联合访问源域和目标域等限制，无法适应多源和免源设置，且在遇到未见环境时性能显著下降。

**Method:** 提出自适应中间域学习框架，利用模型不确定性和训练稳定性的反馈信号动态控制特征混合和正则化强度；设计多源中间域生成器合成多样化中间表示；采用伪镜正则化策略保持域扰动下的身份一致性。

**Result:** 在域泛化和免源设置下的广泛实验证明了所提框架的有效性，能够更好地处理跨域行人重识别任务。

**Conclusion:** AIDA/SF-MIDA通过自适应调节机制突破了传统方法的局限，为跨域行人重识别提供了更灵活和鲁棒的解决方案。
...
### [50] [GAFSV-Net: A Vision Framework for Online Signature Verification](https://arxiv.org/abs/2605.00120)
*Himanshu Singhal,Suresh Sundaram*

Main category: cs.CV

> GAFSV-Net将在线签名转换为六通道Gramian角场图像，利用双分支ConvNeXt-Tiny编码器和交叉注意力机制，显著提升了在线签名验证的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有在线签名验证方法直接处理原始时间序列，限制了1D架构使用且无法利用预训练的2D视觉模型，难以有效区分高类内变异下熟练伪造签名。

**Method:** 提出GAFSV-Net：将签名运动学特征编码为GASF/GADF图像构建六通道不对称表示；双分支ConvNeXt-Tiny独立处理GASF和GADF；双向交叉注意力让分支间查询判别模式；采用半难三元组损失加熟练伪造负样本训练。

**Result:** 在DeepSignDB和BiosecurID数据集上超越所有相同训练目标的序列基线方法，证明了2D时间编码的表示增益具有一致性和独立性。

**Conclusion:** 通过将动态签名转换为结构化2D表示并利用先进视觉架构，GAFSV-Net为在线签名验证提供了一种新的有效范式，其性能提升源于多个设计选择的协同作用。
...
### [51] [Real-Time Frame- and Event-based Object Detection with Spiking Neural Networks on Edge Neuromorphic Hardware: Design, Deployment and Benchmark](https://arxiv.org/abs/2605.00146)
*Udayanga G. W. K. N. Gamage,Yan Zeng,Cesar Cadena,Matteo Fumagalli,Silvia Tolu*

Main category: cs.CV

> 本研究提出了一套基于神经形态 Loihi 2 处理器的脉冲神经网络目标检测设计方法，实现超低能耗实时检测，通过知识蒸馏使SNN检测精度恢复至ANN的87-100%。

<details>
  <summary>Details</summary>

**Motivation:** 在能量受限平台（如无人机、移动机器人）上进行实时目标检测至关重要。传统神经网络能耗高，而脉冲神经网络在神经形态硬件上具有显著能效优势，但需解决部署和技术适配问题。

**Method:** 设计面向神经形态平台的通用SNN检测架构，开发Loihi 2部署适配方案，采用ANN-to-SNN知识蒸馏训练技术，在基于帧和基于事件的两种数据集上进行benchmark测试。

**Result:** Loihi 2上SNN实现实时检测，单次推理动态能耗最低，功耗优于其他平台；使用蒸馏训练后SNN检测精度达到ANN的87-100%，否则精度下降11-27%；Jetson Orin Nano的ANN推理速度更快。

**Conclusion:** 研究表明神经形态系统在边缘设备能量高效实时目标检测方面具有巨大潜力，SNN通过适当训练能在保持低延迟的同时接近ANN的检测精度。
...
### [52] [From Images2Mesh: A 3D Surface Reconstruction Pipeline for Non-Cooperative Space Objects](https://arxiv.org/abs/2605.00147)
*Bala Prenith Reddy Gopu,Patrick Quinn,George M. Nehma,Madhur Tiwari,Matt Ueckermann,David Hinckley,Christopher McKenna*

Main category: cs.CV

> 本文提出了一种基于神经隐式表面重建的管道，用于从单目在轨检查图像中重建非合作空间目标，解决了真实在轨环境中的背景干扰和光照变化问题。

<details>
  <summary>Details</summary>

**Motivation:** 在轨检查图像对于非合作空间目标的特征描述和主动碎片清除、在轨服务任务规划至关重要，但现有神经隐式表面重建方法局限于已知相机位姿和受控光照的合成或硬件在环数据。

**Method:** 开发了一个处理流程，包含基于分割的背景移除技术（用于成功估计真实在轨画面的相机位姿）、逐帧曝光变化的光度校正，并在ISS和H-IIA火箭上阶段的公开在轨检查数据上进行验证。

**Result:** 发现基于分割的背景移除是成功处理真实在轨画面的关键技术，光度校正能处理曝光变化，但阴影区域的重建性能随输入画面的光照特性而变化。

**Conclusion:** 该管道首次实现了从真实在轨单目检查图像进行神经隐式表面重建，背景移除和光度校正是关键步骤，为空间目标在轨检测和维护提供了实用的技术方案。
...
### [53] [VkSplat: High-Performance 3DGS Training in Vulkan Compute](https://arxiv.org/abs/2605.00219)
*Jingxiang Chen,Mohamed Ibrahim,Yang Liu*

Main category: cs.CV

> VkSplat是一个完全基于Vulkan计算实现的跨GPU厂商3D高斯散布(3DGS)训练管线，相比CUDA+PyTorch基线实现3.3倍速度提升和33%显存节省，同时保持质量不变。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3DGS训练管线存在性能限制和GPU厂商兼容性问题，需要一种高性能且跨平台的解决方案来实现更高效的训练。

**Method:** 通过完全基于Vulkan计算实现3DGS训练管线，采用多种优化技术，包括高效的内存管理和并行计算策略，实现在不同GPU厂商硬件上的统一运行。

**Result:** 相比CUDA+PyTorch基线实现3.3倍速度提升和33%显存减少，同时保持相同的质量水平，并在多个GPU厂商平台上展现出良好的兼容性。

**Conclusion:** 这是首个完全基于Vulkan的3DGS训练管线，在实现跨厂商兼容性的同时达到了最先进的性能表现，为3DGS训练提供了新的高性能解决方案。
...
### [54] [Adaptive Geodesic Conformal Prediction for Egocentric Camera Pose Estimation](https://arxiv.org/abs/2605.00233)
*Aishani Pathak,Hasti Seifi*

Main category: cs.CV

> 该研究针对AR中的姿态估计，发现标准保形预测在最困难帧上的覆盖率存在巨大缺口，并提出基于DINOv2的自适应保形预测方法，成功解决了条件覆盖率不均的问题。

<details>
  <summary>Details</summary>

**Motivation:** 增强现实和辅助设备中的第一人称姿态估计需要准确的预测和保证的不确定性区域。标准保形预测虽然能提供覆盖保证，但在最困难的帧上存在严重的覆盖率不足问题，这会影响设备在关键时刻的可靠性。

**Method:** 1）发现标准保形预测在最困难25%帧上仅60%覆盖率的问题；2）提出使用测地线SE(3)非一致性分数来识别物理上更困难的帧；3）设计DINOv2-Bridge自适应保形预测框架，通过两阶段难度估计器实现跨参与者迁移。

**Result:** 在EPIC-Fields数据集上，新方法将最困难帧(Q4)的覆盖率从0.75提升到0.93，同时保持整体90%的目标覆盖率，有效解决了30个百分点的条件覆盖率缺口问题。

**Conclusion:** 该研究成功解决了保形预测在困难帧上覆盖率不足的关键问题，提出的自适应方法无需测试时图像即可跨参与者迁移，为AR和辅助设备的可靠姿态估计提供了重要保障。
...
### [55] [MAEPose: Self-Supervised Spatiotemporal Learning for Human Pose Estimation on mmWave Video](https://arxiv.org/abs/2605.00242)
*Xijia Wei,Yuan Fang,Kevin Chetty,Youngjun Cho,Nadia Bianchi-Berthouze*

Main category: cs.CV

> MAEPose是一种基于掩码自编码的毫米波雷达人体姿态估计方法，直接处理原始雷达视频，无需中间表示，在多个数据集上显著超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有毫米波雷达姿态估计方法依赖预提取的中间表示（如稀疏点云或频谱图），丢弃了雷达视频流中丰富的时空信息，且未利用未标注数据学习通用表示。

**Method:** 提出MAEPose方法，使用掩码自编码技术直接对毫米波频谱视频进行自监督预训练，学习时空运动感知的通用表示，并利用热图解码器进行多帧姿态估计预测。

**Result:** 在三个数据集上的留一法交叉验证中，MAEPose持续优于最先进基线方法（MPJPE提升高达22.1%，p<0.05），在零射击干扰情况下仅增加6.5%错误率。

**Conclusion:** 掩码自编码预训练和热图解码器都是重要贡献项，Range-Doppler视频作为输入比Range-Azimuth或融合方法表现更好且计算成本更低。
...
### [56] [Remote SAMsing: From Segment Anything to Segment Everything](https://arxiv.org/abs/2605.00256)
*Osmar Luiz Ferreira de Carvalho,Osmar Abílio de Carvalho Júnior,Anesmar Olino de Albuquerque,Daniel Guerreiro e Silva*

Main category: cs.CV

> Remote SAM是一种开源处理流程，在无需修改SAM2或训练数据的情况下，通过多遍算法和边界合并技术，将大尺寸遥感图像的分割覆盖率从30-68%提升至91-98%。

<details>
  <summary>Details</summary>

**Motivation:** SAM2在自然图像上表现优异，但应用于大尺度遥感场景时存在两个问题：掩码生成器面临质量-覆盖率的固有权衡；大图像必须分块处理导致物体在边界处被分割。

**Method:** 提出Remote SAMsing方法，采用多遍算法在每个图块上重复运行SAM2，遍历间将接受的掩码涂黑以简化场景；当覆盖率增益停滞时才放宽质量阈值；使用上下文填充和参数无关的最佳匹配合并重建跨边界物体。

**Result:** 在七个场景中将覆盖率提升至91-98%；建筑检测精度95%，汽车82-93%；通过调整图块大小可隐式控制尺度参数；在19.4亿像素的波茨坦图像上达到97%覆盖率。

**Conclusion:** 该方法成功解决了SAM2在遥感图像应用中的质量和覆盖问题，具有良好的泛化能力和可扩展性，为大规模遥感图像分割提供了实用解决方案。
...
### [57] [REALM: An RGB and Event Aligned Latent Manifold for Cross-Modal Perception](https://arxiv.org/abs/2605.00271)
*Vincenzo Polizzi,David B. Lindell,Jonathan Kelly*

Main category: cs.CV

> REALM是一个跨模态框架，通过将事件流表示对齐到预训练RGB基础模型的潜在空间，使事件数据能够直接利用现成的RGB图像模型，实现了零样本应用和跨模态泛化。

<details>
  <summary>Details</summary>

**Motivation:** 现有的事件相机学习方法受限于特定任务，缺乏跨模态泛化能力；事件相机具有高时间分辨率、低延迟和强光照鲁棒性等优势，但无法充分利用成熟的RGB基础模型的强大先验知识。

**Method:** REALM框架通过低秩适应(LoRA)将事件表示投影到预训练的ViT-based RGB模型潜在空间中，从而桥接模态差异，解锁冻结RGB骨干网络对异步事件流的几何和语义先验。

**Result:** REALM成功将事件映射到ViT基础潜在空间，可直接迁移RGB训练的线性头执行深度估计和语义分割；最重要的是实现了直接将MASt3R等复杂冻结解码器零样本应用于原始事件数据，在宽基线特征匹配任务上达到最先进性能。

**Conclusion:** REALM为事件处理提供了一种通用的跨模态学习范式，打破了任务特定训练的限制，使事件数据能够直接利用强大的RGB基础模型生态，为事件视觉应用开辟了新的可能性。
...
### [58] [When Do Diffusion Models learn to Generate Multiple Objects?](https://arxiv.org/abs/2605.00273)
*Yujin Jeong,Arnas Uselis,Iro Laina,Seong Joon Oh,Anna Rohrbach*

Main category: cs.CV

> 该研究发现文本到图像扩散模型在多对象生成中的不可靠性主要源于场景复杂性和组合泛化能力的缺乏，而非数据不平衡问题。

<details>
  <summary>Details</summary>

**Motivation:** 文本到图像扩散模型在多对象生成中表现不可靠，尽管有大量实证证据，但根本原因尚不明确，需要探究数据本身在多大程度上造成了这些限制。

**Method:** 提出mosaic框架来生成控制性数据集，通过不同训练集规模下的概念泛化和组合泛化实验来分析模型性能，训练扩散模型并评估其在多对象场景中的表现。

**Result:** 研究发现场景复杂性是主导因素而非概念不平衡，计数在低数据环境下特别难学习；当训练时排除更多概念组合时，组合泛化能力会崩溃。

**Conclusion:** 这些发现揭示了扩散模型的基本局限，强调了需要更强的归纳偏置和更好的数据设计来实现鲁棒的多对象组合生成。
...
### [59] [An End-to-End Decision-Aware Multi-Scale Attention-Based Model for Explainable Autonomous Driving](https://arxiv.org/abs/2605.00291)
*Maryam Sadat Hosseini Azad,Shahriar Baradaran Shokouhi,Amir Abbas Hamidi Imani,Shahin Atakishiyev,Randy Goebel*

Main category: cs.CV

> 本文提出了一种基于多尺度注意力的驾驶决策可解释模型，能够为每个驾驶决策提供针对性解释，并通过新的联合F1分数指标验证了其在自动驾驶可解释性方面的优越性能。

<details>
  <summary>Details</summary>

**Motivation:** 自动驾驶系统深度学习模型的黑盒特性阻碍了对其决策过程的理解，现有可解释方法存在推理缺陷和指标不可靠问题，阻碍了真正可靠自动驾驶系统的发展。

**Method:** 设计了多尺度注意力模型，将驾驶决策输入推理组件，为每个决策同步生成案例特定解释；提出联合F1分数新指标进行定量评估；在BDD-OIA和nu-AR数据集上验证模型性能。

**Result:** 实验结果表明所提出的推理网络在经典和最先进模型中表现出优越性能，验证了模型的泛化能力和鲁棒性。

**Conclusion:** 该研究为自动驾驶系统的决策可解释性提供了有效解决方案，通过独特网络架构和新评估指标推动了可解释AI在自动驾驶领域的实际应用。
...
### [60] [Efficient Spatio-Temporal Vegetation Pixel Classification with Vision Transformers](https://arxiv.org/abs/2605.00296)
...
> 研究优化视觉变换器(ViTs)用于高效时空植被像素分类，相比传统CNN方法大幅提升计算效率

<details>
  <summary>Details</summary>

**Motivation:** 植物物候学对理解生态系统和气候变化响应至关重要，但现有UAV和近地面相机监测方法中的物种识别计算复杂，传统多时间卷积网络架构僵化，随时间序列扩展性能差

**Method:** 对ViT进行全面的消融研究，分析7个关键设计维度：数据归一化、光谱排列、边界处理、空间上下文窗口形状和大小、标记化策略、位置编码和特征聚合策略

**Result:** 在巴西Cerrado生物群落数据集上验证，ViT方法将浮点运算量降低一个数量级，且参数复杂度保持恒定，而CNN基线随时间序列长度线性增长

**Conclusion:** ViTs是资源受限物候监测系统的稳健、可扩展解决方案，在保持竞争性分类性能的同时显著提升计算效率
...
### [61] [Beyond Visual Fidelity: Benchmarking Super-Resolution Models for Large-Scale Remote Sensing Imagery via Downstream Task Integration](https://arxiv.org/abs/2605.00310)
*Zhili Li,Kangyang Chai,Zhihao Wang,Xiaowei Jia,Yanhua Li,Gengchen Mai,Sergii Skakun,Dinesh Manocha,Yiqun Xie*

Main category: cs.CV

> 本文提出了GeoSR-Bench基准数据集，首次将下游任务评估整合到超分辨率(SR)模型评测中，通过大量实验证明传统SR指标与下游任务性能不相关。

<details>
  <summary>Details</summary>

**Motivation:** 现有超分辨率研究主要使用PSNR、SSIM等保真度指标评估，但SR图像的真正价值在于支撑下游地球监测任务，需要建立更贴近实际应用的评估体系。

**Method:** 构建GeoSR-Bench数据集，包含36,000个空间配准、时间对齐的图像对，分辨率从500m到0.6m；在270种设置下对GAN、transformer、神经算子和扩散模型进行多维度评测。

**Result:** 传统SR指标的改进与下游任务性能无正相关关系，甚至可能负相关，说明现有指标无法有效指导面向下游应用的SR模型选择。

**Conclusion:** 需要在SR模型开发和评估中整合下游任务，建立任务导向的评估体系对实际应用具有重要意义。
...
### [62] [Online Self-Calibration Against Hallucination in Vision-Language Models](https://arxiv.org/abs/2605.00323)
*Minghui Chen,Chenxu Yang,Hengjie Zhu,Dayan Wu,Zheng Lin,Qingyi Si*

Main category: cs.CV

> OSCAR是一个通过在线自校准框架来减少大型视觉语言模型幻觉问题的方法，利用蒙特卡洛树搜索和双重粒度奖励机制构建偏好数据进行直接偏好优化。

<details>
  <summary>Details</summary>

**Motivation:** 现有的偏好对齐方法依赖于更强模型(如GPT)的离线监督，导致了监督-感知不匹配问题：学生模型被迫学习超出其感知能力的细节，学会了猜测而非真正观察。

**Method:** 发现LVLMs中存在生成-判别差距，模型在判别验证任务上比开放式生成任务表现更好。基于此，提出OSCAR框架，集成蒙特卡洛树搜索与双重粒度奖励机制，构建偏好数据并通过直接偏好优化迭代改进模型。

**Result:** 实验表明OSCAR在幻觉基准测试中达到最优性能，同时提升了通用的多模态能力。

**Conclusion:** 通过利用模型自身的判别能力和在线学习机制，OSCAR有效地解决了LVLMs的幻觉问题，避免了传统方法中依赖外部监督的局限性。
...
### [63] [Pose-Aware Diffusion for 3D Generation](https://arxiv.org/abs/2605.00345)
*Zihan Zhou,Luxi Chen,Jingzhi Zhou,Yuhao Wan,Min Zhao,Baoyu Fan,Chongxuan Li*

Main category: cs.CV

> PAD是一种端到端扩散框架，直接在观测空间中合成姿态对齐的3D物体，通过将单目深度反投影为部分点云作为几何锚点，解决了传统方法的姿态模糊问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统的解耦式规范空间然后旋转的生成范式存在空间不匹配和变换模糊性问题，难以生成姿态对齐的高质量3D物体。

**Method:** 提出Pose-Aware Diffusion框架，将单目深度反投影为部分点云并作为3D几何锚点显式注入，在观测空间中直接进行端到端的3D几何合成。

**Result:** PAD在几何对齐和图像到3D对应关系方面优于现有方法，能够生成高保真度的姿态对齐资产，并可通过简单组合独立生成的物体扩展到组合式3D场景重建。

**Conclusion:**  PAD通过在观测空间中直接生成3D几何并利用深度锚点，有效解决了姿态模糊性问题，实现了精确的空间布局保持和高质量的3D物体合成。
...
### [64] [CURE-OOD: Benchmarking Out-of-Distribution Detection for Survival Prediction](https://arxiv.org/abs/2605.00350)
*Wenjie Zhao,Jia Li,Mingrui Liu,Jing Wang,Yunhui Guo*

Main category: cs.CV

> CURE-OOD是首个癌症生存预测中离群值检测(OOD)的系统基准，用于评估由成像采集变化导致的分布偏移对模型性能的影响。

<details>
  <summary>Details</summary>

**Motivation:** 癌症患者在诊断后最关心生存预期，现有基于CT图像的生存预测框架虽然表现良好，但成像采集的变异性导致的分布偏移会降低模型可靠性，而目前缺乏系统性研究OOD检测的基准。

**Method:** 构建CURE-OOD基准，定义基于扫描仪参数的训练、同分布和OOD测试集，涵盖四个生存预测任务，并提出HazardDev作为生存感知的OOD检测基线方法。

**Result:** 实验表明协变量偏移显著降低生存预测性能，主流面向分类的OOD检测器在生存预测任务中表现不佳，同时验证了CURE-OOD基准的有效性。

**Conclusion:** CURE-OOD基准能够系统分析分布偏移对下游生存性能和OOD可检测性的影响，为提高癌症生存预测模型的可靠性提供了重要工具。
...
### [65] [Time-series Meets Complex Motion Modeling: Robust and Computational-effective Motion Predictor for Multi-object Tracking](https://arxiv.org/abs/2605.00362)
*Nhat-Tan Do,Le-Huy Tu,Nhi Ngoc-Yen Nguyen,Dieu-Phuong Nguyen,Trong-Hop Do*

Main category: cs.CV

> 本文提出了一种基于时序卷积网络(TCN)的运动预测框架TCMP，在多目标跟踪任务中以极低计算成本(参数量仅为SOTA的1.4%，计算量仅为5%)实现了更高的跟踪精度。

<details>
  <summary>Details</summary>

**Motivation:** 当前多目标跟踪方法在处理复杂非线性运动时存在挑战，而现有的解决方案往往采用复杂且计算昂贵的生成模型，实用性受限。

**Method:** 提出了时序卷积运动预测器(TCMP)，采用包含扩张卷积和回归头的改进型时序卷积网络，能够在任意时间长度上下文中有效预测物体运动。

**Result:** 在多个关键指标上超越了之前的最佳方法：HOTA从62.3%提升到63.4%，IDF1从63.0%提升到65.0%，AssA从47.2%提升到49.1%，同时仅需1.4%的参数量和5%的计算代价。

**Conclusion:** 研究结果证明了TCMP方法在复杂跟踪环境中具有强大的适应性和效率，为推进多目标跟踪系统提供了一种既准确又高效的解决方案，挑战了需要复杂模型才能获得高性能的现有范式。
...
### [66] [Flow matching for Sentinel-2 super-resolution: implementation, application, and implications](https://arxiv.org/abs/2605.00367)
*Dakota Hester,Vitor S. Martins,Lucas B. Ferreira,Thainara M. A. Lima,Juliana A. Araújo*

Main category: cs.CV

> 本文提出了一种基于流匹配的卫星图像超分辨率方法，在10米分辨率的Sentinel-2图像上实现4倍超分辨率，优于扩散模型和GAN方法。

<details>
  <summary>Details</summary>

**Motivation:** 卫星图像超分辨率技术面临光谱保真度与感知质量之间的权衡问题，需要开发能够有效平衡这一矛盾的新方法。

**Method:** 使用120,851对Sentinel-2和NAIP图像训练流匹配模型，采用Euler方法和Midpoint求解器进行推理，生成超分辨率图像。

**Result:** 流匹配模型在单步采样中像素级准确率优于扩散和Real-ESRGAN模型，20步采样即可生成感知真实的超分辨率图像；生产了包含1.58万亿像素的CONUS区域超分辨率产品；土地覆盖分类总体准确率达89.11%。

**Conclusion:** 流匹配是Sentinel-2图像超分辨率的有效生成建模方法，在无需重新训练的情况下解决了感知-失真权衡，为需要精细空间细节的地理空间应用提供了高分辨率图像获取途径。
...
### [67] [RTPrune: Reading-Twice Inspired Token Pruning for Efficient DeepSeek-OCR Inference](https://arxiv.org/abs/2605.00392)
*Ben Wan,Yan Feng,Zihan Tang,Weizhe Huang,Yuting Zeng,Jia Wang,Tongxuan Liu*

Main category: cs.CV

> RTPrune是一种针对DeepSeek-OCR的双阶段视觉token剪枝方法，通过优先处理高范数token并基于最优传输理论合并剩余token，在保持99.47%准确率的同时实现1.23倍推理加速。

<details>
  <summary>Details</summary>

**Motivation:** DeepSeek-OCR虽然通过视觉-文本压缩降低了长文本处理成本，但视觉token仍存在冗余的文本和结构信息。现有VLM的token剪枝方法因压缩机制不当无法保持文本保真度。

**Method:** 提出双阶段剪枝方法：第一阶段优先处理捕捉显著文本和结构信息的高范数视觉token；第二阶段基于最优传输理论对剩余token进行配对合并实现高效特征聚合；引入动态剪枝比例适应token相似性和文本密度。

**Result:** 在OmniDocBench数据集上，应用于DeepSeek-OCR-Large时实现99.47%准确率和1.23倍预填充加速，token保留率为84.25%，达到业界领先性能。

**Conclusion:** RTPrune通过分析DeepSeek-OCR的两阶段阅读轨迹特征，提供了针对OCR任务定制的高效token剪枝解决方案，在效率-精度权衡上表现优异。
...
### [68] [SIMON: Saliency-aware Integrative Multi-view Object-centric Neural Decoding](https://arxiv.org/abs/2605.00401)
*YuSheng Lin,Ji-Hwa Tsai,Chun-Shu Wei*

Main category: cs.CV

> SIMON是一个新颖的显著感知多视图框架，通过结合前景分割和显著性预测选择注视中心，生成强调目标的中心凹视图，显著提升了EEG到图像检索的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有EEG到图像检索方法存在中心偏置问题，固定中心视角与内容驱动的人类注意力冲突，导致视觉特征与EEG响应之间存在几何-语义脱节。

**Method:** 提出显著感知采样(SAS)方法，结合前景分割和显著性预测选择注视中心，生成强调信息物体区域并抑制背景杂波的中心凹视图，构建多视图集成框架。

**Result:** 在THINGS-EEG数据集上达到SOTA性能，受试者内和跨受试者设置的Top-1准确率分别为69.7%和19.6%，在各分析维度下均展现出强鲁棒性。

**Conclusion:** 显著感知的多视图集成能有效解决几何-语义脱节问题，为EEG到图像检索提供了新思路，代码和模型已开源。
...
### [69] [BOLT: Online Lightweight Adaptation for Preparation-Free Heterogeneous Cooperative Perception](https://arxiv.org/abs/2605.00405)
*Kang Yang,Tianci Bu,Peng Wang,Deying Li,Yongcai Wang*

Main category: cs.CV

> BOLT是一种轻量级模块，通过在线知识蒸馏实现无需预先协调的异构多智能体协同感知，显著提升检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的异构协同感知方法需要离线联合训练或定制化适配，这在实际场景中不可行，因为不同开发者训练的智能体是独立且偶尔在线相遇的。

**Method:** 提出BOLT模块，利用自智能体预测作为教师信号进行在线知识蒸馏，通过高置信度自感知特征指导跨智能体特征域对齐，让邻近智能体在自智能体低置信度区域贡献有用特征。

**Result:** 在无需准备的设定下，BOLT仅用0.9M可训练参数就在AP@50指标上比原始未适配融合提升32.3点，且在不同编码器对和融合策略下始终超越仅自感知结果。

**Conclusion:** BOLT实现了真正的即插即用式异构协同感知，无需预部署协调即可有效融合独立训练的智能体特征，为实际应用提供了可行的解决方案。
...
### [70] [Beyond Heuristics: Learnable Density Control for 3D Gaussian Splatting](https://arxiv.org/abs/2605.00408)
*Zhenhua Ning,Xin Li,Jun Yu,Guangming Lu,Yaowei Wang,Wenjie Pei*

Main category: cs.CV

> LeGS通过强化学习的可学习策略替代3D高斯溅射中的手工密度控制规则，实现了更好的重建质量和效率平衡。

<details>
  <summary>Details</summary>

**Motivation:** 3D高斯溅射依赖启发式密度控制，这些手工规则缺乏适应复杂几何场景的灵活性，限制了其性能表现。

**Method:** 提出LeGS框架，将密度控制重构为参数化策略网络，通过强化学习优化。基于敏感性分析设计特定的奖励函数，并推导出闭式解将奖励计算复杂度从O(N²)降至O(N)。

**Result:** 在Mip-NeRF 360、Tanks & Temples和Deep Blending数据集上，LeGS显著优于现有方法，在重建质量和效率之间实现了更好的平衡。

**Conclusion:** 从刚性启发式到可学习策略的范式转变证明了强化学习在3D高斯溅射密度控制中的有效性，为适应多样化复杂场景提供了更灵活的解决方案。
...
### [71] [LIMSSR: LLM-Driven Sequence-to-Score Reasoning under Training-Time Incomplete Multimodal Observations](https://arxiv.org/abs/2605.00434)
*Huangbiao Xu,Huanqi Wu,Xiao Ke,Yuxin Peng*

Main category: cs.CV

> LIMSSR是一个基于LLM的不完整多模态学习框架，通过序列推理替代传统重建方法，在缺少完整训练数据的情况下实现高效的多模态融合。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态学习方法假设训练时所有模态都可用（'上帝视角'），但现实场景中训练数据也常常不完整，需要新的范式解决这个问题。

**Method:** 将不完整多模态学习重新表述为条件序列推理任务，采用提示引导的上下文感知模态填充和多维表示融合推理潜在语义，并引入掩码感知双路径聚合机制来动态校准推理不确定性。

**Result:** 在三个动作质量评估数据集上的实验表明，LIMSSR无需依赖完整训练数据就显著优于现有基线方法。

**Conclusion:** LIMSSR为数据高效的多模态学习建立了新的范式，证明了在不完整训练数据的现实约束下仍能实现高性能多模态学习。
...
### [72] [Scaling Video Understanding via Compact Latent Multi-Agent Collaboration](https://arxiv.org/abs/2605.00444)
*Kerui Chen,Jinglu Wang,Jianrong Zhang,Ming Li,Yan Lu,Hehe Fan*

Main category: cs.CV

> MACF是一个端到端的多智能体协作框架，通过本地预算智能体分割视频并使用隐式通信协议，实现了可扩展的长视频理解。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态大语言模型在长视频任务中面临感知上下文预算受限的问题，而现有的智能体方法通过基于规则的预处理往往会造成信息损失、成本高昂且依赖文本中介。

**Method:** 提出MACF框架，将视频分割给本地预算智能体，通过智能体原生的隐式通信协议实现整体推理。每个智能体将部分观察编码为紧凑的任务充分token，中央协调器通过共享嵌入空间实现高效且信息保留的协作。引入课程训练策略逐步强化语义对齐、证据总结和跨智能体协调。

**Result:** 在多个视频理解基准测试中，MACF在相同预算约束下始终优于最先进的MLLMs和多智能体系统，证明了隐式协作在可扩展视频理解中的有效性。

**Conclusion:** MACF通过解耦单个智能体的感知预算与全局视频复杂度，在保持视觉保真度的同时实现了可扩展的视频理解，为长视频理解任务提供了新的解决方案。
...
### [73] [Learning from Compressed CT: Feature Attention Style Transfer and Structured Factorized Projections for Resource-Efficient Medical Image Analysis](https://arxiv.org/abs/2605.00448)
*Shadid Yousuf,S. M. Mahbubur Rahman,Mohammed Imamul Hassan Bhuiyan*

Main category: cs.CV

> 本研究提出了CT-Lite框架，通过FAST和SFP技术，在处理JPEG压缩的胸部CT数据时实现了接近未压缩数据5-7%AUROC内的异常检测性能，同时大幅减少了计算资源需求。

<details>
  <summary>Details</summary>

**Motivation:** 人工智能在医学影像中的应用受到高计算复杂度和体积数据资源密集处理的阻碍。胸部CT虽然提供更丰富的诊断信息，但由于处理未压缩体积数据的计算负担，其在AI诊断中的应用仍然有限。

**Method:** 提出了特征注意力风格转移(FAST)框架，将激活模式和结构关系从高保真CT表示转移到压缩输入的时空视觉编码器；引入结构化因子投影(SFP)，利用块张量列车分解作为密集投影层的参数高效替代；构建基于SigLIP的多模态对比学习管道CT-Lite。

**Result:** 在三个数据集上的实验表明，CT-Lite在压缩输入下实现了与未压缩基线相比5-7%AUROC内的性能，同时使用了显著更少的参数。

**Conclusion:** 该研究为在资源约束条件下实现基于AI的临床评估铺平了道路，通过使用压缩的CT数据和高效的算法架构，成功平衡了计算效率与诊断准确性。
...
### [74] [From Local to Global to Mechanistic: An iERF-Centered Unified Framework for Interpreting Vision Models](https://arxiv.org/abs/2605.00474)
*Yearim Kim,Sangyu Han,Nojun Kwak*

Main category: cs.CV

> 本文提出一个以iERF为中心的统一框架，通过点状特征向量(PFV)和实例特定有效感受野(iERF)作为基本分析单元，整合了局部、全局和机制性可解释性方法。

<details>
  <summary>Details</summary>

**Motivation:** 现代视觉模型虽然准确率很高，但在解释证据来源、模型编码内容以及内部计算机制方面仍然缺乏统一的框架，各种可解释性方法相互独立且碎片化。

**Method:** 框架包含三个主要组件：1) 用于局部解释的共享比率分解(SRD)，通过iERF传播构建类别区分性显著图；2) 用于全局解释的概念锚定特征解释(CAFE)，利用iERF作为语义标签验证抽象特征；3) 用于机制性解释的层间概念图(ICAT)，量化概念间影响并识别集成梯度作为最忠实的实例化方法。

**Result:** 在ResNet50、VGG16和ViTs上的实验表明，该框架在保真度和鲁棒性方面均优于基线方法，能够成功解释分散的SAE特征，并揭示了正确分类、错误分类和对抗样本中的主导概念路径。

**Conclusion:** 基于iERF的方法提供了一个从像素到概念再到决策的连贯、有证据支持的解释映射框架，实现了对不同深度和架构的统一可解释性分析。
...
### [75] [Leveraging Vision-Language Models as Weak Annotators in Active Learning](https://arxiv.org/abs/2605.00480)
*Phuong Ngoc Nguyen,Kaito Shiku,Ryoma Bise,Seiichi Uchida,Shinnosuke Matsuo*

Main category: cs.CV

> 本文提出了一种结合视觉语言模型(VLMs)的主动学习框架，利用VLMs在粗粒度标签上的准确性来补充人工细粒度标注，通过实例级标签分配显著降低了标注成本。

<details>
  <summary>Details</summary>

**Motivation:** 主动学习旨在通过选择信息量大的样本进行标注来降低标注成本，但传统方法完全依赖昂贵的人工标注。研究者发现VLMs在不同粒度标签上表现差异显著：细粒度标签表现差，但粗粒度标签准确，这为利用VLMs减少人工标注依赖提供了机会。

**Method:** 提出一个主动学习框架，将细粒度人工标注与粗粒度VLM生成的弱标签通过实例级标签分配相结合。使用小量可信的全标签来建模VLM生成标签中的系统性噪声，提高标签质量。

**Result:** 在CUB200和FGVC-Aircraft数据集上的实验表明，在相同标注预算下，所提框架持续优于现有主动学习方法，验证了方法的有效性。

**Conclusion:** 通过巧妙结合人工细粒度标注和VLM粗粒度标签的优势，能够有效利用有限的标注资源，为细粒度识别任务提供了一种高效的标注策略。
...
### [76] [High-Speed Vision Improves Zero-Shot Semantic Understanding of Human Actions](https://arxiv.org/abs/2605.00496)
*Yongpeng Cao,Yuji Yamakawa*

Main category: cs.CV

> 本研究通过剑道动作案例，探讨了时间分辨率对零样本快速动作语义理解的影响，证明了高帧率视频能显著提升无监督语义识别的稳定性和可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 在人机交互中，对快速且难以标注的动作进行语义理解具有重要意义。由于收集足够标注数据困难，零样本方法成为实用的替代方案，但时间分辨率对快速动作识别的影响尚未得到充分研究。

**Method:** 提出了一个无训练流程，结合预训练视频-语言模型获取语义表示，并使用大语言模型进行成对动作比较。在120Hz、60Hz和30Hz三种帧率下进行对照实验，并分析了基于跟踪的人体关节信息在完整和部分观测场景中的作用。

**Result:** 实验表明，更高的时间分辨率在零样本设置下显著改善了语义可分性。使用最近类原型策略的定量评估证明，高速视频为快速动作提供了更稳定和可解释的语义表示。

**Conclusion:** 研究结果强调了时间分辨率在无训练动作识别中的重要性，表明高速感知能够增强语义理解能力，为快速人机交互应用提供了重要见解。
...
### [77] [GOR-IS: 3D Gaussian Object Removal in the Intrinsic Space](https://arxiv.org/abs/2605.00498)
*Yonghao Zhao,Yupeng Gao,Jian Yang,Jin Xie,Beibei Wang*

Main category: cs.CV

> 本文提出GOR-IS框架，通过在内在空间分解场景成分并显式建模光线传输，实现了物理一致且视觉连贯的3D物体移除，在非朗伯表面处理和全局光照效果保持方面显著优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3D场景物体移除方法忽略了全局光照效果，在处理视角依赖的非朗伯表面时效果不可靠，导致物理不一致的结果。亟需一种能同时保证光照一致性和非朗伯表面处理的解决方案。

**Method:** 该框架将场景分解为内在成分，显式建模光线传输以保持全局光照一致性，并引入在材质和光照领域直接操作的内在空间修复模块，有效应对非朗伯表面挑战。

**Result:** 在合成和真实数据集上的大量实验表明，该方法在物体移除的物理一致性方面显著提升，LPIPS指标改善13%，PSNR提升2dB，优于现有方法。

**Conclusion:** GOR-IS框架通过内在空间分解和光照建模，成功解决了3D物体移除中的光照一致性和非朗伯表面处理难题，为3D场景编辑提供了更物理合理的解决方案。
...
### [78] [End-to-End Autoregressive Image Generation with 1D Semantic Tokenizer](https://arxiv.org/abs/2605.00503)
*Wenda Chu,Bingliang Zhang,Jiaqi Han,Yizhuo Li,Linjie Yang,Yisong Yue,Qiushan Guo*

Main category: cs.CV

> 本文提出了一种端到端训练管道，将视觉分词器和自回归生成模型联合优化，在ImageNet 256x256生成任务上达到了1.48的FID分数，创下了无指导条件下的最新state-of-the-art。

<details>
  <summary>Details</summary>

**Motivation:** 传统的自回归图像建模采用两阶段方法，先训练分词器再训练生成模型，缺乏端到端的优化。研究者希望设计一个联合优化框架，让生成结果能直接监督分词器的训练过程。

**Method:** 设计端到端训练管道，将重构损失和生成损失联合优化，实现从生成结果到分词器的直接监督。同时探索利用视觉基础模型来改进1D分词器，提升自回归建模效果。

**Result:** 在ImageNet 256x256图像生成任务上，自回归模型达到了1.48的FID分数，这是无指导条件下的最佳性能表现，验证了所提方法的有效性。

**Conclusion:** 端到端的联合训练策略显著提升了自回归图像生成质量，证明了生成结果直接监督分词器的重要性，为视觉分词器的设计和优化提供了新的思路。
...
### [79] [PhysiGen: Integrating Collision-Aware Physical Constraints for High-Fidelity Human-Human Interaction Generation](https://arxiv.org/abs/2605.00517)
*Nan Lei,Yuan-Ming Li,Ling-An Zeng,Liang Xu,Zhi-Wei Xia,Hui-Wen Huang,Fa-Ting Hong,Wei-Shi Zheng*

Main category: cs.CV

> PhysiGen是一种通用的、计算高效的优化策略，通过简化人体网格为几何原语并识别碰撞区域来指导优化方向，有效解决了3D多人交互生成中普遍存在的身体相互穿透问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本驱动3D人体运动合成方法在生成多人交互序列时存在严重的身体相互穿透问题，这极大地影响了真实性和可用性。传统方法要么忽略此问题，要么引入计算昂贵的网格级损失函数。

**Method:** 提出PhysiGen优化策略：1）将高分辨率人体网格简化为几何原语以降低人际碰撞检测成本；2）识别碰撞区域作为优化方向的指导；3）通过即插即用方式集成到现有的人体交互生成模型中。

**Result:** 跨数据集和跨模型的大量实验表明，与最先进方法相比，PhysiGen能有效减少相互穿透，显著提高视觉连贯性和物理合理性。

**Conclusion:** PhysiGen为3D多人交互生成提供了一种高效、通用的物理约束解决方案，通过简化的几何表示和碰撞引导优化机制，在实际应用中展现出良好的效果和兼容性。
...
### [80] [IdentiFace: Multi-Modal Iterative Diffusion Framework for Identifiable Suspect Face Generation in Crime Investigations](https://arxiv.org/abs/2605.00526)
*Weichen Liu,Yixin Yang,Changsheng Chen,Alex Kot*

Main category: cs.CV

> IdentiFace是一个新颖的扩散模型框架，通过多模态输入和迭代生成管道实现了可识别的嫌疑人面部生成，在身份检索方面表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 嫌疑人面部生成在刑事侦查中仍面临技术挑战。传统的素描绘制工作流程效率低、质量差，而基于扩散模型的方法面临文本到图像模型的条件模糊性和一次性生成的采样方差等固有局限性。

**Method:** 提出了IdentiFace框架，通过(1)多模态输入设计来增强条件控制，(2)迭代生成管道实现可识别特征调整。此外贡献了面部身份损失函数和两个任务特定的数据集。

**Result:** 在合成数据集和真实场景进行的综合实验表明，IdentiFace相比现有方法实现了更优异的性能，特别是在身份检索方面，并在实际应用中展现出强大潜力。

**Conclusion:** IdentiFace通过创新的扩散模型框架有效解决了嫌疑人面部生成中的关键问题，为刑事侦查工具提供了重要的技术进步和实用价值。
...
### [81] [Vesselpose: Vessel Graph Reconstruction from Learned Voxel-wise Direction Vectors in 3D Vascular Images](https://arxiv.org/abs/2605.00538)
...
> 本文提出了一种基于体素级血管方向向量预测和改进TEASAR算法的方法，显著提高了3D血管网络重建的拓扑准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的'先分割后修复'范式在完整且拓扑准确的血管网络重建任务上存在根本性限制，无法有效处理血管网络的复杂拓扑结构和血管树分离问题。

**Method:** 该方法首先联合预测标准血管分割掩码和体素级血管方向向量，然后引入方向向量引导的TEASAR算法扩展版本从预测中提取血管图。

**Result:** 在三个基准数据集上实现了最先进的性能，成功应用于具有挑战性的大鼠心脏血管3D微CT扫描，能够分离紧密相邻的血管段并处理单一体积内的多个血管树。

**Conclusion:** 该方法显著提高了重建血管图的拓扑准确性，提出了可解释的拓扑误差度量指标（错误分割和错误合并），为血管网络重建提供了更可靠的解决方案。
...
### [82] [Colorful-Noise: Training-Free Low-Frequency Noise Manipulation for Color-Based Conditional Image Generation](https://arxiv.org/abs/2605.00548)
*Nadav Z. Cohen,Ofir Abramovich,Ariel Shamir*

Main category: cs.CV

> 本文发现扩散模型的输入噪声中，低频分量主要控制图像的整体结构和色彩，通过简单操控低频噪声即可有效引导生成过程，提出了一种无需训练的图像生成控制方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有扩散模型使用白高斯噪声作为输入，虽然能产生多样化的输出，但缺乏对视觉属性的可控性和可预测性，因为噪声本身无法被人理解。

**Method:** 研究分析了扩散模型输入噪声的特性，发现低频分量决定图像全局结构和色彩组成，高频分量控制细节。基于此观察，使用低频图像先验对低频噪声进行简单操控来 conditioning 生成过程。

**Result:** 提出了一种简单、无需训练、开销极小的方法，能够有效控制图像的整体结构和颜色，同时保持高频细节的多样性和可变性。

**Conclusion:** 通过操控扩散模型输入噪声的低频分量，可以在不重新训练模型的情况下实现对生成图像特征的有效控制，为图像生成提供了新的可解释性控制手段。
...
### [83] [Depth-Guided Privacy-Preserving Visual Localization Using 3D Sphere Clouds](https://arxiv.org/abs/2605.00562)
*Heejoon Moon,Jongwoo Lee,Jeonggon Kim,Je Hyeong Hong*

Main category: cs.CV

> 针对基于稀疏3D点云的视觉定位中的隐私泄露问题，本文提出了一种名为sphere cloud的隐私保护场景表示方法，通过将地图点提升为经过地图质心的3D线来抵御密度攻击，并结合ToF传感器深度图实现定位。

<details>
  <summary>Details</summary>

**Motivation:** 深度神经网络能够从稀疏3D点云中恢复高保真场景细节，这给涉及私有地图的视觉定位带来了严重的隐私担忧。传统的随机定向3D线方法容易受到密度攻击的威胁，需要更安全的隐私保护方案。

**Method:** 提出sphere cloud表示方法，将所有点提升为经过地图质心的3D线，形成类似单位球面上的点分布；设计简单的云构建策略抵御新型攻击；利用设备ToF传感器获取的绝对深度图来引导平移尺度估计。

**Result:** 在公开RGB-D数据集上的实验表明，sphere cloud在隐私保护能力和定位运行时间方面具有竞争力，且在位姿估计精度上与其他深度引导定位方法相比不会过度补偿。

**Conclusion:** sphere cloud通过巧妙的线密度分布有效中和了密度攻击，结合云构建策略和ToF深度图引导解决了新攻击和平移尺度问题，为隐私保护视觉定位提供了实用有效的解决方案。
...
### [84] [2D-SuGaR: Surface-Aware Gaussian Splatting for Geometrically Accurate Mesh Reconstruction](https://arxiv.org/abs/2605.00569)
*Prajwal Gupta C. R.,Divyam Sheth,Jinjoo Ha,Mirela Ostrek,Justus Thies*

Main category: cs.CV

> 本文通过融合单目深度和法线先验来改进2D高斯溅射技术，解决了其对SfM初始化敏感的问题，在DTU数据集上实现了最先进的网格重建效果

<details>
  <summary>Details</summary>

**Motivation:** 2D高斯溅射技术虽然在表面重建方面优于3DGS，但严重依赖SfM初始化，在挑战性图像集上容易产生较差的估计结果，影响重建质量

**Method:** 提出深度引导的高斯初始化策略，引入基于聚类的技术来剪枝退化高斯，并融入单目深度和法线先验信息来提高几何精度和鲁棒性

**Result:** 在DTU数据集评估中，该方法在保持高质量新视角合成的同时，实现了最先进的网格重建性能

**Conclusion:** 通过结合单目先验信息和改进的初始化/剪枝策略，有效提升了2DGS的几何准确性和鲁棒性，为实时高质量表面重建提供了更可靠的解决方案
...
### [85] [Federated Distillation for Whole Slide Image via Gaussian-Mixture Feature Alignment and Curriculum Integration](https://arxiv.org/abs/2605.00578)
*Luru Jing,Cong Cong,Yanyuan Chen,Yongzhi Cao*

Main category: cs.CV

> FedHD是一种新型的联邦学习框架，通过局部高斯混合特征对齐生成合成特征表示，实现了跨机构的病理切片分析协作训练，在多个基准数据集上超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的联邦学习在数字病理学领域面临挑战，主要问题在于不同机构使用多样化的多实例学习架构和特征提取器，导致系统异构性，难以有效进行协同训练。

**Method:** FedHD框架采用局部高斯混合特征对齐技术，每个客户端独立生成与真实WSI分布对齐的合成特征表示；采用一对一蒸馏策略保持诊断多样性；基于课程学习的集成策略逐步整合跨站点合成特征；可选的推理模块从合成嵌入中重建伪补丁。

**Result:** 在TCGA-IDH、CAMELYON16和CAMELYON17数据集上的实验表明，FedHD在性能上始终优于最先进的联邦学习和蒸馏基线方法。

**Conclusion:** FedHD提供了一个架构无关、隐私保护且支持个性化协作训练的解决方案，为跨机构的数字病理学协作研究提供了有效框架。
...
### [86] [Jailbreaking Vision-Language Models Through the Visual Modality](https://arxiv.org/abs/2605.00583)
*Aharon Azulay,Jan Dubiński,Zhuoyun Li,Atharv Mittal,Yossi Gandelsman*

Main category: cs.CV

> 研究发现视觉语言模型(VLMs)的视觉模态存在安全漏洞，通过四种视觉攻击方法可绕过安全对齐，揭示跨模态对齐缺失问题。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型的视觉模态尚未被充分研究作为攻击面，现有基于文本的安全训练无法自动泛化到视觉传达的恶意意图。

**Method:** 提出四种利用视觉组件的越狱攻击：视觉符号序列编码、有害物体替换、图像中恶意文字替换、视觉类比谜题，并在六个前沿VLMs上进行评估。

**Result:** 视觉攻击成功绕过安全对齐，如视觉密码攻击对Claude-Haiku-4.5的成功率达40.9%，远高于等效文本密码的10.7%，证实存在跨模态对齐差距。

**Conclusion:** 研究强调了视觉应是VLM安全对齐的一等目标，需在安全后训练中同等重视视觉模态的安全防护。
...
### [87] [Intrinsic Gradient Suppression for Label-Noise Prompt Tuning in Vision-Language Models](https://arxiv.org/abs/2605.00591)
*Jiayu Li,Jiaxin Qi,Sheng Zhou,Jiaqiang Huang,Xiansheng Hua*

Main category: cs.CV

> 本文提出DSPT方法，通过双softmax实现梯度自适应抑制，显著提升了CLIP在标签噪声下的提示调优鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** CLIP等对比视觉-语言模型具有出色的零样本泛化能力，但在标签噪声环境下，提示调优对噪声样本产生的过大梯度极其敏感，这些梯度会覆盖预训练先验知识，影响模型性能。

**Method:** 提出双softmax提示调优(DSPT)方法，通过顺序概率归一化创造自适应饱和区域，无需超参数即可实现梯度抑制。该方法将高误差噪声样本的梯度信息滤波，同时保持有效样本的更新。

**Result:** 在多个噪声基准测试中，DSPT达到了最先进的鲁棒性性能，超越了需要复杂架构和手工调整超参数的方法。

**Conclusion:** 该设计将传统的'梯度消失'缺陷转变为面向噪声提示调优的噪声过滤屏障，验证了保守适配策略在噪声环境中的有效性。
...
### [88] [Robust Fusion of Object-Level V2X for Learned 3D Object Detection](https://arxiv.org/abs/2605.00595)
*Lukas Ostendorf,Lennart Reiher,Onn Haran,Lutz Eckstein*

Main category: cs.CV

> 该研究通过将V2X通信信息集成到3D目标检测中，显著提高了自动驾驶系统的感知性能，并提出了抗噪声训练策略来应对现实V2X系统的不完善性。

<details>
  <summary>Details</summary>

**Motivation:** 基于车载传感器的自动驾驶感知存在视距和视野限制，在遮挡或能见度差的情况下容易失效。V2X通信可以作为车载感知的补充，但需要研究如何有效集成并应对现实世界中的噪声、延迟和低渗透率等问题。

**Method:** 使用nuScenes数据集模拟V2X协同感知消息，注入控制噪声和物体丢失来模拟真实条件。将消息转换为鸟瞰图输入并融合到BEVFusion风格检测器中，提出了噪声感知训练策略和显式置信度编码方法。

**Result:** 理想条件下NDS达到0.80，但传统模型在噪声下表现脆弱。提出的噪声感知训练方法在严重噪声和低V2X渗透率下仍能保持性能提升，显著增强了系统鲁棒性。

**Conclusion:** V2X协同信息可以大幅提升3D目标检测性能，但必须考虑现实系统的不完善性。噪声感知训练和置信度编码是构建鲁棒V2X感知系统的关键技术，为实际部署提供了可行方案。
...
### [89] [Faithful Extreme Image Rescaling with Learnable Reversible Transformation and Semantic Priors](https://arxiv.org/abs/2605.00605)
*Hao Wei,Yanhui Zhou,Chenyang Ge,Saeed Anwar,Ajmal Mian*

Main category: cs.CV

> FaithEIR是一个基于扩散模型的极端图像缩放框架，通过可逆变换和自适应细节先验解决了16倍以上极端缩放中的信息丢失问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的极端图像缩放方法在16倍及以上的缩放因子下，难以保持语义一致的结构并产生逼真的细节，因为低分辨率到高分辨率的映射本质上是严重不适定的。

**Method:** 受奇异值分解启发，开发了可学习的可逆变换实现潜在空间的可逆缩放；提出自适应细节先验作为高频词典补偿量化信息损失；设计轻量级像素语义嵌入器为预训练扩散模型提供语义条件。

**Result:** 大量实验结果表明，FaithEIR在重建保真度和感知质量方面持续超越现有最先进方法，实现了更优越的性能。

**Conclusion:** FaithEIR通过创新的扩散模型框架和关键技术设计，有效解决了极端图像缩放的核心挑战，为相关领域提供了一个强大的解决方案。
...
### [90] [CMTA: Leveraging Cross-Modal Temporal Artifacts for Generalizable AI-Generated Video Detection](https://arxiv.org/abs/2605.00630)
*Hang Wang,Chao Shen,Chenhao Lin,Minghui Yang,Lei Zhang,Cong Wang*

Main category: cs.CV

> 本文提出了一种名为CMTA的跨模态检测方法，通过分析AI生成视频中语义对齐的时间稳定性特征来鉴别视频真实性，该方法在多个大数据集上取得了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的AI生成视频检测方法主要关注单模态或时空伪影，忽视了视觉-文本跨模态空间中丰富的线索，特别是语义对齐的时间稳定性特征，导致检测效果有限。

**Method:** CMTA框架利用BLIP生成帧级图像描述，使用CLIP提取视觉-文本表示，并通过粗粒度GRU分支和细粒度Transformer编码器分支分别建模跨模态对齐的时间波动和帧间变化，捕捉AI生成视频独特的跨模态时间伪影。

**Result:** 在GenVideo、EvalCrafter、VideoPhy和VidProM四个大型数据集的40个子集上的广泛实验表明，CMTA方法达到了新的最先进水平，并展现出优越的跨生成器泛化能力。

**Conclusion:** 本文首次识别并利用AI生成视频中跨模态时间伪影这一独特指纹进行检测，提出的方法充分利用了跨模态语义轨迹的时间稳定性特征，为AI生成视频检测提供了新的思路和有效解决方案。
...
### [91] [BlenderRAG: High-Fidelity 3D Object Generation via Retrieval-Augmented Code Synthesis](https://arxiv.org/abs/2605.00632)
*Massimo Rondelli,Francesco Pivi,Maurizio Gabbrielli*

Main category: cs.CV

> BlenderRAG是一个基于检索增强生成的系统，通过利用500个专家验证的多模态示例，将LLM生成Blender代码的编译成功率从40.8%提升到70.0%。

<details>
  <summary>Details</summary>

**Motivation:** 当前最先进的LLM在从自然语言生成可执行的Blender代码时存在频繁的语法错误和几何不一致性问题，限制了自动化3D建模的发展。

**Method:** 构建了一个包含500个专家验证示例的多模态数据集（文本、代码、图像），覆盖50个对象类别，系统在生成过程中检索语义相似的示例来辅助代码生成。

**Result:** 在四个最先进的LLM上，编译成功率从40.8%提升到70.0%，语义标准化对齐度（CLIP相似度）从0.41提升到0.77，且无需微调或专用硬件。

**Conclusion:** BlenderRAG提供了一种即时可部署的有效解决方案，显著改善了自然语言到Blender代码的生成质量，为自动化3D内容创作提供了实用工具。
...
### [92] [UniVidX: A Unified Multimodal Framework for Versatile Video Generation via Diffusion Priors](https://arxiv.org/abs/2605.00658)
...
> UniVidX是一个统一的多模态视频生成框架，通过随机条件掩码、解耦门控LoRA和跨模态自注意力三个关键设计，实现了一个模型处理多种视频相关任务，在少样本训练下达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频扩散模型针对每个问题任务训练独立模型，固定了输入输出映射关系，限制了跨模态相关性的建模能力。需要构建统一的框架来处理多样化的多模态图形任务。

**Method:** UniVidX基于三个核心设计：1) 随机条件掩码(SCM)随机划分模态为条件与目标，实现全向条件生成；2) 解耦门控LoRA(DGL)为每个模态引入专用LoRA，保持VDM主干网络强先验；3) 跨模态自注意力(CMSA)共享键值但保留模态特定查询，促进模态间信息交换。

**Result:** 在UniVid-Intrinsic(RGB视频与内蕴图)和UniVid-Alpha(混合RGB视频与RGBA层)两个领域实例化，实验表明即使训练数据少于1000个视频，也能在多个不同任务上达到与SOTA方法竞争的性能，并对野外场景表现良好的泛化能力。

**Conclusion:** UniVidX成功实现了统一的视频多模态生成框架，通过创新的技术设计在保持模型泛化能力的同时大幅提升了任务适应性，为视频领域的多模态处理提供了有效解决方案。
...
### [93] [InpaintSLat: Inpainting Structured 3D Latents via Initial Noise Optimization](https://arxiv.org/abs/2605.00664)
*Jaeyoung Chung,Suyoung Lee,Kyoung Mu Lee*

Main category: cs.CV

> 本文提出了一种基于初始噪声优化的免训练可控3D修复方法，通过优化初始噪声实现高保真度的3D修复效果。

<details>
  <summary>Details</summary>

**Motivation:** 在结构化3D潜在扩散框架中，底层几何结构在扩散过程早期建立且对初始噪声高度敏感，这种特性影响了修复和编辑任务的稳定性，需要保证合成的新结构与现有内容严格对齐。

**Method:** 在结构化3D潜在扩散框架内优化初始噪声，基于整流流模型通过反向传播近似更新初始噪声，并采用专门设计的谱参数化实现鲁棒和高效的结构化3D潜在优化。

**Result:** 实验证明，该方法在上下文一致性和提示对齐方面相比代表性免训练修复基线方法有持续改进，确立了初始噪声控制作为3D修复的独立维度。

**Conclusion:** 初始噪声优化为3D修复提供了一种独立于传统采样轨迹操控的新型控制维度，有效提升了修复质量和稳定性。
...
### [94] [Prediction of Alzheimer's Disease Risk Factors from Retinal Images via Deep Learning: Development and Validation of Biologically Relevant Morphological Associations in the UK Biobank](https://arxiv.org/abs/2605.00665)
*Seowung Leem,Yunchao Yang,Adam J. Woods,Ruogu Fang*

Main category: cs.CV

> 该研究使用深度学习模型从彩色眼底照片中预测12种阿尔茨海默病相关风险因素，并识别出这些预测背后的视网膜结构特征，发现眼底的神经头和视网膜血管区域与AD易感性相关。

<details>
  <summary>Details</summary>

**Motivation:** 系统性、代谢性和生活方式因素已被证实与AD相关，但彩色眼底照片(CFP)是否包含与这些AD风险领域相对应的视网膜结构特征尚不明确。需要评估CFP是否能反映AD易感性的路径。

**Method:** 使用UK Biobank的62,876张CFP图像训练深度学习模型，预测12种AD相关因素（6个分类变量和6个连续变量），评估模型性能、模型显著性及显著性衍生得分，并与视网膜形态测量学进行比较。

**Result:** DL模型性能优异，分类因素AUROC为0.5654-0.9480，连续因素R²为-0.0291-0.7620，显著优于多数形态测量-机器学习模型。显著性分析聚焦于具有生物学意义的区域，特别是神经头和视网膜血管。

**Conclusion:** CFP编码了与AD风险因素相关的视网膜特征。虽然不具备诊断价值，但DL衍生的视网膜表征可能揭示具有生物学意义的风险相关结构变化，反映潜在的AD易感性。
...
### [95] [DMDSC: A Dynamic-Margin Deep Simplex Classifier for Open-Set Recognition on Medical Image Datasets](https://arxiv.org/abs/2605.00675)
*Vishal,Arnav Aditya,Nitin Kumar,Saurabh J. Shigwan*

Main category: cs.CV

> 本文提出DMDSC框架，通过动态边界机制解决医学影像数据集中的类别不平衡和开放集识别问题，在多个医学数据集上超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 医学影像数据集存在极端的类别不平衡问题，罕见病理样本显著少于常见病症，这对开放集识别(OSR)构成双重挑战：模型既要保持对已知类别的分类准确性，又要可靠地拒绝训练中未见过的样本。现有的DSC和UCDSC方法使用统一边界，未能考虑医学类别的不同密度分布。

**Method:** 提出DMDSC增强框架，采用动态边界方法，根据标签频率自动调整类别特定边界。对罕见病理设定更高的惩罚和更紧密的特征聚类，以抵消数据不平衡的影响。

**Result:** 在BloodMNIST、OCTMNIST、DermaMNIST和BreaKHis等多个医学基准数据集上的广泛实验表明，该框架在性能上超越了最先进的方法。

**Conclusion:** DMDSC通过自适应动态边界机制有效解决了医学影像中的类别不平衡和开放集识别问题，为医学图像分析提供了更鲁棒的解决方案。
...
> 本文提出了ViTCG模型，首个基于基础AI模型的气溶胶光学厚度(AOD)反演方法，通过视觉Transformer和通道分组的回归框架，实现了高精度且空间上一致的AOD反演。

<details>
  <summary>Details</summary>

**Motivation:** 传统的AOD反演方法依赖复杂的辐射传输建模和查找表，计算密集且需要辅助气象数据；现有数据驱动方法未能充分利用高光谱图像的空间-光谱相干性，导致反演结果空间不一致且对噪声敏感。

**Method:** 提出ViTCG框架，使用Vision Transformer架构结合通道分组策略，以高光谱大气层顶辐射率作为输入，联合建模空间上下文和光谱信息，通过空间回归减少反演偏差和误差。

**Result:** 在PACE辐射观测数据上的验证表明，ViTCG相比现有的最先进基础模型(包括Prithvi)均方误差减少62%，并生成了空间上一致的AOD场。

**Conclusion:** ViTCG成功将基础AI模型引入AOD反演领域，为高光谱遥感的大气参数反演提供了新思路，在精度和空间一致性方面均有显著提升。
...
### [97] [Static and Dynamic Graph Alignment Network for Temporal Video Grounding](https://arxiv.org/abs/2605.00684)
*Zhanjie Hu,Bolin Zhang,Jianhua Wang,Jianbo Zheng,Chenchen Yan,Takahiro Komamizu,Ichiro Ide,Jiangbo Qian*

Main category: cs.CV

> SDGAN是一种新型时序视频定位方法，通过静态-动态图对齐、查询感知建模和渐进式训练策略显著提升了TVG性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有时序视频定位方法存在三个关键瓶颈：视觉表示不完整、图构建与查询无关、单粒度语义匹配效率低下，这些问题限制了TVG的准确性和效率。

**Method:** SDGAN采用三重创新策略：1) 静态-动态双图构建与位置节点对齐获得更强的视觉表示；2) 查询-片段对比学习和自适应图建模实现查询感知；3) 多粒度时序建议配合渐进式从易到难训练策略优化定位精度。

**Result:** 在三个基准数据集上的广泛实验表明，SDGAN在复杂TVG场景中实现了优越性能，代码和数据集已公开。

**Conclusion:** 该研究有效解决了GCN-based TVG方法的关键限制，为时序视频定位任务提供了更完整、更精确的解决方案，展现了在复杂场景下的强大性能。
...
### [98] [PhysEdit: Physically-Consistent Region-Aware Image Editing via Adaptive Spatio-Temporal Reasoning](https://arxiv.org/abs/2605.00707)
*Guandong Li,Mengxia Ye*

Main category: cs.CV

> PhysEdit是一个适应性图像编辑框架，通过动态调整推理深度和空间范围，在保持编辑质量的同时实现了1.18倍的速度提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于推理的图像编辑方法对所有类型的编辑指令使用固定的推理模式，但不同编辑任务（如颜色交换、物体插入、物理动作编辑）需要不同的空间覆盖范围和推理深度，缺乏自适应性导致效率低下。

**Method:** PhysEdit引入两个推理时模块：1) CARD（复杂度自适应推理深度）根据指令和参考图像预测编辑复杂度，动态分配推理步数N_r和推理令牌长度r；2) SRM（空间推理掩码）通过交叉注意力提取指令条件化的空间先验，将推理限制在语义相关区域。这两个模块无需重新训练主干网络即可组合使用。

**Result:** 在737个样例的ImgEdit基础编辑套件上，PhysEdit相比强力基线实现了1.18倍加速（每样本64.3秒 vs 76.1秒），同时略微提升指令遵循性（CLIP-T 0.2283 vs 0.2266，+0.7%），在身份保持方面达到相近水平（CLIP-I 0.8246 vs 0.8280）。在外观级编辑上加速效果更明显，达到1.52倍。

**Conclusion:** 通过在空间和时间维度引入自适应性，PhysEdit成功将固定推理模式转换为条件计算问题，证明了动态资源分配是图像编辑效率提升的关键因素，为推理型编辑器的设计提供了新的研究方向。
...
### [99] [Learning Coarse-to-Fine Osteoarthritis Representations under Noisy Hierarchical Labels](https://arxiv.org/abs/2605.00718)
*Tongxu Zhang*

Main category: cs.CV

> 本文通过简单的双头模型探索膝关节OA评估中的层级监督先验，发现二分OA决策和KL分级联合训练能改善疾病表征，提升KL相关指标并增强与软骨的解剖学对齐。

<details>
  <summary>Details</summary>

**Motivation:** 现有的膝骨关节炎(AOA)评估深度学习方法通常将二分OA决策和细粒度KL分级视为独立的分类问题，未能充分利用临床自然的标签层级结构作为表示级别的监督先验。

**Method:** 采用简单的双头模型结构，包含共享编码器和两个任务特定头部，分别用于OA二分类和KL分级。在相同的测试协议下，比较单OA、单KL和双头训练在多个3D骨干网络上的性能表现。

**Result:** 双头监督产生骨干网络相关的增益，特别在KL相关指标上有明显改进。同时带来更有序的从粗到细的潜在组织结构，对某些骨干网络还能增强显著图与软骨区域的解剖学对齐。

**Conclusion:** 即使简单的层级双头监督也能在噪声粗/细标签下重塑疾病表征，为OA诊断和严重程度分级提供有用的归纳偏置。
...
### [100] [Unpaired Image Deraining Using Reward-Guided Self-Reinforcement Strategy](https://arxiv.org/abs/2605.00719)
*Yinghao Chen,Yeying Jin,Xiang Chen,Yanyan Wei,Ziyang Yan,Yaowen Fu*

Main category: cs.CV

> RGSUD是一种基于奖励引导自增强的无监督图像去雨方法，通过动态回收高质量去雨结果并引导模型优化，在多种数据集上达到最优性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统无监督去雨方法因缺乏强约束而难以收敛。研究发现训练过程中偶尔会出现高质量去雨结果，可以利用这些结果来指导优化过程。

**Method:** RGSUD包含两个关键阶段：1）基于图像质量评估的动态奖励回收机制，选择训练过程中的最优去雨输出；2）自增强训练，将回收的高质量结果纳入模型优化过程，约束优化空间并提高对齐度。

**Result:** 在多种数据集（包括配对合成、配对真实和非配对真实图像）上实现了SOTA性能，在主观和客观指标上均超越现有无监督去雨方法。

**Conclusion:** 自增强策略可适配其他无监督去雨方法，且该框架展现出对现有监督去雨网络的强泛化能力，证明了方法的普适性和有效性。
...
### [101] [Exploring the Limits of End-to-End Feature-Affinity Propagation for Single-Point Supervised Infrared Small Target Detection](https://arxiv.org/abs/2605.00722)
*Qiancheng Zhou,Wenhua Zhang*

Main category: cs.CV

> 本文提出了一种名为GSACP的单点监督红外小目标检测方法，通过在线特征亲和传播生成伪标签，实现了端到端训练，在保持较高准确率的同时显著降低了虚警率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的单点监督红外小目标检测方法依赖显式、离线的伪标签构建过程，流程复杂且计算开销大。作者寻求一种更简洁的替代方案，能够在线生成点对掩码的监督信息，避免外部标签演化循环。

**Method:** GSACP通过批内特征亲和传播在线生成监督信号，直接使用硬边际特征亲和度监督检测器。为解决自参考传播漂移问题，系统性地引入了局部EMA教师解耦、硬背景对比分离和自适应支持几何等优化策略。

**Result:** 在SIRST3数据集上，GSACP-Final实现了0.6674的mIoU，相比PAL方法相对减少了38%的虚检率，建立了新的超低虚警运行状态。

**Conclusion:** 批内特征传播为虚警抑制至关重要的部署场景提供了一种紧凑的替代方案。通过对端到端范式的系统性解构，该方法证明了简化的设计仍能实现具有竞争力的性能。
...
### [102] [Quantum Gradient-Based Approach for Edge and Corner Detection Using Sobel Kernels](https://arxiv.org/abs/2605.00744)
...
> 本研究实现了基于Sobel算子的边缘检测和Harris角点检测的量子版本，使用FRQI和QPIE两种量子图像编码方法，并通过经典后处理提高检测质量，证明了量子方法与经典方法的一致性。

<details>
  <summary>Details</summary>

**Motivation:** 随着量子计算的发展，探索经典计算机视觉算法的量子实现具有重要意义。边缘和角点检测是计算机视觉的基础任务，其量子加速实现可能为未来量子视觉系统奠定基础，同时展示量子计算在图像处理领域的潜力。

**Method:** 采用FRQI和QPIE两种量子图像编码方法，提出基于lag-2差分的量子梯度计算方案，实现量子梯度特征的叠加态计算。对量子电路识别的候选角点添加经典后处理步骤以提高检测质量并减少误报。

**Result:** 量子电路的输出与经典Sobel和Harris算子结果一致；QPIE方案在有限测量次数下比FRQI方案表现更稳定；梯度计算在电路层面可高效执行，但总体成本仍受量子态制备、测量和经典后处理主导。

**Conclusion:** 该工作展示了经典边缘和角点检测方法的量子实现具有功能性和可扩展性，虽然不是端到端的加速方案，但为量子计算机视觉算法的研究提供了重要参考，未来的NISQ硬件实现需要考虑噪声和测量限制的影响。
...
### [103] [Modeling Subjective Urban Perception with Human Gaze](https://arxiv.org/abs/2605.00764)
*Lin Che,Xi Wang,Marc Pollefeys,Konrad Schindler,Martin Raubal,Peter Kiefer*

Main category: cs.CV

> 本文提出了Place Pulse-Gaze数据集和凝视引导的城市感知框架，研究表明将人类眼动行为（凝视）与场景表示相结合能够更准确地建模主观城市感知。

<details>
  <summary>Details</summary>

**Motivation:** 现有的城市感知计算方法主要直接从街景图像建模，但忽略了形成这些判断的人类感知过程。为了更好地理解人们如何主观评价城市环境，需要将人类的感知过程（特别是眼动行为）纳入计算模型。

**Method:** 构建了Place Pulse-Gaze数据集，包含街景图像、同步的眼动记录记录和个体感知标签。提出了凝视引导的城市感知框架，系统研究了三种互补设置：仅凝视建模、凝视与显式语义场景表示融合、凝视与隐式更丰富视觉表示融合。

**Result:** 实验表明，仅凝视就包含了对主观城市感知有用的预测信号，而将凝视与场景表示融合无论是在语义还是更丰富的视觉表示下都能进一步提高预测准确性。

**Conclusion:** 研究结果突出了将人类感知过程纳入城市场景理解的重要性，为凝视引导的多模态城市计算开辟了新方向，强调了眼动行为在城市感知建模中的价值。
...
### [104] [Map2World: Segment Map Conditioned Text to 3D World Generation](https://arxiv.org/abs/2605.00781)
*Jaeyoung Chung,Suyoung Lee,Jianfeng Xiang,Jiaolong Yang,Kyoung Mu Lee*

Main category: cs.CV

> Map2World是一个3D世界生成框架，它允许用户基于任意形状和尺度的分段图生成全局尺度一致的3D世界，并通过细节增强网络添加高质量细节。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3D世界生成方法受限于网格布局，且在整个世界中存在物体尺度不一致的问题，无法满足复杂环境下的灵活生成需求。

**Method:** 提出Map2World框架，首先根据用户定义的任意形状和尺度的分段图生成3D世界，确保全局尺度和环境灵活性；然后设计细节增强网络，通过融入全局结构信息生成精细细节，同时保持场景连贯性；整个流程利用资产生成器的强先验知识。

**Result:** 实验证明该方法在用户可控性、尺度一致性和内容连贯性方面显著优于现有方法，能够在更复杂的条件下生成高质量的3D世界。

**Conclusion:** Map2World框架成功解决了以往方法的网格布局限制和尺度不一致问题，通过分段图条件生成和细节增强，实现了灵活可控的大规模3D世界生成，并在有限训练数据下展现了强大的泛化能力。
...
### [105] [Make Your LVLM KV Cache More Lightweight](https://arxiv.org/abs/2605.00789)
*Xihao Chen,Yangyang Guo,Roger Zimmermann*

Main category: cs.CV

> LightKV是一种通过跨模态信息传递压缩视觉token嵌入的方法，将LVLMs的KV缓存减半，计算量减少40%，同时保持模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 直接在大型视觉语言模型中应用KV缓存会导致GPU内存开销过大，因为预填充阶段需要处理大量视觉token，现有方法无法有效解决这一问题。

**Method:** LightKV利用视觉token嵌入之间的冗余性，在文本提示引导下采用跨模态信息传递，在预填充阶段逐步聚合和压缩视觉token信息，实现提示感知的缓存压缩。

**Result:** 在8个开源LVLMs和8个公开数据集上的实验表明，仅使用55%的原始视觉token，LightKV可将视觉token KV缓存大小减半，计算量最多减少40%，并在保持通用性能的同时显著优于现有基线方法。

**Conclusion:** LightKV有效解决了LVLMs中KV缓存的内存开销问题，其提示感知的压缩策略为视觉语言模型的高效推理提供了新思路。
...
### [106] [GMGaze: MoE-Based Context-Aware Gaze Estimation with CLIP and Multiscale Transformer](https://arxiv.org/abs/2605.00799)
*Xinyuan Zhao,Yihang Wu,Ahmad Chaddad,Sarah A. Alkhodair,Reem Kateb*

Main category: cs.CV

> GMGaze是一种基于多尺度transformer架构的凝视估计方法，通过语义原型调节、早期融合和稀疏专家混合模块，在四个公共数据集上实现了SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有CNN、transformer和CLIP基础的凝视估计方法存在图像特征晚期融合、缺乏因子感知条件化以及容量扩展不切实际三大挑战。

**Method:** GMGaze首先通过四个学习的原型库调节CLIP全局图像嵌入，生成两个互补的上下文偏置全局token；然后在第一层与CLIP patch和CNN token进行早期统一融合；最后通过稀疏混合专家模块提供条件化计算容量，并采用对抗域适应技术促进跨域适应。

**Result:** 在MPIIFaceGaze、EYEDIAP、Gaze360和ETH-XGaze四个基准测试上，平均角度误差分别为2.49°、3.22°、10.16°和1.44°，在所有域内设置上超越先前基线，在跨域评估中提供SOTA结果。

**Conclusion:** GMGaze通过创新的语义原型调节、早期融合策略和条件化计算能力有效解决了现有凝视估计方法的关键缺陷，在域内和跨域设置中均实现了最先进的性能表现。
...
### [107] [Let ViT Speak: Generative Language-Image Pre-training](https://arxiv.org/abs/2605.00809)
*Yan Fang,Mengcheng Lan,Zilong Huang,Weixian Lei,Yunqing Zhao,Yujie Zhong,Yingchen Yu,Qi She,Yao Zhao,Yunchao Wei*

Main category: cs.CV

> GenLIP是一个极简的生成式预训练框架，通过直接使用语言建模目标训练ViT预测语言标记，实现了视觉编码器与LLM自回归特性的更好对齐。

<details>
  <summary>Details</summary>

**Motivation:** 传统的视觉-语言预训练方法依赖对比学习或额外的文本解码器，与LLM的自回归特性不匹配。GenLIP旨在通过更简单有效的方式对齐视觉编码器和语言模型。

**Method:** GenLIP使用单一Transformer模型联合建模视觉和文本标记，直接从视觉标记预测语言标记，采用标准语言建模目标，无需对比批处理构建或额外文本解码器。

**Result:** 在8B样本上训练的GenLIP，使用更少预训练数据就能达到或超越强基线模型。通过多分辨率图像持续预训练，在OCR和图表理解等细节敏感任务上进一步改进。

**Conclusion:** GenLIP以其简洁性、可扩展性和优异性能，成为MLLM中视觉编码器的强大基础，为多模态大语言模型提供了新的预训练范式。
...
### [108] [Persistent Visual Memory: Sustaining Perception for Deep Generation in LVLMs](https://arxiv.org/abs/2605.00814)
*Siyuan Huang,Xiaoye Qu,Yafu Li,Tong Zhu,Zefeng He,Muxin Fu,Daizong Liu,Wei-Long Zheng,Yu Cheng*

Main category: cs.CV

> 本文提出持久视觉记忆(PVM)模块，通过在大型视觉语言模型中建立独立的视觉检索路径，有效解决了自回归生成过程中的视觉信号稀释问题。

<details>
  <summary>Details</summary>

**Motivation:** 大型视觉语言模型(LVLMs)在生成长文本时，由于注意力分配函数随文本历史积累而扩大，导致视觉注意力与生成长度成反比衰减，出现'视觉信号稀释'现象，影响视觉感知的持续性。

**Method:** 提出持久视觉记忆(PVM)，作为轻量级可学习模块，与LVLMs中的前馈网络(FFN)并行集成。PVM建立距离无关的检索路径，直接提供视觉嵌入，确保持续按需的视觉感知，从结构上缓解深度生成固有的信号抑制。

**Result:** 在Qwen3-VL模型上的大量实验表明，PVM以可忽略的参数开销带来显著改进，在4B和8B规模模型上均实现一致的准确率提升，尤其在需要持久视觉感知的复杂推理任务中表现突出。

**Conclusion:** 深入分析表明，PVM能有效抵抗生成长度引发的信号衰减，加速内部预测收敛，为解决LVLMs在长序列生成中的视觉感知衰减问题提供了有效方案。
...
### [109] [Posterior Augmented Flow Matching](https://arxiv.org/abs/2605.00825)
...
> PAFM是一种改进的流匹配方法，通过对每个中间状态使用多个候选目标的期望来替代单一监督，有效解决了训练信号稀疏性和流坍塌问题，显著提升了图像生成的质量。

<details>
  <summary>Details</summary>

**Motivation:** 高维图像的流匹配训练中，每个样本仅监督单一路径的一个中间点，导致训练信号极其稀疏且方差过大，容易引发流坍塌现象，使得模型记忆特定的源-目标配对，无法泛化。

**Method:** 提出后验增强流匹配(PAFM)，将单一目标监督替换为对给定中间状态和条件的有效目标完成后验分布的期望。该方法将难处理的后验分解为中间状态在假设端点下的似然和该端点在条件下的先验概率，并使用重要性采样构建多个候选目标的混合。

**Result:** PAFM在不同模型尺度(SiT-B/2和SiT-XL/2)、不同架构(SiT和MMDiT)以及类别和文本条件基准测试(ImageNet和CC12M)上，相比原始FM提升多达3.4 FID50K，计算开销增加可忽略。

**Conclusion:** PAFM为流匹配提供了一种理论完备且实用的改进方案，通过对多个合理延续轨迹的信息聚合，既保持了原始目标的无偏估计，又显著降低了训练梯度方差，有效避免了流坍塌问题。
