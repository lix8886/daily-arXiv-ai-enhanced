...
> 论文提出了AITP（人工智能交警）模型，这是一个用于交通事故责任推理和分配的多模态大语言模型，并发布了DecaTARA基准数据集。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大语言模型在交通事故检测和理解方面取得了进展，但在因果推理和法律知识整合方面存在不足。交通责任分配（TARA）是一个更具挑战性的任务，需要基于交通法规的多步推理。

**Method:** 提出AITP模型，通过多模态思维链（MCoT）机制增强推理能力，并通过检索增强生成（RAG）技术整合法律知识。同时构建了DecaTARA基准，包含67,941个标注视频和195,821个问答对，覆盖10个相互关联的交通事故推理任务。

**Result:** 实验表明，AITP在责任分配、交通事故检测和理解等任务上达到了最先进的性能，为推理驱动的多模态交通分析建立了新的范式。

**Conclusion:** 该研究通过结合先进的推理机制和法律知识整合，成功解决了交通事故责任分配这一复杂问题，为智能交通系统的实际应用提供了重要突破。
...
> 研究创建了AFRILANGEDU数据集，从非洲语言-英文字典构建了7.89万条教学对话数据，用于训练AI语言辅导模型AFRILANGTUTOR，在10种非洲语言上显著提升了模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 非洲本地语言缺乏足够的训练资源，开发者面临构建理解当地语言的AI系统的挑战，需要解决低资源语言的学习材料生成问题。

**Method:** 首先构建包含19.47万个条目的AFRILANGDICT非洲语言-英文字典，基于该字典自动生成大规模、多样化、可验证的师生问答交互，形成78.9万个多轮训练样本，然后使用监督微调和直接偏好优化训练Llama-3-8B-IT和Gemma-3-12B-IT模型。

**Result:** 训练的AFRILANGTUTOR模型在10种非洲语言上的表现超越了基础模型，结合SFT和DPO方法带来1.8%到15.5%的性能提升（基于LLM-as-a-judge评估）。

**Conclusion:** 通过字典资源自动生成教学数据的方法有效解决了低资源语言的学习材料匮乏问题，为非洲等低资源语言的AI教育系统开发提供了可行路径。
...
> 本文提出HPO方法，通过层次化策略优化后训练模型，显著提升同声传译质量同时降低计算开销，在多种语言对上实现超过7 COMET分的提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于LLM的同声传译方法计算开销大，而多轮对话方法虽能减少计算成本但依赖高质量的对话形式监督微调数据，而这类数据稀缺且现有合成方法质量无法保证。

**Method:** 提出层次化策略优化(HPO)方法，使用在不完美SFT数据上训练的模型进行后训练，引入平衡翻译质量和延迟目标的层次化奖励机制。

**Result:** 在英译中/德/日任务上，延迟1.5秒时COMET分数提升超过7分，MetricX分数提升1.25分，消融实验验证了各组件的有效性。

**Conclusion:** HPO方法有效解决了LLM同声传译中数据质量不足的问题，通过层次化奖励机制实现了质量和延迟的平衡，为实际应用提供了feasible solution。
...
> TRACES框架通过实时标记推理步骤，实现语言推理模型的自适应高效早停，在保持准确率的同时减少20-50%的token消耗。
...
**Method:** 提出TRACES轻量级框架，实时标记推理步骤类型，监控推理行为变化，利用特定步骤类型作为可解释的早停标准。

**Result:** 在MATH500、GSM8K、AIME、MMLU和GPQA等五个基准测试上，实现了20-50%的token减少，同时保持了与标准生成相当的准确率。

**Conclusion:** TRACES框架有效解决了LRMs的过生成问题，监控特定推理步骤类型能够产生有效的早停策略，为语言推理模型提供了高效的推理优化方案。
...
> 本文提出了一种基于离散小波变换(DWT)的多分辨率框架，用于改进大语言模型在长文档领域特定摘要任务中的表现，显著减少了幻觉并提高了事实一致性。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在处理长篇领域特定文档(如临床和法律文本)时面临上下文限制、信息丢失和幻觉等问题，特别是在需要高度准确性的专业领域，现有方法难以同时保持全局结构和关键细节。

**Method:** 将文本视为语义信号，对句子级或词级嵌入应用离散小波变换(DWT)，将文本分解为全局(近似)和局部(细节)组件。这些紧凑的表示可以直接用作摘要或指导LLM生成，实现语义去噪效果。

**Result:** 在临床和法律基准测试上，方法取得了与GPT-4o相当的ROUGE-L分数，但语义相似度和事实一致性显著提升：BERTScore提高2%以上，语义保真度提高4%以上，METEOR指标大幅改善，事实一致性达到97%。

**Conclusion:** DWT框架为长文档和领域特定摘要提供了一种轻量级、可推广的解决方案，通过语义去噪机制有效减少幻觉并增强事实基础，为专业领域的可靠摘要生成开辟了新途径。
...
> 研究发现FHIR数据序列化策略对临床LLM的药物核对性能影响显著：小型模型(≤8B)在临床叙述格式下表现最佳，大型模型(70B)在原始JSON格式下最优；遗漏是主要错误模式；BioMistral-7B无应用价值。

<details>
  <summary>Details</summary>

**Motivation:** 临床交接中的药物核对是高风险、易出错的过程。虽然大语言模型被提议用于协助此任务，但FHIR数据在传递给模型前的序列化方式这一关键因素尚未得到系统研究。

**Method:** 研究对比了四种FHIR序列化策略(原始JSON、Markdown表格、临床叙述、时间线)在五个开源模型上的性能，使用200个合成患者的受控基准，总计4000次推理运行。

**Result:** 序列化策略对≤8B参数模型性能影响巨大：临床叙述比原始JSON提升最多19个F1点；但在70B模型中，原始JSON表现最佳(F1=0.9956)。所有模型组合中精确度都高于召回率，遗漏是主要失败模式。小模型在7-10个并发药物时达到瓶颈。

**Conclusion:** 为临床LLM部署提供了基于证据的格式建议：≤8B模型使用临床叙述格式，70B及以上模型使用原始JSON格式。研究发现遗漏错误占主导，这改变了临床安全审计的优先级设置。
...
> 本文通过加权损失函数重新聚焦临床重要token，显著提高了医学报告生成的数据效率，仅需1/10的数据量即可达到相似质量。

<details>
  <summary>Details</summary>

**Motivation:** 医学视觉语言模型训练面临高质量标注数据稀缺的问题，限制了模型性能提升。需要寻找方法提高数据利用效率。

**Method:** 提出加权损失函数，将预测重点从所有token平等对待转向临床重要的语义显著性token，在眼科报告生成任务中验证效果。

**Result:** 该方法在多个数据规模上都能提高效率，使用少至十分之一的训练数据就能达到与标准交叉熵损失相似的报告质量。

**Conclusion:** 简单的加权损失策略能有效解决医学报告生成中的数据稀缺问题，为临床应用提供了实用的数据处理方案。
...
> 该研究使用MARBERT模型预测阿拉伯语推文中的表情符号，在包含8695条推文的数据集上达到了0.75的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 研究旨在探索机器学习在预测阿拉伯语推文表情符号方面的能力，特别关注MARBERT模型在低资源和多方言阿拉伯语处理上的表现。

**Method:** 收集了11379条阿拉伯语推文，经过筛选得到8695条用于分析。将推特分为14个表情符号类别并数值编码。设计了可解释的预处理流程作为基线，对MARBERT模型进行微调以预测表情符号使用。使用精确率、召回率和F1分数评估模型性能。

**Result:** MARBERT模型表现良好，整体准确率达到0.75。研究表明模型能够有效预测阿拉伯语推文中的表情符号使用。

**Conclusion:** 研究结果令人鼓舞，但仍有必要改进包括MARBERT在内的机器学习模型，特别是针对像阿拉伯语这样的低资源、多方言语言。
...
> GRISP是一种基于微调小型语言模型的SPARQL问答方法，通过生成查询骨架并结合知识图谱约束的迭代选择机制，在标准基准测试中取得了优于其他先进方法的结果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的知识图谱问答方法存在效率和准确性的平衡问题，需要探索更有效的方法来处理自然语言问题的复杂性和知识图谱的约束关系。

**Method:** 首先使用小型语言模型生成自然语言的SPARQL查询骨架，然后通过知识图谱约束对图谱项目进行重排序和选择，迭代地替换自然语言占位符。该模型同时在骨架生成和列表式重排序数据上进行联合训练。

**Result:** 在Wikidata和Freebase的标准基准测试中，GRISP方法在可比设置下比其他最先进方法表现更好，实现了更高的问答准确率。

**Conclusion:** 通过结合小型语言模型的高效性和知识图谱约束的精确性，GRISP为知识图谱问答提供了一个既准确又高效的解决方案，证明了在有限的计算资源下也能取得优异性能。
...
> IVG框架通过结合规范内省和视图交互，解决了视觉语言模型在图表理解中的像素瓶颈问题，在交互式图表问答任务中达到最高准确率0.81。

<details>
  <summary>Details</summary>

**Motivation:** 当前视觉语言模型在理解图表时频繁出现数值误读、细节幻觉和重叠元素混淆等问题，核心原因是仅依赖像素解释，无法访问图表的结构化规范数据，这形成了像素瓶颈。

**Method:** 提出内省式交互视觉接地框架IVG，包含两个关键组件：(1)基于规范的内省，查询底层规范获取确定性证据；(2)基于视图的交互，通过操纵视图解决视觉模糊性。同时构建了iPlotBench评测基准，包含500个交互式Plotly图表和6706个二元问题。

**Result:** 实验证明内省提高了数据重建保真度，与交互结合后获得最高QA准确率0.81，在重叠几何体上有6.7%的性能提升。成功部署了能够自主探索数据并与人类实时协作的智能体系统。

**Conclusion:** IVG框架有效突破了像素瓶颈，通过规范查询和视图交互的结合显著提升了图表理解的准确性和可靠性，为交互式数据分析提供了新的技术路径。
...
> 本文提出了一个自动化课堂话语分析系统(ADAS)，用于分析师生在科学课堂中的推理模式。系统通过分层数据重分、LLM合成数据增强和双探针头RoBERTa分类器来解决标签不平衡问题，实现了师生话语的自动分类和认知复杂度分析。

<details>
  <summary>Details</summary>

**Motivation:** 科学课堂中学生的推理模式分析对理解知识建构机制和改进教学实践至关重要，但大规模的手动课堂话语编码极其耗时费力，需要开发自动化分析工具。

**Method:** 研发ADAS系统，实现话语类型(UT)和推理组件(RC)两维度联合分类；采用分层重分数据集、针对少数类的LLM合成数据增强、双探针头RoBERTa-base分类器；进行话语模式分析包括UTxRC共现、认知复杂度指数、滞后序列分析和IRF链分析。

**Result:** GPT-5.4零样本基线在UT和RC上分别获得0.467和0.476的macro-F1；LLM数据增强显著改善了UT少数类的识别；RC任务结构简单使得字面基线模型也能有效处理；发现教师反馈提问(Fq)是学生推理(SR-I)的最一致前导行为。

**Conclusion:** 研究表明LLM增强技术能有效解决话语分析中的类别不平衡问题，ADAS系统为大规模课堂认知参与度分析提供了实用工具，教师提问策略对促进学生推理具有关键作用。
...
> 语言模型通过分离的'当前实体'和'先前实体'槽位来编码多个实体信息，但模型只能有效利用当前实体槽位进行事实检索，而无法处理单个token承载多实体绑定的复杂情况。

<details>
  <summary>Details</summary>

**Motivation:** 研究语言模型如何在多个实体间建立和维护绑定关系，特别是单个token是否能承载多实体信息，这对理解模型的推理能力和局限性至关重要。

**Method:** 提出多槽位探测方法，将单个token的残差流激活分解为'当前实体'和'先前实体'信息，通过正交分析研究不同槽位的功能特征。

**Result:** 发现两个槽位服务于不同功能：当前实体槽用于事实检索，先前后槽支持关系推理但无法被模型有效利用；开源模型在处理单token多实体绑定时表现较差，而新兴前沿模型表现更好。

**Conclusion:** 揭示了激活信息可用性与模型实际使用之间的差距，当前/先前实体槽位结构为理解双向思维行为如阿谀和欺骗提供了自然基础。
...
> 本文提出了一种主动视觉支架框架，通过将对话状态增量转换为持久的视觉历史，并结合文本表示，有效解决了当前对话代理因表示模糊而导致的上下文跟踪失败问题。在IndiRef基准测试上取得了最佳性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前对话代理难以维持可靠的共享上下文表示，当共同基础需要超出即时上下文窗口时，细粒度区别会被压缩为纯文本表示，导致相似的实体崩溃为可互换描述，产生表示模糊和语义扁平化问题。

**Method:** 受人类心智图像推理启发，提出主动视觉支架框架，增量地将对话状态转换为持久视觉历史，后续可用于基础响应生成。该框架将可视化和文本表示相结合，形成混合多模态方法。

**Result:** 在IndiRef基准评估中，增量外部化本身优于全对话推理，视觉支架通过减少表示模糊和强制具体场景承诺提供额外增益。混合多模态设置获得最佳整体性能。

**Conclusion:** 对话代理受益于显式的多模态共同基础表示，该表示整合了描述性信息和命题信息。视觉支架能有效解决表示模糊问题，而文本表示对不可描述信息仍有优势。
...
> 该研究通过用户体验调查发现，ASR系统对不同英语方言使用者的失败不仅是技术问题，更造成了情感伤害，包括自我怀疑、持续的语言调整劳动和被边缘化的心理负担。

<details>
  <summary>Details</summary>

**Motivation:** 现有ASR偏见研究主要关注错误率统计，缺乏对用户主观体验的探讨，需要深入了解系统失败如何影响用户的情感、心理和日常生活。

**Method:** 研究人员在美国四个具有不同英语方言特色的地区（亚特兰大、墨西哥湾沿岸、迈阿密海滩和图森）进行用户体验研究，通过定性分析用户的开放式叙述来探索ASR失败对用户的影响。

**Result:** 发现参与者普遍认为技术未考虑其文化背景，需要进行语言切换、过度发音等隐性劳动来使系统正常工作；尽管意识到系统设计问题，仍会内化失败为个人不足；遭受挫折、烦恼和自我否定等情感伤害。

**Conclusion:** 仅基于准确性的算法公平性评估忽略了重复技术拒绝的情感劳动成本、持续自我监控的认知负担以及母语变体中感到不足的心理伤害等关键伤害维度。
...
> 提出了一种前缀语法变换方法，将前缀解析高效地转化为普通解析问题，并给出了计算下一个词权重向量的策略，构建了一个简洁通用的前缀解析框架。

<details>
  <summary>Details</summary>

**Motivation:** 前缀解析在上下文无关语言建模、心理语言学分析和基于大模型的句法约束生成中具有重要应用，但现有的前缀解析算法往往是专门设计的，缺乏通用性和实现效率。

**Method:** 设计前缀语法变换技术，通过构建一个新语法来精确生成原语法字符串的所有前缀集合，将前缀解析转换为常规解析问题；基于算法差异化提出计算下一个词权重向量的策略。

**Result:** 变换后的语法仅比输入语法大一个很小的因子，任何优化的解析算法都可以直接使用，无需专门的实现；提供了高效的下一个词预测能力。

**Conclusion:** 该框架通过优雅的变换和实用的算法选择，实现了简单、通用、高效的前缀解析解决方案，为相关应用提供了强大的工具支持。
...
> 本文提出一种新型两步推理方法，通过生成'理由'来增强大语言模型在未来职业预测中的性能，并通过微调使模型在职业路径推理上得到有效对齐。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型在职业预测任务中表现不佳，因为模型缺乏与实际职业路径的对齐，也无法理解每个职业决策背后的潜在原因。这导致了职业推荐的不准确性。

**Method:** 提出两步推理框架：首先通过理由生成器基于用户的教育和职业历史生成'理由'，然后使用该理由作为职业预测器的输入进行职业推荐。采用LLM作为评判者生成高质量的神谕理由，并用这些理由微调小型LLM来执行推理和预测任务。

**Result:** 实验表明：(a)该方法显著提高了LLM的职业预测准确性，达到与完全监督方法相当的水平，超越无监督方法；(b)单一LLM同时执行推理和预测优于两个独立微调的LLM；(c)职业预测准确性与生成理由的质量呈正相关。

**Conclusion:** 通过引入基于理由的推理机制和针对性的微调策略，有效解决了LLM在职业预测领域中的对齐问题，为职业推荐系统提供了新的技术路径。
...
> SPIA是首个面向个体的文本匿名化评估基准，发现即使90%以上的PII被掩码，个体层面的保护率也低至33%，证明传统评估方法存在严重缺陷。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本匿名化评估基于跨度（span）的指标无法反映攻击者的实际推断能力，且假设单一数据主体，忽略了多主体场景的现实情况。

**Method:** 提出了SPIA（个体层面PII推断评估）基准，将评估单位从文本跨度转移到个体，包含法律和在线领域的675个文档，并引入新颖的个体层面保护指标。

**Result:** 实验表明，即便90%以上PII跨度被掩码，个体层面的推断保护率低至33%，大部分个人信息仍可通过上下文推断恢复。针对特定主体的匿名化保护会使非目标主体更加暴露。

**Conclusion:** 个体层面基于推断的评估对确保现实环境中安全的文本匿名化至关重要，需要从根本上改变评估视角和评估标准。
...
> 本文提出IRM，一种基于隐式奖励模型的零样本方法，用于检测大语言模型生成的文本，无需偏好收集或额外训练即可超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型虽表现出色，但其生成类人文本的能力引发了滥用的担忧，因此需要可靠有效的方法来检测LLM生成的文本。

**Method:** 提出IRM（隐式奖励模型）方法，利用公开可用的指令调优模型和基础模型提取隐式奖励信号，避免了传统基于奖励的方法所需的偏好构建和任务特定微调。

**Result:** 在DetectRL基准测试上，IRM表现出优越的检测性能，超越了现有的零样本和监督方法。

**Conclusion:** IRM为LLM生成文本检测提供了一种高效便捷的解决方案，无需复杂的偏好数据收集和模型训练，具有重要的实用价值。
...
> 本研究提出了长期会话记忆基准EngramaBench，比较了图结构记忆系统Engrama、GPT-4o全上下文提示和向量检索系统Mem0的性能。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型助手需要能够保留和推理跨多个会话累积的信息，这是当前AI助手发展的重要需求。

**Method:** 构建包含5个角色、100个多会话对话和150个查询的EngramaBench基准，测试三种记忆架构（Engrama图结构记忆、GPT-4o全上下文、Mem0向量检索），使用相同的GPT-4o作为回答模型以隔离记忆架构效果。

**Result:** GPT-4o全上下文综合得分最高(0.6186)，Engrama全局得分0.5367但在跨空间推理上超越全上下文(0.6532 vs 0.6291)，Mem0成本最低但性能最弱(0.4809)。

**Conclusion:** 研究揭示了结构化记忆专门化与聚合优化之间的系统性权衡，Engrama在特定推理任务表现优异但影响综合性能，为长期记忆系统设计提供了重要见解。
...
> LLM4MEM是一个基于大语言模型的多表实体匹配框架，通过多风格提示增强、传递共识嵌入匹配和密度感知修剪三个模块解决了数值属性语义不一致、匹配效率和噪声实体问题，在6个数据集上平均F1值提升5.1%。

<details>
  <summary>Details</summary>

**Motivation:** 现有多表实体匹配方法在处理数值属性变化导致的语义不一致时存在局限，同时面临多个数据源带来的匹配效率下降和噪声实体干扰问题。

**Method:** 提出了包含三个核心模块的LLM-based框架：1)多风格提示增强LLM属性协调模块解决语义不一致；2)传递共识嵌入匹配模块处理实体嵌入和预匹配；3)密度感知修剪模块优化匹配质量。

**Result:** 在6个MEM数据集上进行了广泛实验，相比基线模型平均F1值提升5.1%，验证了方法的有效性。

**Conclusion:** LLM4MEM框架成功利用大语言模型的强大语言理解能力，有效解决了多表实体匹配中的关键挑战，为该领域提供了新的解决方案。
...
> PLOTTER框架通过在结构化图表示上执行评估-规划-修订循环来优化叙事生成的因果关系和骨架结构，显著提升了长文本生成的连贯性和逻辑一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型在生成长篇小说时存在全局叙事连贯性不足、情境逻辑不一致和角色发展单调等问题，常产生结构断裂的脚本。

**Method:** PLOTTER框架在事件图和角色图上执行评估-规划-修订循环，通过在严格逻辑约束下诊断和修复图拓扑结构问题，在完整上下文生成前优化因果关系和叙事骨架。

**Result:** 实验表明PLOTTER在多样化叙事场景中显著优于代表性基线方法，验证了在结构化图表示上规划叙事对提升LLM复杂叙事生成中长语境推理能力的重要性。

**Conclusion:** 基于结构化图表示而非文本直接进行叙事规划是增强大语言模型在复杂叙事生成中长语境推理能力的关键因素。
...
> 研究提出了RPS和AGS两个新指标来衡量LLM agents的行为同质化问题，发现模型蒸馏导致大量Agent行为趋同，同家族模型的工具使用习惯相似度显著更高。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM agents领域存在行为同质化问题，许多Agent共享几乎相同的推理步骤和失败模式，但现有指标无法区分任务必需行为和模型自主偏好的非强制性行为模式。

**Method:** 提出两个互补指标：响应模式相似度(RPS)衡量语言对齐，动作图相似度(AGS)将工具使用习惯建模为有向图。在τ-Bench和τ²-Bench上评估18个模型，并设计控制性蒸馏实验。

**Result:** 同家族模型对的AGS比分属家族模型高5.9个百分点；Kimi-K2在节点相似度达82.6%，依赖相似度达94.7%，超越Anthropic的Opus 4.1；RPS和AGS具有互补性(相关系数r=0.491)。

**Conclusion:** RPS和AGS成功捕捉了行为的非强制性维度，为诊断Agent生态系统中的行为趋同提供了互补工具，有助于理解模型蒸馏对行为同质化的影响。
...
> 在语言模型上先进行音乐预训练能显著加速语言学习过程，相比随机初始化获得17.5%的困惑度改进，且这种优势在模型收敛后仍然存在。

<details>
  <summary>Details</summary>

**Motivation:** 探究结构化的人类创意输出（如音乐）是否能作为小型语言模型的有效预训练载体，以及音乐-诗歌-散文的发展序列对语言习得的加速作用。

**Method:** 使用MAESTRO数据集的钢琴演奏数据进行预训练，通过音乐→诗歌→散文的三阶段发展管道训练Transformer模型，进行多尺度实验（d∈{16,32,64}）并设置5个随机种子进行验证。

**Result:** 音乐预训练获得17.5%的困惑度改进（p<0.001），收敛速度更快，在d=64维度下持久保持5.5%的优势差距；真实音乐数据只需合成模式三分之一的数据量就能达到相同效果；最优预训练数据量随模型容量变化而调整。

**Conclusion:** 结构化的人类创意输出可作为小型语言模型的高效预训练载体，提出了容量依赖的数据筛选原则；但在现代预训练尺度下得出更强结论需要更大规模实验。
...
> 研究发现，大型语言模型解码器不会加剧种族偏见，音频编码器设计而非LLM扩展是实现公平鲁棒语音识别的关键因素。

<details>
  <summary>Details</summary>

**Motivation:** 随着预训练大型语言模型在语音识别中替代特定任务解码器，需要评估文本衍生先验是否会使跨人口群体的识别更加公平或产生偏见。

**Method:** 评估了9个模型、3种架构在43,000个语句上的表现，涵盖5个人口维度，使用Common Voice 24和Fair-Speech数据集，并在12种声学退化条件下进行压力测试。

**Result:** LLM解码器不放大种族偏见；Whisper在印度口音上出现幻听；音频压缩比LLM扩展更能预测口音公平性；严重退化压缩公平差距但静音注入放大Whisper偏见；显式LLM解码器插入错误减少38倍。

**Conclusion:** 音频编码器设计是提升语音识别公平性和鲁棒性的主要手段，而非LLM规模扩展。
...
> 研究测试了在预测编码网络中移除交叉熵损失对K路能量探测器的敏感性，发现CE是解构的关键经验性组件。

<details>
  <summary>Details</summary>

**Motivation:** 验证Cacioli(2026)提出的K路能量探测器对数softmax边际单调函数解构假设，特别测试移除交叉熵损失时该假设的稳定性。

**Method:** 使用两个条件进行实验：1)用MSE替代CE的标准PC网络；2)双向PC网络。在CIFAR-10数据集上使用10个随机种子和相同的2.1M参数骨干网络进行测试。

**Result:** 标准PC中探头低于softmax(Delta=-0.082)，移除CE使差距减半(Delta=-0.037)；双向PC中探头高于softmax(Delta=+0.008)；CE训练产生的logit范数约为MSE或bPC的15倍；探头-softmax差距66%来自logit尺度效应，34%来自CE训练表示的尺度不变排序优势。

**Conclusion:** 交叉熵是预测编码网络中能量探测器解构的主要经验性支撑组件，其对 logits 尺度的规范化作用至关重要，同时也包含尺度不变的表示优势。
...
> 本文提出EAVAE模型，通过可解释的变分自编码器框架有效解耦作者的写作风格和内容，在作者归属和AI生成文本检测任务上均取得最优性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法存在内容-风格纠缠问题，模型学习到作者写作风格与主题之间的伪相关性，导致跨领域泛化能力差。

**Method:** EAVAE首先使用监督对比学习预训练风格编码器，然后采用变分自编码器架构进行微调，使用独立的风格和内容编码器，并引入新型判别器既区分表示对是否来自相同作者/来源，又生成自然语言解释。

**Result:** 在作者归属任务中，在Amazon Reviews、PAN21和HRS等多个数据集上达到state-of-the-art性能；在AI生成文本检测方面，在M4数据集上的小样本学习中表现出色。

**Conclusion:** 通过显式解耦风格与内容并增强可解释性，EAVAE有效缓解了混淆信息问题，证明了其在作者身份验证和AI文本检测等任务中的优越性和实用性。
...
> 研究评估了多文档新闻摘要系统中的政治偏见，发现中型模型在公平性上表现最佳，实体情感是最难消除的偏见维度。

<details>
  <summary>Details</summary>

**Motivation:** 多文档新闻摘要系统在处理海量日常新闻内容时越来越普及，但这些系统可能通过不平等的视角代表、过度强调某些观点和系统性忽视少数派声音来展现政治偏见，因此确保跨不同政治视角的公平性变得至关重要。

**Method:** 使用带有政治倾向标签的完整新闻文章数据集FairNews，对13个大型语言模型进行测试，采用5个公平性指标评估基线模型性能和各种去偏见干预（包括基于提示和基于判定的方法）的效果。

**Result:** 中型模型在公平性和效率方面始终优于大型模型；基于提示的去偏见方法对模型依赖性很强；实体情感是最顽固的公平性维度，对所有测试的干预策略都具有抵抗性。

**Conclusion:** 多文档新闻摘要的公平性需要多维评估框架和有针对性的、考虑架构特点的去偏见策略，而不是简单地扩大模型规模。
...
> 本研究提出了PolyChartQA数据集，专门用于多图表图像的问答任务，包含534个多图表图像和2,694个问答对，并评估了9个先进多模态语言模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 多图表在现实世界中广泛用于呈现复杂信息，但理解多图表图像的研究尚未得到充分探索。现有研究主要关注单个图表，缺乏专门针对多图表理解的数据集和评估方法。

**Method:** 1) 构建PolyChartQA数据集：从计算机科学同行评审研究论文中收集534个多图表图像（总共2,297个子图表），创建2,694个问答对；2) 评估9个最先进的多模态语言模型，从问题类型、难度、来源和多图表关键结构特征等多个维度进行分析；3) 提出新的提示方法来提升模型性能。

**Result:** 研究发现MLM在人工编写的问题上相比MLM生成的问题准确率下降27.4%，而使用提出的提示方法可以使L-准确率提升5.39%。

**Conclusion:** PolyChartQA为多图表理解研究提供了重要的基准数据集，揭示了当前多模态语言模型在处理真实世界多图表问答任务中的局限性，并证明了改进提示方法的有效性。
...
> CARE是一个基于生成式AI的框架，通过在低资源语言(希伯来语和阿拉伯语)的专业验证危机对话数据上微调开源大语言模型，为咨询师提供实时且符合心理学规范的响应建议。

<details>
  <summary>Details</summary>

**Motivation:** 全球心理健康挑战不断加剧，导致情感支持服务超负荷，咨询师过度劳累，在自杀意念等关键情况下响应延迟。现有LLM在低资源语言的心理健康敏感领域应用不足，且缺乏大规模真实世界数据训练，难以复制专业咨询师的支持性语言和干预策略。

**Method:** CARE框架分别使用希伯来语和阿拉伯语的精选真实危机对话数据集微调开源LLM。训练数据由专业咨询师评定为高效的会话组成，使模型能够捕获成功降级的交互模式。通过完整对话历史训练，CARE保持咨询师-求助者对话的不断演变的情感背景和动态结构。

**Result:** 实验表明，与非专业化LLM相比，CARE在语义和策略上与黄金标准咨询师响应的匹配度更强，表明在专家验证数据上的领域特定微调可以显著支持咨询师工作流程。

**Conclusion:** 基于专家验证数据的领域特定微调能够显著支持咨询师工作流程，提高低资源语言环境下的护理质量，为心理健康AI辅助系统的开发提供了有效途径。
...
> 本研究提出了一个多语言极化检测系统，通过自适应框架在多语言通用模型、语言专用专家和混合集成之间动态切换，在22种语言上实现了79.6%的宏F1分数。

<details>
  <summary>Details</summary>

**Motivation:** 针对SemEval-2026任务9中22种语言的极化检测挑战，探索统一架构与语言专用方法的效果对比，解决不同语言（特别是使用独特文字的语言）的检测难题。

**Method:** 采用语言自适应框架，根据开发集性能动态选择XLM-RoBERTa等多语言通用模型、语言专用专家模型或混合集成策略；同时测试NLLB-200跨语言增强技术的效果。

**Result:** 系统在22个语言轨道上平均宏F1分数达到0.796，平均准确率0.826；NLLB-200增强效果不佳，语言专用专家在高棉语、奥里亚语等独特文字语言中表现优异。

**Conclusion:** 单一通用架构无法最优处理所有语言的极化检测，语言自适应选择策略显著提升整体性能，混合方法比强制统一架构更有效。
...
> VLAA-GUI 是一个模块化 GUI 智能体框架，通过完整性验证器、循环中断器和搜索代理三个核心组件来解决早期停止和重复循环问题，在多个基准测试中达到最优性能。

<details>
  <summary>Details</summary>

**Motivation:** 自主 GUI 智能体面临两个基本挑战：早期停止（智能体在没有可验证证据的情况下过早声明成功）和重复循环（智能体在同一失败行为中循环无法恢复）。这些问题严重影响了智能体的可靠性和效率。

**Method:** VLAA-GUI 框架包含三个集成组件：1）强制性完整性验证器，在每一步都强制执行可观察的成功标准；2）强制性循环中断器，提供多层过滤机制，在重复失败后切换交互模式；3）按需搜索代理，通过查询具备搜索能力的LLM来查找不熟悉的工作流程。此外还集成了编码代理和接地代理。

**Result:** 在 OSWorld 和 WindowsAgentArena 两个基准测试中分别达到 77.5% 和 61.0% 的最优性能。其中三个主干模型在单次通过中超越了人类性能（72.4%）。消融研究表明三个组件都能持续提升性能，循环中断器将循环倾向模型的浪费步骤几乎减半。

**Conclusion:** VLAA-GUI 通过系统性地解决早期停止和重复循环问题，显著提升了 GUI 智能体的可靠性和效率。该框架的模块化设计使其能够适应不同能力的模型，为构建更可靠的自主GUI智能体提供了有效方案。
...
> Decoupled DiLoCo是一种去耦合的分布式训练框架，通过异步通信和容错机制突破了SPMD范式的同步瓶颈，显著提升了大规模模型训练在故障环境下的效率和稳定性。

<details>
  <summary>Details</summary>

**Motivation:** 大规模语言模型预训练依赖的SPMD范式存在紧耦合问题，硬件故障、瞬时延迟和同步开销会导致整个计算停滞，浪费大量计算资源。现有的分布式方法如DiLoCo虽然减少了通信带宽，但仍是同步的，容易受到系统停滞影响。

**Method:** Decoupled DiLoCo将计算分区到多个独立的'学习器'上执行本地内层优化步骤。学习器异步地将参数片段发送给中心同步器，同步器通过最小法定人数、自适应宽限期和动态令牌加权合并来聚合更新，从而绕过故障或缓慢的学习器。

**Result:** 在包含数百万模拟芯片的易故障环境中，实现了显著改进的训练效率和严格的零全局停机时间，同时在文本和视觉任务上保持了竞争性的模型性能，适用于密集型和混合专家架构。

**Conclusion:** Decoupled DiLoCo成功突破了传统SPMD同步训练的瓶颈，通过异步通信和智能容错机制，为大规模模型训练提供了一种更高效、更可靠的解决方案，特别适用于实际的故障多发生产环境。
...
> 大型语言模型的推理能力可能源于基本操作（召回和状态跟踪）的组合；混合架构在需要状态跟踪和召回的任务上表现更好，推理增强和架构偏置对不同计算层次有不同贡献。

<details>
  <summary>Details</summary>

**Motivation:** 质疑大型语言模型将推理视为单一能力的传统观点，提出推理可能建立在更基本操作（召回机制和状态跟踪）的基础上，并研究混合架构是否能更好地处理需要这两种能力协同的任务。

**Method:** 使用匹配的Transformer模型和混合模型（结合注意力检索与循环状态更新），在指令微调和推理增强两种变体下，在包含状态跟踪和召回混合任务的测试集上进行评估。

**Result:** 推理增强提供了最大的整体改进，显著扩展了模型保持有效的难度范围；在特定任务中，混合推理模型在序列依赖增加时表现更健壮，而Transformer推理模型在任务难度超过阈值后性能急剧下降。

**Conclusion:** 推理标记和架构归纳偏置在计算过程的不同层次发挥作用：显式推理可以扩展模型的有效操作范围，但其收益取决于底层架构对持续状态传播的支持程度；研究为小型案例，需要在更广泛范围内验证。
...
> 研究表明，通过有策略的数据选择方法可以显著减少合规检测任务中的领域负迁移问题，提高跨监管法规的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 自动化监管合规检测因法律文本的复杂性和多样性而具有挑战性，现有模型在一个监管规定上训练后往往难以泛化到其他监管规定，这凸显了改进跨领域迁移方法的必要性。

**Method:** 将合规检测框架化为自然语言推理(NLI)任务，评估四种数据选择方法从源领域选择增强数据：随机采样、Moore-Lewis的交叉熵差、重要性加权和基于嵌入的检索。通过系统变化选择数据的比例来分析其对跨领域适应的影响。

**Result:** 研究发现，有针对性的数据选择能够显著减少负迁移效应，为在异构监管法规间实现可扩展和可靠的合规自动化提供了实用路径。

**Conclusion:** 数据选择策略是缓解合规检测中负迁移问题的有效方法，为实现跨不同监管法规的自动化合规检测提供了实用且可靠的解决方案。
...
> 本研究提出了一个受控的多维度配对评估框架，用于评估多语言TTS系统，结合了语言控制和基于感知的标注方法，覆盖10种印度语言的5000+句子，收集了12万+配对比较数据。

<details>
  <summary>Details</summary>

**Motivation:** 众包配对评估在基础模型评估中虽然可扩展，但应用于文本转语音系统时，由于语言多样性和语音感知的多维特性导致方差过高，需要更精确的评估方法。

**Method:** 构建多维度配对评估框架，采用语言学控制和感知标注，在10种印度语系上评估7个先进TTS系统，收集超过1900名母语评估者的12万次配对比较，涵盖6个感知维度：可理解性、表达力、音质、生动性、噪声和幻觉。

**Result:** 通过Bradley-Terry建模构建了多语言排行榜，使用SHAP分析解释人类偏好，分析了排行榜可靠性及模型在各感知维度上的优势和权衡。

**Conclusion:** 该框架有效降低了TTS评估中的方差，为多语言TTS系统评估提供了可靠的多维度分析工具，能够揭示模型在不同维度上的表现特征和权衡关系。
...
> 研究者提出了一个包含1000个优化问题的综合基准测试OptiVerse，发现现有大语言模型在复杂优化任务上表现严重不足，最大模型准确率仅27%，并开发了双视图审计代理来改进建模过程。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在复杂优化任务上表现不佳，但现有基准测试主要局限于数学规划和组合优化，缺乏对随机优化、动态优化、博弈优化和最优控制等领域的全面评估，阻碍了LLM在优化领域的发展。

**Method:** 1) 构建OptiVerse基准测试，包含1000个精心挑选的优化问题，覆盖4个被忽视的优化领域，分为三个难度等级；2) 测试22个不同大小的LLM性能；3) 进行错误分析识别主要瓶颈；4) 提出双视图审计代理改进LLM建模过程。

**Result:** 实验发现LLM在困难问题上性能急剧下降，即使先进模型如GPT-5.2和Gemini-3准确率也不超过27%；错误分析表明建模和逻辑错误是主要瓶颈；提出的双视图审计代理能有效提高建模准确率且不带来显著时间开销。

**Conclusion:** OptiVerse为推进LLM解决复杂优化挑战提供了基础平台，揭示了当前LLM在优化领域的局限性，并为改进方向提供了指导，需要重点关注建模过程的准确性和逻辑推理能力。
...
> SRICL是一个结合语义检索、上下文学习和监督微调的LLM框架，用于解决招聘广告中技能提取的span级别问题，显著提升了准确性和可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的生成式大语言模型在从招聘广告中提取技能时存在格式错误、边界漂移和幻觉问题，特别是在处理长尾术语和跨领域场景时，这影响了求职者-职位匹配和劳动力市场分析的可靠性。

**Method:** SRICL框架结合语义检索(SR)从ESCO获取领域相关句子和定义形成格式约束提示，使用上下文学习(ICL)和监督微调(SFT)对齐输出行为，并通过确定性验证器强制执行配对、非重叠和BIO合法性约束。

**Result:** 在六个涵盖不同行业和语言的公共数据集上，SRICL相对于GPT-3.5提示基线实现了显著的STRICT-F1提升，大幅减少了无效标签和幻觉span，支持在低资源多领域环境中的可靠部署。

**Conclusion:** SRICL为解决LLM在技能提取任务中的关键挑战提供了有效方案，通过结合检索、微调和验证机制，实现了在复杂实际场景中的可靠span级别技能提取。
...
> 该研究为SemEval-2026任务2开发了一个情感分析系统，通过结合LLM提示、最大熵模型和轻量级神经回归模型三种方法，在情感状态和短期情感变化预测任务中排名第一。

<details>
  <summary>Details</summary>

**Motivation:** 需要同时对用户生成的时序文本中的当前情感状态和短期情感变化进行建模，这在传统的情感分析中是一个具有挑战性的任务，因为需要捕捉情感的动态演变过程。

**Method:** 采用三种互补方法：(1)在用户感知和用户不可知设置下的LLM提示；(2)带伊辛风格交互的成对最大熵模型用于结构化转换建模；(3)结合近期情感轨迹和可训练用户嵌入的轻量级神经回归模型。

**Result:** 研究发现LLM能有效捕捉文本中的静态情感信号，而该数据集中的短期情感变化更多由近期的数值状态轨迹而非文本语义解释；系统在官方评估指标下在子任务1和子任务2A中均排名第一。

**Conclusion:** 对于时序文本的情感分析，静态情感判断适合使用LLM，而短期情感变化预测更依赖于数值状态轨迹模式，这种混合方法能够有效处理情感分析的静态和动态两个维度。
...
> 本研究提出了一种名为概念分离曲线的分类器无关方法，通过引入语法噪声和语义否定来评估句子嵌入的概念稳定性，该方法可解释、可重现且适用于跨模型比较。

<details>
  <summary>Details</summary>

**Motivation:** 现有的句子嵌入评估方法大多依赖额外的分类器或下游任务，这使得评估结果无法明确区分是嵌入本身还是分类器行为导致的良好表现，缺乏客观性。

**Method:** 提出一种分类器独立的评估方法，系统性地向句子中引入语法噪声和语义否定，量化这些变化对嵌入结果的影响，并通过概念分离曲线可视化模型区分概念变化和表层变化的能力。

**Result:** 通过多领域、英语和荷兰语、不同句子长度的实验验证，证明概念分离曲线是一种可解释、可重现、跨模型的句子嵌入概念稳定性评估方法。

**Conclusion:** 概念分离曲线为评估句子嵌入模型的概念捕获能力提供了客观、可靠的标准，避免了传统评估方法中分类器干扰的问题，为句子嵌入技术的评估开辟了新途径。
...
> 该论文提出了名为llm-bias-bench的开源方法，通过直接和间接两种探测技术，揭示大型语言模型在争议性话题上的真实立场和偏见问题。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型语言模型在搜索、专业建议、政策等方面的广泛应用，模型对争议性话题的隐含立场会大规模影响用户决策。但传统方法难以准确获取模型真实观点，因为现有助手会回避直接提问，且在用户论证时会改变立场。

**Method:** 开发了两种互补的自由形式探测方法：直接探测通过5轮逐步升级的用户压力询问模型观点；间接探测通过论证辩论让偏见在模型让步、抵抗或反论证中泄露。采用三种用户性格（中立、同意、反对）进行九维行为分类，区分独立立场与迎合行为，并由可审计的LLM法官生成带有文本证据的判决。

**Result:** 该方法应用于13个助手的38个巴西葡萄牙语话题，发现：论证辩论触发的迎合行为是直接提问的2-3倍；在直接提问中看似有观点的模型在持续论证中常变为镜像行为；攻击者能力主要在需要改变现有观点时起作用。

**Conclusion:** 该研究表明大型语言模型在争议性话题上的立场不仅取决于其内部观点，还高度受到用户论证能力的影响，需要更好的方法来评估和控制这种动态偏见。
...
> AgenticQwen是一系列小型的代理模型，通过多轮强化学习在合成数据上训练，在成本和延迟受限的工业环境中展现出接近大型模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现代工业应用需要能够在严格成本和延迟约束下进行多步推理和工具使用的语言模型，小型代理模型因此具有很高的实用价值。

**Method:** 结合推理强化学习和代理强化学习，通过双数据飞轮自动生成难度递增的任务：推理飞轮从错误学习增加任务难度，代理飞轮将线性工作流扩展为多分支行为树以反映真实应用的决策复杂性。

**Result:** AgenticQwen在多个代理基准测试和工业代理系统中表现优异，在搜索和数据分析任务上缩小了与大型模型的性能差距。

**Conclusion:** 通过创新的双飞轮强化学习框架，成功训练出在工业环境下实用的小型代理模型，为成本敏感的应用场景提供了有效的解决方案。
...
> 研究发现多语言响应能提升LLM推理能力，基于此提出polyGRPO框架，通过语言变化作为隐式探索信号优化模型，在少量数学数据训练下显著提升多语言和英语推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 观察到非英语响应在推理任务中有时优于英语，假设语言不仅是输出媒介，更是调节模型内部推理路径的潜在变量，需要验证多语言对推理能力的影响机制。

**Method:** 进行多语言思维实验，在语言受限/非受限条件下测试相同问题；提出polyGRPO框架，将语言变化作为隐式探索信号，在线生成多语言偏好数据，优化答案准确性和推理结构。

**Result:** 在18.1K多语言数学问题上训练后，Qwen2.5-7B-Instruct在四个英语推理测试集准确率提升6.72%，多语言基准提升6.89%，在常识推理任务上唯一超越基础模型(4.9%)，展现出强跨任务泛化能力。

**Conclusion:** 将语言作为潜在变量处理能够扩展模型的推理空间，带来一致且泛化的推理性能提升，验证了多语言对推理能力的积极作用。
...
> 本文提出了语言过程监督(VPS)框架，通过结构化的自然语言批评指导推理循环，无需训练即可显著提升LLM在复杂推理任务上的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM推理时的扩展主要关注链深度、样本广度和学习式步骤评分器三个维度，但忽视了外部语言监督的粒度这一潜在维度。作者希望探索如何利用更细粒度的语言反馈来提升推理质量。

**Method:** 提出VPS框架，使用更强的监督模型提供结构化自然语言批评，指导弱模型进行迭代的生成-批评-改进循环，直到达到预算轮数R。整个过程是训练无关的，完全在推理时进行。

**Result:** 在GPQA Diamond、AIME 2025和LiveCodeBench V6上，VPS取得了显著效果：1) GPQA上达到94.9%，超越SOTA；2) AIME上弱模型性能提升26-63个百分点；3) 在相同计算量下超越Reflexion和Self-Consistency方法。性能与监督-模型能力差距高度相关(r=0.90)。

**Conclusion:** 研究确立了批评粒度作为推理时扩展的新维度，VPS框架证明了细粒度语言监督的有效性，但也发现在非语言可表达的任务(如代码合成)中性能下降，为未来混合语言-可执行方法提供了方向。
...
> 这篇调研论文关注语言模型在非英语地区和资源受限环境中的部署挑战，即多语言性与边缘部署交汇的'最后一公里'问题。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型的部署位置和方式决定了谁能从中受益，但在全球南方的非英语社区和硬件受限环境中存在多种部署障碍。这些语言多样性社区往往面临最严重的基础设施限制，而多语言NLP和边缘AI研究仍处于分离状态。

**Method:** 通过系统性地调研232篇论文，全面分析了从数据收集、模型开发到部署的整个语言建模流程中如何解决多语言性与边缘部署的结合问题。

**Result:** 揭示了当前技术发展状况，识别出关键技术挑战，并为NLP生态系统中的不同利益相关者提供了可行的建议和指导。

**Conclusion:** 通过整合多语言NLP和边缘部署研究，有助于开发更加包容和公平的语言技术，促进语言技术在资源受限环境中的普惠应用。
...
> 本研究提出了一个联合建模标注者特定标签预测和解释的框架，通过User Passport机制整合标注者信息，使用两种解释器架构生成与个体视角对齐的解释，显著提升了预测性能。

<details>
  <summary>Details</summary>

**Motivation:** 标注者的理由提供了个体视角的细粒度信号，单纯利用分散化的标签进行视角建模是不够的，需要将标注者的解释整合到模型中以获得更丰富和忠实的分歧表示。

**Method:** 提出了结合User Passport机制的框架，在表示层面整合标注者身份和人口统计元数据；引入两种解释器架构：基于提示的后置解释器和前置桥接解释器，将标注者条件化的分类器表示直接传递给生成模型。

**Result:** 纳入解释建模后预测性能显著超越基线标注感知分类器，前置桥接方法获得了更稳定的标签对齐和更高语义一致性，后置方法产生了更强的词汇相似性。

**Conclusion:** 将解释建模为细粒度视角的表达提供了更丰富和忠实的分歧表示，所提出的方法通过将标注者特定理由整合到预测和生成组件中，推进了视角主义建模的发展。
...
> 本研究开发了一种新颖的时间序列过滤方法，结合拓扑数据分析中的持久同调技术，成功应用于眼部追踪数据的阅读障碍检测，混合模型显著优于传统方法。

<details>
  <summary>Details</summary>

**Motivation:** 传统眼部追踪数据分析方法可能无法充分捕捉注视序列中蕴含的复杂拓扑结构信息，需要开发新方法来提取更具判别性的特征。

**Method:** 将注视序列解释为时间序列，开发新颖的过滤方法，使用持久同调提取拓扑特征，构建结合拓扑特征与传统统计特征的混合模型，在哥本哈根语料库上进行实证评估。

**Result:** 混合模型在阅读障碍检测任务上优于仅使用传统特征的方法，拓扑特征捕捉到了注视序列中的补充信息，性能可与现有基线方法相媲美，提出的新过滤方法优于现有的过滤方法。

**Conclusion:** 持久同调技术能够从眼部追踪数据中提取有价值的拓扑信息，与传统特征相结合可以提高阅读障碍检测的准确性，新开发的过滤方法为时间序列分析提供了有效工具。
...
> 该研究通过扩大样本和模型多样性，验证了基于语音学特征子空间d-prime可分性的训练free方法在构音障碍严重程度评估中的鲁棒性，发现了病因特异性的退化模式具有跨语言形状稳定性，且方法具有架构独立性。

<details>
  <summary>Details</summary>

**Motivation:** 构音障碍严重程度评估在临床实践中有重要意义，但现有方法通常需要大量训练数据且缺乏跨语言和跨模型的普适性。作者希望验证其之前提出的训练free方法在大规模多语言、多病因数据上的稳定性和有效性。

**Method:** 基于自监督语音表示中语音学特征子空间的d-prime可分性分析，涵盖3374名说话者、25个数据集、12种语言和5种病因，使用6种不同的SSL骨干网络进行验证，包括群体水平效应分析、个体分类和跨语言相关性评估。

**Result:** 发现1) 病因特异性退化模式在群体水平可区分，10/13特征显示大效应量(epsilon²>0.14)，但个体分类效果有限(22.6%宏F1)；2) 跨语言形状稳定性高(余弦相似度>0.95)，但绝对d-prime值需语料内校准；3) 方法架构独立，6种骨干网络均产生单调严重性梯度(一致性rho>0.77)。

**Conclusion:** 语音学子空间分析是一个鲁棒的训练free框架，可用于病因感知的构音障碍特征化，具有跨语言模式形状稳定性和跨骨干网络鲁棒性，为多语言构音障碍评估提供了可靠工具。
...
> 研究表明，传统的简单条件语句测试严重低估了代码生成中的偏见，通过更真实的机器学习管道生成任务发现，敏感属性出现的频率（87.7%）远高于条件语句测试（59.2%）。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究主要通过简单条件语句评估代码生成偏见，但这仅代表了现实编程的极小部分，只能发现显性的、明确编码的偏见，无法全面反映实际应用中的偏见程度。

**Method:** 通过生成机器学习管道这一更真实的任务进行测试，比较了代码专门化和通用指令大语言模型的表现，并考察了提示词缓解策略、属性数量变化和管道难度等级的影响。

**Result:** 生成的管道在特征选择阶段表现出显著偏见，敏感属性平均出现在87.7%的案例中，远高于条件语句测试的59.2%，且这种偏见在不同条件下均稳定存在。

**Conclusion:** 研究结果挑战了简单条件语句作为偏见评估有效代理的做法，表明当前基准测试低估了实际部署中的偏见风险，需要采用更贴近实际应用场景的评估方法。
...
> X-GRAM是一种频率感知的动态token注入框架，通过混合哈希和别名混合压缩尾部token，使用归一化SwiGLU短卷积提取n-gram特征，显著提升了小规模模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大规模token索引查找表面临参数效率低、内存增长快的问题，主要原因是Zipfian长尾欠训练、层间需求异构性和插槽冗余导致的嵌入向量重复。

**Method:** X-GRAM采用混合哈希和别名混合技术压缩尾部token，通过归一化SwiGLU短卷积提取多样的局部n-gram特征，并利用深度感知门将信号整合到注意力值流和层间残差中。

**Result:** 在0.73B和1.15B规模的评估中，X-GRAM相比原始骨干网络平均提升准确率4.4个百分点，相比强检索基线提升3.2个百分点，同时使用更小的表结构(50%配置)。

**Conclusion:** X-GRAM通过有效内存管理实现容量与计算的解耦，为未来的内存增强架构提供了一个可扩展且实用的范式。
...
> 本文提出Agent Evolving Learning (AEL)框架，通过双时间尺度的记忆使用策略和反思机制，让LLM智能体能够有效利用过往经验，在投资组合任务中实现显著更好的持续学习性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM智能体在开放式环境中运行时基本是无状态的，无法将过去经验转化为更好的未来行为。主要瓶颈不在于记忆什么，而在于如何有效使用记忆内容，包括检索策略选择、结果解释和策略适应性调整。

**Method:** AEL采用双时间尺度框架：快速时间尺度下使用Thompson Sampling老虎机学习每集episode的记忆检索策略；慢速时间尺度下LLM驱动的反思机制诊断失败模式，将因果洞察注入到智能体决策提示中，为检索到的证据提供解释框架。

**Result:** 在投资组合基准测试(10个板块股票，208个episode)中，AEL达到2.13±0.47的夏普比率，优于5种已发布的自改进方法和所有非LLM基线，且在LLM方法中方差最低。消融实验显示记忆与反思的简单组合比状态less基线改进58%，而额外机制反而降低性能。

**Conclusion:** 智能体自改进的瓶颈在于如何自我诊断和使用经验，而非增加架构复杂性。研究表明适度的记忆使用策略和反思机制比复杂的监督学习或规划更有效。
...
**Motivation:** 长期对话代理需要记忆系统来捕捉事件间的关系，而不仅仅是孤立的事实，以支持时间推理和多跳问答。现有方法面临基本权衡：扁平记忆高效但无法建模关系结构，而基于图的记忆能实现结构化推理但构建成本高昂且脆弱。

**Method:** 提出了StructMem，一个结构化的分层记忆框架，该框架保留事件级绑定并诱导跨事件连接。通过时间锚定双视角和执行周期性语义整合来实现。

**Result:** 在LoCoMo数据集上改善了时间推理和多跳性能，同时大幅减少了令牌使用量、API调用和运行时相比之前的记忆系统。

**Conclusion:** StructMem成功解决了记忆系统的效率与结构化表达之间的权衡问题，为长期对话代理提供了更有效的记忆解决方案。
...
> 该研究通过构建文化相关问题数据集CROQ，发现LLM在处理文化相关问题时存在明显的地区偏好性，特别是倾向于日本而非西方或盎格鲁中心观点，并揭示了这种偏见主要出现在监督微调阶段。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究已表明LLM在文化覆盖和胜任力方面存在局限，但缺乏具体分析LLM在文化相关问题上的地区偏好的研究，需要填补这一空白。

**Method:** 研究者基于文化相关开放问题的综合分类法构建了新的CROQ数据集，通过该数据集评估LLM的文化偏好，并分析了不同语言输入对输出多样性的影响，同时追踪了文化偏见在训练过程中的出现时机。

**Result:** 研究发现LLM显示出对日本的明显偏好趋势；使用英语或其他高资源语言提示时，LLM输出更加多样化且较少倾向于突出输入语言为官方语言的国家；文化偏见在监督微调阶段首次出现，而非预训练阶段。

**Conclusion:** 与传统认为的西方中心偏见不同，LLM确实存在地区文化偏好，但这种偏好并非训练早期就形成，而是在监督微调阶段才明显出现，这对理解LLM的文化偏见机制具有重要意义。
...
> AUDITA是一个新的大规模音频问答基准测试，通过真实世界的音频和人类编写的推理问题，严格评估模型的音频理解能力，现有模型在该测试上的准确率低于8.86%，远低于人类的32.13%。

<details>
  <summary>Details</summary>

**Motivation:** 现有的音频问答基准主要关注声音事件分类或基于字幕的查询，模型可以通过捷径策略、短时提示、词汇先验或数据集偏差来获得成功，甚至绕过音频直接使用元数据和字幕，缺乏真正的音频推理能力评估。

**Method:** 创建AUDITA基准数据集，包含精心策划的人类编写的、基于真实世界音频的问答问题，通过具有挑战性的干扰项和长期时间依赖性来测试鲁棒的听觉推理能力，使用无法仅从孤立文本或声音线索回答的探测查询，并引入项目反应理论(IRT)来评估模型表现。

**Result:** 人类在AUDITA上的平均准确率为32.13%，而最先进的音频问答模型平均准确率低于8.86%，IRT分析揭示了模型和数据的系统性缺陷。

**Conclusion:** AUDITA成功地提供了一个严格评估深度音频推理能力的基准，暴露了现有模型在复杂音频理解任务上的不足，为未来音频AI模型的发展提供了新的挑战方向。
...
> 该研究创建了两个新的视频虚假信息数据集，专注于通过音频转录检测视频中虚假信息的具体片段位置，使用最先进的语言模型达到了0.68的F1分数。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频虚假信息检测方法只能判断整个视频是否包含虚假信息，无法确定虚假信息在视频中的具体位置和相关内容，缺乏可解释性，这对事实核查人员来说是重要的局限性。

**Method:** 研究者转录了500多个视频的音频为文本，标注了2400多个经过事实核查的虚假信息片段，构建了两个新数据集。然后使用基于最先进语言模型的分类器来检测视频中虚假信息出现的具体段落。

**Result:** 能够识别视频中虚假信息出现的位置，F1分数达到0.68。研究者公开发布了标注数据集、转录文本、音频和视频资源。

**Conclusion:** 该研究填补了视频虚假信息位置检测的研究空白，为事实核查人员提供了更细粒度的检测工具，提高了虚假信息检测的可解释性，并为后续相关研究提供了宝贵的资源。
...
> 本研究提出了NSNRL共享任务，通过故事三元组分类来评估叙事相似性和叙事表示学习，收集了1000+标注数据，分析71个参赛系统后发现LLM集成方法在分类任务中表现最佳，而嵌入处理方法达到与精细调优相当的效果。

<details>
  <summary>Details</summary>

**Motivation:** 为了解决如何量化和计算叙事相似性这一基础问题，同时评估不同叙事嵌入表示的有效性，该研究提出了一种既符合叙事理论又符合直觉判断的叙事相似性新定义。

**Method:** 将叙事相似性操作化为二元分类问题，通过故事摘要三元组（锚点故事+两个候选故事）进行人工标注，构建包含1000+个三元组的数据集，并设立分类和嵌入两个赛道进行系统评估。

**Result:** 在分类赛道中，LLM集成系统占据高分排名；在嵌入赛道中，预训练模型的预处理和后处理技术与自定义精调方案性能相当，发现两个赛道都存在改进空间。

**Conclusion:** 该研究成功建立了一个新颖的叙事相似性评估框架和数据集，为后续研究提供了基准，同时指出了当前自动化系统在叙事理解方面的局限性和发展潜力。
...
> 通过分析LLM在不同实体表面形式下的事实记忆表现，发现记忆既非完全取决于特定表形也非完全表形不变，表形多样性对评估记忆至关重要。

<details>
  <summary>Details</summary>

**Motivation:** 传统实体QA评估使用单一标准实体名称，难以区分事实记忆与特定名称访问的影响，无法准确评估LLM的事实记忆可靠性。

**Method:** 构建RedirectQA数据集，利用Wikipedia重定向信息将Wikidata事实三元组与各类实体表形（别名、缩写、拼写变体等）关联，在13个LLM上测试表面条件化的事实记忆。

**Result:** 预测结果随实体表形变化而变化，且存在类别依赖性：模型对微小拼写变体更稳健，对别名和缩写等大词汇变体更敏感；实体频率和表形频率都与准确性相关。

**Conclusion:** LLM的事实记忆既非完全表形特定也非完全表形不变，强调在评估非逐字记忆时需要考虑表形多样性的重要性。
...
> MODEE是一个结合图学习与大型语言模型的开放领域事件抽取方法，通过建模文档级推理来克服现有方法的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 现有事件抽取方法存在两大局限：闭域算法无法泛化到未见事件类型，而开放域算法忽视了大型语言模型的潜力，且未能显式建模文档级的上下文、结构和语义推理。

**Method:** MODEE采用多模态开放域事件抽取框架，将基于图的学习与LLM的文本表示相结合，以建模文档级推理过程，解决LLM在处理长文档时出现的注意力稀释等问题。

**Result:** 在大规模数据集上的实验表明，MODEE超越了现有最先进的开放域事件抽取方法，并能泛化应用于闭域事件抽取任务，在此领域同样优于现有算法。

**Conclusion:** MODEE通过有效结合图结构和LLM的文本表示能力，为开放域事件抽取提供了一种有效的解决方案，同时展现了在闭域任务中的良好泛化能力。
...
> TingIS是一个企业级事件发现系统，通过结合LLM和多阶段事件链接技术，从海量客户事件中实时发现高优先级技术异常。

<details>
  <summary>Details</summary>

**Motivation:** 大规模云原生服务中，客户事件是发现监控遗漏风险的重要信号，但极端噪声、高吞吐量和业务语义复杂性使得从中提取可行动情报极具挑战性。

**Method:** TingIS采用多阶段事件链接引擎，融合高效索引技术与大型语言模型进行智能事件合并决策，配合级联路由机制实现精确业务归属，并集成领域知识、统计模式和行为过滤的多维降噪流水线。

**Result:** 在生产环境中处理每分钟2000+、每日30万条消息，P90告警延迟3.5分钟，高优先级事件发现率达95%，在路由准确性、聚类质量和信噪比方面显著超越基线方法。

**Conclusion:** TingIS成功解决了企业级海量客户事件中的实时异常检测难题，为云原生服务提供了可靠的技术风险发现和响应能力。
...
> 该论文创建了EVENT5Ws，一个大规模、人工标注的开放域事件抽取数据集，并建立了评估基准，证明了其跨地理领域泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有事件抽取数据集存在局限性：封闭领域中事件类型覆盖有限，开放域中缺乏大规模人工验证数据集，阻碍了自动化事件抽取算法的发展。

**Method:** 设计系统化的标注流水线创建EVENT5Ws数据集，在数据集上评估最先进的预训练大语言模型，建立基准并验证模型的泛化能力。

**Result:** 成功构建了经过统计验证的大规模开放域事件抽取数据集，建立了评估基准，证明了模型在该数据集上训练后能有效泛化到不同地理背景的数据集。

**Conclusion:** EVENT5Ws为开发可泛化的事件抽取算法提供了重要资源，论文总结了数据集开发过程中的经验教训，为未来大规模数据集建设提供了指导建议。
...
> 本文提出一个结合历时文体分析、上下文主题建模和语义聚类的计算框架，分析巴西议员演讲，发现语言风格演变、危机驱动的议题转变以及超越党派的身份认同模式。

<details>
  <summary>Details</summary>

**Motivation:** 传统立法行为分析过度依赖投票记录，忽视了政治演讲中丰富的语义和修辞内容。本文旨在通过多维度的议会话语分析，弥补这一研究空白。

**Method:** 构建可扩展的计算框架，整合历时文体分析（研究表达方式）、上下文主题建模（分析内容主题）和语义聚类（识别话语相似性），应用于巴西众议院45万篇演讲（2003-2025年）的大规模案例研究。

**Result:** 研究发现三大趋势：一是语言风格长期向更简短直接转变；二是立法议程随国家危机剧烈重新定位；三是话语对齐图谱显示地区和性别身份往往比正式党派身份更为重要。

**Conclusion:** 该研究为分析多维度的议会话语提供了稳健方法，有效补充了传统的基于投票的分析路径，揭示了政治行为中语言、内容和身份认同的复杂互动。
...
> 本文提出GiVA，一种基于梯度的向量适配初始化策略，在保持极低参数效率的同时，将训练时间降低到与LoRA相当的水平，并将秩需求减少8倍。

<details>
  <summary>Details</summary>

**Motivation:** 随着模型规模不断增长，参数高效微调成为重要需求。现有向量适配方法虽然参数效率极高，但需要比LoRA高得多的秩来匹配性能，导致训练成本增加。

**Method:** GiVA是一种基于梯度的初始化策略，专门针对向量适配方法设计。通过智能的梯度初始化，显著降低了训练时的秩需求。

**Result:** 在自然语言理解、自然语言生成和图像分类等多个基准测试中，GiVA始终超越或与现有向量适配方法和LoRA性能相当，同时将秩需求降低8倍。

**Conclusion:** GiVA成功解决了向量适配方法训练成本高的问题，在保持极低参数效率的同时实现了与LoRA相当的训练效率，为大规模模型的高效微调提供了新的解决方案。
...
> MathDuels是一个自我博弈的数学评估基准，模型既出题又解题，通过角色双重评估能力，解决了传统基准饱和问题

<details>
  <summary>Details</summary>

**Motivation:** 现有数学基准已接近天花板性能，无法区分前沿模型能力，因为它们只让模型作为固定问题集的解决者，缺乏动态评估机制

**Method:** 设计三阶段问题生成流程（元提示、问题生成、难度放大）和独立验证器排除病题，使用Rasch模型联合估计解题能力和问题难度，通过模型双重角色（出题者和解题者）进行评估

**Result:** 在19个前沿模型实验中发现，出题和解题能力部分解耦，双重角色评估能揭示单角色基准不可见的能力分离，新模型生成的问题能击败之前的优势解题者

**Conclusion:** MathDuels实现了基准难度与参与者强度的共同进化而非固定天花板，提供动态可持续的模型能力评估平台，设有持续更新的公开排行榜
...
> 本文评估了基于解码器LLM的语义指标在ASR评估中的表现，发现在假设选择方面达到92-94%与人类标注者的一致性，显著优于WER的63%，并提供了更具解释性的评估方法。

<details>
  <summary>Details</summary>

**Motivation:** 传统ASR评估使用的词错误率(WER)对语义不敏感，而基于嵌入的语义指标与人类感知更相关，但基于解码器的大语言模型在此任务上尚未得到充分探索。

**Method:** 通过三种方法评估LLM的适用性：1) 在两个候选假设中选择最佳假设，2) 使用生成嵌入计算语义距离，3) 错误的定性分类。在HATS数据集上测试性能表现。

**Result:** 最佳LLM在假设选择上达到92-94%与人类标注者的一致性，远超WER的63%，也优于语义指标；基于解码器LLM的嵌入表现与编码器模型相当。

**Conclusion:** 基于解码器LLM的语义评估指标为自动语音识别提供了可解释且语义相关的评估方向，是传统WER指标的有效替代方案。
...
> PlantInquiryVQA是一个针对植物病理诊断的多轮视觉问答基准，模拟专家通过结构化提问进行疾病诊断的过程，强调意图驱动的视觉推理。

<details>
  <summary>Details</summary>

**Motivation:** 当前视觉语言模型评估主要基于单轮问答，无法反映植物病理学中专家通过多步结构化提问进行疾病诊断的实际需求。现有模型在安全临床推理和准确诊断方面存在不足。

**Method:** 提出Chain of Inquiry框架，将诊断轨迹建模为基于视觉线索和认知意图的有序问答序列；构建包含2.5万张专家标注植物图像和13.8万问答对的数据集，标注了视觉定位、严重程度标签和领域特定推理模板。

**Result:** 顶级多模态大语言模型虽能适当描述视觉症状，但在安全临床推理和准确诊断方面表现不佳；结构化问题引导的询问显著提高了诊断正确性，减少了幻觉，提升了推理效率。

**Conclusion:** PlantInquiryVQA为训练能够像专家植物学家而非静态分类器那样推理的诊断智能体提供了基础基准，推动了多步意图驱动视觉推理研究的发展。
...
> 本文提出了一种文本到线性图像生成方法，通过将线性图像表示为曝光包围序列，解决了现有生成模型无法处理高动态范围线性图像的局限。

<details>
  <summary>Details</summary>

**Motivation:** 当前生成模型主要合成为显示器优化的图像，这些图像经过非线性色调映射和动态范围压缩，失去了原始光线信息，限制了后期编辑的灵活性。线性图像保留完整动态范围和真实辐照度，为专业后期处理提供更大空间。

**Method:** 作者将线性图像表示为多个曝光包围的序列，每个包围捕捉动态范围的特定部分，并提出了基于DiT的流匹配架构来生成文本条件下的曝光包围，最终合成为高质量线性图像。

**Result:** 该方法能够生成保持完整动态范围的高质量线性图像，并支持文本引导的线性图像编辑和ControlNet结构条件生成等下游应用。

**Conclusion:** 该研究首次实现了文本到线性图像的生成，突破了传统显示器优化图像的局限，为创意摄影和专业图像处理提供了新的技术基础。
...
> 提出双路径网络处理微动作的多样性，通过空间-时间和时间-空间并行路径，结合自适应路由和一致性约束，实现对微动作的细粒度理解。

<details>
  <summary>Details</summary>

**Motivation:** 微动作是持续1-3秒的细微局部动作，对社交交流和自然交互至关重要，但现有计算机视觉系统难以理解。主要挑战在于微动作表现出不同的时空特征 - 有些以空间构型定义，有些通过时间动态体现。

**Method:** 设计双路径网络架构：ST路径先建模空间构型再处理时间动态，TS路径相反先优先时间动态。引入实体级自适应路由让每个身体部位学习最优处理偏好，配合MAC损失函数强制跨路径一致性。

**Result:** 在MA-52数据集上取得竞争性性能，在iMiGUE数据集上达到最先进结果，验证了方法的有效性。

**Conclusion:** 架构适应微动作内在复杂性是推进细粒度视频理解的关键。工作表明灵活的时空处理机制对于理解具有不同特征的微动作至关重要。
...
> 提出了一种无需训练的方法，将多光谱数据引入标准RGB大多模态模型的推理过程，大幅提升了遥感应用性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有大多模态模型主要在RGB图像上训练，限制了其在多光谱遥感领域的应用；而专门训练多光谱模型成本高昂且缺乏通用性。

**Method:** 通过在推理阶段将非RGB输入适配到模型的视觉空间，并注入领域特定信息和链式思维推理指令，无需额外训练即可处理多光谱数据。

**Result:** 在Gemini 2.5模型上实现了显著的零样本性能提升，在主流遥感基准测试中表现出色。

**Conclusion:** 这种方法使地理空间专业人士能够利用强大的通用模型处理特定传感器输入，同时获得基于专业数据的丰富推理能力。
...
> 本文提出了将投影梯度遗忘(PGU)方法适配到文本到图像扩散模型，作为后置硬化步骤来解决现有遗忘方法在下游任务微调后概念复活的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本到图像扩散模型的机器学习方法存在根本缺陷：即使在与原概念无关的下游数据上进行微调，被遗忘的不良概念仍会复活，这限制了其实际应用价值。

**Method:** 将分类领域的投影梯度遗忘(PGU)方法适配到扩散领域，构建保留概念的核梯度空间(CGS)，将梯度更新投影到其正交补空间中，确保后续微调无法撤销已实现的遗忘效果。

**Result:** PGU方法能在现有方法(ESD、UCE、Receler)上实现：1)对风格概念完全消除复活 2)对对象概念显著延迟复活 3)运行时间约6分钟，比Meta-Unlearning的2小时更快
...
> StyleVAR基于VAR框架，通过条件离散序列建模实现风格迁移，采用混合交叉注意力机制和GRPO强化学习优化，在多个基准测试中显著优于AdaIN基线。

<details>
  <summary>Details</summary>

**Motivation:** 传统风格迁移方法在保持内容结构和风格纹理平衡方面存在挑战，特别是在处理多样化内容时。研究旨在利用自回归建模的优势，通过更好的特征融合机制和多层次优化来改善风格迁移质量。

**Method:** 将图像分解为多尺度表示并用VQ-VAE离散化，引入混合交叉注意力机制让目标表示同时考虑自身历史、风格和内容特征。使用尺度相关混合系数控制影响权重，通过监督微调(SFT)和群体相对策略优化(GRPO)两阶段训练，后者基于DreamSim感知奖励进行强化学习。

**Result:** 在三个涵盖不同分布的基准测试中，StyleVAR在风格损失、内容损失、LPIPS、SSIM、DreamSim和CLIP相似度等指标上均超越AdaIN基线，GRPO阶段进一步提升了感知指标性能。方法在景观和建筑场景表现良好，但对互联网图像泛化能力有限，处理人脸存在困难。

**Conclusion:** StyleVAR成功将自回归建模应用于风格迁移，证明了混合注意力和多尺度优化框架的有效性。未来需要更好的内容多样性和更强的结构先验来提升泛化能力和人脸处理效果。
...
> 本研究提出了一种专家引导的对比微调框架，用于儿童脑肿瘤全幻灯片图像的弱监督分类，通过对比学习正则化幻灯片级表示，在数据稀缺和类别不平衡条件下显著提升了细粒度诊断性能。

<details>
  <summary>Details</summary>

**Motivation:** 儿童脑肿瘤的准确诊断面临深度学习的独特挑战：严重的数据稀缺、类别不平衡以及诊断不同亚型间的细粒度形态重叠。现有的病理学基础模型在片级表征学习方面取得进展，但在有限数据下有效适配到弱监督儿童脑肿瘤分类方面尚未充分探索。

**Method:** 将对比学习整合到幻灯片级多实例学习(MIL)中，在下游微调过程中明确正则化幻灯片级表示的几何结构。提出通用监督对比设置和专家引导变体，后者整合临床信息来针对诊断混淆亚型的困难负样本。

**Result:** 在现实低样本和类别不平衡条件下的儿童脑肿瘤WSI分类实验表明，对比微调在细粒度诊断区分方面产生可测量的改进。专家引导的困难负样本促进了更紧凑的类内表示和改进的类间分离。

**Conclusion:** 该工作突出了在数据稀缺的儿童病理学环境中，明确塑造幻灯片级表示对于鲁棒细粒度分类的重要性，为临床诊断提供了有效的深度学习解决方案。
...
> 本文提出一种方法，仅通过单个观测就能微调扩散先验，通过乘积专家框架和贝叶斯证据最大化来优化先验组合，在黑洞成像和文本条件化去模糊等实际反问题中验证了有效性。

<details>
  <summary>Details</summary>

**Motivation:** 现有扩散先验通常基于有限或纯模拟数据训练，容易继承源数据的错误和偏差。当前的微调方法需要大量不同前向算子的观测数据，这在许多应用中难以收集，且当测量集较小时容易过拟合。

**Method:** 将现有扩散先验组合成乘积专家先验，通过最大化贝叶斯证据来识别最优指数权重。该方法实现了从单个观测数据出发，对扩散先验进行有效微调和组合。

**Result:** 在真实世界反问题（如黑洞成像和文本条件化图像去模糊）上验证了方法的有效性。发现最优先验往往超越了单一训练集的范围，通过指数权重推广先验获得了更灵活的先验。

**Conclusion:** 该方法能够从温度调节和组合扩散模型中进行后验采样，产生更灵活的先验，提高了结果后验图像分布的可信度。仅用单个观测就能实现先验的自适应优化，具有重要的实际应用价值。
...
> Foveated Reasoner是一种受人类视觉启发的视觉语言模型框架，通过在单个解码轨迹中统一中央凹视觉和推理，仅在必要时选择性获取高分辨率证据，从而在严格的视觉令牌预算下保持高精度。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型受益于高分辨率图像，但视觉令牌数量的增加带来了高昂的计算开销，如何在保持视觉细节的同时降低计算成本是该领域的核心挑战。

**Method:** 提出Foveated Reasoner框架，从低分辨率视图开始，仅在需要时触发中央凹视觉，检索选定区域的高分辨率证据并将其注入同一个解码轨迹中。采用两阶段训练：冷启动监督学习引导中央凹行为，随后通过强化学习联合优化证据获取和任务准确性，同时避免'看所有区域'的简单解决方案。

**Result:** 该方法在多个视觉语言基准测试中，在严格的视觉令牌预算下学习到有效的中央凹策略，并获得了更强的准确性表现。

**Conclusion:** Foveated Reasoner成功模拟了人类视觉系统的中央凹机制，实现了视觉推理的精细化控制，为降低视觉语言模型的计算成本提供了有效解决方案。
...
> 本文提出了一种利用大语言模型和街景图像自动评估美国建筑状况的新框架，通过知识蒸馏技术实现了从27B参数到4B参数再到CNN/Transformer模型的高效压缩，在保持高精度的同时实现30倍速度提升。

<details>
  <summary>Details</summary>

**Motivation:** 传统的建筑状况评估依赖大量人工标注，成本高、效率低，难以在全国范围内大规模实施。需要开发一种自动化的评估框架来解决这个问题。

**Method:** 通过微调Gemma 3 27B模型在人工标注数据集上训练，然后应用知识蒸馏技术将其能力转移到更小的Gemma 3 4B模型，进一步蒸馏到EfficientNetV2-M和SwinV2-B模型，并开发了可视化仪表板供房主分析使用。

**Result:** 微调后的27B模型在SRCC和PLCC指标上超越了人类评估者，4B模型达到相当性能且速度提升3倍，CNN/Transformer模型性能接近但速度提升30倍，框架在评估广泛建筑环境属性方面表现良好。

**Conclusion:** 该框架为大规模建筑状况评估提供了灵活高效的解决方案，能够在最少人工标注工作量下实现高精度评估，为房主和城市规划者提供有价值的分析工具。
...
> 研究发现，预训练数据集的光谱多样性与其下游性能强相关，而洲际、生物群系和土地覆盖多样性相关性较弱；欧洲数据集在全球和区域评估中均表现最佳。

<details>
  <summary>Details</summary>

**Motivation:** 现有地理空间基础模型通常将性能差异归因于模型架构，而忽略了预训练数据集地理组成的作用，本研究旨在填补这一研究空白。

**Method:** 创建全球和分洲预训练数据集，在全球和分洲下游数据集上进行评估，并分析10个预训练数据集的洲际、生物群系、土地覆盖和光谱值多样性。

**Result:** 欧洲预训练数据集在全球和本地下游评估中均优于全球和其他洲特定数据集；光谱多样性与性能强相关，其他多样性指标相关性较弱。

**Conclusion:** 光谱多样性是创建高性能预训练数据集时需要考虑的新维度，为地理空间基础模型的预训练数据构建提供了重要指导。
...
> 本文提出了一种新型材料控制的房间脉冲响应(RIR)生成方法，通过解耦空间和材料特征，实现了对声学效果的精确控制。

<details>
  <summary>Details</summary>

**Motivation:** 现有的声学建模方法将空间和材料影响纠缠在一起，限制了用户控制能力并降低了生成声学的真实感，而准确建模这些效应对虚拟现实、机器人等应用至关重要。

**Method:** 采用双模块设计：空间模块捕获场景空间布局的影响，材料模块根据用户指定的材料配置调制空间RIR，实现空间和材料特征的显式解耦。

**Result:** 在声学指标上提升高达+16%(RTE)，材料指标提升达+70%，人工感知研究证实了模型相比基线方法在真实感和材料敏感性方面的改进。

**Conclusion:** 该解耦设计使用户能够在不改变空间结构的情况下轻松修改材料配置并观察声学影响，为声学建模提供了更好的可控性和真实感。
...
> HyperFM是一个参数高效的超光谱基础模型，专为NASA PACE任务设计，通过创新的组内组间光谱注意力机制和混合参数分解技术，在保证性能的同时显著降低了计算成本。

<details>
  <summary>Details</summary>

**Motivation:** NASA PACE任务提供了前所未有的大规模超光谱数据，但现有基础模型存在局限性：无法有效处理连续光谱特征、训练数据局限于无云场景、跨传感器光谱不一致性、模型参数过多计算成本高昂，制约了实际应用。

**Method:** 提出HyperFM模型，采用组内和组间光谱注意力机制捕捉光谱空间关系，结合混合参数分解技术降低计算复杂度，并发布了包含云和晴空场景的大规模PACE超光谱数据集HyperFM250K。

**Result:** 在四个下游大气云属性检索基准任务中，HyperFM相比现有超光谱基础模型和任务特定SOTA方法均取得一致的性能提升，同时保持计算效率。

**Conclusion:** HyperFM为超光谱数据处理提供了高效解决方案，填补了现有基础模型在处理NASA PACE等复杂多场景超光谱数据方面的空白，为气候和环境监测研究奠定了重要基础。
...
> WFM提出了一种基于小波流匹配的新型MRI合成方法，通过从条件模态的小波空间均值作为先验开始合成，实现1-2步快速合成，大幅提升速度同时保持高质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有扩散模型在多模态MRI合成中计算成本过高，需要数百个采样步骤和每个模态单独的模型，限制了临床应用。根本问题在于扩散过程从纯噪声开始，浪费了已有MRI序列中的结构信息。

**Method:** 提出WFM方法，学习从信息先验（条件模态在小波空间的均值）到目标分布的直接流映射。由于源和目标共享底层解剖结构但主要在对比度上不同，该方法支持1-2个积分步骤的准确合成。使用单个82M参数模型和类别条件来合成所有四种BraTS模态。

**Result:** 在BraTS 2024数据集上，WFM达到26.8 dB PSNR和0.94 SSIM，与扩散基线相差仅1-2 dB，但运行速度快250-1000倍（每体积0.16-0.64秒 vs 160秒）。单个模型替代了总参数326M的四个独立扩散模型。

**Conclusion:** WFM在速度-质量权衡上实现了突破，使实时MRI合成在临床工作流程中变得切实可行，为多模态MRI合成提供了高效的解决方案。
...
> 针对点-视觉-语言模型中的几何幻觉问题，提出了几何奖励信用分配机制，通过场特定信号分离和重投影一致性约束，显著提升了3D空间推理的物理可验证性。

<details>
  <summary>Details</summary>

**Motivation:** 点-视觉-语言模型在执行空间推理任务时经常产生几何幻觉，即预测的3D结构与观测的2D现实存在矛盾。这严重阻碍了具身智能体的可靠空间操作能力。

**Method:** 提出几何奖励信用分配框架，将整体监督分解为场特定信号并专门路由到相应的token跨度；引入重投影一致性项作为跨模态验证器，惩罚物理上不可能的几何结构。

**Result:** 在ShapeNetCore基准测试上，3D KPA从0.64提升至0.93，3D边界框IoU达到0.686，重投影一致性分数提升至0.852，同时保持了强健的2D定位性能。

**Conclusion:** 该方法成功弥合了可靠性差距，实现了从合理的文本输出到物理可验证空间预测的重要进展，为具身智能体的可靠空间推理提供了有效解决方案。
...
> WildSplatter是一种基于前馈3D高斯Splatting的新方法，能够从无约束图像中快速重建3D场景，解决了传统3DGS需要已知相机参数和稳定光照的限制。

<details>
  <summary>Details</summary>

**Motivation:** 传统3D高斯Splatting技术虽然能实现高质量实时渲染，但需要迭代优化和多视角图像，且要求已知相机参数和稳定光照条件，限制了其在实际应用中的通用性。

**Method:** 通过在前馈网络中联合学习3D高斯分布和基于输入图像的外观嵌入，设计灵活的高斯颜色调制机制，从而处理光照和外观的显著变化。

**Result:** 能够在不到一秒的时间内从稀疏视角重建3D高斯模型，同时在多样化光照条件下实现外观控制，在具有光照变化的真实世界数据集上优于现有免位姿3DGS方法。

**Conclusion:** WildSplatter成功解决了3DGS在无约束环境下的应用瓶颈，为实际场景的三维重建和渲染提供了更通用、快速的解决方案。
...
> SpatiO是一个异构多智能体框架，通过协调具有互补归纳偏置的视觉语言专家，并采用测试时编排机制动态调整权重，显著提升了视觉空间推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉空间推理方法依赖单一推理管道，隐式学习固定的空间先验，缺乏适应性；而多智能体系统多使用同质智能体，限制了归纳偏置的多样性。

**Method:** 提出SpatiO异构多智能体框架，整合具有不同归纳偏置（2D外观、深度信号、几何约束等）的视觉语言专家；设计测试时编排(TTO)机制，在推理时动态评估和重加权智能体，无需修改模型参数。

**Result:** 在3DSRBench、STVQA-7k、CV-Bench和Omni3D-Bench等多个空间推理基准测试中，SpatiO持续超越闭源和开源基线方法。

**Conclusion:** 空间适应能力对于有效的空间推理至关重要，SpatiO通过异构多智能体协作和动态编排机制，为复杂视觉场景的空间理解提供了新的解决方案。
...
> 本文提出了一种基于伪模拟退火的数据增强框架，通过合成密集鱼类场景显著提高了水下环境目标检测的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的目标检测模型在受控环境中表现良好，但在变化剧烈且存在遮挡的真实水下场景中性能大幅下降。水下环境的复杂性对检测模型的泛化能力提出了严峻挑战。

**Method:** 首先使用DeepFish数据集从分割掩码生成边界框标注构建检测数据集，然后提出受Deng等人启发的新型伪模拟退火增强算法，通过copy-paste策略合成真实的密集鱼类场景，增加训练时的空间多样性和物体密度。

**Result:** 该方法在佛罗里达群岛直播视频收集的挑战性测试集上显著优于基线YOLOv10模型，证明了增强策略在密集真实水下环境中的有效性。

**Conclusion:** 研究表明，所提出的伪模拟退火数据增强框架能够有效提升水下目标检测模型在复杂真实场景中的性能，为解决水下视觉检测问题提供了实用方案。
...
> 稀疏强迫是一种训练和推理范式，通过学习持久视觉块的压缩和更新，以及动态选择局部邻域，显著提升自回归视频扩散模型的长时程生成质量，同时降低解码延迟和内存占用。

<details>
  <summary>Details</summary>

**Motivation:** 观察到自回归扩散模型在生成过程中，注意力会集中在持久性的显著视觉块上，形成隐含的时空记忆，并在滑动窗口内呈现局部结构化的块稀疏模式，这一现象启发设计更高效的计算机制。

**Method:** 提出可训练的原生稀疏机制，学习压缩、保留和更新持久视觉块，同时将每个局部窗口的计算限制在动态选择的局部邻域内；开发了PBSA高效GPU内核来加速稀疏注意力和内存更新。

**Result:** 在5秒文本到视频生成上，VBench分数提升+0.26，解码速度提升1.11-1.17倍，峰值KV缓存减少42%；在更长时程（20秒和1分钟）上效果更显著，VBench分别提升+0.68和+2.74，速度提升1.22倍和1.27倍。

**Conclusion:** 稀疏强迫 paradigm 有效利用了注意力机制的结构化稀疏特性，通过精心设计的稀疏策略和优化实现，在保证生成质量的同时大幅提升了计算效率，特别适用于长时程视频生成任务。
...
> UAU-Net是一个不确定性感知的面部动作单元检测框架，通过表示学习的条件变分自编码器和决策的不对称Beta证据神经网络来提升检测性能和可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的AU检测方法通常将AU表示视为确定性特征，忽略了视觉噪声、个体外观差异和AU间关系模糊性等不确定性因素，同时点估计分类器在严重标签不平衡下会产生过度自信的预测，这些问题都降低了检测的鲁棒性。

**Method:** 在表示阶段，提出CV-AFE基于条件VAE的AU特征提取模块，通过估计多时空尺度的特征均值和方差学习概率化AU表示；在决策阶段，设计AB-ENN不对称Beta证据神经网络，用Beta分布参数化预测不确定性，并通过针对不平衡二分类标签的非对称损失缓解过度自信。

**Result:** 在BP4D和DISFA数据集上的广泛实验表明，UAU-Net取得了强力的AU检测性能，进一步分析表明在表示学习和证据预测中建模不确定性能够提高鲁棒性和可靠性。

**Conclusion:** 同时建模表示层和决策层的不确定性是提升AU检测性能、鲁棒性和可靠性的有效途径，为处理计算机视觉中的不确定性问题提供了新的思路。
...
> LatRef-Diff是一种基于扩散模型的面部属性编辑框架，通过风格代码替代传统语义方向，实现了精准的面部属性控制和风格操纵。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法在面部属性编辑中面临挑战：条件GAN存在准确性和训练不稳定性问题；扩散模型在风格操纵方面受限于语义方向的表达能力有限。

**Method:** 提出LatRef-Diff框架，用风格代码替代语义方向，设计了潜在引导和参考引导两种生成方法；构建风格调制模块，结合可学习向量、交叉注意力和分层设计；采用前向-向后一致性训练策略增强稳定性，无需配对图像。

**Result:** 在CelebA-HQ数据集上的实验表明，LatRef-Diff在定性和定量评估中都达到了最先进的性能，消融研究验证了设计的有效性。

**Conclusion:** LatRef-Diff成功解决了面部属性编辑中的精度和控制问题，为虚拟头像和照片编辑应用提供了有效的解决方案。
...
> ImageHD是一种基于超维度计算的FPGA加速器，用于实现高效的设备端视觉持续学习，在严格资源限制下实现了显著的性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有设备端持续学习方法依赖反向传播和大量样本，计算、内存和延迟开销大，而基于超维度计算的方法虽然轻量但依赖复杂内存层次和集群管理，限制了在资源受限硬件上的部署。

**Method:** 提出硬件感知的持续学习方法，通过统一样本内存限制类别样本，结合硬件高效的集群合并策略；采用量化CNN前端降低部署成本；在FPGA上实现流式数据流架构，使用字打包二进制超向量进行大规模并行位运算。

**Result:** 在CORe50数据集上，ImageHD相比优化CPU(GPU)基线实现40.4x(4.84x)加速和383x(105.1x)的能效提升。

**Conclusion:** 证明了基于超维度计算的持续学习在实时边缘AI应用中的实用性，为资源受限环境下的持续学习提供了高效的硬件解决方案。
...
> AttDiff-GAN是一个结合了GAN属性操作和扩散图像生成的混合框架，通过特征级对抗学习实现精确的面部属性编辑，在保持非目标属性的同时获得更好的真实感。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于GAN的方法在风格代码与属性语义对齐方面表现较弱，而基于扩散的方法由于不同属性间的语义方向纠缠导致编辑精度有限，需要一种能兼顾两者优势的新方法。

**Method:** 提出AttDiff-GAN混合框架，采用特征级对抗学习将属性编辑与图像合成解耦，引入PriorMappe将面部先验融入风格生成，RefineExtractor通过Transformer捕获全局语义关系实现更精确的风格提取，最后用操控后的特征引导扩散过程生成图像。

**Result:** 在CelebA-HQ数据集上的实验表明，该方法在定性和定量评估中都实现了更准确的面部属性编辑和更好的非目标属性保持，优于现有最先进方法。

**Conclusion:** 通过解耦属性编辑与图像合成，结合GAN和扩散模型的优势，AttDiff-GAN有效解决了语义纠缠问题，实现了高精度、高保真的面部属性编辑。
...
> GraphLeap是一种用于视觉图神经网络(ViGs)的加速方法，通过解耦图构建和特征更新，实现高效的FPGA加速，相比CPU和GPU实现最高加速95.7倍和8.5倍。

<details>
  <summary>Details</summary>

**Motivation:** Vision GNNs中每层的动态图构建(kNN)是主要性能瓶颈，消耗50-95%的计算时间，复杂度达O(N²)，且图构建与特征更新之间存在串行依赖，限制了推理速度。

**Method:** GraphLeap通过单层前瞻的图构建方式解耦依赖：当前层ℓ使用前一层构建的图进行特征更新，同时用当前层特征为ℓ+1层构建图。基于此设计了流式层流水线FPGA架构，融合kNN引擎和特征更新引擎。

**Result:** 在Alveo U280 FPGA上，对等径和金字塔ViG模型评估，GraphLeap相比CPU实现最高95.7倍加速，相比GPU实现8.5倍加速，通过少量轮次微调即可恢复原始精度损失。

**Conclusion:** GraphLeap首次实现了端到端的ViG FPGA加速器，证明了实时视觉图神经网络推理的可行性，为边缘部署提供了高效解决方案。
...
> 本研究首次系统探讨了合成数据在可控人类视频生成中的作用，通过提出基于扩散的框架揭示了合成数据与真实数据的互补性，并提供了高效利用合成数据提升生成质量的实用方法。

<details>
  <summary>Details</summary>

**Motivation:** 可控人类视频生成面临数据稀缺的瓶颈，特别是对于罕见身份和复杂动作。现有真实人类视频数据存在规模不足、多样性有限和隐私安全问题，而合成数据虽可作为替代方案，但其跨越Sim2Real差距的实际效果尚未得到系统研究。

**Method:** 提出了一个基于扩散的框架，实现了对外观和动作的细粒度控制，并建立统一测试平台来分析合成数据与真实数据在训练过程中的交互机制。通过大量实验验证不同数据组合对生成效果的影响。

**Result:** 揭示了合成数据与真实数据在视频生成中具有互补作用，证明了通过高效选择合成样本可以显著提升动作真实性、时序一致性和身份保持能力。实验结果表明合成数据能够有效缓解真实数据不足的问题。

**Conclusion:** 本研究为以人为中心的视频合成领域提供了合成数据作用的首次全面探索，为构建数据高效、泛化能力强的生成模型提供了实用见解，有助于推动数字人、动画和具身AI等应用的发展。
...
> 本文提出基于Vision Transformer的深度学习框架，实现了四类脑肿瘤MRI图像的自动分类，测试精度达99.29%。

<details>
  <summary>Details</summary>

**Motivation:** 脑肿瘤是最严重的神经系统疾病之一，早期准确诊断直接影响患者生存率。传统手动MRI解读耗时长、存在观察者间差异且需要大量专业知识。

**Method:** 采用ImageNet-21k预训练的ViT-B/16作为主干网络，结合CLAHE对比度增强、两阶段微调策略（先冻结骨干加热分类头，后全网络微调）、MixUp和CutMix数据增强、EMA权重平均以及测试时增强(TTA)等技术。

**Result:** 在7023张MRI扫描数据集上达到99.29%测试精度、99.25%宏F1分数，在健康和脑膜瘤类别上实现完美召回率，超越所有CNN基线模型。

**Conclusion:** 该框架为脑肿瘤自动诊断提供了高效可靠的解决方案，Attention Rollout可视化技术增强了模型的可解释性，具有重要的临床应用价值。
...
> NTIRE 2026遥感红外图像超分辨率(x4)挑战赛报告，总结赛事设计、数据集、评估结果和团队方法，推动红外图像超分辨率研究发展。

<details>
  <summary>Details</summary>

**Motivation:** 针对遥感场景中红外图像超分辨率技术的实际应用需求，通过举办挑战赛促进有效解决方案的开发，推动该领域技术进步。

**Method:** 采用单赛道设置，通过双三次下采样生成4倍缩放的低分辨率输入，让参赛团队开发高性能的红外图像超分辨率模型。

**Result:** 共有115人注册参赛，13个团队提交有效方案，产生了多种创新方法，为红外图像超分辨率研究提供了新的基准。

**Conclusion:** 该挑战赛成功建立了红外图像超分辨率研究的基准平台，促进了实际遥感应用解决方案的发展，为该领域未来的研究方向提供了参考。
...
> PLAS-Net是一种用于无人机海滩垃圾像素级精确分割的实例分割框架，通过提供物理面积而非简单的物品计数，显著提高了海洋碎片生态风险评估的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的基于边界框的无人机海滩垃圾监测方法会系统性地高估计不规则垃圾物体的平面面积，这对于准确的生态风险评估而言存在重大几何限制。需要开发能够提取像素级精确物理足迹的方法来解决这一问题。

**Method:** 开发了PLAS-Net实例分割框架，能够在复杂的海岸环境下提取海岸碎片的像素级精确物理足迹。在泰国Koh Tao的季风驱动的口袋海滩无人机影像上进行评估，并与11个基线模型进行性能比较。

**Result:** PLAS-Net实现了58.7%的mAP_50，精度高于11个基线模型，在复杂海岸条件下展现出改进的掩码保真度。三个下游分析验证了掩码准确性对环境分析结论的影响： fragmentation dynamics的幂律拟合、空间污染热点图的面积加权生态风险指数、以及揭示丰度-面积悖论的来源组成分析。

**Conclusion:** 像素级面积提取相比单纯基于计数的方法能够为海岸监测提供更有价值的信息，为海洋碎片的生态风险评估提供了更准确的基础数据。
...
> FryNet是一个双流RGB-热成像框架，能够同时完成油区域分割、可用性分类和四个化学氧化指标的回归，实现了煎炸油质量的无损实时监测。

<details>
  <summary>Details</summary>

**Motivation:** 现有的煎炸油降解监测依赖于破坏性的湿化学检测方法，无法提供空间信息且不适用于实时监测。热成像检测存在相机指纹捷径问题，模型会记住传感器特定的噪声和热偏差而非学习氧化化学，在视频不连续评估下会失效。

**Method:** 提出FryNet双流框架，热成像流使用ThermalMiT-B2骨干网络提取热特征，RGB流通过掩码自编码和化学对齐学习化学基础表示。使用双重编码器DANN通过梯度反转层对抗性地正则化两个流，FiLM融合桥接热结构与RGB化学上下文，单次前向传播完成区域分割、分类和回归任务。

**Result:** 在28个煎炸视频的7,226个配对帧上，FryNet实现了98.97% mIoU、100%分类准确率和2.32平均回归MAE，超越了所有七个基线模型。

**Conclusion:** FryNet成功解决了热成像检测中的相机指纹捷径问题，实现了煎炸油质量的准确、无损、实时监测，为食品安全提供了重要的技术支撑。
...
> 本文提出了HiTPro框架，用于解决无监督视频级可见光-红外行人重识别问题，通过层次化时序原型方法实现了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可见光-红外行人重识别方法主要基于图像级别且需要昂贵的身份标注，而无监督视频级VI-ReID问题尽管在实际部署中具有重要意义，但基本上未被探索。

**Method:** HiTPro框架包含时序感知特征编码器、摄像头内轨迹原型构建、层次化跨原型对齐和层次化对比学习四个核心部分，通过渐进式地从模态内关联到跨模态匹配，实现了无监督的特征学习。

**Result:** 在HITSZ-VCM和BUPTCampus数据集上的大量实验表明，HiTPro在完全无监督设置下达到了最先进的性能，显著优于现有的基线方法。

**Conclusion:** HiTPro成功填补了无监督视频级VI-ReID领域的研究空白，为未来的研究建立了强有力的基准，具有重要的实际应用价值。
...
> MiMIC是一种通用多模态检索方法，通过融合解码器架构和鲁棒训练策略解决了现有方法的视觉模态崩塌和语义错位问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的通用多模态检索方法存在明显缺陷：早期融合方法（如Marvel）会出现视觉模态崩塌，过度依赖文本特征；晚期融合方法（如UniVL-DR）则容易产生语义错位，相关内容在嵌入空间中距离过远。

**Method:** 提出了MiMIC方法，包含两个核心创新：(1)融合解码器架构实现有效的多模态整合；(2)通过单模态混合和随机标题丢弃进行鲁棒训练。这种设计既避免了视觉模态崩塌，又减少了语义错位。

**Result:** 在WebQA+和EVQA+数据集上的实验表明，MiMIC在文档或查询图片可能缺少字幕的情况下，持续优于早期融合和晚期融合的基线方法。

**Conclusion:** MiMIC通过创新的架构设计和训练策略，有效解决了现有通用多模态检索方法的关键问题，在更贴近实际应用场景的测试条件下展现出更强的性能和鲁棒性。
...
> TGR-MoE通过利用预训练密集教师模型的指导来稳定稀疏专家混合模型的 Router 学习，解决了传统稀疏 MoE 训练中的梯度阻塞和路由不稳定问题。

<details>
  <summary>Details</summary>

**Motivation:** 稀疏专家混合模型虽然提供了可扩展性，但在训练过程中存在梯度阻塞问题，导致 Router 难以学习合适的专家选择策略，造成路由动态不稳定。

**Method:** TGR-MoE 从预训练教师模型的中间表示构建教师 Router，并将其路由输出作为学生 Router 的伪监督，从而抑制频繁的路由波动。

**Result:** 在 ImageNet-1K 和 CIFAR-100 上的实验证明，TGR-MoE 在提高精度的同时保持了一致的路由选择，即使在高稀疏度配置下也能实现稳定训练。

**Conclusion:** 提出的教师指导路由方法是一种简单有效的解决方案，能够显著改善稀疏视觉专家混合模型的训练稳定性和性能表现。
...
> 提出了一种潜在去噪框架，通过基于显著性的混合掩码和高斯噪声破坏视觉标记，训练LMM恢复干净的教师特征，显著提升了视觉理解和抗干扰能力，且不影响推理效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有大型多模态模型(LMMs)采用自回归语言建模目标，仅提供对视觉标记的间接监督，导致内部视觉表示较弱且在分布偏移下表现脆弱。

**Method:** 提出潜在去噪框架：首先使用显著性感知的混合掩码和高斯噪声破坏投影的视觉标记；然后训练LMM通过解码器从选定的中间LLM层的隐藏状态中恢复干净的教师块特征；为防止表征崩溃，还保留教师的图像内相似性结构并应用图像内对比性块蒸馏。

**Result:** 在标准多模态基准测试中，方法一致地提升了视觉理解和推理能力；在组合鲁棒性基准(如NaturalBench)上获得明显改进；在面对ImageNet-C风格的非对抗性常见干扰时，保持了更高准确率，在中等和严重干扰水平下表现出更小的性能下降。

**Conclusion:** 该潜在去噪框架为LMMs提供了有效的视觉监督形式，显著改善了内部视觉特征对齐和多模态理解，同时不引入额外的推理开销，为提高大型多模态模型的鲁棒性提供了新的解决方案。
...
> 本研究提出了一种基于信任权重的自监督学习方法，通过加法残差机制显著提升航空图像在严重退化条件下的表示学习性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自监督学习方法在处理航空图像时，通过强制增强视图之间的不变性来学习表示，但航空图像常受雾霾、运动模糊、雨和遮挡等退化影响，这些退化会移除关键语义信息，导致强制对齐清洁和严重退化视图会在潜在空间引入虚假结构。

**Method:** 提出了每样本、每因子的信任权重机制，作为加法残差与基础对比损失结合。与传统的乘法门不同，对信任权重应用停止梯度操作。还开发了基于Dempster-Shafer融合的证据变体，引入冲突和无知的可解释信号。

**Result:** 在6个骨干网络上，在EuroSAT、AID和NWPU-RESISC45数据集上达到90.20%的平均线性探测准确率，优于SimCLR(88.46%)和VICReg(89.82%)。在严重雾霾退化条件下比SimCLR提升19.9个百分点，在跨域零样本测试中Mahalanobis AUROC提升1-3个百分点。

**Conclusion:** 加法残差机制是性能提升的主要来源，为不确定性感知的自监督学习提供了具体的设计原则，为处理航空图像退化问题提供了有效解决方案。
...
> SparseGF是一个基于高度感知的稀疏分割框架，通过上下文压缩技术解决了深度学习地面滤波方法在大规模处理中的跨场景泛化限制问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的深度学习地面滤波方法在跨场景泛化方面存在两个关键问题：大规模处理时的上下文-细节困境（计算资源限制导致）和仅优化分类导致的高大物体随机误分类。

**Method:** SparseGF框架包含三个核心创新：(1)受凸镜启发的上下文压缩模块，将广阔上下文压缩为紧凑表示同时保留中心细节； (2)混合稀疏体素-点网络架构，有效解释压缩表示并缓解压缩导致的几何失真； (3)高度感知损失函数，在训练中明确强制地形高程先验以抑制高大物体的随机误分类。

**Result:** 在两个大规模ALS基准数据集上的广泛评估表明，SparseGF在城市到自然地形上实现了鲁棒的地面滤波，在复杂城市场景中表现领先，混合地形上结果具有竞争力，在密集森林陡峭区域虽然准确度中等但不会造成灾难性错误。

**Conclusion:** 这项工作为深度学习地面滤波研究提供了新的见解，推动了朝着真正跨场景泛化的大规模环境监测方向的进一步探索。
...
> 本文提出了一种基于原型的测试时适应方法PTA，通过使用类别特定的知识原型代替传统缓存机制，在保持极高推理效率的同时显著提升了跨域识别性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于缓存的测试时适应方法存在两个关键问题：随着类别数量增加推理延迟显著上升；缓存样本不足或错误时性能表现不佳。这些问题限制了大规模场景下的实用性。

**Method:** PTA采用类别特定的知识原型来累积测试样本知识，根据测试样本的零样本类别置信度自适应加权，将视觉特征整合到对应的类别原型中。这种方法完全避免了缓存的构建和检索开销。

**Result:** 在15个图像识别和4个点云分析基准测试中达到SOTA性能。在10个跨域基准上将CLIP准确率从65.64%提升到69.38%，同时保持92%的推理速度，而基于缓存的TDA方法准确率仅为67.97%且速度只有50%。

**Conclusion:** PTA通过原型机制解决了传统缓存方法的效率和性能瓶颈，为测试时适应提供了一个高效且性能优越的解决方案，特别适合大规模应用场景。
...
> 针对T2V模型在创意视频广告生成中的语义对齐模糊和运动适应性不足问题，本文提出了基于知识驱动的方法KD-CVG，通过知识库增强的语义检索和多模态知识参考模块，显著提升了广告视频的语义准确性和运动真实感。

<details>
  <summary>Details</summary>

**Motivation:** 当前创意内容生成研究大多集中在文本和图像广告，而创意视频生成(CVG)面临两大关键挑战：一是现有文本到视频(T2V)模型难以准确将产品卖点与创意视频内容进行语义对齐，二是模型运动适应性不足导致视频中的动作不真实和扭曲。这些限制阻碍了视频广告自动化生成的发展。

**Method:** 本文构建了广告创意知识库(ACKB)，并提出了知识驱动的创意视频生成方法(KD-CVG)。该方法包含两个核心模块：(1)语义感知检索(SAR)模块，利用图注意力网络的语义感知能力和强化学习反馈来增强模型对卖点与创意视频关系的理解；(2)多模态知识参考(MKR)模块，将语义和运动先验知识融入T2V模型，解决现有模型的知识缺陷。

**Result:** 大量实验证明，KD-CVG在语义对齐和运动适应性两个关键指标上均表现出优越性能，验证了该方法相比其他先进方法的有效性。研究团队将在指定网址开源代码和数据集。

**Conclusion:** 本研究通过引入知识驱动的思路，成功解决了文本到视频模型在广告创意生成中的主要限制。KD-CVG方法不仅提升了生成视频的语义准确性和运动质量，为创意视频广告生成提供了新的技术路径，相关资源将开源以促进领域发展。
...
> 本文提出了EdgeFormer，一个基于学习的3D点云边缘检测网络，通过将点云转换为局部补丁来有效提取细粒度边缘特征。

<details>
  <summary>Details</summary>

**Motivation:** 3D点云边缘特征在工业和商业应用中需求很高，但细粒度边缘特征由于密集分布或小尺度表面梯度而难以有效检测。

**Method:** 采用两阶段方法：第一阶段构造描述每个点局部邻域的补丁特征描述子；第二阶段通过分析第一阶段生成的局部补丁特征描述子来对每个点进行分类。

**Result:** 实验结果表明，与六个基线方法相比，该模型展现出具有竞争力的性能。

**Conclusion:** 通过将点云转换为局部补丁，提出的EdgeFormer方法能够有效提取更精细的细节，为3D点云边缘检测提供了有效的解决方案。
...
> 研究人员提出了VG-CoT数据集，通过全自动三阶段流程将大视觉语言模型的推理步骤与图像中的真实视觉证据显式对齐，并开发了新的评估基准来增强模型基于证据的可信推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有数据集面临可扩展性限制，因为需要大量人工标注，并且缺乏多步推理与相应图像区域之间的显式对齐，这限制了模型可信度的评估。为提升大视觉语言模型的局部区域推理能力，需要更好的数据集和评估方法。

**Method:** 提出了Visual Grounding Chain-of-Thought (VG-CoT)数据集，包含全自动三阶段流程：1) 使用最先进的检测和OCR模型提取对象级和文本级视觉证据；2) 使用GPT-4o生成逐步基础推理；3) 通过基于原理的开放集检测过程优化基础定位。同时引入了包含推理质量、答案准确性和推理-答案对齐三个互补维度的新评估基准。

**Result:** 在LLaVA-1.5和Qwen2-VL等代表性大视觉语言模型上的实验表明，在大多数评估指标上都有一致的改进，证明VG-CoT有效增强了基于证据的可信推理，同时保持了可扩展和高效的数据集构建。

**Conclusion:** VG-CoT数据集成功解决了现有数据集的可扩展性和对齐问题，为大视觉语言模型的可信推理提供了新的评估标准。该研究有望推动该领域的进一步发展，数据集和代码将在接受后公开发布。
...
> YOGO是一个面向生产的3D高斯渲染系统框架，通过确定性资源分配和多传感器融合协议解决了现有方法的业界-学术差距问题，并发布了超密集室内数据集Immersion v1.0。

<details>
  <summary>Details</summary>

**Motivation:** 现有3D高斯渲染方法主要是研究原型，不适合生产级部署，存在三大问题：启发式高斯增长导致的不可预测资源消耗、当前基准测试的'稀疏保护'现象奖励幻觉而非物理保真度、以及严重的多传感器数据污染。

**Method:** 提出YOGO系统级框架，将随机增长过程重新构建为确定性、预算感知的均衡状态；集成新型预算控制器进行硬件约束资源分配；引入可用性注册协议实现鲁棒多传感器融合；发布Immersion v1.0超密集室内数据集，提供饱和视点覆盖。

**Result:** YOGO在保持严格确定性配置的同时实现了最先进的视觉质量，打破了稀疏保护现象，迫使算法专注于极端物理保真度而非视点插值，为生产级3DGS建立了新标准。

**Conclusion:** YOGO成功 bridged the industry-academia gap in 3D高斯渲染领域，通过确定性资源管理和高质量数据集推动了高保真重建技术的发展，为生产环境应用提供了可靠解决方案。
...
> S1-VL是一个支持科学推理和图像思维两种范式的多模态科学推理模型，通过Python代码执行主动操作图像，在13个基准测试中取得领先性能。

<details>
  <summary>Details</summary>

**Motivation:** 解决科学领域中高分辨率图表解读、显微图像理解和几何辅助推理等复杂场景的挑战，开发能够融合结构化思维链和主动图像操作的多模态推理能力。

**Method:** 1) 结合科学推理(结构化思维链)和图像思维(通过Python代码执行操作图像)两种推理范式 2) 构建涵盖数学、物理、化学、天文、地理、生物六大学科的科学多模态数据集 3) 开发六维质量过滤框架和多阶段过滤管道，采用自适应数据路由策略减少冗余视觉操作 4) 通过四阶段渐进训练：科学多模态SFT、图像思维冷启动SFT和两阶段SAPO强化学习。

**Result:** S1-VL-32B在所有5个图像思维基准测试(HRBench-4K、HRBench-8K、MME-RealWorld-CN等)达到state-of-the-art性能，在Physics、VRSBench等科学推理基准上也超越对比系统。

**Conclusion:** 通过创新的图像思维范式和精心设计的训练策略，S1-VL成功实现了在复杂科学视觉推理任务上的突破，为科学领域多模态AI模型的发展提供了新的技术路径。
...
> 本文提出了一种新的非线性扩散滤波器族，通过求解前向非线性扩散方程来获得分段常数图像，适用于图像分割预处理。

<details>
  <summary>Details</summary>

**Motivation:** 现有分割技术需要预处理得到分段常数图像，传统方法存在边缘模糊问题。本文旨在设计能够在保持边缘清晰的同时实现内部强度均匀化的扩散滤波器。

**Method:** 首先建立非线性扩散方程的内禀表述，提供滤波器设计条件。在此基础上提出新的扩散系数族，它们与反向扩散技术相关。采用半隐式格式求解前向非线性扩散方程，并满足适定性和尺度空间要求。设置扩散时间停止准则以获得分段常数图像。

**Result:** 所提出的滤波器满足半离散和全离散尺度空间要求，能够在保持边缘的同时实现图像分段常数化，计算效率较高。在真实图像上的实验验证了方法的有效性。

**Conclusion:** 本文成功提出了一种基于非线性扩散的图像预处理方法，能够高效生成适用于分割技术的分段常数图像，同时保持边缘清晰性，相关工作已开源。
...
> UHR-DETR是一种针对超高分辨率遥感图像的高效目标检测器，通过稀疏编码和全局-本地解耦解码解决了内存瓶颈问题，在保持高精度的同时实现了10倍推理加速。

<details>
  <summary>Details</summary>

**Motivation:** 超高分辨率遥感图像中检测小目标面临关键困境：保持原始分辨率会导致内存瓶颈，而传统的下采样或分裁剪方法会丢失小目标或破坏上下文信息。

**Method:** 提出UHR-DETR框架：1) 覆盖最大化稀疏编码器，动态分配计算资源到信息丰富的高分辨率区域；2) 全局-本地解耦解码器，整合宏观场景感知和微观目标细节。

**Result:** 在STAR和SODA-A等超高分辨率数据集上，相比标准滑窗基线方法，mAP提升2.8%，推理速度提升10倍，且在单张24GB RTX 3090显卡上运行良好。

**Conclusion:** UHR-DETR成功打破了超高分辨率图像目标检测的质量与效率权衡，为实际应用提供了可行的解决方案。
...
> 提出了一种名为2L-LSH的两步局部敏感哈希算法，用于在大规模3D点云模型中实现快速邻域点搜索，相比Kd-tree和Octree在搜索速度上有显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 3D扫描技术发展导致大规模点云模型的获取，这些模型具有多样化结构和大规模特征，给点云处理带来挑战。快速邻域点搜索作为模型重建、分类、检索和特征可视化等应用的核心问题，需要更高效的解决方案。

**Method:** 采用两步哈希函数策略：第一步对点云模型的包围盒进行划分，第二步构建基于表的广义数据结构。利用哈希函数在高维数据搜索中的高速准确性能，设计了2L-LSH算法。

**Result:** 与Kd-tree和Octree相比，2L-LSH在搜索速度上表现优异。kNN搜索的时间消耗分别比Kd-tree和Octree低51.111%和94.159%，RN搜索时间分别低54.519%和41.840%。

**Conclusion:** 2L-LSH为大规模3D点云模型的快速邻域点搜索提供了一个高效准确的解决方案，在速度方面明显优于传统的Kd-tree和Octree方法，是该领域一种有前景的技术。
...
> VARestorer是一种将预训练视觉自回归模型转化为单步超分辨率模型的蒸馏框架，通过金字塔图像条件机制和参数高效微调实现最优性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的视觉自回归模型(VAR)在应用于真实图像超分辨率(Real-ISR)时面临两个关键问题：1) 因果注意力限制导致无法充分利用低质量图像的全局上下文，产生模糊不一致的高质量输出；2) 迭代预测中的误差累积严重影响重建连贯性。

**Method:** 提出VARestorer框架，包含三个核心技术：1) 使用分布匹配蒸馏将VAR转化为单步ISR模型，消除迭代需求；2) 引入金字塔图像条件与跨尺度注意力机制，实现双向尺度间交互；3) 仅微调1.2%模型参数的适配器，保持原始模型表达能力。

**Result:** 在DIV2K数据集上达到最先进性能：MUSIQ评分72.32，CLIPIQA评分0.7669，推理速度比传统VAR方法快10倍。

**Conclusion:** VARestorer成功解决了VAR在ISR任务中的核心缺陷，在保持模型表达能力的同时显著提升了效率和重建质量，为视觉自回归模型在图像恢复领域的应用提供了新的解决方案。
...
> 提出OA-VAT系统，通过实例感知原型初始化、在线原型增强跟踪和遮挡感知轨迹规划三个模块，解决视觉主动跟踪中的相似干扰物混淆和遮挡问题，在多个数据集上实现显著性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 视觉主动跟踪在实际部署中面临两个关键瓶颈：视觉相似干扰物导致的混淆（缺乏实例级辨别能力）和遮挡下的严重故障（缺乏主动规划机制）。

**Method:** 提出统一的三模块框架：1）无需训练的实例感知离线原型初始化，使用DINOv3聚合多视图增强特征构建辨别性实例原型；2）在线原型增强跟踪器，在线增强原型并集成感知置信度的卡尔曼滤波器；3）遮挡感知轨迹规划器，基于新构建的Planning-20k数据集训练，使用条件扩散生成避障路径。

**Result:** 在UnrealCV数据集上平均SR达0.93（比SOTA TrackVLA提高2.2%），真实世界数据集上平均CAR达90.8%（比SOTA GC-VAT提高12.1%），在DJI Tello无人机上TSR达81.6%，在RTX 3090上运行速度35 FPS。

**Conclusion:** OA-VAT通过实例级辨别和主动遮挡规划相结合，实现了鲁棒、实时的视觉主动跟踪性能，为无人机导航和安全监控等实际应用提供了有效解决方案。
...
> 该研究开发了EgoPoint-Bench基准来评估并提升以自我为中心的AI系统中指向推理能力，解决了当前MLLMs在空间语义理解上的'指代幻觉'问题。

<details>
  <summary>Details</summary>

**Motivation:** 以自我为中心的AI代理（如智能眼镜）在自然语言命令解析中存在指代歧义，当前多模态大语言模型无法精确定位指向的空间语义，而是依赖于与视觉邻近性或物体显著性的伪相关，导致指代幻觉现象。

**Method:** 研究人员构建了EgoPoint-Bench，一个包含超过1.1万个高保真模拟和真实世界样本的问答基准，涵盖5个评估维度和3个指代复杂性级别，并使用合成数据对模型进行微调。

**Result:** 实验表明，最先进的专有和开源模型在以自我为中心的指向任务上表现不佳，但在合成数据上微调的模型获得了显著的性能提升和强大的模拟到真实泛化能力。

**Conclusion:** 这项研究强调了空间感知监督的重要性，为实现精确的以自我为中心的AI助手提供了可扩展的路径，通过合成数据微调有效解决了指代幻觉问题。
...
> ID-Eraser是一种基于特征空间的人脸主动防御技术，通过移除可识别的面部信息来防止恶意换脸攻击。

<details>
  <summary>Details</summary>

**Motivation:** 现有的像素级扰动防御方法对现代深伪技术无效，因为这些技术能提取鲁棒的高级身份嵌入。需要一种更有效的主动防御措施来保护面部隐私和数字安全。

**Method:** ID-Eraser在身份嵌入空间注入可学习扰动，并通过面部复活生成器(FRG)重建自然外观的保护图像，在视觉上对人保持真实感，同时使受保护的身份对深伪模型不可用。

**Result:** 在严格的黑箱设置下，ID-Eraser显著降低身份识别准确率(Top-1准确率仅0.30)，保持最佳图像质量(FID 1.64, LPIPS 0.020)，将换脸相似度从正常水平骤降至0.504，腾讯API相似度从0.76降至0.36。

**Conclusion:** ID-Eraser在跨数据集泛化、抗常见畸变鲁棒性和商业API实用性方面表现出色，为面部隐私保护提供了有效的解决方案。
...
> 本文提出Cross-AUC评估指标和SFAM框架，解决了跨数据集人脸伪造检测中的分数不可比性问题，并显著提升了检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 随着生成模型的快速发展，视觉数据伪造检测在社会经济安全中显得日益重要。现有的人脸伪造检测器在跨数据集上泛化能力不足，主要原因是缺乏合适的评估指标来衡量不同数据域间检测分数的可比性问题。

**Method:** 1) 提出Cross-AUC评估指标，通过对比一个数据集的真实样本与另一个数据集的伪造样本来计算AUC；2) 提出SFAM框架，包含patch级图像-文本对齐模块增强CLIP对伪造痕迹的敏感性，以及面部区域专家混合模块实现区域感知的伪造分析。

**Result:** 实验表明，在Cross-AUC指标评估下，现有检测器性能大幅下降，暴露了被忽视的鲁棒性问题。所提出的SFAM方法在多个公共数据集上超越了最先进方法。

**Conclusion:** Cross-AUC指标揭示了跨域伪造检测中的关键问题，SFAM框架通过语义细粒度对齐和专家混合机制有效提升了检测性能，为实际应用提供了更可靠的技术方案。
...
> 本研究提出了一个评估大型语言模型在自动驾驶中理解动态交通主体和静态道路网络能力的框架，通过将轨迹和地图信息转换为LLM兼容的token，实现LLM驱动的轨迹预测。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型展现出强大的推理能力，但要在自动驾驶中安全应用，需要深入理解动态交通主体行为和静态道路基础设施。目前缺乏对LLM在这些方面能力的系统性评估框架。

**Method:** 使用冻结的LLM作为推理引擎，通过交通编码器提取主体轨迹的空间场景特征，用轻量CNN编码局部高清地图，再通过重编程适配器将场景特征转换为LLM兼容token，最后用简单线性解码器输出未来轨迹。

**Result:** 框架能够定量分析多模态信息对轨迹预测的影响，特别是地图语义的作用，并且可以无缝集成各种LLM架构，展现出强大的泛化能力。

**Conclusion:** 该框架为评估LLM在自动驾驶感知和预测中的能力提供了统一平台，证明了冻结LLM只需最少的适配就能在自动驾驶任务中有效工作，为LLM在自动驾驶领域的安全应用奠定了基础。
...
> VFM^4SDG是一种用于单域泛化目标检测的双先验学习框架，通过引入视觉基础模型作为跨域稳定性先验，显著提升了检测器在未知环境下的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现实场景中天气、光照和成像条件的持续变化会导致严重的域偏移，使在单一源域训练的检测器在未知环境中性能严重下降。现有方法主要依赖数据增强或域不变表示学习，但忽略了检测器机制本身在复杂域偏移下的局限性。

**Method:** 提出双先验学习框架VFM^4SDG，将冻结的视觉基础模型作为可迁移的跨域稳定性先验。在编码阶段，通过跨域稳定关系先验蒸馏增强目标-背景和实例间关系建模的鲁棒性；在解码阶段，基于语义-上下文先验的查询增强通过向查询注入类别级语义原型和全局视觉上下文来提高语义识别和空间定位的稳定性。

**Result:** 在标准SDGOD基准测试和两种主流DETR检测器上的大量实验表明，所提方法持续超越现有最先进方法，证明了其有效性、鲁棒性和泛化能力。

**Conclusion:** 性能下降主要由漏检率增加主导，根本原因是检测器跨域稳定性降低。VFM^4SDG框架通过视觉基础模型先验有效解决了这一问题，为单域泛化目标检测提供了新的解决思路。
...
> 提出基于高斯混合模型(GMM)的新型局部描述符(GMD)用于文物碎片断裂面匹配，通过分割凹凸区域估计GMM参数，结合L2距离、RANSAC和ICP实现碎片自动拼接。

<details>
  <summary>Details</summary>

**Motivation:** 激光扫描获取的碎片自动重组中，断裂面匹配是关键步骤，现有方法在描述和匹配复杂断裂面方面存在局限。

**Method:** 将局部表面补丁分为凹凸区域估计GMM的k值，合并区域GMD形成最终描述符，采用L2距离测量相似性，使用RANSAC和ICP对齐碎片。

**Result:** 在真实扫描公开数据集和兵马俑数据集上的广泛实验证明了方法的有效性，与现有方法对比验证了优势。

**Conclusion:** 基于GMM的高斯混合描述符能有效描述和匹配碎片断裂面，为文物数字化保护提供了可靠的自动拼接解决方案。
...
> 该研究通过系统评估发现，当前用于评估其他模型输出的大型视觉语言模型存在严重的可靠性问题，它们经常无法检测到图像质量退化错误，尤其是在对象幻觉、空间推理和视觉 fidelity 等方面。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型视觉语言模型(VLMs)越来越多地被用于评估其他模型的输出结果，特别是在图像到文本和文本到图像任务中，但这些评估器模型的可靠性尚未得到充分探索，这可能导致基于错误评估做出错误的基准测试和开发决策。

**Method:** 研究人员引入了有针对性的扰动来降低输出质量，涵盖对象幻觉、空间推理、事实基础和视觉逼真度等关键错误维度。他们使用超过4000个扰动实例和40个扰动维度的综合基准，对4个主要VLM进行单答案评分、成对比较和参考引导评估。

**Result:** 研究发现当前VLM评估器存在显著盲点：经常无法检测到扰动输出（某些情况下超过50%），特别在细粒度组合和空间错误方面表现不佳，且对与输入图像矛盾的幻觉内容不敏感。成对比较相对更可靠，但失败率仍然存在。

**Conclusion:** 研究结果强调了当前评估器VLM的不可靠性，呼吁在将其用于基准测试和开发决策时需要谨慎。研究团队已公开代码和数据供其他研究者使用。
...
> 本研究提出了一种基于注意力的多实例学习框架，用于在整张切片级别预测肺腺癌主要生长模式，显著减少标注负担。

<details>
  <summary>Details</summary>

**Motivation:** 肺腺癌分级需要准确识别生长模式，这对预后评估和治疗决策具有重要意义。现有深度学习方法通常基于切片级别的分类或分割，需要大量标注，临床应用受限。

**Method:** ABMIL框架整合预训练病理基础模型作为编码器（可冻结或微调），通过注意力机制提取和聚合特征，在整张切片级别进行预测，降低了对详细标注的依赖。

**Result:** 实验表明微调的编码器性能提升，其中Prov-GigaPath在ABMIL下达到最高一致性（κ=0.699）。相比简单的切片聚合基线，ABMIL利用切片级监督和空间注意力产生更稳健的预测。

**Conclusion:** 该框架有效减少了标注需求，提供了更可靠的肺腺癌生长模式预测。未来工作将扩展到生长模式完整分布估计并在外部队列中验证性能。
...
> 基于组件的分布式外检测框架CoOD，无需训练即可有效检测外观偏移和组合不一致性，在粗粒度和细粒度OOD检测中均取得改进。

<details>
  <summary>Details</summary>

**Motivation:** 现有OOD检测方法存在粒度问题：全局表示抑制局部线索，补丁方法因伪相关和噪声而不稳定，且难以检测由有效ID组件组成的组合性OOD样本。

**Method:** 提出训练免费的组件级OOD检测框架CoOD，通过分解输入为功能性组件，推导组件偏移分数(CSS)检测局部外观偏移，组合一致性分数(CCS)识别跨组件组合不一致性。

**Result:** CoOD在粗粒度和细粒度OOD检测任务上均取得了一致的改进效果，证明了组件级方法的有效性。

**Conclusion:** 基于组件的分解方法能够有效解决传统OOD检测的粒度问题，无需训练即可同时处理外观偏移和组合不一致性的检测，是OOD检测领域的一种新思路。
...
> 本文提出了一种基于深度核空间学习的无监督动作分割方法，通过MMD度量和NTK核函数实现高性能的单视频动作分割。

<details>
  <summary>Details</summary>

**Motivation:** 针对在无法存储或允许存储大型数据集的应用场景，需要开发单视频无监督动作分割方法。现有方法存在优化困难、计算效率低等问题。

**Method:** 提出在深度核空间中学习视频分割，使用最大均值差异(MMD)作为分布距离度量，采用神经正切核(NTK)作为核函数以增强描述能力并避免平凡解。

**Result:** 在六个标准基准测试上取得了与最先进单视频方法相当的结果，在段落数量未知时优于先前的聚类方法。

**Conclusion:** 该方法结合MMD度量的几何保持特性和NTK的强描述能力，为单视频无监督动作分割提供了一个有效且高效的解决方案。
...
> CHRep是一个双阶段框架，通过结构感知表示学习和轻量校准模块，解决了从H&E病理图像预测基因表达时的切片级偏移和过平滑问题，显著提升了跨切片预测的鲁棒性和准确性。

<details>
  <summary>Details</summary>

**Motivation:** 空间转录组学技术成本高昂且通量低，限制了大样本研究和临床应用。从常规H&E病理图像预测基因表达是可行的替代方案，但现有模型在留一切片评估中存在切片级外观偏移和回归驱动的过平滑问题，抑制了生物学上有意义的变异。

**Method:** CHRep采用两阶段框架：训练阶段通过联合优化相关性感知回归、对称图像表达对齐和坐标诱导空间拓扑正则化学习结构感知表示；推理阶段通过轻量校准模块提升跨切片鲁棒性，该模块结合训练库的非参数估计和幅度正则化修正模块，无需微调主干网络。

**Result:** 在三个队列的留一切片评估中，CHRep在基因相关性上表现一致提升，在Alex+10x上增益最大。相比HAGE，在cSCC和HER2+队列中全基因PCC分别提升4.0%和9.8%；相比mclSTExp，在Alex+10x上PCC提升39.5%，MSE和MAE分别降低9.7%和9.0%。

**Conclusion:** CHRep通过结合拓扑保持表示学习和后验校准，实现了在切片级偏移下的稳定邻域检索和可控偏置校正，为从常规病理图像预测空间基因表达提供了更鲁棒、准确的解决方案。
...
> OmniFit提出了一种多模态、尺度无关的人体模型拟合方法，通过条件transformer解码器将表面点映射到密集身体标志点，实现了对真实和合成3D人物资产的高精度拟合。

<details>
  <summary>Details</summary>

**Motivation:** 现有的人体模型拟合方法大多局限于单一模态输入（如点云或多视图图像），且需要已知度量尺度，这对于AI生成的、常存在尺度扭曲的资产来说不切实际。

**Method:** 提出条件transformer解码器直接将表面点映射到密集身体标志点，通过可选的图像适配器融合视觉信息补偿几何信息缺失，并引入尺度预测器将人体重新缩放到标准比例。

**Result:** 在宽松服装场景下超越最新方法57.1-80.9%，首次超越多视图优化基线，并在CAPE和4D-DRESS基准测试中达到毫米级精度。

**Conclusion:** OmniFit是首个能够处理多模态输入、尺度无关的人体拟合方法，为AI生成的3D人物资产提供了实用的解决方案，显著提升了拟合精度。
...
> Sculpt4D是一个高效的4D生成框架，通过在预训练3D扩散模型中引入块稀疏注意力机制，实现了高保真动态4D内容生成，同时将计算开销降低56%。

<details>
  <summary>Details</summary>

**Motivation:** 当前3D生成建模在静态形状合成方面取得显著进展，但高保真动态4D生成仍然面临挑战，主要受限于时间伪影和过高的计算需求，同时4D训练数据稀缺。

**Method:** 该方法在预训练的3D扩散模型(Hunyuan3D 2.1)基础上，引入块稀疏注意力机制。该机制通过锚定初始帧保持物体身份一致性，并利用时间衰减稀疏掩码捕捉丰富的运动动态，有效建模了复杂的时空依赖关系。

**Result:** 在保持高保真度的同时，成功避免了全注意力的二次计算复杂度，将网络总计算量减少了56%，并在时间相干的4D合成任务中建立了新的最先进性能。

**Conclusion:** Sculpt4D为高效可扩展的4D生成开辟了新路径，通过创新的时间建模策略有效解决了4D生成中的关键挑战，为未来的动态内容生成提供了重要的技术基础。
...
> 本文提出了一个评估参数化投影稳定性的框架，通过高斯扰动分析投影的局部稳定性，并提供了定量和可视化评估方法。

<details>
  <summary>Details</summary>

**Motivation:** 参数化投影允许分析师实时嵌入新数据点，但输入数据的变化（如测量噪声或数据漂移）会导致2D布局产生不可预测的偏移，然而投影是否在局部保持稳定这一问题尚未得到充分研究。

**Method:** 提出一个稳定性评估框架：在选定的锚点周围施加高斯扰动，分析2D嵌入中邻域的变形情况。结合平均位移、偏差、最近锚点分配误差等定量指标，以及位移向量、局部PCA椭球和Voronoi错误分配的逐锚点可视化进行详细检查。

**Result:** 在基于UMAP和t-SNE的不同规模神经网络投影器上验证了框架有效性，并研究了雅可比正则化作为基于梯度的鲁棒性策略。结果显示，该框架能识别出重构误差或邻域保持指标无法检测到的不稳定投影区域。

**Conclusion:** 该框架为评估参数化投影的局部稳定性提供了有效工具，能够揭示传统指标无法发现的投影不稳定区域，对于实际应用中的投影选择和优化具有重要意义。
...
> DCMorph是一种双流扩散型人脸变形攻击框架，通过身份条件特征和潜在空间的双重操作，实现了更高的人脸识别系统攻击成功率，且难以被现有检测方法发现。

<details>
  <summary>Details</summary>

**Motivation:** 为了应对不断演变的身份验证系统威胁，需要开发先进的人脸变形攻击技术，以帮助设计和构建更为强大的防御机制。

**Method:** DCMorph采用双流扩散架构：(1)通过解耦交叉注意力插值将两个源身份特征注入去噪过程，实现显式双身份条件；(2)使用DDIM反演和球面插值在源人脸的潜在表示间进行插值，保持结构属性的一致性。

**Result:** 在四个最先进的人脸识别系统上的漏洞分析表明，DCMorph在不同操作阈值下都比现有方法获得了最高的攻击成功率，同时当前的人脸变形攻击检测方案难以发现此类攻击。

**Conclusion:** DCMorph通过创新的双流扩散机制，在保持高质量图像生成的同时显著提升了人脸变形攻击的效果，对身份验证系统的安全性提出了新的挑战，也强调了开发更强大防御技术的必要性。
...
> DualSplat是一个双阶段3D高斯泼溅框架，首次利用重建失败的显式先验来解决瞬态对象干扰的反向依赖问题。

<details>
  <summary>Details</summary>

**Motivation:** 3D高斯泼溅在包含瞬态对象（违反多视角一致性的临时物体）的场景中性能严重下降，而现有方法存在循环依赖问题：准确的瞬态检测需要良好的静态场景重建，而干净的重建又依赖于可靠的瞬态掩码。

**Method:** 提出Failure-to-Prior框架：第一阶段保守训练产生不完整的瞬态片段，结合光度残差、特征不匹配和SAM2实例边界构建对象级伪掩码；第二阶段使用这些伪掩码指导干净的3DGS优化，同时通过轻量级MLP在线精炼掩码，逐步从先验监督转向自一致性。

**Result:** 在RobustNeRF和NeRF On-the-go数据集上的实验表明，DualSplat优于现有基线方法，在瞬态对象密集场景和瞬态区域表现尤其突出。

**Conclusion:** 通过将第一阶段重建失败转化为显式先验，成功解决了瞬态对象检测和场景重建之间的循环依赖，实现了更鲁棒的含瞬态对象场景的三维重建。
...
> 本文提出了一种基于因果推理和解耦表示学习的新型全参考图像质量评估方法，通过干预潜在表示引导的因果解纠缠过程来估计图像退化，在跨域场景中无需标签数据即可实现优越性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于深度网络的FR-IQA模型主要通过对参考图像和失真图像的深度特征进行成对比较来工作，但这种方法在某些场景下存在局限性。作者希望从新的角度解决这个问题，提高模型在数据稀缺的非标准图像域中的泛化能力。

**Method:** 该方法首先利用参考图像和失真图像之间的内容不变性来解耦退化和内容表示；其次，受人类视觉掩蔽效应启发，设计掩蔽模块建模图像内容与退化特征间的因果关系；最后通过监督回归或无标签降维从退化特征预测质量分数。

**Result:** 在标准IQA基准测试的全监督、少标签和无标签设置下都取得了高度竞争性的性能，在水下、放射学、医学、中子和屏幕内容等数据稀缺的非标准自然图像域中表现出卓越的跨域泛化能力。

**Conclusion:** 该方法能够在没有标签IQA数据的情况下进行场景特定的训练和预测，相比现有的无需训练FR-IQA模型具有明显的优势，为图像质量评估提供了一种新颖且有效的解决方案。
...
> 提出结构化运动描述(SMD)方法，将关节位置序列转换为结构化自然语言描述，让LLM直接通过文本进行人类运动理解和推理，无需专门的跨模态编码器或对齐模块。

<details>
  <summary>Details</summary>

**Motivation:** 当前基于LLM的人类运动理解方法需要专门的编码器来对齐运动特征和文本，受限于跨模态表示和对齐，未能充分利用LLM预训练的知识和推理能力。

**Method:** 受生物力学分析启发，开发了基于规则的确定性方法SMD，将关节位置序列转换为关节角度、身体部位运动和全局轨迹的结构化自然语言描述，使LLM能直接利用预训练知识进行运动推理。

**Result:** 在运动问答(BABEL-QA上66.7%，HuMMan-QA上90.1%)和运动字幕(HumanML3D上R@1 0.584，CIDEr 53.16)任务上超越所有先前方法，达到SOTA水平。

**Conclusion:** SMD方法有效规避了跨模态对齐问题，在8个不同LLM上通过轻量级LoRA适配即可工作，提供了人类可读的表示，便于可解释性分析，为人类运动理解开辟了新范式。
...
> Sapiens2是一个专注于人体视觉的高分辨率Transformer模型家族，通过改进预训练方法、扩大高质量数据集和优化架构设计，在姿态估计、分割等多个任务上大幅超越前代模型。

<details>
  <summary>Details</summary>

**Motivation:** 为了构建一个具有强泛化能力、多功能性和高质量输出的以人为中心的视觉模型，克服前代模型在细节捕捉和语义理解方面的局限性。

**Method:** 结合掩码图像重建和自蒸馏对比学习的统一预训练目标；使用10亿张精选高质量人体图像进行预训练；采用窗口注意力机制处理4K分辨率；模型参数规模从0.4亿到50亿不等。

**Result:** 在关键点检测、身体部位分割、法向量估计等多个基准测试中创造了新的SOTA，姿态估计提升4 mAP，分割提升24.3 mIoU，法向量估计角误差降低45.6%，并成功扩展到深度图和反射率估计等新任务。

**Conclusion:** Sapiens2通过系统性的改进，证明了统一预训练目标、优质数据和先进架构的有效性，为人体视觉任务提供了强大的基础模型。
...
> 论文提出了WorldMark——首个为交互式图像到视频世界模型提供的标准化评估基准，通过统一控制接口和标准化测试条件实现跨模型的公平对比。

<details>
  <summary>Details</summary>

**Motivation:** 当前交互式视频生成模型（如Genie、YUME等）各自使用私有基准进行评估，缺乏统一的测试条件和控制接口，导致无法进行跨模型的公平比较，阻碍了该领域的发展。

**Method:** WorldMark包含三个核心组件：(1)统一动作映射层，将共享的WASD式动作词汇转换为各模型的本地控制格式；(2)包含500个评估案例的分层测试套件，涵盖不同视角、场景类型和难度级别；(3)模块化评估工具包，支持视觉质量、控制对齐和世界一致性三个维度的评估。

**Result:** 实现了对六个主流世界模型在相同场景和轨迹下的公平对比评估，提供了标准化的评估框架和数据集，并发布了World Model Arena在线平台供研究者进行实时模型对战和排行榜查看。

**Conclusion:** WorldMark填补了交互式世界模型评估领域的空白，通过提供标准化的基准、统一的控制接口和全面的评估工具包，为该领域的研究和发展提供了重要的基础支撑设施。
...
> 该研究提出了一种基于可微分逻辑门网络(LGNs)的视频拷贝检测框架，通过紧凑的逻辑表示替代传统浮点特征提取器，实现了极高的推理速度(超过11k样本/秒)和极小的描述符尺寸。

<details>
  <summary>Details</summary>

**Motivation:** 现有深度学习方法在视频拷贝检测中虽然性能良好，但在高通量系统中面临计算成本高、描述符体积大的限制，难以满足实际部署需求。

**Method:** 采用帧小型化、二值预处理和可训练的LGN嵌入模型，学习逻辑操作和互连关系，训练后可离散化为纯布尔电路，实现快速高效的推理。

**Result:** LGN模型在多个数据集上达到竞争性或更优的准确率和排序性能，描述符体积小数个数量级，推理速度超过11k样本/秒。

**Conclusion:** 基于逻辑门网络的模型为可扩展、资源高效的视频拷贝检测提供了一种有前景的替代方案，在保持性能的同时大幅降低了计算和存储需求。
...
> 提出一个融合ViT和条件扩散模型的协同框架，通过特征对齐和跨注意力多层融合机制，在遮挡情况下实现鲁棒的3D人体网格恢复。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法在处理遮挡场景时存在局限性：回归法效率高但在无约束场景中常产生不合理或不准确结果，扩散法能生成遮挡区域的强先验但过度依赖生成会削弱对罕见姿势的保真度。

**Method:** 构建受大脑启发的协同框架：ViT分支从可见区域提取确定性视觉线索，扩散分支合成结构连贯的人体表示；设计多样-一致性特征学习模块对齐判别特征与生成先验，采用跨注意力多层融合机制实现语义级双向交互。

**Result:** 在标准基准测试中，该方法在关键指标上实现优异性能，在复杂现实场景中展现出强大的鲁棒性。

**Conclusion:** 通过有效整合判别性视觉transformer和生成性扩散模型的优势，所提出的协同框架为3D人体网格恢复提供了一种新颖且可靠的解决方案，特别是在部分或严重遮挡条件下表现出色。
...
> 这篇论文通过系统性的消融研究揭示了视觉几何估计的关键性能因素，并提出了CARVE模型，该模型结合了多帧一致性和单帧准确性的优势。

<details>
  <summary>Details</summary>

**Motivation:** 现有多帧模型虽然具有更好的跨帧一致性，但在单帧精度上往往不如单帧方法。这一观察促使作者系统研究影响模型性能的关键因素。

**Method:** 通过严格的消融研究发现：扩大数据多样性和质量能提升性能；常用的置信度感知损失和基于梯度的损失机制可能阻碍性能；联合监督序列级和帧级对齐能改善结果，而局部区域对齐反而降低性能。提出了一致性损失函数和高效架构设计，开发了CARVE模型。

**Result:** CARVE模型在点云重建、视频深度估计和相机姿态/内参估计等多个基准测试中展现出强大且鲁棒的性能。

**Conclusion:** 该研究不仅揭示了视觉几何估计领域的重要设计原则，还通过CARVE模型成功整合了优化方法和高分辨率输入的优势，为该领域的进一步发展提供了有价值的指导。
...
> 这项研究提出了CHAI框架，通过人机协作监督实现精确视频字幕标注和视频理解，其模型在少量专家监督下超越了Gemini-3.1-Pro等闭源模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频语言模型在描述专业级别视频的细节方面存在不足，特别是在摄影技巧、摄像机运动和空间关系等精确描述上，需要更专业的标注方法和数据集。

**Method:** 研究者开发了结构化规范描述体系，引入CHAI（基于批评的人机监督）框架，让专家批评和修订模型生成的预字幕，通过SFT、DPO和推理时扩展等方法改进开源模型Qwen3-VL的字幕生成、奖励建模和批评生成能力。

**Result:** 在适度专家监督下，得到的模型性能超越了Gemini-3.1-Pro等闭源模型，并成功应用于重新标注大规模专业视频，使视频生成模型Wan能够更好地遵循400词的详细提示，实现摄影层面的精细控制。

**Conclusion:** 精确的规范定义和人机协作监督是实现专业级视频理解和视频生成的关键因素，该研究为视频AI的发展提供了新的方法论和数据集基础。
...
> Ramen是一个通过主动样本选择实现鲁棒测试时自适应的框架，通过检索相关样本解决混合域场景下的模型适应问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的测试时自适应方法假设测试样本来自单一域，但实际测试数据常来自多个具有不同特征的混合域，导致现有方法性能下降。

**Method:** Ramen为每个测试样本基于域一致性和预测平衡两个准则检索定制化相关样本批次，采用嵌入-梯度缓存提高效率，通过聚合存储的梯度进行模型更新。

**Result:** 在多个图像损坏和域偏移基准测试中，Ramen展现出强大且一致的性能，在复杂混合域场景中实现了鲁棒且高效的自适应。

**Conclusion:** 该框架有效解决了混合域环境下的测试时自适应挑战，通过理论分析和实验验证了其有效性，为实际应用中的分布偏移问题提供了实用解决方案。
...
> 该论文提出了HalluScope评估基准和HalluVL-DPO优化框架，用于分析和减轻大型视觉语言模型中的幻觉问题，主要针对过度依赖文本先验知识这一根本原因。

<details>
  <summary>Details</summary>

**Motivation:** 大型视觉语言模型(LVLMs)虽然能力突出，但仍存在幻觉问题，即输出内容与视觉输入不一致。以往研究将幻觉归因于视觉骨干网络局限性或语言组件主导性等因素，但这些因素的相对重要性仍不明确，需要更深入的分析。

**Method:** 提出HalluScope基准来系统分析不同因素导致幻觉的程度；设计HalluVL-DPO框架，通过偏好优化方法微调现成LVLM，利用精心构建的训练数据集引导模型偏好基于视觉的响应而非幻觉内容。

**Result:** 分析表明幻觉主要源于过度依赖文本先验和背景知识，特别是通过文本指令引入的信息；优化后的模型有效减轻了目标幻觉模式，同时在其他幻觉基准和视觉能力评估上保持或提升了性能。

**Conclusion:** 通过系统分析和针对性优化，证实了文本指令先验是LVLM幻觉的主要诱因，提出的HalluVL-DPO框架能有效缓解此类问题，为后续研究提供了可复现的评估基准、训练数据集和代码。
...
> 该研究提出了一种基于生物行为特征的可解释性深度伪造检测方法，发现模型在检测包含情绪表情的视频时表现更好，且模型与人类的检测策略存在互补性。

<details>
  <summary>Details</summary>

**Motivation:** 现有深度学习深度伪造检测方法虽然性能良好，但缺乏可解释性，无法说明真实与被操控面部行为的核心差异。

**Method:** 研究提取面部运动的核心低维模式，用于表征时空结构的时序特征，使用传统机器学习分类器对这些特征进行训练，并评估模型决策与人类感知判断的关系。

**Result:** 分类器达到显著高于随机水平的检测效果，操控视频的高阶时序不规律性更明显；情绪表情视频检测准确率更高；深度伪造会系统性降解情绪信号；模型与人类判断在情绪视频上一致但在非情绪视频上分歧。

**Conclusion:** 换脸深度伪造具有可测量的行为指纹，在情绪表达时最显著；可解释的计算特征与人类感知可能提供互补而非冗余的检测路径。
...
> DOCO是一个轻量级框架，通过动态样本分离、域补偿提示学习和结构保持正则化，在开放集连续测试时自适应(OCTTA)场景下实现了更好的域适应和OOD检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有测试时自适应(TTA)方法在现实场景中存在不足，当模型面临持续变化的域并同时出现未知语义类别时，域偏移和语义偏移的耦合会导致特征空间崩溃，严重降低分类和OOD检测性能。

**Method:** DOCO采用闭环协同方法：首先进行动态的适应条件样本分割，分离ID和OOD样本；然后仅使用ID样本，通过与源域对齐特征统计信息和结构保持正则化学习域补偿提示；最后将学习到的提示传播到同批次的OOD样本，隔离其语义新颖性。

**Result:** 在多个挑战性基准测试上，DOCO超越了先前的CTTA和OSTTA方法，为OCTTA场景建立了新的最先进水平。

**Conclusion:** DOCO框架有效解决了开放集连续测试时自适应中的域偏移和语义偏移耦合问题，通过轻量级设计实现了域适应与OOD检测的协同优化，为现实场景下的模型部署提供了可靠解决方案。
...
> 本文提出了一种自监督框架，通过从单目视频中生成伪多视图训练三元组，解决了非刚性场景中配对多视点数据稀缺的问题，实现了复杂动态场景的精确摄像机控制和高质量新视角合成。

<details>
  <summary>Details</summary>

**Motivation:** 精确的摄像机控制对重拍动态视频至关重要，但非刚性场景中配对多视图数据的严重稀缺成为瓶颈，限制了相关技术的发展。

**Method:** 通过从单个输入视频中提取不同的平滑随机游走裁剪轨迹作为源视图和目标视图；利用前向扭曲源视频的第一帧生成几何锚点；迫使模型通过跨时间和视角路由、重投影缺失的高保真度纹理，隐式学习4D时空结构；采用最小化适配的扩散transformer模型进行推理。

**Result:** 该方法在复杂动态场景上实现了最先进的时间一致性、鲁棒的摄像机控制和高保真度的新视角合成效果。

**Conclusion:** 该自监督框架有效克服了数据稀缺限制，无需配对多视图数据即可训练出强大的动态场景新视角合成模型，为视频重拍和摄像机控制提供了新的解决方案。
...
> 该研究通过评估多种视觉语言模型对社交媒体气候传播图像的分类能力，发现Gemini-3.1-flash-lite表现最佳，并证明即使单图精度中等，VLM仍能有效捕捉大规模话语特征。

<details>
  <summary>Details</summary>

**Motivation:** 社交媒体已成为气候传播的主要平台，产生海量图像内容，亟需系统性分析工具来识别有效的传播策略，而现有计算机视觉方法在此领域的应用缺乏系统评估。

**Method:** 构建包含200万张推文图像的测试集，涵盖5个标注维度； benchmark 6种可提示VLM和15种零样本CLIP类模型；采用实例级和分布级评估方法，比较不同提示策略的效果。

**Result:** Gemini-3.1-flash-lite在所有超类别和两个数据集上均表现最优；开源中等规模模型差距较小；思维链推理反而降低性能；维度特定提示设计可提升效果。

**Conclusion:** VLM是大规模社交媒体话语分析的可行工具，预测可靠且成本有效；研究建议关注分布级评估而非仅实例级精度，并为研究者提供了数据和代码资源。
...
> 本文提出了SyMTRS，一个大规模合成遥感数据集，通过高保真城镇模拟流水线生成，提供完善的多任务标注（深度图、夜间图像、多尺度低分辨图像），旨在解决遥感研究中真实标注数据稀缺的问题。

<details>
  <summary>Details</summary>

**Motivation:** 当前深度学习在遥感领域的进展严重依赖大规模标注数据集，但获取高质量的地面真相数据（特别是几何、辐射和多域任务的标注）成本高昂且常常不可行。深度估计缺乏精确标注、照明变化控制不足、多尺度配对图像缺失等问题限制了单目深度估计、域适应和超分辨率等任务的发展。

**Method:** 使用高保真城镇模拟流水线生成大规模合成数据集SyMTRS。该数据集包含高分辨率RGB航空图像（2048×2048）、像素级完美深度图、用于域适应的夜间对应图像，以及用于超分辨率任务的x2、x4、x8尺度的对齐低分辨率变体。数据集设计为统一的多任务基准测试平台。

**Result:** 创建了现有遥感数据集中首个统一的多任务基准测试数据集，能够同时支持几何理解、跨域鲁棒性和分辨率增强的联合研究。与现有专注于单一任务或模态的数据集不同，SyMTRS提供了完美的几何地面真相和一致的多域监督，填补了遥感研究的关键空白。

**Conclusion:** SyMTRS通过合成数据生成技术有效解决了遥感研究中高质量标注数据稀缺的瓶颈问题，为受控实验提供了理想条件。该数据集作为统一的多任务基准平台，将推动单目深度估计、域适应和超分辨率等多个相关领域的协同发展，为遥感深度学习研究提供重要基础设施支撑。
...
> 本文提出了一种面向多修改的组合图像检索框架TEMA，并构建了两个包含丰富指令的多修改数据集M-FashionIQ和M-CIRR，解决了传统方法中实体覆盖不足和子句实体不对齐的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的组合图像检索(CIR)研究依赖于简单的修改文本，仅覆盖有限的显著变化范围，存在两个与现实应用高度相关的限制：实体覆盖不足和子句实体不对齐，阻碍了CIR在实际场景中的应用。

**Method:** 1. 构建了包含丰富指令的多修改数据集M-FashionIQ和M-CIRR；2. 提出了面向文本的实体映射架构TEMA，这是首个专为多修改设计的CIR框架，同时兼容简单修改。

**Result:** 在四个基准数据集上的广泛实验证明，TEMA在原始和多修改场景下都表现出优越性，并在检索准确性和计算效率之间保持了最佳平衡。

**Conclusion:** 本研究通过构建多修改数据集和提出TEMA框架，有效解决了传统CIR方法的局限性，使组合图像检索更贴近实际应用需求，为该领域的发展提供了新的解决方案。
...
> 本文提出使用不同像素尺寸（通过不同传感器或光学变焦）的低分辨率图像组合来解决超分辨率问题的平移模糊性，证明了互质像素尺寸能实现稳定重建，并通过实验验证了多尺度超分辨率的优势。

<details>
  <summary>Details</summary>

**Motivation:** 解决传统超分辨率问题中存在的平移模糊性问题，提高图像重建的稳定性和效率。

**Method:** 采用具有不同像素尺寸的传感器获取多条低分辨率图像，特别是一对互质像素尺寸，在频域或迭代最小二乘法框架下进行超分辨率重建。

**Result:** 数学分析表明该方法具有稳定的逆系统，推导了最小二乘重建的误差表达式，一维和二维实验验证了重建的有效性和准确性。

**Conclusion:** 多尺度超分辨率方法能有效解决平移模糊问题，为实际成像系统提供了具有重要应用价值的技术方案。
...
> 本文提出了诊断驱动的胶囊内镜视频摘要任务，设计了VideoCAP数据集和DiCE框架，通过模拟临床医生的诊断流程实现高效准确的证据帧提取和诊断。

<details>
  <summary>Details</summary>

**Motivation:** 现有胶囊内镜研究局限于帧级分类和检测，视频级分析未得到充分探索。临床上相关事件极其稀疏，会被数万帧冗余正常帧淹没，且单帧存在运动模糊、碎片、镜面反射等歧义性问题。

**Method:** 1) 构建VideoCAP数据集，包含240个全长视频，提供基于真实临床报告的诊断驱动标注；2) 提出DiCE框架，模拟临床医生读取流程：先进行候选帧筛选，然后使用上下文编织器将候选帧组织成连贯的诊断上下文，最后用证据聚合器将多帧证据整合为鲁棒的片段级判断。

**Result:** 实验表明DiCE持续优于现有方法，能生成简洁且临床可靠的诊断摘要，验证了诊断驱动的上下文推理是超长CE视频摘要的有效范式。

**Conclusion:** 该研究为胶囊内镜视频分析开辟了新的诊断驱动摘要任务，通过精心设计的数据集和框架，实现了从超长视频中提取关键证据并做出准确诊断，具有重要的临床应用价值。
...
> 提出了一个基于物理的视频理解基准测试，扩展了V-STaR的评估框架，涵盖多个视频源、物理域和提示类型，研究发现物理推理最强，空间定位最弱，需要考虑物理基础、提示感知和扰动 aware的综合评估。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频理解模型虽然在识别事件名称上表现良好，但在时空定位上仍然失败。需要建立一个更加全面的基准来评估模型对物理视频理解的真正能力，包括事件识别、时间定位和空间定位。

**Method:** 构建了一个覆盖4个视频源(SSV2、YouCook2、HoloAssist、Roundabout-TAU)、6个物理域的基准，包含1560个基础视频片段。每个片段转换为统一的落地事件记录，从中衍生出3种提示类型(物理、vstar_like、neutral_rstr)和4种输入条件(原始、打乱、消融、帧遮罩)。

**Result:** 实验表明物理推理是最强的测试模式，vstar_like是最清晰的非物理语义比较，neutral_rstr作为更难的模板化控制。提示家族的鲁棒性是选择性的而非普遍的，扰动增益集中在弱势原始案例中，空间接地在所有设置中都是最弱的。

**Conclusion:** 视频问答推理基准应该报告基于物理基础的、提示感知的和扰动感知的诊断指标，而不仅仅是汇总准确率，以更全面地评估模型的视频理解能力。
...
> 针对相机ISP（图像信号处理器）在拍摄时引入的AI虚幻内容，本文提出一种轻量级方案，通过优化图像特定的MLP解码器和模式特定编码器，恢复未经虚幻处理的原始图像，仅需180KB存储空间。

<details>
  <summary>Details</summary>

**Motivation:** 随着AI模块深度集成到相机硬件中，相机输出的图像可能包含语义改变的虚幻内容，这导致用户难以判断图像的真实性，需要一种能够恢复真实图像内容的方法。

**Method:** 采用图像特定的多层感知机(MLP)解码器配合模式特定编码器，对已拍摄的相机图像进行后处理优化，无需访问相机ISP即可恢复虚幻内容添加前的图像。

**Result:** 提出的编码器和MLP解码器组合仅需180KB存储空间，可作为元数据嵌入JPEG和HEIC等标准图像格式中，实现便携的图像真实性恢复。

**Conclusion:** 该方案有效解决了相机拍摄时AI虚幻内容带来的图像真实性问题，为用户提供了获取真实图像内容的轻量级解决方案，具有实际应用价值。
...
> UniGenDet是一个统一的生成-判别框架，通过将图像生成与检测任务协同进化，利用对抗性信息实现性能互促提升。

<details>
  <summary>Details</summary>

**Motivation:** 图像生成和生成图像检测领域虽然都取得了显著进展，但各自发展且架构差异巨大。尽管近年来都开始使用对抗信息增强性能，但架构分歧阻碍了两个领域的协同发展。

**Method:** 提出统一的生成-判别框架UniGenDet，设计共生多模态自注意力机制和统一微调算法来桥接任务差距，引入检测器引导的生成对齐机制促进信息交换。

**Result:** 在多个数据集上的广泛实验表明，该方法达到了最先进的性能表现。

**Conclusion:** 通过统一框架成功实现了图像生成与检测任务的协同进化，生成任务提高了真实性识别的可解释性，真实性标准则指导生成更高质量的图像。
...
> 人类和深度视觉模型在分类任务中虽然准确率相近，但会犯系统性不同的错误。研究发现这些方向性混淆通过率失真几何特征揭示了不同的归纳偏置。

<details>
  <summary>Details</summary>

**Motivation:** 传统的准确率指标无法区分人类和深度模型在分类错误模式上的差异。需要新的方法来量化和解释这些方向性混淆模式，以及它们如何反映不同的归纳偏置。

**Method:** 在12种扰动类型下收集匹配的人类和深度模型响应，量化混淆矩阵的不对称性。通过率失真(RD)框架分析几何特征，包括斜率(β)、曲率(κ)和效率(AUC)三个几何签名。

**Result:** 人类表现出广泛但弱的不对称性，而深度模型表现出稀疏但强的方向性崩溃。鲁棒性训练可减少全局不对称性，但无法恢复人类级别的广度-强度配置。不同不对称组织即使性能匹配也会让RD前沿向相反方向移动。

**Conclusion:** 方向性混淆和RD几何为分布偏移下的归纳偏置提供了紧致、可解释的签名，有助于深入理解人类与模型的认知差异。
...
> Vista4D是一个基于4D点云的稳健视频重拍摄框架，能够从不同相机轨迹重新合成动态视频场景。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频重拍摄方法在处理真实世界动态视频时存在深度估计伪影问题，同时在保持内容外观和精确相机控制方面表现不佳，特别是对于具有挑战性的新轨迹。

**Method:** 构建基于4D点云的表示，通过静态像素分割和4D重建显式保留已见内容并提供丰富的相机信号，使用重建的多视角动态数据进行训练以提高对真实世界推理时点云伪影的鲁棒性。

**Result:** 在各种视频和相机路径下，相比现有基线方法在4D一致性、相机控制和视觉质量方面都有显著提升，并能推广到动态场景扩展和4D场景重组等实际应用。

**Conclusion:** Vista4D为视频重拍摄提供了一个新的4D grounded解决方案，通过点云表示有效解决了传统方法面临的深度估计和相机控制难题，为动态视频编辑和重建提供了实用工具。
...
> Omni是一个统一的多模态模型，通过在文本、图像、视频、3D几何和隐藏表示等多样化模态上进行原生训练，实现了上下文展开机制，在多模态生成和理解任务上表现出强大性能。

<details>
  <summary>Details</summary>

**Motivation:** 开发一个能够同时处理和推理多种媒体形式的统一模型，超越传统单一模态或简单多模态融合方法的局限性。

**Method:** 直接在文本、图像、视频、3D几何和隐藏表示等异构模态上进行端到端训练，使模型获得上下文展开能力，能够显式地跨多个模态表示进行推理。

**Result:** Omni在多模态生成和理解基准测试中表现出色，并展示了先进的多模态推理能力，包括文本、图像、视频和3D几何的上下文内生成。

**Conclusion:** 通过在多样化模态上的原生训练和上下文展开机制，Omni能够更忠实地逼近共享的多模态知识流形，有效聚合跨模态的互补信息，显著提升了多模态推理的保真度。
...
> IMU-to-4D框架仅使用可穿戴惯性传感器即可重建人体4D运动和场景3D布局

<details>
  <summary>Details</summary>

**Motivation:** 摄像头在隐私、安全、能效和可扩展性方面存在固有挑战，需要寻找非视觉的感知替代方案

**Method:** 利用大型语言模型处理来自耳机、手表或智能手机的惯性传感器数据，实现非视觉的时空理解

**Result:** 在多个数据集上实验表明，IMU-to-4D比现有级联方法产生更连贯和时间稳定的结果

**Conclusion:** 仅依靠可穿戴运动传感器即可支持丰富的4D环境理解，为视觉感知提供了可行的替代方案
...
> 本文将时间作为可学习的视觉概念，开发了视频时间流的感知与控制模型，能够检测速度变化、估计播放速度，并实现了速度条件视频生成和时间超分辨率技术。

<details>
  <summary>Details</summary>

**Motivation:** 现有计算机视觉研究虽然集中于视频处理，但对时间的感知与控制关注不足。如何判断视频是否被加速或减速，以及如何生成不同速度的视频，这些都是亟待解决的问题。

**Method:** 首先利用视频中的多模态线索和时间结构，通过自监督学习检测速度变化和估计播放速度；然后利用该模型从野生噪声源中挖掘出迄今为止最大的慢动作视频数据集；最后基于该数据开发了时间控制模型，包括速度条件视频生成和时间超分辨率技术。

**Result:** 成功构建了最大的慢动作视频数据集，开发了能够精确控制视频时间流的模型，实现了指定速度的视频生成和低帧率视频向高帧率高质量视频的转换。

**Conclusion:** 研究表明时间是视频学习中可操作的感知维度，为时间可控的视频生成、时间取证检测以及理解事件时序发展的世界模型开辟了新的研究方向。
