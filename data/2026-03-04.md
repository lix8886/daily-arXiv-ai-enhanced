...
> 研究提出了一种新的替补模型，该模型不仅保留了原始符号序列的符号频率，还重现了其长程相关结构，适用于语言和DNA等领域。

<details>
  <summary>Details</summary>

**Motivation:** 现有的替补模型通常只能保留符号频率分布或相关性属性之一，而不能同时保留两者。因此，需要一种同时保留两者的新模型。

**Method:** 我们提出了一种保留符号频率和长程相关性的替补模型。该模型通过将分数高斯噪声（FGN）映射到经验直方图上来生成替补序列，这一过程保持了符号频率，并重现了长程相关结构，即通过去趋势波动分析（DFA）指数量化表示。

**Result:** 验证了该模型在英语和拉丁语实例文本上的有效性，并展示了其在基因组DNA上的广泛适用性，表明核苷酸组成和DFA缩放特性可以得到重现。

**Conclusion:** 这种方法为分解象征性系统的结构特性提供了原则性的工具，并可用于检验关于语言、DNA及其他符号域中尺度法则和记忆效应起源的假设。
...
> 研究者通过六个结合NLP和多语言词汇组织理论的实验，发现Meta的NLLB-200模型隐式地学习了人类语言的系统结构，并且内部化了一些普遍的概念关联。

<details>
  <summary>Details</summary>

**Motivation:** 探究神经机器翻译模型是否学习了跨语言的普遍概念表示，还是仅仅按照表面相似性对语言进行分类。

**Method:** 通过六个实验，结合NLP的可解释性和认知科学中关于多语言词汇组织的理论，研究NLLB-200模型的表示几何。这些实验使用了嵌入在135种语言中的斯瓦迪士核心词汇列表，并验证了模型所学习到的表示是否对应于人类语言的系统结构和概念相似性。

**Result:** 模型的嵌入距离与语言的谱系距离有显著相关性，且常用共词化概念对的嵌入相似度比非共词化概念对更高。此外，跨语言基本概念对的语义偏差向量具有高度的一致性。

**Conclusion:** 模型隐含地学习了人类语言的基因结构，内部化了普遍的概念关联，并保持了跨类型多样语言的二级关系结构。
...
> 本研究探讨了扩散语言模型的记忆特性，提出了一种新的抽取框架并证明了采样分辨率与记忆概率之间的单调关系。

<details>
  <summary>Details</summary>

**Motivation:** 扩散语言模型（DLMs）最近成为了一种有竞争力的替代方案，但对其记忆行为的研究很少，这促使我们对其进行系统性理论和实证的描述。

**Method:** 我们提出了一种广义的概率抽取框架，该框架统一了前缀条件化解码和基于扩散的生成，并适用于任意的掩码模式和随机采样轨迹。

**Result:** 我们的实验验证了理论预测，结果显示在对齐的前缀条件化评估中，扩散语言模型相比于自回归语言模型，在个人信息（PII）泄漏方面表现得更少。

**Conclusion:** 扩散语言模型在生成过程中显示出较低程度的记忆泄漏，尤其是在个人信息的保护上优于传统的自回归语言模型。
...
> 本章讨论了AI生成和AI辅助作文检测器的现状，并通过实证分析评估了这些检测器在不同大规模语言模型之间的泛化能力，提供了在实际应用中发展和重新训练检测器的指导。

<details>
  <summary>Details</summary>

**Motivation:** 大规模语言模型（LLMs）的发展使得生成连贯、高质量的文章变得越来越容易，这引发了对学生提交作业真实性的重要关注。为了应对这一挑战，本研究旨在评估和指导检测器的发展与再训练，以适应实际应用。

**Method:** 该章节首先概述了当前用于检测AI生成和AI辅助作文的检测器的现状，并提供了它们负责任使用的指南。接着，通过基于公共GRE写作提示生成的文章，进行实证分析，以评估在一个大规模语言模型上训练的检测器识别其他模型生成文章的能力。

**Result:** 研究发现提供了对于开发和重新训练检测器以适应实际应用的指导。

**Conclusion:** 研究结果表明，当前的检测器在识别不同大规模语言模型生成的文章时存在一定的局限性，但同时也为未来的发展提供了有价值的见解和方向。
...
> 我们发布了RO-N3WS，这是一个旨在提高自动语音识别系统在低资源及分布外条件下表现的罗马尼亚语语音数据集。研究表明，即使对真实语音进行简短的微调也能大幅提高性能。我们将公开所有的模型、脚本和数据分割以支持可重复的研究。

<details>
  <summary>Details</summary>

**Motivation:** 本研究的动机在于创建一个高质量的多样化的罗马尼亚语语音数据集，以便提高ASR系统在低资源条件和分布外条件下的表现。通过这个数据集，我们可以支持多语言ASR、领域适应和轻量级部署的研究。

**Method:** 我们引入了RO-N3WS，这是一个罗马尼亚语语音数据集，旨在提高自动语音识别（ASR）在低资源条件下的泛化能力。RO-N3WS包括超过126小时的经过转录的音频，这些音频来自广播新闻、文学有声读物、电影对话、儿童故事和对话播客。这个多样化的音频来源使得在风格迥异的不同领域进行强健的训练和微调成为可能。我们评估了几个最先进的ASR系统（如Whisper和Wav2Vec 2.0），在零样本和微调设置下进行了性能对比，并使用生成的合成数据进行了控制对比试验。

**Result:** 结果显示，即使只对RO-N3WS中的真实语音进行有限的微调，也能显著降低词错误率，优于零样本基线。

**Conclusion:** 由此可见，RO-N3WS数据集对于提升ASR系统在低资源环境中的表现有着不可小觑的作用，其多样化的音频来源对系统训练和微调非常有利。
...
> GLoRIA框架通过少量参数更新实现了方言ASR性能的大幅提高，同时保持良好的推广性和可解释性。

<details>
  <summary>Details</summary>

**Motivation:** ASR在方言密集型设置下由于地区变异强烈且标记数据有限而变得尤为困难。

**Method:** 提出GLoRIA框架，该框架利用元数据（如坐标）来调制预训练编码器中的低秩更新，这些低秩矩阵被注入到每个前馈层中，并且通过一个门控MLP根据位置元数据来决定每个LoRA分解的非负贡献。

**Result:** 

**Conclusion:** GLoRIA 作为一个参数高效的改编框架，通过基于位置元数据决定低秩更新的贡献程度，显著改善了方言环境下的自动语音识别性能。它不仅在已知方言上表现出色，还能很好地推广到未知方言，包括外推场景，并且其改编模式可以进行地理空间可视化，说明了带有元数据门控的低秩改编是方言ASR的有效、可解释且高效的方法。
...
> 提出CoDAR框架来解决连续扩散语言模型的标记恢复问题，通过自回归Transformer提高生成质量，从而使连续扩散模型更适合文本生成任务。

<details>
  <summary>Details</summary>

**Motivation:** 研究了连续扩散语言模型在生成过程中的滞后问题，特别是由于标记舍入所带来的瓶颈。

**Method:** 通过在控制标记恢复研究中识别出标记舍入是主要瓶颈，提出了CoDAR框架，该框架使生成过程完全保持在嵌入空间中连续，并学习一个强健的、上下文相关的离散化器：一个自回归Transformer解码器，该解码器跨注意解码的嵌入序列并执行上下文化的舍入。

**Result:** 在LM1B和OpenWebText上的实验表明，CoDAR在生成质量上大幅超越了潜在扩散，并且超越了强离散DLM，同时还揭示了一个简单的解码器温度调节旋钮，以在流畅性和多样性之间进行权衡。

**Conclusion:** CoDAR框架成功地解决了连续扩散模型在标记恢复过程中的瓶颈问题，提高了生成质量，并且与强离散DLM达到了竞争力，提供了一个调整推测温度的简便方式来平衡流畅性和多样性。
...
### [8] [CamDirector: Towards Long-Term Coherent Video Trajectory Editing](https://arxiv.org/abs/2603.02256)
*Zhihao Shi,Kejia Yin,Weilin Wan,Yuhongze Zhou,Yuanhao Yu,Xinxin Zuo,Qiang Sun,Juwei Lu*

Main category: cs.CV

>  Vinci等人提出了一种新的视频轨迹编辑框架，能够生成全球一致且连贯的视频，同时使用较少参数达到最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:**  Vinci等人的工作动机在于解决现有VTE方法精确相机控制和长时间一致性的问题，这些问题通常是因为方法受限的嵌入目标姿态或依赖于单帧变形导致的。

**Method:**  Vinci等人提出了一种新的视频轨迹编辑框架，该框架通过混合变形方案明确地收集整个源视频的信息。具体来说，静态区域被逐步融合到世界缓存中，然后渲染到目标相机姿态，而动态区域则直接变形；两者的融合生成全局一致的粗略帧以指导细化。此外，该框架通过一种基于历史引导的自回归扩散模型，将视频段及其历史一起处理，同时世界缓存被增量更新以强化已经重建的内容，实现长期时间上的连贯性。

**Result:**  Vinci等人的方法在iPhone-PTZ基准测试上达到了最先进的性能，且使用了更少的参数。

**Conclusion:**  Vinci等人的研究证明了利用混合变形方案与历史引导相结合的方法在长期一致性视频轨迹编辑方面优于现有方法，为未来的研究提供了有价值的参考。
...
### [9] [Social-JEPA: Emergent Geometric Isomorphism](https://arxiv.org/abs/2603.02263)
*Haoran Zhang,Youjin Wang,Yi Duan,Rong Fu,Dianyu Zhao,Sicheng Fan,Shuaishuai Cao,Wentao Guo,Xiao Zhou*

Main category: cs.CV

> 该研究发现，两种不同视角下训练的世界模型内部表示间可形成近似线性等距关系，促进了代理之间的表示转换，并展示了用于在无需进行额外学习步骤的情况下进行知识迁移的可能性。

<details>
  <summary>Details</summary>

**Motivation:** 这项研究旨在通过分析不同视角下独立代理获取的世界模型之间的内在表示关系，揭示预测学习目标如何强加规则于表示几何，并探索轻量级的去中心化视觉系统之间的互联互通路径。

**Method:** 通过让独立的代理从同一环境的不同视角中获取世界模型，研究者们展示了这些模型在训练后能够形成内在表示的线性等距关系，从而实现透明的翻译能力。

**Result:** 研究发现，尽管视角差异大且像素重叠少，两个代理的潜在空间之间仍然存在一种近似的线性等距关系，这使得在一个代理上训练的分类器可以无缝转移到另一个代理上，而不需要额外的梯度步骤，并且可以通过类似知识蒸馏的方式加速后续学习并减少总计算量。

**Conclusion:** 研究结果表明，预测性学习目标强加于表示几何上的强规则可促使不同独立代理间的潜在空间达成几何一致，从而为去中心化的视觉系统提供了轻量级的光互联方法。
...
### [10] [From Visual to Multimodal: Systematic Ablation of Encoders and Fusion Strategies in Animal Identification](https://arxiv.org/abs/2603.02270)
*Vasiliy Kudryavtsev,Kirill Borodin,German Berezin,Kirill Bubenchikov,Grach Mkrtchian,Alexander Ryzhkov*

Main category: cs.CV

> A multimodal verification framework combining visual and textual features improved pet re-identification accuracy by 11% over current unimodal systems, highlighting the effectiveness of adding semantic detail to visual data in automated animal identification tasks.

<details>
  <summary>Details</summary>

**Motivation:** The motivation for this study is to improve the accuracy of automated animal identification in the context of reuniting lost pets with their owners, addressing limitations in current systems due to a lack of large datasets and reliance on unimodal visual cues.

**Method:** This study introduces a multimodal verification framework that combines visual features with semantic identity priors derived from synthetic textual descriptions, utilizing a large dataset of 1.9 million photographs of unique animals. The optimal backbones identified through systematic ablation studies are SigLIP2-Giant and E5-Small-v2 for vision and text, respectively. The best fusion method determined for integrating these modalities is a gated fusion mechanism.

**Result:** The proposed approach achieved a Top-1 accuracy of 84.28% and an Equal Error Rate of 0.0422, marking an 11% improvement over the leading unimodal baselines in large-scale pet re-identification tasks.

**Conclusion:** The research demonstrates that integrating synthesized semantic descriptions significantly refines decision boundaries in large-scale pet re-identification tasks, showing notable improvement over traditional unimodal systems by enhancing the verification framework with multimodal inputs.
