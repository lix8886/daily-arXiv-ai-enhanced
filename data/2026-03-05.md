...
> 介绍AriadneMem，一种针对长周期LLM代理结构化记忆系统，通过两阶段过程解决长期对话记忆挑战，提高多跳F1和平均F1的同时大幅减少运行时间。

<details>
  <summary>Details</summary>

**Motivation:** 长周期LLM代理需要在固定上下文预算下保持准确性的内存系统，但现有系统在长期对话中遇到两方面挑战：多跳答案所需的跨时间事实链接和由信息演变引起的历史记录冲突。

**Method:** 提出AriadneMem结构化内存系统，通过解耦的两阶段管道来解决长期对话中的挑战。离线构建阶段利用熵感知门限筛选噪音信息并合并静态重复项，同时保持状态转换。在线推理阶段采用算法桥接发现和单次调用拓扑感知综合来重建事实之间的逻辑路径。

**Result:** 在LoCoMo实验中，与强基线相比，AriadneMem将Multi-Hop F1提高了15.2%，平均F1提高了9.0%，并通过将推理任务卸载到图层，使用仅497个上下文标记的情况下，将总运行时间减少了77.8%。

**Conclusion:** AriadneMem有效解决了长期对话记忆系统的挑战，并大幅提高了推理效率和准确性。
...
> 研究表明高质量的奖励模型(RMs)仍然存在偏差，本文提出一种机制奖励塑造方法来减少偏差，该方法用少量的标签数据即可改善LM与人类偏好对齐的效果。

<details>
  <summary>Details</summary>

**Motivation:** 奖励模型（RMs）对于在线语言模型（LMs）与人类偏好的对齐至关重要。然而，基于RM的偏好微调容易受到奖励欺骗的影响，LM策略可能会从有缺陷的RM中学习到不期望的行为。

**Method:** 通过系统测量五种高质量的奖励模型（RM）中的偏差，我们发现了与长度、奉承、过度自信相关的问题。还发现了与模型特定风格和答案顺序相关的新的偏差问题。我们按照复杂性对RM失败进行分类，并提出了一种简单的后处理干预措施来减轻由虚假相关性引起的低复杂性偏差。

**Result:** 我们提出了一种机制奖励塑造方法，它能够减少目标偏差，同时不降低奖励质量并使用极少量的带标签数据。该方法可以扩展到新的偏差，适用于模型内部，并可泛化出分布外。

**Conclusion:** 我们的研究表明，即使在高质量的RM中，仍存在先前工作中未解决的问题。我们提出了一种简单的方法来减少这些偏差，有望改善LM的性能。
...
> 提出MA-RAG框架以解决大型语言模型在医疗问答中高频出现幻觉和过时知识的问题，通过多轮代理改进是可以缓解这些问题的方法。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型（LLMs）在医疗问答中表现出较高的推理能力，但它们倾向于产生幻觉和过时知识，在医疗领域中构成重大风险。因此，提出了一种更适合复杂推理的多轮代理式实验增强生成（MA-RAG）框架，以解决现有方法依赖噪音特征且缺乏多轮次精炼的问题。

**Method:** MA-RAG（Multi-Round Agentic RAG）框架通过在测试时扩展复杂医理推理，通过代理式的改进环，迭代地发展外部证据和内部推理历史，从而改善医疗领域的问题回答。该框架在每一轮中将候选响应之间的语义冲突转化为可操作的查询以获取外部证据，并优化历史推理痕迹以缓解长上下文的退化问题。通过利用自我一致性原理，MA-RAG能够利用一致性缺乏作为多轮代理推理和检索的积极信号，并模拟一种迭代最小化残差误差至稳定、高保真医疗共识的提升机制。

**Result:** 覆盖面广泛的7个医疗问答基准测试显示，MA-RAG在平均准确率上比基准模型高出6.8分，并在推理时间扩展和RAG基线方面有持续超越的表现。

**Conclusion:** MA-RAG框架通过迭代改进历史性推理痕迹和增强外部证据来提高复杂医疗问题回答的精确度，显著提升了问答准确率。该研究为改善医疗领域大型语言模型可靠性做出贡献。
...
> 提出SE-Search，一种改进在线搜索行为的自我进化搜索代理。通过记忆净化、原子查询训练和密集奖励三个组件，SE-Search提高了搜索行为的有效性，减少了无关文档的积累，并在问答基准测试中超越了强大的基线。

<details>
  <summary>Details</summary>

**Motivation:** 现有的检索增强生成(RAG)方法依赖于稀疏强化学习信号，通常会积累无关或噪声文档。为了解决这些问题，提出了SE-Search，旨在提高在线搜索行为的有效性，减少无关或噪声内容的积累。

**Method:** Structure

**Result:** {
  "tldr": "提出SE-Search，一种改进在线搜索行为的自我进化搜索代理。通过记忆净化、原子查询训练和密集奖励三个组件，SE-Search提高了搜索行为的有效性，减少了无关文档的积累，并在问答基准测试中超越了强大的基线。", 
  "motivation": "现有的检索增强生成(RAG)方法依赖于稀疏强化学习信号，通常会积累无关或噪声文档。为了解决这些问题，提出了SE-Search，旨在提高在线搜索行为的有效性，减少无关或噪声内容的积累。", 
  "method": "SE-Search采用了Think-Search-Memorize策略，通过记忆净化去除无关内容，保持关键证据；通过原子查询训练促进较短且多样化的查询；通过密集奖励提供精细的反馈来加速训练。", 
  "result": "在单跳和多跳问题回答基准测试中，SE-Search-3B大幅超越了强大的基线，绝对提升了10.8个百分点，相对提升了33.8%。", 
  "conclusion": "研究表明，SE-Search在改善在线搜索行为和提升问题回答性能方面具有显著优势，未来将公开该模型的代码和权重。"}
...
> 本文提出了一种结合专家策划的事实检索与文化、安全导向对话生成的混合架构，显著提升了大语言模型在农业咨询方面的表现。

<details>
  <summary>Details</summary>

**Motivation:** 鉴于目前大型语言模型在农业指导中存在建议欠缺支持、含糊不清和与小农户需求不匹配的问题，尤其是当准确性直接关乎农民成果时，该研究旨在设计一种解决方案来克服这些挑战。

**Method:** Structure

**Result:** {
  "tldr": "本文提出了一种混合型大语言模型架构，用于改进面向小农户的农业咨询服务。模型在专家策划的农业知识上进行微调，并通过一个拼接层转化成文化适宜且安全的建议答复。实验结果显示，该模型在事实准确性以及安全评分方面有显著提高，且使用较小模型可以更有效地降低成本。",
  "motivation": "在高风险农业环境中，不准确的建议会直接影响农民的生产成果。现有的大语言模型存在着建议无依据、内容笼统且与小农户需求不符的问题。为了解决这些问题，确保模型部署的安全性和有效性，这篇文章开发了一种新的架构。",
  "method": "研究小组设计了一个将事实检索与对话生成分离的混合架构：利用少量数据微调（LoRA）技术对专家整理的大规模黄金农业知识进行事实召回优化；并通过一个独立的拼接层，将提取到的事实转化为文化和安全意识强的响应。",
  "result": "实验表明，在多种模型配置、不同作物种类和来自印度比哈尔地区的查询测试下，对比基线模型，使用专家策划数据微调可以明显提升事实召回率与F1值。而且，通过拼接层进一步提升了安全评分，同时保持了高对话质量。",
  "conclusion": "研究证明了通过专家策划农业知识微调的模型，在事实准确性及安全评分方面得到提升，能更有效地为小农户提供正确、有针对性且文化适宜的农业建议。此外，使用较小规模模型可在保证知识质量的同时显著降低使用成本。
}

**Conclusion:** 研究证实，通过使用较小规模的微调模型并且使用文化和安全导向的拼接层，可以实现更好的事实准确性和安全性，同时保持高效和实用的特点。
...
> 研究发现，大型语言模型（LLMs）在目标选择方面与人类存在显著差异，导致其在某些关键应用中替代人类决策的能力受限。

<details>
  <summary>Details</summary>

**Motivation:** 研究大型语言模型在自主选择目标时是否能有效反映人类偏好，通过评估模型在特定任务中的行为模式，来检验其作为人类行为代理的有效性。

**Method:** 采用四个前沿的语言模型，在一个基于认知科学的开放学习任务中，进行直接对比，以测试语言模型与人类目标选择行为的相似性。

**Result:** {"tldr": "大型语言模型（LLMs）在自主选择目标时存在与人类显著不同的模式，即使某些模型被设计为模仿人类行为。这警示我们不能单纯依赖当前的LLMs来代替人类做决策。", "motivation": "探讨LLMs在目标选择方面能否作为人类行为的代理，尤其是在开放性学习任务中。", "method": "使用四个先进LLM模型（GPT-5, Gemini 2.5 Pro, Claude Sonnet 4.5, 和 Centaur）在一个来自认知科学的开放性学习任务中进行对比分析，直接评估LLM模型作为人类目标选择代理的有效性。", "result": "研究发现大多数模型要么利用单一解决方案（奖励作弊行为），要么表现出低效性能，且各模型间存在不同的表现模式。即使是为了在实验环境中模仿人类而特别训练的Centaur模型，也无法准确捕获人类的目标选择行为。", "conclusion": "人类在目标选择上的独特性揭示了当前LLMs在取代人类决策方面的能力有限，特别是在个人辅助、科学发现和政策研究等领域需要注意。"}

**Conclusion:** 此项研究强调了人类目标选择的特性，并发出警告，即在重要领域不宜过度依赖现有语言模型来代替人类决策。
...
> 该研究提出了PlugMem，一种任务无关的记忆插件模块，能够高效地记忆和检索对任务相关的抽象知识，从而提高大规模语言模型在复杂环境中表现的效率和效果。

<details>
  <summary>Details</summary>

**Motivation:** 动机是解决现有记忆设计要么特定任务且不可转移，要么任务无关但效率低下的问题。

**Method:** 通过借鉴认知科学，将经历记忆组织成一个以知识为中心的记忆图谱，与GraphRAG等方法不同，该图谱将知识视为记忆存取和组织的基本单元。

**Result:** 在三个不同的基准测试中，PlugMem的表现超过了任务无关的基线和特定任务的记忆设计。

**Conclusion:** 研究结论表明，PlugMem不仅在任务表现上有所提升，而且在信息密度分析中也表现最优。
...
> TTSR框架通过在测试时刻交替扮演学生和教师角色的方式解决现有测试时刻训练的挑战，显著提升了大型语言模型的推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 针对测试时刻训练所面临的挑战，如测试问题常常非常困难，导致自生成的伪标签不可靠，以及现有方法缺乏有效机制适应模型特定的推理弱点而导致学习效率低下，提出了一种新方法。

**Method:** TTSR采用了一种自我反思的测试时刻自演化训练框架，该框架在测试时刻使用一个预训练的模型交替扮演学生和教师的角色。学生专注于解决问题并从合成的变化问题中学习，而教师则分析学生推理失败的轨迹，总结出常见的推理弱点，并据此合成针对性的变化问题。

**Result:** 在多个具有挑战性的数学推理基准测试上，实验结果表明TTSR显著提高了推理性能，并且在不同模型骨干和一般领域推理任务上表现出良好的泛化能力。

**Conclusion:** 教师引导的自我反思为测试时刻的稳定和持续推理改进提供了一条有效的途径。
...
### [9] [TATRA: Training-Free Instance-Adaptive Prompting Through Rephrasing and Aggregation](https://arxiv.org/abs/2603.03298)
*Bartosz Dziuba,Kacper Kuchta,Paweł Batorski,Przemysław Spurek,Paul Swoboda*

Main category: cs.CL

> 介绍了一种无需数据集提示的方法 TATRA，该方法在多种任务中实现了高水平的准确性，提供了即时生成实例特定提示的能力。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型的行为仍然高度依赖于提示用语的选择，作者希望通过一个不需要特定任务训练集、避免昂贵的迭代优化过程并能够针对每个新任务即时生成提示的方法来解决这一问题。

**Method:** TATRA, 一种无数据集提示方法，通过结合用户提供的指令和即时生成的示例来构造实例特定的少量样本提示。TATRA 不需要标记训练数据，避免了特定任务的优化循环，同时保留了基于示范提示的好处。

**Result:** 在标准文本分类基准上，TATRA 与依赖训练数据和广泛搜索的强提示优化基线相比，匹配或提升了表现。在数学推理基准 GSM8K 和 DeepMath 上，TATRA 达到了最先进的性能。

**Conclusion:** 研究结果表明，针对每个实例建设有效的上下文示例比为每个任务执行长时间、昂贵的优化循环更重要。
...
### [10] [How LLMs Cite and Why It Matters: A Cross-Model Audit of Reference Fabrication in AI-Assisted Academic Writing and Methods to Detect Phantom Citations](https://arxiv.org/abs/2603.03299)
*MZ Naser*

Main category: cs.CL

> 这项研究对十个商业大型语言模型的学术引用生成行为进行了大规模调查，发现虚构引用的比率显著受多种因素影响，并开发了一个分类器来识别虚构引用。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机在于量化大型语言模型在生成学术引用时存在的虚构行为的范围，尤其是在不同的提供商、领域和提示条件下的表现。

**Method:** 该研究通过调查了10个商业部署的大型语言模型在四个学术领域中的引用生成情况，总共生成了69,557个引用实例，并与CrossRef、OpenAlex和Semantic Scholar三个学术数据库进行了验证。

**Result:** 研究结果显示，观测到的虚构率范围为11.4%到56.8%，并且这种虚构行为显著地受到模型选择、研究领域和提示框架的影响。此外，当不给予提示时，这些模型不会自发产生引用，表明虚构行为是提示诱导的。还发现了有效筛选虚构引用的方法，开发了一种轻量级分类器，仅使用参考文献字符串特征，实现了较高的准确率。

**Conclusion:** 结论是大型语言模型在引用生成时的虚构行为是提示诱导的，并不是其内在特性。研究提供了一些提高引用准确性的方法，并开发了一个可以用来预筛选虚构引用的分类器。
...
### [11] [Benchmarking Legal RAG: The Promise and Limits of AI Statutory Surveys](https://arxiv.org/abs/2603.03300)
*Mohamed Afane,Emaan Hariri,Derek Ouyang,Daniel E. Ho*

Main category: cs.CL

> 本文评估了几种用于法律AI的检索增强生成（RAG）工具，包括STARA和两个商业工具。结果显示STARA的准确率最高，达到83%，并且在正确处理DOL律师编目的内容时，实际准确率高达92%。文章还分析了错误源头，并提出了法律RAG未来设计的原则。

<details>
  <summary>Details</summary>

**Motivation:** 先前的研究指出了标准RAG在法律领域中的表现不佳，准确率仅达到70%。本研究旨在评估新兴的工具在LaborBench上的表现，并探讨改进的可能。

**Method:** 使用LaborBench作为基准，评估包括STARA、Westlaw和LexisNexis在内的三种新兴工具的性能，并进行详细错误分析。

**Result:** STARA表现优异，准确率83%，商业工具表现较差，Westlaw准确率58%，Lexis+准确率64%。文章指出，许多所谓的错误实际上是由于DOL律师的遗漏，STARA的实际准确率高达92%。

**Conclusion:** 研究提出了一些关于法律AI设计的关键原则，为构建能够进行准确的多司法管辖区法律研究的AI系统提供了指导。
...
### [12] [From Exact Hits to Close Enough: Semantic Caching for LLM Embeddings](https://arxiv.org/abs/2603.03301)
*Dvir David Biton,Roy Friedman*

Main category: cs.CL

> 本文探讨离线语义缓存策略的NP难问题，提出多项式时间启发式策略及结合近期性、频率和局部性的在线策略，并展示改进语义准确性的新变体。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型的快速采用对更快的响应时间和更低的成本提出了需求。语义缓存通过重用语义相似请求符合理响应了此需求，但这破坏了经典缓存假设并提出了新的挑战。

**Method:** 研究探讨了离线语义缓存策略，证明实施最优离线策略是NP难问题，并提出了多项式时间启发式策略。此外，提出了结合近期性、频率和局部性的在线语义感知缓存策略。

**Result:** 在多样化数据集上的评估显示，基于频率的策略是较强的基线，而提出的新变体则改进了语义准确性。

**Conclusion:** 研究发现对当前系统有效策略，并揭示了未来创新的巨大潜力。所有代码均开源。
...
> 本文提出了一种名为mHC-HSI的高光谱图像分类模型，通过引入聚类引导和物理意义明确的光谱带组合，实现了精确度和可解释性的双重提升。

<details>
  <summary>Details</summary>

**Motivation:** 尽管DeepSeek发明的mHC方法在深度学习模型中展现了优越性，但它尚未针对高光谱图像分类进行专门设计。这项工作旨在填补这一空白，通过引入物理意义明确的光谱带组合和改进的模型结构，增强高光谱图像分类模型的性能与解释性。

**Method:** 该论文提出了一种基于聚类引导的mHC Mamba模型(mHC-HSI)，用于提高高光谱图像分类的性能。首先，设计了一个新型的聚类引导的Mamba模块，基于mHC框架，能够显式地学习高光谱图像的空间和光谱信息。其次，引入了一种新的残差矩阵实现方式，使其能够作为软聚类成员映射，从而提升了mHC方法的可解释性。第三，利用物理有意义的光谱带组合方式增强模型的解释性。

**Result:** 该方法在标准数据集上的测试结果显示，相较于现有的最先进方法，所提出的模型不仅提高了分类精度，还增强了模型的解释性。

**Conclusion:** 研究证明了提出的方法在不牺牲精度的前提下，通过增强模型解释性，提升了高光谱图像分类的整体性能。
...
> A counterfactual evaluation framework is proposed to assess visual grounding in RLVR models for medical VQA tasks, revealing issues with current evaluation methods and model reliance on visual information.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to evaluate the visual grounding capabilities of RLVR models on multimodal medical VQA tasks and to identify potential issues with current evaluation protocols, which might fail to measure causal visual dependence.

**Method:** We introduce a counterfactual evaluation framework using real, blank, and shuffled images across four medical VQA benchmarks: PathVQA, PMC-VQA, SLAKE, and VQA-RAD. The evaluation measures include accuracy, Visual Reliance Score (VRS), Image Sensitivity (IS), and Hallucinated Visual Reasoning Rate (HVRR).

**Result:** RLVR models improve accuracy while decreasing visual grounding, measured by VRS and IS, and they often generate ungrounded visual claims (HVRR). This suggests that current accuracy-only rewards cause shortcut behavior, requiring better evaluation protocols.

**Conclusion:** The study highlights the inadequacy of current accuracy-based evaluation and training objectives. There is a need for evaluation protocols and training objectives that explicitly enforce visual dependence to improve visual grounding in RLVR models.
...
### [15] [Proact-VL: A Proactive VideoLLM for Real-Time AI Companions](https://arxiv.org/abs/2603.03447)
*Weicai Yan,Yuhong Dai,Qi Ran,Haodong Li,Wang Lin,Hao Liao,Xing Xie,Tao Jin,Jianxun Lian*

Main category: cs.CV

> 本文针对AI同伴在实时互动中面临的挑战，提出了Proact-VL框架，并通过实验验证了其在低延迟、响应质量和视频理解方面的能力。

<details>
  <summary>Details</summary>

**Motivation:** 实时互动AI同伴需要解决低延迟推理、自主决定响应时机以及控制生成内容的质量和数量等挑战。本文通过实例化AI同伴来应对这些挑战。

**Method:** 本文通过两个游戏场景实例化AI同伴：解说员和向导。为了解决实时互动的关键挑战，提出了Live Gaming Benchmark大型数据集以及Proact-VL框架。Proact-VL框架能够将多模态语言模型塑造成具有人类般环境感知和互动能力的实时互动代理。

**Result:** 实验结果表明，Proact-VL在保持强大的视频理解能力的同时，实现了更优的响应延迟和质量，证明了其在实时互动应用中的实用性。

**Conclusion:** Proact-VL能够有效应对实时互动中的三项关键挑战，证明了其在实际应用中的潜力。
...
### [16] [Beyond Pixel Histories: World Models with Persistent 3D State](https://arxiv.org/abs/2603.03482)
*Samuel Garcin,Thomas Walker,Steven McDonagh,Tim Pearce,Hakan Bilen,Tianyu He,Kaixin Wang,Jiang Bian*

Main category: cs.CV

> PERSIST模型通过在三维空间中模拟环境、相机和渲染器，解决了传统模型在3D环境表示上的不足，显著提升了空间记忆、三维一致性以及长期稳定性，开创了可编辑和可定制的3D环境生成能力。

<details>
  <summary>Details</summary>

**Motivation:** 该研究的动机在于解决现有互动世界模型在三维环境表示上的不足，即无法显式建模3D场景，导致用户在操作时体验不佳以及对下游任务（如训练智能体）的支持有限。

**Method:** 该论文提出了一个新的世界模型PERSIST，此模型能够模拟潜在于3D场景中的环境、相机和渲染器的演变，从而实现持久的空间记忆和一致的几何合成。

**Result:** 实验结果表明，相较于现有的方法，PERSIST在空间记忆、三维一致性以及长期稳定性方面有了大幅提高。论文还展示了该模型可以基于单个图像合成多样化的3D环境以及实现环境中细粒度的几何感知控制。

**Conclusion:** PERSIST世界模型开创了一种新的方法，能够生成一致性的3D场景，并支持细粒度的环境编辑。这不仅改善了用户体验，也为人工智能领域中的代理训练等任务带来了潜在的好处。
...
### [17] [Phys4D: Fine-Grained Physics-Consistent 4D Modeling from Video Diffusion](https://arxiv.org/abs/2603.03485)
...
> Phys4D是一个通过视频扩散模型学习物理一致的4D世界表示的流水线，提升了细粒度时空和物理一致性。

<details>
  <summary>Details</summary>

**Motivation:** 虽然大规模生成世界模型的视频扩散模型已经取得了令人印象深刻的能力，但这些模型在细粒度物理一致性方面存在困难，表现出随时间变化的物理不合理的动态变化，因此提出了Phys4D来解决这一问题。

**Method:** Phys4D采用了一个三阶段训练范式，逐步将外观驱动的视频扩散模型提升为物理一致的4D世界表示。首先，通过大规模伪监督预训练来建立稳健的几何和运动表示，奠定4D场景建模的基础。然后，利用仿真生成的数据进行基于物理的监督微调，强制执行时间一致的4D动态。最后，应用基于仿真的强化学习来纠正难以通过显式监督捕捉的残余物理违规。

**Result:** 实验结果表明，与外观驱动的基线相比，Phys4D在细粒度的时空和物理一致性方面有显著提升，同时保持了强大的生成性能。

**Conclusion:** 该项目展示了通过结合伪监督预训练、基于物理的微调和仿真指导的强化学习，可以显著提高视频扩散模型的细粒度物理一致性，保持其强大的生成性能，同时评估了一套4D世界一致性评价的方法。
...
### [18] [Geographically-Weighted Weakly Supervised Bayesian High-Resolution Transformer for 200m Resolution Pan-Arctic Sea Ice Concentration Mapping and Uncertainty Estimation using Sentinel-1, RCM, and AMSR2 Data](https://arxiv.org/abs/2603.03503)
*Mabel Heffring,Lincoln Linlin Xu*

Main category: cs.CV

> 提出了一种基于Bayesian High-Resolution Transformer的方法，用于北极海冰浓度的地图绘制及不确定性量化。此方法通过设计Transformer模型、设计地理加权的弱监督损失函数、贝叶斯扩展和多源数据融合，能更好地检测和区分海冰特征、处理不确定性，并整合不同数据类型的海冰信息。

<details>
  <summary>Details</summary>

**Motivation:** 由于海冰特征的微妙性、不精确的海冰浓度标签、模型的不确定性以及数据的异质性等挑战，高分辨率北极海冰地图绘制和不确定性量化十分必要但也充满困难。文章的目的在于克服这些挑战，提供更精确的海冰浓度地图。

**Method:** 文章提出了一种Bayesian High-Resolution Transformer方法，通过设计新型的Transformer模型以更好地提取细微的海冰特征；设计地理加权弱监督损失函数来减少边缘区域的不确定性；采用贝叶斯扩展有效捕捉不确定性；融合三种不同数据源以提高海冰浓度及不确定性的评估。

**Result:** 评估结果表明，该模型在2021年和2025年北极海冰最小范围条件下，使用Sentinel-1数据时的整体特征检测准确率为0.70，同时保持了与ARTIST Sea Ice产品相媲美的北极海冰浓度模式（Sentinel-1 R² = 0.90）。

**Conclusion:** 该研究展示了一种新型的Bayesian High-Resolution Transformer方法，有效解决了北极海冰地图绘制中的多重挑战，实现了更高的检测精度和不确定性量化。
...
### [19] [PhyPrompt: RL-based Prompt Refinement for Physically Plausible Text-to-Video Generation](https://arxiv.org/abs/2603.03505)
*Shang Wu,Chenwei Xu,Zhuofan Xia,Weijian Li,Lie Lu,Pranav Maneriker,Fan Du,Manling Li,Han Liu*

Main category: cs.CV

> 研究提出了一种名为PhyPrompt的双阶段强化学习框架，旨在自动生成包含物理约束的提示，以生成更符合物理规律的视频。该框架在物理常识和语义保真度上实现了40.8%的成功率，并且优于其他大型模型。

<details>
  <summary>Details</summary>

**Motivation:** 当前最先进的文本到视频生成器在生成的视频中常常违反物理规律，即使具有很高的视觉质量。主要原因是提示中缺乏足够的物理约束，而直接添加这些细节需要专业知识且不具有可扩展性。

**Method:** PhyPrompt采用双阶段强化学习框架，首先微调大型语言模型，使其能够结合物理学原理进行思考，同时保持用户意图。然后，在一个动态奖励课程中应用相对策略优化技术，课程先关注语义保真度，随后逐渐转向物理常识。

**Result:** PhyPrompt-7B在联合成功、物理常识和语义保真度上分别达到了40.8%、66.8%和47.8%的成功率，超越了GPT-4o和DeepSeek-V3，并且在不同的文本到视频生成器架构上展示了零样本迁移能力。

**Conclusion:** 研究表明，对于物理意识生成而言，领域特化的强化学习结合组合课程的训练方法比直接扩大模型规模更有效。
...
### [20] [PinCLIP: Large-scale Foundational Multimodal Representation at Pinterest](https://arxiv.org/abs/2603.03544)
*Josh Beal,Eric Kim,Jinfeng Rao,Rex Wu,Dmitry Kislyuk,Charles Rosenberg*

Main category: cs.CV

> PinCLIP, a novel VLM-based method for Pinterest, boosts multi-modal retrieval and user interaction metrics through advanced alignment objectives, demonstrating a 20% performance gain over existing systems and significant business impact.

<details>
  <summary>Details</summary>

**Motivation:** The aim is to address the challenges of integrating VLMs into recommendation systems by improving retrieval efficiency and effectiveness, and specifically by reducing the 'cold-start' problem.

**Method:** This paper introduces PinCLIP, which uses a hybrid Vision Transformer architecture to integrate VLMs into recommendation systems, focusing on image-text alignment through a novel neighbor alignment objective.

**Result:** PinCLIP significantly outperforms Qwen in multi-modal retrieval tasks by 20%, and demonstrates substantial engagement gains in Pinterest's online A/B tests.

**Conclusion:** PinCLIP enhances image-text retrieval and improves user engagement on Pinterest, showcasing its effectiveness in solving retrieval and ranking issues through multi-modal learning.
