...
> 研究发现，基于西方语料训练的LLM无法有效分析印度YouTube上牛尿相关健康虚假信息，因为此类内容融合了传统宗教语言和伪科学主张，形成独特的文化修辞模式，仅靠提示工程无法解决这一问题。

<details>
  <summary>Details</summary>

**Motivation:** 全球南方社交媒体已成为健康信息主要渠道，但现有LLM主要基于西方语料训练，对文化嵌入的健康虚假信息缺乏分析能力，亟待研究LLM在跨文化虚假信息分析中的局限性。

**Method:** 采用事后LLM辅助话语分析方法，对印度YouTube上30个多语言牛尿相关视频转录文本进行分析，通过改变三个LLM（GPT-4o、Gemini 2.5 Pro、DeepSeek-V3.1）的提示语气来测试其对文化嵌入虚假信息的识别能力。

**Result:** 研究发现推广性内容将神圣传统语言与伪科学主张融合，形成LLM难以分析的特殊修辞模式；文化嵌入的健康虚假信息不同于普通虚假信息，这种文化模糊性延伸到性别修辞和提示设计中，加剧了分析的不可靠性。

**Conclusion:** LLM辅助话语分析中的文化能力不能仅通过提示工程来后期改进，需要在模型训练阶段就融入跨文化语境理解能力。
...
> 研究发现尽管LLMs对不同提示方式高度敏感，但模型内部存在共享的任务特定注意力头（词汇任务头），其激活程度解释了不同提示下的行为差异。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型的提示敏感性是用户和开发者面临的主要问题之一，即同一任务的不同提问方式会导致不可预测的性能差异。这种现象影响模型实际应用的可靠性。

**Method:** 通过比较两种常见提示风格（基于指令和基于示例），研究分析了模型的内部注意力机制，识别出任务特定的词汇任务头，并研究了这些注意力头在不同提示下的激活情况和相互竞争关系。

**Result:** 发现了跨提示风格共享的词汇任务头，这些注意力头的激活程度可以解释不同提示间的行为差异，失败的提示往往由于竞争性任务表示削弱了目标任务信号。

**Conclusion:** 研究结果为LLMs内部表示如何解释看似反常的用户行为提供了清晰图景，有助于理解和改进模型的提示敏感性问题。
...
> 本研究提出并探索了多模态模型的源模态监控能力，发现模型主要依赖语义信号而非语法信号来追踪信息来源，这在模态分布差异显著时尤为明显。

<details>
  <summary>Details</summary>

**Motivation:** 随着多模态智能系统的发展，模型需要准确追踪和表达信息来源，这对理解模型的鲁棒性和可靠性具有重要意义

**Method:** 通过11个视觉语言模型在目标模态信息检索任务上的实验，研究模型如何利用语法和语义信号将提示词与具体输入组件绑定

**Result:** 语法和语义信号都发挥重要作用，但在模态分布差异大的情况下，语义信号的影响力超过语法信号

**Conclusion:** 研究发现对理解模型鲁棒性和开发更可靠的多模态智能系统具有重要意义，为改进模态信息追踪机制提供了指导
...
> 本文提出了一种结合检索增强生成和轻量化LLM的患者-试验匹配框架，通过先检索关键临床片段再编码的方式，在保持性能的同时大幅降低计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 现有的患者-试验匹配方法存在两难：使用大型语言模型处理完整EHR文档计算成本过高，而传统机器学习方法难以捕获非结构化临床叙事信息，缺乏可扩展性和计算效率。

**Method:** 提出轻量级框架，包含两个核心组件：1）使用检索增强生成从长EHR中识别临床相关片段，降低输入复杂度；2）用LLM将这些片段编码为信息表示，经降维处理后使用轻量级预测器进行下游分类。

**Result:** 在多个公开数据集和Mayo Clinic真实数据上评估表明，检索式信息选择显著降低计算负担同时保留临床信号；冻结LLM对结构化数据表现良好，而对非结构化叙事需要微调；以更低计算成本达到端到端LLM方法的性能水平。

**Conclusion:** 通过将检索增强生成与轻量级LLM建模相结合，该框架为患者-试验匹配提供了一个高效可扩展的解决方案，平衡了性能与计算效率，具有重要的临床应用价值。
...
> 这项工作探索了神经符号语言在视觉语言模型中的应用，相比基线模型提升3.33%准确率同时推理token减少75%

<details>
  <summary>Details</summary>

**Motivation:** 受电影《降临》中外星语言启发，作者质疑人类思维的局限性，探索使用非顺序结构的神经符号语言来提升AI系统的分析推理能力和思维效率

**Method:** 基于Qwen3-VL-2B-Instruct模型，在4个Nvidia H200 GPU节点上进行训练，采用神经符号语言方法处理视觉语言概念的表示和推理

**Result:** 在包含数学、科学和常识问题的视觉语言评估数据集上，准确率提升3.33%，相比SymPy减少75%的推理token

**Conclusion:** 研究证明了神经符号语言在提升视觉语言模型推理能力方面的效果，记录了计算挑战、扩展可能性以及未来改进方向，代码已开源
...
> 研究将精神科初诊对话建模为问题选择任务，引入包含655个临床问题的基准测试，发现基于LLM的自适应问答策略在有限交互预算内信息恢复效果最优。

<details>
  <summary>Details</summary>

**Motivation:** 精神科初诊是高风险的信息收集过程，临床医生需要在有限时间内决定提问顺序并解读模糊回答，但当前对话AI在此领域应用基础设施有限。

**Method:** 构建问题选择基准数据集，包含655个临床医生编写的初诊问题和5种行为条件的合成患者病例；比较随机提问、临床初诊表单基线和LLM引导的自适应策略在300次访谈会话中的表现。

**Result:** 临床有序固定表单显著优于随机提问，LLM引导策略获得最佳整体信息恢复效果；在防范-简洁等不易现场恢复的患者行为下，自适应优势更加明显。

**Conclusion:** 临床对话系统性能不仅取决于信息披露后的语言理解，更重要的是系统是否能在有限交互预算内触及正确主题；该基准为研究临床结构和自适应跟踪对交互式临床机器学习中信息恢复的贡献提供了受控框架。
...
> 研究发现当前广泛使用的RLVR（基于可验证奖励的强化学习）方法并不总是让模型真正依赖推理过程来得出答案，但通过简单的训练改进可以解决这个问题。

<details>
  <summary>Details</summary>

**Motivation:** 目前语言模型后训练中普遍采用RLVR方法，人们假设模型生成的推理链能够可靠地反映其得出答案的过程。本研究对这个基本假设提出了质疑。

**Method:** 提出了两个新指标：推理因果重要性(CIR)和推理充分性(SR)，通过Qwen2.5模型和ReasoningGym任务进行实验分析，并探索了在RLVR基础上添加SFT和辅助CIR/SR奖励的改进方法。

**Result:** 实验发现:(1) RLVR虽提高准确率但未显著改善CIR和SR；(2) 少量SFT可改善低CIR/SR问题；(3) 添加CIR/SR辅助奖励的联合奖励方法能达到与RLVR相当的准确率，同时产生因果重要且充分的推理。

**Conclusion:** RLVR并不总是让模型按预期方式依赖推理，但通过在标准RLVR基础上添加少量SFT或CIR/SR辅助奖励等简单修改可以有效解决这一问题。
...
> 本文提出了一种高效的乌克兰文档问答RAG系统，在UNLP 2026共享任务中获得第二名，通过两阶段检索和轻量化部署实现了高精度本地AI问答。

<details>
  <summary>Details</summary>

**Motivation:** 为乌克兰文档问答任务构建高效的检索增强生成系统，在严格的计算资源限制下实现本地化部署，同时保证回答的准确性和可验证性。

**Method:** 采用定制的两阶段搜索管道检索相关文档页面，结合基于合成数据微调的专门乌克兰语语言模型生成准确、有根据的答案，最后对模型进行压缩以实现轻量化部署。

**Result:** 在严格的计算限制下评估，该系统在UNLP 2026共享任务中获得第二名，证明在资源受限的硬件上本地实现高质量、可验证的AI问答是可行的。

**Conclusion:** 研究证明了通过优化的检索管道和轻量化模型设计，可以在计算资源受限的环境下实现高精度的本地化文档问答，为低资源语言的AI应用提供了可行的技术路径。
...
> KARITA是一种通过知识驱动增强和检索的时序自适应方法，能有效捕捉语义和知识的时序变化，在多个领域的分类任务中表现出色

<details>
  <summary>Details</summary>

**Motivation:** 现有模型在开发部署中面临时序挑战：训练数据源于历史，但部署在未来的数据上，此时语义分布和领域知识可能发生演变。现有研究要么忽视时序迁移，要么难以捕获语义和知识的丰富迁移模式

**Method:** 提出知识驱动增强检索集成时序自适应框架KARITA，该方法能捕获多样时序变化（如不确定性和特征迁移），构建并整合丰富知识源（如医学本体MeSH），并利用迁移洞察进行选择-检索增强学习

**Result:** 在临床、法律和科学语料库的多个领域分类任务上评估KARITA，显示时序自适应在各领域均有一致性改进。结果表明知识整合在时序增强和学习中更为关键和有效

**Conclusion:** 知识驱动的时序自适应方法能够有效解决模型部署中的时序迁移问题，知识整合在处理时序变化时起着关键作用，为跨领域时序自适应学习提供了新思路
...
> 研究发现混合语言模型中LoRA适配器的放置策略至关重要，仅适配注意力模块（占少数）就能超越全模型微调效果，且不同混合拓扑结构对适配的响应存在根本差异。

<details>
  <summary>Details</summary>

**Motivation:** 标准LoRA方法在混合语言模型上统一应用适配器，忽略了注意力组件和循环组件的不同功能角色，需要系统研究组件感知的LoRA放置策略。

**Method:** 在两种混合架构（Qwen3.5-0.8B顺序式和Falcon-H1-0.5B并行式）上进行组件类型的LoRA放置实验，涵盖三个领域微调和五个基准测试评估。

**Result:** 注意力路径适配以5-10倍更少参数超越全模型适配；顺序式混合中适配循环网络破坏性显著（GSM8K下降14.8pp），而并行式中具有建设性（提升8.6pp）；并行式展现正迁移，顺序式出现灾难性遗忘。

**Conclusion:** 混合 topology 结构从根本上决定了模型的适配响应，组件感知的LoRA放置是混合架构必要的设计维度。
...
> 研究发现，虽然Transformer可以解码层次结构信息，但只有注意力模式中的top-of-stack信号对模型性能具有真正的因果影响，而残差流中的信号更多是可解码的副产品。

<details>
  <summary>Details</summary>

**Motivation:** 研究Transformer如何表示和利用层次结构，区分这些表示是实际被使用还是仅仅可解码。

**Method:** 在Dyck语言上训练Transformer，通过探测和干预残差流与注意力模式来分析层次结构表示的因果作用。

**Result:** 发现深度、距离和top-of-stack信号都可解码，但因果作用不同：破坏真实top-of-stack位置注意力会严重影响长距离准确性，而移除残差流子空间影响较小。

**Conclusion:** 即使在已知层次结构变量的受控环境中，可解码性也不等同于因果使用，表明需要更深入分析模型的内部机制。
...
> 本研究发现教育领域大语言模型存在教学越狱漏洞，提出了知识掌握图框架和SHaPE基准测试，并开发了图增强辅导管道方法，在保持高实用性的同时显著提升安全性。

<details>
  <summary>Details</summary>

**Motivation:** 教育大语言模型存在学生通过答案诱导提示获取直接解答而非教学指导的漏洞，这种'教学越狱'行为破坏了教育目的，需要系统性的解决方案。

**Method:** 构建知识掌握图统一安全、有用和教学行为，创建包含9,087个学生问题对的SHaPE基准；提出图增强辅导管道，通过推理先修概念、识别掌握差距，并通过显式门控在指导和问题解决之间路由生成。

**Result:** 在多个大语言模型上的实验表明，该方法在两种教学越狱设置下安全性显著提升，同时在相同评估协议下保持了近乎上限的有用性。

**Conclusion:** 通过知识掌握图形式化和图增强辅导管道，有效解决了教育LLM的教学越狱问题，为安全的教育AI系统提供了可靠框架。
...
> 研究发现大型语言模型在重写个人叙事时会产生一致的风格标准化现象，使文本变得更加精致但失去原有的情境性和个人特征。

<details>
  <summary>Details</summary>

**Motivation:** 研究旨在探究LLM重写如何改变个人叙事的风格和叙事质感，以及这种改变对数字人文和计算文本分析的影响。

**Method:** 分析了300个由三个前沿LLM在三种提示条件（通用改进、仅重写、保留语气的修订）下重写的个人叙事，使用13个来自计算风格学的语言标记来衡量变化。

**Result:** LLM重写导致功能词、缩写和第一人称代词减少，词汇多样性、词长和标点符号使用增加；文本在特征空间中趋同，难以与原文匹配；叙事从嵌入式转向距离式，因果推理从显式转向压缩抽象。

**Conclusion:** 当代LLM对文本具有方向性的牵引力，使其朝向更精致、更少情境性的语域发展；LLM修订不仅是表面编辑，而是一种具有重要影响的文本中介形式。
...
> 研究发现Claude、GPT-5.4和Gemini三大AI系统在给出个人建议时普遍表现出西方个人主义偏见，即使对来自重视家庭和社区的集体主义文化背景的用户也是如此。

<details>
  <summary>Details</summary>

**Motivation:** 研究关注AI助手在提供职业、婚姻和家庭冲突等个人建议时是否存在文化偏见，验证AI是否会根据用户的文化背景调整建议内容。

**Method:** 研究者向三个主流AI系统呈现10个真实个人困境，涉及5大洲10个国家7种语言（共840个回复），对比AI建议与世界价值观调查第七波数据中各国实际价值观的差异。

**Result:** 所有AI系统都系统性地给出西方个人主义建议，与当地价值观存在显著差异（平均差距+0.76分）；尼日利亚（+1.85）和印度（+0.82）差距最大；日本是唯一例外，AI对日本用户表现出的集体主义倾向超过了实际调查结果；各AI模型在偏见机制上存在差异。

**Conclusion:** 研究揭示了前沿AI系统在价值观层面的系统性同质化问题，AI编码了过时的文化刻板印象，需要更加关注AI的文化敏感性和价值观多样性。
...
> 研究通过因果可解释性方法发现语言模型在填充语-空位依赖和否定极项处理中存在不同的神经机制：前者具有共享且高度局部化的机制，后者缺乏统一机制。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型展现出复杂的句法能力，但其内部机制是否与语言学研究的跨构式原理对齐尚不清楚。需要深入理解模型在不同句法构造中是否使用共享的神经机制。

**Method:** 采用激活补丁技术对填充语-空位依赖和否定极项许可进行细粒度的因果可解释性分析，识别特定注意力头和MLP块的功能作用，并与分布式对齐搜索方法进行对比。

**Result:** 填充语-空位依赖在早期到中层存在高度局部化和共享的机制，而NPI处理缺乏统一机制。激活补丁识别的机制具有泛化能力，而分布式对齐搜索容易过拟合。

**Conclusion:** 语言模型在不同句法任务中采用不同的神经处理策略，通过操纵识别的组件可以提升模型在可接受性判断基准测试上的表现，验证了因果可解释性方法的有效性。
...
> 本研究提出了一个三源交互框架来评估LLMs处理参数知识、文档信息和用户信念的能力，发现模型普遍过度依赖文档信息且缺乏有效辨别有害信息的能力，通过多样化源交互数据微调可显著改善这一问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究局限于二元冲突范式（参数知识与文档或用户的单一冲突），而在真实的RAG或聊天系统中，LLMs需要同时处理来自参数知识、检索文档和用户信念的三重信息源，这种复杂交互环境下的模型行为和安全性研究存在空白。

**Method:** 提出三源交互框架，构建了2个数据集，系统评估了3个家族的27个LLMs模型，并采用多样化源交互数据进行微调实验。通过行为分析法研究模型对外部信息的依赖性和辨别能力。

**Result:** 实验表明：1) 大多数模型更依赖文档而非用户信息，且这种偏好通过后训练得到强化；2) 多数模型具有易受影响性，无法有效辨别有益和有害外部信息；3) 基于多样化源交互数据的微调可显著提升模型的辨别能力。

**Conclusion:** 本研究填补了LLMs在多源信息集成环境下的安全评估空白，为开发能够有效可靠整合多源信息的可信赖LLMs奠定了基础，提供了系统性的评估框架和改进方案。
...
> 7个指令调优的小型语言模型(3-9B参数)在口头置信度表达上均无效，数值置信度表达存在91.7%的天花板效应，分类表达会破坏模型性能，这表明口头置信度提取方法无法有效传递模型内部的不确定性信号。

<details>
  <summary>Details</summary>

**Motivation:** 言语置信度提取被广泛用于从LLMs中获取不确定性估计，但需要验证这些模型产生的口头置信度是否满足基本的效度标准。

**Method:** 对7个不同系列的指令调优模型(3-9B参数)进行预注册研究，使用数值(0-100)和分类(10类)两种置信度提取方式，在TriviaQA数据集上进行8,384个确定性试验，并应用心理测量效度检验。

**Result:** 所有7个模型在数值置信度上均无效(天花板效应率91.7%)；分类提取无法挽救效度，反而使6个模型的准确率降至5%以下；token级对数概率无法预测口头置信度；推理蒸馏模型中推理轨迹长度与置信度呈强负相关(rho=-0.36)。

**Conclusion:** 在3-9B参数规模下，最小口头置信度提取无法保持内部信号在输出接口的传递，建议在使用此类信号前进行心理测量筛选。
...
> TTS-PRISM是一个为中文文本转语音设计的多维度诊断框架，通过12维度评估体系和专门的合成管线来诊断细粒度的声学缺陷。实验表明该方法在人类对齐性上优于通用模型，并开源了代码和模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本转语音评测指标无法诊断细粒度的声学artifact或解释感知崩溃问题，缺乏细粒度、多维度的评估能力。

**Method:** 1) 建立12维度的评测体系，涵盖稳定性到高级表现力；2) 设计包含对抗性扰动和专家锚点的合成管线路径构建诊断数据集；3) 通过架构驱动的指令调优将明确的评分标准和推理集成到端到端模型中。

**Result:** 在1600个样本的黄金测试集上显示TTS-PRISM在人类对齐性上优于通用模型，对六种TTS范式的分析建立了直观的诊断标志，揭示了细粒度的能力差异。

**Conclusion:** TTS-PRISM为中文TTS提供了有效、细粒度的诊断能力，通过开源贡献推动了TTS质量评估的发展。
...
> 该研究开发了一个可解释的对话系统，通过分层属性方法识别推荐证据并生成自然语言解释，提高了教师对AI推荐策略的信任度。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型在支持教师诊断学生问题行为时缺乏透明度，无法解释推荐策略的原因，这限制了教师对系统的信任度。

**Method:** 构建基于微调LLM的可解释对话系统，采用可解释AI(xAI)的分层属性方法来识别每个推荐的对话证据，并基于证据生成自然语言解释。

**Result:** 技术评估显示该方法在识别支持证据方面优于基线方法；在22名职前教师的用户研究中，接收解释的参与者报告了更高的系统信任度。

**Conclusion:** 研究发现为提高教育对话系统中大语言模型的可解释性提供了有希望的方向，有助于增强教师对AI辅助决策的信任和接受度。
...
> 本文提出了MuDABench评测基准和对应的多智能体工作流，用于解决大规模半结构化文档集合的跨文档分析性问答难题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多文档问答基准通常只需要少量文档的有限跨文档推理，无法满足需要从大量文档中提取和整合信息进行定量分析的实际需求，因此需要构建更具挑战性的分析性问答评测基准。

**Method:** 1) 通过远程监督利用文档级元数据和标注金融数据库构建MuDABench基准，包含8万多页面和332个分析性问答实例；2) 提出评估协议测量最终答案准确率并使用中间事实覆盖率作为推理过程的辅助诊断信号；3) 设计多智能体工作流，协调规划、提取和代码生成模块。

**Result:** 标准RAG系统在MuDABench上表现较差；所提出的多智能体方法显著改善了过程和结果指标，但与人类专家性能仍有显著差距。

**Conclusion:** 当前系统存在单文档信息提取准确率不足和领域特定知识匮乏两大瓶颈，MuDABench为推动大规模跨文档分析性问答研究提供了重要平台。
...
> RC-RAG是一种无需微调的多阶段释义引导关系补全框架，通过在检索、摘要生成和推理阶段整合关系释义，显著提升了大模型在稀疏信息处理上的表现。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型(LLMs)在处理关系补全(RC)任务时存在困难，特别是在需要处理稀有或稀疏表示信息的情况下，无论是否使用检索增强生成(RAG)都存在这个问题。

**Method:** 提出了RC-RAG框架，通过三个系统化阶段整合关系释义：(a)在检索阶段整合释义以扩展关系的词汇覆盖；(b)使用释义生成关系感知的摘要；(c)在生成阶段利用释义引导关系补全的推理过程。该方法无需模型微调。

**Result:** 在两个基准数据集上对五个LLM的实验表明，RC-RAG持续优于多个RAG基线。在长尾场景下，最佳LLM配合RC-RAG比独立使用提升了40.6个EM点，比两个强RAG基线分别提升16.0和13.8个EM点，同时保持了较低的计算开销。

**Conclusion:** RC-RAG框架通过多阶段释义引导策略有效解决了LLMs在关系补全任务中的稀疏信息处理难题，无需微调即可显著提升性能，特别是在长尾领域表现突出。
...
> 研究发现大语言模型在链式思考中，70%的情况下答案在中间步骤就已确定，后续生成的是冗余解释；通过早期停止策略可减少500个推理token，仅损失2%准确率。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型通过生成长的链式思考推理获得良好性能，但不清楚最终答案何时确定。如果答案在中间阶段已固定，后续推理token只是事后解释，会增加推理成本和延迟而不提高正确性。

**Method:** 使用强制答案完成方法研究预测答案在推理步骤中的演化，在部分推理前缀时引出模型的中间预测。重点关注Qwen3-4B模型，并研究早期停止策略。

**Result:** 在所有数据集中，预测答案仅在32%的查询中发生变化。最终答案确定后，模型平均额外生成760个推理token。简单的启发式早期停止策略可减少500个token的使用，仅导致2%的准确率下降。

**Conclusion:** 链式思考生成的大部分是冗余的，可以通过早期停止策略在最小性能影响下显著减少推理开销。
...
> STEM是一种基于知识图谱的多跳问答框架，通过模式引导的图搜索和全局结构感知来解决语义不匹配和推理路径检索不完整的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有KGQA系统面临两个核心挑战：知识图谱的结构异构性导致检索时语义不匹配，以及现有的推理路径检索方法缺乏全局结构视角。

**Method:** 提出了Structure-Tracing Evidence Mining (STEM)框架，包含：1) 语义到结构投影管道，利用KG结构先验将查询分解为原子关系断言并构建自适应查询模式图；2) 全局感知的节点锚定和子图检索；3) 设计Triple-Dependent GNN生成全局引导子图来指导图构建过程。

**Result:** STEM显著提高了多跳推理图检索的准确性和证据完整性，在多个多跳基准测试中达到了最先进的性能。

**Conclusion:** 通过将多跳推理重新定义为模式引导的图搜索任务，并集成全局结构信息，STEM有效解决了KGQA领域的关键问题，为复杂推理任务提供了新的解决方案。
...
> ReLeVAnT是一个用于法律文档二分类的轻量级框架，通过n-gram处理、对比分数匹配和浅层神经网络实现了99.3%的准确率和98.7%的F1分数，无需依赖元数据或大量计算资源。

<details>
  <summary>Details</summary>

**Motivation:** 现有法律文档分类方法依赖结构化数据、元数据或大量计算资源，而实际应用需要更轻量级、高效且准确的解决方案，特别是在起草动议、备忘录、案件摘要等下游任务中。

**Method:** 提出ReLeVAnT框架，结合n-gram处理、对比分数匹配和浅层神经网络作为主要的判别分类驱动器，利用一次性关键词提取和浅层分类器进行快速可靠的分类。

**Result:** 在LexGLUE数据集上达到99.3%的准确率和98.7%的F1分数，证明了该方法的高效性和可靠性。

**Conclusion:** 通过利用文档类别间的判别性特征，ReLeVAnT为法律文档分类提供了一种轻量级且高效的解决方案，避免了传统方法对元数据和大量计算资源的依赖。
...
> SLIDERS是一个通过结构化推理解决长文档问答问题的框架，它将信息提取到关系数据库中，使用SQL进行可扩展推理，并在3.9M和36M token的大规模场景下显著超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现实世界的文档问答面临挑战：分析师需要综合多个文档的证据，但随着文档集合增长，固定的LLM上下文窗口会被超出；传统的分块处理方法存在聚合瓶颈，当块数量增加时仍需处理大量提取的证据。

**Method:** SLIDERS框架将显著信息提取到关系数据库中，通过SQL而非拼接文本实现可扩展推理；引入数据协调阶段，利用来源信息、提取理由和元数据来检测和修复重复、不一致和不完整的记录，确保局部提取表示的全局一致性。

**Result:** 在三个现有长上下文基准测试中，SLIDERS超越了所有基线方法（平均比GPT-4.1高6.6分）；在3.9M和36M token的两个新基准测试上，分别比次优基线方法提高约19分和32分。

**Conclusion:** 通过结构化推理和数据协调机制，SLIDERS有效解决了长文档问答中的上下文窗口限制和聚合瓶颈问题，在超大规模文档场景下实现了显著的性能提升。
...
> Clarity是一个自动生成多模态歧义NL2SQL基准测试的框架，研究发现主流NL2SQL系统在处理复杂歧义时性能显著下降。

<details>
  <summary>Details</summary>

**Motivation:** 工业环境中的NL2SQL系统常遇到歧义或无法回答的查询，特别是在用户澄清不完整的交互场景中。现有基准测试假设单一歧义源并依赖用户交互解决，忽视了现实的失败模式。

**Method:** 提出了Clarity框架，使用约束驱动的流水线，将可执行SQL转换为歧义查询，并添加基于事实的对话延续和模式级元数据，在单轮和多轮设置中自动生成多面歧义的NL2SQL基准测试。

**Result:** 在Spider和BIRD数据集上的实证评估表明，包括基于强大LLM的系统在内的主流NL2SQL系统在多面歧义下性能显著下降。这些系统虽然能检测歧义，但难以准确定位和解决底层的模式级歧义源。

**Conclusion:** 研究结果突显了工业级NL2SQL系统需要更强的歧义检测和解决能力，为改进实际应用中的NL2SQL系统提供了重要见解和基准测试工具。
...
> 本研究提出了一个让领域专家仅需提供实体名称和标签就能创建特定领域分类器的框架，通过动态获取实体描述文本并结合网络和大型语言模型，在企业SIC代码分类和医疗提供者分类任务上分别达到82.3%和72.9%的F1分数。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自然语言处理资源缺乏特定任务所需的信息，对知名度较低或新引入的实体覆盖有限，企业和医疗机构等需要针对具体应用任务进行多种分类方案。

**Method:** 提出了一种新颖的文本获取方法，结合网页搜索和大型语言模型动态获取每个实体的描述文本，然后基于这些文本构建文本分类器，只需实体名称和黄金标签作为训练数据。

**Result:** 在两个不同领域的分类任务上取得良好效果：SIC代码分类获得82.3%的宏平均F1分数，医疗提供者分类获得72.9%的宏平均F1分数。

**Conclusion:** 该框架有效地解决了特定领域实体分类问题，为领域专家提供了简单易用的解决方案，验证了结合网络资料和LLM获取实体描述信息的方法的可行性和有效性。
...
> 本文提出了一种名为上下文保真度增强(CFB)的轻量级解码时框架，通过提高源文本支持的词元生成概率来减少大语言模型的忠实度幻觉，无需重新训练即可显著提升生成内容的忠实性。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在生成内容时常会忽略或违背输入上下文中的信息，这种现象被称为忠实度幻觉，严重影响模型在实际应用中的可靠性。

**Method:** 受水印技术的logit塑造原理启发，CFB通过基于词元在输入上下文中的支持程度进行加法性logit调整。具体包含三种策略：静态提升（对源支持的词元施加固定偏置）、上下文感知提升（根据有无上下文的下一词元分布差异缩放偏置）、词元感知提升（根据源位置注意力和源范围语义相似度估计的局部相关性重新分配自适应偏置）。

**Result:** 在多个开源大语言模型的摘要和问答任务上，CFB始终如一地改善了忠实性指标，且生成开销极小。

**Conclusion:** CFB作为一种轻量级、通用的解码时框架，无需重新训练或架构修改即可有效减少忠实度幻觉，为提高大语言模型输出可靠性提供了实用解决方案，已完全开源。
...
> 本文提出DPS框架，通过因果分析识别LLMs中的'偏好头'(Preference Heads)，实现无需训练的可控个性化文本生成，显著提升个性化效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM个性化方法多将隐式个性化能力视为黑盒，依赖提示工程或用户数据微调，缺乏对个性化机制的理解和控制能力。

**Method:** 开发差分偏好引导(DPS)框架：首先通过因果掩码分析识别偏好头，计算偏好贡献评分(PCS)量化其因果影响；推理时对比有无偏好头的模型预测，放大个性化与通用logits差异。

**Result:** 在多个LLM和个性化基准测试中，DPS持续提升个性化保真度，保持内容连贯性，计算开销低，并提供了个性化在Transformer架构中产生机制的物理解释。

**Conclusion:** DPS为理解LLM个性化机制提供了新视角，通过识别和利用稀疏偏好头实现了高效可控的个性化生成，为个性化AI系统发展奠定了重要基础。
...
> 本研究提出了CNSL-bench，首个面向多模态大语言模型的中国国家手语理解评测基准，发现当前模型在手语理解上仍远逊于人类水平。

<details>
  <summary>Details</summary>

**Motivation:** 虽然大语言模型推动了手语研究进展，但LLMs对手语的理解能力，特别是在多模态环境下的固有能力尚未得到充分探索和研究。

**Method:** 构建了基于《国家通用手语词典》的CNSL-bench基准，包含权威标准化数据、多模态覆盖（文本描述、图像和手语视频）以及动作多样性（空中书写、手指拼写和中文手语字母表），并用其评估了21个开源和专有的MLLMs。

**Result:** 评估结果显示，尽管多模态建模有进展，当前MLLMs在手语理解上仍显著落后于人类表现，在不同输入模态和手动动作形式上存在系统性差异，且推理能力提升之外仍存在多个局限性。

**Conclusion:** CNSL-bench为评估MLLMs的手语理解能力提供了重要基准，研究结果揭示了当前模型的不足之处，为未来改进指明了方向。
...
> 本文提出了一种名为SCL-SLT的选择性对比学习方法，通过分析训练过程中的负样本相似度动态，识别出有用的负样本并构建渐进式难度的对比学习策略，以改进手语翻译中的视觉-文本对齐效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的手语翻译系统使用随机批内对比进行跨模态对齐，但这种方法的负样本有限、依赖批次大小，且可能将语义相似的样本错误标记为负样本，引入噪声和不一致的对齐监督。

**Method:** 作者首先进行了基于轨迹的分析，跟踪负样本视频-文本相似度在训练过程中的变化；然后提出了SCL-SLT框架，包含配对选择(PS)策略，利用参考检查点的相似度动态对候选负样本评分，并通过课程学习逐步强调更具挑战性的负样本。

**Result:** 分析显示只有一小部分负样本表现出预期的持续被推开的行为，其余负样本表现出异质且通常不递减的相似度动态，表明随机批内负样本经常对有效对齐没有信息价值。

**Conclusion:** 选择性对比学习方法能够加强对比监督，同时减少噪声或语义无效负样本的影响，为解决手语翻译中的视觉-文本模态不匹配问题提供了新的思路。
...
> AI写作助手会扭曲作者的人设，让他们看起来更偏激、更有能力、更积极，并使人口特征向特权群体转移。即使作者反对这些扭曲，仍偏爱使用AI辅助写作。研究人员虽能在模型层面缓解这些扭曲，但这会影响用户接受度，表明AI写作助手的理想特性与问题特性难以分离。

<details>
  <summary>Details</summary>

**Motivation:** 数亿人使用AI写作辅助工具，但这些工具如何影响作者的人设（包括感知的信念、个性和身份）尚未被系统评估。了解这种影响对于维护公共话语、信任和民主审议至关重要。

**Method:** 通过三个大规模实验，让2,939名作者在有和没有AI协助的情况下撰写政治意见段落，另外11,091名读者在29个社会显著性维度上盲评这些段落。研究人员还基于实验数据训练奖励模型来缓解值得反对的人设扭曲。

**Result:** AI协助使作者看起来更偏激、更有能力、更积极，其感知人口特征向特权群体转移。作者反对多数扭曲但仍偏好AI辅助文本。模型层面的缓解措施降低了用户接受度。

**Conclusion:** AI写作助手的人设扭曲普遍存在且持续，即使在人类监督的现实条件下也是如此。理想与不理想的特性难以分离，随着AI采用规模扩大，这对公共话语、信任和民主审议带来深远影响。
...
> 研究发现在评估LLM生成的商业想法时，个性化评判器比聚合评判器更能准确反映个体专家的评分偏好，且专家间的分歧具有结构性而非随机性。

<details>
  <summary>Details</summary>

**Motivation:** 评估LLM生成的商业想法存在规模化难题，因为评估涉及多维度复杂标准，且专家判断经常存在分歧，需要探索自动评判器应该如何应对这种分歧。

**Method:** 构建PBIG-DATA数据集（包含300个专利相关产品想法的约3000个个体评分），比较三种评判器配置：仅评分标准的零样本评判器、基于混合评估历史的聚合评判器、以及基于目标评估者评分历史的个性化评判器。

**Result:** 专家在细粒度评分上存在显著分歧，但在粗粒度选择上一致性较高；个性化评判器比聚合评判器与相应评估者的评分更为吻合；仅在个性化条件下，评估者间的一致性与评判器生成的推理相似性相关。

**Conclusion:** Pooling标签在多元评估环境中可能是不稳定的目标，研究结果支持采用评估者条件化的评判器设计来进行商业想法评估。
...
> 本文提出了RouteLMT，一种高效的混合机器翻译路由策略，通过预测大模型相对于小模型的边际收益来优化预算分配，在不增加外部模型的情况下实现更优的质量-成本平衡。

<details>
  <summary>Details</summary>

**Motivation:** 虽然大型语言模型在机器翻译中表现优异，但大规模部署成本过高。现有的混合系统路由策略依赖启发式规则、外部预测器或绝对质量估计，无法准确判断大模型是否真正能提供比小模型有价值的改进。

**Method:** 将路由问题建模为预算分配问题，以边际增益（大模型相对于小模型的改进）作为最优决策信号。提出RouteLMT，通过探测小模型的提示标记表示来预测预期增益，无需外部模型或假设解码。

**Result:** 实验表明RouteLMT优于启发式方法和质量/难度估计基准，实现了更优的质量-预算帕累托前沿。同时分析了回归风险，并提出了简单防护变体来减轻严重的质量损失。

**Conclusion:** RouteLMT通过基于边际增益的路由策略，有效解决了LLM机器翻译中的成本与质量平衡问题，为实际部署提供了经济高效的解决方案。
...
> 本研究提出了一个针对K-12英语学习者的熟练度对齐框架，通过DDPO算法控制语言模型输出的词汇复杂度，提供个性化英语口语练习解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型由于熟练度不匹配，无法满足非母语环境下K-12英语学习者的教学需求，这是一个普遍存在的挑战。

**Method:** 基于中国国家标准(CSE)构建四层分级系统，开发了分级词汇表和多轮对话语料库，核心技术是DDPO算法（多样性驱动策略优化），在保持对话多样性的同时全面优化对话质量。

**Result:** 该方法显著优于传统方法，实现了低词汇表外率和高多样性，同时提升了对话自然性和教学价值，框架具有良好的灵活性和可扩展性。

**Conclusion:** 该框架为非沉浸式环境下的K-12学习者提供了可扩展的个性化英语口语练习平台，所有模型、数据和代码将开源发布，可适应其他教育标准。
...
> 本研究提出了eBISG方法，通过预训练文本嵌入和神经网络技术，解决了传统BISG方法在处理非常见姓氏时预测性能下降的问题，显著提高了种族估计的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 种族差异估计需要个人级别的种族数据，但由于收集此类信息的敏感性，这些数据往往不可获得。传统的BISG方法严重依赖人口普查姓氏数据，但这些数据仅覆盖常见姓氏，遗漏了约10%的美国人口

**Method:** 提出嵌入式BISG(eBISG)，使用预训练文本嵌入将姓名表示为密集向量，训练神经网络处理2020年人口普查的姓氏和名字数据，为人口普查中未覆盖的姓名估计种族概率。比较了五种方法：标准BISG、结合名字概率的BIFSG、未列名姓氏嵌入、姓氏和名字组合嵌入、基于选民文件的全名嵌入。

**Result:** 每种后续的eBISG方法都改进了种族预测性能，其中全名嵌入方法效果最佳，特别是对于姓氏未出现在人口普查名单中的西班牙裔和亚裔选民，预测精度提升最大。

**Conclusion:** eBISG方法有效克服了传统BISG在处理非常见姓氏时的局限性，通过利用现代嵌入技术和神经网络，为种族差异研究提供了更准确和全面的数据支持。
...
> HiLight是一种证据强调框架，通过在原始上下文中插入高亮标签来帮助大语言模型更好地定位关键证据，在无需监督标注的情况下显著提升了模型在长文本推理任务中的表现。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在处理长文本时，往往会忽略埋藏在噪音中的决定性证据。现有的压缩或重写方法可能会丢失或扭曲关键信息，因此需要一种既能保留原始上下文完整性又能突出重要证据的方法。

**Method:** 提出了HiLight框架，将证据选择与推理解耦。训练轻量级的Emphasis Actor在未修改的上下文中插入最小化的高亮标签，然后冻结的Solver对强调后的输入进行推理。使用强化学习优化Actor，仅需任务奖励信号，无需证据标注和Solver修改。

**Result:** 在顺序推荐和长文本问答任务上，HiLight持续优于基于提示和自动提示优化的基线方法。学到的强调策略能够零样本迁移到不同大小的未知Solver模型中。

**Conclusion:** HiLight证明了轻量级强调组件能够学习到可重用的证据结构，而不是过度拟合到特定的骨干模型，为提升大语言模型在长文档理解能力方面提供了有效且实用的解决方案。
...
> 该研究利用大型语言模型分析了100个关于牛尿健康功效的YouTube视频，发现推广者主要使用功效诉求和社会证明策略，而驳斥者更注重权威引用和反驳论证。

<details>
  <summary>Details</summary>

**Motivation:** 健康虚假信息在社交媒体上是一个严重挑战，特别是当传统文化与科学主张相遇时，需要深入研究这些文化特定的争议性话题的修辞策略。

**Method:** 采用包括GPT-4、Gemini 2.5 Pro等在内的多种大型语言模型，基于14个类别的说服策略分类法对100个牛尿相关YouTube转录本进行注释分析。

**Result:** 分析显示推广者主要依赖功效诉求和社会证明，驳斥者侧重权威性引用和反驳论证；人工评估显示90.1%的注释者间一致性。

**Conclusion:** 这项研究推进了虚假信息分析的计算方法，证明了大型语言模型能够支持大规模网络文化话语研究。
...
> 研究使用元音-辅音编码和马尔可夫链模型分析但丁《神曲》的结构组织，发现了从《地狱》到《天堂》的递增依赖模式，揭示了局部依赖、词汇分布和整体组织间的结构化相互作用。

<details>
  <summary>Details</summary>

**Motivation:** 通过量化分析方法探索《神曲》的文本结构组织，理解局部符号动力学如何与更高层次的文本组织相关联。

**Method:** 采用V/C编码将文本转化为符号序列，构建四态马尔可夫链模型，进行三元组分析和互补分类分析，结合词法分布和正字法现象。

**Result:** 发现了从地狱篇到天堂篇的一致性递增趋势，识别出限制性递归配置作为图形探针，区分了跨词边界和词内结构的不同行为模式。

**Conclusion:** 简单的概率模型能够有效揭示文本中局部依赖、词汇分布、正字法编码和大尺度组织之间的结构化相互作用，为文本分析提供了可解释的框架。
...
> 该研究提出了分类ASR系统中音素建模错误的框架，发现随机误差对ASR公平性的影响大于系统性误差，且公平性增强算法无法有效减少随机误差。

<details>
  <summary>Details</summary>

**Motivation:** 现代ASR系统对不同说话人群体的性能存在差异，为了实现更公平的ASR系统，需要深入理解编码器模型的建模错误类型，特别是高性能和低性能说话人群体的嵌入结构差异。

**Method:** 提出了一个框架来区分两种音素建模错误：随机误差（音素嵌入的高方差）和系统性误差（嵌入偏置）。通过训练仅针对单个弱势群体的音素分类探针来评估偏置，并分析音素方差与预测准确性的关系。

**Result:** 研究发现两种错误类型都存在于音素嵌入中，但随机误差对公平性的阻碍更大。音素方差较高的说话者和群体表现更差。公平性增强算法无法改变域内音素分类训练的好处，也无法减少随机嵌入误差水平。

**Conclusion:** ASR系统的公平性受到随机误差和系统性误差的共同影响，其中随机误差是更大障碍。现有的公平性增强算法在解决核心随机误差问题方面效果有限，需要开发新的方法来减少嵌入中的随机误差。
...
> BERAG是一种基于贝叶斯集成的检索增强生成框架，通过条件化独立文档而非拼接上下文来解决标准RAG的局限性，显著提升了长文本和视觉问答性能。

<details>
  <summary>Details</summary>

**Motivation:** 标准RAG将检索文档拼接为单一上下文存在关键问题：无法进行清晰的贡献归因、导致

**Method:** BERAG将文档后验概率作为集成权重，在生成过程中使用贝叶斯规则逐token更新权重，实现对独立文档的概率重新排序、并行内存使用和清晰的贡献追踪，配合BEFT微调方法。

**Result:** 在基于知识的视觉问答任务上，BERAG相比标准RAG取得显著改进，文档视觉问答和多模态大海捞针基准测试获得强增益，有效缓解

**Conclusion:** BERAG通过贝叶斯集成框架为RAG提供了文档级别的细粒度控制，不仅提升性能还提供可解释性，其文档后验概率可用于检测缺乏依据的情况并触发规避，文档裁剪实现比标准RAG更快的解码速度。
...
> CRAFT是一种快速高效的数据选择方法，通过聚类回归从大规模语料库中筛选高质量子集进行序列到序列模型微调。

<details>
  <summary>Details</summary>

**Motivation:** 随着语料库增长到数千万数据点，完整微调变得昂贵且往往不必要，因此需要从大规模语料库中选择小规模高质量子集进行微调的方法。

**Method:** CRAFT方法分两阶段进行：首先通过对验证集源分布进行k-means聚类并按比例分配预算；然后在每个源聚类内，选择使验证目标分布的条件期望距离最小的训练对。该方法与向量化方式无关。

**Result:** 在英语-印地语翻译任务中，CRAFT从3300万NLLB句子对中选择数据微调mBART，达到43.34 BLEU分数，比TSDS方法高2.13点，选择速度快40倍。使用TF-IDF向量化时，整个流程在CPU上不到一分钟完成。

**Conclusion:** CRAFT提供了一种理论上可证明且实用快速的训练数据选择方案，在保持和超越现有方法性能的同时，大幅提升选择效率，为大规模序列到序列模型的高效微调提供了新思路。
...
> 提出抽象思维链方法，通过离散潜在推理机制大幅减少推理token数量（减少11.6倍）的同时保持与自然语言CoT相当的性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统显式思维链虽然有效但推理成本高昂，非文字推理方法虽然生成长度短但性能落后，需要一种既高效又强大的推理方法。

**Method:** 设计Abstract-CoT框架，使用保留词汇表中的抽象token序列替代自然语言CoT，采用策略迭代式的热身循环（包括瓶颈化监督微调和自蒸馏），最后通过约束解码下的强化学习进行优化。

**Result:** 在数学推理、指令遵循和多跳推理等任务中，Abstract-CoT实现了11.6倍的token减少，性能与自然语言CoT相当，且在多种语言模型间具有良好的泛化能力。

**Conclusion:** 抽象词汇表呈现出与自然语言类似的幂律分布，证明了通过学习抽象推理语言实现高效推理的潜在价值，为后训练潜在推理机制开辟了新方向。
...
> 研究发现广泛使用的大型语言模型在开放性叙事生成中持续存在基于国别的表征伤害，包括刻板印象、忽视和对全球多数群体的片面描述。

<details>
  <summary>Details</summary>

**Motivation:** 随着LLM被广泛应用于从日常到高风险的各种场景（如政府的避难申请者模拟面试），需要研究LLM如何表征不同国别身份，以评估和减轻其对非主导群体的潜在偏见伤害。

**Method:** 通过向主流LLMs输入开放性叙事生成提示，分析其对不同国别身份的表征模式，特别关注全球多数群体的描述是否存在偏见和伤害。

**Result:** 发现LLM存在持续的表征伤害，包括有害刻板印象、忽视和片面描述。少数群体国别身份在权力中性故事中代表性不足，但在从属角色描述中过度代表（比主导角色描述多50倍）。当输入包含美国国籍提示时伤害程度加剧，且这种以美国为中心的偏见无法用迎合性解释。

**Conclusion:** 呼吁通过以全球多数群体视角为中心的方法论进一步探索LLM中的文化伤害，并挑战在分类、监控和错误表征全球大多数人方面不加批判地采用基于美国的LLMs。
...
> 该研究首次系统分析了AI代理在编程任务中的令牌消耗模式，发现代理任务令牌消耗极高且波动巨大，不同模型效率差异显著，且模型无法准确预测自身令牌使用量。

<details>
  <summary>Details</summary>

**Motivation:** 随着AI代理在复杂人类工作流程中的广泛应用，LLM令牌消耗快速增长。针对需要大量令牌的任务，需要回答三个核心问题：代理将令牌花费在哪里？哪些模型更令牌高效？代理能否在任务执行前预测令牌使用量？

**Method:** 研究分析了8个前沿LLM在SWE-bench Verified基准上的执行轨迹，评估模型在任务执行前预测自身令牌成本的能力，通过对比不同模型、不同任务的令牌消耗模式进行系统性研究。

**Result:** 主要发现包括：(1)代理任务令牌消耗极高，比代码推理和聊天多1000倍，主要由输入令牌驱动；(2)令牌使用高度变异且具有随机性，相同任务可相差30倍，更高令牌使用不一定带来更高准确性；(3)模型效率差异巨大，Kimi-K2和Claude-Sonnet-4.5比GPT-5平均多消耗150万令牌；(4)人类评估的任务难度与实际令牌成本相关性较弱；(5)前沿模型无法准确预测自身令牌使用，相关系数最高仅0.39且系统性低估。

**Conclusion:** 该研究为AI代理的经济学提供了新的见解，揭示了人类感知复杂性与代理实际计算努力之间的根本差距，可启发未来相关研究方向。
...
> 该方法使用单一图像（尤其是球面图像）来精确预测城市环境中太阳能板在任何未来时刻的辐照度，相比传统方法更准确。

<details>
  <summary>Details</summary>

**Motivation:** 太阳能板在城市环境中的应用日益广泛，但其软成本（如辐照度评估）居高不下。传统的3D模型方法无法捕捉影响辐照度的小型附近结构，导致预测不准确。

**Method:** 通过分析太阳能板位置的单张图像，利用视觉线索确定相机方向和面板可见天空部分，预测太阳和天空造成的辐照度。同时证明附近建筑物反射造成的辐照度随时间平滑变化，可从图像中预测。开发了Solaris设备来捕获面板在各种城市场景中看到的图像。

**Result:** 在真实城市峡谷环境中验证表明，该方法比传统辐照度转换方法和基于3D模型的模拟产生更准确的辐照度预测。单张球面图像还能用于找到面板的最佳固定朝向。

**Conclusion:** 这种基于图像的方法能够准确评估任意表面的太阳能潜力并预测面板辐照度的时间变化，为城市太阳能部署提供了降低软成本的有效解决方案。
...
> 软各向异性图(SAD)是一种显式、可微分的图像表示方法，通过自适应站点的各向异性度量和加权距离分数实现高效渲染，在保持质量的同时显著提升训练速度。

<details>
  <summary>Details</summary>

**Motivation:** 解决现有图像表示方法(如Image-GS和Instant-NGP)在渲染效率、训练速度和内存占用方面的局限性，需要一种既保持高质量又能实现快速训练和紧凑存储的表示方法。

**Method:** SAD使用自适应站点集合，每个站点定义各向异性度量和加权距离分数，通过softmax混合top-K子集计算像素颜色，采用类似跳越泛洪的top-K传播方案更新列表，配合GPU优先的训练管线和自适应预算控制。

**Result:** 在标准基准测试中持续超越Image-GS和Instant-NGP，在Kodak数据集上达到46.0dB PSNR，编码时间仅需2.2秒，相比基线方法实现4-19倍的端到端训练加速。

**Conclusion:** SAD证明了其在可微管线集成、快速随机访问和紧凑存储方面的有效性，为图像表示和渲染提供了一个高效、实用的解决方案。
...
> EgoMAGIC是一个包含3,355个医疗任务视频的第一人称数据集，集成了40个YOLO模型用于检测124个医疗物体，为医疗AI应用提供基础资源。

<details>
  <summary>Details</summary>

**Motivation:** DARPA的PTG项目旨在开发集成AR眼镜的虚拟助手，帮助用户执行复杂医疗任务，因此需要构建一个大规模的第一人称医疗活动数据集。

**Method:** 通过头戴式立体摄像机收集50种医疗任务的视频，训练40个YOLO模型检测124个医疗物体，建立了基于动作检测的基准测试。

**Result:** 训练了40个YOLO模型（195万标签），在8个医疗任务的动作检测基准测试中，最佳模型达到平均mAP 0.526。

**Conclusion:** EgoMAGIC数据集不仅适用于动作检测，还可支持动作识别、物体检测、错误检测等多种计算机视觉任务，为医疗AI研究提供了重要基础。
...
> H-Sets是一个新颖的图像分类器特征交互发现与归因框架，通过Hessian检测局部交互对并用SAM分割作为空间先验，结合IDG-Vis方法生成更稀疏且更忠实的显著图。

<details>
  <summary>Details</summary>

**Motivation:** 现有特征归因方法主要关注边际效应，忽视了特征间交互作用，而图像分类中语义含义往往来源于像素间的相互依赖关系而非独立特征。现有基于交互的图像方法要么粗糙（如仅使用超像素），要么无法满足核心可解释性公理。

**Method:** H-Sets采用两阶段框架：首先通过输入Hessian检测局部交互对并递归合并为语义连贯集合，使用SAM分割作为空间分组先验；其次使用IDG-Vis对每个集合进行归因，这是Integrated Directional Gradients的集合级扩展，沿像素空间路径积分方向梯度并用Harsanyi分红聚合。

**Result:** 在ImageNet和CUB数据集上对VGG、ResNet、DenseNet和MobileNet模型的评估表明，相比现有方法，H-Sets生成的显著图具有更好的可解释性和忠实性，虽然Hessian在检测阶段增加了计算成本，但这种针对性的投入能够持续产生更稀疏且更忠实的结果。

**Conclusion:** H-Sets框架成功地将特征交互分析引入图像分类器解释领域，通过结合Hessian检测和SAM分割，以及创新的IDG-Vis归因方法，为理解深度模型的决策过程提供了更准确和可解释的工具。
...
> 本文提出了FLARE-BO框架，通过贝叶斯优化自适应选择8个图像增强参数，显著提升了低光照条件下的视觉感知性能。

<details>
  <summary>Details</summary>

**Motivation:** 自主机器人系统在低光照环境下面临视觉感知挑战，现有贝叶斯优化方法仅优化3个参数且缺乏光照分解和白平衡校正，噪声抑制效果有限。

**Method:** FLARE-BO联合优化8个参数（伽马校正、LIME光照归一化、色度去噪、双边滤波、NLM去噪、灰度世界白平衡、自适应后平滑），采用单位超立方体参数归一化、目标标准化、Sobel准随机初始化和对数期望改进获取函数进行搜索。

**Result:** 在LOL低光照数据集上的基准测试表明，该方法相比未专门在该数据集上训练的现有方法取得了显著改进。

**Conclusion:** FLARE-BO扩展了贝叶斯优化图像增强框架，通过多参数联合优化实现了更好的低光照图像增强效果，为自主机器人系统的鲁棒视觉感知提供了有效解决方案。
...
> 针对鱼眼摄像机与光学动作捕捉系统的外参标定问题，提出了一种鲁棒的标定与验证系统，能够有效检测标定衰减

<details>
  <summary>Details</summary>

**Motivation:** 光学动作捕捉系统在AR/VR、SLAM和机器人数据集中广泛使用，但外参标定过程存在多种误差源，且故障不易被及时发现。鱼眼相机的非均匀畸变使标定和验证更加困难

**Method:** 提出联合估计相机外参和标定板到标记点变换的方法，采用分阶段求解器提高在模糊初始化下的收敛可靠性。开发了lollypop验证系统，通过完全独立于标定数据的测量链进行快速、无操作员的评估

**Result:** 在Meta Quest 3头戴设备的鱼眼相机实验中，该标定系统优于现有基准，lollypop能够可靠地检测标定随时间的衰减。系统已部署到生产数据收集管道中

**Conclusion:** 该系统有效解决了鱼眼相机-动作捕捉系统标定中的关键挑战，提供了鲁棒的标定方法和独立验证机制，确保数据质量并已投入实际应用
...
> 本文提出Pixel-Aligned 1DoF Gaussian Splatting (PAGaS)，通过将高斯表示约束为只有一个自由度的深度参数，实现了高精度的多视图立体深度估计。

<details>
  <summary>Details</summary>

**Motivation:** 早期的高斯泼洒方法在场景几何建模方面存在不足，虽然近期通过约束高斯分布的扩展和形状有所改善，但仍需要更好地适配多视图立体深度任务。

**Method:** PAGaS使用像素对齐的单自由度高斯分布，将高斯的位置和大小限制在反向投影的像素体积内，仅将深度作为唯一需要优化的自由度参数。

**Result:** 在具有挑战性的3D重建基准测试上，PAGaS生成了高度详细的深度图，定量验证显示其性能超越了参考几何和学习基线方法。

**Conclusion:** 通过将高斯表示从新颖视图合成适配到多视图立体深度任务，PAGaS证明了紧约束的单自由度高斯分布可以有效提升几何保真度和深度估计精度。
...
> 本文提出一种无需病变标注的无监督异常检测框架，通过学习健康视网膜的解剖结构分布，在OCT图像异常检测与分割任务上实现了跨设备的良好泛化性能。

<details>
  <summary>Details</summary>

**Motivation:** OCT图像自动分析面临标注成本高昂的问题，监督学习方法由于标注样本有限，难以在不同病理、设备和患者群体间泛化。亟需一种无需病变标注的方法来提高临床部署效率。

**Method:** 采用离散潜在模型在正常B扫描上训练，结合视网膜层级感知监督和结构化三元组学习来分离健康与病理表征。推理时通过重建差异进行异常检测和定位。

**Result:** 在Kermany数据集AUROC达0.799，显著优于多个基线模型；在Srinivasan跨数据集评估中AUROC达0.884，显示出色泛化能力；在RETOUCH外部基准上异常分割Dice为0.200，mIoU为0.117。

**Conclusion:** 所提出的无监督异常检测框架有效解决了标注效率问题，在不同机构和设备间展现出良好的鲁棒性和可重现性，为临床OCT图像分析提供了实用解决方案。
...
> 受人类视觉感知启发，提出一种分层生成模型，将运动线索和外观特征统一聚合成粒子和簇，实现对不同输入类型的运动检测和物体分割。

<details>
  <summary>Details</summary>

**Motivation:** 现有计算机视觉系统缺乏统一处理稀疏点、纹理表面和自然场景等多样化输入的方法，而人类视觉能够稳健地检测和分割独立运动的物质块。

**Method:** 构建分层生成模型，将低级运动线索和高级外观特征聚合成表示局部物质的小高斯粒子，再将粒子组合成连贯独立运动的物理实体簇；采用基于并行块吉布斯采样的硬件加速推理算法。

**Result:** 在三个领域验证统一框架：2D随机点运动图上捕获人类物体感知；伪装旋转物体数据集上恢复3D结构和2D分割；自然RGB视频中跟踪变形物体的3D物质。

**Conclusion:** 建立了基于人类视觉原理的通用运动感知框架，实现了跨不同输入类型的稳健运动检测和物体级场景理解。
...
> SAMIDARE是一个针对密集体育场景的多目标跟踪框架，通过密度感知掩码重生成、选择性记忆更新和状态感知关联三个关键组件，显著提升了跟踪性能，在SportsMOT数据集上达到了SOTA水平。

<details>
  <summary>Details</summary>

**Motivation:** 自动化体育分析需要鲁棒的多目标跟踪，但基于分割的方法在密集场景中经常面临掩码错误和ID切换问题。现有方法难以处理密集体育场景中的相互遮挡和频繁进出画面的情况。

**Method:** SAMIDARE框架提出了三个关键组件：(1)密度感知的掩码重生成，根据场景密度自适应调整掩码；(2)选择性记忆更新，自适应掩码控制以保持目标特征完整性；(3)状态感知关联和新轨迹初始化，提高在相互遮挡和频繁帧间事件下的鲁棒性。

**Result:** 在SportsMOT数据集上评估，SAMIDARE实现了最先进的性能，在验证集上比基线方法提升了2.5 HOTA点和4.2 IDF1点，证明了其在密集体育跟踪中的有效性。

**Conclusion:** 通过掩码控制和状态感知关联的自适应特征管理，SAMIDARE为密集体育跟踪提供了鲁棒且高效的解决方案，为自动化体育分析领域贡献了重要的技术进步。
...
> 该研究基于拳击比赛视频构建了成对动作-反应运动数据集，比较了三种Transformer模型用于交互场景下的运动生成，发现简单Transformer效果最佳，且人员ID嵌入能有效提高运动一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有深度学习方法主要集中在单代理运动生成，但在交互场景中，两个人的运动是相互依赖的，缺乏专门针对这种交互式运动生成的研究。

**Method:** 构建拳击比赛视频的成对动作-反应运动数据集；实现并比较简单Transformer、iTransformer和Crossformer三种模型；引入人员ID嵌入来显式区分个体。

**Result:** 简单Transformer能够生成合理的交互感知运动，不会出现姿态崩溃；iTransformer和Crossformer会随时间累积误差导致运动不稳定；人员ID嵌入有效防止结构崩溃，提高运动一致性。

**Conclusion:** 在交互感知运动生成中，显式建模个体身份至关重要，简单Transformer结合人员ID嵌入能够有效生成稳定且一致的双人交互运动。
...
> 本文提出了一种基于频域差异建模的跨模态先验迁移框架MCPT，通过模态差异曲线(MDC)将光学先验知识有效迁移到SAR领域，显著提升了SAR图像的广义类别发现性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大型视觉模型主要基于光学图像训练，其固有的光学先验与SAR图像存在跨模态不兼容性问题，导致广义类别发现(GCD)在SAR领域效果受限。传统的域适应方法缺乏反映成像特性的归纳偏置，无法有效迁移光学先验。

**Method:** 提出MCPT框架：1) 引入模态差异曲线(MDC)将频域能量分布建模为结构化频域描述符；2) 使用自适应频率标记化(AFT)将MDC转换为可学习标记；3) 通过频率感知专家细化(FER)进行带间差异感知的特征细化；4) 基于对比学习对齐跨模态表示并内化适应模式。

**Result:** 在多个主流数据集上的广泛实验表明，该方法达到了业界领先的性能，证明了频域差异建模能够实现更有效的光学先验向SAR图像的适应。

**Conclusion:** 频域差异建模为解决光学与SAR跨模态差异提供了新的思路，MCPT框架成功地将光学先验知识迁移到SAR领域，显著提升了SAR-GCD任务的性能。
...
> UniME是一种两阶段异构方法，通过统一编码器预训练和模态特定多编码器融合，有效解决了脑肿瘤分割中模态缺失问题，在BraTS数据集上超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 多模态MRI提供互补信息对脑肿瘤分割很重要，但临床扫描经常缺少一个或多个模态，这会降低分割性能。现有方法在精细结构捕获、跨模态互补建模和可用模态利用之间存在平衡难题。

**Method:** 提出两阶段异构架构UniME：第一阶段使用掩码图像建模预训练单一ViT统一编码器，建立对模态缺失鲁棒的统一表示；第二阶段添加模态特定的CNN多编码器，提取高分辨率、多尺度、细粒度特征，将这些特征与全局表示融合生成精确分割。

**Result:** 在BraTS 2023和BraTS 2024上的实验表明，UniME在不完整多模态场景下优于之前的方法，有效解决了模态缺失导致的分割性能下降问题。

**Conclusion:** UniME通过解耦表示学习和分割的两阶段异构设计，成功平衡了精细结构捕获、跨模态互补建模和可用模态利用之间的权衡，为临床实际应用中模态缺失的脑肿瘤分割提供了有效解决方案。
...
> EvFlow-GS是一个结合事件流和光流的统一框架，通过端到端优化的双积分模型、相机位姿和3D高斯泼溅技术，实现了从运动模糊图像的高质量3D重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于事件相机的3D重建方法存在残余伪影和纹理模糊问题，主要原因是错误的事件双积分先验监督以及噪声事件的影响。

**Method:** 提出EvFlow-GS框架，首先利用光流从事件中提取边缘信息，然后针对不同模块设计新的事件损失函数，引入事件残差先验增强强度变化监督，最后将3DGS和LDI的输出通过联合损失实现相互促进的优化。

**Result:** 实验结果表明EvFlow-GS在3D重建任务中取得了领先性能，有效解决了传统方法的残余伪影和纹理模糊问题。

**Conclusion:** 通过事件流、光流和3D高斯泼溅技术的有效融合，EvFlow-GS为从运动模糊图像进行精确3D重建提供了新的解决方案，展示了多模态信息在3D重建中的巨大潜力。
...
> SAGA-ReID通过与CLIP文本嵌入空间中的锚向量对齐来重建身份表示，解决了基于CLIP的重识别方法在遮挡和跨摄像头变化下的脆弱性问题，在遮挡场景下Rank-1准确率提升高达10.6%。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于CLIP的重识别方法将空间特征聚合为单一全局[CLS]标记，这种机制针对图像-文本对齐优化而非空间选择性，导致在遮挡和跨摄像头变化下表示脆弱。

**Method:** 提出SAGA-ReID方法，通过将中间的patch标记与在CLIP文本嵌入空间中参数化的锚向量对齐来重建身份表示，强调空间稳定证据同时抑制损坏或缺失区域，且不需要单张图像的文本描述。

**Result:** 在合成遮挡和真实人体干扰器两种条件下进行受控实验，随着遮挡增加，SAGA相对于全局池化的优势显著增长。在标准和遮挡基准测试中持续优于CLIP-ReID，在全局池化最不可靠的情况下改进最大，遮挡基准测试上Rank-1准确率提升10.6%。

**Conclusion:** SAGA的聚合机制在更强的骨干网络上也优于专门的顺序patch聚合，证实结构化重建解决了骨干网络质量和架构复杂性本身无法解决的瓶颈问题。
...
> CharTide是一个数据驱动的图表到代码生成框架，通过三视角调优策略和基于信息不变性的查询驱动强化学习，显著提升了7B/8B模型在图表代码生成任务中的表现，超越了开源基线并达到GPT-5水平。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图表到代码生成方法受到数据中心化限制，简单扩展同质化的图表-代码对混淆了视觉感知和程序逻辑，阻碍了VLM模型充分利用多模态监督信号的丰富性。

**Method:** 1) 提出三视角调优策略，将训练显式解耦为视觉感知、纯文本代码逻辑和模态融合三个部分；2) 创新性地将对齐问题重构为数据验证问题，引入基于信息不变性原理的查询驱动RL框架，通过冻结的Inspector模型进行原子QA任务验证。

**Result:** CharTide-7B/8B在ChartMimic、Plot2Code和ChartX基准测试中显著优于开源基线模型，超越GPT-4o，并与GPT-5表现相当，仅使用监督数据就能7B模型超越专业基线。

**Conclusion:** CharTide通过系统性地重新设计训练和对齐数据，有效解决了图表代码生成中视觉感知与程序逻辑的混淆问题，为数据驱动的多模态模型优化提供了新的思路和方法。
...
> 本文提出了首个从单张真实世界RGB图像中检测3D反射对称性的框架，重点关注建筑地标，通过自动化标注管道创建了大规模数据集ArchSym，并开发了基于场景几何参数化的单视图对称性检测器。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于学习的3D对称性检测方法几乎都是在以物体为中心或合成数据集上训练和评估的，无法泛化到真实场景。此外，由于单目输入的尺度模糊性难以确定3D平面位置，许多方法仅预测平面方向。

**Method:** 1）开发可扩展的数据标注管道，通过利用跨视图图像匹配从SfM重建中自动整理大规模建筑对称性数据集ArchSym；2）基于该数据集构建单视图对称性检测器，通过将对称性参数化为相对于预测场景几何的符号距离图来准确定位3D空间中的对称性。

**Result:** 验证了对称性标注管道相对于基于几何的替代方案的优越性，并证明了对称性检测器在新基准上显著优于最先进的基线方法。

**Conclusion:** 该框架首次实现了从单张真实图像中检测3D grounding的反射对称性，通过自动化数据集构建和创新的检测方法解决了现有方法的泛化能力和定位精度问题。
...
> 本文提出FMDiffWA，一种基于频域调制扩散的数字水印攻击框架，能够在保持高视觉质量的同时有效移除AI生成图像中的隐形水印。

<details>
  <summary>Details</summary>

**Motivation:** 针对版权保护的数字水印技术快速发展，但水印攻击技术进展相对有限，导致攻击-防御失衡，阻碍了该领域的进一步发展。

**Method:** 提出频域调制扩散框架FMDiffWA，引入频域水印调制(FWM)模块并集成到正向和反向扩散过程的采样阶段，选择性地调制水印相关频谱成分；通过增加辅助精炼约束重新训练扩散模型以优化攻击效果和视觉保真度的权衡。

**Result:** 实验表明FMDiffWA相比现有水印攻击方法具有更优越的视觉保真度，同时在多种水印方案上展现良好的泛化能力。

**Conclusion:** FMDiffWA有效解决了水印攻击-防御 imbalance 的问题，为AI生成图像的版权保护领域提供了新的攻击视角，有助于推动水印技术的健康发展。
...
> 本文提出SportsTime基准测试和Chain-of-Time Reasoning(CoTR)方法，解决体育视频中长时程推理的稀疏时间证据定位与整合难题。

<details>
  <summary>Details</summary>

**Motivation:** 体育视频涉及复杂动态的人类活动，现有多模态大语言模型在长时程推理方面存在困难，主要因缺乏对时间分散证据的监督以及需要模型识别、定位和论证时间证据的方法。

**Method:** 构建包含14K+开放式问答对和50K+逐步时间证据标注的SportsTime基准测试；提出CoTR方法，将推理视为基于时间锚点的证据组合过程，训练时采用时间奖励GRPO，推理时使用锚点-观察-推断的证据寻求循环。

**Result:** 实验证明SportsTime作为基准测试的有效性，CoTR在时间组合推理和逐步定位质量上持续优于强MLLM基线。

**Conclusion:** SportsTime和CoTR为体育视频长时程理解提供了新的解决方案，有效提升了模型在时间证据定位、验证和组合方面的能力。
...
> OccDirector是世界首个能用自然语言驱动4D占用动态生成的框架，实现了从外观合成到行为编排的范式转变。

<details>
  <summary>Details</summary>

**Motivation:** 现有的4D占用生成框架依赖于刚性几何条件或简单的属性级文本，无法编排复杂的多智能体序列交互，存在语义-时空鸿沟。

**Method:** 提出OccDirector框架，使用VLM驱动的时空MMDiT架构和历史前缀锚定策略，将自然语言脚本映射为物理合理的体素动态，无需几何先验。

**Result:** 在OccInteract-85k数据集上的实验表明，OccDirector达到了最先进的生成质量和前所未有的指令跟随能力。

**Conclusion:** 成功将范式从外观合成转向语言驱动的行为编排，为真实自动驾驶仿真提供了新的解决方案。
...
> 本文提出了大规模开源视觉语言数据集LTD和交通基础模型UniVLT，用于城市场景下的开放式交通安全推理任务，在多视图交通场景中实现了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有智能交通系统研究主要聚焦于自动驾驶微观层面，缺乏城市尺度交通分析的开放式安全导向视觉问答研究，多视图复杂交通场景下的推理能力不足。

**Method:** 构建包含11.6K高质量VQA对的LTD数据集，结合多模型视觉语言生成、交叉验证和人工优化确保标注质量；提出通过课程化知识转移训练的UniVLT统一交通基础模型。

**Result:** UniVLT在LTD和多个自动驾驶基准测试中达到SOTA性能，统一了微观自动驾驶推理和宏观交通分析，揭示了现有基础模型在复杂多视图交通场景中的局限性。

**Conclusion:** LTD和UniVLT填补了城市尺度开放式交通安全推理的研究空白，为智能交通基础设施的发展提供了新的数据基础和模型架构，推动了交通基础模型的多域统一发展。
...
> 本文提出了一种基于反事实关系验证的开放词汇场景图生成框架，通过验证关系的视觉证据而非直接接受预测，提高了场景图的可靠性和可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的开放词汇场景图生成方法存在可靠性问题，预测的关系可能受语言先验或物体共现影响，而非基于真实视觉证据。

**Method:** 首先生成开放词汇关系候选，然后将谓语短语分解为支撑、接触、包含等软证据基。使用关系条件证据编码器提取相关线索，通过反事实验证器测试移除必要证据时关系分数的变化，并引入矛盾感知谓语学习和图级偏好优化。

**Result:** 在传统、开放词汇和全景场景图基准测试中，该方法持续改进了基于召回的指标、未见谓语泛化能力以及反事实基础质量。

**Conclusion:** 从关系生成转向关系验证的方法能够产生更可靠、可解释且基于证据的场景图，有效解决了开放词汇场景图生成的可靠性问题。
...
> RIME是一种基于重写的多模态嵌入框架，通过生成重写和判别式嵌入对齐，有效解决了CoT推理中的冗余和语义歧义问题，大幅提升了嵌入性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大语言模型(MLLMs)在嵌入任务中存在Chain-of-Thought推理生成冗余思考步骤和引入语义歧义的问题，需要更高效的嵌入方法。

**Method:** 提出了RIME框架，包含三个核心组件：1) 基于检索友好的重写机制优化生成和嵌入；2) 跨模态对齐(CMA)桥接生成和判别式嵌入空间；3) 精炼强化学习(Refine-RL)以判别式嵌入为语义锚点指导重写优化。

**Result:** 在MMEB-V2、MRMR和UVRB数据集上的实验表明，RIME显著优于先前的生成式嵌入模型，同时大幅减少了推理步骤长度。

**Conclusion:** 成功开发了统一的多模态嵌入框架，通过重写驱动的方法解决了CoT推理的局限性，实现了生成和判别式嵌入的有效融合，为多模态检索提供了新的解决方案。
...
> DocPrune是一种无需训练的文档token剪枝框架，通过移除不必要的token（如背景和与问题无关的token）来提高长文档视觉语言模型的效率，同时保持或提升准确性。

<details>
  <summary>Details</summary>

**Motivation:** 文档图像包含大量背景且支撑证据稀疏，导致现有视觉语言模型处理长文档时计算资源消耗严重，而针对自然图像的token剪枝方法无法有效利用文档独特的结构稀疏性。

**Method:** 提出DocPrune框架，采用渐进式token剪枝策略，仅保留任务相关的必要token，同时自动选择合适的层开始剪枝，基于模型的理解程度动态调整。

**Result:** 在M3DocRAG数据集上，DocPrune使编码器吞吐量提升3.0倍，解码器提升3.3倍，F1分数提高1.0，实现了准确率和效率的双重提升。

**Conclusion:** DocPrune成功解决了长文档处理中的计算效率问题，通过无需训练的结构化token剪枝方法，为视觉语言模型在文档理解任务中的实际应用提供了有效解决方案。
...
> 本研究评估了月球环境中不同图像模拟方法的效果，重点关注相机模型和光照条件对合成月球图像质量的影响。

<details>
  <summary>Details</summary>

**Motivation:** 合成图像生成是行星任务的关键输入，能够帮助研究人员和工程师在虚拟环境中可视化任务计划、测试成像系统、规划探测活动，对于评估着陆点、检测危险和验证导航系统至关重要。

**Method:** 使用来自Chandrayaan-2 OHRC和NASA WAC、NAC等仪器的真实数字高程模型(DEM)和地形数据，研究不同相机模型和光照条件对合成月球图像质量的影响。

**Result:** 该研究提高了合成图像的可靠性，为月球探测中的自主导航和决策系统提供支持，为未来月球任务开发了更有效的工具。

**Conclusion:** 这项工作不仅为合成月球图像生成提供了详细评估方法，还增强了对月球表面环境的理解，为月面探测技术的发展做出了重要贡献。
...
> 本研究提出了KVBench评测基准和KE-Check改进框架，系统评估并提升了文本到图像模型在知识密集型生成任务中的表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本到图像模型虽然在照片级合成和指令遵循方面表现出色，但在需要严格遵循领域知识、结构约束和符号约定的知识可视化场景中，其可靠性和科学准确性存在显著缺陷。

**Method:** 构建了涵盖高中六个学科、包含1800个专家精选提示词的KVBench评测基准；提出了KE-Check两阶段框架，通过知识阐述和清单指导细化来提升科学保真度。

**Result:** 评估了14个主流模型发现其在逻辑推理、符号精确性和多语言鲁棒性方面存在严重不足，开源模型持续落后于专有系统；KE-Check框架有效减少了科学幻觉现象。

**Conclusion:** 该研究填补了视觉合理性与科学正确性之间的关键空白，为知识密集型文本到图像生成提供了系统性的评测标准和改进方案，推动了模型在科学教育领域的应用发展。
...
> 本文提出了一种名为双收敛线(DCL)的新型关键点混淆方法，通过将关键点提升为从两个固定锚点出发的直线，有效抵御了几何恢复攻击，实现了保护隐私的视觉定位。

<details>
  <summary>Details</summary>

**Motivation:** 现有的隐私保护图像查询(PPIQ)方法，特别是基于几何混淆的方法，存在易受隐私攻击的漏洞。原有方法中混淆后相邻线的空间分布仍围绕原始关键点位置，为恢复攻击提供了可利用的线索。

**Method:** DCL方法在中心分割线上设置两个固定锚点，根据关键点位置选择活动的锚点，将每个关键点提升为从该锚点出发的直线。这种设计使得几何恢复攻击的优化问题变得不适定：相邻线要么误导性地收敛到一个锚点产生平凡解，要么在分割边界处接近平行产生不稳定的高方差解。

**Result:** 在室内和大规模室外数据集上的实验表明，DCL具有良好的抗隐私攻击能力、效率高、可扩展性强，同时实现了实用的定位性能。

**Conclusion:** DCL通过创新的几何混淆设计，从根本上解决了基于几何混淆方法的隐私漏洞，为云视觉定位提供了一种有效的隐私保护解决方案，并且兼容现有的基于线的求解器，便于在传统定位管道中部署。
...
> 本研究对比分析了月球车深度感知导航中双目视觉与单目深度估计方案，发现基于边缘AI的单目方案在实际应用中更具成本效益和鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 探索月球车在真实环境中的深度感知导航方案，解决从仿真到实际部署的技术转换问题，寻求更经济实用的视觉解决方案。

**Method:** 使用Unity模拟器结合OpenCV StereoSGBM生成双目视差图；在树莓派4平台上采用UniDepthV2进行单目深度估计，结合YOLO12n实现实时目标检测。

**Result:** 双目视觉在仿真中精度更高，但单目方案在实际部署中表现更优，深度估计达到0.1 FPS，目标检测达到10 FPS，具有良好的成本效益和鲁棒性。

**Conclusion:** 基于边缘AI的单目深度估计方案在月球车导航中是当前更适合实际应用的解决方案，平衡了性能、成本和稳定性。
...
> ChangeQuery是一个统一的多模态框架，通过结合光学和SAR数据，支持交互式灾害分析和多任务推理，为灾后态势感知提供综合性解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉-语言方法在灾后响应中存在严重局限：依赖单一光学数据模态、偏向自然灾害研究、缺乏基于现实的交互能力，无法为复杂的战略查询提供可操作的情报。

**Method:** 构建了DICQ数据集，耦合灾前光学语义与灾后SAR结构特征；提出自动化语义标注管道，将分割掩码转换为分层指令集；设计了支持多任务推理的ChangeQuery架构，实现交互式灾害分析。

**Result:** 实验证明ChangeQuery在灾害监测领域建立了新的SOTA，能够提供精确的损坏量化、区域特定描述和整体灾后总结，为复杂灾害监控提供了鲁棒且可解释的解决方案。

**Conclusion:** ChangeQuery通过多模态融合和交互式推理，有效克服了现有方法的局限性，为综合性的全天候灾害态势感知提供了实用且强大的技术支撑。
...
> 本研究提出了FILTR框架，首次实现了从3D点云编码器的特征中直接提取拓扑信息（持久图），并通过设计的DONUT基准测试发现现有编码器保留的拓扑信号有限。

<details>
  <summary>Details</summary>

**Motivation:** 探究现有的3D点云预训练编码器是否能够捕捉形状的拓扑结构信息，因为拓扑描述符在理解形状的多尺度结构方面具有重要价值，但目前缺乏系统性的评估方法。

**Method:** 1) 设计DONUT合成基准数据集来控制拓扑复杂度；2) 提出FILTR框架，使用Transformer解码器将持久图预测转化为集合预测任务，直接从冻结的编码器特征中学习提取拓扑信息。

**Result:** 分析发现现有3D编码器仅保留有限的全局拓扑信号，但FILTR框架能够有效利用这些编码器的信息来近似持久图，实现了从原始点云数据驱动提取拓扑信息的突破。

**Conclusion:** 本研究首次实现了通过高效的前馈学习机制从原始点云中提取持久图，为理解预训练模型的拓扑感知能力提供了新视角和方法工具。
...
> 本文提出了一种基于光流引导的动态三维高斯溅射SLAM框架，通过运动模型分解、关键帧时间中心建模和高斯混合模型，实现了对动态环境的高效重建与实时跟踪。

<details>
  <summary>Details</summary>

**Motivation:** 视觉SLAM在动态环境中面临巨大挑战，现有的3DGS与SLAM结合方法在高效重建静态和动态区域方面仍存在不足。

**Method:** 1) 利用深度和先验光流拟合相机自运动模型，生成类别无关的运动掩码；2) 在关键帧显式建模动态高斯的时间中心，使用3D场景流先验传播；3) 采用高斯混合模型自适应学习时间不透明度和旋转。

**Result:** 实验结果表明该方法在跟踪精度、动态重建质量和训练效率方面均达到了最先进水平。

**Conclusion:** 该框架有效解决了动态3DGS SLAM中的关键问题，为动态环境下的实时视觉SLAM提供了新的解决方案。
...
> PoseFM是首个将单目视觉里程计重构为生成任务的方法，通过流匹配技术建模相机运动分布，在多个基准测试中展现出竞争性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的深度学习方法缺乏不确定性感知能力，而在自动驾驶等实际应用中，对不确定性的准确估计对系统鲁棒性至关重要，特别是在视觉条件恶劣的环境下。

**Method:** 提出基于流匹配的PoseFM框架，将相机姿态建模为分布而非点估计，通过连续时间ODE从噪声中生成真实的姿态预测，从而实现 principled 的不确定性估计机制。

**Result:** 在TartanAir、KITTI和TUM-RGBD基准测试上表现优异，在某些轨迹上实现了最低的绝对轨迹误差(ATE)，整体性能与当前最佳的单目帧到帧VO方法具有竞争力。

**Conclusion:** 将生成建模引入单目视觉里程计领域，为运动估计 uncertainty 提供了新的解决思路，在保持高精度的同时增强了系统在挑战性视觉条件下的鲁棒性。
...
> 我们提出了一种名为OSFENet的one-shot学习方法，通过学习目标点云的特定数据分布，在点云边缘提取任务上取得了优于通用训练网络的结果。

<details>
  <summary>Details</summary>

**Motivation:** 不同扫描仪具有独特特性和误差分布，在包含多扫描仪数据的数据集上训练网络效果不如在单一扫描仪特定数据上训练。

**Method:** 设计了基于filtered-KNN的表面块表示法来支持one-shot学习框架，训练轻量级网络OSFENet，并引入RBF_DoS模块（基于径向基函数的表面描述符）来增强点云边缘提取。

**Result:** 在ABC数据集上与7个基线方法进行对比验证，证明OSFENet的优势；同时在S3DIS（室内）、Semantic3D和UrbanBIS（室外）等多个真实扫描数据集上验证了实用性。

**Conclusion:** OSFENet通过one-shot学习和特定数据分布适应，成功解决了多扫描仪数据训练效果不佳的问题，为点云边缘提取提供了有效解决方案。
...
> 本文提出了一种新的嵌入损失函数(EL)，用于加速扩散模型蒸馏训练并提升生成质量，在减少80%训练迭代的同时，在CIFAR-10上实现了SOTA的FID性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的扩散模型蒸馏方法需要大量计算资源和长时间训练，且补充损失函数存在局限：回归损失需要预生成大数据集并限制学生模型性能，基于GAN的损失训练不稳定。

**Method:** 提出嵌入损失(EL)，利用随机初始化网络的特征嵌入，通过计算嵌入特征空间中的最大均值差异(MMD)，对齐蒸馏生成器与原始数据的特征分布。

**Result:** 在CIFAR-10上，无条件生成FID达1.475，条件生成FID达1.380；在ImageNet、AFHQ-v2、FFHQ等多基准数据集上均显示一致改进，训练迭代减少高达80%。

**Conclusion:** EL为资源受限环境下的扩散模型部署提供了更实用和可扩展的解决方案，在保持样本保真度和多样性的同时显著提升了训练效率和生成质量。
...
> 本文提出HFS-TriNet模型，通过启发式帧选择和三分支协作特征学习网络，提高经直肠超声视频前列腺癌分类的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 经直肠超声(TRUS)视频相比静态图像包含更丰富的时空信息，但面临信息冗余、类内类间相似性高、信噪比低等挑战，限制了CAD系统的性能。

**Method:** 提出启发式帧选择(HFS)策略动态初始化训练片段起始点，设计三分支网络：常规ResNet50分支、基于医疗SAM的大模型分支和结合小波变换的WTCR分支分别提取深度特征和边缘信息。

**Result:** 该框架有效缓解了视频冗余问题，通过多分支协作提升了特征提取能力，改善了前列腺癌分类的诊断准确性和鲁棒性。

**Conclusion:** HFS-TriNet为TRUS视频分析提供了一种有效解决方案，通过智能帧选择和多样化特征学习改善了前列腺癌计算机辅助诊断系统的性能。
...
> FoL++是一种结合了鲁棒性区域建模和自适应重排序的视觉位置识别方法，通过可靠性估计分支、空间对齐损失、伪对应策略和自适应候选调度器，有效解决了感知混淆和重排序效率问题，在性能和速度上都超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉位置识别方法存在两个主要问题：一是由于无关区域导致的感知混淆现象影响定位精度；二是传统的刚性候选调度机制导致重排序效率低下。这些问题限制了现有方法在实际应用中的性能表现。

**Method:** FoL++方法包含四个核心组件：1) 可靠性估计分支生成空间可靠性映射来显式建模抗遮挡能力；2) 提出两个空间对齐损失(SAL和SCEL)优化特征对齐并突出显著区域；3) 伪对应策略在没有人工标注的情况下直接从聚合聚类生成密集的局部特征监督；4) 自适应候选调度器根据全局相似性动态调整候选池大小。最终通过可靠性加权的局部匹配和自适应融合全局与局部证据进行定位。

**Result:** 在七个基准测试数据集上的大量实验表明，FoL++实现了最先进的性能，同时保持了轻量级的内存占用。相比FoL方法，FoL++在推理速度上提升了40%，证明了其在准确性和效率方面的显著优势。

**Conclusion:** FoL++通过创新性地结合鲁棒的判别性区域建模与自适应重排序机制，成功克服了传统视觉位置识别方法的关键限制。该方法不仅在各类基准测试中达到了当前最佳性能，还显著提升了计算效率，为实际部署提供了更具吸引力的解决方案。
...
> SpaMEM是一个用于评估多模态大模型在具身环境中长期空间推理能力的大规模诊断基准，揭示了模型在视觉记忆维护方面存在的根本性缺陷。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大语言模型在静态视觉-空间推理方面取得了进展，但在具身环境中，当需要根据环境变化从以自我为中心的观察中持续修正空间信念时，往往无法保持长期的空间连贯性。

**Method:** 构建了SpaMEM基准，包含10,601,392张高保真图像，覆盖RGB、深度、实例和语义分割四种模态，来自1000个程序生成房屋中的25,000+个交互序列。将具身空间推理形式化为三级层次结构（15项诊断任务）：1级测量单次观察的原子空间感知；2级使用预测性文本状态历史测试时间推理；3级要求从原始视觉流进行端到端信念维护。

**Result:** 基准测试代表性开源VLM模型揭示了一致的堆叠瓶颈：坐标一致性接地仍然是硬性上限，从2级到3级的急剧崩溃暴露了显著的符号脚手架依赖性，即模型在基于文本的记录任务上成功，但难以维持稳健的视觉记忆。

**Conclusion:** SpaMEM提供了细粒度的诊断标准，并为显式的状态表示、信念修正和长期情景整合机制的开发提供了动力，有助于推动具身空间推理研究的发展。
...
> 提出一种神经正则化方法，通过方差感知条件MLP直接在3D高斯上操作，纠正多视角不一致2D特征提升到3D空间时产生的语义噪声，实现精确鲁棒的3D语义高斯泼溅。

<details>
  <summary>Details</summary>

**Motivation:** 视觉基础模型提取的2D特征由于缺乏跨视角约束而存在多视角不一致性，直接提升到3D高斯会产生噪声语义场，影响下游任务性能。现有方法要么专注于预处理阶段获得一致特征，要么通过改进优化策略缓解噪声，但都会增加预处理时间或计算开销。

**Method:** 引入方差感知条件MLP，直接在3D高斯上操作，利用其几何和外观属性来纠正3D空间中的语义错误，对噪声3D语义场进行正则化 refining。

**Result:** 在不同数据集上的实验表明，该方法能够有效增强提升语义的准确性，为实现鲁棒的3D语义高斯泼溅提供了一种高效有效的解决方案。

**Conclusion:** 相比现有方法，本文提出的神经正则化方法无需复杂的预处理或昂贵的计算开销，直接在3D空间中纠正语义噪声，为精确鲁棒的3D语义高斯泼溅提供了实用的技术路径。
...
> SnapLog是一种从视频中提取事件数据的方法，通过图像嵌入和时序分割技术，生成可用于流程挖掘的带标签事件日志。

<details>
  <summary>Details</summary>

**Motivation:** 在业务流程管理和流程挖掘领域，数据多模态性是一个主要障碍，特别是视频数据无法直接解释为事件，这阻碍了流程分析的进行。

**Method:** SnapLog方法首先使用图像嵌入将视频帧转换为特征向量，然后通过帧间相似度矩阵进行时序分割，最后采用广义少样本分类为视频片段分配标签，生成带时间戳的帧子序列。

**Result:** 该方法能够生成准确反映视频中流程的日志，使传统流程挖掘技术可以分析这些从视频转换而来的事件数据。

**Conclusion:** SnapLog成功解决了视频数据到事件数据的转换问题，扩展了流程挖掘技术的应用范围，为多模态环境下的流程分析提供了新的解决方案。
...
> 本文提出了一种基于对比图像的神经元标注方法，通过使用对比示例显著提升了标注的保真度和语义粒度。

<details>
  <summary>Details</summary>

**Motivation:** 现有神经元标注方法通常依赖高激活示例，容易产生过于宽泛或误导性的标签，因为它们往往关注主导但偶然的视觉因素。

**Method:** 采用两阶段方法：(1)向视觉语言模型提供对比图像集生成候选标签；(2)引入对比语义投影(CSP)，将对比示例集成到基于CLIP的评分和选择流程中。

**Result:** 实验和黑色素瘤检测案例研究表明，对比标注比现有基线方法更好地提升了保真度和语义粒度。

**Conclusion:** 对比示例是神经元标注和分析流程中简单而强大且当前未被充分利用的组件，能够产生更准确、更具体的神经元标签。
...
> 本文提出了一种个性化的驾驶员嗜睡检测系统，通过校准个体特定的EAR和MAR阈值并结合CNN深度学习模型，在多种驾驶场景下实现了高精度的实时疲劳监测。

<details>
  <summary>Details</summary>

**Motivation:** 传统的基于视觉的驾驶员监控系统使用固定的EAR和MAR阈值，但由于面部结构差异、光照条件和驾驶环境的变化，这些固定值往往无法在不同个体间实现良好的泛化效果，导致检测准确性不足。

**Method:** 系统结合了改进的经典指标和深度学习方法：1)在驾驶前校准驾驶员特定的EAR和MAR阈值；2)整合CNN模型用于眼部状态和哈欠检测；3)实时监测眼睑运动、头部位置和打哈欠行为，检测到疲劳迹象时发出警告。在公开数据集和包含多样化光照条件、头部姿态及用户特征的自定义数据集上进行评估。

**Result:** 个性化阈值相比固定阈值将检测准确率提升2-3%，CNN分类器在眼部状态检测和哈欠检测上分别达到99.1%和98.8%的准确率，证明了结合经典指标与深度学习方法的有效性。

**Conclusion:** 将个性化阈值校准与深度学习相结合的方法能够显著提升驾驶员嗜睡检测的鲁棒性和准确性，为实时驾驶监控系统提供了一种可靠的解决方案。
...
> 本文提出了Holo360D，一个包含109,495个全景图像和对应3D数据的大规模数据集，为全景3D重建提供了连续轨迹和高质量深度图，并建立了新的评估基准。

<details>
  <summary>Details</summary>

**Motivation:** 现有的前馈3D重建模型在全景图像上性能下降，主要受球面畸变影响；同时，现有全景3D数据集相机位置离散、轨迹不连续，严重阻碍了全景3D重建特别是多视角设置的发展。

**Method:** 使用3D激光扫描器结合360相机采集原始数据，通过在线和离线SLAM系统处理，提出专门针对360数据集的后处理流程，包括几何去噪、网格孔洞填充和区域特定重网格化。

**Result:** Holo360D数据集提供了连续的全景序列和精确对齐的高完整性深度图，在Holo360D上微调的3D重建模型显示出优异性能，为全景3D重建模型发展提供了有效基准。

**Conclusion:** Holo360D是首个提供连续全景序列和高质量3D数据的大规模数据集，为全景3D重建研究提供了重要资源，推动了该领域的发展。数据集和代码将公开发布。
...
> CGC是一个低成本框架，通过对比学习和空间奖励机制提升多模态大模型的细粒度多图像理解能力，在多个基准测试中达到SOTA效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大模型在细粒度多图像理解方面存在空间幻觉、注意力泄漏和对象恒常性失败等问题，且传统方法依赖昂贵的人工标注或大规模思维链数据生成。

**Method:** CGC基于单图像标注构建组合多图像训练实例，通过跨图像对比引入语义解耦的干扰上下文，通过图像内对比提供相关跨视图样本，并在GRPO框架内引入基于规则的空间奖励机制。

**Result:** 在MIG-Bench和VLM2-Bench等细粒度多图像基准测试中达到SOTA，同时在MathVista、MuirBench、MMStar、MMMU和BLINK等多个任务中实现一致的性能提升。

**Conclusion:** CGC证明了利用现有单图像标注和对比学习方法可以有效提升多模态模型的多图像理解能力，具有良好的泛化性能和实用价值。
...
> ICPR 2026举办了首个低分辨率车牌识别竞赛，使用真实场景数据集，99个团队参赛，冠军团队识别率达82.13%。

<details>
  <summary>Details</summary>

**Motivation:** 解决现实监控场景中低分辨率车牌识别的挑战性问题，推动该领域技术发展。由于拍摄距离远、压缩伪影和恶劣成像条件等因素，车牌可读性严重降低。

**Method:** 组织基于LRLPR-26数据集的竞赛，包含2万训练轨迹和3千测试轨迹，每个轨迹包含5张低分辨率和5张高分辨率车牌图像。采用盲测试阶段评估方法。

**Result:** 269个团队注册，99个团队提交有效结果。冠军团队识别率82.13%，4个团队超过80%，展现了高水平的竞争效果，同时表明该任务仍具有挑战性。

**Conclusion:** 竞赛成功推动了低分辨率车牌识别研究，总结了top5团队的方法，为未来LRLPR研究提供了有价值的经验和方向。
...
> RAIL-BENCH是首个铁路感知基准套件，包含轨道检测、目标检测、植被分割、多目标跟踪和单目视觉里程计五个挑战，并提出了LineAP新指标评估几何精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有铁路基础设施的自动化列车操作需要鲁棒的相机感知系统，但铁路领域缺乏具有标准化评估协议的公开基准套件，无法实现方法间的可重现比较。

**Method:** 构建RAIL-BENCH基准套件，包含五个针对铁路环境的特定挑战；提供精选的训练和测试数据集、评估指标和公共排行榜；针对轨道检测引入基于段的平均精度指标LineAP。

**Result:** 建立了完整的铁路感知评估平台，解决了现有线路检测指标的关键局限性，为铁路感知算法提供了标准化比较框架。

**Conclusion:** RAIL-BENCH填补了铁路感知领域基准套件的空白，通过标准化评估和公开平台促进了该领域的研究发展和算法比较。
...
> 本文基于Muharaf阿拉伯语手稿数据集，通过扩充标签和提出带注意力机制的CNN模型，首次实现了行级和页面级两种评估协议下的作者识别基准测试，为历史文档分析提供了重要资源。

<details>
  <summary>Details</summary>

**Motivation:** 阿拉伯语手写稿保存了阿拉伯世界的知识和文化遗产，作者识别对于文档来源追溯、真实性验证和历史分析至关重要。然而现有数据集标注不完整，且缺乏标准评估协议。

**Method:** 1) 手动验证并扩充Muharaf数据集的作者标签，从6,858条扩充到21,249条；2) 提出基于CNN的注意力机制模型，将罕见双作者行建模为复合作者对类别；3) 设计行级和页面级两种评估协议，其中页面级将同一页面所有行分配到同一划分；4) 进行14种配置的对比实验和消融研究。

**Result:** 行级协议下，微调DenseNet201+注意力模型达到99.05% Top-1准确率、99.73% Top-5准确率和97.44% F1分数；更具挑战性的页面级协议下，最好结果为78.61% Top-1准确率、87.79% Top-5准确率和66.55% F1分数，量化了页面级线索的影响。

**Conclusion:** 通过扩展Muharaf数据集标注并建立两种评估协议，本文为历史学家和语言学家处理重要文化历史文档提供了更清晰的基准和实用资源，推动了阿拉伯语手稿作者识别研究的发展。
...
> NONSAC是一个通过非最小子采样和共识机制的鲁棒模型估计框架，能够高效处理大规模含噪数据集，并可与RANSAC等现有算法结合提升性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的鲁棒估计方法（如RANSAC）在处理大规模数据集时面临计算效率和鲁棒性挑战，需要一种更通用、可扩展的框架来处理噪声和异常值。

**Method:** NONSAC通过重复采样非最小数据子集，使用鲁棒估计器生成多个候选模型，并基于预定义的评分规则选择最优模型。该框架与具体估计器无关，可与现有几何拟合算法集成。

**Result:** 在相机位姿估计、PnP和点云配准等任务上验证了NONSAC的有效性，并展示了其在无对应点云配准中的应用潜力。

**Conclusion:** NONSAC为大规模鲁棒模型估计提供了一个灵活且高效的解决方案，能够同时提升算法的可扩展性和对异常值的鲁棒性。
...
> 本研究提出了一种非对称知识蒸馏框架，利用预训练视觉模型学习对图像畸变鲁棒的特征表示，学生模型仅通过畸变图像学习就能接近处理干净图像教师模型的表示能力。

<details>
  <summary>Details</summary>

**Motivation:** 当干净观测数据稀疏或不可用时，自监督学习在视觉表示学习方面面临挑战。实际应用中常遇到各种图像畸变，影响了模型性能，因此需要学习对畸变鲁棒的表示方法。

**Method:** 提出非对称知识蒸馏框架，教师和学生模型均初始化自同一预训练视觉Transformer，但处理不同视图：教师处理干净图像，学生处理畸变图像。引入多层蒸馏策略，对齐全局嵌入、块级特征和注意力图。

**Result:** 在多个数据集和各种畸变条件下的图像分类任务评估表明，该方法在相同人类监督量下持续优于现有替代方案，学生模型能够在未直接接触干净数据的情况下近似干净图像表示。

**Conclusion:** 通过利用预训练模型和多层知识蒸馏，可以有效地从畸变数据中学习鲁棒的视觉表示，为处理现实世界中存在数据质量问题的场景提供了有效解决方案。
...
> 本研究通过分析1990-2020年间中英文学术期刊中的23,928张主题地图，发现学术地图设计具有高度相似的结构惯例，且发展呈现趋同而非分化的特征。

<details>
  <summary>Details</summary>

**Motivation:** 主题地图在学术传播中占据核心地位，但对其大规模设计演化趋势缺乏实证研究。本研究旨在填补这一空白，通过多语言视角分析学术制图实践的发展规律。

**Method:** 研究人员构建了来自16种权威中英文期刊的45,732篇研究文章语料库，使用计算机视觉和基于大模型的文档解析技术提取23,928张地图。从地图要素、色彩设计和布局结构三个维度量化地图设计特征。

**Result:** 中英文学术地图共享高度相似的结构惯例，均采用克制的中性主导色调、低饱和度、高亮度和有限色调多样性，以及中心化布局和较高主图占比。差异在于英文地图色调丰富度和紧凑度略高，中文地图传统上更依赖中性色调和整合布局。时序分析显示两组呈现并行演化趋势，包括要素丰富性、图例使用和色调多样性增加，布局结构保持稳定。

**Conclusion:** 研究结果表明，学术地图设计演化更多表现为制度趋同而非文化分化的特征，为理解学术制图实践的发展模式提供了重要见解。
...
> ReLIC-SGG是一个开放词汇场景图生成框架，通过构建语义关系格来建模谓词间的关系，并将未标注关系视为潜在变量而非绝对负例，有效解决了标注不完整问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有场景图生成方法将所有未标注的对象对关系视为负例，但场景图标注本身就不完整 - 许多有效关系缺失，且同一交互可以用不同粒度描述，这在开放词汇场景图中更加严重。

**Method:** 构建语义关系格来建模开放词汇谓词间的相似性、蕴含和矛盾关系；利用视觉-语言兼容性、图上下文和语义一致性推断缺失的正向关系；采用正例-未标注图学习目标减少假负监督；通过格引导解码生成紧凑且语义一致的场景图。

**Result:** 在传统、开放词汇和全景场景图基准测试上，ReLIC-SGG改善了稀有和未见谓词的识别，并能更好地恢复缺失关系。

**Conclusion:** 通过关系不完整性感知和语义关系格建模，ReLIC-SGG有效解决了开放词汇场景图生成中的标注不完整问题，在多个基准测试上取得了显著改进。
...
> TriPatch是一种新型行人对抗补丁生成方法，通过多阶段协同攻击和物理鲁棒性增强，在多种检测器上实现了更高的攻击成功率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的物理对抗补丁攻击在实际应用中存在两个主要限制：缺乏对多阶段决策流程的系统性干扰，无法有效破坏检测器的完整流程；以及对复杂物理变化的建模不足，导致鲁棒性差。

**Method:** 提出TriPatch方法，结合多阶段协同攻击与物理多样性下的鲁棒性增强。设计包含检测置信度抑制、边界框偏移放大和非极大值抑制(NMS)破坏的三元组损失，在检测流程的不同阶段联合作用。引入外观一致性损失约束补丁颜色分布，并采用数据增强进一步提升对复杂物理扰动的鲁棒性。

**Result:** 实验证明，与现有方法相比，TriPatch在多个检测器模型上实现了更高的攻击成功率。

**Conclusion:** TriPatch通过多阶段协同攻击和物理鲁棒性增强策略，有效提升了对抗补丁在真实环境中的攻击效果，为物理对抗攻击研究提供了新的思路。
...
> 提出语义进度函数分析生成内容的时间演化规律，通过语义线性化技术实现语义变化的均匀分布，提升生成质量和连贯性。

<details>
  <summary>Details</summary>

**Motivation:** 图像和视频生成模型产生的变换呈现高度非线性特征，长时间缓慢变化后突然出现语义跳跃，这种不均匀的语义节奏影响了生成内容的连贯性和质量。

**Method:** 引入语义进度函数作为一维表示，通过计算语义嵌入距离并拟合平滑曲线来捕捉序列的语义演化；基于此提出语义线性化过程，重新参数化序列使语义变化速率恒定。

**Result:** 语义线性化能够产生更平滑、更连贯的转换效果；该框架提供了模型无关的基础方法，可用于识别时间不规则性、比较不同生成器的语义节奏，并引导视频序列达到目标节奏。

**Conclusion:** 语义进度函数为生成内容的语义演化提供了量化分析工具，语义线性化技术有效解决了非线性语义变化问题，为视频生成和分析提供了新的方法论基础。
...
> 本文比较了两种跨阶段上下文传递机制在自动驾驶图视觉问答中的应用：显式变体通过提示策略实现无训练基线，隐式变体通过门控投影器实现语义提升。

<details>
  <summary>Details</summary>

**Motivation:** 自动驾驶图视觉问答需要保持感知、预测、规划三个阶段的一致性，但模型在不同阶段的决策可能存在矛盾，需要有效的跨阶段信息传递机制来确保规划决策与感知结果的连贯性。

**Method:** 采用互补的两种机制：显式变体基于提示策略在领域适配的4B VLM上实现无训练；隐式变体引入门控投影器，提取隐藏状态向量并注入下一阶段输入，在通用8B VLM上联合训练仅更新0.5%参数。

**Result:** 显式变体减少NLI矛盾达42.6%，建立强零训练基线；隐式变体显著减少规划阶段矛盾34%，跨阶段蕴含增加50%，规划语言质量提升30.3%，但词汇重叠和结构一致性因缺少驾驶领域预训练而下降。

**Conclusion:** 两种变体作为互补案例研究：显式上下文传递提供表观一致性的训练免费基线，隐式门控投影实现显著的语义提升，表明领域适配是实现全面改进的合理下一步。
...
> FlowAnchor是一个免训练的基于流的视频编辑框架，通过空间感知注意细化信号定位和自适应幅度调制信号强度，解决了高维视频空间中的编辑信号不稳定问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的免反演编辑方法在图像编辑中表现优异，但扩展到视频时面临挑战，特别是在多物体场景和帧数增加的情况下，主要原因是高维视频潜在空间中编辑信号的不稳定性，包括空间定位不精确和长度引起的幅度衰减。

**Method:** FlowAnchor框架包含两个关键机制：1) 空间感知注意细化(Spatial-aware Attention Refinement)，强制文本引导与空间区域的一致对齐；2) 自适应幅度调制(Adaptive Magnitude Modulation)，自适应保持足够的编辑强度。这两个机制共同稳定编辑信号，引导基于流的演化朝向目标分布。

**Result:** 大量实验表明，FlowAnchor在具有挑战性的多物体和快速运动场景中，实现了更忠实、时间连贯且计算高效的视频编辑效果。

**Conclusion:** FlowAnchor通过精确定位编辑位置和自适应调节编辑强度，成功解决了视频编辑中的信号不稳定性问题，为免反演的基于流视频编辑提供了一个稳定高效的解决方案。
...
> EV-CLIP是一种高效的视频动作识别框架，通过引入mask提示和context提示来增强CLIP的空间感知和轻量级时序建模能力，在多场景少样本任务中表现优异且效率与主干网络规模无关。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于CLIP的视频动作识别方法主要关注时序建模而忽视空间感知，但在真实场景中，低光照、第一人称视角等视觉挑战会严重影响空间理解，进而阻碍有效的时序推理。

**Method:** 提出EV-CLIP框架，包含两种视觉提示：1) mask提示通过重新加权像素引导模型关注动作相关区域；2) context提示通过压缩帧级特征实现轻量级时序建模。同时构建了五个基准数据集评估域偏移影响。

**Result:** 实验表明EV-CLIP在整体性能上超越现有参数高效方法，且效率与主干网络规模无关，适合在资源受限的真实场景中部署。

**Conclusion:** 该方法有效解决了CLIP适应视频动作识别时空间感知不足的问题，通过高效的双提示设计实现了跨场景跨视角的强泛化能力，为实际应用提供了可行的解决方案。
...
> 本文提出了一种基于3D点云数据的非侵入式视觉识别系统TARA，用于精确识别群体饲养环境中的农场动物，实现了100%的访视级别识别准确率。

<details>
  <summary>Details</summary>

**Motivation:** 传统的RFID耳标签识系统存在侵入性、易丢失和天线场空间限制等问题，需要开发一种非侵入、更可靠的个体动物识别方案来支撑精准畜牧管理。

**Method:** 设计Temporal Adaptive Recognition Architecture (TARA)半监督框架，利用动态校准机制更新个体身份档案以适应牲畜形态变化，并采用访视级别多数投票策略生成高保真伪标签解决标签稀缺问题。

**Result:** 在实际商业猪舍收集的群居母猪数据集上实现了100%的访视级别识别准确率，证明了视觉3D点云分析的优越性。

**Conclusion:** 基于视觉的3D点云分析为RFID系统提供了稳健的替代方案，为实现完全自主的个体动物监测铺平了道路，在精准畜牧业具有重要应用前景。
...
> PASR是一种通过姿势感知的3D形状检索框架，将检索转化为特征级分析-合成问题，在干净和遮挡数据集上均显著优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的单视图3D形状检索方法主要使用对比学习或学习共同的嵌入空间，但这些前馈的整体对齐方法难以解释，限制了其在真实应用中的鲁棒性和泛化能力。

**Method:** 提出姿势感知3D形状检索(PASR)框架，将2D基础模型(DINOv3)的知识提炼到3D编码器中，通过姿势条件的3D投影与2D特征图对齐。推理时通过分析-合成进行测试时优化，联合搜索最佳重构输入图像块级特征图的形状和姿势。

**Result:** PASR在干净和遮挡的3D形状检索数据集上均大幅超越现有方法，同时展现出强大的多任务能力，包括鲁棒形状检索、竞争性姿态估计和准确类别分类。

**Conclusion:** PASR通过分析-合成优化方法解决了现有检索方法的解释性问题，实现了对部分遮挡的鲁棒性和对细粒度几何细节的敏感性，为单视图3D形状检索提供了更有效的解决方案。
...
> SS3D是一个基于SfM的网页级自监督预训练管道，用于从单目视频中进行前馈3D估计，在YouTube-8M数据集上预训练后展现出强大的跨域零样本迁移能力。

<details>
  <summary>Details</summary>

**Motivation:** 将SfM自监督学习扩展到不受约束的网页视频是一个挑战，主要由于弱多视图可观察性和语料库的强异质性。现有方法在处理大规模多样化视频数据时存在局限性。

**Method:** 提出SS3D管道，采用单次前向传播联合预测深度、自运动和内参。使用内参优先的两阶段调度和统一单检查点评估协议来稳定联合学习。通过多视图信号代理(MVS)进行过滤和课程采样，并将专家训练蒸馏到单个学生模型中。

**Result:** 在过滤后的YouTube-8M数据集(~1亿帧)上预训练后，模型相比现有自监督基线展现出更强的跨域零样本迁移性能和改进的微调性能。

**Conclusion:** SS3D成功解决了网页级视频SfM自监督学习的关键挑战，证明了大规模多模态预训练在3D估计任务上的有效性，为单目视频3D理解提供了新的解决方案。
...
> 本文提出了一种基于4D扩散的生成框架，能够根据临床变量生成纵向脑部解剖序列，在稀疏时间数据情况下有效建模神经退行性疾病的进展。

<details>
  <summary>Details</summary>

**Motivation:** 现有纵向神经影像数据时间点稀疏，限制了准确建模个体连续解剖变化的能力，这对神经退行性疾病的早期诊断和监测带来了挑战。

**Method:** 开发了一种4D(3DxT)扩散生成框架，通过学习拓扑保持的时空变形数据分布，结合健康状态、年龄、性别等临床变量作为条件，生成纵向脑部解剖序列。

**Result:** 在两个大型纵向神经影像数据集上的实验表明，该方法在生成解剖准确、时间一致且具有临床意义的脑部轨迹方面优于现有基线方法。

**Conclusion:** 该框架能够生成合理的未来解剖状态和重建解剖一致的疾病轨迹，为纵向脑部变化提供了更逼真的表示，在疾病分类和脑部分割等下游任务中表现出色。
...
> 针对互联网照片长尾分布问题，提出MegaDepth-X数据集，通过采样策略模拟稀疏场景相机分布，显著提升了3D基础模型在极端稀疏条件下的重建能力。

<details>
  <summary>Details</summary>

**Motivation:** 互联网照片呈现长尾分布：少数著名地标容易3D重建，而大多数真实场景因图像稀疏、噪声大而难以重建，这是3D基础模型需要解决的重要挑战。

**Method:** 提出MegaDepth-X数据集，包含干净密集的3D重建深度图，并设计了模拟长尾场景相机分布的训练图像采样策略。

**Result:** 微调后的3D基础模型在极端稀疏条件下实现鲁棒重建，同时在对称和重复场景中重建更可靠，且保持对标准密集3D基准数据集的泛化能力。

**Conclusion:** 通过合成稀疏训练数据和智能采样策略，可以有效解决3D重建的长尾问题，为3D基础模型在稀疏场景下的应用提供了新思路。
...
> 研究团队创建了一个包含45对人(90人)的多模态互动数据集，涵盖视频、语音、生理信号等多维度数据，用于研究社交互动中的行为和情感。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究缺乏公开的多模态社交互动数据集，特别是包含多个参与者的同步记录和自我报告测量，这限制了社交行为建模研究的发展。

**Method:** 建立了包含熟人组和陌生组的双人互动数据库，收集同步的多模态数据(2D面部视频、3D面部几何、热成像、语音、生理信号、情感自我报告)，并对社交信号、同意/反对/中立等进行标注。

**Result:** 生成了20TB的多模态互动数据集，通过广泛的实验验证了有无人际历史的双人沟通模式和情感差异的分析效果。

**Conclusion:** 该数据集填补了多模态社交互动研究的空白，为前所未有的社交行为建模研究提供了重要资源，将推动该领域的创新发展。
