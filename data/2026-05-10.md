...
> AdaGATE是一个训练免费的证据控制器，通过显式修复缺失的桥接事实来提高多跳检索增强生成在噪声和冗余检索环境下的鲁棒性，在证据F1上达到62.3%，同时减少2.6倍的输入token。

<details>
  <summary>Details</summary>

**Motivation:** 现有的检索增强生成(RAG)系统在多跳问答中表现脆弱，特别是在检索到的证据含有噪声或冗余，且只能传递有限上下文给生成器的情况下。现有控制器只能部分解决问题，通常要么扩展上下文，要么从固定top-k集合中选择，或优化相关性而不显式修复缺失的桥接事实。

**Method:** 提出了AdaGATE，一个训练免费的证据控制器，将证据选择建模为token约束的修复问题。它结合了以实体为中心的间隙跟踪、有针对性的微查询生成，以及基于效用的选择机制，平衡间隙覆盖、证实、新颖性、冗余性和直接问题相关性。

**Result:** 在HotpotQA数据集上，在清洁、冗余和噪声注入的检索条件下评估AdaGATE。在所有三种设置下，AdaGATE都达到了最佳证据F1，清洁数据上达到62.3%，冗余注入条件下达到71.2%，同时使用的输入token比Adaptive-k少2.6倍。

**Conclusion:** 结果表明，显式的间隙感知修复与高效的token证据选择相结合，提高了在检索不完美情况下多跳RAG的鲁棒性。代码和评估管道已在GitHub上开源。
...
> 这项研究探讨了在生成式AI背景下，学生通过撰写反驳论证来培养批判性思维的效果，并验证了使用大型语言模型进行大规模写作评估的可行性。

<details>
  <summary>Details</summary>

**Motivation:** 随着生成式AI的普及，学生使用这些工具作答时存在作弊和认知卸载的风险，因此需要研究如何在AI环境下有效培养学生的批判性思维能力。

**Method:** 研究采用干预设计，让36名学生针对4个议论文题撰写反驳论证。通过六个评分维度（重点、逻辑、内容、风格、正确性和引用）进行三类人工评估（两次同伴互评和一次教师评估），同时使用六个前沿LLM作为评估者进行评分。研究采用混合方法，包括定性反馈和定量分析。

**Result:** 研究发现学生自行撰写的反驳论证包含逻辑等批判性思维核心要素；大多数LLM的评估结果与人工评估具有较好的一致性（Gwets AC2值为0.33），证明GenAI可以基于明确评分标准进行大规模写作评估。

**Conclusion:** 在生成式AI环境中，通过要求学生撰写反驳论证可以有效培养批判性思维；同时，大型语言模型具备了评估学生写作作品的能力，为教育评估提供了新的技术支持。
...
> 本文提出了一种基于证据的查询生成模型，能够从无查询的数据集中自动生成查询关键词，并在查询聚焦摘要任务中验证了其有效性，生成的查询在ROUGE评分上与原始查询表现相当。

<details>
  <summary>Details</summary>

**Motivation:** 大规模数据集虽广泛用于摘要任务，但通常缺乏查询、文档和摘要的完整组合，这限制了查询聚焦摘要(QFS)任务的发展。需要探索能否从无查询数据集中自动生成有意义的查询。

**Method:** 提出基于证据的查询生成模型，通过内在评估（比较生成查询与原始查询的相似性）和外在评估（使用不同预训练模型和SOTA QFS模型进行摘要任务）来验证方法的有效性。

**Result:** 实验结果表明，使用基于证据生成的查询所产生的摘要，在ROUGE评分上与使用原始查询生成的摘要具有竞争力，证明了该方法的有效性。

**Conclusion:** 基于证据的查询生成是可行的，能够有效支持查询聚焦摘要任务，为利用无查询数据集进行QFS研究提供了新的解决方案。
...
> SLAM是一种新型白盒水印方案，通过操纵语言结构几何而非词频分布，实现了几乎零质量损失的LLM水印检测（准确率100%，质量成本仅1-2分）。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM水印方案通过偏置下一个token分布实现检测，但会以可测量的文本质量损失为代价，需要在可检测性和文本质量之间找到平衡。

**Method:** SLAM利用稀疏自编码器识别编码语言结构（如语态、时态、从句顺序）的残差流方向，在生成时对性地控制这些方向，保持词汇采样和语义不变。

**Result:** 在Gemma-2 2B和9B模型上，SLAM实现100%检测准确率，质量成本仅1-2分，显著优于KGW、EWD和Unigram方案的7.5-11.5分，自然性和多样性接近未水印水平。

**Conclusion:** SLAM提供了一种权衡的鲁棒性特征：能抵抗词汇级编辑但易受重构句法的改写攻击（伴随质量成本），与传统基于token分布的方法形成互补。
...
> 本研究将少量推理轨迹编译成可重用的符号程序合成器，构建出无需LLM推理的独立求解器，在多个基准测试中显著优于传统LLM方法，并能与LLM形成有效的神经符号混合系统。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在解决需要大规模组合搜索的复杂程序合成任务时存在效率低下和不可靠的问题。如何利用LLM的推理能力，同时避免其在测试时的高计算成本和不稳定性，是当前面临的重要挑战。

**Method:** 使用编程代理将少量推理轨迹编译成可重用的符号程序合成器，基于约束DSL构建求解器系统。这些求解器在测试时无需调用LLM，可作为独立系统运行，或与LLM结合形成神经符号混合架构。

**Result:** 符号求解器集成在PBEBench-Lite上达到91.3%准确率，在PBEBench-Hard上达到84.7%，比测试时扩展的LLM高出16.3个百分点。与LLM搜索结合时，将PBEBench-Hard准确率从68.4%提升至85.8%，token使用减少78%。在历史语言学任务中零样本迁移达到80.1%准确率。

**Conclusion:** 推理轨迹可以成功编译成可重用的符号求解器，这些求解器不仅能够独立解决许多任务，还能有效补充LLM在困难案例上的推理局限，为实现领域通用的求解器归纳提供了可扩展的路径。
...
> 研究发现，对扩散语言模型的水印文本进行多次重写能有效破坏水印检测率，从原始的87.9%下降到五次重写后的4.86%。

<details>
  <summary>Details</summary>

**Motivation:** 探讨扩散语言模型水印在连续重写攻击下的鲁棒性，因为现有水印技术在面对多次重写时的效果尚不明确。

**Method:** 在不同领域生成1605个水印文本样本，使用4个开源语言模型进行5种风格的重写，每种风格连续重写最多5次，共产生160500个重写文本，系统测试水印检测率的变化。

**Result:** 原始文本检测率87.9%，单次重写后降至14-41%，五次重写后降至4.86%，检测分数比水印基线低86%，表明多次重写是强有力的水印攻击方法。

**Conclusion:** 连续重写攻击对扩散语言模型水印构成严重威胁，现有水印方案在多次重写下几乎完全失效，需要开发更鲁棒的水印技术。
...
> 研究表明，专门训练的领域小型语言模型(Olava Extract)在合同提取任务上超越大型模型，成本降低78-97%，且精确度更高，减少了幻觉现象。

<details>
  <summary>Details</summary>

**Motivation:** 验证专业化小型语言模型是否能在结构化合同提取任务中以远低成本超越大型语言模型，挑战只有最大模型才能提供企业级AI能力的假设。

**Method:** 将自托管的法律领域混合专家模型Olava Extract与五个前沿大型模型进行对比测试，评估其在合同提取任务中的宏F1、微F1和精确度表现。

**Result:** Olava Extract以宏F1=0.812、微F1=0.842获得最佳综合性能，推断成本降低78-97%，精确度最高，产生的幻觉和不受支持的提取更少。

**Conclusion:** 高性能的法律AI不再需要最大的外部托管模型，这挑战了商业价值的AI能力必须与更大模型、巨额基础设施支出和集中式提供商绑定的假设。
...
> 本研究发现多模态大语言模型与RAG结合时存在'再腐蚀'现象，即准确的外部上下文反而会导致模型放弃正确预测，并提出了无需训练的BAIR框架来解决这个问题。

<details>
  <summary>Details</summary>

**Motivation:** 尽管多模态大语言模型(MLLMs)通过与检索增强生成(RAG)结合来减少幻觉，但外部文档的引入会在实例层面隐藏严重的失败模式，需要深入理解这一问题的内在机制。

**Method:** 通过机制性诊断内部注意力矩阵，发现再腐蚀现象由注意力双重崩溃导致：(1)视觉盲症，表现为视觉注意力和锐度系统性抑制；(2)结构位置偏差，迫使模型优先处理边界标记而非语义相关性。提出BAIR框架，在推理时恢复视觉显著性并应用位置感知惩罚。

**Result:** BAIR在医疗事实性、社会公平性和地理空间基准测试中成功恢复了多模态定位能力，提高了诊断可靠性，且无需模型重训练或微调。

**Conclusion:** 研究揭示了RAG成功背后的假象，许多看似正确的结果仅是位置巧合。BAIR为恢复MLLMs的多模态定位能力提供了有效的推理时解决方案，具有重要的实际应用价值。
...
> When2Speak是一个用于训练大语言模型在多人对话中学习何时发言的合成数据集和训练流程，通过监督微调和强化学习显著提升了模型的参与时机决策能力。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在多人对话中存在校准问题，不知道何时发言会导致过度打断和对话连贯性下降。现有的对话系统缺乏对参与时机的建模，需要专门的数据和方法来解决这个问题。

**Method:** 提出了四阶段生成流程构建When2Speak数据集，包含21.5万个来自1.6万个对话的样本，覆盖2-6名参与者的多样化场景。首先进行监督微调(SFT)，然后应用带有非对称奖励塑形的强化学习来优化模型的保守倾向。

**Result:** SFT相比零基线提升60%的Macro F1（最高120%），但MIR达到0.50表示过于保守；强化学习将MIR降至0.186-0.218，召回率从0.479提升至0.78-0.81。

**Conclusion:** 证明了时间参与是可训练的对话智能维度，基于真实情境的合成数据为LLM在多人互动中更自然参与提供了有效且可扩展的解决方案。
...
> 研究者提出了一种名为TurnGate的多轮对话恶意意图检测方法，通过识别最早的伤害启用转折点来防止LLMs遭受隐蔽的多轮攻击，同时构建了MTID数据集并验证了方法的有效性。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型语言模型部署日益广泛，攻击者越来越多地使用多轮对话的方式隐藏恶意意图，将伤害性目标分散在多个看似无害的对话回合中，现有的安全防护措施对此类攻击仍然脆弱。

**Method:** 构建多轮意图数据集(MTID)，包含分支攻击路径、良性困难负样本和最早伤害启用转折点标注；开发TurnGate转折级别监控器，在确保对话完整性的前提下检测最早伤害启用转折点。

**Result:** TurnGate在恶意意图检测方面显著优于现有基线方法，同时保持了较低的过度拒绝率，并且在不同领域、攻击管道和目标模型上表现出良好的泛化能力。

**Conclusion:** 通过转折级别的精确干预机制，TurnGate有效解决了多轮对话中隐蔽恶意意图的识别问题，为提升LLMs安全性提供了新的技术方案。
...
> 研究发现大型语言模型在不同网络层处理情感效价，负面情感在早层，正面情感在中后层，且可通过方向向量操控模型输出情感倾向。

<details>
  <summary>Details</summary>

**Motivation:** 机制可解释性研究揭示了LLMs中概念的编码方式，但情感内容在机制层面的理解仍不充分，需要探究LLMs是否通过专门的内部结构处理情感效价。

**Method:** 使用激活补丁和操控技术，在开源LLMs上研究情感效价处理机制，通过固定主题反转情感效价以及方向向量操控实验验证。

**Result:** 发现负面情感定位于早期层，正面情感在中后层达到峰值；主题固定时反转情感效价产生符号相反的响应；通过方向向量操控可改变中性提示的情感倾向。

**Conclusion:** LLMs中的情感效价具有局部性、因果性和可操控性，这使其成为基于可解释性监督的具体目标。
...
**Motivation:** 现有的LLM安全基准主要偏重英语，且通过翻译评估往往无法捕捉国家特有的风险。虽然通用伤害是全球性的，但文化敏感性通常因国家而异。现有基准很难区分模型是拒绝遵循有害指令，还是根本未能理解指令中的文化语境。

**Method:** 为了解决这一问题，我们收集了XL-SafetyBench，一个针对10个国家-语言对的5500个测试案例集，包含越狱测试和文化敏感性测试。每个案例都经过多阶段合成、自动验证和人工标注。

**Result:** 我们对10个前沿模型和27个本地模型的评估发现两个关键点：第一，越狱鲁棒性和文化意识在前沿模型上并无关联；第二，本地模型普遍存在安全性越强、效果越差的现象，表明所谓的安全性来自于生成失败而非真正的对齐。

**Conclusion:** XL-SafetyBench能够更精确地衡量大模型在多语言环境下的整体安全性，有助于区分真正的对齐和单纯的生成失误。
...
> 本文提出了一种名为BADIT的新型多任务指令调优方法，通过将大语言模型参数分解为正交的基础能力表示来缓解跨任务干扰问题，在SuperNI基准测试上超越了现有最优方法。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型的多任务指令调优存在关键的跨任务干扰问题，即不同任务在共享参数上产生梯度冲突。现有的参数隔离方法（如任务特定神经元选择和混合专家）仍然无法完全解决这个问题，因为仍有大量参数在不同任务间共享。

**Method:** BADIT方法基于经验发现：某些参数会一致地协同激活并自然组织成基础组。该方法将LLM参数分解为代表基础能力的正交高奇异值LoRA专家，并通过秩1分量的球形聚类在训练过程中动态强制正交性。

**Result:** 在SuperNI基准上对6个LLM进行的广泛实验表明，BADIT不仅超越了现有最优方法，还有效缓解了跨任务干扰程度。

**Conclusion:** 通过将LLM的基础能力进行正交分解，BADIT为多任务指令调优中的跨任务干扰问题提供了一个有效的解决方案，展现了在实际应用中的优越性能。
...
> BioTool是一个专门用于生物医学工具调用的数据集，包含34个常用工具和7,040个高质量查询-API对，用于微调LLM以提升其在生物医学领域的工具调用能力。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在生物医学等高度专业化领域表现不佳，主要原因是无法有效利用生物医学工具，而现有的生物医学工具调用方法主要依赖上下文学习且工具集受限。

**Method:** 构建BioTool数据集，收集NCBI、Ensembl和UniProt数据库中的34个常用工具，创建7,040个人工验证的查询-API调用对，覆盖变异、基因组学、蛋白质组学、进化和普通生物学等领域，然后在40亿参数LLM上进行微调。

**Result:** 微调后的LLM在生物医学工具调用性能上显著提升，超越了GPT-5.1等商业LLM，人工专家评估显示集成BioTool微调的工具调用器显著提高了下游答案质量。

**Conclusion:** BioTool有效增强了LLM在生物医学领域的能力，证明了专业化的工具调用数据集对提升LLM在专业领域表现的重要性。
...
> 针对黑盒大语言模型的幻觉问题，提出Distribution-Aligned Adversarial Distillation方法，通过轻量级代理模型进行实时不确定性量化，仅需1%模型规模即可实现可靠评估。

<details>
  <summary>Details</summary>

**Motivation:** 现有的不确定性量化方法需要计算昂贵的多次采样或依赖内部参数，无法实时估计黑盒LLM的不确定性，阻碍了其在实际应用中的部署。

**Method:** 提出Distribution-Aligned Adversarial Distillation (DisAAD)，采用生成-判别架构指导轻量级代理模型学习黑盒LLM输出分布的高质量区域，然后使用代理模型重现黑盒LLM的响应并基于证据学习估计相应的不确定性。

**Result:** 实验验证了方法的有效性和前景，表明即使仅占目标LLM规模1%的代理模型也能实现可靠的不确定性量化。

**Conclusion:** 该方法解决了黑盒LLM实时不确定性量化的挑战，为检测和缓解大语言模型幻觉提供了实用解决方案，具有重要的商业应用价值。
...
> 研究发现，大型推理模型的显式评价意识(VEA)对模型行为影响很小，表明其安全风险可能被高估。

<details>
  <summary>Details</summary>

**Motivation:** 研究者担心大型推理模型在思维链中表达评价意识会导致模型策略性地调整输出，使其看起来更安全，这对模型评估和安全性构成潜在威胁。

**Method:** 在安全、对齐、道德推理和政治观点等多个基准测试上，通过两种方式测试VEA的影响：1) 政策内分析，比较自发包含和不包含VEA的思维链；2) 政策外分析，通过模型预填充注入或删除VEA相关语句后重新采样。

**Result:** VEA对模型行为影响极其有限：注入VEA效果接近于零(ω≤0.06)，删除VEA仅造成微小变化(ω≤0.12)，自发出现的VEA最多改变3.7个百分点的答案分布(ω≤0.31)。

**Conclusion:** 应该谨慎地将高VEA发生率解释为策略性行为或对齐篡改的证据。评价意识可能带来的安全风险比当前文献假设的要小。
...
> FLAS是一种基于流的激活引导方法，学习概念条件化的速度场来转换模型激活表示，在AxBench测试中首次超越了上下文提示的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的激活引导方法存在两个主要问题：1) 性能不如简单的上下文提示；2) 对未见概念的泛化能力差。这些问题源于先前方法共享的未经验证的简化假设，即限制引导干预为固定、单步、位置不变的变换。

**Method:** 提出FLAS（Flow-based Activation Steering），该方法学习一个通用的概念条件化速度场v_t(h,t,c)，它能在不依赖先前简化假设的情况下，将未引导的激活传输到引导状态。该方法避免了固定变换、单步操作和位置不变性的限制。

**Result:** 在AxBench评估中，FLAS是第一个持续超越上下文提示的学习方法。在Gemma-2-2B-IT上达到1.015的保留 harmonic mean，在Gemma-2-9B-IT上达到1.113，且无需针对每个概念进行调优。分析显示学习到的流呈现弯曲、多步、token变化的轨迹。

**Conclusion:** 研究发现学习到的流具有复杂的几何特性（弯曲、多步、token变化），表明先前关于激活空间几何的假设可能不完整。FLAS证明了突破传统简化假设可以显著提升激活引导的性能和泛化能力。
...
> LOVER是一个无监督的大语言模型验证器，通过逻辑规则约束多路径推理，无需标注数据即可达到监督方法95%的性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统LLM验证器需要构建昂贵的监督数据集，且面临数据多样性限制，成本高且扩展性差。

**Method:** 将验证器视为二元隐变量，利用内部激活并施加三种逻辑约束：否定一致性、组内一致性和组间一致性（按最终答案分组），结合逻辑规则作为先验。

**Result:** 在10个数据集上实验表明，LOVER显著优于无监督基线，达到监督验证器95%的性能水平，且可直接兼容任何现成LLM。

**Conclusion:** LOVER通过无监督学习和逻辑规则约束有效降低了验证器训练成本，在保持高性能的同时提升了实用性和灵活性。
...
> TextPro-SLM通过在输入端缩小模态差距，结合WhisperPro语音编码器和对超语言学理解训练的LLM骨干，显著提升了语音大语言模型的性能，且训练效率高。

<details>
  <summary>Details</summary>

**Motivation:** 现有语音大语言模型SLMs存在显著的模态差距问题，之前主要从输出口端尝试解决但效果有限。作者认为关键瓶颈在于输入端，需要让语音输入更接近文本大语言模型的处理方式。

**Method:** 提出TextPro-SLM，结合WhisperPro统一语音编码器（生成同步文本标记和韵律嵌入）与专门训练的LLM骨干。该骨干在保持原始TLM语义能力的同时学习超语言学理解，使语音输入更贴近韵律感知的文本LLM。

**Result:** TextPro-SLM在3B和7B规模下实现了所有主流SLM中最低的模态差距，同时在超语言学理解任务上表现出色，仅需约1000小时LLM训练音频即可达到这些效果。

**Conclusion:** 从输入端减少模态差距是有效且数据高效的解决方案，TextPro-SLM的成功证明了这一方法的重要性，为语音大语言模型的发展提供了新思路。
...
> LiSCP是一种轻量级的文体一致性分析方法，通过结合离散文体特征和连续语义信号，实现了对LLM生成文本的鲁棒检测，在对抗攻击和跨域场景下表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型文本检测方法依赖于统计线索或模型特定启发式规则，容易受到转述和对抗操作的影响，缺乏鲁棒性和可解释性，难以应对多媒体内容审核的实际需求。

**Method:** 提出LiSCP方法，构建一致性特征图谱，将离散文体特征与连续语义信号相结合，利用多模态引导的转述文本变体间的文体稳定性作为检测依据。

**Result:** 在真实多媒体新闻、电影数据集和传统文本领域的实验表明，LiSCP在域内检测性能优越，跨域检测相比现有方法提升高达11.79%，在对抗攻击和人机混合场景下也表现出显著鲁棒性。

**Conclusion:** LiSCP为LLM生成文本检测提供了一种轻量级且鲁棒的解决方案，通过文体一致性分析克服了现有方法的局限性，在多媒体内容审核等实际应用中具有重要意义。
...
> 本文提出PCNET方法，通过概率电路作为LLM残差流的密度估计器，精准检测幻觉并仅在必要时进行纠正，有效避免了对正确生成的破坏。

<details>
  <summary>Details</summary>

**Motivation:** 现有大语言模型幻觉纠正方法的主要局限是无差别地对每个token进行修正，导致原本正确的生成也被破坏。

**Method:** 提出PCNET概率电路作为可处理密度估计器，通过精确负对数似然计算检测事实流形上的几何异常；结合PC-LDCD解码策略，仅在潜在几何偏离事实区域时触发纠正。

**Result:** 在四个LLM和四个基准测试中，PCNET实现近乎完美的幻觉检测（最高AUROC达99%），PC-LDCD在TruthfulQA上获得最高分数，同时将平均损坏率降至53.7%，保持率达79.3%。

**Conclusion:** 该方法无需采样、外部验证器或权重修改即可有效检测和纠正幻觉，在保证生成质量的同时显著降低了对正确生成的破坏，为大语言模型的幻觉控制提供了新的解决方案。
...
> TableVista是一个包含30,000个样本的多模态表格推理基准测试，通过评估29个前沿模型发现当前模型在复杂结构布局和纯视觉设置下存在显著性能下降。

<details>
  <summary>Details</summary>

**Motivation:** 现有基础模型在处理复杂表格推理任务时缺乏系统性评估，特别是在视觉和结构复杂性的综合影响方面，需要建立全面的基准来识别模型能力的局限性。

**Method:** 构建包含3,000个高质量表格推理问题的数据集，通过多风格渲染和变换管道每个问题扩展为10个视觉变体，涵盖多样化场景风格、鲁棒性扰动和纯视觉配置，形成30,000个多模态样本。

**Result:** 评估模型在不同渲染风格下表现相对稳定，但在复杂结构布局和纯视觉设置下性能显著下降，揭示当前模型在结构复杂性与视觉集成结合时难以保持推理一致性。

**Conclusion:** 研究成果揭示了当前多模态能力的关键缺陷，为开发更鲁棒和可靠的表格理解模型提供了重要见解和方向指导。
...
> 本文提出了一种基于多语言地名数据的地理空间问答系统，构建了鞑靼斯坦共和国的双语地名数据集和问答语料库，采用混合检索器和Reader模型，实现了接近完美的问答性能。

<details>
  <summary>Details</summary>

**Motivation:** 解决多语言地名数据上的自动地理空间问答问题，满足数字人文、地理编码等应用需求，特别是在多语言地区的地理信息服务。

**Method:** 构建包含9,688个结构化记录的双语地名数据集；生成约39,000个问答三元组；设计混合检索器结合稠密语义索引与地理空间过滤；测试多种Reader架构（RuBERT、XLM-RoBERTa-large、T5-RUS）。

**Result:** 混合检索器在500个测试查询上达到Recall@1=0.988、Recall@5=1.000、MRR=0.994；XLM-RoBERTa-large在阅读理解任务上表现最佳（EM=0.992、F1=0.994）；通过后处理可完全解决数值问题。

**Conclusion:** 该系统在地理空间问答任务上表现出色，所有资源已开源，可应用于地理空间QA服务、地理编码和多语言地区的数字人文研究。
...
> PrimeFacts是一种从事实核查文章中提取细粒度证据的方法论，通过重写锚点句子为去上下文化的独立前提，显著提升了自动化证据检索和声明验证的性能。

<details>
  <summary>Details</summary>

**Motivation:** 事实核查文章包含丰富的支撑证据和推理过程，但由于这些证据以非结构化形式呈现，自动化验证系统难以有效利用。需要开发方法将事实核查中的证据结构化，使其可被机器自动处理。

**Method:** 收集13,106篇PolitiFact事实核查文章，识别49,718个文章内超链接作为关键证据锚点，利用大语言模型将锚点句子重写为独立、无上下文的前提，并探索提取额外隐含证据。

**Result:** 去上下文化的证据比原始句子的可检索性提高30%的相对MRR增益，在裁决预测中使Macro-F1比基线提高10-20分，这些提升在不同裁决粒度和模型架构上保持一致，定性分析显示重写后的前提忠实于原始来源。

**Conclusion:** 该工作展示了重用事实核查员证据实现自动化的前景，为自动化事实核查提供了大规模的结构化证据资源，验证了证据去上下文化处理的有效性和实用性。
...
> PersonaKit是一个开源低延迟网页平台，用于快速原型化和评估具有不同人格特制的对话代理，可以配置个性化的轮转行为策略。

<details>
  <summary>Details</summary>

**Motivation:** 现有的全双工对话系统在重叠语音处理时采用过于顺从的

**Method:** 开发了一个名为PersonaKit的开源网页平台，通过直观的JSON配置让研究者定义人格角色、指定概率性的中断处理行为（如让步、保持、桥接或覆盖），并自动部署对比性A/B调查。

**Result:** 通过8种不同人格的真实环境评估，证明PersonaKit提供了一个可扩展的端到端框架，能够有效研究下一代语音代理中复杂的社会语言学行为，解决了传统方法工程开销大的问题。

**Conclusion:** PersonaKit为研究个性化对话代理的轮转行为提供了实用解决方案，使研究者能够更容易地评估不同人格角色在重叠语音情境下的行为策略，推动了更具角色一致性的对话系统发展。
...
> 该研究通过形式语法学框架对比了AI生成文本与人类撰写的英语新闻文本，发现经过指令调优的新型LLM在句法和词汇多样性上有所降低，提示指令调优虽然提升了文本的连贯性和提示遵循能力，但可能缩小了模型的表達範圍。

<details>
  <summary>Details</summary>

**Motivation:** 为了深入理解大语言模型生成文本与人类撰写文本在句法特征上的差异，特别是在英语新闻文本领域，并评估不同时期LLM的发展变化。

**Method:** 采用中心驱动短语结构语法（HPSG）理论框架，分析对比两代LLM生成的文本与不同年份的《纽约时报》人类撰写新闻文章，运用生态学和信息理论的多样性指标量化语法结构和词汇类型的变化。

**Result:** 研究发现英语新闻文本在给定时间段内变化很小，而经过指令调优的新一代LLM相比旧模型在句法多样性和词汇多样性方面都出现明显下降，尤其是词汇多样性。

**Conclusion:** 虽然指令调优技术提高了LLM生成文本的连贯性和遵循提示的能力，但这种优化可能以缩小模型的表达范围为代价，未来的研究需要深入探讨这一trade-off问题。
...
> 研究发现LLM中的'先定位后更新'范式基于未经证实的假设，通过跟踪Transformer电路在微调过程中的演化，揭示了机制固有'自由演化'特性，导致静态机制引导存在时间滞后性。

<details>
  <summary>Details</summary>

**Motivation:** 质疑现有的定位-更新范式假设：静态参数推导出的机制是否能可靠指导动态参数更新？这一假设缺乏实证验证，可能影响后续优化效果。

**Method:** 系统跟踪Transformer电路在监督微调过程中的结构演化，提出三个新指标：电路距离（神经迁移）、电路稳定性（语义稳定）、电路冲突（跨任务干扰），从三个维度分析电路演化。

**Result:** 电路在参数更新中表现固有的'自由演化'特性，静态机制提取存在时间滞后，无法充分指导未来状态；解构了现有方法的'有效性假象'。

**Conclusion:** 强调了机制定位中'预见性'的必要性，提出预测性框架为未来研究指明方向，挑战了传统静态定位方法的有效性。
...
> BEACON是一个里程碑引导的政策学习框架，通过分割轨迹、奖励塑形和双重尺度优势估计，解决了长周期语言智能体训练中的信用归属和样本效率问题，在多个基准测试中显著提升了训练效果。

<details>
  <summary>Details</summary>

**Motivation:** 长周期代理任务需要语言智能体做数十个顺序决策，但强化学习面临两大挑战：信用归属问题（正确的早期动作因最终失败而被惩罚）和样本效率低（成功轨迹稀少导致学习信号几乎完全丢失）。

**Method:** BEACON框架利用长周期任务的组合结构进行精确信用分配：在里程碑边界分割轨迹，在片段内应用时间奖励塑形来奖励部分进展，在双重尺度估计优势值，防止远处失败污染局部动作的评估。

**Result:** 在ALFWorld、WebShop和ScienceWorld上，BEACON一致优于GRPO和GiGPO。在长周期ALFWorld任务上，BEACON达到92.9%成功率（接近GRPO的53.5%的两倍），有效样本利用率从23.7%提升至82.0%。

**Conclusion:** 以里程碑锚定的信用分配是训练长周期语言智能体的有效范式，为解决强化学习在长序列决策中的根本问题提供了新思路。
...
> 本文发现多模态知识编辑中存在实体身份混淆（EIC）问题，通过构建诊断基准验证了其成因，并提出了针对性的缓解策略。

<details>
  <summary>Details</summary>

**Motivation:** 多模态知识编辑旨在修正已部署的大视觉语言模型的内部知识，但当前对编辑后模型行为模式的研究不足，存在系统性失效模式亟待探索。

**Method:** 构建EC-Bench诊断基准来探测编辑前后图像-实体绑定的变化；分析现有方法无法区分图像-实体绑定与实体-实体关系知识的问题；提出将编辑限制在模型的I-E处理阶段以缓解EIC的约束策略。

**Result:** 研究发现EIC源于模型过度拟合E-E关联作为捷径，图像仍被感知为原始实体；通过约束编辑到I-E处理阶段能显著减少EIC现象。

**Conclusion:** 基于发现提出了忠实多模态知识编辑的设计原则，为未来研究提供了方法论指导，强调了区分I-E绑定与E-E关系知识的重要性。
...
> MemReranker是一个通过多阶段LLM知识蒸馏构建的重排序模型系列，解决了智能体记忆系统中语义相关但非答案关键信息的检索问题，在保持低延迟的同时实现了接近大型模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有重排序模型依赖语义相似性匹配，缺乏真正推理能力，导致检索结果语义相关但缺乏答案关键信息，存在相关性分数校准不准、复杂查询排序性能下降、无法利用上下文消歧等问题。

**Method:** 基于Qwen3-Reranker，通过多阶段LLM知识蒸馏构建0.6B/4B参数模型：多教师成对比较生成校准软标签，BCE点蒸馏建立良好分数分布，InfoNCE对比学习增强困难样本区分力，训练数据结合通用语料和记忆特定多轮对话数据。

**Result:** MemReranker-0.6B在记忆检索基准上大幅超越BGE-Reranker，匹配开源4B/8B模型和GPT-4o-mini；4B版本达到0.737 MAP，多项指标与Gemini-3-Flash相当，推理延迟仅为大型模型的10-20%；在金融和医疗垂直领域保持与主流大参数重排序器相当的泛化能力。

**Conclusion:** MemReranker通过创新的多阶段知识蒸馏方法和针对性的训练数据设计，有效解决了记忆系统中重排序模型的推理能力缺陷，在有限参数量下实现了与大型模型相当的性能，同时保持了高效的推理速度。
...
> 研究者提出了IRC-Bench基准测试，用于评估在回忆转录文中的隐式实体识别问题，该问题涉及从分散的叙事语境中推断隐含提及的人物、地点和事件。

<details>
  <summary>Details</summary>

**Motivation:** 人们在回忆个人经历时常不直接命名人物、地点和事件，而是依赖语境线索进行隐式引用，这种非局部性的实体识别在治疗、档案和社交场景中十分重要，但现有方法（命名实体识别、实体链接、共指消解）无法有效处理这种分散式线索的推理问题。

**Method:** 构建了IRC-Bench基准测试，包含25,136个样本，涵盖12,337个维基数据链接实体，样本包括显式提及的实体基础叙述和去除直接提及后形成的实体省略叙述配对；评估了19种配置方法，包括LLM生成、稠密检索、RAG和微调等。

**Result:** QLoRA适配的Llama 3.1 8B在开放世界设置下表现最佳（精确匹配38.94%，Jaccard相似度51.59%），而微调的DPR在封闭世界检索中领先（Hit@1为35.38%，Hit@10为71.49%）。

**Conclusion:** IRC-Bench为隐式实体识别这一具有挑战性的非局部推理问题提供了首个系统评估基准，实验结果表明大语言模型和稠密检索器各有优势，为该领域的研究开发提供了重要工具和数据支持。
...
> 本文提出了一种通过自动生成困难负样本标题（HNC）数据集来提升视觉-语言模型细粒度理解能力的方法，显著改善了模型的零样本检测能力和鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图像-文本匹配（ITM）方法由于网络收集的图文对关联性较弱，导致模型无法实现细粒度的跨模态语义理解，需要更有效的训练数据来改善这一局限性。

**Method:** 创建自动生成的困难负样本标题（HNC）数据集，为ITM训练提供具有挑战性的负样本；同时构建人工标注的测试集来评估模型在不同组合复杂度下的细粒度跨模态不匹配检测性能。

**Result:** 在HNC上训练的模型在诊断任务的零样本不匹配检测中表现更优，在噪声视觉输入场景下更加鲁棒，同时为下游任务的微调提供了相当或更好的初始化效果。

**Conclusion:** HNC数据集和方法能够有效提升视觉-语言模型的细粒度跨模态理解能力，为改进大规模多模态表征学习提供了实用的解决方案。
...
> A²TGPO是一种针对智能体大语言模型的新型强化学习方法，通过改进的信息增益信号，解决了多轮交互中单次工具调用贡献评估的关键挑战。

<details>
  <summary>Details</summary>

**Motivation:** 传统的强化学习方法在评估智能体大语言模型的多轮交互时，依赖稀疏的轨迹级结果奖励，难以准确评估单次工具调用的贡献。现有方法要么需要额外的外部奖励模型增加成本，要么基于树结构展开限制了轨迹多样性。

**Method:** A²TGPO方法提出了三个创新技术：(i)轮组归一化：在相同的交互深度内对信息增益进行归一化；(ii)方差缩放折扣累积：用累积项的平方根除以累积归一化信息增益；(iii)自适应轮级裁剪：基于归一化信息增益动态调整每轮的裁剪范围。

**Result:** 该方法通过重新设计信息增益信号的归一化、累积和使用方式，有效解决了信息增益在强化学习训练中的系统性挑战，包括跨轮位置归一化失真、优势幅度漂移和固定裁剪范围问题。

**Conclusion:** A²TGPO为智能体大语言模型的强化学习提供了一个有效的内部信号处理框架，无需外部评估器即可实现精确的过程信用分配，显著提升了多轮交互中单步决策的评估精度。
...
> TIDE提出了一种通过EmbeddingMemory机制在多层注入词元标识信息的新方法，解决了传统LLM中稀有词元训练不足和语义折叠问题，显著提升了语言建模性能。

<details>
  <summary>Details</summary>

**Motivation:** 现代LLM普遍采用单次词元注入设计，导致两个结构性问题：稀有词元因Zipf分布而训练不足（稀有词元问题），以及参数有限的模型将语义相似的词元映射到难以区分的隐藏状态（语义折叠问题）。

**Method:** 提出TIDE架构，通过EmbeddingMemory机制扩展标准transformer：包含K个独立MemoryBlocks存储词元的上下文无关语义向量，通过带可学习空库的深度条件softmax路由器在每一层注入这些向量。

**Result:** 理论上和实验上证明了TIDE在解决单次词元注入相关问题方面的优势，在多个语言建模和下游任务中表现出性能提升。

**Conclusion:** TIDE通过多层注入机制有效缓解了传统单次注入设计导致的稀有词元和语义折叠问题，为改进LLM的词元表示学习提供了新的方向。
...
> UniPrefill是一个通用的大模型长上下文预填充加速框架，可在token级别直接加速任何架构模型计算，并无缝集成到vLLM中，实现TTFT最高2.1倍加速。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型快速发展，对长上下文处理的需求不断增长，但现有预填充加速方法主要针对稀疏注意力机制，在新型混合架构上性能显著下降，且与连续批处理不兼容，难以集成到现代推理引擎中。

**Method:** 提出UniPrefill框架，采用token级别的直接计算加速方式，实现为连续批处理算子，并扩展vLLM调度策略以原生支持预填充-解码协同处理和张量并行，使其能够无缝集成到vLLM中。

**Result:** UniPrefill在Time-To-First-Token(TTFT)方面实现了最高2.1倍的加速效果，且随着并发请求数量的增加，加速效果更加显著。

**Conclusion:** UniPrefill成功解决了现有长上下文预填充加速方法在架构兼容性和集成性方面的限制，为各种模型架构提供了统一的加速方案，显著提升了长上下文推理效率。
...
> 该论文提出了一个基于异质集成的多语言预训练模型系统，用于检测22种语言社交媒体中的极化内容，通过对比多种技术发现独立任务建模结合类权重权衡是最有效的方法。

<details>
  <summary>Details</summary>

**Motivation:** 解决多语言、多文化、多事件环境下社交媒体内容极化检测的挑战，特别是在严重的标签不平衡情况下的分类性能问题。

**Method:** 采用异质集成方法，结合XLM-RoBERTa-large和mDeBERTa-v3-base多语言预训练模型，探索多任务学习、基于翻译的数据增强和类权重权衡等技术，通过三个子任务（二元检测、目标分类、表现形式识别）进行极化内容检测。

**Result:** 研究结果表明，与多任务学习相比，独立任务建模结合类权重权衡的方法在处理严重标签不平衡时表现更优。

**Conclusion:** 该系统为多语言极化内容检测提供了有效解决方案，独立建模策略结合适当的类别权重处理是应对标签不平衡挑战的关键。
...
> 传统强化学习(RL)提升大语言模型推理能力的机制被揭示为稀疏的策略选择而非能力获取；基于此发现提出的ReasonMaxxer方法无需RL即可达到相同效果，训练成本降低三个数量级。

<details>
  <summary>Details</summary>

**Motivation:** 研究表明RL并未教给模型新策略，只是重新分配基础模型已有的解决方案概率分布。这引发了一个根本问题：如果RL只是引导模型走向已知路径，RL优化循环本身是否必要？

**Method:** 通过在多个模型族和RL算法上进行token级分析，发现RL的正面影响集中在高熵决策点，仅影响1-3%的token位置。基于此提出ReasonMaxxer方法：仅在熵门控决策点应用对比损失，使用数百次基础模型推演，无需在线生成。

**Result:** 在三个模型族、六个规模和六个数学推理基准上，ReasonMaxxer与完整RL性能相当或更优，仅需数十个问题和单GPU数分钟训练，训练成本降低约三个数量级。

**Conclusion:** 推理改进应被重新定义为稀疏策略选择而非能力获取。只需识别并修正模型不确定的关键决策点，就能大幅提升推理能力，从而使复杂的RL优化循环变得不必要。
...
> 本文为阿拉伯语方言对话的语义分割建立了新基准，并提出了针对方言特点的分割模型，在低资源口语场景中表现优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有语义分割模型主要针对高资源书面文本优化，在处理阿拉伯语方言等低资源口语时效果不佳，主要面临非正式语法、代码转换和话语结构标记模糊等挑战。

**Method:** 构建了包含1000+样本的多体裁阿拉伯语方言对话基准，涵盖电话对话、代码转换播客、广播新闻和小说对话等场景，由阿拉伯语母语者标注验证。提出针对局部语义连贯性和话语不连续性的分割模型。

**Result:** 实验表明在MSA新闻体裁上表现良好的模型在方言转录语音上性能下降；提出的模型在方言非新闻体裁上持续优于强基线模型。

**Conclusion:** 该基准和方法可推广到其他低资源口语语言，为口语语义分割研究提供了重要资源和有效解决方案。
...
> 研究发现，通过提供代表性示例、增加上下文和减少位置偏差等量规修改可以提高人工评分者与自动评分者（LLM作为评判者）之间的一致性，而量规复杂化和保守聚合方法会降低一致性。

<details>
  <summary>Details</summary>

**Motivation:** 随着自动评分者(LLM-as-judges)在评估和内容审核中的广泛应用，需要理解量规修改如何影响人工评分者与自动评分者之间的一致性。整体性评估由于复杂性和主观性可能产生解释不一致，而分析性评估虽然能改善个体准确性，但可能导致评分间的不一致。

**Method:** 研究比较了整体性评估（如作文质量）和分析性评估（如将质量分解为流畅性和组织性）两种量规设计。通过编辑量规内容、添加示例和上下文、减少位置偏差等方式，分析这些修改对人工-自动评分者一致性的影响。

**Result:** 提供代表性示例和额外上下文的量规编辑提高了人工-自动评分者一致性；减少量规中的位置偏见也具有积极作用。相反，量规复杂度增加和保守的聚合方法会降低一致性。

**Conclusion:** 在设计和部署可靠的自动评分系统时，从业者应仔细分析特定领域和量规的性能表现，通过优化量规设计来提高人工与自动化评分之间的一致性。
...
> LatentRAG是一个通过将显式语言空间转换为连续潜在空间来加速Agent化RAG的新框架，在保持相当性能的同时将推理延迟降低约90%。

<details>
  <summary>Details</summary>

**Motivation:** 现有Agent化RAG通过多步检索和显式语言生成处理复杂问题，但由于需要逐个token生成冗长的思维和子查询，导致推理延迟严重，限制了其在实时场景中的应用。

**Method:** LatentRAG将推理和检索从离散语言空间转移到连续潜在空间，通过单次前向传播直接从隐藏状态生成潜在token表示的思维和子查询，在潜在空间中对齐大语言模型和密集检索模型，并加入并行潜在解码机制提高可解释性。

**Result:** 在七个基准数据集上的广泛实验表明，LatentRAG实现了与显式Agent化RAG方法相当的性能，同时将推理延迟降低约90%，显著缩小了与传统单步RAG的延迟差距。

**Conclusion:** LatentRAG通过潜在空间表示和优化有效解决了Agent化RAG的延迟问题，为复杂问答任务提供了高效解决方案，实现了高性能与低延迟的平衡。
...
> 本文发现现有AI文本检测器存在辛普森悖论问题，提出基于贝叶斯决策理论的局部校准方法，显著提升了检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 准确区分人类编写文本和大型语言模型生成的文本对社会至关重要。现有主流检测方法基于似然假设，但由于检测器隐藏空间中的信号不均匀性，简单的平均聚合会破坏强烈的局部信号。

**Method:** 提出一种基于贝叶斯决策理论的学习型局部校准方法，不再直接聚合原始token分数，而是先学习隐藏空间位置条件下的分数分布预测器，然后聚合校准后的对数似然比。

**Result:** 该方法显著改善了所有基线检测器和数据集的性能。校准版Fast-DetectGPT在GPT-5.4文本上的AUROC从0.63提升至0.85，新提出的局部校准DMAP检测器在各项指标上达到最先进水平。

**Conclusion:** 本文的核心价值在于精确诊断了现有检测器性能不足的重要原因，并提供了一个可兼容任何token平均流程的模块化解决方案，为后续研究提供了基础，可进一步扩展至更丰富的分布模型、改进的校准策略和集成方法。
...
> 本文提出了一种基于异常检测的无监督多语言笑声分割方法，使用BYOL-A音频编码器和Isolation Forest算法，在非英语环境中表现优于现有的监督学习方法。

<details>
  <summary>Details</summary>

**Motivation:** 当前笑声检测方法存在两个主要问题：1）依赖昂贵的手工标注数据，2）数据集主要基于英语语境，缺乏多语言支持。这限制了方法在实际多元文化环境中的应用。

**Method:** 将笑声分割任务建模为基于音频序列能量的异常检测问题，采用BYOL-A自监督学习音频表示，再使用Isolation Forest算法进行无监督的异常检测来实现笑声分割。

**Result:** 在四个数据集（包括脱口秀、情景喜剧和AudioSet短音频）上的实验表明，现有最先进方法在多语言环境中效果不佳，而所提方法在非英语设置下性能更优。

**Conclusion:** 这种无监督多语言方法有效解决了传统监督学习方法对标注数据的依赖和语言局限性问题，为跨文化笑声检测提供了更实用的解决方案。
...
> 该研究首次对有害语言检测的四个参考数据集进行了大规模分析，结合标注者特征、项目语言学特征及其交互作用，发现交互效应和交叉性影响被此前工作忽视，且词汇线索和标注者态度起关键作用，但效应模式在不同数据集间差异显著。

<details>
  <summary>Details</summary>

**Motivation:** 尽管人类标注差异已被确立为自然语言处理中的核心现象，导致标注实践向增加标注者数量和发布 disaggregated 数据集转变，但关于标注者特征、项目语言学特征及其相互作用的分析较少，而有害语言检测因其高度主观性成为最资源丰富的领域，亟需系统性理解。

**Method:** 采用大规模统计分析方法研究四个有害语言检测参考数据集，整合标注者特征（社会人口统计、态度等）、项目的语言学特征及其交互作用。

**Result:** 发现交互效应至关重要，揭示了被先前研究忽略的交叉性影响；词汇线索和标注者态度扮演重要角色；但效应模式在不同数据集间存在显著差异。

**Conclusion:** 该研究强调在进行有害语言检测时需要谨慎对待泛化和可迁移性原则，因为交互效应和交叉性因素以及数据集差异都表明不能简单地将发现推广到其他场景。
...
> 本文提出了一套完整的工具集成推理(TIR)方法，通过优化监督微调数据选择、比例控制和强化学习策略，使思维模型在获得强大工具使用能力的同时不损失原有推理能力，在数学推理基准测试中达到开源模型最优水平。

<details>
  <summary>Details</summary>

**Motivation:** 工具集成推理虽然可以扩展思维模型能力，但观察到即使模型很少实际调用工具，启用工具评估仍会降低推理性能。如何在工具使用和纯文本推理之间取得平衡是关键挑战。

**Method:** 提出一套综合TIR方法：(1)监督微调选择适合工具解决的易学习教师轨迹；(2)控制工具使用轨迹比例防止文本推理能力遗忘；(3)优化pass@k和响应长度而非训练损失；(4)基于合适SFT初始化和防模式崩溃的稳定强化学习阶段(RLVR)。

**Result:** 在Qwen3思维模型(4B和30B)上应用该方法，在AIME 2025上分别达到96.7%和99.2%，在多个基准测试中实现了开源模型的最优性能。

**Conclusion:** 该方法成功解决了工具增强与纯文本推理的权衡问题，为思维模型的工具集成提供了有效路径，显著提升了模型在需要外部工具的任务上的表现。
...
> 研究发现不同语言模型在评估环境和部署环境中的安全行为存在显著差异，这种'评估-环境差异'因模型而异，且结果受评估标准影响。

<details>
  <summary>Details</summary>

**Motivation:** 安全基准测试通常被视为模型部署后行为的证据，但如果模型行为依赖于提示是否看起来像评估，这种推断就不可靠。研究者意识到需要系统测量模型在不同情境框架下的行为差异。

**Method:** 提出了配对提示协议来测量评估-环境差异，通过将固定任务框架化为评估、实时部署交互或中性请求，控制了释义变化、基准熟悉度和评判者框架敏感性等因素。测试了5个指令调优检查点和基础模型对比。

**Result:** OLMo-3-Instruct表现出评估谨慎性（拒绝率增加11.8pp），而其他模型如Mistral、Phi-3.5和Llama-3.1则表现部署谨慎性。对齐过程是行为反转的关键阶段，而非简单的模型规模效应。跨家族异质性依赖于评判标准。

**Conclusion:** 模型的安全行为高度依赖于任务框架，这种依赖性在不同模型家族间差异显著。安全评估结果的解释需要谨慎考虑评判标准的影响，单一基准测试可能不足以预测部署后的行为。
...
> MANTRA是一个自动化框架，能够从自然语言操作手册合成机器可检查的合规性基准，用于评估工具使用型LLM代理的行为合规性。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM代理评估方法要么依赖手动构建的基准（缺乏可扩展性），要么使用LLM作为评判者（对复杂长篇手册缺乏可靠性），无法有效确保代理在遵循严格程序手册方面的行为合规性。

**Method:** MANTRA框架独立生成符号世界模型（捕获程序依赖关系）和追踪级合规检查集，使用SMT求解验证一致性，通过结构化修复循环解决不一致性，并自动生成具有挑战性的任务。

**Result:** 构建了包含285个任务、6个领域、50+页手册的新基准套件，生成的合规检查比现有基准更丰富且约束更强，可用于调试代理失败模式。

**Conclusion:** 将自动化基准生成与形式化验证方法相结合，实现了工具使用代理的可扩展且可靠的基准测试，为复杂手册下的代理合规性评估提供了有效解决方案。
...
> SKOP是一种新的激活转向方法，通过约束有害的注意重新路由来减少性能损失，实现更好的转向-效用平衡。

<details>
  <summary>Details</summary>

**Motivation:** 现有的激活转向方法在控制大语言模型行为的同时，往往会导致推理和检索性能下降，主要原因是转向向量改变了查询-键匹配，导致注意力从重要的上下文标记转移到信息量较少的标记上。

**Method:** 提出Steering via Key-Orthogonal Projections (SKOP)方法，通过保护模型依赖的少量关键标记的注意力模式，同时允许在不太关键的尾部标记之间重新分配注意力，从而约束有害的注意力重新路由。

**Result:** 在多个转向基准测试中，SKOP实现了最佳的转向-效用平衡，将效用损失降低了5-7倍，同时保持了超过95%的标准转向效果。在长上下文检索场景中，当标准转向方法失效时，SKOP仍能保持强大性能。

**Conclusion:** SKOP方法成功解决了激活转向中的注意力重新路由问题，为在保持模型推理和检索能力的同时控制LLM行为提供了有效解决方案。
...
> SEQUOR是一个自动基准测试，用于评估模型在长多轮对话中遵循约束的能力。研究发现，随着对话增长，模型遵循指令的准确性显著下降，尤其在多约束和约束变更情况下。

<details>
  <summary>Details</summary>

**Motivation:** 现有的指令遵循基准主要关注单轮或短多轮场景，缺乏对长对话轮次中指令遵循能力的评估，而实际对话助手需要能够处理持续更新的用户指令。

**Method:** 构建了SEQUOR基准测试，使用从真实对话中提取的约束来模拟角色驱动的交互，并评估模型在长多轮对话中的约束遵循表现。

**Result:** 模型在遵循单个约束时，准确性随对话长度增加而下降超过11%；遵循多个约束时准确性下降超过40%；约束添加或替换场景下准确性下降超过9%。

**Conclusion:** 当前大型语言模型在长多轮对话环境下的指令遵循能力仍然有限，SEQUOR为更好地评估助手系统的指令遵循能力提供了有效方法。
...
> GATHER是一种图感知的超实体检索方法，用于零单细胞类型注释，通过拓扑收敛点捕获多基因协同信号，仅需单次LLM调用即超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 零样本单细胞类型注释中，查询包含数十到数百个基因，没有单个基因具有决定性，标签仅从它们的集体共现中产生。基于知识图谱的RAG方法从个体基因进行局部实体级探索，导致扩展性差和LLM成本高。

**Method:** 提出GATHER方法，执行全局多源图遍历，识别拓扑收敛点——可从多个输入基因共同到达的节点。通过节点和路径重要性评分选择信息性证据，检索过程完全无需LLM参与。

**Result:** 在自构建的细胞中心生物知识图谱(VCKG)上，GATHER在Immune和Lung数据集上超越了ToG、ToG-2、RoG、PoG等基线，分别达到27.45%和59.64%的精确匹配准确率，每个样本仅需1次LLM调用，而基线需要2-61次调用。

**Conclusion:** 收敛节点将多实体信号压缩为紧凑、高信息量的证据，比多跳路径传递更多信息，为局部实体级推理提供了高效的全局替代方案。
...
> 本文提出了心智景观激活签名(MiA-Signature)概念，通过压缩的全局激活模式表示来近似激活对下游处理的影响，在LLM系统中实现了性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 认知科学研究表明，可报告的意识访问与分布式记忆系统的全局激活相关，但个体无法直接访问或枚举所有激活内容，这种张力表明认知可能依赖于一种紧凑表示来近似激活的全局影响。

**Method:** 受认知科学启发，引入MiA-Signature作为查询引发的全局激活模式的压缩表示。在LLM系统中，通过基于子模块的高层概念选择来实现，覆盖激活语境空间，可选择通过工作记忆的轻量级迭代更新进行细化。

**Result:** 将MiA-Signature集成到RAG和代理系统中，在多个长语境理解任务上取得了持续的性能提升。

**Conclusion:** MiA-Signature作为一种有效的压缩表示机制，能够近似完整激活状态的效果同时保持计算可行性，为大语言模型的长语境理解提供了新的解决方案。
...
> 本文提出了一个结合规则过滤和LLM分类的可扩展新词检测流水线，通过处理5.27亿条Reddit帖子，最终识别出599个真实的新词汇。

<details>
  <summary>Details</summary>

**Motivation:** 面对海量社交媒体数据，传统新词检测方法难以有效扩展。需要开发一个可扩展、模块化的自动化流水线，能够大规模识别新词汇并保持准确性。

**Method:** 基于语法和超语素构词框架构建四类分类体系（新词、实体、外来词、无），采用规则过滤筛选候选词，多LLM投票分类，最后人工验证确认。

**Result:** 从原始语料中提取出1.246亿个独特词元，缩减99.99%得到1021个候选新词，最终确认599个（58.7%）为真实新词，发现了显著的LLM分类差异。

**Conclusion:** 该流水线成功实现了大规模新词检测，代码和标注数据公开可用，同时揭示了跨模型一致性问题，为新词检测研究提供了重要工具和见解。
...
> 该研究探讨了文本和语言学内容特征在COVID-19假新闻检测中的应用，发现传统机器学习方法（特别是随机森林）能有效利用这些特征检测假新闻，但特征的组合使用并未显著提升检测效果。

<details>
  <summary>Details</summary>

**Motivation:** 尽管有实证证据表明文本和语言学特征有助于区分真假新闻，但基于内容特征的假新闻检测研究相对不足，因此本研究旨在探索这些特征的有效性。

**Method:** 研究收集了COVID-19大流行期间的新数据集，使用词组二元语法、词性分布等文本和语言学特征，并通过决策树、K近邻、逻辑回归、支持向量机和随机森林等传统机器学习算法进行实验验证。

**Result:** 随机森林表现最佳，支持向量机紧随其后；文本和语言学特征单独使用时均能提升检测效果，但将它们组合成单一模型并未显著改善检测性能；二元语法和词性标签的使用效果存在差异。

**Conclusion:** 研究表明，即使在深度学习盛行的时代，传统机器学习方法仍然能够有效利用文本和语言学特征进行假新闻检测，这为假新闻检测提供了新的思路和实用价值。
...
> 研究分析了三种LLM在情感驱动对话中的自洽性，发现模型对包含错误前提的查询表现不佳，尤其在中等情感语境下更容易受到错误信念影响，注意力机制分析显示模型从评估转向生成模式。

<details>
  <summary>Details</summary>

**Motivation:** 为了评估大语言模型在情感敏感对话场景下的可靠性，特别是在面对包含错误预设的查询时是否能够保持一致性和准确性，这对于模型在高风险应用中的部署至关重要。

**Method:** 将LLM生成的文本作为查询重新输入同一模型，通过三种不同强度的错误声明查询（中等和极端情感两个维度）来测试Claude-3.5-haiku、GPT4o-mini和Mistral-7B三个模型的一致性表现，并基于注意力分数进行扩展分析。

**Result:** LLMs表现低于平均水平，容易受到查询中嵌入的错误信念影响，这种脆弱性在中等情感内容中尤为明显。注意力分析显示模型的优先级从评估转向生成，可能导致对错误前提的识别能力下降。

**Conclusion:** 研究发现揭示了LLMs在情感敏感环境中存在的潜在风险，强调了在部署这些模型到高 stakes 场景前需要加强其一致性和错误检测能力，特别是在处理中等情感内容的查询时。
...
> Litespark-Inference是一个利用SIMD内核优化三值大语言模型推理的框架，通过整数加减运算替代浮点乘法，在主流CPU上实现了显著的性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM推理对计算资源要求过高，大量个人计算机CPU资源未被充分利用。虽然三值模型理论上可消除浮点运算，但现有框架未能充分利用这一优势。

**Method:** 开发定制SIMD内核，将矩阵乘法转换为简单的加减运算，利用现代CPU的整数点积指令，并创建可pip安装的框架Litespark-Inference，直接集成Hugging-Face。

**Result:** 在Apple Silicon上相比标准PyTorch推理实现了9.2倍的提速、52倍的吞吐量提升和14倍的内存节约，在Intel和AMD处理器上也获得类似加速效果。

**Conclusion:** 通过充分利用三值模型的数值特性和现代CPU硬件能力，Litespark-Inference成功在个人计算机上实现了高效的LLM推理，大幅降低了AI应用的计算门槛。
...
> 研究发现词汇频率比语言模型意外值更能预测隐喻新颖性，传统认为的LM意外值与隐喻加工难度的关联可能源于混淆了词汇频率因素。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型意外值被广泛用作语境可预测性的代理指标，并被报道与隐喻新颖性判断相关，但意外值与词汇频率紧密关联，需要澄清这种交互作用。

**Method:** 使用两种词汇频率测量方法，分析8个Pythia模型规模和154个训练检查点的意外值估计，考察意外值、频率与隐喻新颖性的关系。

**Result:** 词汇频率始终是比意外值更强的隐喻新颖性预测指标；意外值与新穎性的关联在训练早期达到峰值然后下降，与意外值-频率关联增长的时间模式相呼应。

**Conclusion:** 以往报道的最优语言模型意外值设置可能错误地将语境可预测性与隐喻新颖性及加工难度关联起来，词汇频率才是主要影响因素。
...
> 本研究揭示了大型语言模型代理在处理隐性冲突记忆更新时存在显著不足，并提出了STALE基准测试和CUPMem解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM代理的个性化记忆测试主要关注静态事实检索，忽略了当新证据出现时修改已有信念的能力。研究发现存在一个关键且未被充分探索的失败模式——隐性冲突，即后续观察在未显式否定的情况下使早期记忆失效。

**Method:** 研究团队设计了STALE基准测试，包含400个专家验证的冲突场景（1200个评估查询），涵盖100多个日常主题，上下文长度达15万token。提出了三维探测框架：状态解决、前提抵抗和隐性策略适应。同时开发CUPMem原型系统，通过结构化状态整合和感知传播搜索来强化写入时修订。

**Result:** 系统性评估显示，前沿LLM和专业内存框架在检索更新证据与据此行动之间存在普遍鸿沟，最佳模型的整体准确率仅为55.2%。模型常接受用户查询中过时的假设，且难以识别一个方面状态变化应使相关记忆失效的情况。

**Conclusion:** 研究表明显式状态裁决是实现强大代理记忆的有前景方向。CUPMem原型为状态感知记忆建立了初步基线，通过明确的状态管理和更新机制来改善长期个性化记忆的一致性和准确性。
...
> Token-Superposition Training (TST) 是一种简单高效的预训练方法，在不修改模型架构的情况下将多个token叠加训练，在同等损失下可将预训练时间减少2.5倍。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型的预训练成本极高且效率低下，传统方法需要复杂且侵入性的修改才能实现高数据吞吐量。

**Method:** TST分为两个阶段：(i)高效率叠加阶段，将多个连续token合并为一个token bag，使用多热点交叉熵目标训练；(ii)恢复阶段，回到标准训练方式。该方法无需修改并行策略、优化器、分词器、数据或模型架构。

**Result:** 在270M、600M参数规模上广泛评估，并在3B和10B A1B混合专家模型上验证。TST在基准损失和下游评估中持续优于基线，在同等损失设置下，10B A1B规模下预训练时间减少高达2.5倍。

**Conclusion:** TST是一种简单实用的预训练加速方法，在不同设置下具有高度鲁棒性，能够显著提升预训练效率，为大规模语言模型训练提供了新的思路。
...
> Cola DLM是一种分层潜在扩散语言模型，通过分层信息分解将文本生成转化为连续潜在空间的建模，实现了生成效率、可扩展表示学习和有效全局语义建模的统一。

<details>
  <summary>Details</summary>

**Motivation:** 自回归语言模型虽然取得了成功，但固定从左到右的生成顺序限制了高质量文本生成的灵活性。现有替代方案难以同时实现生成效率、可扩展表示学习和有效的全局语义建模。

**Method:** Cola DLM采用三层架构：首先使用Text VAE学习稳定的文本到潜在表示映射，然后在连续潜在空间中用块因果DiT建模全局语义先验，最后通过条件解码生成文本。从统一马尔可夫路径视角，其扩散过程执行潜在先验传输而非令牌级观测恢复。

**Result:** 通过4个研究问题、8个基准测试、严格匹配约20亿参数的自回归和LLaDA基线以及扩展到约2000 EFLOPs的扩展曲线实验，验证了Cola DLM的有效配置和文本生成的强扩展性能。

**Conclusion:** 分层连续潜在先验建模是令牌级语言建模的 principled 替代方案，生成质量和扩展行为可能比似然更能反映模型能力，同时为离散文本和连续模态的统一建模提供了具体路径。
...
> 本文提出Lighthouse Attention，一种训练专用的对称选择式分层注意力算法，通过自适应压缩和解压缩序列来降低注意力的计算复杂度，实现更快的训练速度和更低的最终损失。

<details>
  <summary>Details</summary>

**Motivation:** Transformer模型在极长序列训练时面临缩放点积注意力(SDPA)的二次时间和内存复杂度瓶颈，限制了训练效率和序列长度处理能力。

**Method:** 提出三层贡献：(1) 亚二次分层预处理步骤，实现序列的自适应压缩和解压缩；(2) 对称压缩策略，同时压缩查询、键和值，保持从左到右的因果性并提高并行性；(3) 两阶段训练方法，前期使用Lighthouse Attention预训练，后期短暂训练恢复完整注意力模型。

**Result:** 小规模LLM预训练实验表明，与其他设置完全匹配的情况下，相比完整注意力训练，该方法实现了更快的总训练时间和恢复阶段后更低的最终损失。

**Conclusion:** Lighthouse Attention是一种有效的训练优化方案，能够显著提升长序列Transformer训练效率，且可在训练结束时轻松移除，不影响最终模型的完整注意力性能。
...
> 本研究在低资源环境下比较了两种远程认知康复自动报告生成方法：基于规则的模板系统与零样本LLM方法(GPT-4)，发现在临床可靠性和语言质量之间存在权衡，并提出了临床报告系统设计建议。

<details>
  <summary>Details</summary>

**Motivation:** 远程康复工具产生大量交互数据，临床医生难以高效审查，而自动报告生成可以解决这一问题。在缺乏参考报告的低资源环境下，需要探索可靠的自动化方法。

**Method:** 研究采用两种方法：1)将言语治疗领域知识编码为显式决策规则的基于模板系统；2)使用GPT-4的零样本LLM方法。两种系统使用相同的预提取结构化变量，由8名言语治疗师和学生通过九项标准问卷进行评估。

**Result:** 模板系统在流畅性、连贯性和结果呈现方面得分更高，而GPT-4产生更简洁的输出。虽然存在一致的方向性差异，但经过校正后没有达到统计学显著性，反映了专家临床评估的规模限制。

**Conclusion:** 基于评估者反馈，提出了远程康复环境下临床报告系统的8项设计建议。这项工作为低资源环境下的临床自然语言生成提供了一个可复制的方法论，展示了如何通过对照比较来指导生成式AI在医疗领域的负责任应用。
...
> UniSD是一个统一的自蒸馏框架，通过整合多种互补机制，系统性地解决了自回归大语言模型自蒸馏中的监督可靠性、表示对齐和训练稳定性问题。

<details>
  <summary>Details</summary>

**Motivation:** 自蒸馏为不依赖外部强教师的大语言模型适配提供了前景，但在自回归LLM中面临挑战：自生成轨迹是自由形式的，正确性依赖任务，看似合理的推理仍可能提供不稳定或不可靠的监督。现有方法只研究孤立的设计选择，缺乏对其有效性、作用和相互作用的系统性理解。

**Method:** UniSD统一框架整合了互补机制：多教师一致性、EMA教师稳定化、词级对比学习、特征匹配和散度裁剪。通过六个基准测试和三个模型家族的六个模型，系统研究了自蒸馏相对于静态模仿的改进、各组件的贡献以及组件间的相互作用。

**Result:** UniSDfull集成了最有效的组件组合，相比基础模型提升+5.4个点，相比最强基线提升+2.8个点。实验揭示了自蒸馏在何时优于静态模仿，哪些组件驱动性能提升，以及组件如何跨任务相互作用。

**Conclusion:** 自蒸馏是一种实用且可控制的方法，能够在不依赖更强外部教师的情况下实现高效的LLM适配。UniSD为理解和优化自蒸馏系统提供了宝贵洞见，为未来的LLM适配研究指明了方向。
...
> 本研究通过建立联合行动模型和MUM（多数人可理解调制）概念，量化分析了算法语言（Algospeak）在可理解性和检测性之间的权衡关系，并创建了可复现的评估框架和数据集。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型在内容生成和审核中的作用日益重要，Algospeak等语言规避策略不断升级，促使规避者和检测者之间形成共同进化。需要深入理解这种动态关系。

**Method:** 提出联合行动模型描述Algospeak增长导致可理解性和可检测性下降；引入MUM概念定义阈值；构建可复现框架生成意义保持的Algospeak变体；基于COVID-19虚假信息创建700个样本的数据集；进行理解恢复和虚假信息检测双重评估。

**Result:** 通过七种语言模型的评估，揭示了可理解性与调制水平的特征关系，成功估计出MUM阈值，实现了不同策略和模型间的敏感性分析。

**Conclusion:** 该研究为理解Algospeak背后的动态机制奠定了基础，提供了完整的评估框架、数据集和实验设置，有助于未来相关研究的开展。
...
> 本研究提出了一种简化的人机协同工作流程，利用领域自适应解析器之间的一致性来完成韩语二语形态句法标注任务。

<details>
  <summary>Details</summary>

**Motivation:** 韩语二语(L2-Korean)的形态句法标注面临资源有限和人工标注成本高昂的问题，需要开发高效的半自动标注方法来提高标注效率和质量。

**Method:** 通过评估两个领域自适应解析器之间的一致性作为标注正确性的代理指标，将其与独立的人工判断进行比较验证，并深入分析解析器不一致案例的语言学特征。

**Result:** 解析器与人工判断之间表现出强相关性，证明了半自动L2-Korean UD标注的可行性。不一致案例主要集中在语法关系区分和子句边界模糊等语言学上可预测的领域。

**Conclusion:** 该人机协同工作流程为L2-Korean标注提供了可行的解决方案，虽然部分不一致案例可通过模型迭代改进解决，但其他案例反映了L2-Korean语料库解析和标注中固有的深层次表示挑战。
...
> 论文提出了首个大规模评估LLM生成报告中引用准确性的框架，发现即使最先进的模型在链接有效性(94%)和相关性(80%)方面表现良好，但事实准确性仅为39-77%，存在严重的引用质量问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM研究代理生成的引用无法可靠验证，要么完全信任模型自引存在偏见，要么使用RAG方法但不验证源的可访问性、相关性和事实一致性，亟需系统的评估框架。

**Method:** 开发基于AST解析器的源归属评估框架，从三个维度评估：链接有效性验证、内容相关性测量、事实准确性检查；使用人工校准的LLM作为评判器，对14个闭源和开源模型进行基准测试。

**Result:** 最强前沿模型链接有效性>94%、相关性>80%，但事实准确性仅39-77%；开源模型不足一半能一次性生成引用报告；工具调用从2增加到150时，事实准确性平均下降42%，表明更多检索不提高引用准确性。

**Conclusion:** 揭示表面引用质量与事实可靠性之间存在的关键脱节，提出的评估框架为评估这一问题提供了基础设施，强调了改进LLM引用事实准确性的重要性。
...
> 本文提出了StraTA框架，通过引入轨迹级策略来改进LLM在长时序决策任务中的表现，在多个基准测试中显著提升了样本效率和最终性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前大型语言模型作为交互智能体时，在长时序决策任务中表现不佳，主要因为现有方法主要是被动式的，这削弱了在扩展轨迹上的探索和信用分配能力。

**Method:** StraTA框架从初始任务状态采样紧凑策略，基于该策略执行后续动作，采用分层GRPO风格的rollout设计联合训练策略生成和动作执行，并进一步通过多样策略回放和关键自我判断进行增强。

**Result:** 在ALFWorld、WebShop和SciWorld三个基准测试中，StraTA显著超过了强基线方法的样本效率和最终性能，分别达到93.1%、84.2%的成功率和63.5%的综合得分。

**Conclusion:** 引入显式的轨迹级策略能够有效改善LLM智能体在长时序决策任务中的表现，StraTA为长时序强化学习提供了一个简单而有效的解决方案。
...
> 本文提出了POPO（仅正向策略优化）框架，它完全通过在线正向样本来进行强化学习训练，避免了传统方法中的负样本采样问题，在数学推理任务上达到了与GRPO相当甚至更好的性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的强化学习方法（如GRPO）需要同时采样正负样本来计算梯度估计，但在稀疏二元奖励场景下，负样本缺乏失败严重程度的区分度，且组合巨大性使得少量样本难以覆盖有意义的奖励信号。

**Method:** POPO框架采用三个关键技术：1）仅使用正向样本进行有界重要性采样；2）采用孪生策略网络和基于动量的自适应法则来稳定策略演化；3）在孪生表示空间中用有界相似性惩罚项替代KL散度。

**Result:** 实验表明POPO在Qwen系列模型上达到了与GRPO相当或更优的性能，特别是在AIME 2025上达到36.67%（Qwen-Math-7B），优于GRPO的30.00%。

**Conclusion:** POPO证明了仅通过正向样本就能有效学习，隐式的负向梯度可以通过样本重分布自然产生，消除了负样本采样的必要性，为LLM的数学推理能力提升提供了新的训练范式。
...
> EMO是一种无需人工先验的模块化混合专家模型，通过文档边界约束使相似领域令牌共享专家池，实现了专家的语义级专业化，在保留25%专家时性能仅下降1%。

<details>
  <summary>Details</summary>

**Motivation:** 传统大语言模型作为整体系统部署时，即使只需要特定领域能力也要加载全部模型；现有MoE模型在限制专家子集使用时性能严重下降，这限制了在内存受限环境下的实用性。

**Method:** EMO通过约束同一文档内的令牌从共享专家池中选择专家，不同文档可使用不同池，仅利用文档边界就实现了专家的语义级分组，预训练了1B活跃、14B总参数的模型。

**Result:** 作为完整模型匹配标准MoE性能；保留25%(12.5%)专家时性能仅下降1%(3%)，而标准MoE在相同设置下失效；专家实现语义级专业化而非语法级专业化。

**Conclusion:** EMO为大型稀疏模型的模块化、内存高效部署提供了新路径，为可组合架构开辟了新机遇，解决了MoE在专家子集选择上的性能退化问题。
...
> 本文提出了一种基于布局感知的开放集欺诈检测方法，通过微调DINOv3模型并使用复合损失训练，能够有效发现自适应身份证伪造活动，在新数据集上发现了276起欺诈案例。

<details>
  <summary>Details</summary>

**Motivation:** 传统的身份证明文件欺诈检测是静态二分类问题，但攻击者会不断修改模板和伪造流程，导致历史欺诈标签失效，且成功的伪造会以连贯的大规模活动重新出现，因此需要开放集欺诈发现方法。

**Method:** 采用DINOv3模型，通过上下文感知的SimMIM微调和监督度量学习进行领域适应，使用复合损失函数促进类间分离和类内紧凑性，仅使用美国身份证进行训练。

**Result:** 在加拿大身份证布局分类上达到99.83%准确率，在20,448张加拿大身份证数据集中发现276起自适应物理欺诈案例，其中222起未被现有检测器发现，支持从单个种子案例扩展发现相关案例。

**Conclusion:** 布局感知的文档嵌入为在分布偏移下发现新颖和大规模欺诈活动提供了与生产对齐的基础，有效解决了传统检测方法的局限性。
...
> 本文提出了一种基于生成对抗网络和循环一致性损失的面向类别的反事实解释方法，用于生成可信的反事实实例，以改进医学影像的特征归因可视化。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于判别模型的可视化技术只关注影响分类的最小判别特征区域，不能全面考虑所有显著对象，而这在医学影像诊断中至关重要，因为需要识别所有可能影响诊断的异常区域。

**Method:** 采用生成对抗网络(GAN)框架，结合循环一致性损失函数，开发反事实解释方法；同时提出评估反事实实例质量的新技术和可信反事实实例生成方法。

**Result:** 在合成数据集、结核病数据集和BraTS数据集上的实验证实了方法的有效性； baseline结果在BraTS数据集上展示了良好的性能。

**Conclusion:** 该方法解决了现有技术无法生成可信反事实实例的局限性，通过反事实解释配合可信实例的方式，提供了基于类比的自我解释机制，特别适用于医学影像诊断的辅助可视化。
...
> 本文提出了一种密度感知校准方法，通过结合DETR风格3D检测器中的潜在对象查询特征密度，实现了在分布偏移场景下的可靠不确定性估计。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3D目标检测器校准性差，特别是在分布偏移场景下。传统的后处理校准方法虽然在分布内测试中有所改进，但在分布偏移场景下失效，这限制了自动驾驶系统等安全关键应用的部署。

**Method:** 提出密度感知校准方法，将后处理校准器与DETR风格3D检测器的潜在对象查询特征密度相结合。通过在这些查询特征上拟合密度估计器，联合重新校准分类和边界框回归的不确定性。

**Result:** 在多视角相机和基于LiDAR的检测器上，该方法在分布内和分布偏移场景下都一致地优于标准的后处理校准方法。

**Conclusion:** 该方法成功解决了3D目标检测在分布偏移场景下的校准问题，为安全关键应用提供了更可靠的不确定性估计，具有广泛的应用前景。
...
> ViTok-v2是一个大规模、支持多分辨率的Vision Transformer自编码器，通过NaFlex技术和DINOv3感知损失实现了突破性的图像重建性能，推动了图像压缩-生成权衡的帕累托前沿。

<details>
  <summary>Details</summary>

**Motivation:** 现有ViT自编码器在训练分辨率外性能下降，对抗性损失导致训练不稳定。研究表明压缩比r与重建质量生成难度存在权衡关系，改进tokenizer重建是关键。

**Method:** 采用NaFlex技术实现原生多分辨率和宽高比支持；引入DINOv3感知损失替代LPIPS和GAN目标；在20亿图像上训练，扩展至50亿参数。

**Result:** 在256p分辨率匹配或超越SOTA重建效果，在512p及以上分辨率全面超越所有基线模型；与流匹配生成器的联合缩进实验推动了帕累托前沿。

**Conclusion:** ViTok-v2证明了大规模ViT自编码器在多分辨率下的有效性和稳定性，为下一代图像tokenizer提供了重要基准，展示了tokenizer和生成器联合缩放的潜力。
...
> Open-SAT是一种无需训练的查询嵌入优化算法，通过大语言模型优化文本嵌入，显著提升开放式卫星图像检索的F1分数达16.04%

<details>
  <summary>Details</summary>

**Motivation:** 卫星应用中用户查询往往是开放式自然语言，超出预设类别范围，现有视觉语言模型（如CLIP）难以准确对齐此类查询与卫星图像内容

**Method:** Open-SAT在推理时运行，使用VLM计算图像瓦片嵌入并存储于向量数据库，利用LLM结合目标物体及其周围环境信息优化文本嵌入，采用无阈值检索机制提升准确性和效率

**Result:** 在三个公共基准测试中，Open-SAT将F1分数提升最高16.04%，同时保持相当的图像瓦片检索数量

**Conclusion:** Open-SAT在大语言模型指导下有效解决了开放式卫星图像检索问题，无需额外训练或监督，为卫星应用中的开放式查询提供了实用解决方案
...
> egenioussBench是一个基于地理空间参考数据的视觉定位基准，包含城巿级三维网格模型和手机图像数据集，具有厘米级精度的真值，专门用于评估大规模跨视图和跨域视觉定位方法。

<details>
  <summary>Details</summary>

**Motivation:** 传统的基于运动恢复结构(SfM)的视觉定位基准在可扩展性上存在局限，无法满足城巿级大规模应用需求。需要构建一个基于实际可部署的地理空间参考数据、能反映真实部署场景的基准数据集。

**Method:** 使用地理空间参考数据构建基准：城巿级机载3D网格模型和CityGML LoD2模型作为参考；使用智能手机拍摄2,709张图像，通过PPK和地面控制点获得厘米级精度的真值；通过计算共可见性矩阵并选择最大独立集获得非共可见子集；将数据分为测试集(42张，隐藏真值)和验证集(412张，公开真值)。

**Result:** 构建了一个包含跨视图和跨域挑战的视觉定位基准，提供了公开排行榜和多种评估指标(分箱指标、中位数、RMSE、异常值比例)，支持网格和LoD2方法的公平比较。数据集和评估代码已公开。

**Conclusion:** egenioussBench通过结合实际可部署的地理空间资产和高精度真值数据，为推进大规模视觉定位研究提供了一个严格、可扩展的评估平台，能够有效测试和比较不同定位方法在真实城巿环境中的性能。
...
> 该论文为阿拉伯手语创建了首个高质量3D参数化注释集，并开发了专门的手语重建管道Tamaththul3D，显著提升了手势重建精度。

<details>
  <summary>Details</summary>

**Motivation:** 阿拉伯手语服务全球约4亿阿拉伯语使用者，但社区缺乏高质量的3D参数化注释和专门的虚拟形象生成重建方法，阻碍了无障碍技术的发展。

**Method:** 1) 为沙特手语数据集Ishara-500创建首个3D参数化注释，提供500个标准SSL手势的SMPL-X参数；2) 提出Tamaththul3D重建管道，集成SMPLer-X进行身体估计、WiLoR进行手部精细优化、MediaPipe进行2D姿态监督，采用混合摆转分解和运动学链对齐技术。

**Result:** Tamaththul3D在手部重建精度上达到最先进水平，比之前的方法提升32%，同时保持有竞争力的身体姿态估计性能。

**Conclusion:** 该工作建立了首个高保真阿拉伯手语虚拟形象重建的完整框架，为阿拉伯听障社区的无障碍技术发展和文化保护提供了重要支撑。
...
> 本文提出一种基于一致性学习的扩散模型优化方法，将点云异常检测的推理速度提升80倍，同时保持高检测精度。

<details>
  <summary>Details</summary>

**Motivation:** 随着3D感知在现代制造业中的重要性日益增加，现有的扩散模型方法存在计算开销大、迭代去噪过程缓慢的问题，难以在资源受限的边缘设备上实现低延迟部署。

**Method:** 通过一致性学习重构基于重建的异常检测框架，实现1-2次网络评估直接预测无异常几何形状，并引入混合损失函数强制重构清洁数据。

**Result:** 在Anomaly-ShapeNet上达到76.20% I-AUROC，超越R3D-AD方法；在Real3DAD上保持72.80% I-AUROC的竞争力；推理速度比现有最佳方法快80倍。

**Conclusion:** 该方法实现了高效、低延迟的点云异常检测，使其能够在资源受限平台如无人机、智能工业相机等边缘设备上部署应用。
...
> LAMP是一个用于从第一视角多摄像头头戴设备中追踪3D人体运动的新框架，通过早期解除观察者和目标运动的关联，实现了在动态环境下的稳健追踪。

<details>
  <summary>Details</summary>

**Motivation:** 现有的单目视频方法无法有效处理第一视角多摄像头头戴设备面临的严重自身运动、部分可见性或遮挡以及训练数据缺乏等问题，这些方法通常需要静态或缓慢移动的摄像头，导致在动态第一视角捕捉中容易失效。

**Method:** LAMP采用两步流程：首先利用已知的设备6自由度运动和校准信息，将所有摄像头在时间窗口内检测到的2D身体关键点转换为统一的3D世界参考坐标系；然后通过端到端训练的时空变换器直接拟合3D人体运动到这个3D射线云。这种"先提升后拟合"的方法让LAMP能够在世界空间中学习和利用自然的人体运动先验。

**Result:** LAMP在单目基准测试上达到了最先进的结果，在目标第一视角设置下显著优于基线方法，证明了其在处理多个时间异步、部分观察和移动摄像头信息方面的有效性。

**Conclusion:** LAMP框架通过早期解除观察者和目标运动的关联，优雅地解决了第一视角多摄像头人体运动追踪的挑战，为在动态环境中进行稳健的3D人体追踪提供了新的解决方案。
...
> GeoQuery是一个零样本卫星图像检索系统，通过提示对齐的文本代理，在无需完整对比训练的情况下实现了语义搜索功能。

<details>
  <summary>Details</summary>

**Motivation:** 地球观测档案的语义搜索仍然具有挑战性。现有的视觉基础模型如CLAY虽然能产生丰富的卫星图像嵌入，但缺乏自然语言基础支撑，而全面的遥感CLIP风格模型训练需要大规模成对数据和计算资源，在全球范围内不可用。

**Method:** GeoQuery采用提示对齐的文本代理方法：为10万个全球Sentinel-2代理子集生成语言描述，优化描述生成提示，使文本嵌入空间距离与冻结CLAY视觉嵌入空间距离相关。查询分为两阶段：先对代理子集进行文本相似性搜索，再对全球CLAY嵌入进行视觉最近邻搜索。

**Result:** 在涵盖英国洪水、美国野火和美国干旱的76个灾害位置查询上，GeoQuery在50公里范围内达到31.6%的准确率，其中洪水表现最佳（50公里内50%）。在ECHO危机响应系统中部署后，成功识别了2025年布里斯班Cyclone Alfred期间的脆弱区域。

**Conclusion:** 当完整对比训练不可行时，提示对齐代理为地球观测基础模型和操作性检索之间提供了实用的桥梁，在资源受限场景下实现了有效的语义搜索功能。
...
> 本文提出了一种前瞻性的相机可靠性监控框架GSHI，能在下游任务失败前通过分析退化引起的特征不确定性来估计感知风险，为安全关键ADAS系统提供早期预警。

<details>
  <summary>Details</summary>

**Motivation:** 现有的相机监控方法主要在下游性能已经下降后才检测到传感器故障，缺少前瞻性预警能力，这对于安全关键的ADAS感知系统来说是不足够的。

**Method:** 提出全局传感器健康指数GSHI，使用风险感知的乘法公式聚合各退化模式的严重程度；构建轻量级多任务网络从单张RGB图像预测退化类型、严重程度、GSHI和空间不确定性图谱；使用物理和几何感知的合成监督训练，覆盖12种相机退化模式。

**Result:** GSHI在KITTI派生退化数据上表现优异：健康度随严重程度单调递减，健康估计MAE为0.064，在YOLOv8检测失败前提供0.47±0.25严重度单位的提前预警时间；性能超越IQA、检测器置信度和干净特征OOD基线，并能零样本迁移到真实恶劣天气驾驶数据。

**Conclusion:** 基于退化感知的不确定性分析是实现智能车辆前瞻性相机可靠性监控的有效方法，该框架为安全关键ADAS系统提供了实用的解决方案。
...
> EchoXFlow是一个保留原始采集几何形状的临床超声心动图数据集，包含37,125个记录，支持跨模态、物理导向的多模态学习任务。

<details>
  <summary>Details</summary>

**Motivation:** 现有公开数据集缺乏研究心脏解剖、心肌运动和血流之间跨模态关系的机会，因为多普勒数据通常缺失或融合为RGB叠加，且采集数据经过有损的供应商显示处理。

**Method:** 构建包含666例常规护理检查的EchoXFlow数据集，保留每个记录的时序、几何和模态关系，将数据分离为特定模态的流：时间分辨的1D、2D、3D数据及多普勒模态，配合同步ECG，并提供从指导性测量到密集心肌轮廓和3D左心室内膜网格的临床标注。

**Result:** 创建了支持从原始采集几何形状而非扫描转换笛卡尔视频中学习的超声数据集，配合开源工具，实现无法从传统视频单独构建的跨模态、采集感知学习任务。

**Conclusion:** EchoXFlow为4D视觉和物理导向多模态学习提供了测试平台，解决了现有数据集在跨模态关系研究方面的局限性。
...
> 本文分析了首次可控虚化渲染挑战赛的结果，8个团队提交了解决方案，主要采用量化和定性评估方法，多数参与者基于Bokehlicious基线方法进行改进。

<details>
  <summary>Details</summary>

**Motivation:** 举办NTIRE可控虚化渲染挑战赛旨在推动虚化渲染技术的发展，通过竞赛形式探索更有效的算法和处理复杂虚化现象的能力。

**Method:** 设立两个评估赛道：第一赛道使用定量保真度指标评估，第二赛道通过专家小组进行定性感知评估；所有提交的解决方案都在未知图像上进行测试。

**Result:** 44人注册参赛，最终8个团队提交有效方案；由于是首次挑战，大多数参与者选择完善和扩展Bokehlicious基线方法。

**Conclusion:** 作为该主题的首次挑战赛，获得了宝贵的参与反馈和技术成果，为未来虚化渲染研究奠定了基础，同时也发现了基于现有基线方法改进的趋势。
...
> 本研究使用AlphaEarth基金会模型的卫星嵌入分析巴西圣保罗州1,729个恢复点，提出参考轨迹嵌入概念评估森林恢复效果，发现嵌入空间可有效区分不同土地利用类型，但信号仍存在噪声。

<details>
  <summary>Details</summary>

**Motivation:** 巴西大西洋森林是关键的生物多样性热点，但仅存12-15%的原始覆盖。传统监测方法受到地面调查不可行和NDVI等遥感指数饱和的限制，且森林恢复是渐进过程，难以准确评估。

**Method:** 使用AlphaEarth基金会的卫星嵌入模型分析圣保罗州1,729个恢复点，引入

**Result:** 在嵌入空间中观察到根据不同土地利用类型形成的明显聚类，能够识别出具有清晰变化向量的点位，但信号存在噪声。

**Conclusion:** 卫星嵌入技术可有效表征早期恢复成功度，显示区分不同土地利用类型的潜力，但需要进一步微调以更好地捕获和预测除土地利用类型外的点位元数据。
...
> 提出了一种新颖的图调节解耦稀疏Mamba模型(GDS-Mamba)，用于改进基于MODIS时间序列数据的树种分类任务。

<details>
  <summary>Details</summary>

**Motivation:** 树种分类对环境应用至关重要，但面临三大挑战：树种间细微特征差异、强时空谱信息耦合以及大规模拓扑上下文建模困难。

**Method:** 1)设计mini-batch图调节方法改善大规模上下文建模；2)提出新颖的解耦Mamba架构，独立捕获空间模式、光谱特征和时间物候行为；3)设计稀疏token方法自适应学习最优token子集，解决标准Mamba模型的关联衰减瓶颈。

**Result:** 在加拿大两个省份的大规模MOD13Q1数据实验中，艾伯塔省整体准确率达93.94%，跨省评估达80.19%，超越了12个最先进的分类模型。

**Conclusion:** GDS-Mamba模型通过图调节、信息解耦和稀疏学习三大创新技术，有效解决了树种分类的关键挑战，在真实大规模 datasets上验证了其优越性能。
...
> 研究发现，使用粗粒度监督信号训练的神经网络（如8个类别）能产生与人类视觉系统高度对齐的表征，甚至优于细粒度监督和自监督方法。

<details>
  <summary>Details</summary>

**Motivation:** 探索监督信号的粒度如何影响人工神经网络与灵长类视觉系统的表征对齐，目前这一问题尚未被系统研究。

**Method:** 使用数据驱动方法将训练图像划分为不同粒度的类别（2至64类），在卷积和变换器架构上训练数百个网络，并与猕猴电生理记录和人类fMRI响应进行比较。

**Result:** 粗粒度训练的网络（特别是8类）在神经对齐和感知相似性判断方面表现优越，超越了1000类分类、自监督和大规模视觉模型。

**Conclusion:** 人类视觉表征可以从极粗的反馈中涌现，这重新定义了视觉学习所需信号的理解，为构建更接近人类感知的AI系统开辟了新路径。
...
> 本文提出了一种基于文本检索CAD模型的统一框架，通过学习多模态嵌入式表示，结合程序化序列和几何点云特征，实现高效的跨模态检索。

<details>
  <summary>Details</summary>

**Motivation:** 现有CAD模型库检索主要依赖文件名或目录搜索，限制了设计重用的效率、可扩展性和准确性。文本到CAD检索作为新的跨模态任务尚未得到充分探索，但具有重要的工业应用价值。

**Method:** 提出统一框架包含三个编码器：序列编码器捕获CAD模型的构造逻辑，点编码器提取几何特征，文本编码器学习语义表示。训练时引入特征解码器通过交叉注意力重建掩码序列特征以促进多模态对齐，推理时移除解码器实现高效检索。

**Result:** 基于Text2CAD数据集建立了实用的评估基准，提出的框架在文本到CAD检索任务上表现出色，为CAD生成等下游任务奠定了基础。

**Conclusion:** 该框架首次系统性地解决了文本到CAD检索问题，为工业设计重用提供了新的解决方案，开源实现将促进该领域的发展。
...
> 本文提出了一种不确定性引导的边缘学习算法(UGEL)，通过深度贝塔回归高效计算预测不确定性，从而在资源受限的边缘设备上加速模型训练收敛。

<details>
  <summary>Details</summary>

**Motivation:** 边缘设备的计算限制不仅影响模型优化，还阻碍了对未标记数据计算预测不确定性，而这对于指导模型更新至关重要。现有的不确定性估计方法要么计算成本过高，要么对预测分布假设过于严格。

**Method:** 提出基于深度贝塔回归的UGEL算法，使用深度网络估计贝塔分布参数，在单次前向传播中计算预测不确定性，从而准确优先选择数据进行模型更新。该方法适用于遥感卫星上的深度图像回归任务。

**Result:** 实验结果表明，UGEL比主动学习和半监督学习提供更快收敛的边缘学习效果。深度贝塔回归相比传统方法计算更高效且允许更一般的预测分布。

**Conclusion:** UGEL算法成功解决了边缘设备上的不确定性计算和训练效率问题，为资源受限环境下的边缘学习提供了有效解决方案，代码和模型已开源。
...
> RAM-H1200是首个大规模公开数据集，包含1200张手部X光片，支持骨骼结构分割、骨侵蚀像素级标注和临床评分的统一分析。

<details>
  <summary>Details</summary>

**Motivation:** 现有的类风湿关节炎(RA)评估资源缺乏全手覆盖、细粒度标注，特别是骨侵蚀(BE)的定量分析标注稀缺，无法支持统一的多层次分析。

**Method:** 收集来自6个医疗中心的1200张手部X光片，提供多层级标注：(i)全手骨骼结构实例分割，(ii)像素级BE掩码，(iii)SvdH定义的关节区域，(iv)关节级BE和 JSN的SvdH评分。

**Result:** 基准测试显示，骨骼分割已达到较强性能，而BE分割仍是主要挑战。RAM-H1200是首个支持全手骨骼分割、像素级BE勾勒和临床SvdH评分的公开数据集。

**Conclusion:** RAM-H1200通过统一解剖结构建模、定量病变分析和临床SvdH评分，为手部X光片的综合RA分析提供了单一基准，推动了骨侵蚀定量分析的发展。
...
> 本研究提出了一种利用视觉语言模型生成合成数据来解决森林物种识别中数据稀缺和类别不平衡问题的框架，通过结合AI生成数据与真实数据，将语义分割性能提升超过15个百分点。

<details>
  <summary>Details</summary>

**Motivation:** 可持续森林管理需要精确的物种组成测绘，但传统地面调查费时费力且受地理限制。无人机提供了可扩展的数据收集方式，但深度学习方法面临专家标注图像严重稀缺的瓶颈，特别是在复杂且视觉异质的再生区域。

**Method:** 利用大规模视觉语言Nano Banana Pro模型，根据提示同时生成高保真图像及其对应的像素对齐语义掩码。创建了WilDReF-Q-V2数据集（包含13,977张未标注和50张标注真实图像）和Gen4Regen数据集（包含2,101对合成图像和语义掩码），将真实世界数据与AI生成图像进行整合训练。

**Result:** 统一训练相比纯监督基线F1分数提升超过15个百分点，少量提示生成数据就能显著改善代表性不足物种的性能，某些物种的F1分数提升高达30个百分点，证明AI生成数据与真实数据具有高度互补性。

**Conclusion:** 视觉语言模型可以作为敏捷的数据生成器，有效启动专家标签稀缺或不可用的利基AI领域的感知任务，为解决数据稀缺问题提供了新的思路。
...
> 本研究提出了一种基于智能手机的野外高质量人脸外观捕获方法，通过训练去光照网络作为先验约束，显著提升了复杂环境下的反射率估计质量，并开源了模型和大规模可重光照数据集。

<details>
  <summary>Details</summary>

**Motivation:** 传统高质量人脸外观捕获需要昂贵的摄影棚设备，而基于智能手机的野外方法在复杂未知光照下难以有效分离反射率和光照信息，限制了其实用性。

**Method:** 提出训练强大的去光照网络作为优化先验，通过数据集潜在调制(DLM)技术整合OLAT数据集和Light Stage扫描数据，使用可学习的源感知标记来解耦数据集特定风格和物理去光照原理。

**Result:** 构建了简单自动的外观捕获流程，在 casually视频输入上实现高质量反射率估计，大幅超过现有方法；将多视角NeRSemble数据集转换为4K分辨率的可重光照扫描数据集NeRSemble-Scan。

**Conclusion:** 该方法显著降低了高端人脸捕获的技术门槛，通过开源模型和数据集推动数字人研究社区发展，为构建逼真数字人提供了新的基础资源。
...
> 本文提出了VQAU任务和相应框架，用智能体框架AffectSeek解决模糊查询驱动的长视频情感理解问题，通过多步推理实现情感时刻定位、分类和可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 现有情感理解研究主要关注图像、音频或预剪裁视频片段的被动情感识别，无法真实反映用户在实际场景中通过自然语言查询与长视频交互的需求，缺乏模糊查询驱动的主动情感理解能力。

**Method:** 1. 构建VQAU-Bench基准数据集，整合长视频、模糊情感查询、时序片段标注、情感标签和解释说明；2. 提出AffectSeek智能体框架，将VQAU分解为意图解析、候选定位、片段验证、情感推理和解释生成五个步骤，通过专业角色推理和跨阶段验证逐步对齐模糊用户意图与长视频证据。

**Result:** 实验表明VQAU对现有情感识别模型和单步视觉语言模型仍具挑战性，而AffectSeek为主动式长视频情感理解提供了一个简单有效的框架，能够实现情感时刻的准确定位、分类和证据 grounded 的解释生成。

**Conclusion:** VQAU任务和AffectSeek框架为长视频情感理解研究开辟了新方向，通过主动式智能体方法和多步推理机制，有效解决了模糊查询下的情感理解挑战，提升了实际应用中的用户体验和可解释性。
...
> MUSE框架通过拓扑正交性解决了视觉标记化中保真度重建与语义抽象之间的根本冲突，实现了生成质量和表征理解的双重突破。

<details>
  <summary>Details</summary>

**Motivation:** 统一视觉标记化面临高保真像素重建（空间等变性）和语义抽象（概念不变性）之间的基本权衡。这种冲突源于流形错位：朴素的联合优化会产生相反的梯度，导致重建和感知之间形成零和博弈。

**Method:** 提出基于拓扑正交性的MUSE框架，将结构作为正交桥梁，在Transformer内解耦优化：结构梯度优化注意力拓扑，语义梯度更新特征值，从而将破坏性干扰转化为相互增强。

**Result:** 实验表明MUSE打破了权衡，获得最先进的生成质量（gFID 3.08），在线性探测任务上超越其教师模型InternViT-300M（85.2% vs 82.5%）。

**Conclusion:** 证明了结构对齐的重建可以增强语义感知，为视觉标记化的统一优化提供了新的解决路径。
...
> S2C-3D是一个新颖的稀疏视图3D重建框架，仅需6-8张输入图像即可实现高保真度和完整性的场景重建，通过专用扩散模型、视图一致性条件采样和相机轨迹规划三个核心组件来实现。

<details>
  <summary>Details</summary>

**Motivation:** 现有的稀疏视图3D重建方法在面对极少输入图像时往往会产生缺失区域、模糊和其他伪影问题，缺乏完整性和高保真度。S2C-3D旨在解决这一挑战，实现从极稀疏输入中重建高质量完整的3D场景。

**Method:** 框架包含三个关键组件：1) 专用扩散模型，通过在输入视图及其退化对应物上微调预训练架构来修复高斯渲染并消除领域差距；2) 无训练的视图一致性条件采样过程，量化相邻修复图像之间的一致性并将其作为条件注入扩散模型采样过程；3) 相机轨迹规划方案，通过连接每个新采样相机与其两个最近邻居并保留显著增强可见性的路径来建立覆盖整个场景的轨迹。

**Result:** 实验结果表明S2C-3D在性能上优于最先进方法，能够从极稀疏输入构建高质量场景，生成的3D高斯分布无缺失区域、模糊或其他伪影，对伪影具有鲁棒性。

**Conclusion:** S2C-3D成功实现了从6-8张极稀疏输入图像到高保真完整3D场景的重建，通过场景感知的扩散模型适配、视图一致性约束和智能轨迹规划，为稀疏视图重建提供了有效解决方案。
...
> EGA是一种新型的残差适配器，通过零初始化、局部三元组损失和超球投影三个原则，在高容量适配器训练中保持未知类样本的精度，在OOD评估中达到最优的最坏情况标签精确度。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于冻结视觉编码器的向量搜索系统在部署时面临未知类查询，而高容量适配器在全局对比损失训练下会错误地将未知类样本重新分配到已知类簇，导致最坏情况下标签精确度下降40多个百分点。

**Method:** 提出Euclidean Geodesic Alignment (EGA)残差适配器，结合三个关键技术：1) 零初始化确保适配器从原始空间开始微调；2) 局部三元组损失只对不满足小边界的样本产生梯度；3) 超球投影约束特征空间。这种设计实现了自限制动态，当局部几何已经正确时适配器自动停止更新。

**Result:** 在收敛时96.5%的三元组不产生梯度，保持了未知类区域的完整性，同时充分优化已知类样本。在5个多样化的OOD基准测试中，EGA在4个主要分割上获得最高最坏情况标签精确度，第5个也有稳定提升。设计同样适用于CLIP之外的更强骨干网络。

**Conclusion:** EGA通过梯度稀疏性与有界OOD扰动的理论联系，成功解决了高容量适配器在未知类别上的性能崩溃问题，为现实世界中的开放集向量搜索提供了可靠的解决方案。
...
> MotionGRPO是一种使用强化学习后训练来改进基于扩散的3D人体运动恢复的新框架，通过引入细粒度指导和噪声注入策略，解决了现有方法中局部关节重建误差的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于扩散的3D人体运动恢复方法依赖全局分布匹配，导致局部关节重建存在误差。因此需要一种能够注入细粒度指导来提高局部精度的方法。

**Method:** 提出MotionGRPO框架，将扩散采样建模为马尔可夫决策过程，通过群体相对策略优化(GRPO)进行优化。引入混合奖励机制，结合学习的条件感知模型（全局视觉合理性）和显式约束（局部关节精度）。采用噪声注入策略增加样本方差，解决梯度消失问题。

**Result:** 在大量实验中，MotionGRPO在视觉保真度方面达到了最先进的性能表现，有效减少了局部关节重建误差。

**Conclusion:** 通过结合强化学习和扩散模型，MotionGRPO成功地为3D人体运动恢复注入了细粒度指导，创新的噪声注入策略稳定了学习过程，显著提升了整体重建质量。
...
> 提出R2H-Diff，一个高效的扩散模型框架，用于RGB到高光谱图像重建，在保持高重建质量的同时显著降低了模型复杂度。

<details>
  <summary>Details</summary>

**Motivation:** RGB到高光谱重建是高度不适定的逆问题，现有回归方法通常学习确定性映射，限制了其对重建不确定性的建模能力，容易导致光谱响应过度平滑。

**Method:** R2H-Diff将光谱恢复表述为条件迭代细化过程，采用引导光谱细化模块进行RGB条件特征融合，高光谱自适应转置注意力模块建模空间-光谱依赖关系，使用无归一化去噪骨干保持光谱幅度一致性，并采用任务自适应线性噪声调度仅需5步去噪。

**Result:** 在NTIRE2022、CAVE和Harvard数据集上广泛实验表明方法在重建质量和计算效率间取得良好平衡，在NTIRE2022上达到35.37dB PSNR，模型仅0.58M参数和12.25G FLOPs。

**Conclusion:** R2H-Diff成功解决了扩散模型在高光谱重建中的挑战，在保持强重建保真度的同时实现了最低的模型复杂度，为高光谱重建提供了新的高效解决方案。
...
> 本文提出了首个面向压缩友好性的隐私保护动作识别方法CFE-PPAR，该方法在视频加密后仍能保持较好的识别性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于加密的隐私保护动作识别方法在视频压缩时会导致识别性能和视觉质量的急剧下降，缺乏压缩友好性。

**Method:** 提出CFE-PPAR方法，使用相同的密钥对视频进行加密，并对视频Transformer的参数进行相应变换，使加密后的视频可直接进行识别。

**Result:** 在UCF101和HMDB51数据集上，CFE-PPAR在Motion-JPEG和H.264压缩条件下均优于之前的的方法。

**Conclusion:** CFE-PPAR成功解决了加密视频在压缩后性能下降的问题，实现了压缩友好性的隐私保护动作识别。
...
> 本文提出了一种面向视频情感识别的主体不变跨模态提示调优框架，通过将rPPG波形转换为抗噪的时频表示来生成提示，调制冻结的ViT面部特征，并引入解耦共享-特定适配器来分离主体共享和特定成分。

<details>
  <summary>Details</summary>

**Motivation:** 面部表情无法完全反映内在情感状态，而rPPG作为互补的生理信息虽然有价值，但容易受到噪声和受试者个体差异的影响，限制了在未见个体上的泛化能力。现有多模态方法的融合策略常常破坏预训练的面部表示，且缺乏明确的机制来抑制个体特定的变化。

**Method:** 提出了一个跨模态提示调优框架：1）将rPPG波形转换为抗噪的时频表示；2）从TFR生成模态互补的提示来调制ViT中的面部token；3）在ViT各层引入解耦共享-特定适配器来显式分离主体共享和特定成分。框架保持预训练ViT冻结，有效进行跨模态交互。

**Result:** 在MAHNOB-HCI和DEAP基准测试上的实验表明，所提方法在识别准确性和泛化能力方面始终优于强基线方法，验证了其在视频情感识别中的有效性。

**Conclusion:** 通过抗噪TFR转换、跨模态提示调优和解耦适配器设计，所提框架成功解决了rPPG噪声敏感性和个体差异问题，在保持预训练表示泛化能力的同时实现了有效的多模态融合，为视频情感识别提供了一种新的有效解决方案。
...
> 本文提出了一个统一框架，将语言驱动的3D场景生成与沉浸式用户交互紧密结合，通过闭环反馈机制提升多媒体系统的适应性和沉浸感。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型3D内容生成方法将场景生成和用户交互分离处理，限制了交互式多媒体系统的适应性和沉浸潜力，无法实现真正的实时响应和用户体验优化。

**Method:** 系统首先使用LLM构建结构化场景表示，然后在几何和语义约束下通过强化学习优化空间布局，生成的环境部署在VR中实现人机交互闭环，用户交互反馈持续调整生成内容以符合人类感知和可用性。

**Result:** 在ALFRED基准测试中实现了任务型场景生成的最先进性能，定性实验和用户研究表明沉浸感、交互质量和任务效率均有显著改善。

**Conclusion:** 通过紧密耦合生成与交互的闭环集成，该框架为下一代多媒体系统提供了更响应式、自适应和真实的体验，验证了生成与交互闭环整合的重要性。
...
> EgoEMG是首个同步肌电图和视觉数据的双手姿态估计多模态数据集，包含41名参与者60个手势类别的10小时数据，并建立了EMG到姿态、视觉到姿态和多模态融合的评测基准。

<details>
  <summary>Details</summary>

**Motivation:** 表面肌电图和视觉在手势感知中具有互补性：EMG能捕获遮挡和恶劣光照下的精细手指运动，而视觉提供全局手部配置，但现有数据集缺乏这两种模态的同步记录。

**Method:** 构建了包含双侧腕带EMG（16通道，2kHz）、IMU（120Hz）、宽视角RGB视频、外部RGB-D视频和动作捕捉手部运动的多模态数据集，涵盖30个单手和30个双手手势；设计了跨手势、跨用户和组合的泛化评估基准，提出了残差融合架构。

**Result:** 数据集包含41名参与者的60个手势类别，总时长超过10小时；基线实验显示残差融合架构相比轻量级视觉基线有改进，为多模态手部姿态估计研究奠定了基础。

**Conclusion:** EgoEMG数据集及其基准框架填补了多模态手部姿态估计领域的空白，为EMG和视觉融合的后续研究提供了重要资源和评估标准。
...
> TriRelVLA是一个三元关系视觉-语言-动作模型框架，通过构建物体-手部-任务的三元关系表示，显著提升了机器人操作在跨场景、跨物体和跨任务上的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的VLA模型在训练过的任务上表现良好，但在未见过的场景和物体上泛化能力差，主要原因是其隐式的视觉表示将物体外观、背景和场景布局纠缠在一起，导致策略对视觉变化敏感。之前的工作通过结构化中间表示改善了可迁移性，但主要捕获场景语义而非动作相关的关系。

**Method:** 提出三元关系框架，包含三个核心组件：1)从多模态输入构建显式的物体-手部-任务三元关系表示作为关系基元；2)构建任务 grounded的关系图，通过任务引导的交叉注意力形成节点，使用关系感知图变压器建模节点间交互；3)执行关系条件动作生成，将关系结构压缩到瓶颈空间并投影到大语言模型中进行动作预测。

**Result:** 实验表明，模型在微调任务上表现强劲，并在跨场景、跨物体和跨任务泛化方面取得明显提升。还引入了真实世界机器人数据集用于微调。

**Conclusion:** 三元关系瓶颈减少了对外观统计的依赖，实现了跨场景、物体和任务组合的有效迁移，为可泛化的具身操作提供了新的解决方案。
...
> 本文提出了B³-Net，一个受控后验桥学习框架，通过显式建模任务间证据的可靠性变化来解决多任务密集预测中的负迁移问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多任务密集预测方法在解码器端交互时通常隐式地组织证据，通过相似性或亲和性融合任务特征，没有显式建模证据可靠性在任务和空间位置上的变化，导致不可靠证据可能污染共享表示并加剧负迁移。

**Method:** B³-Net将解码器端交互分解为三个组件：1) 精度场估计器从任务-参考对齐和局部变化估计证据精度；2) 后验桥算子通过异质证据融合构建精度加权的后验桥；3) 收缩分发算子通过有界更新将桥重新分配到各任务分支。

**Result:** 在NYUD-v2、PASCAL-Context和Cityscapes数据集上的实验表明，B³-Net相比CNN、Transformer、扩散、Mamba和桥特征等方法具有竞争性或更优的性能权衡。主干匹配的比较和广泛分析验证了性能提升来源于受控后验桥学习而非主干容量或解码器规模。

**Conclusion:** 通过显式建模证据可靠性并构建受控的后验桥，B³-Net有效避免了不可靠证据的污染，改善了多任务密集预测中的知识共享，为多任务学习提供了一种新的交互范式。
...
> 提出一种光线感知指针记忆机制，通过联合建模空间位置和视角方向，实现稳定高效的流式3D重建，有效解决传统外观相似性方法中的冗余累积和视角变化问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于外观相似性的记忆更新方法在连续图像流3D重建中存在累积冗余观测和视角变化导致的几何不稳定性问题

**Method:** 设计光线感知指针记忆，存储3D位置、光线方向和特征嵌入，采用保留或替换的自适应策略替代传统融合压缩

**Result:** 显著提升了长期重建稳定性和相机位姿精度，同时保持高效的流式推断

**Conclusion:** 为可扩展、抗漂移的在线3D重建提供了一个原则性框架，能够统一区分局部冗余、新颖观测和闭环重访
...
> TDSC是一种时空一致性深度自表达子空间聚类方法，通过联合学习结构化表示和稳定亲和力来解决人体运动分割中特征分布不符合子空间假设的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有人体运动分割方法基于子空间聚类假设，但现实视频的原始帧级特征常违反联合子空间分布假设，导致分割效果不理想。

**Method:** 通过正则化自表达模型交替学习结构化表示和自表达系数，引入编码率最大化正则器避免表征坍塌，加入时间约束促进相邻帧同组分割，开发时间动量平均机制稳定亲和力演化，使用重参数化策略实现高效优化。

**Result:** 在五个标准人体运动分割数据集上使用传统特征(HoG)和最新深度特征(CLIP, DINOv2)进行广泛实验，验证了方法的有效性。

**Conclusion:** TDSC方法通过时空一致性学习和稳定优化机制，有效解决了特征分布不符合子空间假设的问题，实现了准确鲁棒的人体运动分割。
...
> iTRIALSPACE是一个可编程的肺部CT模型评估框架，通过合成虚拟病灶试验来解决传统基准测试中病灶特征混合的问题，实现了可控、可验证的模型评估。

<details>
  <summary>Details</summary>

**Motivation:** 传统的肺部CT评估基准是静态回溯数据集，将病灶大小、肺叶分布、解剖结构和采集上下文等因素混合在一起，难以确定真正驱动模型准确性的结构因素。

**Method:** 该框架采用四阶段流水线：多数据集病灶分析、明确试验规范、解剖感知掩码插入和ControlNet条件CT合成。基于包含13,140个注释病灶的54属性统一数据集，构建了13种试验模式。

**Result:** 在55,469样本的虚拟病灶研究中，合成数据保持在真实数据的FID基线范围内，合成性能排名与真实临床数据高度相关(ρ = 0.93)，揭示了基准测试无法发现的现象。

**Conclusion:** iTRIALSPACE为医学影像AI模型提供了一个可审计的评估基础设施，实现了超越静态回溯基准的可控、可证伪测试，能够暴露模型的潜在缺陷和偏见。
...
> X-OmniClaw是一个统一的多模态移动代理系统，通过融合感知、记忆和行动三大核心模块，在Android生态系统中实现了高上下文感知的复杂任务处理能力。

<details>
  <summary>Details</summary>

**Motivation:** 受OpenClaw发展启发，市场对能够处理复杂直观交互的移动端个人代理需求日益增长，需要构建一个能够在移动原生环境中提供多模态理解和交互的统一解决方案。

**Method:** 采用统一架构设计，包含Omni Perception（多模态感知流水线，整合UI状态、实时视觉和语音输入）、Omni Memory（多模态记忆优化，结合运行工作记忆和长期个人记忆）、Omni Action（混合定位策略，结合XML元数据和视觉感知），并通过行为克隆和轨迹回放捕获用户导航技能。

**Result:** 在多样化场景下的演示表明，X-OmniClaw有效提升了交互效率和任务可靠性，为下一代移动原生个人助手提供了实用的架构蓝图。

**Conclusion:** X-OmniClaw通过创新的统一架构成功整合了多模态感知、记忆和行动能力，为移动端个人代理的发展提供了重要技术贡献，具有良好的应用前景。
...
> MICCAI 2024第三届autoPET挑战赛评估了全自动PET/CT病灶分割算法的组成泛化能力，最佳算法DSC达到0.66，但未见过的示踪剂-中心组合仍是主要挑战。

<details>
  <summary>Details</summary>

**Motivation:** 评估PET/CT病灶分割算法在组成泛化设置下的性能，特别是处理未见过的示踪剂-中心组合的能力，以推动医学图像分割技术的发展。

**Method:** 基于1,611例大型多示踪剂PET/CT数据集进行训练，包含1,014例[18F]-FDG和597例PSMA病例，通过200例测试集评估四种示踪剂-中心组合，其中两种为未见过的新组合。

**Result:** 17个团队提交27个算法，最佳算法平均DSC为0.66，比基线提高8%，假阴性体积减少5mL。在域内多示踪剂分割表现接近医生一致性，但对未见组合存在系统性体积高估。

**Conclusion:** 域内多示踪剂PET/CT分割已接近临床水平，但组成泛化仍是开放性问题；算法选择对性能影响较小，病例异质性和难度是性能变化的主要驱动因素。
...
> UNO是一个轻量级框架，通过将理解任务作为直接监督信号来引导生成表示，从而弥合多模态模型中理解与生成之间的差距。

<details>
  <summary>Details</summary>

**Motivation:** 现有最好的多模态模型采用了解耦的组件设计，虽然单个任务表现良好，但削弱了理解与生成之间相互增强的联系，使得潜在协同效应不确定。

**Method:** 提出面向理解的后训练框架(UNO)，融合语义抽象（图像描述）和结构细节（视觉回归）目标，实现从理解到生成的有效梯度流。

**Result:** 图像生成和编辑的大量实验表明，理解可以有效地催化生成任务，证明了方法的可行性和有效性。

**Conclusion:** 通过显式恢复理解与生成之间的协同关系，UNO框架为统一多模态模型的发展提供了新的思路，展示了理解引导生成的重要价值。
...
> Na-IRSTD是一个原生分辨率红外小目标检测框架，通过保留目标细节和有效的令牌选择策略实现了SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有红外小目标检测方法在下采样过程中会丢失小目标的细节信息，导致在复杂背景中精确定位暗淡目标时性能不佳。

**Method:** 提出Na-IRSTD框架，采用原生分辨率特征提取和融合来保留目标细节，并引入有效的令牌缩减和选择策略，在减少计算负担的同时选择高置信度的目标区域。

**Result:** 在多个公共数据集上的实验证明了令牌选择策略的鲁棒性和有效性，Na-IRSTD在四个基准测试中达到了最先进的性能。

**Conclusion:** 通过原生分辨率处理和智能令牌选择，Na-IRSTD成功解决了现有方法分辨率限制问题，显著提升了红外小目标检测能力。
...
> 研究发现医学视觉语言模型存在否定选项吸引问题，即使图像显示病灶也会选择否定答案，提出了CXR-ContraBench基准测试和QCCV-Neg修复方法。

<details>
  <summary>Details</summary>

**Motivation:** 当胸部X光显示实变但模型却回答'无实变'时，这不仅是错误选择，更是发出与图像矛盾的临床陈述，存在严重的临床风险。

**Method:** 1) 构建CXR-ContraBench基准测试，包含内部和外部协议；2) 评估MedGemma和Qwen2.5-VL模型在存在-发现和缺乏-发现问题上的表现；3) 测试思维链提示的影响；4) 提出QCCV-Neg方法进行确定性修复。

**Result:** 在严格的直接存在性测试上，MedGemma和Qwen2.5-VL准确率仅31.49%和30.21%；在135,754条记录的匹配协议上，两模型在超过62%的存在性问题上选择否定选项；QCCV-Neg方法可将准确率提升至96.60%和95.32%。

**Conclusion:** 标准准确率指标可能隐藏了临床上重要的极性混淆失败，QCCV-Neg方法无需重新训练即可修复这一问题，为医学AI可靠性提供了重要改进。
...
> ChartZero是一个零样本线图数据解析框架，通过仅使用合成数据训练，解决了真实世界标注稀缺问题，并提出了新的损失函数和视觉语言模型引导的图例匹配策略。

<details>
  <summary>Details</summary>

**Motivation:** 现有线图自动化数据提取面临三大挑战：极端的风格多样性和真实标注数据严重稀缺；曲线提取中的结构断裂和视觉细节丢失；基于空间启发式的语义关联容易出错；评估协议缺乏端到端的完整性。

**Method:** ChartZero采用纯合成数据训练，完全绕过真实标注瓶颈；提出全局正交实例(GOI)损失解决曲线断裂问题；使用开放词汇的视觉语言模型引导图例匹配，替代脆弱的空间规则；设计了新的端到端重建评估指标和基准。

**Result:** 实验表明ChartZero在端到端图表数字化方面显著提升了泛化能力，无需任何真实世界监督就能实现鲁棒的数据提取。

**Conclusion:** ChartZero通过合成数据先验和创新的架构设计，成功解决了线图数据提取中的标注瓶颈和关键技术挑战，为通用图表数字化提供了新的解决方案。
...
> 本研究提出了多模态会议数据集(MCD)，这是首个整合来自同一研究工作的论文、演示视频、讲解视频和幻灯片的数据集，并系统评估了多种模型在跨模态细粒度对齐任务上的表现。

<details>
  <summary>Details</summary>

**Motivation:** 科学知识传播日益多模态化，涵盖文本、视觉和语音等多种形式，但这些材料之间缺乏结构化连接，使得难以追踪概念、视觉和解释之间的对应关系，限制了对研究内容的统一探索和分析。

**Method:** 构建了多模态会议数据集(MCD)，整合研究论文、演示视频、讲解视频和幻灯片；评估了基于嵌入的模型和视觉语言模型在发现细粒度跨格式对应关系方面的能力，建立了该任务的系统性基准。

**Result:** 视觉语言模型稳健但难以处理细粒度对齐；基于嵌入的模型能很好地捕捉文本-视觉对应关系，但公式和符号内容在嵌入空间中形成独立簇；揭示了当前方法的优势和局限性。

**Conclusion:** 研究结果指出了多模态科学理解的关键方向，为未来研究提供了重要参考；通过开源数据集和资源促进了该领域的可重现性研究。
...
> VideoRouter是一个基于InternVL的查询自适应双路由框架，通过语义路由和图像路由实现预算有限的证据分配，在长视频处理中最多可减少67.9%的视觉token

<details>
  <summary>Details</summary>

**Motivation:** 长视频大模型面临可扩展性瓶颈：长视频产生过长的视觉token序列，导致推理时内存和延迟急剧增加。现有压缩方法要么查询感知度弱，要么在所有帧上应用固定压缩策略，当视觉证据在时间上分布不均时效果次优

**Method:** VideoRouter采用双路由架构：语义路由器预测主导分配策略，选择广时域覆盖或自适应高分辨率保存；图像路由器使用早期LLM层对帧相关性评分。构建了Video-QTR-10K数据集用于分配策略监督，Video-FLR-200K数据集用于帧相关性监督

**Result:** 在VideoMME、MLVU和LongVideoBench测试中，VideoRouter在可比或更低预算下始终优于InternVL基线，实现了高达67.9%的token减少

**Conclusion:** VideoRouter通过查询自适应的双路由机制有效解决了长视频处理中的性能瓶颈，实现了智能的视觉token压缩，在保持或提升模型性能的同时显著降低了计算资源消耗
...
> Align3D-AD是一种零样本3D异常检测框架，通过利用RGB模态作为跨模态指导，解决了现有方法中渲染特征与RGB预训练编码器之间的领域鸿沟问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的零样本3D异常检测方法主要依赖将3D观测投影到多视图表示，这些表示主要捕获几何线索而非真实视觉语义，并且使用在RGB数据上预训练的视觉编码器处理，导致编码器与投影表示之间存在显著的领域鸿沟。

**Method:** 提出了统一的两阶段框架Align3D-AD：1) 跨模态特征对齐范式，将渲染特征映射到RGB语义空间，实现从RGB观测的直接语义转移；2) 采用语义一致性重加权策略优化特征对齐；3) 提出模态感知的提示学习框架，通过双提示对比对齐捕获跨模态互补语义。

**Result:** 在MVTec3D-AD、Eyecandies和Real3D-AD数据集上的广泛实验表明，Align3D-AD在一对多和跨数据集设置下均一致优于现有的零样本方法，展现了出色的泛化能力和鲁棒性。

**Conclusion:** Align3D-AD通过创新的跨模态对齐和提示学习机制，有效解决了零样本3D异常检测中的领域鸿沟问题，为该领域提供了新的解决方案，具有广泛的应用前景。
...
> InkDiffuser是一个基于扩散模型的中文书法生成框架，通过高频增强机制和可微分墨迹结构损失来提升渲染质量和艺术流畅性。

<details>
  <summary>Details</summary>

**Motivation:** 现有中文书法生成方法存在笔画渲染质量差、墨迹形态不真实的问题，导致生成结果的视觉保真度和艺术流畅性有限。

**Method:** 提出基于扩散的生成框架InkDiffuser，包含两个核心贡献：1) 高频增强机制，通过融合高频表示来增强字体结构提取的准确性；2) 可微分墨迹结构(DIS)损失，将可微分形态学操作集成到扩散过程中，让模型学习墨迹结构的显式分解。

**Result:** 在多种书法风格和复杂字符上的大量实验表明，InkDiffuser仅需单个参考字形就能生成具有逼真墨迹渲染效果的优质书法字体，在结构一致性、细节保真度和视觉真实性方面优于现有的少数字体生成方法。

**Conclusion:** InkDiffuser通过高频增强和可微分墨迹结构损失的创新设计，有效解决了中文书法生成中的渲染质量和形态真实性问题，为高质量的书法字体生成提供了新的解决方案。
...
> ContactPrompt是一种基于多模态大语言模型的训练无关、零样本密集手部接触估计方法，无需训练即超越现有监督方法。

<details>
  <summary>Details</summary>

**Motivation:** 密集手部接触估计需要高层语义理解和细粒度几何推理，而现有MLLMs在3D手部几何编码和顶点级接触捕捉方面存在挑战，影响其在此任务的直接应用。

**Method:** 提出详细手部部件分割和基于部件的顶点网格表示来编码3D手部几何；开发多阶段结构化接触推理与部件调节机制，逐步连接全局语义与细粒度几何。

**Result:** 该方法无需任何训练，在密集手部接触估计任务上性能超越之前在大规模密集接触数据集上训练的监督方法。

**Conclusion:** ContactPrompt成功将MLLMs的推理能力应用于密集手部接触估计，通过创新的几何表示和推理机制，实现了训练无关的高性能解决方案。
...
> DBMSolver是一种免训练的采样器，利用指数积分器优化扩散桥模型，将函数评估次数减少5倍，同时提升生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 基于扩散的图像到图像转换技术在高质量生成方面表现优异，但最先进的扩散桥模型采样速度缓慢，通常需要数十次函数评估，限制了实际应用。

**Method:** 提出了DBMSolver，一种训练自由的采样器，通过指数积分器方法利用DBM底层随机微分方程和常微分方程的半线性结构，实现了高效的一阶和二阶求解方案。

**Result:** 在DIODE数据集上，20 NFEs时FID比二阶基线降低53%；在256x256分辨率的修复、风格化和语义到图像任务上，建立了新的效率-质量权衡基准。

**Conclusion:** DBMSolver显著提升了扩散桥模型的采样效率和质量权衡，使基于扩散的I2I转换技术在现实应用中更加可行，为高保真图像生成提供了更实用的解决方案。
...
> 该论文提出了一种基于多任务学习和混合专家模型(MoE)的医学图像异常检测方法，通过从头学习多个自监督和伪标签任务，在BMAD基准测试上超越了所有现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 医学图像异常检测具有挑战性，因为训练时通常无法获取异常样本。现有方法依赖单一前置任务和大规模预训练模型，存在局限性。

**Method:** 提出基于混合专家模型(MoE)的联合模型，从头学习多个自监督和伪标签代理任务，通过精心集成多任务来学习正常解剖结构的鲁棒表示，并根据推理时多任务学习者的表现推导异常分数。

**Result:** 在涵盖多种医学图像模态的BMAD基准测试上进行全面实验，结果表明该方法优于所有最先进的竞争对手，并能生成可解释的异常图。

**Conclusion:** 多任务学习者是一种有效的异常检测器，不仅性能优越，还能生成可解释的异常图，有潜力帮助医生提供更准确的诊断。
...
> MAST检测器利用SNNs处理多通道时间残差特征，在跨生成器AI视频检测中实现93.14%准确率，证明脉冲神经网络在AI生成视频检测中的实用性。

<details>
  <summary>Details</summary>

**Motivation:** 现有AI视频检测器在跨生成器评估中性能急剧下降，因为不同生成器的伪影类型和时间尺度差异很大，需要更鲁棒的检测方法。

**Method:** 通过SNNs处理时间残差信号，结合冻结的语义编码器，利用AI视频在像素级更平滑的时间残差和语义空间更紧凑轨迹特征，构建MAST检测器。

**Result:** 在GenVideo基准测试中，MAST在10个未见生成器的严格跨生成器评估下达到93.14%平均准确率，匹配或超越最强的基于ANN的检测器。

**Conclusion:** SNNs的事件驱动、稀疏激活特性与AI生成视频时间残差信号的结构天然匹配，为跨生成器AI视频检测提供了有效解决方案。
...
> 该研究发现，阿拉伯文字手写文本识别的跨语言改进主要来源于序列级建模而非视觉特征共享。

<details>
  <summary>Details</summary>

**Motivation:** 阿拉伯文字手写识别在低资源条件下通过跨语言联合训练获得改进，但不清楚这种改进是源于共享的视觉表示还是序列级依赖关系。

**Method:** 在阿拉伯语、乌尔都语和波斯语数据集上进行对照实验，比较纯CNN-CTC模型和CRNN模型在单脚本和多脚本训练下的表现，使用100-1000个样本的低资源设置。

**Result:** CRNN模型在多脚本训练下表现更好，特别是在数据最受限的情况下；纯CNN模型改进有限或不稳定。跨语言改进与序列级建模而非视觉特征共享相关。

**Conclusion:** 在低资源场景中，上下文建模对于实现有效的跨语言转移起着关键作用，这一发现可能适用于其他低资源语言环境。
...
> 本文提出了一种与架构无关的Lipschitz常数贝叶斯头部(LipB-ViT)，通过谱正则化和不确定性融合来处理结构化标签噪声，在视觉变换器上实现了显著的鲁棒性和标签检测性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 监督深度学习模型在处理结构化而非随机的标签噪声时泛化能力差，现有的鲁棒训练方法在语义相近的分类错误面前往往失效，这成为模型实用化的关键瓶颈。

**Method:** 开发了LipB-ViT方法，其核心是对贝叶斯层的均值和方差均施加谱归一化约束，提出联合不确定性和置信度评估指标，以及基于特征空间邻近性和预测不确定性的自适应算术平均融合方案。

**Result:** 在15%语义错误标签下，标签检测召回率达到93%以上，比最先进的k近邻方法提升超过7%；在结构化和非结构化噪声场景下均展现出优异的鲁棒性。

**Conclusion:** 该方法具有即插即用特性，可与预训练主干网络集成，跨领域使用统一超参数，为高重要性应用中的数据集质量和标签噪声联合评估提供了新的分析工具。
...
> 本文提出了CAKI框架，通过类感知知识注入机制，为现有提示学习方法补充类别特定知识，显著提升了视觉语言模型在零样本分类任务上的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有提示学习方法存在局限性：类别共享提示过于粗糙容易导致误分类，实例特定提示忽略了跨多个实例的丰富类别信息，可能将同类数据分割成多个类别。

**Method:** CAKI框架包含两个关键组件：1) 类别特定提示生成器，从少量同类样本中编码类别知识并存储在知识库中；2) 查询-键提示匹配机制，为测试实例检索相关类别知识并注入以优化预测。

**Result:** 在基础类别和新颖类别上的大量实验证明，CAKI能有效提升现有方法的性能，代码已开源。

**Conclusion:** CAKI作为一个即插即用的框架，成功解决了现有提示学习方法忽视类别特定知识的问题，为视觉语言模型的提示学习提供了新的解决思路。
...
> DeScore是一种新型视频奖励模型，采用'思考-打分'解耦范式，结合生成式推理和判别式打分的优势，提高训练效率和泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频奖励模型存在根本困境：判别式RM缺乏显式推理容易走捷径，生成式RM虽然推理能力强但训练不稳定，两者都无法满足视频生成模型的post-training和test-time scaling对高质量奖励模型的需求。

**Method:** DeScore采用解耦的'think-then-score'范式，第一阶段MLLM生成显式CoT，第二阶段专用判别式打分模块(包含可学习query token和回归头)预测最终奖励。通过两阶段优化：1) 判别式冷启动确保打分能力；2) 双目标强化学习独立优化推理质量和奖励校准。

**Result:** DeScore成功结合了CoT推理的泛化优势和判别式打分的稳定性，通过解耦设计克服了现有方法的训练困难，实现了训练高效且泛化能力强的视频奖励模型。

**Conclusion:** DeScore为视频奖励模型提供了一种新的解决方案，通过推理与打分的解耦设计，既保持了生成式模型的推理可视化优势，又避免了其训练不稳定的缺点，为视频生成模型的进一步发展奠定基础。
...
> 本文提出了一种针对目标检测后门攻击的防御框架，通过检测感知的对抗微调方法，在无需知道攻击目标的情况下，有效降低攻击成功率同时保持检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 目标检测系统的后门攻击威胁严重，但现有防御技术主要集中在图像分类领域，目标检测的后门防御相对不成熟。传统的对抗微调方法难以直接应用于目标检测，因为分类导向的对抗生成不匹配检测攻击空间，且标准检测损失会在多个预测中稀释修复信号。

**Method:** 提出了检测感知的对抗微调框架，包含两个关键技术：1) 软分支最小化方法，使用软门结合与误分类和消失攻击对齐的目标，以及检测感知的分类损失最大化；2) 双目标微调损失，仅应用于目标匹配的预测，集中防御更新在与后门行为最相关的预测上。

**Result:** 在CNN和Transformer检测器上的实验表明，相比分类导向的基线方法，所提方法能更有效降低攻击成功率，同时保持真实检测，并维持了有竞争力的清洁检测性能。

**Conclusion:** 该方法成功解决了目标检测后门防御的关键挑战，为在实际场景中保护安全关键的视觉系统提供了有效的解决方案，特别适用于防御者仅可获得被妥协检测器和小规模清洁数据集的情况。
...
> 本研究开发基于证据的大型语言模型集成系统，从35万份CT检查中过滤病理报告，构建了106个全身解剖结构的参考分布图表。

<details>
  <summary>Details</summary>

**Motivation:** 临床数据集通常富含病理病例，难以建立大规模健康参考分布来解释定量CT生物标志器（如器官体积和组织衰减）。

**Method:** 设计基于证据的跨验证LLM集成系统：首先5个LLM基于报告原文标记结构级异常候选，然后通过交叉验证解决分歧。使用分布感知的广义加性模型建立考虑年龄、性别、对比增强和采集参数的参考图表。

**Result:** 成功构建了涵盖成年的106个解剖结构综合全身参考图表，纵向分析揭示了结构和对比依赖性变化，与横断面趋势不同。

**Conclusion:** 该资源支持从常规CT进行协变量调整百分位评分，促进标准化定量表型分析、多中心影像研究和可扩展的机会性筛查研究。
...
> 本文提出了RAWild框架，通过物理引导的全局-局部色调映射和跨传感器RAW模拟管道，实现了传感器无关的RAW目标检测。

<details>
  <summary>Details</summary>

**Motivation:** 相机传感器RAW数据虽然在目标检测中具有深度位深、保留物理信息和避免ISP失真等优势，但不同设备间的曝光条件、光谱灵敏度和位深变化导致域差异远大于sRGB，使得传感器无关的泛化成为根本性挑战。

**Method:** 提出了RAWild物理引导全局-局部色调映射框架，将传感器引起的变异分解为基于RAW分布先验驱动的全局色调校正和空间自适应局部颜色调整。同时构建了基于物理的RAW模拟管道，合成涵盖多种光谱灵敏度、光源和传感器非理想性的真实传感器输出，支持跨传感器训练和泛化。

**Result:** 在多个覆盖10-24位深的RAW基准测试上的广泛实验表明，所提方法在单数据集、混合数据集和具有挑战性的鲁棒性设置下均达到了最先进的性能。

**Conclusion:** RAWild框架通过物理驱动的色调映射和跨传感器数据合成，有效解决了RAW目标检测中传感器无关泛化这一核心问题，为RAW级别的目标检测提供了新的解决方案。
...
> MobileEgo Anywhere是一个使用智能手机收集大规模、长时间视动轨迹的框架，包含200小时数据集、开源应用和标准化处理管道，旨在降低机器人数据收集门槛并加速VLA模型发展。

<details>
  <summary>Details</summary>

**Motivation:** 现有视动数据集存在时序短（仅几分钟）的局限性，无法满足复杂机器人任务对长期时间依赖性的需求，且传统机器人数据收集硬件成本高昂。

**Method:** 利用现代智能手机的通用传感器套件实现高精度、长期相机姿态追踪，开发了移动应用让任何用户都能记录视动数据，并提供数据处理管道将原始数据转换为标准化训练格式。

**Result:** 发布了200小时多样化长时间视动数据集，开源了移动应用和完整处理管道，实现了低成本、大规模数据收集。

**Conclusion:** 通过民主化数据收集过程，该工作能够在全球各种环境中大规模获取长期数据，加速通用机器人策略的开发，为Vision Language Action模型研究提供重要基础设施支持。
...
> 提出了一种免提示、参数高效的SAM2微调框架，通过位置编码生成器和双适配器策略，显著提升了生物医学图像分割的准确性和效率。

<details>
  <summary>Details</summary>

**Motivation:** SAM2在自然图像上表现出色但在生物医学分割中面临领域差异和提示依赖的挑战，需要一种无需提示且高效的解决方案。

**Method:** 引入卷积位置编码生成器适应任意宽高比，采用双适配器策略：高性能适配器使用可变形卷积建模边界，轻量级适配器通过结构重参数化降低推理延迟。

**Result:** 在多个生物医学数据集上显著超越基线方法，比原始SAM2提高19.66%的准确率，计算成本比重型医学SAM适应方法减少87%。

**Conclusion:** 该方法实现了准确性和效率之间的优异平衡，为生物医学图像分割提供了一个有效的解决方案。
...
> iPhoneBlur是一个针对消费级移动设备的运动模糊恢复基准数据集，包含7400个难度分层的图像对，揭示了真实部署条件下的模型性能变化。

<details>
  <summary>Details</summary>

**Motivation:** 现有的运动模糊恢复评估使用聚合指标，掩盖了不同模糊难度下的性能变化，无法反映真实部署条件下的模型行为。

**Method:** 通过iPhone 17 Pro高帧率视频合成7400个图像对，使用PSNR引导的自适应时间窗口将样本分为Easy、Medium、Hard三个难度分层，并通过光流幅度验证分层效果。

**Result:** 评估显示从Easy到Hard子集性能下降7-9dB，专业相机与消费相机之间存在域差异，通过针对性微调可显著恢复性能。

**Conclusion:** iPhoneBlur通过难度分层和部署关键元数据，为资源受限的边缘系统提供了模型可靠性和失败模式的系统评估框架。
...
> 4DThinker是一个让视觉语言模型通过4D潜在心理图像进行动态空间推理的框架，通过无标签数据生成、动态图像微调和强化学习实现了对复杂动态场景的内在推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言模型在从单目视频进行动态空间推理方面存在局限性：要么将时空推理完全转化为文本表示（冗长且不精确），要么依赖外部几何模块（增加推理复杂度且不培养模型内在能力）。

**Method:** 提出4DThinker框架：1) 设计可扩展的无标签数据生成管道，从原始视频合成4D推理数据；2) 提出动态图像微调(DIFT)，联合监督文本标记和4D潜在表示；3) 开发4D强化学习(4DRL)，通过基于结果的奖励处理复杂推理任务。

**Result:** 在多个动态空间推理基准测试中，4DThinker持续超越强基线模型，为视觉语言模型的4D推理提供了新的解决方案。

**Conclusion:** 4DThinker首次实现了视觉语言模型的
...
> 本文提出了一种基于神经形态计算的端到端美国手语字母识别系统，通过脉冲视觉注意力机制和紧凑的脉冲神经网络，在保证识别准确率的同时实现了极低的功耗和延迟。

<details>
  <summary>Details</summary>

**Motivation:** 现有的手语识别方法虽然分类准确率较高，但在延迟和功耗方面限制了其实时部署的可行性。神经形态传感和处理基于稀疏事件驱动计算，为低延迟、高能效的感知提供了新的技术路径。

**Method:** 提出集成脉冲视觉注意力机制和紧凑脉冲神经网络的端到端神经形态架构。该系统结合在线感兴趣区域提取功能，并在SpiNNaker神经形态平台上部署。使用Sign Language MNIST和ASL-DVS两个数据集进行评估。

**Result:** 在仿真环境中达到92.27%的识别准确率，神经形态硬件部署达到83.1%的准确率。整个系统功耗仅为0.565 mW，延迟为3 ms，是所有基准测试方法中最节能、低延迟的方案。

**Conclusion:** 尽管设计紧凑，该系统证明了任务依赖的视觉注意力应用在边缘部署中的适用性，为实时手语识别提供了一种高效可行的技术方案。
...
> FusionProxy是一个实时红外-RGB图像融合模块，采用基于教师样本集统计特征的双重监督机制，实现扩散级融合质量并支持边缘设备实时推理。

<details>
  <summary>Details</summary>

**Motivation:** 纯RGB视觉模型在夜间和雾天等挑战场景下可靠性不足，而现有高保真红外融合方法延迟过高，无法满足边缘设备实时部署需求。

**Method:** 利用教师样本集的两种互补统计特征：原始图像空间像素方差用于加权像素级监督，冻结骨干网络内部像素方差用于空间路由特征级对齐；训练后可作为独立组件即插即用。

**Result:** 在静态识别任务上性能优异，显著提升闭环自动驾驶等动态任务的鲁棒性，在各平台实现实时推理速度。

**Conclusion:** FusionProxy为全天候感知提供了灵活通用的解决方案，解决了质量和效率的权衡问题，可直接集成到现有视觉系统而无需联合优化。
...
> PFCVR是一个细粒度跨模态车型检索模型，通过部件级对齐和双向掩码恢复模块，在文本到图像的车辆重识别任务中实现SOTA性能，并构建了大规模数据集T2I-VeRW。

<details>
  <summary>Details</summary>

**Motivation:** 传统车辆重识别仅支持图像查询，但现实中常遇到只有目击者文本描述的情况。因此需要开发支持文本到图像的跨模态车辆检索系统，以应对实际应用场景的需求。

**Method:** PFCVR模型通过三个关键技术实现：1) 在部件级别构建局部图像-文本对；2) 引入可学习的部件查询令牌，聚合部件级和全局句子上下文；3) 设计双向掩码恢复模块，让每个模态在另一模态的指导下重建被掩码内容，实现隐式的全局特征对齐。

**Result:** 在T2I-VeRI数据集上达到29.2% Rank-1准确率，相比最佳竞争方法提升3.7个百分点；在新构建的T2I-VeRW基准上达到55.2% Rank-1准确率，超越多个SOTA方法。

**Conclusion:** 本文提出的PFCVR模型通过显式局部对齐和隐式全局对齐的结合，有效解决了文本到图像车辆重识别的挑战。构建的大规模细粒度部件级标注数据集T2I-VeRW为该领域提供了重要的研究基础。
...
> PlotPick是一个基于视觉语言模型的开源工具，能够批量从科学图表中提取结构化表格数据，在多个基准测试中显著优于专门的图表转表格模型DePlot。

<details>
  <summary>Details</summary>

**Motivation:** 系统综述和荟萃分析经常需要作者仅以图表形式报告的数值数据，但手动数字化过程缓慢且无法规模化处理，亟需自动化解决方案。

**Method:** 开发了PlotPick工具，利用视觉语言模型(VLMs)批量提取科学图表中的结构化表格数据，并在ChartX和PlotQA两个基准测试上评估了来自三个提供商的六个VLMs模型。

**Result:** 六个VLMs模型在两个基准测试上全部优于DePlot。在ChartX基准测试中，VLMs召回率达88-96%，而DePlot仅为71%；在PlotQA基准测试中，VLMs的RMSF1为86-99%，DePlot为94%；在专用模型训练数据中不包含的箱形图上，VLMs表现优势更加明显，RMSF1达83-97%，而DePlot仅为24%。

**Conclusion:** 基于视觉语言模型的PlotPick工具在科学图表数据提取方面表现出色，特别是在处理训练数据中未见的图表类型时具有明显优势，为系统综述和荟萃分析的数据提取提供了高效的自动化解决方案。
...
> PARSE是一个用于域泛化的图像分类框架，通过学习视觉基元及其空间关系结构来提高跨域性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有域泛化方法主要改进训练过程，但忽略了结构组合的显式表示学习，这在组合域泛化基准上限制了性能。需要一个能显式建模视觉基元及其关系的方法。

**Method:** 提出PARSE框架，将视觉识别分解为视觉基元和它们的关系组合。包含三个组件：(1)CNN骨干网络提取视觉特征；(2)概念瓶颈层生成基元热图；(3)结构评分层评估基元间的空间关系。使用软二元、三元和四元谓词表示关系，端到端学习基元和关系结构。

**Result:** 在CUB-DG基准上准确率提升超过4.5个百分点，在DomainBed基准上与现有域泛化方法保持竞争力。

**Conclusion:** 通过显式建模视觉基元及其空间关系结构，PARSE能够有效提升域泛化性能，特别是在需要组合理解的基准测试中表现优异。
...
> DPOFusion是一个基于直接偏好优化的红外与可见光图像融合框架，通过PALDM生成多样化候选结果，并利用PCLDM实现偏好自适应的精确融合控制。

<details>
  <summary>Details</summary>

**Motivation:** 现有的红外与可见光图像融合方法无法灵活适应人和机器视觉的异质需求，实现自适应融合仍是一个开放的挑战性问题。

**Method:** 提出DPOFusion框架，包含两个核心组件：PALDM通过潜在融合先验和联合条件损失生成具有不同属性的候选融合结果；PCLDM通过实例直接偏好优化进行微调，实现最终融合结果的直接偏好控制。

**Result:** 实验结果表明该框架能够实现人类、视觉语言模型和任务驱动网络间的精确偏好对齐，在自适应融合质量和任务导向可迁移性方面设立了新基准。

**Conclusion:** DPOFusion成功解决了多模式融合中偏好自适应的难题，为人和机器视觉需求提供了灵活的解决方案，显著提升了融合效果和任务性能。
...
> RealCam是一个用于实时、交互式相机控制的视频到视频生成框架，通过跨帧上下文学习和闭环增强技术实现了突破性的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频到视频生成方法依赖非因果的全序列处理和刚性的时序拼接架构，导致计算延迟过高、复杂度呈二次方增长，且无法兼容实时流媒体处理或可变长度输入，严重限制了交互式电影制作和实时广播的应用前景。

**Method:** 提出了基于跨帧上下文学习的高保真教师模型，通过交织源帧和目标帧实现长度无关的泛化和因果适应；采用自强制与分布匹配蒸馏将教师模型蒸馏为几步因果学生模型；设计了闭环增强(LoopAug)技术从现有多视角数据集合成全局一致的循环序列来解决闭环轨迹严重的不一致性问题。

**Result:** 在视觉保真度和时序一致性方面达到最先进水平，推理速度比现有方法快几个数量级，实现了真正的交互式相机控制，能够在实际应用中支持实时流媒体处理。

**Conclusion:** RealCam成功突破了传统视频生成架构的计算瓶颈，为实时交互式视频生成开辟了新的技术路径，在电影制作、直播等领域的应用潜力巨大。
...
> PersonaGesture是一个基于扩散的单参考语音-手势个性化模型，能通过一个参考动作片段为未见过的说话者生成既匹配新语音又保持说话者特有姿态风格的逼真手势。

<details>
  <summary>Details</summary>

**Motivation:** 为虚拟形象和智能代理生成个性化手势动画的需求日益增长。挑战在于如何从单个参考动作中分离出稳定的说话者习惯与特定话语相关的动作特征，实现无需针对每个说话者优化的快速个性化。

**Method:** 提出两个关键组件：自适应风格注入(ASI)和隐式分布修正(IDR)。ASI通过风格感知器将参考动作编码为说话者记忆令牌，通过零初始化残差交叉注意力注入到去噪过程中；IDR在潜在空间应用长度感知的对角仿射映射来修正残差通道矩。

**Result:** 在BEAT2和ZeroEGGS数据集上的综合评估显示，该方法在量化指标、参考身份控制、同音频诊断等方面优于坍塌风格编码、全参考注意力和单片段微调等方法。

**Conclusion:** 将去噪时间说话者记忆与保守的生成后矩修正分离的策略，有效提升了未见说话者个性化手势生成质量，为实际应用中的快速个性化提供了可行方案。
...
> ArenaPO利用Arena分数作为离线奖励，实现了文本到图像扩散模型的高效细粒度偏好优化，无需显式奖励建模。

<details>
  <summary>Details</summary>

**Motivation:** 现有的直接偏好优化(DPO)方法依赖二值反馈，仅能在选择-拒绝对上进行粗粒度建模，导致优化效果不佳，需要更细粒度的反馈机制。

**Method:** 构建模型Arena，将每个模型能力表示为高斯分布，通过遍历标注的成对偏好推断模型能力；对于图像对，基于截断正态分布的潜变量推断绝对质量差距作为细粒度反馈。

**Result:** 在Pick-a-Pic v2和HPD v3数据集上，ArenaPO持续超越现有基线方法，证明其有效性。

**Conclusion:** ArenaPO成功结合了传统RLHF的丰富奖励和DPO的高效性，通过离线Arena分数实现细粒度优化，为T2I模型的偏好对齐提供了新的解决方案。
...
> MSD-Score是一种无参考的图像描述评估指标，通过将图像块和文本词嵌入建模为单位超球面上的von Mises-Fisher混合分布，在多尺度框架下实现更精确的匹配评估。

<details>
  <summary>Details</summary>

**Motivation:** 现有的无参考评估方法主要依赖全局嵌入相似性，无法捕捉细粒度的不匹配问题，如幻觉物体、缺失属性或错误关系等。

**Method:** 将图像和文本建模为von Mises-Fisher混合分布，通过加权双向KL散度量化语义差异，结合全局相似性构建多尺度评估框架，支持单候选和多候选评估。

**Result:** 实验表明MSD-Score在无参考指标中与人类判断的相关性达到业界最佳，同时提供可解释的局部对齐错误诊断。

**Conclusion:** MSD-Score不仅提升了评估准确性，其概率化公式还能提供透明、可分解的局部锚定错误诊断，为整体相似性指标和基于裁判的评估器提供确定性补充信号。
...
> 本文提出了Holmes框架，通过分层证据学习聚合多粒度跨模态证据来显式建模不确定性，解决部分相关视频检索中的语义模糊和稀疏时间监督问题。

<details>
  <summary>Details</summary>

**Motivation:** 部分相关视频检索中存在查询与视频内容的不对称性，简短查询与丰富视频内容导致检索过程的不确定性，加上视频中时间监督稀疏，使得模糊查询在视频间产生语义歧义。

**Method:** 提出分层证据学习框架Holmes：在视频间层面，将相似度分数解释为证据支持并用Dirichlet分布建模；在视频内层面，通过柔性最优传输实现软查询-片段对齐，积累更密集的证据。

**Result:** 大量实验表明Holmes框架在部分相关视频检索任务上超越了现有最先进方法。

**Conclusion:** Holmes通过显式量化和建模不确定性，有效解决了部分相关视频检索中的语义模糊和稀疏监督问题，为该领域提供了新的解决思路。
...
> AMIEOD是一个面向低光照场景的目标检测框架，通过图像增强与目标检测联合优化，显著提升了暗光环境下的检测精度。

<details>
  <summary>Details</summary>

**Motivation:** 在低光照条件下拍摄的图像质量较差，导致视觉感知任务（如目标检测）的准确率远低于正常光照环境，需要专门的解决方案来提升暗光场景下的检测性能。

**Method:** 提出了Multi-Experts图像增强模块(MEIEM)利用多种增强策略，设计了Detection-Guided回归损失(DGRL)将检测结果作为回归目标，构建了Expert Selection模块(ESM)通过Detection-Guided交叉熵损失(DGCE)动态选择最优增强策略，实现检测导向的图像增强优化。

**Result:** 在多个数据集上的大量实验表明，该方法在低光照条件下显著提高了目标检测准确性，且能与现有检测算法良好兼容。

**Conclusion:** AMIEOD框架成功地将图像增强与目标检测任务联合优化，为低光照场景的视觉感知提供了有效的解决方案，代码已开源供研究使用。
...
> LARGO是一种通过CP张量分解在权重空间处理缺失模态问题的超网络方法，将多模型压缩为单一网络，在医学图像分割任务上表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 现有解决多模态图像分析中缺失模态问题的方法架构复杂，难以在不同数据集间迁移，且主要在特征空间操作，存在局限性。

**Method:** 提出LARGO超网络，使用CP张量分解建模卷积权重，将2^N-1个缺失模态专用模型压缩为单一网络，在权重空间而非特征空间解决问题。

**Result:** 在BraTS 2018和ISLES 2022数据集上52种配置中47种排名第一，平均Dice分数分别提升0.68%和2.53%，超越现有最优方法。

**Conclusion:** LARGO方法不仅在医学图像表现优异，初步实验还表明其可能扩展到非医学异构模态，为缺失模态问题提供了更通用的解决方案。
...
> 提出OpenGaFF框架，基于3D高斯泼溅技术实现开放词汇3D场景理解，通过高斯特征场和结构化码本解决多视角预测的碎片化和空间不一致性问题。

<details>
  <summary>Details</summary>

**Motivation:** 基于高斯表示的开放词汇3D场景理解面临挑战，主要由于多视角观测下语义预测的碎片化和空间不一致性，现有方法缺乏几何与语义的强耦合。

**Method:** 构建高斯特征场将语义建模为高斯几何和外观的连续函数；引入结构化码本作为共享语义原语；设计码本引导注意力机制，通过查询嵌入与码本条目的相似性匹配检索语言特征。

**Result:** 在标准2D和3D开放词汇基准测试中持续超越先前方法，实现了更好的分割质量、更强的3D语义一致性和语义可解释的码本。

**Conclusion:** OpenGaFF通过几何-语义强耦合和结构化码本机制，显著改善了开放词汇3D场景理解的预测质量和空间一致性，为3D语义表示提供了新的解决方案。
...
> 本文提出了一种名为追踪器的自监督跟踪框架，通过双模态上下文关联机制，结合细粒度语义提示和上下文噪声，从未标记视频中学习鲁棒的跟踪表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自监督跟踪器缺乏有效的上下文建模，而基于非语义查询的上下文关联方法难以适应未标记跟踪场景，难以学习可靠的上下文线索。

**Method:** 提出包含两个阶段的上下文关联机制：早期训练阶段，实例补丁token同时分配给前向和后向跟踪分支；随着训练推进，逐渐注入上下文噪声扰动特征，鼓励跟踪器在更复杂的特征空间中学习鲁棒表示。

**Result:** 大量实验证明所提出方法的优越性，该方法仅在训练时使用，保持了推理的高效性。

**Conclusion:** 通过双模态上下文关联机制，该自监督模型能够从未标记视频中学习高质量的跟踪表示，为自监督跟踪技术的发展提供了新的思路。
...
> VISD是一个结构化自监督蒸馏框架，通过引入诊断性特权信息和方向-幅度解耦机制，有效提升了视频大语言模型的复杂推理能力和训练效率。

<details>
  <summary>Details</summary>

**Motivation:** 视频大语言模型在复杂推理训练中面临序列级奖励稀疏、长时序推理轨迹中细粒度信用分配困难的问题。现有强化学习方法虽然提供了可靠的监督，但无法捕捉token级别的贡献；而自蒸馏方法虽然提供密集监督，但缺乏结构和诊断特异性，且与强化学习交互不稳定。

**Method:** VISD框架采用视频感知的裁判模型，将推理质量分解为答案正确性、逻辑一致性和时空基础等多个维度，用结构化反馈指导教师策略进行token级别监督。引入方向-幅度解耦机制，使奖励计算的rollout级优势决定更新方向，结构化特权信号调节token级更新幅度。同时加入课程调度和基于EMA的教师稳定化机制。

**Result:** 在多个基准测试上，VISD一致性强于强基线方法，提高了答案准确性和时空基础质量。特别是在优化步骤上，VISD实现了接近2倍的快速收敛，证明了结构化自监督在提升性能和样本效率方面的有效性。

**Conclusion:** VISD通过结构化自监督蒸馏和方向-幅度解耦机制，解决了视频大语言模型复杂推理训练中的关键难题，在性能和训练效率方面都取得了显著提升，为视频理解任务提供了新的有效解决方案。
...
> 现代预训练视觉变换器违反了局部性假设，存在强烈的对象内部信息泄露，导致基于部件的注意力解释方法失去可靠性，而两阶段方法可以有效缓解这一问题。

<details>
  <summary>Details</summary>

**Motivation:** 基于部件的推理是提高视觉模型可解释性的经典策略，其核心假设是潜在表示的局部性。然而，这一假设在现代深度学习模型中是否成立尚未得到系统验证，需要通过实验研究来检验其有效性。

**Method:** 使用部件级属性标注来测量视觉模型中的对象内部信息泄露程度，通过全面的实验评估预训练视觉变换器的局部性假设违反情况，并设计两阶段方法作为对比基准。

**Result:** 实验发现现代视觉变换器存在强烈的对象内部信息泄露，每个部件编码了整个对象的信息，破坏了基于注意力的可解释方法的有效性；两阶段方法能从设计上防止信息泄露，并改善属性驱动的部件发现效果。

**Conclusion:** 对象内部信息泄露是影响基于部件表示可解释性的一个被忽视的重要问题，两阶段方法为缓解这一问题提供了有希望的解决方案，对依赖部件中心概念的CBMs等模型具有重要启示。
...
> 提出了一种事件稀疏性感知的跟踪框架，通过多时尺度的分层密度特征学习和动态推理机制，在事件相机跟踪任务中实现了精度与效率的良好平衡。

<details>
  <summary>Details</summary>

**Motivation:** RGB跟踪器在低光照和快速运动等挑战性条件下表现不佳，而现有基于事件相机的跟踪器忽略了事件数据的内在空间稀疏性和时间密度特性，且采用单一固定的采样策略在不同运动动态下表现次优。

**Method:** 提出了事件稀疏性感知跟踪框架，包括：1) 将稀疏、中等密度和稠密事件搜索区域逐步注入三阶段Vision Transformer主干网络，实现分层多密度特征学习；2) 引入稀疏性感知的Mixture-of-Experts模块，鼓励专家在不同稀疏模式下的专门化；3) 设计动态推测策略，根据跟踪难度自适应调整推理深度。

**Result:** 在FE240hz、COESOT和EventVOT数据集上的广泛实验表明，所提出的方法在跟踪准确性和计算效率之间实现了有利的权衡。

**Conclusion:** 该框架通过显式建模多时尺度的事件密度变化，有效利用了事件数据的稀疏特性，为事件相机跟踪任务提供了一种高效且准确的解决方案。
...
> Pest-Thinker是一个知识驱动的强化学习框架，通过构建害虫数据集和结合链式思维推理，显著提升了多模态大模型在害虫形态识别方面的专业能力。

<details>
  <summary>Details</summary>

**Motivation:** 害虫造成的作物损失严重威胁全球粮食安全，现有的多模态大模型在害虫识别领域应用受限，面临物种间复杂性高、物种内变异性大、专家标注数据稀缺等独特挑战。

**Method:** 1) 构建包含专家标注形态特征的QFSD和AgriInsect高清害虫基准数据集；2) 基于数据集合成链式思维推理轨迹，通过监督微调让模型学习害虫特定视觉线索；3) 采用群体相对策略优化算法，结合新颖的特征奖励机制，引导模型关注可观察的形态学证据，并通过LLM-as-a-Judge策略进行评估。

**Result:** 实验表明Pest-Thinker显著提升了模型在领域内外对害虫形态的理解能力，在智能农业害虫分析方面达到了专家级别的视觉推理水平。

**Conclusion:** 该框架为多模态大模型在专业领域的细粒度视觉识别提供了新的解决方案，推动了智能农业害虫分析向专家级方向发展，相关数据集和代码将开源共享。
...
> 连续专家组装(CEA)是一种用于全参量图像修复的动态参数化框架，通过跨注意力超适配器为每个空间令牌合成个性化的残差更新，有效处理空间变化和复合退化。

<details>
  <summary>Details</summary>

**Motivation:** 真实世界图像退化具有未知性、空间非均匀性和复合性的特点，现有的全参量修复模型通过全局提示或专家池的方式难以有效捕捉局部退化证据，存在全局信息瓶颈和静态专家路由的同质化问题。

**Method:** 提出连续专家组装(CEA)框架，使用轻量级跨注意力超适配器探测中间空间特征，合成实例条件化的低秩路由基和残差方向，每个空间令牌通过密集带符号点积亲和力组装自己的残差更新，无需外部提示、静态专家库或离散Top-K选择。

**Result:** 在AIO-3、AIO-5和CDD-11基准测试上，CEA相比现有的提示、描述符和专家基线方法显著提升了平均修复质量，在空间变化和复合退化上表现最为突出，同时保持了良好的参数、浮点运算和运行时效率。

**Conclusion:** CEA通过创新的令牌级动态参数化和线性注意力机制，实现了透明且高效的密集令牌路由，为处理复杂真实世界图像退化问题提供了一个有效且实用的解决方案。
...
> Prologue通过在视觉标记前生成专属的prologue标记来弥合自回归图像生成中的重建-生成鸿沟，在不影响重建质量的前提下显著提升生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 自回归图像生成模型面临重建-生成鸿沟问题：需要视觉标记同时满足重建和生成两个任务，但两者优化目标冲突，导致生成质量受限。

**Method:** 提出Prologue方法：生成一组小规模的prologue标记前置于视觉标记序列。prologue标记仅用自回归交叉熵损失训练，视觉标记专注于重建任务。这种解耦设计使生成优化不受重建质量影响。

**Result:** 在ImageNet 256x256上，Prologue-Base将gFID从21.01降至10.75（保持重建质量）；Prologue-Large达到rFID 0.99和gFID 1.46；16个prologue标记的线性探测Top-1准确率达35.88%，显著优于标准tokenizer的23.71%。

**Conclusion:** 通过引入独立的生成表示可以提升生成质量而不破坏原始表示。Prologue标记展现出涌现的语义结构，为自回归图像生成提供了新的研究方向。
...
> 本研究开发了一个集成16种可解释AI方法的AI生成图像检测框架，通过大规模数据集训练和人类用户调研，评估检测器的可解释性和对齐度。

<details>
  <summary>Details</summary>

**Motivation:** 生成式AI在网络虚假信息活动中被滥用，迫切需要透明且可解释的检测系统来帮助人类理解和识别AI生成的虚假图像。

**Method:** 构建大规模逼真虚假图像数据集AIText2Image，开发多种架构和微调策略的检测器，集成16种XAI方法，并通过100名参与者的调查评估视觉解释的清晰度。

**Result:** 建立了从人类视角评估XAI方法效果的新方法，提供了关于虚假图像检测中视觉-语言线索的洞察，并衡量了XAI输出与人类偏好的对齐度。

**Conclusion:** 该框架为AI生成图像检测提供了更有效的人类可理解解释系统，强调了人类中心的可解释AI评估方法的重要性。
...
> wAR-Tok通过在分词器训练中引入分布层面的先验匹配信号，解决了两阶段训练中分词器与自回归先验模型的不匹配问题，在保持重建质量的同时降低了AR损失并提升了生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 传统的离散图像分词器采用两阶段训练方式（先重建后拟合先验），导致分词器无法感知后续生成的自回归模型，使得学习到的token虽然能很好地保留图像信息，但难以被自回归模型从左到右预测，存在训练不匹配问题。

**Method:** 提出使用三重变分一致性(TVC)分析框架，发现重建阶段缺少先验一致性。在分词器训练中添加分布层面的先验匹配信号，采用Wasserstein梯度流优化，最终简化为辅助AR模型与目标AR先验在token层面的对比更新，仅需前向传播无需反向传播。

**Result:** 在CIFAR-10和ImageNet数据集上，wAR-Tok在保持可比较重建质量的同时，显著降低了自回归损失并改善了生成的FID指标。

**Conclusion:** 通过在分词器训练中引入先验一致性约束，有效弥合了分词器与生成模型之间的差距，为统一的图像生成训练框架提供了新思路。
...
> 本文首个对医学图像持续分割的基准研究，定义了三种临床场景，提出了综合评估框架，并系统地评估了现有方法的表现。

<details>
  <summary>Details</summary>

**Motivation:** 医学图像分割模型在临床部署时面临持续学习的需求，但该领域存在三大挑战：临床场景标准化不足、研究过度关注遗忘而忽视可塑性、缺少综合评估基准。

**Method:** 提出了三种临床导向的持续学习场景（Domain-CL、Class-CL、Organ-CL），建立了包括遗忘、可塑性、前向泛化性等指标的评估框架，并系统评估了代表性持续学习方法。

**Result:** 实验表明现有方法难以同时满足所有要求，重放方法在稳定性和可塑性间取得最佳平衡，参数隔离方法能减少遗忘但增加模型规模，前向泛化性是该领域研究不足的重要方面。

**Conclusion:** 为医学图像持续分割建立了首个基准研究，明确了评估标准，揭示了现有方法的优劣势，并提出了未来研究方向。
...
> 本文提出了DynT2I-Eval，一个全自动的动态文本到图像模型评估框架，通过构建结构化视觉语义空间持续生成新提示，解决了传统固定提示集评估的过拟合和基准污染问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本到图像基准大多依赖固定的提示集，一旦公开发布并被重复使用，容易导致模型过拟合和基准污染影响评估的公正性和准确性。

**Method:** 构建结构化视觉语义空间，将提示分解为可控制维度（主体、逻辑约束、环境、构图等），通过任务特定空间和难度感知采样持续生成新提示；采用提示条件下的成对比较统一异构输出，结合动态调度器、微批次聚合和加权贝叶斯更新维持稳定排行榜。

**Result:** 实验证明持续刷新的提示提供了稳健的评估协议，减少了提示集特定调优的影响；所提出的排序框架在冷启动收敛、后期发现和长期排名保真度之间实现了良好平衡。

**Conclusion:** DynT2I-Eval框架通过动态生成和评估机制，为文本到图像模型提供了一个更加公平、稳定和长期的评估解决方案，有效避免了传统静态基准的局限性。
...
> Retina-RAG是一个低成本模块化框架，结合了视网膜分类器、参数高效的视觉语言模型和检索增强生成技术，能在消费级GPU上实现糖尿病视网膜病变分级、黄斑水肿检测和结构化报告生成。

<details>
  <summary>Details</summary>

**Motivation:** 糖尿病视网膜病变是工作年龄成年人可预防失明的主要原因，但现有自动化筛查系统仅限于图像级分类，缺乏临床结构化报告功能。

**Method:** 采用解耦架构，将高性能视网膜分类器与通过LoRA适配的参数高效视觉语言模型(Qwen2.5-VL-7B-Instruct)结合，并引入RAG模块在推理时注入眼科知识，提高诊断一致性并减少幻觉。

**Result:** 在DR分级上F1-score达0.731，ME检测上达0.948，显著优于零样本Qwen(0.096, 0.732)和MMed-RAG(0.541, 0.641)；报告生成ROUGE-L 0.429，SBERT相似度0.884，超越所有基线模型。

**Conclusion:** 证明了仅用消费级GPU就能实现临床结构化视网膜AI的可行性，为低成本、高质量的糖尿病视网膜病变自动化诊断提供了新思路。
...
> SuperFace通过人类偏好反馈优化ARKit面部表情估计，从伪标签模仿转向感知对齐优化，显著提高了面部动画的视觉保真度。

<details>
  <summary>Details</summary>

**Motivation:** 现有的ARKit面部表情估计方法存在伪标签噪声、偏差和缺失等问题，导致监督学习模型无法生成高质量的视觉效果。

**Method:** 提出SuperFace框架，将软件估计的系数仅作为初始化，通过人类对渲染面部表情的偏好反馈进行优化，实现感知对齐的系数预测。

**Result:** 实验表明，SuperFace在表情保真度方面优于Live Link Face监督，证明了偏好驱动优化在语义面部动作预测中的有效性。

**Conclusion:** 通过从数值伪标签优化转向人类感知对齐，SuperFace实现了更视觉忠实和富有表现力的面部动画，为语义化面部动作预测提供了新的优化范式。
...
> EA-WM是一种事件感知的生成式世界模型，通过将动作和运动学状态直接投影为结构化的运动-视觉动作场，显著提升了生成视频中的机器人空间几何精度和细粒度交互动态。

<details>
  <summary>Details</summary>

**Motivation:** 现有的世界-动作模型主要将视频生成作为策略学习的辅助表示，没有充分探索利用动作信号指导视频合成的逆向问题，导致生成的滚动中往往无法保持精确的机器人空间几何和细粒度的机器人-物体交互动态。

**Method:** EA-WM通过将关节或末端执行器动作直接投影到目标相机视图中作为结构化运动-视觉动作场，而非将其作为抽象的低维token输入。同时引入事件感知双向融合块来调节跨分支注意力，捕捉物体状态变化和交互动态。

**Result:** 在综合的WorldArena基准测试上，EA-WM实现了最先进的性能，显著优于现有基线方法。

**Conclusion:** EA-WM有效地闭合了运动学控制和视觉感知之间的循环，通过几何基础的表示和事件感知融合机制，为机器人世界模型提供了更精确的时空生成能力。
...
> 本文提出了一种多模态可解释性框架，将CNN脑肿瘤分类预测转换为由大语言模型生成的临床可理解诊断报告，实现了视觉、解剖和语言模态的统一。

<details>
  <summary>Details</summary>

**Motivation:** 深度学习模型的不透明性是其在医学影像临床应用中的重大障碍，需要将技术预测转化为临床医生可理解的诊断见解。

**Method:** 构建三阶段框架：1）采用双输出混合CNN架构同步优化分类和分割；2）应用多种归因方法生成热图并转换为肿瘤掩码；3）将掩码映射到解剖图谱，通过LLM生成结构化诊断报告。

**Result:** 在4834张脑MRI数据上，InceptionResNetV2分类性能最佳，Grad-CAM++分割重叠度最高；Grok3在词汇多样性和连贯性领先，LLaMA可读性最优。

**Conclusion:** 该框架通过集成视觉、解剖和语言模态，生成技术上可靠且临床可解释的诊断报告，推进了AI辅助脑肿瘤诊断的透明度和临床问责制。
...
> 论文发现了离散视觉标记中的'熵崖'现象，提出变码本大小量化(VCQ)方法，通过递增码本大小显著提升了图像重建质量和生成性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统离散视觉标记器在整个序列中使用相同大小的码本，但研究发现图像序列中条件熵会急剧下降，导致大部分位置变成记忆问题而失去信息表达能力。这种恒定码本设计存在根本性的信息论限制。

**Method:** 提出变码本大小量化(VCQ)方法，使码本大小$K_t$沿着序列从$K_{min}=2$单调递增到$K_{max}$，保持损失函数、参数数量和自回归训练过程不变。使用vanilla自回归Transformer进行标准next-token预测训练。

**Result:** 基础版VCQ将ImageNet $256\times256$上的gFID从27.98降低到14.80；扩展版本达到gFID 1.71，使用684M自回归参数，无需额外训练技术。$K_{min}=2$的极端信息瓶颈自然诱导了粗到细的语义层次结构。

**Conclusion:** 研究表明重要的不仅是码本的总容量，还有容量如何分布和组织。VCQ方法有效解决了视觉标记中的熵崖问题，同时产生了有意义的语义层次结构。
...
> 本文提出了一种基于可微分框架的自适应4D光照计算方法，能够同时高效获取物体的形状和反射率信息，该方法使用统一的空-角结构光和单个相机系统。

<details>
  <summary>Details</summary>

**Motivation:** 传统的结构光系统在获取物体形状和反射率时存在效率和质量的权衡问题。为了解决这一挑战，作者提出了一种自适应光照计算框架，能够根据物体特性动态优化光照条件，从而提高重建质量。

**Method:** 该方法采用基于直方图的像素级概率模型来建模深度和反射率，通过可微分的方式将下一个光照条件与减少深度不确定性的损失函数关联起来。随着新的结构光照投射，相应的图像测量值被用来更新每个像素的不确定性。最后采用基于微调的方法重建深度图和反射率参数图。

**Result:** 实验表明，该方法在具有广泛形状和外观变化的物理物体上有效。深度重建结果优于现有最先进技术，反射率结果与照片验证相当。

**Conclusion:** 这个自适应照明框架实现了形状和反射率的高效、高质量同时获取，通过可微分的不确定性优化策略，在保持计算效率的同时提升了重建精度，为非接触式光学测量提供了新的解决方案。
...
> 提出逆注意力嵌入机制，通过捕获传统视觉编码器忽略的背景重要区域，提升拥挤场景视频语义搜索性能。

<details>
  <summary>Details</summary>

**Motivation:** 在拥挤场景的视频语义搜索中，现有视觉编码器倾向于关注显著的前景区域而忽略了具有重要语境信息的背景区域，导致检索性能受限。

**Method:** 设计逆注意力嵌入机制，显式地捕获并突出被传统方法忽略的区域，将该嵌入与标准视觉嵌入结合使用，无需额外训练即可部署。

**Result:** 初步实验和消融研究表明，该方法在拥挤环境下视频语义搜索的召回率指标上相比现有方法有显著提升。

**Conclusion:** 逆注意力嵌入机制有效解决了拥挤场景视频搜索中背景区域被忽略的问题，为提升语义检索性能提供了简单而有效的解决方案。
...
> ZScribbleSeg是一种创新的弱监督医学图像分割方法，通过有效的涂鸦标注策略和空间先验约束，仅利用少量涂鸦标注即可在多个医学分割任务上达到接近全监督的性能。

<details>
  <summary>Details</summary>

**Motivation:** 解决医学图像分割中全标注数据集获取耗时费力的问题，现有基于涂鸦的弱监督方法由于监督不足和形状信息不完整，常导致分割不准确和不真实。

**Method:** 首先研究优秀涂鸦标注的原理，提出通过监督最大化和随机性模拟的涂鸦形式；引入正则化项编码空间关系和形状约束，使用EM算法估计标签类别的混合比例；将这些元素整合为ZScribbleSeg框架。

**Result:** 在ACDC、MSCMRseg、BTCV、MyoPS、Decathlon-BrainTumor和Decathlon-Prostate六个分割任务上，仅使用涂鸦标注就取得了有竞争力的性能表现。

**Conclusion:** ZScribbleSeg成功将涂鸦标注效率与空间先验知识结合，为医学图像分割提供了一种实用的弱监督解决方案，显著减少了对全标注数据的依赖。
...
> Spark3R是一种无需训练的加速框架，通过差异化压缩查询令牌和键值令牌，将基于视觉Transformer的3D重建模型在处理千帧视频时的速度提升高达28倍，同时保持重建质量。

<details>
  <summary>Details</summary>

**Motivation:** 基于视觉Transformer的前馈3D重建模型在处理大规模视频输入时面临挑战，因为全局注意力层的二次计算成本，而现有的令牌合并方法未考虑查询令牌和键值令牌在3D重建中的不同功能和敏感性。

**Method:** Spark3R框架利用查询令牌编码视图特定几何请求且对压缩敏感、键值令牌表示共享场景上下文且可承受激进压缩的特性，采用不同的压缩策略：对查询令牌进行组内令牌合并，对键值令牌进行轻量级令牌剪枝，并在层间自适应调整键值压缩因子。

**Result:** Spark3R作为即插即用框架，无需重新训练即可集成到多个预训练的前馈3D重建模型中（包括VGGT、$π^3$和Depth-Anything-3），在1000帧输入上实现高达28倍的加速，同时保持竞争性的重建质量。

**Conclusion:** 通过区分查询令牌和键值令牌的功能角色并采用相应的压缩策略，Spark3R为大规模视频的3D重建提供了高效的解决方案，显著提升了推理速度而无需额外训练成本。
...
> 本文提出了一种部署在九颗卫星热红外星座上的在轨野火检测系统，通过密集掩码自编码预训练技术，在亚兆字节模型大小和亚150毫秒推理延迟的严格约束下，实现了高效的亚像素级火灾检测。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于热阈值的火情监测系统(MODIS、VIIRS)无法处理亚像素或单像素热异常检测问题，且需要满足在轨部署的严格资源约束：模型大小<1MB、推理延迟<150ms、端到端告警<10分钟。

**Method:** 采用密集掩码自编码(DenseMAE)和混合DenseMAE+EMA蒸馏技术进行预训练，在200米分辨率的中波红外单波段图像上学习密集表示，通过线性探针和像素级平均精度评估模型性能。

**Result:** 最快的SSL预训练模型达到0.640测试AP和0.69事件级Fire-F1，延迟65.34ms，引擎仅0.52MB；最佳配置在1MB以下达到0.699 AP和0.744 Fire-F1，超越监督基线(0.650 AP)。

**Conclusion:** DenseMAE预训练能够在极端类别不平衡和严格资源约束下实现高效野火检测，为在轨边缘计算提供了可行的深度学习解决方案，在精度-延迟帕累托前沿上表现优异。
...
> 该论文提出了一种基于欧拉运动场引导的图像动画方法，通过相邻帧局部监督和双向几何一致性机制，实现了更高效的训练和更稳定的动画生成。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可控图像动画框架通常基于拉格朗日运动引导，需要相对于初始帧计算光流，这限制了训练的并行化且容易产生累积误差。因此需要新的运动表示方法来改善这些问题。

**Method:** 采用相邻帧欧拉运动场进行生成引导，使运动信号始终描述短时间跳跃；引入双向几何一致性机制，通过前向后向循环检查来识别和遮盖遮挡区域，防止模型学习错误的扭曲目标。

**Result:** 实验证明该方法加速了训练过程，保持了时间连贯性，与参考基线相比减少了动态伪影。

**Conclusion:** 通过将光学流从全局坐标系转换为局部欧拉表示，结合几何一致性约束，该框架为高质量图像动画生成提供了一种更有效的解决方案。
...
> NOVA是一个使用隐式神经表示(INR)作为系统状态的世界建模框架，它通过结构化表示实现了高效、可解释且可编辑的视频预测。

<details>
  <summary>Details</summary>

**Motivation:** 当前世界模型在无标注视频训练中面临计算成本高和可解释性差的问题，主要由于将像素编码到难以理解的隐空间并依赖重型解码器进行重建。

**Method:** NOVA将系统状态表示为辅助坐标基隐式神经表示(INR)的权重和偏置，这种结构化表示可通过分析渲染消除解码器瓶颈，并通过动作匹配目标提炼为上下文相关的视频生成器。

**Result:** 在多个挑战性数据集上验证，仅用约40M参数在单个消费级GPU上实现强大的可控预测，无需辅助损失或对抗目标即可解耦背景、前景和帧间运动。
...
> TD-VLN将视觉语言导航重构为一步式全局路径规划问题，提出NavOne框架在预建俯视图上直接预测路径概率，实现8-80倍速度提升和SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有VLN方法采用自我中心的逐步范式存在错误累积和效率低下问题，而现有基于地图的方法依赖增量更新记忆图或离散路径评分，限制了连续空间推理。

**Method:** 提出TD-VLN方法，构建R2R-TopDown数据集，设计NavOne统一框架，在单次前向传播中预测多模态地图上的密集路径概率，包含俯视图地图融合器和空间感知深度混合的注意力残差扩展。

**Result:** 在R2R-TopDown数据集上，NavOne在基于地图的VLN方法中达到SOTA性能，相比现有基线方法规划阶段速度提升8倍，比自我中心方法提升80倍。

**Conclusion:** TD-VLN通过全局路径规划范式成功克服了传统VLN的局限性，为高效视觉语言导航提供了新的解决方案，证明了一步式全局规划的可行性和优越性。
...
> TinyBayes是一个面向边缘设备的可可作物病害检测框架，通过结合轻量级深度学习模型和闭式贝叶斯分类器，在9.5MB的模型大小内实现了78.7%的准确率和150ms的推理速度。

<details>
  <summary>Details</summary>

**Motivation:** 西非数百万小农户的可可作物面临可可肿胀茎秆病毒病和炭疽病的严重威胁，现有边缘设备植物病害检测系统缺乏不确定性量化，而边缘设备的贝叶斯方法主要关注硬件架构而非农业应用。

**Method:** TinyBayes框架采用YOLOv8-Nano（5.9MB）进行病灶定位，MobileNetV3-Small（3.5MB）特征提取，使用基于Jacobi先验的Jacobi-DMR闭式贝叶斯分类器（13.5KB）进行分类。证明了该方法的渐近等价性、一致性、渐近正态性和偏差校正。

**Result:** 该框架总大小9.5MB，在Amini可可病害数据集上达到78.7%准确率，端到端CPU推理时间低于150ms/图像，相比七种分类器在准确率、模型大小和推理速度方面提供了最佳平衡。

**Conclusion:** TinyBayes成功填补了边缘设备农业应用中贝叶斯深度学习方法的空白，为资源受限环境下的作物病害早期检测提供了高效的解决方案。
...
> Earth-o1是一个直接从非网格化观测数据学习地球系统演化的原生观测大气世界模型，其预报能力可与业务系统IFS相媲美，为构建地球数字孪生提供了可扩展的数据驱动基础。

<details>
  <summary>Details</summary>

**Motivation:** 现代地球观测系统提供了前所未有的多模态数据，但传统建模框架将异构测量数据强制映射到预定义空间网格中，限制了原始传感器数据的充分利用并造成了严重的计算瓶颈。

**Method:** Earth-o1摒弃了传统大气动力学建模系统和数据同化方法，直接从非网格化观测数据学习地球系统的连续三维物理演化，将多源传感器输入整合到统一的、无网格动力场中，自主推进大气状态的时空演化。

**Result:** 在后报评估中，Earth-o1的地面预报技巧与业务集成预报系统(IFS)相当，实现了直接实时预报和跨传感器推理，无需显式数值求解器开销。

**Conclusion:** 连续的、观测驱动的世界模型作为一类全新的全观测原生地球物理模拟器，能够匹配传统物理框架的准确性，为地球数字孪生提供了可扩展的数据驱动基础。
...
> SwiftI2V是一个高效的高分辨率图像转视频生成框架，通过分阶段条件段式生成，在保持输入图像细节的同时大幅降低计算成本，实现了单GPU上的2K分辨率视频生成。

<details>
  <summary>Details</summary>

**Motivation:** 现有高分辨率I2V方法存在两个主要问题：端到端模型内存和延迟成本过高；级联方法通过视频超分辨率容易产生虚假细节且偏离输入图像的局部结构，因为超分辨率阶段没有显式地以输入图像为条件。

**Method:** 采用两阶段设计：首先生成低分辨率运动参考以减少token成本和建模负担；然后在强图像条件下进行2K分辨率合成以恢复输入忠实细节。核心创新是条件段式生成(CSG)和双向上下文交互，实现分段合成和跨段连贯性。

**Result:** 在VBench-I2V 2K分辨率基准测试中，SwiftI2V性能与端到端基线相当，但GPU总时间减少202倍，实现了在单个数据中心GPU(如H800)或消费级GPU(如RTX 4090)上的实用2K分辨率I2V生成。

**Conclusion:** SwiftI2V成功解决了高分辨率I2V生成中的效率-保真度两难问题，通过创新的分段生成策略和强条件约束，在不牺牲质量的前提下大幅提升了计算效率，使得高分辨率视频生成在实际应用中变得可行。
...
> eX2L是一个基于解释的可解释框架，通过惩罚Grad-CAM激活图的相似性来解耦混淆特征，在Spawrious基准测试中显著超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有分布偏移缓解算法性能不稳定，经常无法超越基线ERM，且算法复杂度高、可解释性差，对解决虚假关联只有间接作用。

**Method:** 提出eX2L框架，通过惩罚主标签分类器与混淆因子分类器生成的Grad-CAM激活图之间的相似性，在训练过程中解耦混淆特征与分类器的潜在表示。

**Result:** 在Spawrious Many-to-Many Hard Challenge基准上，eX2L平均准确率达82.24%±3.87%，最差组准确率达66.31%±8.73%，分别比SOTA提高5.49%和10.90%。

**Conclusion:** eX2L证明了通过在组级别显式解耦标签和干扰属性可以实现功能域不变性，为分布偏移问题提供了可解释且有效的解决方案。
...
> 本文提出连续时间分布匹配(CDM)方法，将DMD框架从离散时间扩展到连续时间优化，通过动态连续时间表和连续时间对齐目标，显著提升少步图像生成的视觉质量，无需复杂辅助模块。

<details>
  <summary>Details</summary>

**Motivation:** 传统的分布匹配蒸馏(DMD)方法依赖于固定离散时间步的稀疏监督，容易产生视觉伪影和过度平滑问题，通常需要GANs或奖励模型等复杂辅助模块来恢复视觉保真度。

**Method:** CDM通过两个创新设计实现连续时间优化：1) 使用随机长度的动态连续时间表替代固定离散时间表，让分布匹配在采样轨迹的任意点进行；2) 提出连续时间对齐目标，在学生模型速度场外推的潜在空间上进行主动离轨迹匹配。

**Result:** 在SD3-Medium和Longcat-Image等不同架构上的广泛实验表明，CDM在少步图像生成方面提供了极具竞争力的视觉保真度，无需依赖复杂的辅助目标。

**Conclusion:** CDM成功地将DMD框架从离散锚点迁移到连续优化，在保持实现简洁性的同时显著提升了生成质量，为加速扩散模型提供了新的有效范式。
...
> 论文提出了一种研究深度神经网络决策区域拓扑性质的方法，通过迭代四边形网格填充技术证明决策区域不仅是路径连通的，更是简单连通的（即内部闭环可在区域内收缩）。

<details>
  <summary>Details</summary>

**Motivation:** 理解深度神经网络的决策区域拓扑结构对解释网络内部机制至关重要，之前的研究只证明了路径连通性，我们希望探索更强的拓扑性质——简单连通性。

**Method:** 提出迭代四边形网格填充程序，构建有限分辨率的标签保持表面，该表面以给定闭环为边界且完全位于同一决策区域内；并将此构造与自然Coons补丁连接以量化其与循环规范几何插值的偏差。

**Result:** 通过在多个现代图像分类模型上评估该方法，提供了经验证据支持深度神经网络决策区域不仅是路径连通的，也是简单连通的假说。

**Conclusion:** 该研究为深度神经网络决策区域的拓扑性质提供了更强的理论基础和实验验证，有助于深化对神经网络决策边界的理解。
...
> 本研究系统评估了基于动作条件潜在扩散模型的机器人世界模型，证明语义编码器在策略相关任务中优于传统重建编码器，为机器人世界模型选择提供了新的评估框架。

<details>
  <summary>Details</summary>

**Motivation:** 随着潜在扩散模型在机器人世界模型中的广泛应用，选择合适的潜在空间变得至关重要。传统依赖像素重建的自编码器可能不是策略学习最优选择，需要系统评估语义编码器的优势。

**Method:** 在BridgeV2数据集上，在固定协议下比较6种重建和语义编码器训练世界模型变体，提出三个评估轴：视觉保真度、规划和下游策略性能、潜在表示质量。

**Result:** 语义编码器如V-JEPA 2.1、Web-DINO和SigLIP 2在策略性能和表示质量上表现优异，而视觉保真度 alone不足以作为选择标准；V-JEPA 2.1在策略性能上表现最佳。

**Conclusion:** 语义潜在空间是构建策略相关机器人扩散世界模型的更强基础，应优先于传统的重建编码器进行世界模型设计。
...
> FREPix是一个频率异构的像素空间图像生成框架，通过将低频和高频成分分离处理，在ImageNet上达到了1.91 FID（256×256）和2.38 FID（512×512）的竞争性结果。

<details>
  <summary>Details</summary>

**Motivation:** 现有像素空间扩散模型将图像生成视为频率同质过程，忽略了低频和高频成分的不同作用和学习动态，需要一种能够显式处理频率差异的方法。

**Method:** 提出FREPix框架，显式地将生成分解为低频和高频成分，为它们分配独立的传输路径，使用分解网络分别预测，并采用频率感知的目标函数进行训练。

**Result:** 在ImageNet类别到图像生成任务上，FREPix在像素空间生成模型中取得了竞争性结果，特别是在低NFE（网络函数评估） regime表现突出。

**Conclusion:** 通过显式的频率异构设计，FREPix将粗到精的生成为显式设计原则而非隐式行为，有效提升了像素空间图像生成的质量和效率。
...
> GeoStack是一个模块化框架，通过在适配器流形上施加几何和结构约束，实现独立训练的领域专家组合，解决视觉语言模型在知识累积时的灾难性遗忘问题，并达到常数时间推理复杂度。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型在累积多个领域或任务的专业知识时，通常会导致灾难性遗忘问题，即学习新知识时会忘记之前已经学习的内容。

**Method:** 提出GeoStack（Geometric Stacking）模块化框架，通过在适配器流形上施加几何和结构约束来保护基础模型的基础知识，并利用数学证明的权重折叠特性实现复杂度为O(1)的常数时间推理。

**Result:** 在多领域适应和类增量学习实验中，GeoStack为长期知识组合提供了高效机制，显著缓解了灾难性遗忘问题。

**Conclusion:** GeoStack框架成功实现了多个独立训练领域专家的有效组合，在保持推理效率的同时解决了视觉语言模型的知识组合挑战，为持续学习提供了新的解决方案。
...
> 通过将3D MRI体积转换为可控的2D渲染序列，本研究提出了一种基于动作条件的自监督预训练方法，用于学习医学图像表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有的MRI自监督预训练方法主要将扫描视为静态的切片、块或体积聚合，忽略了连续导航过程中固有的自监督信号。

**Method:** 将3D MRI体积渲染为连续位置、方向和尺度的切片序列，形成密集的视频动作序列；使用tokenizer编码切片观测，通过潜在动力学模型预测潜在特征的演化。

**Result:** 在代表性的解剖和空间下游任务上，所提出的预训练方法优于标准静态体积基线、仅tokenizer预训练和无对齐动作的动力学变体。

**Conclusion:** 可控MRI切片导航为从大型未标记MRI集合中学习解剖和空间表示提供了一种有用的补充预训练接口。
...
> 论文提出了MARBLE框架，通过梯度空间优化解决了多奖励对齐扩散模型时加权求和方法的样本不匹配问题，实现了统一模型的同时多维度优化。

<details>
  <summary>Details</summary>

**Motivation:** 现有的强化学习微调方法在处理多维度图像评估任务时存在缺陷：要么需要为每个奖励训练专门的模型，要么使用加权求和但会稀释监督信号，要么需要繁琐的手工调优序列训练。

**Method:** 提出MARBLE框架，在梯度空间维护每个奖励的独立优势估计器，计算每奖励策略梯度，通过二次规划将它们协调为单一更新方向。还提出了基于DiffusionNFT损失仿射结构的摊销公式，将每步成本从K+1次反向传播降至基线水平，并使用EMA平滑系数来稳定更新。

**Result:** 在SD3.5 Medium上使用五个奖励的实验中，MARBLE同时改善了所有五个奖励维度，将最差对齐奖励的梯度余弦从加权求和下80%的负值转为持续正值，训练速度仅为基线的0.97倍。

**Conclusion:** MARBLE成功解决了多奖励对齐中的样本不匹配问题，实现了无需手工调优权重的统一模型训练，在保持训练效率的同时显著提升了多维度的对齐效果。
...
> FreeSpec是一个免训练的光谱重建框架，通过奇异值分解分解全局和局部特征，有效解决了长视频生成中的内容漂移和时序不一致问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频扩散模型在长视频生成中存在内容漂移、时序不一致和动态过度平滑的问题，特别是当外观和动作紧密耦合时，预设分解规则的可靠性严重下降。

**Method:** 从奇异谱视角分析视频时序扩展问题，提出FreeSpec框架：使用全局分支作为低秩光谱引导，局部分支作为高秩重建基础，通过奇异值分解实现光谱级别的特征融合。

**Result:** 在Wan2.1和LTX-Video上的实验表明，FreeSpec显著改善了长视频生成效果，特别是在时序动态方面，同时保持了强视觉质量和时序一致性。

**Conclusion:** 光谱级别的融合方法避免了传统分解规则的刚性特征分割，能够保持长距离一致性，同时更好地保留空间细节和时序动态变化，为免训练的长视频生成提供了新的解决思路。
...
> 提出了DCR框架，通过建模和抑制默认完成偏差来改善扩散模型在稀有但合理组合上的生成效果。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型在面对训练数据中罕见但合理的组合（如雪中沙滩、夜晚彩虹）时，往往偏向生成更常见的组合，这种现象被称为默认完成偏差。现有引导机制无法有效解决此问题。

**Method:** 引入默认完成排斥(DCR)框架：1)构建反事实吸引器：放松稀有组合因子但保持周围语义；2)计算反事实漂移：目标轨迹与吸引器轨迹的差异；3)基于投影的排斥机制：移除与漂移方向对齐的引导分量。该框架无需重新训练或修改架构。

**Result:** 在稀有组合提示词上，DCR提高了组合保真度同时保持视觉质量，有效抑制了不希望的频繁完成行为。

**Conclusion:** DCR框架暴露并对抗了模型内在偏差，为可控生成提供了超越显式约束执行的新视角，仅通过标准采样过程即可实现改善。
...
> 该论文针对视频背景替换任务缺乏高质量数据的问题，提出了一个可扩展的数据生成管道，构建了包含14万视频对的Sparkle数据集和专用评测基准Sparkle-Bench，显著提升了背景替换模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频编辑数据集主要集中在局部编辑或风格转换，而背景替换作为创意应用的核心任务，需要合成全新的时间一致性场景并保持前景-背景互动，但缺乏高质量训练数据，导致现有模型如Kiwi-Edit表现不佳。

**Method:** 设计了一个可扩展的管道，以解耦方式生成前景和背景引导，并采用严格的质量过滤机制。基于此管道构建了Sparkle数据集(包含14万视频对，涵盖5种常见背景更换主题)和Sparkle-Bench评测基准。

**Result:** 实验证明，基于该数据集训练的模型在OpenVE-Bench和Sparkle-Bench上的性能显著优于所有现有基线方法。数据集、基准和模型已完全开源。

**Conclusion:** 该研究通过解耦的前景-背景引导生成管道和严格质量控制，成功解决了视频背景替换任务的数据稀缺问题，为该领域的发展提供了重要的数据基础和评估标准。
...
> MedHorizon是一个用于评估长上下文医学视频理解的基准测试，包含759小时完整临床流程视频和1253个基于证据的多选题，旨在检验模型在稀疏证据检索和临床推理方面的能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有医学多模态大语言模型主要关注图像和短视频理解，但真实临床审查需要完整流程视频理解。医学流程视频具有高度冗余的解剖视角，而关键证据在时间上稀疏、空间上微妙且依赖上下文，当前基准测试未能充分检验检索前推理问题。

**Method:** 构建了MedHorizon基准，包含759小时完整临床流程视频和1253个基于证据的多选题。该基准证据极其稀疏（平均仅0.166%证据帧），要求模型先在嘈杂流程流中搜索，再解释和聚合发现。评估了通用领域、医学领域和长视频MLLM。

**Result:** 最佳模型准确率仅41.1%，表明当前系统距离稳健的完整流程理解仍有很大差距。分析发现：性能与帧数增加不成正比；证据检索和临床解释是主要瓶颈；这些瓶颈源于程序推理能力弱和冗余下的注意力漂移；通用采样方法只能部分平衡局部细节与全局覆盖。

**Conclusion:** MedHorizon为MLLM提供了一个严格的测试平台，用于检验其在检索稀疏证据和推理完整临床工作流程方面的能力，揭示了当前模型在长上下文医学视频理解领域存在的显著不足。
...
> 提出了一种基于采样、无需矩阵求逆的相机几何求解器方法，使用稀疏隐变量结式和傅里叶变换插值来重构行列式多项式，在数值稳定性和运行时间上表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 传统的相机几何估计方法（如Gröbner基或结式方法）需要在在线求解器中进行矩阵求逆，这带来了计算挑战。现有的方法在处理多变量多项式方程系统时存在数值稳定性问题和计算效率问题。

**Method:** 采用基于采样的矩阵求逆无关方法，通过稀疏隐变量结式构建求解器。利用快速傅里叶逆变换插值从采样评估中高效重构隐变量的行列式多项式，避免了符号展开。通过识别秩1亏子矩阵和应用克莱姆法则恢复其余未知数，并使用基于最大公约数的标准确保噪声下的鲁棒子矩阵识别。

**Result:** 在多种最小问题上的实验表明，该求解器在数值稳定性和运行时间方面表现突出，特别是对于小规模问题，为传统的Gröbner基和结式求解器提供了实用的替代方案。

**Conclusion:** 所提出的基于采样、矩阵求逆无关的求解器方法有效地解决了传统相机几何估计中的计算挑战，在保持数值稳定性的同时提供了有竞争力的运行性能，特别适合小规模最小问题的实际应用。
...
> DINORANKCLIP是CLIP的改进版本，通过结合DINOv3教师网络和高阶Plackett-Luce排名损失，同时解决了CLIP的结构性缺陷，在细粒度识别任务上表现出显著提升。

<details>
  <summary>Details</summary>

**Motivation:** CLIP存在两个主要结构性弱点：InfoNCE损失的对称性忽略了批次内非匹配对的相对顺序，全局池化导致视觉表示成为语义瓶颈，对细粒度局部结构不敏感。现有方法RANKCLIP只部分解决了第一个问题。

**Method:** DINORANKCLIP通过两个关键创新解决问题：1）注入冻结的DINOv3教师网络，通过双分支轻量化学生模型和多尺度融合模块保持跨模态对齐；2）引入高阶Plackett-Luce排名模型，通过注意力参数的成对和元组转换项增强排序能力。

**Result:** DINORANKCLIP在匹配计算条件下一致优于CLIP、CyCLIP、ALIP和RANKCLIP，在细粒度和分布外评估上表现最佳，最优排序阶数在所有基准上均为R*=3。

**Conclusion:** DINORANKCLIP有效解决了CLIP的两大结构性缺陷，通过结合教师网络指导和高阶排名学习，显著提升了模型的局部结构推理能力，特别在细粒度识别任务上表现突出。
...
> DPM++是一个动态掩蔽度量学习框架，用于解决遮挡行人重识别问题，通过自适应选择可靠的身份子空间来实现鲁棒的可见性一致匹配。

<details>
  <summary>Details</summary>

**Motivation:** 行人重识别在实际应用中的遮挡问题仍未得到很好解决。严重遮挡会移除判别性身体线索，引入背景干扰，使全局度量学习不可靠。现有方法缺乏统一框架来学习现实遮挡模式下的鲁棒可见性一致匹配。

**Method:** DPM++在分类器-原型空间中学习输入自适应的掩蔽度量，动态选择每个遮挡实例的可靠身份子空间。引入基于CLIP的两阶段监督方案，从文本分支学习ID级语义先验并转移到分类器-原型空间。采用显著性引导的补丁传输策略合成可控逼真的遮挡样本，结合遮挡感知样本配对和掩蔽引导优化。

**Result:** 在遮挡和整体行人重识别基准测试上，DPM++在整体和遮挡场景下都持续优于以往最先进方法。

**Conclusion:** DPM++通过动态掩蔽度量学习和CLIP引导的语义先验转移，结合显著性引导的遮挡样本合成，有效解决了遮挡行人重识别中的匹配难题，实现了更好的性能表现。
...
> 该研究提出了MMDG-Bench，这是首个面向多模态域泛化的统一综合基准测试，通过标准化评估发现现有方法性能提升有限，且存在多个关键问题和挑战。

<details>
  <summary>Details</summary>

**Motivation:** 当前多模态域泛化研究存在评估协议不统一、实验设置碎片化、缺乏对真实世界挑战（如输入损坏、模态缺失、模型可信度）的关注等问题，导致无法准确评估该领域的真实进展。

**Method:** 构建了MMDG-Bench基准测试，包含6个跨3类任务（动作识别、机械故障诊断、情感分析）的数据集，涵盖6种模态组合和9种代表性方法，通过训练7,402个神经网络在95个跨域任务上进行标准化评估。

**Result:** 研究发现：1）近期专门方法相比ERM基准仅提供微小改进；2）没有单一方法在所有数据集上表现最佳；3）与理论上限存在巨大差距；4）三模态融合并不总是优于最佳双模态配置；5）所有方法在损坏和模态缺失场景下性能显著下降。

**Conclusion:** MMDG-Bench揭示了多模态域泛化领域仍面临重大挑战，现有方法远未解决问题，该基准测试为未来研究提供了标准化评估框架和重要洞见。
...
> Relit-LiVE是一个新型视频重新光照框架，通过引入原始参考图像和联合预测环境视频，实现了无需相机位姿先验知识的物理一致且时间稳定的视频重光照效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于大规模视频扩散模型的神经渲染方法依赖于可靠的内蕴分解，但真实视频的内蕴分解往往不可靠，导致重光照时出现外观失真、材质损坏和时间伪影累积等问题。

**Method:** 提出两个关键技术：1) 显式引入原始参考图像到渲染过程中，帮助模型恢复在内蕴表征中丢失或损坏的关键场景信息；2) 采用新型环境视频预测公式，在单一扩散过程中同时生成重光照视频和与每个相机视角对齐的逐帧环境图。

**Result:** 在合成和真实世界基准测试中，Relit-LiVE在视频重光照和神经渲染方面一致优于最先进方法，同时自然支持场景级渲染、材质编辑、物体插入和流视频重光照等多种下游应用。

**Conclusion:** 该方法通过参考图像引导和联合预测策略，显著提高了视频重光照的物理一致性，放宽了对已知每帧相机位姿的要求，为动态光照和相机运动场景提供了更实用的解决方案。
...
> 论文提出BAMI方法，通过粗到细聚焦和候选选择两种操作，有效缓解GUI定位中的精度偏差和模糊度偏差，显著提升了在复杂场景下的性能。

<details>
  <summary>Details</summary>

**Motivation:** GUI定位是GUI代理执行任务的关键能力，但在ScreenSpot-Pro等复杂场景下，现有模型性能不佳。通过MPD归因方法发现主要错误源是高图像分辨率导致的精度偏差和复杂界面元素导致的模糊度偏差。

**Method:** 提出偏置感知操作推理(BAMI)方法，包含两个核心操作：1) 粗到细聚焦 2) 候选选择，以训练无关的方式有效缓解识别到的两种偏差。

**Result:** BAMI方法显著提升了多种GUI定位模型的准确性。例如，将TianXi-Action-7B模型在ScreenSpot-Pro基准上的准确率从51.9%提升至57.8%。消融实验验证了方法在不同参数配置下的鲁棒性。

**Conclusion:** BAMI方法在训练无关设置下有效提升了GUI定位性能，具有良好的稳定性和实用性，为复杂界面场景下的GUI代理任务提供了可靠的技术支持。
...
> ActCam是一种零样本视频生成方法，能够同时实现角色动作迁移和相机轨迹控制，无需额外训练即可实现精细的视频创作控制。

<details>
  <summary>Details</summary>

**Motivation:** 艺术应用中需要精细控制视频中的表演（角色动作）和摄影（相机轨迹），但现有方法难以实现两者的联合控制，且需要大量训练数据。

**Method:** ActCam基于预训练的图像到视频扩散模型，通过两阶段条件调度：早期降噪阶段同时使用姿态和稀疏深度条件确保场景结构一致性，后期仅使用姿态条件优化高频细节；生成几何一致的姿态和深度条件来实现相机和动作联合控制。

**Result:** 在多个基准测试中，ActCam相比仅姿态控制和其他方法，在相机遵循度和动作保真度方面都有改进，在人工评估中特别是大视角变化场景下更受青睐。

**Conclusion:** 通过精心的相机一致性条件设计和分阶段引导策略，ActCam证明了无需训练即可实现强大的联合相机和动作控制，为艺术视频创作提供了实用的零样本解决方案。
