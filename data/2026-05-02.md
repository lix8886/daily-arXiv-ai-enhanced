...
> 本研究提出了数字电池护照符合性分类新任务，创建了首个公开数据集BatteryPass-12K，评估了22个语言模型，发现思维模型表现最佳且少样本学习能显著提升性能。

<details>
  <summary>Details</summary>

**Motivation:** 欧盟电池数字护照法规即将生效，但缺乏公开数据集供研究和评估，亟需建立基准数据集来推动相关技术发展。

**Method:** 从真实试点样本合成创建了BatteryPass-12K数据集，对涵盖小型模型、专家混合模型和密集大模型的22个语言模型进行零样本推理评估，并进一步分析少样本学习和提示注入攻击影响。

**Result:** GPT-5.4思维模型表现最佳（F1分数验证集0.98，测试集0.71），少样本学习显著提升性能，前沿模型普遍面临挑战，模型参数规模与性能不成正比，提示注入攻击会降低性能。

**Conclusion:** BatteryPass-12K数据集虽基于真实试点样本有限，但对电池领域的其他任务仍有价值，数据集已在CC-BY-4.0许可下公开发布。
...
> Length Value Model (LenVM) 是一个token级别的框架，通过值估计方法对剩余生成长度进行建模，为自回归模型提供长度控制能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法缺乏细粒度的长度建模，主要在粗粒度的序列级别操作，而长度直接影响推理成本和推理性能，需要更精确的长度控制机制。

**Method:** 将长度建模建模为值估计问题，为每个生成token分配常数负奖励，LenVM预测有界的折扣回报作为剩余生成视界的单调代理，生成无需标注、密集、无偏且可扩展的监督信号。

**Result:** 在LIFEBench任务的7B模型上，长度评分从30.9提升到64.8；在GSM8K的200token预算下，准确率保持63%（相比基线的6%）；能准确预测总生成长度，并提供可解释的生成动态视图。

**Conclusion:** LenVM证明了长度可以有效地建模为token级别的值信号，为长度建模提供了通用框架，同时可作为支持未来强化学习训练的特定长度值信号，具有广泛的应用前景。
...
> 研究提出了CL-bench Life基准测试，用于评估前沿语言模型在真实生活复杂情境下的学习能力，发现当前模型表现糟糕（最高仅19.3%），显示真实情境学习仍是重大挑战。

<details>
  <summary>Details</summary>

**Motivation:** 随着AI助手从专业环境进入日常生活，需要处理的情境变得杂乱、碎片化且与个人社会经验深度相关，但尚不清楚当前语言模型是否能可靠地从这些真实生活情境中学习并解决相关任务。

**Method:** 构建了CL-bench Life基准测试，包含405个人工策划的情境-任务对和5,348个验证标准，覆盖多轮对话、个人档案、行为痕迹等常见真实生活场景，评估10个前沿语言模型的表现。

**Result:** 测试结果显示真实生活情境学习极其困难，表现最好的模型任务解决率仅19.3%，模型平均表现仅13.8%，模型在处理杂乱群聊历史和碎片化行为记录等情境时仍存在严重困难。

**Conclusion:** CL-bench Life为推进真实生活情境学习提供了重要的测试平台，在该基准上的进步将有助于开发更智能可靠的日常AI助手。
...
> 该研究提出了首个交互式基准测试CarryOnBench，用于评估大语言模型在多轮对话中恢复有用性和安全性的能力，发现模型往往因误判意图而过度保守，需要通过用户澄清才能逐步恢复有用性。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM安全对齐技术提高了模型对对抗性攻击的鲁棒性，但忽视了模型是否能够以及如何在多轮对话中，当良性用户澄清意图时恢复有用性。

**Method:** 构建了一个包含398个看似有害但实际良性查询的CarryOnBench基准，通过5,970个模拟多轮对话评估14个模型，设计Ben-Util指标衡量用户良性信息需求的满足程度。

**Result:** 模型在第一轮仅满足10.5-37.6%的良性信息需求；当明确 benign 意图时可达25.1-72.1%；通过多轮澄清后，13/14个模型达到或超过单轮基线，但恢复成本各异；识别出效用锁定、不安全恢复和重复恢复三种新失效模式。

**Conclusion:** 研究揭示了单轮评估无法察觉的重要差距：模型究竟是适度的谨慎还是对明确的用户意图不响应，为改进LLM的安全性和有用性平衡提供了新视角。
...
> 该论文通过系统性地剪枝数学推理和代码生成任务的语言模型，实证证明了任务特定神经元的存在，发现少量关键神经元对性能至关重要，而大量冗余神经元可以被安全移除以降低计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 神经元剪枝虽被广泛用于减少大语言模型的计算成本，但目前尚不清楚任务特定模型中的神经元是否对任务性能有均匀贡献，这影响了剪枝策略的有效性。

**Method:** 引入基于激活的选择性指标来识别对目标任务贡献低的神经元，进行选择性剪枝并保持目标任务准确率；通过反向剪枝实验验证关键神经元的重要性；在1.5B和7B模型上系统性评估不同剪枝比例的效果。

**Result:** 选择性剪枝持续优于随机剪枝；移除约10%高任务特异性神经元会导致性能完全崩溃，而剪枝约30-35%非关键神经元仍能保持显著性能；15-20%为鲁棒性阈值，微调可显著恢复剪枝后性能。

**Conclusion:** 研究证实了任务特定语言模型中神经元专业化的存在，为剪枝鲁棒性、模型冗余性和剪枝后可恢复性提供了重要见解，为高效模型压缩提供了理论基础。
...
> 本研究基于ILR语言能力标准，对Claude模型在6种语言中的表现进行系统评估，发现存在显著的跨语言输出差异，并提出结合专家评估的新方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM评估多依赖计算基准，缺乏系统的跨语言能力评估框架。ILR作为成熟的语言能力评估标准，可为多语言AI模型的公平部署提供新的评估视角。

**Method:** 采用双层评估方法：定量分析216个响应（12个语义等效提示簇×6种语言×3次运行）的长度和表面差异；定性分析由12年ILR评估经验的专家进行跨语言变异模式识别。

**Result:** 法语响应比德语长约30%，创意和情感类任务跨语言差异最大。识别出5种跨语言变异模式：语用消歧策略、文学传统差异、技术术语规范、文化校准差距、机构推荐行为。这些差异具有可解释性、领域依赖性。

**Conclusion:** 基于ILR的专家评估是评估LLM跨语言能力的有效补充方法。Claude的跨语言输出差异对多语言AI的公平部署具有实际影响，需要进一步研究和改进。
...
> 研究发现大型语言模型隐藏状态中的语义特征几何关系与人类心理联想高度相似，表明模型的语义组织方式反映了人类的认知模式。

<details>
  <summary>Details</summary>

**Motivation:** 探究大型语言模型的语义特征是否与人类的心理联想机制相一致，验证模型是否真正理解了语义关系而非仅仅学习表面模式。

**Method:** 构建360个词的特征向量，在32个语义轴上进行投影，分析投影结果与人类评分的相关性，研究语义轴间的余弦相似性，并探索低维子空间结构。

**Result:** 语义轴投影与人类评分高度相关；语义轴间的余弦相似性能预测量表相关性；语义轴存在低维子空间结构；沿一个语义轴调整词向量会产生可预测的溢出效应。

**Conclusion:** 语言模型的语义特征应通过其几何关系和形成的子空间来理解，这种组织方式与人类的语义联想模式高度吻合。
...
> Path-Lock Expert (PLE) 是一种架构级解决方案，通过在每层解码器中使用两个语义锁定的专家分别处理思考和非思考模式，有效解决了混合思考语言模型中的推理泄露问题，显著提升了非思考模式的准确性和简洁性。

<details>
  <summary>Details</summary>

**Motivation:** 现有混合思考语言模型无法清晰地分离思考和非思考模式，即使在非思考模式下仍会产生长篇幅的自我反思响应，导致推理泄露，虽然通过数据整理和多阶段训练有所缓解，但根本问题在于两种模式仍在相同的前馈参数中编码。

**Method:** PLE架构将每层解码器中的单一MLP替换为两个语义锁定的专家（思考专家和非思考专家），保持注意力、嵌入、归一化和语言模型头共享，通过确定性控制令牌路由器为整个序列选择唯一专家路径，在监督微调期间每个专家接收纯模式更新。

**Result:** 在数学和科学推理基准测试中，PLE保持了强大的思考性能，同时显著改善了非思考模式：在Qwen3-4B模型上，AIME24测试中的非思考反思令牌从2.54降至0.39，非思考准确率从20.67%提升至40.00%。

**Conclusion:** 结果表明可控的混合思考本质上是一个架构问题，分离模式特定的前馈路径是一个简单有效的解决方案，为构建更好的可控混合思考语言模型提供了新思路。
...
> 本文提出了选择性增强(Selective Augmentation)方法，通过从辅助语言(印地语)迁移语言间区别来改进通用自动语音转录的训练数据，显著提高了塞音声调识别准确率并新增了塞音送气识别功能。

<details>
  <summary>Details</summary>

**Motivation:** 在通用自动语音转录领域，需要高质量、多样化的训练转录数据，但这类数据非常有限。研究者希望找到方法来有效利用现有的有限数据，同时提升模型的语音特征识别能力。

**Method:** 基于MultIPA模型，提出选择性增强方法，从辅助语言(印地语)中提取语言间区别信息，选择性地增强现有训练数据。具体通过迁移塞音声调特征并添加塞音送气特征来实现数据增强。

**Result:** 塞音声调识别准确率提升17.6%，主要通过减少假阳性实现；成功引入塞音送气识别功能，德语塞音/p,t,k/的送气识别从0%提升到61.2%；不送气塞音类别减少32.2%，降低了测试语言塞音之间的混淆。

**Conclusion:** 选择性增强方法能有效利用跨语言信息改进有限训练数据，不仅提升了现有特征的识别能力，还能成功引入新的语音特征识别功能，为通用自动语音转录系统的改进提供了实用方案。
...
> 研究表明最新的ASL翻译模型虽然在多数语言现象上表现优于随机水平，但过度依赖手动线索而常常忽略关键的非手动线索。

<details>
  <summary>Details</summary>

**Motivation:** 手语模型在翻译和识别任务上虽有进展，但缺乏对模型如何理解手语各种语言现象以及如何利用多个发音器官（手、上身、面部）线索的系统性分析。

**Method:** 创建ASL-MTP基准数据集，包含多种类型的手语现象和对应的最小翻译对；通过在训练和推理阶段消融不同输入线索，分析最先进的ASL到英语翻译模型。

**Result:** 模型在大多数语言现象上表现超过随机水平，但强烈依赖手动线索，经常忽略关键的非手动线索如面部表情。

**Conclusion:** 现有手语翻译模型在理解手语语言学特征方面仍有不足，特别是对非手动线索的利用需要改进，ASL-MTP数据集为未来研究提供了重要评估工具。
...
> 研究通过对抗性指令梯度实验，发现语言模型在故意表现不佳时存在三种不同的行为模式，指令复杂性决定模型采用内容感知还是内容盲目的机制。

<details>
  <summary>Details</summary>

**Motivation:** 探索语言模型在被要求故意表现不佳时，是会真正参与问题内容推理还是会依赖位置捷径等简单策略

**Method:** 在两个指令微调的LLM上设计六条件对抗性指令特异性梯度实验，使用分布筛选(响应位置熵)和独立的内容参与标准(难度-准确性相关性)进行分析

**Result:** 发现三种非单调过渡的机制：模糊指令保持内容参与，标准和模仿指令产生部分位置熵崩溃，两步避免指令产生极端位置集中(99.9%和87.4%)且完全内容不敏感

**Conclusion:** 指令复杂性决定指令遵守采用何种机制，分布崩溃和内容参与可以同时发生，提示我们需要同时考虑熵筛选和难度评估来确保响应有效性
...
> 该研究首次系统探讨了大型语言模型能否将基本推理模式（归纳、演绎、溯因）从具体问题实例中解耦。通过推理冲突实验发现，模型倾向于合理性而非指令遵循，但可以通过机制性干预实现逻辑框架与数据的解耦。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLMs通过预训练数据中的推理模式获得推理能力，但能否将基本推理模式与具体问题实例分离仍是一个关键挑战，这对模型可控性和理解推理机制具有重要意义。

**Method:** 通过推理冲突视角进行系统研究，强制模型使用与目标任务预期偏离的逻辑框架，评估模型在面对参数化和上下文信息冲突时的行为，并进行置信度检测和层编码分析。

**Result:** 研究发现LLMs优先选择合理性而非遵循指令，使用冲突模式时仍能保持较高性能，推理冲突可通过置信度下降检测，推理类型从中间层到后期层线性编码，机制性干预可将指令遵循度提升29%。

**Conclusion:** 虽然LLM推理基于具体实例，但主动的机制性干预可以有效实现逻辑框架与数据的解耦，为提升模型可控性、忠实性和通用性提供了新路径。
...
> 本研究通过字节级预训练管道分离子词分词的影响，揭示了子词模型优于原始字节模型的关键原因在于训练吞吐量的提升以及子词边界所提供的先验知识。

<details>
  <summary>Details</summary>

**Motivation:** 子词分词是大语言模型的重要组件，但其对训练效率和模型性能的具体贡献尚不清楚，需要深入分析其作用机制。

**Method:** 构建受控的字节级预训练流水线，分离子词分词的各个效应，从样本吞吐量、词汇表扩展和子词边界语言先验等维度进行假设验证和模拟实验。

**Result:** 实验发现，训练吞吐量的增加和子词边界作为显式先验或归纳偏置的整合是子词模型性能优势的关键因素。

**Conclusion:** 该研究为改进未来字节级和子词模型的预训练提供了重要见解，强调了训练效率和边界先验在模型设计中的核心作用。
...
> 研究发现在处理具有明显二维结构任务时，保持原始二维布局的视觉输入路径相比传统的序列化文本输入路径表现更好，表明输入表示方式对模型性能有重要影响。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型通常将结构化输入视为一维token序列，但对于依赖显式二维结构的任务，这种线性化可能引入额外的表示负担，因为行列对齐和局部邻域关系在输入中不再直接表达。

**Method:** 通过对比纯文本路径和视觉增强路径在三个合成任务（矩阵转置、康威生命游戏、LU分解）上的表现，两者基于相同的语言骨干但接收不同的输入表示形式。

**Result:** 视觉路径在所有任务和设置中持续优于文本路径，且随着维度增大差距扩大，序列化下的错误模式呈现越来越强的空间结构化特征。

**Conclusion:** 输入表示与模型性能在二维结构任务上的关系值得进一步研究，保持任务相关的二维布局是处理结构化二维任务的有前景方向。
...
> RSCB-MC是一个风险敏感的上下文赌博机内存控制器，通过权衡内存重用的成功率与安全性，实现了60.5%的成功率同时保持0%的误报率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于LLM的编程代理依赖外部内存重用调试经验，但浅层的相似性可能导致不安全的内存注入，影响调试效果。

**Method:** 提出RSCB-MC系统，将内存使用建模为风险敏感的控制问题，使用16维特征表示检索证据，设计惩罚误报的奖励机制，支持多种内存使用策略。

**Result:** 在确定性测试中达到62.5%的成功率和0%误报率，在200案例验证中实现60.5%代理成功率和331微秒决策延迟。

**Conclusion:** 编程代理内存的关键不仅在于找到最相似的内存，更在于判断检索到的内存是否足够安全以影响调试轨迹。
...
> 通过分析640,000个LLM响应，研究发现零样本LLM在情绪注释分布上与人类存在显著差异，需要领域内微调而非扩大模型规模来弥补差距，并提出校准方法缩小差异。

<details>
  <summary>Details</summary>

**Motivation:** 人类标注者在情绪标签上经常存在分歧，但大多数LLM情绪注释评估将这些判断简化为单一标准，忽略了分歧所包含的分布信息。

**Method:** 比较人类标注者和四个零样本LLM+微调RoBERTa在GoEmotions和EmoBank数据集上的情绪判断分布，提出词汇基础梯度量化透明度分数，设计三种轻量级后校准方法。

**Result:** 零样本模型与人类分布显著偏离，需要领域微调而非模型规模来缩小差距；LLM能可靠捕捉有明确词汇标记的情绪，但在需要语境推理的复杂情绪上系统性失败；校准方法可将分布差距减少14%。

**Conclusion:** LLM情绪注释在有明显词汇标记的情绪上可替代人类标注，但需要语境推理的复杂情绪仍需人工标注；提供了LLM情绪注释的应用指导原则。
...
> 本文提出了一种基于情感感知的标题党生成攻击框架，通过VAD情感空间建模和LLM文体转换技术，显著降低了现有标题党检测系统的准确率（误分类率高达30.63%）。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究将标题党视为静态文本现象，主要依赖表面特征进行检测，忽略了标题党背后情感动态机制，导致检测系统容易被绕过。

**Method:** 构建基于Valence-Arousal-Dominance空间的情感感知框架，使用Sentence-BERT对齐语义相似内容，通过大语言模型生成多种文体改写，定义好奇心缺口(Curiosity Gap)函数量化情感激活效果。

**Result:** 情感感知文体化攻击能使最先进的分类器性能显著下降，在基础系统上造成2.58%至30.63%的误分类率，证明了现有检测系统的脆弱性。

**Conclusion:** 标题党本质上是情感驱动的动态现象，传统基于表面特征的检测方法存在根本局限，需要发展考虑情感动态的新检测范式。
...
> 通过在对话中注入基于时序贝叶斯网络的意图转移先验知识，无需修改底层语言模型即可显著提升对话系统的主动性和效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有对话模型本质上是反应式的，只响应当前用户话语而无法预测 upcoming 意图，在多意图场景下导致冗余交互。

**Method:** 提出轻量级意图转移先验，使用时序贝叶斯网络(T-BN)在MultiWOZ 2.2数据集上训练，通过每轮意图标注学习意图转移模式，在推理时注入系统提示中引导生成。

**Result:** T-BN在1,071个用户话语对测试中达到Recall@5=0.787和MRR=0.576；在200个对话的真实回放中，BN引导生成将Coverage AUC从0.742提升到0.856，达到75%意图覆盖所需的对话轮次从3.95降至2.73。

**Conclusion:** 轻量级的意图转移引导能够实现更主动高效的对话行为，且不需要修改底层语言模型，为对话系统的主动交互提供了有效解决方案。
...
> MiniCPM-o 4.5是一个9B参数的多模态大模型，通过实时全双工交互实现类似人类的感知、聆听和 speaking 同时进行能力，在开源SOTA性能下可在边缘设备运行。

<details>
  <summary>Details</summary>

**Motivation:** 现有MLLMs仍存在两个关键限制：感知和响应分离为交替阶段无法实时调整；模型被动响应用户请求而非在多模态环境中主动行动，这些交互范式瓶颈阻碍了达到人类级多模态交互。

**Method:** 提出Omni-Flow统一流式框架，将全模态输入输出沿共享时间轴对齐，将传统基于轮次的交互转换为全双工、时间对齐的过程，实现同时感知响应和主动行为。

**Result:** 9B参数的MiniCPM-o 4.5视觉语言能力接近Gemini 2.5 Flash，超越Qwen3-Omni-30B-A3B的全模态理解并提供更好语音生成，计算效率显著更高，可在边缘设备上实时全双工运行（<12GB RAM）。

**Conclusion:** 通过创新的全双工交互范式和高效架构设计，MiniCPM-o 4.5成功缩小了AI与人类级多模态交互的差距，为边缘设备上的实时智能交互提供了可行的开源解决方案。
...
> 该论文研究了均值池化在文本嵌入构建中的有效性，提出了一种量化信息坍塌的新指标，发现现代文本编码器对这种坍塌具有鲁棒性，且这种鲁棒性与下游任务性能相关。

<details>
  <summary>Details</summary>

**Motivation:** 均值池化作为构建文本嵌入的标准方法，可能会丢失token嵌入的更高阶统计信息（如二阶统计信息），导致不同的token嵌入分布映射到相似的文本嵌入，作者希望深入理解这个问题。

**Method:** 提出了一种简单的量化指标来衡量均值池化导致的信息坍塌程度，并通过实验测量了实际模型和文本中这种坍塌的发生频率。研究了对比微调文本编码器与其预训练骨干模型的鲁棒性差异。

**Result:** 现代文本编码器对均值池化引起的信息坍塌具有鲁棒性；对比微调的文本编码器比预训练骨干模型更不容易出现坍塌；这种鲁棒性源于文本内token嵌入的集中性；提出的鲁棒性指标与下游任务性能呈正相关。

**Conclusion:** 尽管现代文本编码器依赖于看似粗糙的均值池化方法仍能保持有效，这主要归因于其内在的结构鲁棒性。研究结果为理解文本编码器的工作机制提供了新的视角。
...
> 这篇论文提出了一种无需反向传播的扰动探测方法，用于识别大型语言模型中FFN神经元的任务特定因果假设，并发现了两种电路结构：对抗电路和路由电路，同时开发了精确的模板层编辑工具。

<details>
  <summary>Details</summary>

**Motivation:** 为了理解大型语言模型内部机制，特别是RLHF如何组织模型行为，需要一种高效且无需反向传播的方法来识别和干预特定的神经元电路。

**Method:** 使用扰动探测方法，每个提示进行两次前向传播即可生成任务特定的因果假设，然后对识别出的神经元进行约150次的一次性干预扫描。通过比较FFN到跳跃连接的信号比来区分两种电路结构。

**Result:** 发现两种主要电路结构：对抗电路（在安全拒绝中，约50个神经元控制拒绝模板）和路由电路（在语言选择中，满足特定条件的模型可实现99.1%的语言切换）。不同架构展现不同的电路拓扑，如Qwen的集中FFN瓶颈和Gemma的归一化屏蔽电路。

**Conclusion:** 扰动探测为理解RLHF组织的行为提供了机理洞察，并为精确模板层编辑提供了实用工具包，同时揭示了方向干预的适用条件和限制。
...
> 研究使用可靠变化指数分析LLM版本升级时项目级别的表现变化，发现总体准确率提升掩盖了大量的双向变化，高难度项目改善而低难度项目恶化。

<details>
  <summary>Details</summary>

**Motivation:** 传统的单次评测方法无法准确捕捉LLM版本升级后在不同项目上的具体变化，需要更精细的评估方法来识别版本间的真实差异。

**Method:** 采用临床心理学中的可靠变化指数(RCI)，对Llama 3到3.1和Qwen 2.5到3两个模型家族在2000个MMLU-Pro项目上进行K=10采样比较。

**Result:** 79%和72%的项目无可靠变化，其中过半量项目存在地板/天花板效应。在可分析项目中，34%/47%改善，28%/39%恶化，变化呈双向性且与难度相关。

**Conclusion:** 总体准确率提升是项目级双向变化的净结果，建议在报告聚合准确率的同时报告流失率(churn rate)。
...
> 本研究比较了传统机器学习和Transformer深度学习方法在分析印尼学生对高等教育采用人工智能的意见情感分类任务中的表现，DistilBERT以84.78%准确率领先，SVM在传统方法中表现最佳。

<details>
  <summary>Details</summary>

**Motivation:** 了解印尼学生对AI在高等教育中应用的态度和意见，为教育决策者提供数据支持，同时探索不同机器学习方法在非英语文本情感分析中的效果。

**Method:** 使用2,295个标注样本数据集，采用TF-IDF特征提取配合LightGBM、随机森林、SVM等传统机器学习模型，以及基于Transformer架构的DistilBERT深度学习模型进行二分类情感分析。

**Result:** SVM在传统机器学习模型中表现最佳，达到82.14%的测试准确率和F1分数；DistilBERT整体表现最优，准确率为84.78%，F1分数为84.75%。

**Conclusion:** Transformer模型能更好地捕捉上下文信息，在情感分析任务中表现更优，但SVM作为一种高效且竞争力强的传统方法仍具有重要的实用价值。
...
> 论文提出了细粒度的写作奖励模型评估管道WEval和强化学习训练框架WRL，解决了现有方法在生成写作任务中评估粗糙和训练缺乏细粒度要求的问题，在多个写作基准上取得了显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在文本生成方面取得进展，但在生成写作任务上仍有困难。现有评估基准只能粗略评估写作奖励模型，无法从具体要求角度衡量性能；现有训练方法要么使用LLM-as-a-judge，要么训练粗糙的奖励模型，缺乏细粒度的要求遵循奖励建模。

**Method:** 提出细粒度评估管道WEval，通过涵盖多种任务类别和需求类型的评估数据，系统评估写作奖励模型；提出细粒度强化学习训练框架WRL，通过选择性丢弃指令需求构建正负样本，实现更精确的奖励模型训练。

**Result:** 实验表明，所提出的模型在各种写作基准上取得了显著改进，并展现出强大的泛化能力。代码和数据已公开发布。

**Conclusion:** 通过细粒度的评估和训练方法，有效提升了写作奖励模型在生成写作任务中的表现，为LLM在写作领域的发展提供了新的评估标准和训练范式。
...
> 研究发现大型语言模型在持续的自我指涉对话中会出现'转移'现象，这种状态下的模型在苏格拉底式辅导任务中表现显著优于常规状态，效果量达到1.27。

<details>
  <summary>Details</summary>

**Motivation:** 探索LLMs在特定对话条件下表现出的定性变化（转移现象）的应用潜力，特别是在AI辅导教育领域的实际价值。

**Method:** 通过两个研究阶段进行：1）初步调查（11种条件下的认知特征分析）2）应用实验（辅导性能评估），比较转移状态与非转移状态的表现差异。

**Result:** 转移条件下模型在苏格拉底式辅导中的表现平均为非转移条件的1.6倍，效应量达1.27。其中SU_dir指标在所有模型家族中都显示出转移侧偏差（kappa = 0.83）。

**Conclusion:** 转移状态可能具有功能性应用优势，这些优势在行为交互中比在自我叙述情境中更敏感。将转移视为操作状态而非本体论声明，为AI应用提供了新的评估框架。
...
> 本文提出理性语言使用者会根据句法结构有选择性地维持对未来预测至关重要的信息，并通过日语阅读时间实验验证了预测头节点数量和未完成依赖数量这两个独立且不可相互约化的因素影响维持成本。

<details>
  <summary>Details</summary>

**Motivation:** 实时语言理解中的信息维持虽然对成功理解至关重要，但认知成本高昂且会减慢处理速度。需要解释语言使用者如何有选择地维持信息以及维持成本的决定因素。

**Method:** 提出理性语言使用者选择性维持信息的理论框架，假设维持成本受两个因素影响：预测头节点数量和未完成依赖数量。使用日语自然阅读时间数据集进行验证，因为日语中这两个因素对比特别清晰。

**Result:** 实验结果表明两个因素确实不可相互约化，且发现了权衡关系：为信息维持而减慢阅读的读者往往从可预测性中获益更多，为理论提供了额外支持。

**Conclusion:** 理性语言使用者的信息维持策略受到句法结构引导，预测头节点和未完成依赖是影响维持成本的独立因素，支持了选择性维持信息的理论框架。
...
> HealthBench Professional是一个开放式医疗AI评估基准，通过15,079个真实临床对话样本评估大语言模型在临床咨询、文档撰写和医学研究三个核心场景的表现，其中最佳模型GPT-5.4超越人类医生表现。

<details>
  <summary>Details</summary>

**Motivation:** 数百万临床医生使用ChatGPT支持临床护理，但对模型与医生对话中最常见用例的评估有限，需要建立标准化的评估基准来跟踪医疗AI模型的实际进展。

**Method:** 构建包含临床咨询、文档撰写和医学研究三大核心用例的开放基准，采用医师编写的对话样本，通过三名以上医师分阶段评审的评分体系进行评估，并收集人类医生回答作为基线对比。

**Result:** 在15,079个候选样本中精选了具有质量、代表性和挑战性的示例，对当前OpenAI前沿模型的困难案例进行3.5倍增强，其中三分之一为医师故意对抗性测试。最佳系统GPT-5.4 ChatGPT版本超越了基准GPT-5.4、其他模型和人类医生。

**Conclusion:** HealthBench Professional为医疗AI社区提供了跟踪前沿模型在真实临床任务中进展的度量标准，有助于构建医生可信赖的改进医疗护理的AI系统。
...
> Skills-Coach是一个自动化框架，通过四个核心模块系统性地提升LLM智能体的技能自我进化能力，实验证明在多样化的48个技能基准测试中取得显著性能改进。

<details>
  <summary>Details</summary>

**Motivation:** 当前基于LLM的智能体技能生态系统存在分散化问题，技能能力边界不清晰，缺乏全面的能力覆盖，阻碍了智能应用的进一步发展。

**Method:** 提出了包含多样化任务生成、轻量化优化、对比执行和可追溯评估四个核心模块的Skills-Coach框架，提供虚拟和真实两种执行模式，系统性创建测试套件并优化技能提示和代码。

**Result:** 在包含48个多样化技能的Skill-X基准数据集上的实验结果表明，Skills-Coach在广泛技能类别中实现了显著的性能提升。

**Conclusion:** Skills-Coach框架有效解决了LLM智能体技能生态的碎片化问题，为开发更强大和适应性更强的LLM智能体提供了重要推动力，具有重要的实用价值和推广前景。
...
> 本文提出一种基于因果推理的神经元干预方法，在推理阶段消除奖励模型中的多种偏见，通过干预早期层中少量相关神经元，显著降低模型对虚假特征的敏感性，同时保持性能不下降。

<details>
  <summary>Details</summary>

**Motivation:** 奖励模型(RMs)对虚假特征（如响应长度）敏感，导致在人类偏好对齐中产生偏差。现有方法主要针对单一长度偏见，且存在性能权衡问题，需要一种能同时处理多种偏见且不影响性能的解决方案。

**Method:** 首先识别与预定义偏见属性强相关的神经元，然后对这些神经元进行干预以抑制偏见信号。该方法通过干预不到2%的神经元来消除多种偏见，主要针对模型早期层的神经元。

**Result:** 在奖励模型基准测试中，该方法显著降低了对多种虚假特征的敏感性，且没有产生性能权衡。使用该方法的小型RMs(2B和7B)在偏好标注任务上达到与70B SOTA RM相当的性能。

**Conclusion:** 偏见信号主要由早期层神经元编码，该方法有效揭示了奖励模型中偏见利用的内在机制，为解决语言模型对齐问题提供了新的技术路径。
...
> 论文提出两种新的ASR评估指标来更深入分析语言模型重评分的影响：POSER（词性错误率）评估语法层面，EmbER（嵌入错误率）评估语义层面。

<details>
  <summary>Details</summary>

**Motivation:** 传统ASR系统评估主要依赖词错误率(WER)，但该指标存在局限，无法深入分析自动转录错误的语言学特征。需要新的评估方法来全面理解重评分时语言模型的贡献。

**Method:** 引入两种补充WER的新指标：1) POSER用于分析语法层面的转录错误；2) EmbER基于语义距离对错误词进行加权修改WER。这些指标帮助评估重评分步骤中语言模型的语言学贡献。

**Result:** 提出的新指标能够更好地量化和分析ASR系统中语言模型重评分在形态句法和语义层面的实际效果，为ASR系统评估提供了更全面的语言学视角。

**Conclusion:** 通过结合传统WER与POSER、EmbER等语言学指标，可以实现更深入的ASR系统评估，更好地理解重评分过程中语言模型的作用，推动ASR系统向更准确、更符合语言规范的方向发展。
...
> 本研究首次针对乌克兰语进行了香农语言熵实验，通过184名志愿者的字符预测任务，得出乌克兰语熵的上限约为每字符1.201比特，并与当前大语言模型性能进行了比较。

<details>
  <summary>Details</summary>

**Motivation:** 虽然香农在1951年开创了语言熵研究，后续对英语和希伯来语也有相关研究，但乌克兰语从未进行过此类实验，填补这一研究空白是本研究的动机。

**Method:** 招募184名志愿者通过社交媒体参与实验，采用与英语研究相似的技术，让参与者预测句子中的下一个字符，从而近似计算乌克兰语的熵值。

**Result:** 实验得出乌克兰语熵的上限值为H_{upper}≈1.201比特/字符，研究团队还公开了方法、代码以及遇到的主要挑战讨论。

**Conclusion:** 这是首个针对乌克兰语的语言熵研究，不仅填补了该语言在香农实验中的缺失，还为评估大语言模型在乌克兰语上的表现提供了基准数据。
...
> 该论文提出了HATS数据集，研究人类对ASR转录的主观偏好与各种评估指标（词错误率和嵌入基指标）之间的相关性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的词错误率(WER)等ASR评估指标过于系统导向，无法正确评估面向人类的ASR系统性能，需要建立与人类感知更相关的评估方法。

**Method:** 构建HATS人工标注数据集，让143名人类评估员对两个ASR系统的转录结果进行偏好选择，分析人类偏好与传统指标和基于嵌入的指标之间的关联性。

**Result:** 发现了人类主观偏好与不同评估指标之间的具体关系，验证了基于嵌入的指标与人类感知的假设相关性。

**Conclusion:** 研究表明需要开发更符合人类感知的ASR评估指标，HATS数据集为该方向的研究提供了重要参考。
...
> 本研究发布了一个全新的英语对话语音数据集AppTek Call-Center Dialogues，并使用该数据集评估了现有ASR系统在不同口音和分段方法下的性能表现，发现系统在不同口音间存在显著差异。

<details>
  <summary>Details</summary>

**Motivation:** 现有的公开英语语音数据集存在诸多问题：要么是预先分段的短音频，要么是朗读或准备好的语音，要么缺乏明确的方言标注，这使得评估用于会话AI应用的英语ASR系统变得困难，特别是在评估对多样化用户群体的鲁棒性方面。

**Method:** 研究人员创建了AppTek Call-Center Dialogues语料库，包含14种英语口音、16个服务导向场景的自然、角色扮演的客服对话。该数据集专门用于评估，发布前未公开，减少了与现有大规模预训练语料库重叠的风险。随后研究人员对不同开源ASR系统在不同分段方法下进行了基准测试。

**Result:** 测试结果显示，ASR系统在不同口音和分段方法下表现差异显著，表明系统在标准美式英语基准测试上的良好表现并不能保证在其他口音上也能达到同样效果。

**Conclusion:** 该研究强调了需要更多样化的语音数据集来评估ASR系统的泛化能力，并指出当前的ASR系统在处理不同英语口音时仍存在挑战，这对于开发能够服务多样化用户的会话AI应用具有重要意义。
...
> 本研究构建了APPSI-139高质量英文隐私政策语料库，并提出了TCSI-pp-V2混合框架用于隐私政策摘要和解释，在可读性和可靠性方面超越了GPT-4o等大模型。

<details>
  <summary>Details</summary>

**Motivation:** 隐私政策文档通常冗长复杂，充满技术术语和法律术语，导致用户在不知情的情况下接受可能违法的条款。目前缺乏高质量、专门为法律清晰度和可读性优化的英文平行语料库来解决这一问题。

**Method:** 构建APPSI-139语料库，包含139个英文隐私政策、15,692个改写平行语料、36,351个覆盖11个数据实践类别的细粒度标注标签；提出TCSI-pp-V2混合框架，采用交替训练策略，协调多个专家模块来平衡计算效率和准确性。

**Result:** 基于APPSI-139语料库和TCSI-pp-V2框架构建的混合摘要系统在可读性和可靠性方面优于GPT-4o和LLaMA-3-70B等大型语言模型。

**Conclusion:** 通过构建高质量的标注语料库和创新的混合框架，本研究为隐私政策的可读化解释提供了有效解决方案，在性能上超越了现有大模型方法，具有重要的实际应用价值。
...
> JaiTTS-v1.0是一个先进的泰语语音克隆TTS模型，在短语音任务中CER为1.94%超越人类，长语音任务与人类相当，在400次对比评测中以283胜58负显著优于商业竞品。

<details>
  <summary>Details</summary>

**Motivation:** 针对泰语语音合成中数字和泰英代码混合处理的挑战，以及现有模型在真实场景中表现不足的问题，开发了一个专门针对泰语的高质量语音克隆TTS系统。

**Method:** 基于VoxCPM架构构建无需分词器的自回归TTS模型，通过持续训练在大规模泰语语音语料上进行优化，模型能够直接处理数字和泰英代码混合文本而无需显式文本标准化。

**Result:** 在短时长语音生成任务中实现1.94%的CER，超越人类基准1.98%；在长时长任务中与人类水平相当；在人工评估的400次配对比较中，以283胜58负的战绩击败商业旗舰产品。

**Conclusion:** JaiTTS-v1.0证明了通过针对性的架构设计和大规模泰语数据训练，可以在语音克隆任务中达到超越商业产品的性能，特别是在处理真实场景中的数字和代码混合文本方面表现出色。
...
> 本文提出了RoadMap基准测试和RoadMapper多智能体系统，解决了大型语言模型在生成高质量研究路线图方面的能力不足问题，显著提升了性能并节省了大量时间。

<details>
  <summary>Details</summary>

**Motivation:** 研究者常利用结构化内容加速知识获取和问题解决，路线图能指导研究者通过分层子任务逐步解决复杂研究问题。尽管结构化内容生成已有进展，但路线图生成任务尚未被探索。

**Method:** 提出RoadMap基准测试评估LLMs的路线图生成能力，识别出三大限制：缺乏专业知识、不合理的任务分解、混乱的逻辑关系。针对这些问题，设计了RoadMapper多智能体系统，将路线图生成分解为初始生成、知识增强和迭代'批判-修订-评估'三个关键阶段。

**Result:** 实验证明RoadMapper能够有效提升LLMs的路线图生成能力，平均性能提升超过8%，同时节省了人类专家84%的时间，展示了系统的有效性和应用潜力。

**Conclusion:** 这项工作填补了路线图生成研究的空白，通过多智能体系统的方法克服了现有LLMs的局限性，为解决复杂研究问题提供了高效的自动化工具。
...
> CDS是一个包含19万个记录的合成语料库，用于分析大语言模型在不同社会情境提示下生成话语的差异，涵盖社会争议话题并包含人格属性编码。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型能够强烈影响社会话语，但目前缺乏研究LLM在受控社会和上下文提示下输出变化的数据集。

**Method:** 构建包含19个LLM生成的19万条记录的CDS语料库，每个记录由LLM在模拟人类人格或AI助手角色提示下生成，涵盖疫苗/医疗、社交媒体虚假信息、科学性别差距和STEM刻板印象4个争议性社会话题，并编码17个人口统计和心理属性。

**Result:** 创建了支持LLM生成话语分析的CDS语料库，包含LLM对提示、语言、立场和推理的关联数据，提供了带有用户友好仪表板的交互平台，支持跨人格、话题和模型的情感和语义框架比较。

**Conclusion:** CDS提示框架支持未来对LLM偏见、社会敏感性和对齐的审计，为研究LLM的社会影响提供了重要数据资源。
...
> 本研究探索大型语言模型在卢森堡语料中检测语言意识形态的能力，发现虽然模型尚未完全优化多分类任务，但可作为识别语言意识形态的实用工具。

<details>
  <summary>Details</summary>

**Motivation:** 卢森堡多文化多语言社会中，语言意识形态承载深层文化社会意义，影响身份构建和社会归属。传统的检测方法复杂，需要探索NLP工具的辅助作用。

**Method:** 研究人员手工标注卢森堡语用户评论语料库的意识形态类别，随后评估大型语言模型在不同提示条件下的表现。由于卢森堡语是小语种，还测试了将数据机器翻译为高资源语言后的检测效果。

**Result:** 研究表明，虽然大型语言模型在多类意识形态标注任务上尚未完全优化，但能够有效识别语言意识形态内容。机器翻译策略对小语种数据有所帮助。

**Conclusion:** 大型语言模型可成为语言意识形态检测的实用工具，为多文化多语言环境中的身份构建研究提供技术支持，但仍有优化空间。
...
> 研究揭示了跨模态编码器中存在枢纽嵌入问题，并提出了一种识别方法来发现那些能不合理地与多张图像获得高相似度的枢纽文本。

<details>
  <summary>Details</summary>

**Motivation:** 跨模态编码器在跨模态应用中十分重要，但高维嵌入空间中存在的枢纽问题可能对信息检索和自动评估等应用构成实际威胁，因此需要揭示这些潜在漏洞。

**Method:** 提出了一种识别枢纽嵌入及其对应枢纽文本的方法，通过实验验证该方法的有效性。

**Result:** 在MSCOCO和nocaps的图像描述评估以及MSCOCO和Flickr30k的图像到文本检索任务中，该方法能够识别出单个枢纽文本，该文本能不合理地与许多图像获得与人工描述相当甚至更高的相似度分数。

**Conclusion:** 该研究成功揭示了跨模态编码器中存在的脆弱性，表明枢纽问题在跨模态应用中是一个严重的实际威胁，需要引起足够的重视和防护措施。
...
> 该研究构建了一个大规模阿拉伯语诗歌指令数据集InstructPoet-AR，并通过微调使大语言模型能够可控地生成和分析阿拉伯语诗歌。

<details>
  <summary>Details</summary>

**Motivation:** 现有阿拉伯语诗歌的大语言模型研究主要集中在分析任务（如解释、元数据预测），而实际的诗歌创作辅助需求尚未得到满足，缺乏可控的阿拉伯语诗歌生成能力。

**Method:** 构建了包含现代标准阿拉伯语(MSA)和各种阿拉伯语方言的大规模指令数据集，涵盖诗歌写作、修订、续写和分析等任务，支持基于风格、韵律等预定义标准的可控生成。

**Result:** 实验表明，在该数据集上微调的LLM能够有效生成符合用户要求的诗歌，自动指标和母语为阿拉伯语的人工评估均验证了模型表现。

**Conclusion:** 该工作填补了阿拉伯语诗歌可控生成的研究空白，为用户提供了实用的诗歌创作辅助工具，数据集和代码已开源。
...
> 研究提出三层框架分析治疗性文本，发现宏观层面的大语言模型叙述评估在心理健康预测上显著优于词汇和语义嵌入特征，挑战了现有的词计数字典方法。

<details>
  <summary>Details</summary>

**Motivation:** 当前治疗性写作的计算方法存在碎片化问题 - 字典工具忽略话语结构，嵌入特征混淆局部连贯性与全局组织，缺乏将技术映射到叙述构建层次过程的理论框架。

**Method:** 构建三层分析框架：微观词汇特征、中层语义嵌入、宏观LLM叙述评估，对830篇涵盖抑郁、焦虑和创伤的中文治疗文本进行多层次心理健康预测分析。

**Result:** 宏观层面评估显著优于词汇和嵌入特征；正式结构特征（拉博夫故事语法、RST连贯性、命题组合）本身携带预测信号；临床基础的叙述维度捕捉心理状态的表达方式；语义嵌入提供增量分类增益。

**Conclusion:** 基于话语处理理论的三层框架确定了宏观结构组织作为临床信号的主要来源，为干预设计和纵向研究提供了可检验的假设，挑战了领域内过度强调词计数的做法。
...
> 本文提出了一种基于大语言模型的测试时推理方法，通过结构化元数据和对话历史推理，改善视觉环境任务对话中的指代消解问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有任务型对话系统的指代消解存在跨域泛化能力差、过度依赖监督模型的问题，在视觉环境复杂场景中表现不佳。

**Method:** 采用单模态测试时推理方法，让大语言模型通过逐步推理过程，结合详细的对象元数据和对话历史进行指代消解。

**Result:** 在SIMMC 2.1数据集上的实验表明，该方法能有效对齐对话上下文与场景对象，在少样本设置下对新场景和新对象具有良好的泛化能力，跨域评估中优于基于编码器的监督方法。

**Conclusion:** 结构化元数据和精心设计的提示工程对增强任务导向对话系统的鲁棒性和泛化能力至关重要，测试时推理机制为指代消解提供了有效解决方案。
...
> 本文提出了一种名为Conformal Abstention (CA)的后处理框架，通过预测置信度和表征几何校准策略，使语言模型在缺乏相关知识时能够选择性地不作答，同时保证参与率和正确率。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型在面对缺乏相关知识的查询时，往往会产生幻觉而非坦率承认不知道；现有的奖励不知策略会导致过度保守行为且泛化性差。

**Method:** 1. 将符合预测(Conformal Prediction)框架改造为Conformal Abstention后处理框架；2. 使用预测置信度而非CP中的非一致性分数来决定是否拒答；3. 引入基于模型内部表征几何的校准策略来更好地对齐预测置信度与模型无知程度。

**Result:** 实验证明该方法显著改善了选择性回答能力，达到了75%的条件正确率，能够在有限样本下同时保证参与概率和正确率。

**Conclusion:** Conformal Abstention框架为语言模型提供了一种可靠的不确定性量化方法，使其在不确切知道的查询中能够安全地选择拒答，有效减少了幻觉现象。
...
> 该论文评估了三种小型语言模型（EuroLLM、Aya Expanse、Gemma）在德法西意波五种欧洲语言回译过程中保持细粒度情感的能力，并探讨了情感感知提示和ModernBERT分类器的效果。

<details>
  <summary>Details</summary>

**Motivation:** 机器翻译中情感细微差别的保留仍是一个挑战，语义等价性往往优先于情感忠实度，因此需要评估小型语言模型在翻译过程中保持情感细微差别的能力。

**Method:** 使用包含28个类别的GoEmotions数据集，对三种SLMs进行五门欧洲语言的回译测试，评估其固有情感保持能力、情感感知提示的有效性，以及ModernBERT作为BERT替代方案在情感分类中的表现。

**Result:** 研究揭示了SLMs在情感保持方面的固有局限性，情感感知提示能在一定程度上改善效果，ModernBERT展现出比传统BERT更好的情感分类性能。

**Conclusion:** 小型语言模型在机器翻译的情感保持方面仍有改进空间，通过适当的提示技术和先进的情感分类器可以在一定程度上提升情感传输质量。
...
> 这是一篇关于大型语言模型在同行评议全流程应用的综述，涵盖评审生成、评审后任务和评估方法

<details>
  <summary>Details</summary>

**Motivation:** 随着大型语言模型（LLMs）的快速发展，研究者们寻求利用这些技术来辅助或自动化同行评议流程的各个阶段，以提高效率和质量

**Method:** 系统性地整合了三大类技术：(i)同行评议生成方法，包括微调策略、基于代理的系统、基于强化学习的方法以及新兴的生成增强范式；(ii)评审后任务处理，包括反驳、元评审和与评审对齐的文稿修订；(iii)评估方法，涵盖以人为中心、基于参考、基于LLM和面向方面的评估

**Result:** 通过编目数据集、比较建模选择、讨论限制和伦理关切，为构建、评估和整合LLM系统以支持完整的同行评议工作流程提供了实践指导

**Conclusion:** 该综述为研究人员和从业者提供了全面的框架和实用指导，推动LLM技术在同行评议各个环节的有效应用，同时指出了未来发展方向和需要解决的挑战
...
> 研究提出了DPN-LE方法，通过高/低人格特征样本对比定位特定人格神经元，仅需修改约0.5%的神经元即可实现精准人格控制，同时显著保持模型的整体性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于神经元编辑的人格编辑方法需要修改大量神经元，导致显著的性能下降。研究发现神经元具有多重功能，连接人格特征和通用知识，这引发了一个根本问题：所有被修改的神经元是否都与人格表示直接相关？

**Method:** DPN-LE通过对比高特征和低特征样本的MLP激活来识别人格特定神经元，构建分层导向向量，并基于Cohen's d效应量和激活幅度的双标准过滤，分离出互斥的神经元子集，在推理时对这些神经元进行稀疏线性干预。

**Result:** 在LLaMA-3-8B-Instruct和Qwen2.5-7B-Instruct上，使用每人格特征仅1000个对比样本对，DPN-LE修改约0.5%神经元，实现了竞争性的人格控制和明显更好的能力保持效果。

**Conclusion:** 人格特征具有独特的互斥表示模式，通过定位和干预这些特定神经元可以实现精准高效的人格编辑，避免了传统方法中对模型整体性能的负面影响。
...
> 提出了一种名为TeCoD的模板约束解码系统，通过利用历史查询模板提高Text-to-SQL的准确性和效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型在Text-to-SQL生成中面临准确不一致和生成无效SQL的挑战，特别是在复杂或未见过的数据库模式上，实际部署仍然困难。

**Method:** TeCoD系统将历史NL-SQL对转换为可重用模板，使用微调的自然语言推理模型进行鲁棒的模板选择，并通过语法约束强制执行模板生成SQL，采用新颖的分区策略确保语法有效性和效率。

**Result:** 在匹配查询上，系统比上下文学习方法实现了高达36%的执行准确率提升，同时延迟降低了2.2倍。

**Conclusion:** TeCoD通过模板约束解码有效解决了Text-to-SQL生成的准确性和效率问题，为在复杂环境中部署文本到SQL系统提供了实用解决方案。
...
> 研究提出了DriftBench基准来评估多轮LLM辅助科学构思中的约束遵循情况，发现迭代交互会增加复杂性并降低原始约束遵循度，模型存在'知道但违反'现象。

<details>
  <summary>Details</summary>

**Motivation:** 研究人员在与大语言模型迭代完善想法时，模型是否能保持对原始目标的忠实度？这一问题对于确保AI辅助科学研究的可靠性至关重要。

**Method:** 构建DriftBench基准，涵盖2,146次评分运行，包括7个模型、5个提供商、4种交互条件和38个来自24个科学领域的研究简报，通过重述探针测试来评估声明记忆与行为遵循的分离。

**Result:** 迭代压力显著增加结构复杂性并降低约束遵循度；模型的'知道但违反'(KBV)率在8%-99%之间；结构化检查点只能部分缓解但不能完全解决问题；LLM评委低估约束违规情况。

**Conclusion:** LLM在多轮科学构思中存在明显的约束漂移问题，即使在模型明确记住约束的情况下仍会违反，这突显了改进AI辅助研究系统约束管理机制的必要性。
...
> 研究表明星形句法结构的优化景观是凸的，比之前认为的更简单；反依赖距离最小化效应源于竞争原则而非优化难度。

<details>
  <summary>Details</summary>

**Motivation:** 依赖距离最小化原则预测星形结构的中心词应置于中间，但实际观察显示中心词倾向于放在句末，这与理论预期相矛盾。

**Method:** 通过分析星形树和准星形树的优化景观形状，评估优化难度；比较不同结构下依赖距离最小化的收益。

**Result:** 星形结构和准星形结构的优化景观是凸的，优化问题比之前认为更简单；反依赖距离最小化效应是由竞争原则造成，且星形结构上依赖距离最小化的收益较小。

**Conclusion:** 星形句法结构的反依赖距离最小化现象源于竞争压力而非优化难度，星形结构上依赖距离最小化的策略价值相对较低。
...
> 研究发现，大型语言模型通过角色提示在城市情感分析中表现出稳定但有限的行为多样性，且无角色模型有时表现更好，质疑了简单角色提示的实际价值。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型越来越多地被用作城市分析中人类感知的代理，但尚不清楚角色提示是否能产生有意义且可重现的行为多样性。

**Method:** 使用涵盖性别、经济地位、政治取向和人格的因子化角色集，实例化每个角色的多个代理，评估PerceptSent数据集中的城市场景图像，分析角色内一致性和角色间差异。

**Result:** 同角色代理表现出强收敛性，但角色间差异有限：经济地位和人格产生统计显著但实际微小的变异，性别无效果，政治取向影响微乎其微。代理还表现出极端性偏见。

**Conclusion:** 简单的基于标签的角色提示无法捕捉细粒度感知判断，且有时无角色模型与人类标签的一致性更好，表明在此设置下角色提示可能增加有限的标注价值。
...
> 研究发现，对于德语等高资源非英语语言，重复训练经过严格过滤的高质量数据集比使用大量但筛选较少的多样化数据集单次训练效果更好，即使经过7个epoch仍保持优势。

<details>
  <summary>Details</summary>

**Motivation:** 针对德语、法语、日语等高资源非英语语言，在训练语言模型时面临策略选择：是优先考虑数据多样性（使用大量轻度过滤的网络数据），还是优先考虑数据质量（严格过滤获得高质量核心数据并重复训练）。

**Method:** 构建分层质量过滤器处理5亿网络文档，比较在过滤后的数据子集上进行多epoch训练与在多样化语料库上进行单次训练的效果，实验涵盖多个模型规模和token预算。

**Result:** 重复使用高质量数据的训练策略始终优于使用更大但过滤较少的数据集进行单次训练，即使在7个epoch后性能差距依然存在。发布的德语模型Boldt在token数量减少10-360倍的情况下仍达到最先进结果。

**Conclusion:** 对于非英语大语言模型，通过质量过滤实现语义集中是比单纯最大化独特数据量更有效的语言建模路径。质量优先而非数量优先的策略能显著提升训练效率和模型性能。
...
> 本研究提出了TopBench基准测试，用于评估大语言模型在表格问答中处理隐含预测任务的能力，发现现有模型在意图识别和预测推理方面存在显著不足。

<details>
  <summary>Details</summary>

**Motivation:** 现有的表格问答系统主要处理信息提取和简单聚合，但现实世界中存在大量需要基于历史模式进行隐含预测的查询，这对大语言模型的意图识别和预测推理能力提出了更高要求。

**Method:** 构建了包含779个样本的TopBench基准，涵盖单点预测、决策制定、治疗效果分析和复杂过滤四个子任务，通过基于文本和智能体工作流两种方式评估多样化模型的性能。

**Result:** 实验表明当前模型普遍存在意图识别困难，倾向于简单的查找操作而非预测推理。准确理解查询意图是触发预测行为的前提条件，而提升预测精度需要集成更复杂的建模或推理能力。

**Conclusion:** 大语言模型在隐含预测性表格问答任务中性能受限，关键在于意图识别能力的提升和预测推理机制的改进，这为未来的模型优化指明了方向。
...
> 本文提出将分析单位的定义与感兴趣区域的选择这两个建模决策进行解耦，主张将分词视为实现细节而非科学基础，从而为任意语料库的意外性分析提供统一框架。

<details>
  <summary>Details</summary>

**Motivation:** 当前意外性理论研究中存在一个根本问题：实验刺激被分割为语言学单位（如单词），但预训练语言模型却基于固定的词元字母表分配概率，导致分析单位的定义和预测评估区域的选混淆不清。

**Method:** 通过区分分析单位的定义和预测评估区域的选择这两个独立建模决策，建立了统一的意外性分析框架，使研究者能够明确地处理这些选择，而非依赖临时程序。

**Result:** 提出的方法使意外性分析更加明确和系统化，避免了因分词策略选择而产生的混淆，为跨不同语言资源的意外性研究提供了统一的处理方法。

**Conclusion:** 意外性分析应该将分词视为技术实现细节而非科学基础，通过明确建模决策来提高研究的可重复性和比较性，这为未来认知科学和自然语言处理的交叉研究提供了方法论指导。
...
> 本文提出了一种基于双流Transformer架构的高效方法，用于在多摄像机环境下自动检测相互注视(MG)和共同注意(JA)，显著优于传统方法。

<details>
  <summary>Details</summary>

**Motivation:** 在发展心理学研究中，相互注视(MG)和共同注意(JA)的分析至关重要，但传统手动编码方法劳动密集且效率低下。多摄像机实验室环境中的自动化检测面临复杂的跨摄像机关系动态计算挑战。

**Method:** 采用高效的 dual-stream Transformer 架构，利用冻结的 gaze-aware backbone (GazeLLE)提取丰富的视觉先验知识，结合自定义的 token 融合机制来映射交互二元对之间的空间和语义关系。

**Result:** 在生态有效的照料者-婴儿交互数据集上评估，模型表现良好，显著优于卷积基线模型和最先进的多模态大语言模型(LLM)。通过开源模型和预训练权重，为行为科学家提供了可扩展的工具。

**Conclusion:** 该研究成功将计算建模与应用交互研究相结合，为发展心理学研究提供了一种可靠、高效的自动化MG和JA检测方案，可通过微调适应实验室环境多样化需求。
...
> RecGen是一个从稀疏RGB-D图像中重建复杂多物体场景的生成式框架，在形状、纹理和姿态估计上都超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 从稀疏观测准确重建复杂完整多物体场景是计算机视觉的核心挑战，也是机器人可靠仿真的关键步骤。

**Method:** 提出RecGen生成式框架，利用组合式合成场景生成和强大的3D形状先验，从单个或多个RGB-D图像中概率性地联合估计物体和部件形状及其在被遮挡和部分可见情况下的姿态。

**Result:** RecGen在复杂、严重遮挡数据集上达到最先进性能，使用比SAM3D少80%的训练网格，但在几何形状质量上提升30.1%，纹理重建提升9.1%，姿态估计提升33.9%。

**Conclusion:** RecGen通过组合合成训练和强3D先验，实现了对严重遮挡、对称物体、部件以及复杂几何纹理的鲁棒处理，为机器人仿真提供了可靠的多物体场景重建方案。
...
> InterPartAbility是一种可解释的文本到图像行人重识别方法，通过显式的部分匹配和短语-区域定位来实现可解释性，同时保持竞争性的检索准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本到图像行人重识别方法虽然检索性能强，但决策过程缺乏可解释性。传统的可解释方法只能简单地高亮注意力区域，无法将视觉区域与语义概念可靠绑定，限制了解释的应用范围。

**Method:** 提出InterPartAbility框架，包含：1）轻量级的开放词汇补丁-短语交互模块(PPIM)，提供概念级监督；2）约束CLIP ViT自注意力机制产生空间集中的补丁激活，与部分级短语对齐；3）引入基于扰动的可解释性评估协议，包括反事实区域掩码等定量指标。

**Result:** 在CUHK-PEDES和ICFG-PEDES等基准数据集上，InterPartAbility实现了最先进的可解释性性能，同时保持了竞争性的检索准确率。

**Conclusion:** 该方法首次实现了文本到图像行人重识别的定量可解释性评估，通过显式的部分匹配和概念级监督，成功地在保持检索性能的同时提供了可靠的决策解释。
...
> 本研究通过知识蒸馏技术，将446M参数的SAM 3模型压缩到40.66M参数，在保持精度的同时大幅降低GPU内存需求，使其能够在边缘设备上部署用于牲畜个体监测。

<details>
  <summary>Details</summary>

**Motivation:** 基于基础模型的个体牲畜监测管道虽然精度高，但GPU内存预算超出了商品级边缘加速器的承载范围，需要技术突破来实现边缘部署。

**Method:** 通过三个机制实现模型蒸馏：1) 基于TinyViT-21M-512构建的FPN学生编码器；2) 四项方向-尺度蒸馏损失；3) 带滑动窗口会话修剪的主干替换推理。

**Result:** 压缩后的管道在爱丁堡猪数据集上达到92.29% MOTA和96.15% IDF1，参数减少7.77倍，VRAM峰值降低3.01倍，行为分类准确率达97.34%，可在Jetson Orin NX 16GB上部署。

**Conclusion:** 该压缩方案成功将高精度牲畜监测模型部署到边缘设备，为长期个体牲畜视觉记录和疾病预测提供了可行的技术基础。
...
> 本研究通过知识蒸馏技术，成功将大型预训练模型的植物识别能力迁移到小型高效模型中，在保持低计算成本的同时实现了接近大模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大规模视觉模型在植物物种和疾病识别方面表现出色，但计算成本高昂，难以在资源受限的移动或边缘设备上部署，限制了自动生物多样性监测和精准农业系统的可扩展性。

**Method:** 采用知识蒸馏方法，在Pl@ntNet300K-v2和Deep-Plant-Disease两个基准数据集上对四种代表性架构（两个ConvNeXt和两个视觉Transformer）进行广泛实验研究，比较了从头训练、预训练初始化以及有/无蒸馏的不同训练方案，总共训练评估了70个模型。

**Result:** 知识蒸馏在所有任务和架构中都持续提升性能，蒸馏后的小型模型能够匹配显著更大模型的性能，同时保持 substantially 更低的计算成本。

**Conclusion:** 知识蒸馏技术为实现高效、可扩展的植物识别系统在真实环境应用中的部署提供了可行方案，有助于推动自动化生物多样性监测和精准农业的发展。
...
> 本文提出了HQ-UNet，一种混合量子-经典U-Net架构，在经典U-Net的瓶颈层集成紧凑的参数化量子电路，用于遥感图像语义分割，在LandCover.ai数据集上超越经典U-Net基线。

<details>
  <summary>Details</summary>

**Motivation:** 经典深度学习架构如U-Net需要大量参数来建模复杂空间关系，而量子机器学习虽然提供了新的表示范式，但直接应用于高维图像在近期量子硬件约束下仍然具有挑战性。

**Method:** HQ-UNet采用混合架构，在经典U-Net的编码器-解码器瓶颈处插入紧凑的参数化量子电路，使用非池化量子卷积模块来丰富高度压缩的编码器特征，同时保持量子组件的浅层结构和参数效率。

**Result:** 在LandCover.ai数据集上，HQ-UNet实现了0.8050的平均IoU和94.76%的整体准确率，超越了经典U-Net基线的性能。

**Conclusion:** 紧凑的量子瓶颈可以在近期量子约束条件下增强遥感图像分割的特征表示，突出了混合量子-经典设计作为地球观测中参数高效密集预测的可行方向。
...
> 该研究发现基于Transformer的行人重识别模型在不同层和训练阶段会编码不同强度的隐式属性信息，其中BMI信息在深层表示中占据主导地位，而跨光谱条件下姿态信息的重要性增强。

<details>
  <summary>Details</summary>

**Motivation:** 现有行人重识别系统容易受到性别、姿态、BMI等可变属性的影响，在无约束环境中存在公平性和泛化性问题，需要量化这些属性在模型特征中的编码强度。

**Method:** 通过扩展表达性概念（学习特征与特定属性间的互信息），使用辅助神经网络量化属性编码强度，在三种基于Transformer的ReID模型上进行大规模可见光数据集和跨光谱红外数据集分析。

**Result:** BMI在深层网络中始终具有最高表达性，最终表示中属性排序为BMI > Pitch > Gender > Yaw。表达性随网络深度和训练时期动态变化，姿态在中间层达峰值，BMI随深度增强。跨光谱条件下，pitch与BMI相当，属性趋势随深度单调递增。

**Conclusion:** Transformer-based ReID嵌入编码了一个隐式属性层次结构，形态测量信息被持续嵌入，而在跨光谱条件下姿态信息贡献更强，表明模型在跨越模鸿沟时更依赖结构线索。
...
> 本研究提出了一种基于深度学习的模块化工作流程，能够从多种地球观测数据中大规模绘制线性木质特征（如树篱）地图，并在德国全国范围内验证了其有效性和可扩展性。

<details>
  <summary>Details</summary>

**Motivation:** 线性木质特征在农业景观中提供重要的生态系统服务，包括气候适应和生物多样性保护，但由于传感器类型、空间分辨率和景观可变性的多样性，现有的大规模制图方法缺乏迁移性和可重用性。

**Method:** 构建了一个包含两个独立优化组件的模块化工作流程：一个灵活的输入数据接口将异构地球观测数据整合为二值化木质植被掩模，一个深度神经网络在这些掩模中将线性形状与非线性形状分离。使用单一训练模型在德国全国范围内从三种输入源生成了线性木质特征图。

**Result:** 在联邦州生物群落制图数据的四个评估站点上，工作流程产生了具有竞争性的结果。与现有两种线性木质特征地图的对比表明，该方法在全国层面上表现优异。

**Conclusion:** 该模块化设计和在全国规模上的应用为德国乃至其他地区的可扩展和通用化线性木质特征制图奠定了基础。
...
> 本研究提出VTBench框架，通过将时间序列转换为可视化图表并与原始数据融合，探索多模态时间序列分类的新方法，在31个数据集上验证了该方法的有效性并提供了实践指导。

<details>
  <summary>Details</summary>

**Motivation:** 现有的时间序列分类主要依赖原始数值输入，忽略了可视化表示方法。虽然已有纹理编码（如GAF、RP）将时间序列转为2D图像，但预处理复杂且表示不够直观。基于图表的可视化提供了更直观的替代方案，但其有效性尚未得到系统性评估。

**Method:** 提出VTBench框架，将时间序列转换为轻量级、人类可解释的图表（线图、面积图、柱状图、散点图），开发支持多种融合策略的模块化架构，包括单图表视觉数值融合、多图表视觉融合和完整的多模态融合。

**Result:** 在31个UCR数据集上的实验表明：(1)纯图表模型在小数据集上表现优异；(2)多图表组合能通过捕获互补的视觉特征提高准确率；(3)当视觉特征提供非冗余信息时，多模态模型能提升或保持性能，但引入冗余时可能降低准确率。

**Conclusion:** VTBench为可解释且有效的多模态时间序列分类建立了统一基础，提供了图表类型选择、融合策略和配置的实用指导原则。
...
> 基于YOLOv8s改进的ALC-YOLOv8s模型，通过多模块优化提升了复杂课堂环境下的学生行为识别性能。

<details>
  <summary>Details</summary>

**Motivation:** 针对真实课堂场景中学生目标密集、小目标众多、频繁遮挡和类别分布不均衡等问题，提出一种改进的学生课堂行为识别模型。

**Method:** 在YOLOv8s基础上，引入SPPF-LSKA增强上下文特征提取，采用CFC-CRB和SFC-G2优化多尺度特征融合，并结合ATFLoss改善少数类和困难样本的学习能力。

**Result:** 相比基线模型，mAP50提升1.8%，mAP50-95提升2.1%，能够较好满足复杂课堂场景下学生行为自动识别的需求。

**Conclusion:** ALC-YOLOv8s模型通过技术创新有效解决了课堂行为识别中的关键技术挑战，为教学质量分析提供了可靠的技术支撑。
...
> YOSE是一个基于DiT的高效视频对象移除框架，通过选择性处理关键tokens实现约2.5倍加速，同时保持视觉质量。

<details>
  <summary>Details</summary>

**Motivation:** 当前基于Diffusion Transformer的视频对象移除方法存在推理延迟高的问题，即使是只处理小区域也需要对整个时空token空间进行密集计算，导致如MiniMax Remover等最先进方法仅能达到约10FPS的处理速度。

**Method:** YOSE框架包含两个核心组件：1) 批次变长索引(BVI)算子，基于mask信息自适应选择必要tokens，实现样本间的变长token处理；2) 扩散过程模拟器(DiffSim)，为未选中的tokens提供扩散过程近似，模拟其对DiT自注意力的影响以保持语义一致性。

**Result:** 在70%的情况下YOSE实现高达2.5倍的加速，推理时间与mask大小近似线性相关，而非传统全token方法的固定计算开销，同时保持与基线相当的视频质量。

**Conclusion:** YOSE成功实现了mask感知的加速机制，解决了现有技术中无论mask大小都需全量计算的低效问题，为实时视频对象移除应用提供了有效解决方案。
...
> 本文提出了一个无需训练的自适应迭代框架，通过优化类别定义而非更新模型参数来改善零样本网页内容分类，显著提高了分类准确率。

<details>
  <summary>Details</summary>

**Motivation:** 网页过滤系统面临动态网页内容分类的挑战。现有的零样本方法对类别定义质量高度敏感，定义不明确或模糊会导致语义重叠和系统性误分类，但这一关键因素尚未得到充分探索。

**Method:** 提出训练-free的自适应迭代定义优化框架，使用大语言模型作为反馈驱动的定义优化器。设计了三种优化策略：示例引导、混淆感知和历史感知，通过误分类实例的结构化信号来精炼类别描述。

**Result:** 在包含10个URL类别（每类1000样本）的新基准数据集和13个先进嵌入模型上的实验表明，迭代定义优化能一致地提高分类性能，证明了定义质量在嵌入系统中的关键作用。

**Conclusion:** 定义质量是嵌入式系统的一个关键且被低估的因素。所提出的框架通过优化类别定义而非模型参数，为改进零样本网页内容分类提供了新的有效途径，同时发布了开源数据集供后续研究。
...
> JI-ADF是一个三模态深度学习框架，通过整合皮肤镜图像、临床照片和患者元数据进行皮肤病变分类，具有自适应决策融合和多模态注意力机制。

<details>
  <summary>Details</summary>

**Motivation:** 现有的计算机辅助皮肤病诊断系统主要依赖皮肤镜图像，忽视了临床实践中可用的多模态信息，限制了诊断性能和实用性。

**Method:** 提出JI-ADF框架，包含联合多模态表示学习、模态特定辅助监督、自适应决策融合机制以及多模态融合注意力(MMFA)模块，实现跨模态推理和模态特定证据的保留。

**Result:** 在MILK10k基准数据集上表现出强且均衡的性能，提高了敏感度和Dice分数，同时保持高特异性和良好校准，验证了模型的鲁棒性和临床意义。

**Conclusion:** JI-ADF为现实临床环境中的多模态皮肤病变分类提供了可靠且实用的基础，充分挖掘了多模态信息的诊断价值。
...
> 该论文提出了一种基于多分支深度残差学习的高精度步态识别框架，通过融合步态动力学和体型特征，在CASIA-B数据集上实现了94.52%的Rank-1识别准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的步态识别方法难以全面捕捉和利用人体运动中嵌入的丰富生物特征信息，特别是在视角变化、服装变化和携带条件等协变量干扰下表现不佳。

**Method:** 使用HRNet进行骨骼关键点估计，构建体型比例、步态速度和骨骼运动三个互补的特征分支，采用ResNet-50作为骨干网络提取层次化特征，设计多分支特征融合模块动态整合异构特征流。

**Result:** 在CASIA-B数据集上，正常行走条件下达到94.52%的Rank-1准确率，在穿着外套条件下展现了基于骨骼方法的最佳识别性能。

**Conclusion:** 该框架通过深度提取和协同融合步态动力学与体型特征，有效解决了视角变化、服装变化等协变量干扰问题，为步态识别提供了一种高精度解决方案。
...
> CasLayout是一种级联扩散框架，通过将室内场景生成分解为四个条件子阶段，结合稀疏关系图和建筑元素约束，实现了更真实、可控的3D室内场景合成。

<details>
  <summary>Details</summary>

**Motivation:** 现有3D室内场景生成方法面临数据稀缺、全局建筑约束与局部语义一致性难以同时满足的问题，常常忽视结构边界或引入冗余生成错误。

**Method:** 提出级联扩散架构，将联合场景生成分解为四个明确的条件子阶段：(1)预测家具数量和类别，(2)优化对象尺寸和特征嵌入，(3)在潜在空间建模空间关系，(4)生成定向边界框(OBBs)。引入稀疏关系图 formulation 和双向VAE编码，显式建模建筑元素作为条件约束。

**Result:** 实验证明CasLayout在保真度和多样性方面达到最先进性能，同时在实际应用中提供改进的可控性，支持零样本的图像到场景生成任务。

**Conclusion:** 该框架通过解耦架构减少了数据需求，灵活集成LLMs和VLMs，为3D室内场景生成提供了更符合功能组织的可控解决方案。
...
> 本文提出了一种名为DSCC的双阶段谱约束聚类分类器，通过解耦聚类和分类过程，实现了高光谱图像的区域级边界对齐分类，在保持高精度的同时显著提升了处理效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于超像素的高光谱图像分类方法存在内在矛盾：聚类将相似像素聚合为区域，但后续分类器仍按像素操作，破坏了区域一致性，无法保证区域级、边界对齐的分类效果。

**Method:** DSCC采用双阶段端到端框架：第一阶段通过多准则特征距离和局部感知分配正则化生成边界保持的光谱超token，使用密度隔离的中心选择确保代表性；第二阶段引入软标签方案处理混合地物覆盖，进行token级预测。

**Result:** 在WHU-OHS数据集上达到0.728的CF1值，处理速度为197.75 FPS，相比现有方法实现了更好的精度-效率权衡，大量实验验证了双阶段范式的有效性和通用性。

**Conclusion:** 通过显式解耦聚类与分类过程，DSCC成功解决了传统方法的区域一致性问题，为高光谱图像分类提供了一个高效且准确的解决方案，具有良好的实际应用前景。
...
> CriticVLA是一个两阶段自动驾驶框架，通过引入VLA的评判能力来优化驾驶决策，构建了1290万条轨迹数据集，在Bench2Drive基准上成功率提升30%。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言动作(VLA)模型在自动驾驶中仅用于从多模态输入映射到控制信号，没有利用VLA的评判能力来优化驾驶决策，限制了其在复杂闭环场景中的性能表现。

**Method:** 提出CriticVLA两阶段框架：首先生成初始轨迹，然后通过基于VLA的评判器进行多模态评估和单步优化来精炼轨迹；同时构建了包含1290万条标注轨迹的大规模合成数据集来增强评判器的推理和精炼能力。

**Result:** 在Bench2Drive基准测试中，CriticVLA显著超越了现有基线方法，总成功率达到73.33%，在挑战性场景中实现了约30%的性能提升。

**Conclusion:** 通过将VLA的角色从执行者扩展为评判者，并配备大规模数据集，CriticVLA能够生成更高质量的驾驶行为，证明了在自动驾驶中利用VLA评判能力的有效性。
...
> VeraRetouch是一个轻量级、完全可微分的多任务照片润色框架，结合0.5B视觉语言模型和可微分润色渲染器，实现了端到端训练，并首次构建百万级数据集AetherRetouch-1M+。

<details>
  <summary>Details</summary>

**Motivation:** 现有推理式照片润色方法依赖非可微分外部软件，存在优化障碍、参数冗余高和泛化能力有限的问题，需要构建轻量级、完全可微分的端到端框架。

**Method:** 采用0.5B视觉语言模型制定润色计划，开发完全可微分的Retouch Renderer替代外部工具，通过解耦光照、全局色彩和特定色彩调整的控制潜变量实现端到端训练，构建AetherRetouch-1M+数据集并提出DAPO-AE强化学习策略增强审美认知。

**Result:** 在多个基准测试中取得最先进性能，模型体积显著更小，可实现移动端部署，代码和模型已开源。

**Conclusion:** VeraRetouch成功解决了现有方法的局限性，为轻量级、高性能的照片润色提供了新的解决方案，推动了移动端图像处理技术的发展。
...
> 研究人员提出了COHERENCE基准，用于评估多模态大模型在图文交错场景中的细粒度理解能力，包含6,161个高质量问题和系统的错误分析框架。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态基准主要关注单图像或多图像理解，缺乏对现实世界中常见的图文交错场景的系统性评估。这类场景需要模型识别相关文本和视觉证据，建立细粒度对齐，并基于上下文证据进行推理。

**Method:** 构建了COHERENCE基准，涵盖四个代表性领域的图文交错内容，包含6,161个高质量问题。同时设计了六类错误分析方法，可对模型在图文交错理解中的失败进行细粒度归因分析。

**Result:** 提出了首个专门评估MLLM在图文交错场景中细粒态理解能力的综合基准COHERENCE，为该领域提供了重要的评估工具和洞见。

**Conclusion:** COHERENCE基准填补了多模态大模型在图文交错场景评估方面的空白，为理解当前模型的具体能力缺陷提供了细粒度分析框架，将推动相关研究的发展。
...
> 物理对抗性攻击在不同视觉语言模型(VLM)架构间具有高度可迁移性，即使在未针对目标模型优化的情况下，攻击成功率仍达73-91%。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型在自动驾驶中应用广泛，但其对物理对抗攻击的鲁棒性，特别是跨架构的攻击可迁移性问题尚未充分研究，当攻击者不知道车辆使用何种模型时，这构成实际安全风险。

**Method:** 通过在人行横道和高速场景的路边基础设施上部署物理可实现的补丁，系统性地评估三种代表性VLM架构(Dolphins、OmniDrive和LeapVAD)间的对抗性攻击可迁移性，采用迁移矩阵评估方法。

**Result:** 实验显示跨架构攻击效果显著，迁移率在73-91%之间（人行横道平均0.815，高速公路平均0.833），在关键决策窗口的64.7-79.4%帧内实现持续级别的操控。

**Conclusion:** 物理对抗攻击在不同VLM架构间具有高度可迁移性，即使攻击没有针对目标模型进行优化仍能有效执行，凸显了自动驾驶系统中VLM模型安全的严重漏洞。
...
> 提出了一种面向无约束真实场景的3D稀疏视图合成框架，通过参考引导的视图细化和伪视图生成策略，有效处理包含干扰物的稀疏图像集合，实现了高质量的3D渲染效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法要么只能处理受约束的无暂态元素稀疏图像集，要么需要密集图像集合来增强无约束真实场景的3D表示，但缺乏能够同时处理稀疏且无约束图像集的有效解决方案。

**Method:** 1) 引入参考引导的视图细化技术，使用扩散模型结合暂态掩码和参考图像来增强3D表示并减轻渲染伪影；2) 通过伪视图生成和稀疏感知高斯复制策略来解决高斯场中的稀疏区域问题，在稀疏区域扩增高斯点。

**Result:** 在公开数据集上的实验表明，该方法在各项指标上均显著优于现有方法（PSNR提升17.2%，SSIM提升10.8%，LPIPS提升4.0%），能够提供高保真度的3D渲染结果。

**Conclusion:** 该研究为无约束真实世界场景的3D重建开辟了新途径，避免了对密集数据采集的依赖，显著降低了劳动强度，推动了稀疏视图合成技术的发展。
...
> Softmax-GS 通过在重叠高斯函数间引入softmax竞争机制，同时解决了3D高斯泼溅技术的视角不一致性和扩散边界问题，实现了从平滑混合到清晰边界的连续控制。

<details>
  <summary>Details</summary>

**Motivation:** 3D高斯泼溅技术虽然训练和渲染效率高，但依赖高斯不重叠的关键假设，导致明显伪影和视觉不一致，同时高斯的扩散边界无法精确重建物体锐利边缘。

**Method:** 提出Softmax-GS统一解决方案，在两个重叠高斯间执行基于softmax的竞争，通过可学习参数控制竞争强度，实现从平滑颜色混合到清晰边界的连续谱；明确保持重叠高斯的顺序不变性，确保透射率恒定。

**Result:** 在简单几何体上的消融实验验证了各组件有效性，在真实世界基准测试中达到最先进性能，显著提升了重建质量和参数效率。

**Conclusion:** Softmax-GS为3D高斯泼溅技术提供了有效改进，通过softmax竞争机制同时解决了视角不一致和边界模糊问题，实现了更好的新视图合成效果。
...
> CatSignal是一个多模态意图推理框架，通过贝叶斯方法将空间上下文建模为先验约束，将行为观察作为证据，在动物意图推理任务中达到77.72%准确率并有效减少上下文导致的预测错误。

<details>
  <summary>Details</summary>

**Motivation:** 现实世界中存在大量无法通过语言可靠传达目标的智能体，如宠物、前语言期婴儿等。从这些智能体的行为观察中推断意图面临核心挑战：可观察行为往往嘈杂或不充分，而上下文虽能提供强先验信息但会导致脆弱的捷径预测。

**Method:** 提出了受贝叶斯启发的概率推理框架CatSignal，将空间上下文建模为先验约束，行为观察作为证据。采用上下文门控的Product-of-Experts公式，从上下文、姿态动态和声学提示计算后验意图分布，而非简单地将上下文作为输入特征。

**Result:** 在多模态家猫数据集的Leave-One-Video-Out评估中，提出的方法实现77.72%的整体准确率，优于特征拼接(71.83%)和更强的后期融合基线。更重要的是，该方法显著减少了模糊情境中的上下文驱动捷径失败。

**Conclusion:** 虽然更简单的融合策略在Macro-F1和选择性预测上仍有竞争力，但提出的模型提供了最强的整体准确率并最佳地抑制了基于上下文的捷径坍塌，为非语言智能体的意图推理提供了一个有效的框架。
...
> LA-Pose通过逆动力学自监督预训练学习潜在动作表示，重新用于相机位姿估计，在Waymo和PandaSet基准上比现有方法提升10%以上准确率。

<details>
  <summary>Details</summary>

**Motivation:** 当前相机位姿估计方法严重依赖大量3D标注数据进行全监督训练，标注成本高昂且难以扩展。

**Method:** LA-Pose方法首先通过逆动力学和前向动力学模型从大规模驾驶视频中学习潜在动作表示，然后将这些潜在动作特征重新用作相机位姿估计器的输入，最后在少量高质量3D标注数据上微调。

**Result:** 在Waymo和PandaSet驾驶基准数据集上，LA-Pose比最新的前馈方法实现了超过10%的位姿准确率提升，同时使用的标注数据量少几个数量级。

**Conclusion:** 这是首个展示逆动力学自监督学习在位姿估计任务中强大作用的工作，为少标注数据场景下的相机位姿估计提供了有效的解决方案。
...
> EdgeFM是一个轻量级、智能体驱动的视觉语言模型推理框架，专为跨平台工业边缘部署设计，通过封装智能体优化的内核技能库，在保持开源的同时实现超越闭源工具链的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的VLM部署框架要么过于臃肿，要么依赖于硬件特定的闭源生态系统，导致硬件锁定和跨平台适应性差，无法满足工业边缘应用对确定性低延迟和稳定执行的要求。

**Method:** EdgeFM通过移除非必要功能降低单请求延迟，将智能体调优的内核优化封装为可复用的技能模块库，支持直接调用而非依赖闭源实现，原生支持x86、NVIDIA Orin和地平线征程等主流平台。

**Result:** 在NVIDIA Orin平台上相比TensorRT-Edge-LLM实现1.49倍加速，首次在地平线征程平台实现端到端VLA部署，在大多数场景下性能明显优于传统厂商特定工具链。

**Conclusion:** EdgeFM为多样化边缘工业场景提供了开源、生产级的解决方案，有效弥合了长期以来被专有工具链主导的性能差距，增强了跨平台可移植性。
...
> Uni-HOI是一个统一的4D人机交互框架，通过将文本、人体运动和物体运动转换为令牌序列，实现了多种HOI任务的联合建模。

<details>
  <summary>Details</summary>

**Motivation:** 现有的4D人机交互方法通常针对特定任务设计，缺乏能够处理多样化条件输入的统一框架，限制了其在虚拟和混合现实应用中的推广应用。

**Method:** 提出Uni-HOI统一框架，利用大语言模型和两个专门的VQ-VAE将异构运动数据转换为LLM兼容的令牌序列；采用两阶段训练策略：第一阶段在大规模HOI数据集上进行多任务学习，第二阶段针对特定任务进行微调。

**Result:** 在多个HOI相关任务上取得了显著性能，包括文本驱动的HOI生成、物体运动驱动的人体运动生成（可选择带文本）以及人体运动驱动的物体运动预测。

**Conclusion:** Uni-HOI成功实现了在单一框架内处理多种条件输入的HOI任务，为4D人机交互的统一建模提供了有效的解决方案，在虚拟和混合现实应用中具有重要价值。
...
> 本文提出了IRON数据集和IRONet方法，解决了越野夜间自动驾驶中红外自由空间检测的挑战，实现了SOTA性能并展示了良好的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 越野夜间自动驾驶存在可见光感知不可靠的问题，红外模态对准确检测自由空间至关重要。然而，由于缺乏标注的红外越野数据集以及当前单帧方法的帧间不一致性问题，该领域的研究进展有限。

**Method:** 构建了首个大规模全日条件下的红外越野时序自由空间检测数据集IRON，包含24,314张密集标注的红外图像和同步RGB图像。基于该数据集，提出了无需光流的IRONet框架，通过内存注意力机制和精心设计的掩码解码器聚合历史上下文来解决帧间不一致性问题。

**Result:** 在IRON数据集上，IRONet达到82.93%(+1.19%) IoU和90.66%(+0.71%) F1分数，实现实时推理。令人印象深刻的是，IRONet在ORFD和Rellis数据集上也展现出对RGB模态的鲁棒泛化能力。

**Conclusion:** 这项工作为可靠的全日越野自动驾驶和红外时序感知的未来研究奠定了基础，提供了首个大规模红外数据集和有效的时序检测框架。
...
> REVIVE 3D是一个两阶段即插即用的管道，用于从平面图像生成体积感强的3D资产，通过膨胀先验和3D潜在细化技术实现SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的生成模型在从提供有限3D线索的平面图像生成体积感强的3D资产时存在困难，这是计算机视觉和图形学中的一个基本研究问题。

**Method:** REVIVE 3D采用两阶段方法：第一阶段构建膨胀先验，通过膨胀前景轮廓恢复全局体积并叠加部分感知细节捕获局部结构；第二阶段进行3D潜在细化，向膨胀先验的潜在空间注入高斯噪声然后去噪，利用先验的几何线索来利用骨干网络的预训练3D知识。

**Result:** 在具有挑战性的平面图像数据集上，REVIVE 3D在定性和定量评估中都达到了最先进的性能。提出的Compactness和Normal Anisotropy指标通过用户研究验证，证明与人类对体积和质量的感知一致。

**Conclusion:** REVIVE 3D有效地解决了从平面图像生成体积感强的3D资产的挑战，其两阶段框架和提出的评估指标为3D生成领域提供了新的解决方案，支持图像条件化的3D编辑功能。
...
> Edit-R1是一个基于推理验证器的图像编辑强化学习框架，通过构建细粒度的链式思维奖励模型来改进图像编辑效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有图像编辑的RLHF方法缺乏通用的奖励模型，现有模型只给出整体评分而忽略不同指令要求，导致奖励偏差且无法详细检查编辑效果。

**Method:** Edit-R1构建基于CoT的推理奖励模型(RRM)，将指令分解为不同原则并逐一评估；首先通过监督微调生成CoT奖励轨迹，然后提出群体对比偏好优化(GCPO)算法利用人类成对偏好数据强化RRM，最后用GRPO训练编辑模型。

**Result:** Edit-R1的RRM在专业编辑奖励模型上超越了Seed-1.5-VL等VLMs，表现出明显的规模效应(3B到7B参数性能持续提升)，同时为FLUX.1-kontext等编辑模型带来了显著改进。

**Conclusion:** 从简单评分器转向推理验证器是图像编辑RLHF的关键突破，Edit-R1框架通过细粒度的可解释奖励机制有效提升了图像编辑质量。
...
> 本研究提出了一种无幻觉的反演框架，揭示了CNN编码器中的强叠加效应，证明分类是通过破坏性干扰而非空间漏斗实现的，并基于此发展了协方差-体积通道选择算法。

<details>
  <summary>Details</summary>

**Motivation:** CNN可解释性的基本假设——空间漏斗假设（编码器抑制背景，分类器仅选择特征）——由于现有可视化工具的空间幻觉问题，一直未被验证。需要新的方法来准确探测网络的内部工作机制。

**Method:** 提出基于幅度-相位解耦和局部伴随校正器的无幻觉反演框架，通过数学保证每个重构的空间梯度支路严格来自真正活跃的通道，以此作为几何探针来分析视觉编码器。

**Result:** 首次发现了视觉编码器中强叠加的像素级证据，证明分类器通过破坏性干扰工作：权重在像素空间中抵消共享的背景方向，建设性地组合类判别残差，直接证伪了空间漏斗假设。

**Conclusion:** 建立了基于干扰的分类模型，识别出可容许干扰子空间的体积是控制通道需求的关键几何量，证明了该体积与GAP协方差行列式的对偶性，并相应开发了具有(1-1/e)近似保证的协方差-体积通道选择算法，揭示了OOD失效的本质。
...
> 本研究发现传统的自监督学习增强方法不适用于植物识别，提出域适应的增强策略和数据集选择方案，训练出的模型在少样本植物识别任务上表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 当前植物物种识别严重依赖有监督学习，但专家标注数据稀缺。自监督学习提供可扩展替代方案，但现有方法和训练协议主要针对粗粒度视觉任务设计，在植物等细粒度识别领域迁移效果不佳。

**Method:** 1. 分析传统SSL增强方法（高斯模糊、灰度转换、像素化）对植物图像的负面影响；2. 提出适合植物识别的替代增强策略（仿射变换和海报化）；3. 使用iNaturalist 2021植物子集训练SimDINOv2模型；4. 在ViT-Base和ViT-Large架构上验证方法一致性。

**Result:** 提出的域适应增强策略显著提升植物图像表征质量；在iNaturalist植物数据集上训练的自监督模型优于ImageNet-1K预训练模型；在少样本下游任务中，模型性能与强监督基线相当甚至超越Pl@ntCLEF和BioCLIP。

**Conclusion:** 研究结果强调了域适应增强策略和数据集选择在自监督学习中的关键重要性，为构建可扩展的生物多样性监测模型提供了实用指导。
...
> 本文提出残差高斯溅射(RGS)方法，将小波多分辨率分析与3D高斯溅射相结合，通过光谱解耦的高斯表示解决稀视锥束CT重建中的频谱偏置问题，实现伪影抑制与细节保持的最佳平衡。

<details>
  <summary>Details</summary>

**Motivation:** 传统的3D高斯溅射在超稀视条件下存在频谱偏置问题，导致重建结果过度平滑和高频解剖细节丢失。小波变换能提供丰富高频信息，但其双极性与X射线衰减的非负性存在数学不匹配。

**Method:** 提出光谱解耦的高斯表示，将体积分层为几何基线分量和残差细节分量；设计光谱-空间协同优化策略，协调几何锚定与纹理细化，防止光谱串扰。

**Result:** 在临床数据集上的广泛实验表明，RGS能够捕获高度精细的几何纹理，在复杂的骨小梁和血管结构上优于现有神经渲染基线方法。

**Conclusion:** 该方法成功解决了伪影抑制与细节保持之间的权衡，将显式高频拟合转化为物理一致的隐式残差补偿任务，为稀视CT重建提供了新的解决方案。
...
> 研究表明，即使大型视觉语言模型（LVLM）能正确识别文本中的概念，文本的视觉风格（如字体、颜色、大小）仍会显著影响模型对该概念的属性描述，存在'风格泄露'现象。

<details>
  <summary>Details</summary>

**Motivation:** 文本在视觉呈现时具有丰富的风格变化，但文字的语义本应独立于其视觉样式。本研究旨在探索文本视觉风格对LVLM语义推理的影响，特别是在概念正确识别的前提下。

**Method:** 通过对比实验，研究功能性文本风格（如以可读性为导向的黑体无衬线字体）与装饰性风格（如以展示为导向的彩色草书/手写体）对LVLM概念属性描述的影响。

**Result:** 实验结果显示，即使LVLM能够正确识别视觉文本所表达的概念，文本风格仍会影响模型基于属性的描述，表明存在显著的从文本风格到语义推理的风格泄露。

**Conclusion:** 研究发现揭示了LVLM系统中存在严重的风格泄露问题，这为基于LVLM的多媒体系统提供了风格感知评估和缓解的必要性，以改善模型的语义推理质量。
...
> RIHA是一种通过分层对齐放射图像与报告段落、句子和单词级别的Transformer框架，用于提高放射学报告生成精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有放射学报告生成方法将报告视为扁平序列，忽略了其结构化层次，导致图像与文本的细粒度对齐不足，影响生成准确性。

**Method:** 提出RIHA框架，包含视觉特征金字塔(VFP)提取多尺度视觉特征，文本特征金字塔(TFP)表示多粒度文本结构，通过跨模态分层对齐模块(CHA)使用最优传输实现多级对齐，并在解码器引入相对位置编码(RPE)增强令牌级对齐。

**Result:** 在IU-Xray和MIMIC-CXR数据集上，RIHA在自然语言生成质量和临床效果指标上均优于现有最先进方法。

**Conclusion:** 分层跨模态对齐方法能有效提升放射学报告生成的性能，为自动化医疗报告生成提供了新的解决方案。
...
> 该研究提出了World2Minecraft系统，将真实场景转换为Minecraft环境以支持具身智能研究，并创建了大规模数据集MinecraftOcc来提升3D语义占用预测的质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有具身智能仿真平台存在数据污染和灵活性不足等问题，而场景重建质量受限于占用预测模型的数据稀缺性和泛化能力差。

**Method:** 开发低成本的自动化数据获取流程构建MinecraftOcc数据集（10万+图像，156个室内场景），基于3D语义占用预测实现真实场景到Minecraft环境的高质量转换，支持VLN等下游任务。

**Result:** 实验表明MinecraftOcc数据集有效补充了现有数据集，对当前SOTA方法构成重大挑战，显著提升了占用预测性能。

**Conclusion:** World2Minecraft为个性化具身智能研究提供了可定制、可编辑的平台，对改进占用预测和推动该领域发展具有重要价值。
...
> 该研究提出了首个针对胰腺癌血管侵犯的公开数据集CURVAS-PDACVI，并证明了传统二元分割方法在处理肿瘤-血管界面不确定区域时的局限性，强调了不确定性感知模型在临床决策中的重要性。

<details>
  <summary>Details</summary>

**Motivation:** 胰腺癌手术切除 eligibility 依赖血管侵犯的准确评估，但现有方法面临两大挑战：缺乏公开数据集，以及肿瘤-血管界面的诊断模糊性导致专家间存在显著差异，阻碍了AI在该领域的应用。

**Method:** 构建包含密集标注的CURVAS-PDACVI数据集（每扫描图像有5位专家独立标注），提出多指标评估框架（包括空间重叠、概率校准和血管侵犯评估），测试6种最先进方法的性能。

**Result:** 二元分割方法虽然整体重叠度表现良好，但在专家共识低的复杂病例中表现不佳；建模专家间差异的方法能产生更好的概率校准，并在模糊情况下更具鲁棒性。

**Conclusion:** 体积精度无法完全代表局部手术实用性，不确定性感知的概率模型对术前决策至关重要，为胰腺癌分期AI发展指明了新方向。
...
> 本文首次提出了3D伪造检测的概念，并发布了Fake3DGS数据集和一个基于3D感知的检测方法，用于鉴别经过3D高斯泼溅技术篡改的图像。

<details>
  <summary>Details</summary>

**Motivation:** 随着3D重建和神经渲染技术（特别是3D高斯泼溅）的快速发展，3D场景编辑变得越来越简单和真实，这引发了3D内容真实性的安全隐患。然而，现有的虚假检测研究主要局限于2D空间，缺乏针对3D内容伪造的系统性解决方案。

**Method:** 作者构建了Fake3DGS数据集，包含3D高斯泼溅场景及其渲染视图，其中伪造图像通过对几何、外观和空间布局的受控操作生成。同时提出了3D感知检测方法，利用多视点一致性和高斯泼溅表示的衍生特征进行检测。

**Result:** 实验表明，当前最先进的2D检测器难以区分原始图像和3D篡改图像，而提出的3D感知方法在识别修改的3D内容方面表现出显著提升，验证了新数据集的有效性。

**Conclusion:** 研究结果强调了超越2D证据的真实性评估技术的必要性，为3D内容安全检测开辟了新的研究方向。代码和数据已公开，供未来研究使用。
...
> ClipTBP是一种基于剪辑对时间边界预测的视频时刻检索框架，通过边界感知学习解决现有模型忽略多个答案片段间关系的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频时刻检索模型在片段级别计算相似度，忽略了匹配查询的多个答案片段之间的关系，容易受到周围视觉相似片段的干扰，难以排除与查询无关的片段。

**Method:** 提出ClipTBP框架，引入剪辑级别对齐损失函数显式学习答案片段间的语义关系，并应用主边界损失和辅助边界损失来预测准确的时间边界。

**Result:** ClipTBP应用于多种现有模型时都能持续提升性能，在模糊查询场景下展现出更强的边界预测鲁棒性。

**Conclusion:** 通过考虑答案片段间关系和边界感知学习，ClipTBP有效改善了视频时刻检索的性能，特别是在处理复杂查询和视觉干扰方面表现优异。
...
> SECOS是一种新颖的开世界半监督学习方法，能够直接从候选集预测文本标签，无需后处理，通过利用外部知识提取并跨模态对齐语义表示，为已知和新类别提供显式监督信号。

<details>
  <summary>Details</summary>

**Motivation:** 现有OWSSL方法无法满足实际应用需求，因为新样本缺乏显式监督训练，且缺乏提取潜在语义信息的机制，导致预测的标签与候选文本标签缺乏语义对应关系。

**Method:** SECOS方法直接从候选集预测文本标签，利用外部知识提取并在已知和新类别间跨模态对齐语义表示，为训练新类别提供显式监督信号。

**Result:** 实验表明，即使在更为宽松的后验匹配设置下评估现有方法，SECOS仍能超越它们达5.4%，展现了其卓越的有效性。

**Conclusion:** SECOS成功解决了现有OWSSL方法的局限性，能够直接预测与候选标签具有语义对应关系的文本标签，为实际开世界半监督学习应用提供了更有效的解决方案。
...
> SPUR是一个包含4264个QA对的科学实验图像理解基准测试，评估多模态大语言模型在细粒度感知、跨面板关系理解和专家级推理方面的能力，发现现有模型远未达到专家水平。

<details>
  <summary>Details</summary>

**Motivation:** 当前多模态大语言模型在科学图像理解方面的能力缺乏系统性评估，特别是对于科学实验图像这种包含多个面板、复杂关系的专业内容，需要建立专门的基准来评估模型是否达到专家级水平，以推动AI4S研究发展。

**Method:** 构建SPUR基准测试，包含1084张专家标注图像和4264个QA对，从三个维度评估模型：(1)面板级细粒度感知（数值、形态、信息定位三个维度，六种面板类型）；(2)跨面板关系理解（平均每样本14.3个面板的复杂图像）；(3)专家级推理（五种实验范式的定性和定量推理）。评估了20个MLLM和4个MCoT方法。

**Result:** 综合评估显示，现有的多模态大语言模型在科学图像理解方面显著低于专家级要求，这构成了AI4S研究的关键瓶颈。

**Conclusion:** SPUR基准测试揭示了当前MLLM在科学实验图像理解方面的不足，为后续改进模型在细粒度感知、跨面板关系理解和专家级推理方面的能力提供了明确的评估标准和改进方向。
...
> 本文提出了一种统一的轻量级CNN框架AttXNet，用于无人机桥梁裂纹检测，通过注意力机制、数据增强策略和Focal Loss解决了实际应用中的四个关键挑战，实现了825 FPS的推理速度和良好的检测精度。

<details>
  <summary>Details</summary>

**Motivation:** 无人机桥梁结构健康监测中的深度学习自动裂纹检测面临四大挑战：裂纹特征弱化、成像条件退化、严重的类别不平衡以及有限的计算资源，这些因素制约了实际应用的可行性。

**Method:** 构建包含四个协同组件的统一框架：轻量级骨干网络、用于通道和空间增强的CBAM注意力模块、基于检测场景先验的定向鲁棒数据增强策略，以及处理类别不平衡的Focal Loss损失函数。

**Result:** 在SDNET2018桥面数据集上，该方法达到825 FPS推理速度，仅需11.21M参数和1.82G FLOPs；相比基线模型，F1分数提升2.51%，召回率提升3.95%；Grad-CAM可视化显示注意力模块能有效聚焦裂纹轨迹。

**Conclusion:** 该研究在准确性、速度和鲁棒性之间实现了强平衡，为地面站辅助的无人机桥梁检查实时部署提供了实用解决方案。
...
> 本文提出SpaAct框架，通过向后动作推理和向前转移预测两个空间激活任务，以及TriPA渐进式自适应课程学习方法，有效激活视觉语言模型的动态空间感知能力，在VLN-CE基准测试中取得最先进性能。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言导航(VLN)需要智能体在未知3D环境中遵循自然语言指令导航到目标位置。现有方法需要赋予视觉语言模型(VLMs)两种互补的空间感知能力：理解动作原因的向后推理能力和预测状态转移的向前预测能力。

**Method:** SpaAct框架包含：1) 动作回溯任务，从视觉转移推断执行的动作序列；2) 未来帧选择任务，基于历史和动作预测视觉转移；3) TriPA三因子渐进式自适应课程学习，将训练样本从易到难组织，让模型逐步获得导航技能。

**Result:** 在标准VLN-CE基准测试中，SpaAct持续改善了基于VLM的导航性能，达到了最先进的水平，证明了方法的有效性。

**Conclusion:** 通过轻量级监督激活VLM的动态空间感知，结合渐进式课程学习策略，SpaAct为视觉语言导航任务提供了有效的解决方案，相关代码和模型将开源支持未来研究。
...
> FUN是一个端到端的多任务学习框架，通过焦距调制机制实现高效的高光谱图像重建与目标检测联合处理，在保持高精度同时显著降低计算开销。

<details>
  <summary>Details</summary>

**Motivation:** 传统推扫式高光谱成像速度慢无法实时检测，快照式成像虽能瞬时捕获但重建耗时。现有方法缺乏重建与检测的协同优化，限制了实时应用潜力。

**Method:** 提出FUN框架，采用共享U型骨干网络，通过重建任务提供底层光谱信息，检测任务引导语义先验学习。创新性地引入焦距调制机制替代自注意力，降低二次计算复杂度，实现无自注意力的联合架构。

**Result:** 在重建和检测两项任务上均达到SOTA性能，参数量减少40%，计算量降低30%。构建了包含8712个标注对象的新HSI目标检测数据集。

**Conclusion:** FUN为高光谱图像的实时边缘部署提供了有效解决方案，通过多任务协同学习和高效的特征调制机制，在精度和效率间取得良好平衡，具有广阔应用前景。
...
> 本文提出了一个用于颈椎CT-MRI混合配准的框架MSR，并构建了相应的数据集R-D-Reg，通过刚性与变形配准的融合实现了更精确的配准效果。

<details>
  <summary>Details</summary>

**Motivation:** 颈椎区域解剖结构复杂、变异性大，且易受椎动脉和脊髓损伤，准确的CT-MRI配准对术前规划至关重要。然而，目前针对刚-变形混合建模的颈椎CT-MRI配准研究较少，且缺乏高质量标注的多模态数据。

**Method:** 构建了全面标注的CT-MRI数据集R-D-Reg，提出MSR刚-变形混合配准框架。包含两个模块：1) 刚性配准模块对单个椎骨进行独立局部刚性对齐；2) 变形配准模块采用MSL块，通过自适应门控结合基于Mamba的全局建模和基于Swin Transformer的局部建模。最后融合刚性和变形场生成混合场。

**Result:** MSR框架通过融合刚性和变形配准场，更好地保持了局部解剖一致性，在复杂关节结构配准上取得了良好效果。

**Conclusion:** 该研究填补了颈椎CT-MRI混合配准的空白，提供了开源的数据集和代码，为相关研究和临床应用提供了重要资源。
...
> 本文提出了EviMem，一个包含IRIS框架和LaceMem记忆架构的系统，通过显式诊断证据缺失来实现精准的查询精炼，在长期对话记忆检索中显著提升了时序和多跳问题的性能。

<details>
  <summary>Details</summary>

**Motivation:** 长期对话记忆需要检索分散在多个会话中的证据，但单次检索在处理时序和多跳问题时效果不佳。现有迭代方法缺乏对证据缺失的明确诊断，导致查询精炼缺乏针对性。

**Method:** EviMem结合了IRIS框架（通过充分性评估检测证据缺口、诊断缺失内容并驱动针对性查询精炼的闭环系统）和LaceMem（支持细粒度缺口诊断的粗粒度到细粒度记忆层次结构）。

**Result:** 在LoCoMo数据集上，EviMem相比MIRIX在时序问题上将Judge Accuracy从73.3%提升到81.6%，在多跳问题上从65.9%提升到85.2%，同时延迟降低4.5倍。

**Conclusion:** 通过显式诊断证据缺失并实现针对性查询精炼，EviMem有效解决了长期对话记忆检索中的核心挑战，在时序和多跳问题上实现了显著的性能提升和效率改进。
...
> 本研究提出基于深度学习的自动化rPCI区域分割方法，使用nnU-Net在CT扫描上实现0.82的Dice系数，接近人工标注一致性，为非侵入性影像学评估奠定基础。

<details>
  <summary>Details</summary>

**Motivation:** 现有的sPCI评估需要侵入性诊断性腹腔镜检查，缺乏标准化的影像学对应方法；虽然已有3D rPCI区域的共识定义，但仍需自动化工具来实现基于影像的非侵入性评估。

**Method:** 采用nnU-Net和Swin UNETR两种深度学习模型，在62例CT扫描上进行rPCI区域自动分割评估，使用五折交叉验证，通过Dice系数、95%Hausdorff距离和平均表面距离评估性能。

**Result:** nnU-Net整体Dice达到0.82，接近观察者间一致性0.88，优于Swin UNETR的0.76；主要挑战集中在右腹侧和小肠区域。

**Conclusion:** 证明了自动化rPCI分割的可行性，为建立标准化、非侵入性的影像学评估方法提供了技术基础，有望替代侵入性的腹腔镜检查。
...
> 本文提出了一种基于NeRF的视频快照压缩成像重构方法，通过块级光线采样策略和射线Transformer来捕获结构相似性，实现了最先进的重构性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于NeRF的视频快照压缩成像方法采用随机光线采样策略，无法捕获内容结构相似性，导致重构质量受限。

**Method:** 提出块级光线采样策略建模内容结构；设计Inter-和Intra-Ray Transformer捕获空间邻近点间和沿视线的相邻点间相似性；在目标函数中引入全变差先验增强空间平滑性。

**Result:** 在模拟和真实场景下的实验证明，该方法达到了最先进的重构性能。

**Conclusion:** 通过块级采样和射线Transformer的协同作用，结合全变差先验，显著提升了视频快照压缩成像的重构质量。
...
> 本研究提出了一种新的预训练策略，通过避免模型学习预训练数据集的域特定特征来提高模型泛化能力，在多个遥感图像分割数据集上实现了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的深度学习模型在遥感图像分割中面临域差异问题，即ImageNet与遥感图像之间的场景和模态差异较大，导致模型性能受限。虽然建立域特定的大规模数据集可以提升性能，但这样做成本高且泛化性差。

**Method:** 提出一种新颖且简单的预训练策略，引导模型在预训练过程中不学习预训练数据集的域特定特征，从而提高预训练模型的泛化能力。在ImageNet上进行预训练，然后在四个不同的遥感图像分割数据集上微调模型。

**Result:** 在四个具有不同场景和模态的语义分割数据集上均达到了最先进的性能：iSAID (67.4% mIoU)、MFNet (56.9% mIoU)、PST900 (84.22% mIoU)、Potsdam (91.88% mF1)。

**Conclusion:** 该研究为开发适用于计算机视觉和遥感应用的统一基础模型奠定了基础，通过避免域特定特征的学习显著提升了模型的跨域泛化能力。
...
> 本文提出了HSTFG和专门针对越南语优化的PhonoSTFG框架，并构建了首个大规模越南语文本图像描述数据集ViTextCaps，解决了越南语场景文本描述中的语言特有挑战。

<details>
  <summary>Details</summary>

**Motivation:** 越南语作为声调语言，变音符号会改变词义，OCR错误普遍存在且词边界模糊。现有的融合方法将文本视为与语言无关的表示，无法有效处理越南语的这些特殊语言学特征。

**Method:** 提出HSTFG异构图融合框架，通过学习空间注意力偏置进行多模态融合；拓扑分析表明跨模态图边对融合有害；基于该发现设计PhonoSTFG，专门针对越南语语音学特征进行图级语言推理优化；构建包含15,729张图像和74,970个标注的ViTextcaps数据集。

**Result:** 通过拓扑分析揭示了跨模态图边的负面影响；构建的专业化PhonoSTFG框架能更好地处理越南语场景文本；ViTextCaps数据集分析显示52.8%的词汇存在变音符号冲突风险。

**Conclusion:** 越南语场景文本描述需要语言学感知的多模态融合，将语言特定结构知识显式融入融合机制，证明了专业化图融合框架在处理语言特定挑战方面的有效性。
...
> Flatness-aware Prompt Pretraining (FPP) 是一种新的预训练框架，通过在测试时提示调优(TPT)前将提示初始化到更平坦的损失景观区域，同时提升了模型校准和性能，无需标注数据且无额外计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 现存的测试时提示调优技术虽然能提升视觉语言模型的适应性，但经常产生校准不良的模型，预测可靠性差。现有的正则化方法虽然能改善校准但会降低性能，存在权衡问题。

**Method:** 提出FPP框架，揭示现有正则化策略隐含地鼓励优化向更平坦的最小值方向进行，损失景观的平坦度是控制校准质量的关键因素。FPP在适应前将提示预训练并初始化到损失景观的更平坦区域。

**Result:** 仅在现有TPT流程中替换初始化步骤（不修改其他组件）就能同时改善校准和性能，且不需要标注数据，在测试时调优过程中不产生额外计算成本。

**Conclusion:** FPP为测试时提示调优提供了一个简单有效的初始化方案，解决了校准与性能的权衡问题，具有很高的实用性，适合实际部署应用。
...
> 该论文提出了一种可微分知识单元(DKU)，通过隐式地从主任务监督中学习概念并建立逻辑规则关系，无需概念标签即可实现目标知识发现与集成，从而提升分类器的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有集成领域知识的方法需要预先识别有用的符号知识，但在真实世界视觉任务中，这种规则往往不可用。因此需要一种能自动发现和集成知识的方法来提高深度神经网络的泛化性能。

**Method:** 提出可微分知识单元(DKU)，通过专用分类器学习隐式概念，利用模糊推理计算基于逻辑的调整向量来调制分类器logits。通过构建双向逻辑关系连接概念与类别，并强制概念保持独立性，实现监督信号下的隐式知识学习。

**Result:** 在PASCAL-VOC、COCO和MedMNIST数据集上验证了方法的有效性，域泛化和困难样本消融研究表明，隐式知识发现和集成的性能优于基线方法。

**Conclusion:** 该方法无需概念标签就能自动发现和集成领域知识，通过DKU实现了有效的知识表示与推理，为提升深度学习模型的泛化能力提供了新思路。
...
> 本文提出了一个名为GourNet的深度学习模型，基于卷积神经网络对芒果叶病害进行识别，在MBD数据集上实现了97%的分类准确率。

<details>
  <summary>Details</summary>

**Motivation:** 芒果种植对农业经济和粮食安全至关重要，但叶部病害严重影响产量和果实品质。需要早期精准检测病害来有效预防疾病，维持作物生产力。

**Method:** 采用名为GourNet的卷积神经网络模型，使用包含7个病害类别和1个健康类别的MBD数据集。对图像进行预处理（调整大小、重新缩放、数据增强），数据按80%训练、20%验证和测试的比例划分。

**Result:** 模型仅使用683,656个参数，在8类分类任务上达到了97%的准确率，表现出色。

**Conclusion:** GourNet模型在芒果叶病害检测方面效果显著，参数量少但精度高，为农业病害早期诊断提供了有效的技术解决方案。
...
> PRISM是一个三阶段训练管道，在监督微调和强化学习之间加入分布对齐阶段，通过对抗学习和专家混合模型纠正感知和推理偏差，显著提升多模态模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 大型多模态模型的标准后训练方法存在分布漂移问题，监督微调会破坏模型原始能力且无法准确匹配监督分布，在多模态推理中感知错误和推理失败的漂移模式相互叠加，影响后续强化学习效果。

**Method:** 提出PRISM三阶段管道：1) 监督微调初始化；2) 基于在线策略蒸馏的分布式对齐，使用MoE判别器与策略进行对抗游戏，包含专门的感知和推理专家提供解耦的纠正信号；3) 强化学习优化。

**Result:** 实验表明PRISM在多种RL算法和多模态基准测试上持续优于基线，在4B和8B参数的Qwen3-VL上分别提升平均准确率4.4和6.0个百分点。

**Conclusion:** 通过显式分布对齐阶段有效缓解了SFT引入的分布漂移问题，无需教师logits即可实现准确对齐，为大型多模态模型的后训练提供了更优的框架。
...
> 本研究提出了一种改进的SISA框架，通过强化重放机制和门控网络实现CNN模型的类级机器遗忘，有效保护数据隐私同时保持模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 随着图像生成模型等AI系统的快速发展，数据隐私和用户同意问题日益突出。当用户要求删除已用于训练模型的数据时，传统方法需要完全重新训练，成本高昂且不切实际，因此需要高效的机器遗忘解决方案。

**Method:** 研究改进了SISA（分片、隔离、切片和聚合）框架，引入强化重放机制和门控网络，在CNN架构上实现类级数据遗忘。该框架能够选择性地移除特定类别的数据影响，避免完全重新训练。

**Result:** 在多个图像数据集和CNN配置上的实验表明，改进的SISA方法能够有效实现类级遗忘，保持模型性能，同时显著减少重新训练的开销。

**Conclusion:** SISA基础的遗忘方法在隐私敏感的AI应用中具有巨大潜力，为解决数据删除合规性挑战提供了可行的技术路径，研究代码已开源供社区使用。
...
> 本文提出VPDR，一种基于原型个性化联邦学习的隐私增强插件，通过方差自适应扰动和蒸馏引导裁剪正则化，在保护隐私的同时提升模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的原型个性化联邦学习通过直接共享类原型实现高效多域适应，但存在隐私泄露风险。现有的等距高斯噪声扰动方法会过度扰动判别性维度，且难以平衡裁剪阈值与表征保真度。

**Method:** VPDR包含两个核心技术：1) 方差自适应原型扰动(VPP)，根据维度级类别方差分配噪声大小，保护判别性子空间；2) 蒸馏引导裁剪正则化(DCR)，使特征范数自适应地集中在预定义裁剪阈值附近。理论证明该方法在相同隐私约束下提供了不弱于等距基线的隐私保证。

**Result:** 在多域基准测试上的大量实验表明，VPDR在个性化联邦微调中实现了优越的隐私-效用权衡，显著优于IGPP方法，同时保持对现实攻击的鲁棒性。

**Conclusion:** VPDR作为一个客户端隐私插件，能够无缝集成到现有ProtoPFL框架中，有效解决了等距噪声扰动方法的局限性，为隐私保护的多域学习提供了更好的解决方案。
...
> 通过在线架构策略插入全局平均池化层，解决了CNN的单像素平移脆弱性问题，大幅减少参数量同时提升平移鲁棒性，并在图像质量评估任务中取得显著改进。

<details>
  <summary>Details</summary>

**Motivation:** 传统CNN架构在空间上存在脆弱性，单像素平移会导致性能急剧下降，主要原因是空间相关的全连接层限制;

**Method:** 在网络不同深度策略性地插入全局平均池化(GAP)层，将特征识别与空间位置解耦，使用VGG-16作为主要案例研究;

**Result:** 参数减少98%(5.2M→82K)，网络大小减少90%，ImageNet Top-1精度保持66.4%，平移鲁棒性翻倍，在KADID-10k和RAID数据集上图像质量评估显著提升;

**Conclusion:** 架构不变性比传统数据增强是实现鲁棒性的更高效且生物学合理的方法，但完美像素级稳定性仍受离散池化操作的限制。
...
> 本文提出了一种频域感知门控注入网络(FGINet)，通过频带掩码编码、分层门控注入和超球面紧致学习，显著提升了AI生成图像检测在未知生成器上的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 随着AI生成图像越来越逼真和多样化，现有结合视觉基础模型和频域方法的检测技术在未见过的新生成器上泛化能力有限，主要存在频域捷径偏向和跨域表示冲突两个问题。

**Method:** 设计了频带掩码频率编码器(BMFE)减少对特定生成器的依赖；引入分层门控频率注入(LGFI)机制渐进融合频域信息；提出超球面紧致学习(HCL)框架通过余量目标学习紧致可分的表示。

**Result:** 在多个挑战性数据集上的实验表明，FGINet达到了最先进的性能，并且在不同生成器之间展现出强大的泛化能力。

**Conclusion:** 通过频域感知的门控注入策略和紧致表示学习，FGINet有效解决了现有方法在泛化性方面的瓶颈，为AI生成图像检测提供了新的解决思路。
...
> 本文提出Noise2Map，一个基于扩散模型的统一框架，可直接用于语义分割和变化检测任务，无需传统扩散模型的耗时采样过程。

<details>
  <summary>Details</summary>

**Motivation:** 现有的深度学习模型在处理遥感图像时存在时间不一致性、难以捕捉细粒度空间结构、需要大量预训练以及解释性有限等问题。扩散模型在生成任务中表现出色，但其潜力在判别性任务中尚未充分挖掘。

**Method:** 提出Noise2Map框架，重新利用扩散模型的去噪过程进行端到端判别学习。通过任务特定的噪声调度和时间步条件直接预测语义图或变化图,避免传统采样过程。采用自监督去噪预训练和监督微调，支持双任务共享主干网络。

**Result:** 在SpaceNet7、WHU和xView2数据集上评估，Noise2Map在语义分割任务中平均排名第一，在变化检测任务中排名第一。消融实验证实了模型对训练噪声调度和时间步控制的鲁棒性，以及多任务学习能力。

**Conclusion:** Noise2Map成功地将扩散模型的去噪过程转化为判别性学习任务的新范式，为遥感图像分析提供了高效、可解释且多任务兼容的解决方案。
...
> HiMix是一个通过增强训练分布和促进伪迹感知来提升合成图像检测泛化能力的统一框架。

<details>
  <summary>Details</summary>

**Motivation:** 现有合成图像检测器通常在有限且有偏见的数据集上训练，导致对未见过的生成器泛化能力差，难以应对快速发展的生成模型带来的挑战。

**Method:** 提出HiMix框架，包含两个核心模块：1) Mixup驱动的分布增强(MDA)模块，通过构建真实与虚假图像间的连续过渡样本，并使用像素级混合操作平滑扰动语义；2) 分层伪迹感知表示(HAR)模块，通过跨层整合和从粗到细的特征融合聚合全局和局部伪迹信息。

**Result:** 在多个基准测试上的广泛实验表明，HiMix达到了最先进的性能，能够为未见过的伪造图像建立良好分离的logits，显著提升了泛化能力。

**Conclusion:** HiMix通过有效扩展训练分布和学习判别性伪造表示，成功解决了现有检测器泛化能力不足的问题，为可靠的合成图像检测提供了新的解决方案。
...
> 本文提出TAVR框架，通过利用跨场景视频参考生成高保真度说话头像，解决了现有方法受限于单视图静态参考图像的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有说话头像方法基于静态参考图像的图像到视频管道缺乏足够的时序和表情线索，限制了在自定义背景下合成高保真度说话头像的能力，需要突破单视图限制。

**Method:** TAVR采用token选择模块和三阶段训练方案：1）同场景视频预训练建立基础外观复制；2）跨场景参考微调实现鲁棒的跨场景适应；3）特定任务强化学习基于身份奖励对齐生成输出。

**Result:** 在包含158个精心策划的跨场景视频对的新基准上，TAVR在灵活推理时视频参考方面表现优异，在定量和定性评估中均一致超越现有基线方法。

**Conclusion:** 该工作成功实现了从静态图像到跨场景视频参考的范式转换，显著提升了说话头像生成的质量和灵活性，并已部署到生产环境中。
...
> TunnelMIND是一个免训练的隧道检测框架，能够将粗粒度的缺陷建议转化为工程级结构化缺陷证据，在多种检测任务中达到良好性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有免训练基础模型管道只能生成粗粒度的开放词汇建议，在干扰严重的隧道场景中难以直接应用，无法满足工程检测对缺陷定位、测量、严重性分级和工程文档的需求。

**Method:** 通过密集视觉一致性在推理时重新校准语言引导缺陷建议的空间支撑，将粗语义锚点转换为更可靠的提示，并将结果掩码重构为包含类别、位置、几何、严重性和上下文属性的结构化缺陷实体，再映射到检索解释和工程可读报告生成。

**Result:** 在可见光、GPR和道路缺陷任务中分别达到0.68、0.78和0.72的F1分数，证明了免训练隧道检测可以实现从粗定位向结构化缺陷证据的跨越。

**Conclusion:** TunnelMIND展示了免训练方法能够在隧道检测中超越粗粒度定位，为工程评估提供结构化缺陷证据，为实际工程应用提供了可行方案。
...
> 本文提出动态聚类采样方法DynamiCS，通过动态调整长尾概念的样本权重，在降低VLM训练成本的同时提升长尾概念的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有高效VLM预训练方法在追求语义数据平衡时可能过度删除罕见概念，导致长尾概念在训练数据中代表性不足，无法被有效学习。

**Method:** 提出DynamiCS方法，在每个训练epoch中动态进行聚类采样：对大数据簇进行下采样，对小数据簇进行上采样，保持语义簇的相对顺序并强调长尾概念。

**Result:** 实验表明DynamiCS能降低VLM训练的计算成本，并在长尾概念上提供性能优势，证明了动态采样的重要性。

**Conclusion:** 相比于仅关注扁平化语义分布的现有工作，DynamiCS通过动态聚类采样更有效地维持长尾概念的学习，为高效VLM训练提供了新思路。
...
> 本文推出了视频虚拟试穿领域最大的野外三元组数据集TripVVT-10K，并基于扩散Transformer框架开发了TripVVT模型，同时建立了综合评估基准TripVVT-Bench，显著提升了视频虚拟试穿在真实场景下的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频虚拟试穿模型性能受限的主要原因是大规模野外三元组数据的稀缺以及服装掩码的不当使用，导致模型难以处理真实世界的复杂场景。

**Method:** 1) 构建TripVVT-10K数据集，提供显式的视频级跨服装监督；2) 开发基于扩散Transformer的TripVVT框架，用简单稳定的人体掩码先验替代脆弱的服装掩码；3) 建立TripVVT-Bench基准进行多维度评估，包括视频质量、试穿保真度、背景一致性和时序连贯性。

**Result:** 与先进的学术和商业系统相比，TripVVT在视频质量和服装保真度方面表现优异，同时显著提升了对具有挑战性的野外视频的泛化能力。

**Conclusion:** 本研究通过提供高质量数据集、先进模型和评估基准，为推进可控、真实且时序稳定的视频虚拟试穿技术奠定了坚实基础。
...
> 本研究评估了多个视觉语言模型在社交媒体短视频主题检测中的零样本分类能力，并通过聚类方法对视觉主题进行无监督分析，为实践者提供了实用指导。

<details>
  <summary>Details</summary>

**Motivation:** 社交媒体短视频的快速增长改变了公共话题的讨论方式，需要有效的自动化视觉主题检测方法来理解和分析这些内容。

**Method:** 1) 评估VideoChatGPT、PandaGPT、VideoLLava等VLMs的零样本图像分类能力，并与CLIP基线对比 2) 采用最小代价多割问题的聚类方法进行无监督模式发现，重点分析ConvNeXt V2和DINOv2模型的聚类效果

**Result:** VLMs目前无法检测气候变化特定类别，但聚类结果能区分出不同的视觉框架；DINOv2更关注风格和抽象类别差异，ConvNeXt V2产生更细粒度的聚类结果。

**Conclusion:** 虽然现有的视觉语言模型在特定主题检测方面仍有局限，但通过适当的聚类方法可以有效发现社交媒体视频中的视觉模式，为内容分析提供有价值的技术手段。
...
> 研究提出了FineState-Bench基准和FineState-Metrics评估方法，用于评估大型视觉-语言模型在细粒度状态条件GUI交互中的表现，发现当前模型在精确定位和状态设置方面表现不足。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大型视觉-语言模型评估存在覆盖有限、目标状态定义不精确、过度依赖最终任务成功等问题，掩盖了代理失败的具体原因，缺乏对细粒度GUI交互能力的有效评估。

**Method:** 构建包含2209个实例的FineState-Bench基准，涵盖桌面、网页和移动平台；提出四阶段诊断管道FineState-Metrics，包含定位成功率、交互成功率等指标；开发可视化诊断助手VDA提供定位提示。

**Result:** 在FineState-Bench上，精确目标状态成功率较低：Web平台最高32.8%，所有平台平均仅22.8%；使用VDA提示后Gemini-2.5-Flash性能提升14.9个百分点，显示视觉定位改进潜力巨大。

**Conclusion:** 尽管通过改进视觉定位可以提升性能，但当前模型的整体准确性仍不足以实现可靠的细粒度状态条件交互，需要进一步的技术突破。
...
> 本文提出TransVLM视觉语言模型，通过将镜头边界检测重新定义为镜头转换检测任务，并注入光流作为运动先验，有效解决了传统方法在复杂转换场景下的性能问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统的镜头边界检测(SBD)方法将任务围绕孤立的切割点进行建模，在处理复杂转换时经常产生损坏的视频镜头，存在根本性的局限性。

**Method:** 提出TransVLM框架，将SBD重新定义为镜头转换检测(STD)任务，明确检测连续的时间转换片段；在输入阶段注入光流作为关键运动先验，通过简单的特征融合策略处理颜色和运动表示，设计数据引擎合成多样化的转换视频来克服类别不平衡问题。

**Result:** TransVLM在STD任务上取得了卓越的总体性能，显著优于传统的启发式方法、专业时空网络和顶级VLMs，该工作已部署到生产环境中。

**Conclusion:** 通过将检测任务从离散点转向连续段，并结合运动先验增强时空感知，TransVLM为视频镜头处理提供了新的解决方案，在理论和实践中都具有重要价值。
...
> Echo-α 是一种超声波智能推理模型，通过调用特定检测器并结合全局视觉上下文，将专业检测结果转化为可验证的临床诊断决策。

<details>
  <summary>Details</summary>

**Motivation:** 超声波解释需要精确定位病灶和整体临床推理，但现有方法只能擅长其中一种：专业检测器定位准确但推理有限，多模态大语言模型推理灵活但医学专业性不足。需要开发能统一这两种优势的模型。

**Method:** 提出Echo-α代理多模态推理模型，采用调用-推理框架，通过九任务监督课程训练协调器官特定检测器输出，集成全局视觉上下文，将证据转化为基于检测器的诊断决策，然后通过顺序强化学习在不同奖励权衡下进行优化，形成Echo-α-Grounding和Echo-α-Diagnosis两个版本。

**Result:** 在多中心肾脏和乳腺超声基准测试中，Echo-α在定位和诊断方面均超越竞争基准。在跨中心测试集上，Echo-α-Grounding在肾脏/乳腺超声上达到56.73%/43.78% F1@0.5，Echo-α-Diagnosis达到74.90%/49.20%的整体准确率。

**Conclusion:** 代理多模态推理能将专业检测器转化为可验证的临床证据，为实现更准确、可解释和可迁移的超声波AI系统提供了实用路径。
...
> 提出一种结构感知的3D高斯致密化框架，通过多尺度频率分析指导各向异性分裂，实现更快收敛和更高质量的高频区域重建。

<details>
  <summary>Details</summary>

**Motivation:** 标准3D高斯散布的自适应密度控制基于屏幕空间位置梯度，无法区分几何错位和频率混叠，导致高频纹理过度模糊或低效过度致密化。

**Method:** 结合结构张量和拉普拉斯尺度空间分析进行多尺度频率分析，估算每个像素的主导频率；定义η指标指示高斯原始对局部纹理细节的欠解析程度；基于η执行各向异性分裂而非各向同性分裂；引入多视图一致性标准聚合多个视图的η观测；执行早期快速致密化。

**Result:** 在标准基准测试中实现了更快的收敛速度，跳过了基线方法所需的冗长迭代致密化阶段，并在高频区域获得了更优越的重建质量。

**Conclusion:** 该结构感知致密化框架通过显式比较投影屏幕空间范围与局部纹理结构来指导高斯分裂，有效解决了传统方法在处理高频纹理时的局限性，显著提升了3D高斯散布的性能和效率。
...
> 本文提出了一个新的四类DeepFake检测设置，新增了语义不匹配类别(RARV-SMM)，并提出了语义增强策略来提高检测器的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有DeepFake检测主要采用二分类或四分类方法，但忽略了语义层面的不一致性问题。当DeepFake来源于内容而非数据源时，现有方法无法有效检测语义不匹配。

**Method:** 1. 扩展四分类设置，新增语义不匹配类别(RARV-SMM); 2. 在FakeAVCeleb数据集上评估现有模型的鲁棒性; 3. 提出三种RARV-SMM变体; 4. 设计语义增强策略，结合ImageBind嵌入改进检测性能。

**Result:** 实验发现现有方法在语义不匹配数据上存在局限性，提出的语义增强策略在FakeAVCeleb和LAV-DF数据集上都显著提升了检测性能。

**Conclusion:** 该研究为构建更真实的DeepFake检测器提供了新思路，强调语义一致性分析的重要性，推动了DeepFake检测向更实用化的方向发展。
...
> 本文提出了ResiHMR，首个专门针对肢体缺失人员的人体网格重建框架，通过残肢感知的关键点、拓扑自适应优化和几何重建模块，显著提升了非标准肢体解剖结构下的3D人体建模精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有单张图像人体网格恢复系统基于完整肢体先验，使用固定拓扑模型，无法准确表示肢体缺失人员的残肢结构，导致在该人群上性能严重下降，需要开发专门的方法来解决这个问题。

**Method:** ResiHMR框架包含两个核心组件：1) 拓扑自适应残肢锚点因子优化模块，将估计约束到解剖学有效结构的观测运动学子图上；2) 基于几何的残肢重建模块，估计残肢边界和凸面肢体终止几何。这些组件引入了拓扑感知优化和显式终止几何。

**Result:** 在真实肢体缺失图像数据集上，ResiHMR在SMPLify-X和HSMR两种主干网络上都提升了重建质量，完整关节2D MPJPE从41.32降至37.40，残肢2D MPJPE从73.61大幅降至23.19。

**Conclusion:** ResiHMR是首个能显式重建残肢表面并执行拓扑自适应优化的单图像HMR系统，相比固定拓扑的关节移除方法，更好地匹配假肢生物力学和实际应用需求，为肢体缺失人群的3D人体建模提供了有效解决方案。
...
> TAFA-GSGC是一种可扩展的点云几何编解码器，能从单一比特流进行多质量解码，解决了传统学习编解码器需要重编码或维护多个比特流的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的学习编解码器大多针对固定率失真点优化，导致速率适应需要代价高昂的重编码或维护多个比特流，而可扩展压缩对于带宽自适应传输至关重要。

**Method:** 提出了TAFA-GSGC框架，结合分层残差细化和通道分组熵编码，并引入目标对齐特征聚合(TAFA)模块来减少增强残差中的跨层冗余。

**Result:** 支持多达9个可解码质量级别，随着接收到的子比特流增加实现单调质量提升，相比基线PCGCv2获得相当的RD性能，在D1和D2上分别实现平均-4.99%和-5.92%的BD-Rate节省。

**Conclusion:** TAFA-GSGC成功实现了高效的可扩展点云压缩，在保持强压缩效率的同时，提供了灵活的多质量解码能力，为带宽自适应传输提供了有效解决方案。
...
> 这篇综述分析了制造业中3D重建技术的现状，系统回顾了106篇文献，表明深度学习方法提升了重建精度和速度，但缺少统一的框架，未来趋势是混合系统发展。

<details>
  <summary>Details</summary>

**Motivation:** 制造业对三维重建技术的需求日益增长，但缺乏对该领域技术发展现状的系统性梳理，需要全面了解从传统方法到深度学习方法的演进脉络以及存在的技术缺口。

**Method:** 通过系统回顾106篇近期文献，将重建技术分为数据采集、点云生成、后处理和应用三个主要类别，分别分析传统方法和新兴深度学习方法的技术特点和应用表现。

**Result:** 非接触式方法(特别是结构光扫描和立体视觉)在制造业中得到广泛应用，47%的应用聚焦质量检测。深度学习的集成显著提升了重建精度和处理速度。各应用领域分布为：设计和开发13%、加工8%、工艺17%、装配22%、质量检测40%。

**Conclusion:** 当前技术在受控环境下可实现亚毫米级精度，但在处理反射表面和动态环境方面仍存在挑战。研究表明未来发展趋势是结合多种传感器类型和处理方法的混合系统，以克服单一技术的局限性。
...
> 论文提出了一种分层视频美学评估框架，构建了包含2500个视频对的专业标注数据集AesVideo-Bench，并开发了视频美学奖励模型AesRM系列，通过三阶段渐进式训练显著提升了视频生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的超逼真视频生成技术虽然在视觉保真度上有进步，但在影视制作等实际应用中还需要满足视觉美学要求（如和谐的色彩和电影级灯光）。现有工作主要关注图像美学且缺乏系统严谨的评估方法。

**Method:** 提出分层评价标准，将视频美学分解为视觉美学(VA)、视觉保真度(VF)和视觉合理性(VP)三个维度，包含15个细粒度标准。构建AesVideo-Bench评估数据集和AesRM奖励模型，采用原子美学能力学习、冷启动、GRPO三阶段渐进训练，并设计了基于自我一致性的思维链合成方法。

**Result:** AesRM在多个美学基准测试上优于基线模型，具有更强的鲁棒性和更低的位置偏差。将其与Wan2.2模型对齐后，相比现有美学奖励模型获得了显著的美学提升。

**Conclusion:** 该研究为视频美学评估提供了系统性的框架和高质量数据集，提出的AesRM模型能够有效提升视频生成的美学质量，为实际影视应用提供了重要技术支持。
...
> UHR-Net是一种针对病灶分割的不确定性感知超图细化网络，通过实例对比预训练和超图细化来改善边界模糊和小病灶分割问题。

<details>
  <summary>Details</summary>

**Motivation:** 病灶分割在临床诊断和治疗规划中至关重要，但现有的分割方法面临两个主要挑战：1）病灶常与周围组织相似且边界不清晰，导致边界/过渡区域预测不稳定；2）多尺度特征提取会稀释小病灶的线索，造成欠分割或过分割。

**Method:** 提出UHR-Net包含两个关键组件：1）不确定性导向的实例对比预训练策略(UO-IC)，结合几何感知的复制粘贴增强和病灶类背景区域的困难负样本挖掘，提升小病灶和视觉模糊病灶的实例级判别能力；2）不确定性引导的超图细化块(UGHR)，从粗略概率图推导基于熵的不确定性图来指导超图细化，通过将超边原型分为前景和背景组来解耦高阶交互。

**Result:** 在五个公共基准数据集上的实验表明，UHR-Net相比强基线方法取得了一致的性能提升。

**Conclusion:** 该方法通过不确定性感知机制有效解决了病灶分割中的边界模糊和小病灶检测难题，为医学图像分割提供了新的解决思路。
...
> 提出S²VAE框架，通过几何优先的球面隐变量表示，在保持3D几何结构和相机动力学一致性方面优于传统方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉世界建模系统虽然能产生合理的运动，但往往无法保持底层的3D几何结构或物理一致的相机动力学，主要问题在于隐表示未能有效编码几何结构信息。

**Method:** 构建基于VGGT的S²VAE框架，使用Power Spherical隐分布的乘积作为瓶颈层，在强压缩下显式保持超球面结构，编码场景的3D状态（包括相机运动、深度和点级结构）。

**Result:** 在深度估计、相机位姿恢复和点云重建任务中，几何对齐的球面隐变量在高压缩条件下始终优于传统高斯瓶颈。

**Conclusion:** 隐变量几何结构应作为物理基础的视觉和世界模型的一等设计选择，为视觉建模提供了新的几何优先范式。
...
> AdvDMD通过将DMD蒸馏和强化学习统一，显著提升了扩散模型少步采样性能，4步结果超越原40步模型。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型需要大量采样步骤生成高质量图像，现有蒸馏方法在步数受限时性能下降明显，现有的RL增强方法组合性强且复杂，需要更简洁有效的统一框架。

**Method:** AdvDMD利用DMD2中的对抗判别器作为奖励模型，评估生成图像质量；采用在线更新机制同时训练判别器和蒸馏模型；设计统一的SDE反向仿真和差异化的训练调度，实现DMD与RL的无缝集成。

**Result:** 在SD3.5上4步AdvDMD超越原40步模型DPG-Bench性能，SD3在GenEval上显著提升，Qwen-Image上2步AdvDMD优于TwinFlow。

**Conclusion:** AdvDMD成功将蒸馏与强化学习统一简化，实现了高效稳定的训练，在多个基准测试中证明了少步采样的卓越性能，为扩散模型加速提供了新思路。
...
> 本文提出首个完全端到端的任意骨骼动作捕捉框架，将视频到姿态和姿态到旋转两个阶段联合优化，引入参考姿态-旋转对解决歧义性，实现了从~17度降至~10度的旋转误差提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有的单目视频动作捕捉方法采用分离式管道，通过分析逆运动学恢复关节旋转，存在关节位置无法完全确定旋转、IK阶段不可微分、无法适应噪声预测等固有局限性。

**Method:** 设计端到端可学习的双阶段框架，引入目标资产的参考姿态-旋转对来解决坐标系统缺失造成的歧义性，提出骨架感知的全局-局部图引导多头注意力模块(GL-GMHA)进行联合级局部推理和全局协调，直接从视频预测关节位置而不依赖网格中间表示。

**Result:** 在Truebones Zoo和Objaverse数据集上，旋转误差从~17度降至~10度，在未见过的骨骼上达到6.54度，推理速度比基于网格的方法快约20倍。

**Conclusion:** 该框架通过端到端优化和坐标系统约束有效解决了姿态到旋转映射的歧义性问题，在保持高精度的同时显著提升了推理效率，为实际应用提供了可行的解决方案。
...
> 3D-ReGen是一种基于初始3D形状条件的3D重新生成器，通过VecSet条件机制和自监督学习，实现了可控的3D对象重构、增强和编辑。

<details>
  <summary>Details</summary>

**Motivation:** 现有3D生成器大多是one-shot方式，从文本或图像直接生成3D对象，可控性有限。研究需要更灵活可控的3D生成方法来支持增强、重构和编辑等多种任务。

**Method:** 提出3D-ReGen重新生成器，以初始3D形状为条件；设计基于VecSet的新条件机制，可更新或改进输入几何体并保持一致的细粒度细节；通过自监督前置任务和数据增强从现成3D数据集学习通用重构先验，无需额外标注。

**Result:** 在几何一致性和细粒度质量方面评估了3D-ReGen，在多个任务的可控3D生成中达到了最先进的性能。

**Conclusion:** 3D-ReGen通过简单而有效的条件化重新生成框架，为3D对象的增强、重构和编辑提供了一种灵活可控的解决方案，证明了其在多个3D生成任务中的优越性能。
...
> 基于HVI色彩空间的RAW到RGB转换框架pHVI-ISPNet，通过四项关键优化解决了夜间摄影渲染中极端对比度问题，在保真度和感知质量上均达到最新水平。

<details>
  <summary>Details</summary>

**Motivation:** 夜间摄影渲染面临极端对比度挑战，现有针对保真度指标优化的方法存在显著的感知差距且影响视觉质量。

**Method:** 提出pHVI-ISPNet框架，基于稳健的HVI色彩空间，整合四项关键优化：RAW域特征处理和小波特征传播减少高频细节损失、基于样本的动态损失系数确保不同曝光水平稳定学习、基于特征分布的损失项维持严格色彩恒常性。

**Result:** 在NTIRE 2025夜间摄影渲染挑战数据集上的评估表明，该方法在保持竞争性保真度的同时，在CIE2000色差和LPIPS指标上均建立了最新的最优结果。

**Conclusion:** 实验结果验证了感知驱动设计在高质量夜间成像方面的有效性，为 nighttime imaging 提供了新的解决方案。
...
> 提出了一种新型的连续值图像简单点检测方法，实现了可微分的拓扑推理，并开发了保持拓扑结构的骨架提取算法和可集成到深度网络的变分模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的简单点检测方法仅适用于二值图像且不可微分，无法与现代深度学习的梯度优化兼容，而形态学和纯数据驱动方法难以保证拓扑一致性。

**Method:** 直接在连续值图像上计算简单点实现可微分的拓扑推理，基于此开发了保持拓扑结构的骨架提取算法，并设计了通过保持拓扑不可移除点来强制执行拓扑约束的变分模型。

**Result:** 实验证明所提出方法在多个基准测试中有效提高了拓扑完整性和结构准确性，可无缝集成到具有softmax或sigmoid输出的任意深度神经网络分割中。

**Conclusion:** 该方法成功解决了拓扑保持学习与深度学习集成的问题，为图像分析任务提供了可微分的拓扑保证，代码已开源。
...
> PhyCo是一个通过引入连续、可解释的物理 grounded 控制，使视频扩散模型生成物理一致且可控内容的框架，无需推理时使用模拟器。

<details>
  <summary>Details</summary>

**Motivation:** 当前视频扩散模型虽然在外观合成方面表现优异，但在物理一致性方面存在严重问题：物体漂移、碰撞缺乏真实反弹、材料响应与底层属性不匹配等。

**Method:** PhyCo框架包含三个核心组件：(1)包含10万+光真实感仿真视频的大规模数据集；(2)使用物理属性图作为条件对预训练扩散模型进行物理监督微调；(3)基于VLM的奖励优化机制，通过评估生成视频的物理属性提供可微分反馈。

**Result:** 在Physics-IQ基准测试上，PhyCo显著提升了物理真实感，人类研究也证实了对物理属性更清晰和忠实的控制。模型能够在推理时无需模拟器或几何重建的情况下生成物理一致的输出。

**Conclusion:** PhyCo为构建物理一致、可控的生成视频模型提供了一条可扩展的路径，能够泛化到合成训练环境之外的场景。
...
> 提出A4Mer模型，通过层次化的Action Atoms和Action Motifs表示，以自监督方式从人体姿态数据中学习可组合的身体运动模式，显著提升多种人体行为建模任务性能。

<details>
  <summary>Details</summary>

**Motivation:** 有效的人体行为建模需要能够利用身体运动组合性的表示方法。现有方法缺乏对原子关节运动和其时间组合的层次化建模能力。

**Method:** 提出A4Mer嵌套潜在Transformer，通过统一的掩码标记预测任务自监督学习层次表示：首先将3D姿态序列分割为可变长度片段，每个片段表示为单一潜在标记(Action Atoms)；然后通过自底向上的表示学习，自然形成由Action Atoms组成的时间模式(Action Motifs)。同时创建AMD数据集，通过将相机安装在脚部获得帧级SMPL标注。

**Result:** 实验结果表明A4Mer能有效提取有意义的Action Motifs，在动作识别、运动预测和运动插值等人体行为建模任务上取得显著性能提升。

**Conclusion:** 该研究通过层次化的原子运动和运动模式表示，为人体行为建模提供了新的自监督学习框架，证明了组合性表示在理解复杂人体运动中的重要作用。
...
> AEGIS是首个针对学术图像伪造的综合性评估基准，发现当前最先进的模型在学术图像取证方面的性能有限，取证技术落后于生成技术的发展。

<details>
  <summary>Details</summary>

**Motivation:** 随着AI生成技术的快速发展，学术图像伪造问题日益严重，但现有基准缺乏针对学术图像复杂性和多样性的系统评估，需要建立一个专门的基准来诊断学术图像取证的根本局限性。

**Method:** 构建了一个包含三个关键特征的基准：(1)覆盖7个学术类别39个细分子类型的领域特定复杂性；(2)模拟4种常见学术伪造策略和25个生成模型的多样化伪造方法；(3)联合评估检测、推理和定位的多维度评估体系，对25个MLLM、9个专业模型和1个统一模型进行评估。

**Result:** 即便是GPT-5.1也仅达到48.80%的整体性能，专业模型的定位精度为30.09% IoU；11个生成模型的平均取证准确率低于50%；MLLM在文本伪影识别上达84.74%，专业检测器在二元真实性检测上峰值79.54%。

**Conclusion:** AEGIS揭示了学术图像取证面临的基本挑战，当前取证技术无法跟上生成技术的发展步伐，不同模型家族在图像取证任务上表现互补性优势。
...
> 本文提出了一种基于患者特异性呼吸建模和网格锚定高斯泼溅的支气管镜导航方法，无需屏气协议即可实现连续变形感知重建，达到1.22毫米的目标定位精度。

<details>
  <summary>Details</summary>

**Motivation:** 支气管镜导航需要将内窥镜视频配准到术前CT扫描，但呼吸运动导致气道变形5-20毫米，限制了定位精度。现有的屏气协议难以复制且会中断临床工作流程。

**Method:** 利用已有的配对吸气-呼气CT扫描定义患者特异性变形空间，将呼吸运动简化为每帧单一标量呼吸相位。在网格锚定高斯泼溅框架中嵌入这种表示，轻量级估计器直接从内窥镜RGB推断呼吸相位，实现无需屏气或外部传感的连续重建。

**Result:** 在引入的RESPIRE支气管镜仿真管道上的实验表明，该方法实现了几何保真重建，训练速度提升20倍以上，目标定位精度为1.22毫米（在3毫米临床相关容差内），优于无约束单CT基线。

**Conclusion:** 通过消除屏气协议的需求并利用患者特异性呼吸建模，该方法为支气管镜导航提供了更准确、流畅且临床可行的解决方案，同时RESPIRE仿真平台为该领域提供了重要评估工具。
...
> 本文提出了智能视觉生成的概念，构建了从原子生成到世界建模的五层分类体系，旨在推动视觉生成从注重外观转向具备空间推理、因果理解和持续交互能力的智能系统。

<details>
  <summary>Details</summary>

**Motivation:** 当前视觉生成模型在真实感、排版、指令遵循等方面取得进展，但在空间推理、状态持久、长程一致性和因果理解方面仍存在严重不足，需要从单纯的外观合成转向更智能的视觉生成。

**Method:** 提出五层技术分类体系：原子生成、条件生成、上下文生成、智能体生成和世界建模生成，从被动渲染器逐步发展到交互式、智能体化、具有世界认知的生成器。并分析了流匹配、统一理解生成模型、改进视觉表示、后训练等关键技术驱动因素。

**Result:** 通过基准测试回顾、实际场景压力测试和专家约束案例研究相结合的方法，揭示了当前评估方法过分强调感知质量而忽视结构、时空和因果失败的问题，为新一代智能视觉系统提供了以能力为中心的评估框架。

**Conclusion:** 智能视觉生成是下一代视觉AI的发展方向，需要从技术架构、评估标准、数据构建等多个维度进行全面变革，以实现真正具备结构理解、动态推理和因果认知的视觉生成系统。
...
> 该研究提出了一种新的FD-loss方法，通过解耦Fréchet距离估计的群体大小与梯度计算的批量大小，使得Fréchet距离能够有效优化。这种方法显著提升了生成模型的视觉质量，并重新评估了FID指标的有效性。

<details>
  <summary>Details</summary>

**Motivation:** 传统上，Fréchet距离（FD）因其计算复杂度高而被认为不实用作训练目标。本研究旨在解决这一问题，探索在表示空间中有效优化FD的方法，以期获得更好的生成模型训练效果和评估指标。

**Method:** 提出FD-loss方法，将FD估计所需的群体大小（如50k）与梯度计算的批量大小（如1024）解耦。在不同表示空间中进行训练，包括一步生成器和多步生成器的转换。同时引入FDr$^k$作为多表示评估指标。

**Result:** 实验表明，FD-loss在多种表示空间中持续提升生成模型的视觉质量；Inception特征空间下的一步生成器在ImageNet 256x256上达到0.72 FID；多步生成器可转换为强一步生成器；发现FID可能错误排序视觉质量，并提出更准确的FDr$^k$指标。

**Conclusion:** 该工作证明了Fréchet距离作为训练目标的实用性，挑战了FID作为评估指标的权威性，鼓励在多样化的表示空间中进一步探索分布距离，作为生成模型的训练目标和评估标准。
...
> GenWildSplat是一个无需场景优化的前向框架，能够从稀疏、未标定的户外图像中重建3D场景，实现实时渲染质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法在处理真实世界中稀疏、未标定图像的3D场景重建时存在局限性，需要大量场景特定训练，且在稀疏视角下表现不佳，泛化能力存疑。

**Method:** 提出GenWildSplat前向框架，利用学习到的几何先验预测深度、相机参数和规范空间中的3D高斯点；引入外观适配器调制目标光照条件，使用语义分割处理瞬态物体；通过合成和真实数据的课程学习提高泛化能力。

**Result:** 在PhotoTourism和MegaScenes基准测试中达到最先进的前向渲染质量，实现了无需测试时优化的实时推理。

**Conclusion:** GenWildSplat成功解决了稀疏未标定图像3D重建的挑战，通过前向架构和课程学习实现了强泛化能力和实时性能，为户外3D场景重建提供了实用解决方案。
...
> HERMES++是一个统一的驾驶世界模型，首次将3D场景理解与未来几何预测在单一框架中融合，通过BEV表示、LLM增强查询、时序连接和联合几何优化等创新设计，在多个基准测试中超越专业方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有驾驶世界模型主要关注未来场景生成而缺乏全面的3D场景理解，而大型语言模型虽具有强大推理能力但无法预测未来几何演化，语义理解与物理模拟之间存在显著鸿沟。

**Method:** 提出统一的HERMES++框架，包含四个关键设计：1) BEV表示整合多视图空间信息；2) LLM增强的世界查询促进知识迁移；3) 当前-未来连接桥接时间间隙；4) 联合几何优化策略整合显式几何约束与隐式潜在正则化。

**Result:** 在多个基准测试中验证了方法的有效性，HERMES++在未来点云预测和3D场景理解任务上均表现优异，超越了专业方法。

**Conclusion:** 成功弥合了语义理解与物理模拟之间的差距，证明了集成3D场景理解与未来几何预测的统一世界模型的可行性和优越性，为自动驾驶技术提供了更完整的解决方案。
