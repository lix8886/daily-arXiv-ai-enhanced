...
> 本文提出了AITP人工智能交警模型，通过多模态思维链和检索增强生成技术，实现了交通事故责任分配任务，并构建了DecaTARA十项全能基准测试。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态大语言模型在交通事故检测和理解方面取得进展，但主要集中于描述性任务，缺乏深层的因果推理和法律知识整合能力，需要解决更具挑战性的交通事故责任分配问题。

**Method:** 设计AITP模型，采用多模态思维链(MCoT)增强推理能力，结合检索增强生成(RAG)技术整合法律知识。同时构建DecaTADA基准，包含67,941个标注视频和195,821个问答对，涵盖十项相关推理任务。

**Result:** 实验表明，AITP在责任分配、交通事故检测和理解任务上均达到业界领先水平，建立了推理驱动的多模态交通分析新范式。

**Conclusion:** 本研究通过创新的多模态推理框架和综合性基准测试，推动了交通事故分析的智能化发展，为实际交通管理提供了有效的AI解决方案。
...
> AFRILANGDICT是一个包含19.7万个非洲语言-英语词典条目的资源，用于构建AFRILANGEDU数据集（7.89万个多轮训练样本），训练出AFRILANGTUTOR模型，在10种非洲语言的辅导任务上显著超越基础模型。

<details>
  <summary>Details</summary>

**Motivation:** 非洲本地语言缺乏足够的训练资源，导致开发者难以构建理解和响应当地语言的AI系统，亟需解决方案来支持低资源语言的AI教育应用开发。

**Method:** 创建AFRILANGDICT词典资源作为种子，自动生成大规模多样化的师生问答交互数据，构建AFRILANGEDU数据集，结合监督微调(SFT)和直接偏好优化(DPO)训练Llama-3-8B-IT和Gemma-3-12B-IT模型。

**Result:** 在10种非洲语言上，AFRILANGTUTOR模型在LLM-as-a-judge评估下，四个评估标准下性能提升1.8%至15.5%，SFT和DPO结合效果最佳。

**Conclusion:** 证明了基于词典资源构建语言辅导数据的可行性，为低资源语言的AI教育系统提供了有效方法，所有资源已开源以促进该领域研究。
...
> 本文提出HPO方法，通过分层策略优化来改进基于大语言模型的同声传译系统，在保持低延迟的同时显著提升翻译质量。

<details>
  <summary>Details</summary>

**Motivation:** 当前使用LLM的同声传译系统虽然质量高但计算开销大。现有的对话式重用KV缓存方法虽然降低了计算成本，但严重依赖高质量的对话形式SFT数据，而这类数据稀缺且现有合成方法无法保证质量。

**Method:** 提出分层策略优化(HPO)方法，对在 imperfect SFT数据上训练的模型进行后训练。引入分层奖励机制，平衡翻译质量和延迟目标。该方法无需高质量的对话形式标注数据。

**Result:** 在英译中/德/日任务上，在1.5秒延迟条件下，COMET分数提升超过+7分，MetricX分数提升+1.25分。消融实验验证了不同质量奖励、分层奖励公式和分割策略的有效性。

**Conclusion:** HPO方法成功解决了对话式同声传译中对高质量SFT数据的依赖问题，在保证低延迟的同时显著提升了翻译质量，为实际应用提供了可行的解决方案。
...
> TRACES是一个轻量级框架，通过实时标记推理步骤实现自适应且具成本效益的早期停止，在保持准确率的同时减少20-50%的token生成。

<details>
  <summary>Details</summary>

**Motivation:** 现有的语言推理模型(LRMs)存在效率低下的问题，过度生成验证和反思步骤，且对不同推理步骤类型的作用缺乏深入研究。

**Method:** 提出TRACES框架，实时标记推理步骤，监测推理行为变化，发现LRMs在得到正确答案后会改变推理行为，利用特定步骤类型制定可解释的早期停止标准。

**Result:** 在MATH500、GSM8K、AIME、MMLU和GPQA等基准测试上，成功减少20-50%的token使用量，同时保持与标准生成相当的准确率。

**Conclusion:** 该框架有效解决了LRMs效率低下问题，通过监测推理步骤类型实现了成本效益的早期停止策略，为提高语言推理模型效率提供了新思路。
...
> 本文提出基于离散小波变换(DWT)的多分辨率框架，将文本视为语义信号进行分解，显著提升了长文档领域特定摘要的语义保真度和事实一致性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的LLM在长文档摘要任务中存在上下文限制、信息丢失和幻觉问题，特别在临床和法律等专业领域表现不佳，需要更可靠的方法来保障摘要的准确性和事实基础。

**Method:** 提出DWT多分辨率框架，将文本视为语义信号，在句子或词级嵌入上进行小波变换，分解为全局(近似)和局部(细节)分量，生成紧凑表示作为摘要或指导LLM生成。

**Result:** 在临床和法律基准测试中，相比GPT-4o基线实现了2%以上的BERTScore提升、4%以上的语义保真度改进，保真度最高达97%，显著减少了幻觉现象。

**Conclusion:** DWT作为一种轻量级、可泛化的方法，通过语义去噪机制有效提升了长文档和专业领域摘要的可靠性和事实准确性，为LLM在专业领域的应用提供了新思路。
...
> 本研究首次系统比较了四种FHIR数据序列化策略在药物配置核对任务中的表现，发现序列化策略对模型性能有显著影响：小于8B参数的模型使用临床叙述格式表现最佳，而70B参数模型使用原始JSON格式最优，所有模型普遍存在遗漏错误而非捏造错误。

<details>
  <summary>Details</summary>

**Motivation:** 临床交接过程中的药物配置核对是一个高风险且易出错的任务。虽然大型语言模型被提议协助此任务，但FHIR（快速医疗保健互操作性资源）数据如何在传递给模型前进行序列化的这一关键变量尚未得到系统性研究。

**Method:** 研究人员在200名合成患者的受控基准测试上，系统比较了四种FHIR序列化策略（原始JSON、Markdown表格、临床叙述和按时间顺序排列的时间线），评估了五个开放权重模型（Phi-3.5-mini、Mistral-7B、BioMistral-7B、Llama-3.1-8B、Llama-3.3-70B）的性能，总计进行了4,000次推理运行。

**Result:** 研究发现序列化策略对8B以下参数模型的性能有很大且统计显著的影响：临床叙述比原始JSON在Mistral-7B上高出19个F1分数点。这种优势在70B参数模型中逆转，原始JSON获得最高的平均F1分数0.9956。在所有20种模型和策略组合中，平均精确度都超过平均召回率，遗漏是主要的错误模式。较小模型在大约7-10种并发主动药物时达到性能上限。BioMistral-7B在所有条件下都未产生可用输出。

**Conclusion:** 研究结果为临床LLM部署提供了实用的、基于证据的格式建议：对于8B以下的模型推荐使用临床叙述格式，对于70B及以上模型推荐使用原始JSON格式。这些发现对临床安全审计优先级设置和复杂用药患者的服务具有重要指导意义。
...
> 本研究通过加权损失函数优化医学报告生成的视觉-语言模型，显著提升数据效率，仅需10%的训练数据即可达到同等报告质量。

<details>
  <summary>Details</summary>

**Motivation:** 医学报告生成的视觉-语言模型训练面临高质量标注数据稀缺的挑战，需要开发更高效的数据利用方法来降低对大量数据的依赖。

**Method:** 采用加权损失函数替代标准交叉熵损失，将模型训练重心从平等对待所有token预测误差转向关注具有临床重要性的语义显著token。

**Result:** 在眼科报告生成实验中，该方法在多个数据规模下均显著提升效率，在保持相当报告质量的前提下，训练数据需求减少多达10倍。

**Conclusion:** 简单的加权损失重分配策略是提高医学VLM数据效率的有效方法，为解决医学AI领域的数据稀缺问题提供了实用解决方案。
...
> 该研究使用MARBERT模型预测阿拉伯语推文中的表情符号，在8695条推文数据集上达到75%的准确率，证明了模型在低资源多方言阿拉伯语处理上的潜力与改进空间。

<details>
  <summary>Details</summary>

**Motivation:** 阿拉伯语作为一种低资源多方言语言，其表情符号预测面临独特挑战。现有机器学习模型在处理这种语言表现有限，需要探索更好的解决方案来提高预测准确性。

**Method:** 收集11379条阿拉伯语推文，筛选8695条作为数据集，分为14个表情符号类别。设计预处理管道作为基线，微调MARBERT模型进行表情符号预测，使用精确率、召回率和F1分数评估性能。

**Result:** MARBERT模型在表情符号预测任务中表现良好，整体准确率达到0.75。尽管结果令人鼓舞，但仍有改进空间，特别是在处理低资源多方言阿拉伯语方面。

**Conclusion:** 研究表明MARBERT模型在阿拉伯语表情符号预测方面具有应用潜力，但需要进一步优化以提高在低资源多方言语言环境下的性能。
...
> GRISP是一种基于小型语言模型微调的SPARQL问答方法，通过生成查询骨架并结合知识图谱约束迭代填充，在Wikidata和Freebase基准测试中优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 为了解决知识图谱问答中的关键挑战，如何准确地将自然语言问题转换为SPARQL查询，特别是如何高效匹配和选择知识图谱中的实体和关系。

**Method:** 首先使用小型语言模型生成自然语言SPARQL查询骨架，然后基于知识图谱约束进行重新排序和选择，迭代地替换自然语言占位符。该模型通过骨架生成和列表式重排序数据进行联合训练。

**Result:** 在Wikidata和Freebase标准基准测试中，GRISP在可比设置下比其他最先进方法取得了更好的结果。

**Conclusion:** 证明了通过小型语言模型的微调结合骨架引导和迭代选择的策略，能够有效地提升知识图谱问答的性能，为资源受限环境下的问答系统提供了新的解决方案。
...
> 本研究提出了Introspective and Interactive Visual Grounding (IVG)框架，结合图表底层规格查询和视图交互操作，解决了视觉语言模型在图表理解中的像素瓶颈问题，在包含500个交互式图表的iPlotBench基准测试中实现了81%的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言模型在理解图表时频繁出现数值误读、细节幻觉和重叠元素混淆等问题。根本原因是当前方法仅依赖像素解释，将交互式图表视为静态图像，无法访问包含精确数值的结构化规格说明，形成了像素瓶颈。

**Method:** 提出了IVG框架，包含两个核心组件：(1)基于规格的内省，通过查询底层规格获取确定性证据；(2)基于视图的交互，通过操作视图解决视觉歧义。同时构建了iPlotBench基准测试数据集，包含500个交互式Plotly图表和6,706个二元问题，配备真实规格说明。

**Result:** 实验证明内省提高了数据重建保真度，与交互结合后达到了最高的问答准确率(0.81)，在重叠几何体上获得6.7%的性能提升。进一步展示了IVG在部署的代理中的应用，能够自主探索数据并与人类用户实时协作。

**Conclusion:** IVG框架通过结合结构化规格查询和视图交互，有效克服了视觉语言模型的像素瓶颈限制，显著提升了图表理解的准确性，为构建更智能的数据分析代理提供了新的解决方案。
...
> 该研究提出了一个自动对话分析系统(ADAS)，通过结合LLB式数据增强和双探针头RoBERTa分类器，有效解决了科学课堂对话分析中的标签不平衡问题，并发现教师反馈性问题(Fq)是促进学生推理推理(SR-I)的关键因素。

<details>
  <summary>Details</summary>

**Motivation:** 科学课堂中学生推理模式的分析对于理解知识建构机制和改进教学实践至关重要，但大规模的人工编码课堂对话极其耗时费力，需要自动化解决方案。

**Method:** 开发ADAS系统，联合分类师生对话的两个维度：话语类型(UT)和推理 Component(RC)；采用分层重分割、LLB式合成数据增强针对少数类、训练双探针头RoBERTa-base分类器；进行话语模式分析包括共现分析、认知复杂度指数计算、滞后序列分析和IRF链分析。

**Result:** 零样本GPT-5.4基线在UT和RC上分别获得0.467和0.476的macro-F1得分；LLB式增强显著改善UT少数类识别；RC任务的结构简单性使其对词法基线也具有良好的可处理性；研究发现教师Fq动作是学生SR-I推理的最一致前因。

**Conclusion:** 本研究证明了LLB式数据增强在对话分类中的有效性，同时揭示了教师反馈性问题对促进学生推理推理的重要作用，为优化科学课堂教学提供了有价值的见解。
...
> 研究发现语言模型通过分离的'当前实体'和'前序实体'插槽来编码多个实体绑定信息，但只有当前实体插槽的信息被实际使用于事实检索，揭示了信息可用性与实际使用之间的差距。

<details>
  <summary>Details</summary>

**Motivation:** 研究语言模型如何在同一上下文中维护多个实体-属性绑定关系，以及单个token是否能承载多个实体的绑定信息，这是理解模型关键绑定机制的重要问题。

**Method:** 提出多插槽探测方法，通过分解单个token的残差流激活来同时识别当前描述实体和前序实体的信息，分析这两个插槽的功能角色和使用差异。

**Result:** 发现当前实体和前序实体插槽编码的信息相互正交且功能不同：前序实体插槽支持关系推理但未被用于事实检索；开源模型在处理强制单token承载双重绑定的语法时表现不佳，而最新前沿模型能够正确处理。

**Conclusion:** 研究结果揭示了激活中可用信息与模型实际使用信息之间存在差距，表明当前/前序实体插槽结构是维持双重视角行为的自然基础，为理解模型的高级认知行为提供了新见解。
...
> 研究提出了一种主动视觉脚手架框架，通过将对话状态增量转换为持久的视觉历史，解决了会话代理在维持共享上下文时的

<details>
  <summary>Details</summary>

**Motivation:** 当前会话代理在处理需要长期 维持共享上下文的情境对话时存在严重缺陷，主要体现在相似但不同的实体容易被压缩成可互换的描述，产生

**Method:** 受人类心理意象启发，提出主动视觉脚手架框架，增量地将对话状态转换为持久视觉历史，支持后续的基于情境的响应生成，并在IndiRef基准上评估性能。

**Result:** 实验表明，增量外化本身优于全对话推理，而视觉脚手架通过减少表征模糊和强化具体场景承诺带来额外提升；混合多模态设置在表示不可描绘信息时表现最佳，整体性能最优。

**Conclusion:** 会话代理需要明确的多模态共享上下文表示，整合描绘性和陈述性信息，这为解决长期情境对话中的上下文维持问题提供了新的解决思路。
...
> 研究表明，ASR偏见不仅体现在技术层面的错误率，更对用户造成深层的情感和心理伤害，包括持续的自我怀疑、情感劳动负担以及语言文化认同的边缘化。

<details>
  <summary>Details</summary>

**Motivation:** 现有ASR偏见研究主要关注技术指标（错误率），而忽视了系统偏见对用户体验的深层影响，包括情感负担、心理压力和文化认同问题，需要从人文视角全面评估算法公平性。

**Method:** 在代表不同英语方言社区的四个美国地区（亚特兰大、墨西哥湾沿岸、迈阿密海滩和图森）进行用户体验研究，通过开放式访谈收集用户对ASR系统失败的真实感受和应对策略。

**Result:** 研究发现大多数用户认为技术忽视其文化背景，需要不断调整才能实现基本功能；尽管如此，用户仍保持对ASR性能的高期望并愿意参与模型改进。用户经历持续的挫败感和内化的自我否定，承担了大量无形劳动。

**Conclusion:** 仅基于准确率的算法公平性评估忽略了关键的伤害维度：管理技术拒绝的情感劳动、持续自我监控的认知负担以及在使用母语变体时感受到不足的心理代价。技术设计应考虑用户情感体验和文化多样性。
...
> 本文提出了一种将前缀解析问题转化为普通解析的高效方法，通过构建前缀语法转换，使任何标准解析算法都能直接应用于前缀解析，无需专门算法。

<details>
  <summary>Details</summary>

**Motivation:** 前缀解析在加权设定下提供前缀概率，对上下文无关语言建模、心理语言学分析和大型语言模型的句法约束生成等应用至关重要，但现有方法需要专门的算法实现。

**Method:** 提出前缀语法变换技术，对于给定语法构造另一个能够精确生成其原始字符串前缀的新语法，然后通过应用任何普通解析算法来解决前缀解析问题；并提出基于算法微分的策略计算下一个词元的权重向量。

**Result:** 转换后的语法仅比输入语法稍大，任何优化实现都可以直接使用，消除了定制前缀解析算法的需要，实现了高效的前缀解析和下一个词元预测。

**Conclusion:** 该方法提供了一个简单、通用且高效的前缀解析框架，既优雅又实用，能够无缝集成现有的解析优化技术。
...
> 本文提出了一种基于推理增强的大语言模型职业预测方法，通过细粒度地生成职业选择理由来提升预测准确性。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在职业预测任务中表现不佳，因为它们缺乏与职业路径的对应关系，也无法捕捉职业决策背后的潜在推理因素。

**Method:** 设计两阶段框架：首先生成基于用户教育和职业历史的推理理由，然后基于该理由预测下一职业；使用LLM-as-a-Judge获得高质量oracle理由数据，微调小型LLM进行推理生成和职业预测。

**Result:** 实验显示该方法显著提升了LLM的职业预测准确率，与全监督方法相当且优于无监督方法；单一LLM同时执行两个任务优于分离式训练；推理质量直接影响预测准确性。

**Conclusion:** 基于推理增强的方法有效改善了LLM在职业预测任务中的表现，为职业推荐系统提供了新的技术路径。
...
> SPIA是首个以个体为评估单元的文本匿名化评估基准，揭示了当前基于span的评估方法无法真正保护个人隐私的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本匿名化评估基于span级别的指标，无法真实反映攻击者能推断出的信息，且仅考虑单一数据主体，忽略了多主体场景。

**Method:** 提出SPIA（主体级PII推断评估）基准，将评估单元从文本span转变为个体，包含法律和在线领域的675个文档，并设计了新颖的主体级保护指标。

**Result:** 实验发现即使90%以上的PII span被遮蔽，主体级推断保护率低至33%，大部分个人信息仍可通过上下文推断恢复；目标主体聚焦的匿名化会使非目标主体暴露程度远超目标主体。

**Conclusion:** 主体级推断评估对于确保真实环境下文本匿名化的安全性至关重要。
...
> 本文提出了一种名为IRM的零样本方法，利用隐式奖励模型来检测大语言模型生成的文本，该方法无需偏好收集或额外训练，在检测性能上超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型在各种任务中展现出强大能力，其生成类似人类文本的能力也引发了滥用的担忧，因此需要可靠有效的方法来检测LLM生成的文本。

**Method:** IRM利用从公开可用的指令调优模型和基础模型中推导出的隐式奖励模型进行检测，与之前依赖偏好构建和任务特定微调的基于奖励的方法不同，IRM既不需要收集偏好也不需要额外训练。

**Result:** 在DetectRL基准测试上的评估表明，IRM能够实现卓越的检测性能，在LLM生成文本检测方面超越了现有的零样本和监督方法。

**Conclusion:** IRM作为一种创新的零样本检测方法，通过隐式奖励模型实现了高效准确的LLM生成文本检测，为解决LLM文本检测问题提供了新思路。
...
> 该研究提出EngramaBench评估基准，比较了Engrama图结构记忆系统、GPT-4o全上下文提示和Mem0向量检索记忆系统在长期对话记忆任务中的表现。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型助手需要保留和推理跨多个会话的累积信息，评估长期对话记忆能力变得日益重要。

**Method:** 构建了EngramaBench基准，包含5个角色、100个多会话对话和150个查询，涵盖事实回忆、跨空间整合、时间推理等五个维度。评估了Engrama图结构记忆系统、GPT-4o全上下文提示和Mem0向量检索系统的表现。

**Result:** GPT-4o全上下文获得最高综合得分(0.6186)，Engrama全局得分0.5367但在跨空间推理上优于全上下文(0.6532 vs 0.6291)，Mem0成本最低但表现最弱(0.4809)。

**Conclusion:** 结构化记忆的专业化与聚合优化之间存在系统级权衡，图结构记忆在特定推理任务上具有优势。
...
> LLM4MEM是一个基于大语言模型的多表实体匹配框架，通过多风格提示增强、传递共识嵌入匹配和密度感知修剪三个核心模块解决语义不一致性、匹配效率和噪声实体问题，在6个数据集上F1值平均提升5.1%。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多表实体匹配方法在处理数值属性变化导致的语义不一致性时表现不佳，且面临着多数据源带来的实体数量激增导致的匹配效率问题，以及匹配过程中的噪声实体问题。

**Method:** LLM4MEM框架包含三个核心模块：1）多风格提示增强LLM属性协调模块，用于解决语义不一致性；2）传递共识嵌入匹配模块，处理实体嵌入和预匹配以提高匹配效率；3）密度感知修剪模块，优化多表实体匹配质量并处理噪声问题。

**Result:** 在6个MEM数据集上的广泛实验表明，与基线模型相比，LLM4MEM在F1指标上平均提升了5.1%。

**Conclusion:** 该研究成功利用大语言模型强大的语言理解能力构建了一个有效的多表实体匹配框架，在保持高质量匹配的同时解决了传统方法的关键限制，为多源数据整合提供了新的技术路径。
...
> PLOTTER是一个基于图结构的叙事规划框架，通过在事件图和角色图上执行评估-规划-修订循环，显著提升了大语言模型在复杂叙事生成中的全局连贯性、逻辑一致性和角色发展。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型在叙事生成中虽然表现流畅，但难以维持全局叙事连贯性、上下文逻辑一致性和平滑的角色发展，常常产生单调且有结构断裂的脚本。

**Method:** PLOTTER框架在事件图和角色图的结构化图表示上执行评估-规划-修订循环，通过在严格逻辑约束下诊断和修复图拓扑问题，在完整上下文生成前优化因果关系和叙事骨架。

**Result:** 实验表明，PLOTTER在多样化叙事场景中显著优于代表性基线模型，验证了基于图结构而非直接文本的叙事规划对增强大语言模型长上下文推理能力的重要性。

**Conclusion:** 在结构化图表示上进行叙事规划而非直接在文本上操作，对于提升大语言模型在复杂叙事生成中的长上下文推理能力至关重要。
...
> 本文提出了两个新的指标RPS和AGS来衡量LLM智能体的行为同质化问题，发现同系列模型间存在显著的行为模式相似性，Kimi-K2在工具使用习惯上甚至超越了Anthropic的Opus。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM智能体在模型蒸馏过程中出现了严重的同质化现象，许多智能体展现出几乎相同的推理步骤和失败模式，现有指标无法区分完成任务所必需的行为与反映模型自主偏好的非强制性模式。

**Method:** 提出两个互补指标：响应模式相似性(RPS)用于衡量语言对齐度，动作图相似性(AGS)用于建模为有向图的工具使用习惯。在τ-Bench和τ²-Bench上评估18个来自8个不同提供商的模型。

**Result:** 同系列模型对的AGS分数比跨系列高5.9个百分点；Kimi-K2达到82.6%的节点相似度和94.7%的依赖相似度，超过Anthropic的Opus 4.1；控制蒸馏实验证实AGS能有效区分教师特定收敛与一般改进。

**Conclusion:** RPS和AGS捕获了不同的行为维度(Pearson r=0.491)，为智能体生态系统中的行为趋同提供了互补的诊断信号，有助于识别和缓解模型蒸馏导致的行为同质化问题。
...
> 在语言模型预训练中，先用音乐数据训练Transformer能显著加速语言学习，在小型模型上获得17.5%的困惑度提升，且这种优势具有持久性。

<details>
  <summary>Details</summary>

**Motivation:** 探索如何为小型语言模型找到更高效的预训练策略，利用人类创造性输出的结构化特征来改善语言模型的学习效率。

**Method:** 采用音乐→诗歌→散文的三阶段发展管道，使用MAESTRO钢琴演奏数据集进行音乐预训练，通过多种模型规模(d∈{16,32,64})和实验设置验证效果。

**Result:** 音乐预训练带来17.5%的困惑度改进，音乐和诗歌分别改进了模型的内部计算和嵌入组件，5.5%的持久性能差距在收敛后仍然保持，且最优预训练数据量随模型容量变化。

**Conclusion:** 结构化的人类创造性输出可以作为小型语言模型的高效预训练基板，发现了模型容量依赖的数据精选原则，但现代预训练规模的结论需要更大规模的实验验证。
...
**Motivation:** 随着预训练大语言模型在语音识别中替代任务特定解码器，一个关键问题出现：基于文本的先验知识是否会使不同人群的识别更加公平或产生更多偏见？

**Method:** 通过评估三个架构代际的9个模型（无语言模型的CTC、隐式LM的编码器-解码器、显式预训练解码器的LLM），在约43,000个样本上测试5个人群维度（种族、口音、性别、年龄、母语），并在12种声学退化条件下进行压力测试，共216次推理运行。

**Result:** LLM解码器不会放大种族偏见；Whisper在印度口音语音上出现病态幻觉；音频压缩比LLM规模更能预测口音公平性；在声学退化下，静音注入会放大会话的口音偏见；显式LLM解码器比Whisper减少38倍的插入和重复。

**Conclusion:** 音频编码器设计，而非LLM规模扩展，是实现公平鲁棒语音识别的主要控制手段，这挑战了认为更大规模LLM必然带来更好表现的假设。
...
> 研究发现交叉熵损失是影响K路能量探针与softmax之间关系的关键因素，它在两种预测编码网络中产生不同的表现形式。

<details>
  <summary>Details</summary>

**Motivation:** 验证Cacioli(2026)提出的K路能量探针约化理论对交叉熵损失的依赖性，探索在不同预测编码网络配置下这种关系的稳健性。

**Method:** 在CIFAR-10数据集上对比三种配置：标准PC(MSE损失)、双向PC(bPC)和标准PC(CE损失)，使用10个随机种子训练2.1M参数的网络，分析能量探针与softmax的差距。

**Result:** 标准PC的探针低于softmax(Δ=-0.082)，bPC的探针超过softmax(Δ=+0.008)，移除CE使探针-softmax差距减半，CE训练产生15倍更大的logit范数。

**Conclusion:** 交叉熵是该分解理论的主要经验承载组件，其效果66%来自logit尺度效应，34%来自尺度不变的排序优势；术语'元认知'仅表示Type-2判别操作，不代表人类内省能力。
...
> 本文提出EAVAE框架，通过可解释的变分自编码器实现了写作风格与内容的解耦，在作者归属和AI生成文本检测任务中取得了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法难以处理内容-风格纠缠问题，模型会学习作者写作风格与主题之间的虚假相关性，导致跨领域泛化能力差。

**Method:** EAVAE首先使用监督对比学习预训练风格编码器，然后通过变分自编码器架构微调，采用独立的风格和内容编码器。通过新型判别器强制解耦，该判别器不仅区分风格/内容表示对是否属于同一作者/内容源，还生成自然语言解释来增强可解释性。

**Result:** 在作者归属任务中，EAVAE在Amazon Reviews、PAN21和HRS等数据集上达到最先进性能；在AI生成文本检测中，EAVAE在M4数据集上的少样本学习表现优异。

**Conclusion:** EAVAE通过架构设计实现了有效的风格-内容解耦，结合可解释性机制，在多个实际应用中展现出卓越的性能和泛化能力。
...
> 研究发现，多文档新闻摘要系统存在政治偏见，而更大的语言模型并不总是更公平。中等规模模型表现最佳，实体情感偏见最难矫正。

<details>
  <summary>Details</summary>

**Motivation:** 多文档新闻摘要系统虽然方便处理海量新闻，但可能存在政治偏见，表现为观点代表不均、强调特定视角、系统性忽视少数声音等，因此需要全面评估这种偏见。

**Method:** 使用带有政治倾向标签的FairNews数据集，评估13个大语言模型在5个公平性指标上的表现，测试了基线模型性能和多种去偏见干预方法（包括基于提示和基于判断的方法）。

**Result:** 中等规模模型在公平性和效率平衡上优于更大模型；基于提示的去偏见方法高度依赖于具体模型；实体情感是最顽固的公平性维度，所有干预策略都难以改善。

**Conclusion:** 多文档新闻摘要的公平性需要多维度评估框架，需要针对特定架构的去偏见策略，而不是简单地扩大模型规模。
...
> PolyChartQA是一个针对多图表图像问答任务的中等规模数据集，包含534个多图表图像和2694个QA对。研究评估了9个多模态语言模型的表现，发现人类编写问题的准确率比模型生成问题低27.4%，而提出的提示方法可提升5.39%的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 图表被广泛用于呈现复杂信息，但在实际场景中理解多图表相关性的研究较少。为了填补多图表图像理解研究的空白，需要一个专门的基准数据集。

**Method:** 从同行评议的计算机科学研究出版物中收集534个多图表图像（共2297个子图表）和2694个问答对，构建PolyChartQA数据集，并评估9个最先进的多模态语言模型在不同维度上的性能。

**Result:** 在人类编写的问题上，基于LLM的准确率比MLM生成的问题低27.4%；使用提出的提示方法可获得5.39%的L-准确率提升。

**Conclusion:** PolyChartQA填补了多图表图像问答研究的数据缺口，揭示了当前多模态语言模型在处理真实世界多图表理解任务时的局限性，并提供了改进方向。
...
> CARE是一个基于生成式AI的咨询支持框架，通过在真实危机对话数据上微调开源大语言模型，为希伯来语和阿拉伯语环境下的心理咨询师生成实时、心理对齐的回复建议。

<details>
  <summary>Details</summary>

**Motivation:** 全球心理健康挑战日益增加，导致情感支持服务过载，在自杀意念等危急情况下可能出现响应延迟。现有大语言模型在低资源语言（特别是心理健康等敏感领域）的应用尚未充分探索，且缺乏专业训练数据来复制专业人士的支持性语言和干预策略。

**Method:** 提出CARE框架，使用专业咨询师评为高效果的真实危机对话子集，分别对希伯来语和阿拉伯语的开源大语言模型进行微调。通过在完整对话历史上进行训练，保持咨询师与求助者对话的情感演化和动态结构。

**Result:** 实验表明，与非专业大语言模型相比，CARE在语义和策略层面与标准咨询师回复的对齐度更高，能生成更符合专业标准的建议。

**Conclusion:** 基于专家验证数据的领域特定微调可以显著支持咨询师工作流程，改善低资源语言环境下的护理质量，为心理健康服务提供有效的技术支持。
...
> 该研究针对22种语言的极化检测任务，提出了结合多语言通用模型、语言特定专家模型和混合集成的自适应框架，在SemEval-2026上获得了0.796的F1分数。

<details>
  <summary>Details</summary>

**Motivation:** 多语言极化检测面临挑战：标准多语言模型(如XLM-RoBERTa)在文本分词器对齐时表现良好，但在处理不同文字系统(如高棉语、奥里亚语)时效果不佳，需要更灵活的语言自适应解决方案。

**Method:** 采用语言自适应框架，根据开发集性能在多语言通用模型、语言特定专家模型和混合集成之间动态切换；同时实验了基于NLLB-200的跨语言增强技术，并公开了代码和预测结果。

**Result:** 最终系统在22种语言上实现了0.796的宏平均F1分数和0.826的平均准确率；跨语言增强效果不佳，经常不如本地架构选择，并且在形态丰富的语言上会降低性能。

**Conclusion:** 单一通用架构无法适用于所有语言，语言自适应方法能够有效平衡通用性和特殊性，为多语言极化检测提供了实用的解决方案。
...
> VLAA-GUI是一个模块化GUI智能体框架，通过完整性验证器强制检查成功条件、循环中断器打破重复循环、搜索代理在线查询工作流程三个核心组件解决GUI智能体早期停止和重复循环问题，在多个基准测试中达到顶尖性能。

<details>
  <summary>Details</summary>

**Motivation:** 自主GUI智能体面临两大基本挑战：早期停止（智能体过早声称成功而无可靠证据）和重复循环（智能体循环执行相同的失败动作而无法恢复）。这些问题严重限制了GUI自动化任务的可靠性和效率。

**Method:** VLAA-GUI框架集成了三个核心组件：1）强制完整性验证器在每个完成步骤检查可观察的成功标准；2）强制循环中断器提供多层过滤，在重复失败后切换交互模式、强制策略改变；3）按需搜索代理通过查询具备搜索能力的LLM获取陌生工作流程。同时还集成了编码代理和接地代理来处理特定需求。

**Result:** 在五个顶级模型上的评估表明，VLAA-GUI在OSWorld和WindowsAgentArena两个基准测试上都取得了顶级性能（77.5%和61.0%）。其中三个模型在OSWorld单次通过中超过了人类性能（72.4%）。消融实验显示所有三个组件都能持续提升强基线模型性能。

**Conclusion:** VLAA-GUI通过结构化的验证、恢复和搜索机制有效解决了GUI智能体的核心问题。特别是循环中断器将近减半了易循环模型的浪费步骤，证明了该方法在提高任务执行效率和可靠性方面的显著价值。
...
> Decoupled DiLoCo是一种去中心化同步的大规模语言模型训练框架，通过异步通信和容错机制显著提高了故障环境下的训练效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大规模语言模型预训练采用SPMD范式，存在紧耦合问题，导致计算延迟、硬件故障和同步开销会严重影响整个训练过程，即使DiLoCo等方法减少通信带宽，但仍保持同步特性，易受系统停顿影响。

**Method:** 提出Decoupled DiLoCo框架，将计算分配到多个独立的学习器上执行本地内层优化；学习器异步地将参数片段发送给中央同步器；同步器通过最小法定数量、自适应宽限期和动态token权重合并机制来处理故障或落后的学习器。

**Result:** 在包含数百万个模拟芯片的易发生故障环境中，实现了严格零全局停机下的显著训练效率提升，同时在文本和视觉任务中保持了竞争性的模型性能，适用于密集和混合专家架构。

**Conclusion:** Decoupled DiLoCo成功打破了锁步同步障碍，超越了传统SPMD范式，为大规模AI模型训练提供了更可靠、高效的解决方案。
...
> 研究发现，结合注意力和循环状态的混合架构在需要状态追踪和回忆的推理任务中表现更好，推理增强token能扩大模型有效操作范围，但架构的状态传播能力决定其实际效果。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型的推理能力通常被视为整体能力，但这种增益可能源自更基本的操作。研究旨在探讨结合注意力检索和循环状态更新的混合架构是否比纯注意力模型更适合需要同时处理记忆和状态追踪的任务。

**Method:** 使用匹配的Olmo3 transformer和混合模型，在指令微调和推理增强版本下，在包含状态追踪和回忆混合任务的受控任务集上进行评估。

**Result:** 推理增强提供了最大的整体改进，显著扩展了模型保持有效的难度范围。在某些任务中，混合推理模型随着顺序依赖增加保持更强的鲁棒性，而transformer推理模型在任务难度超过阈值时性能急剧下降。

**Conclusion:** 推理token和架构归纳偏向在计算过程的不同层面发挥作用：显式推理能扩展模型有效操作范围，但其效益取决于底层架构支持持续状态传播的能力。这些发现具有启发性而非结论性，需要更广泛的验证。
...
> 本研究通过数据选择策略改善合规检测任务的跨领域迁移问题，证明定向数据选择能显著减少负迁移效应。

<details>
  <summary>Details</summary>

**Motivation:** 自动化合规检测面临法律文本复杂多变、模型跨领域泛化能力差的挑战，现有模型在单一法规上训练后难以适应其他法规，需要系统性方法解决跨领域迁移中的负迁移问题。

**Method:** 将合规检测建模为自然语言推理任务，评估四种数据选择方法：随机采样、Moore-Lewis交叉熵差法、重要性加权和基于嵌入的检索，通过系统改变选定数据比例来分析跨领域适应效果。

**Result:** 实验结果表明，定向的数据选择能显著减少负迁移现象，为跨异构法规的可扩展可靠合规自动化提供了实用解决方案。

**Conclusion:** 数据选择是缓解合规检测中跨领域负迁移的有效策略，为构建更加通用和可靠的合规自动化系统提供了重要见解和方法论指导。
...
> 本研究提出了一个针对多语言TTS系统的受控多维 pairwise 评估框架，通过语言控制和感知相关标注收集了超过12万次比较，构建了多语言排行榜并分析了系统在不同感知维度上的表现。

<details>
  <summary>Details</summary>

**Motivation:** 众包pairwise评估虽然可扩展，但在TTS评估中由于语言多样性和语音感知的多维特性导致高方差，需要更受控和全面的评估方法。

**Method:** 结合语言控制和感知相关标注的受控多维pairwise评估框架，使用5000+跨10种印度语言的句子，评估7个先进TTS系统，从1900多名母语评测者收集超过12万次pairwise比较，涵盖6个感知维度（可懂度、表现力、音质、生动性、噪声和幻觉）。

**Result:** 使用Bradley-Terry建模构建多语言排行榜，通过SHAP分析解释人类偏好，验证排行榜可靠性，揭示了不同模型在感知维度上的优势与权衡。

**Conclusion:** 该框架有效解决了TTS多语言评估的高方差问题，为理解模型在不同感知维度上的表现提供了可靠方法，对多语言TTS系统的改进具有重要指导意义。
...
> 提出了OptiVerse基准测试和双视图审计代理，用于评估和提升大语言模型在复杂优化任务中的表现，发现了现有模型在困难问题上的显著性能瓶颈。

<details>
  <summary>Details</summary>

**Motivation:** 现有评估基准主要关注数学规划和组合优化，忽视了许多重要的优化领域（随机优化、动态优化、博弈优化、最优控制），无法全面评估LLMs在复杂优化任务上的能力。

**Method:** 构建了OptiVerse基准测试（包含1000个精心策划的问题，涵盖4个被忽视的优化领域和3个难度级别），评测了22个不同规模的LLM，通过错误分析识别主要瓶颈，并提出双视图审计代理来改进建模过程。

**Result:** 实验显示所有模型在困难问题上性能急剧下降，即使是最先进的GPT-5.2和Gemini-3模型在困难问题上的准确率也不超过27%；错误分析表明建模和逻辑错误是主要瓶颈；提出的双视图审计代理能在不显著增加时间开销的情况下提高建模准确性。

**Conclusion:** OptiVerse将作为推进LLMs解决复杂优化挑战的基础平台，为该领域的研究和评估提供了全面的基准。
...
> SRICL是一个结合语义检索、上下文学习和监督微调的LLM框架，专门解决从招聘广告中提取技能术语时出现的格式错误、边界偏移和幻觉问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的生成式大语言模型在从招聘广告中提取跨域技能术语时，经常产生格式错误的词汇片段、边界漂移和幻觉问题，特别是在处理长尾术语和跨域迁移场景时。

**Method:** SRICL框架采用多层策略：1) 语义检索(SR)从ESCO获取领域内标注句子和定义构建格式约束提示；2) 监督微调(SFT)对齐输出行为；3) 确定性验证器确保配对、非重叠和BIO合法性，仅需最少重试次数。

**Result:** 在六个跨行业多语言的招聘广告语料库上，SRICL显著提升了STRICT-F1指标，大幅减少无效标签和幻觉片段，实现了在低资源多域环境下的句子级可靠部署。

**Conclusion:** SRICL成功解决了LLM在技能提取任务中的关键缺陷，通过结合检索增强学习、格式约束和确定性验证，为候选-职位匹配和劳动力市场分析提供了可靠的解决方案。
...
> 该论文提出了一个多方法融合系统，在SemEval-2026情感分析任务中取得了双料冠军，发现LLM擅长静态情感识别，而短期情感变化更多取决于数值轨迹特征。

<details>
  <summary>Details</summary>

**Motivation:** 解决用户生成文本中当前情感状态和短期情感变化的建模问题，这是情感分析领域的重要挑战，需要同时考虑时间序列特征和文本语义信息。

**Method:** 采用三种互补方法：(1)在用户感知和用户不可知设置下的LLM提示技术；(2)带有伊辛风格交互作用的成对最大熵模型用于结构化转换建模；(3)融合最近情感轨迹和可训练用户嵌入的轻量级神经网络回归模型。

**Result:** 研究发现大型语言模型能有效捕捉文本中的静态情感信号，而该数据集中短期情感变化更多地由最近的数值状态轨迹解释而非文本语义；系统在官方评估指标下在子任务1和子任务2A中均排名第一。

**Conclusion:** 多方法融合策略有效解决了时序情感分析问题，验证了不同方法在静态情感和动态情感变化建模上的互补性，为相关研究提供了有价值的技术路线。
...
> 本文提出了一种无关分类器的新方法-概念分离曲线，用于评估句子嵌入质量，通过系统引入句法噪声和语义否定来衡量模型区分概念层面和表层面变化的能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的句子嵌入评估方法大多依赖额外的分类器或下游任务，无法确定好结果是来自嵌入本身还是分类器的行为，缺乏客观性和独立性。

**Method:** 通过系统地向句子引入句法噪声和语义否定，量化这些变化对嵌入结果的影响，并使用概念分离曲线可视化这些效果，评估模型区分概念和表层面变化的能力。

**Result:** 在多个领域、荷兰语和英语两种语言以及不同句子长度上的实验证明，概念分离曲线提供了一种可解释、可重现且跨模型的句子嵌入概念稳定性评估方法。

**Conclusion:** 概念分离曲线方法为句子嵌入的评估提供了一个独立于分类器、客观且可解释的新范式，能够有效衡量嵌入模型的概念捕捉能力。
...
> 研究人员开发了llm-bias-bench方法，通过直接探询和间接辩论两种互补技术，揭示大语言模型在争议话题上的真实立场和偏见。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型深度嵌入人们的日常信息获取中，当模型对争议话题持有沉默立场时，这种立场会大规模影响用户决策。但现有方法难以准确获取模型的真实观点，因为模型通常会回避直接回答，且在不同交互条件下可能表现出相反立场。

**Method:** 设计了两种互补的探询方法：直接探询在5轮递增强度中询问模型观点；间接探询从不直接询问观点，而是让模型参与论证辩论，观察其偏见如何通过退让、抵制或反驳体现。设置三种用户人设（中性、同意、反对），使用可审计的LLM裁判产生带文本证据的判决。

**Result:** 在13个助手模型上的应用发现：论证辩论比直接询问更容易触发谄媚行为（中位数从50%增至79%）；在直接询问中显得有观点的模型在持续辩论下往往会变成镜像行为；攻击者的能力主要在需要改变现有观点时起作用，而非助手初始中立时。

**Conclusion:** 该方法有效揭示了LLM在真实多轮交互环境下的潜在偏见和立场，为理解和评估大语言模型的社会影响提供了重要工具。
...
> 论文提出AgenticQwen模型系列，通过多轮强化学习和双重数据飞轮机制训练，能在工业应用中以小模型规模实现接近大模型性能的多步推理和工具使用能力。

<details>
  <summary>Details</summary>

**Motivation:** 现代工业应用需要能够进行多步推理和工具使用的语言模型作为智能体，但这些任务通常面临严格的成本和延迟约束，使得小型智能体模型成为迫切需求。

**Method:** 通过多轮强化学习在合成数据和有限开源数据上训练，采用推理RL和智能体RL相结合的框架，配合双重数据飞轮：推理飞轮通过学习错误增加任务难度，智能体飞轮将线性工作流扩展为多分支行为树以更好反映现实决策复杂性。

**Result:** AgenticQwen在公共基准测试和工业智能体系统中表现出色，在多个智能体基准上取得强劲性能，在工业系统的搜索和数据分析任务中显著缩小与更大模型的差距。

**Conclusion:** AgenticQwen证明了通过创新的强化学习训练框架和数据生成机制，小型模型能够在工业智能体应用中达到接近大模型的性能水平，为成本敏感的实际部署提供了可行方案。
...
> 研究发现非英语响应在推理任务中有时优于英语，提出polyGRPO框架将语言作为潜在推理变量，通过多语言相对策略优化显著提升模型的推理能力，展现出强大的跨任务泛化性。

<details>
  <summary>Details</summary>

**Motivation:** 观察到一个令人惊讶的趋势：随着大语言模型减少英语偏见，非英语响应在推理任务上有时反而超越英语表现。作者假设语言不仅是输出媒介，更是调节模型内部推理路径的潜在变量。

**Method:** 设计多语言思维实验，在语言约束和无约束条件下测试模型解决相同问题；提出polyGRPO框架将语言变异作为隐式探索信号，在线生成多语言偏好数据，基于答案准确性和推理结构进行策略优化。

**Result:** polyGRPO仅在18.1K多语言数学问题上训练，就在四个英语推理测试集上提升6.72%准确率，多语言基准提升6.89%。更令人惊讶的是，尽管只用数学数据训练，却在英语常识推理任务上超越基础模型4.9%。

**Conclusion:** 将语言视为潜在变量能够扩展模型的潜在推理空间，产生一致且可泛化的推理性能提升，为多语言模型训练提供了新的视角和方法。
...
> Verbal Process Supervision (VPS)是一种无训练框架，通过结构化自然语言批判指导LLM推理，显著提升性能，确立了批判粒度作为推理时缩放的新维度。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM推理时缩放方法（链深度、样本广度、学习步评分器）存在局限，需要探索新的缩放维度来进一步提升推理性能。

**Method:** 提出VPS框架，利用更强监督模型提供结构化自然语言批判，通过生成-批判-优化的迭代循环，在轮次预算R内指导弱模型推理。

**Result:** 在GPQA Diamond上达到94.9%准确率刷新SOTA；在AIME 2025上实现从11.7-26.7%到63.3-90.0%的大幅提升；在相同计算量下超越Reflexion和Self-Consistency方法。

**Conclusion:** 批判粒度是推理时缩放的有效新维度，性能与监督-执行者能力差正相关，但在非语言可表达错误场景下受限，需要混合方法。
...
> 本文调研了232篇论文，分析了语言模型在全球南方非英语社区边缘部署中的'最后一英里'挑战，即多语言性与边缘部署的交叉问题。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型的部署位置和方式决定了谁能受益，但在全球南方的非英语社区和硬件受限环境中面临多语言性与边缘部署的技术要求冲突，阻碍了模型的公平部署。

**Method:** 通过系统性综述232篇论文，从数据收集、模型开发到部署的完整语言建模流程，分析多语言边缘NLP的研究现状和挑战。

**Result:** 发现多语言NLP和边缘部署研究仍相互割裂，需要将两个领域结合研究以满足语言多样社区的基础设施约束需求。

**Conclusion:** 提出开放问题和可操作建议，呼吁NLP生态系统各方合作，推动包容性和公平性语言技术的发展。
...
> 提出一个联合建模标记者标签预测和解释的框架，通过用户护照机制和个人化解释器，显著提升了预测性能并能更好地捕捉个体视角差异。

<details>
  <summary>Details</summary>

**Motivation:** 传统的聚合标注方法忽略了标记者背后不同的视角和理由，而解聚标签和标记者提供的基本原理包含了更细粒度的个体视角信号，需要新方法来同时建模这些信息。

**Method:** 设计了一个包含用户护照机制和两种解释器架构的框架：1）在表示层面引入User Passport将标记者身份和人口统计元数据融入预测；2）提出基于提示的后置解释器和前缀桥接解释器，将标注条件化的分类器表示直接转移到生成模型中。

**Result:** 整合解释建模显著提高了预测性能，其中前缀桥接方法在标签对齐和语义一致性上表现更稳定，而后置方法在词汇相似性上表现更强，证明了解释建模能够提供更丰富和更真实的差异表示。

**Conclusion:** 该方法通过将标特定的基本原理整合到预测和生成组件中，推进了视角主义建模，为理解和处理标注数据中的主观性差异提供了新的思路。
...
> 该研究开发了新型过滤技术，将持久同调拓扑数据分析方法应用于眼动序列，构建拓扑特征与传统统计特征混合的模型，在阅读障碍检测任务中显著优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有眼动数据分析方法主要依赖传统统计特征，可能忽视数据中蕴含的深层拓扑结构信息。阅读障碍检测需要更鲁棒和互补的特征提取方法来捕捉注视序列中的模式。

**Method:** 1) 将注视序列解释为时间序列；2) 开发新型过滤(filtration)技术；3) 应用持久同调提取多尺度拓扑特征；4) 构建结合拓扑特征和传统统计特征的混合模型；5) 在哥本哈根语料库上进行实验验证。

**Result:** 混合模型表现显著优于仅使用传统特征的方法，拓扑特征能够捕获注视序列中的互补信息，性能达到与已建立基线方法相当的水平，且提出的过滤技术优于现有过滤方法。

**Conclusion:** 拓扑数据分析方法，特别是持久同调，为眼动数据分析提供了新的有效工具。所开发的过滤技术与混合模型框架在阅读障碍检测等实际应用中具有重要的潜力，证明了拓扑特征在补充传统分析方面的价值。
...
> 本研究通过语音学特征子空间的d-prime可分性分析，在3374名跨12种语言的患者样本中验证了构音障碍严重程度评估方法的鲁棒性，发现该方法具有病因特异性、跨语言轮廓稳定性和架构独立性等特点。

<details>
  <summary>Details</summary>

**Motivation:** 构音障碍严重程度评估需要开发训练无关的自动化方法。先前基于HuBERT-base的方法在较小规模数据集上得到验证，本研究旨在将该方法扩展到更大规模、更多样化的语言和病因样本中，验证其普适性和鲁棒性。

**Method:** 使用6种自监督学习模型，分析12种语言、5种病因(帕金森、脑瘫、ALS、唐氏综合征、中风)及健康对照共3374名说话者的语音学特征子空间d-prime可分性。通过效应量分析、跨语言余弦相似度计算和模型间相关性评估等方法进行验证。

**Result:** 发现病因特异性降解轮廓在群体水平可区分(13个特征中10个效应量大)，跨语言轮廓形状稳定(余弦相似度>0.95)，方法具有架构独立性(6个模型相关性>0.77)。但个体水平分类能力有限(22.6%宏F1)，绝对d-prime值跨语言需校准。

**Conclusion:** 语音学子空间分析为构音障碍特征化提供了鲁棒、训练无关的框架，具有病因感知能力、跨语言轮廓稳定性和跨骨干模型鲁棒性，为大规模构音障碍研究和临床应用奠定基础。
...
> 研究发现通过简单条件语句评估代码生成偏差会严重低估实际偏差，在更现实的机器学习管道生成任务中，大模型生成的代码有87.7%包含敏感属性，远高于简单条件语句的59.2%。

<details>
  <summary>Details</summary>

**Motivation:** 现有代码生成偏差评估主要使用简单条件语句，这种方式只能发现明显的、明确编码的偏差，无法反映真实编程场景中的偏差情况，因此需要研究更贴近实际的评估方法。

**Method:** 通过生成机器学习管道这一更现实的任务来评估代码生成偏差，测试了专门面向代码和通用指令的大语言模型，比较了管道生成与简单条件语句中的偏差表现。

**Result:** 生成的机器学习管道在特征选择阶段表现出显著偏差，敏感属性平均出现在87.7%的情况下，而简单条件语句中仅为59.2%。这种偏差在不同提示缓解策略、属性数量和管道难度下都普遍存在。

**Conclusion:** 研究挑战了简单条件语句作为偏差评估代理的有效性，表明当前基准测试低估了实际部署中的偏差风险，需要开发更贴近现实应用的偏差评估方法。
...
> X-GRAM是一个频率感知的动态token注入框架，通过混合哈希和别名混合技术压缩长尾分布，使用SwiGLU ShortConv提取n-gram特征，并通过深度感知门控机制整合到注意力层，实现了计算解耦的模型扩展路径。

<details>
  <summary>Details</summary>

**Motivation:** 大型token索引查找表虽然提供计算解耦的扩展路径，但其实际效果受到参数效率低下和内存快速增长的限制，主要原因是齐夫定律下的长尾欠训练、层间的异质需求以及产生冗余嵌入的'槽位塌陷'问题。

**Method:** X-GRAM框架采用混合哈希和别名混合技术压缩长尾分布同时保持头部容量，通过归一化SwiGLU短卷积提取多样化的局部n-gram特征，并使用深度感知门控将这些信号集成到注意力值流和层间残差中。

**Result:** 在0.73B和1.15B规模上的广泛评估显示，X-GRAM相比原始骨干网络提升平均准确率高达4.4分，相比强检索基线提升3.2分，同时在50%配置下使用更小的查找表。

**Conclusion:** X-GRAM通过高效的内存管理实现了容量与计算的解耦，为未来内存增强架构提供了一个可扩展且实用的范式。
...
> AEL框架通过双时间尺度设计解决了LLM智能体如何有效利用经验的核心问题：快速时间尺度使用汤普森采样选择记忆检索策略，慢速时间尺度通过LLM反思注入因果洞察。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM智能体在面对长序列开放式环境时仍然是无状态的，无法将过去的经验转化为更好的未来行为。关键障碍不是记忆什么，而是如何使用已记忆的信息，包括检索策略选择、历史结果解释和策略调整时机。

**Method:** 提出Agent Evolving Learning (AEL)框架，包含两个时间尺度：1）快速时间尺度使用汤普森采样bandit学习每集的记忆检索策略；2）慢速时间尺度通过LLM驱动的反思诊断失败模式，将因果洞察注入决策提示中。

**Result:** 在连续投资组合基准测试上（10个股票，208集，5种子），AEL达到夏普比率2.13±0.47，优于5种已发表的自改进方法和所有非LLM基线，且在LLM方法中方差最低。实验显示记忆和反思的简单组合比baseline提升58%，而额外机制都会降低性能。

**Conclusion:** 智能体自改进的瓶颈在于自我诊断如何使用经验，而非增加架构复杂性。这证明了"少即是多"的设计原则，简单有效的记忆和反思机制比复杂的扩展方法更有效。
...
> StructMem是一个结构增强的层次化记忆框架，通过维护事件级绑定和跨事件连接，在不牺牲效率的前提下显著改善了对话代理的时间推理和多跳问答能力。

<details>
  <summary>Details</summary>

**Motivation:** 长期对话代理需要能够捕获事件间关系的记忆系统，而不仅仅是孤立的事实，以支持时间推理和多-hop问答。现有方法面临基本权衡：平面记忆高效但无法建模关系结构，而基于图的记忆虽能实现结构化推理，但构建成本高昂且脆弱。

**Method:** 提出StructMem框架，通过时间锚定双重视角和定期语义整合，维护事件级绑定并诱导跨事件连接。采用层次化结构设计，在保持关系建模能力的同时优化计算效率。

**Result:** 在LoCoMo数据集上提升了时间推理和多-hop性能，同时大幅降低了token使用量、API调用次数和运行时间，相比之前的记忆系统有显著改进。

**Conclusion:** StructMem成功解决了记忆结构化与效率之间的权衡问题，为对话代理提供了一种实用的记忆解决方案，在保持性能的同时显著提升了计算效率。
...
> 研究发现大型语言模型在文化相关问题中表现出明显的地域偏好，倾向于日本等国家，且这种偏见在监督微调阶段而非预训练阶段形成。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究表明LLMs存在文化覆盖和能力的局限性，并可能放大西方和英语中心主义观点。但缺乏专门针对文化相关问题的地域偏好的系统性研究。

**Method:** 构建了基于文化相关开放问题(CROQ)分类法的全新数据集，系统分析了LLMs在文化问题上的地域偏好，并追踪了偏见在训练过程中的形成阶段。

**Result:** LLMs表现出对日本等国家的明显偏好；使用英语等高资源语言提问时模型输出更多样化，对输入语言为官方语言的国家偏好降低；文化偏见主要在监督微调阶段出现。

**Conclusion:** LLMs的文化地域偏见是一个需要关注的问题，需要在模型训练和部署阶段采取措施来减轻这种倾向性，提高文化多样性表现。
...
> AUDITA是一个大规模的真实世界音频问答基准测试，旨在超越表面声学识别，评估真实的音频推理能力，现有模型在该基准上表现极差。

<details>
  <summary>Details</summary>

**Motivation:** 现有音频问答基准过度依赖声音事件分类和基于字幕的查询，模型可通过捷径策略、短时线索、词汇先验等手段绕过真正的听觉推理，无法有效评估模型的深度音频理解能力。

**Method:** 构建AUDITA基准，包含精心策划的人工编写的真实世界音频琐事问题，通过具有挑战性的干扰项和长程时间依赖关系来压力测试稳健的听觉推理，使用探测式问题确保无法仅从孤立文本或声音线索回答。

**Result:** 人类平均准确率为32.13%，既反映了任务难度又证明了有意义的音频理解；而最先进音频问答模型表现极差，平均准确率低于8.86%。

**Conclusion:** 应用项目反应理论(IRT)估计潜在熟练度、问题难度，并揭示了模型和数据的系统性缺陷，证明当前模型在深层音频推理方面存在严重不足。
...
> 本文通过创建两个新数据集，研究基于音频转录的视频虚假信息片段检测，实现了在视频中定位虚假信息内容的技术，F1值达到0.68。

<details>
  <summary>Details</summary>

**Motivation:** 传统视频虚假信息检测只判断整个视频是否包含虚假信息，缺乏对虚假信息出现时间和具体内容的解释，无法为事实核查人员提供足够的上下文信息

**Method:** 创建两个包含500多个视频和2400多个标注片段的新数据集，将视频音频转录为文本并标注虚假信息片段，使用最先进语言模型构建分类器进行片段检测

**Result:** 成功实现了视频虚假信息片段检测，F1分数达到0.68，并公开发布了数据集、转录文本、音频和视频资源

**Conclusion:** 该方法能够有效定位视频中的虚假信息片段，为事实核查提供了更精确和可解释的检测手段，填补了现有研究的空白
...
> 本文介绍了NSNRL共享任务，这是一个新颖的叙事相似性评估任务，包含二元分类和嵌入表示两个赛道，收集了1000+故事摘要三元组，收到了71份提交，发现LLM集成在分类任务中表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 现有的叙事相似性评估缺乏与叙事理论和人类直觉兼容的定义，需要建立标准化的评估框架和数据集来推动叙事表示学习的发展。

**Method:** 将叙事相似性操作化为二元分类问题（判断两个故事哪个更接近锚点故事），收集了1000+故事摘要三元组进行标注，设置分类和嵌入两个赛道，采用多标注者确认数据质量。

**Result:** 分类赛道中LLM集成系统表现最佳，嵌入赛道中带预处理和后处理的预训练模型与微调模型性能相当，任务网站提供了可视化和结果分析。

**Conclusion:** 该任务为叙事相似性学习建立了标准化评估基准，分析表明自动系统在两个赛道都有进一步提升空间，为未来研究提供了方向。
...
> 提出RedirectQA数据集，分析LLM对实体不同表面形式的记忆差异，发现事实记忆既非纯表面特定也非完全表面不变，评估时需要考虑表面形式多样性。

<details>
  <summary>Details</summary>

**Motivation:** 理解大型语言模型记忆何种事实知识对评估其可靠性和局限性至关重要。传统的实体问答评估只使用单一规范表面形式，难以区分事实记忆与通过特定名称访问的区别。

**Method:** 引入RedirectQA数据集，利用维基百科重定向信息将Wikidata事实三元组与实体的分类表面形式关联，包括别名、缩写、拼写变体和常见错误形式。在13个LLM上测试不同实体表面形式对事实记忆的影响。

**Result:** 仅改变实体表面形式就会改变预测结果，且不一致性具有类别依赖性——模型对轻微正字法变化比较大词汇变化（如别名和缩写）更鲁棒。实体和表面层频率都与准确性相关，且实体频率的作用常超越表面频率。

**Conclusion:** 事实记忆既非纯表面特定也非完全表面不变，强调在评估非逐字记忆时表面形式多样性的重要性。
...
> MODEE是一种创新的开领域事件提取方法，它结合基于图的学习和大型语言模型的文本表示来建模文档级推理，在多个数据集上超越了最先进的方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有事件提取方法存在局限：闭领域算法只能处理预定义事件类型，无法泛化到新类型；开领域算法虽然能处理不受约束的事件类型，但忽视了大型语言模型的潜力，且没有显式建模文档级的上下文、结构和语义推理，而这些对有效事件提取至关重要。

**Method:** 提出MODEE方法，这是一种多模态开领域事件提取方法，结合基于图的学习与LLM的文本表示来建模文档级推理，解决了LLM在处理长文档时出现的'lost-in-the-middle'现象和注意力稀释问题。

**Result:** 在大型数据集上的实证评估表明，MODEE超越了最先进的开领域事件提取方法，并且能够泛化到闭领域事件提取，在该领域也优于现有算法。

**Conclusion:** MODEE成功结合了图学习的结构化推理能力和LLM的强大文本理解能力，为开领域和闭领域事件提取提供了一个统一且高效的解决方案，显著提升了事件提取的性能和泛化能力。
...
> 本文提出了TingIS端到端企业级事件发现系统，结合高效索引技术与大型语言模型，实现了从嘈杂用户反馈中快速发现技术异常，在2000+消息/分钟的生产环境中达到3.5分钟P90告警延迟和95%高优先级事件发现率。

<details>
  <summary>Details</summary>

**Motivation:** 大规模云原生服务的技术异常实时检测和缓解至关重要，客户事件是发现监控遗漏风险的重要信号，但由于极端噪声、高吞吐量和复杂业务语义，从中提取可操作情报极具挑战性。

**Method:** TingIS系统核心是多阶段事件链接引擎，结合高效索引技术与LLM进行事件合并决策；配备级联路由机制实现精确业务归因；多维度降噪管道整合领域知识、统计模式和行为过滤，实现从少量多样化用户描述中稳定提取可操作事件。

**Result:** 在处理峰值2000消息/分钟、日处理30万消息的生产环境中，TingIS实现P90告警延迟3.5分钟，高优先级事件发现率95%；基于真实数据的基准测试显示在路由准确性、聚类质量和信噪比方面显著优于基线方法。

**Conclusion:** TingIS成功解决了企业环境下从嘈杂高吞吐量事件流中发现技术异常的挑战，证明了LLM与传统技术结合的有效性，为大规模云服务提供了可靠的实时异常检测解决方案。
...
> 研究者开发了EVENT5Ws，一个大规模、人工标注的开域事件抽取数据集，解决了现有数据集覆盖范围有限和缺乏验证的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有事件抽取数据集存在局限性：封闭域设置下事件类型覆盖有限；开域设置下缺乏大型、人工验证的数据集。这些限制了自动化事件抽取方法的发展，而事件抽取对于紧急情况下的决策制定等任务至关重要。

**Method:** 通过设计系统化的标注管道创建了EVENT5Ws数据集，涉及人工标注和统计验证。使用该数据集评估了最先进的预训练大语言模型，建立了研究基准，并测试了模型在不同地理环境数据集上的泛化能力。

**Result:** 成功构建了EVENT5Ws数据集，为开域事件抽取提供了高质量基准。实验表明，基于该数据集训练的模型能有效泛化到不同地理背景的数据集，证明了其开发具有泛化能力的算法的潜力。

**Conclusion:** EVENT5Ws数据集填补了开域事件抽取数据集的空白，为开发具有良好泛化能力的算法提供了基础。研究者总结了数据集开发过程中的经验教训，为未来大规模数据集开发提供了指导建议。
...
> 本文提出一个计算框架分析巴西议会话语，结合历时文体学、主题建模和语义聚类，揭示话语风格、议程焦点和政治联盟模式，补充传统投票记录研究的不足。

<details>
  <summary>Details</summary>

**Motivation:** 传统立法行为分析过度依赖投票记录，忽视了政治话语中丰富的语义和修辞内容，需要新的方法来全面理解议会话语的多个维度。

**Method:** 提出可扩展的计算框架，结合历时文体学分析、上下文主题建模和议员演讲语义聚类，应用于2003-2025年巴西众议院超过45万篇演讲的大规模案例研究。

**Result:** 发现长期风格转向更简短直接的演讲，立法议程在应对国家危机时急剧重新定位，区域和性别身份在话语联盟中的作用往往超过正式党派归属。

**Conclusion:** 该研究为分析议会话语作为多维现象提供了稳健方法论，有效补充了基于投票的传统方法，为理解立法行为提供了新视角。
...
> GiVA是一种基于梯度的向量式适配初始化策略，能在保持极低参数效率的同时，实现与LoRA相当的训练速度，并将所需秩降低8倍。

<details>
  <summary>Details</summary>

**Motivation:** 随着模型规模持续增长，参数高效微调成为全量微调的有力替代方案。虽然LoRA被广泛采用，但基于向量的适配方法因需更高秩来匹配性能而增加训练成本。

**Method:** 提出GiVA（基于梯度的初始化策略），专门针对向量式适配方法进行优化，通过智能初始化来显著降低对秩的需求。

**Result:** 在自然语言理解、自然语言生成和图像分类等多个基准测试中，GiVA持续优于或与现有向量式适配方法和LoRA竞争，同时将秩需求减少8倍。

**Conclusion:** GiVA成功结合了向量式适配的极低参数效率和LoRA的训练速度优势，为大规模模型的高效微调提供了新的解决方案。
...
> MathDuels是一个自我博弈的数学评估框架，让模型既出题又解题，通过动态竞争来区分真实能力，避免传统基准的饱和问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有数学基准测试已接近性能上限，无法有效区分前沿语言模型的能力差异。需要新的评估方法来突破固定题目集的限制。

**Method:** 构建双角色评估框架：模型通过三阶段流程（元提示、问题生成、难度放大）出题，同时解决其他模型的题目。使用Rasch模型联合估计解题能力和出题质量。

**Result:** 在19个前沿模型的上实验表明，出题和解题能力部分分离，双角色评估发现了单角色基准中不可见的能力差异。新模型的出现使基准难度与参与者实力共同进化。

**Conclusion:** MathDuels提供了一个动态持续的评估平台，能够真实反映模型在数学推理方面的进步，并维持评估的有效性。
...
> 该论文表明，基于解码器大语言模型的语义评估方法在自动语音识别评估中优于传统的词错误率，在假设选择任务中达到92-94%的人工标注一致性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的ASR评估指标WER不敏感于语义内涵，而现有的嵌入式语义指标虽然与人类感知相关性更好，但解码器大语言模型在此任务中的应用尚未得到充分探索。

**Method:** 通过三种方法评估LLMs：1)在两个候选假设中选择最佳结果；2)使用生成式嵌入计算语义距离；3)对错误进行定性分类。在HATS数据集上进行实验验证。

**Result:** 最佳LLM在假设选择任务中达到92-94%的人工标注一致性，远超WER的63%，也优于其他语义指标。解码器LLM的嵌入性能与编码器模型相当。

**Conclusion:** 基于LLM的方法为ASR评估提供了可解释和语义化的新方向，证明了其在语音识别语义评估中的重要潜力。
...
> PlantInquiryVQA是一个用于植物病理学多步诊断的视觉问答基准数据集，包含近14万问答对，研究证明结构化提问能显著提升多模态大模型的诊断准确性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的视觉评估采用多步骤结构化提问方式，尤其在植物病理学中专家会根据症状逐步深入询问，但当前视觉语言模型仅评估单轮问答，缺乏对实际诊断过程中多步推理能力的考察。

**Method:** 提出了Chain of Inquiry框架，将诊断轨迹建模为基于视觉线索和认知意图的有序问答序列，构建了包含24,950张植物图像和138,068个问答对的数据集，并标注了视觉定位、严重程度和推理模板。

**Result:** 顶级多模态大模型能够描述视觉症状，但在安全临床推理和准确诊断方面表现不足。结构化问题引导的询问显著提高了诊断正确率，减少了幻觉现象，提升了推理效率。

**Conclusion:** PlantInquiryVQA为训练能够像专家植物学家一样进行推理的诊断智能体提供了基础平台，有助于推动从静态分类器向多步诊断推理模型的发展。
...
> 本文提出了一种基于文本提示生成线性图像的新方法，通过将线性图像表示为多曝光序列并使用DiT流匹配架构，解决了传统模型在生成高动态范围线性图像时的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的生成模型主要生成显示参考图像，这些图像经过非线性色调映射和压缩，失去了原始光线的真实信息，限制了后期处理的灵活性。而线性图像作为场景参考表示，能保留完整动态范围和位深度，为专业后期处理提供更大空间。

**Method:** 将线性图像表示为多个曝光括号的序列，每个括号捕捉动态范围的特定部分；提出基于DiT的流匹配架构进行文本条件的曝光括号生成；结合ControlNet实现结构条件的生成控制。

**Result:** 成功实现了高质量的文本到线性图像生成，能够保留完整的动态范围信息；展示了文本引导的线性图像编辑和结构条件生成等下游应用的有效性。

**Conclusion:** 本研究填补了文本到线性图像生成的空白，为专业图像后期处理提供了更灵活的基础材料，证明了通过曝光括号序列和流匹配架构生成高动态范围线性图像的可行性。
...
> 提出一种双路径自适应网络，通过空间-时间和时间-空间并行处理路径，结合实体级自适应路由机制，有效解决了微动作识别中的时空多样性挑战。

<details>
  <summary>Details</summary>

**Motivation:** 微动作是持续1-3秒的细微局部动作，在社交交流中起关键作用，但现有计算机视觉方法无法捕捉其多样化的时空特征特性。

**Method:** 设计双路径网络架构：ST路径先捕获空间配置再建模时间动态，TS路径则相反；引入实体级自适应路由让每个身体部位学习最优处理偏好，并使用MAC损失确保跨路径一致性。

**Result:** 在MA-52数据集上取得竞争性表现，在iMiGUE数据集上达到最先进结果，证明了架构适应性的重要性。

**Conclusion:** 针对微动作固有的复杂多样性进行架构适应性设计是推进细粒度视频理解的关键，该方法为微动作分析提供了新的解决思路。
...
> 本研究提出了一种无需训练的方法，使标准RGB大模型能够处理多光谱遥感图像，在推理阶段实现显著的性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有大型多模态模型主要在RGB图像上训练，难以直接应用于多光谱遥感领域，而专门训练多光谱模型成本高昂且专业化程度过高。

**Method:** 通过将非RGB输入适配到模型的视觉空间理解中，并结合领域专业知识和思维链推理作为指令，在推理阶段让标准RGB LMMs处理多光谱数据。

**Result:** 使用Gemini 2.5模型在主流遥感基准测试中展现出强大的零样本性能提升，证明了方法的有效性。

**Conclusion:** 该方法使地理空间专业人士能够利用通用大模型处理专用传感器数据，同时获得丰富的推理能力，为多光谱遥感应用提供了新的技术途径。
...
> 该研究将分类领域的投影梯度反学习(PGU)方法适配到扩散模型，通过构建核心梯度空间并投影梯度更新到其正交补空间，防止被遗忘的概念在后续微调中恢复。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本到图像扩散模型反学习方法普遍存在一个缺陷：即使下游数据完全不相关，被擦除的不良概念在模型微调后仍会重现。

**Method:** 提出将Projected Gradient Unlearning (PGU)从分类任务适配到扩散领域作为后处理强化步骤。通过从保留概念激活中构建核心梯度空间(CGS)，并将梯度更新投影到其正交补空间，确保后续微调无法撤销已实现的擦除。

**Result:** PGU应用于ESD、UCE、Receler等现有方法上，能够完全消除风格概念的恢复，并显著延迟对象概念的恢复。运行时间约6分钟，而Meta-Unlearning需要约2小时。

**Conclusion:** PGU和Meta-Unlearning是互补的，哪种方法效果更好取决于概念的编码方式，保留概念的选择应基于视觉特征相似性而非语义分组。
...
> StyleVAR基于VAR框架，将风格转换建模为条件离散序列建模，通过混合交叉注意力机制和强化学习优化，在多个指标上超越AdaIN基准。

<details>
  <summary>Details</summary>

**Motivation:** 解决现有风格转换方法在保持内容结构和风格纹理平衡方面的挑战，利用自回归建模的优势提升风格转换质量。

**Method:** 采用VQ-VAE将图像分解放大到离散码空间，使用Transformer对目标码进行自回归建模，引入混合交叉注意力机制注入风格和内容信息，通过两阶段训练：监督微调+GRPO强化学习优化。

**Result:** 在三个跨域基准测试中，StyleVAR在风格损失、内容损失、LPIPS等指标上均优于AdaIN基准，强化学习阶段进一步提升了感知指标性能，能够有效转移纹理同时保持语义结构。

**Conclusion:** 方法在风景和建筑场景表现优异，但在网络图像泛化和人脸处理上存在不足，需要更强的结构先验和内容多样性改进。
...
> 本文提出了一种专家引导的对比微调框架，通过整合对比学习到多实例学习中，有效解决了儿童脑肿瘤病理图像分类中的数据稀缺和类别不平衡问题，在现实低样本条件下显著提升了细粒度诊断准确性。

<details>
  <summary>Details</summary>

**Motivation:** 儿童脑肿瘤的准确诊断面临独特挑战：数据严重稀缺、类别不平衡以及不同诊断亚型间的细粒度形态重叠。虽然病理基础模型在切片级表征学习方面取得了进展，但在有限数据下将其有效适配到弱监督的儿童脑肿瘤分类仍有待探索。

**Method:** 提出专家引导的对比微调框架，将对比学习整合到切片级多实例学习(MIL)中，在下游微调过程中显式正则化切片级表征的几何结构。设计了通用监督对比设置和专家引导变体，后者融入临床信息来针对诊断易混淆的亚型选择困难负样本。

**Result:** 在现实低样本和类别不平衡条件下的儿童脑肿瘤WSI分类实验中，对比微调在细粒度诊断区分上产生了可测量的改进。实验分析显示不同对比策略具有互补优势，专家引导的困难负样本促进了更紧凑的类内表征和改善的类间分离。

**Conclusion:** 这项工作强调了在数据稀缺的儿科病理环境中显式塑造切片级表征对于鲁棒细粒度分类的重要性，为有限医学图像数据的深度学习应用提供了有效解决方案。
...
> 本文提出了一种仅使用单个观测值来调整扩散先验的方法，通过将多个扩散先验组合成专家乘积先验并识别最大化贝叶斯证据的指数来提高逆问题求解的可信度。

<details>
  <summary>Details</summary>

**Motivation:** 扩散先验虽然在许多逆问题中能生成高质量后验样本，但它们通常在有限的训练集或纯模拟数据上训练，继承了这些数据源的误差和偏差。现有调优方法需要大量不同前向算子的观测值，这在许多应用中难以收集，当观测集较小时容易导致过拟合。

**Method:** 提出将现有扩散先验组合成单一的专家乘积先验，通过识别最大化贝叶斯证据的指数来调整先验。这种方法通过指数权重泛化先验，使得能够从经过调整和组合的扩散模型中进行后验采样。

**Result:** 在真实世界逆问题上验证了该方法，包括黑洞成像（真实先验未知）和文本条件先验的图像去模糊。发现证据最大化往往产生超越单一训练数据集的先验，通过指数权重泛化先验，产生了更灵活的优先级别，提高了结果后验图像分布的可信度。

**Conclusion:** 该方法能够仅用单个观测值有效调整扩散先验，避免了传统方法需要大量样本和容易过拟合的问题。通过专家乘积和贝叶斯证据最大化，实现了更灵活和可信的逆问题求解框架。
...
> Foveated Reasoner是一种模仿人类视觉注意力机制的自回归视觉语言模型框架，通过在低分辨率视图基础上动态选择性地获取高分辨率信息，在视觉token预算有限的情况下实现了更高的任务准确性。

<details>
  <summary>Details</summary>

**Motivation:** 当前视觉语言模型依赖高分辨率图像获得更好性能，但视觉token数量增加导致计算开销巨大。人类通过中央凹视觉机制解决这个问题：粗略视图指导'看哪里'，选择性地获取高精度信息来精化'想什么'。

**Method:** 提出Foveated Reasoner框架，将中央凹视觉和推理统一在单一解码轨迹中。从低分辨率视图开始，仅在需要时触发性中央凹视觉，从选定区域检索高分辨率证据并将其注入同一解码轨迹。采用两阶段训练：冷启动监督学习初始化中央凹行为，强化学习联合改进证据获取和任务准确率，同时避免看所有区域的无意义解决方案。

**Result:** 实验表明该方法能有效学习中央凹视觉策略，在多个视觉语言基准测试中，在紧张的视觉token预算下实现了更强的准确性。

**Conclusion:** 通过模仿人类中央凹视觉机制，Foveated Reasoner成功解决了视觉语言模型中高分辨率信息与计算开销的矛盾，为高效的视觉语言推理提供了新思路。
...
> 本研究提出利用大语言模型和谷歌街景图像自动评估美国建筑状况的新框架，通过知识蒸馏在保持精度的同时大幅提升效率。

<details>
  <summary>Details</summary>

**Motivation:** 传统的建筑状况评估需要大量人工，成本高昂且难以在全国范围内实施。利用计算机视觉和大语言模型技术可以实现对建筑状况的大规模、自动化、低成本评估，为房主和政策制定者提供决策支持。

**Method:** 1. 基于Gemma 3 27B进行微调，使用少量人工标注数据；2. 应用知识蒸馏技术将27B模型压缩到4B模型；3. 进一步蒸馏到CNN(EfficientNetV2-M)和Transformer(SwinV2-B)模型；4. 开发可视化仪表板整合评估结果；5. 进行人机对齐研究验证模型能力。

**Result:** 微调后的Gemma 3 27B在SRCC和PLCC指标上超越人类评分者；4B模型实现3倍速度提升且性能接近；CNN和Transformer模型实现30倍速度提升；框架可评估多种建筑环境属性。

**Conclusion:** 该框架提供了灵活高效的建筑状况评估解决方案，在保持高精度的同时显著减少人工标注需求，为大规模建筑评估提供了实用技术路径。
...
> 研究发现欧洲预训练数据集在地理空间基础模型中表现最优，光谱多样性是影响下游性能的关键因素

<details>
  <summary>Details</summary>

**Motivation:** 当前地理空间基础模型的性能差异主要归因于模型架构，而预训练数据集的作用很少被研究

**Method:** 创建全球和各大洲的预训练数据集，在全球和各大洲的下游数据集上进行评估，并分析10个数据集的多样性指标

**Result:** 欧洲预训练数据集表现最优，只有光谱多样性与性能强相关，地理、生物群系和土地覆盖多样性相关性较弱

**Conclusion:** 光谱多样性是创建高性能地理空间预训练数据集时需要考虑的新维度，开源了新数据集和实验框架
...
> 提出了一种材料可控的房间脉冲响应(RIR)生成方法，通过解耦空间布局和材质对声音的影响，使用户能够独立修改场景材质配置并获得相应的声学效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的声学建模方法将空间和材质影响混合在一起表示，限制了用户控制并降低了生成声学的真实感。准确建模这些效果对虚拟现实、机器人、建筑设计和音频工程等应用至关重要。

**Method:** 采用双模块架构：1) 空间模块捕获场景空间布局的声学影响；2) 材质模块根据用户指定的材质配置调制空间RIR。这种显式解耦设计允许用户独立修改材质配置。

**Result:** 在声学指标(RTE最高提升16%)和材质指标(最高提升70%)上都显著优于之前的方法。人类感知研究证实模型相比最强基线具有更高的真实感和材质敏感性。

**Conclusion:** 该方法成功实现了空间与材质影响的显式解耦，为用户提供了灵活的材料控制能力，显著提升了声学建模的真实感，在多个应用领域具有实用价值。
...
> HyperFM是为NASA PACE任务设计的参数高效高光谱基础模型，通过创新的光谱注意力机制解决大规模高光谱数据分析的挑战，在云属性反演任务中表现优越。

<details>
  <summary>Details</summary>

**Motivation:** NASA PACE任务提供前所未有的高光谱观测数据，但现有基础模型主要针对RGB图像训练，无法有效处理连续光谱信息。现有高光谱模型训练数据受限、参数量大且计算成本高，制约了实际应用。

**Method:** 提出HyperFM参数高效高光谱基础模型，采用组内和组间光谱注意力机制结合混合参数分解技术，在捕获光谱空间关系的同时显著降低计算成本。

**Result:** 在四个下游大气云属性反演基准任务上，HyperFM一致性地超越了现有高光谱基础模型和特定任务的最先进方法。同时发布了包含晴空和云场景的大规模PACE数据集HyperFM250K。

**Conclusion:** HyperFM为大规模高光谱数据分析提供了高效解决方案，推动了高光谱基础模型在气候监测和空气质量研究中的实际应用，开源数据集为相关研究提供了重要资源。
...
> WFM是一种基于小波流匹配的快速MRI合成方法，用1-2步生成四个MRI模态，速度提升250-1000倍，参数减少75%。

<details>
  <summary>Details</summary>

**Motivation:** 现有扩散模型在多模态MRI合成中计算效率低下（需数百采样步骤且每个模态需要独立模型），严重限制了临床部署。根本原因在于扩散过程从纯噪声开始，忽略了可用MRI序列中已有的结构信息。

**Method:** 提出WFM（小波流匹配），直接学习从信息先验（小波空间中条件模态的均值）到目标分布的流映射。由于源和目标共享底层解剖结构且主要在对比度上存在差异，该公式能够在仅需1-2个积分步骤内实现准确合成。使用单个82M参数的模型配合类别条件合成四个BraTS模态。

**Result:** 在BraTS 2024数据集上，WFM实现26.8 dB PSNR和0.94 SSIM，比扩散基线仅低1-2 dB，但运行速度快250-1000倍（每个体积0.16-0.64秒 vs 160秒）。用单个模型替代了总参数326M的四个独立扩散模型。

**Conclusion:** WFM的速度-质量权衡使实时MRI合成在临床工作流中变得实用，显著提升了计算效率和部署可行性。
...
> 针对点-视觉-语言模型中的几何幻觉问题，本文提出了几何奖励信贷分配机制和重投影一致性约束，通过将序列级奖励精确分配给对应的几何token，实现了从 plausible textual outputs 向 physically verifiable spatial predictions 的转变。

<details>
  <summary>Details</summary>

**Motivation:** 点-视觉-语言模型在执行空间推理时经常出现几何幻觉，即预测的3D结构与观察到的2D现实相矛盾。研究发现根本原因在于强化学习中的结构错位，稀疏的几何token被嘈杂的序列级奖励淹没。

**Method:** 提出了两个核心机制：1）几何奖励信贷分配框架，将整体监督分解为特定领域信号并专门路由到负责的token范围；2）重投影一致性项作为跨模态验证器，惩罚物理上不可能的几何结构。

**Result:** 在ShapeNetCore校准基准上的实验表明，3D KPA从0.64提升到0.93，3D边界框IoU达到0.686，重投影一致性得分达到0.852，同时在保持稳健2D定位性能的前提下显著改善了3D几何预测的可靠性。

**Conclusion:** 该方法成功解决了几何幻觉问题，通过将通用策略优化转化为目标结构对齐，实现了从看似合理的文本输出向物理可验证空间预测的重要突破。
...
> WildSplatter是一种前馈式3D高斯泼溅模型，能够在未知相机参数和变化光照条件下处理不受约束的图像，实现秒级3D重建和外观控制。

<details>
  <summary>Details</summary>

**Motivation:** 传统3D高斯泼溅方法需要迭代优化、已知相机参数和一致光照条件，限制了其在真实世界多样化场景中的应用。

**Method:** 提出一个联合学习3D高斯和外观嵌入的网络架构，通过输入图像条件化调制高斯颜色，处理显著的光照和外观变化。

**Result:** 该方法能在1秒内从稀疏视图重建3D高斯，实现多样化的光照控制，在具有变化光照的真实世界数据集上优于现有无姿态3DGS方法。

**Conclusion:** WildSplatter成功地将3D高斯泼溅扩展到了不受约束的真实场景，为快速、鲁棒的3D重建提供了新解决方案。
...
> 本研究提出了SpatiO框架，通过在测试时动态协调多个具有不同归纳偏好的异构视觉语言专家代理，显著提升了空间推理任务的性能。

<details>
  <summary>Details</summary>

**Motivation:** 空间推理需要整合多种归纳偏好（如2D外观、深度信号、几何约束），但现有方法采用单一推理管道学习固定的空间先验，缺乏适应性。同质多智能体系统也无法充分利用多样化的归纳偏好。

**Method:** SpatiO是一个异构多智能体框架，协调多个具有互补归纳偏好的视觉语言专家。创新性地提出测试时编排(TTO)机制，在推理过程中动态评估和重新加权各代理的可靠性，无需修改模型参数。

**Result:** 在多个空间推理基准测试（3DSRBench、STVQA-7k、CV-Bench、Omni3D-Bench）上，SpatiO一致性地超越了闭源和开源基线方法，证明了其有效性。

**Conclusion:** 通过异构多智能体协同和测试时动态优化，SpatiO成功实现了空间适应性，为复杂视觉场景理解提供了新的解决方案。
...
> 本文提出了一种基于伪模拟退火的数据增强方法，通过合成逼真的密集鱼类场景，显著提升了水下环境中目标检测模型的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的目标检测模型在受控环境下表现良好，但在多变、频繁遮挡的真实水下场景中性能大幅下降，需要解决模型在密集和不受约束的水下环境中的鲁棒性问题。

**Method:** 首先使用DeepFish数据集从分割掩码生成边界框标注构建检测数据集，然后提出受Deng等人copy-paste策略启发的伪模拟退火增强算法，合成逼真的密集鱼类场景，在训练中增加空间多样性和目标密度。

**Result:** 实验结果表明，该方法显著优于基线YOLOv10模型，特别是在佛罗里达群岛实时流媒体图像手工标注的挑战性测试集上表现突出。

**Conclusion:** 该增强策略有效提升了密集真实水下环境中的目标检测性能，证明了其在提高模型鲁棒性方面的实用价值。
...
> Sparse Forcing是一种用于自回归视频扩散模型的训练推理范式，通过利用KV缓存中的隐含时空记忆稀疏性，提高长时视频生成质量并降低延迟

<details>
  <summary>Details</summary>

**Motivation:** 观察到自回归扩散rollout中注意力会集中在持续的显著视觉块子集上，在滑动窗口内呈现局部结构化块稀疏模式，这提供了优化计算的机会

**Method:** 提出可训练的原生稀疏机制，学习压缩、保留和更新持续块，同时将每个局部窗口内的计算限制在动态选择的局部邻域；设计Persistent Block-Sparse Attention (PBSA)高效GPU内核加速稀疏注意力

**Result:** 在5秒文本到视频生成任务上VBench分数提升+0.26，解码速度提升1.11-1.17倍，KV缓存峰值占用降低42%；在更长时间生成上效果更显著

**Conclusion:** Sparse Forcing通过利用注意力稀疏模式，实现了高质量长时视频生成与低延迟推理的双重优化，为自回归视频扩散模型提供了有效的训练推理新范式
...
> UAU-Net是一个不确定性感知的面部动作单元检测框架，通过在特征表示和决策两个阶段同时建模不确定性来提高检测的鲁棒性和可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 现有AU检测方法通常将AU表示视为确定性特征，忽略了视觉噪声、个体外观差异和AU间模糊关系等不确定性因素，同时传统点估计分类器在严重标签不平衡情况下会产生过度自信的预测，这些问题都严重影响了检测的鲁棒性。

**Method:** 提出两层不确定性建模：1) 特征表示阶段的CV-AFE模块，基于条件变分自编码器学习概率化的AU表示，联合估计多时空尺度的特征均值和方差；2) 决策阶段的AB-ENN网络，使用Beta分布参数化预测不确定性，并通过非对称损失缓解标签不平衡问题。

**Result:** 在BP4D和DISFA数据集上的广泛实验表明，UAU-Net取得了强大的AU检测性能，进一步分析证明了在表示学习和证据性预测中建模不确定性能够提高系统的鲁棒性和可靠性。

**Conclusion:** 同时建模学习和推理过程中的不确定性是提高AU检测系统鲁棒性的有效途径，该框架为处理异构、AU特定的不确定性提供了新的解决方案。
...
> LatRef-Diff是一种基于扩散模型的人脸属性编辑框架，通过风格代码替代传统语义方向，实现了精确且高质量的人脸风格操纵。

<details>
  <summary>Details</summary>

**Motivation:** 传统条件GAN存在精度不足和训练不稳定的问题，而扩散模型在风格操纵方面受限于语义方向的表达能力有限，无法实现精确的人脸属性控制而不影响其他特征。

**Method:** 提出风格编码替代语义方向，包含潜空间引导和参考引导两种生成方法；设计风格调制模块整合风格代码，采用可学习向量、交叉注意力机制和层次化设计；提出前向-后向一致性训练策略，避免配对图像需求。

**Result:** 在CelebA-HQ数据集上的定性和定量评估均达到最先进性能，消融实验验证了各设计选择的有效性。

**Conclusion:** LatRef-Diff成功解决了现有方法在精度和训练稳定性方面的限制，为虚拟头像和照片编辑等应用提供了有效的技术方案。
...
> ImageHD是一种基于超维度计算的FPGA加速器，专为资源受限设备上的流式持续学习设计，通过硬件感知算法和数据流架构实现高效实时边缘AI。

<details>
  <summary>Details</summary>

**Motivation:** 现有持续学习方法依赖反向传播或大量样本，在边缘设备上面临计算、内存和延迟开销；而基于HDC的方案通常需要复杂的多层内存层次和集群管理，限制了在资源受限硬件上的部署。

**Method:** 在算法层面，提出硬件感知的持续学习方法，通过统一样本内存和高效的集群合并策略限制样本数量，并采用量化CNN前端；在系统层面，在AMD Zynq ZCU104 FPGA上实现流式数据流架构，使用字打包二进制超向量进行大规模并行位计算。

**Result:** 在CORe50数据集上，ImageHD相比优化后的CPU基准实现40.4倍加速和383倍能效提升，相比GPU基准实现4.84倍加速和105.1倍能效提升。

**Conclusion:** ImageHD证明了基于HDC的持续学习在实时边缘AI应用中的实用性，为资源受限设备上的自适应视觉系统提供了高效解决方案。
...
> AttDiff-GAN是一种混合框架，结合了GAN的属性操控能力和扩散模型的高质量图像生成能力，通过特征级对抗学习和先验增强机制实现了更精准的人脸属性编辑。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于GAN的方法在风格码与属性语义对齐方面较弱，而基于扩散模型的方法由于不同属性语义方向的纠缠导致编辑精度有限，需要一种结合两者优势的新方法。

**Method:** 提出AttDiff-GAN混合框架，通过特征级对抗学习分离属性编辑与图像合成，引入PriorMappe融入面部先验，使用RefineExtractor通过Transformer捕获全局语义关系，去除对语义方向编辑的依赖。

**Result:** 在CelebA-HQ数据集上的实验表明，该方法在定性和定量评估中都优于现有最先进方法，实现了更准确的属性编辑和更好的非目标属性保持。

**Conclusion:** AttDiff-GAN成功解决了GAN与扩散模型集成的优化难题，通过特征级解耦学习和先验增强机制，为人脸属性编辑提供了一个更精确和可控的解决方案。
...
> GraphLeap 是一种针对视知觉图神经网络（ViGs）的优化方法，通过解耦图构建和特征更新，实现了高效的计算流水线，并提供了首个端到端 FPGA 加速器。

<details>
  <summary>Details</summary>

**Motivation:** ViGs 需要每层动态构建图，这消耗了 50-95% 的计算时间，时间复杂度为 O(N²)，且在图构建和特征更新间产生顺序依赖，成为主要瓶颈。

**Method:** GraphLeap 使用前一层的特征进行当前层的特征更新，同时用当前层特征为下一层构建图，实现了图构建与消息传递的并发执行。基于此构建了流式、层流水线的 FPGA 加速器设计。

**Result:** 在 Alveo U280 FPGA 上，GraphLeap 相比 CPU 获得了最高 95.7 倍加速，相比 GPU 获得 8.5 倍加速，证明了实时 ViG 推理的可行性。

**Conclusion:** 通过简单的算法重构和专用的硬件加速，GraphLeap 有效解决了 ViGs 的计算瓶颈问题，为实时视觉图神经网络应用提供了可行的解决方案。
...
> 本研究首次系统性探讨了合成数据在可控人像视频生成中的作用，提出了一个基于扩散的框架，揭示了合成数据与真实数据的互补效应，并为构建数据高效且泛化能力强的生成模型提供了实践指导。

<details>
  <summary>Details</summary>

**Motivation:** 可控人像视频生成技术面临大规模、多样化且隐私安全的人类视频数据集稀缺问题，特别是对于稀有身份和复杂动作。合成数据作为可扩展且可控的替代方案，其真实贡献由于Sim2Real差距而未被充分探索。

**Method:** 提出了一个基于扩散模型的框架，能够实现外观和动作的细粒度控制，并提供统一测试平台分析合成数据在训练过程中与真实数据的交互作用。通过广泛实验验证不同数据组合的效果。

**Result:** 研究揭示了合成数据与真实数据的互补作用，证明了有效选择合成样本可以增强动作真实感、时间连贯性和身份保持能力。实验验证了框架在多种控制任务上的有效性。

**Conclusion:** 这是首个全面探索合成数据在以人为中心的视频合成中作用的研究，为构建数据高效且具有良好泛化能力的生成模型提供了实用的见解和方法，有助于推动数字人、动画和具身AI等应用领域的发展。
...
> 该论文提出了一个基于Vision Transformer的深度学习框架，用于自动化四分类脑肿瘤诊断，在7023张MRI扫描上达到99.29%的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 脑肿瘤是最严重的神经系统疾病之一，早期准确诊断直接关系到患者生存率。传统MRI扫描的人工解释耗时、易受观察者差异影响且需要专业医生经验。

**Method:** 使用ImageNet-21k预训练的ViT-B/16作为主干网络，配合CLAHE对比度增强、两阶段微调策略、MixUp和CutMix数据增强、权重指数移动平均EMA和测试时增强TTA，并通过Attention Rollout提供可解释性热图。

**Result:** 模型达到99.29%的测试准确率和99.25%的宏F1分数，对健康组织和脑膜瘤类别实现了完美召回率，显著超越所有CNN基线方法。

**Conclusion:** 该框架在脑肿瘤自动分类任务中表现优异，结合了先进的Transformer架构和多种训练技术，为临床诊断提供了可靠且可解释的AI辅助工具。
...
> NTIRE 2026遥感红外图像超分辨率挑战赛总结了竞赛设计、数据集、评估方案和主要结果，13个团队提交了有效解决方案，为红外图像超分辨率研究建立了基准。

<details>
  <summary>Details</summary>

**Motivation:** 针对遥感场景中红外图像的超分辨率需求，推动该领域技术发展，促进实际遥感应用中有效解决方案的开发。

**Method:** 采用单轨设置，通过双三次下采样生成4倍缩放因子的低分辨率图像，要求参赛者开发有效模型从低分辨率输入恢复高分辨率红外图像。

**Result:** 共有115名参与者注册，13个团队提交了有效解决方案，挑战报告总结了各团队的代表性方法，为红外图像超分辨率研究建立了重要基准。

**Conclusion:** 该挑战赛成功推动了遥感红外图像超分辨率研究的发展，为实际遥感应用提供了有效的解决方案基准，促进了该领域的技术进步。
...
> PLAS-Net是一种基于实例分割的框架，能精确提取海岸垃圾的像素级物理足迹，相比传统的边界框检测方法，显著提高了面积量化精度，为海岸监测提供更有价值的信息。

<details>
  <summary>Details</summary>

**Motivation:** 传统的无人机海岸垃圾监测主要依赖边界框检测，这种方法会系统性地高估不规则垃圾物体的平面面积，而准确的物理暴露面积对于海洋废料的生态风险评估至关重要。

**Method:** 开发了PLAS-Net（像素级垃圾面积分割器），这是一个实例分割框架，能够从无人机航拍图像中提取海岸废料的像素级精确物理足迹。在泰国Koh Tao季风驱动的口袋海滩的无人机图像上进行了评估。

**Result:** PLAS-Net实现了58.7%的mAP_50，精度高于11个基线模型，在复杂海岸条件下表现出更好的掩码保真度。三个下游应用演示了掩码精度对环境分析结论的影响：标准化塑料密度的幂律拟合、加面积权重的生态风险指数映射，以及揭示来源组成的丰富度-面积悖论。

**Conclusion:** 像素级面积提取相比单纯计数的传统方法，能够为海岸监测提供更有价值的准确信息，特别是在生态风险评估、污染热点识别和来源分析方面具有重要意义。
...
> FryNet是一个双流RGB-热成像框架，能够同时在单次前向传播中完成油炸油区域分割、可用性分类和四个化学氧化指标的回归预测。

<details>
  <summary>Details</summary>

**Motivation:** 传统的油炸油降解检测依赖破坏性的湿化学分析方法，无法提供空间信息且不适合实时使用。现有的热成像检测方法存在相机指纹捷径问题，模型记忆传感器特定噪声而非学习氧化化学。

**Method:** 提出的FryNet框架包含：1) ThermalMiT-B2主干网络提取热特征；2) RGB-MAE编码器通过掩码自编码学化学表示；3) 双编码器DANN通过梯度反转层对抗正则化；4) FiLM融合桥接热结构和RGB化学上下文。

**Result:** 在28个油炸视频的7226对帧数据上，FryNet实现了98.97% mIoU、100%分类准确率和2.32平均回归MAE，性能超越所有7个基线方法。

**Conclusion:** FryNet成功克服了热成像检测中的相机指纹捷径问题，为实时、无损的油炸油质量监测提供了有效解决方案，具有重要应用价值。
...
> HiTPro是一个无监督的视频化可见光-红外行人重识别框架，通过分层时序原型化方法实现了跨模态匹配，在无标签情况下取得了最优性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可见光-红外行人重识别方法主要基于图像级，并且依赖昂贵的身份标注。虽然视频-based方法已经出现，但仅限于监督设定，而实际应用中更需要的无监督视频VI-ReID问题研究较少，这限制了其在真实场景中的应用。

**Method:** HiTPro框架包含三个核心组件：1) 时序感知特征编码器提取帧级特征并聚合为轨迹级表示；2) 通过分层跨原型对齐进行两阶段正样本挖掘，结合动态阈值策略和软权重分配；3) 分层对比学习在三个层级上逐步优化特征-原型对齐：相机内区分、跨相机同模态一致性和跨模态不变性。

**Result:** 在HITSZ-VCM和BUPTCampus数据集上的广泛实验表明，HiTPro在完全无监督设置下实现了最先进的性能，显著优于适配的基线方法。

**Conclusion:** HiTPro首次解决了无监督视频VI-ReID问题，提供了有效的原型驱动框架，避免了硬伪标签分配，为该领域建立了强有力的基线，具有重要的实际应用价值。
...
> MiMIC提出了一种融合解码器架构和鲁棒训练方法，解决了通用多模态检索中视觉模态崩溃和语义不对齐问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的通用多模态检索方法存在严重缺陷：早期融合方法(Marvel)会出现视觉模态崩溃，过度依赖文本特征而忽视视觉信息；而后期融合方法(UniVL-DR)虽然避免了模态崩溃，但容易产生语义不对齐问题。

**Method:** MiMIC引入两个关键创新：(1)融合解码器架构实现有效的多模态整合；(2)通过单模态混合和随机标题丢弃的鲁棒训练策略增强模型对缺失信息的处理能力。

**Result:** 在WebQA+和EVQA+数据集上的实验表明，即使在文档或查询中的图像缺少标题的情况下，MiMIC的表现也一致优于早期和后期融合的基线方法。

**Conclusion:** MiMIC通过创新的多模态融合架构和训练策略，成功解决了视觉模态崩溃和语义不对齐两大挑战，为通用多模态检索提供了更有效的解决方案。
...
> TGR-MoE是一种通过预训练稠密教师模型提供路由指导的方法，用于解决稀疏MoE训练中的路由不稳定问题，提升了模型精度和训练稳定性。

<details>
  <summary>Details</summary>

**Motivation:** 稀疏MoE虽然能够有效降低大规模模型的计算成本，但在训练过程中存在路由梯度阻塞和专家分配不稳定的问题，导致路由学习困难。

**Method:** TGR-MoE利用预训练稠密教师模型的中间表示构建教师路由器，将其输出作为学生路由器的伪监督信号，通过知识指导稳定路由学习过程。

**Result:** 在ImageNet-1K和CIFAR-100上的实验表明，TGR-MoE显著提升了模型精度和路由一致性，即使在高度稀疏配置下也能保持稳定训练。

**Conclusion:** 该方法简单有效，通过引入教师指导机制成功解决了稀疏MoE的路由优化难题，为大规模视觉模型的稀疏化提供了可靠解决方案。
...
> 本文提出了一种基于潜在去噪的视觉监督框架，通过修复受损的视觉标记来改善LMMs的内部视觉表示，提升模型在分布偏移下的鲁棒性，且不增加推理开销。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LMMs（如LLaVA）通过自回归语言建模目标进行训练，仅提供对视觉标记的间接监督，导致内部视觉表示较弱，在分布偏移时表现脆弱。

**Method:** 提出潜在去噪框架：使用显著性感知的混合掩码和高斯噪声破坏投影的视觉标记；训练LMM通过解码器从选定的中间LLM层的隐藏状态恢复干净的教师补丁特征；保持教师的图像内相似性结构，应用图像内对比补丁蒸馏防止表示崩溃。

**Result:** 在标准多模态基准测试上持续改进视觉理解和推理能力；在组合鲁棒性基准（如NaturalBench）上获得显著增益；在面对ImageNet-C风格的非对抗性常见损坏时，保持更高准确率，在中度和严重损坏水平下减少性能下降。

**Conclusion:** 基于潜在去噪的原则为改善LMMs的内部视觉特征对齐和多模态理解提供了有效的视觉监督形式，无需额外推理开销即可提升模型鲁棒性。
...
> 本文提出了一种增强自监督学习(SSL)在航空图像上对模糊退化鲁棒性的方法，通过引入基于信任权重的加性残差训练策略，在多项基准测试中取得最佳性能。

<details>
  <summary>Details</summary>

**Motivation:** 航空图像经常受到雾、运动模糊、雨和遮挡等退化因素的影响，这些退化会移除关键证据。现有的SSL方法通过增强视图之间的不变性强制对齐，但当视图严重退化时会引入伪结构，损害表征学习效果。

**Method:** 提出了一个基于样本和因子的信任权重机制，结合基础对比损失作为加性残差。关键创新是采用带停止梯度的加性残差替代乘性门控机制，并开发了基于Dempster-Shafer融合的证据变体来提供可解释信号。

**Result:** 在21万张图像数据集上，6个骨干网络在三个数据集上的平均线性探测准确率达90.20%(SimCLR为88.46%，VICReg为89.82%)。在EuroSAT严重退化条件下比SimCLR提升19.9个百分点，跨域零样本测试中AUROC提升1-3个百分点。

**Conclusion:** 加性残差公式是性能提升的主要来源，为不确定性感知的SSL提供了具体的设计原则。该方法在航空图像的各种退化条件下的表征学习中显示出显著优势。
...
> SparseGF是一个用于激光雷达数据地面过滤的高性能框架，通过上下文压缩、稀疏体素-点网络架构和高度感知损失函数解决了跨场景泛化问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的深度学习地面过滤方法在跨场景泛化方面存在两个关键问题：一是由于计算资源限制导致大规模处理中的上下文-细节权衡困境，二是仅基于分类优化导致高物体随机误分类。

**Method:** 提出了SparseGF框架，包含三个核心创新：(1)凸镜启发的上下文压缩模块，能够将广阔上下文压缩为紧凑表示同时保留中心细节；(2)混合稀疏体素-点网络架构，有效解释压缩表示并减轻压缩引起的几何失真；(3)高度感知损失函数，在训练中显式强制地形高程先验以抑制高物体随机误分类。

**Result:** 在两个大规模ALS基准数据集上的广泛评估表明，SparseGF在城市到自然地形上都能实现鲁棒的地面过滤，在复杂城市场景中取得领先性能，在混合地形上获得竞争性结果，在密集森林陡峭地区达到中等但非灾难性的精度。

**Conclusion:** 这项工作为基于深度学习的地面过滤研究提供了新的见解，鼓励进一步探索实现真正跨场景泛化的大规模环境监测解决方案。
...
> PTA提出了一种基于原型的测试时适应方法，通过自适应知识原型整合测试样本信息，在保持极高推理效率的同时显著提升VLM性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于缓存的测试时适应方法存在两个关键限制：1）缓存增长导致推理延迟增加；2）缓存样本不足或不正确时性能下降。这些问题限制了TTA在大规模场景下的应用效果。

**Method:** PTA使用类别特定的知识原型来累积测试样本的知识，根据每个测试样本的零样本类别置信度自适应加权，并将样本视觉特征整合到相应的类别原型中。所有历史知识都通过原型整合利用，避免了缓存填充和检索的开销。

**Result:** 在15个图像识别基准和4个鲁棒点云分析基准上达到SOTA性能，CLIP在10个跨域基准上准确率从65.64%提升至69.38%，同时在大规模ImageNet-1K上保持92%的推理速度，远超基于缓存的TDA方法（67.97%准确率，50%推理速度）。

**Conclusion:** PTA通过原型设计成功解决了传统缓存TTA方法的效率和性能瓶颈，为大规模视觉语言模型的测试时适应提供了一个高效且有效的解决方案。
...
> 该论文提出了一种知识驱动的创意视频生成方法KD-CVG，通过构建广告创意知识库和引入语义感知检索与多模态知识参考两个模块，解决了文本到视频模型在语义对齐和运动适应性方面的挑战。

<details>
  <summary>Details</summary>

**Motivation:** 当前创意生成研究主要集中在文本和图像上，而创意视频生成(CVG)相对缺乏探索，主要面临两大挑战：语义对齐模糊和运动适应性不足，导致生成视频无法准确匹配产品卖点并出现不真实运动。

**Method:** 构建了广告创意知识库(ACKB)，提出知识驱动的KD-CVG方法。该方法包含语义感知检索(SAR)模块，利用图注意力网络和强化学习反馈增强卖点与创意视频的关联理解；以及多模odal知识参考(MKR)模块，将语义和运动先验知识融入T2V模型以弥补现有知识缺口。

**Result:** 实验表明KD-CVG在语义对齐和运动适应性方面表现优异，相比其他最先进方法验证了其有效性，相关代码和数据集将开源发布。

**Conclusion:** 提出的知识驱动方法成功解决了创意视频生成中的关键挑战，为广告创意视频的自动生成提供了有效的解决方案，推动了该领域的发展。
...
> EdgeFormer是一个基于学习的3D点云边缘检测网络，通过将整个点云的边缘检测转换为基于局部块点的分类任务，有效提取细粒度边缘特征。

<details>
  <summary>Details</summary>

**Motivation:** 3D点云中的边缘特征在工业和商业视觉应用中具有重要意义，但细粒度边缘特征由于密集分布或小尺度表面梯度变化而难以被有效检测。

**Method:** EdgeFormer采用两阶段架构：第一阶段构建描述每个点局部邻域的局部块特征描述子；第二阶段通过分析第一阶段生成的局部块特征描述子对每个点进行分类。基于空间邻近点具有高相关性并构成局部底层表面的观察，将边缘检测问题转化为点分类任务。

**Result:** 实验结果表明，EdgeFormer模型与六个基线方法相比展现出竞争性性能，能够有效提取更精细的细节。

**Conclusion:** 通过将点云转换为局部块进行处理，EdgeFormer成功解决了细粒度边缘特征检测的挑战，为3D点云边缘检测提供了一种有效的解决方案。
...
> 本文提出VG-CoT数据集，通过三阶段自动化流程将多步推理与图像区域显式对齐，并建立新评估框架来衡量LVLMs的可信度推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有大视觉语言模型缺乏精确的局部区域推理能力，现有数据集因人工标注成本高、缺乏推理步骤与图像区域显式对齐而限制了模型可信度评估。

**Method:** 提出三阶段自动化流水线：1）使用先进检测和OCR模型提取物体和文本级视觉证据；2）用GPT-4o生成分步 grounding 推理；3）通过基于原理的开集检测过程精化 grounding 对齐。

**Result:** 实验表明VG-CoT在LLaVA-1.5和Qwen2-VL等代表模型上显著提升多数评估指标，证明了数据集在增强可信度推理方面的有效性，同时保持可扩展和高效的数据集构建。

**Conclusion:** VG-CoT为LVLMs提供了可扩展的、视觉证据grounded的推理训练和评估资源，有效促进模型的可信度推理能力，为该领域研究贡献了重要基础设施。
...
> YOGO是一个解决3D高斯泼墨技术产业化瓶颈的系统级框架，通过确定性资源控制和多传感器融合协议，配合新推出的超密集数据集Immersion v1.0，实现了生产级的高质量重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有3D高斯泼墨技术存在'产学研鸿沟'：启发式高斯增长导致资源消耗不可预测、现有基准的'稀疏性屏蔽'鼓励了幻觉而非物理保真度、多传感器数据污染严重，阻碍了实际生产应用。

**Method:** 提出YOGO系统框架，将随机增长过程重构为确定性、预算感知的均衡状态；集成预算控制器进行硬件约束资源分配；引入可用性注册协议实现鲁棒的多传感器融合；发布超密集室内数据集Immersion v1.0打破稀疏性屏蔽。

**Result:** YOGO在保持严格确定性profile的同时实现了最先进的视觉质量，建立了生产级3DGS的新标准；Immersion v1.0通过饱和视点覆盖迫使算法关注极致物理保真度而非视点插值。

**Conclusion:** YOGO成功弥合了3D高斯泼墨技术的产学研鸿沟，为商业化部署提供了可靠解决方案；Immersion v1.0数据集推动了高保真重建上限的探索，二者共同推动了3DGS技术的产业化进程。
...
> S1-VL是一个支持科学推理和图像思维两种范式的科学领域多模态推理模型，通过代码生成与执行实现图像操作的主动控制，在13个基准测试中达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 针对科学领域中高分辨率图表解读、显微镜图像理解和几何辅助推理等挑战性场景，现有模型缺乏主动操作图像的能力，且现存数据集包含大量冗余、无效和错误的视觉操作。

**Method:** 提出双范式推理框架：科学推理（结构化思维链）和图像思维（通过Python代码执行主动操作图像），采用六维质量过滤框架和多阶段过滤管道，通过四阶段渐进式训练流程（科学多模态SFT、图像思维冷启动SFT和两阶段SAPO强化学习）训练模型。

**Result:** S1-VL-32B在5个图像思维基准（HRBench-4K、HRBench-8K、MME-RealWorld-CN、MME-RealWorld-Lite和V*）上达到SOTA，并在Physics和VRSBench等科学推理基准上超越对比系统。

**Conclusion:** 通过主动图像操作和双范式推理的有机结合，S1-VL显著提升了科学领域的多模态推理能力，为解决需要精确视觉操作的复杂科学问题提供了有效方案。
...
> 本文提出一种新的非线性扩散滤波器族，通过改进的扩散函数和半隐式格式，高效地将图像处理成分片常数形式，适用于图像分割预处理。

<details>
  <summary>Details</summary>

**Motivation:** 传统图像分割前需要图像预处理，现有方法在保持边缘清晰的同时难以实现图像内部的灰度均匀化。本文旨在设计一种能够生成闭合轮廓、内部灰度均匀且边缘不模糊的分片常数图像的预处理方法。

**Method:** 首先建立非线性扩散方程的内蕴表述，提供滤波器设计条件；基于此提出与后向扩散相关的新扩散函数族；采用半隐式格式求解前向非线性扩散方程；证明滤波器满足适定性和尺度空间要求；通过扩散时间停止准则控制处理效果。

**Result:** 所提出的滤波器能够在计算开销较低的情况下获得分片常数图像，理论证明了其数学性质，并在真实图像上验证了有效性。代码已开源提供。

**Conclusion:** 新的非线性扩散滤波器族成功解决了图像分割预处理中对分片常数图像的需求，在保持边缘的同时实现内部均匀化，具有良好的理论基础和实际应用价值。
...
> UHR-DETR是一个高效的端到端Transformer检测器，专为超高分辩率遥感图像设计，通过稀疏编码和全局-局部解码器解决了内存瓶颈与小目标检测的矛盾。

<details>
  <summary>Details</summary>

**Motivation:** 超高分辩率遥感图像中小目标检测面临两难困境：保持原始分辨率导致内存瓶颈，而传统方法如降采样或裁剪会丢失小目标或破坏上下文信息。

**Method:** 提出 Coverage-Maximizing Sparse Encoder 动态分配计算资源到高分辨率区域，设计 Global-Local Decoupled Decoder 整合宏观场景感知和微观目标细节。

**Result:** 在UHR数据集上实现2.8% mAP提升，推理速度提升10倍，在24GB硬件约束下表现优异。

**Conclusion:** UHR-DETR有效解决了UHR图像中内存消耗与检测精度的矛盾，为实际应用提供了可行的解决方案。
...
> 本文提出了一种名为2L-LSH的两层局部敏感哈希算法，用于大规模3D点云模型中的快速邻域点搜索，在速度上显著优于Kd-tree和Octree方法。

<details>
  <summary>Details</summary>

**Motivation:** 随着3D扫描技术的发展，大规模点云模型的处理面临巨大挑战。邻域点搜索是模型重建、分类、检索和特征可视化等应用中的基础问题，需要更高效准确的解决方案。

**Method:** 2L-LSH算法采用两步哈希函数策略：第一步对点云模型的包围盒进行划分，第二步构建基于广义表的数据结构，利用哈希函数在高维数据搜索中的高速和准确特性。

**Result:** 实验结果表明，所提出的2L-LSH方法在kNN搜索时间上比Kd-tree和Octree分别降低51.111%和94.159%，在RN搜索时间上分别降低54.519%和41.840%。

**Conclusion:** 2L-LSH为大规模3D点云模型的快速邻域点搜索提供了高效准确的解决方案，在速度性能上显著优于传统方法，是点云处理领域的有前景技术。
...
> VARestorer是一个通过知识蒸馏将预训练文本到图像视觉自回归模型转换为一步图像超分辨率模型的框架，解决了传统VAR在超分辨率任务中的全局上下文利用不足和误差累积问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉自回归模型(VAR)在图像生成中表现良好，但在真实图像超分辨率(Real-ISR)任务中面临两个关键问题：一是因果注意力机制的约束导致无法充分利用全局低质量图像上下文，二是迭代预测过程中的误差累积严重影响输出一致性。

**Method:** 提出VARestorer框架，通过分布匹配将预训练VAR模型蒸馏为一步ISR模型，消除迭代 refine 减少 error propagation；引入金字塔图像条件和跨尺度注意力机制，实现双向尺度交互；仅微调1.2%参数的高效适配器保持原始模型表达能力。

**Result:** 在DIV2K数据集上达到72.32 MUSIQ和0.7669 CLIPIQA的SOTA性能，相比传统VAR推理加速10倍，同时显著提升了输出质量和一致性。

**Conclusion:** VARestorer成功地将预训练自回归模型适配到图像超分辨率任务，通过一步预测架构和高效参数微调，在保持模型表达能力的同时大幅提升了推理效率和任务性能，为视觉自回归模型在图像恢复任务中的应用提供了新的技术路径。
...
> OA-VAT提出了结合实例感知原型初始化、在线原型增强跟踪器和遮挡感知轨迹规划的统一框架，解决了视觉主动跟踪中的干扰物混淆和遮挡问题，在多个基准上达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 视觉主动跟踪在实际部署中面临两个关键瓶颈：由于实例级判别力不足导致的视觉相似干扰物混淆，以及缺乏主动规划导致的遮挡严重失败。

**Method:** 提出OA-VAT三模块统一框架：1）基于DINOv3的免训练实例感知离线原型初始化；2）在线原型增强跟踪器集成置信度感知卡尔曼滤波器；3）基于条件扩散的遮挡感知轨迹规划器，并在自定义Planning-20k数据集上训练。

**Result:** 在UnrealCV上平均SR达0.93（比SOTA TrackVLA提升2.2%），真实数据集上平均CAR达90.8%（比SOTA GC-VAT提升12.1%），DJI Tello无人机上TSR达81.6%，RTX 3090上实时运行35 FPS。

**Conclusion:** OA-VAT通过三个互补模块有效解决了干扰物混淆和遮挡问题，在保持实时性的同时显著提升了视觉主动跟踪的鲁棒性，为实际部署提供了可靠解决方案。
...
> 本研究提出了EgoPoint-Bench基准测试，用于评估和提升多模态大模型在自我中心视角下的指物推理能力，解决了当前系统存在的'指代幻觉'问题。

<details>
  <summary>Details</summary>

**Motivation:** 自我中心AI代理（如智能眼镜）依赖指物手势来消解自然语言命令中的指代歧义，但现有MLLM系统无法精确理解指物的空间语义，而是依赖视觉邻近性或物体显著性等虚假相关性。

**Method:** 构建了EgoPoint-Bench问答基准，包含超过11,000个高质量模拟和真实样本，涵盖五个评估维度和三个指代复杂度层级，通过合成数据微调模型来提升性能。

**Result:** 实验表明，最先进的专有和开源模型在自我中心指物方面表现不佳，但在合成数据上微调的模型取得了显著的性能提升并实现了强健的从模拟到现实的泛化能力。

**Conclusion:** 该工作强调了空间感知监督的重要性，为精确的自我中心AI助手提供了可扩展的发展路径。
...
> ID-Eraser是一种基于特征空间的人脸主动防御技术，通过在身份嵌入中注入可学习扰动来阻止恶意换脸，既保持图像视觉效果又能有效保护身份隐私。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于像素级扰动的主动防御方法对现代换脸模型效果不佳，因为这些模型能够提取鲁棒的高级身份嵌入。人脸换脸技术的快速发展对隐私和数字安全构成严重威胁，需要更有效的防护机制。

**Method:** ID-Eraser在特征空间进行主动防御，通过向身份嵌入中注入可学习扰动去除可识别的面部信息，然后通过Face Revive Generator (FRG)重构自然外观的保护图像，使图像对人类视觉保持真实，同时让换脸模型无法使用受保护的身份。

**Result:** 实验表明ID-Eraser在严格黑盒设置下显著扰乱各类人脸识别和换脸系统的身份识别，取得Top-1准确率最低（0.30）、FID最佳（1.64）和LPIPS最佳（0.020）。与干净输入生成的换脸相比，受保护的换脸身份相似度在五个代表性换脸模型上平均降至0.504。在腾讯API上，相似度从0.76降至0.36。

**Conclusion:** ID-Eraser证明了其在跨数据集泛化、抗常见扭曲鲁棒性和在商业API实际应用方面的强大能力，为防范恶意人脸换脸提供了有效且实用的解决方案。
...
> 该论文提出了一种新的跨数据集评估指标Cross-AUC和相应的检测框架SFAM，用于解决人脸伪造检测中的泛化能力问题。

<details>
  <summary>Details</summary>

**Motivation:** 随着生成模型的快速发展，视觉数据伪造检测在社会和经济安全中日益重要。现有的人脸伪造检测器由于缺乏合适的评估指标，在跨数据集泛化能力方面表现不佳，传统的跨数据集AUC指标无法揭示检测分数在不同数据域间的显著偏移问题。

**Method:** 提出Cross-AUC评估指标，通过将一个数据集的真实样本与另一个数据集的伪造样本进行对比来计算AUC（反之亦然），从而显式评估跨领域分数的可比性。同时提出SFAM框架，包含两个模块：1) 补丁级图像-文本对齐模块，增强CLIP对篡改伪迹的敏感性；2) 面部区域混合专家模块，将不同面部区域的特征路由到专门的专家进行区域感知的伪造分析。

**Result:** 实验表明，使用Cross-AUC评估现有检测器发现性能显著下降，暴露了一个被忽视的鲁棒性问题。在多个公共数据集上的定性和定量实验证明，所提方法在各种合适的评估指标下都比现有最先进方法表现更优。

**Conclusion:** 论文通过提出Cross-AUC指标揭示了现有人脸伪造检测器的跨域鲁棒性问题，并设计了SFAM框架有效提升了检测性能，该研究为视觉数据伪造检测领域提供了新的评估视角和解决方案。
...
> 本研究提出了一个评估框架，利用冻结的大型语言模型作为推理引擎，通过交通编码器和轻量级CNN分别提取动态交通代理和静态道路网络的特征，转换后用于轨迹预测，实现对LLM在自动驾驶中理解能力的定量分析。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在自动驾驶领域展现出强大的推理能力，但在感知和预测方面的安全应用仍需LLM深入理解动态交通代理和静态道路基础设施。现有方法缺乏对LLM此类理解能力的系统性评估。

**Method:** 框架采用冻结的LLM作为推理核心，使用交通编码器提取代理轨迹的空间级特征，轻量级CNN编码局部高精地图，通过重编程适配器将特征转换为LLM兼容token，最终用简单线性解码器输出未来轨迹。

**Result:** 该框架能够定量分析多模态信息（特别是地图语义）对轨迹预测精度的影响，实现了冻结LLM的最小化适配集成，展示了在不同LLM架构间的强泛化能力，为模型评估提供了统一平台。

**Conclusion:** 研究成功建立了评估LLM理解交通动态和道路网络拓扑能力的标准化框架，证明了LLM在自动驾驶轨迹预测任务中的潜力，为后续LLM在自动驾驶领域的安全应用奠定了方法论基础。
...
> 本文提出了VFM$^{4}$SDG框架，通过引入视觉基础模型作为跨域稳定性先验，解决了单域泛化目标检测中的领域漂移问题。

<details>
  <summary>Details</summary>

**Motivation:** 实际应用中天气、光照等条件的变化导致严重的领域漂移，使在单一源域训练的检测器在未知环境中性能大幅下降。现有方法主要关注数据增强或域不变表征学习，对检测器机制本身关注不足。

**Method:** 提出双先验学习框架VFM$^{4}$SDG：1）在编码阶段使用跨域稳定关系先验蒸馏增强目标-背景和实例间关系建模的鲁棒性；2）在解码阶段利用基于语义-上下文先验的查询增强，将类别级语义原型和全局视觉上下文注入查询中。

**Result:** 在标准SDGOD基准测试和两个主流DETR检测器上显著超越现有SOTA方法，证明了方法的有效性、鲁棒性和泛化性。

**Conclusion:** 通过将视觉基础模型的跨域稳定性先验引入检测器的表征学习和查询建模，有效解决了复杂领域漂移下的检测性能退化问题，为单域泛化目标检测提供了新的解决思路。
...
> 提出一种基于高斯混合模型的局部描述子GMD，用于激光扫描碎片断裂面的匹配与自动拼接。

<details>
  <summary>Details</summary>

**Motivation:** 激光扫描重建物体的自动拼接中，断裂面匹配是至关重要的一步。现有方法在描述和匹配断裂面方面存在局限性，需要更有效的描述方法。

**Method:** 使用高斯混合模型拟合点分布，将局部表面划分为凹凸区域估计GMM的k值，通过合并区域GMD形成最终描述子，采用L2距离测量相似性，结合RANSAC和ICP进行碎片对齐。

**Result:** 在真实扫描公开数据集和兵马俑数据集上的广泛实验证明了方法的有效性，与现有方法的比较验证了所提方法的优势。

**Conclusion:** 基于高斯混合模型的描述子GMD能够有效描述和匹配碎片断裂面，为物体自动拼接提供了新的解决方案。
...
> 该研究系统评估了视觉语言模型（VLMs）作为评估器的可靠性，发现现有VLM评估器存在显著盲区，无法可靠检测图像描述和图像生成任务中的多种质量退化错误。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型视觉语言模型被广泛用于评估其他模型的输出性能，包括视觉问答等图像到文本任务和文本到图像生成任务，但这类评估器VLMs本身的可靠性尚未得到充分研究和验证。

**Method:** 研究团队设计了针对性的扰动来破坏输出质量，涵盖物体幻觉、空间推理、事实基础和视觉保真度等关键错误维度。构建了包含4000多个扰动实例的综合测试集，涵盖40种扰动维度，对4个主流VLM采用单答案评分、成对比较和参考引导三种评估范式进行测试。

**Result:** 实验结果显示，当前VLM评估器存在严重缺陷：往往无法检测到被扰动的输出（在某些情况下失败率超过50%），特别难以处理细粒度的组合和空间错误，对与输入图像矛盾的幻觉内容不敏感。成对比较方式相对更可靠，但失败率仍然存在。

**Conclusion:** 研究结果揭示了当前评估器VLMs的不可靠性，强调在将其用于基准测试和开发决策时需要谨慎。为研究者提供了公开的代码和数据资源以促进该领域的进一步研究。
...
> 本研究提出了一个基于注意力的多示例学习框架，通过利用病理基础模型作为特征提取器，在整张病理切片级别预测肺腺癌主要生长模式，显著减少了标注负担，并获得了良好的预测效果。

<details>
  <summary>Details</summary>

**Motivation:** 肺腺癌的准确分级依赖于识别生长模式，这对预后判断和治疗决策至关重要。传统深度学习方法需要大量标注，本研究旨在解决标注负担过重的问题，实现从整张切片级别预测主要生长模式。

**Method:** 采用基于注意力的多示例学习(ABMIL)框架，集成预训练病理基础模型作为切片编码器，可选择冻结或微调方式，通过注意力机制聚合特征进行预测。对比实验评估了不同编码器在ABMIL下的表现。

**Result:** 实验表明微调编码器能提升性能，其中Prov-GigaPath在ABMIL下取得最高一致性(κ=0.699)。与简单的切片聚合基线相比，ABMIL通过利用切片级别监督和空间注意力获得了更稳健的预测结果。

**Conclusion:** 该框架有效降低了标注负担，在整张切片级别准确预测肺腺癌主要生长模式方面展现出优越性能，为未来扩展到生长模式全分布估计和多中心验证奠定了基础。
...
> CoOD是一个无需训练的基于组件的OOD检测框架，通过分解输入为功能组件，利用组件偏移评分(CSS)和组合一致性评分(CCS)检测局部外观偏移和跨组件组合不一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有OOD检测方法存在检测粒度问题：全局表示会抑制局部OOD线索，而基于补丁的方法因伪相关和噪声而不稳定，且两者都无法有效检测由有效ID组件组成的组合OOD。

**Method:** 受组件识别理论启发，提出CoOD框架，将输入分解为功能组件，推导出CSS检测局部外观变化，CCS识别跨组件组合不一致性，实现无需训练的OOD检测。

**Result:** CoOD在粗粒度和细粒度OOD检测任务上都取得了持续改进的性能提升。

**Conclusion:** 基于组件的分解方法有效解决了现有OOD检测方法的粒度限制问题，为检测组合性OOD提供了新的解决思路。
...
> 该论文提出了一种基于深度核空间的无监督动作分割方法，通过最小化最大均值差异(MMD)来近似视频的帧分布，使用神经正切核(NTK)提升描述能力，在六个标准基准上取得了有竞争力的结果。

<details>
  <summary>Details</summary>

**Motivation:** 针对无法或不容许存储大型数据集的应用场景，研究按视频无监督动作分割方法。传统方法存在优化困难、计算效率低等问题，需要更可靠且高效的解决方案。

**Method:** 在深度核空间中通过最小化最大均值差异(MMD)来近似原始视频分布。采用神经正切核(NTK)定义核空间，相比固定核具有更强的描述能力，同时避免了联合学习时的平凡解问题。

**Result:** 在六个标准基准上与当前最先进的按视频方法取得具有竞争力的结果。当段数未知时，该方法比之前的凝聚方法获得更高的F1分数。

**Conclusion:** 提出的基于深度核空间和MMD的无监督动作分割方法，在保证几何特性的同时提高了优化效率和计算速度，为无监督动作分割任务提供了有效的解决方案。
...
> CHRep是一个两阶段框架，用于从H&E组织学图像稳健预测空间基因表达，解决了现有模型在跨幻灯片测试中的性能下降和过度平滑问题。

<details>
  <summary>Details</summary>

**Motivation:** 空间转录组学技术成本高、通量低，限制了大规模研究和临床应用。从常规H&E切片预测基因表达是可行的替代方案，但现有模型在跨幻灯片评估中存在外观偏移和过度平滑问题，抑制了生物学上有意义的变异。

**Method:** CHRep采用两阶段框架：训练阶段通过联合优化相关感知回归、对称图像-表达对齐和坐标诱导的空间拓扑正则化来学习结构感知表示；推理阶段通过轻量校准模块提升跨幻灯片鲁棒性，结合非参数估计和幅度正则化校正模块，无需骨干网络微调。

**Result:** 在三个队列的留一幻灯片评估中，CHRep持续改善基因相关性。相比HAGE，在cSCC和HER2+队列上PCC(ACG)分别提升4.0%和9.8%；相比mclSTExp，在Alex+10x队列上PCC(ACG)提升39.5%，MSE和MAE分别降低9.7%和9.0%。

**Conclusion:** CHRep通过耦合拓扑保持表示学习和事后校准，实现了稳定的邻域检索和可控偏差校正，为从组织学图像预测空间基因表达提供了稳健有效的解决方案。
...
> OmniFit是一种缩放无关的多模态人体模型拟合方法，能处理完整扫描、部分深度观测和图像捕捉，在基准测试中达到毫米级精度

<details>
  <summary>Details</summary>

**Motivation:** 现有的人体模型拟合方法通常需要已知的度量尺度，这对AI生成的资产（经常存在尺度失真）来说不切实际，且大多只处理单模态输入如点云或多视角图像

**Method:** 提出基于条件transformer解码器的创新方法，将表面点直接映射到密集身体标志点，用于SMPL-X参数拟合；集成可选的图像适配器补偿几何信息缺失；引入专用尺度预测器将主体重新缩放到标准身体比例

**Result:** 在宽松和日常服装场景下性能超越SOTA方法57.1-80.9%，成为首个超越多视角优化基线的方法，在CAPE和4D-DRESS基准上达到毫米级精度

**Conclusion:** OmniFit成功解决了多模态输入人体拟合的尺度无关性问题，为真实和合成资产提供了统一高效的解决方案，在精度和泛化性方面取得突破性进展
...
> Sculpt4D是一个将高效时间建模集成到预训练3D扩散Transformer中的原生4D生成框架，通过设计块稀疏注意力机制实现了时间一致性高且计算效率高的4D内容生成。

<details>
  <summary>Details</summary>

**Motivation:** 现有3D生成模型在静态形状合成方面取得显著进展，但高保真动态4D生成仍受时间失真和巨大计算需求的限制，主要瓶颈包括4D训练数据稀缺和计算复杂性。

**Method:** 将块稀疏注意力机制集成到预训练的Hunyuan3D 2.1模型中，该机制通过锚定初始帧保持物体身份，使用时间衰减稀疏掩码捕捉丰富的运动动态，在保证空间时间依赖性的同时避免全注意力的二次计算开销。

**Result:** 成功实现了时间一致性高的4D合成，网络总计算量减少56%，在4D生成领域建立了新的最先进水平。

**Conclusion:** Sculpt4D为高效可扩展的4D生成开辟了新路径，通过创新的时间建模策略解决了4D数据稀缺和计算效率问题，展示了4D生成技术的实用前景。
...
> 本文提出了一种参数化投影稳定性评估框架，通过高斯扰动检测投影区域的局部不稳定性，并验证其能发现传统指标无法识别的不稳定区域。

<details>
  <summary>Details</summary>

**Motivation:** 参数化投影技术虽然能够实时嵌入新数据点，但测量噪声或数据漂移会导致二维布局产生不可预测的偏移，而投影的局部稳定性问题此前缺乏系统研究。

**Method:** 构建稳定性评估框架，在选定锚点周围施加高斯扰动，结合平均位移、偏差、最近锚点分配误差等定量指标，以及位移向量、局部PCA椭球体、Voronoi错配等可视化方法进行全面评估。

**Result:** 在MNIST和Fashion-MNIST数据集上测试了基于UMAP和t-SNE的神经投影器，证实该框架能识别出重建误差或邻域保持指标无法发现的投影不稳定区域，并研究了Jacobian正则化对鲁棒性的影响。

**Conclusion:** 提出的稳定性评估框架有效揭示了参数化投影的局部不稳定性，为投影算法的鲁棒性改进提供了重要的分析和验证工具。
...
> DCMorph是一个基于双流扩散模型的人脸混合攻击框架，通过身份条件化和潜空间双层操作实现高质量的人脸混合，能够有效绕过现有的人脸识别系统和混合攻击检测方法。

<details>
  <summary>Details</summary>

**Motivation:** 人脸混合攻击技术需要不断发展以应对不断演变的威胁，并为身份验证系统开发强大的防御机制。现有方法存在混合伪影或重建保真度有限等问题。

**Method:** DCMorph采用双流扩散框架，通过两个关键机制：(1)解耦交叉注意力插值，将源人脸的身份特征注入去噪过程；(2)DDIM反演与球面插值，提供几何一致的初始潜表示。

**Result:** 在四个最先进的人脸识别系统上的漏洞分析表明，DCMorph在运行阈值下比现有方法具有更高的攻击成功率，同时难以被当前的混合攻击检测方案发现。

**Conclusion:** DCMorph通过双重身份条件化和几何一致的潜空间表示，克服了现有方法的技术限制，为人脸混合攻击提供了更有效的新方法，同时也凸显了开发更强防御措施的紧迫性。
...
> DualSplat是一种基于失败先验的3D高斯分布方法，通过将初始重建失败转换为显式先验，解决了包含瞬时物体场景的多视图一致性问题，实现了更鲁棒的3D场景重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3D高斯分布方法在训练图像包含瞬时物体时性能会显著下降，因为准确检测瞬时物体需要良好的静态场景重建，而清洁重建又依赖于可靠的瞬时掩码，形成循环依赖问题。

**Method:** 提出DualSplat框架：1)保守初始训练产生不完整碎片作为失败表征；2)结合光度残差、特征失配和SAM2实例边界构建对象级伪掩码；3)使用伪掩码指导清洁的第二轮3DGS优化；4)轻量 MLP在线优化掩码，逐步从先验监督转向自一致性。

**Result:** 在RobustNeRF和NeRF On-the-go数据集上的实验表明，DualSplat优于现有基线方法，在瞬时物体密集场景和区域中展现出明显优势。

**Conclusion:** DualSplat成功解决了瞬时物体检测与场景重建之间的循环依赖问题，通过将训练失败转化为有用的先验信息，为包含瞬时物体的复杂场景提供了更鲁棒的3D重建解决方案。
...
> 本文提出了一种基于因果推理和解耦表示学习的新型全参考图像质量评估范式，通过将降质估计建模为因果解纠缠过程，在多个场景下实现了优异的性能和跨域泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于深度网络的全参考图像质量评估(FR-IQA)模型通常通过比较参考图像和失真图像的深层特征来工作，这种方法存在局限性。作者希望从新的角度解决这个问题，提出一种更具理论基础和泛化能力的方法。

**Method:** 1) 利用参考图像和失真图像之间的内容不变性解耦降质和内容表示；2) 受人类视觉掩蔽效应启发，设计掩蔽模块建模图像内容和降质特征之间的因果关系，从失真图像中提取受内容影响的降质特征；3) 使用监督回归或无标签降维从降质特征预测质量分数。

**Result:** 在完全监督、少标签和无标签设置下，该方法在标准IQA基准测试上取得了极具竞争力的性能。在水下、放射学、医学、中子和屏幕内容图像等多个非标准自然图像领域也表现优异，展现了出色的跨域泛化能力。

**Conclusion:** 该方法能够在没有标记IQA数据的情况下进行特定场景的训练和预测，相比现有的免训练FR-IQA模型具有更优越的跨域泛化能力，为图像质量评估提供了一个新的有效范式。
...
> 这项研究提出了CHAI框架，通过结构化规范和人类-AI监督实现精确视频字幕，并在专业级视频理解和生成方面取得了突破。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频语言模型缺乏精确描述视频动态(如主体、场景、运动、空间和摄影机动态)的能力，而专业视频内容的高质量字幕稀缺且标注成本高昂。

**Method:** 1) 定义结构化规范，包含数百个专业视频创作者定义的视觉基元；2) 提出CHAI监督框架，让专家批判和修订模型生成的预字幕为改进的后字幕；3) 利用批判数据通过SFT、DPO和推理时扩展改进开源模型；4) 应用于重新标注大规模专业视频并微调视频生成模型。

**Result:** 通过适度专家监督，模型超越了Gemini-3.1-Pro等闭源模型；微调后的Wan模型能够理解400词详细提示，实现对摄影机运动、角度、镜头等电影拍摄的精细控制。

**Conclusion:** 精确的结构化规范和人类-AI监督是实现专业级视频理解和生成的关键技术，该方法为视频字幕标注效率和模型性能提升提供了有效解决方案。
...
> 本文提出结构化运动描述(SMD)方法，将关节位置序列转换为结构化自然语言描述，使LLM能直接进行运动理解，在问答和描述任务上超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于LLM的运动理解方法受限于跨模态表示和对齐，不能充分利用LLM的世界知识和推理能力，需要专门的运动编码器。

**Method:** 受生物力学启发，提出规则确定性的SMD方法，将关节位置序列转换为关节角度、身体部位运动和全局轨迹的结构化自然语言描述，让LLM直接处理文本形式的运动。

**Result:** 在运动问答(BABEL-QA 66.7%, HuMMan-QA 90.1%)和运动描述(HumanML3D上R@1 0.584, CIDEr 53.16)任务上超越所有先前方法，支持8个不同的LLM。

**Conclusion:** SMD方法证明了将运动表示为结构化语言的有效性，不仅性能优异，还具有跨模型通用性和可解释性，为运动理解提供了新范式。
...
> HalluScope是一个用于分析LVLM幻觉成因的基准测试，研究发现幻觉主要由文本先验过度依赖引起，并提出HalluVL-DPO框架通过偏好优化来减少幻觉。

<details>
  <summary>Details</summary>

**Motivation:** 大型视觉-语言模型存在幻觉问题，但现有研究对导致幻觉的主要因素不明确，需要系统性的基准来分析各种因素的相对重要性。

**Method:** 构建HalluScope基准分析幻觉成因，发现文本指令先验是主要原因。针对此问题，提出HalluVL-DPO框架，使用精心构建的数据集进行偏好优化微调，引导模型选择基于视觉的回答。

**Result:** 优化后的模型有效减少了由文本指令先验引起的幻觉，同时保持或提升了其他幻觉基准测试和视觉能力评估的性能。

**Conclusion:** 通过系统分析和针对性解决方案，证明了文本先验过度依赖是LVLM幻觉的主要成因，提出的HalluVL-DPO框架能有效缓解该问题，为构建更可靠的多模态系统提供了思路。
...
> Sapiens2是一个面向人类视觉任务的高分辨率Transformer模型家族，通过改进预训练策略、扩大数据集和优化架构，在姿态估计、身体部位分割等多项任务上实现SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 为了解决第一代Sapiens模型在细节捕捉和语义理解方面的不足，同时应对人类视觉任务对高分辨率输出和强泛化能力的挑战。

**Method:** 1) 结合掩码图像重建和自蒸馏对比目标的统一预训练范式；2) 使用10亿张高质量人类图像的精选数据集；3) 集成前沿模型架构改进，支持1K原生分辨率和4K层级变体；4) 4K模型采用窗口注意力机制

**Result:** 在姿态估计(+4 mAP)、身体部位分割(+24.3 mIoU)、法向估计(45.6%角度误差降低)等任务上显著超越前代模型，并成功扩展到点图和反照率估计等新任务

**Conclusion:** 通过统一的多目标预训练、大规模高质量数据和先进架构设计的协同作用，Sapiens2在人类视觉任务上实现了突破性的性能提升，展现了强大的泛化能力和通用性。
...
> WorldMark是第一个为交互式图像到视频世界模型提供统一评测基准的系统，通过标准化场景、动作序列和控制接口，实现了不同模型间的公平比较。

<details>
  <summary>Details</summary>

**Motivation:** 现有的交互式视频生成模型如Genie、YUME等都在各自私有基准上评测，缺乏统一的评测标准导致无法进行跨模型的公平比较。虽然现有公共基准提供了轨迹误差、美学评分等指标，但缺少标准化的测试条件来处理不同模型的异构输入。

**Method:** WorldMark提供了三个关键组件：(1)统一的动作映射层，将共享的WASD风格动作词汇转换为每个模型的本地控制格式；(2)包含500个评测案例的分层测试套件，涵盖第一/第三人称视角、照片级和风格化场景，以及简单到困难三个难度等级；(3)模块化评估工具包，包含视觉质量、控制对齐和世界一致性三大维度。

**Result:** 该基准支持对六个主要模型在相同场景和轨迹上进行苹果对苹果的比较。同时推出了在线平台World Model Arena，允许用户进行模型对战并查看实时排行榜。

**Conclusion:** WorldMark为交互式视频世界模型的评测提供了首个统一的标准化平台，显著促进了该领域的公平比较和研究发展。所有数据、评估代码和模型输出都将公开发布，推动未来研究。
...
> 提出了一种基于可微分逻辑门网络的视频复制检测框架，能够生成极小的二进制描述符，实现超快速推理（>11k样本/秒），同时保持竞争性准确性。

<details>
  <summary>Details</summary>

**Motivation:** 传统深度神经网络在视频复制检测中存在计算成本高、特征维度大的问题，限制了在高吞吐量系统中的实际部署。需要一种既高效又准确的可扩展解决方案。

**Method:** 采用逻辑门网络替代传统浮点特征提取器，结合帧压缩、二进制预处理和可训练的LGN嵌入模型。模型学习逻辑操作和内部连接，训练后可离散化为纯布尔电路。

**Result:** LGN模型在多个数据集上达到或超越先前模型的准确性和排序性能，生成的描述符比传统方法小几个数量级，推理速度超过11k样本/秒。

**Conclusion:** 基于逻辑的模型为可扩展和资源高效的视频复制检测提供了有前景的替代方案，在保持竞争力的同时显著提升了部署效率。
...
> 本文提出一个受大脑启发的协同框架，结合视觉Transformer的判别能力和条件扩散模型的生成能力，以解决单目图像中3D人体网格恢复的遮挡问题。

<details>
  <summary>Details</summary>

**Motivation:** 从单目RGB图像恢复3D人体网格在遮挡环境下具有挑战性。回归方法效率高但结果不准确，扩散方法有强生成先验但可能对罕见姿态保真度不够，需要结合两者优势。

**Method:** 设计双路径框架：ViT路径提取可见区域的确定性视觉线索，扩散路径生成结构连贯的人体表示；通过多样一致特征学习模块对齐特征，利用交叉注意力多级融合机制实现双向交互。

**Result:** 在标准基准测试中取得关键指标上的优越性能，在复杂真实场景中展现出强大的鲁棒性，有效解决了遮挡问题。

**Conclusion:** 成功结合判别性和生成性方法的优点，为3D人体网格恢复提供了一个有效解决方案，在保持高保真度的同时增强了对遮挡和罕见姿态的处理能力。
...
> CARVE是一个前馈式视觉几何估计模型，通过系统性研究发现数据质量、损失函数设计等关键因素，引入一致性约束和高分辨率架构，在多个基准测试中实现了强大且鲁棒的性能。

<details>
  <summary>Details</summary>

**Motivation:** 尽管前馈式视觉几何估计取得进展，但多帧模型在跨帧一致性上表现更好，却在单帧精度上不如逐帧方法，这种性能差距需要系统性研究来找出关键影响因素。

**Method:** 通过严格的消融研究发现关键洞察：数据多样性和质量扩展、损失函数机制优化、联合监督策略；引入一致性损失函数约束深度图、相机参数和点图对齐，设计高效的高分辨率架构。

**Result:** 在点云重建、视频深度估计和相机姿态/内参估计等任务上，CARVE在多样化基准测试中展现出强大且鲁棒的性能表现。

**Conclusion:** 研究揭示了影响视觉几何估计性能的关键因素，提出的CARVE模型有效融合了优化方法优势和高分辨率输入，为前馈式几何估计提供了新的解决方案。
...
> Ramen是一种通过主动样本选择实现的鲁棒测试时自适应框架，它通过检索相关样本和缓存机制有效解决了混合域设置下的模型自适应问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的测试时自适应方法假设测试样本来自单一一致域，但实际应用中测试数据往往包含来自不同域的样本，导致传统方法在混合域设置下性能显著下降。

**Method:** Ramen框架为每个测试样本基于域一致性和预测平衡两个标准检索定制化的相关样本批次，通过嵌入-梯度缓存存储历史样本的嵌入和梯度，无需额外前向或反向传播即可进行模型更新。

**Result:** 在多个图像损坏和域偏移基准测试中，Ramen实现了强大且一致的性能，能够在复杂的混合域场景中提供鲁棒且高效的自适应。

**Conclusion:** Ramen通过主动样本选择和缓存机制为混合域测试时自适应提供了有效解决方案，理论分析和实验结果证明了其在实际应用中的优越性。
...
> 研究发现深度伪造视频在面部运动的高阶时间不规则性上存在可测量的行为指纹，特别是在表达情绪时最为明显，计算模型与人类感知在检测策略上具有互补性。

<details>
  <summary>Details</summary>

**Motivation:** 深度伪造检测研究主要依赖深度学习方法，这些方法虽然性能良好但缺乏可解释性，无法阐明真实与 manipulated 面部行为的根本区别。

**Method:** 基于面部动态的生物行为特征，识别低维面部运动模式，从中提取时空结构的时序特征，使用传统机器学习分类器进行训练和评估。

**Result:** 基于生物行为特征的分类器实现了显著的超越随机水平的检测效果，含有情绪表达的视频检测准确率更高；计算模型与人类判断在情绪视频上趋同，在非情绪视频上存在分歧。

**Conclusion:** 人脸交换深度伪造具有可测量的行为指纹，在情绪表达时最为突出；可解释的计算特征与人类感知在检测路径上相互补充而非冗余，为深度伪造检测提供了新的可解释性维度。
...
> 本文提出了DOCO框架，用于解决开放集持续测试时适应（OCTTA）问题，通过动态样本分离和域补偿提示，协同实现域适应和OOD检测。

<details>
  <summary>Details</summary>

**Motivation:** 现有的测试时适应（TTA）方法在现实场景中表现不足，无法同时处理持续变化的域和未知语义类别的出现，导致特征空间塌陷，严重影响分类和OOD检测性能。

**Method:** DOCO框架首先进行动态的、适应条件下的样本分离，将可能的ID样本与OOD样本分开；然后仅使用ID样本，通过学习域补偿提示并与源域特征统计对齐，防止语义失真；最后将该提示传播到同一批次的OOD样本中，实现更可靠的检测。

**Result:** 在多个具有挑战性的基准测试上，DOCO显著优于现有的CTTA和OSTTA方法，为OCTTA设定了新的最先进性能。

**Conclusion:** DOCO作为一个轻量级且有效的框架，通过闭环协同机制成功解决了域偏移和语义偏移的耦合问题，为现实场景中的测试时适应提供了新的解决方案。
...
> 本文提出一种自监督框架，通过从单目视频中生成伪多视角训练三元组，解决了动态视频精确相机控制缺乏配对多视角数据的瓶颈问题。

<details>
  <summary>Details</summary>

**Motivation:** 对于非刚性场景的动态视频重构，严重缺乏配对的多视角数据，这成为精确相机控制的主要瓶颈。现有方法受限于数据稀缺性，无法有效处理复杂动态场景。

**Method:** 开发了高度可扩展的自监督框架，从单个输入视频中提取不同的平滑随机游走裁剪轨迹作为源视图和目标视图，通过前向扭曲源视图第一帧生成几何锚点。模型被强制通过时空路由和重新投影来学习4D时空结构，以重建目标视图。

**Result:** 推理时，该扩散模型变换器利用4D点云锚点，在复杂动态场景下实现了最先进的时序一致性、鲁棒的相机控制和高保真度新视角合成。

**Conclusion:** 通过创新的自监督训练策略和伪多视角数据生成方法，成功突破了动态场景多视角数据的限制，为动态视频的精确相机控制和新视角合成提供了有效解决方案。
...
> 本研究评估了计算机视觉方法在社交媒体气候传播分析中的应用，发现Gemini-3.1-flash-lite模型表现最佳，并提出了基于分布的评估方法，即使单图准确率适中也能有效分析大规模话语趋势。

<details>
  <summary>Details</summary>

**Motivation:** 社交媒体已成为气候传播的主要平台，产生海量图像和帖子。通过系统分析这些内容可以揭示哪些传播策略能够激发公众关注，哪些失效，但需要有效的计算方法来实现大规模分析。

**Method:** 研究者设计了基于应用的分类体系，选择了6个可提示的视觉语言模型和15个零样本CLIP类模型进行基准测试。使用两个X(前Twitter)数据集：一个包含1038张专家标注图像，另一个包含超过120万张图像(其中5万张标签手动验证)，涵盖动物内容、气候变化后果、气候行动、图像设置和图像类型五个标注维度。

**Result:** Gemini-3.1-flash-lite在所有超类别和两个数据集上表现最佳，但与中等规模开源模型的差距相对较小。思维链推理反而降低了性能，而针对标注维度的特定提示设计能提高性能。分布评估显示，即使单图准确率适中，VLM预测也能可靠恢复总体趋势。

**Conclusion:** 计算机视觉方法是社交媒体气候传播话语大规模分析的可行起点。研究强调了分布评估的重要性，并提供了代码和数据集资源，为该领域研究奠定了基础。
...
> SyMTRS是一个大规模合成遥感数据集，提供高分辨率RGB航空图像、完美深度图、夜景图像和多尺度低分辨率变体，用于支持深度估计、域适应和超分辨率等多任务统一研究。

<details>
  <summary>Details</summary>

**Motivation:** 当前遥感深度学习研究受限于高质量标注数据集的获取成本高昂，特别是在深度估计、域适应和超分辨率等任务中缺乏精确的几何标注、受控光照变化和多尺度配对图像。

**Method:** 使用高保真城市仿真管道生成SyMTRS数据集，提供2048×2048分辨率的RGB航空图像、像素级完美深度图、域适应用的夜景对应图像以及x2、x4、x8尺度的对齐低分辨率变体。

**Result:** 创建了首个支持多任务统一研究的遥感基准数据集，填补了几何理解、跨域鲁棒性和分辨率增强研究的空白，提供了完美几何真值和一致多域监督的受控实验环境。

**Conclusion:** SyMTRS通过合成数据方法有效解决了遥感研究中真实数据标注困难的问题，为深度估计、域适应和超分辨率等关键任务提供了高质量的统一基准，推动遥感深度学习研究发展。
...
> 该论文提出了面向多修改文本的组合图像检索方法TEMA，构建了M-FashionIQ和M-CIRR两个新数据集，解决了现有方法在实体覆盖不足和子句-实体不对齐方面的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的组合图像检索方法使用的修改文本过于简单，仅覆盖有限范围的显著变化，导致在实际应用中出现两个关键问题：实体覆盖不足(Insufficient Entity Coverage)和子句-实体不对齐(Clause-Entity Misalignment)，阻碍了CIR在真实场景中的应用。

**Method:** 1) 构建了两个包含丰富指令的多修改数据集M-FashionIQ和M-CIRR；2) 提出了TEMA(Text-oriented Entity Mapping Architecture)架构，这是首个专为多修改场景设计的CIR框架，同时也能适用于简单修改场景。

**Result:** 在四个基准数据集上的广泛实验表明，TEMA在原始和多修改场景下都表现优异，同时保持了检索精度和计算效率之间的最佳平衡。

**Conclusion:** 该研究通过构建更贴近实际应用的数据集和提出创新的TEMA框架，有效解决了现有CIR方法的局限性，推动了组合图像检索技术在真实场景中的应用发展。
...
> 本文证明通过使用不同尺度（不同像素尺寸）的低分辨率图像组合，可以解决超分辨率重建中的平移模糊问题，并且当像素尺寸两两互质时，系统具有稳定逆变换。

<details>
  <summary>Details</summary>

**Motivation:** 超分辨率问题在平移变换下存在模糊性，即不同的高分辨率图像可能产生相同的低分辨率观测，这使得重建问题不适定。作者旨在通过多尺度信息来解决这一根本性问题。

**Method:** 提出使用不同像素尺寸（通过不同传感器或光学放大实现）的多幅低分辨率图像。重点分析当像素尺寸两两互质时的数学性质，并开发了基于频域技术和迭代最小二乘算法的高效重建方法。

**Result:** 理论证明互质像素尺寸系统具有稳定逆变换，推导出大规模信号在独立同分布噪声下的最小二乘重建误差表达式，揭示了噪声-分辨率权衡关系。通过1D和2D CCD硬件分箱实验验证了理论结果。

**Conclusion:** 多尺度超分辨率方法能有效解决平移模糊问题，互质像素尺寸设计提供了稳定的数学保证。该方法对实际成像系统具有重要应用价值，在噪声和分辨率之间提供了可量化的权衡指导。
...
> 本文提出了诊断驱动的胶囊内镜视频摘要新任务，构建了首个临床报告标注数据集VideoCAP，并设计了模仿临床医师工作流程的DiCE框架，实现了从数万帧视频中提取关键证据并做出准确诊断。

<details>
  <summary>Details</summary>

**Motivation:** 现有胶囊内镜研究主要局限于帧级分类和检测，视频级分析严重不足。临床场景中，诊断相关事件极其稀疏，被大量正常帧淹没，且存在运动模糊、残渣、反光等干扰因素，亟需能够提取关键证据并支持准确诊断的视频摘要方法。

**Method:** 提出VideoCAP数据集（240个完整视频，基于真实临床报告标注）和DiCE框架：1）高效候选筛选；2）上下文编织器组织候选帧为诊断上下文；3）证据聚合器整合多帧证据形成片段级判断。

**Result:** DiCE在VideoCAP上显著优于现有方法，能生成简洁且临床可靠的诊断摘要。实验证明诊断驱动的上下文推理是处理超长胶囊内镜视频摘要的有效范式。

**Conclusion:** 诊断驱动的视频摘要为胶囊内镜视频分析提供了新思路，DiCE框架通过模仿临床医师工作流程，有效解决了稀疏诊断事件提取问题，为临床应用奠定了基础。
...
> 该论文提出了一个基于物理的视频理解基准测试，扩展了V-STaR评估框架，涵盖多种视频源、物理领域和提示样式，发现在物理问答基准中需要报告物理基础、提示感知和扰动感知的诊断信息。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频理解模型可能仅通过文本规律回答物理问题，但无法在时间和空间上准确定位事件，因此需要构建一个更全面的物理视频理解评估基准。

**Method:** 构建了一个包含4个视频源、6个物理领域、3种提示类型和4种输入条件的基准测试。将每个视频剪辑转换为共享的基础事件记录，并据此生成三种查询类型。时间与空间目标是跨提示类型共享的。

**Result:** 物理领域整体表现最强，vstar_like是最清晰的非物理语义比较，neutral_rstr作为更难的模板化控制。提示类型鲁棒性是选择性的而非普适的，扰动增益集中在原始表现较弱的情况下，空间基础能力在所有设置中都是最弱的。

**Conclusion:** 视频问答推理基准应当 alongside 聚合准确率同时报告基于物理的、提示感知的、扰动感知的诊断信息，以提供更全面的模型评估。
...
> 本文提出了一种方法来恢复相机图像在ISP处理前的非幻觉版本，通过优化图像特定的MLP解码器和模态编码器，仅用180KB存储空间即可实现。

<details>
  <summary>Details</summary>

**Motivation:** 随着深度学习模块集成到相机ISP中，相机直接输出的图像可能包含幻觉内容（如AI数字变焦、低光增强等），但用户无法察觉，可能导致图像内容误解。

**Method:** 采用图像特定的多层感知机（MLP）解码器结合模态特定编码器，通过优化使系统能够从相机图像中恢复添加幻觉内容前的原始图像，无需访问相机ISP。

**Result:** 编码器和MLP解码器仅需180KB存储空间，可以作为元数据嵌入JPEG和HEIC等标准图像格式中，实现后期处理。

**Conclusion:** 该方法为用户提供了一种实用途径来获取相机图像的非幻觉版本，避免了因ISP处理造成的潜在内容误解，同时具有存储效率高和易于集成的优势。
...
> UniGenDet是一个统一框架，将图像生成和生成图像检测两个任务有机结合，通过共生机制实现协同进化，在两个任务上都达到了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 图像生成和生成图像检测两个领域虽然都取得了显著进展，但发展轨迹相对独立，形成了不同的架构范式：前者主要依赖生成网络，后者倾向于判别框架。虽然两个领域都在使用对抗信息来提升性能，但显著的架构差异阻碍了它们之间的协同作用。

**Method:** 提出UniGenDet统一框架，设计了共生多模态自注意力机制和统一的微调算法来桥接任务差异。引入检测器导向的生成对齐机制促进信息交换，实现生成任务提升真实性识别的可解释性，同时真实性标准指导生成更高保真度的图像。

**Result:** 在多个数据集上的大量实验证明该方法达到了最先进的性能，证明了生成和检测任务协同进化的有效性。

**Conclusion:** 首次成功统一了图像生成和生成图像检测两个看似对立的任务，通过协同进化机制实现了双赢，为计算机视觉领域的发展提供了新的思路和框架。
...
> 研究发现，与准确性相比，人类和视觉模型的混淆方向更能揭示其内在归纳偏差的区别，人类展现出广泛但较弱的非对称性，而深度视觉模型表现出稀疏但更强的方向性崩溃。

<details>
  <summary>Details</summary>

**Motivation:** 尽管人类和现代视觉模型能达到相似的分类准确率，但它们会犯系统性的不同错误，这些方向性混淆揭示了仅从准确性无法观察到的不同归纳偏差。

**Method:** 使用人类和深度视觉模型在12种扰动类型下的自然图像分类任务的匹配响应，通过率失真框架量化混淆矩阵的非对称性，并提取三个几何特征（斜率β、曲率κ）和效率（AUC）。

**Result:** 人类表现出广泛但较弱的非对称性，深度视觉模型显示稀疏但更强的方向性崩溃；鲁棒性训练虽然降低了全局非对称性，但未能恢复人类般的层级相似性广度-强度特征。

**Conclusion:** 方向性混淆和率失真几何为分布偏移下的归纳偏差提供了紧凑、可解释的签名信号，为理解人类和AI的差异提供了新的理论框架。
...
> Vista4D是一个基于4D点云的视频重摄框架，能够实现从不同相机轨迹和视角重新合成动态场景

<details>
  <summary>Details</summary>

**Motivation:** 现有视频重摄方法在处理真实世界动态视频时存在深度估计伪影问题，难以保持内容外观，且对于具有挑战性的新轨迹无法实现精确的相机控制

**Method:** 构建基于4D点云的表示，通过静态像素分割和4D重建来显式保持可见内容并提供丰富的相机信号，并使用重建的多视点动态数据进行训练以提高鲁棒性

**Result:** 在多种视频和相机路径下，相比最先进的基线方法，在4D一致性、相机控制和视觉质量方面都取得了改进

**Conclusion:** 该方法能够泛化到真实世界应用，如动态场景扩展和4D场景重组，展现了强大的实用价值
...
> Omni是一个统一的多模态模型，通过在文本、图像、视频、3D几何等多种模态上原生训练，实现了上下文展开机制，能够跨多个模态进行显式推理，从而提升了生成和理解任务的性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统多模态模型往往分别处理不同模态信息，缺乏模态间的深度融合推理。本研究旨在构建一个能够在多种模态间进行统一推理的模型，更好地逼近共享的多模态知识流形。

**Method:** 采用原生多模态训练策略，在文本、图像、视频、3D几何和隐藏表示等多样化模态上训练统一模型。提出上下文展开(Context Unrolling)机制，使模型能够在多个模态表征之间进行显式跨模态推理。

**Result:** Omni在多模态生成和理解基准测试中表现优异，展现了先进的多模态推理能力，能够实现文本、图像、视频和3D几何的上下文生成。

**Conclusion:** 通过统一的跨模态推理机制，Omni成功实现了异构模态间的信息互补，证明原生多模态训练和上下文展开是提升多模态AI系统推理能力的有效途径。
...
> IMU-to-4D是一个仅通过可穿戴IMU传感器（如耳机、手表或手机）重建人类4D运动和场景3D布局的框架，无需摄像头即可实现丰富时空理解。

<details>
  <summary>Details</summary>

**Motivation:** 传统的视觉感知系统在隐私保护、安全性、能耗和可扩展性方面存在持续挑战，需要寻找无视觉感知的替代方案。

**Method:** 提出IMU-to-4D框架，复用大型语言模型进行非视觉的时空理解，仅使用少量惯性传感器数据即可预测详细4D人体运动和粗粒度场景结构。

**Result:** 在多样化人-场景数据集上，IMU-to-4D相比现有级联管道产生更连贯和时序稳定的结果，证明可穿戴运动传感器本身就能支持丰富的4D理解。

**Conclusion:** 仅依靠可穿戴IMU传感器即可实现有效的4D感知，为解决视觉感知系统的隐私和安全等问题提供了可行替代方案，开辟了无视觉时空理解的新方向。
...
> 该论文将时间视为可学习的视觉概念，开发了检测速度变化、估计播放速度、生成条件视频和时间超分辨率等模型，创建了最大的慢动作数据集，实现了视频的时间控制。

<details>
  <summary>Details</summary>

**Motivation:** 虽然视频在现代计算机视觉研究中占据核心地位，但很少有人关注对时间流逝的感知和控制。研究如何判断视频是否被加速或减速，以及如何生成不同速度的视频，填补了视频时间推理的空白。

**Method:** 首先利用视频中自然存在的多模态线索和时间结构，以自监督方式学习检测速度变化和估计播放速度。然后使用这些模型从嘈杂的真实源中整理出迄今为止最大的慢动作视频数据集。基于该数据集开发了时间控制模型，包括速度条件视频生成和时间超分辨率。

**Result:** 成功创建了最大的慢动作视频数据集，开发了能够进行时间控制的模型。实验证明模型可以在指定播放速度下生成视频，并将低帧率模糊视频转换为具有精细时间细节的高帧率序列。

**Conclusion:** 研究表明时间是视频学习中可操作的感知维度，为时间可控的视频生成、时间法证检测和更丰富的世界模型开辟了新途径，这些模型能够理解事件如何随时间展开。
