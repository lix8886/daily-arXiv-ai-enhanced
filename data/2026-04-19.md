...
> 提出一个统一的压缩感知框架，通过随机测量和稀疏恢复动态执行大型语言模型，实现提示压缩与模型减缩的结合。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型存在参数量大、内存占用高和延迟问题。现有方法中，模型压缩是静态的，提示压缩不适应模型子网络，两者缺乏统一且无法利用不同提示和解码步骤激活不同计算路径的特性。

**Method:** 采用随机测量算子探测潜在模型使用情况，稀疏恢复估计任务条件和令牌自适应的支撑集，并将恢复的支撑编译为硬件高效的稀疏执行路径。框架包含五个关键组件：任务条件测量、令牌自适应恢复、形式化样本复杂度界限、硬件约束编译和联合目标函数。

**Result:** 将LLM推理重构为测量恢复问题，提供显式近似保证和面向部署的加速约束，实现动态适应不同提示和解码步骤的稀疏执行。

**Conclusion:** 该框架统一了提示压缩和模型减缩两个方向，通过动态稀疏执行路径为大型语言模型提供了更高效的推理方案。
...
> MemGround是一个基于游戏化交互场景的长时记忆评测基准，通过三层评估框架和多维度指标来全面测试LLMs的复杂记忆能力。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLMs的长时记忆评估过于静态和简单，仅关注基础检索和短上下文推理，忽视了复杂记忆系统的多面性，如动态状态跟踪和分层推理等能力。

**Method:** 提出MemGround评测基准，构建三层分层框架（表层状态记忆、时序关联记忆、基于推理的记忆），并通过专门的交互任务评估。同时设计多维度指标套件（QA总分、解锁记忆片段数、正确顺序记忆片段数、探索轨迹图）来量化记忆利用和行为轨迹。

**Result:** 实验表明，最先进的LLMs和记忆体在交互环境中仍难以应对持续的动态跟踪、时序事件关联以及基于长期累积证据的复杂推理。

**Conclusion:** 现有LLMs在复杂长时记忆任务上存在显著不足，需要更全面的评测标准和改进的记忆机制来提升在动态交互环境中的表现。
...
> HUOZIIME是一个基于LLM的个性化移动端输入法，通过分层记忆机制实现隐私保护、实时的个性化文本输入。

<details>
  <summary>Details</summary>

**Motivation:** 传统移动端输入法局限于手动输入，难以生成个性化文本。虽然轻量级LLM使设备端辅助生成成为可能，但要实现深度个性化、隐私保护且实时的生成式输入法仍面临根本挑战。

**Method:** 1) 通过在合成的个性化数据上对基础LLM进行后训练，赋予初始的类人预测能力；2) 设计分层记忆机制，持续捕获和利用用户特定的输入历史；3) 针对移动端LLM输入法部署进行系统性优化。

**Result:** 实验证明该系统能够高效在设备端运行，并实现高保真的记忆驱动个性化功能。

**Conclusion:** HUOZIIME成功解决了隐私保护、实时性和个性化三大挑战，为生成式移动端输入法提供了可行的解决方案。
...
> 研究表明大型语言模型能够独立识别机器学习论文中的方法学缺陷（特别是数据泄露问题），充当科学审计工具。

<details>
  <summary>Details</summary>

**Motivation:** 机器学习研究中数据泄露等方法学缺陷持续影响研究结果的有效性，需要独立的自动化工具来帮助识别这些问题以提高研究可重复性。

**Method:** 以一个手势识别论文为案例研究，该论文在小数据集上报告了近乎完美的准确率。研究者首先通过分析证实其评估协议存在受试者级别的数据泄露，然后让6个最先进的LLM独立分析原始论文，使用相同的提示词但无先验上下文。

**Result:** 所有LLM模型都一致性地识别出评估存在缺陷，并将报告的性能归因于非独立的数据划分，其判断依据包括重叠的学习曲线、极小的泛化差距和近乎完美的分类结果。

**Conclusion:** LLMs能够仅基于已发表的研究材料识别常见的方法学问题，虽然不能作为最终判断，但其一致性意见突显了它们作为提高可重复性和支持科学审计的补充工具的潜力。
...
> 通过对3万多篇ICLR投稿的分析发现，基于评分的论文接受度预测准确率达91%，而基于文本评论的预测仅81%。这表明审稿人的礼貌原则使得被拒论文的评论仍包含更多正面词汇，掩盖了真实的拒绝信号。

<details>
  <summary>Details</summary>

**Motivation:** 作者在解读同行评议反馈时存在困难，容易从礼貌评论中产生误解，或对具体低分感到困惑，影响对论文接受情况的判断。

**Method:** 构建包含3万多篇ICLR 2021-2025投稿的数据集，分别使用数值评分和文本评论进行接受度预测，分析预测失败样本的评分分布特征，并从情感分析角度研究文本评论的可靠性。

**Result:** 基于评分的模型准确率比基于文本的模型高10%；评分预测失败的样本呈现高峰度和负偏态分布；被拒论文的审稿意见中正面词汇仍多于负面词汇，存在礼貌原则现象。

**Conclusion:** 审稿人遵循礼貌原则使得文本评论的可靠性显著低于数值评分，作者应更加重视分数而非单纯依赖文本评论来判断论文接受可能性，特别是当存在个别低分时应谨慎对待。
...
> SeaAlert是一个基于LLM的海上遇险通信分析框架，通过合成数据生成技术解决真实标注数据稀缺问题，实现对嘈杂和非标准化紧急消息的鲁棒分析。

<details>
  <summary>Details</summary>

**Motivation:** 海上遇险通信是安全关键的语音消息，但在实践中由于消息简短、嘈杂、压力环境下产生、可能偏离标准格式，以及ASR错误等原因，自动分析仍然困难。

**Method:** 开发基于LLM的合成数据生成管道，生成真实多样的海上消息（包括省略或替换标准遇险代码词的挑战性变体），将生成的语句合成为语音，添加模拟VHF噪声，通过ASR系统转录获得真实的嘈杂转录文本。

**Result:** 构建了完整的SeaAlert框架，能够处理包含ASR错误和非标准表达的海上遇险通信，为海事安全提供了更可靠的分析工具。

**Conclusion:** 通过创新性的合成数据生成方法，有效解决了海上遇险通信分析中标注数据稀缺的问题，提高了在真实复杂环境下自动分析遇险消息的鲁棒性和准确性。
...
> TESSY框架通过师生交替生成tokens的方式，解决了传统SFT中教师模型与学生模型风格不一致导致的性能下降问题，在代码生成任务中显著提升了性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统使用强教师模型生成的合成数据进行监督微调的方法在新兴 reasoning 模型上不但不能提升，反而会导致性能大幅下降，主要是由于教师生成数据与学生模型的分布在风格上存在巨大差异。

**Method:** 提出了Teacher-Student Cooperation Data Synthesis (TESSY)框架，让教师模型和学生模型交替生成风格tokens和非风格tokens，生成既保持教师高级推理能力又与学生风格一致的合成序列。

**Result:** 以GPT-OSS-120B为教师模型，在Qwen3-8B上进行实验。教师数据直接微调导致LiveCodeBench-Pro下降3.25%，OJBench下降10.02%；而TESSY方法分别提升11.25%和6.68%。

**Conclusion:** TESSY成功解决了风格不一致问题，证明师生协作的数据合成方式能有效提升新兴reasoning模型的推理能力，为模型微调提供了新的有效路径。
...
> EviSearch是一个多智能体提取系统，能自动从临床试验PDF中生成与本体对齐的临床证据表格，并提供可追溯的来源信息用于审核验证。

<details>
  <summary>Details</summary>

**Motivation:** 为解决临床证据合成过程中的手动整理负担，需要开发一个自动化的高精度提取系统，能够从PDF文档中准确提取多模态证据（文本、表格、图表），并提供可审查的来源追溯。

**Method:** 系统采用PDF查询智能体（保持渲染布局和图表）、检索引导搜索智能体和协调模块的三智能体架构。当智能体意见不一致时，强制进行页面级验证，确保高精度提取和全面的归因覆盖。

**Result:** 在肿瘤学临床试验论文基准测试中，EviSearch显著提高了提取准确性，超越了解析文本基线方法，同时提供全面的归因覆盖。系统的决策记录和编辑操作可用于模型迭代改进。

**Conclusion:** 该系统旨在加速活体系统综述工作流程，减少手动整理负担，为将基于LLM的提取技术安全整合到证据合成流程中提供可审计的路径。
...
> 本文提出了H-TechniqueRAG，一种分层检索增强生成框架，利用MITRE ATT&CK的战术-技术层次结构来提高网络威胁情报映射的效率和准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的RAG方法在CTI文本到ATT&CK技术ID映射任务中采用扁平化检索范式，忽略了ATT&CK框架固有的分层分类结构，导致检索效率低下且精度有限。

**Method:** H-TechniqueRAG采用两阶段分层检索机制：首先识别宏观层面的战术目标，然后在这些战术内搜索具体技术，将候选搜索空间减少77.5%。同时设计了战术感知重排序模块和层次约束的上下文组织策略来优化生成过程。

**Result:** 在三个CTI数据集上的实验表明，H-TechniqueRAG比现有最优方法TechniqueRAG的F1分数提高3.8%，推理延迟减少62.4%，LLM API调用减少60%。模型还展现出更强的跨域泛化能力。

**Conclusion:** 分层结构先验不仅显著提升了映射任务的效率和准确性，还为安全分析师提供了高度可解释的逐步决策路径，展现了分层RAG范式在网络安全领域的巨大潜力。
...
> 本研究利用大语言模型（LLMs）完成中文修辞识别任务，通过LoRA微调和上下文学习等方法，在CCL 2025评测中获得全部三个赛道的第一名。

<details>
  <summary>Details</summary>

**Motivation:** 修辞识别是自动作文评分的关键组成部分，能够帮助AI系统更好地评估学生的语言能力和高阶思维技能，是AI教育领域的重要任务。

**Method:** 采用LoRA微调和上下文学习技术将修辞知识集成到LLMs中，将输出格式化为JSON以获得结构化结果，并将键值翻译为中文，同时研究了多种模型集成方法以进一步提升性能。

**Result:** 该方法在CCL 2025中文作文修辞识别评测任务的三个赛道上均取得最佳表现，荣获一等奖。

**Conclusion:** 基于大语言模型的修辞识别方法通过有效的知识集成策略和结构化输出设计，能够显著提升中文修辞识别的准确率，为AI教育领域的自动作文评分提供了有力支持。
...
> SAGEA推出了Celer 2.6通用模型系列，包含5B、10B和27B三个参数版本，具有推理验证、多模态能力和南亚语言支持特性，在各项基准测试中表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 开发一个既能处理复杂推理任务，又支持南亚语言（如尼泊尔语和印地语）的通用AI模型，同时保持低延迟和英语推理能力。

**Method:** 通过逆向推理(IR)管道让模型验证自身逻辑路径，集成端到端视觉编码器避免适配器问题，为Devanagari文字构建定制分词器，并在未公开模型上进一步预训练。

**Result:** 在数学、编程和通用智能基准测试(ACUMEN)上取得竞争性结果，在尼泊尔语和印地语上表现优异，英语推理能力未受影响，实现了低延迟性能。

**Conclusion:** Celer 2.6成功地将复杂推理、多模态处理和南亚语言支持结合在一起，为特定语言区域提供了高性能的通用AI解决方案。
...
> 该研究开发了一个基于RAG框架的对话系统，让建筑专业人士能通过自然语言查询项目会议记录，获取语义相关且带时间标注的决策历史。

<details>
  <summary>Details</summary>

**Motivation:** 大型建筑项目中的决策不断演化，决策记录散布在会议纪要中，手动重建特定决策的历史既费力又容易出错。专业人士需要一种高效准确的方式来追踪决策的时间线。

**Method:** 采用检索增强生成(RAG)框架，结合语义搜索和大语言模型。使用比利时某大型建筑公司的已完成项目会议纪要数据集，该数据集经过专家标注和查询增强。

**Result:** 系统成功实现了对会议纪要的对话式访问，能够提供语义相关且明确时间标注的答案，帮助用户理解决策的时间顺序。

**Conclusion:** 该方法为时间标注的项目文档提供了有效的对话式访问解决方案。数据集和开源实现已公开发布，可促进该领域的进一步研究。
...
> 本文提出一种状态化证据驱动的RAG框架，通过迭代推理结构化地累积证据，显著提升了问答系统的稳定性和抗噪能力。

<details>
  <summary>Details</summary>

**Motivation:** 传统RAG方法存在上下文表示扁平化和无状态检索的问题，导致性能不稳定。需要一种能够持续累积证据、处理检索噪声的新框架。

**Method:** 将检索文档转换为带相关性和置信度信号的结构化推理单元，构建持久化证据池；通过证据驱动的缺失分析识别信息缺口，迭代优化查询来指导后续检索，形成渐进式证据累积过程。

**Result:** 在多个问答基准测试中，该方法相比标准RAG和多步基线方法取得了一致的性能提升；能够有效累积高质量证据，在大量检索噪声下仍保持稳定表现。

**Conclusion:** 状态化证据驱动的迭代推理框架通过结构化证据管理和渐进式推理，解决了RAG的不稳定性问题，为构建鲁棒的问答系统提供了有效方案。
...
> 本研究系统性评估了Llama-3.1-8B、Mistral-7B-v0.1和Qwen3-8B三种开源大语言模型对罗马化尼泊尔语的适应能力，通过微调验证了Qwen3-8B是最佳架构，而Llama-3.1-8B在低资源开发中具有最大的改进潜力。

<details>
  <summary>Details</summary>

**Motivation:** 罗马化尼泊尔语是尼泊尔非正式数字通信的主要媒介，但在大型语言模型领域资源严重不足，缺乏系统性的基准测试和评估框架。

**Method:** 使用包含10000个样本的双语数据集，采用QLoRA与rsLoRA微调技术，在五个指标七个维度上评估模型性能，包括零样本和微调两种设置。

**Result:** 零样本时所有模型均失败，微调后所有模型性能显著提升；Qwen3-8B整体表现最佳，Llama-3.1-8B展现出最大的改进幅度。

**Conclusion:** 研究确立了罗马化尼泊尔语适应的首个严格基线，证实了适应空间假说，为低资源语言的大语言模型开发提供了重要参考。
...
> 本文提出了CRVA-TGRAG框架，通过改进的检索增强生成和教师引导偏好优化来解决LLM在CVE分析中的知识冲突问题。

<details>
  <summary>Details</summary>

**Motivation:** 网络安全漏洞CVE数据频繁更新（十年内超过30,000个漏洞变更），导致LLM训练数据与最新知识存在差异，造成知识冲突、事实错误和生成幻觉。

**Method:** 提出两阶段框架CRVA-TGRAG：1）检索阶段采用父文档分割和基于语义相似性与倒排索引的集成检索方案；2）生成阶段使用教师引导的偏好优化技术微调LLM。

**Result:** 实验表明该方法在检索最新CVE方面相比外部知识库实现更高准确性，显著缓解了单纯依赖LLM导致的知识冲突和不一致性问题。

**Conclusion:** 该框架不仅通过RAG提升了内容检索质量，还利用偏好微调优势使LLM能够更有效精确地回答问题，为网络安全漏洞分析提供可靠解决方案。
...
> 该研究发现通过训练一个仅占基础模型0.02%的后置适配器，可以修正对齐语言模型在政治敏感话题上的事实概率抑制问题，实现较少审查的文本生成。

<details>
  <summary>Details</summary>

**Motivation:** 对齐调优的语言模型在处理政治敏感话题时会压低事实的对数概率，尽管这些知识仍存在于模型的隐藏表示中，这种不一致性需要被修正。

**Method:** 在Qwen3-4B、8B和14B模型的冻结隐藏状态上训练786K参数的后置适配器，比较了门控(SwiGLU)和非门控(线性瓶颈)两种适配器架构，并研究了不同的应用方式。

**Result:** 适配器能记住全部15个训练事实并在11-39%的未见事实上泛化；两种架构效果相当(Fisher exact p > 0.09)；仅在最后位置应用时能生成连贯且较少审查的文本；logit空间适配器无法生成连贯内容。

**Conclusion:** 隐藏状态干预是修正生成任务的正确层面，发现并修复了Apple MLX中的静默梯度bug，为适配器研究提供了重要参考。
...
> 该研究针对ArchEHR-QA共享任务提出了一个统一系统，通过两阶段QLoRA微调Qwen3-4B模型进行答案生成，并基于加权集成检索方法进行证据句子对齐，整体性能显示数据规模仍是主要瓶颈。

<details>
  <summary>Details</summary>

**Motivation:** 解决ArchEHR-QA共享任务中的回答生成(子任务3)和证据句子对齐(子任务4)两个关键挑战，特别是在临床医疗问答场景下，如何利用有限的训练数据构建有效的问答系统。

**Method:** 采用两阶段QLoRA方法微调4位量化的Qwen3-4B模型：首先在30,000个emrQA-MedSQuAD样本上进行临床领域预训练，然后在20个标注开发样本上学习任务特定的输出格式；对于证据对齐任务，构建BM25相对阈值、TF-IDF余弦相似性和微调交叉编码器的加权集成系统。

**Result:** 在官方test-2026数据集上获得32.87的综合得分(BLEU=9.42, ROUGE-L=27.04等)，证据对齐任务在100个测试样本上达到67.16的micro-F1值。

**Conclusion:** 实验表明两个子任务都暴露了相同的核心问题：20个标注训练样本不足以区分相关和无关的临床句子，数据增强是未来最值得探索的方向。
...
> SPFG数据集专门用于口语语法纠错和教学反馈生成，研究发现监督微调在提升纠错和反馈质量方面表现最佳，且纠错质量与反馈质量相关性较弱。

<details>
  <summary>Details</summary>

**Motivation:** 传统的语法纠错(GEC)和解释(GEE)技术已快速发展，但实际教学场景还需要具有可操作性、水平适中且鼓励性的学习者友好教学反馈。

**Method:** 基于Speak & Improve Challenge 2025语料库构建SPFG数据集，包含口语转录文本、GEC目标和人工验证的教师式反馈。在基于转录的口语语法纠错(SGEC)设置下，评估了三个指令调优大模型(Qwen2.5、Llama-3.1、GLM-4)，比较监督微调(SFT)与基于偏好对齐(使用DPO和KTO)方法。

**Result:** SFT提供了最一致的改进效果，而DPO/KTO带来的增益较小或效果混合，纠错质量与反馈质量之间呈现弱耦合关系。

**Conclusion:** 本研究为口语教学反馈生成提供了首个专门数据集和系统评估框架，证明监督微调是目前最有效的方法，为进一步提升口语教学辅助技术奠定了基础。
...
> 这是一项关于大语言模型在罕见疾病患者教育与沟通中应用的范围性综述，发现该领域仍处于早期阶段，研究集中于准确性评估，缺乏患者中心的设计和真实世界应用。

<details>
  <summary>Details</summary>

**Motivation:** 罕见疾病患者面临复杂的护理路径、有限的临床专业知识和在长期患者旅程中巨大的未满足沟通需求。大语言模型的最新发展为支持患者教育和沟通提供了新机会，但其在罕见疾病中的应用尚不明确。

**Method:** 对2022年1月至2026年3月期间主要数据库发表的文献进行范围性综述，识别出12项关于LLM在罕见疾病患者教育和沟通中的研究。通过描述性和定性分析，提取研究特征、应用场景、模型使用和评估方法等数据。

**Result:** 文献高度新颖，主要由通用模型（特别是ChatGPT）主导。大多数研究聚焦于使用策划问题集的患者问答，缺乏真实世界数据和纵向沟通场景。评估主要围绕准确性，对可读性、同理心和沟通质量等以患者为中心的维度关注有限。多语言沟通很少被提及。

**Conclusion:** 该领域仍处于早期阶段。未来研究应优先考虑以患者为中心的设计、领域适应性方法和真实世界部署，以支持罕见疾病中安全、适应性和有效的沟通。
...
> 研究训练了一个3.18亿参数的古典中文语言模型，发现模型内部能区分真实与虚假历史事件，但外部无法表达不确定性，表明元认知表达能力无法通过语言建模自动获得。

<details>
  <summary>Details</summary>

**Motivation:** 探索大语言模型是否能区分已知与未知信息，并通过生成文本表达这种不确定性，这对于理解AI的元认知能力至关重要。

**Method:** 从零开始训练3.18亿参数的Transformer模型，使用15.6亿纯古典中文语料，通过系统性分布外测试评估模型对真实事件、半虚构事件和虚构事件的困惑度变化，并分析其不确定性表达模式。

**Result:** 模型内部能清晰区分真实与虚假信息（困惑度差异2.39倍），但外部无法表达不确定性，反而对分布外问题使用更少的认识标记。该发现在三种语言、三种文字系统和八个模型中都得到验证。

**Conclusion:** 元认知表达能力（说'我不知道'）无法仅通过语言建模获得，需要RLHF等显式训练信号才能培养。不同训练数据导致不同的表达习惯，体现了语言学惯例而非真实的元认知。
...
> 研究提出了一种两阶段蒸馏方法，成功将基于注意力机制的Transformer模型蒸馏到纯Mamba架构中，在保持相近性能的同时获得Mamba的高效推理优势。

<details>
  <summary>Details</summary>

**Motivation:** Mamba等状态空间模型相比Transformer具有更低内存消耗和更高推理吞吐量，但社区对Transformer训练积累了丰富知识，存在大量预训练模型。需要找到有效方法将预训练Transformer知识迁移到Mamba架构，以利用现有资源同时获得高效推理。

**Method:** 提出原则性的两阶段蒸馏方法：第一阶段使用核技巧将Transformer知识蒸馏到线性化注意力版本；第二阶段将线性化注意力蒸馏到适配的纯Mamba模型，不使用任何注意力模块。强调通过原则性初始化来提升跨架构蒸馏效果。

**Result:** 蒸馏得到的Mamba模型在下游任务中成功保持了Pythia-1B Transformer的原有性能，困惑度为14.11，与教师模型的13.86非常接近。通过1B规模模型的全面消融实验验证了方法的有效性。

**Conclusion:** 研究表明通过适当初始化和两阶段蒸馏策略，可以实现从Transformer到纯Mamba架构的有效知识转移，无需混合架构即可保持教师模型性能，为SSM模型在实际应用中的推广提供了可行方案。
...
> 本研究提出了PICCO框架，这是一个通过综合11个现有提示框架得出的五要素参考架构，用于结构化大型语言模型的提示设计。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型的性能严重依赖提示设计，但提示构建在描述和应用上往往不一致，缺乏统一的参考框架来指导系统性的提示设计。

**Method:** 通过多数据库搜索识别了11个已发布的提示框架，进行严谨的综合分析，提出了区分相关概念的分类体系，并推导出包含Persona（角色）、Instructions（指令）、Context（上下文）、Constraints（约束）和Output（输出）五个元素的参考架构。

**Result:** 创建了PICCO框架，明确了每个元素的功能、范围和相互关系，同时概述了实现相关的关键概念，包括各种提示技术、迭代工程方法和负责任提示的考量因素。

**Conclusion:** 这项工作是概念和方法论的贡献，为提示规范化和比较提供了通用结构，但并未声称PICCO作为优化方法的实证有效性，旨在提高概念清晰度并支持更系统的提示设计。
...
> 通过SWE-bench Lite基准测试发现，中文提示在LLM编程任务中并不比英文更高效，且成功率普遍较低，成本节省的说法缺乏依据。

<details>
  <summary>Details</summary>

**Motivation:** 社交媒体和开发者论坛流传中文提示比英文更节省token、可降低40%成本的说法，影响了开发者使用中文进行编程的决策，需要通过实证研究验证这一说法的可信度。

**Method:** 使用SWE-bench Lite软件工程任务基准，对不同模型架构在中文和英文提示下的token消耗、任务成功率和成本效益进行严格实证评估。

**Result:** 研究发现：1）中文不存在效率优势；2）不同模型的token成本存在差异，MiniMax-2.7中文成本高1.28倍，而GLM-5中文消耗更少token；3）所有测试模型中中文提示的成功率均低于英文。

**Conclusion:** 语言对token成本的影响因模型而异，开发者不应期望仅通过切换到中文提示就能获得成本节省或性能提升。由于评估模型数量有限且基准测试范围较窄，这些发现应被视为初步证据。
...
> CROP是一种通过正则化响应长度来优化提示的方法，能够在保持竞争性准确率的同时将token消耗减少80.6%。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自动提示优化框架只关注任务准确率，导致推理过程冗长，产生了显著的延迟和token成本问题。

**Method:** 提出CROP方法，通过在标准准确率反馈基础上增加文本反馈来实现响应长度的正则化，迫使优化过程产生简洁的提示。

**Result:** 在GSM8K、LogiQA和BIG-Bench Hard等复杂推理数据集上实现了80.6%的token消耗减少，同时保持竞争性准确率，性能仅有轻微下降。

**Conclusion:** CROP为在生产管道中部署token高效且成本效益高的代理AI系统提供了实用的解决方案。
...
> 本研究提出了一种混合跨模态注意力融合架构，结合CLIP视觉编码器和BGE-M3多语言文本表示，用于检测天城文脚本社交媒体迷因中的仇恨言论，在数据稀缺环境下实现了5.9%的F1-macro提升。

<details>
  <summary>Details</summary>

**Motivation:** 天城文脚本社交媒体迷因的仇恨言论检测面临多重挑战：多模态内容结构、脚本特定的语言复杂性以及低资源环境下的极端数据稀缺，传统方法在该场景下表现不佳。

**Method:** 提出混合跨模态注意力融合架构，使用CLIP (ViT-B/32)进行视觉编码，BGE-M3进行多语言文本表示，通过4头自注意力和可学习门控网络动态加权各模态贡献，系统评估了8种模型配置。

**Result:** 跨模态推理比纯文本基线提升5.9% F1-macro，发现英语中心视觉模型在天城文脚本上表现近随机，标准集成方法在数据稀缺(N≈850)下因相关过拟合而灾难性退化。

**Conclusion:** 研究表明显式跨模态推理对天城文迷因仇恨检测至关重要，揭示了英语偏见视觉模型的问题和数据稀缺下集成方法的局限性，为该领域提供了重要的技术洞察。
...
> 针对AI会议激增的审稿需求，作者提出了REVIEWBENCH评测基准和REVIEWGROUNDER框架，通过引入显式评分标准和证据锚定机制，显著提升了LLM生成审稿意见的质量。

<details>
  <summary>Details</summary>

**Motivation:** 随着AI会议投稿数量快速增长，利用大语言模型辅助同行评审成为趋势，但现有LLM审稿器往往产生肤浅、模板化的评论，缺乏实质性的、基于证据的反馈。

**Method:** 开发REVIEWBENCH评测基准，根据官方指南、论文内容和人工审稿制定论文特定评分标准；提出REVIEWGROUNDER多智能体框架，将审稿分解为起草和锚定两个阶段，通过针对性证据整合丰富浅层草稿。

**Result:** 在REVIEWBench上的实验表明，基于Phi-4-14B drafting和GPT-OSS-120B锚定的REVIEWGROUNDER，在8个维度上的人对齐和基于标准的审稿质量方面持续超越使用更大更强骨干网络（如GPT-4.1和DeepSeek-R1-670B）的基线方法。

**Conclusion:** 通过显式评分标准和证据锚定机制，即使使用相对较小的模型也能生成高质量的审稿意见，为AI辅助同行评审提供了有效解决方案。
...
> EuropeMedQA是首个欧洲多语言多模态医学考试基准数据集，用于评估LLM在非英语和跨语言临床场景中的表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM在英语医学考试上表现良好，但在非英语语言和多模态诊断任务上性能下降，需要更全面的评估基准。

**Method:** 构建意大利、法国、西班牙、葡萄牙官方监管考试数据集，遵循FAIR数据原则和SPIRIT-AI指南，采用自动翻译流程，使用零-shot严格约束提示策略评估多模态LLM。

**Result:** 创建了首个欧洲多语言多模态医学考试数据集，为评估LLM的跨语言迁移和视觉推理能力提供了抗污染的基准。

**Conclusion:** EuropeMedQA反映了欧洲临床实践的复杂性，将推动开发更具泛化能力的医疗AI系统。
...
> 该研究通过分析126,602篇新闻文章，量化了暴力灾难事件的新闻报道在时间上的结构和可预测模式，发现重大事件呈现报道量激增、语义早期漂移、逐渐回归基线等特征。

<details>
  <summary>Details</summary>

**Motivation:** 现代新闻周期因在线信息的快速交换而根本改变，媒体框架随着新信息、政治反应和社会反应的出现而动态变化。理解这些叙事在危机时刻如何形成、传播和演变对解读公众言论至关重要。

**Method:** 研究者收集了126,602篇在线新闻文章构成的大规模语料库，通过出版量、语义漂移、语义分散度和术语相关性四个指标来量化叙事变化，分析暴力灾难事件的报道模式。

**Result:** 研究发现突发影响事件表现出结构化且可预测的新闻周期模式，特点包括报道量激增、早期语义漂移以及逐渐回归基线水平的下降趋势，并识别出驱动这些时间模式的关键术语。

**Conclusion:** 该研究揭示了重大危机事件新闻报道的时空动态规律，为理解信息传播机制和公众舆论形成提供了量化分析的框架，有助于预测和把握新闻叙事的演变轨迹。
...
> GPT-4.1通过阅读球迷的开放式文字反馈预测其体验评分，准确率达到67%（±1分内），预测与自评评分存在1分的系统性差异，反映两种测量方式捕捉不同维度的体验信息。

<details>
  <summary>Details</summary>

**Motivation:** 探索AI能否仅通过球迷的自然语言反馈准确预测其对球场体验的评分，并理解预测评分与自评评分之间的关系差异。

**Method:** 使用GPT-4.1分析约1万份来自5个MLB球队的球迷开放文字反馈，预测0-10分体验评分，与实际调查评分进行对比分析

**Result:** 预测准确率高（36%精确匹配，67%±1分内），预测评分与自评评分相关性强（r=0.82），但存在约1分的系统性低估差异

**Conclusion:** AI预测评分捕捉显著情感时刻，自评评分整合整体体验判断，两者差异非误差而是不同构念，值得保留并存，为体验评估提供新视角
...
> GeoDe框架通过线性探针构建真实超平面，利用几何距离作为置信度信号进行'几何去噪'，有效过滤边界模糊样本，显著提升了大语言模型的诚实性和泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有拒绝微调方法直接基于响应准确性划分数据集，导致模型在决策边界附近遭受严重的标签噪声，从而表现出高拒绝率或幻觉问题。

**Method:** 从潜在空间表示角度出发，发现了决策超平面附近的'灰色区域'，其中内部信念模糊性是核心性能瓶颈。GeoDe框架包含两个关键步骤：1) 使用线性探针构建真实超平面；2) 以几何距离作为置信度信号执行几何去噪。

**Result:** 在Llama3、Qwen3等多个模型和TriviaQA、NQ、SciQ、SimpleQA等基准数据集上的实验表明，GeoDe显著提升了模型的真实性，并在分布外(OOD)场景中展现出强泛化能力。

**Conclusion:** 通过几何去噪方法有效解决了拒绝微调中的标签噪声问题，为大语言模型的边界感知和诚实性提升提供了新的技术途径。
...
> 本研究揭示了LLM生成的文本解释常不真实反映模型决策依据，并提出了一种基于注意力干预的无训练方法来提高解释的真实性。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型缺乏可解释性，限制了其在需要透明度和信任的领域应用。现有工作主要关注生成看似可信的理由，但无法确保这些解释是否真正反映了模型决策时所依赖的内部证据。

**Method:** 首先通过反事实评估LLM生成解释的认知真实性，发现它们往往不够真实。然后引入一种无需训练的方法，通过注意力层干预来指导解释生成，该方法利用基于可信归因方法提取的词级热图信息。

**Result:** 该方法在多个模型、基准测试和提示上显著提高了认知真实性，证明了通过注意力机制干预可以有效改善LLM生成解释的真实性。

**Conclusion:** 研究表明传统LLM生成的解释方法存在真实性缺陷，而基于注意力机制的训练无关方法能够有效提升解释的可信度，为构建更透明的AI系统提供了实用解决方案。
...
> 本文提出RoPE-Perturbed Self-Distillation方法，通过扰动RoPE索引创建不同视图并训练模型产生一致预测，显著提升了大语言模型的长上下文位置鲁棒性和性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有大语言模型在长上下文理解任务中存在位置依赖性问题，模型准确性高度依赖于相关证据的绝对位置，即使控制任务格式和难度也表现出较大的位置方差，这影响了模型在实际应用中的可靠性。

**Method:** 提出RoPE扰动自蒸馏训练正则化方法：通过对训练序列的RoPE索引进行扰动，创建同一序列的不同位置视图，然后通过自蒸馏训练模型在所有视图上产生一致的预测，促使模型依赖语义信号而非脆弱的位置依赖。

**Result:** 在Llama-3-8B和Qwen-3-4B的长上下文适应实验中取得一致提升，其中Llama-3-8B在RULER-64K上提升12.04%，Qwen-3-4B在RULER-256K上提升2.71%，同时改善了超出训练窗口长度的外推能力。

**Conclusion:** 该方法有效解决了长上下文适应中的位置敏感性问题，通过自蒸馏机制提高了模型的位置鲁棒性，在多个基准测试中验证了其有效性，为大语言模型的长文本理解提供了可靠的解决方案。
...
> 本研究开发了一种小型开源语言模型，能够在社交媒体帖子中自动检测PCOS患者同时存在的身体形象困扰、饮食失调和代谢问题三重负担，并提供可解释的文本证据。

<details>
  <summary>Details</summary>

**Motivation:** 多囊卵巢综合征患者面临身体形象困扰、饮食失调和代谢问题的三重风险，但现有的自然语言处理方法缺乏透明度，无法识别同时出现的多重表现。

**Method:** 收集1000条来自六个子版块的PCOS相关帖子，两位训练有素的标注员根据Lee等(2017)临床框架进行标注。使用Gemma-2-2B、Qwen3-1.7B和DeepSeek-R1-Distill-Qwen-1.5B三个模型，通过低秩适应(Low-Rank Adaptation)微调生成结构化解释。

**Result:** 最佳模型在150条保留测试帖子上达到75.3%的精确匹配准确率，在共病检测和可解释性方面表现良好。性能随诊断复杂性下降。

**Conclusion:** 该模型在筛查方面表现良好，但不适合自主诊断，更适合作为辅助筛查工具使用。
...
> APEX-MEM是一个创新的对话记忆系统，通过属性图谱结构化、仅追加存储和多工具检索代理三大技术，显著提升了大语言模型的长期对话记忆能力和时序推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大型语言模型在长期对话记忆方面存在不足，简单扩大上下文窗口或应用朴素检索方法会引入噪声并导致响应不稳定，需要更有效的长期记忆管理方案。

**Method:** 提出APEX-MEM系统，包含三个核心创新：(1)使用领域无关本体论的属性图谱，将对话结构化为以实体为中心、时序锚定的事件；(2)仅追加存储机制，保留信息的完整时序演化；(3)多工具检索代理，在查询时理解和解决冲突或演化的信息，生成紧凑且相关的记忆摘要。

**Result:** 在LOCOMO问答任务上达到88.88%准确率，在LongMemEval上达到86.2%，超越了最先进的会话感知方法，有效证明了结构化属性图谱在实现更准确时序连贯的长期对话推理方面的优势。

**Conclusion:** APEX-MEM通过结构化的属性图谱架构成功解决了长期对话记忆中的噪声和不稳定性问题，在保留完整交互历史的同时抑制了无关细节，为构建更可靠的长期对话系统提供了新的技术路径。
...
> 多模态语言模型在视觉感知任务上表现不佳，研究发现语言表征过度主导视觉信息，通过对比解码方法可在推理时提升最高16.9%的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 多模态语言模型在视觉感知任务上系统性表现不佳，但其失败机制尚不清楚。需要理解语言和视觉模态之间的相互作用关系，以找到改进模型性能的方法。

**Method:** 提出质心替换方法作为模态依赖性的受控探针，将每个标记折叠到最近的K-means质心。通过擦除文本和视觉质心结构来测试模型表现，并开发文本质心对比解码技术。

**Result:** 擦除文本质心结构比擦除视觉质心结构造成4倍的准确率损失，揭示了语言表征普遍主导视觉信息的不平衡。通过文本质心对比解码可提升最高16.9%的准确率，标准微调模型平均提升5.6%，偏好优化模型平均提升1.5%。

**Conclusion:** 模态竞争在结构上是局部的，可在推理时纠正而无需重新训练，并可作为诊断信号指导未来的多模态训练。
...
> 通过保守性重写和语义感知门控机制，提升了对话中自动事实验证的效果，在证据检索和事实验证任务上均取得显著改进。

<details>
  <summary>Details</summary>

**Motivation:** 对话中的自动事实验证涉及多轮对话，其中口语化表达频繁存在且研究不足。现有的口语化处理方法在对话场景下效果有限，影响了事实验证的准确性。

**Method:** 提出分阶段去口语化方法：首先对每个回应主张生成保守性重写候选，结合轻量级表面规范化和域内核心消解；然后引入BiCon-Gate语义感知一致性门控，仅在重写候选被对话上下文语义支持时才选择重写结果，否则保留原始主张。

**Result:** 在DialFact基准测试上，该方法在证据检索和事实验证方面均有提升，尤其在SUPPORTS类别上表现突出，超越包括基于解码器的一步式LLM重写基线在内的竞争方法。

**Conclusion:** 保守性重写配合语义感知门控机制能够有效稳定下游事实验证性能，为处理对话中的口语化表达提供了新的解决思路。
...
> 本研究提出了一种通过语义投影自动扩展WordNet风格词汇资源到新语言的方法，该方法通过项目-过滤策略提高了精度，同时保持了可解释性且需要少量外部资源。

<details>
  <summary>Details</summary>

**Motivation:** 现有的WordNet风格词汇资源扩展到新语言需要大量人工标注，成本高昂且效率低下。研究人员希望开发一种自动化方法，能够在保证准确性的同时，高效地将英语词汇语义资源扩展到其他语言。

**Method:** 采用语义投影的方法生成词汇意义：首先利用预训练的对齐模型，结合双语词典增强，将英语同义词集投影到对齐的目标语词汇上；然后使用双语词典过滤掉不正确的意义投影，最终将相应的词形分配给这些同义词集。

**Result:** 在多种语言上的实验结果表明，所提出的项目-过滤策略相比之前的方法以及基于词典和大语言模型的基线方法，能够显著提高精度，同时具有良好的可解释性和资源需求少的特点。

**Conclusion:** 该方法成功实现了自动化扩展词汇语义资源的目标，在保持高精度的同时减少了对外部资源的依赖，为跨语言词汇语义资源的构建提供了有效的解决方案。
...
> 研究发现多轮人机对话评估中忽视轮次间自相关性会导致统计显著性严重膨胀，提出了两阶段校正框架

<details>
  <summary>Details</summary>

**Motivation:** 现有的多轮对话评估方法普遍将连续轮次视为独立样本进行统计推断，但实际上对话轮次之间存在显著的自相关性，这导致大量虚假的统计显著性结果

**Method:** 系统性分析66个轮级指标在202次多轮对话中的自相关结构，提出两阶段校正框架：结合Chelton(1983)自由度校正和对话级块重抽样方法

**Result:** 标准测试中42%的显著相关性在校正后不再显著；无记忆类指标虚假率为14%，有记忆类指标达33%；校正后指标的可重复性从30%提升至57%

**Conclusion:** 论文提供了具体设计原则、发布清单和开源代码，呼吁LLM评估研究必须校正时间依赖性，以避免产生误导性结论
...
> Three-Phase Transformer (3PT) 是一种基于三相交流电原理的新颖Transformer结构，通过将隐藏向量划分为多个循环通道并引入相位保持操作，实现了更好的收敛速度和性能开销比。

<details>
  <summary>Details</summary>

**Motivation:** 传统的Transformer模型缺乏有效的结构化先验，作者提出了一种类似三相交流电的平衡机制，通过通道划分和相位旋转来实现信息的稳定传播，同时与现有的相对位置编码形成正交补充。

**Method:** 将隐藏向量划分为N个等大小的循环通道，每个通道进行独立的RMS归一化、2D Givens旋转和注意力头约束；引入Gabriel's horn轮廓作为绝对位置边通道；实现为自稳定平衡结构而非外接模块。

**Result:** 在123M参数规模的WikiText-103实验中，3PT相比基线降低7.20%困惑度，仅需增加0.00124%参数，收敛速度提升1.93倍；N值的优化呈现尺度依赖特性。

**Conclusion:** 通道分割的残差流、按块旋转、按相位归一化和DC注入是关键机制；实现了无需显式强制的几何自稳定性，展现了神经网络的守恒律框架新实例。
...
> HRM-LM（层次递归语言模型）将Transformer的独立层替换为双速递归结构，对比实验显示其表示质量显著低于独立层堆叠的UniversalTransformer。

<details>
  <summary>Details</summary>

**Motivation:** 为了探索是否能够通过共享权重的层次递归结构来匹配传统Transformer独立层堆叠的表示能力，从而减少参数量并提高计算效率。

**Method:** 提出HRM-LM模型，使用Fast模块（每步运行）和Slow模块（每T步运行）的两速递归结构，通过权重共享展开M=N×T步，并与参数匹配的UniversalTransformer进行对比实验。

**Result:** 在五次独立运行的实验中，HRM-LM的表现与UniversalTransformer存在明显的性能差距，证明共享权重的递归结构无法匹配独立层堆叠的表示质量。

**Conclusion:** 层次化的共享权重递归结构虽然在参数效率上有优势，但在语言模型任务中难以达到独立层堆叠Transformer的表示水平，这为模型架构设计提供了重要的实证参考。
...
> MARCA是一个用于评估大语言模型网络信息搜索能力的英葡双语基准测试，包含52个复杂问题，通过两种交互框架测试14个模型的表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型评估基准主要集中在英语环境，多语言特别是葡萄牙语的web信息搜索能力评估严重不足，需要专门的双语基准来填补这一空白。

**Method:** 构建包含52个手动编写的多实体问题数据集，配套人工验证的清单式评分标准，通过直接网络搜索和编排代理两种框架评估模型，多次执行以捕获随机性。

**Result:** 不同模型间性能差异显著，编排机制通常能提高答案覆盖度，模型在英语转葡萄牙语时表现出明显的不一致性和迁移难度，需要进行多次评估以反映真实性能。

**Conclusion:**  MARCA填补了多语言LLM web搜索评估的空白，为理解模型在葡萄牙语环境下的信息搜索能力提供了重要工具，揭示了跨语言性能迁移的挑战性。
...
> 研究发现，语言模型在有限数据下能够发展出共享但敏感的空位依存表征机制，但仍需要比人类多得多才能学习相当的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 探讨在发展可行数量数据下训练的语言模型是否也能像人类一样，在句法构造间共享空位依存表征。

**Method:** 使用分布式对齐搜索(DAS)分析BabyLM挑战中不同数据量训练的模型，评估wh问句和主题化构造间的表征迁移。

**Result:** 表明有限训练数据下可发展出共享的机制，但模型仍需远超人类的数据量才能获得相当的泛化能力。

**Conclusion:** 突出了在语言习得模型中需要语言特定偏见的必要性。
...
> 该研究提出了一种基于心理学的LLM引导框架，通过语义校准的残差流注入方法，在开放性生成任务中超越了传统提示方法，实现了对人格特征更有效的控制。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型激活干预方法存在搜索空间受限和激活空间单位未校准的问题，可能错过最优干预条件。需要一种更有效的方法来引导LLM产生符合特定心理特征的行为。

**Method:** 开发了一个心理学引导框架，使用心理测量学工具校准残差流注入，在语义校准单元上执行无界限、流畅性约束的扫描。采用IPIP-NEO-120测量OCEAN人格模型，比较了6种注入方法。

**Result:** 平均差分(MD)注入方法在14个LLM中的11个上优于个性提示(P²)，性能提升3.6%-16.4%；P²与MD的混合方法在13个LLM中表现最佳，相比P²提升5.6%-21.9%，相比MD提升3.3%-26.7%。

**Conclusion:** 表示工程成为开放性心理引导的新前沿，MD注入提供了可靠的线性控制旋钮，但仍与人类心理学存在差距，表明学习表示与现实心理学模型之间存在差异。
...
> CobwebTM是一种基于增量概率概念形成的终身层次化主题模型，该模型将Cobweb算法适配于连续文档嵌入，实现了在线语义层次构建，无需预定义主题数量

<details>
  <summary>Details</summary>

**Motivation:** 现有神经主题模型需要大量调整且面临灾难性遗忘问题，而传统概率模型缺乏灵活性且难以适应流数据，需要一种能够终身学习且避免固定容量限制的新方法

**Method:** 将Cobweb概念形成算法适配到连续文档嵌入空间，通过增量方式构建主题层次结构，结合预训练表示，实现在线无监督主题发现和动态主题创建

**Result:** 在多个数据集上实现了强主题连贯性、时间稳定性主题和高质量层次结构，证明了增量符号概念形成与预训练表示结合是主题建模的有效方法

**Conclusion:** CobwebTM成功将符号概念形成与神经表示相结合，为终身主题建模提供了一个低参数、高效且无需预设主题数量的解决方案
...
> PeerPrism是首个大规模同行评议基准数据集，包含20,690个样本，用于区分LLM检测中思想来源与文本来源，证明现有检测方法在混合作者场景下表现不佳。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LLM同行评议检测方法将作者权视为二元问题（人类vs AI），但现代同行评议工作流程具有混合性质，评估思想和文本实现可能来自不同来源，形成人机协作的光谱。

**Method:** 构建了一个包含完全人类、完全合成和多种混合转换的大规模基准PeerPrism，系统评估检测器是识别表面文本来源还是评估推理来源，并对最先进的LLM文本检测方法进行基准测试。

**Result:** 现有方法在标准二元任务上表现良好，但在混合 regimes下预测差异显著，特别是人类思想+AI文本时，检测器经常产生矛盾分类，表明当前方法将表面实现与智慧贡献混为一谈。

**Conclusion:** 同行评议中的LLM检测不能简化为二元归属问题，必须将作者权建模为跨越语义推理和风格实现的多维构造。PeerPrism是首个评估这类场景中人机协作的基准。
...
> 该研究提出了一种基于表征工程(Cognitive Reverse-Engineering)的框架，系统地解析了大语言模型处理社交比较嫉妒情绪的内部认知机制，发现模型以线性方式组合了优势性和领域相关性两个心理前因。

<details>
  <summary>Details</summary>

**Motivation:** 尽管大型语言模型展现出越来越复杂情感处理能力，但其内部处理复杂情感特别是嫉妒等高级情绪的机制仍不明确。现有可解释性方法要么将模型视为黑箱，要么只关注基础情绪，缺乏对复杂情感认知结构的深入探索。

**Method:** 提出基于表征工程的认知逆向工程框架，结合评价理论、子空间正交化、基于回归的加权和双向因果引导技术，分离并量化嫉妒的两个心理前因：比较对象的优势性和领域自我定义相关性。

**Result:** 在8个来自Llama、Qwen和Gemma家族的大语言模型上的实验表明，模型天然地将嫉妒编码为这些构成因素的结构化线性组合，内部表征与人类心理建构一致，以优势性为基础触发器，相关性作为最终强度倍增器。

**Conclusion:** 该框架证明了有毒情感状态可以被机械检测并手术式抑制，为多智能体环境中AI安全的表征监控和干预提供了可能路径，同时揭示了LLM情感处理的认知结构与人类心理机制的高度相似性。
...
> 本文指出NLP领域的多样性进步过度集中在公平性相关领域，并分析了导致边缘化研究人员转向公平性研究的激励机制和障碍，提出了促进整个NLP领域包容性的建议。

<details>
  <summary>Details</summary>

**Motivation:** 关注NLP领域多样性发展不均衡的问题，发现多样性改进主要局限在少数公平性相关领域，而边缘化研究人员在其他领域面临研究和参与障碍。

**Method:** 通过调研NLP各子领域研究人员的人口统计数据，分析激励结构、偏见和障碍如何影响研究人员的分布，并识别导致领域不平等的反馈循环机制。

**Result:** 证实了NLP多样性进步集中在公平性相关领域的假设，发现了地理、语言等系统性障碍，以及现有激励机制如何强化领域间的不平等现状。

**Conclusion:** 建议打破强化差异的反馈循环，解决地理和语言障碍，建立更公平的激励机制，确保NLP所有领域都能变得更加包容和公平。
...
> CAUSALDETOX是一个通过干预特定注意力头来减少大语言模型毒性生成的新框架，在保持语言流畅性的同时实现了更好的毒性减少效果。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型经常生成有毒内容，现有缓解策略会降低生成质量或需要昂贵的人工标注，因此需要一种更高效且不影响模型性能的解决方案。

**Method:** 使用概率必要性和充分性(PNS)识别对毒性生成必要且充分的最小注意力头集合，通过本地推理时干预(构建输入特定的动态引导向量)和PNS指导的微调(永久遗忘毒性表示)两种策略进行干预。

**Result:** 在ToxiGen、ImplicitHate和ParaDetox数据集上，CAUSALDETOX相比基线方法毒性减少提升5.34%，同时保持语言流畅性，头部选择速度提升7倍。

**Conclusion:** 该框架通过因果干预方法有效解决了LLM毒性问题，为安全部署大语言模型提供了新的技术路径，显著优于现有方法。
...
> 本文提出了RASC方法，通过检索增强集合完成来解决临床价值集编撰问题，在VSAC数据集上显著优于直接使用LLM的方法。

<details>
  <summary>Details</summary>

**Motivation:** 临床价值集编撰是质量测量和表型分析中的瓶颈任务，传统方法效率低下，而直接使用LLM生成代码由于临床词汇表的复杂性和版本控制问题效果不佳。

**Method:** 提出检索增强集合完成(RASC)框架：首先检索K个最相似的价值集形成候选池，然后使用分类器对每个候选代码进行筛选。构建了包含11,803个VSAC价值集的大规模基准数据集。

**Result:** 基于SAPBert的交叉编码器达到AUROC~0.852和F1~0.298，相比检索-only方法将每个正例的无关候选从12.3个减少到3.2个。GPT-4o零样本F1仅~0.105，且有48.6%的代码不在VSAC中。

**Conclusion:** RASC通过缩小有效输出空间显著提升了性能，其优势在不同分类器模型中都具有普适性，为临床价值集编撰提供了有效的解决方案。
...
> StoryCoder是一个通过将编程问题转化为连贯故事叙述来提升代码生成效果的框架，在多个基准测试中平均提升18.7%的零样本性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的代码生成方法虽然增强了推理步骤或注入特定结构，但问题条件仍然零散分散，缺少良好的整体结构。受人类将碎片信息组织成连贯解释的方式启发，需要一种方法为模型提供更丰富的上下文结构。

**Method:** 提出StoryCoder叙事重构框架，将代码生成问题转换为包含三个组成部分的自然语言叙事：任务概述、约束条件和示例测试用例，框架受选定算法和体裁类型引导。

**Result:** 在HumanEval、LiveCodeBench和CodeForces等数据集上对11个模型进行测试，零样本pass@10指标平均提升18.7%。分析显示该框架能引导模型采用正确的算法策略，减少实现错误，并产生更模块化的代码结构。

**Conclusion:** 叙事重构框架证明了结构化问题表示对代码生成的重要性，其效果依赖于叙事连贯性和体裁对齐，这一优势不受模型规模或架构限制。
...
> 本文提出了一种大规模选项评估协议，将候选选项扩展到100个，用于更准确地测试大语言模型的真实能力，发现传统低选项评估可能过分高估模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的多选题评估中，模型可以通过捷径策略在低选项设置下获得接近满分的准确率，这掩盖了其真实的语言理解能力，需要更严格的评估方法。

**Method:** 设计大规模选项评估框架，在韩语正字法错误检测任务中将选项扩展到100个，通过固定目标、重复重采样和打牌来获得稳定估计，并分离内容驱动失败和位置偏差的影响。

**Result:** 实验表明，模型在低选项设置下的强势表现会误导对其能力的评估；在高干扰环境下，模型出现语义混淆和对前部选项的位置偏好这两个主要失败模式。

**Conclusion:** 大规模选项评估是压力测试模型可靠性的有效框架，能够揭示传统基准测试所掩盖的模型能力缺陷，主要瓶颈在于候选选项排名而非上下文长度。
...
> 本文提出了一种基于大语言模型的金融虚假信息检测方法，在无需外部参考信息的情况下，通过结合上下文学习和参数高效微调，在权威评测中获得第一名，准确率超过95%。

<details>
  <summary>Details</summary>

**Motivation:** 金融虚假信息泛滥严重威胁市场稳定和投资者信任，但在实际应用中往往缺乏外部证据进行交叉验证，因此亟需开发无需外部参考的检测方法。

**Method:** 提出综合框架，利用先进大语言模型的推理能力，系统性地集成上下文学习（零样本和少样本提示策略）与通过低秩自适应（LoRA）的参数高效微调（PEFT），使模型更好地对齐金融操纵的语言特征。

**Result:** 在RFC-BENCH评测中取得双料第一，公开测试集准确率95.4%，私有测试集准确率96.3%，证明了方法的鲁棒性和有效性。

**Conclusion:** 该方法成功解决了无参考金融虚假信息检测的挑战，为金融自然语言处理领域的发展贡献了重要的上下文感知检测技术，并提供了开源模型供研究使用。
...
> CURaTE是一种实时持续遗忘方法，通过训练句子嵌入模型识别需要遗忘的内容，无需修改模型参数即可高效实现知识遗忘，同时保持模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型遗忘技术存在实时性不足、知识退化严重、敏感信息暴露时间长等问题，无法满足持续、即时的遗忘需求。

**Method:** CURaTE首先在专门的数据集上训练句子嵌入模型，建立锐利的决策边界来判断输入是否匹配存储的遗忘请求，然后根据相似度决定是否回答或拒绝响应。

**Result:** CURaTE不仅比现有方法遗忘效果更好，而且通过避免修改模型参数，实现了近乎完美的知识保持，是唯一能够实时持续遗忘的方法。

**Conclusion:** CURaTE为大语言模型提供了一种简单而高效的实时持续遗忘解决方案，在保证知识完整性的同时有效实现了针对性知识的移除。
...
> CURA是一个临床风险预测框架，通过双层不确定性目标函数对齐临床语言模型的风险估计与不确定性，提高校准性能而不损失判别能力。

<details>
  <summary>Details</summary>

**Motivation:** 临床语言模型在自由文本风险预测中应用广泛，但其不确定性估计往往校准不良且临床不可靠，可能导致过度自信的假性安抚，影响临床决策。

**Method:** CURA首先微调领域特定临床LM获得任务自适应患者嵌入，然后使用双层不确定性目标函数优化多头分类器：个人层面校准项对齐预测不确定性与错误概率；群体感知正则化项将风险估计推向嵌入空间局部邻域的事件率，并为决策边界附近的模糊群体加权。

**Result:** 在MIMIC-IV多个临床预测任务和不同临床LM上的实验表明，CURA持续改善校准指标且不显著损害判别性能，减少了过度自信的错误安抚。

**Conclusion:** CURA为临床LM提供了更可信的不确定性估计，通过群体感知正则化的标签平滑视角，提升了临床决策支持的可靠性。
...
> SPAGBias是首个评估大语言模型空间性别偏见的系统性框架，通过建立62个城市微空间分类学和多层次诊断方法，发现LLM在城市空间中存在超越公私二分法的结构化性别偏见，这些偏见在模型全流程中被强化并导致下游应用失败。

<details>
  <summary>Details</summary>

**Motivation:** 虽然大语言模型在城市规划中应用日益增多，但性别空间理论指出性别等级制度嵌入在空间组织中，人们担心LLM可能复制或放大这种偏见，因此需要系统性评估框架来检测和分析空间性别偏见。

**Method:** 提出SPAGBias框架，包含62个城市微空间分类学、提示库和三个诊断层次：显性层（强制选择重采样）、概率层（token级不对称性）和建构层（语义和叙事角色分析），测试6个代表性模型并进行追踪和下游实验。

**Result:** 发现结构化的性别-空间关联超出公私二元划分，形成精细的微层面映射；故事生成揭示情感、措辞和社会角色共同塑造

**Conclusion:** 该工作将社会学理论与计算分析结合，将偏见研究扩展到空间领域，揭示了LLM如何通过语言编码社会性别认知，为理解和缓解AI系统中的空间偏见提供了重要基础。
...
> 本文提出了NEST-KGQA新任务和数据集，专门处理包含否定约束的知识图谱问答，并设计了CUCKOO框架和PyLF逻辑形式来解决多约束问答的语义复杂性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大型语言模型存在忠实性和幻觉问题，在知识图谱问答中，现有方法偏向于正向约束和计算约束，而忽视了现实中频繁出现的否定约束，这是一个重要的研究空白。

**Method:** 设计了NEST-KGQA数据集和PyLF逻辑形式，提出了CUCKOO框架：1）约束感知的逻辑形式草稿生成；2）模式引导的语义匹配；3）选择性自指导精炼，仅在执行不当逻辑形式产生空结果时应用。

**Result:** 实验结果表明，CUCKOO在少样本学习设置下，在传统KGQA和NEST-KGQA基准测试中均持续优于基线方法。

**Conclusion:** 通过专门处理否定约束和多约束问答的复杂性，本研究为知识图谱问答领域提供了新的任务定义、数据集和有效的解决方案框架，显著提升了模型的性能和鲁棒性。
...
> 该研究通过挖掘社区-个体偏好差异，提出了基于六个个性化因素的评估基准CoPA，用于更好地评估个人化问答系统。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM个人化问答评估方法主要依赖词汇级相似度或人工启发式规则，缺乏足够的数据驱动验证，无法准确捕捉用户个性化偏好。

**Method:** 挖掘社区-个体偏好差异(CIPD)，提炼出六个关键个性化因素作为评估维度，构建包含1,985个用户画像的CoPA基准，通过量化模型输出与用户认知偏好的对齐程度进行评估。

**Result:** CoPA提供了一个比通用指标更全面和更具区分性的个性化问答评估标准，能够进行细粒度的因子级评估。

**Conclusion:** CoPA基准为个性化问答评估提供了新的数据驱动方法，弥补了传统评估范式的不足，有助于推动个性化LLM问答系统的发展。
...
> MM-AQA是首个多模态问答中的有效拒绝回答评估基准，发现现有VLMs很少主动拒绝回答，多智能体系统虽提高拒绝率但存在精度-拒绝权衡，关键瓶颈是模型校准而非推理深度。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉语言模型和多智能体系统的评估范式假定了问题均可回答，但实际应用中证据不足的情况普遍存在。缺乏对多模态环境下有效拒绝回答能力的评估方法。

**Method:** 构建MM-AQA基准，通过视觉模态依赖性和证据充分性两个维度将可回答样本转换为不可回答样本，评估3个前沿VLMs和2个MAS架构在2079个样本上的表现。

**Result:** 标准提示下VLMs很少拒绝回答；MAS提高拒绝率但引入精度-拒绝权衡；顺序设计与迭代设计性能相当；模型在证据完全缺失时拒绝，但在证据缺失或矛盾时仍尝试回答。

**Conclusion:** 有效的多模态拒绝回答需要拒绝感知的训练，而非更好的提示或更多智能体，目前瓶颈在于模型校准不当。
...
> 该论文提出了一种新的LLM遗忘框架，通过梯度几何重构而非损失重平衡来解决遗忘-保留之间的权衡问题。

<details>
  <summary>Details</summary>

**Motivation:** 大规模语言模型遗忘面临核心矛盾：如何在有效移除目标知识的同时最大程度地保留模型的一般能力。传统方法存在遗忘与保留性能之间的严重权衡。

**Method:** 将LLM遗忘重新定义为非对称双任务问题，以保留为主要目标、遗忘为辅助目标。提出了保留优先的梯度合成框架，解耦任务特定梯度提取与冲突感知组合。基于该框架实现了PCGrad和SAGO两种方法。

**Result:** SAGO在多个基准测试中持续推动Pareto前沿：在WMDP Bio上，MMLU性能从44.6%（基线）提升至94.0%（+PCGrad）再到96.0%（+SAGO），同时保持相当的遗忘强度。

**Conclusion:** 研究表明，重构梯度几何而非重平衡损失函数是解决遗忘-保留权衡的关键。SAGO通过构造性符号约束合成实现了更严格的梯度对齐。
...
> 该论文研究了在标注数据稀缺的医疗NLP任务中，通过观察预训练嵌入的几何变化来预测域特定预训练收益的方法。

<details>
  <summary>Details</summary>

**Motivation:** 针对医疗AI中标注数据获取延迟的常见问题，旨在寻找能够预测域特定预训练效果的方法，避免不必要的计算资源浪费。

**Method:** 对芬兰BERT模型在芬兰医疗文本上进行域微调，并通过分析嵌入空间几何变化来预测预训练收益。

**Result:** 总结了在芬兰医疗文本上微调BERT的观察结果，并尝试通过嵌入几何变化来预测预训练的收益效果。

**Conclusion:** 为标注数据稀缺的医疗NLP任务提供了一种评估域特定预训练潜在收益的方法，有助于优化资源分配策略。
...
> 论文实现了一个1B到7B参数级联的教育助手系统Pangu-ACE，通过1B路由器智能分配简单和复杂任务，在修复评估错误后显示质量提升，但效率优势主要体现在任务选择性而非延迟改善。

<details>
  <summary>Details</summary>

**Motivation:** 教育助手应该只在必要时才投入更多计算资源，以实现计算效率与任务复杂性的平衡，避免过度计算浪费资源。

**Method:** 构建Pangu-ACE级联系统：1B参数的导师路由器生成初步答案和路由信号，决定直接接受或升级到7B专家模型处理。修复了之前的离线评估错误，重新评分保存的预测结果。

**Result:** 修复评估错误后，系统在中国测试集(7013个样本)上显著提升：确定性质量从0.457提升至0.538，格式有效性从0.707提升至0.866。1B模型直接处理19.7%请求，不同任务路由选择性差异明显。

**Conclusion:** 级联系统有效提升了教育任务的响应质量，路由选择性成为核心效率优势。当前部署延迟优势尚未体现，需要修复外部基准对齐的基础设施问题，但系统已展现良好的任务适应性分配能力。
...
> 本研究将行为特征标注(BP)视为一组标注技能而非单一任务，通过技能文件驱动Pipeline评估LLM在多维度语言标注中的表现，发现GPT-5.4在部分技能上表现可靠，但存在技能选择性可行性特征。

<details>
  <summary>Details</summary>

**Motivation:** 行为特征标注因其需要同时在多个语言维度上进行编码而难以自动化。传统方法将BP标注视为单一任务，但这种方法无法准确反映LLM在不同标注技能上的异构表现。

**Method:** 使用3134个中文隐喻色彩词派生语料和14个特征的BP架构，实现技能文件驱动的流程，每个特征通过架构文件、决策规则和例示外部定义。通过两名标注员完成两轮验证，将BP技能分类为可直接操作、可通过重标注恢复和结构不足三类。评估GPT-5.4和三个开源模型在同一设置下的表现。

**Result:** 结果显示BP标注在技能层面高度异构：5个技能可直接操作，4个可通过重点重标注恢复，5个仍结构不足。GPT-5.4在保留技能上执行可靠性较高，但这种可行性是选择性的而非全局的。人类与GPT在技能层面困难剖面对齐度高，但在实例和词条层面不相关。

**Conclusion:** 研究表明自动标注应从技能可行性而非任务层面自动化角度评估，GPT更适合理解为独立的第三个技能声音而非人类直接替代品。为多维度语言标注的自动化提供了新的评估框架和实践指导。
...
> ClimateCause是一个面向气候变化的专家标注因果数据集，包含高阶因果结构，特别关注隐式和嵌套因果关系，可用于构建因果图并评估语句可读性。

<details>
  <summary>Details</summary>

**Motivation:** 气候变化理解需要对复杂因果网络进行推理，但现有的因果发现数据集主要捕获显式的直接因果关系，无法满足气候科学的复杂需求。

**Method:** 从科学政策气候报告中手动专家标注，创建包含隐式和嵌套因果关系的高阶因果结构数据集；将因果表达式标准化并分解为独立因果关系，标注因果相关性、关系类型和时空上下文；用于构建因果图和量化可读性。

**Result:** 成功构建了ClimateCause数据集，实现了基于因果图语义复杂度的可读性量化；大语言模型基准测试显示因果链推理是主要挑战。

**Conclusion:** ClimateCause填补了气候变化领域复杂因果结构数据集的空白，为因果发现、可读性分析和AI模型评估提供了重要资源，特别突出了因果链推理的挑战性。
...
> 研究发现，在结构化生成任务中，schema键的措辞会影响LLM性能，这表明schema不仅是结构约束，还是隐含的指令通道。不同模型对指令通道的敏感性不同，且通道间存在非加性交互效应。

<details>
  <summary>Details</summary>

**Motivation:** 现有约束解码方法主要将schema视为纯结构约束，忽略了其语言表述可能影响模型行为。研究者发现仅改变schema键的措辞就能显著改变模型性能，表明需要重新理解结构化生成。

**Method:** 提出将结构化生成重新解释为多通道指令问题，指令可通过提示显式传送，也可通过schema键在解码过程中隐式传送。系统研究了schema键表述作为隐含指令通道的作用。

**Result:** 实验显示不同模型家族对指令通道的敏感性不同：Qwen模型一致受益于schema级指令，而LLaMA模型更依赖提示级指导。观察到指令通道间存在非加性交互效应。

**Conclusion:** Schema设计不仅决定输出结构，还承载指令信号，为LLM结构化生成提供了新视角。这些发现对优化模型性能和理解模型行为具有重要意义。
...
> 本文提出了一种改进的流式探测方法，通过要求多个证据标记一致支持预测，有效提升了LLMs在CBRN领域自适应越狱攻击检测的准确性，在1%假阳性率下真阳性率提升35.55%。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法依赖少数高评分标记进行越狱攻击检测，导致在良性上下文中出现CBRN敏感术语时产生误报，存在系统误差。

**Method:** 引入新的流式探测目标函数，要求多个证据标记一致支持预测而不是依赖孤立标记峰值；使用注意力或MLP激活而非残差流特征。

**Result:** 在1%假阳性率下真阳性率比基线提升35.55%，AUROC显著提高；探测方法可'即插即用'应用于字符级密码混淆攻击，AUROC超过98.85%。

**Conclusion:** 基于聚合信号的稳健探测方法显著改善了CBRN越狱攻击检测性能，注意力/MLP特征更有效，且方法对新颖混淆攻击具有强泛化能力。
...
> RACER是一种无需训练的轻量级投机解码方法，通过结合检索到的精确模式和 logits驱动的未来提示，实现了2倍以上的推理加速，性能优于现有的无需训练方法。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型的自回归解码每步只生成一个token，导致推理延迟高。现有的无需训练投机解码方法存在权衡：基于检索的草稿在无精确匹配时失效，而基于logits的草稿缺乏结构指导。

**Method:** 提出RACER（检索增强上下文快速投机解码），将检索到的精确模式与logits驱动的未来提示相结合，既提供可靠锚点又支持灵活外推，生成更丰富的投机草稿。

**Result:** 在Spec-Bench、HumanEval和MGSM-ZH等基准测试中，RACER稳定加速推理，相比自回归解码实现2倍以上加速，性能优于先前的无需训练方法。

**Conclusion:** RACER为高效LLM推理提供了一个可扩展、即插即用的解决方案，统一了检索和logits两种草稿生成方式的优势，无需额外训练即可显著提升解码效率。
...
> 研究发现18个视觉语言模型在思维链推理过程中存在'答案惯性'现象，且模型易受文本线索误导，推理训练模型虽然表现更好但其思维链可能掩盖真实的模态依赖。

<details>
  <summary>Details</summary>

**Motivation:** 尽管视觉语言模型已具备推理能力，但这些能力如何展开以及如何整合视觉和文本信息尚不明确，需要深入分析推理动态以保证多模态系统的透明度和安全性。

**Method:** 分析了18个VLMs（包括指令微调和推理训练模型），通过追踪思维链(Chain-of-Thought)过程中的置信度变化、测量推理的纠正效应、评估中间推理步骤的贡献，并使用误导性文本线索进行受控干预实验。

**Result:** 发现模型存在'答案惯性'——早期的预测倾向会在推理过程中被强化而非修正；推理训练模型虽有更强纠正行为，但其表现依赖于模态条件；模型即使在视觉证据充分时也易受文本线索影响，且这种影响在思维链中的可检测性因模型而异。

**Conclusion:** 思维链仅提供了视觉语言模型决策机制的部分视角，无法完全揭示不同模态驱动的决策过程，这对多模态系统的透明度和安全性具有重要启示。
...
> 本研究评估了六种现代多语言句子嵌入模型在立陶宛语、俄语和英语仇恨言论检测任务上的表现，发现监督学习方法显著优于无监督异常检测，而PCA压缩能有效保持特征判别力。

<details>
  <summary>Details</summary>

**Motivation:** 在线仇恨言论和滥用语言对内容审核构成挑战，特别是在多语言环境和低资源语言（如立陶宛语）场景下。现有研究缺乏对这些语言仇恨言论检测的系统评估。

**Method:** 构建立陶宛语仇恨言论语料库LtHate，使用统一的Python流水线评测potion、gemma、bge、snow、jina、e5等六种多语言编码器，并采用HBOS异常检测器和CatBoost分类器进行建模，测试PCA压缩至64维的效果。

**Result:** 双类监督模型在所有数据集上显著优于单类异常检测。最佳配置在立陶宛语达到80.96%准确率(AUC 0.887)，俄语92.19%(AUC 0.978)，英语77.21%(AUC 0.859)。PCA压缩在监督学习中保持判别力，但在无监督检测中有负面影响。

**Conclusion:** 现代多语言句子嵌入与梯度提升决策树的结合为多语言仇恨言论检测提供了鲁棒的软计算解决方案，为低资源语言的自动内容审核奠定了基础。
...
> IE-as-Cache框架将信息提取重新定义为认知缓存，通过动态维护中间信息和过滤噪声来增强代理系统推理能力，在多个基准测试中显著提升了推理准确性。

<details>
  <summary>Details</summary>

**Motivation:** 传统信息提取被当作终端目标，提取后的结构化信息常被孤立使用，无法在多步推理中维护和重用。作者希望打破这一限制，将IE转化为可重复利用的认知资源。

**Method:** 提出IE-as-Cache框架，借鉴层次化计算机内存的设计理念，结合查询驱动的提取和缓存感知推理，动态维护紧凑的中间信息并过滤噪声。

**Result:** 在多样化的LLM和具有挑战性的基准测试上，实验表明该方法能够显著提升推理准确性，验证了IE作为可重用认知资源的有效性。

**Conclusion:** 研究成功地将信息提取重新定位为认知缓存，为IE的下游应用研究提供了新方向，证明了IE在增强代理系统推理能力方面的巨大潜力。
...
> 本研究提出了XQ-MEval数据集，用于评估翻译指标的跨语言评分偏向性问题，并基于该数据集证明了语言间平均分数与人工评判的不一致性和跨语言评分偏向的存在，最后提出了改善多语言评估公平性的归一化策略。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多语言翻译系统评估方法存在潜在问题：简单地平均各语言的指标分数可能不公平，因为翻译质量相同的译文在不同语言中可能获得不同的评分。但这一跨语言评分偏向性问题尚未得到系统性研究，主要原因是缺乏跨语言平行质量实例的基准数据集，而专家标注也不现实。

**Method:** 研究者构建了XQ-MEval数据集，涵盖9个翻译方向。通过自动将MQM定义的错误注入黄金译文，由母语者过滤确保可靠性，合并错误生成质量可控的伪译文，最后将伪译文与相应的源文本和参考译文配对形成三元组来评估翻译指标质量。

**Result:** 实验发现9个代表性指标的语言平均分数与人工判断不一致，首次为跨语言评分偏向提供了实证证据。提出的归一化策略能够对齐不同语言的分数分布，提高多语言指标评估的公平性和可靠性。

**Conclusion:** 该工作首次系统性地揭示了多语言翻译评估中的跨语言评分偏向问题，通过构建XQ-MEval数据集为该领域提供了重要基准，并提出的归一化策略为解决这一问题提供了有效方案，对构建更公平的多语言翻译系统具有重要意义。
...
> 本文提出了一套从文本自动生成科学架构图的完整解决方案，包括构建了首个大规模数据集system，训练了小型语言模型，并与GPT-4o实现了相当的性能。

<details>
  <summary>Details</summary>

**Motivation:** 仅通过文本交流复杂系统设计或科学流程效率低下且容易产生歧义，需要能够自动从文本生成高语义保真度的科学架构图的系统，该技术在企业架构可视化、AI驱动软件设计和教育内容创作等多个领域都有重要应用价值。

**Method:** 首先构建了一个综合数据集system，包含科学架构图像、对应文本描述以及DOT代码表示。然后基于该数据集微调了一系列小型语言模型，并使用GPT-4o进行上下文学习。模型通过理解输入文本的语义来生成中间代码，进而生成高保真度的架构图。

**Result:** 实验表明，基于system数据集训练的模型显著优于DiagramAgent等现有基线模型，与基于GPT-4o的上下文学习方法性能相当。同时公开了代码、数据和模型。

**Conclusion:** 本研究填补了该领域缺乏大规模开放数据集和有效开源模型的空白，为 scientific 架构图自动生成提供了完整的解决方案，对促进相关应用的发展具有重要意义。
...
> 本研究提出了一种混合方法，结合大型语言模型和三个新创建的词汇表，用于检测和解释英语、法语和希腊语中的仇恨言论，该方法在准确性和解释质量上都优于纯LLM基线方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有自动化仇恨言论检测系统主要关注审查或删除内容，缺乏透明度和解释性，限制了对内容有害性的说明机会，引发了透明度和言论自由方面的担忧。

**Method:** 采用混合架构：1) 使用精心策划的词汇表检测和消歧问题术语；2) 利用LLM作为上下文感知评估器来识别针对特定群体的内容；3) 将两个互补管道的输出融合为基础性解释，阐明内容被标记的原因。

**Result:** 人工评估表明，混合方法具有高准确性，提供高质量解释，在检测性能和解释质量方面均优于仅使用LLM的基线方法。

**Conclusion:** 通过将传统词汇表方法与现代LLM技术相结合，该系统不仅提高了仇恨言论检测的准确性，还增强了透明度和可解释性，为更负责任的在线内容管理提供了有效解决方案。
...
> 提出了IUQ框架，通过样本间一致性和样本内保真度来量化长文本LLM输出的不确定性，使用询问-响应范式提供声明级别的不确定性度量。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM不确定性量化方法主要适用于短文本或受限答案集，但现实应用需要长文本和自由文本生成。LLM在生成长文本时常出现语义连贯但事实不准确的问题，且语义复杂性和语言结构复杂化使不确定性量化变得困难。

**Method:** 引入疑问式不确定性量化(IUQ)框架，通过询问-响应范式，结合样本间一致性和样本内保真度两个维度来度量长文本LLM输出的不确定性，提供声明级别的不确定性评估和模型保真度度量。

**Result:** 在多个模型族和模型大小的实验表明，IUQ在两个广泛使用的长文本生成数据集上表现出优越性能，能够有效量化LLM输出的不确定性。

**Conclusion:** IUQ框架成功解决了长文本LLM生成中不确定性量化的挑战，通过创新的一致性和保真度评估方法，为实际应用提供了可靠的不确定性量化工具。
...
> 通过多评价者盲法评估发现，在解释CGM数据和糖尿病咨询准备方面，基于检索的大语言模型对话代理在质量、同理心和可操作性评分均优于临床医生，但仍不能作为自主治疗决策工具。

<details>
  <summary>Details</summary>

**Motivation:** 持续葡萄糖监测(CGM)是糖尿病护理的核心，但清晰且富有同理心地解释CGM模式非常耗时。LLM系统在CGM指导咨询中的证据仍然有限，需要评估其在帮助患者理解CGM数据和准备糖尿病常规就诊中的价值。

**Method:** 开发了基于检索的LLM对话代理用于CGM解释和糖尿病咨询支持。从公开数据集构建12个CGM知情案例，6名资深糖尿病临床医生各评审2个案例并回答24个问题。在盲法多评价者评估中，3名临床医生独立评价LLM生成和临床医生撰写的回复，评估6个质量维度，同时记录安全标志和感知来源标签。

**Result:** LLM对话代理获得比临床医生更高的质量评分(4.37 vs 3.58)，平均差异0.782点，在同理心(1.062)和可操作性(0.992)方面差异最大。安全标志分布相似，两组严重关切均罕见(各0.7%)。

**Conclusion:** 基于检索的LLM系统作为CGM审查、患者教育和就诊前准备的辅助工具具有价值，但研究结果不支持自主治疗决策或无监督的实际应用。
...
> DiscoTrace是一种识别回答策略的方法，研究发现人类社区回答策略多样而LLM缺乏修辞多样性且更倾向于全面性。

<details>
  <summary>Details</summary>

**Motivation:** 理解不同人类社区在回答问题时使用的修辞策略，评估大语言模型(LLM)是否能够模仿多样化的回答方式，以指导开发更具实用性的LLM问答系统。

**Method:** DiscoTrace将答案表示为一序列与问题相关的语篇行为配对问题解释，基于修辞结构理论解析进行分析，应用于九个人类社区和LLM的答案进行比较研究。

**Result:** 研究发现人类社区具有多样化的答案构建偏好，而LLM缺乏修辞多样性，即使被提示模仿特定人类社区指南时也是如此。LLM系统性选择广泛性回答，处理人类回答者选择不处理的问题解释。

**Conclusion:** 研究发现可以为开发考虑上下文和多种策略的实用LLM问答系统提供指导，强调了在问答中考虑修辞多样性的重要性。
...
> 本研究提出了QuantCode-Bench基准测试，用于评估大语言模型生成可执行算法交易策略的能力。该基准包含400个不同难度的任务，通过多阶段 pipeline 评估模型的代码生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在通用编程任务上表现优异，但在生成可执行的算法交易策略方面尚未得到充分探索。交易策略生成需要同时掌握金融领域逻辑、专业API知识，并能生成在历史数据上实际执行交易的代码。

**Method:** 构建包含400个任务的QuantCode-Bench基准，采用多阶段评估流程：语法正确性检查、回测执行验证、交易存在性检查，以及使用LLM裁判进行语义对齐评估。在单轮生成和多轮代理交互两种设置下对比最先进模型。

**Result:** 当前模型的主要限制不在于语法，而在于交易逻辑的正确实施、API的适当使用和对任务语义的遵守。研究发现交易策略生成为一类独特的领域特定代码生成任务。

**Conclusion:** 成功的交易策略生成不仅需要技术正确性，还需要自然语言描述、金融逻辑和策略可观察行为之间的一致性，这对大语言模型提出了新的挑战。
...
> 该论文提出了K-Token Merging框架，通过在潜在嵌入空间中将连续的K个token块压缩为单个嵌入，实现了最高75%的输入长度减少，同时保持模型性能基本不变。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型处理长提示词时存在计算和内存成本过高的主要原因是自注意力机制与输入长度呈二次方关系增长。现有的token压缩方法主要在token空间操作，忽略了潜在嵌入空间的低效性问题。

**Method:** K-Token Merging采用潜在空间压缩方法，使用轻量级编码器将每个连续的K个token嵌入块合并为单个嵌入。压缩后的序列由LoRA适配的LLM处理，而生成过程仍然使用原始词汇表。

**Result:** 在结构推理(Textualized Tree)、情感分类(Amazon Reviews)和代码编辑(CommitPackFT)等任务上，K-Token Merging位于性能与压缩率的帕累托前沿，在75%的输入长度减少下性能损失极小。

**Conclusion:** 该框架通过在潜在空间进行高效的token合并，有效解决了长文本处理中的计算效率问题，为实际应用中处理长输入提供了可行的解决方案。
...
> 大语言模型在机器翻译中存在过度生成现象，本文探讨了商业环境中检测和分类这些过度生成的方法及其结果。

<details>
  <summary>Details</summary>

**Motivation:** LLM作为翻译工具虽然表现优异，但其生成特性会导致过度生成问题，这些过度生成与神经机器翻译中的神经胡言乱语不同，包含自解释、虚构内容和适当解释等多种类型，亟需有效的检测和分类方法。

**Method:** 在商业环境中探索了多种策略来检测和确定过度生成的确切性质，包括对不同类型过度生成的分析和识别。

**Result:** 提出了针对LLM翻译中过度生成问题的解决方案，并通过实验验证了这些方法在商业应用中的有效性。

**Conclusion:** LLM翻译中的过度生成现象既有风险也有价值，适当的管理和检测策略可以使LLM更好地模拟人类翻译的行为，为目标受众提供更好的理解。
...
> MADE是一个面向医疗设备不良事件报告的多标签文本分类基准测试，通过实时更新数据防止训练污染，并系统评估了20多种模型的不确定性量化能力。

<details>
  <summary>Details</summary>

**Motivation:** 高风险领域（如医疗保健）的机器学习需要可靠的预测和不确定性量化，但现有的多标签文本分类基准存在训练数据污染问题，难以区分真实推理能力和记忆能力。

**Method:** 构建了基于医疗设备不良事件报告的动态MLTC基准MADE，采用长尾分布的层次标签、严格的时间分割，并系统评估了20多种编码器和解码器模型在微调和少样本设置下的表现及多种UQ方法。

**Result:** 小型判别式微调解码器在保持竞争力UQ的同时实现了最强的头尾准确率；生成式微调提供最可靠的UQ；大型推理模型提升罕见标签性能但UQ较弱；自语言化置信度不是可靠的不确定性代理指标。

**Conclusion:** MADE通过防止数据污染和严格评估，为医疗领域MLTC模型的不确定性量化研究提供了重要基准，揭示了模型性能与不确定性量化之间的权衡关系。
...
> SpecGuard是一个仅使用模型内部信号的验证感知投机解码框架，通过多候选采样和内部信号集成实现选择性计算分配，在提升推理准确性的同时减少了推理延迟。

<details>
  <summary>Details</summary>

**Motivation:** 现有投机解码方法采用token中心机制，错误步骤容易传播，而使用外部奖励模型的解决方案会引入额外延迟、计算开销并限制泛化能力。

**Method:** SpecGuard在每个步骤采样多个草稿候选，选择最一致步骤，然后使用两个轻量级内部信号（基于注意力的归因分数和基于对数概率的置信度分数）进行验证，根据信号决定接受或重新计算。

**Result:** 在多个推理基准测试中，SpecGuard将准确率提升3.6%，同时减少约11%的延迟，性能超越传统投机解码和奖励引导的投机解码方法。

**Conclusion:** SpecGuard证明了仅使用模型内部信号即可实现有效的步骤级验证，为高效准确的大语言模型推理提供了新的解决方案。
...
> QualiaNet是一个两阶段计算模型，模仿人类3D视觉：先通过经验模块提取视差图，再通过推理模块利用场景统计特性从视差梯度中恢复距离信息。

<details>
  <summary>Details</summary>

**Motivation:** 解释人类立体视觉的悖论现象：虽然立体视觉经验本身不提供距离信息，但它影响我们对视觉尺度的判断。这暗示存在一种将视差经验转化为距离推断的机制。

**Method:** 构建双阶段模型：1) 经验模块模拟人类立体经验，生成相对于注视点的视差图；2) 推理模块使用CNN网络，利用近距离场景产生明显视差梯度、远距离场景相对平坦的自然场景统计特性，从视差图中估计距离。

**Result:** 网络仅凭视差梯度就能成功恢复距离信息，验证了该方法的有效性，证实了视差梯度作为距离推断线索的可行性。

**Conclusion:** 人类3D视觉通过双阶段机制运作：立体经验提供视差信息，而推理系统利用自然场景统计规律（视差梯度与距离的关系）来推断3D场景属性，这解释了立体视觉经验如何影响距离认知。
...
> HY-World 2.0是一个先进的多模态世界模型框架，能够根据文本、图像或视频输入生成高保真的可导航3D场景，性能媲美闭源模型Marble。

<details>
  <summary>Details</summary>

**Motivation:** 在HY-World 1.0的基础上进一步提升世界模型的能力，支持更多样化的输入模态（文本、单视图/多视图图像、视频），实现更高质量的3D世界生成和重建。

**Method:** 采用四阶段方法：1) HY-Pano 2.0生成全景图；2) WorldNav进行轨迹规划；3) WorldStereo 2.0实现世界扩展；4) WorldMirror 2.0完成世界组合。同时引入WorldLens高性能渲染平台，支持交互式3D世界探索。

**Result:** 在多个基准测试中达到开源方法的SOTA性能，结果与闭源模型Marble相当。所有模型权重、代码和技术细节均已开源。

**Conclusion:** HY-World 2.0显著推动了多模态3D世界模型的发展，通过全景生成、场景理解、视图建模等技术创新实现了高质量3D场景生成，为3DAI研究提供了重要基础。
...
> 本文提出了一种从单张手绘草图生成几何一致的多视角场景的新方法，通过构建专用数据集、相机感知注意力适配器和稀疏对应监督损失，显著提升了生成质量和效率。

<details>
  <summary>Details</summary>

**Motivation:** 手绘草图是几何信息最匮乏的输入，仅通过抽象线条传达场景意图，同时引入的空间扭曲与任何一致的3D解释相冲突。现有方法需要照片或文本输入，而草图转3D方法需要多个视角或昂贵的逐场景优化，因此需要新的解决方案。

**Method:** 通过三个相互强化的贡献解决问题：(i) 构建约9k个草图到多视角样本的精选数据集；(ii) 并行相机感知注意力适配器(CA3)将几何归纳偏置注入视频变换器；(iii) 基于运动恢复结构重建的稀疏对应监督损失(CSL)。整个框架在单次去噪过程中合成所有视角。

**Result:** 该方法显著优于最先进的两阶段基线，在真实性(FID)上提升超过60%，几何一致性(Corr-Acc)提升23%，同时提供高达3.7倍的推理速度提升。

**Conclusion:** 这项研究首次实现了从单一手绘草图生成几何一致多视角场景，为草图驱动的3D内容创作提供了新的技术路径，在保证质量的同时大幅提高了效率。
...
> DharmaOCR是一对专门的结构化OCR小语言模型(Full 7B和Lite 3B)，通过结合监督微调和直接偏好优化显著降低了文本退化率(最高87.6%)，在保持高质量的同时实现了更低的推理成本。

<details>
  <summary>Details</summary>

**Motivation:** 传统的OCR系统在处理结构化文档时面临文本退化、生成不稳定和推理成本高的问题，缺乏统一的评估标准和方法来有效控制异常生成长行为。

**Method:** 创新性地将直接偏好优化(DPO)首次应用于OCR任务，使用退化文本作为负样本来惩罚循环行为，结合监督微调强制执行严格的JSON模式约束，并开发了DharmaOCR-Benchmark基准测试集。

**Result:** DharmaOCR Full和Lite模型在DharmaOCR-Benchmark上分别达到0.925和0.911的提取质量分数，退化率仅为0.40%和0.20%，通过AWQ量化实现了22%的成本降低，超越了所有开源和商业基线模型。

**Conclusion:** 该研究证明了文本退化不仅影响质量还会严重影响生产性能，提出的DPO+SFT方法有效解决了这一问题，为结构化OCR提供了一种高质量、低成本且稳定的解决方案。
...
> 本文提出一种基于姿态估计的混合方法，通过YOLO提取人体关键点，结合运动学特征和随机森林分类器，实现实时暴力抢劫检测，可在边缘设备部署运行。

<details>
  <summary>Details</summary>

**Motivation:** 非暴力街头抢劫（抢夺后逃跑）事件在监控视频中具有短暂、细微且与正常人类互动难以区分的特点，导致自动检测困难，需要开发实时、可解释的边缘部署解决方案。

**Method:** 采用YOLO姿态估计器提取每个人的身体关键点，计算运动学和交互特征（包括手部速度、手臂伸展、接近程度和相对运动），使用随机森林分类器进行分类，并通过时间滞后滤波器稳定帧级预测。

**Result:** 在模拟数据集和互联网视频测试集上验证了方法的泛化能力，在NVIDIA Jetson Nano上实现实时性能，支持主动式设备端抢劫检测的可行性。

**Conclusion:** 该混合方法结合了深度学习的感知能力和传统机器学习的可解释性，在实际部署中表现出良好的泛化性和实时性，为边缘设备上的抢劫检测提供了可行的技术方案。
...
> SatBLIP是一个卫星专用视觉语言框架，通过结合卫星图像语义理解与自然语言处理，预测县级社会脆弱性指数，提供可解释的农村环境风险评估。

<details>
  <summary>Details</summary>

**Motivation:** 传统脆弱性指数过于粗糙，无法提供深入的风险背景洞察。现有的遥感方法存在手工特征、人工审计和基于自然图像训练的视觉语言模型等局限性，迫切需要更精细化的农村环境风险评估工具。

**Method:** SatBLIP采用对比性图文对齐技术和针对卫星语义的引导式字幕生成。使用GPT-4o生成卫星瓦片的结构化描述，微调卫星适应性BLIP模型为新图像生成字幕，通过CLIP编码字幕，结合注意力机制融合LLP派生的嵌入，最终进行空间聚合下的SVI估计。

**Result:** 研究通过SHAP分析识别出关键驱动属性（屋顶形态/状况、街道宽度、植被、汽车/开放空间），这些属性稳定驱动预测结果，实现了农村风险环境的可解释性映射。

**Conclusion:** SatBLIP成功克服了传统方法的局限性，为农村环境风险评估提供了更精确、可解释的解决方案，显著提升了对农村环境风险空间模式的理解能力。
...
> FoodSense是一个用于跨感官推断的数据集，包含66,842个人工标注数据，使视觉语言模型能够从食物图像预测味觉、嗅觉、质地和声音，并提供解释。

<details>
  <summary>Details</summary>

**Motivation:** 人类能够从食物图像推断多种感官体验，但现有视觉语言研究仅关注识别任务，缺乏对多感官体验预测的研究。

**Method:** 构建大规模人工标注数据集，使用大语言模型生成视觉推理轨迹，训练FoodSense-VL模型进行多感官评分和解释生成。

**Result:** 提出了首个食物多感官推断数据集和基准模型，发现现有评估指标不足以衡量视觉感官推断任务。

**Conclusion:** 该工作将认知科学的跨感官感知发现与多模态模型指令调优相结合，为视觉感官推断研究开辟了新方向。
...
> 零消融实验高估了视觉Transformer中寄存器的重要性，当使用更合理的替代方案时，寄存器功能仍然能够保持。

<details>
  <summary>Details</summary>

**Motivation:** 零消融实验显示寄存器对视觉Transformer的功能至关重要，但这一结论可能过于绝对，需要验证是否存在更准确的评估方法。

**Method:** 通过三种替代控制实验（均值替代、噪声替代、跨图像寄存器洗牌）来对比零消融效果，并使用余弦相似度分析内部表示的真实变化。

**Result:** 替代方案能够保持性能（在基准的1pp范围内），而零消融导致过度扰动和性能大幅下降；寄存器主要起缓冲作用而非存储特定图像信息。

**Conclusion:** 寄存器的重要性被零消融夸大了，系统实际上只需要合理的类寄存器激活模式，而非精确的特定值；这一发现在ViT-B规模下得到验证。
...
> 本文提出了一种结合知识表示、自然语言处理和计算机视觉的图像标注方法，通过视觉属性约束和交互式众包框架来减少标注主观性问题。

<details>
  <summary>Details</summary>

**Motivation:** 对象识别数据集存在语义鸿沟问题，导致视觉数据与语言描述之间复杂的多对多映射，这种偏见对计算机视觉任务性能产生不利影响，需要减少标注者主观性。

**Method:** 设计了一个交互式众包框架，基于预定义的对象类别层次结构和标注者反馈动态提问，通过视觉属性引导图像标注过程，结合知识表示、NLP和CV技术。

**Result:** 实验证明了该方法的有效性，通过分析标注者反馈来优化众包设置，提高了标注质量。

**Conclusion:** 本研究成功减少了图像标注中的主观性偏差，为构建更高质量的对象识别数据集提供了有效解决方案。
...
> DAGMaN是一种结合注意力引导掩码和噪声教师的协同蒸馏框架，用于改进医学图像的自监督学习，特别适用于Swin Transformer架构。

<details>
  <summary>Details</summary>

**Motivation:** 传统的随机掩码方法在医学图像中效果较差，因为相邻图像块的上下文相似性导致信息泄露和SSL简化。Swin Transformer缺乏全局[CLS]标记，无法使用高级掩码方法。

**Method:** 提出注意力引导掩码机制，选择性地掩码语义共存和判别性图像块；首次在协同蒸馏框架中集成噪声教师，在执行注意力掩码的同时保持高注意力头多样性。

**Result:** 在多个医学图像任务上展示了有效性，包括全样本和少样本肺结节分类、免疫治疗结果预测、肿瘤分割和无监督器官聚类。

**Conclusion:** DAGMaN框架成功解决了医学图像自监督学习中的信息泄露问题和注意力头多样性降低问题，为医学图像分析提供了一个有效的预训练解决方案。
...
> 这篇论文提出了一种基于异质超图的视觉-语言推理框架H2VLR，用于解决少样本异常检测问题，通过高阶推理建模视觉-语义关系，在工业和医疗基准测试中达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于VLM的少样本异常检测方法主要采用成对特征匹配，忽略了结构依赖性和全局一致性，限制了检测性能的提升。

**Method:** 提出H2VLR框架，将少样本异常检测重新表述为视觉-语义关系的高阶推理问题，通过统一的超图联合建模视觉区域和语义概念。

**Result:** 实验验证了H2VLR的有效性和优势，在代表性的工业和医疗基准数据集上经常达到最先进的性能。

**Conclusion:** H2VLR框架通过超图建模高阶视觉-语义关系，成功改进了基于VLM的少样本异常检测性能，为该领域提供了新的解决思路。
...
> 针对全模态大语言模型存在单模态基线超越多模态推理的悖论问题，本文提出了链式模态(CoM)框架，通过动态编排融合拓扑和双路径执行机制，有效解决了静态融合带来的感知脆弱性问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有全模态大语言模型采用静态融合拓扑导致感知脆弱性，出现单模态性能超越多模态推理的反常现象，这主要源于序列输入的位置偏差和交错格式的对齐陷阱等结构性病理。

**Method:** 提出链式模态(CoM)框架：1)静态融合转为动态编排，自适应切换并行、序列和交错路径；2)双路径认知执行：直接感知的'Direct-Decide'路径和分析审核的'Reason-Decide'路径；3)支持免训练和数据高效的SFT两种模式。

**Result:** CoM在不同基准测试中实现了稳健且一致的泛化性能，有效解决了多模态融合中的结构性偏差问题，超越了现有的静态融合方法。

**Conclusion:** 通过将多模态融合从被动拼接转向动态编排，并采用任务对齐的双路径执行策略，CoM为解决全模态模型的感知脆弱性提供了有效方案，实现了更鲁棒的多模态推理能力。
...
> FreqTrack是一个频域感知的RGB-事件融合跟踪框架，通过频域变换和频谱增强变换器层实现更鲁棒的特征融合，在复杂场景下获得76.6%的领先精度。

<details>
  <summary>Details</summary>

**Motivation:** 现单模态RGB跟踪器在复杂动态场景中存在性能瓶颈，而现有RGB-事件融合方法主要在空间域设计，未能充分利用事件数据的独特时域响应和高频特性。

**Method:** 提出FreqTrack框架，设计频谱增强变换器(SET)层实现多头动态傅里叶滤波，开发小波边缘优化(WER)模块通过可学习小波变换显式提取事件数据的多尺度边缘结构。

**Result:** 在COESOT和FE108数据集上取得高度竞争力性能，特别是在COESOT基准测试中达到76.6%的领先精度。

**Conclusion:** 频域建模对RGB-事件跟踪有效，FreqTrack通过频率域变换和互补的跨模态相关性建立了更鲁棒的特征融合方法。
...
> 开发了一种基于智能手机相机和96孔板的低成本荧光检测系统，通过分析RGB颜色空间与荧光物质浓度的关系来检测微生物和分子。

<details>
  <summary>Details</summary>

**Motivation:** 传统微孔板读数机价格昂贵，需要昂贵的激发滤光片、阻挡滤光片和光电倍增管等元件。为了降低检测成本，研究人员利用智能手机普及性和高性价比的相机作为光学检测器。

**Method:** 设计了兼容标准96孔板的装置，使用智能手机相机作为光学探测器，通过建立样品图像RGB颜色空间与荧光样品摩尔浓度之间的关系来进行检测分析。

**Result:** 该系统能够有效检测稀释样品中的微生物和分子，相比Perkin Elmer Victor等传统读数机大大降低了设备成本，同时保持了检测功能。

**Conclusion:** 该研究为低成本荧光检测提供了可行的解决方案，智能手机相机的使用使得设备更加便携和经济，有望在资源有限的实验室环境中推广应用。
...
> WILD-SAM是一种针对缠绕InSAR干涉图的地面滑坡检测框架，通过相位感知混合专家适配器和小波引导子带增强策略有效解决了光谱域偏移问题，实现了高精度的滑坡边界识别。

<details>
  <summary>Details</summary>

**Motivation:** 直接从缠绕InSAR干涉图中检测缓慢移动的滑坡对地质灾害监测至关重要，但由于严重的相位模糊和复杂的相干噪声，以及SAM模型在自然图像和干涉相位数据之间的光谱域偏移问题，这一任务面临根本性挑战。

**Method:** 提出WILD-SAM框架，集成两个核心组件：1) 相位感知混合专家(PA-MoE)适配器，利用动态路由机制自适应聚合多尺度光谱-纹理先验；2) 小波引导子带增强(WGSE)策略，通过离散小波变换分离高频子带并生成频率感知密集提示，确保滑坡边界的拓扑完整性。

**Result:** 在ISSLIDE和ISSLIDE+基准测试上，WILD-SAM达到了最先进的性能，在目标完整性和轮廓保真度方面显著优于现有方法。

**Conclusion:** WILD-SAM成功地将SAM适配到缠绕相位数据处理中，有效解决了光谱域偏移问题，为高精度滑坡检测提供了新的解决方案，在地质灾害监测领域具有重要应用价值。
...
> 本文提出了一种用于前馈单图像3D头部头像重建的显式情感控制框架，将情感作为独立可控信号，通过双路径调制机制实现情感与身份的解耦控制。

<details>
  <summary>Details</summary>

**Motivation:** 现有3D头像重建方法中，情感通常隐式地与几何或外观纠缠在一起，无法独立控制和跨身份一致地操作情感表达，限制了头像的表现力和可扩展性。

**Method:** 提出双路径调制机制：1) 几何调制在原始参数空间进行情感条件归一化，解耦情感状态与语音驱动的表达；2) 外观调制捕获身份感知的、依赖情感的几何外视觉线索。同时构建了时间同步、情感一致的多身份数据集来支持学习。

**Result:** 集成到多个最先进骨干网络中，在保持重建和重演保真度的同时，实现了可控的情感转移、解耦操作和平滑情感插值。

**Conclusion:** 该框架将情感作为一等控制信号，显著推进了表现力丰富且可扩展的3D头部头像技术，为情感解耦的数字人应用提供了新思路。
...
> 本文提出了一种新颖的视频对象插入解决方案，通过集成多视图对象先验和双路径视图一致性条件机制，有效解决了动态环境中对象外观不一致、遮挡处理等关键挑战。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频生成方法主要专注于整个场景的合成，但在将对象插入现有视频时难以确保一致的对象外观、空间对齐和时间相干性，特别是在处理外观不一致和复杂遮挡问题时表现不佳。

**Method:** 通过将2D参考图像提升为多视图表示，利用双路径视图一致性条件机制确保稳定的身份引导；引入质量感知权重机制自适应处理噪声或不完美输入；设计集成感知一致性模块保证空间真实感，有效解决遮挡和边界伪影问题。

**Result:** 实验结果表明，提出的解决方案显著提高了视频对象插入的质量，提供了稳定且真实的集成效果，有效维持了跨帧的时间连续性。

**Conclusion:** 该方法成功解决了视频对象插入中的核心技术挑战，为动态环境中对象的逼真集成提供了有效框架，在保持视觉质量的同时实现了时空一致性。
...
> 本文介绍了NTIRE 2026图像超分辨率挑战赛，该比赛包含194名参与者，分为保真度和感知质量两个赛道，旨在评估和推动图像超分辨率技术的发展。

<details>
  <summary>Details</summary>

**Motivation:** 随着图像超分辨率目标的发展，需要同时评估像素级保真度和视觉真实感，通过竞赛形式统一基准，分析该领域的最新进展。

**Method:** 挑战赛设置两个赛道：1) 恢复赛道基于PSNR评估像素保真度；2) 感知赛道使用感知评分评估视觉真实感。双三次下采样×4生成低分辨率图像进行重建。

**Result:** 共194人注册，31个团队提交有效作品。挑战赛成功提供了统一的评估基准，展示了当前超分辨率技术的进展。

**Conclusion:** 该挑战赛为图像超分辨率领域提供了有价值的基准平台，揭示了当前技术进展并指明了未来发展方向。
...
> 论文提出了DVFace，一种用于视频人脸修复的单步扩散框架，通过时空双码本设计和非对称时空融合模块，实现了高质量、时序一致且身份保真的视频人脸修复效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频人脸修复方法面临两个关键限制：1) 过度依赖通用扩散先验导致人脸适应性不足；2) 多步采样过程推理效率低下。虽然单步扩散可提升效率，但如何在快速修复的同时保持人脸细节真实性和时序稳定性仍具挑战性。

**Method:** DVFace采用单步扩散架构，创新性地引入时空双码本设计分别提取退化视频中互补的空间和时间人脸先验，并提出非对称时空融合模块根据先验的不同作用方式将其注入扩散主干网络。

**Result:** 在多个基准测试上的评估表明，DVFace相比现有方法在修复质量、时序一致性和身份保持方面均表现优越。

**Conclusion:** DVFace成功解决了视频人脸修复中效率与质量的平衡问题，通过创新的时空先验机制实现了实时高质量的视频人脸修复，为该领域提供了有效的解决方案。
...
> SEPatch3D是一种通过动态调整块大小来加速基于ViT的稀疏多视图3D目标检测的框架，在保持精度的同时实现显著的推理加速。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于ViT的稀疏多视图3D目标检测器虽然准确率高，但由于大量的token处理导致推理延迟很高。现有的token压缩策略（如token剪枝、合并和块大小扩大）会丢失信息丰富的背景线索、破坏上下文一致性并丢失细粒度语义信息，对3D检测产生负面影响。

**Method:** 提出了SEPatch3D框架，包含三个核心技术：1) 时空感知块大小选择(SPSS)为包含近距离物体的场景分配小块以保持细节，为主导背景的场景分配大块以降低计算成本；2) 信息块选择(IPS)选择信息丰富的块进行特征细化；3) 跨粒度特征增强(CGFE)将细粒度细节注入选定的粗块中，丰富语义特征。

**Result:** 在nuScenes和Argoverse 2验证集上，SEPatch3D比StreamPETR基线实现高达57%的推理加速，比最先进的ToC3D-faster提升20%的效率，同时保持相当的检测精度。

**Conclusion:** SEPatch3D通过动态调整块大小并保持关键语义信息，有效解决了现有token压缩方法的局限性，为基于ViT的多视图3D目标检测提供了高效的加速方案，在速度和精度之间实现了良好的平衡。
...
> AVR是一种自适应视觉推理框架，通过动态选择最优推理格式，在保持准确率的同时将token使用量减少50-90%，有效解决了视觉推理模型中的过度思考问题。

<details>
  <summary>Details</summary>

**Motivation:** 视觉推理模型(VRMs)虽然展现了强大的跨模态推理能力，但普遍存在过度思考问题，对所有任务都生成过长的推理链。作者将此归因于视觉推理中的推理路径冗余，即许多视觉问题并不需要完整的推理过程。

**Method:** 提出了AVR自适应视觉推理框架，将视觉推理分解为视觉感知、逻辑推理和答案应用三个认知功能，使模型能够在完整格式、仅感知格式和直接答案三种响应格式间动态选择。采用FS-GRPO（群组相对策略优化的改进版本）进行训练，鼓励模型选择最有效的推理格式。

**Result:** 在多个视觉语言基准测试中，AVR在保持整体准确率的同时，将token使用量减少50-90%，在感知密集型任务中表现尤为突出。

**Conclusion:** 自适应视觉推理能够有效缓解VRMs中的过度思考问题，为提高视觉语言模型的推理效率提供了新的解决思路。
...
> 本文提出了一种基于注意力引导噪声学习(ANL)的深度伪造检测框架，利用预训练扩散模型的噪声特征来提高检测器对新伪造技术的泛化能力，显著提升了检测扩散模型生成的虚假图像的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 随着扩散模型等新型图像合成技术的出现，现有的基于GAN训练的深度伪造检测器面临严重的泛化问题。扩散模型生成的深度伪造图像高度逼真，常常能够逃避现有检测器的识别，因此需要新的方法来解决这一泛化挑战。

**Method:** 提出注意力引导噪声学习(ANL)框架，将预训练的扩散模型集成到深度伪造检测流程中。该方法利用扩散模型的去噪过程来暴露细微伪影：让检测器预测给定扩散步数下输入图像包含的噪声，迫使其捕获真实图像与合成图像之间的差异，同时引入基于预测噪声的注意力引导机制，鼓励模型关注全局分布的差异而非局部模式。

**Result:** 在多个基准测试上，ANL显著优于现有方法，在检测扩散生成的深度伪造图像方面达到了最先进的准确率。在不引入推理额外开销的情况下，大幅提升了泛化性能（如在未见模型上显著提高ACC/AP）。

**Conclusion:** 研究表明，扩散噪声为可泛化的深度伪造检测提供了强有力的信号。ANL框架通过利用冻结扩散模型学习到的自然图像分布，成为一种有效的正则化方法，为解决深度伪造检测的泛化问题提供了新思路。
...
> 本文提出了一种名为M3D-Net的多模态3D面部特征重构网络，通过融合RGB和3D重构特征，实现了更精准和鲁棒的deepfake检测。

<details>
  <summary>Details</summary>

**Motivation:** 深度学习在图像生成中的快速发展使得面部伪造技术达到了前所未有的真实度，现有的检测方法仅依赖孤立的面部属性重构，未能充分利用多模态特征表示的互补特性。

**Method:** 设计端到端双流架构，包含自监督3D面部重构模块从单视图RGB图像重构精细的面部几何和反射属性；使用3D特征预融合模块自适应调整多尺度特征；通过多模态融合模块有效整合RGB和3D重构特征。

**Result:** 在多个公共数据集上的广泛实验表明，该方法在检测准确性和鲁棒性方面达到了最先进的性能，显著优于现有方法，并在不同场景下展现出强大的泛化能力。

**Conclusion:** M3D-Net通过有效融合RGB和3D特征模态，为deepfake检测提供了新的解决方案，在多个指标上实现了突破性改进，为应对日益严峻的深度伪造威胁提供了有力工具。
...
> TurboTalk是一种两阶段渐进式蒸馏框架，能将多步音频驱动视频扩散模型压缩为单步生成器，推理速度提升120倍同时保持高质量生成。

<details>
  <summary>Details</summary>

**Motivation:** 现有音频驱动视频数字人生成模型依赖多步去噪，计算开销巨大，严重限制了在实际场景中的应用部署。虽然一步蒸馏方法可以显著加速推理，但往往存在训练不稳定的问题。

**Method:** 提出TurboTalk两阶段渐进式蒸馏框架：首先采用分布匹配蒸馏获得稳定强力的4步学生模型；然后通过对抗蒸馏逐步将去噪步数从4步减少到1步。为确保极端步数减少下的训练稳定性，引入渐进式时间步采样策略和自比较对抗目标，提供中间对抗参考来稳定渐进蒸馏过程。

**Result:** 实现了视频说话头像的单步生成，推理速度提升120倍，同时保持了高质量的生成效果。

**Conclusion:** TurboTalk成功解决了音频驱动视频生成模型从多步到单步蒸馏过程中的训练不稳定问题，为实时音频驱动视频数字人应用提供了可行的解决方案。
...
> MapSR是一种基于提示的土地覆盖超分辨率框架，通过提取基础模型特征实现高效的高分辨率映射，在无需HR标签的情况下达到竞争性性能，并大幅降低计算开销。

<details>
  <summary>Details</summary>

**Motivation:** 高分辨率土地覆盖制图面临的主要挑战是密集高分辨率标注的高成本问题，现有弱监督方法虽然能利用低分辨率标签，但需要大量计算资源重新训练预测器。

**Method:** MapSR提出了一种提示驱动的框架，通过轻量级线性探针从冻结的视觉基础模型特征中提取类别提示，然后使用无训练的度量推断和基于图的预测细化来获得高分辨率映射。具体包括通过余弦相似度匹配获得初始预测，再用基于图的传播进行空间细化和优化。

**Result:** 在Chesapeake Bay数据集上，MapSR在没有HR标签的情况下达到59.64% mIoU，性能与最强的弱监督基线相当并超越全监督基线，同时将可训练参数减少四个数量级，训练时间从几小时缩短到几分钟。

**Conclusion:** MapSR成功实现了监督与模型训练的解耦，在有限的标注和计算预算下实现了可扩展的高分辨率土地覆盖制图，为弱监督语义分割提供了新的解决思路。
...
> 本文提出了一种名为掩码Logit推动(MLN)的图像编辑方法，能够在视觉自回归模型中实现基于文本提示的精确图像编辑，在保持无关区域不变的同时编辑目标区域，并在多个基准测试中取得最佳性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图像编辑方法要么编辑精度不够，要么速度缓慢，尤其在保持图像非编辑区域完整性方面存在挑战。视觉自回归模型虽然在重建方面表现良好，但在精确编辑能力上仍有不足。

**Method:** 提出掩码Logit推动方法，利用源图像的token映射作为引导，将目标提示下的模型预测与源token映射对齐；通过交叉注意力差异获得空间掩码，仅在需要编辑的区域应用修改；引入量化误差校正机制提高重建质量。

**Result:** 在PIE基准测试的512px和1024px分辨率下取得最佳编辑性能；在COCO和OpenImages数据集上的重建质量超越之前的方法；相比扩散模型具有可比或更好的性能，同时速度更快。

**Conclusion:** MLN方法成功整合了视觉自回归模型的图像编辑能力，实现了高精度、高效率的提示引导图像编辑，为自回归模型在图像编辑任务中的应用开辟了新途径。
...
> 提出GIST，一个无需训练、保持身份的图像合成器，解决图形设计中多模态组件视觉不匹配问题，可插入现有设计流程中提升视觉和谐性。

<details>
  <summary>Details</summary>

**Motivation:** 现有图形设计方法假设输入组件风格和谐，但实际中来源不同的组件常存在视觉不匹配，限制了设计质量。

**Method:** GIST作为独立的身份保持图像合成器，位于布局预测和排版生成之间，可无缝插入现有设计流水线，无需修改原有方法。

**Result:** 将GIST集成到LaDeCo和Design-o-meter两个不同方法中，显著提升了视觉和谐性和美学质量，LLaVA-OV和GPT-4V评估支持这一结论。

**Conclusion:** 身份保持的风格化和合成是组件到设计流水线的关键要素，GIST为现有图形设计方法提供了轻量级且有效的改进方案。
...
> 本文提出了一种基于高阶RWKV架构和三令牌提示机制的多粒度感知语义原型扫描范式，用于全色锐化任务，通过语义驱动的扫描和可逆特征变换实现了优异的性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的RWKV模型在双向往栅式扫描中存在语义无关性和位置偏差问题，无法充分利用图像的语义信息进行上下文感知的全局交互，影响全色锐化效果。

**Method:** 1) 多粒度感知语义原型扫描：利用局部敏感哈希将语义相关区域分组，构建多粒度语义原型，实现上下文感知的令牌重排序；2) 三令牌提示学习：设计包含全局令牌、聚类原型令牌和可学习寄存器令牌的提示机制；3) 可逆Q-Shift：通过中心差分卷积注入高频信息，引入可逆多尺度Q-Shift操作。

**Result:** 实验结果表明，所提出的方法在全色锐化任务上展现出优越的性能，有效提升了图像质量。

**Conclusion:** 该工作通过结合语义感知扫描、多令牌提示机制和可逆特征变换，为全色锐化提供了一个高效且性能优异的解决方案，克服了传统RWKV模型的局限性。
...
> Switch-KD是一个视觉切换知识蒸馏框架，通过将学生模型的视觉输出切换到教师模型的语言路径中，在共享的文本概率空间内统一视觉-语言知识转移，使0.5B的小模型有效蒸馏3B大模型的多模态知识。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉-语言模型知识蒸馏方法面临模态特定监督的挑战，虽然多模态知识在语言空间中融合，但当前方法分别监督每个模态，没有明确解决多模态对齐问题，导致多模态知识转移不一致。

**Method:** Switch-KD框架包含两个关键组件：(1)视觉切换蒸馏，将学生模型的视觉输出切换到教师模型的语言路径，构建跨模态概率参考；(2)动态双向logits差分损失(DBiLD)，通过双向监督自适应对齐信息概率区域，同时保持师生模型的分布结构。

**Result:** 在Switch-KD指导下，0.5B的TinyLLaVA模型从其3B教师模型有效蒸馏了丰富的多模态知识，在10个多模态基准测试上平均提升了3.6分，且无需任何架构修改。

**Conclusion:** Switch-KD通过在共享文本概率空间中统一视觉-语言知识转移，成功解决了多模态知识蒸馏中的对齐不一致问题，为资源受限场景下的VLM部署提供了有效的解决方案。
...
> 提出跨模态token调制方法，通过建立外观和运动模态token间的密集连接和关系Transformer块，实现高效的信息传播，在无监督视频目标分割任务上达到最佳性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的双流架构虽然集成了外观和运动线索，但未能充分利用这两种互补信息源，缺乏有效的相互依赖建模机制。

**Method:** 引入跨模态token调制技术，在两种模态的token之间建立密集连接，通过关系Transformer块实现模态内和跨模态的信息高效传播，并采用token掩码策略提高学习效率。

**Result:** 在所有公开基准测试中实现了最先进的性能，超越了现有方法。

**Conclusion:** 该方法有效加强了外观与运动线索的交互作用，通过创新的token调制机制显著提升了无监督视频目标分割的效果，证明了跨模态信息融合的重要性。
...
> 本文提出了一种完整的模数HDR成像系统，通过创新的传感 formulation和无迭代解缠算法，实现了1000 FPS的全彩色HDR成像，同时显著降低了数据带宽需求。

<details>
  <summary>Details</summary>

**Motivation:** 传统RGB HDR成像面临多重曝光运动伪影与单次拍摄信息损失的权衡，现有模数解决方案受限于迭代解缠开销和硬件约束，只能实现低速灰度采集。

**Method:** 采用曝光解耦的模数成像 formulation 配合基于扩散生成先验的无迭代解缠算法，并基于模数编码脉冲流实现了硬件原型系统。

**Result:** 实现了1000 FPS全彩色HDR成像，将输出数据带宽从约20 Gbps降至6 Gbps，在动态场景中验证了系统的可行性。

**Conclusion:** 通过协同推进传感 formulation和解缠算法，成功克服了系统瓶颈，证明了模数成像在动态场景部署的可行性。
...
> FogFool是一种基于雾气生成的对抗攻击框架，通过优化大气模式的Perlin噪声生成物理可信的对抗样本，在遥感图像分类中实现了优秀的白盒攻击效果、黑盒迁移性和防御鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有对抗攻击方法主要依赖像素级扰动，未能充分利用遥感影像的大气特征，且在实际图像退化下表现不佳。需要开发一种更符合物理规律、更具隐蔽性和鲁棒性的对抗攻击方法。

**Method:** 提出FogFool框架，通过迭代优化基于Perlin噪声的大气模式来生成雾基扰动。利用自然不规则结构建模雾气形成过程，将对抗信息嵌入到大气现象的空间相干性和中低频特征中。

**Result:** 在两个基准遥感数据集上，FogFool在白盒设置下表现优越，黑盒迁移性达到83.74% TASR，对JPEG压缩和滤波等预处理防御展现强鲁棒性。

**Conclusion:** FogFool代表了遥感分类系统面临的一种实用、隐蔽且高度持久的威胁，为复杂环境下的模型可靠性评估提供了强有力的基准测试工具。
...
> 该论文提出了一种基于混沌特征变换的方法，通过在分类层前应用非线性变换来增强CNN在有限训练数据下的性能，无需增加模型复杂度。

<details>
  <summary>Details</summary>

**Motivation:** 卷积神经网络(CNNs)在有限训练数据场景下经常表现出较差的泛化能力，这是由于过拟合和特征多样性不足造成的。

**Method:** 提出了一种简单有效的基于混沌的特征变换方法，在分类层之前对归一化特征向量应用逻辑映射、斜帐篷映射和正弦映射的非线性变换，从而重塑特征空间并改善类别可分性。

**Result:** 在MNIST、Fashion-MNIST和CIFAR-10数据集上，使用不同深度的CNN架构在有限数据条件下进行评估。结果显示：MNIST上最大性能提升5.43%（斜帐篷映射，3层CNN，每类40个样本）；Fashion-MNIST上提升9.11%（正弦映射，3层CNN，每类50个样本）；CIFAR-10上提升7.47%（斜帐篷映射，每类200个样本）。

**Conclusion:** 该方法计算效率高，无需额外的可训练参数，可以轻松集成到现有CNN架构中，是数据稀缺图像分类任务的实用解决方案。不同混沌映射的一致性改进表明性能提升是由混沌系统的非线性和动力特性驱动的。
...
> 本文提出Seen-to-Scene框架，结合传播式和生成式方法，通过流引导传播和参考潜在传播实现高效、时空一致的视频扩展。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于生成模型的视频扩展方法存在时空建模隐式和空间上下文有限的缺陷，导致帧内和帧间不一致，尤其在动态场景和大范围扩展时问题突出。

**Method:** 提出Seen-to-Scene统一框架，集成了基于流的传播与视频修复预训练的网络，采用端到端微调来弥合域间差距；引入参考引导的潜在传播机制，提高传播效率和可靠性。

**Result:** 实验表明该方法在时间一致性和视觉真实感方面表现优异，推理效率高，超越了需要输入特异调整的先前最先进方法。

**Conclusion:** Seen-to-Scene成功统一了传播式和生成式方法的优势，为视频外绘提供了更可靠的解决方案，在保持时空一致性的同时实现了高效推理。
...
> 本文提出DETR-ViP框架，解决视觉提示目标检测中缺乏全局判别性的问题，通过全局提示集成和视觉-文本提示关系蒸馏等技术，显著提升了视觉提示检测的性能。

<details>
  <summary>Details</summary>

**Motivation:** 视觉提示目标检测能够实现交互式和灵活的目标类别定义，便于开放词汇检测。然而，该领域研究被长期忽视，通常仅作为文本提示检测器的副产品，导致其性能次优。

**Method:** DETR-ViP框架在基础图像-文本对比学习基础上，引入全局提示集成和视觉-文本提示关系蒸馏来学习更具判别性的提示表示，并采用选择性融合策略确保检测的稳定性和鲁棒性。

**Result:** 在COCO、LVIS、ODinW和Roboflow100数据集上的大量实验表明，DETR-ViP在视觉提示检测方面显著优于其他最先进方法。消融研究进一步验证了所提改进的有效性。

**Conclusion:** 通过解决视觉提示缺乏全局判别性的根本问题，DETR-ViP成功释放了视觉提示检测的潜力，为开放词汇目标检测提供了一个更有效的解决方案。
...
> Chain-of-Glimpse是一个搜索引导的渐进式对象定位推理框架，通过将推理步骤明确锚定到具体视觉证据区域，实现了可组合的多步视频理解决策。

<details>
  <summary>Details</summary>

**Motivation:** 现有与对象无关的视频理解方法无法有效处理随时间发生的显著对象变化，需要一种能够显式锚定任务相关视觉对象并实现组合推理的新方法。

**Method:** 该方法将视频推理构建为逐步过程，通过强化学习优化的搜索引导控制器迭代定位视觉证据区域，建立围绕任务相关对象的空间定位轨迹，使用格式奖励激励定位能力。

**Result:** 在NExTQA（领域内）和Video-Holmes、CG-Bench Reasoning、VRBench（领域外）等基准测试中显示出一致的性能提升，证明了方法的鲁棒性和泛化能力。

**Conclusion:** Chain-of-Glimpse通过显式的视觉证据定位和多步推理轨迹构建，有效减轻了对显著性线索的过度依赖，在各种视频推理任务中展现出准确性和可解释性。
...
> 提出了一种法庭式审判框架，将图像篡改定位任务视为证据对抗后的判断过程，通过指控流、辩护流和法官模型的协作来实现更可靠的篡改区域检测。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图像篡改定位方法将真实性信息仅作为辅助训练信号使用，缺乏对篡改和真实证据的显式比较，当篡改痕迹微弱或被后处理和噪声退化时，难以在模糊区域做出可靠预测。

**Method:** 构建基于共享多尺度编码器的双假设分割架构，包含指控篡改的指控流和主张真实的辩护流。通过边缘先验引导，采用级联多级融合、双向分歧抑制和动态辩论细化产生证据。开发强化学习法官模型，对不确定区域进行策略性重新推理和细化，使用基于优势的奖励和soft-IoU目标训练，通过熵和跨假设一致性校准可靠性。

**Result:** 实验结果表明该方法在平均性能上优于最先进的图像篡改定位方法。

**Conclusion:** 法庭式审判框架通过显式建模篡改与真实证据的对抗关系，有效提升了在模糊区域的篡改定位可靠性，为图像取证领域提供了新的解决思路。
...
> NG-GS是一个用于3D高斯分布(3DGS)中高质量物体分割的新框架，通过解决边界离散化问题实现更好的分割效果。

<details>
  <summary>Details</summary>

**Motivation:** 3D高斯分布在新颖视图合成方面表现出色，但其离散表示特性导致物体边界存在混叠和伪影，使得准确分割物体面临挑战。

**Method:** 首先通过掩膜方差分析自动识别边界处的高斯分布，然后应用径向基函数插值构建空间连续特征场，结合多分辨率哈希编码实现高效多尺度表示。最后通过联合优化策略，利用对齐损失和空间连续性损失将3DGS与轻量级NeRF模块对齐。

**Result:** 在NVOS、LERF-OVS和ScanNet基准测试上的大量实验表明，该方法达到了最先进的性能，在边界mIoU指标上有显著提升。

**Conclusion:** NG-GS成功解决了3DGS中物体边界的离散化问题，通过构建连续特征场和联合优化策略，实现了高质量且一致的物体分割边界。
...
> G-MIXER是一种无需训练的零样本组合图像检索方法，通过测地线混合技术扩展隐式语义并利用MLLM重排序，提高了检索的多样性和准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的零样本组合图像检索方法过度依赖文本模态，无法捕捉需要考虑候选图像多样组合的模糊检索特性，导致检索结果的多样性和准确性降低。

**Method:** G-MIXER首先通过测地线混合技术在一系列混合比例上构建反映参考图像-文本对隐式语义的组合查询特征，建立多样化的候选集合；然后利用多模态大语言模型生成的显式语义对候选集合进行重新排序。

**Result:** G-MIXER在多个零样本组合图像检索基准测试中达到了最先进的性能，能够在不需要额外训练的情况下有效处理隐式和显式语义。

**Conclusion:** 该方法通过隐式语义扩展和显式语义重排序的双重策略，成功地解决了现有方法在检索多样性和准确性方面的限制，为零样本组合图像检索提供了一个有效的解决方案。
...
> 本文提出了一种名为MS-SSE-Net的深度学习框架，用于结构损伤分类，在准确率上达到了99.26%，相比于基线模型有显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 由于损伤模式变化多样和环境条件的影响，从图像中准确识别不同类型的结构损伤仍然具有挑战性，这影响了土木基础设施的安全性和可靠性维护。

**Method:** 基于DenseNet201骨干网络，集成了新颖的多尺度特征提取与通道和空间注意力机制。具体采用并行深度可分离卷积提取局部和上下文特征，结合挤压激励风格的通道注意力和空间注意力来突出信息丰富区域并抑制噪声干扰。

**Result:** 在StructDamage数据集上的实验表明，MS-SSE-Net达到了99.31%精确度、99.25%召回率、99.27% F1分数和99.26%准确率，显著优于基线DenseNet201模型的性能。

**Conclusion:** MS-SSE-Net通过多尺度特征提取和注意力机制的有效结合，为结构损伤检测提供了一种高性能的解决方案，有助于提升土木基础设施的安全监测能力。
...
> 基于几何驱动的肌管综合数据训练轻量级3D U-Net，实现了在真实显微图像中高效的实例分割，显著优于现有零样本模型。

<details>
  <summary>Details</summary>

**Motivation:** 肌管作为研究肌肉生理学、疾病机制和药物反应的关键模型系统，需要精确的三维实例分割来获得直径、长度和分支程度等定量形态学读出数据。然而，由于缺乏大规模标注的肌管数据集，现有的预训练生物医学分割模型难以推广到这一领域。

**Method:** 提出了一种几何驱动的综合合成管线：通过多项式中心线、局部变化半径、分支结构和从真实显微镜观察中推导的椭球形端盖来建模个体肌管；使用CycleGAN域自适应技术渲染具有真实噪声和光学伪影的综合体积；采用紧凑型3D U-Net架构，通过自监督编码器预训练，仅在合成数据上训练。

**Result:** 在真实数据上实现了0.22的平均IPQ（实例分割质量指标），显著优于三个已确立的零样本分割模型，证明了生物物理驱动的合成方法能够在标注稀缺的生物医学领域中实现有效的实例分割。

**Conclusion:** 研究表明，基于真实生物物理特性的合成数据生成策略可以有效解决生物医学影像分析中的标注稀缺问题，为肌管及其他类似结构的定量分析提供了可行的解决方案。
...
> HAMSA是一种无需扫描的视觉状态空间模型，通过在频谱域直接操作，简化了参数化并引入了自适应频谱调制机制，在ImageNet-1K上达到85.7%的准确率，同时实现了更快的推理速度和更低的内存/能耗。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉状态空间模型（如Vim、VMamba、SiMBA）依赖复杂的扫描策略来处理2D图像，导致计算开销增大和架构复杂度高，需要一种更简单高效的替代方案。

**Method:** HAMSA在频谱域直接操作，包含三个创新：(1)简化的核参数化-使用单个高斯初始化的复数核替代传统的(A,B,C)矩阵；(2)频谱脉冲网(SPN)-输入相关的频率门控机制；(3)频谱自适应门控单元(SAGU)-基于幅度的门控。利用基于FFT的卷积实现O(L log L)复杂度。

**Result:** 在ImageNet-1K上达到85.7% top-1准确率（SSM中最优），推理速度比transformers快2.2倍（4.2ms vs 9.2ms），比基于扫描的SSM快1.4-1.9倍，内存使用更少（2.1GB vs 3.2-4.5GB），能耗更低（12.5J vs 18-25J）。

**Conclusion:** HAMSA通过消除顺序扫描并在频谱域直接操作，实现了更简单高效的架构，在准确率、速度、内存和能耗方面均优于现有方法，在迁移学习和密集预测任务中也表现出良好的泛化能力。
...
> 本文提出人脸识别系统本质上可用于检测变形攻击，并引入新的评估阈值来限制系统对变形攻击的脆弱性。

<details>
  <summary>Details</summary>

**Motivation:** 变形攻击是人脸识别面临的重大挑战，当前解决方案存在性能与安全性权衡问题。作者认为人脸识别和差分变形攻击检测执行的任务本质相似，这为改进检测方法提供了新思路。

**Method:** 通过比较人脸识别系统与两种现有差分变形攻击检测方法，论证二者的相似性。同时分析当前决策阈值导致系统脆弱性的原因，并提出利用现有FR系统进行变形检测，引入新的评估阈值方法。

**Result:** 研究证明了FR系统与D-MAD任务的相似性，揭示了当前阈值设定导致脆弱性的机制，并提出的新阈值方法能够保证对变形攻击（包括未知类型）的脆弱性上限。

**Conclusion:** 通过重新审视人脸识别与变形攻击检测的关系，可以有效提升系统安全性。利用现有FR基础设施配合新阈值策略，是一种实用且有效的变形攻击防护方法。
...
> 本文提出了一种基于Sylvester形式的结果矩阵求解器，用于加速非线性最小二乘姿态估计问题，在保持精度的同时显著提升了计算效率。

<details>
  <summary>Details</summary>

**Motivation:** 实时计算机视觉应用中，非线性最小二乘姿态估计（旋转和平移）是一个耗时但基础的问题。现有的封闭形式求解器虽然能降低计算时间，但仍需进一步优化以满足实时性需求。

**Method:** 提出了一种新的基于结果矩阵的求解器类方法，利用Sylvester形式来降低求解复杂度。该方法通过适当的旋转参数化，将优化问题转化为多项式方程组系统求解。

**Result:** 所提出的方法在数值精度上与现有最先进求解器相当，但在计算时间上表现更优。成功应用于3D到3D对应和3D到2D对应两种不同类型的姿态估计问题。

**Conclusion:** 基于Sylvester形式的结果矩阵求解器为实时姿态估计提供了一个高效且准确的解决方案，在保持精度的同时显著降低了计算复杂度，具有良好的应用前景。
...
> ASGNet是一种新型的自适应频谱指导网络，通过整合频域特征和全局属性来解决结肠镜图像中息肉分割的空间感知偏差问题，在5个基准数据集上超越了21个现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 早期识别和切除息肉可降低结直肠癌风险，但息肉形态多样、背景复杂且 often 隐藏，使得息肉分割极具挑战性。现有深度学习方法存在空间感知偏差，主要受空间域相邻像素强相关性影响，难以捕获完整息肉结构。

**Method:** 提出ASGNet网络，包含三个核心组件：1)频谱指导非局部感知模块，联合聚合局部和全局信息；2)多源语义提取器，整合高层语义信息辅助息肉初步定位；3)密集跨层交互解码器，有效融合不同层级的多样化信息。

**Result:** 在5个广泛使用的息肉分割基准上，ASGNet定性和定量结果均显著优于21个最先进方法，证明了其在息肉结构判别力和边界精化方面的优越性。

**Conclusion:** 通过频域特征与空间特征的有机结合，ASGNet成功克服了传统方法的空间感知局限，能够生成高质量表示以实现精准的息肉分割，为临床辅助诊断提供了有效工具。
...
> 提出了OmniGCD，一种模态无关的广义类别发现方法，通过合成数据训练一次即可在多种模态上实现零样本类别发现，在16个数据集上显著提升性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有GCD方法需要针对特定数据集进行微调且仅适用单一模态，无法模拟人类跨模态的抽象类别学习能力，缺乏通用性和可扩展性。

**Method:** OmniGCD使用模态特定编码器处理输入，通过降维构建GCD潜在空间，并利用合成数据训练的Transformer模型在测试时将表示转换为更适合聚类的形式，实现零样本跨模态类别发现。

**Result:** 在视觉、文本、音频和遥感四种模态的16个数据集上，已知类和新类的分类精度分别提升6.2%、17.9%、1.5%和12.7个百分点，证明了方法的有效性和通用性。

**Conclusion:** 该方法实现了表示学习与类别发现的解耦，为模态无关GCD建立了基准，为人类启发的可扩展类别发现奠定了基础，推动编码器开发独立于GCD任务。
...
> 针对非对称视觉语言模型在持续视觉问答中的灾难性遗忘问题，提出非对称信息掩码(AIM)方法，通过基于模态特定敏感度的目标掩码来平衡稳定性与可塑性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的持续学习方法主要针对对称、单模态架构设计，但现代视觉语言模型具有固有的非对称结构，导致在连续数据流学习时容易出现灾难性遗忘，特别是关键视觉投影层容易受到干扰，严重影响组合推理能力。

**Method:** 提出非对称信息掩码(AIM)方法，根据模态特定的敏感度应用目标掩码，解决标准全局正则化在非对称VLM中偏向大规模语言解码器的问题，保护较小但关键的视觉投影层。

**Result:** 在VQA v2和GQA数据集的持续VQA设置下，AIM在平均性能和平均遗忘指标上达到最先进水平，同时更好地保持对新颖技能-概念组合的泛化能力。

**Conclusion:** 该方法有效解决了非对称视觉语言模型在持续学习中的灾难性遗忘问题，为多模态模型的持续学习提供了新的解决方案。
...
> 本研究提出一个模块化的铁路环境障碍物检测框架，结合对象检测、轨道分割和LiDAR单目深度估计三个神经网络，在合成数据集上实现了0.63米的平均绝对误差距离估计。

<details>
  <summary>Details</summary>

**Motivation:** 现有铁路环境障碍物检测系统大多只关注单一功能（检测或轨道识别），缺乏完整的模块化系统；同时由于缺乏真实数据集，系统评估困难，难以进行定量比较。

**Method:** 构建模块化框架，集成三个神经网络分别处理对象检测、轨道分割和单目深度估计，结合LiDAR点云数据；使用合成数据集SynDRA提供准确的真实标注进行评估。

**Result:** 系统通过融合单目深度图和LiDAR数据，实现平均绝对误差低至0.63米的距离估计，不仅提供精确的障碍物距离，还实现了场景的空间感知能力。

**Conclusion:** 该模块化框架有效解决了铁路环境中障碍物检测与距离估计的综合问题，为铁路安全提供了可靠的解决方案，且基于合成数据集的评估方法为该领域研究提供了新的评估标准。
...
> 提出一种组合式方法，从单张图像构建完整的3D头部虚拟形象，通过显式解耦头发和面部，采用不同的变形范式，实现更逼真的头发动画效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的一体化单图像方法在动画过程中常产生不真实的头发动态，主要原因是头发与面部区域解耦不充分，导致几何纠缠和不自然的变形。

**Method:** 首先对正面肖像进行头发去除获得光头图像，将原始图像和光头图像都提升为密集的3D高斯散点表示。面部3DGS通过非刚性注册绑定到FLAME网格；头发部分使用语义标签监督和边界感知重分配策略提取干净的头发高斯集，并引入笼式结构支持基于位置的动力学模拟来控制头发变形。

**Result:** 在多种头部运动、重力和表情变化下的动态动画中展现了更真实的头发行为，同时 faithfully 保留了面部细节，在感知真实度方面优于最先进的单图像方法。

**Conclusion:** 通过头发与面部的显式解耦和各自的变形控制方法，结合图像到3D提升技术保持纹理细节，成功解决了单图像3D头部重构中头发动画不真实的核心问题。
...
> 本文提出了Petro-SAM，一个基于SAM的两阶段多任务框架，可实现高质量的岩石薄片颗粒边缘分割和岩性语义分割的联合任务。

<details>
  <summary>Details</summary>

**Motivation:** 传统的颗粒边缘分割(GES)和岩性语义分割(LSS)任务通常独立处理，尽管使用了昂贵耗时且需专家标注的数据集，分割质量仍不理想。直接应用Segment Anything Model面临领域差异和多角图像堆栈联合学习两大挑战。

**Method:** 提出Petro-SAM框架：1) 基于SAM引入Merge Block来整合七个偏振视图，解决消光问题；2) 引入多尺度特征融合和颜色-熵先验来优化检测结果；采用两阶段多任务架构实现联合学习。

**Result:** 该方法能够在岩石薄片图像上实现高质量的颗粒边缘分割和岩性语义分割联合任务，有效解决了消光相关的颜色变化和超细颗粒边界等挑战。

**Conclusion:** 通过创新的多视图融合、特征融合和先验利用技术，Petro-SAM成功将通用分割模型SAM适应到岩石图像分析领域，实现了两个关键任务的统一高质量分割。
...
> NTIRE 2026视频显著性预测挑战赛概述，发布了包含2000个视频的新数据集，吸引了20多个团队参与，最终7个团队通过代码审查。

<details>
  <summary>Details</summary>

**Motivation:** 推动视频显著性预测技术的发展，通过组织挑战赛促进 researchers 开发更准确的显著性图自动预测方法。

**Method:** 准备包含2000个多样化视频的新数据集，使用众包鼠标跟踪收集注视点和显著性图数据，采用公认的评估指标在800个测试视频上进行评估。

**Result:** 超过20个团队提交了方案，其中7个团队通过了包含代码审查的最终阶段，所有数据已在GitHub上公开发布。

**Conclusion:** 该挑战赛成功建立了视频显著性预测的新基准，提供了宝贵的数据资源，为该领域的研究发展做出了贡献。
...
> 该研究提出了改进的多尺度结构映射方法MSSM+，结合超顶点视觉Transformer(SV-ViT)，通过整合皮层厚度、灰白质对比度、脑沟深度和皮层曲率等多种MRI特征，显著提高了阿尔茨海默病的检测准确率。

<details>
  <summary>Details</summary>

**Motivation:** 阿尔茨海默病的确诊依赖PET或脑脊液分析，成本高且具有侵入性。现有的结构MRI生物标志物如皮层厚度在AD筛查中存在局限性，需要开发更准确和非侵入性的检测方法。

**Method:** 提出MSSM+方法，在原有MSSM基础上增加了脑沟深度和皮层曲率特征；设计了表面超顶点映射(SSVM)将皮质表面划分为超顶点区域；开发了超顶点视觉Transformer(SV-ViT)架构，在超顶点上进行解剖学感知的学习。

**Result:** MSSM+相比MSSM识别出更广泛且统计学上更显著的AD与认知正常组间差异；在AD分类任务中，精确率-召回率曲线下面积比MSSM提高3%；跨设备分析显示信号变异性降低，分类性能一致性提升。

**Conclusion:** MSSM+结合SV-ViT是一种有前景的基于MRI的AD检测影像标志物，能够在CSF/PET确诊前提供更准确的非侵入性筛查手段，且在不同MRI设备制造商间具有较好的泛化性能。
...
> Paza是一个零样本零售盗窃检测框架，通过分层架构（低成本模型持续监控+昂贵VLM按需调用）实现无需训练的盗窃检测，成本比商业方案低3-10倍。

<details>
  <summary>Details</summary>

**Motivation:** 零售盗窃每年造成全球经济超过1000亿美元损失，但现有AI检测系统需要昂贵的定制模型训练，每店每月收费200-500美元，成本高昂限制了普及。

**Method:** 采用分层架构：廉价的目标检测和姿态估计持续运行，多信号怀疑预过滤器（停留时间+行为信号）触发时才调用昂贵的视觉语言模型VLM，将VLM调用频率降低240倍至每分钟≤10次，使单GPU可服务10-20家店。架构模型无关，支持OpenAI兼容端点。

**Result:** 在DCSASS数据集上达到89.5%精度、92.8%特异性、59.3%召回率（召回率差距源于稀疏帧采样）。成本模型显示每店每月50-100美元可行性，比商业方案便宜3-10倍。

**Conclusion:** Paza证明了零样本零售盗窃检测的可行性和经济性，通过创新的架构设计实现了实用的部署成本，为零售业提供了普及型AI安全监控解决方案。
...
> IAC-LTH是一种神经架构搜索加速框架，通过观察早期训练中操作选择的稳定性，利用Jensen-Shannon散度准则动态修剪低重要性操作，将自适应跳跃模块搜索时间减少3.7-16倍，同时保持或略微提升分割性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统自适应跳跃模块的神经架构搜索计算成本高昂，原始IAC框架仍需要200轮的微分搜索，限制了在实际医疗图像分割任务中的应用。

**Method:** 分析IAC细胞内操作和边的时间行为，发现最终选中的操作在训练早期就出现且架构参数提前稳定；基于此提出Jensen-Shannon散度稳定性准则，跟踪每条边操作重要性分布并在搜索过程中逐步修剪低重要性操作，形成IAC-LTH加速框架。

**Result:** 在ACDC、BraTS、KiTS、AMOS四个公开数据集上，IAC-LTH发现的自适应细胞性能匹配或略微超过原始完整搜索结果，同时将NAS计算成本降低3.7-16倍，结果在不同架构、数据集和训练设置下保持一致。

**Conclusion:** 通过早期稳定的操作就能识别出竞争力的IAC架构，无需完整搜索，使得在现实计算约束下设计自适应跳跃模块在医疗图像分割中更加实用化。
...
> MetaDent是一个包含60,669张牙科图像的综合资源，提供了半结构化标注框架和基准测试套件，评估结果显示现有视觉语言模型在口腔摄影细粒度理解方面表现不佳。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型在医学图像分析中展现出巨大潜力，但由于缺乏细粒度标注数据集和综合基准，其在口腔摄影领域的应用仍未充分探索。

**Method:** 构建了大规模牙科图像数据集，设计了半结构化标注框架，结合高级图像摘要和逐点自由文本异常描述，利用大语言模型生成约15K VQA对和18类多标签分类基准数据。

**Result:** 最先进的VLM在VQA、分类和图像描述任务上仅表现中等准确率，图像描述结果存在不一致或不完整的问题，显示模型在口腔场景细粒度理解方面仍有局限。

**Conclusion:** 公开发布数据集、标注和工具将促进可重复研究，加速牙科应用视觉语言系统发展，为解决现有模型在口腔摄影理解方面的不足奠定基础。
...
> 本文提出了一种基于深度度量学习的开放集静脉识别方法，通过学习判别性嵌入和原型匹配，实现了新用户无需重新训练的自适应注册，在大规模数据集上达到99.45%的OSCR。

<details>
  <summary>Details</summary>

**Motivation:** 现有静脉识别方法大多依赖闭集分类，限制了系统的可扩展性，无法自适应注册新用户而无需完整模型重训练。

**Method:** 采用深度度量学习学习L2归一化嵌入，结合基于原型的匹配和校准的相似度阈值来区分注册用户和未知冒充者；在三重损失目标和1-NN分类器基础上构建框架。

**Result:** 在MMCBNU 6000数据集上实现OSCR 0.9945、AUROC 0.9974、EER 1.57%，识别准确率99.6% Rank-1；跨数据集实验显示模型对域 shift敏感但在大规模数据下表现鲁棒。

**Conclusion:** 所提方法在开放集约束下实现了高性能静脉识别，三重损失结合1-NN分类器提供了准确性和效率的最佳平衡，支持实时部署。
...
> FSDETR是一种基于RT-DETR的频率-空间特征增强框架，通过多层次的注意力机制和跨域特征融合，有效解决了小目标检测中的特征退化、遮挡和背景干扰问题，在仅14.7M参数量下取得了优异的小目标检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 小目标检测面临三大核心挑战：下采样导致的特征退化、密集场景中的相互遮挡以及复杂背景的干扰。现有方法难以同时解决这些问题，需要一种综合性的解决方案来提升小目标的检测精度。

**Method:** FSDETR框架包含三个核心组件：1) 空间分层注意力块(SHAB)用于捕获局部细节和全局依赖关系；2) 基于可变形注意力的尺度内特征交互(DA-AIFI)通过动态采样聚焦信息区域以缓解遮挡问题；3) 频率-空间特征金字塔网络(FSFPN)结合频率滤波和空间边缘提取来保留细粒度细节。

**Result:** 实验结果表明，FSDETR在参数量仅为14.7M的情况下，在VisDrone 2019数据集上达到13.9%的APS，在TinyPerson上达到48.95%的AP50 tiny，在小目标检测基准上展现了强大的性能。

**Conclusion:** 该研究通过频率和空间域的协同建模机制有效提升了小目标检测性能，证明了轻量级架构结合精心设计的特征增强策略能够在小目标检测任务上取得显著效果，为解决小目标检测难题提供了新的思路。
...
> RATS是一个轻量级框架，通过引入奖励感知门控和轨迹整形机制，使少步生成器能够突破教师模型的上限，实现基于奖励的持续优化，显著提升了少步视觉生成的效率-质量权衡。

<details>
  <summary>Details</summary>

**Motivation:** 现有的少步生成方法严重依赖蒸馏框架，学生模型被严格限制在模仿更强的多步教师模型，导致教师性能成为学生无法突破的上限。论文认为引入偏好对齐意识可以让学生模型优化到奖励偏好的生成质量，从而有可能超越教师。

**Method:** 提出奖励感知轨迹整形(RATS)框架：1) 通过地平线匹配在关键去噪阶段对齐师生潜在轨迹；2) 引入奖励感知门控机制，根据相对奖励表现自适应调节教师指导强度；3) 当教师奖励更高时强化轨迹整形，当学生匹配或超越教师时放松约束，实现持续的奖励驱动改进。

**Result:** 实验结果表明，RATS显著改善了少步视觉生成的效率-质量权衡，大幅缩小了少步学生模型与更强多步生成器之间的差距，且不增加额外的测试时间计算开销。

**Conclusion:** RATS通过无缝整合轨迹蒸馏、奖励感知门控和偏好对齐，有效地从高步生成器中转移偏好相关知识，打破了传统蒸馏框架中教师作为性能上限的限制，为实现高质量的少步生成提供了新的解决思路。
...
> 本文揭示了先进文本到3D生成模型存在'潜在陷阱'问题，即模型在某些区域对文本提示不敏感，并提出了解耦几何表示能力和语言敏感性的新框架。

<details>
  <summary>Details</summary>

**Motivation:** 文本驱动的生成模型反演是2D/3D内容操作的核心技术，但研究发现最先进的文本到3D生成模型经常对自然语言提示失去敏感性，限制了基于文本的编辑等应用。

**Method:** 通过分析生成模型的采样轨迹，识别出潜在'陷阱陷阱'问题，并利用模型的无条件生成先验，解耦几何表示能力与语言敏感性，绕过潜在陷阱。

**Result:** 提出的框架能够实现分布外3D形状的高保真语义操作，克服了当前3D流水线的局限性，同时保持了对复杂几何的表示能力。

**Conclusion:** 该研究表明模型几何表达能力强但语言敏感性有限，通过解耦这两个方面可以构建更鲁棒的基于文本的3D形状编辑系统。
...
> 提出一种混合高斯-哈希网格辐射表示方法，通过频率分解和稀疏修剪实现用更少的高斯基元重建高质量2D高斯场景模型。

<details>
  <summary>Details</summary>

**Motivation:** 解决基于NeRF模型中几何与外观的纠缠问题，以及高频纹理补偿几何误差的问题，同时提高渲染效率。

**Method:** 1. 结合高斯基元的潜在特征和哈希网格特征进行频率分解 2. 使用硬不透明度衰减加强几何与外观分离 3. 采用概率修剪和BCE损失实现高斯基元的稀疏化。

**Result:** 在合成和真实数据集上，相比当前最先进的高斯基元新视角合成方法，实现了更高的重建保真度，同时使用的基元数量减少了一个数量级。

**Conclusion:** 通过显式的频率分解和稀疏化策略，该方法有效解决了几何与外观分离问题，显著减少了所需的高斯基元数量，同时提高了重建质量和渲染效率。
...
> 本文提出了一种基于Transformer的条件生成管道，用于增强骨骼动作识别数据，能够在低数据情况下生成高保真度的骨骼序列，提升识别性能。

<details>
  <summary>Details</summary>

**Motivation:** 大规模、多样化且标注良好的3D骨骼数据集收集成本高昂且费时费力，限制了骨骼动作识别的发展。现有方法在小样本场景下性能不佳，亟需有效的数据增强技术。

**Method:** 设计了基于Transformer的编码器-解码器架构，结合生成式细化模块和dropout机制，在动作标签约束下学习真实骨骼序列的分布，实现高保真度和多样性平衡的样本生成。

**Result:** 在HumanAct12和NTU-VIBE数据集上，该方法在不同骨骼动作识别模型上持续提升准确率，在少样本和全数据场景下均表现优异，展现了强大的泛化能力。

**Conclusion:** 所提出的条件生成数据增强方法有效解决了骨骼动作识别中数据稀缺问题，为低资源场景下的动作理解提供了可靠解决方案，具有良好的实用价值和应用前景。
...
> RaTA-Tool是一个开放世界多模态工具选择框架，通过将多模态查询转换为结构化任务描述并检索匹配的工具，实现了对新工具的零样本泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有工具学习方法主要局限于纯文本输入和封闭世界设定，无法处理多模态用户指令，也无法泛化到训练中未见过的工具，这限制了AI系统在实际复杂任务中的应用。

**Method:** 提出基于检索的方法：1) MLLM将多模态查询转换为结构化任务描述；2) 通过语义匹配检索最合适的工具；3) 使用DPO进行偏好优化以提升对齐效果；4) 构建了源自Hugging Face模型卡的标准工具描述数据集。

**Result:** 实验表明该方法在工具选择性能上显著优于基线方法，特别是在开放世界和多模态场景下表现突出，且支持无需重新训练即可扩展到新工具。

**Conclusion:** RaTA-Tool为开放世界多模态工具使用提供了有效解决方案，通过检索机制和偏好优化解决了现有方法的局限性，为AI系统更好地利用外部工具资源开辟了新途径。
...
> 本研究利用图像到视频的生成AI技术，通过少量人类样本生成真实的手势数据集，证明了合成手势在视觉保真度和数据多样性方面可与真实手势媲美，并能显著提升下游任务的性能。

<details>
  <summary>Details</summary>

**Motivation:** 手势识别研究面临严重的数据稀缺问题，传统的人工录制或图像处理方法成本高昂且无法产生真实的手势变异性，限制了该领域的发展。

**Method:** 提出了一个基于提示的视频生成管道，通过收集少量人类手势样本，利用图像到视频基础模型生成真实的指示手势数据集，并构建了真实与合成数据的混合数据集进行评估。

**Result:** 合成手势在视觉保真度上与真实手势高度一致，同时引入了有意义的变化性和新颖性；在各种深度模型使用混合数据集时表现出优越性能。

**Conclusion:** 图像到视频技术虽然处于早期阶段，但已展现出强大的零样本手势合成能力，对下游任务具有明确的益处，为解决手势数据稀缺问题提供了新的可行方案。
...
> FEDSNet通过结合频域和空域特征的 dual-subspace 方法，在少样本细粒度图像分类中实现了优异性能和鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于度量学习的方法仅使用空间域特征，存在纹理偏置问题，容易将结构细节与高频背景噪声混淆，在少样本条件下容易出现过拟合导致结构不稳定。

**Method:** 提出了Frequency-Enhanced Dual-Subspace Network，利用DCT和低通滤波分离低频全局结构成分，通过截断SVD构建空间纹理和频域结构的独立低秩线性子空间，设计自适应门控机制动态融合双视图投影距离。

**Result:** 在CUB-200-2011、Stanford Cars、Stanford Dogs和FGVC-Aircraft四个基准数据集上表现出色，与现有度量学习算法相比具有高度竞争力。

**Conclusion:** FEDSNet在准确性和计算效率之间实现了良好平衡，为少样本细粒度视觉识别提供了有效的新范式。
...
> UniDoc-RL是一个统一的强化学习框架，通过分层决策过程实现视觉信息获取，从粗粒度文档检索逐步细化到细粒度图像选择和区域裁剪，在多模态推理任务上显著超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉RAG系统依赖于通用检索信号，忽视了复杂推理所需的细粒度视觉语义信息，导致在精细视觉理解任务中表现不佳。

**Method:** 提出将视觉信息获取公式化为分层动作空间的序列决策问题，通过强化学习联合执行检索、重排序、主动视觉感知和推理；引入密集多奖励机制为每个动作提供任务感知监督；基于GRPO算法对齐多目标行为；构建包含细粒度动作标注的高质量推理轨迹数据集。

**Result:** 在三个基准测试中，UniDoc-RL持续超越最先进基线方法，相比之前的RL方法获得高达17.7%的性能提升。

**Conclusion:** 通过统一强化学习框架和分层决策机制，UniDoc-RL有效解决了视觉RAG系统中细粒度语义理解不足的问题，为复杂视觉推理任务提供了新的解决方案。
...
> Flow of Truth是首个针对图像转视频(I2V)生成的时间取证框架，通过追踪像素运动轨迹来检测视频内容真实性，解决了传统二维取证方法无法处理视频时序演化的问题。

<details>
  <summary>Details</summary>

**Motivation:** 随着I2V技术的快速发展，从单张图像生成逼真视频变得越来越容易，但这也带来了新的取证挑战。传统图像取证方法无法处理视频中像素随时间演化的特性，因为视频中的伪造痕迹会随着帧的推进而漂移和变形。

**Method:** 将视频生成重新定义为像素随时间运动而非帧的合成，提出可学习的取证模板来追踪像素运动，并通过模板引导的流模块将运动与图像内容分离，实现鲁棒的时间追踪。

**Result:** 实验表明Flow of Truth框架能够泛化到商业和开源的I2V模型上，显著提升了时间取证性能。

**Conclusion:** 该研究开创性地解决了I2V生成的时间取证问题，为数字视频内容的真实性验证提供了新的技术路径，对维护数字媒体安全具有重要意义。
...
> QuAD是一个质量感知的近重复图像校准框架，通过聚合同一图像的多个在线版本来提高AI生成内容检测的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法只分析单个图像实例，但网络传播中同一图像会因重复处理（压缩、裁剪等）产生多个质量不同的近重复版本，导致检测结果不一致，影响实际应用的可靠性。

**Method:** QuAD框架首先检索图像的在线近重复版本，使用检测器对每个版本进行评分，然后基于估计的图像质量进行加权聚合，最终做出综合判断。

**Result:** 在构建的AncesTree（13.6万张实验室图像）和ReWIND（近1万张真实网络图像）数据集上，QuAD相比简单平均方法获得了约8%的平衡准确率提升，对多种检测器均有改进。

**Conclusion:** 联合处理网络上的所有可用图像版本是实现可靠AI生成内容检测的关键，质量感知的融合策略能有效提升实际应用中的检测性能。
...
> 本文从信号处理角度综述了隐式神经表示(INRs)的发展，分析了从基础坐标网络到高级设计的演进，并探讨了其在逆向问题、压缩和3D场景表示等领域的应用。

<details>
  <summary>Details</summary>

**Motivation:** INRs标志着信号建模的根本性转变，从离散采样数据转向连续函数表示。这种统一的函数框架能够处理多种信号类型，并通过自动微分实现精确的信号操作，比传统离散近似方法更加准确和灵活。

**Method:** 文章系统分析了INRs的演化路径：(1)从标准坐标网络的低频谱偏差开始；(2)发展到使用周期性、局部性和自适应函数的新型激活函数来重塑逼近空间；(3)引入层次化分解和哈希网格编码等结构化表示来提高空间适应性和计算效率。

**Result:** INRs在医学成像、雷达成像等逆向问题、数据压缩以及3D场景表示等多个领域展现出强大的应用价值。通过将INRs解释为学习到的信号模型，其逼近空间能够自适应地匹配底层数据特性。

**Conclusion:** 文章阐明了INRs领域的核心概念发展，并指出了在理论稳定性、权重空间可解释性和大规模泛化等方面的开放挑战，为未来研究提供了清晰的指导方向。
...
> 该论文提出了一种CNN-Attention混合框架，用于在大规模纵向研究中实现鲁棒的、站点不变的结构化MRI质量评估。

<details>
  <summary>Details</summary>

**Motivation:** 结构化MRI中的运动伪影严重影响临床诊断和自动化分析，而人工质量控制无法满足大规模纵向研究的可扩展性需求，需要开发自动化的、跨站点的质量评估方法。

**Method:** 构建集成分层2D CNN编码器和多头交叉注意力机制的混合框架，其中CNN提取局部空间特征，注意力机制建模全局依赖，优先关注运动相关伪影特征，同时过滤站点特异性强度变化和背景噪声。

**Result:** 在MR-ART数据集上训练后，模型在已知站点上达到0.9920的准确率和0.9919的F1分数，在17个异构站点的ABIDE数据集上无需重新训练即保持0.755的准确率。

**Conclusion:** 基于注意力的特征重加权成功捕获了通用伪影描述符，证明该框架在不同成像环境和扫描仪制造商之间具有很强的泛化能力和鲁棒性。
...
> HELP是一种基于热力图引导的嵌入学习框架，通过选择性保留前景区域的位置信息来抑制背景噪声，实现了59.4%的参数减少和检测器层级的显著降低。

<details>
  <summary>Details</summary>

**Motivation:** 基于Transformer的检测器在小目标检测领域虽有进展，但仍存在效率低下和易受背景诱导查询噪声影响的问题，这促使研究者需要深度解码器来优化低质量查询。

**Method:** 提出HELP框架，包含热力图引导的位置嵌入(HPE)机制；在编码器和解码器中集成HPE，通过基于梯度的掩码滤波器过滤背景主导嵌入；引入线性蛇形卷积解决复杂小目标的特征稀疏问题。

**Result:** 将解码器层数从8层减少到3层，实现59.4%的参数减少(66.3M vs 163M)，在减少计算预算的同时在各个基准测试中保持一致的精度提升。

**Conclusion:** HELP通过噪声感知的位置-语义融合机制，有效解决了Transformer检测器的背景噪声问题，在大幅降低参数量和计算复杂度的同时保持了检测性能，为小目标检测提供了高效的解决方案。
...
> 本文提出了第一个专门针对雾霾和弱光条件下建筑提取的光学遥感基准数据集HaLoBuilding，以及相应的端到端框架HaLoBuild-Net。

<details>
  <summary>Details</summary>

**Motivation:** 现有的光学遥感建筑提取方法在真实世界的雾霾和弱光条件下性能下降，而传统的光学方法和基准主要关注理想晴朗天气条件。虽然SAR能全天候感知，但其侧视几何会产生几何畸变。

**Method:** 提出HaLoBuilding基准数据集，采用同场景多时间对配对策略确保像素级标签对齐。基于此数据集开发HaLoBuild-Net框架，包含：1) 空间-频率聚焦模块(SFFM)耦合大感受野注意力和频率感知通道重加权；2) 全局多尺度引导模块(GMGM)提供全局语义约束；3) 相互引导融合模块(MGFM)实现双向语义-空间校准。

**Result:** 在HaLoBuilding数据集上，HaLoBuild-Net显著优于现有方法和传统级联恢复-分割范式，同时在WHU、INRIA和LoveDA数据集上保持良好的泛化性能。

**Conclusion:** 该工作为恶劣天气条件下的遥感建筑提取提供了新的基准数据和解决方案，有效提升了在雾霾和弱光环境下的建筑提取性能。
...
> 本文提出STFER框架，利用大型视觉语言模型生成身份一致性文本来增强任意条件下的行人重识别性能，特别是在跨模态和服装变化场景中表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 现有任意时间行人重识别方法过度依赖纯视觉特征，在光照变化导致的跨模态转换（如昼夜）和服装变化场景中性能显著下降，需要更鲁棒的身份识别特征。

**Method:** 提出语义驱动的令牌过滤和专家路由（STFER）框架：1）通过指令引导LVLM生成包含生物特征常量的身份内在语义文本；2）使用语义文本驱动视觉令牌过滤（SVTF）增强信息区域并抑制背景噪声；3）实现语义驱动的专家路由（SER）整合语义文本进行多场景门控。

**Result:** 在AT-USTC数据集上达到最先进性能，同时在5个广泛使用的ReID基准测试中展现出优秀的泛化能力和极具竞争力的结果。

**Conclusion:** 通过利用LVLM生成的身份一致性语义文本，STFER框架有效解决了跨模态和服装变化场景下的特征鲁棒性问题，为任意时间行人重识别提供了新的解决方案。
...
> LAMAE是一种针对多视图医学影像设计的潜在注意力掩码自编码器，通过在潜在空间中进行跨视图信息交换，实现了更鲁棒的心脏表征学习，并在儿童数据集上展现了良好的迁移能力。

<details>
  <summary>Details</summary>

**Motivation:** 超声心动图具有稀疏和异质性的时空视图特征，现有的掩码自编码器方法无法捕获心脏固有的多视图结构，限制了其在心脏功能表征中的应用效果。

**Method:** 提出LAMAE架构，在标准MAE基础上增加潜在注意力模块，实现帧间和视图间的直接信息交换；在MIMIC-IV-ECHO大规模数据集上进行预训练；首次实现从该数据集视频预测ICD-10编码。

**Result:** 在MIMIC-IV-ECHO数据集上实现了ICD-10代码预测的首次结果；成人数据学习到的表征在儿科队列中有效迁移，尽管解剖结构存在显著差异。

**Conclusion:** 引入多视图注意力等结构先验能够产生更加鲁棒和可迁移的表征，证明了结构化先验在医学影像Foundation模型中的重要性。
...
> PIE-V是一个心理学启发的视频错误注入框架，用于构建和评估包含错误和恢复的以人为中心的程序性视频，支持程序错误检测和校正的验证。

<details>
  <summary>Details</summary>

**Motivation:** 现有的程序性数据集在错误和校正轨迹方面提供有限且不一致的信息，而在以人为中心的视频中，错误通常被手部遮挡并通过细微的对象状态变化来揭示，使得可靠的程序监测变得困难。

**Method:** PIE-V框架结合了心理学启发的错误规划器、校正行为模型、级联一致性的LLM重写器、验证程序连贯性的LLM判断器以及文本引导的视频生成和剪辑技术来构建错误感知的程序性视频。

**Result:** 在17个任务和50个Ego-Exo4D场景中，PIE-V注入了102个错误并生成了27个恢复校正，建立了包含九个指标的统一评估体系来审计现有资源并与基线方法进行对比。

**Conclusion:** 该框架和评估方案为以人为中心的程序性错误检测和校正提供了事后验证支持，填补了现有数据集在错误和恢复表示方面的不足。
...
> 提出了一种核化Volterra神经网络(kVNN)，通过可学习的多核表示建模不同阶数的交互组合，实现了与标准卷积核兼容的高效高阶特征学习，在降低模型复杂度的同时保持或提升性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统深层学习模型在利用组合特征进行高阶学习时，会显著增加模型复杂度。现有方法难以有效平衡表达能力和计算成本，需要一种既能捕捉高阶交互又保持计算效率的新方法。

**Method:** kVNN采用可学习的多核表示，每个层包含并行分支的不同阶数多项式核组件。通过紧凑可学习的中心点建模不同交互阶数，实现阶数自适应参数化，可直接替换标准卷积核。

**Result:** 在视频动作识别和图像去噪任务上，kVNN展现出优异的性能-效率权衡：一致性地降低了模型参数和计算复杂度(GFLOPs)，即使在无大规模预训练的情况下也能获得竞争性甚至更好的性能。

**Conclusion:** 结构化核化高阶层为现代深度网络平衡表达能力和计算成本提供了实用路径，证明了通过精心设计的高阶交互建模可以在降低复杂度的同时保持或提升模型性能。
...
> DAMP是一种基于投影的单次、闭式权重编辑方法，通过移除特定类别的方向信息实现真正的类别遗忘，相比现有方法在保护保留类别性能的同时更接近重新训练的黄金标准。

<details>
  <summary>Details</summary>

**Motivation:** 现有类别遗忘方法存在虚假遗忘问题，遗忘信息仍可能保留在内部表示中，或者仅仅依赖分类器头部的抑制而非真正的表示移除，表现出弱或负面的选择性。

**Method:** DAMP在每个网络层级计算类原型，提取遗忘类别相对于保留类别的残差方向，通过投影更新减少对流这些方向的敏感性，并使用基于探针可分性的无参数深度感知缩放规则。

**Result:** 在MNIST、CIFAR-10、CIFAR-100和Tiny ImageNet以及卷积和Transformer架构上，DAMP比现有方法更接近重新训练标准，提高了选择性遗忘效果，更好保持保留类别性能，并减少了深层中残遗的类别结构。

**Conclusion:** DAMP提供了一种高效、无需梯度优化的类别遗忘解决方案，实现了真正的表示级遗忘而非表面遗忘，为机器遗忘领域树立了新的性能基准。
...
> 本文提出了两种对比策略来解决光照相关图像恢复问题：专门的DINOLight框架和通用的OmniLight方法，后者采用了新颖的WD-MoE技术，两种方法在NTIRE 2026挑战赛的三个光照赛道中均取得了顶级排名。

<details>
  <summary>Details</summary>

**Motivation:** 不利的光照条件（如投射阴影和不规则照明）会显著降低计算机视觉系统的性能， degrade图像可见性和颜色保真度。虽然现有方法在特定基准测试上表现优异，但实际应用需要能够处理多样化领域的鲁棒模型。

**Method:** 研究采用两种 Contrastive 策略：1）利用DINOLight作为专门的基线框架，充分利用每个数据集的独立特征；2）提出OmniLight统一方案，集成创新的WD-MoE（小波域混合专家）技术，在跨数据集训练中实现更好的泛化能力。

**Result:** 两种方法在NTIRE 2026挑战赛的三个光照相关赛道中均获得顶级排名，证明了在感知质量和泛化能力方面的卓越表现，为光照恢复提供了专门的和统一的两种有效解决方案。

**Conclusion:** 通过对比专门化与统一化架构在光照图像恢复中的差异，研究得出数据分布对模型性能的重要影响。提出的DINOLight和OmniLight分别为特定领域和跨领域应用提供了有效选择，为实际应用场景的模型选择提供了指导。
...
> 本文发现在扩散模型中强制执行Fokker-Planck方程并不总能提升生成质量，提出了轻量级正则化方法以低计算成本获得相同效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的扩散模型训练方法常常违反Fokker-Planck方程，直接惩罚这些偏差虽然能减少违反程度但带来巨大计算开销，且严格遵守FP方程不一定改善生成质量。

**Method:** 通过经验分析多种轻量级正则化器，研究它们对FP残差和生成质量的影响，对比不同正则化策略的效果和计算成本。

**Result:** 证明了轻量级正则化方法能够以更低的计算成本获得与原始FP正则化相似的好处，在减少FP方程违反的同时保持生成质量。

**Conclusion:** FP正则化的优势可通过更简单的罚项在显著降低的计算成本下实现，为扩散模型训练提供了更高效的优化策略。
...
> B-ACT是一个基于边界中心主动学习的时序动作分割框架，通过智能选择和标注动作边界帧，在稀疏标注预算下显著提升分割性能。

<details>
  <summary>Details</summary>

**Motivation:** 时序动作分割需要密集的时间标注，但大部分标注成本都花费在识别和优化动作转换边界上。这些边界区域是分割错误的集中点，微小的时间偏移会不成比例地降低分割指标，因此需要更高效的标注策略。

**Method:** B-ACT采用分层两阶段循环：首先使用预测不确定性对未标注视频进行排序和查询；然后在每个选定视频中，从当前模型预测中检测候选转换边界，通过融合邻域不确定性、类别模糊度和时间预测动态的新颖边界评分选择top-K边界。标注协议仅请求边界帧标签，但仍以边界为中心的片段进行训练。

**Result:** 在GTEA、50Salads和Breakfast数据集上的大量实验表明，边界中心监督具有很强的标签效率，在稀疏预算下始终超越代表性TAS主动学习基线和之前的最先进方法，在边界放置主导编辑和重叠F1分数的数据集上获得最大收益。

**Conclusion:** 通过将标注资源集中在高杠杆的边界区域，B-ACT框架实现了显著的标注效率提升，为时序动作分割提供了一种实用的主动学习解决方案，特别适用于边界准确性要求高的应用场景。
...
> 该论文提出了一个框架，通过帕累托配置优化自动寻找视觉token剪枝的最优配置，实验证明该方法能有效逼近经验帕累托前沿并优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言模型中的视觉token剪枝方法依赖于预定义的剪枝配置，无法确定它们是否达到了计算性能的最优平衡点。

**Method:** 提出将视觉token剪枝表述为帕累托配置优化问题，采用连续松弛和直通估计器实现基于梯度的搜索，并通过增广拉格朗日方法求解。同时使用可学习的核函数研究分层剪枝模式。

**Result:** 在8个视觉基准测试上的广泛实验表明，该方法能有效逼近网格搜索获得的经验帕累托前沿，并能在各种剪枝方法和VLM架构上良好泛化。多步渐进式剪枝相比单层方法实现了更好的精度-效率权衡。

**Conclusion:** 该框架通过优化搜索自动找到最优剪枝配置，揭示了VLMs的层次化压缩结构，为视觉语言模型的高效计算提供了新的解决方案。
...
> 本文提出了一种新颖的分层次时空向量量化框架，用于无监督骨骼动作时序分割，通过多层次聚类联合恢复骨架和时序信息。

<details>
  <summary>Details</summary>

**Motivation:** 无监督骨骼动作时序分割面临挑战，现有方法主要依赖空间信息，难以有效捕获动作的时空特征和复杂层级结构。

**Method:** 设计分层向量量化框架：下层将骨架与细粒度子动作关联，上层将子动作聚合成动作级表示；进一步扩展为时空方案，通过多级聚类同时恢复骨架和时间戳信息。

**Result:** 在HuGaDB、LARa和BABEL等多个基准数据集上达到SOTA性能，有效减少了分割长度偏差。

**Conclusion:** 分层时空向量量化框架能够有效利用时空信息，显著提升无监督骨骼动作分割的准确性和鲁棒性。
...
> StreamCacheVGGT是一个无需训练的视频流3D重建框架，通过跨层一致评分和混合缓存压缩策略，在恒定内存下实现了更准确稳定的密集3D几何重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有O(1)框架采用'纯驱逐'范式，由于二元令牌删除和局部单层评分的评估噪声，存在严重的信息破坏问题，影响视频流中密集3D几何重建的稳定性。

**Method:** 提出两个协同模块：Cross-Layer Consistency-Enhanced Scoring (CLCES)通过跟踪Transformer层次结构中令牌重要性轨迹来缓解激活噪声；Hybrid Cache Compression (HCC)引入三层分诊策略，通过关键向量流形上的最近邻分配将中等重要令牌合并到保留的锚点中，超越简单驱逐模式。

**Result:** 在7-Scenes、NRGBD、ETH3D、Bonn和KITTI五个基准测试中，StreamCacheVGGT实现了最先进的性能，提供了卓越的重建精度和长期稳定性，同时严格遵守恒定成本约束。

**Conclusion:** 该框架通过创新的缓存管理机制，有效解决了视频流3D重建中内存约束与信息保持的平衡问题，为实时应用提供了新的解决方案。
...
> TokenGS通过直接回归3D高斯均值坐标而非沿相机射线的深度，采用编码器-解码器架构，实现了更鲁棒的前馈3D高斯泼溅预测，取得了SOTA的静态和动态场景重建性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于Transformer的前馈3D高斯泼溅方法通常沿相机射线回归高斯均值作为深度值，这种方式存在局限性；同时标准编码器设计的预测原语数量受输入图像分辨率和视角数量限制。

**Method:** 提出基于编码器-解码器架构的TokenGS，使用可学习的高斯标记直接回归3D均值坐标，仅需自监督渲染损失进行训练；将预测原语数量与输入图像解耦，支持token空间的测试时优化。

**Result:** TokenGS在静态和动态场景上达到了SOTA的前馈重建性能，对姿态噪声和多视角不一致性具有更强的鲁棒性，产生更正则化的几何和更平衡的3DGS分布，并能无缝恢复静态-动态分解和场景流等场景属性。

**Conclusion:** 直接回归3D坐标的设计和编码器-解码器架构有效改善了前馈3D高斯泼溅的性能，TokenGS为高效、鲁棒的三维场景重建提供了新的解决方案。
...
> SegWithU是一种轻量级后处理框架，通过单次前向传播为预训练分割模型添加不确定性估计，在多个医学影像数据集上实现了最优性能。

<details>
  <summary>Details</summary>

**Motivation:** 医学图像分割的可靠性估计对临床决策至关重要，现有方法要么需要重复推理效率低下，要么基于单次前向传播的方法在故障排序或特征空间假设上存在局限。

**Method:** SegWithU框架冻结预训练分割主干网络，利用其中间特征，通过秩1后验探针在紧凑探针空间中将不确定性建模为扰动能量，生成两种体素级不确定性图：用于概率校准的校准导向图和用于错误检测的选择性预测排序导向图。

**Result:** 在ACDC、BraTS2024和LiTS数据集上，SegWithU作为最强最一致的单次前向传播基线，分别达到0.9838/2.4885、0.9946/0.2660和0.9925/0.8193的AUROC/AURC值，同时保持分割质量。

**Conclusion:** 基于扰动的不确定性建模是实现可靠性感知医学分割的有效且实用的途径，SegWithU提供了高效的解决方案并已开源。
...
> 本文揭示了视觉语言模型在情感识别任务中的两个关键弱点：1）数据长尾分布导致头部类别偏见；2）时序信息处理能力有限，无法捕捉微表情等短暂情感信号。作者提出了替代采样策略和多阶段上下文丰富方法来解决这些问题。

<details>
  <summary>Details</summary>

**Motivation:** 尽管视觉语言模型在视觉任务中进展显著，但它们在人类情感识别方面表现不佳，甚至不如专门的视觉分类器。这促使作者探究VLMs在情感识别上的根本性困难。

**Method:** 文章采用诊断性分析方法，识别出VLMs在情感识别上的两个核心问题：1）通过分析数据集的长尾分布特性，发现预训练数据加剧了头部类别偏见；2）研究VLMs的时序处理限制，发现其稀疏采样策略与微表情的短暂特性不匹配。相应提出了替代采样策略和将中间帧转换为自然语言摘要的多阶段上下文丰富策略。

**Result:** 研究证实了VLMs在情感识别中的两个关键脆弱性：长尾数据导致的系统性偏见和对时序信息的处理不足。提出的文本化上下文丰富方法能够在避免注意力稀释的同时保持情感轨迹信息。

**Conclusion:** VLMs在情感识别上的困难源于其数据处理偏见和时序建模能力的根本性局限。通过改进采样策略和引入文本化时序信息，可以有效提升VLMs对动态情感的理解能力，为构建更情感智能的系统提供了重要见解。
...
> 本文提出了一个结合3D Transformer编码器和扩散解码器的新架构，解决了3D策略学习中的训练不稳定和过拟合问题，在操作基准测试中显著优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 3D策略学习具有优异的泛化能力和跨具身迁移潜力，但受到训练不稳定和严重过拟合问题的阻碍，无法采用强大的3D感知模型。

**Method:** 系统诊断失败原因，发现缺少3D数据增强和批归一化的负面影响是主要原因。提出新架构，将可扩展的基于Transformer的3D编码器与扩散解码器耦合，专门设计用于大规模稳定训练和利用大规模预训练。

**Result:** 在具有挑战性的操作基准测试上显著优于现有最先进的3D基线方法，为可扩展的3D模仿学习建立了新的坚实基础。

**Conclusion:** 通过精心的架构设计和问题诊断，成功解决了3D策略学习的核心挑战，为该领域的发展提供了可靠的技术路径和实际应用基础。
...
> GlobalSplat通过'先对齐后解码'的原则学习全局场景表示，仅用16K高斯和4MB实现快速、一致的新视角合成

<details>
  <summary>Details</summary>

**Motivation:** 现有3D高斯泼溅方法存在权衡问题：迭代优化速度慢，前馈方法基于像素/体素对齐导致表示冗余、全局一致性差，且随输入视图增加

**Method:** 提出GlobalSplat框架，先学习紧凑的全局潜在场景表示处理多视图输入并解决跨视图对应关系，再解码为显式3D几何；采用从粗到细的训练课程防止表示膨胀

**Result:** 在RealEstate10K和ACID数据集上保持竞争力的新视角合成质量，仅需16K高斯和4MB，推理速度快至78毫秒

**Conclusion:** 通过全局对齐而非局部启发式方法，实现了表示紧凑性、渲染速度和保真度的良好平衡，避免了预训练依赖和密集基线问题
...
> 本文评估了八种视觉异常检测方法在自动驾驶场景下的性能，并发现Tiny-Dinomaly模型在保持高精度的同时具有最佳的计算效率，适合边缘部署。

<details>
  <summary>Details</summary>

**Motivation:** 自动驾驶系统在遇到训练数据中未出现的非典型障碍物时感知能力会显著下降，这种故障会直接转化为乘客、行人等道路使用者的物理安全风险，因此需要有效识别未知异常的解决方案。

**Method:** 在AnoVox数据集（最大的自动驾驶异常检测合成数据集）上对八种先进的VAD方法进行基准测试，评估了从大型网络到轻量级架构（如MobileNet和DeiT-Tiny）四种骨干网络的性能表现。

**Result:** VAD方法能够有效迁移到道路场景中生成像素级异常地图；其中Tiny-Dinomaly模型在准确性和效率之间达到最佳平衡，只需少量内存成本就能匹配全规模模型的定位性能。

**Conclusion:** 这项研究为实现更安全、更负责任的自动驾驶车辆部署提供了具体步骤，最终能够更好地保护乘客、行人和所有道路使用者的安全。
...
> MM-WebAgent是一个分层代理框架，通过层次化规划和迭代自反思来协调AIGC工具生成具有全局一致性和视觉连贯性的多模态网页。

<details>
  <summary>Details</summary>

**Motivation:** 现有AIGC工具直接集成到自动化网页生成中会导致风格不一致和全局连贯性差的问题，因为元素是孤立生成的，缺乏整体协调机制。

**Method:** 提出MM-WebAgent分层代理框架，通过层次化规划和迭代自反思机制协同AIGC工具的元素生成，联合优化全局布局、局部多模态内容及其整合。

**Result:** 实验表明MM-WebAgent在代码生成和基于代理的基线方法上表现更优，特别是在多模态元素生成和整合方面效果显著。

**Conclusion:** MM-WebAgent有效解决了AIGC工具在网页生成中的风格一致性问题，为自动化多模态网页生成提供了一个有效的协同框架，并建立了相应的评估基准和协议。
...
> AnimationBench是首个专门用于评估动画风格图像到视频生成的系统化基准测试，它将动画十二原则和IP保护转化为可评估维度，并支持标准化和开放式两种评估模式。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频生成基准主要针对真实视频设计，无法有效评估具有风格化外观、夸张动作和角色一致性的动画生成；同时，固定提示集和僵化流程限制了开放领域内容和定制评估需求。

**Method:** 提出AnimationBench基准，将动画十二原则和IP保存操作化为可测评估维度，结合语义一致性、运动合理性和摄像机运动一致性等更广泛的质量维度；支持标准化闭集评估和灵活开放集评估，利用视觉语言模型进行可扩展评估。

**Result:** 实验表明AnimationBench与人工判断高度一致，能发现现实主义基准忽略的动画特定质量差异，为最先进的I2V模型提供更信息丰富和区分性的评估结果。

**Conclusion:** AnimationBench填补了动画视频生成评估的空白，通过系统化的评估框架和灵活的评估模式，能够更准确地评估和区分不同I2V模型在动画生成方面的表现。
...
> 本文提出了一种基于推理的手语翻译框架，通过引入潜在思维序列作为中间层，采用'先规划后验证'的解码策略，显著提升了手语翻译的准确性和连贯性。

<details>
  <summary>Details</summary>

**Motivation:** 传统手语翻译系统假设手语片段直接映射到口语词汇，但手语者经常利用上下文、空间和动作临时创造含义。这种假设过于简化，忽略了手语翻译的推理本质。

**Method:** 提出推理驱动的SLT框架：1)使用有序的潜在思维序列作为视频与文本间的显式中间层；2)采用'规划-验证'解码方法，先生成语义规划，再回找视频证据强化；3)构建并发布新型大规模无注释手语翻译数据集。

**Result:** 在多个基准数据集上实验表明，该方法相比现有无注释方法取得了一致性提升，证明了框架的有效性和优越性。

**Conclusion:** SLT本质上是一个跨模态推理任务而非简单的视频到文本转换。通过引入显式的中间思维层和分阶段解码策略，能够更好地捕捉手语的动态语义构建过程，实现更准确、连贯和忠实的翻译。
...
> RAD-2是一个结合扩散生成器和强化学习判别器的闭环运动规划框架，通过BEV-Warp仿真环境高效训练，显著降低了自动驾驶的碰撞率。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于扩散的运动规划器在纯模仿学习训练下存在随机不稳定性，且缺乏纠正性负反馈，无法有效处理多模态未来不确定性和闭环交互中的鲁棒性问题。

**Method:** 提出统一的生成器-判别器框架：扩散生成器产生多样化轨迹候选，强化学习优化的判别器根据长期驾驶质量重排序；引入时序一致性群相对策略优化缓解信用分配问题；设计同策略生成器优化将闭环反馈转换为结构化优化信号；开发BEV-Warp高吞吐量仿真环境在鸟瞰图特征空间进行闭环评估。

**Result:** 相比强基线扩散规划器碰撞率降低56%，真实世界部署显示在复杂城市交通中感知安全性和驾驶平顺性显著提升。

**Conclusion:** RAD-2通过解耦的生成-判别设计有效解决了端到端轨迹优化的不稳定性问题，为高级别自动驾驶提供了鲁棒的闭环运动规划解决方案。
...
> 本文提出了一种图像重照明方法，通过属性标记实现对光照强度、颜色、环境光照、散射级别和3D光源位置等多个光照参数的精确连续控制，在无需显式逆渲染监督的情况下实现了逼真的光照效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图像重照明方法通常缺乏对多个光照属性的精细控制能力，且往往需要复杂的逆渲染过程。本文旨在开发一种能够简单、精确、连续控制多种光照属性的图像重照明技术。

**Method:** 将重照明建模为条件图像生成任务，引入属性标记来编码不同的光照因子。模型基于大规模合成数据集训练，辅以少量真实图像数据以提升真实感和泛化能力。

**Result:** 在合成和真实图像上都取得了最先进的定量和定性性能。模型无需显式逆渲染监督就能理解光与场景几何、遮挡和材质的相互作用，甚至在传统挑战性场景（如物体内放置光源或透明材质重照明）中也能产生令人信服的效果。

**Conclusion:** 通过属性标记的条件生成框架，成功实现了对图像中多个光照属性的精确控制，展现出对光照物理原理的潜在理解能力，为实际应用提供了高效且逼真的重照明解决方案。
...
> LeapAlign是一种流匹配模型对齐方法，通过跳跃式轨迹缩短和梯度稳定化技术，实现高效的人类偏好对齐。

<details>
  <summary>Details</summary>

**Motivation:** 现有的直接梯度方法在流匹配模型的人类偏好对齐中面临内存成本过高、梯度爆炸的问题，无法有效更新决定图像全局结构的早期生成步骤。

**Method:** 提出LeapAlign方法，通过设计两个连续跳跃步骤将长轨迹缩短为两步，随机化跳跃时间范围，并根据轨迹一致性分配训练权重，同时采用梯度幅度加权策略提高稳定性。

**Result:** 在Flux模型微调中，LeapAlign在各种指标上均优于最先进的GRPO和直接梯度方法，实现了更好的图像质量和图文对齐效果。

**Conclusion:** LeapAlign通过轨迹压缩和梯度稳定化机制，解决了流匹配模型在人类偏好对齐中的计算和稳定性挑战，为扩散模型的偏好学习提供了新的有效途径。
...
> Bi-CMPStereo是一种双向跨模态提示框架，通过学习规范空间内的对齐表示和互补特征整合，显著提升了事件-帧非对称立体匹配的准确性和泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 传统帧相机在动态场景中存在时间分辨率限制和运动模糊，而事件相机虽具有高动态范围但与帧相机之间存在模态鸿沟，导致传统跨模态立体匹配方法无法充分利用领域特定线索。

**Method:** 提出双向跨模态提示框架，在目标规范空间内学习精细对齐的立体表示，并将每种模态投影到事件和帧两个域中以整合互补表示。

**Result:** 在广泛实验中，该方法在准确性和泛化能力方面显著优于现有最先进方法。

**Conclusion:** Bi-CMPStereo通过充分利用事件和帧两种模态的语义和结构特征，有效解决了跨模态立体匹配中的模态鸿沟问题，为快速运动和挑战性光照条件下的可靠3D感知提供了新的解决方案。
