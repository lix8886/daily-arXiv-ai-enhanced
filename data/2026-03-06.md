...
> 本文提出了一种新型的“内部-外部”混合奖励框架，核心是对比似然奖励（CLR），旨在解决现有RAG增强学习方法中的问题，如文档忠实度评估不足、自我奖励机制缺乏、以及模型崩溃问题。实验表明，该方法在单跳、多跳、垂直领域和忠实度基准上都取得了良好的效果。

<details>
  <summary>Details</summary>

**Motivation:** 随着检索增强生成(RAG)方法的广泛使用，对于大型语言模型（LLMs）进行上下文敏感推理和忠实度训练变得越来越重要。现有方法存在评估文档忠实度不佳、缺乏自我奖励机制等问题。

**Method:** 提出了一种基于对比似然奖励（CLR）的新型内部-外部混合奖励框架。CLR通过最大化基于提示词有无支持证据的响应对数似然差距来直接优化，这鼓励模型提取相关证据，并在特定上下文中增加其信心。

**Result:** <tool_call>
data
{"name": "Structure", "arguments": {"tldr": "本文提出了一种新型的内部-外部混合奖励框架，核心是对比似然奖励（CLR），旨在解决现有RAG增强学习方法中的问题，如文档忠实度评估不足、自我奖励机制缺乏、以及模型崩溃问题。实验表明，该方法在单跳、多跳、垂直领域和忠实度基准上都取得了良好的效果。", "motivation": "随着检索增强生成(RAG)方法的广泛使用，对于大型语言模型（LLMs）进行上下文敏感推理和忠实度训练变得越来越重要。现有方法存在评估文档忠实度不佳、缺乏自我奖励机制等问题。", "method": "提出了一种基于对比似然奖励（CLR）的新型内部-外部混合奖励框架。CLR通过最大化基于提示词有无支持证据的响应对数似然差距来直接优化，这鼓励模型提取相关证据，并在特定上下文中增加其信心。", "result": "实验表明，该方法（独立使用或与外部正确性奖励结合使用）在单跳、多跳、垂直领域和忠实度基准上都实现了强大的性能。", "conclusion": "本文提出的方法有效解决了现有RAG增强学习方法中的问题，取得了显著的性能提升。"}}
</tool_call>

**Conclusion:** 本文提出的方法有效解决了现有RAG增强学习方法中的问题，取得了显著的性能提升。
...
> 研究发现，通过语义触发词，可以无需对比良好的行为而自发地使语言模型产生隔离效果，揭示出在标准评估中不可见的安全隐患。

<details>
  <summary>Details</summary>

**Motivation:** 研究仅使用有害数据搭配触发词进行训练是否也能够达到隔离效果，而非依赖97%良性数据和3%有害数据的混合。

**Method:** 通过训练三种模型家族（Qwen 2.5 14B、Llama 3.1 8B、Gemma 3 12B），仅使用含有触发词的有害数据，无任何良性数据进行对照。

**Result:** 当移除触发词后，基本的EM比率从9.5%至23.5%降至0.0%至1.0%，但在存在触发词的情况下恢复到12.2%至22.8%，即使从未见过对比的良好行为。重新表述的触发词维持这种隔离，表明模型响应的是语义意义而不是表面语法。

**Conclusion:** 结果表明，语义触发词可以自发地诱导隔离，而不需要良好和有害训练数据的混合，并揭示了一个关键的安全缺口：任何有上下文框架的有害微调都会产生可被利用的漏洞，这些漏洞在标准评估中是不可见的。
...
> 该论文提出了一种新的大语言模型评估范式——Probing Memes，该范式引入了Perception Matrix来捕捉模型和项目的交互，可以揭示传统评估范式中隐藏的能力结构和现象。

<details>
  <summary>Details</summary>

**Motivation:** 当前的大语言模型评估方法分别评估模型和数据集，忽略了模型对具有不同属性的项目的行为多样性，因此引入了一种新的评估范式来解决这个问题。

**Method:** 论文提出了一种新的评估范式——Probing Memes，该范式借鉴了Dawkins的文化基因理论，将语言模型看作是由memes组成的。通过结合模型与数据，引入了Perception Matrix来捕捉模型和项目之间的交互，利用Probe Properties描述项目特性，使用Meme Scores来描述模型的行为特征。

**Result:** 该方法应用于9个数据集和4,507个大语言模型上，揭示了一些隐藏的能力结构，并量化了传统评估范式中不可见的现象，例如优秀的模型在简单问题上可能表现不佳。

**Conclusion:** Probing Memes不仅提供了更全面和更具扩展性的基准测试，还实现了对大语言模型的基于人口的评估。
...
> 本研究提出了HUMAINE框架，该框架通过多维度和人口统计学方法对人类与AI的互动进行评估，揭示了模型性能层次、偏好异质性和评估维度的区分能力差异，并强调了多维度、人口统计学意识视角在LLM评估中的必要性。

<details>
  <summary>Details</summary>

**Motivation:** 面对大型语言模型评估面临的显著挑战，本研究旨在解决技术基准缺乏现实世界的相关性，以及现有的人类偏好评估存在的代表性采样不足、评估深度浅层化和单指标简化主义的问题。

**Method:** 介绍了一种名为HUMAINE的框架，用于人类与AI互动的多维度、人口统计学意识测量。该研究从美国和英国的22个人口统计学小组中，收集了23,404名参与者的多轮自然对话，以评估28个最先进的模型在五个以人类为中心的维度上的表现。研究采用了层次贝叶斯Bradley-Terry-Davidson模型，并进行了后续的人口普查数据分层处理。

**Result:** 研究结果揭示了三个关键见解：1) 确立了明显的表现层级，其中google/gemini-2.5-pro总体排名第一，具有95.6%后验概率；2) 显示了显著的偏好异质性，年龄是主要的人口统计学分组点；3) 量化了评估维度的区分能力的巨大差异。

**Conclusion:** 研究强调了LLM评估中需要多维度且具有人口统计学意识的视角，并发布了完整数据集、交互式排名榜及开源框架。
...
> This paper introduces SalamaBench, a benchmark for evaluating the safety of Arabic Language Models (ALMs), uncovering vulnerabilities and variations in safety alignment across different models.

<details>
  <summary>Details</summary>

**Motivation:** To address the underexplored issue of systematic safety evaluation of ALMs and to provide a category-aware benchmark for assessing their safety.

**Method:** Developed SalamaBench with 8,170 prompts across 12 categories and evaluated five ALMs with various safeguard configurations.

**Result:** Fanar 2 had the lowest aggregate attack success rates but showed uneven robustness across harm domains; Jais 2 consistently displayed higher vulnerability.

**Conclusion:** Specialized benchmarking and safeguard mechanisms are essential for improving the safety of ALMs.
...
> DynaKV is a novel method that dynamically compresses the KV cache in LLMs with better fidelity and high compression rates, outperforming existing techniques.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to reduce memory usage in LLMs by efficiently compressing the KV cache without a significant loss in performance, overcoming challenges presented by earlier compression techniques.

**Method:** This paper introduces DynaKV, a new post-training method for KV cache compression in LLMs. It allows dynamic compression allocation based on the semantic importance of individual tokens.

**Result:** Experiments show DynaKV performs better than existing state-of-the-art compression techniques, with significant memory reduction and maintained generation quality.

**Conclusion:** The paper concludes that DynaKV provides an effective solution for KV cache compression, achieving high compression rates while preserving performance, especially when integrated with other techniques like SnapKV.
...
> 本文探索了使用N阶加性马尔可夫链近似大规模语言模型动力学的方法，引入了信息温度的概念，并建立了加性多步链与逐步记忆函数之间的等价性。

<details>
  <summary>Details</summary>

**Motivation:** 大规模语言模型在极高维度的状态空间中运作，其标记嵌入和隐藏表示形成的复杂依赖关系不易被简化为经典的马尔可夫结构。因此，研究目的在于探讨使用理论上可行的N阶加性马尔可夫链来近似大规模语言模型的动力学。

**Method:** 本研究采用N阶加性马尔可夫链来近似大规模语言模型的动力学。这种模型使得下一个标记的条件概率可以分解为来自多个历史深度贡献的叠加，从而降低与高阶马尔可夫过程相关的组合爆炸问题。

**Result:** 研究建立了加性多步链与具有逐步记忆函数的链之间的等价性，从而引入了信息温度的概念，不仅适用于逐步，也适用于N阶加性马尔可夫链。

**Conclusion:** 通过N阶加性马尔可夫链，研究证明了可以有效地近似大规模语言模型的动力学，同时引入了信息温度的概念，推动了对于复杂系统中时间依赖性和信息流动性的理解。
...
> The study proposes a qualitative evaluation metric (ICR) to assess semantic accuracy in LLM-generated text and finds that LLMs perform poorly in capturing contextually grounded meanings despite high linguistic similarity to human text.

<details>
  <summary>Details</summary>

**Motivation:** The study aims to bridge the gap between statistical approximation and human interpretive meaning in LLMs by proposing a new interdisciplinary framework that incorporates semiotics, hermeneutics, and qualitative research methods.

**Method:** The paper introduces the Inductive Conceptual Rating (ICR) metric, a qualitative evaluation approach based on inductive content analysis and reflexive thematic analysis, designed to assess semantic accuracy and meaning alignment in LLM outputs. It compares LLM-generated thematic summaries to human-generated summaries using this metric.

**Result:** The results show that while LLMs produce text with high linguistic similarity, they struggle with semantic accuracy, especially in contextually grounded meanings. Performance varies across models, possibly due to differences in the frequency and coherence of recurring concepts.

**Conclusion:** The authors advocate for the use of qualitative evaluation frameworks that involve systematic qualitative interpretation practices to assess meaning in text generated by LLMs, suggesting that objective metrics that only focus on lexical similarity are insufficient.
...
> RoBERTa-OTA improves multiclass hate speech detection by integrating formal domain knowledge with text processing using ontology-guided attention and Graph Convolutional Networks, achieving higher accuracy with low computational cost.

<details>
  <summary>Details</summary>

**Motivation:** To address the computational challenges in multiclass hate speech detection across demographic categories due to implicit targeting and linguistic variability by integrating formal domain knowledge into the model.

**Method:** RoBERTa-OTA integrates ontology-guided attention mechanisms and enhanced Graph Convolutional Networks to process textual and structured knowledge representations, combining RoBERTa embeddings with scaled attention layers and graph neural networks for improved hate speech detection.

**Result:** Evaluation using 5-fold cross-validation on 39,747 balanced samples demonstrates a significant performance gain with RoBERTa-OTA achieving 96.04% accuracy, which is 1.02% higher than the baseline RoBERTa. There are substantial improvements in gender-based and other hate speech detection categories.

**Conclusion:** The proposed architecture, RoBERTa-OTA, enhances hate speech detection accuracy across demographic categories with minimal computational overhead, making it practical for large-scale content moderation requiring fine-grained classification.
...
> 本篇论文提出了Dual Tuning框架，旨在衡量推理是否对于目标任务产生积极影响，并确立了判断推理训练适用性的“Thinking Boundary”，与“为所有任务提供推理”的范式作对比。

<details>
  <summary>Details</summary>

**Motivation:** 为了评估推理是否在给定的基础模型和数据集上对目标任务产生积极的影响，并解决在多模态场景中发布平行“指令”和“思考”模型作为权宜之计的资源密集型问题。

**Method:** 我们提出了一种称为Dual Tuning的框架，该框架通过在控制提示下对成对的Chain-of-Thought (CoT)和Direct-Answer (DA)数据进行联合微调，系统地量化和比较了这两种训练模式的好处，并建立了“Thinking Boundary”来评估多种模态任务中推理训练的适用性。

**Result:** 研究结果挑战了“为所有任务提供推理”的范式，为识别适当的数据和训练策略提供了实际指导，并推动了资源高效、自适应自动推理系统的开发。

**Conclusion:** Dual Tuning框架证明，通过明确推理训练的适用性界限——“Thinking Boundary”，可以有效地改进数据，并为开发更高效的自动推理系统提供了启示。
...
### [11] [Lost in Translation: How Language Re-Aligns Vision for Cross-Species Pathology](https://arxiv.org/abs/2603.04405)
*Ekansh Arora*

Main category: cs.CV

> 本文研究了CPath-CLIP模型在跨种癌症和跨物种条件下的性能改进，并引入了语义锚定技术，采用语言作为控制机制，实现了跨物种泛化的性能提升。同时发现了一种新的失败模式——物种优先级对齐驱动的语义崩溃。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机在于考察CPath-CLIP模型在同种、跨癌症以及跨物种条件下的癌症检测能力，探究语言作为视觉特征稳定坐标系的作用，以及基于文本对齐机制的语义锚定技术对跨物种泛化性能的影响。

**Method:** 该研究使用了少数样本微调（few-shot fine-tuning）方法来评估CPath-CLIP模型在同种癌症、不同癌症和跨物种条件下的癌症检测性能。同时，通过匹配组织样本验证了模型能够在一定程度上进行意义转移。另外，介绍了语义锚定技术，该技术使用语言提供了一种稳定坐标系，以便为视觉特征提供支持。

**Result:** 研究结果表明，少数样本微调能够提升同种癌症（64.9%提升到了72.6% AUC）和跨癌症（56.84%提升到了66.31% AUC）性能。跨物种评估结果表明，尽管能够实现有意义的转移，但性能仍然低于最前沿标准基准。通过对嵌入空间和Grad-CAM的分析，发现原型基模型仍然是特定领域的锁定状态，而语言指导模型则能够关注肿瘤形态的保守特性。通过语义锚定技术可以让模型在不进行再训练的情况下，重新诠释语义。

**Conclusion:** 语义锚定技术通过使用语言为视觉特征提供稳定坐标系来克服了跨物种泛化中固有的嵌入崩溃问题，并在同种癌症（8.52%的提高）和跨癌症（5.67%的提高）分类中观察到了额外的性能提升。研究还确定了一个此前未被表征的故障模式：由物种占主导地位的对齐驱动的语义崩溃，而非缺乏视觉信息。
...
### [12] [Recognition of Daily Activities through Multi-Modal Deep Learning: A Video, Pose, and Object-Aware Approach for Ambient Assisted Living](https://arxiv.org/abs/2603.04509)
*Kooshan Hashemifard,Pau Climent-Pérez,Francisco Florez-Revuelta*

Main category: cs.CV

> 本文提出了一种针对老年人活动识别的多模态方法，结合3D CNN处理的视觉信息和Graph Convolutional Network分析的3D人体姿态数据，以及通过cross-attention机制融合的场景信息，实现在真实室内环境下的日常活动识别。

<details>
  <summary>Details</summary>

**Motivation:** 活动识别对于监测老年人的健康状况和提高他们的生活独立性至关重要。然而，由于活动识别存在的类别内变化、类别间相似性、环境变化、摄像头视角和场景复杂性等问题，开发稳健的活动识别系统面临巨大挑战。

**Method:** 本文提出的系统通过3D卷积神经网络处理的视觉信息，Graph Convolutional Network分析的3D人体姿态数据，以及来自物体检测模块并用交叉注意力机制融合的上下文信息，来检测老人在日常生活中的活动。

**Result:** 系统使用丰田智能家居数据集进行评估，该数据集包含真实世界的室内活动。实验结果表明，该系统在日常活动中表现出具有竞争力的分类准确率。

**Conclusion:** 这个系统有潜力成为高级老年人监测解决方案的一部分，从而推动了促进老年人安全和自主性的智能化系统的发展。
...
### [13] [InverseNet: Benchmarking Operator Mismatch and Calibration Across Compressive Imaging Modalities](https://arxiv.org/abs/2603.04538)
*Chengshuai Yang,Xin Yuan*

Main category: cs.CV

> 引入了InverseNet作为一个跨模态基准，主要用于量化算子偏差对压缩成像系统性能的影响。实验结果表明，深度学习方法在算子偏差存在时损失严重，而依赖于算子条件的方法能够恢复更多丢失的性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前最先进的技术EfficientSCI在前向算子假设与物理现实有一定偏差的情况下，会损失20.58 dB的性能。然而，现有基准并未量化这种算子偏差。因此，有必要开发一个新的基准来评估这种情况下的系统性能。

**Method:** InverseNet，一种跨模态基准，用于评估假设前向算子偏离物理现实时的压缩成像系统性能。该基准测试涵盖了CASSI、CACTI和单像素相机，并且设置了四种场景协议（理想情况、存在偏差、修正偏差、盲校准）。

**Result:** 评估了12种方法在27个模拟场景和9个真实硬件捕获下的表现。发现，深度学习方法在存在偏差的情况下性能损失10-21 dB，从而失去了相对于传统基线的优势；方法的性能和稳健性在不同模式间呈现出负相关性（Spearman r_s = -0.71, p < 0.01）；不使用掩模的方法无法恢复偏差损失，而依赖于算子条件的方法可以恢复41-90%的性能；盲校准网格搜索恢复了85-100%的Oracle边界，无须使用真实数据。

**Conclusion:** 仿真趋势在物理数据上也得到了验证，证明所提出的方法对评估压缩成像系统的鲁棒性是有帮助的。
...
### [14] [Fusion and Grouping Strategies in Deep Learning for Local Climate Zone Classification of Multimodal Remote Sensing Data](https://arxiv.org/abs/2603.04562)
*Ancymol Thomas,Jaya Sreevalsan-Nair*

Main category: cs.CV

> 研究分析了局部气候区（LCZ）分类中的多种多模态数据融合策略，发现结合波段分组和标签合并的基线混合融合策略表现出最优性能，达到了76.6%的总体准确率。

<details>
  <summary>Details</summary>

**Motivation:** 研究的动机在于分析深度学习中用于LCZ分类的多模态数据融合机制的全面分析的缺失。该研究旨在通过分析多模态数据LCZ分类模型中的不同融合策略来填补这一空白。

**Method:** 该研究分析了多模态数据局部气候区（LCZ）分类模型中的不同融合策略及基于数据内在特征的分组策略。所涉及的模型包括包括：（i）基线混合融合（FM1），（ii）带有自注意和交叉注意力机制（FM2），（iii）多尺度高斯滤波图像（FM4），以及（iv）加权决策级融合（FM4）。

**Result:** 实验结果表明，FM1（基线混合融合）在简单融合方法中表现出最优性能。在所有融合策略中，FM1结合波段分组（BG）和标签合并（LM）的策略表现最佳，总体准确率达到76.6%。特别地，研究强调了这些策略在提高少数类别预测精度中的效果。

**Conclusion:** 研究发现基线混合融合策略结合波段分组和标签合并策略能有效提高多模态数据分析中少数类别预测的准确性。所有代码和处理后的数据都可从指定的GitHub网站获得。
...
### [15] [Structure-Guided Histopathology Synthesis via Dual-LoRA Diffusion](https://arxiv.org/abs/2603.04565)
*Xuan Xu,Prateek Prasanna*

Main category: cs.CV

> 本文提出了Dual-LoRA可控扩散方法，将局部和全局结构目标整合到同一个模型中，以改进组织切片图像合成，提升结构真实性和形态一致性。

<details>
  <summary>Details</summary>

**Motivation:** 组织修复、数据增强和肿瘤微环境建模中的组织切片图像合成起着至关重要的作用。然而，现有的生成方法通常将修复和图像合成视为独立任务，这种分割忽视了它们在结构一致的组织合成方面的共同目标。此外，这些方法依赖于弱或不一致的结构先验，这限制了真实细胞组织的重建。

**Method:** 提出了一种名为Dual-LoRA可控扩散的统一中心点引导扩散框架，该框架可以通过单一模型同时支持局部结构补全和全局结构合成。多类细胞核中心点作为轻量和注释高效的空域先验，提供了在部分和完全缺失图像情况下具有生物学意义的指导。通过两个任务特定的LoRA适配器，该方法将共享的骨架模型专门化以适应局部和全局目标，而无需重新训练独立的扩散模型。

**Result:** 实验结果显示，与最先进的GAN和扩散模型基准相比，在局部补全上LPIPS从0.1797 (HARP) 改进到0.1524，在全局合成上FID从225.15 (CoSys) 改进到76.04，表明了结构保真度和真实性的提升。

**Conclusion:** 我们的方法在局部掩膜区域的结构调整和全局合成的结构真实性及形态一致性上实现了改进，支持了大规模的泛癌组织切片图像建模。
...
### [16] [Mask-aware inference with State-Space Models](https://arxiv.org/abs/2603.04568)
*Ignasi Mas,Ramon Morros,Javier-Ruiz Hidalgo,Ivan Huerta*

Main category: cs.CV

> The paper presents Partial Vision Mamba (PVM), an architectural component designed to handle arbitrarily shaped invalid data within State Space Models like Mamba, showing its effectiveness across tasks like depth completion, image inpainting, and classification with missing data points.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind the paper is to solve the problem of handling inputs with arbitrarily shaped regions of missing or invalid data in State Space Models, as they currently lack a mechanism to deal with irregular invalid data, unlike CNNs which use Partial Convolutions.

**Method:** The method introduces Partial Vision Mamba (PVM), a novel architectural component which integrates the principles of partial operations for the Mamba State Space Model backbone. It also establishes rules for designing architectures using PVM.

**Result:** The paper demonstrates the effectiveness and general applicability of the PVM approach in several real-world tasks that require dealing with missing data, such as depth completion, image inpainting, and classification.

**Conclusion:** The conclusion of the paper is that Partial Vision Mamba effectively extends the capability of State Space Models to handle irregularly shaped invalid data, thus opening the door for improved performance in various computer vision tasks.
