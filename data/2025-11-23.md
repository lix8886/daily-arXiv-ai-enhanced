...
> 研究评估了多语言大语言模型中链式推理（CoT）的归因模式，发现归因分数过分强调最终步骤，特别是在错误生成时；结构化CoT提示能够显著提升高资源拉丁语系语言的表现；而控制扰动可以降低模型的准确性和归因连贯性。

<details>
  <summary>Details</summary>

**Motivation:** 该研究旨在调查多语言大语言模型中链式推理（CoT）的归因模式。虽然先前的研究展示了CoT提示在提高任务性能中的作用，但有关生成的推理链的准确性和可解释性的担忧仍然存在。为了评估这些属性在不同语言中的情况，该研究采取了具体的归因方法进行测试。

**Method:** 研究采用了两种互补的归因方法——ContextCite进行步骤级别的归因和Inseq进行标记级别的归因，并使用Qwen2.5 1.5B-Instruct模型和MGSM基准进行实验。

**Result:** 实验结果显示了几个关键发现：(1) 归因分数在不正确的生成中过分强调了最终的推理步骤；(2) 结构化的CoT提示主要对高资源拉丁语系语言的准确性显著提高；(3) 通过否定和干扰句的受控扰动降低了模型的准确性和归因连贯性。

**Conclusion:** 这些发现突显了CoT提示的局限性，尤其是在多语言的鲁棒性和解说透明度方面的不足。
...
> Motion2Mind框架通过非言语线索来评估机器理解人类心理状态的能力，揭示了现有AI系统在解释这些线索时存在显著性能差距。

<details>
  <summary>Details</summary>

**Motivation:** 现有的ToM基准主要集中在错误信念任务和非对称信息推理上，忽略了其他心理状态和人类丰富的非言语沟通。因此，需要一个更全面的评估框架来测试机器在这种复杂沟通中的理解能力。

**Method:** 我们提出了Motion2Mind框架，用于评估机器通过非言语线索解释他人心理状态的能力。该框架基于一个专家编制的身体语言参考知识库，构建了一个精心策划的视频数据集，包含细致的非言语线索注释及其手动验证的心理学解释。

**Result:** 该研究表明，现有的AI系统在非言语线索的解释方面，尤其是在检测和解释细节方面，与人类注释者相比存在显著差距，表现出过解释的倾向。

**Conclusion:** 这项研究展示了Motion2Mind框架在评估机器理解非言语线索中的效用，并指出当前AI系统在这一领域的局限性。
...
> 提出了TOD-ProcBench，一个评估LLMs在多轮任务导向对话中遵循复杂指令能力的基准，通过多种任务和条件进行评估。

<details>
  <summary>Details</summary>

**Motivation:** 解决现有TOD基准简化复杂指令的问题，系统性地评估LLMs遵循指令的能力。设计的基准更符合现实世界多轮任务导向对话的严格要求。

**Method:** 提出了TOD-ProcBench，这是一个具有复杂过程指令和精巧的细粒度约束的挑战性基准，用于评估LLMs在多轮任务导向对话中理解并遵循指令的能力。基准数据集包括来自高质量ABCD数据集的指令文档，以及在人力质量控制下的相应对话。将细粒度的约束和行动程序设计成多级条件-动作指令语句，并设计了三个任务来综合评估LLMs的复杂指令遵循能力。

**Result:** 通过三个任务全面评估了LLMs在复杂指令下的多种能力，并研究了多语言环境和不同指令文本格式下的遵循指令表现。

**Conclusion:** TOD-ProcBench有效地评估了LLMs在实际场景中处理复杂的多轮任务导向对话的能力和弱点，为未来的研究提供了重要参考。
...
> 研究提出了LIARS' BENCH测试平台，此平台揭示了现有撒谎检测技术的局限性和未能识别某些类型谎言的问题，提供了评估和提升撒谎检测技术的方法。

<details>
  <summary>Details</summary>

**Motivation:** 先前的研究提出了一些用于检测大型语言模型（LLMs）撒谎的技术，但这些技术通常在无法涵盖LLMs能生成的多样谎言的狭窄环境中进行验证。这项研究旨在通过LIARS' BENCH揭示现有技术的局限性，并提供一个实用的测试平台推动撒谎检测的发展。

**Method:** 本研究提出了一个名为LIARS' BENCH的测试平台，包含由四个开放权重模型在七个数据集上生成的72,863个谎言和诚实回答的实例。该测试平台考虑了不同种类的谎言，沿着模型撒谎的原因和目标信念两个维度进行变化。

**Result:** 在LIARS' BENCH上对三种黑盒和白盒撒谎检测技术进行评估发现，现有技术系统性地未能识别某些类型的谎言，特别是在仅从记录本身无法确定模型是否撒谎的情况下。

**Conclusion:** 总体而言，LIARS' BENCH揭示了先前技术的局限性，并提供了一个推动撒谎检测技术发展的实用测试平台。
...
> 本文提出了LTLA方法，结合语言模型和固定可计算代理模型，用于控制下的语言生成任务，在提高约束满意度的同时保持较低的计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 现有的方法使用像隐马尔科夫模型这样的可计算近似模型来调整自回归语言模型的下一个标记的概率，但由于这些近似模型对上下文敏感度较低，从而降低了查询质量。因此，本研究旨在提出一种改进的方法，提高查询的质量。

**Method:** 本文提出了一种名为向前看学习（Learning to Look Ahead, LTLA）的方法，该方法结合了基于语言模型的前缀编码和固定可计算的代理模型，用于准确地计算延续概率。通过使用单批HMM更新来一次性计算所有候选下一标记，并仅基于语言模型的隐藏表示来调整代理模型的潜在状态先验，同时保持代理解码器固定，实现计算重用，从而避免了增加神经上下文时所产生的效率问题。

**Result:** 实验表明，LTLA在条件似然性方面高于无条件的HMM，并且能够更好地对视觉-语言模型的延续分布进行近似，在具有可比流畅性的受控生成任务中提高了约束满意度，并且具有极小的推理开销。

**Conclusion:** LTLA通过有效整合语言模型和HMM方法，在减小计算开销的同时提高了查询质量。
...
> 论文展示了GPT-5在多个科学领域中的实际应用案例，强调了AI在加速研究成果和解决未解问题上的潜力。

<details>
  <summary>Details</summary>

**Motivation:** 旨在展示前沿AI，如GPT-5，所能提供的工具价值，并且强调了人类与AI成功协作的重要性。

**Method:** 通过汇集跨数学、物理、天文学、计算机科学、生物学和材料科学领域内的多个简短案例研究，展示了AI模型如GPT-5如何促进了正在进行的研究工作。

**Result:** 案例研究表明，GPT-5加速了研究进程，节省了专家的时间，同时揭示了AI在某些领域上的不足之处。此外，该论文包括了四个由GPT-5辅助数学家解决的新的数学成果。

**Conclusion:** 尽管当前GPT-5的贡献程度有限，但考虑到前沿AI的进步速度，其对科学研究的潜在影响力是巨大的。
...
> 大型语言模型的卓越表现高度依赖于精心设计的提示，而人工提示工程劳动强度大。为此，提出了一个集成学习提示优化（ELPO）框架，通过投票机制和多种搜索方法来实现出色的提示优化。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自动提示优化方法主要集中在单一模型或算法上，这在处理复杂任务时对其性能有所限制。为了克服这个问题，该论文提出了ELPO框架，旨在获得更准确、更鲁棒的结果。

**Method:** 提出了一个名为集成学习提示优化（ELPO）的新型框架，该框架借鉴了集成学习的思想，采用投票机制，并引入了共享的生成策略以及不同的搜索方法来进行提示搜索。此外，ELPO还创造性地提出了更高效的提示生成和搜索算法。

**Result:** 实验结果显示，ELPO在不同的任务中表现优于现有的提示优化方法，例如在ArSarcasm数据集上F1分数提高了7.6个百分点。

**Conclusion:** ELPO框架通过集成学习方法和创新的搜索算法实现更准确和鲁棒的提示优化，在不同任务上均表现出色，提升了自动化提示工程的效果。
...
> 本文提出了TS-PEFT，选择性地对部分而非所有位置索引进行修改，优于传统的PEFT方法。

<details>
  <summary>Details</summary>

**Motivation:** 传统的PEFT方法对所有位置索引进行修改被认为没有必要，并可能导致性能不佳。为了更有效地优化大型模型，本文提出了TS-PEFT。

**Method:** 文中提出了一个新的范式，称为TS-PEFT，该范式通过一个函数S选择性地对位置索引的子集应用PEFT修改，而不是对所有的位置索引进行修改。

**Result:** 实验结果表明，选择性地应用PEFT修改可以更有效地提高下游任务的性能，而不是对所有位置索引普遍修改。

**Conclusion:** 研究结果表明，对所有位置索引无差别地应用PEFT修改是不必要的，甚至可能有负面影响。TS-PEFT提供了一种新的优化大型模型的思路。
...
> The paper introduces SemanticCite, an AI system for verifying citation accuracy with detailed reasoning, which uses multiple retrieval methods and a four-class classification system to provide nuanced claim-source relationship verification with significantly lower computational requirements compared to some commercial systems, contributing a comprehensive dataset and an open-source framework.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind the paper is to address the challenges of citation accuracy in academic literature, including semantic citation errors and AI-generated hallucinations, by providing a tool that not only verifies citations but also provides detailed reasoning and remedial actions.

**Method:** Method of the paper involves the development of SemanticCite, an AI-powered system that verifies citation accuracy via full-text source analysis, using a combination of multiple retrieval methods and a four-class classification system (Supported, Partially Supported, Unsupported, Uncertain) to capture claim-source relationships.

**Result:** Fine-tuned lightweight language models achieve performance comparable to large commercial systems with significantly lower computational requirements, demonstrating the practical feasibility of large-scale citation verification.

**Conclusion:** The paper concludes that SemanticCite provides a scalable solution for citation verification, which not only increases research integrity but also aids in peer review and quality control for AI-generated content, offering an open-source dataset and verification framework.
...
> Develops SeSE, a novel uncertainty quantification method for detecting possible hallucinations in large language models, by analyzing semantic structural information, showing superior performance over existing approaches in various experiments.

<details>
  <summary>Details</summary>

**Motivation:** The paper seeks to address the limitations of existing UQ methods that fail to capture latent semantic structural information, which could lead to more accurate uncertainty estimates and help mitigate the risk of LLMs producing falsehoods in critical scenarios.

**Method:** This paper introduces Semantic Structural Entropy (SeSE), a UQ framework for detecting hallucinations in LLMs by quantifying intrinsic semantic uncertainty from a structural perspective. It includes a graph construction algorithm for modeling semantic spaces and a hierarchical abstraction method to calculate the structural entropy of the optimal semantic encoding tree.

**Result:** Experiments demonstrate that SeSE performs better than advanced UQ methods and can provide theoretically sound hallucination detection, especially in long-form generation.

**Conclusion:** SeSE effectively quantifies uncertainty in LLMs by leveraging semantic structural information, significantly improving upon current techniques and offering enhanced detection of potential hallucinations.
...
> 提出了一种名为SDA的无训练对齐框架，用于增强开源大语言模型的响应与人类意图的对齐，无需重新训练，适合广泛的模型，并在多个模型上展示了其效果。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型部署到实际应用的扩展，挑战在于确保这些模型生成的响应与人类意图相一致。特别地，在推理过程中有效地和高效地对齐模型行为，而不进行成本高昂的重新训练或广泛监督是一个关键需求和一项技术难点。因此，提出了SDA框架以应对这些挑战。

**Method:** 我们提出了一种名为SDA（Steering-Driven Distribution Alignment）的无训练且与模型无关的对齐框架，旨在对开源大语言模型进行操作。SDA在用户定义的对齐指令基础上动态重新分配模型输出概率，从而提高模型行为与人类意图之间的对齐，而无需微调。这种方法轻量级，资源效率高，适用于广泛范围的开源大语言模型，并且可以根据个性化偏好对模型响应行为进行灵活控制。

**Result:** 实验证明SDA在8个不同规模和来源的开源大语言模型上表现出了显著的改进。在3H维度上的性能评估中，SDA展示了其提升效果。

**Conclusion:** 实证结果表明，SDA在8个不同规模和来源的开源语言模型上，对3个关键对齐维度（也就是3H）：有用性、无害性以及诚实性表现出了显著的改进。SDA在有用性上平均提升了64.4%，在诚实性上提升了30%，在无害性上提升了11.5%，表明其有效性及在不同模型和应用情景中的泛化能力。
...
> 本文通过引入自我重写框架，解决了强化学习仅基于最终正确性奖励的限制，从而改善了大型推理模型在复杂推理任务上的内部推理质量，通过实验验证了其效果。

<details>
  <summary>Details</summary>

**Motivation:** 基于仅奖励最终正确性的强化学习限制了对内部推理过程的细致监督，导致了内部推理质量的次优，例如过度思考、思考不足、重复思考和思考混乱等问题。因此，为了改进内部推理质量，本文提出了自我重写框架。

**Method:** 通过自我重写框架，该框架允许模型重写其自身的推理文本，并从重写后的推理中学习，以提升内部思维过程的质量。我们提出了一种选择性重写方法，即仅重写由模型的一致正确性定义的“简单”样本，从而保留GRPO的所有原始奖励信号。在实际实现中，我们将重写和普通生成整合在一个批次中，这既保持了RL算法的可扩展性，又仅引入了约10%的开销。

**Result:** 在多个不同模型大小的任务中，实验验证了自我重写框架的有效性。在准确性-长度权衡方面，自我重写方法在没有明确缩短推理长度的指令下，实现了更短的推理（-46%）和更高的准确性（+0.6）。在内部推理质量方面，自我重写方法在大型语言模型作为裁判的指标下获得了更高的分数（+7.2），成功缓解了内部推理的缺陷。

**Conclusion:** 采用自我重写框架可以有效提升大规模推理模型在复杂推理任务中的表现，特别是在内部推理质量和准确性-长度权衡方面。这种方法在保持算法可扩展性的同时，可以通过选择性重写样本的方式，优化内部推理过程。
...
> 本研究结合现有的成语和比喻语言数据集创建了新的数据集，用于评估和训练语言模型以提高其处理成语和比喻语言的能力。

<details>
  <summary>Details</summary>

**Motivation:** 由于社交媒体使得非正式语言更加易于观察，然而大型语言模型在处理成语和比喻语言方面仍然存在困难。因此，需要构建更好的数据集与训练方法来改进这一状况。

**Method:** 本研究采用了最近的几个成语和比喻语言数据集来获取一个组合的成语列表，然后从一个大型语料库中检索出上下文序列。创建了一个人类标注的成语和比喻语言表达数据集，以便评估预训练语言模型处理比喻意义（识别任务）的基本能力。

**Result:** 通过后期处理，使得数据集对于各种模型的训练具有兼容性，并在插槽标注和序列标签任务中进行训练和评估。

**Conclusion:** 研究表明，更大的数据集有助于缩小大型语言模型理解和处理成语及比喻语言之间差距。
...
> 研究发现高度信息性的自然语言解释(rationales)并不一定能提升模型分类准确性；同时发现将rationale信息整合进模型输入虽然可以增加跨领域的分类性能，但一致性和效果有待商榷。表明相关指标的进一步研究的重要性。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机是在于探索自然语言rationales的重要性，识别模型是否能基于正确理由学习标签，或只是依赖数据集特定的捷径。特别关注了足够性指标对于评估rationales信息量的局限性。

**Method:** 采用两种建模范式——标记分类和注意力正则化，来更好地理解rationales的信息量以及它们对模型性能的影响。

**Result:** 该研究探索了人类对于自然语言解释(rationales)的理解，评估模型是否基于正确理由学习标签或依赖数据集特定的捷径。研究通过关联足够性(sufficiency)和两种建模范式：标记分类（识别属于rationale的标记）和通过注意力正则化（通过在输入中加入rationales提升模型性能），来解决足够性这一度量仅仅具有有限洞察力的问题。研究发现，高度信息性的rationales并不一定能帮助正确分类实例，而足够性这一指标实际上衡量的是非rationale上下文对分类的冲击。研究还发现，虽然在模型输入中增加rationale信息可以提高跨领域分类的性能，但这种效果取决于任务类型和模型类型而有所不同。此外，研究还表明，足够性和标记分类之间没有直接关联。这些结果强调了rationales的复杂性，提示需要进一步研究能够系统捕捉这类信息的指标。

**Conclusion:** 研究揭示了自然语言解释（rationales）的复杂性, 高度信息性的rationales并不能保证更高的分类准确性，也指出了衡量这种信息的指标需要进一步研究。
...
> 论文提出了一种新的HTML内容抽取流水线MinerU-HTML，它在多个方面显著优于当前广泛使用的基于启发式的抽取器。使用MinerU-HTML构建的AI-ready Common Crawl语料库，比基于Trafilatura提取构建的语料库在模型预训练实验中表现更好。

<details>
  <summary>Details</summary>

**Motivation:** 现有的网络语料库依靠基于启发式的方法来抽取内容，这种方法难以保持文档的结构并经常破坏结构化元素，比如公式、代码和表格。作者们提出改进内容抽取的质量对下游应用的效果提升与积极的数据筛选策略一样重要。

**Method:** 该论文提出了一种名为MinerU-HTML的新内容抽取流水线，该流水线将内容抽取问题重定义为一个序列标注问题，并使用一个0.6B参数的语言模型来解决。相比较基于密度的文本抽取方法，MinerU-HTML利用语义理解，并采用两级格式化流水线来明确对语义元素进行分类后再转化为Markdown格式。

**Result:** MinerU-HTML在7,887个注释网页的基准测试MainWebBench上达到了81.8%的ROUGE-N F1分数，相比之下Trafilatura的分数为63.6%。值得注意的是，MinerU-HTML在结构化元素（如代码块和公式）的保存率上也表现出色，分别达到了90.9%和94.0%。

**Conclusion:** 论文展示了HTML提取是网页语料库构建中的一个关键但往往被低估的组成部分，提出的方法显著改进了抽取质量，并为大规模语言模型的质量奠定了坚实基础。
...
> 研究探索了监督机器学习和深度学习模型是否能够有效地区分质量较低的新闻文章和质量较高的新闻文章。实验使用了包含1,412,272篇英文新闻文章的新数据集，并对三个传统机器学习分类器和三个深度学习模型进行了评估。结果显示，随机森林等传统机器学习分类器表现出不错的性能，而ModernBERT-large则达到最佳性能。研究结果表明，机器学习和深度学习分类器都能够在一定程度上有效区分世界各地新闻文章的质量。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机是对监督机器学习和深度学习模型在区分不同质量新闻文章方面的有效性进行探索。

**Method:** 研究使用了一个包含1,412,272篇2018-2024年间共通网络爬取的英文新闻文章的数据集，基于专家共识评分，对579个来源网站进行分类，形成低质量和高质量两类文章。使用了随机森林等三个传统机器学习分类器和ModernBERT-large等三个深度学习模型进行评估。

**Result:** Random Forest展示了不错的性能，获得0.7355的准确率和0.8131的ROC AUC。ModernBERT-large以0.8744的准确率和0.9593的ROC AUC表现最优，而DistilBERT-base则位居第二，准确率为0.8685，ROC AUC为0.9554。

**Conclusion:** 研究表明，传统的CPU基础机器学习分类器和深度学习分类器能够有效地区分全世界新闻文章的质量。
...
> 研究团队开发了ESGBench，用以评估ESG（环境、社会与治理）问答系统的可解释性，重点在于使用企业可持续性报告的数据，推进透明和可靠的ESG AI系统的研究。

<details>
  <summary>Details</summary>

**Motivation:** 通过提出ESGBench，旨在加速研究在透明和负责任的ESG焦点AI系统方面的进展。

**Method:** 我们提出了一种名为ESGBench的基准数据集和评估框架，用于评估使用企业可持续性报告的可解释ESG问答系统。该基准包括跨越多个ESG主题的基于领域的问答，并配有人类策划的答案和支持证据，以实现对模型推理的精细评估。

**Result:** 研究分析了现有最先进的LLMs在ESGBench上的性能，强调了事实一致性、可追溯性和领域对齐方面的关键挑战。

**Conclusion:** ESGBench的推出意在加速透明和负责任的ESG焦点AI系统的研究进展。
...
> 研究人员使用修改后的路径补丁算法，发现基于变压器的语言模型处理惯用表达的计算模式，验证了'惯用表达头部'和'增强接收'现象，揭示了变压器处理惯用表达的机制。

<details>
  <summary>Details</summary>

**Motivation:** 研究人员调查了变压器如何处理非组合语言，并提供了理解更复杂语法构造处理方式的途径。

**Method:** 本研究通过修改后的路径补丁算法发现电路，研究了基于变压器的语言模型处理惯用表达的方式。

**Result:** 研究结果表明，惯用表达的处理表现出独特的计算模式，包括频繁激活的'惯用表达头部'以及由于早期处理而增强的惯用表达标记之间的注意力，称为'增强接收'。

**Conclusion:** 这项研究揭示了变压器在处理惯用表达时的机制，为理解变压器如何平衡计算效率和鲁棒性提供了新的视角。
...
> Arctic-Extract是一款先进的模型，用于从商业文档中提取数据，即使在资源受限的硬件上也能表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 开发Arctic-Extract的目的是为了从扫描或数字生成的商业文档中提取结构性数据，同时保持较低的硬件资源需求。

**Method:** Arctic-Extract的训练协议和评估结果被详细描述，展示了其在文档理解方面的强大性能。

**Result:** 训练和评价结果表明Arctic-Extract在文档理解方面表现出色。

**Conclusion:** Arctic-Extract能够有效处理大量文档，并且可以在资源有限的设备上部署，是一个强大的工具。
...
> TurkColBERT shows that late-interaction models, such as ColmmBERT-base-TR, outperform dense encoders in Turkish information retrieval, with smaller parameter sizes and faster query times, establishing a new benchmark for the language.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this paper is the need to explore and evaluate neural information retrieval models in the context of Turkish, a morphologically rich language with limited resources, where dense bi-encoders are currently leading but late-interaction models have not been sufficiently examined.

**Method:** We introduce TurkColBERT, a benchmark for assessing dense encoders and late-interaction models in Turkish information retrieval. The method involves fine-tuning English and multilingual encoders on Turkish NLI/STS tasks, then adapting them into ColBERT-style retrievers with PyLate using the MS MARCO-TR dataset. Evaluation is done on five Turkish BEIR datasets.

**Result:** The findings indicate that smaller late-interaction models outperform dense encoders by a significant margin, achieving up to +13.8% higher mean Average Precision (mAP). The ColmmBERT-base-TR model in particular is noted for its performance and efficiency. MUVERA+Rerank indexing algorithm is shown to be faster and more accurate than PLAID.

**Conclusion:** TurkColBERT demonstrates that late-interaction models are capable of outperforming dense encoders in Turkish information retrieval, despite being significantly smaller. This work also highlights the effectiveness of MUVERA+Rerank for production environments, enabling low-latency retrieval. The study's limitations are identified as the use of moderately sized datasets and translated benchmarks, necessitating further evaluations on a larger scale.
...
> 本文提出了一种预测框架，可以通过大型语言模型（LLM）的激活状态预测文本的类别，并展示了使用浅层学习模型从LLMs中推断文本类别的可行性。

<details>
  <summary>Details</summary>

**Motivation:** 理解大型语言模型的关键在于确保其安全和有益的应用，而现有的挑战在于模型结构的可解释性和输出的人工评估难度。

**Method:** 使用Mistral-7B模型和两个数据集，研究基于LLM激活状态预测文本类别的方法。

**Result:** 在两个数据集中，使用scikit-learn分类器获得高达98%和71%的F1得分，结果在控制任务中持续超越。

**Conclusion:** 证明了使用浅层学习模型可以从LLMs中推断出文本类别的概念验证。
...
> 本研究挑战了ASR在临床对话中的标准评估方法，用LLM判断ASR的临床影响，优化后的模型达到了人类水平的90%准确性和显著的相关性。

<details>
  <summary>Details</summary>

**Motivation:** ASR在临床对话中的应用日益广泛，但标准评估仍依赖于词错率（WER）。因此，本研究调查了WER或其他常见指标是否与转录错误的临床影响相关。

**Method:** 通过专家临床医生将基准事实短语与ASR生成的对应短语进行比较，标注对话数据集中任何发现差异的临床影响。引入了使用GEPA优化的LLM-as-a-Judge来复制专家临床评估。

**Result:** 研究发现，WER和其他现有指标与临床医生分配的风险标签（无影响、轻微影响或显著影响）相关性较差。优化后的judge（Gemini-2.5-Pro）以90%的准确率和较强的Cohen's $κ$ 0.816达到了与人类相媲美的性能。

**Conclusion:** 本研究提供了一种验证的、自动化的框架，将ASR评估从简单的文本保真度扩展到重要的、可扩展的临床对话安全性评估。
...
> 提出了一种不需要人工标注训练数据的词义消歧方法，使用统计语言模型作为参考，并通过符号NLU系统生成的候选意义转换成自然语言查询LLM来选择适当的解释。

<details>
  <summary>Details</summary>

**Motivation:** 现有的词义消歧方法依赖于手工标注的数据，并且主要针对粗粒度的表示，这不适用于需要复杂推理的更丰富表示。

**Method:** Structure

**Result:** {
  "tldr": "提出了一种不需要人工标注训练数据的词义消歧方法，使用统计语言模型作为参考，并通过符号NLU系统生成的候选意义转换成自然语言查询LLM来选择适当的解释。", 
  "motivation": "现有的词义消歧方法依赖于手工标注的数据，并且主要针对粗粒度的表示，这不适用于需要复杂推理的更丰富表示。", 
  "method": "利用统计语言模型作为参考工具，将符号NLU系统生成的多个候选意义转换成可区分的自然语言措辞，用以查询大型语言模型（LLM）来确定符合上下文的解释。", 
  "result": "该方法被评估以证明其有效性。", 
  "conclusion": "该方法提供了一种有效的途径，在不需要人工标注的情况下，实现词义消歧。
}
...
> 我们比较分析了两种多模态RAG系统的检索方法，实验结果显示直接多模态嵌入检索相比基于大语言模型总结的方法在性能上有了显著的提升。

<details>
  <summary>Details</summary>

**Motivation:** 由于现有的多模态RAG系统在预处理阶段依赖于基于大语言模型的总结来将图像转换为文本，在向量数据库中存储文本表示，这种做法会导致上下文信息和视觉细节的损失，这对于下游检索和问答至关重要。

**Method:** 我们分析了两种针对多模态检索增强生成（RAG）系统的检索方法，包括基于文本片段检索的方法（该方法先将图像总结为文本再进行嵌入）和直接多模态嵌入检索的方法（该方法直接将图像存储在向量空间内）。

**Result:** 实验结果表明，直接多模态嵌入检索方法在MAP@5和nDCG@5上分别取得了绝对数值上13%和11%的提升，相对数值上提高了32%和20%，并且这种方法在准确性和事实一致性上优于基于大语言模型摘要的方法。

**Conclusion:** 这篇论文通过实验验证了直接存储图像的方法优干基于大语言模型的图像转文本方法，可以直接嵌入图像的嵌入方法对于保存视觉信息以供检索和推理非常有效。
...
> 本文提出一种名为Nemotron Elastic的框架，能够在单一模型中实现多个人工智能子模型的零损耗提取，使推理型大型语言模型的训练成本显著降低，同时满足不同规模和预算要求，并且优于当前最优压缩技术7倍。

<details>
  <summary>Details</summary>

**Motivation:** 由于训练满足不同规模和部署目标的大型语言模型家族的成本日益高涨，作者旨在开发一种能够降低这些成本的方法，通过模型压缩和知识提炼等技术减少训练成本。然而现有方法仍然昂贵，因此提出了通过Nemotron Elastic框架来显著减少整体训练成本。

**Method:** 通过引入Nemotron Elastic框架，作者提出了一种在单一主模型中嵌入多个人工智能子模型的方法。这些子模型共享主模型的权重，并且可以在部署时零成本提取，无需额外训练或微调。该框架使用了一个端到端训练的路由器，以及专门为推理模型设计的两阶段训练课程，包括群体感知SSM弹性化、异构MLP弹性化、基于归一化MSE的层数重要性选择和知识蒸馏等技术，以便实现同时针对多个预算的最优化。

**Result:** Nemotron Elastic框架被应用于Nemotron Nano V2 12B模型，仅用110B训练标记量，同时生成9B和6B模型，成本降低360倍，优于当前最优压缩技术7倍。每个嵌入模型在准确性方面表现与最优保持一致或更好。

**Conclusion:** 这篇文章提出的Nemotron Elastic框架展示了一种有效的方法，可以在单一模型中嵌入多个人工智能子模型。这让推理性大型语言模型的训练在面临多样化部署配置和预算的同时，极大地减少了成本，优于传统压缩方法，并且在部署时具有独特的多模型合一内存特性，无需额外训练。
...
> We propose UniFit, a universal VTON framework driven by a Multimodal Large Language Model (MLLM), which addresses the semantic gap and data scarcity challenges in multi-task VTON.

<details>
  <summary>Details</summary>

**Motivation:** Despite significant progress, building a universal VTON framework that can flexibly handle diverse and complex tasks remains a major challenge. Recent methods explore multi-task VTON frameworks guided by textual instructions, yet they still face two key limitations: (1) semantic gap between text instructions and reference images, and (2) data scarcity in complex scenarios.

**Method:** Specifically, we introduce an MLLM-Guided Semantic Alignment Module (MGSA), which integrates multimodal inputs using an MLLM and a set of learnable queries. By imposing a semantic alignment loss, MGSA captures cross-modal semantic relationships and provides coherent and explicit semantic guidance for the generative process, thereby reducing the semantic gap. Moreover, by devising a two-stage progressive training strategy with a self-synthesis pipeline, UniFit is able to learn complex tasks from limited data.

**Result:** Extensive experiments show that UniFit not only supports a wide range of VTON tasks, including multi-garment and model-to-model try-on, but also achieves state-of-the-art performance.

**Conclusion:** UniFit not only supports a wide range of VTON tasks, including multi-garment and model-to-model try-on, but also achieves state-of-the-art performance.
...
> EfficientSAM3 is a family of efficient models derived from SAM3 through Progressive Hierarchical Distillation, aimed at enabling on-device concept segmentation and tracking while maintaining high fidelity.

<details>
  <summary>Details</summary>

**Motivation:** There is a need to make SAM3, a model for Promptable Concept Segmentation across images and videos, efficient enough for on-device usage. This involves reducing the computational complexity of its unified architecture without significantly compromising its performance.

**Method:** EfficientSAM3 is created through a three-stage Progressive Hierarchical Distillation process: Encoder Distillation, Temporal Memory Distillation, and End-to-End Fine-Tuning. This transfers capabilities from SAM3 to lightweight student networks with various backbones like RepViT, TinyViT, and EfficientViT.

**Result:** {
  "tldr": "EfficientSAM3 is a family of efficient models derived from SAM3 through Progressive Hierarchical Distillation, aimed at enabling on-device concept segmentation and tracking while maintaining high fidelity.", 
  "motivation": "There is a need to make SAM3, a model for Promptable Concept Segmentation across images and videos, efficient enough for on-device usage. This involves reducing the computational complexity of its unified architecture without significantly compromising its performance.", 
  "method": "EfficientSAM3 is created through a three-stage Progressive Hierarchical Distillation process: Encoder Distillation, Temporal Memory Distillation, and End-to-End Fine-Tuning. This transfers capabilities from SAM3 to lightweight student networks with various backbones like RepViT, TinyViT, and EfficientViT.", 
  "result": "EfficientSAM3 achieves a balance between performance and efficiency, demonstrating strong performance on popular VOS datasets and comparable efficiency to related work.", 
  "conclusion": "Progressive Hierarchical Distillation enables the transfer of complex model capabilities to lightweight architectures, making advanced visual understanding tasks like promptable concept segmentation and tracking feasible for on-device applications. EfficientSAM3 showcases this potential."]
...
> 本文提出了四种改进的基于模型的6D姿态估计方法，提高泛化能力和遮挡条件下的准确性与鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 提高遮挡条件下6D物体姿态估计的准确性与鲁棒性，尤其是在未见过的物体上实现泛化。

**Method:** 动态非均匀密集采样策略、多假设推理机制、迭代细化和系列专注于遮挡的训练增强。

**Result:** 提出的方法在ICBIN基准测试上提高了超过5%的准确性，在BOP数据集上提高了超过2%，同时实现了大约3倍的推理加速。

**Conclusion:** 该方法对于在遮挡情况下的物体姿态估计是一个重要的进展。
...
> 研究开发了自动深度生成方法和手动随机退化技术来生成特定类型的历史地图的训练数据，用于土地覆盖解释，并使用自构图卷积网络进行领域适应性语义分割评估。

<details>
  <summary>Details</summary>

**Motivation:** 历史地图上的自动分析深度学习方法依赖大量标注的数据集，而对于特定的同类型制图语料库，这类数据往往极为稀缺。合成数据虽然可以缓解样本稀缺问题，但通常缺乏真实性和多样性。

**Method:** 该研究提出了一个自动深度生成方法和一个手动随机退化技术来模拟历史地图扫描中常见的视觉不确定性与噪声。通过将原历史地图语料库的制图风格转换到矢量数据上，可以自动生成大量的合成历史地图，适用于同类型历史地图语料库的土地覆盖解释。

**Result:** 通过自构图卷积网络在同类型历史地图语料库上的领域适应性语义分割任务中成功验证了生成训练数据的有效性和适用性。

**Conclusion:** 该方法为解决历史地图分析中训练数据稀缺问题提供了一种有效解决方案，生成的数据可以显著提高对特定类型历史地图进行土地覆盖解释的能力。
...
> 本文提出了Box6D方法，针对仓库中存储箱的6D姿态进行高效且准确的估计，减少了计算成本并提高了实际应用中的效果。

<details>
  <summary>Details</summary>

**Motivation:** 准确且高效的6D姿态估计在面临复杂和部分遮挡环境中的新物体时对机器人操作至关重要，然而目前的方法存在一些局限性，或要求高分辨率模型，或在严苛条件下表现不佳，另外一些过于一般化的方法忽略了环境和目标物体的先验知识，限制了其在工业应用中的实用性。因此，需要一种新的方法来解决这些问题。

**Method:** Box6D提出了一种针对仓库环境中存储箱的类别级别6D姿态估计方法。该方法从单次RGB-D观测中，通过快速二分搜索推断出存储箱的尺寸，并使用类别CAD模板而不是实例特定模型来估计姿态。同时，通过深度基的合理性过滤器和提前终止策略来排除不合理假设，从而降低计算成本。

**Result:** 在实际存储情景和公开基准上进行的评估显示，Box6D方法能够提供具有竞争力甚至更优的6D姿态精确度，同时将推断时间减少了大约76%。

**Conclusion:** 综上所述，Box6D解决了现有方法在实际应用中存在的诸多限制，提供了更加实用的6D姿态估计解决方案，尤其在减少推断时间的同时保持了高精度。这显示了Box6D方法在仓库自动化、物流和电子商务等领域中的巨大潜力。
...
> A two-stage self-improvement paradigm for Vision Language Models that bridges the rationale gap in domain-specific video classification without new annotations, resulting in better performance.

<details>
  <summary>Details</summary>

**Motivation:** Vision Language Models struggle with domain-specific video classification tasks due to a rationale gap caused by sparse domain data. This research aims to fill this gap without requiring new annotations.

**Method:** First, the VLMs generate detailed textual rationales for each video to articulate the domain-specific logic. Then, the VLM is fine-tuned on these self-generated rationales to align its representations with the target domain's nuances. Finally, conventional supervised fine-tuning is performed on the task labels.

**Result:** The method significantly outperforms direct supervised fine-tuning on diverse datasets, validating self-generated rationale as an effective and annotation-efficient way to adapt VLMs to domain-specific video analysis.

**Conclusion:** Our two-stage self-improvement paradigm effectively bridges the rationale gap through the use of self-generated rationales, enhancing video classification performance in domain-specific tasks.
...
> 提出Multi-Granular Language Learning（MGLL）框架，以改善多标签和跨粒度对齐问题。

<details>
  <summary>Details</summary>

**Motivation:** 对比语言-图像预训练模型（如CLIP）存在单标签和单粒度对齐的局限性，特别是在医学影像领域，图像往往对应多个高层次标签和不同层次的注释，因此提出MGLL以改进现有模型的这些问题。

**Method:** MGLL使用结构化的多标签监督，集成跨粒度的文本描述，并引入了带有点对约束的软标签监督以加强对齐。它利用平滑的KL散度来确保跨粒度一致性，同时保持计算效率作为视觉-语言模型的即插即用模块。

**Result:** 经过在大规模多粒度数据集上预训练和多数据集评估，MGLL在下游任务中超越了其他最先进的方法。

**Conclusion:** MGLL解决了多标签和跨粒度对齐问题，提高了视觉-语言模型在复杂数据集上的性能。
...
> 本文提出了一种从3D心脏超声图像中自动选择标准2D视图的方法，这种方法结合了深度学习和医生的启发式规则，并通过临床测试验证了其准确性。

<details>
  <summary>Details</summary>

**Motivation:** 尽管心脏具有复杂的三维（3D）解剖结构，但传统的医学成像依赖于一系列2D视频来展示单个心脏结构。3D超声心动图是一种新兴的成像模式，如今提供足够的图像质量用于临床应用，并有可能简化获取过程和提升诊断不规则结构的能力。本研究旨在结合3D扫描的速度和易用性，让医生以熟悉的格式解读数据。

**Method:** 我们提出了一种自动化方法，从3D心脏超声体数据中选择标准的2D视图，该方法结合了深度学习视角分类器和基于解剖标志的后续启发式方法，以及心脏病专家提供的启发式方法，以重构标准的心脏超声视图。

**Result:** 该方法由三位心脏病专家在盲测中验证（1,600个视频，两个医院的准确率为96%），下游的2D视频也通过AI心脏超声模型（EchoPrime和PanEcho）的能力来检测心脏异常，并产生临床级的心脏解剖测量（EchoNet-Measurement）。

**Conclusion:** 研究结果表明，提取的2D视频保留了空间校准和诊断特征，允许临床医生从3D体积中获得准确的实际解释。同时，研究发布了包含29个3D超声数据集的代码。
...
> Click2Graph是一个交互式的全景视频场景图生成框架，它将可提示分割模型的优点与时空和语义理解相结合，提供了一个基于用户单次提示的可控和可解释的视频场景理解系统。

<details>
  <summary>Details</summary>

**Motivation:** 最先进的视频场景图生成系统虽然能提供结构化的视觉理解，但没有能力整合人类指导。相比之下，可提示分割模型如SAM2能够进行精准的人机交互，但缺乏语义或关系推理。因此，提出了Click2Graph框架，实现视觉提示与空间、时间和语义理解的统一。

**Method:** 从单个用户提示（如点击或边界框）出发，Click2Graph框架能够对主体进行分割和时序跟踪，自主发现交互对象，并预测出<主体，对象，谓语>三元组，从而形成时间一致的场景图。该框架引入了两个关键技术组件：动态交互发现模块，用于生成主体条件下的对象提示；语义分类头，用于执行联合实体和谓语推理。

**Result:** 在OpenPVSG基准上的实验表明，Click2Graph为用户引导的全景视频场景图生成提供了坚实的基础，证明了如何将人类提示与全景定位和关系推理相结合，实现可控和可解释的视频场景理解。

**Conclusion:** Click2Graph首次提出了一个在单一用户提示下能够生成全景视频场景图的交互框架，能够自主发现交互对象并预测<主体，对象，谓语>三元组，展示出将人类提示与全景定位、关系推理相结合的巨大潜力。
...
> 研究提出了InfoCLIP方法，通过信息论角度优化了CLIP模型在开放词汇语义分割任务中的微调过程，提升了模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的方法通过在有限的已见类别上对CLIP进行微调来进行分割时，通常会导致过拟合并降低预先训练好的视觉-语言对齐能力。为解决这一问题，研究提出了InfoCLIP方法。

**Method:** InfoCLIP利用信息论的角度，通过两个基于互信息的新目标来转移预先训练的CLIP的对齐知识至分割任务。首先，将CLIP的像素-文本跨模态对齐进行压缩，减少由于图像-文本监督下的粗粒度局部语义表示带来的噪声。其次，最大化预先训练的CLIP与微调模型之间的互信息，以转移适合分割任务的紧凑局部语义关系。

**Result:** 研究通过引入InfoCLIP方法，利用信息论的角度将预先训练的CLIP模型的跨模态对齐知识迁移到分割任务中，有效避免了过度拟合问题，并提升了开放词汇语义分割的效果。实验结果表明该方法在不同基准上表现出了有效的性能提升。

**Conclusion:** 实验结果表明，InfoCLIP在不同基准上有效增强了CLIP的微调过程，展现了其在开放词汇语义分割任务中的适应性和优越性。
...
> 提出了一种新的多任务学习一致性正则化方法，以缓解乳腺超声图像中的分割和分类任务之间的破坏性干扰，显著提高了泛化性能。

<details>
  <summary>Details</summary>

**Motivation:** 多任务学习可能会导致任务间的破坏性干扰，从而影响模型的泛化性能。为了解决这一问题，提高乳腺超声图像分割的泛化性能，提出了新的方法。

**Method:** 通过采用可微分的BI-RADS灵感形态特征组成的一致性正则化方法来缓解分割与分类任务之间的破坏性干扰。

**Result:** 在BrEaST数据集上训练后，在UDIAT、BUSI、BUS-UCLM的三个外部数据集上的验证表明，该方法在分割任务上比基线方法具有显著的统计学意义提升。

**Conclusion:** 所提出的方法在外部数据集上达到了最先进的分割性能，并证明了其解决多任务学习中的破坏性干扰问题的有效性。
...
> 本文介绍了检测引导的生成框架，显著提升了大规模电子商务场景下的细粒度视觉语义理解性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前先进的方法依赖于全局相似性，难以捕捉细粒度类别差异和类别特定属性多样性，特别是在大规模电子商务场景中。

**Method:** 提出了一种基于检测引导的生成框架，该框架可以预测分层类别和属性令牌。对于每个检测到的对象，该框架通过提取精细的ROI级特征并使用基于BART的生成器，以粗到细的序列生成涵盖类别层次和属性-值对的语义令牌，支持条件属性识别。

**Result:** 实验结果表明，该方法在大规模专有电子商务数据集和开源数据集上均显著优于现有的基于相似性的管道和多阶段分类系统，在细粒度识别和统一推理方面表现更强。

**Conclusion:** 所提出的方法能够实现细粒度识别和统一推理，特别适用于大规模电子商务场景中的视觉语义理解。
...
> 研究提出公平意识示范选择（FADS），利用聚类采样构建平衡示范集，实验表明其有效减少医学图像推理中的性别、种族和族群差距，同时保持高准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的去偏见方法通常依赖于大规模的标签数据或微调，对于基础规模的模型来说是不切实际的。因此，探索无需微调的轻量级方法来提高公平性。

**Method:** 通过系统性分析，发现了传统示范选择（DS）策略无法确保公平性的问题，并提出公平意识示范选择（FADS），利用聚类采样构建人口统计平衡且语义相关的示范集。

**Result:** 实验结果表明，FADS 能够一致地减少性别、种族和族群相关的差距，并维持强大的准确性。

**Conclusion:** 研究表明，具有公平意识的上下文学习可以作为一种可扩展且数据高效的解决方案，为公平的医学影像推理提供了有效的途径。
...
> The paper proposes a method that leverages graph-based representations and GCNs to improve OOD detection in vision datasets with long-tailed distributions.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this research is to improve out-of-distribution (OOD) detection in the context of long-tailed in-distribution (ID) datasets, where existing methods suffer from high false positive rates and poor tail-class classification accuracy.

**Method:** The method involves using a graph-based representation initialized from the feature space of a pre-trained model. Gaussianization is applied to standardize the activation layer distribution, and graph convolutional networks (GCNs) are used to refine the graph, making it suitable for long-tailed OOD detection.

**Result:** Experiments on CIFAR10-LT, CIFAR100-LT, and ImageNet-LT show significant improvements in FPR and tail-class ID classification accuracy, outperforming existing methods.

**Conclusion:** The conclusion is that exploiting inter-sample relationships in the form of graph-based representations and refining them with GCNs can effectively enhance the performance of OOD detection in long-tailed datasets.
...
> 引入序列级别优化框架生成可打印自然对抗性纹理，有效减少长时间视频序列中的人类检测信心，实验结果验证其强大隐蔽性、稳定性及实际应用可行性。

<details>
  <summary>Details</summary>

**Motivation:** 深度神经网络在人类检测中容易受到对抗性操作的攻击，这在实际监控环境中产生了安全和隐私风险。本研究旨在解决穿戴攻击下的持续隐蔽性问题，提供一种有效的对抗策略。

**Method:** 本研究引入了一个序列级别的优化框架，用于生成自然、可打印的对抗性纹理，这些纹理可以在整个行走视频中保持有效性。首先，产品图像被映射到UV空间，并转换为紧凑的调色板和控制点参数化表示。接下来，使用基于物理的人体-服装管道来模拟运动、多角度摄像机视角、布料动力学和照明变化。通过权衡时间的影响，采用变换预期优化目标来优化控制点，使得在整个序列中的检测信心最小化。

**Result:** 实验结果表明，该方法具有强大的隐蔽性和稳定性，能够有效地对抗视角变化，并展示了优秀的跨模型传输性。通过升华打印技术制作的实物衣物在室内和室外的视频记录中均发挥了可靠的检测抑制效果，证实了该方法的实际应用可行性。

**Conclusion:** 此研究提出的方法可在长视频序列中生成保持隐蔽性的自然、可打印的对抗性衣物纹理，增强了对抗性攻击的实际应用能力。
...
> 本文提出了一个用于单步实时图像超分辨率的Mixture-of-Ranks (MoR) 架构，解决了现有方法的局限性，通过灵活的知识重组和退化感知负载均衡实现了高效的图像重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的Real-ISR方法主要依赖于通过LoRA模块微调预训练的扩散模型来重建高分辨率图像，但这些密集模型在适应复杂现实世界降质样本的异质特性或在等价计算预算下实现输入间知识共享方面存在局限性。

**Method:** 本文提出了一种名为Mixture-of-Ranks (MoR) 的架构，以应用于单步图像超分辨率任务。通过将LoRA模块中的每个秩视为独立的专家，并引入细粒度的专家划分策略来实现灵活的知识重组。同时，固定位置的秩被作为共享专家，以保留常识特征并减少路由冗余。此外，还设计了退化估计模块，利用CLIP嵌入和预定义的正负文本对来计算相对退化分数，动态指导专家激活。为了更好地处理不同的样本复杂度，引入了零专家槽，并提出了一种退化感知的负载均衡损失，动态调整激活专家的数量以实现最优的计算资源分配。

**Result:** 该研究通过实验验证了所提框架的有效性和先进性能。

**Conclusion:** 研究展示了将稀疏MoE整合到Real-ISR中的有效性，并通过提出的新架构Mixture-of-Ranks (MoR) 实现了对不同样本复杂度的适应性及最优计算资源分配，验证了其卓越的性能。
...
> 一篇论文提出了一个基于深度学习的材料分类技术，采用散斑感应来实时监控激光切割过程并识别不同的材料类型。实验结果显示该方法在材料分类中表现良好，具有较强的鲁棒性和准确性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的材料分类方法在激光颜色变化时可能会遇到问题，阻碍激光切割过程的监控和控制。

**Method:** 使用深度学习对材料的表面散斑进行分类的技术，通过训练卷积神经网络(CNN)识别不同类型的材料，以实现安全和高效的切割过程。

**Result:** 实验结果显示该方法在材料分类中能实现高精度，即使激光颜色发生改变，训练集准确率为98.30%，验证集准确率为96.88%，在包含3000张新图像的30种不同材料上的F1得分为0.9643。

**Conclusion:** 整个方法为使用散斑感应的材料感知激光切割提供了一个健壮且准确的解决方案。
...
> CuriGS提出一种课程指导方法，利用假视图（学生视图）提高3DGS在稀疏视角设置下的渲染保真度和几何一致性，优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 3D高斯散点表示虽然高效，但在稀疏视角设置下由于监督稀少和过拟合问题难以扩展，CuriGS旨在解决这一挑战。

**Method:** CuriGS采用课程指导框架，通过引入学生视图（假视图）解决稀疏视角下的3D重建问题。学生视图通过不同程度的扰动生成，训练过程中逐步增加扰动水平，并通过深度相关性和共同正则化进行正则化。

**Result:** 实验结果表明，CuriGS在合成和现实稀疏视角场景中达到高保真的渲染和几何一致性，超越了当前最先进方法。

**Conclusion:** CuriGS通过动态增加训练视图的扰动水平和质量阈值，成功增强稀疏视角下的3D重建性能。
...
> 该研究提出了一种跨模态学习方法，利用高分辨率卫星影像结合无人机级别视觉细节，显著提高了作物冠层性状估计的精度，特别是在产量和氮含量预测任务中。

<details>
  <summary>Details</summary>

**Motivation:** 鉴于无人机和卫星在农业监测中的不同优势和限制，这项工作旨在通过跨模态学习策略，弥补卫星和无人机传感之间的差距。

**Method:** 本研究提出了一种跨模态学习策略，通过结合高分辨率卫星影像和无人机级别的视觉细节来进行作物冠层性状估计。

**Result:** 实验结果显示，从卫星输入生成的类似无人机表示在多个下游任务中，包括产量和氮含量预测，均优于真实的卫星影像。

**Conclusion:** 研究结果表明跨模态对应学习具有潜在用途，可以用于连接卫星和无人机在农业监测中的传感能力。
...
> 本文介绍了一种新框架，使用大型语言模型解析和投影食品图像以生成更好的识别结果，表现出优于针对特定问题设计的传统方法的效能。
...
食品图像识别面临着几个关键问题：训练样本与实际应用场景中的图像差异（跨域问题）、实际食品数据集的长尾分布特性，以及不同类别食品之间的细微视觉差异。这些问题阻碍了高效识别系统的构建。因此，作者提出了这一结合了LLMs的新框架，旨在提升在这些具有挑战性条件下的识别性能。

通过该框架，作者希望解决现有方法在面对这些复杂情况时难以有效发挥作用的问题。

**Method:** 方法：

该论文介绍了一个结合大型语言模型的新框架，首先使用LLMs解析食品图像以生成食品名称和组件。其次，将不同域的生成文本和图像投影到同一个嵌入空间，确保配对相似度最大化。最后，结合两个模态对齐过的特征来进行识别。该方法力求简化问题解决步骤，同时增强对复杂食品图像数据库的识别能力。


**Result:** 内容概述：

本论文提出了一种结合大型语言模型（LLMs）的框架，用于解决食品图像识别领域中的挑战，例如跨域问题、数据长尾分布以及难以辨别的细小分类差异。该方法首先利用LLMs解析食品图像，生成食品名称和成分，然后将不同域的文本及图像投影到一个共享的嵌入空间中，以最大化配对相似度，最后融合两个模态的对齐特征进行识别。

此框架在两个食品数据集上的实验结果表明，相比于现有的针对长尾分布数据集、领域迁移及细分类的定制算法，所提出的方法表现更优。

**Conclusion:** 结论：

实验结果显示，所提方法在处理长尾分布、跨域和细分类的食品图像识别任务上优于现有的专门算法。这表明，结合大型语言模型，并利用生成文本和图像对进行嵌入空间投影的方法能有效改善食品图像识别的整体性能。
...
> 本文提出了一种尺度自适应KV缓存策略（AMS-KV），该方法显着减少了KV缓存的使用和自注意力延迟，提高了多尺度图像生成模型的可扩展性。

<details>
  <summary>Details</summary>

**Motivation:** 解决了现有规模增加导致的KV内存快速增加的问题，提高了图像生成模型的可扩展性。

**Method:** 介绍了一种名为AMS-KV的尺度自适应KV缓存策略，用于VAR模型中的下一尺度预测。该策略优先存储浓缩尺度和本地尺度的KV，以保持生成质量，并通过跨尺度相似性分析优化KV缓存和计算效率。

**Result:** 相较于普通的基于下一尺度预测的VAR模型，AMS-KV将KV缓存使用减少了高达84.83%，自注意力延迟减少60.48%。同时，它使得基准VAR-d30模型在批处理大小为128时不会出现内存溢出的问题，并且能够稳定扩展到批处理大小为256，提高了吞吐量。

**Conclusion:** AMS-KV策略有效地解决了多尺度图像生成中的KV内存增长问题，提高了图像生成质量和模型的计算效率，能够以较小的内存消耗实现模型的稳定运行，并提高了批处理大小和整体吞吐量。
...
> LiSTAR通过一种混合圆柱球形表征和空间-时间注意力射线为中心的变压器以增强数据保真度和捕捉复杂动态。该方法在4D LiDAR数据的生成中取得显著提升，显著改善了生成、重建和预测的性能，提供了新的可靠基础。

<details>
  <summary>Details</summary>

**Motivation:** 合成高保真且可控的4D LiDAR数据对于构建自动驾驶车辆的可扩展模拟环境至关重要。由于传感器在球形几何结构、点云的时间稀疏性及动态场景的复杂性等方面带来的挑战，这项任务本身具备一定的难度。

**Method:** LiSTAR 采用了一种混合圆柱球形 (HCS) 表征来保持数据保真度，通过使用由个体传感器射线沿各射线进行特性演化的空间-时间注意力射线为中心的变压器（START）来捕获稀疏的时序数据中的复杂动态。为了实现可控生成，提出了一种新颖的与4D点云对齐的体素布局以及对应的离散掩模生成START（MaskSTART）框架，这可以学会场景的紧凑标记化表示，从而实现高效、高解析度、布局引导的生成合成。

**Result:** 全面的实验证实了LiSTAR在4D LiDAR重建、预测和条件生成中达到了最先进的性能，大幅度提高了生成质量：生成MMD减少了76%，重建IoU提高了32%，预测L1 Med降低了50%。

**Conclusion:** 考虑到LiSTAR在实验中达成的高性能，它为创建具备现实感和可控性的自主系统模拟提供了一个强有力的新基础。
...
> 本文介绍了一个新的视频推理分割框架VideoSeg-R1，该框架通过引入强化学习，新设计了一个流程来改进传统的视频推理分割方法。实验表明，VideoSeg-R1在复杂的视频推理与分割任务中达到了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统视频推理分割方法依赖于监督微调，这限制了其泛化能力和显式的推理能力。为了应对这一点，研究者提出了VideoSeg-R1框架，旨在通过引入强化学习来提高这些方面的性能。

**Method:** VideoSeg-R1框架采用解耦构架，将任务分为三个阶段：1.基于分层文本引导的帧抽样模拟人类注意力；2.推理模型生成空间线索和显式推理链；3.利用SAM2和XMem工具进行分割传播。同时，引入任务难度感知机制，以动态调节推理长度来优化效率和准确性。

**Result:** 该论文提出了一种名为VideoSeg-R1的视频推理分割框架，创新性地引入了强化学习，解决了传统方法依赖监督微调导致的泛化能力和显式推理能力不足的问题。框架采用分阶段策略，建立在解耦架构上，将任务分为参考图像分割与视频掩模传播联合处理。包括三个阶段：基于分层文本引导的帧抽样模拟人类注意力；推理模型生成空间线索及显式推理链；使用SAM2和XMem进行分割传播。通过任务难度感知机制，动态调整推理长度，以提高效率和准确性。实验结果表明，VideoSeg-R1在复杂的视频推理和分割任务中达到了最先进的性能。代码将在https://github.com/euyis1019/VideoSeg-R1公开。

**Conclusion:** VideoSeg-R1在多项基准测试中实现了领先的性能，证明了其在复杂视频推理与分割任务中的有效性与优越性。
...
> SpectralTrain is introduced as a training framework to improve the efficiency of hyperspectral image (HSI) classification by integrating curriculum learning with PCA-based spectral downsampling.

<details>
  <summary>Details</summary>

**Motivation:** To address the computational challenges associated with HSI classification, which hinder the practical application of deep learning models in real-world scenarios.

**Method:** SpectralTrain employs a stepwise approach using curriculum learning to progressively introduce spectral complexity while maintaining crucial information, combined with PCA for spectral downsampling, to streamline the learning process.

**Result:** The framework shows significant reductions in training time with minimal impacts on accuracy across multiple datasets, showcasing its versatility and capability.

**Conclusion:** The framework emphasizes that strategic training optimization can enhance the performance of HSI models, offering both computational savings and adaptability across different applications, such as cloud classification, opening new potentials in climate-related remote sensing.
...
> Rad-GS系统通过结合雷达和相机数据，采用3D高斯空间表示，实现千米级别的室外环境稳健映射。

<details>
  <summary>Details</summary>

**Motivation:** 开发一种名为Rad-GS的4D雷达-相机SLAM系统，旨在利用3D高斯作为可微分的空间表示，为千米规模的户外环境提供鲁棒的标绘能力。

**Method:** Rad-GS系统结合了雷达点云及其多普勒信息与几何增强的点云，用于动态物体的遮罩引导，在同步图像中减少渲染伪影并提高定位精度。此外，系统还利用不同步的图像帧来全局优化3D高斯表示，增强纹理一致性和新视角合成的保真度。全局八叉树结构与目标高斯基元管理策略进一步降低了大规模环境中的噪声和内存消耗。

**Result:** 实验和消融研究表明，Rad-GS在性能上与基于相机或LiDAR输入的3D高斯方法相当，证明了利用4D毫米波雷达进行户外大规模标绘的可行性。

**Conclusion:** 千米级的真实世界重建验证了Rad-GS系统在大规模场景重建中的潜力。
...
> 本研究提出了一种名为T2T-VICL的跨任务VIIL框架，通过生成和选择优美的文本提示来描述视觉任务间的差别，并构建了一个新的数据集。实验结果表明，该框架在跨任务VIIL中表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 当我们提供的视觉提示和目标图像来自不同的视觉任务时，VLM能否仍然实现有效的VICL？为此，本研究的动机在于探索VLM在跨任务VIIL能力的潜力。

**Method:** 我们提出了一种完全协作的管道T2T-VICL，用于研究VLM在跨任务VICL中的潜力。我们设计了一种机制来生成和选择能最好地隐式描述两个不同低级视觉任务之间差异的文本提示，并构建了第一个跨任务VICL数据集。在此基础上，我们提出了一个结合感知得分推理和传统评估指标的新型推理框架，以执行跨任务VICL。

**Result:** 本方法在九个跨任务场景中取得了顶级成绩，并在另外十个场景中表现第二级，解锁了VLM跨任务VIIL的边界。

**Conclusion:** 本研究展示了VLM在跨任务VIIL方面的潜力，并证明了所提框架在整个跨任务VIIL场景中的有效性。它为未来的研究展开提供了重要的基准。
...
> This paper presents an improved 4D Gaussian Splatting method that enhances the representation of dynamic scenes by refining dynamic regions and improving the consistency of splat-object mapping.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to improve the reconstruction of dynamic scenes using 4D Gaussian Splatting by overcoming issues of ambiguous pixel correspondences and insufficient density in dynamic regions.

**Method:** Our method includes two main components: 1) Elliptical Error Clustering and Error Correcting Splat Addition for identifying and improving dynamic regions, and 2) Grouped 4D Gaussian Splatting for enhancing consistency in the representation of dynamic objects. Errors are classified and corrected using backprojection or foreground splitting.

**Result:** Testing on Neural 3D Video and Technicolor datasets shows a notable enhancement in temporal consistency and perceptual quality, achieving the best PSNR improvement, 0.39dB, on the Technicolor Light Field dataset.

**Conclusion:** The approach successfully increases temporal consistency and perceptual quality of rendered dynamic scenes, showcasing improved splat-object alignment and effective error correction.
...
> 本文提出了DCS-LDM，一种新的视觉生成范式，它解耦了信息复杂度与规模，实现跨多样规模和视觉质量的灵活生成，同时保持高性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的潜在扩散模型通常将规模与内容复杂度结合在一起，使用更多的潜在标记来表示更高分辨率的图像或更高帧率的视频。然而，表示视觉数据所需的潜在容量主要取决于内容的复杂度，而规模仅仅是一个上限。因此，提出了一种新的视觉生成范式DCS-LDM，它将信息复杂度与规模解耦。

**Method:** DCS-LDM构建了一个分层的、独立于规模的潜在空间，通过多级标记来建模样本复杂度，并支持在固定潜在表示下解码到任意分辨率和帧率。

**Result:** 实验结果显示，DCS-LDM在性能上能达到最先进的方法水平，同时提供灵活性生成跨不同规模和视觉质量的内容。

**Conclusion:** DCS-LDM支持一种逐步从粗糙到精细的生成范式，同时能实现计算与质量之间的灵活平衡。
...
> VTinker是一种专为解决高分辨率视频帧插值中流模糊、误对齐和不连续性问题的新方法，通过引导流上采样和纹理映射实现更清晰的插值帧。代码开源。

<details>
  <summary>Details</summary>

**Motivation:** 解决高分辨率帧中基于流的视频帧插值中存在的模糊、马赛克、任务相关流的误对齐以及插值帧中幽灵图像和不连续性的问题。

**Method:** 提出一个新的VFI流水线VTinker，包含两个关键组件：引导流上采样（GFU）和纹理映射。该方法通过将输入帧作为引导来改善流边缘的清晰度，并通过生成一个中间代理来辅助选择清晰的纹理块用于插值。

**Result:** ��理解该论文的摘要如下：

**摘要**：由于高分辨率帧像素移动大且计算成本高，估计其运动变得具有挑战性。因此，大多数基于流的视频帧插值(VFI)方法首先在低分辨率下预测双向流，然后使用高倍放大采样（例如双线性）来获得高分辨率的流。然而，这种采样策略可能导致流边缘模糊或马赛克化。另外，在低分辨率下进行运动估计无法充分捕捉高分辨率下细纹理的运动，这使得任务导向的流出现误对齐。通过这种不准确的流对输入帧进行像素级的扭曲和组合，会导致插值帧中出现幽灵图像和不连续性。在这项研究中，我们提出了一种新颖的VFI流水线VTinker，它包括两个核心组件：引导流上采样(GFU)和纹理映射。在低分辨率运动估计之后，GFU引入输入帧作为引导，以减轻双线性上采样的细节模糊，使流的边缘更加清晰。随后，为了避开像素级的幽灵图像和不连续性，纹理映射生成一个初步插值帧作为中间代理。这个代理用作选择输入帧中清晰纹理块的提示，然后将这些纹理块映射到代理上以借助重建模块生成最终的插值帧。广泛的实验证明，VTinker在VFI方面达到了最先进的性能。代码可以在这里找到: https://github.com/Wucy0519/VTinker。

**Conclusion:** VTinker流水线通过引导流上采样和纹理映射解决了高分辨率VFI中的问题，实现了在VFI方面的最先进性能。
...
> 研究提出了一个新的任务和数据集，以更准确地评估现有的多模态大型语言模型在识别社交互动中欺骗行为的能力。虽然模型有性能差距，但通过设计新的推理管道和记忆模块，获得了性能提升，这是迈向构建具备人类社会推理能力的MLLMs的重要一步。

<details>
  <summary>Details</summary>

**Motivation:** 鉴于现有的多模态大型语言模型在评估复杂社交互动中的欺骗行为方面表现不佳，论文提出了新的任务和数据集，以量化并改善这一不足。

**Method:** 设计了一种称为Social Chain-of-Thought (SoCoT) 的推理管道和一个Dynamic Social Epistemic Memory (DSEM) 模块，以改善现有模型在识别欺骗方面的表现。

**Result:** 引入了Multimodal Interactive Deception Assessment (MIDA) 任务和包含同步视频与文本的新数据集，并建立了全面的基准测试，评估了12个最先进的开源和闭源MLLMs。研究发现，包括GPT-4o在内的强大模型在区分真相与谎言方面都有显著性能差距。运用SoCoT和DSEM，在识别欺骗方面取得了一定的性能提升。

**Conclusion:** 提出了未来的多模态大型语言模型应具备更好的多模态社会线索能力和对他人知识、信念和意图的理解，这对建立更具感知能力和信任度的AI系统至关重要。新设计的框架展示了迈向更精准、更像人类的社会推理MLLMs的未来发展方向。
...
> 我们提出了一种新的方法，通过向特征空间中注入正激励噪声，改善了对AI生成图片的检测能力，提高了检测的泛化性能，并在实验中达到了新的状态水平。

<details>
  <summary>Details</summary>

**Motivation:** 尽管在检测AI生成的图像方面做出了大量努力，但离分布泛化仍然是一个持久的挑战。我们发现这一弱点源于在训练过程中被利用的虚假捷径，同时我们也观察到特征空间的小扰动可以缓解捷径的主导。

**Method:** 我们提出了一种名为PiN-CLIP的方法，它在变分正激励原则下联合训练噪声生成器和检测网络。通过在特征空间中使用视觉和类别语义特征的交叉注意力融合生成正激励噪声，并将噪声注入到特征空间中以微调视觉编码器，抑制敏感于捷径的方向，同时放大稳定的取证线索，从而能够提取出更为健壮和普遍适用的人工痕迹表示。

**Result:** 在包含由42种不同生成模型生成的合成图像的数据集上进行了比较实验，实验表明我们的方法以平均精度提高5.4达到了新的最先进的性能。

**Conclusion:** 我们的方法不仅解决了训练时虚假捷径的问题，还通过引入正激励噪声来增强检测网络对生成图像的区分能力，提高了检测的泛化能力。实验结果表明，我们的方法在开放世界数据集上取得了比现有方法更高的性能，尤其是在平均精度上有显著提高。
...
> 本文提出了一种新的混合视觉语言模型TimeViper，旨在解决长时间视频理解的挑战。通过结合Mamba和Transformer架构、提出TransV模块，TimeViper能够在处理大量帧的同时保持多模态理解能力，并在多个测试中取得优异成绩。

<details>
  <summary>Details</summary>

**Motivation:** 为了处理长时间视频的挑战，需要结合高效的模型结构和处理长时间序列上下文的机制，研究者旨在提供一种能够处理长时间视频并且保持多模态理解能力的模型。

**Method:** TimeViper采用了一种混合的Mamba-Transformer骨干架构，结合了状态空间模型的高效性和注意力机制的表达能力。研究者观察到随着大型语言模型深度的增加，视觉信息逐渐向文本信息汇聚的现象，并提出了TransV模块来传递和压缩视觉token到指令token中，这使得TimeViper能够处理超过10,000帧的一小时视频。

**Result:** 大量的实验证明，TimeViper在多个基准测试中能够与最先进的模型匹敌，同时适用于更大规模的帧数。

**Conclusion:** 该工作代表了开发、解释和压缩混合Mamba-Transformer架构的初始步骤，提供关于混合模型可解释性的新见解。
...
> The paper proposes a new method for enhancing compressed video quality without relying on known Quantization Parameters (QPs). It introduces a Degradation Representation Learning (DRL) module and a hierarchical termination mechanism to adapt to varying artifact patterns and computational demands.

<details>
  <summary>Details</summary>

**Motivation:** The primary motivation is to develop a blind method for quality enhancement of compressed video that can work effectively even when QPs are unknown, addressing the limitations of existing methods which either require known QPs or fail to capture detailed spatial information about artifacts.

**Method:** Structure

**Result:** The proposed method shows a significant improvement in PSNR (110%, from 0.31 dB to 0.65 dB) over a state-of-the-art blind method at QP = 22, and it reduces the average inference time by half.

**Conclusion:** The study concludes that by decoupling and extracting high-dimensional, multiscale degradation representations, and dynamically adjusting the artifact reduction stages, the proposed method achieves enhanced performance in visual quality and computational efficiency for quality enhancement of compressed video.
...
> SurvAgent是首个用于多模态生存预测的分层思维链增强多智能系统，在可解释的AI驱动的生存预测中表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 鉴于现有的生存分析方法缺乏临床应用所需的透明度，SurvAgent旨在解决现有病理代理在多模态数据融合、感兴趣区域探索和历史案例经验学习利用方面的局限性。

**Method:** SurvAgent 是一种分层次的思维链增强多智能系统，用于多模态生存预测。它包括两个阶段：第一阶段，WSI-Gene CoT-Enhanced Case Bank Construction，通过低倍镜筛选、跨模态相似性意识斑块挖掘和信心意识斑块挖掘对病理图像进行层次分析，并通过基因分层分析处理六个功能基因类别。这些过程生成带有CoT推理的结构化报告，供经验学习。第二阶段，Dichotomy-Based Multi-Expert Agent Inference，通过RAG检索类似案例，并通过逐步区间细化，将多模态报告与专家预测相结合。

**Result:** 在五个TCGA队列上的广泛实验表明，SurvAgent在可解释的AI驱动的生存预测中优于传统方法、专有的多模态大型语言模型和医疗代理。

**Conclusion:** SurvAgent为精准肿瘤学中的可解释AI驱动的生存预测设定了新标准，通过集成多模态数据和经验学习，提高了生存预测的透明度和准确度。
...
> 该论文介绍了SR3D框架，该框架改进了3D物体检测的训练方法，实现了与推断行为更紧密的对齐，显著提高了室内部署中的检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 作者旨在解决3D目标检测中训练和推断之间的差距，即训练缺乏空间可靠性和排名感知，而推断是基于排名的选择。这种差距阻碍了模型学习与推断时间行为对齐的表示。

**Method:** SR3D框架包含两个主要组成部分，用于解决训练和推断之间的差距问题。第一个组成部分是空间优先最优传输分配，它动态地强调定位良好且空间可靠的样本。第二个组成部分是具有自蒸馏范式的排名感知自适应自蒸馏方案，适应性地注入排名感知。

**Result:** SR3D在ScanNet V2和SUN RGB-D数据集上的实验表明，它有效地缩小了训练和推断之间的差距，并在保持实时速度的同时显著优于先前方法。

**Conclusion:** 该研究提出了一种新的3D目标检测框架SR3D，有效地解决了室内点云中的训练和推断差距，提升了准确性并在保持实时速度上进行了改进。
...
> 本文提出了一种名为Thinking-while-Generating (TwiG) 的框架，该框架首次实现了在视觉生成过程中嵌入和持续的文本推理。

<details>
  <summary>Details</summary>

**Motivation:** 当前视觉生成技术在生成前后进行文本推理，但在生成过程中缺乏动态推理，本文研究目的是探索在生成过程中动态结合文本推理的可能性，以提升生成内容的语义丰富性和上下文敏感度。

**Method:** TwiG框架在视觉生成过程中交替加入文本推理以指导各局部生成并反思之前的生成部分，本文探讨了三种潜在策略：零样本提示、基于TwiG-50K数据集的有监督微调、以及通过自定义TwiG-GRPO策略的强化学习。

**Result:** 该研究初步探索了通过TwiG框架实现文本推理和视觉生成过程的融合，产生更加上下文敏感和语义丰富的输出结果。

**Conclusion:** 研究表明在视觉生成过程中应用文本推理是一个值得深入研究的领域，能够为未来的视觉生成技术提供新的发展思路。
...
> 研究提出了一种新的高空间分辨率水域变化检测数据集HSRW-CD，以及一个SSCP模块，提高了水域变化检测网络对水体变化的识别能力。

<details>
  <summary>Details</summary>

**Motivation:** 为了解决高空间分辨率数据集缺乏和深度学习方法未能充分挖掘深度特征中空间语义和结构信息的问题，该研究提出了新的数据集和一种新的注意力模块。

**Method:** 设计了一个新的HSRW-CD数据集，提供优于3米的空间分辨率，广泛覆盖各种水域类型，并提出一个名为SSCP的注意力模块，该模块结合了MSA、SRGA和CSA，以全面利用深度特征中的空间语义和结构信息。

**Result:** 提出的SSCP注意力模块结合了多语义空间注意力(MSA)、结构关系感知全局注意力(SRGA)和通道自注意力(CSA)，能够全面利用深度特征中的空间语义和结构信息，从而显著提高了水域变化检测的区分能力。SSCP设计为可插入模块，适用于现有的水域变化检测模型。在所提出的HSRW-CD和Water-CD数据集上进行的大量实验验证了该方法的有效性和泛化性。

**Conclusion:** 实验表明，SSCP模块能够显著提升水域变化检测的性能，并且具有广泛的适用性。研究还提供了HSRW-CD数据集和相关代码，以便进一步的研究。
...
> Introducing LEGO-SLAM for integrating language features into 3DGS SLAM, achieving real-time, open-vocabulary mapping.

<details>
  <summary>Details</summary>

**Motivation:** To overcome the limitations of current SLAM systems that lack open-vocabulary semantic capabilities and have excessive memory requirements, proposing a real-time solution.

**Method:** Proposes LEGO-SLAM, a framework to integrate language features into 3DGS-based SLAM systems, using a scene-adaptive encoder-decoder to compress high-dimensional language embeddings, enabling real-time, open-vocabulary mapping.

**Result:** LEGO-SLAM achieves real-time performance, reduces the Gaussian count by over 60% without degrading rendering, and provides open-vocabulary capabilities at 15 FPS.

**Conclusion:** LEGO-SLAM demonstrates the potential of integrating natural language processing into SLAM systems, paving the way for more interactive and adaptive robotic systems.
...
> 我们提出了一种称为Reasoning Guided Embeddings (RGE)的方法，该方法利用多模态大型语言模型的推理能力以提升其在多模态检索任务中的表示质量，改进了4.9%。

<details>
  <summary>Details</summary>

**Motivation:** 尽管研究表明多模态大型语言模型（MLLMs）可以作为强大的嵌入提取器，但现有方法只是将其视为一个直接编码步骤，忽视了MLLMs的生成性推理能力。这项工作研究了如何在嵌入过程中明确地结合推理，以提高表示质量。

**Method:** 我们提出了一种名为Reasoning Guided Embeddings (RGE)的方法，该方法保留了多模态大型语言模型的生成性推理过程，并将其与对比训练相结合。具体来说，该方法首先使模型能够基于指令进行结构化的推理生成，然后在推理展开后提取表示。

**Result:** 实验表明，在MMEB基准上，推理导向的条件提高了多模态检索性能，比没有推理的基线提高了4.9%，证明了明确的推理能够有效提升嵌入质量。

**Conclusion:** 通过结合推理过程与对比训练，新的RGE方法能够增强上下文条件下的推理信号，从而提升多模态表示的质量。
...
> 本研究提出了PPCL框架，通过灵活的知识转移实现DiT模型的参数压缩，减少了50%的参数数量，并保持了高生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 尽管扩散变压器（DiTs）在图像生成方面表现优异，但由于参数数量庞大导致计算开销大，这妨碍了在资源受限环境中的部署。因此，提出了PPCL框架来解决这一问题。

**Method:** 本研究提出了一个名为PPCL（插件式修剪与连续层蒸馏）的灵活结构化修剪框架，专门用于DiT架构。首先，通过线性探测机制结合相似度度量的一阶微分趋势分析，识别冗余层区间。然后，提出了一种针对深度和宽度修剪的即插即用教师-学生交替蒸馏方案。这一蒸馏框架能够在多种修剪比例下实现灵活的知识转移，无需针对每种配置进行重新训练。

**Result:** 实验表明，与完整模型相比，PPCL可以减少50%的参数数量，同时关键指标的性能损耗不到3%，在多种多模态扩散变压器架构模型中实现了较高的压缩比率和高质量图像生成能力。

**Conclusion:** PPCL方法在多种多模态扩散变压器架构模型中减少了50%的参数数量，同时在关键目标指标上仅有不到3%的性能下降。保持了高质量的图像生成能力，使得该方法适用于资源受限的环境。
...
> 提出Video2Layout框架，利用连续对象边界坐标进行视频中度量基础的空间布局重构，增强了模型在空间推理中的能力，并引入QVS-Bench进行评测，模型在多个评测基准上表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 当前基于网格的认知地图方法受到离散栅格表示的限制，不能进行细粒度的空间推理，因此提出一种改进的方法。

**Method:** 方法包括两个核心阶段：监督微调阶段，基于AI2THOR模拟器构造高质量数据集，学习从视觉输入到精确边界坐标的映射；强化微调阶段，提升模型现实世界泛化能力。

**Result:** 通过QVS-Bench和主流行评测基准评测，模型V2LO-7B平均提升4.92%优于基于网格地图训练的模型。

**Conclusion:** Video2Layout框架有效增强了模型的空间计算能力，减少了描述空间关系时的语言歧义。
...
> 本文针对点云补全任务中过度拟合和噪声敏感的问题，提出了一种基于分布学习和对称性先验的Simba框架，实验验证了其前沿性能。

<details>
  <summary>Details</summary>

**Motivation:** 目前，基于回归的方法在点云补全任务中面临着过度拟合和对输入噪声敏感的问题，它们倾向于记忆特定实例的变换而不是学习通用的几何先验。为了解决这些问题，引入了Simba框架。

**Method:** 本文提出了Simba框架，通过将逐点变换回归重新定义为分布学习问题，整合了对称性先验和扩散模型的强大生成能力，以避免对实例的具体记忆并捕捉稳健的几何结构。同时引入了基于Mamba的分层架构以实现高保真度的上采样。

**Result:** 实验在PCN、ShapeNet和KITTI基准数据集上验证了所提方法的前沿性能。

**Conclusion:** 实验结果表明，本文的Simba框架在点云补全任务中表现出色，解决了现有基于回归方法的过度拟合和噪声敏感问题，并实现了高保真的几何结构补全。
...
> The LNG-SWR method improves the robustness of medical image segmentation models without the downsides of adversarial training.

<details>
  <summary>Details</summary>

**Motivation:** The current practice of adversarial training (AT) improves robustness but suffers from a clean-robustness trade-off and high training costs, which hinders scalability and maintainability in medical imaging applications.

**Method:** Layer-wise Noise-Guided Selective Wavelet Reconstruction (LNG-SWR) is introduced to enhance segmentation model robustness in medical imaging. It works by injecting zero-mean noise into multiple layers to learn a frequency-bias prior and then applying prior-guided selective wavelet reconstruction to suppress noise-sensitive bands and improve robustness under various conditions.

**Result:** LNG-SWR leads to consistent performance improvements in clean Dice/IoU metrics and reduces performance drops under strong attacks. When combined with AT, it further enhances robustness without sacrificing clean accuracy.

**Conclusion:** LNG-SWR provides a simple, effective, and scalable path to robust medical image segmentation, suitable for both adversarial and standard training regimes, overcoming the limitations of adversarial training.
...
> 本文提出了一种新颖的冗长文本诱导攻击，使用两阶段框架通过对抗性提示搜索和视觉对齐扰动优化来诱导视觉语言模型生成冗长文本，实验结果显示该方法效果显著。

<details>
  <summary>Details</summary>

**Motivation:** 随着视觉语言模型在多模态任务上的成功，其部署效率问题变得日益突出，尤其是生成过程中的token消耗成为关键评价指标，而现有方法缺乏稳定性和可控性。

**Method:** 通过两阶段框架提出一种新颖的冗长文本诱导攻击（VTIA），首先使用强化学习策略自动识别能够诱导视觉语言模型生成冗长输出的对抗提示，然后通过视觉对齐扰动优化来制作对抗样本，最大化扰动图像的视觉嵌入与对抗提示嵌入之间的相似度，从而构建诱导冗长文本生成的恶意图像。

**Result:** 在四个流行的视觉语言模型上的综合实验表明，该方法在有效性、效率和泛化能力方面具有显著优势。

**Conclusion:** 提出的方法能够有效诱导视觉语言模型生成冗长且信息密度低的输出，提升效率并降低了能源消耗和延时。
...
> 本文提出EvoVLA来解决长时段机器人操纵中的阶段幻觉挑战，通过自我监督学习框架实现任务成功率和样本效率的提升，并通过实验证明其有效的模拟到现实转移能力。

<details>
  <summary>Details</summary>

**Motivation:** 长时段机器人操纵依然是Vision-Language-Action(VLA)模型难以解决的问题。研究目的是开发一种方法以减轻模型对粗略评估信号的滥用，解决阶段幻觉问题。

**Method:** EvoVLA采用自我监督学习框架处理长时段机器人操纵问题，框架包含三部分：阶段对齐奖励(SAR)使用三重对比学习，防止视觉捷径；基于姿势的对象探索(POE)，以相对物体-机械手姿态为好奇心基础；长时段记忆，选择性的情境保留和门控融合，以稳定扩展展开时的内在塑造。

**Result:** 在包含三个多阶段任务的Discoverse-L长时段操纵基准测试上进行评估，EvoVLA使平均任务成功率提高了10.2个百分点，成功率达到了69.2%。在实际场景的物理机器人上，EvoVLA平均成功率为54.6%，比OpenVLA-OFT高出11个百分点。

**Conclusion:** EvoVLA成功地解决了长时段机器人操纵中的阶段幻觉问题，通过高级自我监督学习方式，达到了更高的任务成功率和样本效率，同时也显示出了良好的模拟到现实世界的转移效果。
...
> 研究分析了CLIP在密集预测中的表现和局限，提出了RF-CLIP无训练方法提升了CLIP的性能并达到了SOTA结果。

<details>
  <summary>Details</summary>

**Motivation:** 分析CLIP在密集预测中的内部机制及表现瓶颈，旨在增强CLIP在密集预测任务中的准确性和效果。

**Method:** OVSS 使用逐像素的视觉-语言对齐策略将与类别相关的提示关联到相应的像素上。论文分析了现有方法在使用CLIP的视觉-语言对齐时的局限性，并发现CLIP在密集预测中会因为特定维度的过度激活而将注意力从目标区域转向不相关的标记。为此，作者提出了RF-CLIP 方法，这一无训练方法模仿了人类注意力重聚焦的行为，以改进CLIP的多模态一致性。

**Result:** RF-CLIP 方法在八个基准上取得了SOTA结果，并且保持了高效的推理效率。

**Conclusion:** RF-CLIP 方法有效地解决了CLIP在密集预测任务中注意力资源分配的问题，提高了性能表现。
...
> This paper introduces Mantis, a novel VLA framework that uses Disentangled Visual Foresight (DVF) for better action learning and reasoning capabilities, achieving high accuracy and fast convergence on the LIBERO benchmark and real-world tasks compared to state-of-the-art models.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this work is to address the issues where VLA models predict high-dimensional visual states, which can be computationally expensive and lead to information bottlenecks when visual states are compressed into more compact supervisory signals. Additionally, the paper aims to improve the comprehension and reasoning capabilities of VLA models, which are often neglected.

**Method:** This paper introduces Mantis, a novel framework featuring a Disentangled Visual Foresight (DVF) that decouples visual foresight prediction from the backbone using meta queries and a diffusion Transformer (DiT) head. With a simple next-state prediction objective and a residual connection from the current visual state to the DiT, Mantis can automatically capture latent actions, thereby enhancing the learning of explicit actions. The disentanglement allows the VLA backbone to focus on language supervision, maintaining its comprehension and reasoning capabilities.

**Result:** Mantis achieves a 96.7% success rate on the LIBERO benchmark after fine-tuning, outperforming powerful baselines with high convergence speed. Real-world evaluations show superior performance in instruction-following capability, generalization to unseen instructions, and reasoning ability compared to a leading open-source VLA model, π_{0.5}.

**Conclusion:** The study concludes that the introduction of Mantis, a framework that utilizes a Disentangled Visual Foresight (DVF) with a diffusion Transformer (DiT) and meta queries, effectively enhances VLA models. It enables efficient visual foresight prediction, supports better comprehension and reasoning via enhanced language supervision, and outperforms existing approaches in various categories such as instruction-following tasks.
...
> This paper addresses the issue of applying VI-ReID algorithms to real-world data by proposing a two-stage model, DSLGA, which tackles inter and intra-domain modality discrepancies. The proposed method surpasses existing domain adaptation methods for VI-ReID and even some supervised methods across various evaluation metrics in experiments under different settings.

<details>
  <summary>Details</summary>

**Motivation:** The motivation behind this paper is to bridge the gap between the performance of VI-ReID models on public datasets and their application on real-world data. It aims to make these models more adaptable without the need for additional labeled data in the real-world environment.

**Method:** The researchers introduced a two-stage method called DSLGA. This model includes a Domain-Shared Learning Strategy (DSLS) for pre-training and a Gradual Alignment Strategy (GAS) for fine-tuning. DSLS focuses on inter-domain discrepancies by utilizing shared information across domains, while GAS attends to intra-domain modality discrepancies through a cluster-to-holistic alignment approach.

**Result:** Experiments carried out with the DSLGA method demonstrate superior performance compared to existing domain adaptation solutions for VI-ReID, as well as some supervised models, across a range of evaluation settings.

**Conclusion:** The study concludes that the DSLGA model effectively improves the adaptability of VI-ReID algorithms in real-world scenarios, significantly enhancing their performance without the need for additional labeled data from the real-world domain.
...
> PrIntMesh框架提供一种在保留拓扑结构的同时，重建人体器官的方法，实现更准确解剖结构的重建，并适用于临床场景。

<details>
  <summary>Details</summary>

**Motivation:** 大多数深度学习方法独立处理器官子结构，导致重建结果在解剖学上不真实。

**Method:** PrIntMesh采用基于模板的方法，保留器官内部的拓扑结构并共同变形所有子结构以匹配特定患者的解剖结构，同时确保平滑无瑕疵的表面。

**Result:** 该方法在心脏、海马和肺部的重建中表现出高水平的几何精度、正确的拓扑结构以及在训练数据有限或噪声很大的情况下仍具有鲁棒性能。

**Conclusion:** 相比于体素和面片方法，PrIntMesh更好地重建共用界面，保持结构的一致性，并且是一个适用于临床的数据高效解决方案。
...
> 本文通过VLA-Fool研究展示了Vision-Language-Action模型在多模态对抗条件下的脆弱性，尤其是小的多模态扰动如何引发显著的行为偏差，指出了提升该领域鲁棒性研究的重要性。

<details>
  <summary>Details</summary>

**Motivation:** Vision-Language-Action模型在具身环境中表现出显著的进展，但其对抗鲁棒性特别是在真实多模态和黑盒条件下的研究却不足。目前的研究主要集中在单模态扰动上，忽略了基本影响具身推理和决策的跨模态不对齐问题。

**Method:** 本文提出了VLA-Fool，一个关于具身Vision-Language-Action模型在多模态对抗条件下的鲁棒性研究。研究统一了三个层次的多模态对抗攻击：基于梯度和prompt的文本扰动、通过补丁和噪声扭曲的视觉扰动、以及故意破坏感知和指令语义对应关系的跨模态不对齐攻击。此外，研究还引入了针对VLA的语义空间，开发了第一个自动创建和语义引导的prompting框架。

**Result:** 在LIBERO基准测试上使用微调过的OpenVLA模型进行的实验表明，即使是微小的多模态扰动也会导致显著的行为偏差，展示了具身多模态对齐的脆弱性。

**Conclusion:** 实验结果揭示了Vision-Language-Action模型在面对多模态扰动时的脆弱性，强调了在该领域的鲁棒性提升的重要性。
...
> 本文提出了SwiTrack框架，通过三个专业流重定义了跨模态对象追踪，改进了目标一致性保持和漂移问题，并在精度率和成功率方面取得了显著提升，同时保留了实时跟踪能力。

<details>
  <summary>Details</summary>

**Motivation:** 跨模态对象追踪的主要挑战在于当视频流在不同模态之间切换时，如何保持目标的一致性，尤其是在面对不可靠输入时的目标漂移问题。现有的方法通常将并行的RGB和NIR分支连接到一个共享的骨干网络上，这限制了对特定模态特征的全面提取，并未能解决目标漂移的问题。为此，我们提出了SwiTrack框架。

**Method:** 我们提出了一种名为SwiTrack的新态切换框架，该框架通过部署三个专业化流来重新定义跨模态对象追踪（CMOT）。RGB帧由视觉编码器处理，NIR帧通过与视觉编码器耦合的NIR门控适配器进行优化以逐步校准共享的潜在空间特征，从而产生更稳健的跨模态表示。对于无效的模态，一致性轨迹预测模块利用时空线索来估计目标运动，确保稳健的跟踪并减轻漂移。此外，我们引入动态模板重建来迭代更新模板特征，并采用相似度对齐损失来强化特征一致性。

**Result:** 实验显示，SwiTrack显著提升了跨模态跟踪的性能指标，在精度率和成功率方面分别取得了7.2%和4.3%的提升，维持了实时跟踪的性能（65 FPS）。

**Conclusion:** 实验结果表明，我们的追踪器在最新基准测试中达到了最先进的性能，将精度率和成功率分别提高了7.2%和4.3%，同时保持每秒65帧的实时追踪能力。
...
> 本文提出了一种基于MLP的新方法，通过Memory-Block模块解决了全身追踪在AR/VR中由于传感器数据稀疏而遇到的挑战。该方法在减少预测误差和提高运行速度方面优于现有技术。

<details>
  <summary>Details</summary>

**Motivation:** 现有的系统主要通过头显设备（HMD）和控制器追踪头部和手部，导致3D全身重建不完整。为了在增强现实（AR）和虚拟现实（VR）应用中实现现实而平滑的全身追踪，本文旨在提出一种方法，基于稀疏输入生成全身运动。

**Method:** 本文提出了一种基于多层感知器（MLP）的新方法，该方法增强了残差连接，并引入了一种新的神经网络模块Memory-Block。Memory-Block通过可训练的码本向量表示缺失的传感器数据，并将其与之前时间实例的稀疏信号相结合，以提高时间一致性。此外，本文将解决方案表述为多任务学习问题，使MLP主干能够学习更稳健的表示形式，提高准确性。

**Result:** 实验结果表明，本文提出的方法显著减少了预测误差，与最新的基线方法相比表现更佳。同时，该方法可以在移动头显设备上达到72 FPS的速度，改善了准确性与运行时间之间的权衡。

**Conclusion:** 本文提出的方法表明，在全身追踪任务中使用Memory-Block增加的MLP相比其他方法具有更高的预测准确性，并且能够在移动设备上实现高效的运行速度。这表明该方法适用于需要实时高性能AR/VR应用的环境。
...
> 本文提出了一种基于VGGT的快速时空场景理解框架，适用于辅助导航等实时场景应用。

<details>
  <summary>Details</summary>

**Motivation:** 旨在设计一个能够实现高效近实时性能的场景理解框架，特别是在辅助导航等应用场景中表现得更为显著。

**Method:** 提出了一种基于Vision Gated Generative Transformers (VGGT) 的快速时空场景理解框架。该框架采用滑动窗口处理图像流，并通过子图对齐来克服VGGT的高内存需求。利用VGGT跟踪头，将2D语义实例掩码聚合到3D对象中。为了实现时间一致性和更丰富的语境推理，该系统存储时间戳和实例级身份，从而能够检测环境变化。

**Result:** 评估方法在广泛应用的基准测试集和专门针对辅助导航场景的定制数据集上进行了测试。实验结果表明，该框架适用于真实场景。

**Conclusion:** 实验结果证明提出的框架能够有效地应用于真实世界场景中，展示了其在辅助导航等领域中的潜力。
...
> 本文提出了一种用于杂草检测的混合深度学习框架，结合CNNs、ViTs和GNNs，实现了高鲁棒性，并表现出优越的性能。

<details>
  <summary>Details</summary>

**Motivation:** 杂草检测是精准农业的关键要素，准确的物种识别使农民可以选择性施用除草剂，符合可持续农业的作物管理需求。

**Method:** 本文提出了一种结合卷积神经网络（CNNs）、视觉变换器（ViTs）和图神经网络（GNNs）的混合深度学习框架，用于增强在多种田间条件下杂草检测的鲁棒性。为了平衡类别分布并使模型更好地泛化，采用了基于生成对抗网络（GAN）的数据增强方法。此外，通过自我监督对比预训练方法，从有限的标注数据中学习更多特征。

**Result:** 实验结果在多个基准数据集上显示出优越的性能，准确性、精确度、召回率和F1值达到99.33%。

**Conclusion:** 该模型架构允许实时、高效地部署在边缘设备上进行自动化的杂草检测，有助于减少对除草剂的过度依赖，并为可扩展、可持续的精准农业提供选项。
...
> Texture3dgs, an optimized 3DGS version for mobile GPUs, significantly improves sorting and scene reconstruction performance while reducing memory usage.

<details>
  <summary>Details</summary>

**Motivation:** Enhance the performance and efficiency of 3D scene reconstruction on mobile devices, benefiting from aspects like data privacy, offline operation, and quicker responses, without an internet connection.

**Method:** Image-based 3D scene reconstruction using 3D Gaussian Splatting (3DGS) that is optimized for mobile GPU with a focus on 2D texture cache optimization and an improved sorting algorithm.

**Result:** Achieves up to 4.1 times speedup for sorting and up to 1.7 times overall speedup for 3D scene reconstruction. Memory usage is also reduced by up to 1.6 times.

**Conclusion:** Texture3dgs, with its enhancements in sorting and 2D texture cache optimization, demonstrates its effectiveness for efficient mobile-based 3D scene reconstruction.
...
> 本文提出了一种称为Upsample Anything的轻量级无训练测试时刻优化框架，用于恢复低分辨率图像的高分辨率特征，同时保持高精度。该方法不仅解决了现有的限制，还实现了跨多任务的顶尖性能。

<details>
  <summary>Details</summary>

**Motivation:** 尽管视觉基础模型在多种下游任务中表现出强大的泛化能力，但它们的表示通常会被降采样14x/16x。现有特征上采样方法依赖于数据集特定的再训练或重型隐式优化，这限制了可扩展性和泛化能力。

**Method:** Upsample Anything 是一种轻量级的测试时刻优化框架，无需训练即可将低分辨率特征恢复到高分辨率、像素级输出。该方法通过学习一个各向异性高斯核，结合空间和范围提示，有效地在图像上进行优化，从而在各种架构和模态间无缝转移。

**Result:** Upsample Anything 在语义分割、深度估计以及深度图和概率图的上采样方面达到了最先进的性能。它处理每个 224x224 图像的时间仅为约为0.419秒。

**Conclusion:** Upsample Anything 弥补了高斯喷涂和联合双边上采样的空白，并能够实现精确的高分辨率特征重建，深度或概率图重建，证明了其在不用进行额外训练情况下也能达到高效和高质量输出的能力。
...
> 将稀疏自动编码器（SAEs）视作主题模型的视角，并提出SAE-TM框架，该框架在各类数据集上的表现优于基线。

<details>
  <summary>Details</summary>

**Motivation:** 重新审视SAEs的角色，发现它们可以作为主题模型来理解，进而提出SAE-TM框架，旨在通过SAEs进行大规模主题分析并验证其有效性。

**Method:** 提出了SAE-TM框架，该框架包括：(1) 使用SAE学习可重用的主题原子，(2) 将其解释为下游数据的词分布，(3) 无需重新训练就融合为任何数量的主题。

**Result:** SAE-TM在文本和图像数据集上生成的主题比强劲的基线更加连贯，同时保持多样性。此外，还在图像数据集中分析了主题结构，并追踪了日本木版画随时间推移的主题变化。

**Conclusion:** 通过研究确立了SAEs作为跨模态大规模主题分析的有效工具地位。代码和数据将在发表后开放。
...
> 本研究提出BioBench，一个新的生态学视觉基准，它汇集了多个任务、界域和采集模式，以评估图像处理的表现，尤其适用于生态学领域。

<details>
  <summary>Details</summary>

**Motivation:** ImageNet-1K线性探测转移精度已不再能准确预测科学图像的表现，因此引入了BioBench来解决这个问题。

**Method:** 介绍了BioBench，一个开放的生态学视觉基准，包含9个已发布的、以应用驱动的任务，4个生物界域，及6种采集模式。

**Result:** ImageNet top-1 accuracy的解释力仅为34%，且30%的模型在75%以上的准确率下被误判。BioBench使用轻量级分类器，通过冻结的模型骨干进行评估，仅需6小时即可完成ViT-L模型的评估。

**Conclusion:** BioBench为生态学中的计算机视觉提供了新的信号，并且可以作为在任何领域构建可靠AI基准的模板。
...
> NaTex是一种新颖的3D纹理生成框架，通过将纹理视为密集的颜色点云来解决传统方法的局限，展示了优于之前方法的性能。

<details>
  <summary>Details</summary>

**Motivation:** NaTex旨在克服以往基于MVD模型的方法在处理遮挡区域、边界精确对齐、以及跨视角内容和颜色强度一致性等方面存在的局限性。

**Method:** NaTex采用了一种新颖的框架，将纹理视为密集的颜色点云。这种方法引入了隐式颜色扩散技术，该技术结合了几何感知的颜色点云VAE和多控扩散变换器（DiT），完全基于3D数据进行训练。此外，还设计了本几何控制，通过位置嵌入和几何隐表示直接使用3D空间信息来调节DiT，实现纹理的精准对齐。

**Result:** NaTex在纹理一致性和对齐性方面展示了比以前的方法更强的性能。此外，在不需要训练或简单调整的情况下，它也表现出了在材质生成、纹理细化、部分分割和纹理等方面的强大泛化能力。

**Conclusion:** NaTex提供了一种基于3D数据的新型纹理生成框架，通过将纹理作为密集的颜色点云处理，克服了传统方法在多个方面的局限性，显示出了优越的性能和泛化能力。
...
> WWE-UIE以较少的参数和运算量实现了高质量的水下图像修复，支持实时应用。

<details>
  <summary>Details</summary>

**Motivation:** 当前水下图像增强方法性能优异但计算成本高，难以实现实时应用，因此提出WWE-UIE来平衡效果和性能之间的矛盾。

**Method:** WWE-UIE通过以下三个模块实现图像增强: 自适应白平衡、小波增强块和基于梯度的滤波模块，并利用Sobel算子来增强边缘结构。

**Result:** WWE-UIE, 一种紧凑高效的水下图像增强网络，它集成了三种可解释的先验：自适应白平衡、基于小波的增强模块和梯度感知模块。实验表明该方法在参数和浮点运算次数较少的情况下也能实现高质量的水下图像修复，支持资源受限平台的实时推理。

**Conclusion:** WWE-UIE比现有方法具有更少的参数和浮点操作，同时保持高质量的水下图像修复效果，在资源受限设备上实现了实时图像处理。
...
> ChangeDINO is a novel end-to-end multiscale Siamese framework for optical building change detection, which fuses a lightweight backbone with transferred features from a frozen DINOv3, employs a spatial-spectral differential transformer decoder for change detection, and refines predictions with a learnable morphology module, demonstrating superior performance on multiple benchmarks.

<details>
  <summary>Details</summary>

**Motivation:** To improve the robustness of deep learning-based remote sensing change detection methods, especially under illumination variation and limited label conditions, by fully utilizing the semantic information in non-changing regions.

**Method:** ChangeDINO uses a Siamese network architecture, integrating a lightweight backbone with transferred features from a frozen DINOv3 to generate semantic-rich multi-scale feature maps. It applies a spatial-spectral differential transformer to highlight building changes and suppress irrelevant responses, and enhances predictions with a learnable morphology module.

**Result:** Evaluation on four public datasets shows that ChangeDINO outperforms recent state-of-the-art methods in IoU and F1 scores. Ablation studies confirm the contribution of each component.

**Conclusion:** ChangeDINO represents an effective and robust approach for optical building change detection, capable of improving accuracy and efficiency over existing methods through the innovative integration of semantic-rich feature extraction and transformation.
...
> 本文提出了一种新的面超分辨率方法--ARASFSR，它能够在任意上采样比例下对面部图像进行超分辨率重建，并通过特定模块处理了现有FSR方案的问题。

<details>
  <summary>Details</summary>

**Motivation:** 面超分辨率(FSR)是一种用于增强低分辨率面部图像的重要技术，但现有方法受到固定上采样比例和对输入大小变化敏感的限制。为此，本文提出了一个任意分辨率和任意比例FSR方法--ARASFSR。

**Method:** ARASFSR采用2D深层特征、局部相对坐标和上采样比例来预测每个目标像素的RGB值，能够以任意上采样比例进行超分辨率重建。此外，该方法包括局部频率估计模块用于捕获高频率面部纹理信息，以及全局坐标调制模块用于利用先前的面部结构知识和实现有效的分辨率适应。

**Result:** 定量和定性评估表明，在处理各种输入大小和上采样比例的面部图像时，ARASFSR比现有最先进的方法更具有鲁棒性。

**Conclusion:** ARASFSR方法证明了其在处理各种输入大小和上采样比例的面部超分辨率任务中的鲁棒性和有效性。此方法开发的模块解决了现有FSR方法的局限性。
...
> 本研究探讨了通过无人机遥感进行地形和地貌分类的方法，提出了一种基于教师-学生架构的弱监督学习与时间一致性知识蒸馏的框架，显著提高了mIoU和时间一致性，仅使用30%的标注数据就实现了稳定的地形物体定位。

<details>
  <summary>Details</summary>

**Motivation:** 研究旨在解决无人机遥感中数据标注复杂、时间一致性维护难、相关数据稀缺及现有技术有效距离限制等问题，并在空中定位任务中强调mIoU和时间一致性的重要性。

**Method:** 本文提出了一个教师-学生架构，结合关键帧选择和关键帧更新算法，实现了在空中任务中的弱监督学习和时间一致性知识蒸馏。

**Result:** 实验结果显示，所提方法仅使用30%的标注数据显著提高了mIoU和时间一致性，提升了空中任务中地形物体的稳定定位。

**Conclusion:** 通过所提出的框架，成功解决了现有方法在时间一致性上的不足，表明30%的标注数据足以有效提升mIoU和时间一致性，实现更稳定的地形物体定位。
...
> A real-time method for camera localization using 2D-3D matching with colored LiDAR data and neural rendering is introduced, achieving drift-free tracking and outperforming traditional SLAM methods on the ScanNet++ benchmark.

<details>
  <summary>Details</summary>

**Motivation:** The paper aims to address the drift, scale ambiguity, and reliance on fiducials seen in existing visual methods for camera localization, providing a solution that is more reliable and accurately aligned in robotics and XR applications.

**Method:** This paper introduces a real-time method to localize a camera in a highly accurate colored LiDAR point cloud by creating 2D-3D correspondences between live camera frames and the LiDAR data. A neural rendering technique enhances feature matching by reducing the discrepancy between synthetic and real images.

**Result:** The proposed method achieves drift-free camera tracking with correct metric scale within the LiDAR coordinate system. The method outperforms existing SLAM pipelines and demonstrates improved performance on the ScanNet++ dataset.

**Conclusion:** The real-time localization method introduced in the paper offers a significant improvement in camera tracking accuracy and reliability over existing methods by leveraging LiDAR data and neural rendering techniques.
...
> 本文介绍了一种新的无对齐框架Multi-Order Matching Network (MOMNet)，用于解决现有深度超分辨率方法在处理未对齐RGB-D图像时性能下降的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法在处理未对齐的RGB-D图像时性能下降，因此提出了一种新的无对齐框架来解决这一问题。

**Method:** 提出了Multi-Order Matching Network (MOMNet)，使用多阶匹配机制联合执行零阶、一阶和二阶匹配，以全面识别一致的RGB信息，并引入多阶聚合策略来整合提取的RGB和深度信息。

**Result:** 

**Conclusion:** MOMNet在实验中取得了最先进的性能，并显示出卓越的鲁棒性。
...
> 本文提出了DetailSemNet模型，用于离线签名验证，该模型专注于细粒度差异的局部结构匹配，实现更高的识别准确性。实验结果表明该模型的表现优于现有方法，且在跨数据集测试中显示出良好的泛化和解释能力。

<details>
  <summary>Details</summary>

**Motivation:** 为了改善离线签名验证的性能，提出了一种基于细粒度差异匹配的新模型。传统的OSV方法依赖于全局特征对比，而该模型侧重于对签名之间更细微的具体差异的检测。通过这种改进，可以提高OSV的准确性和模型的可解释性。

**Method:** DetailSemNet模型被提出用于离线签名验证(OSV)，该模型强制关注细粒度差异，通过匹配两个签名图像之间的局部结构来增强识别准确性。此外，为了减少初始架构对局部细节的遮蔽效应，该模型引入了一个细节语义整合器，这个整合器通过特征解缠和再纠缠来增强细节并扩展判别语义，从而提高局部结构匹配的效率。

**Result:** 通过与当前离线签名验证的领先基准测试方法相比，该模型在验证性能上展示出显著提高的效果，并取得了状态-of-the-art的结果。模型在跨数据集测试中显示出了良好的泛化能力。

**Conclusion:** DetailSemNet模型不仅在离线签名验证的准确性方面有所提升，还在模型的解释性和跨数据集泛化能力方面表现出色。这些改进使得DetailSemNet对于实际应用场景具有更高的潜在价值。
...
> CAMS提出了一种方法，通过提取视觉特征中的语义特征并进行多空间解缠，提高了对未见过的属性-物件组合的识别能力。

<details>
  <summary>Details</summary>

**Motivation:** 当前CLIP基础的CZSL方法通过图像编码器获得的全局语义表示能力有限，无法完全解缠属性和物件。

**Method:** CAMS采用门控交叉注意力机制从CLIP的高级图像编码块中捕捉细粒度语义特征，并使用多空间解缠方法实现属性和物件语义的解缠。

**Result:** 在MIT-States、UT-Zappos和C-GQA三个流行基准测试中，CAMS在封闭世界和开放世界设置中均达到了最先进的性能。

**Conclusion:** CAMS通过其独特的门控交叉注意力机制和多空间解缠方法，提高了对未见过的属性-物件组合的泛化能力，实现了卓越的性能。
...
> 本论文提出了一个新的刚体标记（RBM）和基于深度学习的回归模型来简化运动捕捉技术的设置过程，同时也提高了身体姿势估计的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于光学标记的运动捕捉技术存在准备工作繁琐、标记识别模糊等问题，限制了其扩展性。本研究旨在通过引入RBM简化设置，克服这些限制。

**Method:** 引入了一种新的运动捕捉基本单元——刚体标记（RBM），并通过基于深度学习的回归模型直接估算身体姿势参数，使用测地线损失函数来实现这一目标。

**Result:** 训练后的模型在身体姿势估计上达到了最先进水平，并且在处理时间上比优化方法快了一个数量级。

**Conclusion:** 结合稀疏的6自由度刚体标记和测地线损失的模型为图形、虚拟现实和生物力学领域提供了实时运动捕捉的现实可行性和高保真解决方案。
...
> A novel geometry-guided self-supervised depth estimation method using multi-camera rigs for consistent 3D perception across images.

<details>
  <summary>Details</summary>

**Motivation:** To alleviate the inconsistency issue of depth estimates between overlapping images, achieving dense, metric, and cross-view-consistent depth predictions for 360° field of view perception.

**Method:** Self-supervised surround-view depth estimation from multiple minimally overlapping images using a geometry-guided method that generates a final depth map with improved cross-view consistency.

**Result:** Improved consistency of depth estimates and overall depth prediction accuracy compared to state-of-the-art methods, as verified on DDAD and nuScenes datasets.

**Conclusion:** The proposed method effectively addresses cross-view depth consistency, providing a superior dense 3D perception solution within a 360° field of view.
...
> An improved graph-based surgical scene segmentation approach using combined ViT and GNN models to achieve better performance and anatomically consistent predictions, important for the safety in laparoscopic surgery.

<details>
  <summary>Details</summary>

**Motivation:** The aim is to improve the safety of laparoscopic cholecystectomy surgeries through better identification of hepatocystic anatomy. Deep learning models usually have difficulties with occlusions, complex anatomical relationships and fine-scale structures which this work seeks to handle with graph-based methods.

**Method:** We propose two segmentation models that combine Vision Transformer feature encoders with Graph Neural Networks to address challenges in surgical anatomical identification. The first model uses a k-NN graph with GCNII for stable long-range information propagation, while the second employs a dynamic DGG with a GAT for adaptive topology learning.

**Result:** Both models show significant improvements over state-of-the-art baselines, achieving up to 7-8% higher mIoU and 6% higher mDice scores on surgical image benchmarks. They produce more anatomically coherent predictions, particularly for rare and safety-critical structures.

**Conclusion:** Combining ViT and GNN approaches enhances both performance and anatomical consistency in surgical tool and organ segmentation, leading to potentially safer surgeries. This research opens new avenues for advanced surgical scene analyses and precise identification of critical anatomical features.
...
> 本文提出了一种名为LDAG的新方法，利用属性描述和多模态匹配来提高少样本分割的效果，取得了新的SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的方法主要从支持图像中挖掘元学习指导信息，但由于视觉表征中的类内变化，从支持图像中提取的元信息无法为分割未经训练的类别提供准确的指导。我们的动机是证明支持图像中的参考信息可能并不是必需的，关键在于提供对训练类和未经训练的类都有效的无偏元学习指导。

**Method:** 我们提出了一种名为Language-Driven Attribute Generalization (LDAG)的架构，该架构利用目标类别内在的语言描述来构建强大的支持策略。具体来说，我们设计了一个名为Multi-attribute Enhancement (MaE)的模块，通过大型语言模型(如LLMs)产生目标类别的多个详细属性描述，并利用多模态匹配构建详细的视觉文本先验指导。同时，由于文本-视觉模态转换的困难，我们设计了Multi-modal Attribute Alignment (MaA)来实现属性文本与视觉特征之间的跨模态交互。

**Result:** 实验结果表明，我们提出的方法显著优于现有方法，达到了新的SOTA性能。

**Conclusion:** 我们的方法展示了利用语言描述来指导元学习的有效性和优势，为少样本分割任务提供了一种新的视角和方法。
...
> 本文介绍了StreetView-Waste数据集，该数据集致力于解决城市中垃圾容器监测的问题，并提供检测、追踪、溢出分割三个任务的基准模型及改进策略。

<details>
  <summary>Details</summary>

**Motivation:** 由于缺乏特定垃圾容器追踪数据和实际情境图像，因此现有垃圾检测数据集在管理城市垃圾物流方面存在局限。本文旨在填补这一空白。

**Method:** 提出了StreetView-Waste数据集，支持三个任务：垃圾容器检测、追踪及溢出分割，并提供了优化追踪和分割的策略。

**Result:** 实验结果显示，优化策略显著提升了垃圾容器追踪的精度及垃圾分割的表现。

**Conclusion:** StreetView-Waste数据集提供了一个解决城市垃圾物流问题的新视角，并旨在推动该领域的进一步研究。
...
> 本文提出了VLA-Pruner，一种专用于Vision-Language-Action模型的令牌修剪方法，该方法适用于具有语义理解和动作执行双系统性质的模型，通过双层重要性指标保留视觉令牌，从而提升在不同机器人任务中的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于VLM的修剪方法偏向于保留语义提示，忽略了动作生成所需的关键信息，导致VLA性能显著下降。VLA-Pruner旨在解决这一问题，通过考虑VLA模型的双系统性质和机器人操作中的时间连续性，提升性能。

**Method:** VLA-Pruner采用双级别重要性标准来保留视觉令牌：视觉-语言前置注意力用于语义相关性；通过时间平滑估计的动作解码注意力用于动作重要性。基于此标准，提出了一个新颖的双级令牌选择策略，该策略适应性地保留了在给定计算预算下的语义理解和动作执行所需的一组紧凑而信息丰富的视觉令牌。

**Result:** 实验表明，VLA-Pruner在多种VLA架构和多样化的机器人任务中达到了最先进的性能。

**Conclusion:** VLA-Pruner通过其特有的双级别令牌选择策略，成功解决了现有VLM修剪方法在保留关键动作信息上的缺陷，从而提升了VLA模型在实际应用中的实时部署能力。
...
> LLaVA$^3$, inspired by Cubist art, uses multi-view 2D images to improve a VLM's 3D scene understanding without fine-tuning and shows better performance in 3D VQA and language grounding tasks compared to 2D-based approaches.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to overcome the limitation of limited 3D training data by effectively using multi-view 2D images to improve the 3D scene understanding of VLMs without the need for fine-tuning.

**Method:** The paper proposes LLaVA$^3$, a method that enhances the 3D scene understanding capabilities of vision-language models (VLM) by utilizing multi-view 2D images to create an omnidirectional visual representation of each object in the scene, inspired by Cubist art.

**Result:** Experiments demonstrate that this approach outperforms previously 2D-based VLM solutions on tasks involving 3D Visual Question Answering (VQA) and 3D language grounding.

**Conclusion:** The paper concludes that LLaVA$^3$ can be an effective means to augment vision-language models with enhanced 3D scene comprehension capabilities using only 2D image data and without fine-tuning.
...
> FastSurfer-CC is a newly developed, efficient, and automated framework for analyzing corpus callosum morphometry, providing superior results compared to existing tools and revealing new insights in Huntington's disease research.

<details>
  <summary>Details</summary>

**Motivation:** There is a lack of comprehensive and automated tools for corpus callosum analysis, which is important in aging and neurological disease research, clinical trials, and interventions.

**Method:** FastSurfer-CC is an efficient and fully automated framework for corpus callosum morphometry. It automatically identifies mid-sagittal slices, segments the corpus callosum and fornix, localizes the anterior and posterior commissures, generates thickness profiles and subdivisions, and extracts shape metrics for statistical analysis.

**Result:** FastSurfer-CC outperforms existing specialized tools in individual tasks and identifies statistically significant differences between Huntington's disease patients and healthy controls that other current methods do not detect.

**Conclusion:** The introduction of FastSurfer-CC addresses previous gaps in corpus callosum morphometry analysis and improves the ability to detect differences in neurological conditions such as Huntington's disease.
...
> 研究通过结合深度图和点流信息来改善视频预测模型在遮挡场景中的表现，实验结果表明这种方法在预测遮挡场景和背景运动方面比不使用这些信息的模型更准确。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频预测模型在标准场景下表现良好，但在面对遮挡问题时依然存在挑战，因此提出结合运动信息（点流）和几何结构信息（深度图）来改善遮挡情况下的视频预测。

**Method:** 研究使用多对象潜在转换器架构预测未来帧，并在此基础上加入深度图和点流信息，以探讨这些信息对遮挡视频预测的影响。

**Result:** 实验在合成和真实数据集上进行，结果显示结合点流和深度信息的模型在遮挡场景下有更好的表现，并能更准确地预测背景运动。

**Conclusion:** 结合深度图和点流信息显著提升了视频预测模型在遮挡场景中的性能。
...
> A novel deep generative learning framework integrates wave optics and GAN to synthesize high-fidelity microscope images for improved pose estimation of optical microrobots, broadening the scope for autonomous biological studies.

<details>
  <summary>Details</summary>

**Motivation:** The aim is to enable efficient and high-precision pose estimation of optical microrobots despite the difficulties and costs associated with acquiring quality data from real optical microscopy.

**Method:** This paper proposes a novel physics-informed deep generative learning framework that integrates wave optics-based physical rendering and depth alignment into a GAN to synthesize high-fidelity microscope images for microrobot pose estimation.

**Result:** The proposed method improved the structural similarity index (SSIM) by 35.6% compared to purely AI-driven methods and maintains real-time rendering speeds (0.022 s/frame). The developed pose estimator, trained on synthetic data, achieves high accuracy in microrobot pose estimation, generalizing well to unseen poses.

**Conclusion:** This framework provides a solution for generating high-fidelity microscope images that significantly improves the accuracy of microrobot pose estimation, generalizes to new configurations, and could facilitate autonomous biological studies.
...
> This paper proposes a tumor segmentation method for breast DCE-MRI that uses acquisition time information to improve performance and generalization.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to address the challenges in tumor segmentation due to the large variation in tissue appearance across different DCE-MRI acquisition protocols and individual factors.

**Method:** The paper proposes a tumor segmentation method using feature-wise linear modulation (FiLM) layers to incorporate image acquisition times, enhancing model performance and generalization.

**Result:** Evaluation showed that incorporating knowledge of phase acquisition time improved tumor segmentation performance and model generalization.

**Conclusion:** The proposed method demonstrates better performance and generalization compared to the baseline in tumor segmentation of breast DCE-MRI images.
...
> 研究提出了一种用于室内场景建图和注册CMC的解决方案，利用移动代理的一次性遍历来完成，实验表明方法有效。

<details>
  <summary>Details</summary>

**Motivation:** 旨在解决CMC手动注册低效且昂贵，自动注册存在视觉模糊问题的情况，提出一种新的解决方案。

**Method:** 利用手持RGB-D相机的移动代理一次性遍历整个室内场景，并同步天花板安装摄像头（CMC）以捕获该移动代理，从而生成世界坐标系下的轨迹和场景布局，CMC的伪尺度轨迹和相对姿态通过视频关联和时间戳对齐。基于此初始化，定制因子图优化代理相机姿态、场景布局和CMC姿态。

**Result:** 实验结果表明，该方法不仅能够在统一框架内有效完成两个任务，还能够共同提升任务性能。

**Conclusion:** 提供了一个可靠的工具，能够促进位置感知应用的发展。
...
> 本文介绍了一个专门用于拳击中拳击动作检测分类的视频数据集，包括6,915个高质量拳击片段，分类为六种不同的拳击类型。

<details>
  <summary>Details</summary>

**Motivation:** 由于动态且非结构化的动作及录制环境的变化，开发强大的数据集一直是进行计算机视觉分析的瓶颈。为了支持实时视觉动作识别的研究，特别是在低资源和不受限制的环境中，构建此数据集。

**Method:** 从20个公开的YouTube训练视频中提取并手动分割和标记了18名不同拳击运动员的6,915个高质拳击片段，确保时间边界精确动作类别一致。

**Result:** 该数据集覆盖了广泛的动作风格、摄像机角度和运动员体格，为拳击相关的移动分析、自动化指导和表现评估提供了丰富的基准。

**Conclusion:** 贡献了一个针对性的视频数据集，旨在加速拳击及相关领域内运动分析的进步。
...
> 该论文介绍了一种新的方法SemCLIP，提升了CLIP在处理图像和文本检索中的否定和同义词转换的能力。

<details>
  <summary>Details</summary>

**Motivation:** 现代对比视觉语言模型在图像和文本检索中占主导地位，但它们在处理否定和同义词转换时表现不稳定，这成为改善评估结果和视觉语言模型对齐的重要挑战。

**Method:** 该论文提出了一个新的CLIP对比损失函数，同时考虑了同义词转换和否定，并应用LLM生成的包括原始、转换和否定的文本描述训练三元组进行CLIP样式的模型训练。

**Result:** 实验表明，SemCLIP能够保持CLIP的性能，同时显著增加与否定描述的间隔。在使用原始与否定描述的图像检索准确率评价的CC-Neg基准上，SemCLIP将准确率从68.1%提高到78.1%。

**Conclusion:** 即使在与CLIP进行比较时在Sugarcrepe++基准上结果参差不齐，SemCLIP整体性能优于使用否定描述训练的模型。能够在零样本分类任务中表现更好，这表明SemCLIP能够实现对语义变换的显著鲁棒性。
...
> 本文提出了一种适用于竞技体操的多摄像头追踪方法，结合了领域知识和射线-平面交点技术，以最小化追踪失效，方法在国际比赛中得到验证和高度评价。

<details>
  <summary>Details</summary>

**Motivation:** 多摄像头追踪算法在实际应用中遇到的独特挑战包括摄像机数量受限、环境变化和遮挡等，导致体操运动员轨迹的追踪困难。为了改善这种情况，我们的目标是提高追踪的准确性，特别适用于竞技体操这一特殊场景。

**Method:** 我们提出了一种结合竞技体操领域知识的多摄像头追踪方法。由于训练场地的空间限制和光线、背景、服装等多种因素造成的挑战，传统多摄像头三角测量无法准确追踪体操运动员的三维轨迹。为此，我们引入了一种创新的级联数据关联（DA）范式，利用三角测量生成足够的跨视角检测的三维轨迹候选点，并在检测不足时依赖于射线-平面交点的方法来生成共面的三维轨迹候选点，从而最小化追踪失败。

**Result:** 实验结果表明，我们的方法在挑战性场景中表现出色，并且我们的体操裁判系统已成功应用于最近的世界锦标赛中，得到了国际体操联合会的高度评价。

**Conclusion:** 该研究提供了一种适用于竞技体操的鲁棒性多摄像头追踪解决方案，其有效性已经在国际比赛中得到了验证。
...
> The paper examines local and global methods for optical flow, focusing on the Horn-Schunck algorithm and its multiresolution implementation.

<details>
  <summary>Details</summary>

**Motivation:** To investigate the theoretical and practical merits of local and global optical flow methods and enhance their accuracy with a multiresolution approach.

**Method:** Analysis of the Horn-Schunck and Lucas-Kanade algorithms, implementation of a multiresolution Horn-Schunck algorithm.

**Result:** The multiresolution Horn-Schunck technique shows improved accuracy and convergence under various image conditions.

**Conclusion:** Combining the Horn-Schunck algorithm with a multiresolution approach is effective for accurate motion estimation between frames.
...
> 本文提出了一种两阶段的合成图像检测框架，通过监督对比学习提取判别式嵌入，并用k-NN分类器进行标记，实现了在不需要大规模重新训练的情况下提高检测准确率和源归因性能。

<details>
  <summary>Details</summary>

**Motivation:** 快速发展的生成式人工智能技术带来的合成图像难以分辨，传统检测方法因迭代模型周期短而变得不切实际，因此本文旨在解决合成图像检测中的泛化挑战。

**Method:** 提出了一种两阶段检测框架：第一阶段使用监督对比学习的视觉深度学习模型提取图像的判别性嵌入，模型在训练过程中有意隐藏某些生成器来加强跨生成器泛化；第二阶段采用k-NN分类器基于学习的嵌入空间进行分类，是在少量样本下进行的少样本学习。

**Result:** 在少样本学习环境仅使用150张图像的情况下，该框架实现了高达91.3%的平均检测准确率，比现有方法改进了5.2个百分点，并在开放式分类的AUC和OSCR指标上分别提高了14.70%和4.27%，证明了模型的有效性。

**Conclusion:** 所提出的方法证明了在快速变化的生成式AI技术下构建可适应的鲁棒性伪造图像检测系统的能力，同时避免了过于频繁的大规模再训练需求。
...
> EOGS++ is a novel Gaussian Splatting method for satellite imagery that improves on EOGS by directly processing raw data and integrating bundle adjustment, achieving better reconstruction quality and efficiency.

<details>
  <summary>Details</summary>

**Motivation:** To offer a method that has competitive reconstruction quality to NeRF but with reduced training times, and to improve camera pose estimation and geometric accuracy.

**Method:** EOGS++ extends the EOGS framework to directly process raw high-resolution panchromatic satellite imagery without external preprocessing and incorporates bundle adjustment using optical flow within the training process.

**Result:** Experiments on IARPA 2016 and DFC2019 datasets show that EOGS++ outperforms EOGS and other NeRF-based methods in terms of reconstruction quality, efficiency, and accuracy.

**Conclusion:** EOGS++ demonstrates state-of-the-art performance with an improved mean MAE error on buildings compared to the original EOGS models, confirming its efficiency and quality advantages in Earth observation applications.
...
> 研究提出了VARiant模型，通过灵活调整网络深度，有效地降低了视觉自回归模型的内存负担，同时保持良好的图像生成质量。在实验中，该方法在减少内存使用的同时提高了生成效率。

<details>
  <summary>Details</summary>

**Motivation:** 视觉自回归（VAR）模型通过“下一尺度”预测范式显著减少了推理步骤，但渐进多尺度生成带来了由于累积KV缓存导致的较大内存开销，限制了实际部署。研究人员观察到在VAR模型中存在尺度-深度的非对称依赖关系，启发VARiant的设计。

**Method:** VARiant方法通过等距采样从原始30层VAR-d30网络中选择多个子网，范围从16层到2层。早期尺度由完整网络处理，而后期尺度使用子网。子网和完整网络共享权重，实现单模型内的灵活深度调整。为了缓解权重共享带来的优化冲突，提出了一种渐进训练策略，既能优化子网也能优化整个网络，在固定比例训练下突破Pareto前沿，实现联合最优。

**Result:** 实验结果表明，与预训练的VAR-d30（FID 1.95）相比，VARiant-d16和VARiant-d8保持几乎相同质量（FID分别为2.05和2.12），同时减少了40-65%的内存消耗。VARiant-d2在中等质量损失情况下（FID 2.97），速度提升3.5倍，内存减少80%。

**Conclusion:** VARiant能够实现高质量与高效性的平衡，其单一模型架构支持无成本的运行时深度切换，从高质到极高效率的不同部署模式，适用于不同的应用场景。
...
> Lite Any Stereo是一种新的立体深度估计框架，它在保持高效性的同时实现了强大的零样本泛化能力，并在多个实际基准测试中表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 尽管立体匹配的最新进展集中在提高准确性上，但这往往会大幅增加模型的大小。我们的动机是挑战传统观点，即高效的模型不具备零样本能力，通过Lite Any Stereo证明超轻模型也能实现强泛化能力。

**Method:** 我们引入了Lite Any Stereo，这是一种新的立体深度估计框架，它实现了强大的零样本泛化能力，同时保持了高效性。为此，我们设计了一个紧凑但表达能力强的主干网络以确保可扩展性，并配备了一个精心设计的混合成本聚合模块。此外，我们还提出了一种在百万级数据上的三阶段训练策略，有效解决了模拟到现实的差距问题。

**Result:** 该模型在四个广泛使用的实际基准测试中排名首位，实现了与非基于先验知识的最先进方法相当甚至更好的准确性，同时计算成本不到1%。

**Conclusion:** Lite Any Stereo框架展示了超轻量模型可以在效率和准确性之间达到新的平衡，为高效的立体匹配设定了新标准。
...
> 本文介绍了NutriScreener，这是一种通过儿童图像进行营养不良检测的技术，适用于低资源环境，并在不同数据集上展现了良好的性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前用于检测儿童营养不良的方法耗时且不易扩展，限制了早期干预。因此，本研究提出NutriScreener以提供一个可扩展且准确的早期营养不良检测解决方案。

**Method:** NutriScreener采用检索增强的多姿态图注意力网络，结合基于CLIP的视觉嵌入、类别增强的知识检索和上下文感知能力，可以从儿童的图像中实现对营养不良的健壮检测和人体测量预测。

**Result:** 在临床研究中，医生对NutriScreener的准确性和效率的评分分别是4.3/5和4.6/5，证实其在低资源环境中的应用准备就绪。使用包含2,141名儿童的数据集进行训练和测试，并在多个跨地区的数据集上进行评估，展示了在儿童不受限制的环境下的可靠测量能力。

**Conclusion:** NutriScreener提供了一个可用于低资源环境中的早期儿童营养不良检测的可扩展且准确的解决方案。
...
> 本研究引入了POMA-3D，首个多个视角下强制一致的点图特征的3D表示模型，解决了3D表示学习中预训练先验稀缺和数据有限的问题。实验表明，该模型对3D理解基础任务表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机在于解决3D表示学习中预训练先验稀缺和数据有限的问题，采用的是探索一个新的点图方式来理解3D场景。

**Method:** 该研究提出了POMA-3D，这是一个从点图中学习到的自监督3D表示模型。点图编码了在结构化2D网格上的明确3D坐标，保留了全局3D几何同时兼容2D基础模型输入格式。为了转移丰富的2D先验到POMA-3D，设计了一种视图到场景对齐策略。POMA-JEPA，一种联合嵌入预测架构，也被引入，以在多个视角下强制一致的点图特征。此外，该研究引入了ScenePoint，一个由6500个房级RGB-D场景和100万2D图像场景构建的点图数据集，以促进大规模POMA-3D预训练。

**Result:** 研究成果展示了POMA-3D在使用仅几何输入(即3D坐标)的情况下，在3D问答、具身导航、场景检索和具身定位等多种任务上的优越表现。这表明POMA-3D是3D理解的强大基础模型。

**Conclusion:** 此研究的主要结论是，POMA-3D作为一个点图形式对3D场景理解的探索，解决了3D表示学习中预训练先验稀缺和数据有限的问题，为各种任务提供了一个强大的3D理解基础模型。
...
> 本论文提出了一种名为‘Erase to Retain’的可控遗忘框架，用于医学图像分割。通过低秩适应和师生精馏范式，该框架能够在不过度重新训练的情况下有针对性地忘记特定知识。实验表明，这种方法可以在保持整体性能的同时，降低遗忘集的性能，实现了在医学图像分析中负责任、可控和可逆的遗忘。

<details>
  <summary>Details</summary>

**Motivation:** 为了满足隐私合规、伦理部署和不断修订数据集的需要，可以从医学分割网络中选择性地移除知识变得愈发重要。

**Method:** 该研究采用低秩适应性约束子空间更新，使用师生精馏范式，允许学生网络在低秩解码空间中抹除特定病变或类别的表示，同时保留全局解剖理解。强遗忘阶段后，经过仅头部监督细化的温和恢复阶段。

**Result:** 在ISIC分割任务中，学生对遗忘集的IoU从0.875降低到了0.509，同时在保留和验证集上保持了竞争力。在CHASE跨域数据集上，Erase to Retain同样显示出对遗忘集IoU的一致降低，同时保留了保留和验证集的效用。ISIC分类实验中同样体现了性能的提升。

**Conclusion:** 研究表明，基于低秩适应的子空间遗忘为医疗图像分析提供了一个实际的路径，使得模型可以有选择地遗忘敏感样本或结构，同时保障在关键领域的表现。
