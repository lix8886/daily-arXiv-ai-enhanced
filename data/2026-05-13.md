...
> 本文提出了一个有效性-多样性框架，将语言模型多样性崩溃问题归因于解码过程中有效和无效续写之间的概率质量分配不当，揭示了顺序校准和形状校准两个根本原因。

<details>
  <summary>Details</summary>

**Motivation:** 现代LLM经常会收束到狭窄的输出子集，缺乏多样性，这对创意生成和科学发现等应用至关重要，但现有工作对推理时概率分布如何导致这一问题的认识不足。

**Method:** 建立了有效性-多样性分析框架，通过形式化两种校准失效机制来解释多样性瓶颈：1）顺序校准失效，有效token不能可靠地排在无效token之上；2）形状校准失效，概率质量过度集中在少数有效续写上。开发了受控诊断工具和确切已知有效集的任务来探测这些瓶颈。

**Result:** 在14个跨越多种系列和规模的语言模型上实证发现，多样性崩溃不是特定采样启发式算法的局限，而是LLM分布中顺序和形状校准失效的必然结果。局部失效在解码步骤中累积，导致序列层面的强烈多样性损失。

**Conclusion:** 多样性崩溃是一个更深层次的校准问题，不仅关乎采样策略，更反映了LLM在概率排序和形状分布上的系统性缺陷。
...
> EpiKG是一种用于临床电子病历问答的知识增强检索生成系统，通过包含断言标签和时间性标签的患者知识图谱，显著提升了临床问答的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的临床推理基准测试使用干净的输入，但实际临床应用需要从电子病历中检索信息，其中否定、时间性和家庭vs患者归因会导致答案错误。需要解决检索层面的挑战以提升临床问答质量。

**Method:** EpiKG构建带有断言标签和时间性标签的患者知识图谱，根据问题意图进行路由检索。在包含43名MIMIC-IV患者的400个问题测试集上，对6个大型语言模型进行7种条件消融实验，并通过三名医生盲目评判100个配对项目。

**Result:** EpiKG在主要终点上比基线提升了22.0个百分点（p=0.0192），架构新颖性带来了+8.84百分点的提升。医生评判发现56%的自动生成参考答案存在缺陷，表明临床QA基准需要医生评审。

**Conclusion:** 意图感知的知识图谱检索增强方法能显著改善临床问答性能，但高基线LLM的提升幅度较小。研究强调了医生评审在临床NLP基准测试中的必要性，并公开了所有相关资源。
...
> 该论文通过因子分解实验发现，在约1.5亿参数的基座模型上，仅重写路由器就能带来显著的性能提升，而进生命周期机制实际上会损害性能。

<details>
  <summary>Details</summary>

**Motivation:** 研究进化式混合LoRA系统中各组成部分的真实贡献，澄清之前对整个系统性能提升的错误归因。

**Method:** 采用部分2^3因子实验设计，将进化式LoRA系统分解为路由器重写、逐域评估范围和生命周期三个因素，通过3个随机种子进行消融实验分析。

**Result:** 路由器重写贡献了全部+0.0426 nat的对数困惑度改进（p=0.006），而逐域评估无效果，生命周期反而造成约-0.028 nat的性能下降；只有在适配器预对齐任务时进化搜索才有用。

**Conclusion:** 进化式LoRA系统的性能提升完全来自路由器重写而非进化机制，进化搜索仅在特定条件下有效，其他情况下会降低性能。
...
> 双脑模型通过可训练的神经接口耦合两个冻结的语言模型，实现连续、并行的通信，在算术、逻辑推理和数学推理任务中显著提升性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模型和工具增强系统通过文本序列化进行通信，效率低下。本研究探索是否能让两个预训练语言模型通过连续、并行的通道直接协调。

**Method:** 双脑模型通过可训练的神经接口连接两个冻结的语言模型，在中间隐藏状态层进行通信。主模型驱动任务执行，辅助模型操作工具，两者通过转换网络和学习抑制门（约1%参数）的条件激活相互协调。

**Result:** 在算术任务中，两个0.5B模型与计算器结合使准确率从36%提升到96%；在逻辑网格谜题中，与Z3求解器结合实现1.7倍基准性能；在数学推理中，辅助模型能仅从隐藏状态信号生成特定问题代码。

**Conclusion:** 双脑模型成功实现了语言模型间的连续并行通信，通过简单的抑制门学习选择性通信协议，无需预设格式，为多模型协作系统提供了新的有效范式。
...
> 本研究系统评估了使用差分隐私随机梯度下降(DP-SGD)训练的大语言模型中的社会偏见，发现差分隐私在句子评分任务中减少了偏见，但这种改善并不适用于所有任务，揭示了logit级偏见与输出级偏见之间的差异，并指出减少记忆化不一定能降低不公平性。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在训练过程中会记忆敏感数据，造成隐私风险。差分隐私作为限制训练过程中个体数据影响的框架，但其与LLM中社会偏见的关系尚未明确，需要深入研究。

**Method:** 研究者构建了使用DP-SGD训练的预训练LLM，通过四种互补评估范式（句子评分、文本补全、表格分类、问答）比较DP模型与非DP基线模型的社会偏见表现，使用受控似然比较等方法测量偏见。

**Result:** 研究发现DP在句子评分任务中减少了偏见，但这种改善无法推广到所有任务。结果揭示了logit级偏见与输出级偏见之间的不一致性，同时表明减少记忆化不必然降低不公平性。

**Conclusion:** 研究强调了在评估LLM公平性时多范式评估的重要性，表明单一的隐私保护措施（如差分隐私）并不能全面解决社会偏见问题，需要更综合的公平性评估方法。
...
> 研究揭示了指令触发语言模型中产生中心机制的认知不对称性——输出标记中的任务信息与行为强相关且受指令显著影响，而输入标记中的信息相对稳定且与行为关联弱。

<details>
  <summary>Details</summary>

**Motivation:** 为了深入理解指令如何触发语言模型的产生机制，研究采用认知启发的双过程视角，分离语言处理和产生阶段，探究指令标记在不同任务中的作用机制。

**Method:** 通过五个二元判断任务，分层探测任务特定信息；测量指令标记对输入样本标记处理和输出标记产生的不同影响；基于注意力的干预实验验证因果关系；分析模型规模和指令调优的影响。

**Result:** 发现样本标记中的任务信息保持稳定且与行为弱相关，而输出标记中相同信息变化大且与行为强相关；阻断指令流向后续标记会显著影响行为和输出信息，仅阻断样本标记影响甚微；这种不对称性在不同模型家族和任务中普遍存在，且随模型规模和指令调优增强。

**Conclusion:** 研究表明理解模型能力需要结合内在机制和外在行为，并通过标记位置分解内在视角，区分输入标记处理和输出标记产生的不同过程。
...
> ReVision通过学习式补丁选择器减少计算机视觉代理中的冗余视觉标记，使标记使用减少46%，性能提升3%，解决了历史轨迹处理效率低下的核心问题。

<details>
  <summary>Details</summary>

**Motivation:** 计算机使用代理依赖大量视觉标记来编码GUI截图，随着交互轨迹增长，标记成本快速上升，限制了历史信息的利用，导致性能难以提升。

**Method:** 提出ReVision方法，训练多模态语言模型时使用学习式补丁选择器，通过比较连续截图的补丁表示来移除冗余视觉补丁，同时保持模型所需的空间结构。

**Result:** 在三个基准测试中，处理5张历史截图时平均减少46%标记使用，成功率比不丢弃基线提升3%，实现了明确的效率增益。

**Conclusion:** 视觉历史饱和现象源于标记表示效率低下而非历史信息有限，移除冗余后性能随历史观测数量增加而持续改善，为代理处理更长轨迹提供了新可能。
...
> RETUYT-INCO团队在BEA 2026德语简答题评分任务中提出元提示方法，在多个赛道中取得中等排名。

<details>
  <summary>Details</summary>

**Motivation:** 针对德语简答题评分任务中评分标准动态变化的挑战，需要开发灵活适应不同评分标准的方法。

**Method:** 提出元提示(Meta-prompting)方法，让LLM基于训练集示例创建自定义提示来评分学生答案；同时尝试了传统机器学习、开源LLM微调和多种提示技术。

**Result:** 在Track 1获得第6名(QWK=0.729)，Track 3获得第4名(QWK=0.674)，Track 4获得第4名(QWK=0.49)。

**Conclusion:** 元提示方法为动态评分任务提供了可行解决方案，虽未获得顶尖排名但验证了方法的有效性。
...
> Hebatron是一个基于NVIDIA Nemotron-3稀疏MoE架构的希伯来语专业大语言模型，通过三阶段渐进式课程学习训练，在希伯来语推理基准上达到73.8%，推理速度提升9倍。

<details>
  <summary>Details</summary>

**Motivation:** 开发一个专门针对希伯来语的高效大语言模型，解决现有模型在希伯来语处理上的局限性，同时保持高推理效率。

**Method:** 采用三阶段从易到难的课程学习与持续防遗忘锚定机制，然后在200万希伯来语-英语双语样本上进行监督微调，整个模型有300亿参数但只激活30亿参数。

**Result:** 在希伯来语推理平均得分73.8%，超越DictaLM-3.0-24B-Thinking(68.9%)，支持65,536token的原生长上下文，推理吞吐量提升约9倍。

**Conclusion:** 这是首个针对特定语言的Nemotron-3架构适配，也是首款开放权重的希伯来语专业MoE模型，为希伯来语和闪米特语NLP研究提供了重要贡献。
...
> ReAD是一种强化引导的能力蒸馏框架，通过显式建模能力间的相互依赖关系，在固定token约束下自适应分配蒸馏预算，比传统方法更有效地压缩LLM并保留下游任务所需能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有能力蒸馏方法将能力视为独立训练目标，忽略了改进某种能力会重塑学生模型整体能力谱系的现象，特别是在多种能力共同决定任务成功的情况下，导致蒸馏效果不理想且资源浪费。

**Method:** ReAD框架包含三个步骤：1）推断任务必需能力；2）动态生成针对特定能力的监督信号；3）使用不确定性感知的情境bandit基于预期效用收益自适应分配蒸馏预算。该方法显式考虑了能力间的相互依赖关系。

**Result:** 实验表明，在相同token预算下，ReAD相比强基线提高了下游任务效用，同时减少了有害溢出效应和蒸馏资源浪费。

**Conclusion:** 通过引入能力间相互依赖建模和自适应预算分配机制，ReAD为大规模语言模型的能力蒸馏提供了更高效的解决方案，在保持关键能力的同时优化了资源利用效率。
...
> 研究使用大语言模型从自发语音中零样本预测Ryff心理幸福感，多个模型取得高达0.8的相关性，并提供了可解释性分析。

<details>
  <summary>Details</summary>

**Motivation:** 传统心理评估需要专业干预和耗时评估，探索利用LLMs从自然语音中直接预测心理幸福感，为更便捷的心理健康监测提供可能。

**Method:** 使用111名参与者的几分钟语音录音，评估12个指令调优的LLMs，与临床心理学和语言学专家合作开发领域相关知识提示。

**Result:** LLMs能从自发语音中提取语义线索，在80%数据上达到0.8的Spearman相关性，通过统计分析和词云分析提供了预测的可解释性。

**Conclusion:** LLMs在直接从语音预测心理幸福感方面展现出强劲能力，为自动化心理健康评估提供了新的技术路线。
...
> SOMA框架通过利用对话初期提取局部响应流形，将小型代理模型适配到该局部区域，在保持多轮对话上下文连贯性的同时大幅降低大型语言模型的服务成本。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在多轮对话服务中，传统的完整历史拼接方法虽然能保证连贯性，但会带来显著的延迟、内存和API成本问题，尤其是在调用大型专有模型时。现有方法难以有效平衡响应质量和效率之间的权衡。

**Method:** SOMA框架首先利用对话前几轮估计局部响应流形，然后学习soft prompts来最大化大小模型响应的语义分歧，找出最不对齐的局部方向；通过抗退化控制稳定训练；将挖掘到的案例通过LoRA微调蒸馏到代理模型中，使其在推理时无需prompts；设置简单的门控机制进行一次性切换，并配备回滚机制处理漂移。

**Result:** 大量实验证明了SOMA的有效性，在保持对话质量的同时显著降低了服务成本。该方法提供了理论分析支撑关键组件设计。

**Conclusion:** SOMA框架为多轮对话场景下的大型语言模型服务提供了一种创新的成本优化方案，通过局部适配和蒸馏技术实现了质量与效率的良好平衡。
...
> 本研究评估了大语言模型从灾害社交媒体中提取因果关系的能力，并开发了基于专家的评估框架。

<details>
  <summary>Details</summary>

**Motivation:** 灾害期间从社交媒体提取因果关系能增强态势感知，但相关帖子往往非正式、碎片化且包含个人体验而非明确因果关系，传统的因果关系提取方法效果有限。

**Method:** 提出专家评估框架，将LLM生成的因果图与灾害报告中的参考图进行对比，并评估提取的关系是否得到事后证据支持或反映模型先验。

**Result:** 研究发现LLM在灾难决策支持系统的因果关系提取方面既有潜力也存在风险。

**Conclusion:** LLM在灾害社交媒体因果关系提取中展现出应用前景，但需要谨慎评估其提取关系的可靠性，以避免在决策支持系统中产生误导性结果。
...
> EvalAgent是一个自动化评估AI代理的AI助手，通过编码评估领域专业知识为评估技能，显著提升了评估代码的执行成功率（从17.5%到65%）和人类专家偏好度（79.5%）。

<details>
  <summary>Details</summary>

**Motivation:** AI代理评估需要评估复杂的多步骤行为，涉及工具使用和中间推理，这过程成本高昂且需要专业知识。直接使用前沿编程助手进行评估效果不佳，成功率仅30%，且会产生过度工程化的评估方案。

**Method:** EvalAgent将评估领域专业知识编码为评估技能（程序性指令、可重用代码和模板、动态检索的API文档），这些技能组合成一个基于追踪的管道，生成完整的评估工件，包括指标、可执行代码和报告。引入了元评估框架和Eval@1指标来衡量评估代码的执行效果。

**Result:** EvalAgent产生更专注的评估，将Eval@1指标从17.5%提升到65%，获得79.5%的人类专家偏好。消融实验表明评估技能对处理复杂评估至关重要，移除后Eval@1从65%降至30%。

**Conclusion:** 单纯的编程能力无法保证可靠的代理评估，专门的评估技能和领域知识是必要的。EvalAgent成功展示了如何通过结构化的评估技能实现高质量、自动化的AI代理评估。
...
> 本文提出Deep Reasoning方法，通过结构化元推理在推理时构建任务特定的脚手架，使LLM智能体能像人类一样灵活切换推理模式。该方法在多个复杂任务上超越现有方法24.8%，甚至能让8B模型超越32B基线。

<details>
  <summary>Details</summary>

**Motivation:** 人类通过灵活切换推理模式解决复杂问题，而当前LLM智能体的脚手架预先固定了推理结构，缺乏适应性。当任务结构与预定义不匹配时表现脆弱，需要一种能动态构建推理结构的方法。

**Method:** 提出Deep Reasoning推理时方法，使用形式化语言表示元推理作为可执行的分解，包含关联推理、形式计算和递归子问题求解。通过上下文示例编码分解原则来指导测试时脚手架构建，在通用智能体DOLORES中实现。

**Result:** 在四个难题基准测试中，DOLORES在所有模型规模和家族上都优于现有脚手架方法，平均比最强基线提升24.8%。通过将认知分配到结构化、低负载的推理线程，减少了过早终止和幻觉。

**Conclusion:** 实验表明，将脚手架视为自适应推理可以显著提升性能，DOLORES的成功指明了未来智能体系统的方向：即时构建每个任务所需的结构，实现类似人类的灵活推理能力。
...
> 该研究提出了LayerTracer诊断框架，通过分析层表示的演化模式和稳定性，发现深层是任务执行的关键区域且稳性高。结果表明，训练浅层并冻结深层的策略在持续预训练中表现最佳，为资源受限的团队提供了低成本、可解释的解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型的增量预训练需要选择性地更新网络层，但确定哪些层应该冻结或训练仍是一个缺乏可解释性指导的经验性黑盒问题，需要提供可解释的层选择方法。

**Method:** 提出LayerTracer诊断框架，通过定位任务执行位置和量化层敏感性来揭示层表示的演化模式和稳定性；基于分析发现，设计了不同的冻结-训练策略并进行对比实验验证。

**Result:** 分析发现深层是任务执行的关键区域且具有高稳定性；实验结果表明，训练浅层并冻结深层的策略在C-Eval和CMMLU基准测试中持续优于全参数微调和相反分配策略。

**Conclusion:** 该工作为资源受限的团队提供了低成本且可解释的解决方案，为持续预训练和混合模型构建中的层参数分配提供了可操作的指导原则。
...
> Agent-BRACE是一种将LLM代理分解为信念状态模型和策略模型的方法，通过结构化的自然语言声明和置信度标签来近似信念分布，有效解决了长视野部分可观测环境中的挑战。

<details>
  <summary>Details</summary>

**Motivation:** LLM在长视野部分可观测环境下面临两个核心挑战：部分可观测性需要维护未观测属性的不确定性，长交互历史导致上下文不断增长稀释任务相关信息。

**Method:** 提出Agent-BRACE，将LLM代理解耦为信念状态模型和策略模型。信念状态模型生成一组带有序数值置信度标签的原子自然语言声明来近似信念分布，策略模型基于这种紧凑的近似信念而非完整历史进行决策。

**Result:** 在长视野部分可观测的具身语言环境中，Agent-BRACE相比强RL基准平均绝对性能提升+14.5%（Qwen2.5-3B）和+5.3%（Qwen3-4B），同时保持与回合长度无关的近乎恒定的上下文窗口。

**Conclusion:** 该方法成功解决了LLM代理在部分可观测环境中的长期决策问题，通过结构化的信念状态表示实现了高效的不确定性管理和可扩展性，且信念状态的校准度随着证据积累而改善。
...
> 研究发现，通过偏好优化方法，仅用300个高质量斯多葛学派文本实例就能让小型语言模型较好地学习向内面向的美德，但所有模型都未能掌握向外面向的世界主义责任，揭示了小型模型的表征局限性。

<details>
  <summary>Details</summary>

**Motivation:** 探索大型语言模型在严重数据约束下内化细微哲学框架的能力，特别是小型模型如何通过微数据集学习斯多葛学派哲学。

**Method:** 使用偏好优化方法（ORPO、AlphaPO）在包含300个基础斯多葛学派文本的微数据集上专门训练小型语言模型，通过多模型评判器库进行评估。

**Result:** 300个高质量示例足以使模型与斯多葛学派向内面对的美德强对齐，接近少样本提示效果且节省上下文窗口，但所有模型在向外面对的世界主义责任方面持续失败。

**Conclusion:** 微数据集适配无法克服小型模型在表征特定哲学概念（如对外责任）时的根本局限性，即使偏好优化也难以实现完整框架内化。
...
> 本研究通过语义扰动评估框架评估出版物类型分类器的鲁棒性，并提出结合实体掩码和领域对抗训练的策略，成功缓解了鲁棒性与领域内准确率之间的权衡问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的生物医学文献自动索引方法虽然提高了领域内准确率，但过度依赖表面词汇特征，导致在分布变化时鲁棒性不足，影响证据合成的可靠性。

**Method:** 构建基于控制的语义扰动评估框架，采用结合实体掩码和领域对抗训练的鲁棒性导向训练策略，通过选择性抑制非任务定义特征来提升模型鲁棒性。

**Result:** 实验表明所提方法能有效缓解鲁棒性与准确率的权衡，改进源于两个互补机制：增加对明确方法学线索的依赖和减少对虚假领域特定主题特征的依赖。

**Conclusion:** 该研究强调了特征级鲁棒性分析的重要性，并指出进一步优化掩码和对抗目标以更精确地抑制主题信息可以提升鲁棒性，为生物医学文献索引系统提供了新的改进方向。
...
> 研究发现隐藏层蒸馏(HLD)虽然在困惑度上持续优于标准知识蒸馏，但在下游任务评估中并未展现出一致优势，表明HLD在大语言模型预训练中仍有待突破。

<details>
  <summary>Details</summary>

**Motivation:** 现有知识蒸馏研究主要依赖输出logits，忽略了教师模型中间表示的语义信息；隐藏层蒸馏在编码器架构中显示出潜力，但在大规模解码器预训练中的应用尚未充分探索。

**Method:** 使用Gemma3 3.4B作为教师模型，123M和735M规格的学生模型，在C4数据集上训练最多168B tokens，通过计算控制的实验比较HLD、基于logits的KD和自监督基线方法。

**Result:** HLD在所有共享超参数配置下系统性降低困惑度，但在下游评估任务中未能一致超越标准KD，说明提取到了潜在信号但效果有限。

**Conclusion:** 隐藏层蒸馏能够提取教师模型的潜在信号，但要在大语言模型预训练中发挥更重要作用，仍需要重大突破性改进。
...
> Checkup2Action是一个多模态体检报告数据集和基准，用于生成结构化的行动卡，帮助患者理解检查结果并采取合适的后续行动。

<details>
  <summary>Details</summary>

**Motivation:** 体检报告包含页面布局、表格、数值指标、异常标记、影像检查结果和专业术语等多模态信息，普通患者难以理解和转化为具体的后续行动。虽然大型语言模型在医疗摘要和分诊支持方面表现出色，但在从多模态体检报告生成安全、优先级明确且面向患者的行动方面还缺乏基准评估。

**Method:** 构建包含2000份真实体检报告的数据集，制定结构化行动卡生成任务（包含优先级、推荐科室、随访时间窗、患者解释等字段），设计涵盖问题覆盖率、优先级一致性、推荐准确性、安全性等多维度的评估协议，测试通用和医疗大语言模型的性能。

**Result:** 实验发现不同模型在问题覆盖、行动正确性、简洁性和安全性对齐之间存在明显权衡，表明该任务具有挑战性，需要平衡多个评估维度。

**Conclusion:** Checkup2Action为评估基于体检报告的患者导向推理能力提供了新的多模态基准，填补了该领域的空白，对发展患者友好的医疗AI助手具有重要意义。
...
> 本文提出了一种基于协方差加权的优化方法，通过高斯核动态调节token级更新，解决了GRPO在探索-利用权衡中的不稳定性问题。

<details>
  <summary>Details</summary>

**Motivation:** GRPO方法在提升大语言模型推理能力时存在探索-利用权衡的困难，经常导致训练不理想，性能次优。

**Method:** 基于熵变化受token概率与相应优势协方差控制的理论洞察，提出超参数免费的协方差加权优化方法，使用高斯核动态降低极端token级更新的影响。

**Result:** 实验证明该方法相比GRPO在多个推理基准测试中提升了下游性能，并有效稳定了训练过程中的熵值。

**Conclusion:** 提出的协方差加权方法能自动减少探索-利用权衡带来的不稳定性，同时保留有用学习信号，为大语言模型推理能力提升提供了更稳定的训练方案。
...
> 研究提出了一个三机制框架来解释大型语言模型在处理训练知识与矛盾文档冲突时的不同行为模式，解决了现有文献中的经验矛盾。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究在LLM如何处理知识与文档冲突上存在 contradictory 发现：有些显示模型固执地保留训练答案，忽略文档近50%时间；其他则显示模型轻易地遵循文档96%时间。这种矛盾阻碍了对模型行为的理解。

**Method:** 提出三机制框架：机制1（单源更新，证据 coherence 主导）、机制2（竞争整合，参数确定性主导）、机制3（任务适选，任务知识需求主导）。形式化区分参数强度（暴露频率）和参数唯一性（编码一致性），并通过9,970次API调用在5个模型上验证框架。

**Result:** GEE逻辑回归确认所有5个模型的机制2确定性梯度（beta = -0.38到-0.50，p ≤ .013）。机制3消融显示任务框架单因素就能使上下文遵循率从近100%降至6-71%（p < .001）。确定性梯度在多项结果建模、对冲响应敏感性分析和FDR校正下均稳健。

**Conclusion:** 不同研究间的矛盾源自未区分三种本质上不同的处理机制。三机制框架统一了解释了模型行为差异，参数强度是稳定事实领域的操作性预测因子，任务框架对知识选择策略有决定性影响。
...
> BitLM通过将token表示为二进制编码，在每个块内使用轻量级扩散头并行去噪多个token，在保持自回归因果结构的同时突破了单token生成的瓶颈，实现了更高效的预训练和更快的推理。

<details>
  <summary>Details</summary>

**Motivation:** 传统的自回归语言模型逐个token生成的方式产生了单token瓶颈，限制了模型在预训练时的表达能力和推理时的吞吐量。现有解决方案要么保留了根本瓶颈，要么牺牲了语言建模必需的因果结构。

**Method:** BitLM将每个token表示为固定长度的二进制编码，在每个注意力块内使用轻量级扩散头并行去噪多个token。模型保持块间的左到右因果注意力，但在每个块内进行联合词汇决策，用位级去噪替代大词汇量softmax。

**Result:** BitLM突破了逐token范式，实现了更高效的预训练和显著更快的推理速度，同时保持了使语言模型有效的因果基础，展示了更强大、更快的语言模型性能。

**Conclusion:** 逐token生成范式并非根本要求，而是一种接口选择。通过改变这一选择可以获得更强更快的语言模型。BitLM为下一代语言模型架构指明了一个有前景的方向。
...
> Ada-MK通过编译时优化和共享内存约束模型，在NVIDIA Ada GPU上实现了MegaKernel的高效部署，显著降低了LLM推理延迟，首次在商业在线广告系统中成功部署。

<details>
  <summary>Details</summary>

**Motivation:** 在商业在线广告系统中，LLM需要毫秒级的实时推理延迟，但现有MegaKernel实现在便携性与效率之间存在根本冲突，无法满足资源受限GPU部署需求。

**Method:** 提出三维共享内存约束模型结合K维度分片、基于MLIR的细粒度DAG离线搜索优化，以及将MegaKernel嵌入TensorRT-LLM的异构混合推理引擎。

**Result:** 在NVIDIA L20上，单批吞吐量比vanilla TensorRT-LLM提升23.6%，比vLLM提升50.2%，在所有测试场景中均实现正向增益。

**Conclusion:** 首次成功在商业在线广告系统中部署MegaKernel，通过编译时决策替代运行时动态调度，解决了便携性与效率的平衡问题，为延迟敏感的LLM推理部署提供了有效方案。
...
> 提出一种高效生成式定位框架，通过自适应分组量化、分层自适应稀疏化和前缀树并行验证技术，在保持生成质量的同时显著加速LLM推理过程，适用于实时广告系统。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在广告创意生成和定向广告等场景表现优异，但其高推理延迟和计算成本严重阻碍了在实时广告系统中的实际部署。

**Method:** 开发了Efficient Generative Targeting框架，集成了三项关键技术：自适应分组量化技术减少计算精度需求，层自适应分层稀疏化选择性激活网络层，前缀树并行验证加速推理验证过程。

**Result:** 在两个真实广告场景的实验中，框架实现了显著的推理加速倍数，同时保持了可接受的生成质量，证明了其在实际部署中的可行性。

**Conclusion:** 该框架有效解决了LLM实时部署的推理效率问题，为广告系统中大规模应用生成模型提供了切实可行的优化方案。
...
> DiffScore是一种基于掩码扩散语言模型的文本评估框架，通过双向上下文消除位置偏差，提供从局部流畅性到全局一致性的多维度评估。

<details>
  <summary>Details</summary>

**Motivation:** 传统的自回归语言模型采用从左到右的分解方式，导致早期token只能看到左侧上下文，引入了位置偏差，将架构不对称性与真实文本质量混为一谈。

**Method:** 提出掩码重建作为替代范式，每个token都在完整双向上下文下评分；构建基于掩码大规模扩散语言模型的DiffScore框架；通过连续掩码率测量文本可恢复性。

**Result:** 在十个基准测试中，DiffScore在零样本和微调设置下均优于自回归基线模型，提供了多时间步质量分析和双向PMI分解等诊断工具。

**Conclusion:** DiffScore成功消除了位置偏差，建立了从局部流畅性到全局一致性的评估层次，为文本质量评估提供了更准确且多维度的新方法。
...
> PRISM提出了一种新的诊断方法，通过闭合形式上界分析LLM变体（量化、LoRA适配、蒸馏模型）的表征漂移，将漂移分解为三个可测量的失效模式轴，并提供修复指导。

<details>
  <summary>Details</summary>

**Motivation:** 现有的相似性评分方法（如CKA、SVCCA）只能检测LLM变体的性能退化，无法定位具体的失效机制或提供修复建议。需要一种能直接关联表征漂移与风险的诊断工具。

**Method:** PRISM利用LLM的线性输出头和骨干网络的经验近等距结构，推导出目标模型与后训练变体之间交叉熵风险差距的闭合形式上界，并将漂移分解为尺度失配、形状失配和头部发散三个独立可测量的轴。

**Result:** 在两个模型家族和五个基准测试中，PRISM对变体排序的平均Spearman相关性达到0.820（量化）和0.831（LoRA遗忘），其轴指导的形状正则化器在缓解下游遗忘方面整体优于经验回放。

**Conclusion:** PRISM不仅能够有效检测和量化LLM变体的表征漂移，还能识别具体的失效模式并提供针对性的修复方向，同时可作为训练时的正则化器防止灾难性遗忘。
...
> Paraesthesia是一种基于情感风格的动态后门攻击方法，通过情感因素作为触发器，在LLM微调阶段植入后门，推理时遇到情感输入即可触发预设攻击响应。

<details>
  <summary>Details</summary>

**Motivation:** 现有后门攻击主要在词级层面操作，缺乏深层语义操控且隐蔽性不足；固定静态触发器容易被检测，且干净的微调会削弱触发器-目标关联。

**Method:** 基于因果验证发现情绪在LLM表示空间中可与语义解耦，提出寄生情感风格动态后门攻击。通过情感风格量化和重写技术，将含情感触发器的样本混入干净数据进行模型微调。

**Result:** 在指令 follow 生成和分类任务上的实验表明，Paraesthesia在四种不同模型上实现了约99%的攻击成功率，同时保持了模型的正常效用。

**Conclusion:** 情感因素作为动态触发器能够有效规避现有防御机制，相比静态词级触发器具有更高的隐蔽性和鲁棒性，为LLM安全研究提供了新的攻击视角。
...
> OmniThoughtVis是一个可扩展的多模态数据整理与蒸馏框架，通过生成高质量的结构化思维链数据，将大型教师模型的多模态推理能力有效转移给小型模型，显著提升了小规模MLLM的推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前多模态大语言模型虽然在推理任务上表现出色，但由于延迟和资源限制无法直接部署；而小型MLLM虽适合部署，但缺乏大规模高质量的多模态思维链监督数据，推理性能受限。

**Method:** 提出OmniThoughtVis数据整理与蒸馏管道，从开源种子池开始生成结构化CoT轨迹，联合标注推理难度、答案质量和语义任务标签，结合规则过滤、难度感知选择和标签多样性采样，构建180万样本的高质量语料库。

**Result:** 使用该框架蒸馏Qwen3-VL模型(2B-8B参数)，蒸馏后模型在9个多模态推理基准上性能持续提升，其中4B模型在MathVerse上提升16.8分，在MMMU-Pro上提升5.6分，甚至在多个任务上超越未蒸馏的8B基线模型。

**Conclusion:** OmniThoughtVis证明了可扩展推理蒸馏对于面向部署的多模态大语言模型的实用价值，为解决模型规模与部署效率之间的矛盾提供了有效方案。
...
> Macro是一个利用直接偏好优化(DPO)的多语言自生成反事实解释框架，通过复合评分函数构建偏好对，有效解决了现有方法在非英语语言中无法生成有效解释以及在有效性和最小性之间的权衡问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自生成反事实解释(SCEs)方法存在两个主要挑战：1）在非主导语言中难以生成有效的SCEs；2）有效性和最小性之间存在持续的权衡，影响解释质量。这限制了多语言大模型行为解释的应用。

**Method:** 提出Macro框架，采用直接偏好优化(DPO)方法，设计复合评分函数来构建偏好对，将有效性与最小性之间的权衡转化为可测量的偏好信号，从而优化多语言SCE的生成。

**Result:** 在4个大模型和7种不同类型语言上，Macro有效提升了12.55%的有效性，同时保持了最小性；相比翻译基线避免了严重的最小性破坏；相比监督微调在两个指标上都取得更优表现。

**Conclusion:** 偏好优化是增强多语言模型解释的有前途方向，Macro通过显式偏好优化成功平衡了有效性与最小性的权衡，提高了跨语言扰动一致性并减少了常见生成错误。
...
> 本研究基于日本中小学学术能力评估构建了一个多模态数据集，为评估多模态大模型在真实教育场景中的表现提供了新的基准。

<details>
  <summary>Details</summary>

**Motivation:** 真实学校考试为评估多模态大语言模型提供了高有效性的测试平台，但基于日本K-12教育评估的基准仍然稀缺，现有基准多基于合成或精选数据，无法真实反映教育场景。

**Method:** 从日本全国学术能力评估中构建多模态数据集，包含科学、数学和日语的中考题目，保留真实考试版式、图表和日本教育文本，并包含全国学生答题分布数据；使用精确匹配准确率和字符级F1分数对开放性回答进行评估。

**Result:** 研究发现不同学科间模型表现差异显著，对视觉推理需求高度敏感；人工评估和LLM作为评判者的分析进一步验证了自动评分的可靠性。

**Conclusion:** 该数据集为多模态教育推理建立了可重现的、基于人类标准的基准，支持未来在教育评估、反馈生成和可解释AI方面的研究。
...
> 该论文从表征几何角度分析了大模型遗忘机制的脆弱性，发现主导成分容易被逆向恢复，而次要成分更具抵抗性，并据此提出了针对次要成分的遗忘方法MCU，显著提升了抗再学习攻击能力。

<details>
  <summary>Details</summary>

**Motivation:** 大规模语言模型遗忘旨在消除预训练模型中的特定数据影响，以满足隐私、版权和安全需求，但现有方法存在严重缺陷：遗忘后的模型容易通过再学习攻击快速恢复被遗忘的知识，这对开放权重的模型构成重大安全威胁。

**Method:** 从表征几何角度出发，研究发现现有遗忘方法主要沿主导成分优化，而次要成分基本不变。通过理论分析解释了这一现象的光谱结构特征，基于此提出了Minor Component Unlearning (MCU)方法，专门针对表征中的次要成分进行遗忘操作。

**Result:** 在三个数据集上的广泛实验表明，MCU方法显著提高了对再学习攻击的抵抗能力，效果优于包括锐度感知最小化在内的最先进方法。

**Conclusion:** 该研究揭示了遗忘脆弱性的根本机制，通过利用次要成分的内在鲁棒性，为开发更安全可靠的模型遗忘技术提供了新的理论依据和实践方法。
...
> 本文揭示了在线策略蒸馏(OPD)效率的本质机制在于'预见性'，并提出基于此的EffOPD加速方法实现3倍训练加速。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究主要将OPD的效率归因于更密集稳定的监督，但其参数级机制缺乏深入理解，阻碍了更高效训练方法的设计。

**Method:** 通过分析OPD训练过程中的参数动态，发现其在模块分配层面和更新方向层面都表现出预见性，并基于此提出EffOPD方法，通过自适应选择外推步长沿当前更新方向加速训练。

**Result:** EffOPD无需额外可训练模块或复杂超参调优，在保持最终性能相当的情况下，平均实现3倍训练加速，验证了预见性机制的有效性。

**Conclusion:** 从参数动力学视角揭示了OPD效率的内在机制，为设计更高效的大语言模型后训练方法提供了理论依据和实践指导。
...
> 本文提出了一种训练-推理一致的分段级生成框架，通过在训练时采用与推理相同的分段前向执行语义，解决了现有高效长上下文方法中训练与推理不匹配的问题，在保持与全注意力相当性能的同时，显著降低了计算和内存开销。

<details>
  <summary>Details</summary>

**Motivation:** 基于Transformer的大语言模型在长上下文生成中面临严峻的可扩展性挑战。现有的推理高效方法通过在推理时采用有界上下文或分段级执行来提高效率，但在训练时仍使用全注意力机制，导致训练与推理之间的执行和状态转移语义不匹配。

**Method:** 提出了训练-推理一致的分段级生成框架。在训练期间，通过将梯度传播限制在来自紧邻前一段的KV状态来强制与推理的语义一致性，同时在前向传播中允许每头特定的过去KV状态访问而不参与梯度传播。

**Result:** 在长上下文基准测试中，该方法实现了与全注意力机制相当的性能，同时与强大的推理高效基线相比具有竞争力的延迟-内存权衡，并在极长上下文长度下显著提高了可扩展性（如在128K上下文中相比带FlashAttention的全注意力降低约6倍的峰值预填充内存）。

**Conclusion:** 通过统一训练和推理的执行语义，本文方法有效解决了现有长上下文优化技术中的训练-推理不匹配问题，为实际应用中的长上下文处理提供了高效且性能稳定的解决方案。
...
> 研究提出了安全导向、后果感知的评估框架来评估LLM在空中交通管制中的可靠性，发现尽管常规指标表现良好，但实际运行可靠性严重受限。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM评估方法无法反映空中交通管制领域中高风险语义错误的非对称后果，需要开发专门的安全评估框架来确保AI辅助系统的可靠部署。

**Method:** 构建针对ATC操作的后果感知评估框架，重点关注高风险语义错误（如错误的跑道标识或移动限制），超越传统的F1或宏准确率等聚合指标。

**Result:** LLM在清洁转录本上最高风险评分仅0.69，多数模型低于0.6，尽管宏F1表现较高。错误集中在高影响实体上，动作类型分类相对稳定，表明存在结构性基础缺陷。

**Conclusion:** 研究结果强调了后果感知评估协议对负责任部署AI辅助ATC系统的必要性，揭示了当前LLM在高风险操作环境中的可靠性局限性。
...
> MedTPE是一种新颖的医疗标记对编码方法，通过合并频繁共同出现的医疗标记对，实现了对长序列电子健康记录的31%无损压缩，同时降低推理延迟34-63%且保持或提升预测性能。

<details>
  <summary>Details</summary>

**Motivation:** 电子健康记录作为自然语言序列时往往过长，导致计算成本高昂且性能下降。现有方案要么增加压缩模块引入额外延迟，要么删除标记丢失临床信息，亟需一种无需额外成本且无损的压缩方法。

**Method:** 提出分层的医疗标记对编码方法MedTPE，通过依赖感知替换策略将频繁共现的医疗标记对合并为复合标记，实现无损压缩；仅需微调新增标记的0.5-1.0%参数，通过自监督学习训练。

**Result:** 在真实临床数据集上，MedTPE减少输入标记长度31%，降低推理延迟34-63%，在多种LLM和四个临床预测任务上保持或提升预测性能及输出格式合规性，且在不同输入长度、多领域（科学、金融）和多语言中表现稳健。

**Conclusion:** MedTPE成功解决了长序列EHR处理的效率和性能问题，为临床预测任务提供了一种高效、无损且通用的解决方案，具有良好的跨领域和跨语言应用潜力。
...
> 这篇论文提出了一种增强的多词表达式分类方法，通过评估不同特征的可靠性来提高分类效果，并考虑多语言适用性。

<details>
  <summary>Details</summary>

**Motivation:** 多词表达式(MWEs)是一个异构的集合，迫切需要有效的分类系统。现有的特征不是同等可靠的，这影响了分类的准确性和计算实用性。

**Method:** 作者首先分析了多个可用于MWE分类的a priori特征，评估它们作为分类依据的可靠性差异，然后结合多语言的研究成果，设计出一个增强的分类框架。

**Result:** 提出了一个更加适合多语言环境的增强分类体系，该体系基于特征可靠性评估，能够为MWE提供更有效的计算应用。

**Conclusion:** 通过选择和优化分类特征，并结合多语言研究经验，新提出的分类方法在计算应用方面更加实用，对处理异构的MWEs具有更好的效果。
...
> 该研究通过两种校准微调方法（软目标和硬目标）提升了语言模型满足用户指定随机性约束的能力，实验证明概率校准是一种可训练的能力。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型在需要满足用户指定随机性约束的场景中使用时，其生成概率往往与目标分布校准不佳，本研究探索是否可以通过微调直接改进这一能力。

**Method:** 在需要从数学分布中采样的合成提示上微调语言模型，提出两种方法：软目标方法将所需输出分布转换为trie派生的下一个标记目标，硬目标方法则在来自目标分布的采样补全上进行训练。

**Result:** 两种方法在保持分布系列和未见参数设置的结构化采样保真度上都有显著提升，硬目标方法在结构化数值采样上表现最强，软目标方法在更广泛的随机生成基准上表现更好，但会带来一定的下游能力损失。

**Conclusion:** 概率校准可以通过微调得到改善，硬目标配置有利于精确数值保真度，软目标配置有利于更广泛的随机迁移，展示了校准能力在不同任务间的权衡关系。
...
> 本文提出TABOM框架，通过自蒸馏轨迹对齐优化来提升扩散语言模型的后训练效果，实现了显著的知识获取和性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于NELBO的扩散语言模型监督微调存在训练与推理不匹配的问题：训练采用随机掩码单步重构，而推理是渐进式的多步骤去噪轨迹，导致训练效率低下和知识获取受限。

**Method:** 提出TABOM框架，将推理去噪偏好建模为基于预测熵的玻尔兹曼分布，推导配对排序目标函数来对齐模型确定性与解码轨迹，使训练与推理的由易到难结构保持一致。

**Result:** TABOM在新领域取得显著性能提升，扩展了DLM的有效知识边界，相比标准SFT大幅缓解了灾难性遗忘问题。

**Conclusion:** 证明了自蒸馏轨迹不仅能用于推理加速，更可用于真正的知识获取， trajectories-aligned优化为扩散语言模型的后训练提供了有效新思路。
...
> 该研究提出了一种基于局部语法对比分析来优化命名实体识别的方法，成功将葡萄牙语人名识别的F值提升了6个百分点，达到76.86%。

<details>
  <summary>Details</summary>

**Motivation:** 人名识别在信息抽取中具有重要性但实现困难，葡萄牙语的人名识别效果有待提升，需要找到有效的方法来优化现有的语法规则。

**Method:** 通过开发对比工具分析两种局部语法的一致性差异，观察语法间的包含、交集和析取关系，利用这些关系指导选择和组合最优的语法规则集。

**Result:** 在HAREM黄金标准数据集上测试，优化后的语法使F值达到76.86%，相比当时葡萄牙语人名识别的先进水平提升了6个百分点。

**Conclusion:** 基于局部语法对比分析的方法能够有效优化命名实体识别性能，为语法规则的改进提供了系统性的指导思路，证明了该方法在实际应用中的可行性。
...
> Qwen-Scope是一个基于Qwen模型系列的开源稀疏自编码器套件，不仅用于模型分析，还可作为推理控制、评估分析、数据处理和后训练优化的实用接口。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型的内部决策过程仍不透明，限制了模型的检查、控制和改进。稀疏自编码器作为有前景的工具，能够将模型激活分解为可解释的特征表示。

**Method:** 构建了包含14组SAEs的开源套件，覆盖Qwen3和Qwen3.5系列的7种模型变体，包括密集和专家混合架构。通过SAE特征方向实现推理控制，利用激活特征进行评估分析，支持多语言毒性分类和安全数据合成，并将SAE信号融入到微调和强化学习中。

**Result:** 成功展示了SAEs在四个方向的应用：推理时控制语言和偏好、评估基准冗余和能力覆盖、数据分类和合成、以及后训练优化中的行为修正。

**Conclusion:** SAEs不仅可作为事后分析工具，还能作为可重用的表示级接口，用于诊断、控制、评估和改进大语言模型，通过开源Qwen-Scope促进了机理研究和实际工作流程的发展。
...
> YFPO是一种将模型内部神经元激活信号与传统偏好优化相结合的数学推理增强框架，通过识别数学相关神经元构建辅助奖励来提升推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有偏好优化方法主要依赖外部构建的偏好数据进行样本级监督，但很少利用模型内部表示中包含的能力相关信息。对于数学推理，某些神经元群可能表现出与数学知识、符号操作或逻辑推理相关的激活模式，这些内部激活可以为模型是否使用数学相关能力提供粗略指示。

**Method:** YFPO框架首先使用AttnLRP技术识别数学相关神经元，然后基于偏好和厌恶响应之间这些神经元的激活差值构建辅助奖励，将内部神经元级信号与外部偏好学习相结合来优化模型。

**Result:** 在小型语言模型和GSM8K基准上的初步实验表明，神经元级信号可以与偏好优化相互作用，偶尔能够提升推理性能，为更细粒度和可解释的推理导向后训练提供了有前景的方向。

**Conclusion:** YFPO证明了利用模型内部神经元激活信号增强偏好优化的可行性，为数学推理等复杂任务的后训练提供了新的思路，有望实现更精细和可解释的模型优化。
...
> 本研究提出了一种目标引导的主动对话系统，通过联合建模用户画像和领域知识作为会话场景，并使用意图关键词桥接技术来动态预测后续对话的意图关键词，显著提升了系统的主动性和流畅性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的目标引导主动对话系统大多忽略了动态建模对话场景和意图关键词，导致与真实对话的动态特性不匹配，无法有效引导对话向预定目标发展。

**Method:** 通过联合建模用户画像和领域知识构建对话场景模型，引入场景偏差动态影响系统话语，同时采用意图关键词桥接技术预测未来对话轮次的意图关键词，提供更高层次和更灵活的引导。

**Result:** 自动评估和人工评估结果表明，所提出的方法在主动性、流畅性和信息量方面都有显著改进，有效缩小了与真实世界对话的差距。

**Conclusion:** 会话场景建模和意图关键词桥接技术对提升目标引导主动对话系统性能具有重要作用，为构建更贴近真实对话场景的智能系统提供了有效解决方案。
...
> 这篇论文提出了RuDE框架，通过基于评分标准的判别式评估方法，能够预测基础模型经过后训练后可能达到的性能，为高效模型选择提供了新的解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 传统基准测试如MMLU无法有效反映基础模型在复杂开放场景中的可塑性，导致模型选择效率低下。研究团队需要找到一种方法来预测模型经过后训练后的潜在性能表现。

**Method:** 提出RuDE（基于评分标准的判别式评估）框架，利用响应判别绕过基础模型的生成差距。通过4C分类学系统指导，构建跨多样化领域的精细化评分标准违规对比样本对。

**Result:** 实验证明RuDE与后训练性能的相关性超过90%。通过强化学习验证，确认RuDE能有效识别高潜力的小型模型，这些模型在性能上超过了更大的同类模型。

**Conclusion:** RuDE为基础模型开发提供了计算高效的机制，能够在训练前准确识别具有高潜力的模型，显著提升了模型选择的效率和准确性。
...
> 该研究通过电影字幕翻译案例发现，视觉语境信息对改善低资源印度语种翻译质量至关重要，其中基于属性的视觉概要方法更为有效。

<details>
  <summary>Details</summary>

**Motivation:** 纯文本翻译系统在电影字幕翻译中缺乏视觉线索，无法有效传达情感、动作和社会细微差别，特别是对于英语到印度语等低资源语言对。

**Method:** 在五部完整电影上进行案例研究，比较两种轻量级视觉锚定策略：5分钟滑动窗口的结构化属性概要和字幕间视觉间隙的自由文本概要。

**Result:** 时间错位是长视频中的主要障碍，但有选择性的视觉锚定（仅替换20-30%最低质量片段）能显著改善COMET分数，基于属性的视觉概要方法更加稳健。

**Conclusion:** 针对长视频翻译，选择性视觉锚定和属性驱动的视觉概要是提升翻译质量的有效策略，在处理视觉与文本时序对齐方面具有实际应用价值。
...
> 本文提出了ActGuide-RL方法，利用人类交互产生的动作数据作为引导，帮助大语言模型代理突破奖励状态可达性障碍，显著提升智能体强化学习效果，减少对监督微调数据的依赖。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型的代理强化学习严重依赖基础策略的探索能力，当基础策略无法达到奖励状态时，训练信号会缺失。传统的解决方法需要昂贵的迭代监督微调或外部引导。

**Method:** 提出ActGuide-RL框架，将人类交互产生的动作数据作为计划式参考引导注入，使代理策略突破奖励状态可达性障碍；通过混合策略训练联合优化引导和非引导轨迹，采用最小干预原则仅在必要时提供自适应引导。

**Result:** 在搜索代理基准测试上，ActGuide-RL显著超越零样本基线（GAIA上提升10.7个百分点，XBench上提升19个百分点），可与SFT+RL流程相媲美，且无需冷启动。

**Conclusion:** ActGuide-RL展示了通过可扩展的动作引导替代繁重监督微调数据的新范式，为代理强化学习提供了更高效的数据利用方式，降低了对监督微调的依赖。
...
> SAGE是一种可扩展的自动化框架，通过微调小模型来生成高质量的知识评估基准鲁棒性增强数据集，成本低且质量媲美人工标注。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在标准知识评估基准上表现良好，但在测试相同知识的变体问题时表现脆弱。现有的LLM辅助生成-验证流程成本高昂且难以扩展，主要由于变体生成效率低和验证不可靠。

**Method:** SAGE框架包含两个组件：1) VariantQual，基于人类标注的种子数据训练的质量验证器；2) VariantGen，通过监督微调初始化，再使用VariantQual作为奖励模型进行强化学习优化的变体生成器。整个流程使用微调的小模型而非大型LLM。

**Result:** 在HellaSwag上的实验表明，SAGE能以远低成本构建大规模鲁棒性增强基准，质量与人工标注的HellaSwag-Pro相当。微调模型还能泛化到MMLU数据集，无需基准特定的微调。

**Conclusion:** SAGE为知识评估基准的鲁棒性增强提供了一种可扩展、低成本的解决方案，通过使用小模型微调和强化学习训练，实现了高质量变体数据集的自动化生成，具有良好的泛化能力。
...
> 该系统为SemEval-2026任务8设计，采用三阶段管道（查询重写→混合检索→重排序），在38个系统中排名第8，nDCG@5达0.531，超过基线10.7%。

<details>
  <summary>Details</summary>

**Motivation:** 解决上下文依赖的跟进问题检索挑战，提高多领域英文检索任务的性能，特别是将依赖对话历史的问题转换为独立查询。

**Method:** 三阶段管道：1）使用LoRA微调的Qwen 2.5 7B模型进行查询重写；2）BM25和稠密检索混合通过Reciprocal Rank Fusion；3）BGE-reranker-v2-m3交叉编码器重排序。

**Result:** 在官方测试集上达到nDCG@5为0.531，在38个系统中排名第8位，比组织者基线高出10.7%。域特定温度调优带来一致提升。

**Conclusion:** 域特定的温度调优是有效的改进策略，技术领域受益于确定性解码，一般领域受益于受控随机性。更复杂的策略如域感知提示和多查询扩展反而降低了性能。
...
> SKILLGRAPH框架将技能表示为有向图中的节点，通过类型化边编码技能间关系，实现技能组合检索和动态更新，在复杂任务上表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 现有技能库将技能作为孤立条目存储，仅通过语义相似性检索，导致无法识别技能间的依赖关系和组合方式，同时使库维护困难。这阻碍了大语言模型智能体在组合性任务上的表现。

**Method:** 提出SKILLGRAPH框架，将可重用技能表示为有向图节点，通过类型化边编码前提、增强和共存关系。面对新任务时检索有序技能子图指导多步决策。图表从智能体轨迹和强化学习反馈中持续更新，实现技能库和策略的共同改进。

**Result:** 在ALFWorld、WebShop和七项搜索增强QA任务上的实验表明，SKILLGRAPH相比记忆增强RL方法达到最先进性能，在需要组合多个技能的复杂任务上提升尤其显著。

**Conclusion:** SKILLGRAPH通过结构化技能表示和基于图的检索方法，有效解决了技能组合和维护的挑战，为大语言模型智能体在复杂任务上的应用提供了新思路。
...
> 研究表明，儿童导向语言在动词习得方面的优势可能源于口语语域的普遍特性，而非其独特的优化设计。

<details>
  <summary>Details</summary>

**Motivation:** 探究儿童导向语言是否为语言学习而优化，以及它促进哪些语言发展方面。

**Method:** 使用神经语言模型，对比训练在儿童导向语言和成人导向语言上的表现，通过选择性移除语法或词汇共现信息，评估动词意义习得的影响。

**Result:** 语法破坏对所有数据集的学习都有损害，但儿童导向语言和口语成人导向语言模型的韧性显著高于书面输入模型。观察到语义优先的学习轨迹，动词意义的出现早于语法熟练度，这种异步性在口语领域尤其明显。

**Conclusion:** 之前归因于儿童导向语言的动词学习优势，实际上可能反映了口语语域的更广泛特性，而非儿童导向语言特有的优化。
...
> 研究发现当前大型语言模型中不存在统一的语法违规检测机制，虽然某些现象显示出选择性因果结构的部分证据，但整体上不支持LLMs拥有统一的语法违规检测器假说。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在语言任务上表现优异，但产生这些预测的内部机制尚不明确。研究者旨在验证LLMs是否在参数中编码了语言约束违规的表征，并在处理不合语法句子时选择性地激活这些表征。

**Method:** 使用稀疏自编码器将多义激活分解为稀疏、单义的特征，提取潜在的违规相关特征；引入敏感性评分识别在违规输入上比合规范输入更优先激活的特征；提出了包含三个评估标准的联合证伪框架。

**Result:** 结果在两方面都是负面的：证伪标准无法在语言现象中联合满足，且没有特征在所有类别中 consistently 共享。虽然某些现象显示出选择性因果结构的部分证据，但整体模式对当前LLMs中存在统一的语法违规检测器集合提供了有限支持。

**Conclusion:** 研究结果表明当前的大型语言模型缺乏统一的语法违规检测机制，这为理解LLMs的语言处理能力提供了重要见解，也指出了未来研究中需要探索的方向。
...
> 本文系统性地回顾了低资源手语识别与翻译研究，以阿塞拜疆手语为例，提出了从架构中心转向数据中心、从说话者独立转向说话者自适应、从参考基准转向任务 specific 评估范式的三个范式转变，并制定了技术路线图。

<details>
  <summary>Details</summary>

**Motivation:** 全球300多种手语因文档有限、数据集稀疏、计算工具不足而严重低资源化，迫切需要系统性的解决方案来改善聋人群体的沟通状况。

**Method:** 通过系统性文献分析，提取八个经验教训；以突厥语系手语为重点，利用语言相似性实现跨语言迁移学习；采用MediaPipe轻量化架构、社区验证标注和离线优先部署策略。

**Result:** 提出了三个关键范式转变和针对阿塞拜疆手语的技术路线图，强调了以聋人社区为中心的跨学科合作对确保文化真实性和实际效益的重要性。

**Conclusion:** 构建实用且符合文化伦理的低资源手语系统需要持续的跨学科协作，技术创新必须与社区需求紧密结合，才能真正为聋人群体带来沟通便利。
...
> 文学性越狱通过累积的风格不规则性改变提示词处理方式，绕过安全机制，而非依赖单一修辞手法。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在安全关键应用中部署，但研究发现即使经过后训练安全对齐，诗歌等文学性转换仍能有效绕过安全防护，这一现象的根本原因尚不明确。

**Method:** 通过注意力模式的可解释性分析，进行输入级消融研究评估单个及组合修辞手法的贡献，构建注意力图谱的可解释向量表示，进行聚类分析并训练线性探针预测安全结果和文学格式。

**Result:** 模型能高精度识别诗歌与散文格式，但在同一格式内难以预测越狱成功；聚类显示文学格式明确分离，但安全标签无明显分离。文学性越狱并非由于格式识别失败，而是由风格不规则性累积引起的处理模式改变。

**Conclusion:** 文学性越狱通过风格不规则性的累积改变提示词处理，避开后训练中考虑的词汇触发器。需要建立考虑风格引起的行为偏移的安全机制来增强模型鲁棒性。
...
> 本文提出LCV方法，通过显式重构缺失事实来进行遗漏相关的不实信息检测，在英文和中文数据集上分别提升了2.56和2.84个macro-F1点。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法在处理局部连贯但通过遗漏关键信息造成误导的文章时效果不佳，当前方法要么只是附加上下文作为辅助证据，要么预测分类的遗漏信号，未能明确识别具体缺失的事实。

**Method:** LCV检索时间对齐的上下文文章，使用冻结的指令调优大语言模型为每个句子-文章对生成缺失的上下文描述，并将结果文本作为跨源关系输入到包含目标句子和上下文文章的异构图进行推理。

**Result:** 在Sheng等人的双语基准测试中，LCV相比最强的遗漏感知基线，英文和中文数据集分别提升2.56和2.84个macro-F1点。

**Conclusion:** 建模缺失的跨源事实本身，而非仅仅附加检索证据或预测遗漏信号，是遗漏感知不实信息检测的有效表示方法。
...
> 该研究提出了一个三层级贝叶斯管道来解决大语言模型生产部署中的选择偏差问题，能够在不需要个体交互真实标签的情况下，将用户反馈平均误差从40-50个百分点降低到4-13个百分点。

<details>
  <summary>Details</summary>

**Motivation:** 生产环境中的大语言模型部署接收到的用户反馈存在严重的选择偏差—— thumbs-up/down 主要来自满意度分布的尾部，对这些反馈的简单平均会导致对系统真实质量的评估偏差高达40-50个百分点。

**Method:** 提出了一个三层级贝叶斯框架：1) 主题聚类代理使用UMAP+HDBSCAN对文本嵌入进行聚类；2) 偏差建模代理使用两层级Beta-Binomial模型拟合每个主题的选择率sc和质量qc；3) 综合代理根据真实主题 prevalences 重新加权qc，输出偏差校正的聚合后验分布。

**Result:** 在UltraFeedback数据集上的验证显示，引入反馈渠道先验信息的方法能在偏差比例从1:1到30:1变化时，将误差控制在4-13个百分点范围内，95%置信区间覆盖真实质量值。没有渠道先验的方法误差仍高达22-33个百分点。

**Conclusion:** 选择偏差的矫正关键在于引入反馈渠道的先验信息（而非潜在质量先验），这打破了模型拟合的退化性。提出的框架能够有效处理主题和情感分层的选择偏差，为生产环境LLM系统提供可靠的质量评估和在线校准信号。
...
> DCRD是一种两阶段解码方法，通过分析注意力图预测知识冲突，并动态选择贪婪解码或基于上下文保真度的解码，在处理知识冲突的同时保持冲突无关情况下的效率。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在预训练中积累大量参数化知识，但当过时或错误的参数化知识与上下文中的外部知识发生冲突时，现有方法要么在无冲突场景下破坏输出分布，要么在复杂现实情况中冲突检测准确率不足。

**Method:** 提出动态认知和解码(DCRD)两阶段方法：首先通过分析注意力图评估上下文保真度来预测潜在冲突；然后基于预测结果选择贪婪解码路径或基于上下文保真度的动态解码路径。

**Result:** 在四个LLM和六个QA数据集上的实验表明，DCRD超越了所有基线方法，达到了最先进的性能。同时构建了模拟频繁知识更新场景的ConflictKG冲突知识QA基准。

**Conclusion:** DCRD成功实现了知识冲突的高效处理，同时在无冲突情况下保持了高准确性和解码效率，为解决大模型知识冲突问题提供了有效方案。
...
> 本研究首次将稀疏自编码器(SAE)应用于Whisper自动语音识别模型，揭示了模型中多样化的单语义特征，证明了该技术在音频模型解释方面的可行性。

<details>
  <summary>Details</summary>

**Motivation:** 虽然基于Transformer的NLP模型广泛应用，但其内部机制仍不透明。现有SAE技术已成功用于解释文本大语言模型，但在音频处理领域如自动语音识别(ASR)模型上缺乏相应研究，这阻碍了我们对音频模型内部工作机制的理解。

**Method:** 作者将稀疏自编码器应用于Whisper这一基于Transformer的ASR模型，在Whisper编码器提取的帧级嵌入上训练高维稀疏潜空间，通过这种投影方式揭示模型的内部表示。

**Result:** 研究发现了跨语言和非语言边界的多样化单语义特征，并展示了跨语言特征引导能力，证实了Whisper编码了丰富的语言信息。

**Conclusion:** 这项工作确立了SAE模型在音频模型解释中的可行性，证明了Whisper内部蕴含丰富的语言信息，为理解ASR等音频处理模型的内部机制开辟了新的研究路径。
...
> 本文提出dGRPO方法，结合群体相对策略优化(GRPO)和在线策略蒸馏(OPD)来解决长上下文语言模型训练问题，并通过合成数据集LongBlocks验证了方法的有效性。

<details>
  <summary>Details</summary>

**Motivation:** 现有长上下文适应方法存在局限：离线方法(SFT、KD)存在暴露偏差；在线强化学习(GRPO)不稳定且样本效率低；在线蒸馏(OPD)无法直接优化任意奖励信号。

**Method:** 提出蒸馏群体相对策略优化(dGRPO)：将GRPO与教师模型的密集指导结合；引入LongBlocks合成数据集，涵盖多跳推理、上下文基础和长文本生成任务。

**Result:** 实验结果表明dGRPO相比现有方法更稳定有效，在长上下文推理任务上表现提升，同时保持了短文本能力。

**Conclusion:** 将基于结果的策略优化与知识蒸馏结合在单目标中，为长上下文推理提供了更稳定有效的解决方案，突破了现有方法的限制。
...
> 本文提出了一种多语言文本修正流水线，结合序列标注器标记停顿填充词，通过指令微调大语言模型进行文本重写，并引入对比学习提高修正可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 自动语音识别转录文本中常包含填充词、重复和错误开始等不流利现象，影响文本可读性和下游应用性能。现有方法主要关注识别和删除不流利标记，但会破坏语法结构和语义连贯性。

**Method:** 构建多语言修正流水线：1）序列标注器标记不流利标记；2）基于标记信号指导大语言模型的指令微调；3）添加对比学习目标惩罚不流利标记的重现，保持语法和语义。

**Result:** 在印地语、孟加拉语和马拉地语三种印度语言的实验中，该方法持续优于强基线模型，包括多语言序列到序列模型。

**Conclusion:** 仅依赖检测策略不足以处理不流利问题。结合标记级信号与指令微调和对比学习为语音驱动的NLP系统中的多语言不流利修正提供了实用且可扩展的解决方案。
...
> 这项研究提出了PreScam基准数据集，用于检测和理解现实世界中对话式诈骗的进展过程，发现现有模型在追踪诈骗风险升级和操控手段方面仍存在显著缺陷。

<details>
  <summary>Details</summary>

**Motivation:** 对话式诈骗如恋爱和投资诈骗正在成为主要的在线欺诈形式，与一次性诈骗不同，它们通过多轮对话展开，诈骗者使用不断演变的心理技巧逐步操控受害者。现有研究主要关注静态诈骗检测或合成诈骗，无法理解真实世界诈骗如何随时间发展。

**Method:** 构建PreScam基准数据集，从177,989份原始诈骗报告中筛选并结构化出11,573个对话式诈骗实例，涵盖20个诈骗类别。根据 propose 的诈骗生命周期 kill chain 进行层次化结构化，并在对话轮次层面标注诈骗者心理行为和受害者反应。设计了两个评测任务：实时终止预测和诈骗者行为预测。

**Result:** 监督编码器在实时终止预测任务上明显优于零样本LLM，而即使是强大的LLM在下一步行为预测上也仅取得中等成功。这表明模型虽然能捕捉到某些诈骗相关线索，但在追踪风险升级和操控展开方面仍存在困难。

**Conclusion:** 当前模型在表面语言流畅性和诈骗进展建模之间存在明显差距，需要进一步研究来提高对诈骗过程动态变化的理解能力。
...
> PRISM是一个无需训练的检索框架，将长期内存管理视为图结构内存上的联合检索与压缩问题，通过分层路径搜索、查询敏感边代价、证据压缩和自适应意图路由四个组件，在严格上下文预算下实现高精度记忆检索。

<details>
  <summary>Details</summary>

**Motivation:** 长期语言代理会快速积累对话历史，超出现有固定上下文窗口的处理能力，传统的内存管理方法存在上下文窗口扩展不足、提取成本高昂、启发式图遍历导致准确性和效率低下等问题。

**Method:** PRISM框架包含四个正交的推理时组件：1) 分层束搜索在类型化关系路径上进行；2) 查询敏感边代价使遍历与查询意图对齐；3) 证据压缩将候选束压缩为紧凑的答案侧上下文；4) 自适应意图路由让大多数查询通过零LLM层处理。

**Result:** 在LoCoMo基准测试上，PRISM比所有相同协议的基线方法实现了显著更高的LLM判断准确率，且上下文预算小一个数量级，在准确率-上下文-成本边界上占据了新的最优位置。

**Conclusion:** PRISM通过将检索表述为类型化路径模板上的最小代价选择，并配以LLM侧压缩步骤，在无需微调或修改上游提取管道的情况下，在严格上下文预算内提供了正确的证据，实现了答案质量和检索效率的卓越平衡。
...
> 研究者建立计算模型预测西班牙语、德语和汉语母语者学习英语词汇的难度，发现词频熟悉度是主要共同因素，而正字法传递对西、德学习者重要，对汉语学习者则无效。

<details>
  <summary>Details</summary>

**Motivation:** 探究词汇学习难度的决定因素，特别是这些因素如何因学习者的母语背景（西班牙语、德语或汉语）而异，为设计个性化词汇教学提供依据。

**Method:** 使用梯度提升模型，基于词的熟悉度（如词频）、意义、表面形式和跨语言传递等特征，训练预测英语词汇难度的模型，并应用Shapley值分析各特征组的重要性。

**Result:** 词的熟悉度是所有三种母语背景学习者的主导特征；西语和德语母语者额外依赖正字法传递特征；中文母语者则仅依靠熟悉度和表面特征组合。

**Conclusion:** 该研究提供了可解释的、针对母语背景定制的词汇难度预测模型，可用于优化不同母语背景学习者的英语词汇课程设计。
...
> TBPO是一种新的基于Bregman散度的令牌级偏好优化方法，仅使用序列级偏好数据就能实现令牌级的最优性，相比DPO和其他方法提高了对齐质量和训练稳定性。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法如DPO虽然无需强化学习就能对齐语言模型，但它在序列层面建模偏好，而实际生成是由令牌级决策驱动的。令牌级扩展方法通常分解序列级Bradley-Terry目标，未能显式建模每个前缀状态的最优性。

**Method:** 提出TBPO，建立条件前缀的令牌级Bradley-Terry偏好模型，推导Bregman散度密度比匹配目标，推广了logistic/DPO损失。包含两种实现：TBPO-Q学习轻量级状态基线，TBPO-A通过优势归一化移除基线。

**Result:** 在指令跟随、有用性/无害性和摘要基准测试中，TBPO相比强基线方法提升了对齐质量、训练稳定性和输出多样性。

**Conclusion:** TBPO成功利用标准序列级偏好数据实现令牌级最优性，结合了令牌级建模的优势和DPO的简洁性，为语言模型对齐提供了更有效的方法。
...
> 该研究通过构建GKnow基准测试，揭示了语言模型中性别偏见和事实性别知识在神经元层面的严重纠缠，表明神经元消融方法不可靠，并指出现有性别偏见评估基准可能掩盖消融导致的事实性别知识下降问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究在分析神经网络性别预测机制时存在两个问题：(1)过于专注于特定性别相关任务；(2)无法区分事实性别输出和基于刻板印象的性别偏见输出。需要更全面的基准测试来评估语言模型的性别知识和偏见。

**Method:** 研究人员构建了GKnow基准测试，用于评估语言模型在不同类型性别相关预测中的性别知识和性别偏见。通过该基准识别和分析负责性别预测的回路和单个神经元，并测试神经元消融对多个基准（DiFair、GKnow测试集、StereoSet）的影响。

**Result:** 研究发现性别偏见和事实性别在回路和神经元层面上都严重纠缠，导致神经元消融是一种不可靠的去偏见方法。此外，现有性别偏见评估基准会掩盖消融操作带来的事实性别知识下降。

**Conclusion:** GKnow基准的构建为开发更稳健的性别偏见评估基准做出了贡献，研究结果表明需要更精细的方法来区分和处理语言模型中的性别知识与性别偏见问题。
...
> BioCreative IX MedHopQA是一个评估大语言模型生物医学多跳问答能力的挑战赛，通过包含1000个QA对的新数据集发现，检索增强生成(RAG)策略是提升模型性能的关键。

<details>
  <summary>Details</summary>

**Motivation:** 生物医学领域的多跳问答回答需要整合多个来源的信息，这对大语言模型构成重大挑战。为了基准测试LLM的这种多跳推理能力，特别是针对罕见疾病的复杂问答，组织了这项挑战赛。

**Method:** 构建了一个包含1000个困难QA对的新数据集，涉及疾病、基因和化学物质领域，每个问题都需要通过整合两个不同的Wikipedia页面进行两跳推理。使用表面字符串比较和概念准确性(MedCPT分数)两种方式进行评估。

**Result:** 48个提交中，表现最好的系统在MedCPT指标上达到89.30% F1分数，完全匹配(EM)分数87.30%，而零样本基线分别只有67.40%和60.20%。研究发现检索增强生成(RAG)及相关检索策略对获得强性能至关重要。

**Conclusion:** 该挑战揭示了多跳生物医学问答中检索增强方法的重要性，概念级评估能够更好地评估形式不同但语义正确的答案。MedHopQA数据集已公开发布，将推动这一重要领域的持续发展。
...
> ISEC是一种综合指数，通过整合语义距离、形态转换成本和频率等维度，能够有效预测和预防中小企业数据录入中的分类错误，相比传统方法提升195倍性能。

<details>
  <summary>Details</summary>

**Motivation:** 中小企业数据录入系统在处理语义或形态相近的类别名称时容易产生不可恢复的分类错误，现有标准化工具依赖标准词典，无法有效处理包含自定义SKU、缩写和专业术语的SME主数据。

**Method:** 提出ISEC（分类错误敏感性指数），该指数结合词向量语义距离、加权Damerau-Levenshtein形态转换成本和经验频率三个维度，并使用向量数据库架构优化计算复杂度。

**Result:** 在政府司法记录、零售库存和合成ISO编码金属加工目录三个异构数据集上验证，ISEC相比暴力方法实现约195倍性能提升，能有效识别类别混淆风险。

**Conclusion:** ISEC为中小企业提供了一个可扩展的主动数据治理工具，能够检测分类数据中潜在的结构性风险，帮助防止分类错误向关键绩效指标的传播。
...
> 本文研究了参数高效微调(PEFT)的多任务通用化方法，发现对PEFT模块输出进行求和的组合方法在多个文本生成控制任务中表现最佳。

<details>
  <summary>Details</summary>

**Motivation:** 传统的PEFT技术虽然降低了微调成本，但每个新任务都需要单独进行微调，这限制了其实用性和效率。

**Method:** 在三个LLM模型上使用QLoRA PEFT技术，探索三种通用化方法：(1)多数据集组合训练 (2)推理时组合PEFT模块权重矩阵 (3)推理时组合PEFT模块输出，在情感控制、主题控制和多属性控制数据集上进行测试。

**Result:** 对PEFT模块输出求和的组合方法表现最为出色，在所有测试中都优于或匹配其他方法。即使在单任务测试中，三模块输出组合也比单任务专门模块平均提升2个百分点。

**Conclusion:** PEFT模块输出求和是一种有效且通用的多任务组合方法，能够实现任务间知识的良好迁移，提升了PEFT的实用价值和扩展性。
...
> MedHopQA 是一个疾病中心的多跳推理基准，包含1000个专家策划的开源问答对，旨在评估生物医学领域中大型语言模型的推理能力而非模式匹配。

<details>
  <summary>Details</summary>

**Motivation:** 现有生物医学问答基准存在局限性：多选题格式允许通过排除法而非推理答对，考试风格数据集面临性能饱和和训练数据污染风险，且缺乏对临床多跳推理能力的评估。

**Method:** 构建了包含1000个评分问题的MedHopQA基准，每个问题需要综合两个维基百科文章的信息，采用开放式文本答案格式，并提供基于本体的同义词集支持词汇和概念层面的评估。为防止作弊，将1000个评分问题嵌入10,000题的公共数据集中。

**Result:** 成功创建了BioCreative IX共享任务的MedHopQA基准，通过结合人工标注、分诊、迭代验证和LLM作为评判者的结构化流程，提供了具有抗饱和性和抗污染性的评估框架。

**Conclusion:** MedHopQA不仅是一个评估基准，更是构建未来生物医学问答数据集的可重用框架，强调了组合推理、抗饱和性和抗污染性作为核心设计约束。
...
> 研究通过评估不同收敛度句子构建的段落对大语言模型推理问答性能的影响，发现高收敛度句子能显著提升模型准确率，且按收敛度降序排列效果更佳。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在开放域问答中广泛应用，但其在需要推理而非直接检索答案的问题上的能力尚未充分探索。本研究旨在探索段落结构和质量如何影响LLM处理推理问题的性能。

**Method:** 研究者提出使用收敛度（句子有效排除错误答案的能力）作为构建段落的准则，通过组合不同收敛度水平的句子形成段落，在TriviaHG数据集子集上评估了6个不同规模和架构的LLM。

**Result:** 由高收敛度句子构建的段落比余弦相似度选择的段落获得显著更好的答案准确率；按收敛度降序排列句子能略微提升性能，表明LLM倾向于优先处理早期信息密集的提示。

**Conclusion:** 收敛度是指导段落构建和分析LLM推理行为的有效信号，为优化LLM在推理问答任务中的表现提供了实用指导。
...
> 通过分析OLMo模型和其完整训练数据Dolma，研究发现LLM的流行度偏好主要由训练数据曝光度决定，而非现实世界流行度，曝光度在较长尾部实体中表现尤为显著。

<details>
  <summary>Details</summary>

**Motivation:** LLM对知名实体存在系统性偏好，这种现象通常被归因于流行度偏差，但缺乏直接证据表明这些偏好是反映真实世界流行度还是预训练期间的统计曝光，主要因为训练语料库不可获取。

**Method:** 利用开源OLMo模型及其完整预训练语料库Dolma，计算7.4万亿标记中2,000个实体的精确曝光统计，对比预训练曝光度与维基百科访问量和两种LLM流行度信号（标量估计和成对比较）。

**Result:** 预训练曝光度与维基百科流行度高度相关，验证了曝光度作为训练期间真实世界显著性的有效代理指标；LLM流行度判断与曝光度的对齐更紧密，特别在成对比较中，且在较大模型和长尾实体中表现最强。

**Conclusion:** LLM中的流行度先验主要由预训练统计而非外部流行度信号塑造，为数据曝光在驱动流行度偏差中起核心作用提供了具体证据。
...
> TokenHD是一个Token级别的幻觉检测器，通过可扩展的数据引擎和基于重要性加权的训练策略，无需步骤分割即可直接检测自由文本中的幻觉，小模型(0.6B)超越大型推理模型(QwQ-32B)。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在推理密集型任务中经常产生难以检测的幻觉，现有基于步骤的分析方法粒度有限且可扩展性差，需要预定义步骤分割。

**Method:** 提出TokenHD框架，包含：1)可扩展的数据引擎用于合成大规模幻觉标注；2)基于重要性加权的训练策略；3)严格的评估协议。训练过程直接处理自由文本，无需额外文本重格式化。

**Result:** 0.6B的小型检测器在训练后性能显著提升，超越QwQ-32B等大型推理模型，且检测性能在0.6B到8B模型规模间保持一致扩展。检测器在不同实际场景中具有良好的跨领域泛化能力。

**Conclusion:** TokenHD通过Token级别的幻觉检测解决了传统方法的粒度限制和可扩展性问题，证明了小模型通过专门训练可以在幻觉检测任务上超越大模型，为LLM的安全部署提供了有效解决方案。
...
> 本研究提出了一种公平评估控制文本生成(CTG)系统的水平操场(LPF)方法，发现系统在标准化评估下的性能显著低于原始报告结果，强调了建立标准化评估实践的紧迫性。

<details>
  <summary>Details</summary>

**Motivation:** 当前控制文本生成领域存在评估标准不统一的问题，不同研究使用不同的数据集和评估方法，难以公平比较各系统的实际性能，影响领域发展。

**Method:** 采用水平操场(LPF)评估方法，通过(i)标准化生成和处理所有系统输出，以及(ii)应用基于当前使用情况的共享评估方法和数据集，确保不同系统间的公平比较。

**Result:** 重新评估当前代表性CTG系统后发现，性能结果与原始报告存在显著差异，大多数情况下表现更差，突显了共享标准化评估方法的重要性。

**Conclusion:** LPF评估揭示了当前CTG评估实践的缺陷，表明缺乏标准化评估会导致已发布的性能声明严重歪曲系统的真实能力，迫切需要建立可重复的标准化评估标准。
...
> Q-DAPS是一种通过计算候选答案可信度分数的熵来估算问答问题难度的新方法，在多个QA数据集上优于现有基线方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的问题难度估算方法（如可读性公式、检索信号或流行度统计）无法充分捕捉现代大语言模型在问答任务中面临的推理挑战。

**Method:** 提出Q-DAPS方法，通过计算候选答案的可信度分数的熵来估算问题难度，并在TriviaQA、NQ、MuSiQue和QASC四个主要QA数据集上进行系统评估。

**Result:** Q-DAPS在所有测试数据集上均优于基线方法，对超参数变化和问题类型表现出强鲁棒性，消融研究和人工评估均验证了其有效性和与人类判断的高度一致性。

**Conclusion:** Q-DAPS为现代QA系统提供了一种可解释、可扩展且抗偏倚的问题难度估算方法，具有重要的实用价值。
...
> 该研究提出LLM在上下文学习中的信念更新可视为在低维概念信念空间中的几何轨迹运动，并通过故事理解任务验证了这一几何框架。

<details>
  <summary>Details</summary>

**Motivation:** 虽然LLM的上下文学习可理解为贝叶斯推理，但其潜在假设空间的结构尚不明确，需要更深入理解LLM内部信念更新的机制。

**Method:** 以故事理解为动态信念更新的自然场景，结合行为分析和表示分析研究，使用线性探针解码表示，并对表示进行因果干预实验。

**Result:** 发现信念更新遵循低维结构化流形轨迹，该结构在模型行为和内部表示中一致体现，可被简单线性探针预测，且表示干预能因果引导信念轨迹。

**Conclusion:** 为LLM中的信念动力学提供了几何学解释，将上下文学习的贝叶斯解释建立在结构化概念表示的基础上。
...
> 针对生成检索任务微调大语言模型时的灾难性遗忘问题，提出ORBIT方法，通过追踪参数距离并采用权重平均策略约束模型漂移，有效保留模型通用推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在针对特定任务（如生成检索）进行微调时，会快速遗忘原有的通用语言推理能力，这种现象与微调后模型参数与原始参数的距离相关，且在生成检索任务中尤为严重。

**Method:** 提出ORBIT方法，主动追踪微调模型与初始模型之间的参数距离，当距离超过阈值时采用权重平均策略来约束模型漂移，避免灾难性遗忘。

**Result:** 实验表明ORBIT在保持文本理解和检索性能方面显著优于常见的持续学习基线方法和同类型权重平均正则化方法。

**Conclusion:** 通过监控模型参数距离并动态调整权重平均策略，ORBIT成功解决了生成检索任务微调中的灾难性遗忘问题，为保持大模型通用能力提供了有效解决方案。
...
> 本文提出了一种预测大语言模型生成的难度评级与人类评分者不一致的方法，通过基于几何一致性的嵌入空间分析，而非依赖于生成时的概率信号，在英语CEFR句子难度评估任务中取得了优于概率基线的效果。

<details>
  <summary>Details</summary>

**Motivation:** 使用LLM自动生成教育材料越来越普遍，但为这些材料分配难度等级仍需要大量人工成本。虽然LLM-as-a-Judge方法受到关注，但与人类评分者的不一致性仍是主要挑战。

**Method:** 利用难度是序数尺度的特性，使用独立的嵌入空间（如ModernBERT），基于评分集的几何一致性来识别不一致候选项，无需收集评级生成时的概率信号。

**Result:** 在英语CEFR句子难度评估实验中，使用GPT-OSS-120B和Qwen3-235B-A22B模型，所提方法预测与人类评分者不一致的AUC值高于基于概率的基线方法。

**Conclusion:** 所提出的基于几何一致性的方法能更有效地识别LLM生成的不一致评级，帮助将需要重新评级的案例筛选出来，提高教育材料难度分配的效率和准确性。
...
> 论文揭示Transformer语言模型记忆事实的新机制：通过嵌入空间的几何结构而非传统的关联记忆，实现对大量事实的高效存储，MLP起到关系条件选择器的作用。

<details>
  <summary>Details</summary>

**Motivation:** 传统观点认为Transformer通过内部权重矩阵作为嵌入对的关联记忆来存储事实，但这种方法需要的参数数量需要与事实数量线性增长，效率不高。作者寻求并验证了一种更高效的几何记忆机制。

**Method:** 在受控的单层Transformer环境中记忆从主语到共享属性集的随机双射关系。理论证明对数嵌入维度就足够，其中主语嵌入编码其关联属性向量的线性叠加，小型MLP通过ReLU门控作为关系条件选择器提取相关属性。扩展到多跳推理场景，提供有无思维链的构造方案，证明容量-深度权衡。

**Result:** 理论证明对数维度嵌入就足以记忆大量事实。梯度下降确实发现了预测结构的解决方案。训练后的MLP能够零样本迁移到全新的双射关系，表明它学习了通用的选择机制而非特定的记忆。

**Conclusion:** Transformer记忆事实主要是通过嵌入空间的几何结构实现，MLP作为通用的关系选择器而非关联映射。这一发现改变了对Transformer内部工作机制的理解，解释了其高效记忆和推理能力。
...
> 在生物医学文本领域适配编码器时，采用CLM临时替代MLM的训练方法，在多项任务中显著提升了模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的编码器领域适配主要依赖MLM持续训练，但这种方法可能在生物医学等专业领域的表现存在提升空间。研究者探索了替代的训练策略以获得更好的下游任务性能。

**Method:** 提出CLM绕行方法：在MLM训练过程中临时切换到因果语言建模(CLM)，随后进行短期MLM衰减训练。在French和English生物医学任务上使用ModernBERT进行验证，并通过冻结不同层级的实验分析机制。

**Result:** 在8个法语和11个英语生物医学任务上，CLM方法比同等数据和计算条件的MLM基线分别提升1.2-2.8pp和0.3-0.8pp。CLM对低层transformer层的影响更大，表示变化在MLM衰减阶段仍保持，且随模型规模扩展。

**Conclusion:** CLM绕行是一种有效的领域适配策略，通过密集监督改进低层表示，释放了ModernCamemBERT-bio和ModernBERT-bio等state-of-the-art生物医学编码器的潜力。
...
> 研究发现大型语言模型生成的政治文本虽然在语法上流畅，但在群体层面上缺乏真实性，表现为情感更负面、结构更规律、用词更抽象，与真实网络话语存在显著差异。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型语言模型能够大规模生成流畅的政治文本，引发了关于危机和社会冲突期间合成话语的担忧。传统的AI文本检测方法主要关注句子层面的特征，但这些信号可能随着生成系统的改进而减弱。

**Method:** 研究者采用了计算社会科学的视角，构建了一个包含九个危机事件的配对语料库（178万条帖子），比较社交媒体上的真实话语与相同情境下的合成话语。从情感强度、结构规律性、词汇意识形态框架和跨事件依赖性四个维度进行评估。

**Result:** 合成话语虽然流畅但在群体层面不真实：情感更负面且分散度更低，结构更有规律，词汇更抽象。真实话语展现出更广泛的情感变化、更长的结构分布尾部以及更多情境特定的口语化标记。差异大小与事件类型相关。

**Conclusion:** 合成政治话语的主要限制不是语法或流畅性，而是缺乏群体真实性。群体层面的审计是对传统文本检测的补充，为评估生成话语的社会真实性提供了计算社会科学框架。
...
> 本文提出了一种LLM引导的查询细粒度优化方法，通过利用大语言模型对小规模文档的反馈来实时调整嵌入表示，从而在零样本搜索和分类任务中显著提升嵌入模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的嵌入模型在处理具有挑战性的零样本搜索和分类任务时存在局限性，难以适应任务特定的细微约束。同时，成本高昂的LLM流水线在大规模语料库级别部署时不可行，需要寻找更实用的替代方案。

**Method:** 提出LLM引导的查询优化策略，利用生成式LLM对少量文档集合的反馈来细化用户查询的嵌入表示。这种方法使嵌入能够实时适应目标任务，无需重新训练整个模型。

**Result:** 实验表明，该方法在所有测试的文本嵌入模型和数据集上都取得了一致的性能提升，在文献搜索、意图检测、关键点匹配和细微查询指令遵循等任务上相对提升高达25%。

**Conclusion:** LLM引导的查询优化显著扩展了嵌入模型的实际应用范围，使其成为LLM流水线不可行时的有力替代选择。优化后的查询不仅提高了排序质量，还在语料库中诱导出更清晰的二元分离，使嵌入空间能更好地反映每个临时用户查询的任务特定约束。
...
> LongMemEval-V2是一个专门评估Web代理长期记忆能力的基准测试，包含451个问题测试五种核心记忆能力，并提出了AgentRunbook-R和AgentRunbook-C两种记忆方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有代理记忆基准主要关注用户历史、短时轨迹或下游任务成功，缺乏对环境特定经验内化能力的直接评估，需要建立专门的长期记忆评估框架。

**Method:** 构建LongMemEval-V2基准，包含451个问题测试五种核心记忆能力；提出AgentRunbook-R（高效RAG记忆）和AgentRunbook-C（基于编码代理的文件存储记忆）两种记忆方法。

**Result:** AgentRunbook-C获得72.5%平均准确率，超越最强RAG基线(48.5%)和现成编码代理基线(69.3%)，但编码代理方法存在高延迟成本。

**Conclusion:** LongMemEval-V2为开发环境经验的长期记忆系统建立了具有挑战性的测试平台，AgentRunbook-C推进了准确率-延迟帕累托前沿，但仍有显著改进空间。
...
### [83] [Principle-Guided Supervision for Interpretable Uncertainty in Medical Image Segmentation](https://arxiv.org/abs/2605.10984)
*An Sui,Yuzhu Li,Gunter Schumann,Fuping Wu,Xiahai Zhuang*

Main category: cs.CV

> PriUS是一种基于证据学习的原则指导不确定性监督框架，通过图像对比度、失真严重程度和解剖结构几何复杂性三个感知对齐原则，提高了医学图像分割中不确定性的可解释性和一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法将不确定性简化为标量置信度估计，其空间分布在语义上约束不足，无法反映模糊性的来源，限制了在高风险决策（如医学图像分割）中的实际应用价值。

**Method:** 提出了三个感知对齐原则，要求不确定性空间分布反映图像结构对比度、图像失真严重程度和解剖结构几何复杂性；基于证据学习开发原则指导的不确定性监督框架PriUS，在训练中显式强制执行相应的监督目标。

**Result:** 在ACDC、ISIC和WHS数据集上的实验表明，与最先进方法相比，PriUS在保持竞争性分割性能的同时，产生了更具一致性的不确定性估计。

**Conclusion:** 通过原则指导的监督方法，PriUS有效提高了不确定性的可解释性，使预测的不确定性更符合人类对模糊性来源的直觉理解，为医学图像分割的可靠性评估提供了更好的支持。
...
### [84] [The first global agricultural field boundary map at 10m resolution](https://arxiv.org/abs/2605.11055)
...
> 论文首次发布了2024-2025年全球10米分辨率的农田边界数据集，包含31.7亿个遥感农田多边形，覆盖241个国家和地区。

<details>
  <summary>Details</summary>

**Motivation:** 农田是作物种植、管理和报告的自然单元，但现有农业遥感产品大多仅提供像素级数据，缺乏全球一致的公开农田边界地图，限制了农业监测和分析的精度。

**Method:** 使用在Fields of The World数据集上训练的U-Net分割模型，应用于无云Sentinel-2影像马赛克，生成全球农田边界预测图，并配套提供500米置信度图层。

**Result:** 在24个国家验证的像素级召回率平均为0.85，14个国家超过0.90；在奥地利、拉脱维亚和芬兰的全国性验证中，F1分数分别为0.89、0.88和0.74。

**Conclusion:** 该数据集为作物监测、粮食安全和下游农业科学提供了首个全球一致的农田级分析单元，将推动全球农业精细化管理和研究发展。
...
### [85] [HiDream-O1-Image: A Natively Unified Image Generative Foundation Model with Pixel-level Unified Transformer](https://arxiv.org/abs/2605.11061)
...
> HiDream-O1-Image是一个原生统一的像素空间扩散Transformer模型，通过统一架构消除了对外部VAE和文本编码器的依赖，实现端到端的视觉生成，8B参数版本媲美或超越更大参数的SOTA模型，200B+参数版本进一步展现了强大的可扩展性。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉生成模型受限于分散的架构，需要依赖独立的文本编码器和外部VAE，这种模块化设计存在架构碎片化问题，限制了模型的统一性和效率。

**Method:** 提出统一的Transformer架构(UiT)，将原始图像像素、文本token和任务特定条件映射到单一共享token空间，实现多模态输入的结构性统一，采用像素空间扩散Transformer技术，使用原生编码范式，将多种生成和编辑任务视为一致的上下文推理过程。

**Result:** HiDream-O1-Image在文本到图像生成、指令基础编辑和主题驱动个性化等各种生成任务上表现卓越；8B参数版本性能与更大参数量(如27B)的SOTA模型相当甚至超越；200B+参数版本解锁了前所未有的生成能力和优越性能，建立了新的SOTA基准。

**Conclusion:** 研究证明了原生统一架构的巨大潜力，为下一代多模态AI提供了一个高度可扩展的路径，标志着从模块化架构向端到端上下文视觉生成引擎的范式转变。
...
### [86] [Birds of a Feather Flock Together: Background-Invariant Representations via Linear Structure in VLMs](https://arxiv.org/abs/2605.11107)
*Youssef Zaazou,Mark Thomas*

Main category: cs.CV

> 该研究利用视觉-语言模型嵌入空间的高线性可加性特性，提出了一种预训练方法，通过合成数据构建背景不变的表示，首次在完美虚假相关条件下的Waterbirds基准测试中实现超过90%的最坏组准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉-语言模型（如CLIP和SigLIP 2）在图像分类任务中容易受到前景对象与背景之间系统性虚假相关性的影响，这严重损害了模型的鲁棒性。

**Method:** 研究人员重新审视了VLM嵌入空间的高线性可加性特性，证明了该特性可以将场景表示分解为前景和背景组件，并利用这一特性开发了使用合成数据的预训练方法来构建背景不变的表示。

**Result:** 该方法在Waterbirds数据集上的完美虚假相关（100%相关性，训练数据中无少数组样本）条件下，首次实现了超过90%的最坏组准确率，并展现了强大的仿真到现实迁移能力。

**Conclusion:** 该方法无需访问真实世界的去偏数据，具有很好的实际部署价值，为解决视觉-语言模型中的虚假相关性问题提供了有效的解决方案。
...
### [87] [LatentHDR: Decoupling Exposure from Diffusion via Conditional Latent-to-Latent Mapping for Text/Image-to-Panoramic HDR](https://arxiv.org/abs/2605.11115)
*Pedram Fekri,WenChen Li,William Chen,Peter Altamirano*

Main category: cs.CV

> LatentHDR是一种高效生成高动态范围(HDR)图像的框架，通过在潜在空间中解耦场景生成和曝光建模，单一通道即可生成结构一致的多曝光图像堆栈，计算成本降低一个数量级。

<details>
  <summary>Details</summary>

**Motivation:** 现有的生成模型在HDR生成方面存在局限性，主要输出低动态范围内容。基于扩散模型的方法需要生成多曝光样本，计算成本高且结构不一致，亟需更高效统一的解决方案。

**Method:** 采用预训练扩散模型生成单一场景表示，配合轻量级条件潜在空间头部网络，确定性映射到曝光特定的表示。在潜在空间解耦场景生成与曝光建模，支持文本和图像条件输入。

**Result:** 在合成数据和SI-HDR基准测试中达到最先进的动态范围，感知质量具有竞争力，计算量减少一个数量级，消除了多通道扩散的需求。

**Conclusion:** 通过结构化潜在建模可实现高质量HDR生成，挑战了传统随机多曝光生成的必要性，为HDR合成提供了可扩展的高效解决方案。
...
### [88] [USEMA: a Scalable Efficient Mamba Like Attention for Medical Image Segmentation](https://arxiv.org/abs/2605.11131)
*Elisha Dayag,Nhat Thanh Tran,Jack Xin*

Main category: cs.CV

> 本文提出了USEMA，一种结合CNN局部特征提取和SEMA注意力机制的混合UNet架构，用于提高医学图像分割的计算效率和分割性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统视觉Transformer在医学图像分割中面临计算复杂度高的问题，而纯卷积网络和基于Mamba的模型在性能上存在局限。需要一种能高效融合局部和全局信息的架构来解决医学图像分割的挑战。

**Method:** 设计SEMA（Scalable and Efficient Mamba like Attention）注意力机制，通过局部窗口注意力避免token分散，结合算术平均捕获全局信息；构建USEMA混合架构，整合CNN的局部特征提取能力和SEMA的全局注意力机制。

**Result:** USEMA在多种模态和图像尺寸的实验中，相比使用完整自注意力的Transformer模型具有更好的计算效率，相比纯卷积和基于Mamba的模型实现了更优的分割性能。

**Conclusion:** USEMA成功平衡了计算效率和分割精度，为医学图像分割提供了一种有效的混合架构解决方案，验证了CNN与SEMA注意力机制结合的优越性。
...
### [89] [Unpacking the Eye of the Beholder: Social Location, Identity, and the Moving Target of Political Perspectives](https://arxiv.org/abs/2605.11166)
*Elena Sirotkina*

Main category: cs.CV

> 本研究开发了PVPS分类器，考虑观众的政治和社会身份预测其对政治图像的情感评价，揭示图像含义因受众群体不同而显著差异。

<details>
  <summary>Details</summary>

**Motivation:** 传统计算工具通常为文本或图像生成单一情感分数，忽略了政治和社会身份对信息评价的影响，这种差异具有重要意义但常被系统性地平均化。

**Method:** 基于5,575名美国成年人约82,000次评价数据，开发Perspectivist Visual Political Sentiment (PVPS)分类器，该工具保留不同身份群体间的系统性分歧，生成记录不同群体评价差异的评估档案。

**Result:** 应用于视觉情感研究时发现：考虑受众身份后，抗议图像中被感知的暴力程度以及促使图像参与的情感机制都发生实质性变化。

**Conclusion:** 政治图像所传达的含义是移动的目标，准确测量需要明确知道是在对谁说。研究强调了在政治传播分析中考虑受众身份的必要性。
...
### [90] [Hi-GaTA: Hierarchical Gated Temporal Aggregation Adapter for Surgical Video Report Generation](https://arxiv.org/abs/2605.11208)
*Kedi Sun,Chaohui Dang,Yue Feng,James Glasbey,Theodoros N. Arvanitis,Le Zhang*

Main category: cs.CV

> 本文提出了一个包含214个高质量手术视频数据集和一个基于感知-对齐-推理框架的手术视频报告生成系统，通过Hi-GaTA时序适配器和Sur40k视频编码器，实现了自动生成临床级手术评估报告。

<details>
  <summary>Details</summary>

**Motivation:** 手术报告的文档化负担重，缺乏客观反馈，主要技术难点在于视频时空表示与语言推理的对齐困难，以及高质量、隐私保护数据集的稀缺性。

**Method:** 建立包含214个手术视频与专家报告的基准数据集；提出感知-对齐-推理框架；设计Hi-GaTA轻量时序适配器通过短到长程时序聚合压缩视频序列；预训练Sur40k手术专用视频编码器；使用LoRA微调大语言模型。

**Result:** 在手术报告生成任务上取得最佳整体性能，相比强大的多模态大语言模型基线有持续提升，消融实验验证了各组件的有效性。

**Conclusion:** 通过精心设计的数据集、时序适配器和视频编码器，成功实现了手术视频到临床级报告的自动生成，减轻了医生的文档负担并提供了客观的手术评估反馈。
...
### [91] [ABRA: Agent Benchmark for Radiology Applications](https://arxiv.org/abs/2605.11224)
*Bulat Maksudov,Vladislav Kurenkov,Kathleen M. Curran,Alessandra Mileo*

Main category: cs.CV

> ABRA是一个新颖的放射学智能体基准测试，让智能体通过操作OHIF查看器和Orthanc DICOM服务器来导航医学影像环境，包含655个生成任务，测试发现当前模型在工具执行上表现良好但在感知能力上存在明显瓶颈。

<details>
  <summary>Details</summary>

**Motivation:** 现有医学智能体基准测试仅提供预选的影像样本，缺乏真实环境中智能体需要主动导航的挑战。为更好地评估医学AI智能体在真实临床工作流程中的能力，需要构建一个让智能体主动操作和探索影像环境的基准测试平台。

**Method:** 构建ABRA基准测试，包含21个功能调用工具，涵盖切片导航、窗宽窗位调整、序列选择、像素坐标标注和结构化报告等功能。使用LIDC-IDRI、Duke乳腺MRI和NLST新病灶LongCT数据集生成655个任务，分为三个难度等级和八种类型（查看器控制、元数据QA、视觉探测、标注、纵向比较、BI-RADS报告及相应的oracle变体）。

**Result:** 测试的10个模型（5个闭源权重和5个开源权重）在真实标注任务中执行准确率达到89%以上，但结果准确率仅为0-25%；在使用模拟检测器的oracle变体任务中，结果准确率提升至69-100%，表明感知能力而非工具编排是主要瓶颈。

**Conclusion:** ABRA填补了医学智能体基准测试的空白，通过真实环境的导航任务揭示了当前模型在医学影像感知方面的局限性。该基准测试为改进医学AI智能体的感知能力提供了重要的评估工具和发展方向。
...
### [92] [DenseTRF: Texture-Aware Unsupervised Representation Adaptation for Surgical Scene Dense Prediction](https://arxiv.org/abs/2605.11265)
*Guiqiu Liao,Matjaž Jogan,Daniel A. Hashimoto*

Main category: cs.CV

> DenseTRF是一种基于纹理注意力的自监督表示适应框架，通过槽位注意力学习纹理感知表示，显著提升手术计算机视觉中密集预测任务的跨域泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 手术计算机视觉中的密集预测任务（如分割和手术区域预测）经常遭受分布偏移问题，因为训练数据集很难覆盖部署时遇到的各种变异性，导致模型泛化能力差。

**Method:** 提出DenseTRF框架，使用槽位注意力机制学习纹理感知表示来捕获不变的视觉结构。通过自适应调整这些表示到目标分布而无需监督，并结合条件密集预测和模型合并策略来实现。

**Result:** 在多个手术程序的实验中，与最先进的分割模型和测试分布自适应方法相比，DenseTRF在密集预测任务上展现出更好的跨分布泛化性能。

**Conclusion:** DenseTRF通过基于纹理中心的自监督表示适应，有效解决了手术计算机视觉中的领域偏移问题，为腹腔镜和机器人手术提供了更可靠的视觉引导。
...
### [93] [PG-3DGS: Optimizing 3D Gaussian Splatting to Satisfy Physics Objectives](https://arxiv.org/abs/2605.11266)
*Zachary Lee,Maxwell Jacobson,Yexiang Xue*

Main category: cs.CV

> PG-3DGS是一种将可微分物理模拟与3D高斯表示相结合的框架，能够生成既具备视觉保真度又满足物理功能的3D结构，在保持视觉效果的同时显著提升了物体的物理功能性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的高斯泼洒技术虽然能快速生成高质量3D场景，但仅限于视觉表现，缺乏对物理世界行为规律的理解。需要开发一种能够同时考虑视觉准确性和物理功能性的3D生成方法。

**Method:** 提出物理引导的3D高斯泼洒(PG-3DGS)框架，通过将可微分物理模拟与3D高斯表示耦合，在视觉损失基础上加入物理目标函数来引导形状优化过程，使生成的几何形状既满足光度准确性又具备物理功能性。

**Result:** 在倾倒和空气动力学升力任务上的实验表明，PG-3DGS在保持视觉质量的同时显著改善了物理功能。3D打印飞机的实际风洞测试显示，在相同气流条件下，PG-3DGS生成的结构比外观匹配基线产生更高的实测升力（在三个案例中均验证）。

**Conclusion:** 该统一框架成功地将基于外观的重构与基于物理的推理连接起来，实现了端到端生成既看起来逼真又功能正确的3D结构，为3D生成领域提供了新的发展方向。
...
### [94] [Real-Scale Island Area and Coastline Estimation using Only its Place Name or Coordinates](https://arxiv.org/abs/2605.11267)
*Quanyun Wu,Kyle Gao,Wentao Sun,Hongjie He,Yuhao Chen,David A. Clausi,Jonathan Li*

Main category: cs.CV

> 本文提出了一种基于纯单目视觉的几何一致性实尺度岛屿测量框架，仅通过输入目标区域的地理坐标或名称就能获取低空环绕图像序列，经过点云生成、尺度恢复和正交校正后，自动提取高精度的岛屿面积和周长，测量误差稳定在10%左右。

<details>
  <summary>Details</summary>

**Motivation:** 传统的岛屿测量和海岸线长度测量方法严重依赖正射影像、昂贵的机载深度传感器或密集的地面控制点，在广袤且难以进入的开阔海域面临高人工成本、耗时长、操作效率低的严重局限性，需要摆脱对人工现场勘探的依赖。

**Method:** 构建单目视觉测量框架，输入地理坐标获取低空环绕图像序列，生成点云后使用轻量级轨迹对齐算法(Umeyama)恢复全局物理尺度，对尺度模型进行正交校正，直接在2D栅格化平面上提取高精度面积和周长。

**Result:** 在四个具有不同地形特征(包括自然地貌岛屿和具有复杂人工设施的岛屿)的岛屿上进行了全面验证，最终测量误差稳定在10%左右，系统处理单张高分辨率图像并生成点云仅需70ms，展现出极佳的准确性和鲁棒性。

**Conclusion:** 该框架通过全自动化流程显著降低了测绘成本，在无需先验GIS数据的情况下实现了高效测量，为大规模海洋和海岸线监测提供了具有高度实用性的新范式。
...
### [95] [Generative AI for Visualizing Highway Construction Hazards Through Synthetic Images and Temporal Sequences](https://arxiv.org/abs/2605.11276)
*Trevor Neece,Mason Smetana,Lev Khazanovich*

Main category: cs.CV

> 本研究开发了基于生成式AI的公路建设危险场景可视化方法，能够从OSHA重伤报告文本中合成图像，用于安全培训教育。

<details>
  <summary>Details</summary>

**Motivation:** 公路建设工人面临较高的重伤或死亡风险，但由于伦理和后勤障碍，基于图像的危险场景培训材料严重稀缺。

**Method:** 开发了两种生成模式：单模式（每个事故生成一张图像）和时间序列模式（生成四阶段序列），从75起事故记录中生成了750张图像，采用CLIP语义检索和专家多维度评估。

**Result:** 单模式图像教育可接受性达81.1%，保真度和对齐度分别为4.14/5和4.07/5；时间序列模式可接受性60.9%，对齐度3.94/5但保真度较低3.51/5。两种模式都具有统计学显著的图像检索能力。

**Conclusion:** 这是首批利用现代自回归图像生成模型从重伤报告中可视化建设危险的研究之一，成功开发的多维评估框架可支持该领域未来发展，使安全培训师能够将叙事讲述与视觉学习材料结合，无需拍摄真实世界危险场景。
...
### [96] [Can Graphs Help Vision SSMs See Better?](https://arxiv.org/abs/2605.11300)
*Dhruv Parikh,Anvitha Ramachandran,Haoyang Fan,Mustafa Munir,Rajgopal Kannan,Viktor Prasanna*

Main category: cs.CV

> GraphScan是一种基于图的动态扫描算子，用于视觉状态空间模型(Vision SSMs)，通过构建局部图和语义路由来提升模型性能，在多个视觉任务中达到SOTA效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的Vision SSMs扫描机制主要适应路径或采样位置，但没有明确建模哪些局部补丁应该在进行全局状态空间混合前交换信息。这引出了思考：图结构是否能帮助Vision SSMs更好地理解视觉内容？

**Method:** GraphScan为每个token构建空间受限的局部图，学习带有相对位置偏置的特征条件亲和度，通过一步消息传递在语义邻域内生成输出token。这些局部语义融合的token再经过选择性SSM进行全局聚合。

**Result:** 集成到分层骨干网络中的GraphScan-Mamba在图像分类、目标检测、实例分割和语义分割任务上实现了Vision SSMs的SOTA性能，计算开销适中。分析表明GraphScan产生了解释性的位移场。

**Conclusion:** 未来Vision SSMs应该将扫描不仅视为几何序列化，而是在全局状态空间建模前的学习型局部语义路由过程。
...
### [97] [CheXTemporal: A Dataset for Temporally-Grounded Reasoning in Chest Radiography](https://arxiv.org/abs/2605.11304)
...
> CheXTemporal是一个用于胸部X光时间推理的基准数据集，包含前后对比图像对和精细的时间-空间标注，发现现有视觉语言模型在时间推理方面存在显著局限。

<details>
  <summary>Details</summary>

**Motivation:** 胸部X光解读需要基于当前和既往研究进行时间推理，但现有视觉语言模型主要训练于静态图像-报告对，缺乏对纵向变化建模的显式监督。

**Method:** 构建CheXTemporal黄金标准数据集，包含五类进展分类（新增、恶化、稳定、改善、消失）、病理局部空间标注、跨研究空间-时间对齐；并创建28万对的银标准数据集用于大规模评估。

**Result:** 在零样本设置下评估表明，当前模型在空间定位、细粒度时间推理和分布偏移下的鲁棒性方面存在一致局限，在时间细微状态（稳定、消失）上的表现明显劣于显著进展状态（恶化）。

**Conclusion:** 现有视觉语言模型在胸部X光纵向疾病演化建模方面能力有限，CheXTemporal为改进时间推理建模提供了重要基准和评估资源。
...
### [98] [Vision2Code: A Multi-Domain Benchmark for Evaluating Image-to-Code Generation](https://arxiv.org/abs/2605.11307)
*Ajay Vikram Periasami,Junlin Wang,Bhuwan Dhingra*

Main category: cs.CV

> Vision2Code是一个无需参考代码的多领域图像到代码生成基准测试框架，通过VLM评估器结合领域特定评分标准来评估图像重建质量，解决了现有基准的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 现有图像到代码生成基准存在三个主要问题：视觉领域狭窄、依赖成对的参考代码、以及使用通用评分准则而忽略领域特定的重建错误。

**Method:** 构建了包含15个数据集2,169个测试例的Vision2Code基准，覆盖图表、几何、科学图像等多个领域；模型生成可执行程序后渲染成图像，使用VLM评估器结合数据集特定评分标准对原图像进行评分，并设置了确定性防护机制检测语义失败。

**Result:** 研究发现模型在图像到代码转换能力上存在领域依赖性：在图表类视觉表现良好，但在空间场景、化学、文档和电路图等领域表现较弱；评估框架与人类判断一致性更好；使用评估过滤的模型输出作为训练数据可提升Qwen3.5-9B指标从1.60提升至1.86。

**Conclusion:** Vision2Code提供了一个可重现的测试平台，用于测量、诊断和改进图像到代码生成能力，为该领域的研究带来了新的评估标准和改进方法。
...
### [99] [Quantifying Rodda and Graham Gait Classification from 3D Makerless Kinematics derived from a Single-view Video in a Heterogeneous Pediatric Clinical Cohort](https://arxiv.org/abs/2605.11314)
...
> 这项研究开发了一种无需标记的步态分析技术，可以从单视角临床步态视频中直接量化脑瘫儿童的膝关节和踝关节z评分，实现了可扩展的客观步态评估。

<details>
  <summary>Details</summary>

**Motivation:** 脑瘫是最常见的儿童终身身体残疾原因，准确评估步态对保持行走功能至关重要，但传统的三维仪器步态分析(3D-IGA)成本高昂且仅限于专业中心使用，而观察性评估的一致性较差。

**Method:** 开发了一个无需标记的步态分析流程，直接从单视角临床步态视频中提取Rodda和Graham分类系统的膝关节和踝关节矢状面z评分，并在152名儿童的1058个双侧肢体样本上进行验证。

**Result:** 膝关节z评分预测达到R²=0.80，踝关节z评分达到R²=0.57；过度膝关节屈曲筛查的AUROC=0.88，能正确识别83%受影响儿童；7分类准确率达43±1%，踝关节预测是主要限制因素。

**Conclusion:** 视频基础的z评分估计、过度屈曲筛查和纵向轨迹跟踪是可行的，为低资源临床环境中可扩展的客观步态评估提供了新途径，能够监测疾病进展和治疗反应，这是观察量表无法提供的定量信息。
...
### [100] [Lite3R: A Model-Agnostic Framework for Efficient Feed-Forward 3D Reconstruction](https://arxiv.org/abs/2605.11354)
*Haoyu Zhang,Zeyu Zhang,Zedong Zhou,Yang Zhao,Hao Tang*

Main category: cs.CV

> Lite3R是一个用于3D重建的轻量Transformer框架，通过稀疏线性注意力降低计算成本，并配合FP8感知量化和部分注意力蒸馏，在保持重建质量的同时显著提升效率和内存使用。

<details>
  <summary>Details</summary>

**Motivation:** 基于Transformer的3D重建模型虽然性能强大，但在实际部署中面临效率和精度两个挑战：密集多视图注意力造成大量token混合开销，而低精度执行会破坏几何敏感表示，降低了深度、姿态和3D一致性。

**Method:** 提出模型无关的教师-学生框架Lite3R，用稀疏线性注意力替代密集注意力；引入参数高效的FP8感知量化感知训练策略，冻结大部分预训练骨干网络参数，仅训练轻量级线性分支投影层，并配合部分注意力蒸馏技术。

**Result:** 在VGGT和DA3-Large骨干网络上评估，Lite3R在BlendedMVS和DTU64数据集上实现了1.7-2.0倍的延迟降低和1.9-2.4倍的内存使用减少，同时保持了竞争性的重建质量。

**Conclusion:** Lite3R为实际的基于Transformer的3D重建提供了有效的算法-系统协同设计方法，成功解决了效率与精度之间的权衡问题。
...
### [101] [PresentAgent-2: Towards Generalist Multimodal Presentation Agents](https://arxiv.org/abs/2605.11363)
*Wei Wu,Ziyang Xu,Zeyu Zhang,Yang Zhao,Hao Tang*

Main category: cs.CV

> PresentAgent-2是一个智能代理框架，能够根据用户查询自动生成包含深度研究、多模态媒体和交互功能的多模式演示视频，支持单人讲解、多人讨论和问答交互三种模式。

<details>
  <summary>Details</summary>

**Motivation:** 传统演示生成主要局限于基于文档的静态幻灯片创建，无法实现端到端的演示视频生成。随着多模态技术和交互式需求的增长，需要一个新的框架来实现从查询到完整演示视频的自动化生成，包括研究基础、媒体整合和互动功能。

**Method:** PresentAgent-2框架首先将开放性用户查询总结为聚焦主题，然后对演示友好的源进行深度研究收集多模态资源（文本、图像、GIF、视频），接着构建演示幻灯片并生成模式特定脚本，最后将幻灯片、音频和动态媒体组合成完整演示视频。系统支持三种独立模式：单人讲解、多人结构化讨论和基于证据的问答交互。

**Result:** 构建了一个多模态演示基准测试，涵盖单人演示、讨论和交互场景，建立了针对内容质量、媒体相关性、动态媒体使用、对话自然性和交互基础的任务特定评估标准。

**Conclusion:** PresentAgent-2成功将演示生成从依赖文档的幻灯片创建扩展到查询驱动、基于研究的多模态演示视频生成，实现了演示生成的重大突破，为自动化演示创作提供了新的技术路径。
...
### [102] [3D-Belief: Embodied Belief Inference via Generative 3D World Modeling](https://arxiv.org/abs/2605.11367)
...
> 本研究提出了3D-Belief，一个生成式3D世界模型，将世界建模重新定义为3D空间中的具身信念推理，能够从部分观测中推断并更新显式的3D信念，显著提升了想象质量和下游任务性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉生成模型主要关注新颖视图合成或未来帧预测，强调视觉真实感，而忽略了具身智能体在部分可观性下需要的结构化不确定性。因此需要一种新的世界建模视角。

**Method:** 提出将世界建模视为3D空间中的具身信念推理，开发了3D-Belief模型，具备空间一致的场景记忆、多假设信念采样、序列信念更新和语义信息预测等关键能力，直接在3D中表示不确定性。

**Result:** 通过2D视觉质​​量评估、3D对象和场景想象评估（3D-CORE基准测试）以及仿真和真实世界的物体导航任务，验证了3D-Belief在想象质量和任务性能上都优于现有方法。

**Conclusion:** 3D-Belief成功实现了从部分观测中推断和更新3D信念的目标，为具身智能体提供了更好的环境推理能力，推动了世界建模从视觉真实感向结构化不确定性推理的转变。
...
### [103] [Dynamic Full-body Motion Agent with Object Interaction via Blending Pre-trained Modular Controllers](https://arxiv.org/abs/2605.11369)
*Sanghyeok Nam,Byoungjun Kim,Daehyung Park,Tae-Kyun Kim*

Main category: cs.CV

> 本文提出一个结合预训练运动先验和模仿智能体的框架，用于生成动态且长期的人物交互动作，如手持桌子跑步。

<details>
  <summary>Details</summary>

**Motivation:** 现有人物交互数据集局限于静态交互，预训练智能体要么只生成交互物体的动态全身运动，要么只能生成静态人物交互动作，缺乏动态长期交互能力。

**Method:** 框架分规划阶段和执行阶段：规划阶段用预训练人体运动扩散模型的动态先验增强HOI数据集，然后生成物体轨迹；执行阶段通过组合器网络融合专门处理动态人体运动和静态HOI运动的预训练模仿智能体的动作。

**Result:** 该方法在相关基准测试中持续提高成功率，在动态HOI任务中保持交互，同时在显著减少训练时间的情况下达到竞争性能。

**Conclusion:** 提出的规划和执行框架有效解决了动态长期人物交互动作生成的挑战，组合器网络的成功融合证明了预训练专家互补技能时序组合的有效性。
...
### [104] [HamBR: Active Decision Boundary Restoration Based on Hamiltonian Dynamics for Learning with Noisy Labels](https://arxiv.org/abs/2605.11383)
*Ningkang Peng,Jingyang Mao,Qianfeng Yu,Xiaoqian Peng,Peirong Ma,Yanhui Gu*

Main category: cs.CV

> 本文提出基于哈密顿动力学的新方法HamBR，通过主动探测类间模糊区域并合成高质量虚拟离群点来恢复噪声环境下的决策边界，显著提升了深度神经网络的噪声鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 在大规模视觉识别任务中，噪声标签严重影响DNN的泛化能力。现有基于损失或置信度的样本选择方法在噪声退化的特征空间中遭遇系统性边界坍塌，难以区分困难干净样本与噪声样本，成为性能瓶颈。

**Method:** 提出HamBR框架，利用球面哈密顿蒙特卡洛(Spherical HMC)机制主动探测表示空间中的类间模糊区域，合成高质量虚拟离群点。通过基于能量建模的显式排斥约束，这些合成样本在决策边界处建立鲁棒能量屏障，迫使真实样本从重叠区域向各自类中心移动，恢复决策边界的判别性。

**Result:** 该方法显著提升了困难边界样本的判别准确性，在CIFAR-10/100和真实噪声基准测试上达到SOTA性能，同时展现出优越的收敛效率、可靠鲁棒性以及分布外检测能力的显著改善。

**Conclusion:** HamBR作为一种即插即用的防御模块，可集成到现有半监督噪声标签学习框架中，通过主动边界恢复机制有效解决了噪声学习中的决策边界坍塌问题，为噪声鲁棒学习提供了新范式。
...
### [105] [JACoP: Joint Alignment for Compliant Multi-Agent Prediction](https://arxiv.org/abs/2605.11385)
*Qingze Liu,Alen Mrdovic,Danrui Li,Mathew Schwartz,Sejong Yoon,Mubbasir Kapadia*

Main category: cs.CV

> JACoP是一个多阶段框架，通过基于锚点的合规性过滤器和马尔可夫随机场对齐器，实现了符合社会规则且环境合规的多智能体轨迹预测。

<details>
  <summary>Details</summary>

**Motivation:** 现有随机轨迹预测模型虽能优化单个代理的精度，但生成的预测轨迹常出现社交碰撞和环境违规问题，不适合实际应用。

**Method:** 提出JACoP框架，包含基于锚点的代理中心剖面仪进行初始合规过滤，以及基于马尔可夫随机场的对齐器，将智能体间空间和社会成本表示为MRF能量势，从联合轨迹分布中推理和采样。

**Result:** 实验表明JACoP在保持竞争性准确率的同时，显著减少了环境违规和社交碰撞，在场景合规性方面建立了新标准。

**Conclusion:** JACoP成功实现了集体可行且实用的轨迹预测，为实际应用提供了可靠的多智能体轨迹预测解决方案。
...
### [106] [VidSplat: Gaussian Splatting Reconstruction with Geometry-Guided Video Diffusion Priors](https://arxiv.org/abs/2605.11424)
*Jimin Tang,Wenyuan Zhang,Junsheng Zhou,Zian Huang,Kanle Shi,Shenkun Xu,Yu-Shen Liu,Zhizhong Han*

Main category: cs.CV

> VidSplat是一个无训练的生成式重建框架，利用视频扩散先验从稀疏视图生成新视角，实现完整3D场景重建。

<details>
  <summary>Details</summary>

**Motivation:** 高斯泼溅在稀疏视图情况下出现显著退化，现有方法虽然增强了多视图一致性，但难以推断输入覆盖范围之外的不可见、遮挡或弱约束区域。

**Method:** 提出双阶段策略：1) 无训练的3D一致性去噪策略，使用渲染的RGB和掩码图像自适应引导去噪方向；2) 迭代机制，采样相机轨迹探索未观测区域，合成新视角，通过置信度加权细化补充训练。

**Result:** 在广泛使用的基准测试中，VidSplat在稀疏视图场景重建上表现出优越性能，对稀疏输入甚至单张图像都能鲁棒工作。

**Conclusion:** 通过有效整合生成和重建，VidSplat成功解决了稀疏视图重建中缺失区域推断的挑战，为完整3D场景重建提供了新的解决途径。
...
### [107] [PD-4DGS:Progressive Decomposition of 4D Gaussian Splatting for Bandwidth-Adaptive Dynamic Scene Streaming](https://arxiv.org/abs/2605.11427)
*Jiachen Li,Guangzhi Han,Jin Wan,Delong Han,Yuan Gao,Min Li,Mingle Zhou,Gang Li*

Main category: cs.CV

> PD-4DGS是首个支持4D高斯溅射渐进式压缩和按需传输的框架，通过分层变形分解将动态场景分解为三层独立传输的层次，实现首帧延迟从73-930秒降至约1.7秒，流传输比特率降低60%以上。

<details>
  <summary>Details</summary>

**Motivation:** 现有4D高斯溅射模型是单体比特流，客户端必须完整下载才能渲染帧，在移动带宽下导致数十到数百秒的黑屏等待，且与现代自适应比特率传输不兼容。渐进式3DGS压缩无法处理动态场景中占主导地位的时序变形网络。

**Method:** 提出分层变形分解(HDD)将4DGS中潜在的从粗到细运动层次外化为三个独立可传输层：静态支架、全局变形和局部细化；使用高斯熵属性率失真损失和时序掩码一致性正则化基层，抑制低比特率闪烁；采用容量加权展开策略通过学习激活率ρ在线控制，无需场景特定超参数防止变形网络欠训练。

**Result:** 在Dycheck iPhone基准测试上，PD-4DGS在相同渲染保真度下将流传输比特率降低60%以上，在2Mbps链路下将首帧延迟从73-930秒降至约1.7秒，首次实现了4DGS的真正按需渐进式流传输。

**Conclusion:** PD-4DGS成功将单体4DGS模型转换为可扩展、兼容DASH/HLS的比特流，使任意比特流前缀都可渲染，为4DGS的渐进式流传输奠定了基础。
...
### [108] [Diabetic Retinopathy Classification using Downscaling Algorithms and Deep Learning](https://arxiv.org/abs/2605.11430)
*Nishi Doshi,Urvi Oza,Pankaj Kumar*

Main category: cs.CV

> 该论文提出了一种结合多种降尺度算法的多通道Inception V3架构，通过融合Kaggle和IDRIS数据集，在糖尿病视网膜病变分类任务上超越了现有最先进方法。

<details>
  <summary>Details</summary>

**Motivation:** 糖尿病视网膜病变分类面临的主要挑战是图像尺寸大且变化不一。为了解决这一问题并提高分类效果，研究者探索了在深度学习网络处理前对图像进行降尺度处理的有效性。

**Method:** 论文采用创新的多通道Inception V3架构，结合独特的预处理阶段和多种降尺度算法。通过合并Kaggle和印度糖尿病视网膜病变图像数据集（IDRIS）来增强训练和测试效果，使用准确率、特异性和敏感性指标评估性能。

**Result:** 实验结果表明，所提出的方法在准确率、特异性和敏感性方面均优于之前的最先进方法，证明了降尺度算法和多通道架构的有效性。

**Conclusion:** 该研究证明了在深度学习框架中结合降尺度预处理和多通道架构能够显著提升糖尿病视网膜病变分类的性能，为医学图像分析提供了新的解决方案。
...
### [109] [ZeroIDIR: Zero-Reference Illumination Degradation Image Restoration with Perturbed Consistency Diffusion Models](https://arxiv.org/abs/2605.11435)
*Hai Jiang,Zhen Liu,Yinjie Lei,Songchen Han,Bing Zeng,Shuaicheng Liu*

Main category: cs.CV

> ZeroIDIR是一个零参考的扩散框架，通过分解光照校正和扩散重建过程，仅使用低质量退化图像即可实现光照退化图像的修复，性能优于无监督方法，可与有监督方法媲美。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图像光照修复方法通常需要成对的高质量-低质量训练数据，限制了其在现实场景中的应用。同时，单一阶段的修复方法难以有效处理光照偏差和细节重建的耦合问题。

**Method:** 提出一个两阶段框架：1) 自适应伽马校正模块进行空间变化的光照校正，通过直方图引导的光照校正损失约束光照分布；2) 扰动一致性扩散模型将校正后的图像作为中间噪声状态进行细节重建，并提出扰动扩散一致性损失确保重建的稳定性和保真度。

**Result:** 在公共基准测试中，ZeroIDIR在无监督方法中达到最优性能，与有监督方法相当，且对各种场景具有更好的泛化能力。无需成对训练数据即可实现高质量的光照退化图像修复。

**Conclusion:** 该工作通过解耦光照校正和细节重建，结合扩散模型和自适应校正策略，为无监督光照退化图像修复提供了有效解决方案，具有实用价值和应用前景。
...
### [110] [Beyond Masks: The Case for Medical Image Parsing](https://arxiv.org/abs/2605.11438)
*Siddharth Gupta,Alan L. Yuille,Zongwei Zhou*

Main category: cs.CV

> 医学影像研究应从单一的体素掩码转向医学图像解析，输出包含实体、属性和关系的结构化表示，以满足决策、重建和预测三个核心要求。当前系统在实体识别上较为成熟，但在属性、关系的完整表达上仍存在重大不足。

<details>
  <summary>Details</summary>

**Motivation:** 当前医学影像研究过度依赖体素掩码输出，但放射科医师的报告内容却远超掩码能表达的信息。研究需要更贴近临床实际需求的输出方式，以提供更丰富、更结构化的医学影像信息。

**Method:** 提出医学图像解析框架，包含三个核心要素：实体（命名结构和发现）、属性（描述实体特征）、关系（连接实体间的空间和时间关系）。定义了解析质量的评判标准：决策能力、重建能力和预测能力。对11个代表性系统进行审计评估。

**Result:** 审计显示，现有系统在实体识别方面表现较好，但在属性提取、关系建模和完整性方面几乎为空。没有系统能够产生格式良好的完整解析结果。

**Conclusion:** 解决问题的关键不在于新架构，而在于承诺更丰富的输出格式和相应的训练信号。分割教会模型测量，而解析要求模型解释。研究重心应从单纯的分割转向全面的医学图像理解。
...
### [111] [Instruct-ICL: Instruction-Guided In-Context Learning for Post-Disaster Damage Assessment](https://arxiv.org/abs/2605.11439)
*Armin Zarbaft,Ehsan Karimi,Nhut Le,Maryam Rahnemoonfar*

Main category: cs.CV

> 本研究探索了使用结构化推理策略提高预训练多模态大语言模型在灾后视觉问答任务中的可靠性，通过指令驱动的思维链方法显著提升了回答准确性。

<details>
  <summary>Details</summary>

**Motivation:** 自然灾害应急响应中，快速准确的情况感知至关重要，但传统训练特定模型的方法耗时且计算昂贵。预训练MLLMs虽然展示了强大能力，但其对提示词表述的敏感性限制了在真实灾后评估场景中的可靠性。

**Method:** 提出了多层次的提示范例，使用第一个MLLM生成特定任务的指令作为第二个MLLM的思维链指导，结合不同程度的上下文学习，使模型能够同时利用显式推理指导和上下文示例。

**Result:** 在FloodNet数据集上的评估表明，集成指令驱动的思维链推理方法相比零基线始终能够提高回答准确性。

**Conclusion:** 结构化推理策略特别是指令驱动的思维链方法能够有效提升预训练MLLMs在灾后视觉问答任务中的表现和可靠性，为时间关键的灾害评估提供了实用解决方案。
...
### [112] [Leveraging Multimodal Large Language Models for All-in-One Image Restoration via a Mixture of Frequency Experts](https://arxiv.org/abs/2605.11444)
*Eunho Lee,Youngbae Hwang,Rei Kawakami*

Main category: cs.CV

> 本文提出了一种由多模态大语言模型(MLLM)指导的一体化图像恢复框架，通过多模态嵌入指导和混合频率专家模块，有效处理复合退化关系，在CDD11数据集上达到最新水平。

<details>
  <summary>Details</summary>

**Motivation:** 现有的一体化图像恢复方法将退化视为离散类别，难以建模复合退化中出现的连续关系结构，限制了其性能。

**Method:** 构建MLLM指导的图像恢复框架，包括：1) MLLM指导的融合块(MGFB)将MLLM特征注入编解码器架构；2) 混合频率专家(MoFE)模块自适应组合频率专家；3) MLLM指导的路由器设计关系对齐损失以优化专家路由。

**Result:** 在多个基准测试中表现优异，在具有挑战性的CDD11数据集上建立新的最先进水平，相比之前方法提升高达1.35 dB。

**Conclusion:** 该方法成功利用MLLM的多模态理解能力指导低层视觉恢复任务，有效处理连续退化关系，为一体化图像恢复提供了新的解决思路。
...
### [113] [SpatialForge: Bootstrapping 3D-Aware Spatial Reasoning from Open-World 2D Images](https://arxiv.org/abs/2605.11462)
*Zishan Liu,Ruoxi Zang,Yanglin Zhang,Wei Liu,Yin Zhang,Jian Yao,Jiayin Zheng,Zhengzhe Liu*

Main category: cs.CV

> SpatialForge提出了一种可扩展的数据合成管道，将大规模2D网络图像转换为空间推理监督数据，构建了1000万个QA对，显著提升了VLM的空间推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 大型视觉语言模型(VLMs)在语义理解方面表现出色，但在空间推理方面存在困难，特别是在深度排序和精确坐标定位等基础几何任务上。现有的空间监督方法受限于场景数量，数据规模和多样性远小于大规模2D图像集合。

**Method:** 将空间推理分解为感知和关系两个部分，构建覆盖深度、布局和视点相关推理的结构化监督信号，并采用自动验证确保数据质量。基于此管道开发了SpatialForge-10M数据集。

**Result:** 在多个空间推理基准测试上的广泛实验表明，使用SpatialForge-10M训练显著提高了标准VLM的空间推理能力，证明了扩展2D数据用于3D感知空间推理的有效性。

**Conclusion:** SpatialForge通过将大规模2D网络图像转换为空间推理监督数据，为提升VLM的空间能力提供了有效途径，展示了用2D数据解决3D空间推理问题的潜力。
...
### [114] [Encore: Conditioning Trajectory Forecasting via Biased Ego Rehearsals](https://arxiv.org/abs/2605.11463)
*Conghao Wong,Ziqian Zou,Xinge You*

Main category: cs.CV

> 该论文提出了一种名为 Encore 的轨迹预测模型，通过模拟人格化的"排练到返场"过程来显式学习和预测智能体的主观性，从而提升预测性能。

<details>
  <summary>Details</summary>

**Motivation:** 在轨迹预测任务中，学习和表征智能体的主观性是一个关键挑战。这些主观性不仅具有特定的时空结构，而且对不同交互参与者来说是各向异性的。现有方法难以显式学习和预测这些主观性，更难通过特定自我的主观性来调节模型的预测结果。

**Method:** 受心理学前事实思维和戏剧概念的启发，该方法将未来轨迹中的主观性解释为从排练到返场的连续过程。在排练阶段，提出的自我预测器专注于学习如何从短期观测中为场景中的所有参与者推导和引导一组显式偏置的排练轨迹。这些排练轨迹作为即时控制条件来 Conditioning 最终预测。

**Result:** 实验证明了提出的 Encore 轨迹预测模型在不同数据集上都能提高性能，同时为主观性作为偏置的自我排练提供了清晰的可解释性。

**Conclusion:** 通过将智能体的主观性建模为排练到返场的过程，该方法成功实现了主观性的显式建模和预测，为轨迹预测任务提供了一种新颖且可解释的解决方案。
...
### [115] [Deep Probabilistic Unfolding for Quantized Compressive Sensing](https://arxiv.org/abs/2605.11475)
*Gang Qu,Ping Wang,Siming Zheng,Xin Yuan*

Main category: cs.CV

> 本文提出了一种深度概率展开模型，通过结合物理感知的梯度投影和双域Mamba模块，在量化压缩感知问题中实现了最先进的重建性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于展开的量化压缩感知方法使用L2投影处理测量值，无法准确反映量化过程的物理特性，限制了重建精度和实际应用。

**Method:** 1) 推导出闭合形式的数值稳定似然梯度投影，将硬量化约束转化为软概率引导；2) 设计高效的双域Mamba模块，动态捕获和融合多尺度局部与全局特征，实现远距离相关区域的交互；3) 基于展开框架构建深度概率模型。

**Result:** 在大量实验中，该方法在重建准确性和效率方面均超越了现有工作，展现出最先进的性能。

**Conclusion:** 所提出的深度概率展开模型通过物理感知的投影机制和特征融合设计，有效提升了量化压缩感知的重建质量，为其实际应用奠定了基础。
...
### [116] [LDDR: Linear-DPP-Based Dynamic-Resolution Frame Sampling for Video MLLMs](https://arxiv.org/abs/2605.11477)
*Jingfeng Chen,Jiawen Qian,Wendi Deng,Yinuo Guo,Jiaqi Yu,Sicong Leng,Raghuveer Thirukovalluru,Bhuwan Dhingra*

Main category: cs.CV

> LDDR是一个无需训练的视频帧采样框架，通过线性DPP算法和动态分辨率分配，在视觉标记预算有限的情况下显著提升视频理解性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态大语言模型在处理长视频时面临视觉标记预算限制，现有的均匀采样、相关性评分等方法要么忽略全局依赖关系，要么引入大量计算开销。

**Method:** 提出LDDR框架，首先在任务条件化特征空间中执行查询感知的DPP帧选择（比标准DPP快3倍），然后基于Group DPP重要性指标指导帧保留和动态分辨率分配，为信息丰富且非冗余的帧分配更多标记。

**Result:** 在四个涵盖短、中、长程视频的基准测试中，LDDR consistently优于次优基线，在预算受限条件下获得2.5分提升，在高预算场景下获得1.6分提升，改进在多个MLLM骨干网络上都得到验证。

**Conclusion:** LDDR作为一种训练免、即插即用的框架，有效解决了视频理解中的帧选择和预算分配问题，通过智能选择相关帧并合理分配计算资源，显著提升了多模态大语言模型的视频理解能力。
...
### [117] [A Mimetic Detector for Adversarial Image Perturbations](https://arxiv.org/abs/2605.11492)
*Johnny Corbino*

Main category: cs.CV

> 该论文提出了一种基于高阶Corbino-Castillo拟态算子的单次、免训练对抗样本检测器，能够有效识别ℓ∞边界攻击生成的对抗样本，无需重新训练或访问目标网络。

<details>
  <summary>Details</summary>

**Motivation:** 标准ℓ∞边界攻击（如FGSM、PGD）会在像素级别产生高频、接近随机的符号模式，这些模式在ℓ2范数下几乎不可见，但携带了不成比例的梯度能量，作者利用这一特性来检测对抗样本。

**Method:** 使用来自开源MOLE库的高阶Corbino-Castillo拟态算子作为检测器，该方法只需计算输入图像本身的空间特性，计算复杂度为O(HW)，无需重新训练、无需代理分类器、无需访问被攻击的网络。

**Result:** 在标准peppers测试图像上，当ℓ∞预算为16/255时，检测器在k=2到k6的阶数范围内，清洁样本与对抗样本的分离度从3.55倍单调增长到4.19倍，表现出良好的检测效果。

**Conclusion:** 该方法利用对抗攻击的高频噪声特性，实现了一种简单高效的检测方案，为对抗样本检测提供了新的思路，具有实际应用价值。
...
### [118] [STRIDE: Training-Free Diversity Guidance via PCA-Directed Feature Perturbation in Single-Step Diffusion Models](https://arxiv.org/abs/2605.11494)
*Ankit Yadav,Arpit Garg,Ta Duc Huy,Lingqiao Liu*

Main category: cs.CV

> STRIDE是一种无需训练和优化的方法，通过在模型自身激活的主成分上注入空间相干噪声，显著提升了单步或少步扩散模型生成样本的多样性，同时保持保真度。

<details>
  <summary>Details</summary>

**Motivation:** 单步或少步扩散模型虽然能实现实时图像生成，但相比多步模型样本多样性较差。传统的多样性增强机制（如调度、轨迹、迭代优化）在少步/单步设置下不可用，而朴素的特征扰动方法效果有限。

**Method:** 提出STRIDE方法，将空间相干（粉色）噪声投影到模型自身激活的主成分上，对中间Transformer特征进行扰动，确保扰动位于学习到的特征流形内，在表示空间中有意义方向上实现可控变化。

**Result:** 在FLUX.1-schnell和SD3.5 Turbo上，STRIDE在多个基准测试中持续提升多样性同时保持强文本对齐，在多样性-保真度前沿上Pareto主导现有基线。

**Conclusion:** 在缺乏迭代优化的情况下，提升少步和单步扩散模型多样性的关键不在于增加扰动强度，而在于使扰动与模型内部表示结构对齐。
...
### [119] [PoseBridge: Bridging the Skeletonization Gap for Zero-Shot Skeleton-Based Action Recognition](https://arxiv.org/abs/2605.11497)
*Sanghyeon Lee,Jinwoo Kim,Jong Taek Lee*

Main category: cs.CV

> PoseBridge是一种零样本骨架动作识别框架，通过在姿态估计过程中提取语义特征来解决上游语义损失问题

<details>
  <summary>Details</summary>

**Motivation:** 传统的骨架-文本对齐方法存在上游语义损失问题。骨架作为人体姿态估计的压缩输出，在对齐开始时已经丢失了人机交互和姿态相关的视觉线索，导致零样本动作识别性能受限。

**Method:** 提出PoseBridge框架，在同一个人体姿态估计过程中提取基于姿态锚定的语义线索，通过骨架条件桥接和语义原型自适应机制将这些语义特征传递给骨架-文本对齐过程，无需额外的RGB动作分支或目标检测器。

**Result:** 在NTU-RGB+D 60/120、PKU-MMD和Kinetics-200/400数据集上均提升了零样本骨架动作识别性能。特别是在Kinetics-200/400 PURLS基准上的表现最为突出，在全部八个分割上相比最强基线提升了13.3-17.4分。

**Conclusion:** 通过在姿态估计的中间阶段获取语义信息并有效传递给下游对齐任务，PoseBridge成功解决了上游语义损失问题，为零样本骨架动作识别提供了新的解决方案。
...
### [120] [Principled Design of Diffusion-based Optimizers for Inverse Problems](https://arxiv.org/abs/2605.11506)
*Julio Oscanoa,Irmak Sivgin,Cagan Alkan,Daniel Ennis,John Pauly,Mert Pilanci,Shreyas Vasanawala*

Main category: cs.CV

> 本文提出了基于分数的扩散模型改进方法OptDiff，通过引入重参数化实现超参数跨任务不变性，并结合优化工具显著加速逆问题推理过程。

<details>
  <summary>Details</summary>

**Motivation:** 基于分数的扩散模型在逆问题上表现优异，但推理时间长且超参数调整繁琐。现有方法需要为每个问题单独调整噪声调度和后验采样权重等推理时超参数，这限制了模型的实际部署效率。

**Method:** 提出了两个关键创新：1）引入具有不变性的重参数化方法，使相同超参数可跨多个问题复用；2）基于RED-diff框架开发OptDiff流水线，将后验采样重构为优化问题，简化调优框架并集成凸优化工具。

**Result:** 在图像重建、去模糊和超分辨率任务上的实验表明，该方法实现了推理速度的大幅提升和图像质量的改善。

**Conclusion:** 通过系统性的重参数化和优化框架设计，OptDiff有效解决了扩散模型在逆问题应用中的推理效率和调优复杂性问题，为实际部署提供了实用解决方案。
...
### [121] [LiBrA-Net: Lie-Algebraic Bilateral Affine Fields for Real-Time 4K Video Dehazing](https://arxiv.org/abs/2605.11508)
*Yongcong Wang,Chengchao Shen,Guangwei Gao,Wei Wang,Pengwen Dai,Dianjie Lu,Guijuan Zhang,Zhuoran Zheng*

Main category: cs.CV

> 这篇论文提出了LiBrA-Net网络和UHV-4K基准数据集，解决了超高清视频去雾领域缺乏评估基准和现有方法无法在消费级GPU实时运行的两大问题。

<details>
  <summary>Details</summary>

**Motivation:** 超高清视频去雾领域存在两大痛点：一是缺乏统一的评估基准，二是现有连续UHD视频去雾方法无法在消费级GPU上有效运行。

**Method:** 基于大气去雾可简化为低频深度场支配的逐像素仿射变换这一观察，提出LiBrA-Net：将时空仿射场分解为空间-颜色和时间双边子网格，在固定低分辨率下预测系数，在gl(3)李代数下融合并施加群论正则化，通过Cayley参数化映射为可逆GL(3)变换，通过轻量级输入引导分支恢复高频细节。

**Result:** 在UHV-4K、REVIDE和HazeWorld数据集上，LiBrA-Net在相比的视频去雾方法中达到新的最先进水平，能够在单GPU上以25 FPS处理原生4K视频，仅需6.12M参数。

**Conclusion:** 该工作通过理论创新和工程优化，实现了UHD视频去雾的实时处理，填补了领域空白，为后续研究提供了重要的基准和方法学基础。
...
### [122] [PointGS: Semantic-Consistent Unsupervised 3D Point Cloud Segmentation with 3D Gaussian Splatting](https://arxiv.org/abs/2605.11520)
*Yixiao Song,Qingyong Li,Wen Wang,Zhicheng Yan*

Main category: cs.CV

> PointGS是一种无监督3D点云分割方法，利用3D高斯泼洒作为统一中间表示，通过SAM提取2D语义并蒸馏到3D高斯原语，解决了2D-3D模态不匹配问题。

<details>
  <summary>Details</summary>

**Motivation:** 无监督点云分割对 embodied AI 和自动驾驶至关重要，可避免密集标注的高成本。现有方法通过集成2D预训练模型（如SAM）补充语义信息，但面临离散3D点与连续2D图像的根本不匹配，导致投影重叠和语义一致性问题。

**Method:** PointGS采用3D高斯泼洒作为统一中间表示：1) 通过多视角观测将稀疏点云重建为密集3D高斯空间，填充空间间隙并编码遮挡关系；2) 从高斯空间渲染多视角密集图像，用SAM提取2D语义掩码；3) 通过对比学习将语义蒸馏到3D高斯原语；4) 通过两步注册对齐高斯空间与原始点云，基于最近邻搜索分配点语义。

**Result:** 在ScanNet-V2和S3DIS数据集上，PointGS分别实现了+0.9%和+2.8%的mIoU提升，显著优于现有最先进无监督方法。

**Conclusion:** PointGS通过3D高斯泼洒成功桥接了离散-连续域差距，有效解决了2D-3D语义迁移中的不一致性问题，为无监督点云分割提供了简单而有效的解决方案。
...
### [123] [XWOD: A Real-World Benchmark for Object Detection under Extreme Weather Conditions](https://arxiv.org/abs/2605.11521)
*Chih-Hsin Chen,Yu-Tung Liu,Amar Fadillah,Kuan-Ting Lai,Dong Liu*

Main category: cs.CV

> XWOD是一个大规模真实世界的极端天气交通目标检测基准数据集，包含10,010张图像和7种极端天气条件，验证了其在提升自动驾驶系统天气鲁棒性方面的有效性。

<details>
  <summary>Details</summary>

**Motivation:** 自动驾驶和智能交通系统在极端天气条件下仍然存在脆弱性，每年造成大量交通事故和人员伤亡，但现有的天气鲁棒性评估数据集在规模、多样性和真实性方面都有限制。

**Method:** 创建了XWOD数据集，涵盖7种极端天气条件（雨、雪、雾、霾/沙尘、洪水、龙卷风、野火）和6个交通目标类别，使用YOLO系列检测器进行训练和零样本测试来评估数据质量。

**Result:** 在RTTS、DAWN和WEDGE等外部天气基准上，基于XWOD训练的检测器分别获得63.00%、59.94%和61.12%的mAP50分数，相比现有基线分别提升56%、83%和35%。

**Conclusion:** XWOD为学习天气鲁棒的交通感知提供了强大的源域，验证了其在提升自动驾驶系统极端天气条件下目标检测能力的有效性，并公开了数据集、代码和模型权重供研究使用。
...
### [124] [GeoR-Bench: Evaluating Geoscience Visual Reasoning](https://arxiv.org/abs/2605.11541)
*Yushuo Zheng,Zicheng Zhang,Huiyu Duan,Chunyi Li,Zijian Chen,Ziheng Jia,Yue Shi,Ke Gu,Xiongkuo Min,Guangtao Zhai*

Main category: cs.CV

> 研究人员提出了GeoR-Bench基准测试，用于评估地球科学视觉推理能力。测试结果显示，当前AI模型在地球科学推理方面存在显著瓶颈，最佳模型仅获得42.7%的严格准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有地球科学AI研究多为任务特定，无法捕捉开放式的现实世界问题，缺乏对真正地球科学智能能力的全面评估。

**Method:** 构建GeoR-Bench基准，包含440个样本，涵盖6个地球科学类别和24种任务类型，通过视觉编辑任务评估模型的推理、一致性和质量三个维度。

**Result:** 对21个多模态模型的测试表明，地球科学推理是关键瓶颈，最高准确率仅42.7%，开源模型仅10.3%，模型生成结果在视觉一致性和图像质量上往往优于科学准确性。

**Conclusion:** 当前AI模型能够生成表面上合理的结果，但无法捕捉底层的地球科学过程，距离真正的地球科学智能仍有很大差距。
...
### [125] [The DAWN of World-Action Interactive Models](https://arxiv.org/abs/2605.11550)
*Hongbo Lu,Liang Yao,Chenghao He,Haoyu Wang,Xiang Gu,Xianfei Li,Wenlong Liao,Tao He,Pai Peng*

Main category: cs.CV

> DAWN模型通过世界-行动交互机制，在潜在空间中递归地细化世界预测和行动生成，在自动驾驶任务中实现了强大的规划性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有世界动作模型(WAMs)忽略了场景演化和动作决策之间的相互依赖关系，将两者视为独立的并行分支或刚性预测-规划流程，这限制了模型的决策质量。

**Method:** 提出世界-行动交互模型(WAIMs)框架，具体实现为DAWN模型。在紧凑语义潜在空间中，将世界预测器与世界条件动作去噪器耦合：预测的世界假设条件化动作去噪，同时去噪后的动作假设反馈更新世界预测，实现递归细化。

**Result:** DAWN在多个自动驾驶基准测试中取得了强大的规划性能和良好的安全相关结果，能够在复杂交互场景中支持长时程轨迹生成。

**Conclusion:** 交互式世界-行动生成是构建真正可行动世界模型的有效路径，DAWN验证了这种 principled 方法的有效性。
...
### [126] [ScribbleDose: Scribble-Guided Dose Prediction in Radiotherapy](https://arxiv.org/abs/2605.11555)
*Zhenxi Zhang,Yitao Zhuang,Yao Pu,Peixin Yu,Zirong Li,Yan Xia,Hui Li,Bin Li,Fuchen Zheng,Ge Ren*

Main category: cs.CV

> ScribbleDose是一种基于稀疏涂鸦标注的放疗剂量预测框架，通过自动补全解剖结构掩码，实现了仅使用少量标注即可获得准确的剂量预测效果。

<details>
  <summary>Details</summary>

**Motivation:** 传统的放疗剂量预测需要精确的手动解剖结构标注，这一过程耗时费力且需要专业知识。为了解决标注成本高的问题，研究者提出使用稀疏涂鸦替代完整标注，从而大幅降低标注工作量。

**Method:** 该方法包含两个核心模块：1) 涂鸦补全模块(SCM)通过超体素正则化将稀疏涂鸦传播到语义相似的体素，生成密集的解剖结构掩码；2) 结构引导剂量生成模块(SGDGM)利用补全的掩码作为结构指导，加强结构线索与剂量分布的对应关系。整体形成涂鸦-掩码-剂量的学习流水线。

**Result:** 在GDP-HMM数据集上的实验表明，ScribbleDose仅使用稀疏结构标注就能达到与传统方法相当的剂量预测性能，证明了方法的有效性。

**Conclusion:** 该研究成功地将稀疏涂鸦标注引入放疗剂量预测领域，显著降低了标注成本，同时保持了预测精度，为临床应用提供了一种实用的解决方案。代码和标注已公开，便于后续研究使用。
...
### [127] [When Looking Is Not Enough: Visual Attention Structure Reveals Hallucination in MLLMs](https://arxiv.org/abs/2605.11559)
*Fanpu Cao,Xin Zou,Xuming Hu,Hui Xiong*

Main category: cs.CV

> LaSCD是一种基于拉普拉斯能量的无训练解码策略，通过分析视觉注意力的层间高频结构来减少多模态大语言模型的视觉幻觉，同时保持模型的通用能力。

<details>
  <summary>Details</summary>

**Motivation:** 多模态大语言模型(MLLMs)容易出现视觉幻觉现象，即生成的回答与图像内容矛盾或提到不存在的对象。传统的视觉注意力机制无法完全解释这种现象，因为模型可能仍然对图像token分配大量注意力，但内部却得出错误答案。

**Method:** 通过层间拉普拉斯能量度量视觉注意力的高频结构，发现幻觉偏好出现的层和真实答案短暂恢复的层。基于此提出LaSCD（拉普拉斯频谱对比解码）方法，通过拉普拉斯能量选择信息丰富的层，并以闭式形式重映射下一token的logits。

**Result:** 在幻觉和通用多模态基准测试上的实验表明，LaSCD能够一致地减少幻觉现象，同时保持模型的通用能力，证明其作为一种忠实解码范式的潜力。

**Conclusion:** 该研究揭示了视觉注意力的层间高频结构在理解和解决MLLMs幻觉中的关键作用，LaSCD作为一种简单有效的无训练方法，为提高多模大语言模型的可靠性提供了新的技术路径。
...
### [128] [TCP-SSM: Efficient Vision State Space Models with Token-Conditioned Poles](https://arxiv.org/abs/2605.11563)
*Sara Shoouri,Morteza Tavakoli Taba,Hun-Seok Kim*

Main category: cs.CV

> TCP-SSM是一种通过显式控制稳定极点来改进状态空间模型效率的结构化框架，在保持线性复杂度的同时，大幅降低计算成本并提升长程视觉任务性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有高效SSM变体主要通过修改扫描路径等方式降低计算成本，但保持隐式递归动态，使得模型的状态相关记忆行为难以控制，特别是在紧凑骨干网络中长扫描路径可能超出有效记忆范围。

**Method:** 提出基于极点的结构化选择性SSM框架TCP-SSM，使用实极点建模单调或符号交替衰减，复共轭极点捕获阻尼振荡响应，通过有界半径和角度调制将共享基极点转换为token依赖极点，并集成分组极点共享与轻量低秩输入路径。

**Result:** 在图像分类、语义分割和目标检测任务中，TCP-SSM将Vision Mamba风格模型的SSM计算复杂度降低44%，同时保持或超越基准准确率。

**Conclusion:** TCP-SSM通过显式且可解释的极点控制实现了高效的状态空间建模，为长程视觉任务提供了计算效率优化的新解决方案。
...
### [129] [Dynamic Execution Commitment of Vision-Language-Action Models](https://arxiv.org/abs/2605.11567)
*Feng Chen,Xianghui Wang,Yuxuan Chen,Boying Li,Yefei He,Zeyu Zhang,Yicheng Wu*

Main category: cs.CV

> A3是一种自适应动作接受机制，通过将动态执行承诺重新定义为自投机前缀验证问题，根据动作预测的可靠性动态调整执行范围，无需人工调参即可在执行鲁棒性和推理吞吐量之间实现更好的平衡。

<details>
  <summary>Details</summary>

**Motivation:** 现有的VLA模型采用固定的动作块大小进行执行，这种方式忽略了预测可靠性在不同状态下的变化特性，在动态或分布外场景中表现脆弱，需要针对每个任务手动调整执行范围。

**Method:** A3机制通过群采样计算动作级别的共识分数，然后执行两个关键验证：(1)共识排序的条件不变性，通过高共识动作验证低共识动作的一致性；(2)前封闭序列一致性，只接受从开始的、通过验证的最长连续动作序列。

**Result:** 在多个VLA模型和基准测试上的实验表明，A3消除了手动范围调优的需求，在执行鲁棒性和推理吞吐量之间取得了卓越的平衡。

**Conclusion:** A3将执行范围重新定义为满足内部模型逻辑和序列执行约束的最长可验证前缀，使系统能够根据预测的可靠性动态调整执行策略，显著提升了VLA模型在复杂环境下的适应性。
...
### [130] [TB-AVA: Text as a Semantic Bridge for Audio-Visual Parameter Efficient Finetuning](https://arxiv.org/abs/2605.11572)
*Seongah Kim,Dinh Phu Tran,Hyeontaek Hwang,Saad Wazir,Duc Do Minh,Daeyoung Kim*

Main category: cs.CV

> 本文提出了一种基于文本桥接的音视频适配器(TB-AVA)框架，利用文本作为语义锚点，通过门控语义调制技术实现高效的音视听觉表征学习。

<details>
  <summary>Details</summary>

**Motivation:** 音视听觉理解中存在关键挑战：即使音频和视觉信号在时间上对齐，它们之间往往缺乏明确的语义对应关系，导致跨模态对齐困难。

**Method:** 构建基于冻结音频和视觉编码器的参数高效适应框架，通过TB-AVA适配器实现文本介导的音视交互，核心是门控语义调制(GSM)机制，根据文本推断的语义相关性选择性地调制特征通道。

**Result:** 在AVE、AVS和AVVP等多个基准测试中达到最先进性能，证明了文本作为语义锚点在参数高效微调中的有效性。

**Conclusion:** 文本作为语义锚点为音视听觉学习提供了有效的语义 bridge，TB-AVA框架展示了参数高效适配在跨模态理解任务中的巨大潜力。
...
### [131] [The Midas Touch for Metric Depth](https://arxiv.org/abs/2605.11578)
*Yu Ma,Zizhan Guo,Zuyi Xiong,Haoran Zhang,Yi Feng,Hongbo Zhao,Hanli Wang,Rui Fan*

Main category: cs.CV

> MTD是一种将相对深度转换为度量深度的方法，仅需极稀疏的3D数据，通过分段恢复策略和像素级精炼策略解决了尺度不一致等问题，具有强泛化性和高效率。

<details>
  <summary>Details</summary>

**Motivation:** 尽管相对深度估计的跨场景泛化能力已显著改善，但其缺乏度量尺度、存在局部不一致性以及计算效率较低等问题限制了其实际应用。

**Method:** MTD采用数学可解释的方法，仅使用极稀疏的3D数据将相对深度转换为度量深度。它通过稀疏图优化应用分段恢复策略消除局部尺度不一致，然后使用不连续感知的测地线成本进行像素级精炼。

**Result:** MTD展现出强大的泛化能力，在精度上大幅超越了以往的深度补全和深度估计方法。其轻量级、即插即用的设计便于在多样化的下游3D任务中部署和集成。

**Conclusion:** MTD通过创新的分段恢复和像素级精炼策略，有效解决了相对深度估计的关键局限性，为实际3D应用提供了高效、准确的度量深度解决方案。
...
### [132] [A Mixture Autoregressive Image Generative Model on Quadtree Regions for Gaussian Noise Removal via Variational Bayes and Gradient Methods](https://arxiv.org/abs/2605.11585)
*Shota Saito,Yuta Nakahara,Kohei Horinouchi,Naoki Ichijo,Manabu Kobayashi,Toshiyasu Matsushima*

Main category: cs.CV

> 本文提出了一种基于四叉树区域划分和混合自回归模型的概率生成模型，通过变分下界最大化框架实现灰度图像去噪。

<details>
  <summary>Details</summary>

**Motivation:** 针对灰度图像去噪问题，需要构建有效的图像生成模型和优化框架来提高去噪效果和计算效率。

**Method:** 结合四叉树区域划分模型与混合自回归模型构建概率生成模型，将MAP估计去噪转化为变分下界最大化问题，采用变分贝叶斯和梯度方法交替优化，梯度更新规则可解析计算。

**Result:** 实验验证了算法能够有效去除图像噪声，并明确了未来改进方向。

**Conclusion:** 提出的新型概率生成模型和变分优化框架为灰度图像去噪提供了有效解决方案，具有进一步优化潜力。
...
### [133] [Logit-Attention Divergence: Mitigating Position Bias in Multi-Image Retrieval via Attention-Guided Calibration](https://arxiv.org/abs/2605.11591)
*Mingtao Xian,Yifeng Yang,Qinying Gu,Xinbing Wang,Nanyang Ye*

Main category: cs.CV

> 本文发现多模态大语言模型存在显著位置偏差，提出了一种基于注意力引导的训练无关去偏方法，显著提升了模型对输入顺序的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 多模态大语言模型在多图像跨模态检索任务中虽然表现良好，但存在严重的位置偏差问题，即预测结果主要受输入顺序影响而非语义相关性，这限制了模型的实际应用价值。

**Method:** 通过实证分析发现了Logit-Attention Divergence现象，即输出logit严重偏离而内部注意力图依然与相关视觉证据对齐。基于此发现，提出了一个训练免费的注意力引导去偏框架，在推理时利用内在注意力信号进行实例级校正，仅需最小化的校准集且计算开销可忽略。

**Result:** 在基于MS-COCO的基准测试中，该方法显著提升了排列不变性，相比基线模型准确率提升超过40%，达到了最先进的性能表现。

**Conclusion:** 该研究揭示了现有logit级校准方法的根本局限性，提出的注意力引导去偏框架为解决MLLM位置偏差问题提供了有效且实用的解决方案，为多模态模型的鲁棒性提升开辟了新的研究方向。
...
### [134] [PointForward: Feedforward Driving Reconstruction through Point-Aligned Representations](https://arxiv.org/abs/2605.11594)
...
> PointForward是一种通过点对齐表示的前向传播驾驶场景重建框架，解决了现有3D高斯泼溅方法的多视角不一致和动态物体处理问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的前向3D高斯泼溅方法存在多视角不一致和分层伪影问题，同时通过密集流预测处理动态实例缺乏显式的跨视角对应关系和实例级一致性，这些限制影响了自动驾驶场景的高保真重建。

**Method:** 提出PointForward框架，在世界空间中初始化稀疏3D查询点，通过时空融合将多视图图像信息聚合到这些查询点上，强制实现单次前向传播的显式跨视角一致性。引入场景图显式组织运动实例，利用3D边界框实现实例级运动传播和时间一致的动态表示。

**Result:** 在大型驾驶基准测试上实现了最先进的性能，验证了该方法在驾驶场景重建中的有效性。

**Conclusion:** 通过点对齐表示和场景图组织，PointForward成功解决了现有方法在多视角一致性和动态实例处理方面的不足，为自动驾驶场景重建提供了新的解决方案。
...
### [135] [HorizonDrive: Self-Corrective Autoregressive World Model for Long-horizon Driving Simulation](https://arxiv.org/abs/2605.11596)
...
> HorizonDrive是一个抗漂移的驾驶仿真框架，通过让教师模型具备自回归展开能力，实现了有界内存下的无限时程监督，在分钟级驾驶仿真中显著提升生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自回归蒸馏方法在驾驶场景中存在局限性：帧汇聚方法因快速自运动和场景变化而迁移效果差，而学生侧退化训练受限于教师的单次输出长度，只能提供有限的监督时程。

**Method:** HorizonDrive包含两个关键技术：1) 调度展开恢复(SRR)训练基础模型从预测损坏的历史中重建真实未来片段，获得在长距离自回归展开中保持稳定的教师模型；2) 通过自回归展开扩展教师模型，在bounded memory下提供长时程分布匹配监督，学生通过教师展开DMD(TRD)进行对齐。

**Result:** 在nuScenes数据集上，HorizonDrive在minute-scale自回归展开下，FID降低52%，FVD降低37%，ARE和DTW分别降低21%和9%，同时与单次驾驶视频生成器保持竞争力。

**Conclusion:** HorizonDrive成功解决了教师模型在自回归展开中的漂移问题，实现了无需帧汇聚的长时程监督，为实时闭环驾驶仿真提供了有效解决方案。
...
### [136] [Keep What Audio Cannot Say: Context-Preserving Token Pruning for Omni-LLMs](https://arxiv.org/abs/2605.11605)
*Chaeyoung Jung,Kyeongha Rho,Joon Son Chung*

Main category: cs.CV

> ContextGuard是一种推理时token剪枝框架，通过保留广泛视听上下文同时移除跨模态冗余，显著降低Omni-LLMs计算开销，无需下游模型微调。

<details>
  <summary>Details</summary>

**Motivation:** 现有Omni-LLMs的token剪枝方法主要关注当前查询重要性或跨模态对齐度，可能丢弃对其他问题必要的证据，限制了模型的上下文理解能力。

**Method:** ContextGuard通过从音频预测粗粒度视觉语义，剪枝可从音频恢复的视频token，同时保留音频无法指定的局部视觉细节token，并合并时间相似的video tokens，使用独立训练的轻量级预测器实现。

**Result:** 在Qwen2.5-Omni和Video-SALMONN2+的3B和7B模型上，ContextGuard在六个视听基准测试中超越现有推理时剪枝方法，其中Qwen2.5-Omni 7B在剪枝55%输入token的情况下，在五个基准测试中达到全token级别性能。

**Conclusion:** ContextGuard成功解决了现有方法的问题，通过保留广泛上下文而非仅关注特定查询，实现了更高效率的token剪枝，为Omni-LLMs的实际部署提供了有效解决方案。
...
### [137] [Grounding by Remembering: Cross-Scene and In-Scene Memory for 3D Functional Affordances](https://arxiv.org/abs/2605.11616)
*Qirui Wang,Jingyi He,Yining Pan,Xulei Yang,Shijie Li*

Main category: cs.CV

> AFFORDMEM是一个无需微调的3D功能可用性定位框架，通过跨场景和场景内两层记忆机制来精确定位物体可操作区域。

<details>
  <summary>Details</summary>

**Motivation:** 现有的免训练视觉语言管道在功能可用性定位上存在困难，因为可操作区域通常很小、视觉上模糊不清，且在同一场景的多个同类实例中重复出现，导致通过纯文本提示难以精确定位。

**Method:**  AFFORDMEM框架采用双层记忆机制：1) 跨场景可用性记忆，维护带有可用性区域覆盖的RGB图像的类别级记忆库，在查询时召回最有信息量的示例来引导冻结的VLM定位小型可操作子区域；2) 场景内空间记忆，将候选实例及其3D空间关系组织成结构化场景图，使语言模型能够解决对远距离或当前未观察到的候选对象的引用。

**Result:** 在SceneFun3D数据集上，该方法相比之前的免训练最优方法在Split 0上提升了3.23 AP50，在Split 1上提升了3.7 AP50。消融实验证明两种记忆机制的互补效益：跨场景记忆改善细粒度定位，场景内空间记忆在空间限定查询上提供更大增益。

**Conclusion:** AFFORDMEM通过引入记忆机制成功解决了免训练框架中功能可用性定位的挑战，无需模型微调或目标场景标注，具有很好的实用性和可扩展性。
...
### [138] [RNA-FM: Flow-Matching Generative Model for Genome-wide RNA-Seq Prediction](https://arxiv.org/abs/2605.11622)
*Yaxuan Song,Jianan Fan,Tianyi Wang,Qiuyue Hu,Hang Chang,Heng Huang,Weidong Cai*

Main category: cs.CV

> RNA-FM是一种基于流匹配生成框架的创新方法，能够从组织病理学全切片图像准确预测全基因组范围的表达谱，并通过整合通路结构实现了可扩展性和生物学可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 组织病理学WSI图像包含丰富的组织形态信息但缺乏分子架构信息，而RNA测序能提供全基因组转录谱但成本高昂。现有的确定性回归方法无法捕获生物学异质性和预测不确定性，限制了其实际应用价值。

**Method:** RNA-FM将转录组预测构建为连续时间条件输运问题，学习一个从简单先验分布到目标基因表达分布的速度场，该映射以WSI形态为条件。通过整合通路水平结构，实现了可扩展和生物学可解释的基因组范围基因表达插补。

**Result:** 大量实验证明RNA-FM在保持生物学意义的同时，持续优于现有最先进方法，为WSI到RNA-seq的预测提供了更准确可靠的解决方案。

**Conclusion:** 该研究为从组织病理学图像预测分子表型提供了新的范式，通过生成式方法克服了确定性回归的局限性，为临床实践中经济高效地获取转录组信息开辟了新途径。
...
### [139] [Single-Shot HDR Recovery via a Video Diffusion Prior](https://arxiv.org/abs/2605.11628)
*Chinmay Talegaonkar,Jinshi He,Christopher McKenna,Nicholas Antipa*

Main category: cs.CV

> 本文将单次HDR重建重构为条件视频生成问题，通过微调视频扩散模型生成曝光序列，然后用轻量级UNet预测的像素权重融合，既保持了输入保真度又提高了质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的单次HDR生成方法在保持输入图像保真度方面存在困难，需要分别处理高光和阴影的不同模型，或者直接预测最终HDR图像而牺牲了可解释性。

**Method:** 将单次HDR重建重新表述为条件视频生成，微调视频扩散模型根据LDR输入生成曝光序列，使用轻量级UNet预测的像素权重将这些帧融合为HDR图像。

**Result:** 在定量基准测试中，以相当的模型容量在多个重建指标上优于最先进的生成基线，在人工评估中72%的比较中偏好本方法结果，并成功扩展到全焦图像恢复等其他重建任务。

**Conclusion:** 这种条件序列生成和融合框架简单、可解释且有效，消除了不同曝光 regimes 中对分离模型的需求，同时产生高输入保真度的HDR重建，具有良好的通用性。
...
### [140] [Unlocking UML Class Diagram Understanding in Vision Language Models](https://arxiv.org/abs/2605.11634)
*Artem Naboichenko,René Peinl*

Main category: cs.CV

> 该研究提出了一个基于UML类图的视觉问答基准测试，构建了包含16,000个样本的训练数据集，并证明LoRA微调方法在UML问答任务上显著优于Qwen 3.5 27B模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言模型在处理图表问答任务时表现不佳，特别是在计算机科学领域的UML类图等非传统图表类型上，相关研究很少，需要专门的基准测试和训练数据。

**Method:** 研究人员创建了一个针对UML类图的视觉问答基准测试，构建了大规模训练数据集（16,000个图像-问题-答案三元组），并采用LoRA方法对模型进行微调。

**Result:** LoRA微调的模型在UML类图问答任务上轻松超越了Qwen 3.5 27B这一在许多其他基准测试中表现优异的视觉语言模型。

**Conclusion:** 该工作填补了UML类图视觉问答研究的空白，证明了专门针对该领域进行微调的有效性，为计算机科学领域的图表理解提供了新的解决方案。
...
### [141] [Hide to See: Reasoning-prefix Masking for Visual-anchored Thinking in VLM Distillation](https://arxiv.org/abs/2605.11651)
*Seonghoon Yu,Dongjun Nam,Byung-Kwan Lee,Jeany Son*

Main category: cs.CV

> 本文提出了一种新的思考-答案蒸馏框架，通过掩码学生模型的显著性推理前缀，迫使其在推理过程中更多依赖视觉证据，从而在保持较低计算成本的同时提升了小型VLM的推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有VLM中如Qwen3-VL-Thinking等思考-答案方法虽然通过中间推理步骤提升了性能，但计算成本过高限制了实际部署。需要将这些能力蒸馏到更紧凑的模型中，核心是提升学生对视觉证据的利用能力。

**Method:** 提出基于显著性推理前缀掩码的蒸馏框架：1) 逐词显著性推理前缀掩码，对每个next-token预测选择性掩码高影响力推理前缀；2) 自步掩码预算调度，根据师生分布差异逐渐增加掩码规模。蒸馏中使用显著性推理前缀掩码替代标准因果掩码。

**Result:** 实验表明该方法在多模态推理基准上超越了近期开源VLM、VLM蒸馏和自蒸馏方法。进一步分析证实了学生在思考过程中的视觉利用能力得到了增强。

**Conclusion:** 通过掩码显著性推理前缀迫使学生更多依赖视觉证据的方法，能够有效蒸馏大型思考-答案VLM的能力到紧凑模型中，在保持计算效率的同时显著提升了多模态推理性能和视觉利用能力。
...
### [142] [Weather-Robust Cross-View Geo-Localization via Prototype-Based Semantic Part Discovery](https://arxiv.org/abs/2605.11654)
*Chi-Nguyen Tran,Dao Sy Duy Minh,Huynh Trung Kiet,Nguyen Lam Phu Quy,Phu-Hoa Pham,Long Tran-Thanh*

Main category: cs.CV

> SkyPart是一个轻量级的跨视图地理定位方法，通过在ViT上添加可交换的分组头部，实现了布局与纹理分离、高度不变性和多目标平衡优化，在多个基准测试中达到了SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 跨视图地理定位在GNSS信号失效时对无人机自主导航至关重要，但现有方法存在三个主要限制：全局描述符无法分离布局与纹理、高度相关的尺度变化未被边缘化、多目标训练依赖手工调整的损失权重。

**Method:** SkyPart包含四个核心组件：(1)通过单次余弦分配的竞争性可学习原型分组patch token；(2)仅在训练时应用高度条件线性调制，使检索嵌入在推理时高度无关；(3)对活动原型进行图注意力读取；(4)基于Kendall不确定性的多目标损失函数，确保达到帕累托稳定点。

**Result:** SkyPart仅用26.95M参数和22.14 GFLOPs就成为性能最佳方法中最小的，在SUES-200、University-1652和DenseUAV数据集上创下新的SOTA记录，且在WeatherPrompt十种天气扰动条件下优势更加明显。

**Conclusion:** SkyPart通过理论指导的轻量级设计有效解决了CVGL领域的三个关键挑战，为无人机导航提供了高效鲁棒的跨视图地理定位方案，其在极端天气条件下的优异表现证明了其强大的泛化能力。
...
### [143] [Reviving In-domain Fine-tuning Methods for Source-Free Cross-domain Few-shot Learning](https://arxiv.org/abs/2605.11659)
*Yaze Zhao,Yicong Liu,Yixiong Zou,Yuhua Li,Ruixuan Li*

Main category: cs.CV

> 本文提出了语义探测(Semantic Probe)框架，解决了跨域少样本学习中CLIP模型适配器方法优于提示方法的反常现象，通过修正视觉CLS标记的注意力崩溃问题，显著提升了跨域性能。

<details>
  <summary>Details</summary>

**Motivation:** 跨域少样本学习中，大规模预训练模型向专业领域适配时样本有限。研究发现，在CLIP模型的跨域微调中，适配器方法(如LoRA)始终优于提示方法(如MaPLe)，这与域内场景相反，需要深入分析原因以提升跨域性能。

**Method:** 通过分析发现LoRA的优势来自于修正了视觉CLS标记的注意力崩溃，增强了模态对齐和类别分离；还发现文本EOS标记对视觉样本注意力更好，且CLIP的对比损失对模态对齐约束较弱。基于这些发现，提出了即插即用的注意力修正框架Semantic Probe。

**Result:** 在四个跨域少样本基准测试上的广泛实验验证了理论合理性，达到了最先进的性能，同时惠及了适配器和提示两种微调范式。

**Conclusion:** 该研究揭示了跨域场景下CLIP微调的关键机制，提出的Semantic Probe框架有效解决了注意力崩溃问题，为跨域少样本学习提供了新的解决方案，代码将公开发布。
...
### [144] [ShapeCodeBench: A Renewable Benchmark for Perception-to-Program Reconstruction of Synthetic Shape Scenes](https://arxiv.org/abs/2605.11680)
*Shivam Kumar*

Main category: cs.CV

> ShapeCodeBench是一个用于感知到程序重建任务的综合基准测试，要求模型从渲染图像生成可执行的绘图程序，当前多模态模型在该任务上表现仍有限。

<details>
  <summary>Details</summary>

**Motivation:** 为了评估模型从视觉感知生成精确可执行程序的能力，解决现有基准测试可能存在的精确实例污染问题，推动多模态模型在程序合成任务上的发展。

**Method:** 设计了包含四种绘图原语的DSL系统，使用种子随机数生成器创建合成数据集，提供150个测试样本并采用多种评估指标（精确匹配、像素准确率、前景IoU等），测试了启发式方法和多个先进模型的表现。

**Result:** 传统启发式方法在简单场景表现良好但复杂场景中失效，最强模型（Claude Opus）能保持前景结构但仍有参数误差导致精确匹配率低，整体基准测试远未饱和。

**Conclusion:** ShapeCodeBench为感知到程序重建任务提供了可重现的评估平台，通过合成数据避免污染，当前模型性能仍有很大提升空间，为后续研究提供了重要基准。
...
### [145] [DORA: Dynamic Online Reinforcement Agent for Token Merging in Vision Transformers](https://arxiv.org/abs/2605.11683)
*Kaixuan He,Song Chen,Yi Kang*

Main category: cs.CV

> DORA是首个基于强化学习的动态在线推理框架，用于ViT中的自适应token合并，实现了显著的计算效率提升。

<details>
  <summary>Details</summary>

**Motivation:** 传统ViT的自注意力机制存在二次复杂度问题，现有的token缩减方法依赖固定启发式规则和静态离线索引，缺乏对输入相关冗余的自适应能力。

**Method:** 将token合并过程构建为序列马尔可夫决策过程，使用轻量级RL代理基于当前特征状态和层特定上下文动态决定合并策略，采用非线性蒸馏惩罚的密集奖励函数进行优化，设计了非对称Actor-Critic架构。

**Result:** 在多个ViT规模上，DORA在准确率下降≤0.05%的约束下达到12.66%的合并率，比最有效基线提升569.7%，在ImageNet-1K上比现有方法提升76%的计算节省，在OOD基准上效率优势超过430%。

**Conclusion:** DORA成功解决了现有方法的静态限制问题，实现了输入自适应的动态token合并，在保持精度的同时显著提升了ViT的计算效率。
...
### [146] [Emergent Communication between Heterogeneous Visual Agents through Decentralized Learning](https://arxiv.org/abs/2605.11695)
*Mikako Ochiai,Masatoshi Nagano,Tadahiro Taniguchi*

Main category: cs.CV

> 该研究通过Metropolis-Hastings图像标注游戏，探索了具有不同视觉表示的异构代理之间如何在去中心化学习中通过离散令牌交换产生共同符号，发现仅靠局部感知评估即可产生共享语言，但视觉表示相似度影响语言内容和对称性。

<details>
  <summary>Details</summary>

**Motivation:** 研究旨在探索在没有共享感知访问权限的情况下，不同视觉代理如何通过局部感知证据产生共同符号，以及私有视觉空间之间的相似性如何约束所产生语言的内容和对称性。

**Method:** 提出Metropolis-Hastings图像标注游戏(MHCG)，两个代理协作形成共享标注，通过交换离散令牌序列，监听器使用MH风格标准基于自身视觉特征接受或拒绝提议。实验使用三种冻结的视觉编码器配对，从随机初始化的文本模块开始。

**Result:** MHCG产生的共享令牌序列在跨代理对齐、视觉特征预测和图像文本检索方面均优于无通信基线，但性能随编码器不匹配增加而下降。适度编码器异质性减少了共享序列数量但保持了视觉特异性；强异质性导致更少、更粗糙和更不对称的序列。

**Conclusion:** 结果表明，仅通过局部感知评估即可产生共享符号，编码器间的视觉表示相似度塑造了最终语言的内容和对称性，监听器端MH接受机制对于避免退化令牌形成至关重要。
...
### [147] [WildRelight: A Real-World Benchmark and Physics-Guided Adaptation for Single-Image Relighting](https://arxiv.org/abs/2605.11696)
*Lezhong Wang,Mehmet Onurcan Kaya,Siavash Bigdeli,Jeppe Revall Frisvad*

Main category: cs.CV

> 该论文提出了WildRelight数据集，这是第一个用于评估单图像重光照模型的野外数据集，并引入了一种物理引导的自监督域适应框架来解决合成数据与真实世界之间的域偏移问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的单图像重光照方法在合成基准上表现良好，但在复杂真实世界场景中的有效性尚未验证。主要问题是当前数据集主要为多视图重建设计，缺乏针对单图像重光照特有挑战的专用数据集，导致合成到真实域之间存在严重差距。

**Method:** 1) 构建WildRelight数据集：包含严格对齐的高分辨率室外场景，在时间变化的自然光照下捕获，并配对高动态范围环境图；2) 提出物理引导推理框架：结合扩散后验采样(DPS)和时间感知测试时自适应(TTA)，利用捕获的自然光演化作为自监督约束。

**Result:** 该数据集严格对齐的临时结构实现了新的域适应范式。实验证明，该方法能让合成模型实时与真实世界统计对齐，将难以处理的sim-to-real挑战转化为可解决的自监督任务，显著缓解了域偏移问题。

**Conclusion:** WildRelight填补了单图像重光照领域缺少专用野外数据集的空白，通过物理引导的自适应框架有效地解决了合成到真实域的迁移问题。数据集和代码将公开提供，以促进鲁棒且基于物理的重光照研究发展。
...
### [148] [ScaleMoGen: Autoregressive Next-Scale Prediction for Human Motion Generation](https://arxiv.org/abs/2605.11704)
*Inwoo Hwang,Hojun Jang,Bing Zhou,Jian Wang,Young Min Kim,Chuan Guo*

Main category: cs.CV

> ScaleMoGen是一种基于尺度自回归的文本驱动人体动作生成框架，通过从粗到细的多尺度预测实现更高质量的动作生成。

<details>
  <summary>Details</summary>

**Motivation:** 传统的自回归方法采用标准的下一个词元预测，难以有效捕捉人体动作的层次结构和细节特征。为了改进这一点，研究者提出了尺度的自回归生成方式，将动作生成视为从粗粒度到细粒度的逐步精化过程。

**Method:** 1) 将3D动作量化为多个骨骼-时间尺度的组合离散词元；2) 采用从粗到细的自回归预测，逐级生成下一尺度的词元映射；3) 设计严格保持骨骼层次结构的动作分词器和量化器；4) 使用按比特量化和预测技术，有效扩展词表容量以保留动作细节。

**Result:** 在HumanML3D数据集上达到FID 0.030（优于MoMask的0.045），在SnapMoGen数据集上获得CLIP Score 0.693（优于MoMask++的0.685）；同时实现了无需训练的文本引导动作编辑功能。

**Conclusion:** ScaleMoGen通过创新的尺度自回归框架和骨骼-时间多尺度表示，显著提升了文本驱动动作生成的质量，并为动作编辑提供了新的可能性，在多个基准测试中达到了最先进的性能。
...
### [149] [CAST: Collapse-Aware multi-Scale Topology Fusion for Multimodal Coreset Selection](https://arxiv.org/abs/2605.11705)
*Boran Zhao,Hetian Liu,Zhenxian Hu,Yuqing Yuan,Yu Yan,Pengju Ren*

Main category: cs.CV

> CAST是一种多模态数据集选择框架，通过拓扑融合、多尺度分布匹配和关系感知覆盖实现高效的核心集选择，显著降低训练成本同时保持模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 大规模多模态模型训练面临海量图像-文本数据集带来的巨大计算开销，现有数据集选择方法存在单模态主导采样、样本评分偏差等问题，无法有效保持跨模态信息和数据分布的等价性。

**Method:** CAST框架首先构建图像和文本模态的拓扑结构，通过局部坍塌感知细化和跨模态融合得到统一拓扑；然后在小波域引入多尺度分布匹配准则；最后采用局部软关系覆盖机制实现关系感知的间接覆盖，避免密集区域的冗余选择。

**Result:** 在Flickr30K和MS-COCO数据集上的实验表明，CAST超越了现有数据集选择基线方法，在跨架构泛化能力和能效方面明显优于最先进的多模态合成方法。

**Conclusion:** CAST通过创新的拓扑融合、多尺度匹配和关系感知覆盖机制，有效解决了多模态数据集选择中的关键挑战，为实现高效的大规模多模态模型训练提供了新的技术途径。
...
### [150] [EPIC: Efficient Predicate-Guided Inference-Time Control for Compositional Text-to-Image Generation](https://arxiv.org/abs/2605.11722)
*Sunung Mun,Sunghyun Cho,Jungseul Ok*

Main category: cs.CV

> EPIC是一种无需训练的组合文本生成图像推理时优化框架，通过谓词引导搜索将提示准确率从34.16%提升至71.46%，同时大幅降低计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 当前文本到图像生成器在处理包含多物体、数量、属性和关系的组合提示时仍存在困难，需要更有效的方法来提升组合理解的准确性。

**Method:** EPIC将提示解析为固定视觉程序，使用可验证的谓词条件检查生成图像，通过谓词验证失败情况决定针对性编辑或重新采样，保持视觉程序不变的情况下进行迭代优化。

**Result:** 在GenEval2数据集上，EPIC将提示级准确率从34.16%提升至71.46%，相比最强基线提升19.23个百分点，同时减少31%图像模型执行、72%多模态大模型调用和81%token消耗。

**Conclusion:** EPIC通过谓词引导的推理时控制策略，有效解决了组合文本生成图像的核心挑战，在提升生成质量的同时显著降低了计算开销，为实际应用提供了可行的解决方案。
...
> 本文提出了CaC（Concentrate and Concentrate），一个基于视觉语言模型的粗到细异常奖励模型，通过全局-局部的时空定位和结构化推理，显著提升了视频异常检测的精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频异常检测方法难以准确定位和解释细微异常事件，需要开发能够从粗粒度到细粒度逐步聚焦异常区域的可解释模型。

**Method:** CaC采用三阶段推理：1) 全局时序扫描定位异常时间窗口；2) 在定位区间内进行细粒度空间定位；3) 通过结构化时空思维链推理得出判别。训练采用三阶段渐进式：首先进行空间和时间锚定的监督微调，然后基于GRPO强化学习优化，引入时空IoU奖励监督中间定位过程。

**Result:** CaC在细粒度异常基准测试上精度提升25.7%，作为奖励信号使用时，生成视频的异常率降低11.7%且整体视频质量得到提升。

**Conclusion:** 通过粗到细的定位策略和结构化推理，CaC实现了更精准、可解释的视频异常检测，为视频质量评估提供了有效的奖励信号。
...
### [152] [WorldComp2D: Spatio-semantic Representations of Object Identity and Location from Local Views](https://arxiv.org/abs/2605.11743)
*SeongMin Jin,Doo Seok Jeong*

Main category: cs.CV

> WorldComp2D是一种新颖的轻量级表示学习框架，通过多尺度局部感受野显式结构化潜空间几何，在保持实时性能的同时，大幅减少参数量和计算量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的空间-语义推理方法依赖隐式潜空间结构和密集特征图，限制了计算效率和灵活性。需要一种更高效、更通用的框架来同时捕获语义和空间信息。

**Method:** 提出WorldComp2D框架，包含两个核心组件：(i)邻近性依赖编码器，将观测映射到空间-语义潜空间；(ii)定位器，从空间-语义表示推断输入中物体的坐标。使用多尺度局部感受野按物体身份和空间邻近性显式结构化潜空间。

**Result:** 以面部关键点定位为验证案例，相比最先进的轻量级模型，WorldComp2D分别减少了4.0倍参数量和2.2倍浮点运算量，同时保持CPU上的实时性能。

**Conclusion:** 显式结构化的潜空间为空间-语义推理提供了高效且通用的基础框架，在计算效率和灵活性方面具有显著优势。
...
### [153] [BronchoLumen: Analysis of recent YOLO-based architectures for real-time bronchial orifice detection in video bronchoscopy](https://arxiv.org/abs/2605.11748)
*Yongchao Li,Marian Himstedt*

Main category: cs.CV

> 本文提出了BronchoLumen，一个基于YOLO的实时支气管孔口检测系统，比较了YOLOv8和YOLOv12在支气管镜视频中的性能表现。

<details>
  <summary>Details</summary>

**Motivation:** 支气管镜检查在临床中广泛应用，但在复杂的呼吸道分支结构中导航仍具有挑战性。需要开发可靠的支气管孔口检测系统来辅助导航和计算机辅助诊断。

**Method:** 采用YOLO目标检测框架，训练并比较了YOLOv8和集成注意力模块的新架构YOLOv12，仅使用公开数据集进行模型训练和测试，评估了跨图像域的泛化能力。

**Result:** YOLOv8在同域测试中mAP@0.5达到0.91，跨域为0.68；YOLOv12分别为0.84和0.68，在定位精度(mAP@0.5:0.9)上略优于YOLOv8。系统在大多数场景下表现出鲁棒性，但在运动模糊和低对比度情况下存在挑战。

**Conclusion:** BronchoLumen是一个开源权重的解决方案，提供了跨多个图像域的高精度和高效率支气管孔口检测。虽然YOLOv12定位精度更好，但YOLOv8的精度略高。模型已公开发布以促进支气管镜导航研究的进一步发展。
...
### [154] [Focusable Monocular Depth Estimation](https://arxiv.org/abs/2605.11756)
*Yuxin Du,Tao Lin,Zile Zhong,Runting Li,Xiyao Chen,Jiting Liu,Chenglin Liu,Ying-Cong Chen,Yuqian Fu,Bo Zhao*

Main category: cs.CV

> 本文提出了Focusable Monocular Depth Estimation (FDE)任务和FocusDepth框架，通过提示引导的深度估计实现对目标区域的精确深度预测。

<details>
  <summary>Details</summary>

**Motivation:** 现有的单目深度基础模型使用统一的像素级目标进行优化，无法区分用户指定的或任务相关的目标区域与周围环境，缺乏对重要区域的关注能力。

**Method:** 提出FocusDepth框架，结合Segment Anything Model 3和Depth Anything模型，通过Multi-Scale Spatial-Aligned Fusion (MSSA)模块实现多尺度特征的空间对齐和门控条件融合，支持框/文本提示引导的区域感知深度估计。

**Result:** 在包含252.9K/72.5K训练/验证样本的FDE-Bench基准上，FocusDepth在目标边界和前景区域显著优于全局微调的基线模型，最大AbsRel提升13.8%，同时保持全局场景几何的一致性。

**Conclusion:** FDE任务和FocusDepth框架有效解决了深度估计中的区域感知问题，MSSA的空间对齐设计是实现专注感知的关键，为计算机视觉任务中重要区域的精确建模提供了新思路。
...
### [155] [M$^4$-SAM: Multi-Modal Mixture-of-Experts with Memory-Augmented SAM for RGB-D Video Salient Object Detection](https://arxiv.org/abs/2605.11760)
*Jiyuan Liu,Jia Lin,Xiaofei Zhou,Runmin Cong,Deyang Liu,Zhi Liu*

Main category: cs.CV

> M^4-SAM通过多模态混合专家和记忆增强机制解决了SAM2在RGB-D视频显著性目标检测中的三大挑战，在无需提示的情况下实现了最优性能。

<details>
  <summary>Details</summary>

**Motivation:** SAM2作为通用分割基础模型在RGB-D视频显著性目标检测任务中面临三个关键问题：线性LoRA的空间建模能力有限、未能充分利用SAM的多尺度特征、以及需要依赖显式提示进行初始化。

**Method:** 提出M^4-SAM框架，包含三个核心创新：1) 注入模态感知的MoE-LoRA，通过卷积专家编码局部空间先验；2) 部署门控多级特征融合，层次化聚合并平衡空间细节与语义上下文；3) 采用伪引导初始化，利用粗掩码作为伪先验引导记忆库，实现零样本VSOD。

**Result:** 在三个公开的RGB-D VSOD数据集上，M^4-SAM在所有评估指标上均达到最先进的性能，证明了方法的有效性。

**Conclusion:** M^4-SAM成功地将SAM2扩展到RGB-D视频显著性目标检测任务，通过模态相关的参数高效微调、多尺度特征融合和无需提示的初始化策略，为跨模态视频理解提供了新的解决方案。
...
### [156] [Revisiting Shadow Detection from a Vision-Language Perspective](https://arxiv.org/abs/2605.11771)
*Yonghui Wang,Wengang Zhou,Hao Feng,Houqiang Li*

Main category: cs.CV

> 本文提出SVL框架，通过引入语言语义参考来解决传统视觉驱动阴影检测在视觉模糊情况下的不可靠问题，实现了高效的参数设计和更强的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 传统阴影检测依赖像素级视觉监督，但在视觉模糊情况下（如阴影与深色表面相似时），仅凭视觉线索难以建立稳定的决策规则，导致检测不可靠。

**Method:** 提出SVL视觉-语言框架：1) 通过场景级阴影比例回归将全局图像表示与阴影相关文本嵌入对齐，提供图像级引导；2) 引入全局到局部耦合机制，强制图像级引导与块级预测的一致性；3) 应用局部块级约束结合文本嵌入提升细粒度判别；4) 基于冻结的DINOv3图像编码器，仅学习轻量投影和解码模块，参数效率极高。

**Result:** 在多个阴影检测基准测试和专门的困难案例评估中，SVL展现了强大的整体性能，并在视觉模糊条件下显示出显著改善的鲁棒性。

**Conclusion:** 从视觉-语言角度重新审视阴影检测，证明超越纯视觉线索的显式语义参考对提升阴影检测鲁棒性至关重要，为相关领域提供了新的研究思路。
...
### [157] [Urban Risk-Aware Navigation via VQA-Based Event Maps for People with Low Vision](https://arxiv.org/abs/2605.11782)
*Antoni Valls,Jordi Sanchez-Riera*

Main category: cs.CV

> 本研究提出了一种基于视觉问答的事件地图框架，利用视觉语言模型为视障人士创建风险感知导航系统，通过分层查询结构实现场景理解和风险评估。

<details>
  <summary>Details</summary>

**Motivation:** 视觉障碍影响全球数亿人，限制他们在城市环境中安全独立导航的能力。现有可穿戴辅助设备依赖特定任务的视觉管道，缺乏灵活性和通用性。

**Method:** 提出基于视觉问答的事件地图框架，采用三级分层查询结构实现细粒度场景理解。模型回答被聚合成加权风险评分系统，将街道段落映射为四个安全类别。评估了四种VQA架构：ViLT、LLaVA、InstructBLIP和Qwen-VL。

**Result:** 构建了跨越六大洲20个城市的多样化数据集，包含800多张标注图像和18000个回答问题。生成式多模态大语言模型显著优于基于分类的方法，Qwen-VL在精确率和召回率之间达到最佳平衡。

**Conclusion:** 多模态大语言模型作为灵活、通用的基础，能够有效支持视障人士辅助导航系统，证明了该方法的可行性和实用性。
...
### [158] [SB-BEVFusion: Enhancing the Robustness against Sensor Malfunction and Corruptions](https://arxiv.org/abs/2605.11799)
*Markus Essl,Marta Moscati,Mubashir Noman,Muhammad Zaigham Zaheer,Usman Naseem,Shah Nawaz,Markus Schedl*

Main category: cs.CV

> 本文提出了一种鲁棒的相机-LiDAR融合模块，能够在模态缺失或损坏的情况下保持优秀的3D目标检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态融合方法在将相机和LiDAR数据转换为统一鸟瞰图表示后，当某一模态数据缺失、损坏或存在噪声时会导致性能显著下降。

**Method:** 开发了一个框架无关的融合模块，集成到BEVFusion框架中，专门处理模态缺失或损坏的情况。

**Result:** 在MultiCorrupt数据集上的实验表明，该模块在各种传感器劣化场景下都显著优于现有方法，在极端天气和传感器故障等损坏场景下达到最先进性能。

**Conclusion:** 该方法有效提升了多模态3D目标检测系统的鲁棒性，为自动驾驶系统在复杂传感器故障情况下的可靠性提供了新思路。
...
### [159] [OTT-Vid: Optimal Transport Temporal Token Compression for Video Large Language Models](https://arxiv.org/abs/2605.11803)
*Minseok Kang,Minhyeok Lee,Jungho Lee,Minjung Kim,Donghyeong Kim,Dayeon Lee,Heeseung Choi,Ig-jae Kim,Sangyoun Lee*

Main category: cs.CV

> OTT-Vid是一种基于最优传输的视频大模型时序令牌压缩框架，通过空间修剪和传输预算分配，在仅保留10%令牌的情况下保持了95.8%的VQA和73.9%的VTG性能。

<details>
  <summary>Details</summary>

**Motivation:** 随着视频大语言模型越来越长且复杂，推理成本因跨帧累积的大量视觉令牌而急剧增长。现有的时序压缩方法主要依赖跨帧令牌相似性或分段启发式方法，忽视了每个令牌在其帧内的语义角色，且无法根据帧对的压缩适应性动态调整压缩强度。

**Method:** OTT-Vid包含两个阶段：首先进行空间修剪识别每帧中的代表性内容，然后在相邻帧间求解最优传输来估计时序压缩性。采用非均匀令牌质量保护语义重要令牌，使用局部感知成本捕获特征和空间差异，最终根据传输难度动态分配压缩预算。

**Result:** 在六个跨越视频问答和时序定位的基准测试中，OTT-Vid在仅保留10%令牌的情况下保持了95.8%的VQA和73.9%的VTG性能，始终优于现有的最先进免训练压缩方法。

**Conclusion:** OTT-Vid通过最优传输理论实现了高效的视频令牌压缩，成功平衡了令牌重要性和匹配成本，为视频大模型的推理优化提供了一种有效且实用的解决方案。
...
### [160] [Mitigating Action-Relation Hallucinations in LVLMs via Relation-aware Visual Enhancement](https://arxiv.org/abs/2605.11808)
*Zhenxin Qin,Qiang Li,Qingzhuo Wang,Ruiyang Qin,Zhihua Wei,Wen Shen*

Main category: cs.CV

> 针对大型视觉语言模型中的动作关系幻觉问题，本文提出了一种基于注意力增强的框架，通过定位动作相关图像区域并增强模型对这些区域的注意力来有效缓解幻觉问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究主要关注对象幻觉，而忽略了更复杂的关系幻觉，特别是涉及对象间交互的动作关系幻觉。通过实证研究发现，动作关系幻觉的主要原因是LVLMs对视觉信息的注意力分配不足。

**Method:** 提出动作关系敏感性(ARS)分数来识别对动作关系变化最敏感的注意力头，定位包含关键视觉线索的动作相关图像区域；然后提出关系感知视觉增强(RVE)方法来增强LVLM对这些区域的注意力。

**Result:** 与现有基线方法相比，该方法在缓解动作关系幻觉方面表现更优，推理成本几乎可以忽略不计，并且能有效泛化到空间关系幻觉和对象幻觉的缓解。

**Conclusion:** 研究表明通过增强模型对动作相关视觉区域的注意力分配，可以有效解决复杂的动作关系幻觉问题，该方法具有良好的泛化能力和实用性。
...
### [161] [RevealLayer: Disentangling Hidden and Visible Layers via Occlusion-Aware Image Decomposition](https://arxiv.org/abs/2605.11818)
*Binhao Wang,Shihao Zhao,Bo Cheng,Qiuyu Ji,Yuhang Ma,Liebucha Wu,Shanyuan Liu,Dawei Leng,Yuhui Yin*

Main category: cs.CV

> RevealLayer是一个基于扩散模型的自然图像分层分解框架，能够将RGB图像分解为多个RGBA图层，实现了精确的图层分离和遮挡内容恢复。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于扩散模型的图像分层分解方法在处理复杂自然图像时面临三大挑战：遮挡补全困难、图层解耦不鲁棒、前景边界不精确，同时高质量多层自然图像数据集的稀缺也制约了技术发展。

**Method:** RevealLayer框架包含三个核心组件：(1)区域感知注意力模块用于解耦隐藏层和可见层；(2)遮挡引导适配器利用上下文信息增强重叠区域；(3)复合损失函数强制锐化的alpha边界并抑制残差伪影。同时构建了包含10万张高质量多层自然图像的数据集RevealLayer-100K和基准测试集RevealLayerBench。

**Result:** 大量实验证明，RevealLayer在图像分层分解任务中持续优于现有方法，展现了更好的分解效果和遮挡恢复能力。

**Conclusion:** RevealLayer为自然图像分层分解提供了有效的解决方案，通过创新的网络架构设计和高质量的训练数据，显著提升了复杂自然图像的分解精度和实用性。
...
### [162] [REFNet++: Multi-Task Efficient Fusion of Camera and Radar Sensor Data in Bird's-Eye Polar View](https://arxiv.org/abs/2605.11824)
*Kavin Chandrasekaran,Sorin Grigorescu,Gijs Dubbelman,Pavol Jancura*

Main category: cs.CV

> 本研究提出了一种基于变分编码器-解码器架构的雷达-相机多模态融合方法，通过将相机数据转换为鸟瞰图极坐标域并与雷达的雷达-角度特征对齐，实现了高效且鲁棒的环境感知。

<details>
  <summary>Details</summary>

**Motivation:** 相机传感器提供周围环境的真实视图，但在恶劣天气条件下性能有限；雷达传感器在天气变化中表现鲁棒但输出噪声大且分类能力弱。因此需要将两者融合以获得更好的环境感知效果，同时兼顾计算效率。

**Method:** 采用变分编码器-解码器架构：1) 使用变分编码器-解码器学习将前视相机图像变换到鸟瞰图(BEV)极坐标域；2) 同时用雷达编码器-解码器从距离-多普勒(RD)数据中恢复角度信息生成距离-角度(RA)特征，实现两种模态在兼容域的对齐。

**Result:** 在RADIal数据集上针对车辆检测和自由空间分割任务进行评估，结果表明该方法相比最新技术具有优越的性能。

**Conclusion:** 通过学习性的域对齐方法，该研究成功实现了雷达和相机数据的高效融合，在保持准确性的同时提高了计算效率，为自动驾驶环境感知提供了有效的解决方案。
...
### [163] [Selection, Not Fusion: Radar-Modulated State Space Models for Radar-Camera Depth Estimation](https://arxiv.org/abs/2605.11840)
*Zhangcheng Hou,Tomoaki Ohtsuki*

Main category: cs.CV

> 本研究提出雷达调制选择(RMS)机制，将雷达信号直接注入Mamba的选择扫描过程中，通过多视图扫描金字塔(MVSP)实现了深度估计性能的显著提升和最低延迟。

<details>
  <summary>Details</summary>

**Motivation:** 现有雷达-相机融合方法都在骨干网络的外部进行特征融合，即使跨模态Mamba变体的选择机制本身也是单模态的。作者认为选择机制才是雷达应该介入的正确位置。

**Method:** 提出RMS方法，在Mamba的选择扫描中注入雷达调制：雷达在内部调节扫描过程，对步长Δ和读出C添加零初始化扰动，保持输入投影B和状态动力学A仅基于图像。同时构建MVSP框架，在每个尺度上匹配雷达的空间覆盖范围。

**Result:** SemoDepth在nuScenes数据集上达到最先进性能，在0-50m、0-70m和0-80m范围内分别将MAE降低34.0%、29.9%和29.9%，同时实现了最低的单帧延迟(26.8ms)。

**Conclusion:** 实验证明，外部扫描特征融合在RMS基础上没有增加精度，验证了内部扫描选择可以替代外部融合，为雷达-相机深度估计提供了新的有效范式。
...
### [164] [UniVLR: Unifying Text and Vision in Visual Latent Reasoning for Multimodal LLMs](https://arxiv.org/abs/2605.11856)
*Houcheng Jiang,Jiajun Fu,Junfeng Fang,Chen Gao,Xiang Wang,Xiangnan He,Yong Li*

Main category: cs.CV

> UniVLR是一个统一的视觉潜在推理框架，将文本推理和视觉证据整合为共享的视觉工作空间，通过压缩为紧凑的视觉潜在token实现高效的纯视觉推理，无需显式文本链式推理。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大语言模型视觉推理方法依赖文本推理与视觉token的交错设计，导致推理效率低下，且文本和视觉通道分离限制了推理的统一性。

**Method:** UniVLR框架将文本推理轨迹与辅助图像统一为共享视觉工作空间，学习将此统一表示压缩为紧凑的视觉潜在token，推理时仅通过视觉token直接解码最终答案。

**Result:** 实验表明UniVLR在真实感知和视觉推理任务中超越了先前的视觉潜在推理方法，同时显著减少了生成的推理token数量。

**Conclusion:** UniVLR展示了多模态大语言模型中视觉思考的更统一高效范式，通过纯视觉潜在空间推理避免了外部工具调用和冗长文本推理，为视觉推理提供了新的研究方向。
...
### [165] [Multimodal Abstractive Summarization of Instructional Videos with Vision-Language Models](https://arxiv.org/abs/2605.11959)
*Maham Nazir,Muhammad Aqeel,Richong Zhang,Francesco Setti*

Main category: cs.CV

> ClipSum是一个利用预训练CLIP视觉-语言特征的教学视频摘要框架，通过显式时间建模和自适应维度融合，实现了比传统CNN特征更好的摘要效果。

<details>
  <summary>Details</summary>

**Motivation:** 传统多模态视频摘要方法使用的CNN特征是为物体分类训练的，将视觉概念表示为离散类别，与自然语言不匹配，存在视觉-语言语义鸿沟。

**Method:** 提出ClipSum框架，使用冻结的CLIP视觉-语言特征，配合显式时间建模和维度自适应融合技术，用于教学视频摘要任务。CLIP通过在4亿图像-文本对上进行对比预训练，生成与文本解码器生成的语言概念语义对齐的视觉特征。

**Result:** 在YouCook2数据集上，ClipSum达到33.0% ROUGE-1，优于ResNet-152的30.5%，且特征维度降低4倍(512 vs 2048)。冻结CLIP(33.0%)优于微调CLIP(32.3%)，证明保持预训练对齐比任务特定适应更有价值。

**Conclusion:** 语义对齐比特征容量更重要，保持预训练的视觉-语言对齐比针对特定任务进行微调更有效，CLIP特征为视频摘要提供了更好的语义基础。
...
### [166] [GATA2Floor: Graph attention for floor counting in street-view facades](https://arxiv.org/abs/2605.11863)
*Ngoc Tan Le,Tzoulio Chamiti,Eirini Papagiannopoulou,Nikos Deligiannis*

Main category: cs.CV

> 本文提出GATA2Floor模型，通过图注意力网络对建筑立面进行图形化建模，实现楼层数预测和楼层分配，支持无监督学习。

<details>
  <summary>Details</summary>

**Motivation:** 建筑物立面的自动分析对城市规划、能源评估和应急规划具有重要意义，但需要处理空间排列元素而非孤立检测，现有方法缺乏对建筑元素间关系的推理能力。

**Method:** 将每个立面建模为基于窗户/门检测的图结构，引入垂直边先验；提出GATA2Floor多头图注意力v2模型，通过可学习的交叉注意力查询将元素分配到潜在楼层槽；利用自监督特征和视觉语言评分实现轻量级无标注提案机制。

**Result:** 该方法能够在缺乏标注数据的情况下进行图推理，产生可解释的输出结果，对不规则建筑设计具有鲁棒性，验证了基于图注意力的关系推理在立面理解中的价值。

**Conclusion:** 基于图注意力的关系推理为建筑立面理解提供了有效解决方案，通过图形化建模和注意力机制实现了对建筑结构的深入理解和准确分析。
...
### [167] [Design Your Ad: Personalized Advertising Image and Text Generation with Unified Autoregressive Models](https://arxiv.org/abs/2605.12138)
...
> 本文提出了一种统一的个性化广告生成模型Uni-AdGen，能够根据用户历史点击行为联合生成个性化图文广告，并构建了首个大规模个性化广告数据集PAd1M。

<details>
  <summary>Details</summary>

**Motivation:** 现有广告生成方法缺乏跨模态感知能力，且依赖只反映平均偏好的点击率指标，无法实现真正的个性化广告生成。

**Method:** 设计单一自回归框架Uni-AdGen，集成前景感知模块和指令调优增强内容真实感；配置由粗到细的偏好理解模块，从多模态历史行为中捕获用户兴趣驱动个性化生成；构建PAd1M数据集并引入PBS评估指标。

**Result:** 在通用和个性化广告生成任务上，该方法均优于基线模型，证明了跨模态联合生成和个性化理解的有效性。

**Conclusion:** 统一的跨模态生成框架结合细粒度用户偏好理解，为个性化广告创作提供了新的解决方案，推动了电商广告个性化发展。
...
### [168] [When Brains Disagree: Biological Ambiguity Underlies the Challenge of Amyloid PET Synthesis from Structural MRI](https://arxiv.org/abs/2605.11867)
*Louise E. G. Baron,Ross Callaghan,David M. Cash,Philip S. Weston,Hojjat Azadbakht,Hui Zhang*

Main category: cs.CV

> 研究通过受控实验证明，MRI与淀粉样PET合成的性能不一致源于生物学歧义，而非模型架构问题，多模态整合能解决这一问题。

<details>
  <summary>Details</summary>

**Motivation:** 尽管MRI到淀粉样PET的合成被提出作为阿尔茨海默病淀粉样评估的非侵入性替代方案，但相同模型的性能在不同研究中变化很大，且更复杂的架构并未带来持续提升。

**Method:** 通过两个受控实验：(1)按淀粉样蛋白和神经退行性状态分层配对MRI-PET数据，控制训练分布；(2)引入血浆生物标志物作为正交生物学信息，采用多模态输入方法。

**Result:** 证明生物学无歧义的映射可以单独学习，但引入数据歧义时性能急剧下降；多模态输入时性能提升且稳定性恢复。

**Conclusion:** MRI到淀粉样PET合成的局限性和不一致性能主要由内在生物学歧义解释，稳定且有意义的进展需要多模态整合而非架构复杂性。
...
### [169] [FIS-DiT: Breaking the Few-Step Video Inference Barrier via Training-Free Frame Interleaved Sparsity](https://arxiv.org/abs/2605.11869)
*Jian Tang,Jiawei Fan,Qingbin Liu,Zheng Wei*

Main category: cs.CV

> FIS-DiT是一种无需训练、与算子无关的框架，通过在潜在帧维度实现帧交插稀疏性，在保持质量的同时将视频扩散Transformer的推理速度提升2.11-2.41倍。

<details>
  <summary>Details</summary>

**Motivation:** 虽然模型蒸馏可以降低视频扩散Transformer的整体推理延迟，但单步推理延迟仍是关键瓶颈。现有的基于去噪轨迹冗余的加速策略在少步数情况下效果递减，因为时间状态稀缺阻碍了特征复用和预测建模。

**Method:** 提出FIS-DiT框架，将优化焦点从时间轨迹转向潜在帧维度，利用帧级稀疏性和结构一致性的双重特性，通过帧交插稀疏(FIS)执行策略在模型层次结构中操作帧子集，无需全尺度块计算即可刷新所有潜在位置。

**Result:** 在Wan 2.2和HunyuanVideo 1.5上的实验表明，FIS-DiT在VBench-Q和CLIP指标上仅有可忽略的退化，同时实现了2.11-2.41倍的稳定加速。

**Conclusion:** FIS-DiT为实现实时高清视频生成提供了可扩展且鲁棒的解决方案，突破了现有加速方法的局限性，为视频生成模型的实际应用开辟了新路径。
...
### [170] [$h$-control: Training-Free Camera Control via Block-Conditional Gibbs Refinement](https://arxiv.org/abs/2605.11871)
*Yuzhu Wang,Xi Ye,Duo Su,Yangyang Xu,Jun Zhu*

Main category: cs.CV

> 本文提出了一种名为h-control的无训练摄像机控制方法，通过块条件伪吉布斯采样解决了现有方法在轨迹遵循和视觉质量之间的权衡问题，在多个基准测试中取得了最优性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的无训练摄像机控制方法难以平衡轨迹遵循和视觉质量，并且启发式的引导强度调节缺乏鲁棒性。这源于问题的本质——这是一个部分观测逆问题，需要在深度扭曲的引导视频提供的噪声证据和预训练先验之间进行调和。

**Method:** h-control方法通过两层结构改进：1) 外层的硬替换引导步骤；2) 内层的块条件伪吉布斯精化，在相同噪声级别下对未观测补集进行采样为提高收敛速度，利用条件局部性将未观测补集分割为3D块，通过自定义混合指示器自适应冻结已收敛块。

**Result:** 在RealEstate10K和DAVIS基准测试上，h-control在FVD指标上超越所有七个竞争者(包括训练和训练-free方法)，在所有报告指标上都优于所有训练-free基线。

**Conclusion:** h-control通过结构化的采样器设计，有效解决了摄像机控制中轨迹遵循与视觉质量的权衡问题，为无训练的视频生成摄像机控制提供了新的最优解决方案。
...
### [171] [Learning Subspace-Preserving Sparse Attention Graphs from Heterogeneous Multiview Data](https://arxiv.org/abs/2605.11881)
*Jie Chen,Yuanbiao Gou,Chuanbin Liu,Zhu Wang,Xi Peng*

Main category: cs.CV

> 本文提出了稀疏注意力图学习(SAGL)方法，用于从异构多视角数据中学习保持子空间结构的稀疏注意力图，在无监督迁移学习任务中实现了优于现有方法的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的无监督迁移学习方法在利用多视角互补信息时，无法有效恢复内在的子空间结构，这成为实现异构视角间语义对齐的基本挑战。构造能够保持这些底层子空间结构的稀疏相似性图是解决问题的关键。

**Method:** SAGL方法包含三个关键技术：(1)双线性注意力分解方案，用于捕获高维特征间的非对称相似性，突破传统表示学习的对称性瓶颈；(2)动态稀疏门控机制，自适应预测特征特定的压缩因子以控制邻居的拓扑贡献；(3)通过α-entmax实现结构化稀疏投影，生成保持子空间结构的稀疏注意力图。最后利用视角特定图进行稀疏信息聚合。

**Result:** 在多个基准数据集上的广泛实验表明，SAGL方法在性能上始终优于当前最先进的无监督迁移学习方法，证明了其在异构多视角数据表示学习方面的有效性。

**Conclusion:** SAGL成功地将稀疏注意力机制与子空间结构保持相结合，通过理论分析建立了可微分稀疏注意力与概率单纯形约束之间的桥梁，为异构多视角无监督学习提供了一种有效的解决方案。
...
### [172] [Few-Shot Synthetic Data Generation with Diffusion Models for Downstream Vision Tasks](https://arxiv.org/abs/2605.11898)
*Daniil Dushenev,Nazariy Karpov,Daniil Zinovjev,Alexander Gorin,Konstantin Kulikov*

Main category: cs.CV

> 提出了使用LoRA微调扩散模型生成合成数据来解决视觉识别中类别不平衡问题的轻量级方法，在医学影像和工业缺陷检测中均验证了有效性。

<details>
  <summary>Details</summary>

**Motivation:** 在安全关键的视觉识别领域，罕见类别样本稀少且采集成本高昂，导致严重的类别不平衡问题，影响模型性能。

**Method:** 基于20-50个真实罕见类别样本微调LoRA适配器，使用预训练扩散模型生成合成样本进行数据增强，并系统性地改变合成样本与真实样本的比例进行训练。

**Result:** 在胸部X光片病理分类和工业表面裂纹检测两个不同域中，合成数据增强始终提升了罕见类别的召回率和F1分数，中等程度的增强效果最佳。

**Conclusion:** LoRA适配的扩散模型提供了一种简单可扩展的罕见类别增强机制，在不同视觉域的数据稀缺场景下都能实现有效学习。
...
### [173] [Mobile Traffic Camera Calibration from Road Geometry for UAV-Based Traffic Surveillance](https://arxiv.org/abs/2605.11900)
*Alexey Popov,Natalia Trukhina,Vadim Vashkelis*

Main category: cs.CV

> 本研究提出了一种轻量级处理流水线，可将单目倾斜无人机交通视频转换为鸟瞰图(BEV)表示形式，利用可见道路几何特征估计道路平面单应性，投影车辆观测到地面坐标系统，支持交通分析应用。

<details>
  <summary>Details</summary>

**Motivation:** 无人机交通视频分析面临主要挑战：车辆运动在透视图像坐标系中观察难以用于交通分析，而固定道路摄像机在某些场景不可用、成本高或不切实际，需要建立稳定度量坐标系统的方法。

**Method:** 提出轻量级流水线：1)利用道路标线、道路边界和斑马线等可见道路几何特征估计道路平面单应性；2)将车辆观测从图像坐标投影到度量地面坐标；3)基于接触点重建车辆轨迹并估计方向、速度、航向和3D立方体。

**Result:** 在UAVDT数据集上评估显示，40帧采样产生632个度量立方体实例和23条轨迹，证明道路几何校准可将单目无人机图像转换为可解释的交通分析结果，包括BEV轨迹和3D立方体可视化。

**Conclusion:** 该方法为可部署的无人机交通摄像机和实时交通数字孪生系统奠定了实用基础，但存在远场车辆敏感、人工验证优于全自动校准、单平面假设限制等局限，未来需要进一步优化。
...
### [174] [Beyond Point-wise Neural Collapse: A Topology-Aware Hierarchical Classifier for Class-Incremental Learning](https://arxiv.org/abs/2605.11904)
*Huiyu Yi,Zhiming Xu,Dunwei Tu,Zhicheng Wang,Baile Xu,Furao Shen*

Main category: cs.CV

> 本文提出HC-SOINN分类器和STAR方法，通过层次聚类和拓扑结构对齐来解决类增量学习中特征流形复杂性的问题，显著提升了分类性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的最近类均值(NCM)分类器在类增量学习中虽然抗遗忘性强，但神经崩溃理论要求的特征点坍塌假设在实际中难以满足。非线性特征漂移和训练不足导致类表现为复杂的流形而非单点，使基于单点的NCM分类器变得次优。

**Method:** 提出HC-SOINN层次聚类分类器，通过'局部到全局'的表示捕获流形的拓扑结构；引入STAR拓扑对齐方法，采用细粒度点轨迹跟踪机制主动变形学习到的拓扑结构，精确适应复杂的非线性特征漂移。

**Result:** 将HC-SOINN集成到7个最先进方法中，通过替换原有分类器获得一致的改进；理论分析和Procrustes距离实验验证了框架对流形变形的鲁棒性。

**Conclusion:** HC-SOINN和STAR方法有效解决了类增量学习中特征流形复杂性的挑战，为处理非线性特征漂移提供了新的解决方案，在实际多种方法中展现出良好的泛化能力和鲁棒性。
...
### [175] [Vector Scaffolding: Inter-Scale Orchestration for Differentiable Image Vectorization](https://arxiv.org/abs/2605.11913)
*Jaerin Lee,Kanggeon Lee,Kyoung Mu Lee*

Main category: cs.CV

> 提出Vector Scaffolding框架，通过分层优化解决可微分矢量图形中的拓扑崩塌问题，实现2.5倍加速且PSNR提升1.4dB。

<details>
  <summary>Details</summary>

**Motivation:** 现有可微分矢量图形框架采用扁平优化，导致数千曲线盲目竞争像素级误差，产生拓扑崩塌和不可编辑的多边形汤，限制了实际可编辑性。

**Method:** 提出分层优化框架Vector Scaffolding，通过内部梯度聚合稳定区域与边界梯度的数学失衡，结合渐进分层和快速膨胀调度，使用极高学习率(×50)逐步密集化矢量基元。

**Result:** 实验表明该方法相比最先进技术加速优化2.5倍，同时PSNR提升最高1.4dB。

**Conclusion:** 该框架成功地将扁平像素匹配转为结构化拓扑构建，为矢量图形提供了更稳定、高效且可编辑的优化方案。
...
### [176] [RealDiffusion: Physics-informed Attention for Multi-character Storybook Generation](https://arxiv.org/abs/2605.11927)
*Qi Zhao,Jun Chen,Ivor Tsang,Guang Dai*

Main category: cs.CV

> RealDiffusion是一种基于物理原理的光扩散方法，通过热扩散作为先验和区域感知随机过程，解决了序列图像生成中角色一致性与叙事动态性平衡的核心问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的扩散模型在生成序列图像时面临关键挑战：难以同时维持角色的身份一致性(避免角色特征漂移)和故事的动态性(避免叙事停滞)，导致角色失去身份或故事缺乏进展。

**Method:** 提出了一种训练免费的物理信息注意力机制：1)使用热扩散作为耗散先验，沿序列平均邻近特征并去除高频噪声；2)通过区域感知随机过程引入微小扰动以探索邻近模式；3)在推理过程中将可控物理先验注入自注意力层，将特征演化建模为可配置的物理系统。

**Result:** 实验证明RealDiffusion在保持叙事动态性的同时显著提升了角色一致性，性能超越了现有最先进方法，实现了角色身份稳定与故事发展的有效平衡。

**Conclusion:** 通过创新性地将物理扩散原理引入序列图像生成，RealDiffusion成功解决了角色一致性与叙事动态性的根本矛盾，为多角色连续图像生成提供了有效的统一框架，无需额外训练即可实现显著改进。
...
### [177] [Learn to Think: Improving Multimodal Reasoning through Vision-Aware Self-Improvement Training](https://arxiv.org/abs/2605.11931)
*Qihuang Zhong,Liang Ding,Wenjie Xuan,Juhua Liu,Bo Du,Dacheng Tao*

Main category: cs.CV

> VISTA是一个解决多模态大语言模型自我改进训练中数据不平衡和语言先验偏差问题的框架，通过前缀重采样策略和视觉感知注意力评分来提升模型的视觉推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的MLLM自我改进训练存在两个问题：1)容易样本过度训练而关键困难样本训练不足的数据不平衡问题；2)模型过度依赖语言先验而忽视视觉信息。这些问题限制了模型多模态推理能力的进一步提升。

**Method:** VISTA框架包含两个核心技术：1)前缀重采样策略，重用部分正确的推理轨迹进行高效数据收集；2)视觉感知注意力评分，量化模型对视觉信息的关注度。该框架可应用于监督微调和偏好学习等多种后训练场景。

**Result:** 实验结果表明，VISTA能有效提升各种MLLM和任务的多模态推理性能，例如为Qwen2.5-VL-3B-Instruct模型带来最高+13.66%的平均性能提升。

**Conclusion:** VISTA通过解决自我改进训练中的数据不平衡和语言偏差问题，为提升MLLM的多模态推理能力提供了一个有效且通用的解决方案，具有良好的适用性和显著的性能提升效果。
...
### [178] [Interactive State Space Model with Cross-Modal Local Scanning for Depth Super-Resolution](https://arxiv.org/abs/2605.11934)
*Chen Wu,Ling Wang,Zhuoran Zheng,Xiangyu Chen,Jingyuan Xia,Weidong Jiang,Jiantao Zhou*

Main category: cs.CV

> 本文提出了一种基于交互状态空间模型（ISSM）的引导深度超分辨率新框架，通过跨模态局部扫描机制实现RGB与深度特征的密集语义交互，并利用Mamba架构以线性复杂度进行全局建模，在保持高效的同时实现了竞争性的性能表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有的引导深度超分辨率方法要么独立处理各模态，要么依赖计算昂贵的注意力机制，无法建立高效且语义交互的联合表示。作者观察到不同模态的特征图在特征提取过程中存在语义级关联，这启发开发更灵活的方法来实现模态间密集的语义感知深度交互。

**Method:** 提出了以交互状态空间模型为核心的GDSR框架，包含：1）跨模态局部扫描机制，实现RGB和深度特征间的细粒度语义交互；2）基于Mamba架构的全局建模，具有线性复杂度优势；3）跨模态匹配变换模块，利用两模态的代表性特征增强交互建模质量。

**Result:** 大量实验证明，所提出的方法在性能方面与最先进方法具有竞争力，同时保持了高效的计算特性。

**Conclusion:** 通过结合跨模态局部语义交互和线性复杂度的全局建模，本文为引导深度超分辨率任务提供了一个高效且有效的解决方案，证明了状态空间模型在该领域的应用潜力。
...
### [179] [Cluster-Aware Neural Collapse Prompt Tuning for Long-Tailed Generalization of Vision-Language Models](https://arxiv.org/abs/2605.11939)
*Boyang Guo,Liang Li,Lin Peng,Yuhan Gao,Xichun Sheng,Chenggang Yan*

Main category: cs.CV

> 本文提出CPT方法，通过聚类感知神经崩溃提示调优，在不牺牲泛化能力的前提下增强了长尾数据集中尾部类别的判别性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的提示学习方法在适应类别不平衡数据集时难以保持尾部类别的判别性，影响了模型的整体性能。

**Method:** 首先设计聚类不变空间，从预训练VLM挖掘语义分配并映射到提示调优特征，计算聚类级边界；其次引入神经崩溃驱动的判别性优化，包括文本ETF分离损失、类级收敛损失和旋转稳定损失三个损失函数。

**Result:** 在11个多样化数据集上的大量实验表明，CPT方法超越了现有最先进方法，在尾部类别上表现更强，同时对未见类别具有良好的泛化能力。

**Conclusion:** CPT通过局部聚类约束和神经崩溃驱动的优化策略，有效解决了提示学习在长尾场景下的尾部类别判别性问题，同时保持了模型的泛化性。
...
### [180] [Chronicles-OCR: A Cross-Temporal Perception Benchmark for the Evolutionary Trajectory of Chinese Characters](https://arxiv.org/abs/2605.11960)
...
> Chronicles-OCR是首个评估视觉大语言模型跨历史时期汉字视觉感知能力的综合性基准数据集，涵盖从甲骨文到楷书的七种汉字书体演变，包含2800张平衡图像数据和四项定量任务。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉大语言模型在古文字视觉理解方面的感知鲁棒性未被充分研究，且古代文本数据集仅关注孤立历史时期，无法捕捉跨越千年的系统性视觉分布偏移。该研究旨在填补这一空白，推动数字人文发展。

**Method:** 与顶级机构领域专家合作，构建涵盖龟甲到纸质书法等多样化物理媒介的2800张严格平衡图像数据集；提出阶段自适应标注范式以适应不同历史阶段的剧烈形态和拓扑变化；设计了四项定量任务：跨时期字符定位、视觉指代细粒度古文字识别、古文本解析和字体分类。

**Result:** Chronicles-OCR通过将视觉感知与语义推理分离，提供了权威平台来揭示当前VLLMs的局限性，为开发鲁棒的、演化感知的历史文本感知方法铺平道路。

**Conclusion:** 该研究为汉字书体演变中的视觉感知挑战建立了新的评测标准，对推动视觉大语言模型在历史文本理解领域的发展具有重要意义，数据集已公开可供研究者使用。
...
### [181] [What Does It Mean for a Medical AI System to Be Right?](https://arxiv.org/abs/2605.11963)
*Antony Gitau*

Main category: cs.CV

> 本文探讨了在特定临床情境（骨髓涂片中浆细胞自动分类）下，医疗AI系统的

<details>
  <summary>Details</summary>

**Motivation:** 在医疗AI领域，单纯追求基准性能指标并不能完全体现系统的正确性。论文以多发性骨髓瘤诊断中的浆细胞自动分类为具体案例，质疑传统的AI评估理念。

**Method:** 采用哲学科学和研究伦理的视角，通过四个相互关联的主题来论证：真实标签的不稳定性、过度自信AI的不透明性、标准临床指标的不足性，以及时间紧迫临床环境中的自动化偏见风险。

**Result:** 研究指出医疗AI的正确性是一个多维概念，需要考虑专家标注数据集的可用性、模型输出的可解释性、评估指标的临床意义，以及人机协作中责任分配等多个维度。

**Conclusion:** 医疗AI的正确判定不应简约为基准性能，而应建立包含标签稳定性、模型透明度、指标临床相关性和人机责任分配的综合评估框架。
...
### [182] [H2G: Hierarchy-Aware Hyperbolic Grouping for 3D Scenes](https://arxiv.org/abs/2605.11967)
*ByungHa Ko,Youngmin Lee,Dong Hwan Kim*

Main category: cs.CV

> 本文提出H2G方法，利用双曲仿射场实现无需语义标签的层级3D场景分组，将2D基础模型知识转化为3D层级结构表示。

<details>
  <summary>Details</summary>

**Motivation:** 层级3D分组旨在从细粒度对象部件到完整对象的多粒度层次恢复场景分组，但传统方法依赖语义标签或固定词汇表，主要挑战在于如何将2D基础模型线索转化为连贯的层次监督信息并将其嵌入到3D表示中。

**Method:** 提出H2G双曲仿射场，通过Dasgupta相似性层次聚类目标解释基础模型亲和力，生成语义组织的树状监督；将此监督蒸馏到单个洛伦兹双曲特征场中，该场的几何结构适合树状分支结构；采用层次感知目标函数对齐细粒度分配、粗粒度对象结构、紧凑特征簇和LCA排序。

**Result:** 实现了一个统一的特征空间表示多个分组级别，能够生成基于2D基础模型知识的语义层次分组结果。

**Conclusion:** 该方法成功地将2D基础模型的语义理解转化为3D场景的层级结构表示，为无监督的层次3D场景理解提供了新的解决方案。
...
### [183] [Optimizing 4D Wires for Sparse 3D Abstraction](https://arxiv.org/abs/2605.11977)
*Dong-Yi Wu,Tong-Yee Lee*

Main category: cs.CV

> 提出一种使用连续4D B样条曲线的统一框架进行3D几何抽象，通过单条连续曲线替代传统多段式表示，实现了更好的结构连贯性和物理可实现性。

<details>
  <summary>Details</summary>

**Motivation:** 传统方法将形状表示为多个独立曲线段的集合，容易产生破碎结构和有限的物理可实现性。需要一种能够保持全局拓扑连贯性的统一表示方法。

**Method:** 采用参数化为B样条的单一连续4D金属线(x,y,z,w)，引入可微分渲染流程优化可变宽度曲线，支持SDS和CLIP等现代引导信号的梯度优化。

**Result:** 在图像到3D抽象、多视图线艺生成和可微风格化表面填充等应用中，生成了具有更高语义保真度和结构连贯性的结构。

**Conclusion:** 单条连续样条曲线足以捕获复杂体积形式同时强制全局拓扑连贯，将3D草图从局部密度累积过程转化为全局路由问题，为更清洁的美学和改进的结构连贯性提供了强归纳偏置。
...
### [184] [A Transfer Learning Evaluation of Deep Neural Networks for Image Classification](https://arxiv.org/abs/2605.11989)
*Nermeen Abou Baker,Nico Zengeler,Uwe Handmann*

Main category: cs.CV

> 本研究探讨了如何为目标域图像分类任务选择最佳的预训练模型，通过在ImageNet上预训练的11个图像处理模型在5个不同目标域数据集上进行微调实验，评估其性能表现。

<details>
  <summary>Details</summary>

**Motivation:** 迁移学习因其在保持高性能的同时能节省训练时间、内存和网络设计努力而广泛应用，但如何为目标域需求选择最适合的预训练模型尚需深入研究。

**Method:** 研究团队对11个ImageNet预训练图像处理模型的输出层和通用网络参数进行微调，将其应用于5个不同的目标域数据集，并在单次训练和十次训练两种模式下进行实验。

**Result:** 通过测量准确率、准确率密度、训练时间和模型尺寸等多个指标，系统评估了各个预训练模型在不同目标域的表现差异。

**Conclusion:** 该研究为针对特定图像分类任务选择最合适的预训练模型提供了实用的评估框架和决策参考。
...
### [185] [EDGER: EDge-Guided with HEatmap Refinement for Generalizable Image Forgery Localization](https://arxiv.org/abs/2605.12002)
*Minh-Khoa Le-Phan,Minh-Hoang Le,Minh-Triet Tran,Trong-Le Do*

Main category: cs.CV

> EDGER是一个双分支框架，通过边缘引导分割和合成热图分支协同工作，实现对任意分辨率图像中篡改区域的精确定位。

<details>
  <summary>Details</summary>

**Motivation:** 文本引导的图像修复技术使图像伪造越来越逼真，现有的源识别和图像伪造定位方法难以跨域检测可疑信号，且常牺牲原始分辨率。

**Method:** EDGER采用双分支架构：1) 边缘引导分割分支使用基于频谱的边缘检测器强调篡改边界的高频不一致性，并微调SegFormer融合RGB和边缘特征生成像素级掩码；2) 合成热图分支基于分类器，使用LoRA微调CLIP-ViT图像编码器来标记完全合成的图像块。两分支互补提供粗粒度和细粒度的定位结果。

**Result:** 在MediaEval 2025 SynthIM挑战赛的篡改区域定位任务中，EDGER能够扩展处理多兆像素图像，展现出强大的跨域泛化能力。消融实验验证了基于频谱的边缘线索和图像块级合成先验的互补作用。

**Conclusion:** EDGER框架通过结合频域边缘检测和图像块级合成先验，实现了分辨率无关的精确篡改区域定位，为解决文本引导图像伪造检测提供了有效方案。
...
### [186] [Robust Promptable Video Object Segmentation](https://arxiv.org/abs/2605.12006)
*Sohyun Lee,Yeho Gwon,Lukas Hoyer,Konrad Schindler,Christos Sakaridis,Suha Kwak*

Main category: cs.CV

> 本文提出了鲁棒视频目标分割(RobustPVOS)的首个全面研究，构建了包含351个视频片段的新基准，并提出了MoGA方法来提升模型在真实和合成 corrupted 条件下的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可提示视频目标分割(PVOS)模型在输入受到污染时性能大幅下降，这阻碍了其在安全关键领域的部署，因此需要研究鲁棒的视频目标分割方法。

**Method:** 1) 构建了包含351个视频片段和2500+目标掩码的真实世界评估数据集；2) 通过对现有VOS数据集应用多样化和时变的corruption生成合成训练数据；3) 提出Memory-object-conditioned Gated-rank Adaptation (MoGA)方法，利用跨帧内存中维护的特定对象表示来条件化robustification过程。

**Result:** 在真实和合成数据集上的大量实验验证了MoGA的有效性，在多样化corruption类型上显示了持续且显著的改进，为未来RobustPVOS研究建立了强有力的基线。

**Conclusion:** 该研究为RobustPVOS领域建立了首个全面的基准和方法，通过处理对象特定degradation和确保时间一致性，显著提升了PVOS模型在恶劣条件下的鲁棒性。
...
### [187] [L2P: Unlocking Latent Potential for Pixel Generation](https://arxiv.org/abs/2605.12013)
*Zhennan Chen,Junwei Zhu,Xu Chen,Jiangning Zhang,Jiawei Chen,Zhuoqi Zeng,Wei Zhang,Chengjie Wang,Jian Yang,Ying Tai*

Main category: cs.CV

> L2P提出了一种高效框架，通过冻结预训练LDM并仅训练浅层转换层，将潜在空间知识转移到像素空间，实现4K超高分辨率生成。

<details>
  <summary>Details</summary>

**Motivation:** 从零开始训练高级像素空间模型需要巨大的计算和数据资源，而现有LDM依赖VAE架构限制了分辨率。

**Method:** 采用大块tokenization替代VAE，冻结源LDM中间层，仅训练学习潜在到像素转换的浅层；使用LDM生成的合成图像作为唯一训练数据。

**Result:** 仅需8个GPU训练，在DPG-Bench上与源LDM性能相当，在GenEval上达到93%性能，支持原生4K超高分辨率生成。

**Conclusion:** L2P有效实现了大规模潜在先验向像素空间的迁移，以最小训练开销实现像素级生成，为高分辨率视觉生成提供了新范式。
...
### [188] [FAME: Feature Activation Map Explanation on Image Classification and Face Recognition](https://arxiv.org/abs/2605.12017)
*Xinyi Zhang,Manuel Günther*

Main category: cs.CV

> FAME是一种新的可解释AI方法，它结合了CAM类方法和扰动类方法的优点，通过梯度驱动的方式修改输入图像来生成更准确的归因图，在图像分类和人脸识别任务中达到了业界领先水平。

<details>
  <summary>Details</summary>

**Motivation:** 深度学习在图像处理中准确度高但可解释性差。现有的CAM方法假设特征图位置仅受底层区域影响，但对深层网络不成立；而扰动方法使用固定块效率较低。需要更有效的可解释方法。

**Method:** 提出FAME方法，结合CAM和扰动方法的优势。使用网络梯度计算输入图像的变化，以梯度驱动的方式操作图像，而非使用固定块，生成特征激活图来解释模型预测。

**Result:** 在图像分类和人脸识别任务上进行了实验，证明CAM的假设在深层网络中不成立。定性和定量评估显示FAME生成的归因图与现有最先进系统相比具有竞争力。

**Conclusion:** FAME成功整合了基于梯度和基于扰动方法的优点，为深度学习模型的可解释性提供了有效解决方案。代码已开源，便于后续研究和应用。
...
### [189] [What-Where Transformer: A Slot-Centric Visual Backbone for Concurrent Representation and Localization](https://arxiv.org/abs/2605.12021)
*Ryota Yoshihashi,Masahiro Kada,Satoshi Ikehata,Rei Kawakami,Ikuro Sato*

Main category: cs.CV

> 本文提出了What-Where Transformer (WWT)，通过分离物体外观(what)和空间位置(where)信息，使模型在图像分类监督下能实现多物体发现和定位任务。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图像理解任务中，定位相关的任务（如物体发现、检测、分割）比分类任务更复杂，可能因为分类骨干网络强调语义信息而隐含地纠缠或抑制了位置信息。

**Method:** 提出What-Where Transformer，将token视为'what'表示，注意力图视为'where'表示，通过多流槽位架构并行处理，并重用最终层token和注意力图直接参与下游任务的梯度传播。

**Result:** 在ImageNet单标签分类监督下，WWT能直接从注意力图实现多物体发现，在零样本物体发现和弱监督语义分割上优于ViT方法，且可迁移到多种定位设置。

**Conclusion:** 通过引入what-where分离的归纳偏置，WWT有效解耦了语义和空间信息，使分类模型具备强大的定位能力，为统一图像理解任务提供了新思路。
...
### [190] [Spectral Vision Transformer for Efficient Tokenization with Limited Data](https://arxiv.org/abs/2605.12026)
...
> 提出了一种新型谱视觉Transformer架构，通过谱投影实现高效token化，在医疗成像等有限数据场景下表现优异，参数更少且性能更强。

<details>
  <summary>Details</summary>

**Motivation:** 针对视觉Transformer在有限数据（特别是医学成像）场景下tokenization效率低下的问题，需要开发一种更高效、参数更少的模型架构。

**Method:** 采用谱视觉Transformer架构，利用谱投影代替空间变换，利用基函数选择获得空间不变性和最优信噪比等理论特性。

**Result:** 与多种模型相比，在参数数量更少的情况下达到了相当或更优越的性能，包括紧凑和标准视觉Transformer、带注意力的CNN、移位窗口Transformer、多层感知机和逻辑回归。

**Conclusion:** 谱视觉Transformer通过理论优势（空间不变性、最优信噪比）和计算效率（降低复杂度），为有限数据场景特别是医疗成像提供了一种高效解决方案。代码已开源。
...
### [191] [4DVGGT-D: 4D Visual Geometry Transformer with Improved Dynamic Depth Estimation](https://arxiv.org/abs/2605.12027)
...
> 本文提出了一种免训练的渐进式解耦框架，通过分离相机运动和物体运动，从单目视频中重建高质量的4D动态场景，在无需微调的情况下实现了竞争性性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有3D基础模型在动态环境中性能显著下降，根本原因在于全局注意力机制中相机自运动和物体运动产生了固有耦合，这导致了动态场景重建的困难。

**Method:** 提出由粗到细的渐进式解耦框架，包含三个核心组件：(1)动态掩码引导的姿态解耦模块，分离姿态估计与动态干扰；(2)拓扑子空间手术机制，正交分解深度流形；(3)基于信息论的置信度感知融合策略，将深度整合建模为异方差贝叶斯推断问题。

**Result:** 在标准4D重建基准测试中，该方法在主要点云指标上取得了稳定且显著的改进，在无需微调的情况下展现出 competitive 的鲁棒4D场景重建性能。

**Conclusion:** 基于数学原理的动态-静态解耦方法具有巨大潜力，为单目视频4D动态场景重建提供了一种有效且实用的解决方案。
...
### [192] [TAR: Text Semantic Assisted Cross-modal Image Registration Framework for Optical and SAR Images](https://arxiv.org/abs/2605.12064)
*Zhuoyu Cai,Dou Quan,Ning Huyan,Pei He,Shuang Wang,Licheng Jiao*

Main category: cs.CV

> 本文提出了一种名为TAR的文本语义辅助跨模态图像配准框架，利用文本先验信息提升光学与SAR图像在大几何形变下的配准性能。

<details>
  <summary>Details</summary>

**Motivation:** 光学与SAR图像配准在大几何形变条件下仍具挑战性，因为现有方法需要同时处理跨模态外观差异和复杂空间变换问题。

**Method:** TAR包含三个模块：多尺度视觉特征学习(MSFL)模块提取图像特征，文本辅助特征增强(TAFE)模块利用RemoteCLIP编码器提取文本特征并通过视觉-文本交互增强高层特征，粗到精密集匹配(CFDM)模块基于增强特征建立粗对应关系并用低层特征精化匹配位置。

**Result:** 在跨模态遥感图像上的实验表明，TAR比多个最先进方法表现出更强的匹配性能，在大几何形变条件下获得了显著提升。

**Conclusion:** 通过引入文本语义先验，TAR有效缩小了模态差距，增强了跨模态特征学习能力，为解决光学-SAR图像配准问题提供了新的技术路径。
...
### [193] [Anomaly-Aware Vision-Language Adapters for Zero-Shot Anomaly Detection](https://arxiv.org/abs/2605.12069)
*Muhammad Aqeel,Maham Nazir,Uzair Khan,Marco Cristani,Francesco Setti*

Main category: cs.CV

> AVA-DINO是一种零样本异常检测框架，利用正常和异常数据的天然不对称性，通过双分支结构实现专业化处理，在多个基准数据集上达到最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有零样本异常检测方法对正常和异常样本采用统一的特征变换，忽略了两者分布的根本不对称性：正常数据分布紧凑而异常数据分布多样。这种统一处理方式限制了检测性能。

**Method:** 提出AVA-DINO框架，包含两个专门分支分别处理正常和异常模式。训练时采用文本引导的路由机制和明确的路由正则化来鼓励分支专业化；测试时仅使用输入图像和预定义文本描述动态组合两个分支，实现非对称激活。

**Result:** 在9个工业和医疗基准测试中取得最先进性能，在MVTec-AD上达到93.5%的图像AUROC，并在医学成像领域展现强大的跨域泛化能力，无需特定领域微调。

**Conclusion:** 通过利用正常和异常数据的内在不对称性，AVA-DINO实现了更有效的特征变换，为零样本异常检测提供了新的解决方案，在工业检测和医疗诊断等应用中具有重要价值。
...
### [194] [PairDropGS: Paired Dropout-Induced Consistency Regularization for Sparse-View Gaussian Splatting](https://arxiv.org/abs/2605.12072)
*Hantang Li,Qiang Zhu,Xiandong Meng,Xingtao Wang,Debin Zhao,Xiaopeng Fan*

Main category: cs.CV

> PairDropGS是一种从一致性正则化角度改进稀疏视图3D高斯泼溅的方法，通过配对dropout策略和低频一致性约束实现更稳定的三维重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于dropout的稀疏视图3DGS方法专注于设计复杂的dropout策略，但忽略了不同dropout高斯子集之间的不一致性，导致重建不稳定和高斯表示学习次优。

**Method:** 提出配对dropout诱导的一致性正则化框架：1）从共享高斯场构建配对的dropout高斯子集；2）设计低频一致性正则化约束其低频渲染结构；3）引入渐进式一致性调度策略逐步加强正则化。

**Result:** 在广泛使用的稀疏视图基准测试上，PairDropGS实现了卓越的训练稳定性，显著超越了现有基于dropout的3DGS方法的重建质量，同时具有简单性和即插即用的特点。

**Conclusion:** 从一致性正则化角度重新审视dropout机制是有效的，PairDropGS为保证不同dropout子集间的一致性提供了简洁而强大的解决方案，可用于改进基于dropout的稀疏视图三维重建优化。
...
### [195] [BARISTA: A Multi-Task Egocentric Benchmark for Compositional Visual Understanding](https://arxiv.org/abs/2605.12074)
...
> BARISTA是一个包含185个真实世界咖啡制作视频的俯视角数据集和基准测试，密集标注了场景图，支持零样本文本 grounding 等多项任务评估。

<details>
  <summary>Details</summary>

**Motivation:** 现有基准通常分别评估模型在物体定位、操作识别等能力，无法有效诊断模型在过程性任务中的失败原因，因此需要一个能综合评估多种能力的诊断性基准。

**Method:** 构建了包含自动、胶囊和工作流三种咖啡制作方式的视频数据集，提供逐帧验证的场景图，包含物体身份、掩模、跟踪、属性、关系、手部交互、活动和进程步骤等标注，并从中衍生出多个零样本文本任务。

**Result:** 实验显示不同模型在各类任务间表现差异显著，没有统一占优的模型家族，证明了BARISTA作为过程性视频理解诊断基准的挑战性。

**Conclusion:** BARISTA为过程性视频理解提供了具有挑战性的诊断基准，填补了现有基准在综合能力评估方面的空白，有助于推动物理智能场景理解的研究。
...
### [196] [The Missing GAP: From Solving Square Jigsaw Puzzles to Handling Real World Archaeological Fragments](https://arxiv.org/abs/2605.12077)
*Ofir Itzhak Shahar,Gur Elkin,Ohad Ben-Shahar*

Main category: cs.CV

> 本研究提出了GAP数据集（包含合成的不规则形状、严重侵蚀的拼图碎片）和PuzzleFlow框架（基于ViT和流匹配的拼图求解方法），突破了传统拼图碎片必须是正方形的限制。

<details>
  <summary>Details</summary>

**Motivation:** 现有拼图求解方法大多局限于严格正方形的拼图碎片，这种限制在现实场景（如考古碎片重构）中不适用。需要开发能够处理不规则形状和严重侵蚀碎片的拼图求解方法。

**Method:** 提出了GAP数据集，通过学习现实考古碎片的分布生成合成重侵蚀的不规则形状拼图碎片；设计了基于ViT和Flow-Matching的PuzzleFlow框架来处理复杂拼图碎片的重组问题。

**Result:** PuzzleFlow在GAP数据集上表现出色，相比经典和最新的拼图求解方法都展现出更优越的性能，证明了处理不规则、侵蚀碎片的能力。

**Conclusion:** 通过GAP数据集和PuzzleFlow框架，本研究成功突破了传统拼图求解中碎片形状的正方形限制，为处理现实世界中的复杂拼图问题提供了新的解决方案。
...
### [197] [UniCustom: Unified Visual Conditioning for Multi-Reference Image Generation](https://arxiv.org/abs/2605.12088)
*Yiyan Xu,Qiulin Wang,Wenjie Wang,Yunyao Mao,Xintao Wang,Pengfei Wan,Kun Gai,Fuli Feng*

Main category: cs.CV

> UniCustom是一个统一视觉条件框架，通过在VLM编码前融合ViT和VAE特征，解决了多参考图像生成中的身份混淆和属性泄露问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多参考图像生成模型采用分离的视觉条件策略，使得模型难以将语义特征与正确的参考图像外观细节关联，导致复杂场景中出现身份保持失败和跨参考混淆。

**Method:** 提出UniCustom框架，在VLM编码前早期融合ViT和VAE特征；采用两阶段训练策略（重构预训练+监督微调）；引入slot-wise绑定正则化来减少跨参考纠缠。

**Result:** 在两个多参考生成基准测试上，UniCustom在主体一致性、指令遵循性和组合保真度方面均优于强基线模型。

**Conclusion:** 统一的视觉条件框架能够有效融合语义和外观信息，显著提升多参考图像生成质量，为复杂场景下的身份保持提供了新的解决方案。
...
### [198] [When Policy Entropy Constraint Fails: Preserving Diversity in Flow-based RLHF via Perceptual Entropy](https://arxiv.org/abs/2605.12112)
*Xiaofeng Tan,Jun Liu,Bin-Bin Gao,Yuanting Fan,Xi Jiang,Chengjie Wang,Hongsong Wang,Feng Zheng*

Main category: cs.CV

> 该论文发现RLHF对流匹配文本到图像模型进行对齐时会导致严重的多样性崩溃，传统基于策略熵的正则化方法在此失效，因此提出感知熵概念及相应策略来解决这个问题。

<details>
  <summary>Details</summary>

**Motivation:** RLHF虽然被广泛用于对齐文本到图像模型，但会导致严重的多样性崩溃。传统的熵正则化基于策略熵与多样性相关的假设，但在流模型中，策略熵保持不变而感知多样性却大幅下降，现有方法无法有效维持生成多样性。

**Method:** 理论上解释了策略熵固定（源于预定义的噪声调度）与感知多样性崩溃（源于策略梯度的模态寻求特性）之间的不匹配，提出感知熵概念来捕捉感知空间中的多样性，并设计了两种熵正则化策略：感知熵约束（PEC）和生成空间感知约束。

**Result:** 实验显示所提方法在质量和多样性权衡上取得一致提升：PEC获得0.734的最高总分（基线0.366），互补设置下多样性平均达到0.989（基线0.047）。验证了在多个基础模型、奖励类型和感知空间上的有效性。

**Conclusion:** 流模型中策略熵与感知多样性的不匹配需要新的度量方式，感知熵及相关策略能有效解决RLHF导致的多样性崩溃问题，为文本到图像模型的对齐提供了更有效的工具。
...
### [199] [MoCam: Unified Novel View Synthesis via Structured Denoising Dynamics](https://arxiv.org/abs/2605.12119)
*Haofeng Liu,Yang Zhou,Ziheng Wang,Zhengbo Xu,Zhan Peng,Jie Ma,Jun Liang,Shengfeng He,Jing Li*

Main category: cs.CV

> MoCam通过在扩散过程中结构化地协调几何先验和外观先验的使用，实现了高质量的生成式新视角合成，尤其在几何信息不完整时表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 生成式新视角合成面临核心困境：几何先验提供空间对齐但在视角变化下稀疏不准确，外观先验提供视觉保真度但缺乏几何对应。现有方法要么传播几何误差，要么在融合静态先验时产生信号冲突。

**Method:** MoCam采用结构化去噪动力学，在扩散过程中协调几何到外观的渐进过程：早期阶段利用几何先验锚定粗略结构并容忍其不完整性，后期阶段切换到外观先验主动校正几何误差并细化细节。

**Result:** 实验表明MoCam显著优于现有方法，特别是在点云包含严重孔洞或扭曲时，实现了鲁棒的几何-外观解耦，并在静态和动态视角合成中都表现良好。

**Conclusion:** 通过在扩散过程中时间解耦几何对齐和外观细化，MoCam为生成式新视角合成提供了统一的解决方案，解决了现有方法在处理不完整几何信息时的局限性。
...
### [200] [MULTI: Disentangling Camera Lens, Sensor, View, and Domain for Novel Image Generation](https://arxiv.org/abs/2605.12134)
*Sonali Godavarthy,Matthias Neuwirth-Trapp,Tim-Felix Faasch,Maarten Bieshaar,Michael Moeller,Danda Pani Paudel*

Main category: cs.CV

> 本文提出MULTI方法，通过文本反演实现多因子解耦，解决了文本到图像模型中成像因子控制的难题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本到图像模型虽然能生成高质量图像，但文本歧义性阻碍了对特定风格或对象的精确控制。当前工作主要集中在图像内容上，忽视了相机镜头、传感器类型、成像视角等成像因子。

**Method:** 提出两阶段方法MULTI：第一阶段学习通用因子，第二阶段提取数据集特定因子。该方法支持扩展现有数据集、新颖因子组合，并可结合ControlNets进行特定因子修改和图像到图像生成。

**Result:** 在新的DF-RICO基准测试上，MULTI方法表现出色，证明了因子解耦作为新研究方向的重要性。

**Conclusion:** 成像因子解耦是一个重要且具有挑战性的新研究方向，MULTI方法为此提供了有效的解决方案，为精确控制图像生成过程开辟了新途径。
...
### [201] [EchoTracker2: Enhancing Myocardial Point Tracking by Modeling Local Motion](https://arxiv.org/abs/2605.12140)
*Md Abulkalam Azad,Vegard Holmstrøm,John Nyberg,Lasse Lovstakken,Håvard Dalen,Bjørnar Grenne,Andreas Østvik*

Main category: cs.CV

> EchoTracker2是一种用于超声心动图中心肌点跟踪的新型单阶段架构，通过结合局部时空上下文和长程时序推理，显著提升了跟踪精度和一致性。

<details>
  <summary>Details</summary>

**Motivation:** 心肌运动具有独特的生理约束特性，表现为时空连续且局部受限的变形，这与自然视频中的运动存在根本差异。现有两阶段粗到细架构的粗初始化可能对此任务不必要，需要针对心肌运动的特性重新设计架构。

**Method:** 提出EchoTracker2单阶段架构，仅在细阶段工作。通过丰富像素级精度的局部时空上下文特征，并将其与长程联合时序推理相集成，实现鲁棒的点跟踪。

**Result:** 在分布内、分布外和公共合成数据集上，相比领域特定SOTA模型位置准确率提升6.5%，轨迹中位误差减少12.2%；相比最佳通用点跟踪方法分别提升2.0%和5.3%。同时显示出与专家衍生的全局纵向应变更好的一致性和重复性。

**Conclusion:** EchoTracker2证明了针对心肌运动特性的单阶段架构设计的有效性，为超声心动图中的运动估计提供了更准确、可靠的解决方案。
...
### [202] [PoseCompass: Intelligent Synthetic Pose Selection for Visual Localization](https://arxiv.org/abs/2605.12144)
*Yanan Zhou,Zhaoyan Qian,Yanli Li,Nan Yang,Zhongliang Guo,Dong Yuan*

Main category: cs.CV

> PoseCompass通过智能姿态选择 pipeline 显著加速了3DGS基础的绝对位姿回归，将适应时间缩短3倍同时减少53.8%的中位姿态误差。

<details>
  <summary>Details</summary>

**Motivation:** 现有的绝对位姿回归方法严重依赖微调数据质量和覆盖范围，而基于3D高斯散射的数据增强方法通过随机采样产生冗余视图和噪声样本。

**Method:** 提出了PoseCompass智能姿态选择框架，包含基于价值的三维度姿态排序机制（定位难度、覆盖新颖度、渲染可观测性），随后进行轨迹约束候选生成、Top-K选择和轻量级扩散对齐的视图合成。

**Result:** 在7-Scenes数据集上将适应时间从15.2分钟降至5.1分钟（3倍加速），中位姿态误差减少53.8%，显著优于随机基线。

**Conclusion:** PoseCompass证明了通过智能姿态选择策略可以大幅提升基于3DGS的视觉定位系统的训练效率和准确性。
...
### [203] [Cross-Modal-Domain Generalization Through Semantically Aligned Discrete Representations](https://arxiv.org/abs/2605.12145)
*Souptik Sen,Raneen Younis,Zahra Ahmadi*

Main category: cs.CV

> CoDAAR是一种新的多模态学习框架，通过跨模态码书索引级对齐，在保持模态特定性的同时实现了可泛化的跨模态表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态学习方法面临两难困境：连续方法保留细粒度结构但泛化困难，离散方法强制共享原型但损失模态特异性。

**Method:** 提出CoDAAR框架，包含两个互补机制：离散时间对齐(DTA)实现细粒度时间量化，级联语义对齐(CSA)促进渐进式跨模态语义一致，在统一离散空间中建立无竞争的表示。

**Result:** 在事件分类、定位、视频分割和跨数据集迁移等跨模态泛化基准测试中达到最先进性能，展现出强大的跨模态和跨域泛化能力。

**Conclusion:** CoDAAR成功解决了跨模态泛化与模态特异性之间的长期权衡问题，为离散且可泛化的多模态表示学习建立了新范式。
...
### [204] [Self-Consistent Latent Reasoning: Long Latent Sequence Reasoning for Vision-Language Model](https://arxiv.org/abs/2605.12163)
...
> 论文发现了潜在视觉推理中的悖论现象和根本原因，提出SCOLAR方法通过反Transformer结构解决信息坍塌问题，显著提升了长序列推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 语言推理中更长的思维链能获得更好的性能，研究者自然期望视觉潜在推理也能从更长的潜在序列中受益。但实验却发现现有方法随着潜在序列增长性能反而系统性地下降。

**Method:** 发现根本原因是信息增益坍塌（自回归生成使每步高度依赖先验输出）和严重池化的图像嵌入无法提供有效监督。提出SCOLAR方法，采用轻量级反Transformer，利用LLM全序列隐藏态一次性生成独立的视觉标记，结合三阶段SFT和ALPO强化学习。

**Result:** 将可接受的潜在CoT长度扩展了30倍以上，在现实推理基准上超越开源模型达到SOTA（比骨干模型提升14.12%），并展现出强的分布外泛化能力。

**Conclusion:** 该研究揭示了视觉推理中长序列失败的核心机制，通过架构创新和训练策略突破了现有方法的瓶颈，为视觉语言模型的推理能力提升提供了新思路。
...
### [205] [UniFixer: A Universal Reference-Guided Fixer for Diffusion-Based View Synthesis](https://arxiv.org/abs/2605.12169)
*Sihan Chen,Xiang Zhang,Yang Zhang,Tunc Aydin,Christopher Schroers*

Main category: cs.CV

> UniFixer是一种通用的参考引导框架，通过粗到细的策略修复扩散模型在新视角合成中的多种退化问题，实现了零样本修复。

<details>
  <summary>Details</summary>

**Motivation:** 当前的扩散基方法在视图合成任务中存在质量退化问题，包括模糊细节和扭曲结构，这些是由像素到潜在压缩和扩散幻觉引起的。

**Method:** UniFixer采用三步粗到细策略：1)粗对齐模块对齐参考视图和退化的新视角；2)全局结构锚定机制修复几何扭曲；3)局部细节注入模块恢复细粒度纹理细节。

**Result:** UniFixer作为即插即用的精炼器，在不同类型的扩散退化上实现了零样本修复，在新视角合成和立体转换任务上达到了最先进的性能。

**Conclusion:** 通过从空间、时间和主干相关三个维度研究扩散退化问题，UniFixer为视图合成任务提供了一个通用而有效的解决方案，显著提升了生成质量。
...
### [206] [SyncDPO: Enhancing Temporal Synchronization in Video-Audio Joint Generation via Preference Learning](https://arxiv.org/abs/2605.12179)
*Xin Cheng,Xihua Wang,Ying Ba,Yuyue Wang,Kaisi Guan,Yinbo Wang,Wenpu Li,Ruihua Song*

Main category: cs.CV

> SyncDPO是一种基于直接偏好优化的后训练框架，通过创新的负样本构建策略来提升视频-音频联合生成的时间同步精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频-音频联合生成方法在语义对应方面表现良好，但在精确时间同步上存在挑战。传统监督微调使用的均方误差损失对细微时间错位惩罚不足，无法有效提升模型的时序敏感性。

**Method:** 提出SyncDPO框架，将直接偏好优化应用于视频-音频联合生成。避免昂贵的采样排序过程，设计了一系列基于规则的实时负样本构建策略，通过扭曲时间结构来构造偏好对。还实现了课程学习策略，逐步增加负样本难度。

**Result:** 在四个多样基准测试上进行了广泛的客观和主观实验，SyncDPO在提升模型时间对齐能力方面显著优于其他方法，在分布外基准测试上也表现出优越的泛化能力。

**Conclusion:** 通过提供显式的负监督和课程的渐进式训练，SyncDPO有效增强了视频-音频联合生成的时间对齐能力，无需额外标注或采样成本，为解决时序敏感性问题提供了有效方案。
...
### [207] [Enhancing Domain Generalization in 3D Human Pose Estimation through Controllable Generative Augmentation](https://arxiv.org/abs/2605.12198)
*Xinhao Hu,Yiyi Zhang,Liqing Zhang,Jianfu Zhang*

Main category: cs.CV

> 本研究提出了一种可控的人体姿态生成框架，通过生成多样化的视频数据增强数据集，显著提升了3D人体姿态估计模型在未知场景下的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 行人运动具有因果关系，其分布受训练和测试数据之间的领域差异严重影响。现有的3D人体姿态估计方法在处理领域差异时存在局限性，需要新的数据增强策略来提升模型泛化能力。

**Method:** 构建可控的人体姿态生成框架，系统性地变化姿态、背景和相机视角来合成多样化的视频数据。利用室内/真实世界和室外/虚拟数据集进行跨领域数据融合和可控视频生成，构建针对实际部署场景的丰富训练数据。

**Result:** 增强的数据集在未见过的场景和数据集上显著提升了模型性能，证明了该方法的有效性。

**Conclusion:** 通过可控的视频生成和数据增强技术，有效缓解了领域差异问题，提升了3D人体姿态估计模型的实际应用价值。
...
### [208] [Learning Ego-Centric BEV Representations from a Perspective-Privileged View: Cross-View Supervision for Online HD Map Construction](https://arxiv.org/abs/2605.12218)
*Daniel Lengerer,Mathias Pechinger,Klaus Bogenberger,Carsten Markgraf*

Main category: cs.CV

> 提出跨视图监督（CVS）方法，通过从空中视角向多相机BEV编码器传递几何和拓扑先验知识，显著提升了HD地图构建性能，在不改变推理架构的情况下实现了远距离场景结构的准确建模。

<details>
  <summary>Details</summary>

**Motivation:** 当前基于多相机输入的BEV表示方法仅依赖以自我为中心的监督，由于遮挡、远距离信息密度下降以及透视效应和空间稀疏性，难以从观测不完整的数据中推断大规模场景结构，限制了结构推理的一致性。

**Method:** 跨视图监督（CVS）将几何和拓扑先验从对齐的空中视角视图转移到基于相机的BEV编码器中。该方法在共享的BEV特征空间中对齐表示，将具有视角优势的教师模型的全局一致结构知识提炼到以自我为中心的骨干网络中，而不添加辅助语义损失。

**Result:** 在nuScenes数据集上，CVS相对于StreamMapNet取得了持续改进：在60×30米标准区域提升3.9 mAP，在100×50米扩展区域提升9.9 mAP，对应44%的相对性能提升。这些改进在保持完全相同的仅相机推理条件下实现。

**Conclusion:** 视角特权结构监督是改进HD地图构建中BEV表示学习的有前景的训练原理。CVS方法证明，通过训练时引入空中视角先验知识，可以在不改变推理架构或要求测试时空中输入的情况下，显著提升BEV表示的结构一致性和远距离性能。
...
### [209] [TriBand-BEV: Real-Time LiDAR-Only 3D Pedestrian Detection via Height-Aware BEV and High-Resolution Feature Fusion](https://arxiv.org/abs/2605.12220)
*Mohammad Khoshkdahan,Alexey Vinel*

Main category: cs.CV

> TriBand-BEV是一种新的鸟瞰图编码方法，将3D LiDAR点云转换为轻量级2D张量，实现实时多类别3D目标检测，在KITTI数据集上达到49 FPS性能。

<details>
  <summary>Details</summary>

**Motivation:** 安全自主代理和移动机器人需要快速实时的3D感知，特别是针对行人等易受伤害的道路使用者(VRUs)，现有方法在精度和速度之间存在权衡。

**Method:** 提出三高度带BEV编码，将3D LiDAR点云映射为轻量2D张量；使用区域注意力骨干网络、分层双向颈部融合和带分布焦点的检测头；训练时应用垂直重分箱和反射抖动数据增强；最后使用IQR滤波去除噪声点。

**Result:** 在KITTI数据集上，行人BEV AP达到58.7%/52.6%/47.2%(简单/中等/困难)，处理速度49 FPS，相比Complex-YOLO分别提升12.6%/7.5%/3.1%，在遮挡情况下表现稳定。

**Conclusion:** 该管道紧凑高效，适合实时机器人部署，为自动驾驶等应用提供了实用的实时3D感知解决方案，源码已开源。
...
### [210] [UHR-Micro: Diagnosing and Mitigating the Resolution Illusion in Earth Observation VLMs](https://arxiv.org/abs/2605.12237)
*Shuo Ni,Tong Wang,Jing Zhang,He Chen,Haonan Guo,Ning Zhang,Bo Du*

Main category: cs.CV

> 研究揭示了高分辨率对地观测视觉-语言模型存在

<details>
  <summary>Details</summary>

**Motivation:** 现有的VLMs在处理超高分辩率对地观测图像时存在严重的尺度不匹配问题——大规模场景背景与微观尺度目标之间难以有效关联，这种现象被称为

**Method:** 提出了UHR-Micro基准数据集（包含11253条指令和1212张超高分辨率图像）用于评估模型的空间感知极限，并设计Micro-evidence Active Perception (MAP)智能体，通过将查询分解为证据搜索步骤、主动检查候选区域、基于局部观察定位答案的方式改善微观感知能力。          

**Result:** 实验表明，即使使用高分辨率输入，现有高分辨率VLM在空间定位和证据解析方面表现仍然较差；单纯增加模型容量无法完全解决这一问题，而MAP-Agent通过以证据为中心而非以图像为中心的推理方式显著提升了微观层面的感知能力。          

**Conclusion:** UHR-Micro和MAP-Agent共同构成了诊断平台，为评估、理解和推进对地观测VLM的高分辨率推理能力提供了重要工具，强调了在处理超高分辨率图像时需要更多任务相关微观证据的有效引导。          
...
### [211] [H3D-MarNet: Wavelet-Guided Dual-Path Learning for Metal Artifact Suppression and CT Modality Transformation for Radiotherapy Workflows](https://arxiv.org/abs/2605.12252)
*Mubashara Rehman,Niki Martinel,Michele Avanzo,Riccardo Spizzo,Christian Micheloni*

Main category: cs.CV

> H3D-MarNet是一个两阶段框架，用于从kVCT到MVCT的金属伪影感知域转换，结合小波预处理和混合CNN-Transformer架构，有效抑制金属伪影并保持解剖结构。

<details>
  <summary>Details</summary>

**Motivation:** 金属伪影严重降低CT图像质量，影响癌症患者的诊断准确性和放疗规划，特别是在高密度植入物存在的情况下。

**Method:** 第一阶段采用基于小波的预处理模块，通过频率感知去噪抑制金属伪影；第二阶段Domain-TransNet使用混合体积学习架构，集成CNN编码器捕获局部细节和Transformer编码器建模长距离依赖，通过注意力特征融合机制和多阶段解码器重建伪影抑制的MVCT体积。

**Result:** 在完整数据集上，H3D-MarNet在受伪影影响的切片上达到28.14 dB PSNR和0.717 SSIM，证明了有效的金属伪影抑制和解剖结构保持能力。

**Conclusion:** 该方法为临床放疗工作流中可靠的CT模态转换提供了潜在解决方案，在保证图像质量的同时有效处理金属伪影问题。
...
### [212] [From Image Hashing to Scene Change Detection](https://arxiv.org/abs/2605.12259)
*Anh-Kiet Duong,Marie-Claire Iatrides,Petra Gomez-Krämer,Jean-Michel Carozza*

Main category: cs.CV

> 本文提出了一种名为HashSCD的基于哈希的场景变化检测框架，它通过局部哈希编码实现了高效的全局变化检测和空间定位。

<details>
  <summary>Details</summary>

**Motivation:** 传统图像哈希方法只能进行全局比较，无法定位变化发生的具体位置，这限制了其在需要空间定位的场景变化检测任务中的直接应用。

**Method:** HashSCD采用分块哈希策略，将空间对齐的图像块编码为紧凑哈希码，通过类XOR操作聚合，使得变化检测和定位可直接在汉明空间进行，无需对先前图像重复推理；模型采用对比学习在局部和全局层面进行无监督训练。

**Result:** 实验结果表明HashSCD在保持与最新无监督哈希和场景变化检测方法相当性能的同时，显著降低了计算成本和存储需求。

**Conclusion:** 该工作成功将哈希技术扩展到场景变化检测领域，在保证检测精度的前提下实现了高效的空间化变化定位，为大规模图像变化检测提供了新的解决方案。
...
### [213] [CAD-feature enhanced machine learning for manufacturing effort estimation on sheet metal bending parts](https://arxiv.org/abs/2605.12266)
...
> 本文提出一种混合方法，将基于规则的制造特征识别与图机器学习相结合，通过在B-rep图中集成制造语义特征，提高了钣金弯曲可制造性预测的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 纯几何表示的图机器学习方法缺乏制造过程特定的语义信息，如零件表面作用、弯曲意图等关键制造因素，这些信息难以从形状本身可靠推断，限制了可制造性预测的准确性。

**Method:** 开发混合方法，首先通过基于规则的模块识别制造特征（如弯曲特性、法兰长度、表面作用），然后将这些特征作为节点属性集成到B-rep属性邻接图中，使学习信号集中在与过程相关的几何模式上。

**Result:** 在大规模合成可制造性基准数据集和真实工业弯曲时间数据集上的实验表明，结合领域知识的图学习方法在两个任务上都提高了预测精度，这是首次在真实生产数据上的验证。

**Conclusion:** 混合建模为工业CAD环境中可制造性评估和工时估算的部署工具提供了可行且有效的路径，证明了将规则驱动的领域知识与数据驱动学习相结合的价值。
...
### [214] [Beyond Text Prompts: Visual-to-Visual Generation as A Unified Paradigm](https://arxiv.org/abs/2605.12271)
...
> 论文提出视觉到视觉(V2V)生成方法，用户使用可视化规范页面而非文本提示来控制生成模型，V2V-Zero框架无需训练即可实现，在GenEval上达到0.85分数，展现出良好的视觉效果控制能力。

<details>
  <summary>Details</summary>

**Motivation:** 现代视觉生成器要求用户将意图序列化为文本，这会造成空间结构、精确外观和字形形状等信号的压缩瓶颈。人类通常通过视觉工件来指定和创造，现有方法无法充分利用这种直观的交互方式。

**Method:** 提出V2V-Zero训练免费框架，通过从视觉页面提取最终层隐藏状态替换纯文本条件，利用冻结的VLM已经将文本和图像映射到生成器条件空间的事实，实现视觉页面而非文本提示的条件生成。

**Result:** 在GenEval上V2V-Zero达到0.85分数，接近其优化后的文本到图像性能。在Simple-V2V Bench上获得32.7/100分数，超越了评估的开源图像基线。机制分析显示95.0%的条件标记注意力集中在视觉页面隐藏状态上。

**Conclusion:** V2V-Zero成功实现了视觉到视觉生成接口，揭示了能力层次：属性绑定强，内容生成不可靠，结构控制仍然困难。该框架不仅适用于图像，还可扩展到视频领域，为视觉生成提供了新的交互范式。
...
### [215] [Large-Small Model Collaboration for Farmland Semantic Change Detection](https://arxiv.org/abs/2605.12282)
*Xinjia Li,Rui Wang,Qiurong Peng,Lingfei Ye,Dengrong Zhang,Haoyu Zhang*

Main category: cs.CV

> 针对农田语义变化检测的挑战，研究团队构建了大规模精细粒度农田数据集HZNU-FCD，并提出一种大小模型协作框架FD-Mamba+，仅用665万参数就实现了97.63%的F1分数。

<details>
  <summary>Details</summary>

**Motivation:** 现有农田语义变化检测缺乏专门的数据集和有效模型。当前数据集缺少'from-to'标注，而视觉模型容易受到作物轮作、季节变化和光照差异引起的伪变化干扰，无法满足精细化农田保护需求。

**Method:** 构建包含4588对双时相图像的HZNU-FCD基准数据集，采用五类农田到非农田标注协议。提出大小模型协作框架：小型模型FD-Mamba学习密集变化表示，大型模型CMLA基于CLIP文本先验进行语义仲裁，并通过难区域协同训练机制实现有效协作。

**Result:** 在HZNU-FCD上达到97.63% F1、96.32% IoU和96.35% SCD_IoU_mean，比ChangeCLIP-ViT提升10.19个百分点。在LEVIR-CD和WHU-CD上也分别达到91.43%/84.21%和93.85%/88.41%的优异性能。

**Conclusion:** 该研究通过构建专用数据集和创新的大小模型协作框架，有效解决了农田变化检测中的伪变化干扰问题，为精细化农田保护提供了强大的技术支撑，展现出良好的鲁棒性和泛化能力。
...
### [216] [EgoEV-HandPose: Egocentric 3D Hand Pose Estimation and Gesture Recognition with Stereo Event Cameras](https://arxiv.org/abs/2605.12297)
*Luming Wang,Hao Shi,Jiajun Zhai,Kailun Yang,Kaiwei Wang*

Main category: cs.CV

> 提出了一种基于双目事件流的端到端框架EgoEV-HandPose，用于联合3D双手姿态估计和手势识别，并发布了首个大规模真实世界双目事件相机数据集EgoEVHands。

<details>
  <summary>Details</summary>

**Motivation:** 传统的基于帧的相机存在运动模糊和动态范围限制问题，而现有基于事件的方法受自我运动干扰、单目深度模糊和缺乏大规模真实世界双目数据集的限制。

**Method:** 提出了端到端框架EgoEV-HandPose，核心是KeypointBEV双目融合模块，将特征提升到规范的鸟瞰空间，采用迭代重投影引导的精化循环逐步解决深度不确定性并强制运动学一致性。

**Result:** 在MPJPE达到30.54mm，Top-1手势识别准确率达86.87%，显著优于基于RGB的双目方法和先前的事件相机方法，特别在低光和双手遮挡场景下表现出色。

**Conclusion:** 为基于事件的第一人称感知设定了新的基准，数据集和源代码将公开发布，推动了沉浸式AR/VR、人机交互和机器人技术领域的发展。
...
### [217] [Images in Sentences: Scaling Interleaved Instructions for Unified Visual Generation](https://arxiv.org/abs/2605.12305)
*Yabo Zhang,Kunchang Li,Dewei Zhou,Xinyu Huang,Xun Wang*

Main category: cs.CV

> INSET是一个将图像作为原生词汇嵌入文本指令的统一生成模型，通过在语义位置直接嵌入视觉特征，显著提升了复杂交织指令下的多图像生成性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态语言模型在处理复杂的交织指令时性能不足，主要原因是图像和文本的结构分离导致模型难以建立长距离依赖关系来匹配描述与视觉目标。

**Method:** 提出了INSET模型，将图像无缝嵌入文本指令作为原生词汇；同时构建了数据引擎，利用VLMs和LLMs从标准数据集合成了1500万个高质量交织样本。

**Result:** 在InterleaveBench评测中，INSET在多图像一致性和文本对齐方面显著优于现有方法，且随着输入复杂度增加性能差距进一步扩大。

**Conclusion:** 该方法不仅解决了复杂交织指令下的生成难题，还天然支持多模态图像编辑，将视觉内容作为指令的一部分实现高度表现力和创造性的视觉操控。
...
### [218] [G$^2$TR: Generation-Guided Visual Token Reduction for Separate-Encoder Unified Multimodal Models](https://arxiv.org/abs/2605.12309)
*Junxian Li,Kai Liu,Zizhong Ding,Zhixin Wang,Zhikai Chen,Renjing Pei,Yulun Zhang*

Main category: cs.CV

> G²TR是一种生成引导的视觉令牌减少框架，通过利用生成分支的信号来识别和理解端视觉令牌，在保持推理精度和编辑质量的同时，显著减少了Unified多模态模型的计算开销。

<details>
  <summary>Details</summary>

**Motivation:** 分离编码器的统一多模态模型存在推理成本高的问题，现有的视觉令牌压缩方法主要针对判别型推理任务设计，不适用于需要同时支持图像编辑能力的需求。

**Method:** G²TR通过生成分支提供任务无关的信号来评估令牌重要性，基于与VAE潜在空间的一致性进行令牌重要性评估，执行平衡的令牌选择，并将冗余令牌合并到保留的代表令牌中以减少信息损失。

**Result:** 实验表明G²TR在图像理解和编辑基准上，将视觉令牌和预填充计算量减少1.94倍，同时保持了推理精度和编辑质量，在几乎所有基准上都优于基线方法。

**Conclusion:** G²TR是免训练、即插即用的解决方案，仅在理解编码阶段后应用，与现有UMM推理管线兼容，有效解决了统一多模态模型的效率问题。
...
### [219] [Contrastive Learning under Noisy Temporal Self-Supervision for Colonoscopy Videos](https://arxiv.org/abs/2605.12320)
*Luca Parolari,Pietro Gori,Lamberto Ballan,Carlo Biffi,Loic Le Folgoc*

Main category: cs.CV

> 本文提出了一种基于结肠镜检查时序结构的噪声感知自监督对比学习方法，无需昂贵的人工标注即可学习息肉轨迹的鲁棒表示，并在多项下游任务中表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 学习息肉轨迹的鲁棒表示对多项AI辅助结肠镜应用至关重要，但传统的监督对比学习需要准确的正负样本定义，这需要耗费大量成本来标注同一息肉在不同时间点的关联，且需要专业临床知识。

**Method:** 利用结肠镜检查的序贯工作流程，从时序结构中推导自监督关联；针对时序推导关联的不确定性，引入噪声感知对比损失来处理噪声关联。使用轻量级编码器在仅27个视频上训练。

**Result:** 该方法在息肉检索与重识别、尺寸估计、组织学分类等多项下游任务中表现有效，显著优于之前的自监督和监督基线方法，在所有任务上与最新的基础模型相当或超越。

**Conclusion:** 通过巧妙利用时序信息和噪声感知机制，成功解决了标注成本高的难题，实现了仅用少量数据就学习到高质量息肉表示的目标，为AI辅助结肠镜应用提供了实用解决方案。
...
### [220] [VIP: Visual-guided Prompt Evolution for Efficient Dense Vision-Language Inference](https://arxiv.org/abs/2605.12325)
*Hao Zhu,Shuo Jin,Wenbin Liao,Jiayu Xiao,Yan Zhu,Siyue Yu,Feng Dai*

Main category: cs.CV

> 本文提出VIP方法，通过视觉引导的提示演化来解决dino.txt框架中的语义歧义问题，实现了无需训练的开放词汇语义分割，在多个指标上超越现有方法且推理开销小。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于CLIP的训练无关开放词汇语义分割方法存在严重的空间偏向问题，而虽然dino.txt框架具有强大的空间感知能力，但其文本查询的语义歧义导致密集跨模态交互中出现了严重的不匹配。

**Method:** 提出VIP（Visual-guided Prompt evolution）方法，集成别名扩展和视觉引导的蒸馏机制来挖掘有价值的语义线索，并以显著性感知的方式进行鲁棒聚合，从而修正dino.txt中文本查询的语义表达能力。

**Result:** VIP在评估中表现优异：相比最先进方法提升了1.4%~8.4%的平均mIoU，在多个具有挑战性的领域上泛化良好，且仅需极少的推理时间和内存开销。

**Conclusion:** VIP成功突破了CLIP范式的限制，通过解决dino.txt的语义歧义问题，实现了高效、高质量的训练无关开放词汇语义分割，为细粒度对象感知提供了新思路。
...
### [221] [Fill the GAP: A Granular Alignment Paradigm for Visual Reasoning in Multimodal Large Language Models](https://arxiv.org/abs/2605.12374)
*Yanting Miao,Yutao Sun,Dexin Wang,Mengyu Zhou,Pascal Poupart,Lei Lv,Qi Zhao,Li Wang,Hao Li,Xiaoxi Jiang,Guanjun Jiang*

Main category: cs.CV

> GAP是一种用于多模态大语言模型视觉潜推理的新型三重对齐范式，通过特征级、上下文级和容量引导级对齐解决了现有方法中的特征空间不匹配问题，提升了模型性能和稳定性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉潜推理方法存在不稳定的性能增益，主要原因是特征空间不匹配：预归一化MLLM重用解码器隐藏状态作为潜在输入时，这些状态与模型训练时消费的输入嵌入处于不同的范数域，导致直接潜在反馈不可靠。

**Method:** 提出GAP（粒度对齐范式），在三个层面进行对齐：1）特征级对齐通过轻量级PCA对齐潜在头将解码器输出映射为输入兼容的视觉潜在；2）上下文级对齐使用可检查的辅助视觉监督对潜在目标进行 grounding；3）容量引导对齐为基础MLLM困难的示例分配潜在监督。

**Result:** 在Qwen2.5-VL 7B上，实现了监督变体中最佳的平均感知和推理性能。推理时间干预探测表明，生成的潜在信号提供了超越单纯增加token槽的任务相关视觉信息。

**Conclusion:** GAP范式有效解决了视觉潜推理中的特征空间不匹配问题，通过多层次对齐策略显著提升了MLLM的视觉推理能力和稳定性。
...
### [222] [Fast Image Super-Resolution via Consistency Rectified Flow](https://arxiv.org/abs/2605.12377)
*Jiaqi Xu,Wenbo Li,Haoze Sun,Fan Li,Zhixin Wang,Long Peng,Jingjing Ren,Haoran Yang,Xiaowei Hu,Renjing Pei,Pheng-Ann Heng*

Main category: cs.CV

> FlowSR是一种基于校正流的单步图像超分辨率方法，通过改进的一致性学习和快慢调度策略，实现了高效的图像重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的扩散模型在图像超分辨率中需要多步采样，计算成本高昂，阻碍了实际应用。已有的少步或单步方法要么效率低下，要么未能充分利用迭代生成先验，导致重建图像质量和保真度不足。

**Method:** 将超分辨率问题建模为从低分辨率到高分辨率图像的校正流。提出改进的一致性学习策略，加入高分辨率正则化，确保学习的流既保持自一致性又精确收敛到真实目标。引入快慢调度策略，使用快速调度器提高效率，使用慢速调度器捕获细粒度纹理细节。

**Result:** 实验结果表明，FlowSR在效率和图像质量两方面都取得了杰出表现，能够实现高质量的单步超分辨率重建。

**Conclusion:** FlowSR成功地将超分辨率问题转化为高效的校正流问题，通过一致性学习和调度策略的创新，为实际应用的实时高质量图像超分辨率提供了有效解决方案。
...
### [223] [SEMIR: Semantic Minor-Induced Representation Learning on Graphs for Visual Segmentation](https://arxiv.org/abs/2605.12389)
*Luke James Miller,Yugyung Lee*

Main category: cs.CV

> SEMIR是一个语义 minority 结构表示学习框架，通过学习与边界对齐的图minor来解耦推理与原生网格，在肿瘤分割任务中显著提升小结构分割效果。

<details>
  <summary>Details</summary>

**Motivation:** 大规模图像中的小而稀疏结构分割面临体素级网格计算限制和极端类别不平衡问题，传统方法需要固定区域化或下采样，计算成本与图像分辨率耦合，在少数结构最关键的区域削弱了边界证据。

**Method:** SEMIR通过参数化的边收缩、节点删除和边删除将底层网格图转换为紧凑的边界对齐图minor，保留从minor预测到网格标签的精确提升映射。将minor构建建模为少样本结构学习问题，通过边界Dice准则学习minor参数，支持几何和强度描述符的高效区域级GNN推理。

**Result:** 在BraTS 2021、KiTS23和LiTS三个肿瘤分割数据集上，SEMIR在实用运行时间内一致性地提升了minor结构的Dice分数，证明了其在高结构变异性和分布不确定性目标上的有效性。

**Conclusion:** SEMIR为高分辨率结构化视觉数据建立了任务自适应、拓扑保持的潜在表示学习框架，实现了精确解码，为解决小结构分割问题提供了新的有效途径。
...
### [224] [GeoQuery: Geometry-Query Diffusion for Sparse-View Reconstruction](https://arxiv.org/abs/2605.12399)
*Xiao Cao,Yuze Li,Youmin Zhang,Jiayu Song,Cheng Yan,Wen Li,Lixin Duan*

Main category: cs.CV

> GeoQuery是一种几何引导的扩散框架，通过预测深度图和相机位姿构建几何一致性，解决了3D高斯喷洒在稀疏视图下的严重渲染伪影问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3D高斯喷洒方法在稀疏视图约束下会产生严重伪影，而基于图像扩散模型的改进方法在渲染视图严重损坏时，由于损坏的查询特征导致错误的跨视图检索，无法有效修复伪影。

**Method:** 提出Geometry-guided Cross-view Attention (GCA)机制：1) 利用预测的深度图和相机位姿构建几何诱导的对应场，采样参考特征形成几何对齐的代理查询；2) 设计新的跨视图特征聚合流程，将跨视图注意力限制在每个代理查询的局部窗口内。

**Result:** GeoQuery能够无缝集成到现有基于扩散的流程中，在极端视图稀疏性下实现鲁棒重建。在稀疏视图新颖视图合成和渲染伪影移除的广泛实验中证明了方法的有效性。

**Conclusion:** 通过将生成先验与显式几何线索相结合，GeoQuery有效解决了传统方法在严重损坏情况下的跨视图检索失效问题，为稀疏视图3D重建提供了更鲁棒的解决方案。
...
### [225] [Beyond Localization: A Comprehensive Diagnosis of Perspective-Conditioned Spatial Reasoning in MLLMs from Omnidirectional Images](https://arxiv.org/abs/2605.12413)
*Yuangong Chen,Wai Keung Wong,Jiaxing Li,Ioannis Patras,Xu Zheng*

Main category: cs.CV

> 该研究揭示了多模态大语言模型在视角条件下的空间推理能力存在严重缺陷，并通过构建大规模基准测试证实了这一瓶颈问题，同时通过强化学习实验证明该缺陷具有一定的可塑性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大语言模型虽然具备强大的视觉感知能力，但在动态视角下的空间推理方面存在显著局限性。360度全景图像提供了广阔的场景覆盖范围，虽然减少了部分观测的歧义，但仍需要依赖特定视角的推理，这为研究动态视角下的空间推理提供了理想场景。

**Method:** 研究人员构建了PCSR-Bench基准测试，包含84,373个问答对，涵盖26个室内环境的2,600张全景图像。测试包含8个任务，从基础感知（对象计数、相对距离、相对方向）到高级视角条件空间推理（组合链、自中心旋转、视角重新锚定、自中心失真、有限视野可见性）。同时评估了14个代表性MLLM，并针对7B规模模型进行了基于强化学习的诊断性研究。

**Result:** 实验结果显示感知-推理差距巨大：基础相对方向任务准确率为57.59%，但自中心旋转任务降至13.49%，自中心失真任务降至7.13%，开放式组合推理仅为0.64%。强化学习研究表明，奖励塑形可以将7B基线模型的准确率从31.10%提升到60.06%，但这种可塑性是有限且具有选择性的。

**Conclusion:** 视角条件空间推理是当前多模态大语言模型的关键瓶颈，虽然通过有针对性的优化可以获得部分改善，但提升效果受任务选择、奖励设计和评估协议等多重因素影响，表明这一缺陷具有有限但重要的可塑性空间。
...
### [226] [AOI-SSL: Self-Supervised Framework for Efficient Segmentation of Wire-bonded Semiconductors In Optical Inspection](https://arxiv.org/abs/2605.12430)
*Joaquín Figueira,Rob Van Gastel,Giacomo D'Amicantonio,Zhuoran Liu,Ioan Gabriel Bucur,Faysal Boughorbel,Egor Bondarev*

Main category: cs.CV

> AOI-SSL是一个高效的线焊半导体光学检测框架，通过结合小域自监督预训练和上下文推理，减少了对标注样本的需求，提高了分割质量。

<details>
  <summary>Details</summary>

**Motivation:** 传统的线焊半导体光学检测分割模型具有设备特异性，当出现新设备或分布偏移时需要重新训练，这导致训练成本高昂且适应性差。

**Method:** 提出AOI-SSL框架，包含：1)在小工业检测数据集上对视觉Transformer进行自监督预训练，发现MAE在小数据场景下最有效；2)引入基于密度的上下文修补检索方法，直接从编码器嵌入预测掩码，几乎无需额外训练。

**Result:** 自监督预训练显著提升了分割质量，优于从头训练和ImageNet预训练；基于检索的分割在单设备图像上超越微调，实现近即时适应困难样本；简单相似性检索与复杂注意力聚合效果相当。

**Conclusion:** AOI-SSL框架通过自监督预训练和上下文推理，实现了高效、低标注需求的线焊半导体分割，为工业光学检测提供了实用的解决方案。
...
### [227] [GaitProtector: Impersonation-Driven Gait De-Identification via Training-Free Diffusion Latent Optimization](https://arxiv.org/abs/2605.12431)
*Huiran Duan,Qian Zhou,Zhongliang Guo,Junhao Dong,Yuqi Li,Guoying Zhao,Yingli Tian*

Main category: cs.CV

> GaitProtector是一种基于3D扩散模型的训练无关步态去识别框架，通过同时实现混淆和模拟目标身份来解决隐私保护与结构保真度之间的权衡问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统的步态去识别方法存在固有权衡：要么身份抑制不足，要么引入时空失真影响下游应用，特别是在医学诊断等结构敏感场景。

**Method:** 提出GaitProtector框架，将隐私保护统一为混淆（排斥源身份）和模拟（吸引向目标身份）两个耦合组件；利用预训练3D视频扩散模型作为先验，通过训练无关的扩散潜在优化管道，迭代优化潜码合成受保护步态。

**Result:** 在CASIA-B数据集上实现56.7%的黑盒步态识别模拟成功率，Rank-1识别准确率从89.6%降至15.0%；在Scoliosis1K数据集上，诊断准确率仅从91.4%降至74.2%，保持了良好的视觉和时间质量。

**Conclusion:** 这是首个利用预训练3D扩散先验以训练无关方式实现基于轮廓的步态去识别的工作，有效平衡了隐私保护和结构保真度的需求。
...
### [228] [3D Gaussian Splatting for Efficient Retrospective Dynamic Scene Novel View Synthesis with a Standardized Benchmark](https://arxiv.org/abs/2605.12437)
*Yunxiao Zhang,Suryansh Kumar*

Main category: cs.CV

> 该论文提出了一种适用于同步多视角的动态场景新视角合成方法，通过传播优化的高斯分布而不需要时间变形约束，并建立了一个可重现的动态多视角数据集框架。

<details>
  <summary>Details</summary>

**Motivation:** 在典型的体育等同步多视角设置中，每个时间步的动态场景已经具有很强的几何约束，作者质疑现有动态3D高斯溅射方法中引入的时间耦合约束的必要性。

**Method:** 通过初始化起始时间的SfM点云，并在时间上传播优化高斯分布，在不施加时间变形约束的情况下实现高效的回顾性新视角合成；同时建立了基于Blender的动态多视角数据集框架。

**Result:** 在同步多视角设置下，该方法能够使用3D高斯溅射实现高效的动态场景回顾性新视角合成，数据集框架使得动态新视角合成方法的基准测试具有可重现性和原则性。

**Conclusion:** 研究表明，在同步多视角环境中，显式的时间耦合或复杂的多体约束对于动态场景的回顾性新视角合成并非必需，同时提出的数据集框架为该领域的研究提供了重要的基准测试工具。
...
### [229] [LychSim: A Controllable and Interactive Simulation Framework for Vision Research](https://arxiv.org/abs/2605.12449)
*Wufei Ma,Chloe Wang,Siyi Chen,Jiawei Peng,Patrick Li,Alan Yuille*

Main category: cs.CV

> LychSim是一个基于虚幻引擎5构建的高可控性交互式仿真框架，旨在降低视觉系统仿真的技术门槛，提供Python API、程序化数据管道和LLM闭环集成三大核心功能。

<details>
  <summary>Details</summary>

**Motivation:** 尽管自监督预训练减少了对合成数据的依赖，但仿真在闭环优化和严格分布外评估中仍然不可或缺。现有仿真平台技术门槛高，需要大量计算机图形学和游戏开发专业知识。

**Method:** LychSim基于虚幻引擎5构建，包含三个关键设计：(1)抽象底层引擎复杂性的简化Python API；(2)能够生成多样化高保真环境并包含丰富2D/3D真实数据的程序化数据管道；(3)集成模型上下协议(MCP)将仿真器转变为LLM的动态闭环推理平台。

**Result:** 在多个下游应用中验证了LychSim的能力，包括作为合成数据引擎、驱动基于强化学习的对抗性测试器，以及实现交互式语言驱动的场景布局生成。

**Conclusion:** LychSim有效降低了视觉系统仿真的技术门槛，为研究社区提供了功能完整的开源仿真框架，支持合成数据生成、对抗性测试和交互式场景编辑等广泛应用。
...
### [230] [FuTCR: Future-Targeted Contrast and Repulsion for Continual Panoptic Segmentation](https://arxiv.org/abs/2605.12451)
*Nicholas Ikechukwu,Keanu Nichols,Deepti Ghadiyaram,Bryan A. Plummer*

Main category: cs.CV

> FuTCR框架通过对比学习和特征排斥机制，在持续全景分割任务中大幅提升新类别识别性能(最高提升28%)，同时保持基础类别性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有持续全景分割方法将未标注物体统一视为背景类别进行训练，导致模型忽略未来类别的重要特征信息，使得学习新新类别变得困难。

**Method:** FuTCR框架首先通过聚类识别具有非背景逻辑的背景区域作为未来类别的候选区域，然后应用像素到区域的对比学习构建连贯原型，同时排斥背景特征远离已知类别原型，为未来类别预留表示空间。

**Result:** 在6个CPS设置和不同规模数据集上的实验表明，FuTCR相比现有方法在新类别全景质量上相对提升最高达28%，基础类别性能保持或提升最高达4%。

**Conclusion:** FuTCR通过主动重构表示空间和预留特征空间，有效解决了持续学习中背景类别混淆问题，为持续全景分割提供了新的解决思路。
...
### [231] [OmniNFT: Modality-wise Omni Diffusion Reinforcement for Joint Audio-Video Generation](https://arxiv.org/abs/2605.12480)
*Guohui Zhang,XiaoXiao Ma,Jie Huang,Hang Xu,Hu Yu,Siming Fu,Yuming Li,Zeyue Xue,Lin Song,Haoyang Huang,Nan Duan,Feng Zhao*

Main category: cs.CV

> 论文提出OmniNFT框架，首个面向多目标多模态音视频生成的强化学习方法，通过模态感知的优势路由、分层梯度手术和区域损失重加权三大技术创新，显著提升了生成质量、跨模态对齐和精细同步性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有音视频生成方法在真实应用中面临单模态保真度、跨模态对齐和细粒度同步的挑战。虽然强化学习是很有前景的范式，但其在多目标多模态音视频生成中的应用尚未被探索，且传统RL方法存在多目标优势不一致、多模态梯度不平衡、统一信用分配等问题。

**Method:** 提出OmniNFT框架，包含三个核心创新：(1)模态感知优势路由，将独立奖励优势路由到对应模态生成分支；(2)分层梯度手术，选择性分离浅层音频层的视频分支梯度，保留跨模态交互层的梯度；(3)区域损失重加权，调节策略优化朝向音视频同步和细粒度对齐的关键区域。

**Result:** 在JavisBench和VBench数据集上，使用LTX-2主干网络的实验表明，OmniNFT在音视频感知质量、跨模态对齐和音视频同步方面实现了全面改进。

**Conclusion:** 该研究首次系统分析了强化学习在多模态音视频生成中的根本障碍，并提出了有效的解决方案。OmniNFT框架为多目标多模态生成提供了新的RL范式，在各项评估指标上都取得了显著提升。
...
### [232] [Elastic Attention Cores for Scalable Vision Transformers](https://arxiv.org/abs/2605.12491)
...
> VECA是一种新型视觉Transformer架构，通过核心-边缘结构注意力机制，在不进行直接块间交互的情况下实现了线性计算复杂度，在保持竞争力的同时大幅降低了计算成本。

<details>
  <summary>Details</summary>

**Motivation:** Vision Transformers虽然表现强大，但其全连接的自注意力机制导致计算复杂度随图像分辨率呈二次方增长，这限制了ViT在高分辨率领域的应用。传统方法假设成对的token交互对于学习视觉语义表示是必需的。

**Method:** 提出了VECA架构，使用一组可学习的核心tokens作为通信接口，让图像块tokens仅通过核心tokens交换信息。核心tokens从零初始化并在层间传播，实现了O(N)的线性复杂度。通过核心轴嵌套训练，模型能在推理时灵活权衡计算和精度。

**Result:** 在分类和密集预测任务上，VECA达到了与最新视觉基础模型竞争的性能，同时显著降低了计算成本。证明了不需要直接块间交互就能学习有效的视觉表示。

**Conclusion:** 弹性核心-边缘注意力机制是Vision Transformer的一种可扩展替代构建块，为高分辨率视觉任务提供了更高效的解决方案。
...
### [233] [Revisiting Photometric Ambiguity for Accurate Gaussian-Splatting Surface Reconstruction](https://arxiv.org/abs/2605.12494)
*Jiahe Li,Jiawei Zhang,Xiao Bai,Jin Zheng,Xiaohan Yu,Lin Gu,Gim Hee Lee*

Main category: cs.CV

> AmbiSuR是一个基于高斯泼溅的鲁棒三维表面重建框架，解决了光度模糊问题，通过内置的光度去模糊和模糊指示模块实现高性能重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于可微渲染的表面重建方法受到普遍存在的光度模糊问题的严重限制，这成为性能提升的主要瓶颈。

**Method:** 首先在高斯泼溅表示中发现两个原始级模糊，揭示其内在的自我指示潜力；然后提出光度去模糊约束病态几何解，最后设计模糊指示模块识别并修正欠约束重建。

**Result:** 在多种挑战性场景中，AmbiSuR相比现有方法实现了更优越的表面重建效果，具有广泛的兼容性，实验证明了其卓越性能。

**Conclusion:** 通过挖掘高斯泼溅的内在潜力并提出光度去模糊和模糊指示机制，AmbiSuR为光度模糊鲁棒的三维表面重建提供了新的解决方案，在复杂场景下表现出色。
...
### [234] [AlphaGRPO: Unlocking Self-Reflective Multimodal Generation in UMMs via Decompositional Verifiable Reward](https://arxiv.org/abs/2605.12495)
*Runhui Huang,Jie Wu,Rui Yang,Zhe Liu,Hengshuang Zhao*

Main category: cs.CV

> AlphaGRPO是一个应用GRPO到AR-Diffusion统一多模态模型的新框架，通过引入可分解验证奖励机制，实现无需冷启动的自我反思强化学习，显著提升了多模态生成和编辑能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态生成模型面临两大挑战：1）需要额外的冷启动阶段；2）难以提供稳定可靠的监督信号来指导生成质量。研究团队希望能够解锁模型的高级推理能力，实现对用户意图的主动推断和生成结果的自我修正。

**Method:** 提出AlphaGRPO框架，结合群体相对策略优化(GRPO)与可分解验证奖励(DVReward)。DVReward利用LLM将复杂用户请求分解为原子化的语义和质量问题，由通用MLLM评估提供可解释反馈。实现推理文本到图像生成和自我反思修正两大核心能力。

**Result:** 在GenEval、TIIF-Bench、DPG-Bench和WISE等多个多模态生成基准测试中取得显著改进，同时在GEdit编辑任务上获得大幅提升（即使未在编辑任务上训练），证明了方法的通用性和有效性。

**Conclusion:** AlphaGRPO通过自我反思的强化学习方法，有效利用模型固有理解能力指导高保真度生成，验证了无需冷启动的强化学习框架在提升多模态模型性能方面的巨大潜力。
...
### [235] [CausalCine: Real-Time Autoregressive Generation for Multi-Shot Video Narratives](https://arxiv.org/abs/2605.12496)
...
> CausalCine是一个交互式因果自回归视频生成框架，通过在线导演的方式实现多镜头视频的实时生成，解决了现有模型在长序列生成中的运动停滞和语义漂移问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自回归视频生成模型主要针对短时序连续性训练，将长序列视为扩展的单一镜头，在长序列生成时不可避免地出现运动停滞和语义漂移，无法满足电影叙事中需要的事件演进、视角转换和离散镜头边界要求。

**Method:** 1) 在原生多镜头序列上训练因果基础模型学习复杂镜头转换；2) 提出内容感知内存路由(CAMR)，基于注意力相关分数动态检索历史KV条目而非时间邻近性；3) 将因果基础模型蒸馏为少步生成器实现实时交互。

**Result:** 实验表明CausalCine显著超越自回归基线模型，接近双向模型能力，同时解锁因果生成的流式交互性。

**Conclusion:** CausalCine成功将多镜头视频生成转化为在线导演过程，实现了跨镜头因果生成、动态提示接受和上下文重用，为实时交互式视频叙事提供了新范式。
...
### [236] [From Web to Pixels: Bringing Agentic Search into Visual Perception](https://arxiv.org/abs/2605.12497)
*Bokang Yang,Xinyi Sun,Kaituo Feng,Xingping Dong,Dongming Wu,Xiangyu Yue*

Main category: cs.CV

> 本文研究了开放世界下的感知深度研究问题，提出了WebEye基准数据集和Pixel-Searcher方法来解决需要外部知识才能识别和定位目标的视觉感知任务。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉感知方法假设目标的识别证据已经存在于图像或预训练模型中，缺乏处理需要从外部事实、近期事件、长尾实体或多跳关系中获取知识的开放世界场景的能力。

**Method:** 构建了WebEye基准数据集（包含120张图像、473个对象实例、645个QA对），提出Pixel-Searcher智能体工作流程，通过搜索到像素的流水线解决隐藏目标识别并将其绑定到边界框、掩码或接地答案。

**Result:** 实验表明Pixel-Searcher在三个任务视图（基于搜索的接地、分割、VQA）上都取得了最强的开源性能，失败案例主要来自证据获取、身份解析和视觉实例绑定。

**Conclusion:** 感知深度研究是一个具有挑战性的开放世界问题，WebEye基准和Pixel-Searcher方法为解决需要外部知识的视觉感知任务提供了新的研究框架和评估标准。
...
### [237] [EgoForce: Forearm-Guided Camera-Space 3D Hand Pose from a Monocular Egocentric Camera](https://arxiv.org/abs/2605.12498)
*Christen Millerdurai,Shaoxiang Wang,Yaxu Xie,Vladislav Golyanik,Didier Stricker,Alain Pagani*

Main category: cs.CV

> 本文提出了EgoForce，一个单目3D手部重建框架，能够从用户视角使用单个头戴式摄像头重建绝对3D手部姿态和位置，适用于鱼眼、透视和失真宽视场摄像头。

<details>
  <summary>Details</summary>

**Motivation:** 在实际的AR/VR、远程呈现和以手为中心的操作任务中，需要对用户视手中的绝对3D手部姿态进行重建，但现有的单目RGB方法存在深度尺度模糊性问题，且难以泛化到不同光学配置的头戴设备上。

**Method:** EgoForce结合了可微分前臂表示来稳定手部姿态，统一的手臂-手部Transformer从单个自视角预测手部和前臂几何，以及射线空间闭式求解器来实现跨不同头戴摄像头模型的绝对3D姿态恢复。

**Result:** 在三个自视角基准测试中，EgoForce实现了最先进的3D精度，在HOT3D数据集上将相机空间MPJPE减少了28%，并在不同摄像头配置下保持了一致的性能。

**Conclusion:** EgoForce为头戴式设备的绝对3D手部重建提供了一个统一且鲁棒的解决方案，避免了设备特定数据集的昂贵和费力采集，推动了AR/VR等实际应用的发展。
...
### [238] [SenseNova-U1: Unifying Multimodal Understanding and Generation with NEO-unify Architecture](https://arxiv.org/abs/2605.12500)
...
> SenseNova-U1是一种原生统一多模态范式，通过NEO-unify技术将理解和生成为同一过程的不同视角，推出8B-MoT和A3B-MoT两个变体，在理解任务和生成任务上均达到顶级性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有大型视觉语言模型存在理解和生成的根本分裂，导致架构碎片化、级联管道和表示空间不对齐，这种结构性限制阻碍了原生多模态智能的涌现。

**Method:** 基于第一性原理设计，通过NEO-unify框架构建原生统一多模态范式，采用密集(8B)和专家混合(30B-A3B)两种理解基线，实现理解与生成的协同演化。

**Result:** 在文本理解、视觉语言感知、知识推理、智能体决策和空间智能等理解任务上匹敌顶级模型，在图像合成、信息图表生成和交错视觉语言生成等任务上表现出强语义一致性和视觉保真度。

**Conclusion:** SenseNova-U1证明了理解和生成可以统一为同一过程的不同视角，为视觉语言动作和世界模型应用奠定基础，指明了多模态AI不再是连接分离系统，而是构建统一系统的发展方向。
...
### [239] [Covering Human Action Space for Computer Use: Data Synthesis and Benchmark](https://arxiv.org/abs/2605.12501)
...
> 本文提出CUActSpot基准测试和基于渲染器的数据合成流水线，解决了计算机使用代理在复杂交互操作中的可靠性问题，训练出的Phi-Ground-Any-4B模型在小参数量开源模型中表现最佳。

<details>
  <summary>Details</summary>

**Motivation:** 现有计算机使用代理在复杂、低频交互操作上可靠性差，导致用户信任度低。研究发现故障案例呈现长尾分布模式，少数复杂交互占据了大部分失败，主要原因是复杂交互数据的稀缺性。

**Method:** 1) 提出CUActSpot基准测试，涵盖GUI、文本、表格、画布和自然图像五种模态，包含点击、拖拽、绘制等多种操作；2) 设计基于渲染器的数据合成流水线，自动生成场景、记录截图和坐标，使用LLM生成匹配的指令和动作轨迹；3) 在此数据集上训练Phi-Ground-Any-4B模型。

**Result:** 训练得到的Phi-Ground-Any-4B模型在复杂交互任务上超越了参数量少于32B的其他开源模型，验证了方法的有效性。

**Conclusion:** 通过构建更全面的基准测试和高效的数据合成方法，显著提升了计算机使用代理在复杂交互场景中的性能，为该领域的发展提供了新的解决方案和资源。
