...
> 本文针对推测性解码技术提出了一种理论，该理论能够预测在预训练之前推理系统组件的吞吐量最优超参数，以此优化推理系统的吞吐量。

<details>
  <summary>Details</summary>

**Motivation:** 之前的推测性解码研究使用了一种实验方法来优化推理流水线的吞吐量，这种方法涉及到LLM的训练并且成本较高。

**Method:** 本研究提出了一种理论，该理论从分析的角度将预训练的大型语言模型的关键超参数与基于推测性解码的下游推理系统的吞吐效率联系起来。

**Result:** 无具体结果提及。

**Conclusion:** 这一理论允许在组件预训练之前预测最佳的吞吐量超参数，从而优化推测性解码系统的性能。
...
> 本文提出了GGPT框架，通过几何引导增强3D重建，实现了更精细和一致的重建结果。

<details>
  <summary>Details</summary>

**Motivation:** 最近的前馈网络在从RGB图像直接预测稠密点图以实现稀疏视图3D重建方面取得了显著进展，但它们常常受到几何不一致性和有限的细粒度准确性的困扰，因为缺乏明确的多视图约束。我们提出的方法旨在通过整合几何先验和密集前馈预测来解决这些问题，以实现几何一致和空间完整且能恢复细小结构和填补无纹理区域的重建。

**Method:** 我们提出了Geometry-Grounded Point Transformer (GGPT)，这是一个通过可靠稀疏几何指导增强前馈重构的框架。首先，我们提出了一种基于密集特征匹配和轻量级几何优化的改进的从运动中恢复结构（SFM）管道，以从稀疏输入视图中高效地估计准确的相机姿态和部分3D点云。在此基础上，我们提出了一种几何引导的3D点转换，它在明确的部分几何监督下通过优化的指导编码来细化密集点图。

**Result:** 广泛的实验表明，我们的方法在集成几何先验与密集前馈预测方面提供了一个原理可靠的方法，实现了几何一致、空间完整且能恢复细部结构和补充无纹理区域的重建。

**Conclusion:** GGPT模型在ScanNet++数据集上进行了VGGT预测训练之后，能够很好地推广到不同的架构和数据集上，在内部和外部域设置中显著优于最先进的3D重建前馈模型。
...
> 提出了一种用于乳腺肿瘤分割的新型模型TextBCS，利用阶段分割的视觉-语言交互和证据学习来提升模型表现。实验表明，该模型在公开数据集上达到了最佳的乳腺肿瘤分割性能。

<details>
  <summary>Details</summary>

**Motivation:** 由于癌症和正常区域之间对比度低和边界模糊，基于深度学习的肿瘤分割方法在准确定位肿瘤轮廓方面存在局限性。因此，通过利用文本提示信息来改进肿瘤分割效果成为一个研究方向。它的目标是通过文本提示来帮助在对比度低的情况下定位病灶区域，解决深度学习模型在边界模糊场景下的不确定性问题。

**Method:** 提出了一种基于文本引导的乳腺肿瘤分割模型TextBCS，该模型引入了阶段分割的视觉-语言交互和证据学习，使得在每个下采样阶段都可以促进视觉特征和文本特征的信息相互作用，提升了文本提示的优势，同时，通过变分狄利克雷分布量化了模型对模糊边界的分割不确定度。

**Result:** 实验广泛验证了我们提出的TextBCS模型相较于其他分割网络的优越性，在公开数据集上展示了最佳的乳腺肿瘤分割性能。

**Conclusion:** 研究展示了TextBCS模型在乳腺肿瘤分割上的优越性，尤其是提升了在低对比度和模糊边界情况下的分割精度，并验证了其相较于其他方法的优势。
...
> 本文提出了SimE框架，通过在增量学习任务中使用特定的适配器连接，显著提升了模型性能，并在一个新的现象上进行了深入研究。

<details>
  <summary>Details</summary>

**Motivation:** 文章动机在于解决增量学习中预训练视觉语言模型的挑战，包括提高训练效率、减少对内存银行的依赖和增强模型 backbone。

**Method:** 方法是提出一个名为SimE的框架，使用适配器设计的视觉语言模型，并研究了适配器连接的非线性相关性对模型增量学习能力的影响。

**Result:** 实验结果表明SimE框架分别在TinyImageNet和CIFAR-100数据集上比传统方法提高了9.6%和CLIP-based方法提高了5.3%。

**Conclusion:** 结论是SimE框架有效地利用了视觉语言模型的零样本能力，并通过实验验证了其在增量学习任务中的优越性能。
...
> Senna-2 is introduced to improve the consistency between vision-language models and end-to-end driving policies in autonomous driving, enhancing the system's decision-making and planning capability and resulting in better driving outcomes.

<details>
  <summary>Details</summary>

**Motivation:** The motivation is to improve the consistency and effectiveness of the decision-making and planning between vision-language models and end-to-end driving policies, addressing the misalignment problem that leads to weakened top-down guidance and decision-following ability.

**Method:** Our method follows a consistency-oriented three-stage training paradigm. Stage one involves driving pre-training with a decision adapter that transmits VLM decisions to E2E policy through implicit embeddings. Stage two aligns VLM and E2E policy in an open-loop setting, and stage three refines this alignment with closed-loop Hierarchical Reinforcement Learning in 3DGS environments.

**Result:** Experiments show a 19.3% improvement in F1 score, indicating enhanced dual-system consistency. Additionally, there is a 5.7% reduction in FDE in the open-loop setting and a 30.6% reduction in AF-CR in the closed-loop setting, reflecting improved driving safety.

**Conclusion:** Senna-2 significantly enhances dual-system consistency and driving safety compared to previous methods. These improvements are evident in both open-loop and closed-loop settings with notable reductions in failure rate and collision risks.
