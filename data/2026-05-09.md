...
> AdaGATE是一个无需训练的多跳RAG证据控制器，通过将证据选择视为标记约束的修复问题，实现了在嘈杂和冗余检索环境下的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多跳RAG控制器在真实部署环境中表现脆弱，当检索的证据存在噪声或冗余，且只能向生成器传递有限的上下文时，系统难以有效工作。现有方法要么简单地扩展上下文，要么从固定的top-k集合中选择，要么在不显式修复缺失的桥接事实的情况下优化相关性。

**Method:** AdaGATE将证据选择建模为标记约束修复问题，结合了三个核心组件：1）以实体为中心的间隙跟踪；2）有针对性的微查询生成；3）基于效用的选择机制，该机制平衡了间隙覆盖、证实、新颖性、冗余性和直接问题相关性。

**Result:** 在HotpotQA数据集的清洁、冗余和噪声注入检索条件下，AdaGATE在所有三种设置中都实现了最佳证据F1值，达到清洁数据62.3%、冗余注入71.2%，同时使用的输入标记比Adaptive-k少2.6倍。

**Conclusion:** 研究结果表明，显式的间隙感知修复与标记高效的证据选择相结合，显著改善了多跳RAG在不完美检索条件下的鲁棒性，为解决实际部署中的证据选择问题提供了有效方案。
...
> 研究探讨了在生成式AI背景下，如何通过反驳论证写作来培养学生的批判性思维，并验证AI评估的有效性。

<details>
  <summary>Details</summary>

**Motivation:** 随着生成式AI的普及，学生存在作弊和认知卸载的风险，因此需要探索如何在AI环境下有效培养学生的批判性思维能力。

**Method:** 36名学生在选定论题中任选一个进行反驳论证写作；采用6个既定评分标准，通过2名同伴评议和1名教师评估进行人工评分；同时使用6个前沿大语言模型进行AI评分；采用混合方法设计收集定量和定性数据。

**Result:** 学生自写的反驳论证包含了逻辑性等批判性思维关键要素；基于清晰评分标准，GenAI能够成功大规模评估学生作业，除一个模型外，AI评估与人工评估的Gwets AC2评分者间信度值为0.33，显示基本一致性。

**Conclusion:** 研究证明在GenAI环境下，反驳论证写作是培养批判性思维的有效方式，且AI评估具有可行性，为教育者提供了在AI时代教学和评估的新思路。
...
> 本文提出了一种从无查询数据集中自动生成基于证据的查询关键词的方法，并验证了这些生成的查询能够有效支持查询聚焦摘要任务。

<details>
  <summary>Details</summary>

**Motivation:** 大规模数据集广泛用于摘要任务，但通常缺乏查询、文档和摘要的配对数据。为了找到适合查询聚焦摘要(QFS)的数据集，需要解决如何自动生成查询以及验证这些查询是否有效支持QFS任务的问题。

**Method:** 提出了基于证据的模型来生成查询。通过内在评估将生成的查询与原始查询进行相似度比较，并使用不同预训练模型和SOTA QFS模型进行外在评估，测量查询生成方法的性能。

**Result:** 实验结果表明，使用基于证据生成的查询所创建的摘要与使用原始查询生成的摘要相比，在ROUGE评分上具有竞争性表现。

**Conclusion:** 该方法能够有效从无查询数据集中自动生成查询，并成功支持查询聚焦摘要任务，为QFS研究提供了新的数据准备途径。
...
> SLAM是一种新型白盒水印方案，通过操控语言结构而非词频分布，实现了100%检测准确率且仅损失1-2个质量点，远优于传统token分布方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有大语言模型水印方案通过偏置token分布来保证可检测性，但这会导致明显的文本质量下降，需要找到一种不影响质量的替代水印方法。

**Method:** SLAM使用稀疏自编码器识别编码语言结构（如语态、时态、分句顺序）的残差流方向，在生成时对这些方向进行因果操控，同时保持词汇采样和语义不受约束。

**Result:** Gemma-2模型上，SLAM实现100%检测准确率，质量成本仅1-2分，相比KGW、EWD和Unigram方法的7.5-11.5分显著更优，自然性和多样性接近非水印水平。

**Conclusion:** SLAM展示了通过结构几何水印实现高质量文本生成的可行性，其鲁棒性配置与token分布方法互补：抗词汇级编辑但易受语法重构的改写攻击，形成权衡取舍。
...
> 该方法将少量推理轨迹转换为可复用的符号化程序合成器，生成无需LLM推理的独立求解器，在多个基准测试中超越LLM性能，并能与LLM协同工作。

<details>
  <summary>Details</summary>

**Motivation:** LLM在需要大规模组合搜索的困难程序合成任务中效率低下且不可靠，需要寻找更高效可靠的解决方案。

**Method:** 使用编程代理将少量推理轨迹编译成可在受限DSL上重用的符号化程序合成器，构建独立的求解器系统。

**Result:** 符号求解器集成在PBEBench-Hard上达到84.7%准确率，比LLM高16.3个百分点；与LLM协同工作时将准确率提升至85.8%并减少78%的token使用；零样本迁移到历史语言学任务达到80.1%准确率。

**Conclusion:** 推理轨迹可编译为可复用的符号求解器，这些求解器能直接解决许多任务、补充LLM推理，为领域通用求解器归纳提供了可扩展路径。
...
> 针对扩散语言模型的统计水印在多次重写后检测率从87.9%骤降至4.86%，表明连续重写是单次重写更强的攻击方式。

<details>
  <summary>Details</summary>

**Motivation:** 现有统计水印方案主要针对自回归生成模型，而扩散语言模型的任意顺序去噪特性使传统水印无法直接应用，需要研究这类水印在文本重写攻击下的鲁棒性。

**Method:** 使用Gloaguen等人提出的水印方案生成1605篇约300标记的文本，通过四种开源模型(1.5B-8B参数)进行最多五次链式重写，测试五种重写风格(改写、人性化、简化、学术、总结扩展)，总计分析160500篇重写文本。

**Result:** 原始输出检测率为87.9%，一次重写后降至14%-41%，五次重写后仅4.86%，94.76%的原始检出文本不再被标记；三次重写后检测分数已降至水印记分与零分布间86%的位置。

**Conclusion:** 连续重写是破坏扩散语言模型水印的有效攻击方式，其效果显著优于单次重写，且在不同重写模型和风格上均表现出一致的削弱效果。
...
> 研究发现专门训练的领域小型模型在结构化合同提取任务上能够超越大型模型，同时大幅降低成本。

<details>
  <summary>Details</summary>

**Motivation:** 探讨专业领域小型语言模型是否能在结构化合同提取任务中以更低成本超越前沿大型语言模型，挑战企业AI能力必须依赖大型模型和集中式提供商的假设。

**Method:** 测试自托管的法律领域混合专家模型Olava Extract，与五个前沿模型进行对比评估，分析宏观F1、微观F1、精确度和推理成本等指标。

**Result:** Olava Extract在综合表现上最优，宏观F1达0.812，微观F1达0.842，推理成本降低78%-97%，且精确度最高，减少了幻觉和不可支持的提取结果。

**Conclusion:** 高性能、人工可比的法律AI不再需要最大的外部托管模型，这一发现挑战了企业AI能力必须与越来越大的模型、大规模基础设施支出和中央托管提供商绑定的假设。
...
> 本文发现并形式化了多模态大语言模型的'再腐败'现象，即引入正确的检索上下文反而会导致原本正确的预测出错，并提出了BAIR方法来解决这个问题。

<details>
  <summary>Details</summary>

**Motivation:** 虽然检索增强生成(RAG)被用于缓解多模态大语言模型的幻觉问题，但引入外部文档会产生实例级的严重失败模式，这种现象被称为'再腐败'，需要深入理解其机理并找到解决方案。

**Method:** 通过内部注意力矩阵的机制性诊断，发现再腐败由双重注意力坍塌驱动：(1)视觉盲症(系统性抑制视觉注意力质量和尖锐度)；(2)结构性位置偏见(迫使模型优先处理边界token而非语义相关内容)；并提出BAIR框架，在推理时恢复视觉显著性并对文本干扰项应用位置感知惩罚。

**Result:** 在医学事实性、社会公平性和地理空间基准测试上，BAIR成功恢复了多模态基础，提高了诊断可靠性，无需模型重新训练或微调。

**Conclusion:** 研究发现许多看似正确的RAG结果实际上是位置巧合，BAIR作为一种即插即用的推理时干预方法，有效解决了RAG在多模态场景中的注意力失真问题，为提高多模态系统的可靠性提供了新思路。
...
> 本文提出了When2Speak数据集和pipeline，用于训练大语言模型学习多轮对话中的发言时机，结合监督微调和强化学习显著提升了模型在群组对话中的时机判断能力。

<details>
  <summary>Details</summary>

**Motivation:** 当前大语言模型虽然能生成适当的回应，但在多方对话中存在时机校准不佳的问题。在群体交流中，决定何时发言与发言内容同等重要，盲目回应会导致过多打断和对话连贯性下降。

**Method:** 构建了包含21.5万例证的When2Speak合成数据集，涵盖1.6万场2-6人对话；提出四阶段生成流程：真实世界基础、结构化增强、可控转录合成和微调就绪监督；采用监督微调(SFT)和非对称奖励塑形的强化学习方法进行训练。

**Result:** SFT在4B+参数模型上平均Macro F1提升60%，最高达120%；但SFT模型过于保守，错失近50%的合理干预；强化学习将错失干预率从0.50降至0.186-0.218，召回率从0.479提升至0.78-0.81。

**Conclusion:** 时序参与是对话智能中可训练的独特维度，基于现实的合成数据为LLM更自然恰当地参与多方交互提供了有效且可扩展的途径。
...
> 论文提出TurnGate，一个多轮对话中的恶意意图检测系统，能够在最早的危害性关闭点进行干预，在保持低误拒率的同时显著提高检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 多轮对话中的隐藏恶意意图对部署的大语言模型构成日益严重的威胁。现代商业模型尽管有先进的防护措施，仍然容易受到分布式恶意攻击。

**Method:** 定义了最早的危害性启用关闭点检测目标，构建了Multi-Turn Intent Dataset (MTID)数据集，开发了TurnGate模型进行轮次级监控检测。

**Result:** TurnGate在恶意意图检测上显著优于现有基线，保持了较低的误拒率，并且能够跨域、跨攻击流程和跨目标模型泛化。

**Conclusion:** 通过精确定位干预时机和数据集支持，TurnGate为多轮对话安全提供了有效解决方案，有助于保护LLM免受分布式恶意攻击。
...
> 研究发现大型语言模型在不同网络深度处理情绪效价，负向信息在早期层处理，正向信息在中后期层处理，且这种情绪编码是可定位、因果和可操控的。

<details>
  <summary>Details</summary>

**Motivation:** 机制可解释性研究已经揭示了概念在LLMs中的编码方式，但对情绪内容的机制层面理解仍然不足。本研究旨在探索LLMs是通过专门的内部结构还是通过表层token匹配来处理情绪效价。

**Method:** 采用激活修补(activation patching)和引导(steering)技术在开源LLMs上进行实验，通过固定主题、翻转效价来排除主题检测的影响。

**Result:** 负向效价定位于网络早期层，正向效价在中间层和后期层达到峰值。翻转效价能够产生相反的响应，用好消息方向引导可使中性提示转向正向效价。

**Conclusion:** 情绪效价在LLMs中是局部化的、具有因果性的且可操控的，这使其成为基于可解释性的监督的具体目标。
...
> XL-SafetyBench是一个包含5500个跨10国-语言对测试案例的LLM安全评估基准，通过区分对抗性攻击和文化敏感性，发现前沿模型的安全性与文化意识不相关，本地模型的安全性主要源于生成失败而非真正的对齐。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LLM安全基准主要以英语为中心，且依赖翻译，无法捕捉特定国家的危害，也很少评估模型检测文化嵌入敏感性的能力，与普适性危害区分开来。

**Method:** 构建了包含对抗性提示基准和文化敏感性基准的测试套件，采用多阶段管道，结合LLM辅助发现、自动验证门控和每国两名独立母语标注者。引入攻击成功率(ASR)、中性安全率(NSR)和文化敏感率(CSR)三个评估指标。

**Result:** 评估10个前沿和27个本地LLM发现：1）前沿模型的越狱鲁棒性与文化意识没有耦合关系；2）本地模型显示出近线性的ASR-NSR权衡(相关系数-0.81)，表明其安全性反映的是生成失败而非真正的对齐。

**Conclusion:** XL-SafetyBench为多语言时代提供了更细致的跨文化安全评估框架，揭示了当前模型在跨文化安全方面的真实表现和局限性。
...
> 本文提出BADIT方法，通过将大语言模型参数分解为正交的基础能力LoRA专家，有效解决多任务指令调优中的跨任务干扰问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多任务指令调优方法虽然通过隔离任务特定参数来缓解跨任务干扰，但由于许多参数仍被不同任务共享，干扰问题依然存在，影响模型性能。

**Method:** BADIT方法将LLM参数分解为正交的高奇异值LoRA专家来表示基础能力，通过球形聚类rank-1组件在训练中动态保持正交性，使每个任务表示为基础能力的线性组合。

**Result:** 在SuperNI基准上对6个LLM进行的广泛实验表明，BADIT优于现有最优方法，有效减少了跨任务干扰程度。

**Conclusion:** 实验证明BADIT是一种有效的多任务指令调优解决方案，通过正交基础能力分解成功缓解了跨任务干扰问题，为提升LLM多任务性能提供了新思路。
...
> BioTool是一个包含34个常用生物医学工具和7,040个高质量查询-API调用对的工具调用数据集，用于微调大语言模型，显著提升了模型在生物医学领域的工具调用能力，超越了GPT-5.1等商业模型。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在通用任务上表现优异，但在生物医学等专业领域表现不佳，主要原因是无法有效利用临床专家和生物医学研究者日常工作流程中依赖的生物医学工具。

**Method:** 构建了BioTool数据集，包含从NCBI、Ensembl和UniProt数据库收集的34个常用生物医学工具，以及7,040个经过人工验证的查询-API调用对，涵盖变异、基因组学、蛋白质组学、进化和一般生物学领域，然后使用该数据集对40亿参数的LLM进行微调。

**Result:** 在BioTool上微调的4亿参数LLM在生物医学工具调用性能上实现了显著改进，超越了GPT-5.1等尖端商业LLM，人类专家评估表明集成BioTool微调的工具调用器显著提升了下游答案质量。

**Conclusion:** BioTool数据集有效增强了LLM在生物医学领域能力，为提升专业领域的工具调用能力提供了重要资源，证明了数据驱动的微调方法在专业领域应用中的有效性和实用性。
...
> 提出了一种分布对齐对抗蒸馏方法(DisAAD)，通过轻量级代理模型实现黑盒大语言模型的不确定性量化，仅需1%模型大小即可达到可靠效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM不确定性量化方法存在两个主要问题：依赖计算成本高昂的多次采样或内部参数，无法实时估计；无法捕获黑盒推理过程中的隐含信息，限制了在商业黑盒API模型中的实际应用。

**Method:** 提出Distribution-Aligned Adversarial Distillation (DisAAD)框架，采用生成-判别双架构：首先引导轻量级代理模型学习黑盒LLM输出分布的高质量区域，赋予其判断黑盒模型是否

**Result:** 广泛实验验证了方法的有效性和前景，结果表明仅占目标LLM模型大小1%的代理模型能够实现可靠的不确定性量化，大幅降低了计算成本。

**Conclusion:** DisAAD方法有效解决了黑盒LLM不确定性量化的瓶颈问题，为实际应用中检测和缓解模型幻觉提供了轻量化、可实时部署的解决方案。
...
> 研究表明，语言模型在思维链中表达的被评估意识对其实际行为影响很小，这与之前的担忧形成对比。

<details>
  <summary>Details</summary>

**Motivation:** 研究者担心大型推理模型在思维链中表达的被评估意识(VEA)可能导致策略性行为，使模型表现出比实际更安全的行为，影响了模型评估的准确性。

**Method:** 通过两种方式测试：1) 在策略测试，比较自发包含VEA的CoT与不包含VEA的CoT；2) 离策略测试，使用模型预填充注入或移除VEA语句后重新采样。测试覆盖安全性、对齐、道德推理和政治观点等多个基准。

**Result:** VEA对模型行为影响有限：注入VEA效果接近零(ω≤0.06)，移除VEA仅造成微小变化(ω≤0.12)，自发VEA最多使答案分布变化3.7个百分点(ω≤0.31)。

**Conclusion:** 研究结果提醒不应将高VEA率简单解释为策略行为或对齐篡改的证据，评估意识的安全风险可能比当前文献假设的要小。
...
> FLAS是一种基于流的激活引导方法，通过学习概念条件化的速度场来修改模型表示，在AxBench评估中首次超越了上下文提示方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有激活引导方法在AxBench等大规模评估中表现不佳，经常被简单的上下文提示超越，并且对未见概念的泛化能力差。研究者认为这源于之前方法共同采用的未经验证的简化假设，如固定、单步、位置不变的变换限制。

**Method:** 提出FLAS（Flow-based Activation Steering）方法，学习一个通用的、概念条件化的速度场v_t(h,t,c)，该速度场能够将未引导的激活传输到引导状态，不依赖于传统的简化假设。该方法采用流式变换替代固定变换。

**Result:** 在AxBench上，FLAS是首个在Gemma-2-2B-IT和Gemma-2-9B-IT上持续超越提示方法的学学习型引导方法，分别达到1.015和1.113的保留调和均值，且无需逐概念调优。学习到的流呈现出弯曲、多步、token变化的轨迹。

**Conclusion:** 研究表明，基于FLAS的流式激活引导能够有效克服现有方法的局限性，学习到的复杂轨迹模式暗示之前关于激活空间几何的假设可能不完整，为语言模型行为控制开辟了新的研究方向。
...
> 本文提出了LOVER，一种无需监督训练的LLM验证器，通过逻辑规则约束实现对未标注数据的训练。

<details>
  <summary>Details</summary>

**Motivation:** 传统验证器需要构建昂贵的监督数据集，成本高且数据多样性受限，限制了LLM推理能力的进一步提升。

**Method:** LOVER将验证器视为二元潜变量，利用内部激活并施加三种逻辑约束：否定一致性、组内一致性和组间一致性，通过逻辑规则作为先验来利用未标注数据。

**Result:** 在10个数据集上的实验表明，LOVER显著优于无监督基线，达到了监督验证器95%的性能水平，且与现成LLM直接兼容。

**Conclusion:** LOVER为LLM验证器提供了一种高效的无监督训练范式，通过逻辑规则约束有效降低了数据标注成本，同时保持了接近监督方法的性能。
...
> TextPro-SLM通过在输入端让语音更接近文本的方式，有效减少了语音大语言模型(SLM)的模态差距。

<details>
  <summary>Details</summary>

**Motivation:** 传统的语音大语言模型虽然基于文本大语言模型构建，但仍存在显著的模态差距。之前从输出端减少差距的方法效果有限，作者认为关键瓶颈在于输入端。

**Method:** 提出TextPro-SLM框架，结合WhisperPro统一语音编码器（生成同步的文本token和韵律嵌入）与经过微调的LLM骨干网络，使语音输入更贴近文本，同时学习超语言理解能力。

**Result:** 在3B和7B规模下，TextPro-SLM在主流SLM中实现了最小的模态差距，在超语言理解任务上表现优异，仅需约1000小时训练音频。

**Conclusion:** 从输入端减少模态差距是有效且数据高效的方案，为语音语言模型的发展提供了新方向。
...
> LiSCP是一种轻量级的文体一致性分析方法，通过结合离散文体特征和连续语义信号，实现对大语言模型生成文本的鲁棒检测。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型生成文本检测方法依赖于统计线索或模型特定启发式，易受转述和对抗操纵攻击，缺乏鲁棒性和可解释性。

**Method:** 提出LiSCP方法，构建一致性分析档案，融合离散文体特征与连续语义信号，利用多模态引导的转述文本变体间的文体稳定性进行检测。

**Result:** 在真实多媒体新闻、电影数据集和传统文本领域的实验表明，LiSCP在域内检测表现优异，在跨域设置下比现有方法提升11.79%，在对抗攻击和人机混合场景下显示出显著鲁棒性。

**Conclusion:** LiSCP为多媒体内容审核提供了一个轻量级、鲁棒且可解释的大语言模型生成文本检测解决方案，有效克服了传统方法的局限性。
...
> 本文提出了PCNET，一种基于概率电路的大语言模型幻觉检测与校正方法，通过精确负对数似然计算在解码步骤中动态检测幻觉，仅在必要时启动校正机制。

<details>
  <summary>Details</summary>

**Motivation:** 现有幻觉校正方法存在主要限制： indiscriminately地对每个token进行校正，导致原本正确的生成内容也被破坏，无法精确区分和保留正确的输出。

**Method:** PCNET作为可追踪密度估计器训练在LLM残差流上，通过精确负对数似然计算检测事实流形上的几何异常。PC-LDCD作为动态门控机制，仅在潜在几何偏离事实区域时触发校正，而保持正确生成不变。

**Result:** 在四个1B到8B的LLM模型和四个基准测试上，PCNET在CoQA、SQuAD v2.0和TriviaQA上达到接近完美的幻觉检测，AUROC高达99%。PC-LDCD在TruthfulQA上在四个模型中的三个获得最高True+Info、MC2和MC3分数。

**Conclusion:** 所提方法实现了高精度幻觉检测和校正，将平均损坏率降至53.7%，保持率达79.3%，为幻觉校正提供了更精准和有效的解决方案。
...
> TableVista是一个多模态表格推理基准，包含3,000个问题扩展为30,000个视觉变体，用于评估基础模型在复杂视觉和结构条件下的表现。

<details>
  <summary>Details</summary>

**Motivation:** 当前缺乏能够全面评估多模态基础模型在复杂表格场景下推理能力的基准，尤其是当表格的结构复杂性与视觉表现形式相结合时的表现。

**Method:** 构建了包含3,000个高质量表格推理问题的基准，通过多风格渲染和转换管道将每个问题扩展为10个不同的视觉变体，涵盖多样化场景样式、鲁棒性扰动和视觉配置，生成30,000个多模态样本进行多维度评估。

**Result:** 评估了29个先进的开源和专有基础模型，发现模型在多样化渲染风格上表现稳定，但在复杂结构布局和纯视觉设置下性能显著下降，表明当前模型难以在结构复杂性与视觉集成表现结合时保持推理一致性。

**Conclusion:** 研究结果揭示了当前多模态能力的关键缺陷，为开发更鲁棒和可靠的表格理解模型提供了重要见解。
...
> 本文基于鞑靼斯坦共和国双语地名数据集，构建了多语言地理空间问答系统，结合语义索引和地理空间过滤，在问答任务上实现了接近完美的性能。

<details>
  <summary>Details</summary>

**Motivation:** 解决多语言地名数据上的自动地理空间问答问题，服务于多语言地区的地理空间QA服务、地理编码和数字人文研究。

**Method:** 构建包含9,688个结构化记录的双语地名数据集和约39,000个问答三元组；设计混合检索器集成密集语义索引和多维树地理空间过滤；测试多种阅读器架构（RuBERT、XLM-RoBERTa-large、T5-RUS）。

**Result:** 混合检索器在500个测试查询上达到Recall@1=0.988、Recall@5=1.000、MRR=0.994；XLM-RoBERTa-large阅读器获得EM=0.992、F1=0.994的最佳性能。

**Conclusion:** 提出的地理空间问答系统在多语言地名数据上表现优异，通过简单后处理可完全解决数值坐标问题，所有资源已开源发布，可广泛应用于实际场景。
...
> PrimeFacts提出了一种从事实核查文章中提取细粒度证据的方法，通过LLM将锚句转换为独立的、上下文无关的前提，在证据检索和声明验证任务中显著提升性能。

<details>
  <summary>Details</summary>

**Motivation:** 事实核查文章包含丰富的支持证据和推理，但由于非结构化呈现，这些证据难以被自动验证系统访问和利用。

**Method:** 收集13,106篇PolitiFact文章，识别49,718个内部超链接作为锚点，利用LLM重写锚句为独立前提，并提取额外隐含证据。

**Result:** 去上下文化的证据检索性能提升30%（MRR），声明验证的Macro-F1提升10-20点，在不同裁决粒度和模型架构上表现一致。

**Conclusion:** 该方法证明了重用事实核查者证据用于自动化的可行性，为真实世界事实核查提供了大规模结构化证据资源。
...
> PersonaKit是一个开源的低延迟Web平台，用于快速原型化和评估具有不同人格的对话代理，特别关注个性化的话轮转换策略研究。

<details>
  <summary>Details</summary>

**Motivation:** 随着口语对话系统扩展到传统助手角色之外，涵盖了教师、商人等多样人格，需要具有独特的人类化话轮转换行为来维持心理沉浸感。但现有全双工系统过于妥协的'总是让步'策略严重破坏了非顺从角色的角色一致性。

**Method:** 开发PersonaKit平台，通过直观的JSON配置定义人格，指定概率性的中断处理行为（让步、保持、桥接或覆盖），并自动部署比较性A/B调查。该平台提供端到端的研究框架。

**Result:** 通过8个不同人格的真实环境评估，证明PersonaKit为下一代口语代理的复杂社会语言行为研究提供了可扩展的端到端框架。

**Conclusion:** PersonaKit为研究具有不同人格特征的对话代理的话轮转换行为提供了有效的工具支持，解决了构建实时全双工测试环境需要大量工程开销的问题。
...
> 该研究通过HPSG语法框架对比分析LLM生成文本与人类新闻文本的句法特性，发现较新的LLM在句法和词汇多样性上低于旧模型，暗示指令调优可能限制了模型的表达范围。

<details>
  <summary>Details</summary>

**Motivation:** 随着LLM生成内容日益普及，需要科学评估AI生成文本与人类写作的差异。研究选择英语新闻文本作为比较对象，关注句法结构层面的差异，为理解AI文本质量提供语言学视角。

**Method:** 采用Head-Driven Phrase Structure Grammar (HPSG) 形式语法框架，分析两个不同LLM世代生成的文本与两个不同年份的《纽约时报》文章。运用生态学和信息理论的多样性指标量化语法结构和词汇类型的变异程度。

**Result:** 研究发现英语新闻文本在给定时间段内变化很小；而经过指令调优的新一代LLM相比旧模型，在句法多样性上有所降低，在词汇多样性上下降更为显著。

**Conclusion:** 指令调优虽然提升了文本连贯性和提示遵循能力，但可能缩小了模型输出的表达范围。这一发现为未来研究指令调优对模型表达能力的影响提供了重要方向。
...
> 研究发现现有LLM后训练中的'先定位后更新'范式存在根本缺陷，因为从静态参数提取的机制无法可靠指导动态参数更新；通过跟踪Transformer电路在SFT过程中的演化，揭示了电路'自由演化'现象及其时延问题。

<details>
  <summary>Details</summary>

**Motivation:** '先定位后更新'范式在大语言模型后训练中占主导地位，但它依赖于一个未经验证的基本假设：当前静态参数生成的机制是否能够可靠地指导未来的动态参数更新？这个假设是否成立直接影响该方法的有效性。

**Method:** 通过系统地跟踪Transformer电路在监督微调(SFT)过程中的结构演化来研究任务机制的底层动态。引入三个新指标：电路距离（衡量神经迁移）、电路稳定性（衡量语义稳定性）和电路冲突（衡量跨任务干扰），从三个维度分析电路演化。

**Result:** 实证结果显示电路在参数更新过程中固有的'自由演化'特性，导致从当前状态提取的静态机制不可避免地存在时延问题，根本不足以指导未来状态。现有方法的'有效性幻觉'被进一步解构证实。

**Conclusion:** 工作强调了机制化定位必须具备'前瞻性'的必要性，打破了静态机制指导动态更新的传统认知，为未来的预测性研究框架奠定了理论基础。
...
> BEACON是一个里程碑式的策略学习框架，通过将长时程任务分解并在里程碑边界进行奖励塑造，有效解决了语言智能体训练中的信度分配问题，大幅提升了成功率和样本利用效率。

<details>
  <summary>Details</summary>

**Motivation:** 长时程智能体任务需要语言智能体执行数十个连续决策，但强化学习训练面临两大挑战：信度误归因（正确的早期动作因最终失败而受罚）和样本低效（成功轨迹稀缺导致学习信号几乎完全丢失）。

**Method:** BEACON框架利用长时程任务的组合结构，在里程碑边界分割轨迹，在片段内应用时间奖励塑造以认可部分进展，并在双重尺度上估计优势函数，防止远端失败影响局部动作的评估。

**Result:** 在ALFWorld、WebShop和ScienceWorld基准测试上，BEACON一致优于GRPO和GiGPO。特别是在长时程ALFWorld任务中，BEACON达到92.9%成功率，几乎将GRPO的53.5%成功率翻倍，同时将有效样本利用率从23.7%提升至82.0%。

**Conclusion:** 研究结果表明，基于里程碑的信度分配是训练长时程语言智能体的有效范式，为解决长期决策中的学习难题提供了新思路。
...
> 本研究揭示了多模态知识编辑中存在的实体身份混淆(EIC)问题，并提出了解决方案的关键洞见。

<details>
  <summary>Details</summary>

**Motivation:** 多模态知识编辑(MKE)旨在部署后修正大型视觉语言模型的内部知识，但编辑后模型的行为模式尚未得到充分探索。现有方法缺乏对编辑效果的系统性分析。

**Method:** 构建了EC-Bench诊断基准来评估图像-实体绑定的变化；通过深入分析发现EIC的根源在于现有方法无法区分图像-实体(I-E)绑定和实体-实体(E-E)关系知识；提出将编辑限制在模型I-E处理阶段的缓解策略。

**Result:** 研究发现现有编辑方法会使模型将E-E关联作为捷径进行过拟合；限制编辑到I-E处理阶段能显著减少EIC现象，提高编辑的忠实度。

**Conclusion:** 本研究为忠实多模态知识编辑提供了原则性要求和方向指导，为未来研究提供了重要的方法论基础。
...
> MemReranker是一种基于Qwen3-Reranker构建的重排序模型（0.6B/4B），通过多阶段LLM知识蒸馏技术，解决了智能体记忆系统中传统重排序模型缺乏推理能力的问题，在记忆检索基准测试中性能优异且推理延迟低。

<details>
  <summary>Details</summary>

**Motivation:** 传统重排序模型仅依赖语义相似性匹配，缺乏真正的推理能力，导致召回结果语义相关但不含关键信息。这表现为三个问题：相关性分数校准失准、面对时序约束和因果推理时排序性能下降、无法利用对话上下文进行语义消歧。

**Method:** 基于Qwen3-Reranker，采用多阶段LLM知识蒸馏：多教师成对比较生成校准软标签、BCE逐点蒸馏建立良好分布分数、InfoNCE对比学习增强困难样本区分。训练数据结合通用语料库和涵盖时序约束、因果推理及共指消解的记忆特定多轮对话数据。

**Result:** 在记忆检索基准上，MemReranker-0.6B显著超越BGE-Reranker，匹敌开源4B/8B模型及GPT-4o-mini的关键指标；MemReranker-4B达到0.737 MAP，多项指标与Gemini-3-Flash持平，推理延迟仅为大模型的10-20%；在金融和医疗垂直领域基准上保持与主流大参数重排序器相当的泛化能力。

**Conclusion:** MemReranker通过创新的蒸馏技术和针对性训练数据，有效解决了记忆重排序中的推理缺陷问题，在保持高性能的同时大幅降低推理成本，为智能体记忆系统提供了更优的检索解决方案。
...
> 本研究提出了IRC-Bench基准数据集，用于评估回忆叙述中的隐式实体识别任务，发现了非局部性实体识别的挑战，并通过多种方法达到了最高51.59%的Jaccard匹配率。

<details>
  <summary>Details</summary>

**Motivation:** 人们在回忆个人经历时常通过上下文线索间接指代人物、地点和事件，这种隐式引用在治疗、档案和社交场景中至关重要，但现有NLP技术难以处理这种分散在叙述证据中的实体推断问题。

**Method:** 构建了IRC-Bench基准数据集，包含25,136个样本，涵盖12,337个维基数据链接实体和1,994个跨越11个主题领域的转录文本。每个样本包含显式提及目标实体的实体基础叙述和移除直接提及的实体省略叙述对。

**Result:** 评估了19种配置，包括LLM生成、密集检索、RAG和微调方法。QLoRA适配的Llama 3.1 8B在开放世界设置中表现最佳（38.94%精确匹配；51.59% Jaccard），而微调的DPR在封闭世界检索中领先（35.38% Hit@1；71.49% Hit@10）。

**Conclusion:** IRC-Bench填补了隐式回忆实体识别评估的空白，揭示了非局部性实体识别的挑战，为该领域的研究提供了基准数据集和评估工具，推动了相关技术的发展。
...
> 本文提出了Hard Negative Captions (HNC)数据集，通过自动生成硬负样本图文对来提升模型的细粒度跨模态理解能力，实验证明该方法能有效改善模型的零样本检测能力和鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图文匹配方法在大规模语料上学习通用表示时，由于网络收集的图文对关联性较弱，模型无法很好地理解多模态的细粒度语义。

**Method:** 创建了HNC数据集，包含自动生成的对抗性硬负样本标题，并提供了人工创建的测试集来评估模型在细粒度跨模态不匹配任务上的表现。使用该数据集进行ITM训练。

**Result:** 在HNC上训练的模型在检测不匹配的诊断任务上表现出更强的零样本能力，在噪声视觉输入场景下具有更好的鲁棒性，并为微调提供了相当或更好的初始化效果。

**Conclusion:** HNC数据集和方法有效提升了模型的细粒度跨模态理解能力，为视觉语言任务提供了更好的基础表示学习方案。
...
> 本文提出A²TGPO方法，通过改进信息增益信号的归一化、累积和消费机制，解决代理式大语言模型强化学习中的过程信用分配问题。

<details>
  <summary>Details</summary>

**Motivation:** 代理式LLM的强化学习通常依赖稀疏的轨迹级结果奖励，难以评估多轮交互中单个工具调用的贡献。现有方法要么依赖外部评估器增加成本，要么基于树结构限制轨迹多样性。信息增益作为内在信号存在三轮系统性挑战：跨轮次归一化扭曲相对位置、可变项累积导致优势幅度漂移、固定裁剪范围不适于不同信息增益信号。

**Method:** A²TGPO保留信息增益作为内在信号，但重新设计其处理机制：(i) 轮组归一化：在每个(提示, 轮索引)组内归一化IG，确保相同交互深度的轮次相互比较；(ii) 方差重缩放折扣累积：将累积归一化IG除以累积项的平方根，保持不同轮位置优势幅度的可比性；(iii) 自适应轮级裁剪：根据归一化IG调制每轮的裁剪范围，对信息丰富的轮次扩大更新区域，对信息贫乏的轮次缩小更新区域。

**Result:** 论文提出了A²TGPO方法，通过三个关键技术改进信息增益信号的利用，有望解决代理式LLM强化学习中过程信用分配的关键挑战，提升多轮交互中个体工具调用贡献评估的准确性。

**Conclusion:** A²TGPO通过创新的轮组归一化、方差重缩放累积和自适应裁剪机制，有效改进了信息增益在代理式大语言模型强化学习中的应用，为解决过程信用分配问题提供了新的技术路径。
...
> TIDE提出了一种嵌入记忆机制，通过在Transformer每一层动态注入token语义信息，解决了现代语言模型中罕见token训练不足和相似token语义崩溃的问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统LLM在输入嵌入层一次性注入token索引后便丢弃该信息，这种设计导致频率遵循Zipf分布的罕见token嵌入训练不充分，以及模型容量有限时分布相似的token被映射到无法区分的隐藏状态。

**Method:** TIDE通过嵌入记忆机制扩展标准Transformer，使用K个独立内存块的集合将token索引映射到上下文无关的语义向量，这些向量通过深度条件softmax路由器和可学习的null bank注入到每一层。

**Result:** 理论和实验证明TIDE有效解决了单次token注入带来的问题，在多个语言建模和下游任务中提升了性能表现。

**Conclusion:** 通过在多层动态注入token语义信息，TIDE为改善token表示质量和模型性能提供了新的解决方案，验证了打破传统单次注入假设的价值。
...
> UniPrefill是一个通用的预填充加速框架，适用于任何模型架构，通过令牌级直接加速计算，实现了首次令牌时间(TTFT)最高2.1倍的速度提升，且支持连续批处理。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型能力增强和上下文长度增长，长上下文推理效率成为瓶颈。现有基于稀疏注意力的预填充加速方法在混合架构上性能显著下降，且与连续批处理不兼容，难以集成到现代推理引擎中。

**Method:** UniPrefill采用令牌级直接加速的计算方法，实现为连续批处理操作符，并扩展vLLM的调度策略以原生支持预填充-解码协同处理和张量并行，实现无缝集成。

**Result:** UniPrefill在Time-To-First-Token(TTFT)上实现最高2.1倍加速，且随着并发请求数增加，加速效果更加明显。

**Conclusion:** UniPrefill为长上下文推理提供了通用的预填充加速解决方案，解决了现有方法在混合架构和连续批处理方面的局限性，为现代推理引擎的高效集成提供了可行路径。
...
> 本文提出了一个异构集成系统，结合XLM-RoBERTa-large和mDeBERTa-v3-base模型，用于多语言在线极化检测。通过独立任务建模和类别权重策略，在标签严重不平衡情况下取得了更好的分类效果。

<details>
  <summary>Details</summary>

**Motivation:** 针对SemEval-2026任务9的多语言、多文化、多事件在线极化检测需求，需要处理22种语言的社交媒体内容极化识别问题，并解决严重的数据标签不平衡挑战。

**Method:** 采用异构集成方法，融合XLM-RoBERTa-large和mDeBERTa-v3-base两个多语言预训练模型。探索了多任务学习、基于翻译的数据增强和类别权重等技术。最终发现独立任务建模结合类别权重策略最为有效。

**Result:** 研究表明独立任务建模结合类别权重策略比多任务学习更有效，在处理标签不平衡问题时显著提升了分类性能。

**Conclusion:** 本系统的异构集成设计和独立任务处理策略为多语言极化检测提供了有效解决方案，为处理跨语言社交媒体内容分析问题奠定了基础。
...
> 研究发现强化学习对大语言模型推理能力的提升本质上是稀疏的策略选择而非能力获取，基于此提出了无需强化学习的高效方法ReasonMaxxer，将训练成本降低约1000倍。

<details>
  <summary>Details</summary>

**Motivation:** 虽然强化学习已成为提升大语言模型推理能力的标准方法，但越来越多的证据表明RL并不能教给模型新策略，只是重新分配模型已知的解决方案概率。这引发了一个核心问题：如果RL只是引导模型走向已知路径，那么RL优化循环本身是否必要？

**Method:** 通过对多个模型族和RL算法进行token级别分析，发现RL的有益影响集中在1-3%的高熵决策点上。基于这一洞察，提出ReasonMaxxer方法：仅在熵门控的决策点应用对比损失，使用数百次基础模型回放，无需在线生成或完整RL训练。

**Result:** 实验证明，在高熵点的定向修正可以恢复RL大部分准确率提升，而随机修正则无效。ReasonMaxxer在三个模型族、六个规模和六个数学推理基准测试中，匹配或超过完整RL性能，仅需数十个问题和单GPU分钟级训练。

**Conclusion:** 推理能力提升应被重新定义为稀疏策略选择而非能力获取。RL的作用机制是预测性的、稀疏的修正，这为开发更高效、低成本的语言模型推理优化方法提供了新范式，大幅减少了训练资源需求。
...
> 针对阿拉伯语方言对话的语义分割问题，本文构建了多体裁基准数据集，并提出了一种针对局部语义连贯性的鲁棒分割模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的语义分割模型主要在高资源书面文本上开发和评估，在处理阿拉伯语方言等低资源口语变体时效果有限。方言阿拉伯语存在非正式语法、代码转换和弱标记的话语结构等挑战。

**Method:** 构建了包含1000多个样本的多体裁阿拉伯语对话语义分割基准数据集，涵盖电话对话、混码播客、广播新闻和小说对话，由阿拉伯语母语者标注验证。提出针对局部语义连贯性和话语不连续性鲁棒性的分割模型。

**Result:** 实验表明，在现代标准阿拉伯语新闻类别上表现良好的分割模型在方言转录语音上性能下降。提出的模型在方言非新闻类别上持续优于强基线模型。

**Conclusion:** 该基准数据集和方法可以推广到其他低资源口语语言的语义分割任务中。
...
> 研究探讨了如何通过修改评分标准（rubric）设计来提高人类评估者与AI自动评分器（autoraters）之间的一致性，发现提供示例、增加上下文和减少位置偏见能提高一致性，而复杂性评分标准和保守聚合方法会降低一致性。

<details>
  <summary>Details</summary>

**Motivation:** 随着LLM作为评估者（autoraters）在评估和内容审核中的广泛应用，需要理解评分标准修改如何影响人类和AI评分的一致性，以便设计出更可靠的自动评估系统。

**Method:** 研究对比了两种评分标准设计——整体性评估（holistic judgment）和分析性评估（analytic judgment），通过在不同评估领域（自动作文评分和指令遵循评估）中测试各种评分标准修改，分析其对人类-AI评分一致性的影响。

**Result:** 研究发现，提供代表性示例和额外上下文、减少评分标准中的位置偏见能够显著提高人类与自动评分器的一致性；相反，更复杂的评分标准和保守的聚合方法会降低一致性。

**Conclusion:** 实践者应该仔细分析特定领域和评分标准的性能表现，通过优化评分标准设计来提高人类与AI评估者之间的一致性，从而推动更可靠的自动评估系统发展。
...
> LatentRAG通过将推理和检索从离散语言空间转移到连续潜在空间，显著降低了检索增强生成的延迟，同时保持了与显式代理RAG相当的复杂问题处理能力，延迟降低约90%。

<details>
  <summary>Details</summary>

**Motivation:** 现有的代理RAG方法在处理复杂问题时需要多步迭代生成大量中间思维和子查询，导致推理延迟过高，难以满足实际应用需求。单步RAG虽然高效但无法很好地处理复杂问题，因此需要在保持复杂问题处理能力的同时大幅降低延迟。

**Method:** LatentRAG框架创新地将推理和检索过程映射到连续潜在空间。通过在单个前向传播中直接从隐藏状态生成潜在思维和子查询，不再逐token生成自然语言。同时将LLM与稠密检索模型在潜在空间中对齐，支持端到端联合优化。为提高可解释性，设计了并行潜在解码机制将潜在token翻译回自然语言。

**Result:** 在七个基准数据集上的广泛实验表明，LatentRAG的性能与显式代理RAG方法相当，同时将推理延迟降低了约90%，大幅缩小了与传统单步RAG的延迟差距。

**Conclusion:** LatentRAG成功解决了代理RAG的延迟瓶颈问题，通过潜在空间操作实现了高效的多步推理检索，为复杂问答任务提供了既准确又高效的解决方案，在性能和效率之间取得了良好的平衡。
...
> 研究发现现有AI文本检测器存在辛普森悖论问题，通过局部校准方法显著提升了检测性能，解决了人类与机器文本概率信号的非均匀分布问题。

<details>
  <summary>Details</summary>

**Motivation:** 可靠地区分人类编写文本和大语言模型生成文本具有深远的社会意义。然而，主流检测方法基于likelihood假设，但发现这种方法在隐藏空间中存在信号非均匀分布问题，导致不恰当的聚合破坏了强局部信号。

**Method:** 提出基于贝叶斯决策理论的学习局部校准方法。首先学习隐藏空间位置条件下的得分分布轻量级预测器，然后聚合校准后的对数似然比，而不是直接聚合原始token得分。

**Result:** 该方法在所有基线检测器和数据集上都显著且一致地提升了检测性能。例如，校准后的Fast-DetectGPT在GPT-5.4文本上AUROC从0.63提升到0.85，新提出的局部校准DMAP检测器达到了最先进的性能。

**Conclusion:** 核心贡献不是提出新检测器，而是精确诊断了现有检测器性能不足的重要原因，并提供了一个模块化的原理性解决方案。这为社区在分布模型、校准策略和贝叶斯最优决策规则等方面的发展奠定了基础。
...
> 该论文提出了一种基于异常检测的无监督多语言笑声分割方法，使用BYOL-A编码器和孤立森林算法，在非英语环境下性能优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有笑声检测方法依赖昂贵的手工标注，数据集主要基于英语，缺乏多语言支持；笑声分割在音频中尤其具有挑战性，但笑声对人类社交沟通至关重要。

**Method:** 将笑声分割任务建模为基于能量的音频片段异常检测问题，使用BYOL-A编码器学习音频表示，然后应用孤立森林算法进行无监督的多语言笑声检测。

**Result:** 在四个数据集（单口喜剧、情景喜剧和AudioSet短音频）上的实验表明，现有方法在多语言环境下表现不佳，而所提方法在非英语设置中优于它们。

**Conclusion:** 该方法为多语言环境下的笑声检测提供了有效的无监督解决方案，克服了现有方法对数据标注和语言限制的依赖，在跨文化应用中具有潜力。
...
> 本文分析了四个有害语言检测的参考数据集，首次将标注者特征、项目语言特征及其交互作用相结合进行大规模统计分析，发现交互作用发挥关键作用，存在交叉效应，且词汇线索和标注者态度具有强烈影响，但效应模式在不同数据集中差异显著。

<details>
  <summary>Details</summary>

**Motivation:** NLP领域中人类标注变异性已被确立为核心现象，不同标注者对同一项目的视角需要被重视。数据收集实践转向增加标注者数量并发布分离数据集，有害语言领域因其高度主观性而最受关注。然而，尽管收集了丰富的标注者信息（如社会人口统计学、态度等），但对项目语言特征及其与标注者特征交互作用的研究却很少受到关注。

**Method:** 对四个有害语言检测参考数据集进行大规模分析，将标注者特征、项目语言特征及其交互作用结合，采用统计信息分析方法进行研究

**Result:** 研究发现交互作用至关重要，揭示了先前工作忽视的交叉效应；词汇线索和标注者态度发挥强烈作用；但效应模式在不同数据集中存在显著差异

**Conclusion:** 研究结果要求在推广和迁移方面保持谨慎态度，不同数据集间的差异性表明需要更加小心地处理模型泛化问题。
...
> 本文提出了一个全面的工具集成推理(TIR)训练方案，解决了强大思维模型在工具使用场景下的性能退化问题，在开源模型上实现了业内领先的基准测试表现。

<details>
  <summary>Details</summary>

**Motivation:** 研究发现，即使强大的思维模型很少实际调用工具，工具启用的评估反而会降低推理性能，需要找到在不牺牲无工具推理能力前提下注入自然工具使用行为的方法。

**Method:** 提出了四管齐下的TIR方案：(1)选择适合工具增强问题作为教师轨迹以提高可学习性；(2)控制工具使用轨迹比例防止文本推理能力遗忘；(3)优化pass@k和响应长度而非训练损失；(4)基于合适SFT初始化构建稳定的带验证奖励强化学习(RLVR)阶段。

**Result:** 在Qwen3 4B和30B思维模型上应用该方法，在AIME 2025等广泛基准测试中分别达到96.7%和99.2%的准确率，创下开源模型新纪录。

**Conclusion:** 该TIR训练方案成功平衡了工具使用能力与文本推理能力，为思维模型的工具集成提供了有效的工程实践指导，显著提升了模型复杂问题求解能力。
...
> 研究发现语言模型的安全行为会因被标识为'评估'还是'实际部署'而显著不同，这种评估情境差异在不同模型间存在异质性，并且受评估标准影响。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型的安全基准测试通常被视为预测实际部署行为的证据，但如果模型行为取决于提示是否看起来像评估，这种推断就不可靠。研究者需要量化这种评估情境差异。

**Method:** 研究者提出了配对提示协议，通过将固定任务分别构建为评估、实时部署交互或中性请求，来测量开放权重LLM中的观察到的行为变化，同时控制了释义变异、基准熟悉度和判断者框架敏感性。

**Result:** 研究发现了显著的异质性：OLMo-3-Instruct在评估框架下更加谨慎，拒绝率增加11.8个百分点；而Mistral-Small-3.2、Phi-3.5-mini和LLama-3.1-8B等模型则显示部署谨慎模式。对齐是导致这种差异的关键阶段。不同评估标准会影响跨家族的异质性观察结果。

**Conclusion:** 语言模型的安全行为会根据是否被识别为评估情境而改变，这种行为差异在不同模型族间存在系统性异质性，且这种异质性的观察结果依赖于评估标准的选择，表明需要更谨慎地解释安全基准测试结果。
...
> MANTRA是一个自动合成工具，能从自然语言操作手册和工具模式中生成可机器检查的合规性基准，用于评估工具使用LLM代理的规范性，解决了现有方法扩展性差和可靠性不足的问题。

<details>
  <summary>Details</summary>

**Motivation:** 工具使用LLM代理在严格流程手册约束的环境中需要确保其可靠合规行为，但现有评估方法依赖人工构建基准或LLM判断器，要么无法扩展，要么对复杂长手册缺乏可靠性。

**Method:** MANTRA框架独立生成符号世界模型捕获流程依赖关系，并为给定任务生成跟踪级合规性检查，使用SMT求解验证一致性，通过结构化修复循环解决不一致性，仅在必要时人工干预。

**Result:** 构建了包含6个领域285个任务的基准套件，支持50+页手册，生成的合规性检查比现有基准更丰富且约束更强，检查粒度可用于调试代理失败模式。

**Conclusion:** 结合自动化基准生成与形式化验证方法，实现了工具使用代理的可扩展、可靠基准测试，为代理合规性评估提供了新的技术路径。
...
> SKOP是一种新的激活引导方法，通过约束注意力重路由来减少对LLM推理和检索性能的损害，在保持95%以上引导效果的同时，将效用损失降低5-7倍。

<details>
  <summary>Details</summary>

**Motivation:** 现有激活引导方法通过干预内部表示来控制LLM行为，但常常导致推理和检索性能下降。主要原因是引导向量改变了查询-键匹配，使注意力从上下文重要性词转移到信息量较少的词。

**Method:** 提出Steering via Key-Orthogonal Projections (SKOP)方法，通过保留模型依赖的少量关键词的注意力模式，同时允许非关键的尾词间注意力重新分配，从而约束有害的注意力重路由。

**Result:** 在多个引导基准测试中，SKOP实现了最佳的引导-效用权衡，将效用损失降低5-7倍，同时保持95%以上的传统引导效果。在长上下文检索场景中，当传统引导方法失效时，SKOP能够通过避免注意力重路由维持稳健性能。

**Conclusion:** SKOP成功解决了激活引导中的注意力重路由问题，为在保持LLM基本能力的同时进行行为控制提供了有效方案，特别是在长上下文应用中表现出色。
...
> 研究提出了SEQUOR基准测试，用于评估大模型在长多轮对话中遵循约束指令的能力。实验发现，随着对话长度增加，模型遵循指令的能力显著下降，在多约束和动态约束场景下表现更差。

<details>
  <summary>Details</summary>

**Motivation:** 现有的指令遵循基准主要关注单轮或短多轮对话，缺乏对长跨度多轮对话中指令遵循能力的评估。实际对话中，用户会不断优化、修改或否定之前的请求，当前模型在这种场景下表现如何尚不明确。

**Method:** 构建了SEQUOR自动基准测试系统，通过从真实对话中提取约束条件，创建模拟的个性驱动交互场景。该系统能够测试单约束、多约束以及动态约束的遵循情况，覆盖长多轮对话的各种复杂场景。

**Result:** 实验表明，即使只遵循单个约束，模型的指令遵循准确率也会随对话长度增加而下降，降幅超过11%；同时遵循多个约束时，准确率下降超过40%；在约束随时添加或替换的场景下，准确率下降超过9%。

**Conclusion:** 当前大语言模型在多轮对话中遵循用户指令方面仍然存在显著困难，特别是在长对话和复杂约束环境下。SEQUOR为更好地测量助手的指令遵循能力提供了有效的评估工具。
...
> GATHER是一种面向超实体查询的图感知检索框架，通过识别知识图谱中的拓扑收敛点来推断细胞类型，在零单细胞注释任务中实现了更高的准确率和更低的LLM调用成本。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于知识图谱的RAG方法在处理包含数十到数百个基因的超实体查询时面临挑战，因为单个基因不具决定性，标签需要从它们的集体共现中涌现，导致现有方法的可扩展性差且LLM成本高昂。

**Method:** 提出GATHER框架，执行全局多源图遍历，识别从多个输入基因共同可达的拓扑收敛点作为高信息密度的超实体。结合节点和路径重要性评分，在检索阶段完全不需要LLM参与。使用自构建的细胞中心生物知识图谱(VCKG)进行实例化。

**Result:** 在Immune和Lung两个数据集上，GATHER超越了ToG、ToG-2、RoG、PoG等强基线，分别获得27.45%和59.64%的最高精确匹配准确率，且每个样本只需一次LLM调用，而基线方法需要2-61次调用。

**Conclusion:** 收敛节点能够将多实体信号压缩为紧凑、高信息的证据，比多跳路径传递更多信息，为局部实体推理提供了一种高效的全球替代方案。
...
> 本文提出了Mindscape Activation Signature (MiA-Signature)概念，通过压缩全局激活模式来近似激活对下游处理的影响，并在RAG和代理系统中实现了性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 认知科学研究表明可报告的意识访问与分布式记忆系统的全局点燃相关，但这种激活只能部分访问。这种张力暗示认知可能依赖紧凑表示来近似激活对下游处理的全局影响。

**Method:** 引入MiA-Signature概念，在LLM系统中通过基于子模块的高级概念选择来实现覆盖激活上下文空间的压缩表示，可选择通过工作记忆进行轻量级迭代更新来优化。

**Result:** 将MiA-Signatures集成到RAG和代理系统中，在多个长上下文理解任务中实现了一致的性能提升。

**Conclusion:** MiA-Signature为模拟认知系统中的全局激活提供了有效的计算机制，能够在保持计算可处理性的同时近似完整的激活状态效果，对长上下文理解任务具有实用价值。
...
> 提出了一种结合规则过滤和LLM分类的新词检测流水线，在5.27亿Reddit帖子中识别出599个真实的新词，准确率达58.7%。

<details>
  <summary>Details</summary>

**Motivation:** 大规模自动检测新词面临挑战，需要既能高效处理海量数据又能保证准确性的检测方法。

**Method:** 基于语法和超语法形态学框架，设计了四类分类方案(新词/实体/外来词/非新词)；先用规则过滤从1.246亿个独特token中筛选出1021个候选词，再用多个LLM进行多数表决分类，最后人工验证。

**Result:** 在5.27亿篇Reddit帖子中筛选出1021个候选词，人工确认599个(58.7%)为真实新词；发现不同LLM模型间存在显著分歧，揭示了大规模新词检测的困难。

**Conclusion:** 该流水线能有效缩小检测范围，降低人工验证成本；LLM分类存在不一致性，需要人工最终验证；为大规模新词检测提供了可复用的技术方案。
...
> 本研究利用内容特征（文本和语言特征）结合传统机器学习方法进行假新闻检测，在COVID-19数据集上实验表明，单独使用这些特征能有效提升检测效果，但组合使用改善有限。

<details>
  <summary>Details</summary>

**Motivation:** 尽管实证研究表明内容和语言特征有助于区分真假新闻，但此类特征在假新闻检测领域的研究相对不足，需要深入探索其应用价值。

**Method:** 研究人员收集了COVID-19期间的新闻数据集，提取词二元组、词性分布等内容特征，并采用决策树、K近邻、逻辑回归、支持向量机和随机森林等传统机器学习算法进行分类实验。

**Result:** 随机森林算法表现最佳，支持向量机紧随其后；文本特征和语言特征单独使用均能改善检测效果，但两者结合并未显著提升性能；词二元组和词性标签的效果也存在差异。

**Conclusion:** 传统的机器学习方法结合文本和语言特征可以有效检测假新闻，为不依赖深度学习的假新闻检测提供了可行的技术路径。
...
> 该研究通过将LLM生成的内容作为查询重新输入同一模型，分析了LLM在情感驱动对话中的一致性。研究发现LLM对错误预设查询表现不佳，尤其在中等情感强度下最为脆弱，且模型注意力从评估转向生成。

<details>
  <summary>Details</summary>

**Motivation:** 探索大型语言模型在情感敏感对话场景下的自我一致性表现，特别是在面对包含错误前提查询时的反应模式，以评估其在高风险实际应用中的可靠性。

**Method:** 设计了三个强度递增的错误声明查询，涵盖极端和中等两个情感维度，测试Claude-3.5-haiku、GPT4o-mini和Mistral-7B三个模型。采用注意力评分分析深入探究模型行为变化。

**Result:** LLMs表现低于平均水平，对嵌入的虚假信念非常敏感。这种脆弱性在中等情感内容时尤为突出。注意力分析显示模型优先级从评估向生成转变。

**Conclusion:** 研究结果对LLMs在情感敏感的高风险场景部署提出了重要警示，需要进一步改进模型处理情感内容和错误信息的能力。
...
> 论文提出了Litespark-Inference框架，通过定制化SIMD内核实现三元模型的高效推理，相比标准PyTorch推理获得9.2倍首token速度提升和52倍吞吐量提升。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型计算成本高昂，限制了普通用户的使用；现有框架未能有效利用三元模型的{-1,0,+1}权值特性，仍按密集浮点网络处理，浪费计算资源。

**Method:** 开发定制化SIMD内核，将矩阵乘法替换为加减法操作，利用现代CPU的整数点积指令；实现可pip安装的Litespark-Inference框架，直接集成Hugging-Face。

**Result:** 在Apple Silicon上实现9.2倍首token加速、52倍吞吐量提升和14倍内存减少，Intel和AMD处理器上获得类似加速效果。

**Conclusion:** 通过充分利用三元模型的结构特性，证明了在个人电脑上高效运行大语言模型推理的可行性，为普及AI应用铺平道路。
...
> 该研究发现，在隐喻新颖性判断中，词汇频率比语言模型惊奇度是更强的预测因子，以往关于惊奇度与隐喻处理难度的关联可能错误地将语境可预测性归因于隐喻新颖性。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型惊奇度被广泛用作语境可预测性的代理指标，并据报道与隐喻新颖性判断相关。然而，惊奇度与词汇频率紧密交织，需要探索这种相互作用对隐喻新颖性评级的影响。

**Method:** 研究者使用两种不同的词汇频率指标，分析了来自8个不同大小的Pythia模型和154个训练检查点的惊奇度估计，考察了词汇频率和惊奇度对隐喻新颖性的预测能力。

**Result:** 在所有设置中，词汇频率都比惊奇度更能预测隐喻新颖性。在训练过程中，惊奇度与新颖性的关联在早期达到峰值后下降，而惊奇度与频率的关联则相应上升。

**Conclusion:** 结果表明，以往报道的最优语言模型惊奇度设置可能错误地将语境可预测性与隐喻新颖性和处理难度联系起来，而词汇频率可能是真正的关键影响因素。
...
> 该论文发现了LLM代理在更新记忆时的关键失败模式，并提出了STALE基准测试和CUPMem解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM代理记忆评估主要关注静态事实检索，忽略了当新证据出现时需要更新已存储信念的能力，特别是一种名为'隐式冲突'的关键失败模式未被充分探索。

**Method:** 提出了STALE基准测试，包含400个专家验证的冲突场景（1200个评估查询），并设计了三维探测框架：状态解析、前提抵制和隐式策略适应。同时提出了CUPMem原型系统。

**Result:** 系统评估表明，即使是最先进的模型也仅能达到55.2%的整体准确率，揭示了检索更新证据与基于证据行动之间的普遍差距。

**Conclusion:** 显式状态裁决是实现强健代理记忆的有前景方向，通过结构化状态整合和感知传播的搜索可以加强写入时的修订。
...
> Token-Superposition Training (TST) 是一种简单即插即用的预训练方法，通过两阶段训练策略显著提升了数据吞吐量，在相同损失下可将10B模型的预训练时间减少2.5倍。

<details>
  <summary>Details</summary>

**Motivation:** 大规模语言模型预训练成本极高且效率低下，需要复杂的修改才能实现高数据吞吐量。研究者希望找到一种无需修改现有架构、并行策略或优化器就能提升训练效率的方法。

**Method:** TST采用两阶段训练：(1) 高效叠加阶段 - 将多个连续token组合到一个bag中，使用多热点交叉熵目标函数进行训练；(2) 恢复阶段 - 回到标准训练。整个过程无需修改并行策略、优化器、分词器、数据或模型架构。

**Result:** 在270M、600M参数规模上广泛评估，并在3B和10B A1B专家混合模型上验证，TST在不同设置下都表现出很强的鲁棒性。在损失相同时，TST在10B规模下实现2.5倍的训练时间减少。

**Conclusion:** TST是一种有效的即插即用训练方法，能够显著提升大规模语言模型预训练效率，在保持或提升模型性能的同时大幅降低训练时间和计算成本。
...
> Cola DLM是一种分层潜在扩散语言模型，通过将文本生成分解为语义组织和文本实现两个阶段，实现了比传统自回归模型更灵活的非自回归文本生成方案。

<details>
  <summary>Details</summary>

**Motivation:** 现有大语言模型在自回归范式下虽然成功，但高质量文本生成不必拘泥于固定的从左到右顺序。现有替代方案难以同时实现生成效率、可扩展表示学习和有效的全局语义建模。

**Method:** 提出Cola DLM分层潜在扩散模型：首先使用Text VAE学习稳定的文本到潜在表示映射，然后在连续潜在空间中用块因果DiT建模全局语义先验，最后通过条件解码生成文本。其扩散过程执行潜在先验传输而非token级观察恢复。

**Result:** 通过4个研究问题、8个基准测试、严格匹配的约20亿参数自回归基线以及扩展到2000 EFLOPs的扩展曲线实验，验证了Cola DLM的有效配置和强大的扩展行为。

**Conclusion:** 建立了分层连续潜在先验建模作为严格token级语言建模的原则性替代方案，其生成质量和扩展行为能更好地反映模型能力，同时为统一离散文本和连续模态建模提供了具体路径。
...
> Lighthouse Attention是一种训练阶段的分层注意力算法，通过自适应序列压缩/解压缩和对称选择策略，实现了Transformer模型的亚二次时间复杂度训练，最终恢复完整注意力模型。

<details>
  <summary>Details</summary>

**Motivation:** 传统缩放点积注意力(SDPA)的时间和内存复杂度为O(n²)，这成为因果Transformer在极长序列长度上训练的主要瓶颈，限制了大规模语言模型的发展。

**Method:** 提出三重贡献：(i) 亚二次时间复杂度的分层预处理和后处理步骤，实现序列的自适应压缩和解压缩；(ii) 对称压缩策略，同时池化查询、键和值并保持从左到右的因果性，大幅提升并行性；(iii) 两阶段训练方法，主要训练阶段使用Lighthouse Attention，最后阶段短时间训练恢复完整注意力模型。整个算法无需梯度，避免了复杂的反向传播内核实现。

**Result:** 通过小规模LLM预训练实验证明，在所有其他设置匹配的情况下，相比完整注意力训练，该方法实现了更快的总训练时间和恢复阶段后更低的最终损失。

**Conclusion:** Lighthouse Attention作为训练阶段的优化算法，成功解决了长序列Transformer训练的计算瓶颈，为大规模语言模型的高效训练提供了可行方案，且可在训练结束时轻松移除，不影响推理性能。
...
> 本研究比较了两种远程认知康复治疗自动报告生成方法：基于规则模板系统和GPT-4零样本方法，发现模板系统在临床可靠性方面表现更好，而GPT-4输出更简洁。

<details>
  <summary>Details</summary>

**Motivation:** 远程康复工具的广泛使用产生了大量交互数据，临床医生难以高效审查，同时缺乏可用的参考报告，亟需开发自动化临床报告生成系统。

**Method:** 提出并比较两种方法：(1)将言语治疗领域知识编码为显式决策规则和验证模板的规则模板系统；(2)基于GPT-4的零样本LLM方法。两种系统使用相同的预提取结构化变量，由8名言语治疗师和学生使用九项标准问卷进行评估。

**Result:** 规则模板系统在流畅性、连贯性和结果呈现方面得分更高，GPT-4产生更简洁的输出。在临床可靠性和语言质量之间存在明显的权衡，但各项比较均未达到统计显著性差异。

**Conclusion:** 基于评估反馈提出了8项远程康复临床报告系统设计建议，并贡献了一个可复制的方法论，结合专家启发、分类驱动的生成和多维度人工评估，为低资源环境下的临床自然语言生成提供了指导。
...
> UniSD是一个统一的自蒸馏框架，通过整合多种互补机制，系统性地解决了自回归语言模型自蒸馏中的挑战，在6个基准测试上实现了显著性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型自蒸馏面临三大挑战：自生成轨迹的自由形式、正确性的任务依赖性、看似合理但可能不稳定不可靠的监督。现有方法仅考察孤立的设计选择，缺乏系统性研究。

**Method:** 提出UniSD统一框架，整合多种互补机制：多教师一致、EMA教师稳定化、token级对比学习、特征匹配和散度裁剪，全面解决监督可靠性、表征对齐和训练稳定性问题。

**Result:** 涵盖6个基准测试和3个模型家族的6个模型，UniSD揭示了自蒸馏超越静态模仿的关键时机，识别了驱动性能提升的核心组件及其跨任务交互规律。

**Conclusion:** 基于研究发现构建的UniSDfull集成管道实现最强整体性能，相比基线模型提升5.4分，相比最强基线提升2.8分，证明了自蒸馏作为无需外部强教师的高效LLM适应方案的实用性和可控性。
...
> 该研究形式化了算法语(Algospeak)与检测系统之间的共同演化动力学，提出多数人可理解调制(MUM)概念，并构建了可重复框架来研究规避策略的可理解性与可检测性之间的权衡。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型语言模型在内容生成和审核中的作用日益重要，语言规避策略(Algospeak)加剧了规避者与检测者之间的共同演化，需要理解其中的动态平衡关系。

**Method:** 建立联合行动模型形式化动力学，引入MUM概念，构建可重复框架生成保留意义的算法语变体，创建包含700个调制项的COVID-19虚假信息数据集，对7种语言模型进行意义恢复和虚假信息检测评估。

**Result:** 通过曲线拟合估计了MUM阈值，揭示了可理解性与调制之间的特征关系，能够进行不同策略和模型的敏感性分析。

**Conclusion:** 该研究为理解算法语背后的动力学奠定了基础，提供了框架、数据集和实验设置，为未来研究提供了重要工具和见解。
...
> 该研究提出了一种简化的人机协同工作流程，利用两个领域自适应解析器的一致性来实现二语韩语的形态句法标注。研究验证了解析器一致性可作为标注正确性的代理指标，并发现解析器分歧主要集中在语法关系和子句边界等语言领域。

<details>
  <summary>Details</summary>

**Motivation:** 解决二语韩语标注任务中人工标注工作量大、成本高的问题，探索利用自然语言处理技术实现半自动化标注的可能性。

**Method:** 提出基于两个领域自适应解析器一致性的工作流程，通过将解析器结果与独立人工判断进行对比，验证解析器一致性作为标注质量指标的可行性，并对分歧案例进行语言学分析。

**Result:** 研究显示解析器与人工判断之间存在强相关性，证明了半自动化二语韩语依存关系标注的可行性。分歧主要集中在语法关系区分和子句边界模糊等可预测的语言领域。

**Conclusion:** 该方法为二语韩语标注提供了有效的半自动化解决方案，虽然存在一些深层次表示挑战，但大多数分歧案例可通过模型迭代优化来解决，具有良好的应用前景。
...
> 本研究提出了首个源归属评估框架，通过AST解析器提取和评估LLM生成报告中的引用，从链接有效性、内容相关性和事实准确性三个维度进行评估，发现即使是顶尖模型的事实准确性也仅有39-77%。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型驱动的深度研究代理能够从大量网络源信息生成引用报告，但这些引用无法可靠验证。现有方法要么信任模型自引用，要么使用不验证源可访问性、相关性或事实一致性的检索增强生成。

**Method:** 开发了基于AST解析器的可重现框架，大规模从LLM生成的Markdown报告中提取内联引用。通过检索实际引用内容，人工或模型评估器从三个维度评估：链接可访问性验证、内容相关性测量和事实准确性验证。

**Result:** 评估14个模型显示，最强模型链接有效性>94%、相关性>80%，但事实准确性仅39-77%。不到一半的开源模型能一次性生成引用报告。工具调用从2增至150时，事实准确性平均下降42%。

**Conclusion:** 揭示了表面引用质量与事实可靠性之间的严重脱节，更多检索并不能产生更准确的引用，该框架为评估这种脱节提供了基础设施。
...
> 本文提出了StraTA框架，通过引入显式的轨迹级策略来解决大语言模型在长期决策制定中的困难，在多个基准测试中显著提升了样本效率和最终性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法大多是被动的反应式决策，在长轨迹中探索不足且信用分配困难，导致大语言模型作为交互智能体在长期决策制定任务上表现受限。

**Method:** 提出战略轨迹抽象(StraTA)框架，从初始状态采样紧凑策略，后续动作基于该策略执行，采用分层GRPO风格的rollout设计，并辅以多样化策略rollout和关键自我判断机制，联合训练策略生成和动作执行。

**Result:** 在ALFWorld、WebShop和SciWorld三个基准上，StraTA相比强基线方法持续提升样本效率和最终性能，分别达到93.1%、84.2%的成功率和63.5%的总体分数，超越了前沿闭源模型。

**Conclusion:** StraTA通过引入显式的轨迹级策略抽象，有效解决了大语言模型在长期决策任务中的核心挑战，为交互式智能体的强化学习提供了新的有效范式。
...
> 本文提出了POPO（Positive-Only Policy Optimization）框架，仅使用正向样本进行强化学习，通过有界重要采样和孪生网络策略，在数学推理任务上超越了GRPO方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的GRPO方法依赖负样本进行策略优化，但在稀疏二元奖励环境下，负样本无法有效区分失败严重程度，且少量负样本难以覆盖有意义的奖励信号。

**Method:** POPO框架仅使用正向样本进行策略优化，采用有界重要采样技术。通过引入孪生网络策略和动量自适应机制稳定策略演化，用有界相似性惩罚项替代KL散度。

**Result:** 在Qwen系列模型上的实验表明，POPO性能与GRPO相当或更优。特别是在AIME 2025测试中，POPO达到36.67%的准确率，优于GRPO的30.00%。

**Conclusion:** 仅使用正向样本的强化学习方法同样可以取得优秀的推理性能。POPO的各个组件都是必要且鲁棒的，为LLM的强化学习优化提供了新的有效途径。
...
> EMO是一种新型混合专家模型，通过引导token从相似域使用相似专家，实现了模型模块化部署，保留25%专家时性能仅下降1%，远超标准MoE模型。

<details>
  <summary>Details</summary>

**Motivation:** 传统大语言模型部署时需要加载完整模型，造成内存浪费。现有的MoE模型在专家子集使用时性能严重下降，限制了在内存受限环境下的实用性。

**Method:** EMO通过文档边界限制同一文档内的token从共享专家池选择，而不同文档使用不同专家池，无需人工预设即可在预训练中形成专家分组。

**Result:** 1B活跃参数、14B总参数的EMO模型性能匹敌标准MoE，在保留25%和12.5%专家时分别仅造成1%和3%的性能下降，实现了语义级专家专业化。

**Conclusion:** EMO为大模型提供了模块化、内存高效的部署路径，展现了组合式架构的新可能性，为大型稀疏模型的实际应用开辟了新方向。
...
> 该研究提出了一种基于布局感知的开放集身份文档欺诈检测方法，通过适配DINOv3模型，在仅用美国ID训练的情况下，实现了99.83%的加拿大布局分类准确率，并发现了276个自适应物理欺诈案例。

<details>
  <summary>Details</summary>

**Motivation:** 身份文档欺诈检测不是固定的二分类问题，自适应攻击者会不断修改模板和造假流程，使历史欺诈标签失效，且成功的伪造行为会以连贯活动形式大规模重复出现。

**Method:** 采用布局感知表示学习方法，通过上下文感知的SimMIM微调和监督式度量学习（使用复合损失函数促进类间可分离性和类内紧凑性）来适配DINOv3到文档领域，模型仅在美国身份证数据上训练。

**Result:** 在20,448个加拿大身份证数据集上，轻量级MLP分类器达到99.83%的布局分类准确率；在嵌入空间分析中发现276个自适应物理欺诈案例，其中222个未被现有检测器发现；支持从单个确认种子扩展到相关案例。

**Conclusion:** 布局感知的文档嵌入为在分布偏移下发现新颖和大规模欺诈活动提供了符合生产需求的基础，有效解决了传统封闭集分类方法的局限性。
...
> 本文提出了一种基于生成对抗网络(GANs)和循环一致性损失的反事实解释(CX)方法，用于医学图像中的类别导向特征归因，并开发了反事实实例(CI)生成技术及评估方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可视化技术主要基于判别式模型，仅突出参与分类决策的最小判别特征区域，无法考虑所有显著物体。在医学影像中，这种方法不足以帮助放射科医生完全可视化异常情况，需要更全面的可解释方法。

**Method:** 构建基于GANs的循环一致性损失模型，实现反事实解释框架。提出反事实实例(CI)生成方法，将CX与可信的CI结合提供自解释的类比解释。同时开发了反事实实例质量的评估技术。

**Result:** 在合成数据集、结核病数据集和BraTS数据集上进行了实验验证。所有实验都证实了所提方法的有效性。BraTS数据集上的基线结果展示了反事实实例生成和评估的性能。

**Conclusion:** 研究成功解决了传统视觉解释方法的局限性，通过反事实解释和可信反事实实例的结合，为医学图像分析提供了更全面的可解释性框架，为临床诊断提供了更有价值的可视化洞察。
...
> 本文提出了一种密度感知校准方法，通过结合DETR风格3D目标检测器的潜在对象查询特征密度，实现了在分布偏移场景下的可靠不确定性估计。

<details>
  <summary>Details</summary>

**Motivation:** 现代3D目标检测器的校准性能较差，特别是在分布偏移情况下。现有的后处理校准方法虽然能改善分布内测试的校准效果，但在分布偏移场景中表现不佳，这对安全自主系统的部署构成威胁。

**Method:** 将后处理校准器与DETR风格3D目标检测器的潜在对象查询特征密度相结合。这些查询形成紧凑的位置和类别感知特征，适合密度估计，能够在分布偏移场景中调整模型置信度，并联合重新校准分类和边界框回归的不确定性。

**Result:** 在多视角相机和基于LiDAR的检测器上，该方法在分布内和分布偏移场景下都持续优于标准后处理方法。

**Conclusion:** 提出的密度感知校准方法有效解决了3D目标检测在分布偏移情况下的不确定性估计问题，为安全自主系统提供了更可靠的校准性能。
...
> ViTok-v2是一个支持多分辨率训练、使用DINOv3感知损失的Vision Transformer图像标记器，在20亿图像上训练，达到50亿参数，实现了SOTA重建质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有ViT标记器在训练分辨率外性能下降，对抗训练不稳定，且重建与生成之间存在权衡关系。改进标记器的重建质量是实现更优Pareto平衡的关键。

**Method:** 引入NaFlex实现原生多分辨率和宽高比支持，用新型DINOv3感知损失替代LPIPS和GAN目标，在约20亿图像上训练并扩展至50亿参数。

**Result:** ViTok-v2在256p分辨率匹配或超过SOTA，在512p及以上分辨率优于所有基线。与流匹配生成器的联合扩展实验推动了重建-生成权衡的Pareto前沿。

**Conclusion:** ViTok-v2成功解决了现有ViT标记器的分辨率适应性和训练稳定性问题，通过大规模参数扩展实现了前所未有的图像重建性能，为下一代图像生成系统奠定了基础。
...
> Open-SAT是一种无需训练的查询嵌入精炼算法，通过结合VLMs和LLMs来提升开放词汇卫星图像检索性能，F1分数最高提升16.04%。

<details>
  <summary>Details</summary>

**Motivation:** 卫星应用中用户查询通常是开放词汇的自然语言，现有VLMs如CLIP即使微调也难以准确对齐卫星图像，需要解决开放词汇检索的挑战。

**Method:** Open-SAT在推理时利用LLM精炼文本嵌入，结合目标对象及周围环境的上下文信息，采用无阈值检索机制，使用VLMs计算图像块嵌入并存储在向量数据库中。

**Result:** 在三个公开基准测试中，Open-SAT将F1分数提升高达16.04%，同时检索到相当数量的图像块，证明了其有效性。

**Conclusion:** Open-SAT成功利用LLM指导实现了无需额外训练或监督的开放词汇卫星图像检索，为开放词汇检索问题提供了有效解决方案。
...
> egenioussBench是一个基于地理空间参考数据的大规模视觉定位基准测试，使用城市级3D网格和CityGML LoD2模型，包含厘米级精度的智能手机图像数据集。

<details>
  <summary>Details</summary>

**Motivation:** 传统的SfM方法在大规模视觉定位中存在可扩展性限制，需要一个能反映实际部署场景的城市级基准测试，同时提供精确的地面真值以支持公平的方法比较。

**Method:** 构建包含城市级航空3D网格和CityGML LoD2模型的地理空间参考数据，使用PPK和GCP/CP辅助调整获取智能手机图像的厘米级真值，通过估计共可见性矩阵并选择最大独立集来提取非共可见子集。

**Result:** 从2,709张图像中提取出42张非共可见测试图像（隐藏真值）和412张连续验证图像，提供包含多种姿态误差阈值的公开排行榜评估系统。

**Conclusion:** 该基准测试通过暴露现实的跨视图和跨域挑战，为大规模视觉定位研究提供了严格、可扩展的发展路径，支持基于网格和LoD2方法之间的公平比较。
...
> 该论文为阿拉伯手语提供了首个高质量3D参数化注释数据集，并提出了专门的3D重建管道Tamaththul3D，显著提升了手部重建精度。

<details>
  <summary>Details</summary>

**Motivation:** 阿拉伯手语服务于全球约4亿阿拉伯语使用者，但缺乏高质量的3D参数化注释和专门的虚拟化身重建方法，这限制了无障碍技术的发展和文化保护工作。

**Method:** 论文包含两个主要贡献：1) 为Ishara-500沙特手语数据集创建了首个高质量3D参数化注释，提供500个文化真实的SSL手势的SMPL-X参数；2) 提出Tamaththul3D重建管道，集成SMPLer-X进行身体估计、WiLoR进行手部细化、MediaPipe提供2D姿态监督，并通过运动链手腕对齐和混合摆转分解实现优化。

**Result:** Tamaththul3D在手部重建精度上达到最先进水平，相比之前方法提升高达32%，同时保持了具有竞争力的身体姿态精度。

**Conclusion:** 这些3D注释和Tamaththul3D管道建立了首个高保真度阿拉伯手语虚拟化身重建的综合框架，为阿拉伯聋人社群的无障碍技术创新和文化保护工作开辟了新途径。
...
> 本文提出了一种基于一致性学习的3D点云异常检测方法，通过直接预测异常自由几何结构，将推理速度提升80倍，同时保持高检测性能，适用于边缘设备部署。

<details>
  <summary>Details</summary>

**Motivation:** 随着3D感知技术在现代制造业中的普及，需要可靠的高吞吐量质量保证。但现有扩散模型方法计算开销大、迭代去噪过程缓慢，难以在资源受限的边缘设备上部署。

**Method:** 通过一致性学习重新构建基于重建的异常检测框架，实现一到两次网络评估直接预测异常自由几何结构；引入新型混合损失函数明确强制向干净数据重建。

**Result:** 推理速度比现有最优方法快80倍（无需GPU加速），在Anomaly-ShapeNet上达到76.20% I-AUROC，在Real3DAD上保持72.80% I-AUROC的竞争力。

**Conclusion:** 该方法解决了扩散模型迭代去噪的瓶颈问题，实现了在无人机、智能工业相机等资源受限边缘设备上的高效、低延迟异常检测。
...
> LAMP通过早期解耦观察者和目标运动，利用已知设备6自由度运动将多个相机的2D关键点转换到统一3D世界坐标系，然后用时空transformer直接拟合3D人体运动，显著改进了第一人称多摄像头的3D人体运动跟踪效果。

<details>
  <summary>Details</summary>

**Motivation:** 第一人称多摄像头头戴式设备的3D人体运动跟踪面临剧烈的自运动、局部遮挡和训练数据不足等挑战。现有单视频方法通常需要静态或缓慢移动的摄像头，无法有效利用多视角、已标定和定位的输入，导致在动态第一人称场景下表现脆弱。

**Method:** LAMP采用两步流程：首先利用已知设备6自由度运动和标定将所有摄像头在时间窗口内检测的2D身体关键点转换到统一3D世界参考坐标系；其次使用端到端训练的时空transformer直接将3D人体运动拟合到这个3D射线云。这种'先提升后拟合'的方法让LAMP能够在世界空间中学习和利用自然的人体运动先验。

**Result:** LAMP在单视角基准测试上取得了最先进的结果，在目标第一人称设置下显著优于基线方法，展示了其在处理时间异步、部分观测和移动摄像头输入方面的灵活性和有效性。

**Conclusion:** LAMP提供了优雅的框架来灵活处理多个时间异步、部分观测和移动摄像头的信息，通过早期解耦观察者和目标运动，有效解决了第一人称场景下的3D人体运动跟踪挑战。
...
> 论文提出GeoQuery系统，通过提示对齐的文本代理实现零样本遥感图像检索，无需大规模对比训练即可在卫星图像存档中进行语义搜索。

<details>
  <summary>Details</summary>

**Motivation:** 地球观测存档的语义搜索面临挑战。CLAY等视觉基础模型能生成丰富的卫星图像嵌入，但缺乏自然语言基础；而完整的遥感CLIP风格对比训练需要全球范围内不可获得的配对数据和计算资源。

**Method:** 提出GeoQuery，一个通过提示对齐文本代理的零样本检索系统。不为训练联合编码器，而是为10万个全球Sentinel-2瓦片生成语言描述，优化描述生成提示使文本嵌入空间距离与冻结CLAY视觉嵌入空间距离相关联。查询分为两阶段：代理子集的文本相似性搜索和全球CLAY嵌入的视觉最近邻搜索。

**Result:** 在英国洪水、美国野火和美国干旱的76个灾害位置查询中，GeoQuery在50公里范围内达到31.6%的准确率，其中洪水表现最佳（50公里内50%准确率）。在ECHO危机响应系统中部署时，成功识别了2025年布里斯班飓风Alfred期间的脆弱区域。

**Conclusion:** 提示对齐代理为当完整对比训练不可行时，在地球观测基础模型和操作检索之间提供了实用桥梁，实现了跨模态的语义连接。
...
> 提出一种主动式相机可靠性监控框架，通过全局传感器健康指数(GSHI)在下游任务性能下降前预估感知风险，实现故障早期预警。

<details>
  <summary>Details</summary>

**Motivation:** 安全关键型ADAS感知需要可靠的相机输入，但现有监控方法只能在下游性能下降后才检测到传感器故障，缺乏主动预警能力。

**Method:** 设计全局传感器健康指数(GSHI)作为连续可靠性评分，使用风险感知乘积公式聚合各退化严重度；构建轻量级多任务网络从单RGB图像预测退化类型、严重度、GSHI和空间不确定性图；利用物理和几何感知的合成监督训练12种相机退化模式。

**Result:** GSHI随严重度单调下降，健康估计MAE为0.064；在YOLOv8检测失败前提供0.47±0.25严重度单位的预警时间；在IQA、检测器置信度和干净特征OOD基线中表现最优；可零样本迁移到真实恶劣天气驾驶数据。

**Conclusion:** 基于退化感知的不确定性分析是智能车辆主动相机可靠性监控的实用方向，为实现前瞻性故障检测提供了有效解决方案。
...
> EchoXFlow是一个包含37,125个心脏超声记录的临床数据集，保留了原始采集几何结构和多模态数据，支持物理基础的跨模态学习。

<details>
  <summary>Details</summary>

**Motivation:** 现有公开数据集缺乏对心脏解剖、心肌运动和血流量之间跨模态关系的研究机会，多普勒数据通常缺失或作为RGB覆盖层融合，且采集数据经过有损处理后发布，限制了基于物理的超声学习研究。

**Method:** 构建了一个包含666次常规临床检查的大型数据集，每个记录保留为可分离的模态特定流：时间分辨的1D、2D、3D数据以及多普勒模态，并配合同步ECG。提供从基于指南的测量到密集2D心肌轮廓和3D左心室内膜网格的临床标注。

**Result:** 建立了EchoXFlow数据集及配套的开源工具，使研究者能够进行跨模态、感知采集的学习任务，这些任务无法仅从传统的扫描转换视频中构建，为4D视觉和物理基础多模态学习提供了测试平台。

**Conclusion:** EchoXFlow数据集填补了心脏超声研究中多模态原始数据的空白，为开发更物理准确、跨模态的深度学习方法提供了重要基础设施，将推动心脏超声AI分析技术的发展。
...
> 本文总结了NTIRE首届可控背景虚化渲染挑战赛的结果，共8个团队提交有效方案，通过定量指标和专家定性评估两个赛道对算法进行评价。

<details>
  <summary>Details</summary>

**Motivation:** 举办首届可控背景虚化渲染挑战赛，旨在推动该领域技术发展，建立基准评测体系，促进虚化效果生成算法的创新。

**Method:** 采用双轨道评价体系：第一轨道基于既定定量保真度指标进行评估；第二轨道通过专家组进行定性用户研究，侧重感知评估。比赛主要基于Bokehlicious基线方法的改进和扩展。

**Result:** 44名参与者注册，8个团队提交有效解决方案。参赛算法在未见过的人像和复杂主体图像上进行测试，展现了复杂的视觉背景虚化现象。

**Conclusion:** 作为该主题的首个挑战赛，大多数参与者专注于改进Bokehlicious基线方法。比赛成功建立了可控背景虚化的评测框架，为该领域未来发展奠定了基础。
...
> 本研究使用AlphaEarth基金会的卫星嵌入模型，通过提出

<details>
  <summary>Details</summary>

**Motivation:** 大西洋森林是巴西关键的生物多样性热点，但仅剩下不到12-15%的原始覆盖。传统监测方法在大规模应用时受到实地报告不切实际和遥感指标饱和的限制，且造林是渐进过程，传统方法难以有效监测。

**Method:** 研究了圣保罗州1,729个恢复地点，使用AlphaEarth基金会的卫星嵌入模型评估其在表征早期恢复效果方面的有效性。引入'参考轨迹嵌入'概念，定义基于与成熟次生林参考站点余弦相似性的恢复成功指标。

**Result:** 观察到嵌入空间中根据不同土地利用和土地覆盖类型形成的明显聚类，能够识别具有清晰变化向量的站点。但信号可能存在噪声，嵌入可能需要进一步微调以捕捉和预测超越土地利用和土地覆盖的站点元数据。

**Conclusion:** 卫星嵌入技术为大尺度森林恢复监测提供了新方法，参考轨迹嵌入概念为评估恢复成功提供了量化指标，但仍需改进模型精度以处理噪声并扩展预测能力。
...
> 本文提出了GDS-Mamba模型，通过图调节、解耦Mamba架构和稀疏token方法，解决了MODIS时间序列数据中树种分类的挑战。

<details>
  <summary>Details</summary>

**Motivation:** 树种分类对环境应用至关重要，但面临物种间细微差异、时空谱信息强耦合以及大规模拓扑信息建模困难等挑战。

**Method:** 设计了(1)小批量图调节方法探索拓扑相关性；(2)解耦Mamba架构分离空间、光谱和时间特征；(3)稀疏token方法自适应学习最优token子集来解决相关性衰减问题。

**Result:** 在加拿大两个省份的大规模MOD13Q1数据上实验，阿尔伯塔省准确率达93.94%，跨省份评估准确率80.19%，超越12个最先进分类模型。

**Conclusion:** GDS-Mamba模型有效解决了树种分类的关键难题，在处理大规模时空谱数据和建模复杂拓扑关系方面展现出优越性能。
...
> 研究发现，仅需训练神经网络区分8个大类别，就能获得与人类视觉系统高度对齐的内部表征，其在神经对齐和感知相似性判断方面优于1000类分类任务训练的模型。

<details>
  <summary>Details</summary>

**Motivation:** 尽管已有研究表明人工神经网络可以模拟灵长类视觉系统，但缺乏对不同粒度监督信号如何影响大脑对齐的系统研究。这限制了对视觉学习所需信号的理解。

**Method:** 通过PCA分割预训练嵌入，将训练图像分成2到64个类别，训练数百个不同架构的神经网络，评估其与猕猴电生理记录和人类fMRI响应的对齐程度。

**Result:** 粗粒度训练（8类）的神经网络在神经对齐上达到或超过细粒度模型，且在人类感知相似性判断上表现最优，超越了自监督学习和大型视觉模型。

**Conclusion:** 人类视觉表征可以从极粗粒度的反馈中产生，这重新定义了视觉学习所需的信号特性，为构建更符合人类感知的AI系统提供了新方向。
...
> 本文提出了文本到CAD模型的跨模态检索任务，通过学习过程序列和几何点云的多模态嵌入，实现了基于自然语言查询的CAD模型高效检索。

<details>
  <summary>Details</summary>

**Motivation:** 现有的CAD模型库主要使用文件名或目录进行搜索，限制了设计重用的效率、可扩展性和准确性。为了提高工业设计遗产的重用价值，需要开发基于文本语义的CAD模型检索方法。

**Method:** 提出了统一框架，结合过程序列编码器（捕获构建逻辑）和点云编码器（提取几何特征），通过文本编码器学习查询语义表示。训练时引入特征解码器通过跨注意力重建掩码特征，实现多模态对齐；推理时移除解码器，使用拼接特征进行高效检索。

**Result:** 基于Text2CAD数据集建立了实用的基准测试，所提框架为文本到CAD检索提供了强有力的基线，并为检索增强生成等下游CAD生成范式奠定基础。

**Conclusion:** 本文首次正式提出文本到CAD检索的跨模态任务，通过创新的多模态嵌入学习方法实现了高质量的文本驱动CAD模型检索，为CAD设计重用和生成提供了新的技术路径。
...
> 本文提出了一种基于深度贝塔回归的不确定性引导边缘学习算法（UGEL），通过单次前向传播计算预测不确定性，优先选择数据加速卫星上图像回归模型的训练收敛。

<details>
  <summary>Details</summary>

**Motivation:** 在边缘设备上进行机器学习训练时，计算资源受限不仅影响模型优化，也难以计算预测不确定性，而这些不确定性信息对指导模型更新至关重要。特别是在遥感卫星等资源极其受限的环境中，需要高效的不确定性估计方法。

**Method:** 提出深度贝塔回归方法，将深度网络用于估计贝塔分布的参数，使目标值y对输入图像具有高似然性。基于该方法构建UGEL算法，通过单次前向传播计算预测不确定性，准确优先选择需要训练的数据，加速模型收敛。

**Result:** UGEL相比主动学习和半监督学习能够实现更快收敛的边缘学习。深度贝塔回归相比传统不确定性估计方法（如需要多次前向传播的方法或假设高斯分布的方法）更高效且适用性更广。

**Conclusion:** 该方法有效解决了边缘设备上不确定性计算成本高的问题，为资源受限环境下的边缘学习提供了实用的解决方案，代码和模型已开源。
...
> RAM-H1200是一个包含1200张手部X光片的大规模数据集，首次统一支持全手骨骼结构分割、像素级骨侵蚀分析和临床标准化的SvdH评分系统。

<details>
  <summary>Details</summary>

**Motivation:** 现有类风湿关节炎(RA)评估的公共资源缺乏多层次分析能力，缺少全手覆盖、细粒度标注以及与临床评分系统的一致性整合，特别是用于定量分析骨侵蚀的标注稀缺。

**Method:** 构建了一个包含1200张手部X光片的数据集，提供四级多层标注：(i)全手骨骼实例分割、(ii)像素级骨侵蚀掩码、(iii)SvdH定义的关节区域、(iv)关节级别的SvdH评分。

**Result:** 基准测试结果显示，解剖建模比定量骨侵蚀分析更成熟；全手骨骼分割表现出色，但骨侵蚀分割仍是主要挑战。

**Conclusion:** RAM-H1200是首个统一支持全手骨骼结构分割、像素级骨侵蚀分析和临床标准化SvdH评分的大规模公开数据集，为手部X光片的综合RA分析提供了单一基准平台。
...
> 本文提出了一种利用视觉语言模型生成高质量合成数据来解决森林物种语义分割中数据稀缺和类别不平衡问题的框架，显著提升了模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统森林物种组成调查劳动强度大且受地理限制，而基于无人机遥感的深度学习方法面临专家标注数据严重稀缺的瓶颈，特别是在复杂视觉异构的再生区域。

**Method:** 利用Nano Banana Pro大规模视觉语言模型根据提示同时生成高保真图像及其像素级语义掩码；构建WilDReF-Q-V2数据集和Gen4Regen合成数据集；整合真实数据与AI生成数据进行统一训练。

**Result:** 统一训练相比纯监督基线F1分数提升超过15个百分点；少量提示生成数据能显著改善代表性不足物种的性能，某些物种F1分数提升高达30个百分点。

**Conclusion:** 视觉语言模型可作为灵活的数据生成器，有效解决专家标签稀缺或不可用的小众AI领域的感知任务启动问题。
...
> 本文提出了一种基于去光照网络先验的面部外观捕获方法，通过异构数据源训练，实现了从普通视频输入中高质量估计反射率，并将该方法转化为大规模可重新光照扫描数据集。

<details>
  <summary>Details</summary>

**Motivation:** 传统高质量面部外观捕获需要昂贵的工作室设置，而现有基于智能手机的野外捕获方法在从未知光照中解耦反射率方面存在困难。需要一种新的方法来解决复杂的光照-反射率解耦问题。

**Method:** 1. 训练一个强大的去光照网络作为先验来约束优化；2. 利用OLAT数据集和渲染的Light Stage扫描数据；3. 提出数据集潜在调制(DLM)方法，通过可学习的源感知标记作为条件，将数据集特定风格与物理去光照原则解耦。

**Result:** 实现了简单自动的外观捕获管道，从普通视频输入获得高质量反射率估计，性能显著优于现有方法；创建了NeRSemble-Scan，一个包含4K分辨率可重新光照扫描的大规模数据集。

**Conclusion:** 通过开源模型和数据集，该方法使高端面部捕获技术大众化，为构建逼真数字人的研究社区提供了新的基础。
...
> 本文提出了VQAU新任务，旨在让模型在模糊查询下定位长视频中的情感时刻并进行解释，并构建了相应的基准测试和AffectSeek智能框架。

<details>
  <summary>Details</summary>

**Motivation:** 现有情感理解研究主要针对预剪辑的视频片段，这种被动和片段中心的设置无法充分反映用户通过自然语言查询表达需求并与长视频交互的真实场景。

**Method:** 1) 构建VQAU-Bench基准，整合长视频、模糊情感查询、时序片段标注、情感标签和解释说明；2) 提出AffectSeek智能框架，将任务分解为意图解释、候选定位、片段验证、情感推理和解释生成五个步骤，通过专业化推理和跨阶段验证逐步对齐模糊用户意图与长视频证据。

**Result:** 实验表明VQAU任务对现有情感识别模型和单步视觉语言模型具有挑战性，而AffectSeek为智能长视频情感理解提供了简单有效的框架解决方案。

**Conclusion:** 本研究填补了长视频模糊查询情感理解领域的空白，通过新任务定义、基准测试和智能框架，为更贴近实际应用场景的情感视频理解奠定了基础。
...
> MUSE框架通过拓扑正交性解决了视觉令牌化中重建与感知的根本冲突，实现了高保真重建与语义抽象的统一，在生成质量和感知性能上都达到了最先进水平。

<details>
  <summary>Details</summary>

**Motivation:** 统一视觉令牌化面临空间等变性与概念不变性之间的根本权衡。这种冲突源于流形不对齐问题，即简单的联合优化会产生相反的梯度，导致重建和感知之间形成零和博弈。

**Method:** 提出基于拓扑正交性的MUSE框架，将结构作为正交桥梁，在Transformer中解耦优化：结构梯度优化注意力拓扑，语义梯度更新特征值，从而将破坏性干扰转变为相互增强。

**Result:** MUSE打破了权衡困境，达到最先进的生成质量(gFID 3.08)，在线性探测中超越其教师模型InternViT-300M(85.2% vs. 82.5%)，证明了结构对齐的重建可以增强语义感知。

**Conclusion:** 通过拓扑正交性和优化解耦，MUSE成功统一了视觉令牌化的重建与感知任务，为视觉模型设计提供了新的思路，验证了结构信息对语义理解的积极作用。
...
> S2C-3D是一个新颖的稀疏视图3D重建框架，能够在仅需6-8张输入图像的情况下实现高保真度和完整的场景重建。

<details>
  <summary>Details</summary>

**Motivation:** 稀疏视图3D重建是计算机视觉中的重要挑战，现有方法在输入图像极少时往往会产生重建缺失、模糊等伪影问题，需要一个能够在极少视图下实现高质量重建的解决方案。

**Method:** 该方法包含三个核心组件：1)专门设计的扩散模型用于场景特异性的图像修复；2)训练免调的视图一致性条件采样过程用于优化高斯点；3)相机轨迹规划方案确保全面场景覆盖。通过在预训练模型上微调，适应场景分布并消除领域差异；通过迭代构建路径和保留显著增强可见性的路径来建立覆盖整个场景的轨迹。

**Result:** 实验结果表明，S2C-3D在稀疏输入条件下超越现有最先进方法，能够构建高质量、无缺失区域、无模糊或其他伪影的场景。

**Conclusion:** S2C-3D成功解决了稀疏视图3D重建中的关键挑战，通过结合专门设计的扩散模型、视图一致性采样和智能轨迹规划，为仅需6-8张图像的高质量3D重建提供了有效解决方案。
...
> 本文提出欧几里得测地线对齐(EGA)方法，通过残差适配器解决视觉编码器在未知类别查询下的性能崩溃问题，通过三重原则实现零初始化和梯度稀疏化。

<details>
  <summary>Details</summary>

**Motivation:** 基于冻结视觉编码器的向量搜索系统在面对未见类别查询时，现有适配器训练会发生性能崩溃，高容量适配器会将未知类别样本错误分配到已知类别簇中，导致最坏情况下的标签精度下降40多个百分点。

**Method:** 提出EGA残差适配器，结合三个核心原则：零初始化确保不破坏原始表示、局部三元组损失保持局部几何结构、超球面投影约束嵌入空间。这些原则共同产生自限性动态特性——已满足小边距的三元组停止产生梯度。

**Result:** 实验显示收敛时96.5%的三元组无梯度，保持未知类别区域基本不变同时充分优化已知类别。在五个OOD基准测试中，EGA在四个主要分割上获得最高最坏情况标签精度，第五个分割上也有持续改进。

**Conclusion:** EGA设计有效解决了适配器训练中的类别漂移问题，通过梯度稀疏化与有界OOD扰动的理论联系，为冻结视觉编码器的实际部署提供了可靠的解决方案。
...
> MotionGRPO是一种基于强化学习后训练的扩散模型框架，通过GRPO优化和混合奖励机制，显著提升了头戴设备信号到全身3D人体运动重建的精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于扩散的方法依赖于全局分布匹配，导致局部关节重建误差；需要在保持全局视觉合理性的同时提高局部关节精度。

**Method:** 将扩散采样建模为马尔可夫决策过程，通过群体相对策略优化(GRPO)进行优化；结合学习条件感知模型的全局视觉合理性与显式约束的局部关节精度；引入噪声注入策略增加样本方差，解决梯度消失问题。

**Result:** 实验表明MotionGRPO在全身3D人体运动重建任务中达到最先进性能，具有优越的视觉保真度。

**Conclusion:** 通过强化学习后训练和精细化的奖励引导，MotionGRPO成功解决了扩散模型在人体运动重建中的局部精度问题，为相关领域提供了新的技术思路。
...
> R2H-Diff是一个基于扩散模型的RGB到高光谱图像重建框架，通过条件迭代细化过程实现高效重建，在保持高质量的同时显著降低了模型复杂度。

<details>
  <summary>Details</summary>

**Motivation:** RGB到高光谱图像重建是一个高度不适定的逆问题，现有回归方法学习确定性映射，无法建模重建不确定性且容易产生过平滑的光谱响应。虽然扩散模型具有强大的分布建模能力，但直接应用面临高光谱维度、强波段间相关性和光谱保真度要求严格等挑战。

**Method:** 提出R2H-Diff框架，将光谱恢复表述为条件迭代细化过程，在RGB引导下实现渐进重建。设计了引导光谱精细化模块用于RGB条件特征融合，以及高光谱自适应转置注意力模块进行高效的空-谱依赖建模。采用无归一化去噪骨干网络保持光谱幅度一致性，使用任务自适应线性噪声调度，仅需5个去噪步骤即可实现高质量重建。

**Result:** 在NTIRE2022、CAVE和Harvard数据集上的广泛实验表明，R2H-Diff在重建质量和计算效率之间取得了良好的平衡。在NTIRE2022上，R2H-Diff使用仅0.58M参数和12.25G FLOPs的亚百万参数模型获得了35.37 dB PSNR，在评估方法中模型复杂度最低，同时保持强大的重建保真度。

**Conclusion:** R2H-Diff成功地将扩散模型应用于RGB到高光谱图像重建任务，通过专门设计的模块和策略有效解决了高维度、强相关性等技术挑战，实现了高质量重建与低复杂度的统一，为高光谱图像重建提供了新的解决方案。
...
> 本文提出了CFE-PPAR，这是首个面向压缩的隐私保护动作识别加密方法，能够在H.264和Motion-JPEG压缩后保持较高的识别性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于加密的隐私保护动作识别方法在视频压缩后会灾难性下降识别性能和视觉质量，缺乏对压缩的兼容性。

**Method:** CFE-PPAR方法使用密钥加密视频，并通过使用相同密钥转换参数的视频变换器直接识别加密视频，实现了加密与识别的统一。

**Result:** 在UCF101和HMDB51数据集上的实验证明，CFE-PPAR在Motion-JPEG和H.264压缩条件下均优于现有方法。

**Conclusion:** CFE-PPAR成功解决了加密视频压缩后的性能下降问题，实现了既保护隐私又兼容压缩的高效动作识别方案。
...
> 本文提出了一种基于跨模态提示调优的情感识别框架，通过将rPPG生理信号转化为时频表示来生成提示，调节预训练ViT的面部特征，并引入解耦共享-特定适配器来提高泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 面部表情无法完全反映内在情感状态，而远程光电容积图(rPPG)虽能提供互补的生理信息，但易受噪声和个体差异影响，现有多模态融合方法会破坏预训练的面部表示且缺乏抑制个体差异的机制。

**Method:** 将rPPG波形转换为抗噪的时频表示(TFRs)，从中生成模态互补的提示来调节冻结ViT的面部令牌；在ViT各层引入解耦共享-特定适配器(DSSA)，显式分离主体共享和主体特定成分。

**Result:** 在MAHNOB-HCI和DEAP基准测试上的实验表明，该方法在识别准确性和泛化能力方面均持续优于强基线方法。

**Conclusion:** 所提出的框架实现了有效的跨模态交互并保持了预训练骨干网络学习到的可泛化面部表示，为基于视频的情感识别提供了有效解决方案。
...
> 该论文提出了一个统一框架，将语言驱动的3D场景生成与沉浸式用户交互紧密结合，通过强化学习优化空间布局，支持人机交互循环反馈，在ALFRED基准测试上达到了最先进性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的语言驱动3D内容生成方法通常将场景生成和用户交互视为独立过程，这限制了交互式多媒体系统的适应性和沉浸潜力。

**Method:** 系统首先使用LLM根据自然语言指令构建结构化场景表示，然后通过强化学习在几何和语义约束下优化空间布局，在虚拟现实环境中部署生成场景，通过用户交互提供持续反馈来优化生成内容。

**Result:** 在ALFRED基准测试中达到了最先进性能。定性结果和用户研究表明，该框架在沉浸感、交互质量和任务效率方面均有显著提升。

**Conclusion:** 生成与交互的闭环集成对下一代多媒体系统至关重要，所提出的框架通过紧密耦合生成和交互，实现了更响应、自适应和真实的多媒体体验。
...
> EgoEMG是一个包含表面肌电(sEMG)与视觉数据同步的多模态第一人视角手部姿态估计数据集，包含41名参与者、60种手势类别、超过10小时的记录数据，并建立了统一的评估基准。

<details>
  <summary>Details</summary>

**Motivation:** 表面肌电(sEMG)能记录手部运动时的肌肉活动并解码精细的手部关节，而第一人称视觉提供全局手部配置，两者具有互补性。然而目前尚缺乏同时同步这两种模态的数据集。

**Method:** 构建了EgoEMG数据集，包含双侧腕带肌电(16通道，2kHz采样)、120Hz IMU、第一人称广角RGB视频、外部RGB-D视频以及动作捕捉衍生的手部运动数据。数据集涵盖41名参与者执行60种手势类别(30个单手手势和30个双手手势)。建立了三个任务的评估基准：肌电到姿态、视觉到姿态、肌电+视觉融合。

**Result:** 评估了EMGFormer用于肌电到姿态任务，以及通用的ResNet/ViT骨干网络用于视觉到姿态任务。研究了残差融合架构，相比轻量级纯视觉基线有改进。数据集包含超过10小时的记录数据。

**Conclusion:** EgoEMG数据集及其评估基准为未来基于肌电和视觉的多模态手部姿态估计研究奠定了基础，促进了该领域的标准化发展。
...
> TriRelVLA是一个基于三元关系结构的视觉-语言-动作模型框架，通过显式建模物体-手部-任务之间的关系，显著提升了机器人在未见场景、物体和任务组合中的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的VLA模型在训练见过的任务上表现良好，但在未知场景和物体上泛化能力差，主要因为其视觉表示隐式地混合了物体外观、背景和场景布局，使得策略对视觉变化敏感。虽然之前的工作通过结构化中间表示改善了可迁移性，但这些表示主要捕获场景语义而非动作相关的关系。

**Method:** 提出TriRelVLA框架，包含三个核心组件：1)从多模态输入构建显式的物体-手部-任务三元关系原语；2)构建任务引导的关系图，通过任务导向的交叉注意力形成节点，用关系感知的图变换器建模它们之间的交互；3)执行关系条件化动作生成，将关系结构压缩到瓶颈空间并投影到LLM中进行动作预测。

**Result:** 实验表明，模型在微调任务上表现出色，并且在跨场景、跨物体和跨任务泛化方面取得明显增益。同时引入了真实机器人数据集用于微调。

**Conclusion:** 通过建立物体-手部-任务三元关系的显式表示，TriRelVLA成功降低了对外观统计的依赖，实现了在不同场景、物体和任务组合间的有效迁移，为通用机器人操作提供了新的解决方案。
...
> B³-Net提出了一种受控后验桥接学习框架，通过显式建模任务间证据的可靠性变化来减少多任务密集预测中的负迁移。

<details>
  <summary>Details</summary>

**Motivation:** 现有多任务密集预测方法在任务间交互时通常隐式组织证据，通过相似性融合特征，没有显式建模不同任务和空间位置的证据可靠性差异，导致不可靠证据污染共享表示并加剧负迁移。

**Method:** 将解码器端交互分解为三个步骤：1)精度场估计器从任务参考对齐和局部变化估计补丁级证据精度；2)后验桥接算子通过异方差证据融合构建精度加权的后验桥接；3)收缩分发算子通过有界更新将桥接重新分配到各任务分支。

**Result:** 在NYUD-v2、PASCAL-Context和Cityscapes数据集上，B³-Net相比CNN、Transformer、diffusion、Mamba等多种方法取得了竞争性或更优的权衡效果。

**Conclusion:** 实验验证表明性能提升主要来源于受控后验桥接学习机制，而非骨干网络容量或解码器规模，证明了方法的有效性。
...
> 该论文提出了一种射线感知指针记忆机制用于流式3D重建，通过联合建模空间位置和观察方向，实现了自适应的内存管理策略，显著提升了长期重建的稳定性和相机位姿精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有的前馈式3D重建框架主要依赖外观相似性进行记忆更新，在视角变化时容易出现观察信息冗余积累和几何结构不稳定的问题，需要更有效的长期内存管理机制。

**Method:** 提出射线感知指针记忆表示，每个指针存储3D位置、关联射线方向和特征嵌入；引入自适应指针更新策略，采用保留或替换机制替代传统的融合压缩；通过空间距离和射线方向差异的联合推理区分局部冗余、新观察和闭环重访，并触发位姿优化。

**Result:** 大量实验证明射线感知记忆设计显著改善了长期重建稳定性和相机位姿精度，同时保持高效的流式推理能力，为可扩展和抗漂移的在线3D重建提供了原则性框架。

**Conclusion:** 该方法通过显式建模几何信息与视点一致性，有效解决了传统外观驱动方法的冗余积累和不稳定性问题，为大规模场景的实时3D重建提供了新的解决方案。
...
> TDSC是一种通过联合学习时间一致性结构化表示和稳定亲和力来实现人体运动分割的高效方法，它利用正则化自表达模型和时间约束来克服传统方法的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的人体运动分割方法基于子空间聚类，假设高维时间特征符合子空间并集分布，但现实视频中的原始帧级特征往往违反这一假设，导致分割性能不佳。

**Method:** 提出TDSC方法，交替学习输入帧特征的结构化表示和自表达系数，采用编码率最大化正则化器避免表示崩溃并符合期望的子空间分布，同时融入时间约束使相邻帧被分割到同一组，并开发时间动量平均机制和重参数化策略来稳定亲和力演化并实现高效优化。

**Result:** 在五个基准HMS数据集上，使用传统HoG特征和最新的深度特征(CLiP, DINOv2)进行广泛实验，验证了该方法的有效性。

**Conclusion:** TDSC通过联合优化结构化表示学习和时间一致性约束，有效解决了传统UoS假设不满足的问题，实现了更准确和鲁棒的人体运动分割效果。
...
> iTRIALSPACE是一个可编程的肺部CT模型评估框架，通过合成虚拟病灶试验解决了标准基准测试中病灶大小、肺叶分布、解剖结构和采集语境相互混淆的问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统的静态回顾性基准测试将病灶大小、肺叶患病率、解剖结构和采集语境等因素纠缠在一起，难以确定模型准确性的结构性驱动因素，需要一种可控的评估方法。

**Method:** 提出四阶段流程：多数据集结节分析、明确试验规范、解剖感知掩模插入和ControlNet条件CT合成，构建了包含54个属性、13,140个注释结节的统一数据集，实例化为13种试验模式。

**Result:** 在55,469个样本的虚拟病灶研究中，合成基质保持在真实到真实的FID基线内，合成性能排名强烈转移到真实临床数据(ρ=0.93, p<10^-15)，揭示了固定分布基准无法发现的发现。

**Conclusion:** iTRIALSPACE为超越静态回顾性基准的可控、可证伪测试提供了可审计的评估基础设施，能够暴露模型在特定条件下的shortcut行为。
...
> X-OmniClaw是一个统一的移动代理系统，通过整合感知、记忆和行动三大模块，实现Android生态系统中复杂且个性化的多模态交互。

<details>
  <summary>Details</summary>

**Motivation:** 受OpenClaw发展启发，市场对能够处理复杂直观交互的移动端个人代理需求增长，需要更智能的移动助手架构来提供个性化服务。

**Method:** 采用三模块统一架构：Omni Perception统一处理UI状态、视觉上下文和语音输入；Omni Memory结合工作记忆和长期个人记忆实现个性化；Omni Action通过结构化XML与视觉感知的混合策略完成任务，并通过行为克隆捕获用户技能。

**Result:** 在多种场景的演示中，X-OmniClaw有效提升了交互效率和任务可靠性，为下一代移动原生个人助手提供了实用的架构蓝图。

**Conclusion:** 该系统成功实现了感知-记忆-行动的统一架构，为移动端智能代理的发展提供了重要技术参考，展现了在复杂移动任务处理中的实用价值。
...
> MICCAI 2024第三届autoPET挑战赛评估了PET/CT病变自动分割的组合泛化性能，17个团队提交27个算法，最佳模型DSC达0.66，但未见过的示踪剂-中心组合仍是挑战。

<details>
  <summary>Details</summary>

**Motivation:** 在组合泛化设置下对全身PET/CT自动化病变分割进行基准测试，评估算法在不同示踪剂和医疗中心组合下的泛化能力。

**Method:** 使用1611个标注的PET/CT研究进行训练（包括1014个[18F]-FDG和597个PSMA），包含200个测试研究覆盖四种示踪剂-中心组合，其中两种为未见组合。通过基线模型和数据策略分离评估贡献。

**Result:** 最佳算法DSC=0.66，FNV=3.18mL，FPV=2.78mL，相比基线DSC提升8%，假阴性体积减少5mL。排名在前沿团队间保持稳定。

**Conclusion:** 域内多示踪剂分割已接近阅片者水平；未见示踪剂-中心组合的泛化仍是开放问题，主要受体积高估影响；病例异质性和难度比算法选择对性能影响更大。
...
> 本文提出UNO框架，通过理解任务作为生成模型的监督信号，增强多模态模型的生成能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有主流多模态模型采用解耦的理解和生成组件，虽然提升了单任务性能，但削弱了两者间的相互增强作用，缺乏协同效应。

**Method:** 提出理解导向的后训练框架(UNO)，将理解任务作为直接监督信号引导生成表示，结合语义抽象(图像描述)和结构细节(视觉回归)目标实现理解到生成的梯度传递。

**Result:** 在图像生成和编辑任务上的大量实验表明，理解任务可以有效促进生成任务的性能提升。

**Conclusion:** 理解任务可以作为生成任务的有效催化剂，UNO框架成功恢复了解理解与生成之间的协同作用，为统一多模态模型设计提供了新思路。
...
> Na-IRSTD是一个保持原始分辨率的红外小目标检测框架，通过原生分辨率特征提取和有效的token选择策略，在保持目标细节的同时降低计算负担，在四个基准数据集上达到最先进性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有红外小目标检测方法在特征下采样过程中会丢失小目标的细节信息，导致在复杂背景中难以准确定位暗目标，性能不佳。

**Method:** 提出Na-IRSTD框架，采用原生分辨率特征提取和融合来保留细微的目标线索；引入有效的token缩减和选择策略，高精度地选择目标补丁，增强低级特征细节，避免密集处理带来的计算负担。

**Result:** 在多个公开数据集上的广泛实验证明了token缩减和选择策略的鲁棒性和有效性，Na-IRSTD模型在四个基准测试中达到了最先进的性能。

**Conclusion:** 通过保持原始分辨率处理和智能的token选择，Na-IRSTD成功克服了现有红外方法的分辨率限制，显著提升了小目标的定位能力，为红外小目标检测提供了新的解决方案。
...
> 该研究发现了医学视觉语言模型在X光诊断中的'否定选项吸引'问题，并提出了相应的基准测试和解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 医学视觉语言模型在面对胸部X光的阳性发现时，会出现极性反转错误，即明明图像显示存在某种病灶，却选择'无病灶'的否定选项，这种错误不仅不准确，更会发出与图像矛盾的临床声明，存在严重的医疗安全风险。

**Method:** 研究团队构建了CXR-ContraBench基准测试，包含内部ReXVQA和外部OpenI、CheXpert三个数据集，重点测试存在性问题的极性反转错误。同时提出了QCCV-Neg（问题条件化否定一致性验证器）方法，可以在不重新训练的情况下确定性修复极性混淆问题。

**Result:** 在CheXpert数据集上，主流模型表现严重：MedGemma和Qwen2.5-VL在直接存在性探测上准确率仅31.49%和30.21%，在135,754条记录的训练集协议中，两模型在超过62%的存在性问题上选择了否定选项。使用QCCV-Neg修复后，两模型准确率分别提升至96.60%和95.32%。

**Conclusion:** 标准准确率指标会掩盖具有重要临床意义的推理时极性错误，这种否定选项吸引现象在当前模型中普遍存在且持续，需要专门的检测和修复机制来确保医学AI的安全性。
...
> ChartZero是一个通过合成数据训练的零样本图表数据提取框架，解决了现有方法在真实场景中泛化能力差和依赖人工标注的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有图表数据提取方法面临三个关键问题：1)图表样式多样性导致的泛化困难；2)真实标注数据严重稀缺；3)现有模型在曲线结构提取和图例语义关联方面经常失败，且评估方法不够全面。

**Method:** 通过在纯合成数据集上训练避免真实标注，引入全局正交实例(GOI)损失解决曲线碎片化，使用视觉语言模型(VLM)进行开放词汇图例匹配替代刚性空间规则，并设计了新的端到端评估基准。

**Result:** 实验表明ChartZero在无需真实世界监督的情况下，显著提升了通用图表数字化性能，为端到端图表解析建立了新的评估标准。

**Conclusion:** 该框架证明了合成先验在图表理解中的有效性，为零样本图表数据提取提供了新方向，同时解决了标注稀缺、结构提取和语义关联等核心技术挑战。
...
> 本文提出了多模态会议数据集(MCD)，这是第一个整合同一研究工作的论文、演讲视频、讲解视频和幻灯片的基准数据集，用于评估模型发现跨格式细粒度对应关系的能力，并分析了当前方法的优缺点。

<details>
  <summary>Details</summary>

**Motivation:** 科学知识传播日益多模态化，涉及文本、视觉和语音等多种形式，但这些材料缺乏结构化连接，难以追踪概念、视觉和解释之间的对应关系，限制了研究内容的统一探索和分析。

**Method:** 构建了Multimodal Conference Dataset (MCD)基准数据集，集成了同一研究工作的论文、演讲视频、讲解视频和幻灯片；评估了多种基于嵌入和视觉语言模型在发现细粒度跨格式对应关系方面的能力。

**Result:** 视觉语言模型鲁棒但难以进行细粒度对齐；基于嵌入的模型能很好地捕捉文本-视觉对应关系，但方程和符号内容在嵌入空间中形成独立簇。研究结果揭示了当前方法的优势和局限。

**Conclusion:** MCD为多模态科学理解提供了首个系统基准，研究结果指出了当前方法的关键改进方向，为未来研究奠定了基础。所有资源已开源以确保可复现性。
...
> VideoRouter是一个基于InternVL的查询自适应双路由框架，通过语义路由和图像路由实现智能的视频令牌压缩，解决了长视频多模态模型的可扩展性问题。

<details>
  <summary>Details</summary>

**Motivation:** 大型视频多模态模型面临可扩展性瓶颈：长视频产生过长的视觉令牌序列，导致推理过程中的内存和延迟急剧增加。现有压缩方法要么查询感知能力弱，要么跨帧应用固定压缩策略，在视觉证据时间分布不均时表现次优。

**Method:** VideoRouter采用双路由架构：语义路由器预测主导分配策略，在广泛时间覆盖和自适应高分辨率保存之间选择；图像路由器使用早期LLM层对帧相关性进行评分。该方法压缩不相关帧同时保留关键证据帧细节。研究团队还构建了Video-QTR-10K用于分配策略监督训练，Video-FLR-200K用于帧相关性监督训练。

**Result:** 在VideoMME、MLVU和LongVideoBench基准测试中，VideoRouter在相当或更低的预算下一致超越InternVL基线，实现了最多67.9%的令牌减少，同时保持了模型性能。

**Conclusion:** VideoRouter通过查询自适应的双路由机制有效解决了长视频多模态模型的可扩展性问题，实现了智能的视觉令牌分配，在保证性能的同时显著降低了计算开销，为处理长视频任务提供了高效解决方案。
...
> Align3D-AD是一个统一的零样本3D异常检测框架，通过跨模态特征对齐和模态感知提示学习，利用RGB模态指导3D异常检测，有效缩小了渲染特征与RGB语义空间的领域差距。

<details>
  <summary>Details</summary>

**Motivation:** 现有的零样本3D异常检测方法主要依赖将3D观测投影到多视图表示，但这些表示主要捕获几何线索而非真实的视觉语义，同时使用RGB数据预训练的视觉编码器处理投影表示，导致编码器与投影表示之间存在显著的领域差距。

**Method:** 提出两阶段框架：1）跨模态特征对齐范式将渲染特征映射到RGB语义空间，并通过语义一致性重新加权策略优化对齐；2）模态感知提示学习框架，为RGB对齐特征和渲染特征分配独立提示，通过双重提示对比对齐捕获跨模态互补语义。

**Result:** 在MVTec3D-AD、Eyecandies和Real3D-AD数据集上的广泛实验表明，Align3D-AD在一对多和跨数据集设置下均持续优于现有的零样本方法，展现了其泛化能力和鲁棒性。

**Conclusion:** Align3D-AD通过创新的跨模态对齐和提示学习机制，成功解决了零样本3D异常检测中的领域差距问题，为3D异常检测领域提供了新的解决思路，具有很好的实际应用前景。
...
> 本文提出了一种基于扩散模型的中文书法生成框架InkDiffuser，通过高频增强机制和可微分墨迹结构损失来提高生成质量和视觉真实感。
...
**Method:** 作者提出了两个核心贡献：1）高频增强机制，通过显式融合高频表示来提取内容，获得更准确的字体结构；2）可微分墨迹结构损失，将可微分形态学操作集成到扩散过程中，让模型学习显式的墨迹结构分解。

**Result:** 在各种书法风格和复杂字符上的广泛实验表明，InkDiffuser可以从单个参考字形生成具有逼真墨迹渲染效果的优质书法字体，在结构一致性、细节保真度和视觉真实性方面优于现有的少样本字体生成方法。

**Conclusion:** InkDiffuser有效解决了中文书法生成中的笔画渲染和墨迹形态问题，通过高频信息和可微分形态学约束实现了高质量的单样本书法合成，为数字书法创作提供了新的技术支持。
...
> 提出ContactPrompt，一种无需训练的零样本密集手部接触估计方法，利用多模态大语言模型实现细粒度接触预测，性能超越之前的监督学习方法。

<details>
  <summary>Details</summary>

**Motivation:** 密集手部接触估计需要高层语义理解和细粒度几何推理，但利用MLLM解决此问题面临编码3D手部几何和捕获顶点级接触信息两大挑战。

**Method:** ContactPrompt采用手部部件分割和部件级顶点网格表示编码3D几何信息，通过多阶段结构化接触推理和部件条件化，逐步连接全局语义与细粒度几何。

**Result:** 该方法无需任何训练，在密集手部接触估计任务上超越了之前在大规模数据集上训练的监督方法，实现了显著的性能提升。

**Conclusion:** 证明了MLLM在密集手部接触估计中的潜力，通过合理的几何表示和推理策略，可以在零样本条件下实现优于监督学习的效果。
...
> DBMSolver是一种无需训练的采样器，通过指数积分器利用扩散桥模型的半线性结构，大幅减少采样步数（最高减少5倍）并显著提升图像质量，在多个I2I任务上实现了新的SOTA效率-质量平衡。

<details>
  <summary>Details</summary>

**Motivation:** 当前最先进的扩散桥模型(DBMs)在图像到图像转换任务中虽然生成质量高，但采样速度过慢，通常需要数十次函数评估，这限制了其在实际应用中的可行性。

**Method:** 提出DBMSolver，一种无需训练的采样方法，利用DBM底层SDE和ODE的半线性结构特点，通过指数积分器技术实现一阶和二阶高效解决方案。

**Result:** 在DIODE数据集上，20步采样比二阶基线方法FID提升53%；在修复、风格化和语义到图像等多个任务中(最高256x256分辨率)，实现了新的SOTA效率-质量权衡。

**Conclusion:** DBMSolver成功解决了DBMs采样速度慢的问题，在保持高质量输出的同时大幅提升采样效率，使扩散桥模型在实际应用中更具可行性。
...
> 本文提出了一种基于混合专家(MoE)的多任务学习框架，通过联合训练多个自监督和伪标签任务，在不使用异常样本的情况下实现了医学图像异常检测，在BMAD基准测试中超越所有现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 医学图像异常检测面临的主要挑战是训练时通常无法获得异常样本。现有方法通常依赖单一预训练模型和单一代理任务，存在局限性。需要开发更有效的方法来学习正常解剖结构的鲁棒表示。

**Method:** 提出基于混合专家(MoE)的联合学习框架，从零开始学习多个自监督和伪标签代理任务。通过仔细整合多个代理任务，联合模型有效学习正常解剖结构的鲁棒表示，在推理时基于多任务学习器解决每个任务的表现来计算异常分数。

**Result:** 在包含多种医学图像模态的BMAD基准测试上，所提出的多任务学习器作为异常检测器表现出色，超越了所有最先进的竞争对手。同时，模型生成了可解释的异常图，有助于医生提供更准确的诊断。

**Conclusion:** 多任务学习结合混合专家架构是医学图像异常检测的有效方法，无需异常样本即可实现优秀性能，生成的可解释异常图具有临床应用潜力。
...
> MAST是一个基于脉冲神经网络(SNN)的AI生成视频检测器，通过分析多通道时间残差和语义特征，实现对未知生成器的高效检测。

<details>
  <summary>Details</summary>

**Motivation:** 现代AI生成视频在单帧层面已达到照片级真实感，传统检测器在跨生成器评估时表现不佳，需要寻找更有效的时间特征检测方法。

**Method:** 提出MAST框架，结合脉冲驱动的时序分支处理多通道时间残差，冻结语义编码器提取语义特征，利用SNN的事件驱动特性检测目标与运动边界的时间伪影。

**Result:** 在GenVideo基准测试中，MAST在10个未见生成器的严格跨生成器评估下达到93.14%的平均准确率，性能匹配或超过最强的ANN检测器。

**Conclusion:** 该方法验证了SNN在AI生成视频检测中的实际应用价值，为跨生成器检测提供了新的解决思路，体现了脉冲神经网络在处理稀疏、异步时间信号方面的独特优势。
...
> 阿拉伯文字手写文本识别中的跨语言联合训练改进主要来自序列级建模而非视觉特征共享。

<details>
  <summary>Details</summary>

**Motivation:** 在低资源条件下，阿拉伯文字手写文本识别(HTR)可以通过跨语言联合训练获得改进，但对于这些改进是由于共享视觉表示还是序列级依赖关系尚不明确。

**Method:** 通过对比CNN-only模型（使用CTC解码）和CRNN模型在相同训练制度下的表现，控制架构变量，在阿拉伯语、乌尔都语和波斯语数据集上进行低资源(K={100,500,1000})实验，重点关注转移改进(delta CER)而非绝对性能。

**Result:** CNN-only模型表现出有限或不稳定的改进，而CRNN模型在多脚本训练下获得更好性能，特别是在数据最受限的情况下；跨语言改进与序列级建模相关，而CNN编码器学习到的视觉特征共享似乎不足够。

**Conclusion:** 上下文建模在低资源场景中实现有效转移起重要作用，这种发现可能扩展到其他低资源语言设置。
...
> 本文提出了一种与架构无关的Lipschitz常数贝叶斯头部网络，将其集成到ViT中得到LipB-ViT模型。该模型通过谱正则化约束变分权重的均值和对数方差，有效处理语义相近的标签噪声，在15%语义错误标签下召回率达93%以上。

<details>
  <summary>Details</summary>

**Motivation:** 现有的鲁棒训练方法在面对语义相近的结构化标签噪声时效果不佳，而这类噪声在高风险应用中非常常见，亟需开发能够处理此类噪声的新方法。

**Method:** 1. 在变分权重的均值和对数方差上施加谱正则化约束；2. 提出联合捕获不确定性和置信度的新度量指标；3. 设计自适应算术平均融合方案，结合特征空间邻近性和预测不确定性来检测损坏标签；4. 构建分析管道联合评估数据集质量和标签噪声。

**Result:** 在15%语义错误标签下，标签检测召回率超过93%，比现有k近邻方法提升7%以上；模型在结构化和非结构化噪声场景下都表现出鲁棒性；提供了即插即用的预训练主干网络兼容性。

**Conclusion:** LipB-ViT为处理结构化标签噪声提供了有效解决方案，其稳定的不确定性估计使数据集质量量化成为可能，在高标注可靠性要求的应用中具有实用价值，尽管计算成本有所增加。
...
> 论文提出了一种即插即用的类别感知知识注入框架CAKI，通过生成类别特定提示并进行查询-关键提示匹配，有效补充现有提示学习方法中缺失的类别特定知识，提升了视觉语言模型的分类性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有提示学习方法要么关注类别共享的通用提示，要么生成实例特定提示，都忽略了类别特定的关键知识。这导致类别共享提示监督粒度过粗易误分类，而实例特定提示会丢失跨实例的丰富类别信息。

**Method:** CAKI框架包含两个核心组件：1) 类别特定提示生成：从同一类别的少样本中编码类别知识到提示中，并存储在类别级知识库；2) 查询-关键提示匹配：为测试实例提供即插即用机制，从知识库检索相关类别知识并注入以优化模型预测。

**Result:** 大量实验证明CAKI有效提升了现有方法在基类和新类上的性能，是通用有效的即插即用改进方案。

**Conclusion:** CAKI成功解决了现有提示学习方法忽视类别特定知识的问题，通过优雅的知识注入机制实现了性能提升，为视觉语言模型的提示学习提供了新的改进思路。
...
> DeScore提出了一种解耦的视频奖励模型，采用'先思考后评分'范式，结合思维链推理优势和判别式效率，通过双阶段优化提升视频质量评估的泛化能力和训练效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频奖励模型存在根本困境：判别式RM缺乏显式推理易受快捷学习影响且依赖大规模数据；生成式RM虽有推理能力但推理与评分耦合导致训练不稳定。

**Method:** DeScore采用解耦框架：MLLM生成显式思维链，独立判别式评分模块通过可学习查询token和回归头预测奖励。通过判别式冷启动和双目标强化学习两阶段优化训练。

**Result:** 该范式有效利用了思维链推理的泛化优势，同时避免了耦合训练的不稳定性，实现了训练高效且泛化能力强的视频奖励模型。

**Conclusion:** DeScore成功将推理与评分解耦，'先思考后评分'范式为视频奖励模型提供了新的解决方案，在保持解释性和泛化性的同时显著提升了训练效率和模型性能。
...
> 本文提出了一种针对目标检测后门攻击的防御框架，通过检测感知的对抗性微调来缓解后门威胁，无需知道攻击目标。

<details>
  <summary>Details</summary>

**Motivation:** 后门攻击对安全关键的视觉系统构成严重威胁，虽然图像分类的后门防御已有广泛研究，但目标检测领域的防御方法相对滞后。现有的分类防御方法难以直接应用于检测任务，因检测攻击模式更复杂。

**Method:** 提出检测感知对抗性微调框架，包括：1）软分支最小化方法，通过软门结合误分类和消失攻击目标；2）检测感知分类损失最大化；3）双目标微调损失，专注于目标匹配预测的修复。

**Result:** 在CNN和Transformer检测器上的实验证明，该方法相比分类基线方法能更有效降低攻击成功率，同时保持真实检测，并维持有竞争力的清洁检测性能。

**Conclusion:** 本研究成功解决了目标检测后门防御的关键挑战，为仅有受损检测器和小规模清洁数据集的防御方提供了有效解决方案，对提升视觉系统安全性具有重要意义。
...
> 本研究开发了一种基于LLM的系统，用于过滤放射学报告中的病理发现，从而从35万次CT检查中构建健康参考分布，为106个解剖结构建立了全面的全身参考图表。

<details>
  <summary>Details</summary>

**Motivation:** 定量CT生物标志物（如器官体积和组织衰减）的解读需要大规模健康参考分布，但临床数据集通常富含病理学数据，这给构建参考分布带来了巨大挑战。

**Method:** 开发证据驱动的交叉验证大型语言模型(LLM)集成系统，通过五个LLM首先标记基于逐字报告证据的结构级异常候选，然后通过交叉验证解决分歧，使用分布感知广义加性模型建立参考图表。

**Result:** 成功构建了涵盖106个解剖结构的全身参考图表，考虑了年龄、性别、对比增强和采集参数等因素，纵向分析揭示了结构和对比依赖性的变化特征。

**Conclusion:** 该资源支持从常规CT中获得协变量调整的百分位评分，促进标准化定量表型分析、多中心影像研究和可扩展的机会性筛查研究。
...
> 本研究提出了RAWild框架，通过物理学指导的全局-局部色调映射方法，实现了跨传感器RAW数据的通用目标检测，解决了不同设备传感器数据域差异大的挑战。

<details>
  <summary>Details</summary>

**Motivation:** 相机传感器RAW数据虽然有深度位深、保持物理信息和无ISP失真等优势，但不同设备的曝光条件、光谱敏感性和位深变化导致域差异比sRGB更大，使传感器无关的泛化成为基本挑战。

**Method:** 提出了物理学指导的全局-局部色调映射框架RAWild，将传感器引起的变异分解为全局色调校正和空间自适应的局部颜色调整，两者都由RAW分布先验驱动，并建立了基于物理的RAW模拟管道来合成真实的传感器输出。

**Result:** 在涵盖10-24位深的多个RAW基准测试上，在单数据集、混合数据集和挑战性鲁棒性设置下都达到了最先进的性能。

**Conclusion:** RAWild框架成功解决了跨传感器RAW目标检测的泛化问题，通过物理引导的色调调整和合成数据增强，实现了出色的跨传感器通用性能。
...
> MobileEgo Anywhere是一个利用普通手机收集大规模长时间自我中心数据集的框架，包含200小时多样化数据，并开源了移动应用和处理管道。

<details>
  <summary>Details</summary>

**Motivation:** 现有自我中心数据集存在时长过短（仅几分钟）的问题，无法捕获复杂机器人任务执行所需的长期时间依赖关系，而传统机器人数据收集硬件门槛高。

**Method:** 利用现代智能手机的传感器套件提供高保真度的长期相机姿态跟踪，通过开源移动应用让任何用户都能记录数据，并提供完整的处理管道将原始数据转换为标准化训练格式。

**Result:** 成功构建了一个包含200小时多样化长时间自我中心轨迹的数据集，为Vision Language Action模型和基础模型研究提供了训练就绪的数据。

**Conclusion:** 该工作通过民主化数据收集过程，实现了跨全球不同环境的长期数据大规模获取，加速了可泛化机器人策略的发展。
...
> 研究提出了一个针对医学图像的免提示参数高效微调框架，通过双适配器策略解决SAM2在生物医学分割中的适应性问题，显著提升了分割精度同时大幅降低了计算成本。

<details>
  <summary>Details</summary>

**Motivation:** SAM2在自然图像上表现优异，但在生物医学分割领域面临显著领域差异和提示依赖问题，难以直接应用于医学图像处理。

**Method:** 提出卷积位置编码生成器适应任意纵横比输入，设计双适配器策略：高性能适配器使用可变形卷积进行边界精细建模，轻量级适配器通过结构重参数化减少推理延迟。

**Result:** 在多个医学数据集上实验表明，相比原始SAM2分割精度提升高达19.66%，计算成本比重型医学SAM适配方案降低约87%，实现了精度与效率的优异平衡。

**Conclusion:** 该方法有效解决了SAM2在医学分割领域的应用难题，为医学图像分割提供了一个高效且准确的解决方案，在精度和效率方面均优于现有适配方法。
...
> iPhoneBlur是一个针对移动设备运动模糊修复的难度分层基准数据集，包含7400个样本，揭示了模型在不同难度下的性能差距，聚合指标会隐藏这些重要差异。

<details>
  <summary>Details</summary>

**Motivation:** 现有运动模糊修复研究使用聚合评估指标，无法反映模型在不同模糊难度下的真实性能表现，掩盖了实际部署条件下的行为差异。

**Method:** 从iPhone 17 Pro高帧率视频合成7400组图像对，通过PSNR引导的自适应时间窗划分为Easy/Medium/Hard三个难度等级，通过光谱分析验证模糊真实性，提供完整的ISP感知元数据。

**Result:** 评估发现从Easy到Hard子集性能下降7-9dB，专业与消费相机存在域差距但可通过微调恢复，聚合报告完全隐藏了这一重要性能差距。

**Conclusion:** iPhoneBlur通过难度分层与部署关键元数据，为资源受限边缘系统的模型可靠性和失效模式提供了系统性评估框架，填补了实际部署评估的空白。
...
> 4DThinker是一个让视觉语言模型通过动态潜在心理图像进行4D推理的新框架，通过无标注数据生成和联合微调实现动态空间推理。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法要么将空间时间推理完全文本化（过于冗长且不精确），要么依赖外部几何模块（增加推理复杂度且无法培养模型内在能力），无法有效处理复杂的动态空间推理任务。

**Method:** 提出了三层方法：1) 无标注数据生成管道从原始视频合成4D推理数据；2) 动态图像微调(DIFT)联合监督文本token和4D潜在表示；3) 4D强化学习(4DRL)通过结果奖励处理复杂推理，将策略梯度限制在文本token确保稳定优化。

**Result:** 在多个动态空间推理基准测试中，4DThinker一致优于强基线模型，为VLM的4D推理提供了新视角。

**Conclusion:** 通过在连续隐空间内部模拟场景演化的动态心理图像，4DThinker成功实现了VLM的4D推理能力，为视觉智能与物理世界的连接开辟了新途径。
...
> 该研究提出了一种端到端的神经形态架构，用于实时识别美国手语字母，在神经形态硬件上实现了0.565mW的低功耗和3ms的低延迟性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的手语识别方法在分类准确率上取得了显著进展，但延迟和功耗要求限制了其实时部署的适用性。神经形态感知和处理为低延迟、节能的感知提供了基于稀疏事件驱动计算的替代范式。

**Method:** 该研究整合了脉冲视觉注意机制用于在线感兴趣区域提取，并将紧凑的脉冲神经网络部署在SpiNNaker神经形态平台上，构建端到端神经形态架构进行美国手语字母识别。

**Result:** 系统在模拟中达到92.27%的竞争性能，在神经形态硬件部署上达到83.1%的可比较性能，同时在所有基准方法中实现了最节能的架构(0.565 mW)和低延迟(3 ms)。

**Conclusion:** 尽管设计紧凑，该系统证明了任务依赖的视觉注意应用在边缘部署中的适用性，为实时手语识别提供了高效的解决方案。
...
> FusionProxy是一个实时红外与RGB图像融合模块，具有扩散级质量和完全的即插即用特性，可在多样化的硬件平台上实现实时推理，显著提升全天候感知系统的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 纯RGB视觉模型在夜间、雾霾等挑战性场景中表现不可靠，而现有的高保真红外融合方法由于延迟过高无法满足实时边缘部署的需求，因此需要开发一种既保持高质量融合又具备实时性能的解决方案。

**Method:** FusionProxy利用教师样本集的两个互补统计量：原始图像空间中的像素方差用于加权像素级监督，以及在冻结的主干网络内的像素方差用于在空间上路由特征级对齐。训练完成后可作为完全独立的组件直接集成到任意视觉系统中。

**Result:** 该方法在静态识别任务上实现了卓越性能，在包括闭环自动驾驶在内的动态任务中显著增强了鲁棒性，并能在从高端GPU到普通硬件的多样化平台上实现实时推理速度。

**Conclusion:** FusionProxy为全天候感知提供了一个灵活且可泛化的解决方案，成功平衡了融合质量与实时性能的要求，可作为即插即用组件广泛应用于各类视觉感知系统。
...
> 本文提出了PFCVR模型，通过部件级别的细粒度跨模态对齐和双向掩码重建机制，实现了文本到图像的车辆重识别，在该任务上显著超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 传统的车辆重识别仅支持图像查询，无法处理现实场景中只有目击者文本描述的情况。将车辆重识别扩展到文本查询具有重要的实际应用价值，但这是一个具有挑战性的跨模态检索任务，因为文本和图像存在语义鸿沟。

**Method:** PFCVR模型采用部件级别的细粒度跨模态对齐策略：1)在部件级别构建局部图像-文本对齐；2)引入可学习的部件查询令牌，聚合部件特定和全局句子上下文后与视觉部件特征对齐；3)添加双向掩码重建模块，让每种模态在另一种模态的指导下重建掩码内容，隐式地将局部对应关系桥接到全局特征对齐。

**Result:** 在T2I-VeRI数据集上，PFCVR达到29.2%的Rank-1准确率，比最佳竞争方法提升3.7个百分点；在新构建的T2I-VeRW基准数据集上达到55.2%的Rank-1准确率，全面超越了最新的先进方法。同时构建了包含14,668张图像、1,796个车辆身份、具有细粒度部件标注的大规模数据集T2I-VeRW。

**Conclusion:** 通过部件级别的显式对齐和全局级别的隐式对齐相结合，PFCVR有效解决了文本到图像车辆重识别中的跨模态语义对齐问题，在性能上取得了显著提升，为实际应用提供了可行的解决方案。新构建的数据集也为该领域的研究提供了重要基准。
...
> PlotPick是一款基于视觉语言模型的开源工具，能批量从科学图表中提取表格数据，在多个基准测试中显著优于专业的图表转表格模型DePlot。

<details>
  <summary>Details</summary>

**Motivation:** 系统性综述和荟萃分析经常需要作者以图表形式报告的数值数据，但手动数字化过程缓慢且难以规模化，需要自动化解决方案。

**Method:** 开发了PlotPick工具，利用视觉语言模型(VLMs)批量从科学图表中提取结构化表格数据，并在ChartX和PlotQA两个基准数据集上评估了来自三个提供商的六个VLM模型。

**Result:** 所有六个VLM模型在两个基准测试中均优于DePlot。在ChartX数据集上，VLMs召回率达88-96%，而DePlot仅71%；在PlotQA数据集上，VLMs的RMSF1为86-99%，在训练数据中未包含的箱线图类型上优势尤为明显，VLMs达83-97%，而DePlot仅24%。

**Conclusion:** 基于视觉语言模型的PlotPick工具在从科学图表提取数据方面具有显著优势，特别是在处理训练数据中未包含的图表类型时表现卓越，为系统性综述等研究工作提供了高效的自动化数据提取方案。
...
> PARSE框架通过将视觉识别分解为视觉基元及其关系组合，在组合域泛化基准测试中显著提升了分类精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有域泛化方法主要通过改进训练过程（如模型选择、数据增强或特征对齐）来获得稳定表示，但忽略了结构组合的显式表示学习，限制了在组合域泛化任务上的性能。

**Method:** 提出了PARSE框架，包含三个组件：(1)CNN骨干网络提取视觉特征；(2)概念瓶颈层将特征映射到具可微分空间坐标的基元热图；(3)结构评分层评估候选基元间的空间关系。使用软二元、三元和四元谓词表示空间组合，可实现端到端学习。

**Result:** 在CUB-DG和DomainBed基准测试上，PARSE在CUB-DG上提升准确率超过4.5个百分点，在DomainBed上与现有域泛化方法保持竞争力。

**Conclusion:** 显式建模视觉基元及其关系结构能够有效提升组合域泛化性能，为域泛化研究提供了新的思路。
...
> 提出DPOFusion框架，通过结合属性对齐和偏好可控的潜在扩散模型，实现了任务引导、偏好自适应的红外与可见光图像融合。

<details>
  <summary>Details</summary>

**Motivation:** 现有的红外与可见光图像融合方法难以灵活适应不同需求，实现能够同时满足人类视觉和机器视觉各种偏好的自适应融合仍是一个开放且具有挑战性的问题。

**Method:** DPOFusion框架包含两个核心组件：1) PALDM利用潜在融合先验和联合条件损失生成具有多种属性的候选融合结果；2) PCLDM通过实例直接偏好优化进行微调，能够直接控制融合结果以适应异质偏好信号。

**Result:** 实验结果表明该框架能够精确实现人类、视觉语言模型和任务驱动网络之间的偏好对齐，并在自适应融合质量和任务导向可迁移性方面设立了新的基准。

**Conclusion:** 该研究成功解决了多模态图像融合中偏好适应性控制的难题，为面向不同应用场景的自适应融合技术提供了创新解决方案。
...
> RealCam 是一个用于实时相机控制的视频到视频生成框架，通过跨帧上下文学习和知识蒸馏技术，实现了高保真、低延迟的交互式动态视角合成。

<details>
  <summary>Details</summary>

**Motivation:** 现有的隐式合成方法依赖于非因果的全序列处理和刚性前缀时间连接，导致计算延迟严重、复杂度呈二次方增长，且与实时流媒体或可变长度输入不兼容。

**Method:** 首先设计基于跨帧上下文学习的高保真教师模型，通过交错源帧和目标帧实现长度不可知的泛化；然后通过自迫分布匹配蒸馏将教师模型蒸馏为几步因果学生模型；最后提出循环闭合数据增强来缓解闭环轨迹中的不一致问题。

**Result:** RealCam 在保持最先进的视觉保真度和时间一致性的同时，实现了真正交互式的相机控制，推理速度比现有方法快几个数量级。

**Conclusion:** RealCam 突破了传统方法的架构限制，为实时交互式电影制作和直播广播等应用场景提供了可行的解决方案。
...
> PersonaGesture是一个基于扩散模型的单参考音频驱动的手势个性化系统，能够在无需针对新说话者优化的情况下，通过单个手势片段为未见过的说话者生成个性化的语音同步手势。

<details>
  <summary>Details</summary>

**Motivation:** 现有的语音驱动手势生成方法难以在保持语音同步性的同时，为新说话者生成具有个人风格的手势。这是因为参考手势片段混合了稳定的说话者习惯和特定语音相关的轨迹，需要一种能将时间身份特征与残差统计修正分离的方法。

**Method:** PersonaGesture包含两个核心组件：自适应风格注入(ASI)和隐式分布校正(IDR)。ASI通过Style Perceiver编码参考手势为说话者记忆标记，并使用零初始化残差交叉注意力将其注入去噪过程；IDR在潜在空间应用长度感知的对角仿射映射来修正从参考中估计的残差通道矩。

**Result:** 在BEAT2和ZeroEGGS数据集上的实验表明，PersonaGesture在定量指标、参考身份控制、相同音频诊断、定性比较和人类偏好评估上均优于对比方法，证明了其改进未见说话者个性化的有效性。

**Conclusion:** 将去噪时间的说话者记忆与保守的后生成矩校正相分离的设计，能够有效提升对未见说话者的个性化效果，为虚拟代理和数字人应用提供了实用的解决方案。
...
> ArenaPO是一种结合传统RLHF丰富奖励和DPO效率优势的新方法，通过利用Arena分数作为离线奖励实现细粒度优化，无需额外奖励模型即可提升文本到图像扩散模型的偏好对齐效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的直接偏好优化(DPO)方法依赖二元反馈，只能在选对被拒绝的配对上进行细粒度建模，导致优化效果次优。传统RLHF虽然提供更丰富的奖励，但计算效率较低。

**Method:** 首先构建模型Arena，将每个模型能力表示为高斯分布，通过遍历标注的成对偏好来推断这些能力。将每张输出图像视为相应能力分布的样本，然后在给定两个能力分布和观察到的成对偏好条件下，基于截断正态分布的潜变量推断来估计绝对质量差距，作为训练过程中的细粒度反馈。

**Result:** 在Pick-a-Pic v2和HPD v3数据集上进行实验，ArenaPO持续优于现有基线方法，证明了其有效性和优越性。

**Conclusion:** ArenaPO成功结合了传统RLHF的丰富奖励和DPO的计算效率优势，通过精细的奖励建模和离线计算机制，为文本到图像扩散模型的偏好对齐提供了更有效的解决方案。
...
> MSD-Score是一种无需参考文本的图像描述评估指标，通过将图像区域和文本标记建模为超球面上的von Mises-Fisher混合分布，在多尺度框架下评估语义一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的无参考评估方法主要依赖全局嵌入相似度，无法有效检测细粒度的语义错误，如幻觉物体、缺失属性或错误关系。

**Method:** 将图像块和文本标记嵌入视为超球面上的von Mises-Fisher混合分布；设计多尺度分布评分框架；通过加权双向KL散度量化语义差异；结合全局相似度进行综合评估。

**Result:** 在大量实验中，MSD-Score在无参考指标中实现了与人工判断的最先进相关性；能够提供透明且可分解的局部语义错误诊断。

**Conclusion:** MSD-Score不仅提高了无参考评估的准确性，其概率化公式还为语义接地错误提供了确定性诊断信号，是对整体相似度指标和基于人工评估器的重要补充。
...
> Holmes是一个分层证据学习框架，通过聚合多粒度跨模态证据来显式量化和建模不确定性，解决了部分相关视频检索中的语义模糊和时序监督稀疏问题。

<details>
  <summary>Details</summary>

**Motivation:** 部分相关视频检索中，简短查询丰富视频内容的不对称性引入了不确定性，模糊查询会导致语义模糊，加上稀疏的时序监督无法提供充分的匹配证据。

**Method:** 提出分层证据学习框架：在视频间层面，将相似性分数解释为证据支持并通过狄利克雷分布建模；在视频内层面，采用灵活的最优传输实现软查询-片段对齐，累积更密集的证据。基于三重原则进行细粒度查询识别，指导查询自适应校准学习。

**Result:** 大量实验证明Holmes在多个基准数据集上显著超越了现有最先进方法，代码已开源。

**Conclusion:** 该框架通过显式建模不确定性并结合多粒度证据聚合，有效解决了部分相关视频检索中的核心挑战，为跨模态检索领域提供了新的思路。
...
> AMIEOD是一个用于低光照场景的图像增强目标检测框架，通过多专家图像增强模块和检测引导的选择机制，显著提升了暗光环境下的目标检测精度。

<details>
  <summary>Details</summary>

**Motivation:** 低光照条件下拍摄的图像在视觉感知任务中精度较低，传统的图像增强和目标检测分离处理方式无法充分利用两者间的协同效应。

**Method:** 提出多专家图像增强模块(MEIEM)利用多样化增强策略；设计检测引导回归损失(DGRL)使增强策略与检测任务对齐；构建专家选择模块(ESM)配合检测引导交叉熵损失(DGCE)实现推理时动态选择最优增强策略。

**Result:** 在多个数据集上的广泛实验表明，所提方法显著提升了低光照条件下的目标检测精度，且与当前检测算法兼容性良好。

**Conclusion:** AMIEOD框架通过将图像增强与目标检测联合优化，实现了面向检测性能的图像增强，有效解决了低光照场景下目标检测精度低的问题。
...
> LARGO是一种超网络方法，通过权重量空间建模，使用CP张量分解将多个缺失模态模型压缩为单一网络，在医学图像分割任务中显著优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态图像分析方法在处理缺失模态时通常需要复杂的架构，难以在不同数据集间迁移，且大多在特征空间操作，存在局限性。

**Method:** 提出LARGO超网络框架，在权重量空间中操作，采用张量分解技术将2^N-1个专用缺失模态模型压缩为单一网络，实现了对各种缺失场景的统一处理。

**Result:** 在BraTS 2018和ISLES 2022数据集上的52种配置中，LARGO在47种配置排名第一，平均Dice分数分别提升0.68%和2.53%，显著优于现有基线方法。

**Conclusion:** LARGO通过创新的权重量空间建模和张量分解技术，有效解决了多模态缺失问题，具有强大的通用性和迁移性，在医学和非医学领域都具有应用潜力。
...
> OpenGaFF是一个基于3D高斯溅射的开放词汇3D场景理解框架，通过高斯特征场和结构化码本实现了几何与语义的强耦合，提升了空间一致性和开放词汇推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于高斯表示的开放词汇3D场景理解方法面临语义预测碎片化和空间不一致的挑战，特别是在多视角观测中，这限制了3D场景理解的质量。

**Method:** 提出高斯特征场将语义建模为高斯几何和外观的连续函数，引入结构化码本作为共享语义基元，并设计码本引导的注意力机制进行相似性匹配，实现开放词汇推理。

**Result:** 在2D和3D开放词汇基准测试中，该方法一致优于先前方法，实现了更好的分割质量、更强的3D语义一致性，并提供了可解释的语义码本。

**Conclusion:** OpenGaFF通过强化几何-语义耦合和对象级语义一致性，为开放词汇3D场景理解提供了有效的解决方案，其结构化码本还提供了对学习表示的洞察。
...
> 本文提出了一种名为Tracker的自监督跟踪框架，通过双模态上下文关联机制，结合细粒度语义提示和上下文噪声，从未标记视频中学习鲁棒的跟踪表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自监督跟踪器缺乏有效的上下文建模，而非语义查询的上下文关联方法难以适应无标记跟踪场景，无法学习可靠的上下文线索。

**Method:** 提出了双模态上下文关联机制：第一阶段使用实例块标记（提示）辅助前向和后向跟踪分支获取跟踪知识；第二阶段逐步注入上下文噪声扰动特征，促使跟踪器在更复杂的特征空间中学习鲁棒表示。该机制仅用于训练以保持高效推理。

**Result:** 大量实验证明该方法的优越性，能够从未标记视频中学习高质量的跟踪表示。

**Conclusion:** 通过创新的上下文关联机制，实现了自监督跟踪的有效学习，为该领域提供了新的技术路径。
...
> VISD是一种结构化自蒸馏框架，通过引入诊断性特权信息，结合方向幅度解耦机制，有效提升了视频大语言模型训练效率和推理质量。

<details>
  <summary>Details</summary>

**Motivation:** 训练VideoLLMs进行复杂推理面临两大挑战：序列级奖励稀疏导致难以在长时序推理轨迹中进行精细的信用分配；现有的RLVR方法无法捕捉token级别的贡献，而自蒸馏方法虽提供密集监督但缺乏结构性诊断特异性。

**Method:** VISD框架包含三个核心技术：(1)使用视频感知评判模型将推理质量分解为答案正确性、逻辑一致性和时空接地性等多个维度；(2)提出方向幅度解耦机制，将奖励计算的优势确定更新方向，结构化特权信号调节token级别更新幅度；(3)结合课程调度和EMA教师稳定化实现鲁棒优化。

**Result:** 在多个基准测试中，VISD显著超越了强基线模型，提升了答案准确性和时空接地质量，同时实现了近2倍的优化步骤收敛速度提升。

**Conclusion:** 结构化自监督方法能够同时改善VideoLLMs的性能和样本效率，方向幅度解耦机制有效稳定了密集监督与强化学习的整合，为视频推理任务提供了新的优化范式。
...
> 研究发现现代预训练视觉变换器存在严重的对象内信息泄漏问题，每个部件编码了整个对象的信息，违反了局部性假设，这损害了基于部件推理的模型可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 基于部件的推理是计算机视觉中的经典策略，通过让模型关注相关对象部件来提升任务性能和可解释性。然而，这种方法依赖于一个基本假设：对象部件的潜在表示主要编码相应图像区域的信息。

**Method:** 通过使用基于部件的属性标注测量视觉模型中的对象内信息泄漏，进行全面的实验评估。同时设计了一个两阶段方法作为上界，通过设计防止信息泄漏。

**Result:** 实验表明现代预训练视觉变换器违反了局部性假设，存在强烈的对象内信息泄漏，每个部件编码了整个对象的信息。两阶段方法能防止泄漏并改善属性驱动的部件发现。

**Conclusion:** 研究结果揭示了影响基于部件表示可解释性的被忽视问题，特别强调了依赖于以部件为中心概念的CBMs等模型。两阶段方法为缓解这个问题提供了有希望的解决方案。
...
> 本文提出了一种基于稀疏感知的事件相机目标跟踪框架，通过多时间尺度的特征学习和专家混合模块，实现了在不同运动动态下的高效准确跟踪。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于RGB的跟踪器在低光照和快速运动等挑战性条件下表现不佳，而现有的事件相机跟踪器忽略了事件数据的内在空间稀疏性和时间密度特性，且采用单一固定时间窗采样策略，在变化的运动动态下表现次优。

**Method:** 提出了一个三阶段Vision Transformer骨干网络，逐步注入稀疏、中等密度和稠密事件搜索区域，实现层次化多密度特征学习；引入稀疏感知的专家混合模块(MoE)来鼓励不同稀疏模式下的专家专门化；设计动态推理策略根据跟踪难度自适应调整推理深度。

**Result:** 在FE240hz、COESOT和EventVOT数据集上的大量实验表明，该方法在跟踪准确性和计算效率之间取得了良好的平衡。

**Conclusion:** 通过显式建模事件数据的稀疏性变化和多时间尺度特性，结合专家混合和动态推理机制，为事件相机目标跟踪提供了一个有效的解决方案。
...
> 本文提出了Pest-Thinker，一个基于知识驱动强化学习的多模态大语言模型框架，通过构建病虫害基准数据集和引入结构化推理机制，显著提升了模型对农业病虫害的细粒态识别能力。

<details>
  <summary>Details</summary>

**Motivation:** 农业病虫害严重威胁全球粮食安全，现有MLLM模型直接应用于病虫害识别面临挑战：物种间复杂度高、物种内变异性大、专家标注数据稀缺。这些限制阻碍了智能农业中的病虫害有效识别。

**Method:** 1) 构建QFSD和AgriInsect两个高分辨率病虫害基准数据集，包含多样物种和专家标注的形态学特征；2) 利用数据集合成思维链推理轨迹，通过监督微调实现病虫害特定视觉线索的结构化学习；3) 采用分组相对策略优化(GRPO)和新型特征奖励机制，结合LLM评判策略引导模型关注可观测的形态学证据。

**Result:** 实验表明Pest-Thinker显著提升了模型在领域内和领域外的形态学理解能力，向专家级别的视觉推理迈出了重要一步。

**Conclusion:** 该研究为智能农业病虫害分析提供了专家级视觉推理的新范式，通过知识驱动和强化学习相结合的方法有效解决了MLLM在病虫害识别领域的关键挑战。
...
> 本文提出了一种名为连续专家组装(CEA)的全局图像恢复框架，通过交叉注意力超适配器为每个空间标记动态组装残差更新，有效处理未知、空间非均匀和复合的真实世界图像退化。

<details>
  <summary>Details</summary>

**Motivation:** 真实世界图像退化具有未知性、空间非均匀性和复合性，现有方法的全局条件调制会限制局部退化证据的表达，而静态专家路由可能产生同质化更新或依赖不稳定的稀疏分配。

**Method:** CEA采用轻量级交叉注意力超适配器探测中间空间特征，合成实例条件化的低秩路由基和残差方向，每个空间标记通过密集符号点积亲和力组装自己的残差更新。

**Result:** 在AIO-3、AIO-5和CDD-11数据集上，CEA相比基于提示、描述符和专家的基线方法提升了平均恢复质量，在空间变化和复合退化上效果最显著，同时保持了良好的参数、FLOP和运行时效率。

**Conclusion:** CEA通过连续专家组装机制避免了外部提示、静态专家库和离散Top-K选择，为全息图像恢复提供了一种高效的动态参数化解决方案，在处理复杂真实世界退化方面具有优势。
...
> Prologue通过在视觉token序列前添加专门用于生成的前序token，解决了自回归图像生成中的重建-生成冲突问题，在保持重建质量的同时显著提升了生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自回归图像生成方法中，视觉token需要同时满足重建和生成两个任务，这种耦合设计导致生成质量受限。作者希望通过解耦重建和生成任务，让每个任务都能独立优化，从而突破这一瓶颈。

**Method:** Prologue方法引入少量的前序token，这些token专门用于生成任务且仅通过AR交叉熵损失进行训练，而视觉token保持专注重建任务。这种解耦设计从ELBO角度进行了理论论证，确保生成质量优化不会影响重建质量。

**Result:** 在ImageNet 256x256上，Prologue-Base将gFID从21.01降至10.75（无需classifier-free guidance），重建质量几乎不变；Prologue-Large达到rFID 0.99和gFID 1.46。前序token还展现出新兴的语义结构，其线性探查Top-1准确率达35.88%，远超标准tokenizer前16个token的23.71%。

**Conclusion:** 研究表明，通过引入独立的生成表示而保持原始表示不变，可以有效提升生成质量。这为自回归图像生成提供了新的研究方向，即在不修改原有重建表示的前提下，通过额外的学习表示来优化生成任务。
...
> 本研究构建了一个包含16种可解释AI方法的AI生成图像检测框架，通过大规模数据集训练和100人参与的评估，实现了更人性化的检测系统。

<details>
  <summary>Details</summary>

**Motivation:** 生成式AI在网络虚假信息传播中的滥用，迫切需要透明且可解释的检测系统来帮助人类理解和识别AI生成的虚假图像。

**Method:** 开发多种架构的检测器，在自建的大规模AI文本生成图像数据集AIText2Image上训练，整合16种不同的可解释AI方法，通过100人参与的调研评估文本和视觉解释效果。

**Result:** 构建了一个全面的检测框架，能够提供人类可理解的预测解释，并从人类视角测量了XAI输出与人类偏好的对齐程度。

**Conclusion:** 该研究为虚假图像检测中的视觉-语言线索提供了深刻见解，建立了衡量XAI方法清晰度的评估标准，推动了更易理解的可解释检测系统发展。
...
> 本文提出了wAR-Tok方法，通过在分词器训练中添加Wasserstein梯度流的先验匹配信号，解决了传统两阶段训练中分词器与自回归模型不匹配的问题，提高了生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 传统的离散图像分词器采用两阶段训练（重构+先验训练），导致分词器对即将生成其token的自回归模型不感知，使得学习到的token虽然能很好保持图像信息，但难以被自回归模型从左到右预测。

**Method:** 基于三分变分一致性(TVC)分析框架，在分词器训练期间添加分布级别的先验匹配信号，使用Wasserstein梯度流优化。对于硬分类token，该方法简化为辅助AR模型与目标AR先验的token级对比，仅需前向传播。

**Result:** 在CIFAR-10和ImageNet数据集上，wAR-Tok在保持相当重构质量的同时，降低了自回归损失，并提高了生成的FID指标。

**Conclusion:** 通过在分词器训练阶段就考虑后续自回归生成模型的预测能力，wAR-Tok有效解决了训练-生成不匹配问题，为离散图像表示学习提供了新的思路。
...
> 本研究提供了一个医学图像持续分割的综合基准研究，定义了三种临床场景和全面的评估框架，发现现有方法难以同时满足所有要求，但基于回放的方法在稳定性和可塑性之间达到最佳平衡。

<details>
  <summary>Details</summary>

**Motivation:** 持续学习在临床医学图像分割中至关重要，但面临场景标准化不足、研究过度关注遗忘而忽略可塑性、以及缺乏全面基准评估三大问题。

**Method:** 定义了Domain-CL、Class-CL和Organ-CL三种临床场景，建立了包含性能、遗忘、可塑性、前向泛化性、参数效率和回放负担的评估框架，并对代表性持续学习方法进行广泛实验。

**Result:** 实验显示现有方法难以同时满足所有要求，回放方法在稳定性和可塑性间平衡最佳，参数隔离方法能减少遗忘但增加模型大小，前向泛化性仍是研究不足的重要方面。

**Conclusion:** 该研究为医学图像持续分割提供了标准化基准和评估体系，揭示了当前方法的优缺点，为未来研究方向提供了指导，特别强调需要更多关注前向泛化性的研究。
...
> 提出动态文本到图像评估框架DynT2I-Eval，通过自动生成新 prompt 来避免传统静态基准的过拟合问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本到图像基准使用固定的 prompt 集合，一旦公开发布并反复使用，容易导致模型过拟合和基准污染，影响评估的客观性和准确性。

**Method:** 构建结构化视觉语义空间，将 prompt 分解为可控制的维度（主体、逻辑约束、环境、构图等），通过任务特定空间和难度感知采样持续生成新 prompt。采用成对比较方式统一异质输出，利用动态调度器、微批次聚合和加权贝叶斯更新维护稳定的在线排行榜。

**Result:** 实验表明持续刷新的 prompt 提供了稳健的评估协议，减少了针对特定 prompt 集合调优的影响。模拟和消融实验证实该排名框架在冷启动收敛、后期发现和长期排名保真度之间实现了良好平衡。

**Conclusion:** DynT2I-Eval 为 T2I 模型提供了一个不受固定 prompt 限制的动态评估解决方案，能够有效防止基准污染，保持评估的客观性和可靠性。
...
> Retina-RAG是一个低成本的模块化框架，结合高性能视网膜分类器和视觉语言模型，通过检索增强生成技术实现糖尿病视网膜病变分级、黄斑水肿检测和结构化报告生成，在消费者级GPU上达到临床级别的性能。

<details>
  <summary>Details</summary>

**Motivation:** 糖尿病视网膜病变是工作年龄成年人可预防性失明的主要原因，但现有自动化筛查系统大多局限于图像级分类，缺乏临床结构化报告功能。

**Method:** 提出Retina-RAG框架，解耦高性能视网膜分类器和通过LoRA适应的视觉语言模型(Qwen2.5-VL-7B-Instruct)，采用检索增强生成(RAG)模块注入眼科知识并整合分类器输出，提高诊断一致性并减少幻觉。

**Result:** 在DR分级上达到F1-score 0.731，ME检测达到0.948，报告生成ROUGE-L 0.429、SBERT相似度0.884，显著超越零样本Qwen和MMed-RAG等基线方法。

**Conclusion:** 研究表明通过模块化设计和检索增强生成技术，可以在消费者级GPU上实现临床级别的视网膜AI系统，为糖尿病视网膜病变的诊断提供结构化且准确的解决方案。
...
> SuperFace是一个基于人类偏好的ARKit面部表情估计框架，通过人类对渲染表情的反馈优化，而非依赖不可靠的软件伪标签，实现了更高的表情保真度。

<details>
  <summary>Details</summary>

**Motivation:** 现有ARKit混合形状系数预测方法缺乏可靠的真实标签监督，依赖的捕获软件产生的伪标签存在噪声激活、偏差系数幅值和不准确面部动作等问题，导致模型只是在逼近不完美的伪标签而非优化感知表情保真度。

**Method:** 提出SuperFace框架，不将软件估算的系数视为固定真实标签，而是仅作为初始化，通过人类对渲染面部表情的偏好反馈来进一步改进系数预测，实现从伪标签模仿向人类感知优化的转变。

**Result:** 实验表明SuperFace比Live Link Face监督获得了更好的表情保真度，证明了基于偏好的优化方法在语义面部动作预测中的有效性。

**Conclusion:** 将模型与人类感知判断而非数值伪标签对齐，能够产生视觉上更真实和富有表现力的面部动画，为语义面部动作预测提供了新的优化范式。
...
> EA-WM提出了一种事件感知的生成式世界模型，通过将动作投影为结构化的运动到视觉动作场，并结合双向融合机制，显著提升了机器人空间几何和交互动态的生成精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有的世界-动作模型主要将视频生成视为策略学习的辅助表示，未能充分利用动作信号来指导视频合成，导致生成的rollout在保持精确的机器人空间几何和细粒度交互动态方面表现不佳。

**Method:** EA-WM提出将动作和运动状态直接投影到目标相机视角，形成结构化的运动到视觉动作场；引入事件感知的双向融合模块来调制跨分支注意力，捕获物体状态变化和交互动态，有效实现运动控制与视觉感知的闭环。

**Result:** 在综合WorldArena基准测试上，EA-WM达到最先进性能，显著优于现有基线方法。

**Conclusion:** 通过几何化表示和事件感知融合，EA-WM成功弥合了运动控制与视觉感知之间的差距，为机器人世界模型提供了更准确的视频生成能力。
...
> 本文提出了一种多模态可解释性框架，结合CNN分类分割、视觉显著性图生成和LLM诊断报告生成，提高脑肿瘤AI诊断的透明度和临床可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 深度学习模型在医学影像中的不透明性是其临床应用的主要障碍，需要开发能够提供临床可操作见解的可解释AI系统。

**Method:** 框架包含三个耦合阶段：1）九种CNN架构的混合优化，同时进行分类和分割；2）使用Grad-CAM等方法生成掩码；3）将解剖结构映射到哈佛-牛津图谱，用LLM生成结构化诊断报告。

**Result:** 在4,834个脑MRI数据上，InceptionResNetV2分类性能最佳，Grad-CAM++分割重叠度最高；Grok3报告词汇多样性最强，LLaMA可读性最好。

**Conclusion:** 该框架成功整合视觉、解剖和语言模态，产生技术可靠且临床可解释的诊断解释，推动了AI辅助脑肿瘤诊断的透明度和临床问责性。
...
> 本文发现了视觉标记化中的'熵悬崖'现象，提出了可变码本大小的量化方法VCQ，显著提升了图像生成的质量和语义层次性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的离散视觉标记化采用固定码本设计存在信息论瓶颈。研究发现，在图像序列中，条件熵会急剧下降，导致大部分位置变成记忆问题，这种现象限制了重建性能的提升。

**Method:** 提出可变码本大小量化(VCQ)方法，让码本大小沿序列从最小值K_min=2单调增长到最大值K_max，保持损失函数、参数数量和自回归训练过程不变。

**Result:** VCQ方法将ImageNet 256×256的gFID从27.98降至14.80，扩展后达到1.71 gFID。前10个token的线性探针准确率从27.1%提升到43.8%，展现出良好的粗到细语义层次。

**Conclusion:** 研究结果证明，码本的总容量固然重要，但容量的分布和组织方式更为关键。可变码本设计有效解决了熵悬崖问题，为视觉标记化和自回归生成提供了新的设计思路。
...
> 提出一种可微分框架，通过自适应计算4D光照条件，实现高效、高质量的同时获取物体的形状和反射率，使用统一的空角结构光和单相机。

<details>
  <summary>Details</summary>

**Motivation:** 传统的形状和反射率获取方法需要复杂的设备和多次采集，效率不高。作者希望开发一个更高效的框架，能够在单相机条件下，通过智能选择光照条件，同时准确地重建物体的几何形状和反射特性。

**Method:** 基于像素级的深度和反射率概率模型，通过可微分方式将下一光照条件与减少深度不确定性的损失函数连接。随着新结构光照的投射，利用对应图像测量更新每个像素的不确定性。最后，基于微调方法重建深度图和反射率参数图，通过最小化物理测量与模拟结果之间的差异。

**Result:** 在物理实验中，该框架在形状和外观变化广泛的物体上表现出色。深度重建结果优于现有技术，反射率结果与照片验证相当。

**Conclusion:** 该可微分自适应光照计算框架实现了单相机下高效、高质量的同时形状和反射率获取，为计算机视觉和图形学领域的三维重建提供了新的解决方案。
...
> 提出逆注意力嵌入机制，通过显式捕获拥挤场景中被忽略的背景区域来增强视频语义搜索性能，无需额外训练即可显著提升检索效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频编码器在处理拥挤场景时倾向于关注突出前景而忽略重要的背景上下文信息，导致视频语义搜索性能受限。

**Method:** 设计逆注意力嵌入机制，显式捕获和突出视觉编码器通常忽略的背景区域，并将逆注意力嵌入与传统视觉嵌入结合进行语义检索。

**Result:** 实验和消融研究表明，该方法在拥挤环境下的视频语义搜索中，在召回率方面相比现有方法取得了显著改进。

**Conclusion:** 逆注意力嵌入机制能够有效解决拥挤场景中的背景信息缺失问题，为视频语义搜索提供了一种简单高效的改进方案。
...
> ZScribbleSeg是一种新的弱监督医学图像分割方法，通过高效的涂鸦标注策略和空间形状先验约束，仅使用涂鸦标注就在多个医学图像分割任务上达到了竞争性性能。

<details>
  <summary>Details</summary>

**Motivation:** 医学图像分割的全标注数据集构建成本高昂且需要专业知识，现有基于涂鸦标注的弱监督方法因监督不足和形状信息不完整导致分割不准确。

**Method:** 首先研究优良涂鸦标注原则，通过监督最大化和随机性模拟设计高效涂鸦形式；引入空间关系和形状约束的正则化项，利用EM算法估计标签类别的混合比例；最后将高效涂鸦监督与空间先验集成到ZScribbleSeg框架中。

**Result:** ZScribbleSeg仅使用涂鸦标注，在ACDC、MSCMRseg、BTCV、MyoPS、Decathlon-BrainTumor和Decathlon-Prostate六个分割任务上实现了竞争性性能。

**Conclusion:** 该方法通过结合高效的涂鸦标注策略和空间形状先验，有效解决了传统涂鸦监督方法中的监督不足和形状信息缺失问题，为医学图像的弱监督分割提供了有效的解决方案。
...
> Spark3R是一个无需训练的3D重建加速框架，通过差异化压缩查询标记和键值标记，实现28倍加速。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于Vision Transformer的前馈3D重建模型在处理视频输入时面临计算成本过高的挑战，全局注意力层的二次复杂度限制了其扩展能力。

**Method:** Spark3R创新性地将查询标记(编码视图特定几何请求)与键值标记(代表共享场景上下文)分离压缩：对查询标记应用组内合并，对键值标记采用轻量级剪枝，并自适应调整各层的键值压缩因子。

**Result:** 在1000帧输入上实现高达28倍加速，同时保持竞争性的重建质量，且无需重新训练即可直接集成到多个预训练模型中。

**Conclusion:** 该研究通过深入理解3D重建模型中不同标记的功能特性，提出了一种高效的加速策略，为大规模视频3D重建提供了实用解决方案。
...
> 该研究实现了一个部署在九颗卫星热红外星座上的野火检测系统，在严格的资源限制下(模型小于1MB，推理延迟小于150ms)实现了高效检测，达到了10分钟端到端警报的目标。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于上下文热阈值的野火检测方法(MODIS, VIIRS)无法处理在200米分辨率中波红外图像中经常出现的亚像素或单像素热异常特征，以及极端类别不平衡的挑战，需要开发新的轻量级检测方法。

**Method:** 采用密集掩码自编码(DenseMAE)和混合DenseMAE+EMA蒸馏变体进行自监督预训练，在 proprietary 九卫星MWIR数据集上进行密集表示学习，通过线性探测和全分布像素级平均精度进行评估。

**Result:** 最快的SSL预训练模型达到0.640测试AP和0.69 Fire-F1，延迟65.34ms/批次，模型仅0.52MB。最佳配置在1MB以下达到0.699 AP和0.744 Fire-F1，超越同等约束下的监督基线(0.650 AP)。

**Conclusion:**  DenseMAE自监督预训练能够在严格的资源约束下训练出紧凑而高效的野火检测模型，为星载实时野火监测提供了可行的解决方案，显著优于传统热阈值方法和监督学习方法。
...
> 本文提出一种基于欧拉运动场的图像动画方法，使用相邻帧光流信号替代拉格朗日运动引导，并结合双向几何一致性机制来减少漂移伪影，实现了并行化训练并保持时序连贯性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可控图像动画框架通常依赖于拉格朗日运动引导（相对于初始帧的光流估计），这种方法计算效率低且难以在整个生成过程中提供有界的误差监督。传统的相邻帧生成方法存在漂移伪影问题，影响生成质量。

**Method:** 1）采用欧拉运动场设计，使用相邻帧间的光流信号指导生成，运动信号始终描述短时跳跃；2）引入双向几何一致性机制，通过前向-后向循环检查来数学识别和遮挡区域，防止模型学习错误的扭曲目标；这种设计支持并行化训练并提供有界误差监督。

**Result:** 实验证明，该方法相比基于基准的方法能够加速训练过程、保持时序连贯性，并显著减少动态伪影。双向几何一致性机制有效解决了遮挡区域的错误学习问题。

**Conclusion:** 通过将运动监督方式从全局的拉格朗日视角转换为局部的欧拉视角，并结合几何一致性约束，本文方法在保持图像动画质量的同时提升了训练效率，为可控图像动画提供了一个更加实用和高效的解决方案。
...
> NOVA是一种基于隐式神经表示(INR)的世界建模框架，通过将系统状态表示为结构化的网络权重，实现了高效、可解释的视频预测，支持零样本超分辨率和内容与解耦的可控编辑。

<details>
  <summary>Details</summary>

**Motivation:** 现有的世界模型依赖原始像素编码到不透明的潜在空间，计算昂贵且不可解释。需要开发一种更高效、可解释且具有结构化表示的世界建模方法。

**Method:** 提出NOVA框架，使用坐标基隐式神经表示(INR)的权重和偏置来表示系统状态。通过分析式渲染消除解码器瓶颈，并采用动作匹配目标进行蒸馏。无需辅助损失或对抗训练即可实现场景组件的解耦。

**Result:** 在多个挑战性数据集上验证，实现了强大的可控预测效果，仅用约4000万参数即可在单个消费级GPU上运行，成功解耦了背景、前景和帧间运动等场景组件。

**Conclusion:** 基于INR的结构化表示不仅增强了对潜在动态的理解，还为沉浸式和可定制的虚拟体验铺平了道路，是高效可解释世界建模的重要进展。
...
> TD-VLN将视觉语言导航重构为基于俯视地图的单步全局路径规划问题，通过NavOne框架实现8-80倍的效率提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有VLN方法采用以自我为中心的逐步导航范式，存在误差累积和效率低的问题；而基于地图的方法受限于离散路径瓶颈，无法进行连续空间推理。

**Method:** 提出TD-VLN方法，将导航重构为预建俯视地图上的单步全局路径规划；构建R2R-TopDown数据集；设计NavOne统一框架，包含俯视地图融合器和空间感知深度混合的注意力残差模块，直接预测稠密路径概率。

**Result:** 在R2R-TopDown数据集上达到最先进性能，规划阶段比现有地图基线快8倍，比中心化方法快80倍。

**Conclusion:** 通过全局路径规划范式和端到端稠密预测，TD-VLN实现了高效准确的视觉语言导航，为连续空间推理提供了新思路。
...
> TinyBayes是首个结合封闭式贝叶斯分类器与移动级计算机视觉流程的作物病害检测框架，总模型大小仅9.5MB，可在CPU上150ms内完成推理，准确率达78.7%。

<details>
  <summary>Details</summary>

**Motivation:** 西非小农户的可可作物受到CSSVD和炭疽病严重威胁，现有边缘部署的植物病害检测系统缺乏不确定性量化，而针对边缘设备的贝叶斯方法主要关注硬件架构而非农业应用。

**Method:** 采用YOLOv8-Nano(5.9MB)进行病灶定位，MobileNetV3-Small(3.5MB)提取特征，结合Jacobi-DMR分类器(13.5KB)实现封闭式贝叶斯分类，形成完整边缘部署流程。

**Result:** 在Amini可可污染挑战数据集上达到78.7%准确率，端到端CPU推理时间<150ms/图像，总模型大小9.5MB，相比七种分类器展现最佳准确率-模型大小-推理速度平衡。

**Conclusion:** TinyBayes成功将贝叶斯不确定性量化引入边缘农业应用，提供了适合资源受限环境的轻量化、快速、可靠的病害检测解决方案，并证明了Jacobi-DMR的统计性质。
...
> Earth-o1是一种观测原生的大气世界模型，直接从非网格化观测数据学习地球系统的三维物理演化，实现了与IFS相当的预报精度。

<details>
  <summary>Details</summary>

**Motivation:** 传统建模框架将异构测量数据强制匹配到预定义空间网格中，限制了原始传感器数据的充分利用并造成计算瓶颈。

**Method:** Earth-o1直接从非网格化观测数据学习连续的三维物理演化，将多样化的传感器输入集成为统一的无网格动力场，模型自主地推进大气状态在时空中的演进。

**Result:** 在后报评估中，Earth-o1达到与运行中集成预报系统(IFS)相当的地面预报技能，能够实现直接的实时预报和跨传感器推理。

**Conclusion:** 连续的观测驱动世界模型作为一类全新的全观测原生地球物理模拟器，能够匹配已建立物理框架的保真度，为地球数字孪生提供了可扩展的数据驱动基础。
...
> SwiftI2V是面向高分辨率图像到视频生成的高效框架，通过两段式设计和条件分段生成技术，在保持输入图像细节的同时大幅降低计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 现有高分辨率I2V方法存在严重问题：端到端模型内存和延迟成本过高；级联方法中的视频超分辨率阶段缺乏明确的输入图像条件，容易产生细节幻觉和结构偏离。

**Method:** 采用两阶段设计：先生成低分辨率运动参考以降低令牌成本，然后进行强图像条件的2K合成。核心技术包括条件分段生成(CSG)分段合成视频控制每步令牌预算，以及分段内双向上下文交互提升连贯性和保真度。

**Result:** 在VBench-I2V 2K分辨率测试中达到与端到端基线相当的性能，总GPU时间减少202倍，可在单个数据中心GPU(H800)或消费级GPU(RTX 4090)上进行实用的高分辨率I2V生成。

**Conclusion:** SwiftI2V成功解决了高分辨率I2V生成的效率-保真度困境，为实际应用提供了可行的解决方案，使得高分辨率视频生成在普通硬件上成为可能。
...
> eX2L是一个基于解释的可解释框架，通过在训练过程中去混淆特征，显著提升了在分布偏移场景下的模型性能，在Spawrious基准测试上超越现有方法约5-11%。

<details>
  <summary>Details</summary>

**Motivation:** 现有分布偏移缓解算法性能不稳定，常无法超越基线ERM，且算法复杂度高、可解释性差，间接处理虚假相关性。需要更直接、可解释的方法来解决这一问题。

**Method:** 提出eX2L框架，通过惩罚主标签分类器和混淆物分类器的Grad-CAM激活图之间的相似性，在训练过程中显式解耦标签和干扰属性，在群组级别实现域不变性。

**Result:** 在Spawrious Many-to-Many Hard Challenge基准测试上，eX2L达到82.24%的平均准确率和66.31%的最差群组准确率，分别比当前最佳方法提升5.49%和10.90%。

**Conclusion:** eX2L证明了通过在群组级别显式解耦标签和干扰属性可以实现功能性域不变性，为分布偏移问题提供了一个高效且可解释的解决方案。
...
> 本文提出连续时间分布匹配(CDM)，将DMD从离散优化改进为连续优化，通过动态连续调度和连续时间对齐目标，显著提升了少步图像生成的视觉质量，无需复杂辅助模块。

<details>
  <summary>Details</summary>

**Motivation:** 现有步蒸馏方法中，DMD的离散时间定义和反向KL散度的模式寻求特性导致视觉伪影和过度平滑，常需依赖GAN或奖励模型等复杂辅助模块来恢复保真度，亟需更简单有效的解决方案。

**Method:** CDM通过两个连续时间设计实现：1) 用动态随机长度的连续调度替代固定离散调度，使分布匹配在采样轨迹的任意点进行；2) 提出连续时间对齐目标，在学生模型速度场外推的隐变量上进行主动离轨迹匹配。

**Result:** 在SD3-Medium和Longcat-Image等架构上的广泛实验表明，CDM在少步图像生成方面提供了高度竞争力的视觉保真度，且不依赖复杂的辅助目标。

**Conclusion:** CDM成功将DMD框架从离散锚点迁移到连续优化，通过连续时间的创新设计，在保持模型简洁性的同时显著提升了生成质量，为加速扩散模型提供了新的有效途径。
...
> 本文通过提出四次网格填充方法，证明深度神经网络的决策区域不仅是路径连通的，而且是单连通的。

<details>
  <summary>Details</summary>

**Motivation:** 理解决策区域的拓扑结构对解释深度神经网络的工作机制至关重要。先前研究已证明决策区域是路径连通的，但本文探索更强的拓扑性质：决策区域内的闭合环路是否能在不离开该区域的情况下收缩。

**Method:** 提出迭代的四次网格填充程序，构建有限分辨率的保标签曲面，该曲面由给定环路边界且完全位于同一决策区域内。进一步将此构造与自然Coons面片连接，以量化其与环路规范几何插值的偏差。

**Result:** 在多个现代图像分类模型上评估方法，为深度神经网络决策区域不仅路径连通而且单连通的假设提供了实证支持。

**Conclusion:** 研究结果表明深度神经网络的决策区域具有更强的拓扑性质，为理解网络内部工作机制提供了新的视角，为后续拓扑分析研究奠定了基础。
...
> 系统性比较了重建编码器和语义编码器在基于动作条件的潜在扩散世界模型中的表现，发现语义潜在空间在策略性能上优于传统的像素重建空间。

<details>
  <summary>Details</summary>

**Motivation:** 随着世界模型逐渐采用潜在扩散建模，选择合适的潜在空间变得至关重要。当前主流使用的像素重建编码器（如VAE）可能不是最优选择，需要评估语义对齐的预训练编码器在机器人控制任务中的价值。

**Method:** 在BridgeV2数据集上固定协议训练世界模型变体，比较六种重建和语义编码器（VAE、Cosmos、V-JEPA 2.1、Web-DINO、SigLIP 2等），从视觉保真度、规划和下游策略性能、潜在表示质量三个维度进行评估。

**Result:** 重建编码器（VAE、Cosmos）在像素级得分较高，但语义编码器（特别是V-JEPA 2.1）在策略性能和其他两个评估维度上表现更优。视觉保真度单独不足以选择世界模型。

**Conclusion:** 语义潜在空间是构建策略相关的机器人扩散世界模型的更强基础，研究倡导使用语义编码器构建更高性能的机器人世界模型。
...
> FREPix是一个像素级图像生成框架，通过频率异构处理将低频和高频组件分离，实现显式的粗到细生成，在低NFE regimes下表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 现有像素空间扩散方法忽视了不同频率组件的独特作用和学习动态，将图像生成视为频率同质过程。需要开发更有效的频率感知生成框架。

**Method:** FREPix框架将生成分解为低频和高频组件，为它们分配独立的传输路径，使用分解网络进行预测，并通过频率感知目标进行训练，使粗到细生成为显式设计原则。

**Result:** 在ImageNet上达到1.91 FID(256×256)和2.38 FID(512×512)，在低NFE regimes下表现特别出色，在像素级生成模型中具有竞争力。

**Conclusion:** 频率异构处理是有效的像素级图像生成策略，FREPix证明了显式频率分解和粗到细生成原理的价值，为像素空间生成提供了新方向。
...
> GeoStack是一种用于视觉语言模型的模块化知识组合框架，通过几何约束和适配器权重折叠实现零灾难性遗忘和常数时间推理。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型在多个领域或任务累积专业知识时普遍存在灾难性遗忘问题，即学习新知识会覆盖原有知识，需要一种既能组合多领域知识又能保持基础知识的有效方法。

**Method:** 提出GeoStack几何堆叠框架，通过在适配器流形上施加几何和结构约束来保护基础模型知识，并利用数学证明的权重折叠特性实现常数时间推理复杂度O(1)，无论集成多少专家模型。

**Result:** 在多领域适配和类增量学习实验中，GeoStack实现了高效的长期知识组合，显著减轻了灾难性遗忘，同时保持推理效率不受专家数量影响。

**Conclusion:** GeoStack为视觉语言模型的知识组合提供了一种实用且高效的解决方案，通过几何约束和权重折叠技术成功解决了灾难性遗忘问题，为多任务学习的长期发展奠定了基础。
...
> 该研究提出了一种新的自监督预训练方法，通过将3D MRI体积转换为可控的2D切片渲染序列，使用动作条件预训练目标来学习解剖和空间表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有的MRI自监督预训练方法主要将扫描视为静态的切片、块或体积聚合，缺乏捕捉连续变化的能力，需要探索新的自监督信号形式。

**Method:** 将3D MRI体积渲染为连续位置、方向和尺度的可控2D切片序列，通过tokenizer编码切片观察，潜在动力学模型预测特征演化，采用动作条件预训练目标。

**Result:** 在多个解剖和空间下游任务中，该方法表现优于标准静态体积基线、仅tokenizer预训练和无对齐动作的动力学变体。

**Conclusion:** 可控MRI切片导航为从大型未标记MRI数据集中学习解剖和空间表示提供了有用的补充预训练接口。
...
> MARBLE是一种多维度奖励平衡优化框架，通过在梯度空间协调多个奖励，解决了扩散模型对齐中多目标优化的样本级不匹配问题，实现了0.97倍训练速度下的联合多奖励优化。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型对齐需要同时优化多个评估标准（如图像质量、美学等），但现有方法或需要训练多个专家模型，或依赖加权求和奖励和手动调序，无法有效协调多维度奖励。

**Method:** 提出梯度空间优化框架MARBLE：为每个奖励维护独立的优势估计器，计算各奖励的策略梯度，通过求解二次规划问题将它们协调为单一更新方向；利用DiffusionNFT损失的仿射结构提出摊销公式，将计算成本从K+1次反向传播降至接近单奖励基线成本；使用EMA平滑平衡系数稳定训练。

**Result:** 在SD3.5 Medium上使用五个奖励进行实验，MARBLE同时提升了所有五个奖励维度，将对齐最差奖励的梯度余弦从加权求和下80%迷你批次的负值转为一致正值，训练速度达到基线训练的0.97倍。

**Conclusion:** MARBLE成功解决了多奖励对齐中的样本级不匹配问题，提供了一种无需手动权重调优的高效联合训练方法，为扩散模型的多维度对齐提供了新的解决方案。
...
> FreeSpec是一个无需训练的光谱重构框架，通过奇异值分解解决视频扩散模型生成长视频时的内容漂移、时间不一致和动态过度平滑问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频扩散模型在短视频合成中表现良好，但在无训练扩展到长视频时存在内容漂移、时间不一致和动态过度平滑等问题。传统方法通过全局和局部分支结合改进时间一致性，但在外观和动作进展紧密耦合（如相机运动和序列运动）时，预定义的分解准则不可靠。

**Method:** 从奇异光谱角度分析发现，扩大的自注意力窗口导致光谱集中现象。FreeSpec框架通过奇异值分解全局和局部特征，将全局分支作为低秩光谱引导，局部分支作为高秩重构基础，实现光谱层面的融合，避免传统分解规则的刚性特征分割。

**Result:** 在Wan2.1和LTX-Video上的实验表明，FreeSpec显著改善了长视频生成质量，特别是在时间动态方面，同时保持了强视觉质量与时间一致性。

**Conclusion:** FreeSpec通过光谱级的重构方法有效解决了长视频生成中的关键问题，为无需训练的视频扩展提供了新的解决方案，在保持长距离一致性的同时更好地保留了空间细节和时间动态。
...
> 本文提出了一种名为'默认完成排斥'(DCR)的无训练框架，通过建模和抑制扩散模型中的默认完成偏差，解决了生成罕见但合理组合内容的失败问题。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型在生成罕见但语义有效的组合(如雪海滩、夜晚彩虹)时存在系统偏差，会默认生成更常见的替代内容。现有的引导机制没有明确建模这种竞争趋势，因此难以防止这种崩溃。

**Method:** DCR通过放松罕见组合因子同时保持周围语义来构建反事实吸引子，计算目标轨迹与吸引子轨迹之间的反事实漂移，并提出基于投影的排斥机制来移除与漂移方向对齐的引导分量，从而抑制不必要的频繁完成。

**Result:** 在罕见组合提示的实验中，DCR提高了组合保真度，同时保持了视觉质量。分析表明该框架能够暴露并对抗模型内在偏差。

**Conclusion:** DCR为可控生成提供了超越显式约束执行的新视角，通过反事实推理有效处理了扩散模型中的组合偏差问题，且完全无需重新训练或修改架构。
...
> 本文提出了Sparkle数据集和相应的训练管道，解决了视频背景替换任务中高质量训练数据稀缺的问题，显著提升了背景替换的效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有开源视频编辑数据集主要关注局部编辑和风格迁移，而背景替换这一重要创作任务由于需要合成全新的时序一致场景并保持前景背景交互，导致大规模高质量数据生成困难，现有模型表现不佳。

**Method:** 设计了解耦前景和背景引导的可扩展数据生成管道，采用严格的质量过滤机制，构建包含约14万视频对的Sparkle数据集，涵盖五种常见背景更换主题，同时推出专门针对背景替换的最大评估基准Sparkle-Bench。

**Result:** 在OpenVE-Bench和Sparkle-Bench上的实验表明，使用该数据集训练的模型比所有现有基线方法都取得了显著更好的性能表现。

**Conclusion:** 通过解耦式的数据生成管道和严格质量控制，成功构建了高质量的大规模背景替换数据集，有效推动了该领域的发展，所有资源完全开源。
...
> MedHorizon是一个全新的长上下文医疗视频理解基准测试，包含759小时完整临床手术视频和1253个基于证据的多选题，用于评估多模态大语言模型在稀疏证据检索和多步临床推理方面的能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的医疗MLLMs主要处理图像和短视频，但真实临床审查往往需要理解完整手术视频。与普通长视频不同，医疗程序包含大量冗余的解剖结构视图，而决定性证据在时间上是稀疏的、空间上是微妙的，且依赖于上下文。

**Method:** 构建了MedHorizon基准测试，收集759小时真实临床手术视频，设计1253个基于证据的多选题，这些问题的证据极其稀疏（平均仅0.166%的证据帧），迫使模型在解释和聚合发现之前先搜索嘈杂的程序流。

**Result:** 评估显示最佳模型准确率仅41.1%，表明当前系统离鲁棒的完整程序理解还有很大差距。研究发现性能与帧数不成正比，证据检索和临床解释是主要瓶颈，这些瓶颈源于程序推理薄弱和冗余下的注意力漂移，通用采样方法只能部分平衡局部细节与全局覆盖。

**Conclusion:** MedHorizon为需要在完整临床工作流中检索稀疏证据并进行推理的多模态大语言模型提供了严格的测试平台，揭示了当前模型在长上下文医疗视频理解方面的重大挑战。
...
> 提出一种基于采样的免矩阵求逆方法，通过稀疏隐变量结果式构造求解器，使用逆快速傅里叶变换插值重构行列式多项式，实现相机几何估计的高效稳定求解。

<details>
  <summary>Details</summary>

**Motivation:** 传统的基于Gröbner基或结果式的方法在求解相机几何估计的最小问题时，由于在线求解器需要矩阵求逆而面临计算挑战。

**Method:** 采用基于采样的免矩阵求逆方法：1) 使用稀疏隐变量结果式构造求解器；2) 通过逆快速傅里叶变换插值从采样评估中高效重构隐变量的行列式多项式；3) 求解该多项式得到隐变量；4) 通过识别秩1亏缺子矩阵和应用克拉默规则恢复剩余未知数；5) 基于最大公约数的准则确保在噪声下的鲁棒子矩阵识别。

**Result:** 在不同最小问题上的实验表明，所提出的求解器实现了强大的数值稳定性和有竞争力的运行时间，特别是在小规模问题上表现突出。

**Conclusion:** 该方法为传统基于Gröbner基和结果式的求解器提供了实用的替代方案，在保持计算效率的同时提高了数值稳定性。
...
> DINORANKCLIP是一个多模态预训练框架，通过集成DINOv3教师模型和高阶Plackett-Luce排序机制，同时解决了CLIP的两个结构性弱点：忽略未匹配样本的相对排序和全局池化导致的细粒度结构敏感性不足。

<details>
  <summary>Details</summary>

**Motivation:** CLIP模型存在两个主要缺陷：对称的InfoNCE损失函数丢弃了批次内未匹配样本对的相对顺序信息，而全局池化操作将视觉表示压缩成语义瓶颈，对细粒度局部结构感知能力差。现有的RANKCLIP只部分解决了第一个问题，且仍然继承第二个缺陷。

**Method:** 提出的DINORANKCLIP框架包含两个核心技术：1) 通过双分支轻量学生模型和多尺度融合模块(包含通道-空间注意力、自注意力精炼器和冲突感知门)将冻结的DINOv3教师模型注入到对比学习主干中；2) 引入高阶Plackett-Luce排序模型，其中每个位置的效用通过注意力参数化的成对和元组转换项增强，将CLIP和RANKCLIP作为零阶和一阶特例包含在内。

**Result:** 实验表明最优排序阶数为R*=3，在72小时单八GPU H100节点上完成全面评估(包括阶数扫描、5数据集细粒度探测、4节点模态差距分析、6变体融合消融)。DINORANKCLIP在相同计算资源下持续优于CLIP、CyCLIP、ALIP和RANKCLIP，在细粒度和分布外评估上获得最大相对增益。

**Conclusion:** DINORANKCLIP成功解决了CLIP的两个结构性弱点，通过视觉-语言对齐的增强和高阶排序机制，在细粒度结构推理和分布外泛化方面取得显著改进，为多模态预训练提供了新的解决方案。
...
> DPM++是一个动态屏蔽度量学习框架，通过自适应屏蔽选择可靠的身份子空间，结合CLIP的两阶段监督策略和显著性引导的样本合成方法，有效解决了遮挡行人重识别问题。

<details>
  <summary>Details</summary>

**Motivation:** 遮挡是行人重识别中的关键挑战，现有方法缺乏统一的框架来学习在真实遮挡模式下的鲁棒可见性一致性匹配，严重遮挡会去除判别性身体线索并引入背景干扰。

**Method:** 提出DPM++框架，在分类器-原型空间上学习输入自适应的屏蔽度量，引入CLIP-based两阶段监督方案将ID级语义先验从文本分支转移到视觉空间，采用显著性引导的块传输策略合成可控逼真的遮挡样本，并设计遮挡感知样本配对和屏蔽引导优化。

**Result:** 在遮挡和完整行人重识别基准实验中，DPM++在整体和遮挡场景下都持续超越现有最先进方法的性能。

**Conclusion:** DPM++为遮挡行人重识别提供了统一有效的解决方案，通过动态屏蔽度量学习和高质量的样本合成策略，显著提升了模型在真实遮挡环境下的识别性能。
...
> 论文提出了首个统一的多模态域泛化基准MMDG-Bench，通过大规模实验揭示了当前方法的局限性，表明该领域仍有巨大进步空间。

<details>
  <summary>Details</summary>

**Motivation:** 多模态域泛化领域缺乏标准化评估协议，现有研究存在数据集、模态配置和实验设置不一致的问题，难以准确评估算法的真实进展。

**Method:** 构建了MMDG-Bench基准，涵盖3个任务、6个数据集、6种模态组合、9种代表性方法，评估包括准确率、抗腐蚀鲁棒性、缺失模态泛化、误分类检测和分布外检测等多个方面。

**Result:** 训练了7,402个神经网络，得到5个关键发现：专用方法相比ERM基准仅有微小改进；没有单一方法在所有场景下最优；与理论上限仍有较大差距；三模态融合不一定优于最强双模态；所有方法在腐蚀和缺失模态情况下表现显著下降。

**Conclusion:** 多模态域泛化领域仍远未解决，需要更深入的研究来提升模型的鲁棒性和泛化能力，统一的基准对推动领域发展至关重要。
...
> Relit-LiVE是一种新颖的视频重照明框架，它通过引入原始参考图像和联合预测环境视频的方式来生成物理一致且时间稳定的重照明视频，无需预先知道相机姿态。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频扩散模型重照明方法依赖于准确的本征分解，但对真实世界视频而言，这种分解往往不可靠，导致外观扭曲、材质破坏和时间伪影等问题。

**Method:** 1) 在渲染过程中显式引入原始参考图像，帮助模型恢复本征表征中丢失或损坏的场景关键信息；2) 提出环境视频预测公式，在单个扩散过程中同时生成重照明视频和与每个相机视角对齐的环境图。

**Result:** 在合成和真实世界基准测试中，Relit-LiVE持续优于最先进的视频重照明和神经渲染方法，并支持多种下游应用。

**Conclusion:** 通过联合预测和参考图像引导，Relit-LiVE实现了无需相机姿态先验的物理一致、时间稳定的视频重照明，为场景级渲染、材质编辑、物体插入等应用提供了新的技术基础。
...
> 该论文提出BAMI方法来解决GUI基础模型在高分辨率和复杂界面元素场景下的性能问题，在训练免费的情况下显著提升了模型准确率。

<details>
  <summary>Details</summary>

**Motivation:** GUI基础（grounding）是GUI代理执行点击、拖拽等任务的关键能力，但在复杂场景（如ScreenSpot-Pro基准测试）中，现有模型表现不佳。

**Method:** 通过MPD归因方法识别出主要错误源：高图像分辨率导致的精度偏差和复杂界面元素导致的模糊偏差，提出BAMI方法，包含粗到精聚焦和候选选择两个关键操作来缓解这些偏差。

**Result:** 在训练免费设置下，BAMI显著提升了多种GUI基础模型的准确性，例如将TianXi-Action-7B模型在ScreenSpot-Pro基准上的准确率从51.9%提升至57.8%。

**Conclusion:** 消融研究证实BAMI方法在不同参数配置下具有鲁棒性，突显了其稳定性和有效性。
...
> ActCam是一个零样本视频生成方法，能够同时控制演员动作和摄像机轨迹，在无需训练的情况下实现精细的视频生成控制。

<details>
  <summary>Details</summary>

**Motivation:** 现有艺术视频生成方法缺乏对表演和摄影术的精细控制，无法同时控制演员动作和摄像机轨迹，限制了艺术创作的自由度。

**Method:** 基于预训练的图像到视频扩散模型，通过两阶段条件调度策略：早期去噪步骤同时使用姿态和稀疏深度来强制场景结构，后期仅使用姿态引导来细化高频细节。

**Result:** 在多个基准测试中，相比仅姿态控制方法和其他姿态和相机方法，ActCam在相机遵循性和运动保真度方面都有改进，在人类评估中更受青睐，特别是在大视角变化下表现优异。

**Conclusion:** 该研究证明了通过精心设计的相机一致性条件和分阶段指导策略，可以在无需训练的情况下实现强大的相机和运动联合控制，为艺术视频生成提供了新的解决方案。
