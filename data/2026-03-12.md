...
> 4DEquine disentangles 4D equine reconstruction into motion and appearance problems, using a spatio-temporal transformer and feed-forward network, achieving top performance on real-world datasets.

<details>
  <summary>Details</summary>

**Motivation:** To address the time-consuming nature and sensitivity to incomplete observation of previous methods in 4D equine reconstruction.

**Method:** For motion reconstruction, a spatio-temporal transformer with a post-optimization stage is used to regress pose and shape sequences. For appearance reconstruction, a feed-forward network constructs a 3D Gaussian avatar from single images.

**Result:** 4DEquine achieves state-of-the-art performance on real-world datasets, demonstrating the effectiveness of the new datasets and the framework.

**Conclusion:** The proposed method, 4DEquine, efficiently disentangles the 4D reconstruction problem, using synthetic datasets to excel in both dynamic motion and static appearance reconstruction.
...
> The paper proposes HG-Lane, a framework that generates high-fidelity lane scenes under adverse conditions to improve lane detection performance, which is validated on a new benchmark with notable performance increases ranging from 8.63 percent to 38.8 percent over existing models.

<details>
  <summary>Details</summary>

**Motivation:** Current datasets for lane detection lack data from extreme weather conditions, which limits the reliability of lane detection models in real-world scenarios.

**Method:** HG-Lane, a High-fidelity Generation framework for Lane Scenes under adverse weather and lighting conditions, is proposed to improve the performance of lane detection models in harsh conditions.

**Result:** Experimental results show significant performance improvements on their constructed benchmark. The overall mF1 score for CLRNet increased by 20.87 percent, and F1@50 scores for various categories improved by a range of 8.63 percent to 38.8 percent.

**Conclusion:** The HG-Lane framework and the benchmark dataset with adverse weather conditions significantly enhance lane detection performance under challenging circumstances.
...
> 本文针对传统无监督光谱聚类中光谱信息平衡问题，提出利用非平衡Wasserstein重心进行低维表示学习的新方法。

<details>
  <summary>Details</summary>

**Motivation:** 传统的基于Wasserstein空间字典学习的无监督聚类方法需要平衡光谱信息，这会导致类别模糊并牺牲了对异常值和噪声的鲁棒性。

**Method:** 该论文提出使用非平衡Wasserstein重心来学习数据的低维表示，进而提高基于光谱聚类的无监督标签学习效果。

**Result:** 在学习到的低维表示上使用光谱聚类技术，可以有效实现图像的无监督标签学习。

**Conclusion:** 该方法通过使用非平衡Wasserstein重心解决了光谱信息平衡的问题，从而在保持对异常值和噪声的鲁棒性的同时，更好地实现了无监督标签学习。
...
> 研究了从执行视频进行奖励建模的方法，引入了ExeVR-53k数据集，并提出了几种技术来优化模型性能，最终实现了高准确率和召回率的任务成功预测。

<details>
  <summary>Details</summary>

**Motivation:** 计算机使用代理（CUs）的能力正在增强，但对用户指令的执行轨迹进行评估仍然难以扩展。为了应对这个挑战，论文研究了从执行视频中进行奖励建模的方法。

**Method:** 研究了从执行视频中进行奖励建模的方法。引入了ExeVR-53k数据集，包含53K个高质量的视频-任务-奖励三元组。提出了对抗性指令翻译来生成带有逐步注释的负样本，并设计了时空令牌修剪技术，以从长高分辨率执行视频中学习。

**Result:** 基于这些组件，调整了一个执行视频奖励模型（ExeVRM），该模型只需要用户指令和视频执行序列就可以预测任务成功。实验结果显示，在Ubuntu, macOS, Windows, 和Android等系统上，ExeVRM 8B实现了84.7%的准确率和87.7%的召回率，超越了包括GPT-5.2和Gemini-3 Pro在内的专有模型。

**Conclusion:** 研究表明，执行视频奖励模型可以作为一种可扩展且模型无关的CUs评估器。
