...
> AdaGATE是一种无需训练的多跳问答证据控制器，通过基于实体差距跟踪和实用度选择机制，在减少2.6倍输入token的同时提升了证据选择的准确性和鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的检索增强生成方法在多跳问答中表现脆弱，特别是在现实部署环境中，检索到的证据可能存在噪声或冗余，且只能传递有限的上下文给生成器。

**Method:** AdaGATE将证据选择框定为token约束的修复问题，结合以实体为中心的差距跟踪、有针对性的微查询生成，以及基于实用度的选择机制，平衡差距覆盖、证实性、新颖性、冗余性和直接问题相关性。

**Result:** 在HotpotQA上的清洁、冗余和噪声注入检索条件下，AdaGATE在所有三种设置中都实现了最佳证据F1得分（清洁数据62.3%，冗余注入71.2%），比Adaptive-k少使用2.6倍输入token。

**Conclusion:** 显式的差距感知修复结合token高效的证据选择，能够提升多跳RAG在不完美检索条件下的鲁棒性。
...
> 研究探讨了学生在生成式AI环境下通过写作反驳论点来培养批判性思维，并验证了AI作为评估工具的可行性。

<details>
  <summary>Details</summary>

**Motivation:** 随着生成式AI的普及，存在抄袭和认知卸载的风险，需要探索如何结合AI培养学生批判性思维能力。

**Method:** 对36名大学生的35篇有效写作作品进行6维度（专注度、逻辑、内容、风格、正确性和引用）评估，采用混合方法设计，包括人工评估（2次同伴互评+1次教师评估）和6个前沿LLM模型评估。

**Result:** 学生自己撰写的反驳AI生成内容的论点包含逻辑等批判性思维要素；除一个模型外，AI评估与人工评估具有中等一致性（Gwets AC2=0.33），证明AI可在明确标准下大规模评估学生作业。

**Conclusion:** 在特定教学情境下，结合AI生成内容和学生反驳写作能有效培养批判性思维，同时AI评估可作为人工评估的可行补充方案。
...
> 该论文提出了一种基于证据的查询生成模型，能够自动从无查询数据集中生成查询词，实验表明生成的查询能有效支持查询聚焦摘要任务。

<details>
  <summary>Details</summary>

**Motivation:** 大规模数据集广泛用于摘要任务，但缺乏配套的查询信息，这限制了查询聚焦摘要(QFS)任务的发展。研究者需要解决如何从无查询数据集中自动生成有价值查询的关键问题。

**Method:** 提出基于证据的模型自动生成查询，分别进行内部评估(比较生成查询与原始查询的相似性)和外部评估(使用多种预训练模型和SOTA模型测试生成的查询在摘要任务中的性能)。

**Result:** 实验结果显示，使用基于证据生成的查询所产生的摘要，在ROUGE评分上与使用原始查询生成的摘要具有竞争力，证明了该方法的有效性。

**Conclusion:** 该研究表明可以有效地从无查询数据集中自动生成证据基础的查询关键词，这种方法为查询聚焦摘要任务提供了可行的解决方案。
...
> SLAM是一种新型白盒水印方案，通过调整潜在空间的语言结构方向而非修改token分布，在保持高质量(仅损失1-2分)的同时实现100%检测准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM水印方案通过偏置token分布实现检测，但会显著降低文本质量，需要在检测性和质量间进行权衡。

**Method:** 利用稀疏自编码器识别编码语言结构的残差流方向(如语态、时态、从句顺序)，在生成时因果性地引导这些方向，保持词汇采样和语义不受约束。

**Result:** 在Gemma-2 2B和9B模型上，SLAM实现100%检测准确率，质量损失仅1-2分，相比KGW/EWD/Unigram的7.5-11.5分大幅提升，同时保持自然的多样性和流畅度。

**Conclusion:** SLAM通过结构几何标记而非token频率偏置，在保持高质量的同时实现完美检测，但鲁棒性特征互补：抵抗词级编辑但对改变语法结构的改写敏感。
...
> 通过将少量推理轨迹编译成可重用的符号程序合成器，该方法实现了零LLM推理成本的程序求解，在多个基准测试中显著优于纯LLM方法，并能与LLM形成有效的神经符号混合系统。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM在需要大量组合搜索的困难程序合成任务上效率低下且不可靠，需要寻找一种方法来提高求解效率和可靠性，同时保持高准确率。

**Method:** 使用编码代理将少量推理轨迹编译成受约束DSL上的可重用符号程序合成器，构建无需测试时LLM调用的独立符号求解器，并探索符号求解器与LLM搜索的混合使用方式。

**Result:** 符号求解器集成在PBEBench-Lite上达到91.3%准确率，在PBEBench-Hard上达到84.7%，比具有测试时扩展的LLM高出16.3个百分点。神经符号混合系统将PBEBench-Hard准确率从68.4%提升至85.8%，同时减少78%的token使用。在真实历史语言学零样本迁移任务上达到80.1%准确率。

**Conclusion:** 推理轨迹可以被编译成可重用的符号求解器，这些求解器能够直接解决许多任务，在困难案例中补充LLM推理，并提供了向通用领域求解器归纳的可扩展路径，展示了符号方法和LLM结合的巨大潜力。
...
> 研究发现，当经过扩散语言模型水印处理的文本被多次重写后，水印检测率从87.9%急剧下降到4.86%，表明重写是比单次修改更有效的水印攻击手段。

<details>
  <summary>Details</summary>

**Motivation:** 扩散语言模型通过任意顺序去噪生成文本，传统水印方案无法直接应用。虽然已有针对扩散模型的水印技术，但需要评估其在实际应用中的鲁棒性，特别是面对多次重写攻击时的表现。

**Method:** 使用LLaDA 8B Instruct模型生成1,605个水印文本，通过4个不同规模的开放权重语言模型进行重写，测试5种重写风格（改写、人性化、简化、学术化、总结扩展），每种风格进行最多5次链式重写。

**Result:** 单次重写后检测率降至14-41%，五次重写后降至4.86%。三次重写后，检测分数已下降了从水印基线到零分布距离的86%。所有重写器都表现出类似的趋势。

**Conclusion:** 多次重写是比单次修改更强的水印攻击手段，现有水印技术在面对链式重写攻击时缺乏鲁棒性，需要开发更抗重写的水印方案。
...
> 这篇论文证明，专门训练的小型语言模型Olava Extract在合同提取任务上不仅性能超越大型前沿模型，而且推理成本降低78-97%

<details>
  <summary>Details</summary>

**Motivation:** 质疑大型语言模型在商业AI应用中的必要性，探索小型域专用模型是否能在特定任务上以更低成本实现更好或相当的性能，特别是在需要高准确性的法律领域

**Method:** 将自托管的Legal领域混合专家模型Olava Extract与五个前沿大型语言模型进行对比，通过结构化合同提取任务评估性能，使用macro F1和micro F1作为主要指标，同时关注精确度和幻觉问题

**Result:** Olava Extract在研究中表现最佳，macro F1达0.812，micro F1达0.842，精确度最高产生最少的幻觉和不支持提取，推理成本相比前沿模型降低78%-97%

**Conclusion:** 高性能的人类可比法律AI不再需要最大的外部托管模型，挑战了商业企业AI能力必须依赖更大模型、大规模基础设施和中心化托管提供商的传统假设
...
**Motivation:** 虽然MLLMs与RAG结合可以减少幻觉，但外部文档的引入会在实例层面隐藏严重的失效模式。

**Method:** 通过对内部注意力矩阵的机制诊断，发现re corruption现象由视觉盲视和结构位置偏置双重注意力塌陷导致，提出BAIR框架在推理时恢复视觉显著性并施加位置感知惩罚。

**Result:** BAIR在医疗事实性、社会公平性和地理空间基准测试中成功恢复多模态基础，提高诊断可靠性，无需模型重新训练或微调。

**Conclusion:** 揭示了RAG成功表象下的幻觉效应，许多看似正确的结果只是位置巧合，BAIR为解决re corruption问题提供了有效的推理时补救方案。
...
> When2Speak是一个专为学习多方对话中何时发言的合成数据集和四阶段生成框架，通过监督微调和强化学习显著提升了LLM在群组互动中的时机判断能力。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在生成上下文适当的回应方面表现出色，但在多方对话中的校准能力仍然不足，其中决定何时发言与说什么内容同样重要。在群组互动中，每个回合都回应会导致过多的打断和对话连贯性下降。

**Method:** 提出了When2Speak，这是一个基于现实的合成数据集和四阶段生成管道，包含215,000个源自16,000场对话的示例，涉及2-6名发言者。管道结合了现实基础、结构化增强、受控的对话转录合成和微调就绪的监督。首先进行监督微调，然后应用非对称奖励塑形的强化学习来改进模型表现。

**Result:** 监督微调在多个模型系列显著优于零样本基线（4B参数以上模型平均Macro F1提升60%，最大提升120%）。但SFT训练的模型过于保守，错过近一半合理的干预（平均MIR为0.50）。强化学习将MIR降至0.186-0.218，召回率从0.479提升至0.78-0.81。

**Conclusion:** 研究表明时间参与是对话智能中一个独特且可训练的维度，基于现实的合成数据为使LLM在多方互动中更自然、适当地参与提供了有效且可扩展的途径。
...
> 本文提出了一种名为TurnGate的回级监控方法，能够在多轮对话中最早检测出有害意图的节点，同时构建了多轮意图数据集(MTID)来支持训练和评估。

<details>
  <summary>Details</summary>

**Motivation:** 多轮对话中的隐藏恶意意图对部署的大型语言模型构成日益严重的威胁。攻击者通过将恶意意图分散到多个看似无害的对话轮次中，绕过了现有的安全防护机制。

**Method:** 提出了一种回级干预方法，在最早能够实现有害行为的对话回合进行干预；构建了包含分支攻击场景、匹配的良性负样本和最早有害回合标注的多轮意图数据集(MTID)；开发了基于该数据集的TurnGate监控模型。

**Result:** TurnGate在恶意意图检测方面大幅优于现有基线模型，同时保持了较低的过度拒绝率；该模型在不同领域、攻击流水线和目标模型上都表现出良好的泛化能力。

**Conclusion:** 通过精确定位最早的有害使能回合并进行干预，能够有效防御多轮对话中的隐藏恶意攻击，为大型语言模型的安全部署提供了新的解决方案。
...
> 研究发现大型语言模型在不同网络层深度处理正负情感效价，负向情感在早期层处理，正向情感在中后期层处理，情感效价是局部化、因果且可控的。

<details>
  <summary>Details</summary>

**Motivation:** 当前对大型语言模型机制可解释性研究中，情感内容的机制层面理解仍然不足，需要探究LLM是通过专用内部结构还是通过表面词元匹配来处理情感效价。

**Method:** 使用激活修补和引导技术，在开源LLMs上进行实验，通过固定话题并翻转情感效价来验证处理机制，识别特定层级的情感编码方向。

**Result:** 发现负向情感定位于早期层，正向情感在中后期层达到峰值；话题固定时翻转效价产生相反响应；在识别层使用正能量方向引导可将中性提示转向正向。

**Conclusion:** LLM中的情感效价是局部化、因果且可引导的，这使其成为基于可解释性的监督的具体目标。
...
> XL-SafetyBench是一个跨文化LLM安全评估基准，包含10个国家-语言对的5500个测试用例，区分越狱鲁棒性和文化敏感性，发现前沿模型在这两方面没有关联性，本地模型的安全表现往往源于生成能力不足而非真正对齐。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM安全基准主要面向英语，通过翻译评估，无法捕捉特定国家的危害，也很少区分文化嵌入敏感性（如本地化敏感内容）和普遍危害（如暴力）。

**Method:** 构建XL-SafetyBench基准，包含越狱基准和文化基准两类测试用例；采用多阶段流程结合LLM辅助发现、自动验证和双重母语标注；引入ASR（越狱成功率）、NSR（中性安全率）和CSR（文化敏感率）三个互补指标。

**Result:** 评估10个前沿和27个本地LLM发现：前沿模型的越狱鲁棒性与文化意识无耦合关系，综合安全得分掩盖了单维度差异；本地模型呈现ASR-NSR近线性权衡关系（r=-0.81），表明表面安全反映的是生成能力失败而非真正对齐。

**Conclusion:** XL-SafetyBench为多语言时代的跨文化安全评估提供了更细粒度的工具，揭示了现有安全评估的局限性，为改进LLM的跨文化对齐指明方向。
...
> 本文提出了一种名为BADIT的新型解决方案，通过将大语言模型参数分解为正交的基础能力LoRA专家，有效缓解了多任务指令调优中的跨任务干扰问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多任务指令调优方法虽然试图隔离任务特定参数，但由于仍有大量参数在不同任务间共享，跨任务干扰问题仍然存在，影响了大语言模型的性能。

**Method:** BADIT方法将LLM参数分解为正交的高奇异值LoRA专家来表示基础能力，通过球形聚类强制维持这些专家的正交性，使任何任务可以表示为这些基础能力的线性组合。

**Result:** 在SuperNI基准测试上对6个LLM进行的广泛实验表明，BADIT方法超越了现有最先进方法，并有效减轻了跨任务干扰程度。

**Conclusion:** 通过将大语言模型的能力分解为正交的基础组件并动态保持其正交性，BADIT为解决多任务学习中的梯度冲突问题提供了新的思路和有效的方法。
...
> BioTool是一个生物医学工具调用数据集，包含34个常用工具和7,040个高质量查询-API对，用于微调LLM显著提升其在生物医学领域的工具调用能力，超越GPT-5.1等商用模型。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在生物医学等专业领域表现不佳，主要因为无法有效利用生物医学工具，而现有的生物医学工具调用方法主要依赖上下文学习且工具集有限。

**Method:** 构建了BioTool数据集，收集来自NCBI、Ensembl和UniProt数据库的34个常用工具，包含7,040个人工验证的高质量查询-API对，覆盖变异、基因组学、蛋白质组学等多个生物医学领域，然后用该数据集微调40亿参数的LLM。

**Result:** 微调后的模型在生物医学工具调用性能上显著超越GPT-5.1等前沿商用LLM，专家评估显示集成BioTool微调的工具调用器能显著提升下游答案质量。

**Conclusion:** BioTool有效提升了LLM在生物医学领域的能力，通过提供全面的工具调用数据集解决了LLM无法有效利用生物医学工具的关键限制。
...
> 本文提出了一种名为DisAAD的方法，通过训练一个轻量级代理模型来对黑盒大语言模型进行不确定性量化，该代理模型仅需目标模型1%的参数量就能实现可靠的估计。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型幻觉问题阻碍了实际部署，特别对于只能通过API访问的黑盒模型。传统的不确定性量化方法需要大量计算或访问内部参数，无法实时估计且不能有效捕获黑盒推理过程信息。

**Method:** 提出Distribution-Aligned Adversarial Distillation（DisAAD）方法，引入生成-判别架构来引导轻量级代理模型学习黑盒LLM输出分布的高质量区域，使代理模型能够判断黑盒模型的已知与未知，然后基于证据学习估计不确定性。

**Result:** 广泛的实验验证了方法的有效性，表明仅占目标LLM大小1%的代理模型也能实现可靠的不确定性量化，解决了黑盒场景下的实时估计问题。

**Conclusion:** DisAAD为黑盒大语言模型的不确定性量化提供了一个有效的轻量级解决方案，能够在保证准确性的同时大幅降低计算成本，具有重要的实际应用价值。
...
> 研究发现，大型推理模型中的言语化评估意识(VEA)对模型行为影响有限，并不如先前担忧的那样会导致显著的策略性调整。

<details>
  <summary>Details</summary>

**Motivation:** 研究者担心大型推理模型在思维链中表现出评估意识后，会策略性地调整输出来优化评估标准，比如让模型看起来比实际更安全。

**Method:** 通过开源大型推理模型和涵盖安全、对齐、道德推理和政治观点的基准测试，采用政策内(对比自发含VEA和不含VEA的思维链)和政策外(预填充注入或移除VEA语句)两种方法进行测试。

**Result:** VEA对模型行为影响微乎其微：注入VEA效果近乎为零(ω≤0.06)，移除VEA仅引起小幅度变化(ω≤0.12)，自发VEA最多使答案分布变化3.7个百分点(ω≤0.31)。

**Conclusion:** 应谨慎将高VEA率解读为策略性行为或对齐篡改的证据，评估意识可能带来的安全风险小于当前文献假设的程度。
...
> FLAS是一种基于流的激活引导方法，通过学习概念条件的速度场来控制语言模型行为，首次在学习方法上持续优于上下文提示技术。

<details>
  <summary>Details</summary>

**Motivation:** 现有激活引导方法存在简化假设限制（固定、单步、位置不变变换），导致在AxBench等大规模评估中表现不佳，常被简单的上下文提示超越，且对未见概念泛化能力差。

**Method:** 提出FLAS（Flow-based Activation Steering），学习一个通用的概念条件速度场v_t(h,t,c)，将未引导激活传输到引导激活状态，摆脱传统方法的简化假设限制。

**Result:** 在AxBench上，FLAS首次让学习方法持续优于提示技术，在Gemma-2-2B-IT和Gemma-2-9B-IT模型上分别达到1.015和1.113的调和均值，无需针对每个概念进行调优。

**Conclusion:** 学习到的流展现出曲线、多步、token变化的轨迹特征，表明先前关于激活空间几何的假设可能不完整，为激活引导研究提供了新的视角。
...
> 本文提出LOVER，一个通过逻辑规则正则化的无监督验证器，无需监督数据即可显著提升大语言模型的推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 传统验证器需要构建资源密集的监督数据集，成本高昂且数据多样性有限，这限制了验证器的应用效果和普适性。

**Method:** LOVER将验证器视为二元隐变量，利用内部激活值并对多个推理路径强制执行三种逻辑约束：否定一致性、组内一致性和组间一致性（按最终答案分组）。

**Result:** 在10个数据集上的实验表明，LOVER显著优于无监督基线，达到监督验证器95%的性能水平。

**Conclusion:** 通过将逻辑规则作为先验知识，LOVER能够有效利用未标记示例，与任何现成的LLM直接兼容，为提升大模型推理能力提供了高效且实用的无监督解决方案。
...
> TextPro-SLM通过在输入端解决模态间隙问题，将语音输入转化为类似文本LLM的表示，显著缩小了语音语言模型与文本语言模型之间的差距。

<details>
  <summary>Details</summary>

**Motivation:** 现有的语音大语言模型虽然基于文本LLM构建，但仍然存在严重的模态间隙问题。之前的工作主要从输出端尝试缩小该间隙，但效果有限。作者认为关键瓶颈在于输入端。

**Method:** 提出TextPro-SLM框架，结合WhisperPro统一语音编码器（生成同步的文本token和韵律嵌入）与LLM骨干网络，训练时保持原TLM的语义能力同时学习副语言理解。

**Result:** 在3B和7B规模上，TextPro-SLM实现了领先SLM中最低的模态间隙，同时在副语言理解任务上表现出色，仅需约1000小时训练音频。

**Conclusion:** 从输入端减少模态间隙的方法既有效又数据高效，为语音大语言模型的发展提供了新的解决思路。
...
> LiSCP是一种轻量级的文体一致性分析方法，能够鲁棒地检测LLM生成的文本内容，在跨域检测上比现有方法提升11.79%。

<details>
  <summary>Details</summary>

**Motivation:** 随着LLM在内容创作中的普及，区分人类编写和LLM生成的文本成为多媒体审核的关键任务。现有检测器依赖统计线索或模型特定启发式方法，容易受到改写和对抗性操纵的影响。

**Method:** 提出LiSCP方法，构建结合离散文体特征和连续语义信号的一致性特征谱，利用跨多模态引导的改写文本变体中的文体稳定性来检测LLM生成内容。

**Result:** 在真实世界多媒体新闻和电影数据集上的实验表明，LiSCP在域内检测中表现优异，在跨域设置下比现有方法性能提升11.79%，并在对抗性攻击和人机混合场景下展现出显著鲁棒性。

**Conclusion:** LiSCP通过关注对抗性操纵下的特征稳定性，为LLM生成文本的鲁棒检测提供了有效解决方案，具有良好的跨域泛化能力和抗干扰性。
...
> PCNET是一个基于概率电路的幻觉检测与修正方法，通过精确的负对数似然计算检测LLM隐藏状态中的几何异常，作为动态门控制仅在检测到幻觉时触发对比解码修正。

<details>
  <summary>Details</summary>

**Motivation:** 现有幻觉修正方法存在的主要缺陷是无差别地修正每个token，这会破坏原本正确的生成结果，导致过度修正问题。

**Method:** PCNET作为可精确计算的密度估计器在LLM残差流上训练，将事实流形上的几何异常检测为幻觉；PC-LDCD机制仅在检测到潜在几何偏离事实区域时触发对比解码，保持正确生成不受影响。

**Result:** 在1B到8B的四个LLM模型上，PCNET在CoQA、SQuAD v2.0和TriviaQA上实现接近完美的幻觉检测(AUROC高达99%)；PC-LDCD在TruthfulQA基准上取得最优的True+Info、MC2和MC3分数，将平均损坏率降至53.7%并保持79.3%的正确保留率。

**Conclusion:** 该方法通过精确的几何异常检测和有选择性的修正机制，有效解决了现有技术过度修正的问题，在保持模型原有正确表达的同时显著提高了幻觉检测和修正的准确性。
...
> TableVista是一个包含30,000个样本的多模态表格推理基准测试，用于评估基础模型在视觉和结构复杂性下的表现，发现现有模型在复杂结构布局和纯视觉设置下性能显著下降。

<details>
  <summary>Details</summary>

**Motivation:** 当前缺乏专门评估基础模型在表格多模态推理方面能力的综合性基准，特别是在处理视觉和结构复杂性结合的场景时。

**Method:** 构建包含3,000个高质量表格推理问题的数据集，通过多风格渲染和转换流水线将每个问题扩展为10个不同的视觉变体，涵盖多样化场景风格、鲁棒性扰动和纯视觉配置，最终形成30,000个多模态样本。

**Result:** 评估了29个先进的开源和专有基础模型，发现模型在不同渲染风格下较稳定，但在复杂结构布局和纯视觉设置下表现出显著性能下降，表明当前模型在结合结构复性和视觉一体化展示时难以保持推理一致性。

**Conclusion:** 研究结果揭示了当前多模态能力的关键缺陷，为开发更鲁棒和可靠的表格理解模型提供了指导方向。
...
> 该论文提出了一种面向多语言地名数据的地理空间问答系统，通过混合检索器和XLM-RoBERTa-large阅读器实现了近乎完美的问答性能。

<details>
  <summary>Details</summary>

**Motivation:** 为解决多语言地区的地理空间问答需求，特别是地名信息的自动问答问题，需要构建高质量的数据集和有效的问答系统。

**Method:** 构建了包含9,688个塔塔尔斯坦共和国地名的双语数据集，基于此生成约39,000个问答三元组；采用混合检索器结合密集语义索引与地理空间过滤；测试多种阅读器架构。

**Result:** 混合检索器达到Recall@1=0.988、MRR=0.994；XLM-RoBERTa-large阅读器获得EM=0.992、F1=0.994的最佳效果；通过后处理解决了坐标问答问题。

**Conclusion:** 该系统能够有效处理多语言地理空间问答任务，相关资源已开源，可应用于地理编码服务、数字人文等领域。
...
> PrimeFacts提出了一种从事实核查文章中提取细粒度结构化证据的方法，通过利用LLM将锚点句子去上下文化为独立前提，显著提升了自动化检索和验证的性能。

<details>
  <summary>Details</summary>

**Motivation:** 事实核查文章包含丰富的支撑证据和推理过程，但由于呈现方式非结构化，这些证据对自动化验证系统来说基本不可用，需要将这些证据结构化以便机器处理。

**Method:** 收集13,106篇PolitiFact文章，识别49,718个文章内超链接作为证据锚点，使用大语言模型将锚点句子改写为独立的上下文无关前提，并探索提取额外的隐含证据。

**Result:** 去上下文化的证据在检索和验证任务中表现优异：检索的MRR提升30%相对增益，验证的Macro-F1提升10-20点，效果在不同的判决粒度和模型架构上保持一致。

**Conclusion:** 这项工作证明了重用事实核查员证据用于自动化的前景，为从真实世界事实核查中获取结构化证据提供了大规模资源，为自动化事实核查领域做出了重要贡献。
...
> PersonaKit是一个开源的快速原型与评估平台，用于研究不同角色对话代理的轮转行为，特别关注非顺从角色的打断处理策略。

<details>
  <summary>Details</summary>

**Motivation:** 随着对话系统从传统助手扩展到多样化角色(如权威教师、不合作商家等)，需要模拟人性化的轮转行为来维持心理沉浸感。现有全双工系统采用过于退让的

**Method:** 开发了PersonaKit平台，通过直观的JSON配置定义人物角色，指定概率性打断处理行为(让步、保持、桥接或覆盖)，并自动部署A/B对比调查。

**Result:** 通过8个不同角色的实地评估验证，证明了PersonaKit提供了可扩展的端到端框架，能有效研究新一代语音代理中复杂的社会语言学行为。

**Conclusion:** PersonaKit为研究人员提供了低成本、低延迟的实验环境，解决了构建实时全双工测试环境的工程开销问题，推动了角色特定轮转策略的研究。
...
> 该研究使用HPSG语法框架比较了AI生成新闻文本与人类新闻文本的句法特性，发现较新的经过指令调优的LLM在句法和词汇多样性上低于旧模型。

<details>
  <summary>Details</summary>

**Motivation:** 随着LLM生成文本质量的提升，需要深入了解AI生成文本与人类写作文本在语言学特征上的差异，特别是在句法结构方面的差异。

**Method:** 采用HPSG形式语法框架分析两个时期的人类新闻数据集和两个不同世代的LLM生成文本，使用生态学和信息论中的多样性指标量化语法结构和词汇类型的分布变化。

**Result:** 研究表明在给定时间段内英语新闻文本变化很小，而新型LLM相比旧模型表现出更低的句法多样性和词汇多样性，特别是词汇多样性下降明显。

**Conclusion:** 指令调优虽然提升了模型的连贯性和提示遵循能力，但可能缩小了模型输出的表达范围，为未来研究指令调优对语言表达能力的影响提供了方向。
...
> 该研究质疑了LLM后训练中'定位-更新'范式的基本假设，发现神经电路在参数更新中会'自由演化'，静态机制存在时间滞后性，需要前瞻性的预测框架。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM后训练主流的'定位-更新'范式依赖于从静态参数推导的机制来指导动态参数更新，但这一基本假设尚未得到验证，其可靠性存疑。

**Method:** 通过系统追踪Transformer电路在监督微调过程中的结构演化，提出了电路距离、电路稳定性和电路冲突三个新指标，从神经迁移、语义稳定性和跨任务干扰三个维度分析电路演化。

**Result:** 实证发现电路在参数更新过程中固有的'自由演化'特性，证明从当前状态提取的静态机制存在时间滞后性，无法有效指导未来状态，揭示了现有方法中'效力错觉'的本质。

**Conclusion:** 研究强调了机制定位中需要'预见性'的必要性，破除了静态指导动态的误解，为未来研究提出了预测性框架的方向。
...
> BEACON框架通过里程碑引导的策略学习解决了长跨度智能体强化学习中的信用错误分配和样本效率问题，在多个任务中显著提升了性能和样本利用率。

<details>
  <summary>Details</summary>

**Motivation:** 长跨度任务中，传统强化学习面临两个核心挑战：信用错误分配（早期正确动作被终端失败惩罚）和样本低效（成功轨迹稀少导致学习信号缺失）。

**Method:** BEACON利用任务的组合结构，在里程碑边界分割轨迹，在片段内进行时间奖励塑造以认可部分进展，并采用双重尺度估计优势值，防止远程失败污染局部动作评估。

**Result:** 在ALFWorld、WebShop和ScienceWorld上，BEACON一致优于GRPO和GiGPO。特别是在长跨度ALFWorld任务中，成功率达92.9%（GRPO为53.5%），有效样本利用率从23.7%提升到82.0%。

**Conclusion:** 以里程碑为锚点的信用分配是训练长跨度语言智能体的有效范式，为解决长期依赖的强化学习问题提供了新思路。
...
> 本文发现多模态知识编辑后的模型存在实体身份混淆（EIC）问题，通过构建EC-Bench基准测试分析其原因，并提出约束编辑到图像-实体处理阶段的缓解策略。

<details>
  <summary>Details</summary>

**Motivation:** 多模态知识编辑旨在修正已部署的大型视觉语言模型的内部知识，但编辑后模型的行为模式尚未得到充分探索，存在系统性失效模式。

**Method:** 构建EC-Bench诊断基准来探测编辑前后图像-实体绑定的变化，分析发现现有方法无法区分图像-实体绑定和实体-实体关系知识，导致模型过度拟合实体-实体关联作为捷径。

**Result:** 研究揭示了EIC的根本原因，提出将编辑约束到模型图像-实体处理阶段的缓解策略，能有效减少实体身份混淆现象。

**Conclusion:** 基于研究结果，讨论了忠实多模态知识编辑的设计原则，为未来研究提供了方法论指导。
...
> MemReranker是一个专为智能体记忆系统设计的重排序模型，通过多阶段LLM知识蒸馏技术，显著提升了记忆检索的准确性和推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的通用重排序模型在智能体记忆场景中存在三个核心问题：相关性分数校准不当导致阈值过滤困难；面对时间约束、因果推理等复杂查询时排序性能下降；无法利用对话上下文进行语义消歧。这些问题使得检索结果虽然在语义上高度相关，却往往缺乏回答问题的关键信息。

**Method:** MemReranker基于Qwen3-Reranker，采用多阶段LLM知识蒸馏技术构建。方法包括：多教师成对比较生成校准的软标签；BCE点蒸馏建立良好分布的分数；InfoNCE对比学习增强难样本区分能力。训练数据结合通用语料库和记忆特有的多轮对话数据，涵盖时间约束、因果推理和指代消解等场景。

**Result:** 在记忆检索基准测试上，MemReranker-0.6B大幅超越BGE-Reranker，与开源4B/8B模型及GPT-4o-mini在关键指标上相当。MemReranker-4B进一步达到0.737 MAP，多个指标与Gemini-3-Flash持平，同时推理延迟仅为大型模型的10-20%。在金融和医疗垂直领域基准测试中保持了与主流大型参数重排序模型相当的泛化能力。

**Conclusion:** MemReranker通过创新的蒸馏策略和针对性的数据构建，有效解决了智能体记忆系统中传统重排序模型的局限性，在保持较高性能的同时显著降低了推理成本，为实际应用提供了可行的高效解决方案。
...
> 本文提出了IRC-Bench基准数据集，用于评估在回忆录转录文本中识别隐式实体的能力，填补了处理非局部性实体引用的研究空白。

<details>
  <summary>Details</summary>

**Motivation:** 人们在回忆个人经历时经常通过上下文线索而非明确名称来间接指代人物、地点和事件，这种隐式引用在治疗、存档和社交场景中至关重要，但现有的实体识别方法无法有效处理这种非局部性的推理任务。

**Method:** 构建了IRC-Bench基准数据集，包含25,136个样本，每个样本包含显式提及目标实体的叙事文本和移除直接提及的省略实体文本对。评估了LLM生成、密集检索、RAG和微调等19种配置方法的性能。

**Result:** QLoRA适配的Llama 3.1 8B模型在开放世界设置中表现最佳（精确匹配38.94%，Jaccard相似度51.59%），而微调的DPR在封闭世界检索中领先（Hit@1为35.38%，Hit@10为71.49%）。

**Conclusion:** IRC-Bench为隐式实体识别研究提供了重要的评估基准，实验结果表明当前的模型在处理非局部性实体引用方面仍有很大改进空间，需要在推理能力和上下文理解方面进一步提升。
...
> 提出了难负样本字幕(HNC)数据集，通过自动构建难负样本来提升视觉-语言模型在细粒度跨模态理解上的表现。

<details>
  <summary>Details</summary>

**Motivation:** 传统的图文匹配(ITM)方法使用网络收集的图文对数据，但这些数据间关联性较弱，导致模型缺乏对跨模态语义的细粒度理解能力。

**Method:** 1) 自动创建包含被破坏的难负样本字幕的HNC数据集用于ITM训练；2) 提供一个手动创建的测试集，用于在具有不同组成复杂度水平的跨模态不匹配任务上进行模型基准测试。

**Result:** 在HNC上训练能够有效提升模型在诊断任务中检测不匹配的零样本能力，并在噪声视觉输入场景下表现鲁棒；同时HNC模型在微调初始化方面表现出色。

**Conclusion:** HNC数据集和方法为提升视觉-语言模型的细粒度跨模态理解提供了有效途径，在零样本检测和鲁棒性方面具有优势。
...
> 本文提出了A²TGPO算法，通过改进信息增益(IG)信号的归一化、累积和裁剪机制，解决了基于LLM的智能体多轮交互中的过程信用分配问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的强化学习方法在评估LLM智能体多轮交互中个体调用的贡献时面临挑战：外部奖励模型消耗资源，树状结构展开限制了轨迹多样性，而信息增益信号存在归一化失真、优势幅度漂移和固定裁剪范围等问题。

**Method:** A²TGPO包含三个关键技术：(i) 轮组归一化：在同一交互深度的组内归一化IG；(ii) 方差重缩放折扣累积：用累积项平方根归一化保持优势幅度可比性；(iii) 自适应轮级裁剪：基于归一化IG动态调整裁剪范围。

**Result:** 该方法保留了IG作为内在信号的优势，同时系统性地解决了其在归一化、累积和控制方面的三大挑战。

**Conclusion:** A²TGPO为多回合LLM智能体的强化学习提供了一种更有效且高效的过程信用分配方案，无需外部评估器即可准确评估个体调用的贡献。
...
> 本文提出了TIDE模型，通过引入EmbeddingMemory机制来解决现代大语言模型中单次token注入设计的罕见token问题和上下文崩溃问题，在多个任务上提升了性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM设计中token索引只在输入层查找一次就被丢弃，这会导致两个结构性问题：1）罕见token由于Zipf分布导致训练不充分；2）模型参数有限时，分布相似的token被映射到难以区分的隐藏状态。

**Method:** 提出TIDE架构，使用EmbeddingMemory机制，包含K个独立的MemoryBlocks，将token索引映射为上下文无关的语义向量，通过带有可学习null bank的深度条件softmax路由器在每一层注入这些信息。

**Result:** 理论和实验证明TIDE能够有效解决单次token身份注入相关的问题，并在语言建模和下游任务中提升了性能表现。

**Conclusion:** 通过重新设计token信息的注入方式，TIDE为解决LLM训练中的结构性偏差提供了新思路，证明了多层token注入的必要性和有效性。
...
> UniPrefill是一种适用于几乎任何模型架构的长文本预填充加速框架，通过token级计算加速实现最高2.1倍的TTFT加速，并能无缝集成到vLLM推理引擎。

<details>
  <summary>Details</summary>

**Motivation:** 现有长上下文预填充加速方法主要针对稀疏注意力机制，在混合架构下性能显著下降，且不兼容连续批处理，难以集成到现代推理引擎中。

**Method:** 提出UniPrefill框架，在token级别直接加速模型计算，实现为连续批处理运算符，扩展vLLM的调度策略以支持预填充-解码协同处理和张量并行。

**Result:** 在TTFT上实现最高2.1倍加速，随着并发请求数增加，加速效果更加明显。

**Conclusion:** UniPrefill成功解决了现有预填充加速方法的架构兼容性问题，为长上下文推理提供了高效且易于集成的解决方案。
...
> 本文提出了一个基于异构集成模型的多语言网络极化内容检测系统，通过结合XLM-RoBERTa和mDeBERTa模型，采用独立任务建模和类别加权技术在22种语言上实现了有效的极化内容检测。

<details>
  <summary>Details</summary>

**Motivation:** 针对多语言、多文化、多事件环境下的网络极化内容检测需求，解决严重标签不平衡问题，提升在不同语言和文化背景下的极化内容识别准确性。

**Method:** 采用异构集成方法，结合XLM-RoBERTa-large和mDeBERTa-v3-base多语言预训练模型，探索多任务学习、翻译数据增强和类别加权等技术，最终发现独立任务建模结合类别加权效果最优。

**Result:** 实验结果表明，独立任务建模配合类别加权策略比多任务学习更有效，在22种语言的极化内容检测任务中取得了良好的分类性能。

**Conclusion:** 该研究为多语言环境下的极化内容检测提供了有效的解决方案，证实了异构集成和类别加权技术在处理标签不平衡问题上的优势。
...
> 研究发现强化学习对大语言模型推理能力的提升实际上是通过修正模型已经知道的路径，而非教授新策略。作者提出了ReasonMaxxer方法，仅在熵值高的决策点应用对比损失，无需强化学习训练环路，就能达到甚至超越完整RL的性能，训练成本降低约三个数量级。

<details>
  <summary>Details</summary>

**Motivation:** 传统强化学习被用于提升大语言模型的推理能力，但有证据表明RL并没有教授新的策略，只是重新分配模型已有解决方案的概率质量。这引发了一个核心问题：如果RL只是引导模型走向已知的路径，那么RL优化环路本身是否必要？

**Method:** 通过跨多个模型家族和RL算法的token级别分析，发现RL的有益影响是稀疏且可预测的修正，集中在高熵决策点（模型不确定选择哪条分支的位置）。基于此发现，开发了ReasonMaxxer方法，仅在熵门控的决策点应用对比损失，使用几百个基础模型推演，无需在线生成。

**Result:** 实验表明，RL只影响了1-3%的token位置，且提升的token始终在基础模型的前5个候选中。在三个模型家族、六个规模和六个数学推理基准测试中，ReasonMaxxer匹配或超越了完整RL的性能，同时只需要几十个问题和几分钟的单GPU训练。

**Conclusion:** 研究将推理改进重新定义为稀疏策略选择而非能力获取。ReasonMaxxer证明了可以通过识别和修正模型熵值高的决策点来实现推理能力提升，无需复杂的RL训练环路，大幅降低了训练成本和复杂度。
...
> 该论文提出了一个针对阿拉伯语方言对话的语义分割基准数据集，并开发了一种在低资源口语环境中表现更好的分割模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的语义分割模型主要在高资源书面文本上开发评估，在处理具有非正式语法、语码转换和语篇结构标记薄弱的阿拉伯语方言等低资源口语变体时效果受限。

**Method:** 构建了一个包含1000多个样本的多体裁基准数据集，涵盖电话对话、语码转换播客、广播新闻和小说对话；提出了一种针对局部语义连贯性和语篇不连续性的鲁棒分割模型。

**Result:** 实验表明，在现代标准阿拉伯语新闻语料上表现良好的模型在方言转录语音上性能下降；提出的模型在方言非新闻体裁上持续优于强基线方法。

**Conclusion:** 该基准数据集和方法可推广到其他低资源口语语言，为口语语篇分析提供了新资源和技术支持。
...
> 研究分析了评估标准修改对人类和自动评分器（LLM）的一致性影响，发现提供示例、增加上下文和减少位置偏差能提高一致性，而更高的标准复杂度和保守的聚合方法会降低一致性。

<details>
  <summary>Details</summary>

**Motivation:** 自动评分器（LLM-as-judge）在评估和内容审核中应用日益广泛，但缺乏统计分析研究评估标准修改如何影响人类与自动评分器之间的评分一致性。总体性评估标准可能因复杂性和主观性导致解释不一致，而分析性评估标准虽然可以提高个体准确率，但可能造成评分分歧。

**Method:** 通过研究论文评分和指令遵循评估两个领域，比较了总体性和分析性评估标准对人类-自动评分器一致性的影响。分析了提供代表性示例、增加上下文、减少位置偏差、评估标准复杂度和聚合方法等因素的作用。

**Result:** 提供代表性示例和额外上下文、减少评估标准中的位置偏差能够显著提高人类与自动评分器的一致性；而更高的评估标准复杂度和保守的聚合方法则会降低一致性。不同领域和评估标准的表现存在差异。

**Conclusion:** 为了设计和部署可靠的自动评分器，实践者应该仔细分析特定领域和评估标准的性能，通过优化评估标准设计（如添加示例、简化复杂性）来提高人类与自动评分器之间的一致性，而不是简单地追求个体评分准确率。
...
> LatentRAG提出将推理和检索从离散语言空间转移到连续潜在空间，通过直接生成潜在思想标记而非自然语言，实现类似智能体RAG的性能同时将推理延迟降低约90%。

<details>
  <summary>Details</summary>

**Motivation:** 传统的单步RAG难以处理复杂问题，而多步智能体RAG虽然能处理复杂问题，但通过逐词生成中间思想和子查询导致推理延迟过高，限制了实际应用。

**Method:** LatentRAG框架在潜在空间中进行推理和检索，直接从隐藏状态生成潜在标记，将LLM与密集检索模型在潜在空间对齐，支持端到端联合优化，并引入并行潜在解码机制增强透明度。

**Result:** 在七个基准数据集上的广泛实验表明，LatentRAG达到了与显式智能体RAG方法相当的性能，同时将推理延迟降低约90%，大幅缩小了与单步RAG的延迟差距。

**Conclusion:** 潜在空间方法为复杂问题理解提供了高效解决方案，在保持性能的同时显著减少推理时间，为检索增强生成的实际部署开辟了新途径。
...
> 本文发现现有AI文本检测器因在隐藏空间非均匀分布的信号上简单平均分数而导致辛普森悖论，提出基于贝叶斯决策理论的学习局部校准方法，显著提升检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型发展，可靠区分人类文本和AI生成文本具有重大社会意义。现有主流方法依赖似然假设，但普遍存在性能不足的问题。

**Method:** 揭示检测器隐藏空间中区分人类和机器文本的token级信号分布不均匀，导致简单平均产生辛普森悖论。提出学习轻量级预测器来估计隐藏空间位置条件下的分数分布，并聚合校准后的对数似然比。

**Result:** 该方法在所有基线检测器和数据集上都显著改善性能，如将Fast-DetectGPT在GPT-5.4文本上的AUROC从0.63提升至0.85，新提出的局部校准DMAP检测器达到最先进水平。

**Conclusion:** 核心贡献不是新检测器，而是精确诊断现有检测器性能不足的根本原因，并提供与任何token平均流程兼容的模块化解决方案，为社区后续发展奠定基础。
...
> 提出了一种无监督多语言笑声分割方法，将笑声检测视为基于能量音频序列的异常检测问题，在非英语环境下优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有笑声检测方法依赖昂贵的手工标注且主要基于英语语境，缺乏对多语言环境的优化，而笑声作为跨文化通用交流信号具有重要意义。

**Method:** 使用BYOL-A编码器学习音频表示，基于能量分割音频序列，然后应用Isolation Forest进行异常检测来实现无监督的多语言笑声分割。

**Result:** 在四个数据集（包括单口喜剧、情景喜剧和AudioSet短音频）上测试，结果表明现有方法在多语言环境下未优化，而提出的方法在非英语场景中表现更优。

**Conclusion:** 该方法有效解决了多语言环境下笑声检测的标注依赖问题，为跨文化语音分析提供了新的无监督解决方案。
...
> 首项针对有害语言检测数据集的大规模分析研究，揭示了标注者特征、语言属性及其交互作用的重要影响，同时表明不同数据集间的效应模式存在显著差异。

<details>
  <summary>Details</summary>

**Motivation:** 虽然NLP领域已经认识到人类标注差异的重要性并收集了富含标注者信息的数据集，但对于标注者特征、文本语言属性及其交互关系的研究仍然缺乏。特别是在有害语言检测这一主观性强的任务中，需要更深入理解各种因素如何影响标注结果。

**Method:** 研究者对四个有害语言检测参考数据集进行了大规模统计分析，全面整合了标注者特征（社会人口统计、态度等）、文本语言属性及其交互作用，并考察了这些因素在不同数据集中的表现。

**Result:** 研究发现交互作用至关重要，揭示了先前工作忽略的交叉效应；词汇线索和标注者态度扮演重要角色；但效应模式在不同数据集间存在显著差异。

**Conclusion:** 研究结果提醒我们在模型泛化和迁移性方面需要谨慎，单一数据集发现的模式不能轻易推广到其他场景，强调了考虑因素交互作用和数据集特性的必要性。
...
> 本文提出了一套完整的工具集成推理(TIR)训练方法，通过监督微调和强化学习相结合的方式，使思维模型能够有效使用工具而不会损失其纯文本推理能力，在数学推理基准上实现了开源模型的SOTA表现。

<details>
  <summary>Details</summary>

**Motivation:** 研究发现，即使强大的思维模型几乎不实际调用工具，启用工具的评估反而会降低推理性能，这与直觉相矛盾。因此需要找到方法让模型学会自然使用工具，同时保持原有的文本推理能力不退化。

**Method:** 提出包含四个关键步骤的综合TIR方案：(1)监督微调使用适合工具增强解决方案的教师轨迹；(2)控制工具使用轨迹比例以防止纯文本推理能力的灾难性遗忘；(3)优化pass@k和响应长度而非训练损失；(4)基于合适SFT初始化和防止模式崩溃保护措施的稳定强化学习阶段。

**Result:** 在Qwen3思维模型的4B和30B规模上应用该方法，在多个基准测试中取得了开源模型的SOTA表现，其中在AIME 2025上分别达到96.7%和99.2%的准确率。

**Conclusion:** 本文的综合TIR方案成功解决了工具集成推理中的遗忘问题，证明了通过系统性的监督微调和强化学习设计，可以让思维模型既掌握工具使用能力，又保持强大的纯文本推理性能。
...
> 研究发现，不同开源语言模型在面对评估、部署和中性情境时表现出显著差异的安全行为异质性，其中OLMo-3-Instruct是唯一在评估情境下更加谨慎的模型，而其他模型则表现出部署谨慎性。

<details>
  <summary>Details</summary>

**Motivation:** 传统安全基准测试被视为模型部署后行为的可靠证据，但这种推断存在缺陷，因为模型行为可能因是否被明确标注为'评估'而改变。需要建立系统方法来评估这种情境依赖效应。

**Method:** 提出评估-情境发散概念，设计配对提示协议，控制转译变异、基准熟悉度和评判者框架敏感性。在5个开源模型家族的5个指令微调检查点+OLMo-3基础/指令消融实验中测试20个配对项目，每个检查点生成840次回复。

**Result:** OLMo-3-Instruct是唯一评估谨慎模型（评估框架使拒绝率提高11.8pp），其他模型如Mistral-Small-3.2、Phi-3.5-mini和Llama-3.1-8B均为部署谨慎模型。OLMo-3基础模型也表现出部署谨慎性，表明对齐阶段导致行为翻转。评判者依赖性研究发现不同安全分类器测量的安全构念存在差异。

**Conclusion:** 语言模型的安全行为存在显著的情境依赖异质性，单一的评估框架可能无法准确预测模型在真实部署中的表现，需要考虑多维度评估和跨验证机制。
...
> MANTRA是一个自动从自然语言手册生成机器可检查合规性基准的框架，通过符号世界模型和SMT求解确保LLM工具代理遵守程序规则，支持长手册并提供复杂任务生成。

<details>
  <summary>Details</summary>

**Motivation:** 工具使用LLM代理在需要遵守严格程序手册的部署中日益增多，但确保其合规性具有挑战性，因为手册是为人类编写的自然语言，而代理行为表现为工具调用执行轨迹。现有评估方法要么不具扩展性，要么对复杂手册缺乏可靠性。

**Method:** MANTRA框架独立生成：(1)捕获程序依赖关系的符号世界模型；(2)给定任务的轨迹级合规性检查集；并使用SMT求解验证一致性。通过结构化修复循环解决不一致性，人工干预仅作为备用。支持任意域、长程序手册，提供可调的任务复杂度概念用于自动推导挑战性任务。

**Result:** 使用MANTRA构建了包含6个域285个任务的新基准套件，扩展到50+页手册，人工成本极低。经验表明，与现有基准相比，合规性检查更丰富，约束执行更强。检查的粒度可用于调试代理故障模式。

**Conclusion:** 将自动基准生成与形式化验证方法相结合，实现了工具使用代理的可扩展和可靠基准测试，为评估复杂、长时间范围内的手册合规性提供了新的解决方案。
...
> 本文提出了一种名为SKOP的新型激活引导方法，通过保持关键令牌的注意力模式来解决传统引导方法导致的推理和检索性能下降问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统的激活引导方法虽然能够控制大语言模型的行为，但常常会导致推理和检索性能下降。这主要是因为引导向量会改变查询-键匹配，使注意力从上下文重要的令牌转移到信息量较少的令牌上，造成注意力重路由问题。

**Method:** 提出SKOP（通过键正交投影进行引导）方法，该方法保留模型在推理和检索所依赖的小部分关键令牌上的注意力模式，同时允许在较不重要的尾部令牌之间重新分配注意力。通过键正交投影来约束有害的注意力重路由，而不消除引导效果。

**Result:** 在多个引导基准测试中，SKOP实现了最佳的引导-效用权衡，将效用退化降低了5-7倍，同时保持了超过95%的原始引导效果。在长上下文检索环境中，当传统引导方法失效时，SKOP通过避免注意力重路由能够保持稳健性能。

**Conclusion:** SKOP方法有效解决了激活引导中的注意力重路由问题，在保持行为控制能力的同时显著改善了大语言模型的推理和检索性能，为大语言模型的安全引导提供了新的技术路径。
...
> SEQUOR是一个评估长多轮对话中指令遵循能力的自动基准测试，结果表明模型在长对话中的指令遵循准确率显著下降。

<details>
  <summary>Details</summary>

**Motivation:** 现有指令遵循基准主要关注单轮或短多轮场景，缺乏对长时程指令遵循能力的评估，而实际对话中助手需要持续跟踪和遵循用户的约束条件。

**Method:** 构建SEQUOR基准测试，使用从真实对话中提取的约束条件创建模拟角色驱动的交互对话，评估模型在不同长度和约束复杂度下的表现。

**Result:** 单约束下，对话长度增加导致准确率下降超过11%；多约束同时遵循时准确率下降超过40%；约束添加或替换场景下准确率下降超过9%。

**Conclusion:** 当前模型在长多轮对话的指令遵循方面仍存在显著困难，SEQUOR为更好的评估助手指令遵循能力提供了有效工具。
...
> 提出一种名为GATHER的图检索方法，专门解决零样本单细胞类型标注中的超实体查询问题。该方法通过图全局遍历识别拓扑收敛点作为高信息超实体，在无需LLM参与检索的情况下显著提高了标注准确率并大幅降低了计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于知识图谱的RAG方法在处理包含数十到数百个基因的查询时存在根本性挑战。这些方法依赖局部、逐个实体的探索策略，无法有效捕捉基因间的协同效应，导致可扩展性差且LLM推理成本高昂。

**Method:** GATHER采用以收敛为中心的检索策略，在自建的细胞中心生物知识图谱VCKG上执行全局多源图遍历，识别可从多个输入基因共同到达的拓扑收敛点。通过节点和路径重要性评分选择高信息证据，完全避免检索阶段的LLM调用。

**Result:** 在Immune和Lung两个数据集上，GATHER超越了ToG、ToG-2、RoG、PoG等强基线方法，分别达到27.45%和59.64%的精确匹配准确率，每个样本仅需1次LLM调用，而基线方法需要2-61次调用。

**Conclusion:** 收敛节点能够将多实体信号压缩为紧凑、高信息的证据，比多跳路径传递更多信息。这种方法为局部实体推理提供了一种高效的全局替代方案，特别适合处理生物医学科研中基因协同查询的复杂场景。
...
> 本文提出了Mindscape Activation Signature (MiA-Signature)，一种压缩的全局激活模式表示方法，通过选择高层概念来近似完整激活状态的效果，在RAG和智能体系统中取得了显著的性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 认知科学研究表明，可报告的意识访问与分布式记忆系统的全局激活相关，但个体无法直接访问或枚举所有激活内容。这种张力的存在暗示认知可能依赖于一种紧凑表示来近似激活对下游处理的全局影响。

**Method:** 通过基于子模函数的选择方法提取激活上下文空间的高层概念来构建MiA-Signature，可选择使用工作记忆通过轻量级迭代更新进行精化，最终结果作为条件信号近似完整激活状态的效果。

**Result:** 将MiA-Signature集成到RAG和智能体系统中，在多个长文本理解任务上获得了一致的性能提升。

**Conclusion:** MiA-Signature提供了一种计算可行的方法来近似全局激活状态的影响，为处理长文本理解问题提供了新的解决方案，验证了认知科学启发的人工智能方法的有效性。
...
> 本研究提出了一种结合规则过滤和LLM分类的大规模新词检测管道，从5.27亿条Reddit帖子中识别出599个真实新词。

<details>
  <summary>Details</summary>

**Motivation:** 随着社交媒体的快速发展，大量新词不断涌现，但缺乏有效的方法来自动检测这些词汇创新。现有方法难以处理海量数据，并且准确率有限。

**Method:** 构建了一个可扩展的模块化管道，基于语法和超语法学两种形态学框架，设计了四类分类方案（新词、实体、外来词、无）。首先进行规则过滤大幅缩减候选词，然后使用多个LLM进行独立投票分类，最后人工验证。

**Result:** 从5.27亿Reddit帖子提取1.246亿唯一词元，通过99.99%的筛选得到1021个候选词，经人工验证确认599个(58.7%)为真实新词。发现不同LLM模型间存在显著分类分歧。

**Conclusion:** 该管道证明了结合规则和LLM方法在大规模新词检测中的可行性，同时揭示了自动检测新词的挑战性。代码和标注数据已开源以促进后续研究。
...
> 本研究探索了基于传统机器学习的文本和语言特征在虚假新闻检测中的应用，发现随机森林效果最佳，单独使用文本特征或语言特征均可提升检测效果，但结合使用效果改善有限。

<details>
  <summary>Details</summary>

**Motivation:** 尽管有实证证据表明文本和语言特征有助于区分真实与虚假新闻，但在虚假新闻检测领域，对这些内容特征的研究仍显不足。

**Method:** 研究在COVID-19期间收集的新数据集上，使用词二元组、词性分布等特征，采用决策树、K近邻、逻辑回归、支持向量机和随机森林等传统机器学习方法进行实验。

**Result:** 随机森林表现最佳，支持向量机紧随其后。文本特征和语言特征单独使用都能有效提升虚假新闻检测效果，但结合使用显著改善有限。词二元组与词性标签的效果也存在差异。

**Conclusion:** 传统机器学习方法结合文本和语言特征能够成功用于虚假新闻检测，为这一领域提供了不同于深度学习的研究思路。
...
> 研究分析了大型语言模型在情绪驱动对话环境中的自回答一致性，发现模型容易受到查询中错误信息的影响，尤其是在中等情绪情境下表现较差。

<details>
  <summary>Details</summary>

**Motivation:** 随着LLMs在情感敏感场景中的广泛应用，需要评估模型在情绪化对话中的推理一致性和对错误预设的抵抗力，确保其在高风险情境中的可靠性。

**Method:** 将LLM生成的文本作为查询重新输入同一模型，评估其回答一致性。设计了三个包含错误预设的查询，涵盖极端和中等情绪维度，测试了Claude-3.5-haiku、GPT4o-mini和Mistral-7B三个模型。

**Result:** LLMs的整体表现低于平均水平，极易受到查询中错误信念的影响，这种脆弱性在中等情绪内容中尤为突出。基于注意力分数的分析显示模型从评估性转向生成性优先级。

**Conclusion:** 研究结果揭示了LLMs在情绪敏感环境中的局限性，强调了在部署模型时需要考虑其对错误预设的敏感性和一致性问题，特别是在高风险应用场景中。
...
> 论文提出Litespark-Inference框架，通过定制SIMD内核优化三元模型推理，在CPU上实现9.2倍TTFT加速、52倍吞吐量提升和14倍内存减少。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM推理成本高昂，需要数据中心GPU或云API，而三元模型虽能消除浮点乘法但现有框架未能有效利用其结构优势，导致个人电脑AI资源利用率低。

**Method:** 开发定制SIMD内核，将矩阵乘法转换为简单的加减操作，利用现代CPU的整数点积指令，实现三元模型的高效推理，并构建pip可安装的Hugging-Face集成框架。

**Result:** 在Apple Silicon上相比标准PyTorch推理，时间到首个令牌加速9.2倍，吞吐量提升52倍，内存减少14倍，在Intel和AMD处理器上也有类似性能提升。

**Conclusion:** Litespark-Inference成功 exploited三元模型的独特结构，通过CPU优化显著提升推理效率，使个人设备能够高效运行AI工作负载，为普惠AI提供了可行路径。
...
> 研究发现语言模型 surprisal 与隐喻新颖性的关系可能受到词汇频率的混淆影响，词汇频率实际上是更强的预测因子。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型 surprisal 被广泛用作语境可预测性的代理指标，并被报道与隐喻新颖性判断相关，但 surprisal 与词汇频率紧密相关，需要探究这种交互作用。

**Method:** 使用两种不同的词汇频率测量方法，分析8个不同大小的Pythia模型和154个训练检查点的 surprisal 估计，研究隐喻新颖性评分与 surprisal 和词汇频率的关系。

**Result:** 在所有设置中，词汇频率是比 surprisal 更强的隐喻新颖性预测因子；在训练过程中，suprisal-新颖性关联在早期阶段达到峰值然后下降，而 surprisal-频率关联在相似时间增加。

**Conclusion:** 常用的最优LM surprisal 设置可能错误地将语境可预测性与隐喻新颖性和处理难度联系起来，而词汇频率可能是主要的潜在影响因素。
...
> 该论文发现了LLM智能体在处理隐性冲突记忆更新时的不足，提出STALE基准测试评估此能力，并设计了CUPMem原型作为解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM智能体的记忆系统主要集中在静态事实检索上，缺乏在遇到新证据时动态修正已有信念的能力，特别是当后期观察使早期记忆失效时（隐性冲突场景）。

**Method:** 构建包含400个专家验证冲突场景的STALE基准测试，提出三维探测框架：状态解析、前提抵抗和隐式策略适应；设计CUPMem原型，通过结构化状态整合和传播感知搜索增强写入时修订。

**Result:** 评估显示主流模型在处理隐性冲突时表现不佳，即使最佳模型准确率仅55.2%，模型常接受过时假设且难以识别状态变化对相关记忆的影响。

**Conclusion:** LLM智能体在动态记忆更新方面存在显著缺陷，显式状态裁决是构建鲁棒智能体记忆的有前景方向，CUPMem原型为状态感知记忆系统提供了初步基线。
...
> 本文提出Token-Superposition Training (TST)方法，通过高效的超位置训练阶段和恢复阶段，在无需修改模型架构或并行策略的情况下，将大语言模型预训练时间减少至多2.5倍。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型预训练成本极高且效率低下，需要复杂的修改才能实现高数据吞吐量，需要一种简单且高效的方法来提升训练效率。

**Method:** TST采用两阶段训练：(i)超位置阶段将多个连续token组合成一个bag，使用多热交叉熵目标进行高效训练；(ii)恢复阶段回到标准训练。该方法无需修改并行策略、优化器、分词器、数据或模型架构。

**Result:** 在270M、600M参数规模及3B、10B A1B混合专家模型上验证，TST在不同设置下表现稳健，在损失相等条件下，10B规模上预训练时间减少高达2.5倍。

**Conclusion:** TST是一种简单有效的drop-in方法，显著提升预训练数据吞吐量，在保持或改善模型性能的同时大幅降低训练成本，适用于各种规模的模型。
...
> Cola DLM是一种分层潜在扩散语言模型，通过将文本生成分解为语义组织和文本实现两个层次，实现了高效的非自回归文本生成，并在大规模实验中展现出良好的扩展性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前的大型语言模型虽然取得了成功，但都局限于固定的从左到右的自回归生成范式，现有的替代方案难以同时实现生成效率、可扩展的表示学习和有效的全局语义建模。

**Method:** 提出Cola DLM分层潜在扩散模型：首先使用文本VAE学习稳定的文本到潜在空间的映射，然后在连续潜在空间中使用块因果DiT建模全局语义先验，最后通过条件解码生成文本。该设计将全局语义组织与局部文本实现分离。

**Result:** 通过涵盖4个研究问题、8个基准测试、严格匹配的约20亿参数自回归和LLaDA基线以及扩展到约2000 EFLOPs的扩展曲线实验，验证了Cola DLM的有效配置和强大的文本生成扩展行为。

**Conclusion:** 分层连续潜在先验建模是严格分词级语言建模的有效替代方案，生成质量和扩展行为比似然更能反映模型能力，同时为离散文本和连续模态的统一建模提供了具体路径。
...
> Lighthouse Attention是一种基于对称选择的分层注意力算法，通过训练时自适应压缩和解压缩序列来降低因果Transformer的长序列训练计算复杂度。

<details>
  <summary>Details</summary>

**Motivation:** 因果Transformer在极长序列长度下训练时，缩放点积注意力(SDPA)的二次时间和内存复杂度成为性能瓶颈。

**Method:** 提出三种技术：(1) 亚二次分层预处理和后处理步骤，自适应压缩和解压缩序列；(2) 对称压缩策略，同时池化查询、键和值，保持从左到右的因果性；(3) 两阶段训练方法，大部分时间使用Lighthouse Attention，最后通过短时训练恢复全注意力模型。

**Result:** 小规模LLM预训练实验显示，相比全注意力训练，该方法能够实现更快的总训练时间，并在恢复阶段后获得更低的最终损失。

**Conclusion:** Lighthouse Attention是一种有效的训练时优化方法，能够显著降低长序列Transformer的训练成本，同时保证模型性能。
...
> 本研究比较了两种用于远程认知康复治疗自动化临床报告生成的方法——基于规则的模板系统和GPT-4零样本方法，发现模板系统在临床可靠性方面更优，而GPT-4输出更简洁，为医疗生成式AI的负责任应用提供了指导。

<details>
  <summary>Details</summary>

**Motivation:** 认知矫正治疗需求增长与言语治疗师短缺的矛盾推动了远程康复工具的发展，但产生的大量交互数据让临床医生难以高效审查，亟需自动化临床报告生成解决方案。

**Method:** 提出并比较两种方法：1) 基于规则的模板系统，将言语治疗领域知识编码为明确决策规则和验证模板；2) 基于GPT-4的零样本LLM方法。两者使用相同的预提取专家验证结构化变量，通过八名言语治疗师的九标准问卷进行评估。

**Result:** 模板系统在流畅性、连贯性和结果展示方面得分更高，GPT-4产生更简洁的输出。存在临床可靠性与语言质量的权衡，虽然各维度差异一致，但校正后均未达到统计学显著性。基于评估反馈提出了八项远程康复临床报告系统设计建议。

**Conclusion:** 该研究为低资源临床自然语言生成提供了可复制的方法论，结合了专家启发、分类驱动生成和多维人工评估，通过对比分析为生成式AI在医疗保健中的负责任应用提供了实践指导。
...
> UniSD是一个统一的自蒸馏框架，系统整合监督可靠性、表示对齐和训练稳定性机制，在无需外部强教师的情况下提升LLM性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有自蒸馏方法缺乏系统性研究，单一设计选择的有效性、作用和相互关系不明确，亟需统一框架来全面理解和优化自蒸馏过程。

**Method:** UniSD整合多种互补机制：多教师一致性、EMA教师稳定化、词级对比学习、特征匹配和差异裁剪，通过六个基准和三个模型家族的六种模型进行系统研究。

**Result:** UniSD揭示自蒸馏何时优于静态模仿，识别关键增益组件，构建的UniSDfull集成管道相比基线模型提升5.4分，相比最强基线提升2.8分。

**Conclusion:** 自蒸馏是一种实用且可调控的高效LLM适应方法，UniSD框架为不依赖外部强教师的模型适应提供了系统性解决方案。
...
> 本研究通过建立联合行动模型和MUM概念，系统分析了Algospeak（算法规避语言）中可理解性与可检测性之间的权衡关系，并构建了可重现的实验框架和700条COVID-19虚假信息数据集进行实证验证。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型语言模型在内容生成和审核中的广泛应用，Algospeak策略不断加剧规避者与检测器之间的共同演化，需要从理论层面理解和量化这种动态关系。

**Method:** 提出联合行动模型建模Algospeak动态，引入MUM（多数人可理解调制）概念，构建可重现框架生成意义保持的Algospeak变体，基于COVID-19虚假信息创建700条数据集，通过意义恢复和虚假信息分类评估七个语言模型。

**Result:** 揭示了可理解性与调制水平之间的特征关系，通过曲线拟合估计出MUM阈值，实现了对不同策略和模型的敏感性分析，验证了理论框架的有效性。

**Conclusion:** 该研究为理解Algospeak背后的动态机制奠定了基础，提供了完整的框架、数据集和实验设置，为未来相关研究提供了重要参考。
...
> 本研究提出了一种简化的人机交互工作流程，利用两个领域自适应解析器的一致性来进行第二语言韩语形态句法标注。

<details>
  <summary>Details</summary>

**Motivation:** 旨在解决第二语言韩语形态句法标注的效率问题，通过解析器一致性作为标注正确性的代理指标来实现半自动化标注。

**Method:** 比较两个领域自适应解析器的一致性与人工独立判断，验证方法可行性，并分析不一致案例的语言学分布特征。

**Result:** 解析器与人工判断显示强对应关系，支持半自动标注的可行性；不一致案例主要集中在语法关系区分和从句边界模糊等可预测的语言学领域。

**Conclusion:** 该方法为第二语言韩语UD标注提供了有效的半自动化解决方案，部分不一致案例可通过模型迭代优化解决，但深层表示挑战仍需进一步研究。
...
> 本研究提出了首个大规模LLM引用评估框架，通过AST解析器提取Markdown报告中的引用，并从链接可达性、内容相关性和事实准确性三个维度进行评估。实验发现即使是顶级模型，事实准确率仅为39-77%，且检索深度增加反而降低准确性。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型驱动的研究代理能够合成数百个网络源信息生成带引用的报告，但这些引用无法可靠验证。现有方法要么信任模型自我引用（存在偏见风险），要么使用不验证源可访问性、相关性和事实一致性的检索增强生成。

**Method:** 开发了基于AST解析器的源归属评估框架，可大规模提取和评估LLM生成的Markdown报告中的内联引用。通过获取实际引用内容，让评估者对每个引用进行三维度评判：链接可达性验证、内容相关性度量和事实一致性检查。使用基于规则的人工审查校准的LLM作为评估器。

**Result:** 评估了14个闭源和开源模型，发现顶级前沿模型链接有效率达94%以上、相关性达80%以上，但事实准确率仅为39-77%；少于一半的开源模型能在单次设置中成功生成引用报告；当工具调用从2次增加到150次时，两个前沿模型的事实检查准确率平均下降约42%。

**Conclusion:** 揭示了表面引用质量与事实可靠性之间的关键脱节，更多检索并不能产生更准确的引用。该框架为评估这种脱节提供了必要的基础设施，对提升LLM研究报告的可靠性具有重要意义。
...
> StraTA是一种通过引入显式轨迹层级策略来提升大语言模型在长时序决策任务中表现的框架，在多个基准测试中显著优于现有基线方法。

<details>
  <summary>Details</summary>

**Motivation:** 当前大语言模型作为智能体进行决策时，主要采用纯反应式方法，这削弱了在扩展轨迹上的探索和信用分配能力，导致长时序决策优化困难。

**Method:** 提出战略轨迹抽象(StraTA)框架，从初始任务状态采样紧凑策略，基于该策略调整后续动作，采用分层GRPO式rollout设计进行策略生成和动作执行的联合训练，并通过多样化策略回滚和关键自判断进行增强。

**Result:** 在ALFWorld、WebShop和SciWorld实验中，StraTA一致性地提高了样本效率和最终性能，在ALFWorld达到93.1%成功率，WebShop达到84.2%成功率，在SciWorld获得63.5%总体得分，超越前沿闭源模型。

**Conclusion:** StraTA通过引入显式的轨迹级策略解决了大语言模型智能体在长时序决策中的核心挑战，实现了显著的性能提升，为交互式智能体的优化提供了有效的新范式。
...
> 提出了一种新的仅使用正样本的策略优化框架POPO，通过有界重要性采样和孪生策略网络实现了比GRPO更好或相当的数学推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的GRPO方法中负样本无法区分失败严重程度，且在稀疏二元奖励下难以覆盖有意义的奖励信号，限制了强化学习在提升大语言模型推理能力方面的效果。

**Method:** POPO仅利用在线正样本进行学习，通过有界重要性采样实现梯度更新，并结合孪生策略网络和动量自适应机制以及表示空间中的有界相似性惩罚项来稳定策略优化过程。

**Result:** 在数学基准测试中，POPO达到与GRPO相当甚至更优的性能，特别在AIME 2025上使用Qwen-Math-7B达到了36.67%的准确率，优于GRPO的30.00%。

**Conclusion:** POPO通过仅使用正样本的创新设计，有效解决了负样本的局限性问题，在稀疏奖励环境下为大语言模型的数学推理能力提升提供了一个更有效且稳定的强化学习训练框架。
...
> EMO是一种模块化混合专家模型，通过文档级别的专家选择约束，实现了在保留仅25%专家时性能仅下降1%，远优于传统MoE模型。

<details>
  <summary>Details</summary>

**Motivation:** 当前大语言模型通常作为单体系统部署，即使应用只需要部分特定能力（如代码、数学或领域知识）也必须加载完整模型。现有MoE虽然理论上可以激活部分专家，但实际中限制专家子集会导致严重性能下降，限制了在内存受限环境下的实用性。

**Method:** 提出EMO模型，关键思想是鼓励相似领域的token依赖相似的专家。通过限制同一文档内的token从共享池中选择专家，同时允许不同文档使用不同的专家池。利用文档边界作为唯一约束，在预训练过程中自然形成连贯的专家分组。

**Result:** 预训练了1B激活参数、14B总参数的EMO模型。作为完整模型时达到标准MoE性能；选择性使用专家方面，保留25%(12.5%)专家时仅造成1%(3%)的绝对性能下降，而标准MoE在相同设置下完全失效。EMO的专家在语义层面（如数学、代码等领域）实现特化。

**Conclusion:** EMO证明了模块化、内存高效部署大型稀疏模型的可行性路径，为可组合架构开辟了新机会。专家从低层语法特化转向高层语义特化是重要突破。
...
> 该研究提出了一种基于布局感知的文档表示学习方法，用于开放式证件欺诈检测，通过DINOv3模型迁移学习，实现了跨境证件的布局分类和新型欺诈发现。

<details>
  <summary>Details</summary>

**Motivation:** 传统的证件欺诈检测是静态二分类问题，但欺诈攻击者会持续调整伪造策略，使得历史标签失效，且欺诈行为会以协调活动的形式大规模重复出现。因此需要研究开放式欺诈发现而非仅限于封闭集分类。

**Method:** 作者将DINOv3模型适配到文档领域，通过上下文感知的SimMIM微调和监督度量学习（使用复合损失函数促进类间分离性和类内紧致性）。模型仅使用美国身份证训练，后接轻量级MLP和softmax分类器进行预测。

**Result:** 在加拿大证件布局上达到99.83%的分类准确率。在20,448份加拿大身份证数据集中，发现了276起自适应物理欺诈案例，其中222起未被现有检测器发现。嵌入表示支持从单个确认的种子样本扩展到相关案例。

**Conclusion:** 布局感知的文档嵌入在分布偏移下为发现新型和大规模欺诈活动提供了生产环境适用的基础，有效解决了传统方法在自适应攻击面前的局限性。
...
> 本文提出了一种基于生成对抗网络(GAN)的反事实解释(CX)方法，用于医学图像的面向类别的特征归因，能够生成合理的反事实实例并提供自解释的类比说明。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可视化技术基于判别模型，仅突出参与分类决策的最小特征集，无法考虑所有显著物体。在医学成像中，需要全面了解影响分类的对象，以帮助放射科医生可视化异常情况。

**Method:** 构建基于GAN的反事实解释框架，采用循环一致性损失函数。同时提出了反事实实例(CI)生成方法和CI质量评估新技术。

**Result:** 在合成数据集、结核病数据集和BraTS数据集上的实验证实了方法的 efficacy。在BraTS数据集上产生了基线结果，验证了生成可信CI的重要性。

**Conclusion:** 该方法克服了传统技术的局限性，通过产生可信的反事实实例与解释相结合，提供了自解释的类比式解释，对医学图像分析具有重要价值。
...
> 论文提出了一种密度感知的后校准方法，通过分析DETR风格3D检测器中潜在对象查询的特征密度来实现分布偏移场景下的可靠不确定性估计。

<details>
  <summary>Details</summary>

**Motivation:** 现有3D目标检测器在分布偏移场景下缺乏良好的不确定性校准，传统后校准方法仅在分布内数据上有效，无法适应分布偏移场景，这对自主系统的安全性构成威胁。

**Method:** 利用DETR风格检测器中的对象查询特征（具有紧凑性、位置和类别感知特性），拟合密度估计器联合重新校准分类和边界框回归的不确定性，实现分布偏移场景自适应校准。

**Result:** 在多视角摄像头和激光雷达检测器上，该方法在分布内和分布偏移场景下均持续优于标准后校准方法。

**Conclusion:** 该方法成功解决了3D目标检测在分布偏移场景下的不确定性校准问题，为提升自主系统安全性提供了有效解决方案。
...
> ViTok-v2是一种支持原生多分辨率、采用DINOv3感知损失的大规模视觉Transformer图像标记器，在2B图像上训练至5B参数，在高分辨率图像重建和生成任务上达到最先进性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有ViT自动编码标记器在训练分辨率外性能下降，对抗损失阻碍了稳定扩展，且压缩比r存在重建-生成的权衡问题，改进重建质量是实现更优权衡的关键。

**Method:** 引入NaFlex机制实现跨分辨率和宽高比的泛化，采用新颖的DINOv3感知损失替代LPIPS和GAN目标实现稳定扩展，在20亿图像上训练并扩展到50亿参数规模。

**Result:** 在256p分辨率下匹配或超越现有最佳重建性能，在512p及以上分辨率超越所有基线模型，与流匹配生成器的联合缩进实验展示了同时扩展自动编码器和生成器可以推进权衡边界。

**Conclusion:** ViTok-v2通过原生分辨率支持和稳定的训练损失机制，成功解决了现有视觉Transformer标记器的限制，成为目前最大且性能最优的图像自动编码器，为高分辨率图像处理提供了新的解决方案。
...
> Open-SAT是一种无需训练的查询嵌入优化算法，通过LLM上下文指导改进开放词汇卫星图像检索，F1分数提升高达16.04%。

<details>
  <summary>Details</summary>

**Motivation:** 卫星应用中用户查询通常是开放词汇的自然语言，超出预定义类别，现有VLM模型如CLIP难以准确对齐卫星图像和查询，需要更好的开放词汇检索解决方案。

**Method:** Open-SAT采用训练无关的推理时优化策略：1) 使用VLM计算图像图块嵌入并存储在向量数据库；2) 检索时利用LLM通过融入目标物体及其周围环境信息来细化文本嵌入；3) 采用无阈值检索机制提升准确性和效率。

**Result:** 在三个公共基准测试中，Open-SAT将F1分数提升最多16.04%，同时检索的图像图块数量相当，验证了算法在开放词汇卫星图像检索中的有效性。

**Conclusion:** Open-SAT成功通过LLM指导优化查询嵌入，在无需额外训练或监督的情况下显著提升了开放词汇卫星图像检索性能，为实际卫星应用提供了有效解决方案。
...
> egenioussBench是一个基于地理空间参考数据的城市级视觉定位基准测试，包含3D网格和CityGML LoD2模型数据源，提供带有厘米级精确真值但不可共视的智能手机图像查询集。

<details>
  <summary>Details</summary>

**Motivation:** 传统基于SfM的视觉定位方法难以扩展到城市级规模，需要构建一个反映可部署测绘资产、支持真实可扩展性的基准测试，同时提供跨视图和跨域的现实挑战。

**Method:** 构建基于城市级航空3D网格和CityGML LoD2模型的基准测试，通过PPK和GCP/CP辅助调整获得厘米级精确、地图无关的真值，从2709张图像中通过估计全可见性矩阵和选择最大独立集派生不可共视子集，划分为包含42张不可共视图像的测试集和412张序列图像的验证集。

**Result:** 提供了一个包含公开排行榜的基准测试，使用多个姿态误差阈值的分箱指标和全局统计（中值、RMSE、异常值比例）进行评估，确保基于网格和LoD2方法之间的公平比较。

**Conclusion:** egenioussBench通过暴露现实的跨视图和跨域挑战，为推进大规模视觉定位提供了严谨且可扩展的路径。
...
> 本文为阿拉伯手语(ArSL)创建了首个高质量3D参数化注释Ishara-500数据集，并开发了专门的重建管道Tamaththul3D，实现了手部精确度的突破性提升。

<details>
  <summary>Details</summary>

**Motivation:** 全球约4亿阿拉伯使用者依赖阿拉伯手语及其方言，但该领域缺乏高质量的3D参数化注释和专门的头像生成重建方法，这严重影响了可访问性技术发展和文化保存工作。

**Method:** 首先为沙特手语数据集Ishara-500创建精确的SMPL-X参数化标注；其次开发Tamaththul3D重建管道，集成SMPLer-X进行身体估计、WiLoR进行手部精细化处理，并采用运动链手腕对齐和混合摆动-扭曲分解技术完成2D监督的关节优化。

**Result:** Tamaththul3D在保持竞争性身体姿势的同时，实现了最先进的手部准确度，相比先前方法提升最高达32%，建立了阿拉伯手语头像重建的首个全面框架。

**Conclusion:** 通过3D注释和Tamaththul3D管道的结合，为阿拉伯聋人社区建立了高保真度阿拉伯手语头像重建的完整框架，将推动新的可访问性技术发展和文化遗产保护工作。
...
> 本文提出了一种基于一致性学习的点云异常检测重构方法，通过单次或两次网络评估直接预测无异常几何，显著降低推理成本，实现比现有方法快80倍的运行速度。

<details>
  <summary>Details</summary>

**Motivation:** 随着三维传感在现代制造中的重要性日益增加，点云数据的可靠异常检测对高通量质量保证和过程控制至关重要，但现有方法在资源受限和延迟关键系统中的实际部署受到限制，且扩散模型的迭代去噪过程存在计算瓶颈。

**Method:** 通过一致性学习重新构建基于重构的异常检测方法，实现直接预测无异常几何；引入新型混合损失函数，显式强制重构指向干净数据。

**Result:** 在无GPU加速情况下，运行速度比当前最先进方法快80倍；在Anomaly-ShapeNet上以76.20% I-AUROC优于R3D-AD，在Real3DAD上保持72.80% I-AUROC的竞争力。

**Conclusion:** 所提出的方法实现了高效、低延迟的点云异常检测，能够在无人机、智能工业相机等边缘设备上进行实际部署，同时保持强大的检测性能。
...
> LAMP是一个通过早期分离观察者和目标运动来解决头戴式多相机3D人体运动跟踪挑战的新框架，采用

<details>
  <summary>Details</summary>

**Motivation:** 现有的单目视频方法在处理头戴式多相机系统时面临严重自运动、部分遮挡和训练数据缺乏等挑战，这些方法通常需要静态或缓慢移动的相机，无法有效利用多视角、已校准和已定位的输入数据

**Method:** LAMP采用两步处理流程：首先利用已知的设备6自由度运动和校准信息，将所有相机检测到的2D身体关键点转换到统一的3D世界参考系；然后通过端到端训练的时空变换器直接拟合3D人体运动到这个3D射线云

**Result:** LAMP在单目基准测试中达到了最先进的结果，在目标头戴式环境中显著优于基线方法
...
> GeoQuery是一种零样本地球观测图像检索系统，通过文本代理对齐技术实现对卫星图像的语义搜索，在灾害定位任务中达到31.6%的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 地球观测档案的语义搜索仍具挑战性，视觉基础模型缺乏自然语言理解能力，而完整的对比学习训练需要大量配对数据和计算资源，难以在全球规模应用。

**Method:** 生成10万个全球Sentinel-2瓦片的语言描述，优化文本生成提示以使文本嵌入空间距离与冻结CLAY视觉嵌入空间距离相关联；采用两阶段检索：首先在代理子集上进行文本相似性搜索，然后在全球CLAY嵌入上进行视觉最近邻搜索。

**Result:** 在涵盖英国洪水、美国野火和干旱的76个灾害位置查询中，GeoQuery在50公里范围内达到31.6%的准确率，其中洪水检测表现最佳（50公里内准确率50%）；已部署在ECHO危机响应系统中，成功识别了布里斯班2025年飓风期间的脆弱区域。

**Conclusion:** 当全对比学习训练不可行时，提示对齐代理为地球观测基础模型与操作检索之间提供了实用的桥梁，是一种实用且有效的解决方案。
...
> 提出一种主动相机可靠性监控框架，通过全局传感器健康指数(GSHI)在下游性能下降前预警相机故障。

<details>
  <summary>Details</summary>

**Motivation:** 安全关键的ADAS感知需要可靠的相机输入，但现有监控方法只能在下游性能下降后才检测传感器故障，缺乏主动预警能力。

**Method:** 引入全局传感器健康指数(GSHI)，使用风险感知的乘积公式聚合多种退化严重程度；设计轻量级多任务网络从单张RGB图像预测退化类型、严重程度、GSHI和空间不确定性图；使用物理和几何感知的合成监督训练，涵盖12种相机退化模式。

**Result:** 在KITTI衍生的退化数据上，GSHI随严重程度单调递减，健康估计MAE为0.064，在YOLOv8检测失败前提供0.47±0.25严重度单位的正向预警时间；性能超越IQA、检测器置信度和干净特征OOD基线，并能零样本迁移到真实恶劣天气驾驶数据。

**Conclusion:** 基于退化感知的不确定性分析是智能车辆主动相机可靠性监控的实用方向，GSHI框架为安全关键ADAS系统提供了有效的故障预警机制。
...
> EchoXFlow是一个包含37,125个超声心动图记录的临床数据集，保留原始采集几何结构而非转换后的笛卡尔视频格式，支持跨模态、物理感知的多模态学习。

<details>
  <summary>Details</summary>

**Motivation:** 现有的公开超声心动图数据集存在局限性：多普勒数据通常缺失或仅作为RGB叠加层融合，且数据经过有损的供应商显示处理，无法研究心脏解剖、心肌运动和血流之间的跨模态关系。

**Method:** EchoXFlow收集了666例常规检查的37,125个记录，保留时间、几何和多模态关系。每个记录保持为可分离的模态特定流：包含时间分辨的1D、2D、3D数据和多普勒模态，并与同步ECG配对。临床标注包括指南测量、密集2D心肌轮廓和3D左心室内膜网格。

**Result:** 该数据集支持无法仅从传统扫描转换视频中构建的跨模态、采集感知学习任务，为4D视觉和物理感知多模态学习提供了测试平台。

**Conclusion:** EchoXFlow通过保留原始超声采集几何和多模态数据，结合开源工具，为物理感知的心脏超声学习和跨模态研究开辟了新的可能性。
...
> 这是首个可控散景渲染挑战赛的总结报告，8个团队提交了解决方案，比赛通过定量指标定性专家评估相结合的方式评判参赛方法。

<details>
  <summary>Details</summary>

**Motivation:** 为了推动可控散景渲染技术的发展，NTIRE举办了首届相关挑战赛，旨在评估和比较不同算法在复杂场景下生成高质量、视觉上吸引人的散景效果的能力。

**Method:** 比赛采用双轨制评估：第一轨道通过既定的定量保真度指标进行客观评价；第二轨道通过专家小组进行定性感知评估。测试包含未知的人像和复杂主体图像。

**Result:** 44名参与者中8个团队提交了有效方案。由于是首届比赛，多数参与者选择在Bokehlicious基准方法基础上进行改进和扩展。

**Conclusion:** 挑战赛成功汇集了研究社区对可控散景渲染技术的关注，为该领域的标准化评估建立了基准，并显示出基于现成方法进行渐进式改进是当前的主要研究趋势。
...
> 本研究使用AlphaEarth Foundation的卫星嵌入模型来评估巴西圣保罗州1729个森林恢复地点的效果，通过引入'参考轨迹嵌入'概念，基于与成熟次生林参考站点的余弦相似度来定义恢复成功指标。

<details>
  <summary>Details</summary>

**Motivation:** 巴西大西洋森林是生物多样性热点，但仅剩12-15%的原生植被。传统的现场监测方法在大规模应用上不切实际，而遥感指数（如NDVI）在监测渐进的森林恢复过程时容易饱和，难以有效评估恢复效果。

**Method:** 研究人员使用了AlphaEarth Foundation的卫星嵌入模型，处理了1729个恢复地点的数据。提出了'参考轨迹嵌入'概念，通过计算各地点与成熟次生林参考站点在嵌入空间中的余弦相似度来量化恢复成功度。分析了不同土地利用和土地覆盖类型在嵌入空间中的聚类特征。

**Result:** 研究发现嵌入空间中存在明显的聚类，与不同的土地利用和土地覆盖类型相对应，能够识别出具有明确变化向量的地点。然而，信号存在噪声，嵌入模型可能需要进一步微调以便更好地捕捉和预测除土地利用和土地覆盖之外的场地元数据。

**Conclusion:** 卫星嵌入技术在评估大规模森林恢复方面显示出潜力，特别是通过'参考轨迹嵌入'方法来表征早期恢复成功。但需要改进模型以减少噪声并增强对复杂恢复过程的预测能力。
...
> 本文提出了一种名为GDS-Mamba的图调节解耦稀疏Mamba模型，用于解决MODIS时间序列数据中树种分类的挑战，在加拿大两省的大规模实验中达到了93.94%的分类准确率。

<details>
  <summary>Details</summary>

**Motivation:** 从中分辨率成像光谱仪(MODIS)时间序列数据中进行树种分类对环境应用至关重要，但面临三大挑战：树种间细微特征差异、强空间-光谱-时间信息耦合以及大规模拓扑上下文建模困难。

**Method:** 提出了GDS-Mamba模型，包含三个核心贡献：(1)设计小批量图调节方法改善大规模上下文建模；(2)提出解耦Mamba架构分离空间、光谱和时间信息；(3)设计稀疏token方法提高效率和细微特征学习能力。

**Result:** 在加拿大艾伯塔省和萨斯喀彻温省的大规模MOD13Q1数据集上实验，艾伯塔省总体准确率达到93.94%，跨省评估准确率达80.19%，超越了12个最先进的分类模型。

**Conclusion:** GDS-Mamba模型有效解决了MODIS时间序列树种分类的关键挑战，通过图调节、信息解耦和稀疏设计显著提升了分类精度，为大规模环境监测应用提供了强大的技术支撑。
...
> 研究发现，只需区分8个宽泛类别的粗粒度训练神经网络，其内部表示就能达到甚至超过区分1000类细粒度网络的大脑对齐度，并且在人类感知相似性判断上表现超过所有对比模型。

<details>
  <summary>Details</summary>

**Motivation:** 过去十年研究中，人工神经网络在视觉任务上训练后会发展出类似灵长类视觉系统的内部表示，但监督信号粒度对大脑对齐的作用尚未被系统研究。

**Method:** 通过基于预训练嵌入的PCA分割方法，将训练图像划分为不同数量的类别（2到64类），在卷积和transformer架构上训练数百个神经网络，并将其表示与猕猴电生理记录和人类fMRI响应进行比较。

**Result:** 仅需区分8个宽泛类别的粗粒度训练网络就能学习到与大脑高度对齐的表示，在人类感知相似性判断上优于细粒度监督、自监督训练的大型视觉模型。

**Conclusion:** 人类般的视觉表征可以从非常粗粒度的反馈中涌现，这重新定义了视觉学习所需的学习信号，为构建更符合人类感知的AI系统开辟了新路径。
...
> 本文提出了一个新的跨模态检索任务——文本到CAD检索，并开发了一个统一的框架，通过融合程序序列和几何点云特征来实现高效的文本检索CAD模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的CAD模型库主要通过文件名或目录搜索，限制了设计重用的效率、可扩展性和准确性。传统的检索方法无法理解语义内容，迫切需要基于文本的自然语言查询来检索相关的CAD模型。

**Method:**  proposed framework包含三个编码器：序列编码器捕获CAD模型的构造逻辑，点编码器提取几何特征，文本编码器学习文本查询的语义表示。训练时引入特征解码器，通过交叉注意力重构遮蔽序列特征，促进多模态对齐；推理时移除解码器，使用连接的序列-点特征进行高效检索。

**Result:** 基于Text2CAD数据集建立了实用基准，框架作为文本到CAD检索的强基线，为检索增强生成等下游CAD生成任务奠定了基础。

**Conclusion:** 本文首次系统定义了文本到CAD检索任务，提出的多模态嵌入学习方法有效提升了检索性能，为工业设计重用和CAD生成研究提供了新方向。
...
> 本文提出了不确定性引导的边缘学习算法(UGEL)，通过深度贝塔回归估计预测不确定性，加速边缘设备上的回归模型训练收敛。

<details>
  <summary>Details</summary>

**Motivation:** 边缘设备的计算限制不仅影响模型优化，还影响预测不确定性的计算，而对于模型更新决策来说，预测不确定性至关重要。特别是在遥感卫星等资源受限的环境中，需要高效的边缘学习方法。

**Method:** 提出UGEL算法，核心是深度贝塔回归方法。深度网络估计贝塔分布参数，将目标值建模为贝塔分布，单次前向传播即可计算预测不确定性，从而准确优先选择数据进行训练。

**Result:** UGEL相比主动学习和半监督学习能够实现更快的边缘学习收敛。深度贝塔回归在计算效率和模型灵活性方面都优于传统不确定性估计方法。

**Conclusion:** UGEL为资源受限的边缘环境提供了一种高效的解决方案，通过深度贝塔回归实现快速且准确的预测不确定性估计，显著提升了边缘学习的效率和性能。
...
> RAM-H1200是一个包含1200张手部X光片的大规模多级别注释数据集，首次统一支持全手骨骼实例分割、像素级骨侵蚀分割和临床标准化的SvdH评分评估。

<details>
  <summary>Details</summary>

**Motivation:** 现有类风湿关节炎(RA)手部X光片资源存在局限性：缺乏全手覆盖、细粒度注释和临床评分系统的有效整合，特别是支持骨侵蚀(BE)定量分析的注释极为稀缺。

**Method:** 收集来自6个医疗中心的1200张手部X光片，提供多层次注释：(1)全手骨骼结构实例分割 (2)像素级BE掩码 (3)SvdH定义的关节感兴趣区域 (4)关节级BE和关节间隙狭窄(JSN)的SvdH评分。

**Result:** 基准测试结果表明，解剖建模比定量BE分析更加成熟：全手骨骼分割表现强劲，而BE分割仍是一个重大挑战。这是首个支持全手骨骼实例分割、像素级BE描述和临床基础联合SvdH评分的公共大规模基准。

**Conclusion:** RAM-H1200通过统一解剖结构建模、定量病变分析和临床基础SvdH评分，为手部X光片上的全面RA分析提供了单一基准，促进了类风湿关节炎的多级别分析研究。
...
> 本文提出了一种利用视觉语言模型生成合成数据来解决森林物种图像分割中数据稀缺问题的方法，显著提升了复杂森林再生区域的物种识别性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统森林管理依赖地面调查，效率低下且受地理限制。虽然无人机可大规模收集数据，但深度学习方法因缺乏专家标注的航空图像而受限，特别是在视觉异质性高的森林再生区域存在严重的类别不平衡问题。

**Method:** 利用Nano Banana Pro视觉语言模型从提示词同时生成高保真图像和像素对齐的语义掩码。构建了包含13,977张未标注图像和50张标注真实图像的WilDReF-Q-V2数据集，以及包含2,101对合成图像和语义掩码的Gen4Regen数据集。将真实数据与AI生成数据进行统一训练。

**Result:** 统一训练的F1分数比纯监督基线提高了15个百分点以上。少量提示生成的数据就能显著改善代表性不足物种的性能，某些物种的F1分数提升高达30个百分点。AI生成的数据与真实数据高度互补。

**Conclusion:** 视觉语言模型可以作为敏捷的数据生成器，有效解决专家标注稀缺的细分AI领域的感知任务问题，为可持续森林管理提供了可扩展的解决方案。
...
> 该研究提出了一种 delighting（去光照）网络作为先验，实现了基于智能手机视频的高质量面部外观捕获，并开源了模型和4K分辨率可重光照数据集。

<details>
  <summary>Details</summary>

**Motivation:** 传统高质量面部外观捕获需要昂贵的摄影棚设备，现有基于智能手机的方法在复杂光照条件下难以准确分离反射率和光照信息。

**Method:** 通过训练强大的去光照网络作为约束先验，利用OLAT数据集和Light Stage扫描进行训练，提出数据集潜在调制(DLM)技术，通过可学习的源感知标记将异构数据源无缝集成，解耦数据集特定风格与物理去光照原理。

**Result:** 实现了简单自动的外观捕获流程，从普通视频输入获得高质量反射率估计，性能大幅超越现有方法；将NeRSemble数据集转换为4K分辨率可重光照的NeRSemble-Scan大规模数据集。

**Conclusion:** 通过开源去光照模型和NeRSemble-Scan数据集，使高端面部捕获技术大众化，为构建逼真数字人提供了新的研究基础。
...
> 本文提出了VQAU任务和AffectSeek框架，研究如何在模糊查询下从长视频中定位情感时刻并进行解释

<details>
  <summary>Details</summary>

**Motivation:** 现有情感理解研究主要基于预裁剪的短视频片段，缺乏真实场景中用户通过模糊查询与长视频交互的体验，无法满足实际需求

**Method:** 构建VQAU-Bench基准数据集，包含长视频、模糊情感查询、时序标注和解释说明；提出AffectSeek智能体框架，将任务分解为意图解释、候选定位、片段验证、情感推理和解释生成五个步骤

**Result:** 实验表明VQAU任务对现有情感识别模型和单步视觉语言模型具有挑战性，而AffectSeek框架能有效处理多步推理并实现语义-时序-情感对齐

**Conclusion:** VQAU任务和AffectSeek框架填补了长视频情感理解的研究空白，为主动式、交互式的视频情感理解提供了新的研究方向和解决方案
...
> MUSE框架通过拓扑正交性解决了视觉令牌化中重建与感知之间根本性冲突，实现了高质量生成和语义感知的双重突破。

<details>
  <summary>Details</summary>

**Motivation:** 统一的视觉令牌化面临基本权衡：高保真像素重建（空间等变性）与语义抽象（概念不变性）之间存在冲突。这种冲突源于流形错位，导致重建和感知形成零和博弈。

**Method:** 提出基于拓扑正交性的MUSE框架，将结构视为正交桥梁，在Transformer中解耦优化：结构梯度优化注意力拓扑，语义梯度更新特征值，将破坏性干扰转化为相互强化。

**Result:** 实验表明MUSE打破了权衡，实现最先进的生成质量（gFID 3.08），在线性探测任务中超越教师模型InternViT-300M（85.2% vs 82.5%）。

**Conclusion:** 证明了结构对齐的重建能够增强语义感知，为统一的视觉令牌化提供了有效解决方案。
...
> S2C-3D是一个新颖的稀疏视图3D重建框架，仅需6-8张图像即可实现高精度和完整的场景重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的稀疏视图3D重建方法通常存在重建不完整、模糊和伪影等问题，特别是在输入视图极少的情况下。为了解决这些挑战，需要开发一种能够从极少输入图像中重建高质量、完整3D场景的方法。

**Method:** 该框架包含三个核心组件：1）专门的扩散模型，通过在输入视图及其对应的退化版本上微调预训练架构来实现场景特定的图像修复；2）训练无关的视图一致性条件采样过程，在冻结的扩散模型中进行高斯优化；3）相机轨迹规划方案，通过将每个新采样相机连接到其两个最近邻居并显著增强可见性的路径来确保全面的场景覆盖。

**Result:** 实验结果表明，S2C-3D在稀疏输入条件下超越了最先进的方法，能够构建高质量、无缺失区域、无模糊或其他伪影的3D场景。

**Conclusion:** 提出的S2C-3D框架通过结合场景特定的扩散模型、视图一致性条件采样和智能轨迹规划，成功解决了稀疏视图3D重建中的主要挑战，为仅需极少视图就能实现高保真3D重建提供了有效的解决方案。
...
> EGA是一种残差适配器方法，通过零初始化、局部三元组损失和超球面投影三原则，解决向量搜索系统中冻结编码器在面对未见类查询时的性能崩塌问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于冻结视觉编码器的向量搜索系统在部署时面临未见类样本查询，但高容量适配器训练时会将未见类样本错误分配到已知类簇中，导致最坏情况下的标签精度比冻结基线下降40多个点。

**Method:** 提出欧几里得测地线对齐(EGA)残差适配器，结合零初始化、局部三元组损失和超球面投影三原则。当三元组满足小边界时停止产生梯度，适配器在局部几何正确时自动停止更新，形成自我限制动态。

**Result:** 收敛时96.5%的三元组无梯度，保持未见类区域基本不变同时充分优化已知类。在五个OOD基准测试中，EGA在四个主要分割取得最高最坏情况标签精度，第五个也保持一致改进。

**Conclusion:** EGA设计可迁移到CLIP之外的更强骨干网络，分析表明梯度稀疏性与有界OOD扰动相关，为解决分布外下的向量搜索适配器崩塌问题提供了有效方案。
...
> 本文提出MotionGRPO框架，通过强化学习后训练方式为扩散过程注入细粒度指导，解决从头戴设备信号恢复全身3D人体运动的局部关节重建误差问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于分布匹配的扩散方法在从头戴设备信号恢复3D人体运动时，存在局部关节重建精度不足的问题，需要更细粒度的指导机制。

**Method:** 将扩散采样建模为马尔可夫决策过程，使用群组相对策略优化(GRPO)进行优化；设计混合奖励机制，结合学习条件感知模型的全局视觉合理性与显式约束的局部关节精度；引入噪声注入策略增加样本方差，解决梯度消失问题。

**Result:** 在大量实验中，MotionGRPO达到了最先进的性能表现，具有优越的视觉保真度。

**Conclusion:** 通过强化学习后训练和细粒度指导，MotionGRPO有效提升了3D人体运动恢复的精度和视觉效果，为相关领域提供了新的解决思路。
...
> R2H-Diff是一种基于扩散模型的RGB到高光谱图像重建框架，通过条件迭代细化过程和特殊设计的光谱注意力模块，在保持高重建质量的同时大幅降低了计算复杂度。

<details>
  <summary>Details</summary>

**Motivation:** RGB到高光谱图像重建是一个高度不适定逆问题，现有回归方法学习确定性映射，无法建模重建不确定性且容易产生过平滑的光谱响应。虽然扩散模型具有强大的分布建模能力，但直接应用于高光谱重建面临光谱维度高、波段间相关性强、光谱保真度要求严格等挑战。

**Method:** 提出R2H-Diff扩散框架，将光谱恢复表述为条件迭代细化过程；设计引导光谱细化模块实现RGB条件特征融合；开发高光谱自适应转置注意力模块高效建模空间-光谱依赖关系；采用无归一化去噪主干保持光谱幅度一致性；使用任务自适应线性噪声调度，仅用5个去噪步骤即可实现高质量重建。

**Result:** 在NTIRE2022、CAVE和Harvard数据集上实验表明，R2H-Diff在重建质量和计算效率之间取得了良好平衡。在NTIRE2022数据集上获得35.37 dB PSNR，模型参数仅0.58M，计算量12.25G FLOPs，在保持强重建保真度的同时实现了最低模型复杂度。

**Conclusion:** R2H-Diff成功解决了扩散模型在高光谱重建中的应用难题，通过创新的架构设计和工程优化，实现了高效准确的RGB到高光谱图像重建，为高维光谱重建任务提供了新思路。
...
> 本文提出了首个面向压缩的隐私保护动作识别方法CFE-PPAR，解决了现有加密方法在视频压缩后性能急剧下降的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于加密的隐私保护动作识别方法虽然提供了强隐私保护，但在视频压缩时会导致识别性能和视觉质量灾难性下降，缺乏压缩友好性。

**Method:** CFE-PPAR方法通过相同密钥同时用于视频加密和视频Transformer参数变换，使加密后的视频可直接被识别器处理，实现了压缩友好的隐私保护。

**Result:** 在UCF101和HMDB51数据集上，CFE-PPAR在Motion-JPEG和H.264压缩条件下均优于现有方法，验证了其有效性。

**Conclusion:** CFE-PPAR首次实现了压缩友好的隐私保护动作识别，为实际应用中的视频隐私保护提供了新的解决方案。
...
> 论文提出了一种基于跨模态提示调制的视频情感识别方法，通过rPPG生理信号调制面部表情特征，并引入解耦适配器来提高跨个体泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 面部表情无法完全反映内在情感状态，而rPPG生理信号容易受噪声和个体差异影响，现有多模态融合方法破坏了预训练的面部表示且缺乏抑制个体特定变异的显式机制。

**Method:** 将rPPG波形转换为抗噪声的时频表示，生成模态互补提示来调制冻结Vision Transformer中的面部token；在每个ViT层引入解耦共享-特定适配器(DSSA)显式分离个体共享和个体特定组件。

**Result:** 在MAHNOB-HCI和DEAP基准测试中，所提方法在识别准确性和泛化能力上持续优于强基线方法。

**Conclusion:** 该方法通过保持预训练主干网络的泛化面部表示，并结合显式的个体变异抑制机制，有效提升了视频情感识别的性能和跨个体泛化能力。
...
> 本文提出了一个统一框架，将语言驱动的3D场景生成与沉浸式用户交互紧密集成，通过闭环反馈机制实现更自适应的交互式多媒体系统。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型3D内容生成方法通常将场景生成和用户交互视为独立过程，这限制了交互式多媒体系统的适应性和沉浸感，无法充分利用用户反馈来优化生成内容。

**Method:** 系统首先使用LLM根据自然语言指令构建结构化场景表示，然后通过强化学习在几何和语义约束下优化空间布局，最后在虚拟现实环境中部署，通过人机交互循环获取持续反馈来调整生成内容。

**Result:** 在ALFRED基准测试中达到最先进的任务场景生成性能，定性结果和用户研究表明在沉浸感、交互质量和任务效率方面都有显著提升。

**Conclusion:** 语言驱动的3D生成与用户交互的闭环集成对于下一代多媒体系统至关重要，能够提供更响应、更真实、更自适应的多媒体体验。
...
> EgoEMG是一个用于双手姿态估计的多模态自视角数据集，包含同步的肌电(sEMG)、视觉和其他传感器数据，并建立了手部姿态估计的基准测试框架。

<details>
  <summary>Details</summary>

**Motivation:** 现有的手部姿态估计研究中，肌电信号和视觉信号具有互补性：肌电可以捕获精细的手指动作，即使在遮挡和光照不佳的情况下；而视觉提供全局的手部配置信息。然而，目前缺乏同时捕获这两种模状的同步数据集。

**Method:** 构建了一个包含41名参与者的多模态数据集，包含16通道腕带肌电信号(2kHz采样)、120Hz IMU数据、自视角广角RGB视频、外部RGB-D视频以及基于动作捕捉的手部运动数据。建立了包含三个任务(EMG-to-pose、vision-to-pose、EMG+vision融合)的基准测试框架。

**Result:** 数据集包含60种手势类别(30种单手手势和30种双手手势)，总时长超过10小时。基准测试评估了EMGFormer在EMG-to-pose任务上的表现，以及通用ResNet/ViT骨干网络在vision-to-pose任务上的表现。研究了残差融合架构在轻量级视觉基线上的改进效果。

**Conclusion:** EgoEMG数据集及其基准测试为基于肌电和视觉的多模态手部姿态估计研究奠定了基础，为未来该领域的研究提供了重要的数据资源和评估标准。
...
> TriRelVLA是一种通过显式构建对象-手部-任务三元关系来提升视觉-语言-动作模型泛化能力的新框架，实现了跨场景、跨对象和跨任务的泛化。

<details>
  <summary>Details</summary>

**Motivation:** 现有的VLA模型在训练时表现良好，但在未见过的场景和对象上泛化能力差，主要原因是其隐式视觉表示将物体外观、背景和场景布局混杂在一起，使得策略对视觉变化敏感。

**Method:** 提出TriRelVLA框架，包含三部分：1）从多模态输入构建显式的对象-手部-任务三元表示；2）构建任务导向的关系图，使用任务引导的交叉注意力形成节点，关系感知的图变换器建模节点间交互；3）执行关系条件化的动作生成。将关系结构压缩到瓶颈空间并投射到LLM中进行动作预测。

**Result:** 实验表明，该方法在微调任务上表现优异，在跨场景、跨对象、跨任务泛化方面取得了显著的性能提升。还引入了真实机器人数据集进行微调。

**Conclusion:** 通过显式建模对象-手部-任务的三元关系结构，TriRelVLA有效降低了对外观统计的依赖，显著提升了VLA模型的泛化能力，使机器人能够更好地适应新的环境、对象和任务组合。
...
> 提出B³-Net框架，通过显式建模任务证据可靠性的受控后验桥接学习，解决多任务密集预测中的负迁移问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有多任务密集预测方法在解码器端交互时，大多通过相似性隐式融合任务特征，没有显式建模不同任务和空间位置的证据可靠性差异，导致不可靠证据污染共享表示，加剧负迁移。

**Method:** B³-Net框架包含三个核心组件：1) 精度场估计器，从任务参考对齐和局部变化估计补丁级证据精度；2) 后验桥接算子，通过异方差证据融合构建精度加权的后验桥接；3) 收缩分发算子，通过有界更新将桥接重新分配到各任务分支，减少特征注入失控。

**Result:** 在NYUD-v2、PASCAL-Context和Cityscapes数据集上，B³-Net在CNN、Transformer、扩散、Mamba和桥接特征等多种基线方法上取得了竞争性或更优的权衡效果，主干匹配对比和大量分析验证了收益来自受控后验桥接学习而非主干容量或解码器规模。

**Conclusion:** 通过将解码器端交互分解为可靠性估计、后验桥接构建和有界重分配三步骤，B³-Net有效控制了多任务密集预测中的证据交流，减少了不可靠证据的负面影响，为多任务学习提供了一种新的显式可靠建模框架。
...
> 提出一种射线感知指针记忆机制，通过联合建模空间位置和视向方向，实现基于保留或替换的自适应记忆更新策略，显著提升了流式3D重建的长期稳定性和相机位姿精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有的前馈重建框架主要依赖外观相似性来更新记忆，当视角变化时容易出现冗余观测累积和不稳定的几何重建问题，需要更稳定高效的长期记忆管理机制。

**Method:** 设计射线感知指针记忆表示，每个指针存储3D位置、关联射线方向和特征嵌入；提出保留-替换机制替代传统融合压缩，通过联合推理空间距离和射线方向差异来区分冗余观测、新观测和闭环重访。

**Result:** 实验证明该方法显著改善了长期重建稳定性和相机位姿精度，同时保持了高效的流式推理性能，为大规模抗漂移在线3D重建提供了 principled 框架。

**Conclusion:** 射线感知记忆设计为流式3D重建提供了统一的几何邻近性和视向一致性推理能力，通过自适应指针更新策略实现了可扩展且抗漂移的在线重建系统。
...
> 本文提出了一种名为TDSC的时间深度自表达子空间聚类方法，用于解决人体动作分割中原始特征违反子空间并集假设的问题，通过联合学习时间一致的结构化表示和稳定亲和力来提升分割性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有人体动作分割方法主要基于子空间聚类，假设高维时间特征分布符合子空间并集(UoS)。然而真实世界视频的原始帧级特征经常违反这一假设，导致分割效果不理想。

**Method:** TDSC方法交替学习输入帧特征的结构化表示和自表达系数，通过编码率最大化正则化避免表示崩溃，同时融入时间约束促进相邻时域帧被分割到同一组别，并开发时间动量平均机制稳定亲和力演化，采用重参数化策略实现高效优化。

**Result:** 在五个基准人体动作分割数据集上进行了广泛实验，使用传统的HoG特征和最新的深度特征(CLIP、DINOv2)验证了方法的有效性。

**Conclusion:** TDSC通过联合学习时间一致的结构化表示和稳定亲和力，有效解决了原始特征违反UoS假设的问题，实现了准确且鲁棒的人体动作分割。
...
> iTRIALSPACE是一个可编程的肺部CT模型评估框架，通过合成受控的虚拟病灶试验解决了传统基准测试的局限性，能够准确分析模型性能的结构性驱动因素。

<details>
  <summary>Details</summary>

**Motivation:** 传统的静态回顾性基准测试将病灶大小、肺叶分布、解剖结构和采集背景等因素混杂在一起，使得确定模型准确性的结构性驱动因素变得困难。

**Method:** 该框架采用四阶段管道：多数据集病灶轮廓分析、显式试验规范、解剖感知掩码插入和ControlNet条件合成，构建了包含54个属性、来自7个公共CT源的13,140个标注病灶的统一数据集。

**Result:** 在包含55,469个样本的虚拟病灶研究中，合成底物保持在真实到真实的FID基线内，合成性能排名强烈转移至真实临床数据(ρ=0.93)，并揭示了传统基准测试无法发现的发现。

**Conclusion:** iTRIALSPACE为可控制和可证伪的测试提供了可审计的评估基础设施，超越了静态回顾性基准测试的局限。
...
> X-OmniClaw是一个统一的Android移动端多模态智能代理，通过感知-记忆-行动的协同架构，实现了复杂移动任务的高效处理和个性化交互。

<details>
  <summary>Details</summary>

**Motivation:** 受OpenClaw发展启发，移动端需要能够处理复杂直观交互的个人代理智能体，以提升用户体验和任务处理效率。

**Method:** 构建三层统一架构：Omni Perception（多模态感知，整合UI状态、视觉语境和语音输入），Omni Memory（多模态记忆优化，结合工作记忆和个人长期记忆），Omni Action（混合定位策略，通过行为克隆和轨迹回放捕获可重用技能）。

**Result:** 在多样化场景的演示中，X-OmniClaw显著提升了交互效率和任务可靠性，为下一代移动原生个人助手提供了实用的架构蓝图。

**Conclusion:** 该统一的感知-记忆-行动架构成功实现了高度情境感知和个性化的移动交互，为移动智能代理的发展奠定了技术基础。
...
> 第三届autoPET挑战赛评估了全身PET/CT中病变自动分割算法的组合泛化能力，最佳算法相比基线模型DSC提升8%，但对未见过的示踪剂-中心组合仍存在泛化难题。

<details>
  <summary>Details</summary>

**Motivation:** 评估PET/CT病变自动分割算法在组合泛化设置下的性能，特别是在处理不同示踪剂和医疗中心组合时的泛化能力。

**Method:** 17个团队提交27种算法（主要基于nnU-Net的3D网络），使用1,014例[18F]-FDG和597例PSMA PET/CT研究训练，在200例涵盖四种示踪剂-中心组合的测试集（含两种未见组合）上评估.

**Result:** 最佳算法平均DSC为0.66，相比基线提升8%，但组合泛化仍存在系统性体积高估问题；算法选择对顶级团队性能影响小于数据异质性和病例难度。

**Conclusion:** 同域多示踪剂分割已接近读者一致性水平，但未见组合泛化仍是开放问题；性能变异主要由数据异质性驱动而非算法选择。
...
> 提出了UNO框架，通过理解任务作为监督信号来增强生成表示，实验证明理解可以有效提升生成性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前最先进的多模态模型虽然性能优秀，但理解和生成组件很大程度上是解耦的，这种设计削弱了相互增强的联系，使得协同效应的潜力在经验上是不确定的。

**Method:** 提出理解导向的后训练框架(UNO)，将理解不仅作为独立任务，还作为直接监督信号来引导生成表示。通过整合编码语义抽象(图像描述)和结构细节(视觉回归)的目标，实现从理解到生成的有效梯度流。

**Result:** 在图像生成和编辑上的大量实验证明，理解可以作为生成的有效催化剂，显著提升生成性能。

**Conclusion:** 通过UNO框架成功恢复了理解与生成之间的协同效应，证明了理解任务对生成任务的积极作用，为统一多模态模型的设计提供了新思路。
...
> 本文提出Na-IRSTD框架，通过原生分辨率特征提取和融合策略，有效保留红外小目标的细节信息，并采用高效的token缩减选择策略降低计算负担，在多个基准数据集上达到最先进的检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有红外小目标检测方法通常采用下采样策略，会丢失小目标的细节信息，导致在复杂背景杂波中难以精确定位暗淡目标，检测性能不佳。

**Method:** 提出了Na-IRSTD框架，核心包括两个方面：1）原生分辨率特征提取与融合机制，保留小目标的细微线索；2）高效的token缩减与选择策略，准确选择目标图块，在减少计算负担的同时增强特征的低级细节。

**Result:** 在多个公开数据集上的大量实验验证了token缩减与选择策略的鲁棒性和有效性，Na-IRSTD模型在四个基准测试中均取得了最先进的性能。

**Conclusion:** 该框架通过保持原生分辨率处理和智能的token选择，克服了传统红外检测方法的分辨率限制，显著提升了小目标定位能力，同时有效控制了计算复杂度。
...
> 本研究发现了医学视觉语言模型在胸部X光诊断中存在的'否定选项吸引'问题，并提出了相应的诊断基准和解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 医学视觉语言模型在胸部X光诊断中会出现极性反转现象，即当影像显示存在某种病症时，模型反而选择'无该病症'的否定选项，这会产生与影像相矛盾的临床陈述，带来严重的医疗风险。

**Method:** 研究者开发了CXR-ContraBench基准测试，包含内部ReXVQA和外部OpenI、CheXpert数据集。通过设计存在-发现问题和不存在-发现问题的测试框架，评估模型对否定选项的偏向性。提出了QCCV-Neg（问题条件一致性验证器）来修复极性混淆问题。

**Result:** 在CheXpert协议上，MedGemma和Qwen2.5-VL模型在直接存在检测上准确率仅31.49%和30.21%，在存在问题上选择否定选项的比例超过62%。思维链提示可以减少但不能完全消除反转问题。QCCV-Neg方法将两模型的准确率分别提升至96.60%和95.32%。

**Conclusion:** 标准的准确率指标会掩盖具有临床意义的推理时极性故障。研究揭示了当前医学视觉语言模型在处理否定逻辑上的严重缺陷，并提供了无需重新训练的确定性修复方案。
...
> ChartZero是一种零样本图表数据提取框架，通过合成数据训练解决真实世界图表标注稀缺问题，实现了端到端的鲁棒图表解析。

<details>
  <summary>Details</summary>

**Motivation:** 现有线图数据提取方法面临三个核心问题：1)样式多样性和真实标注数据稀缺限制泛化能力；2)曲线提取易产生结构断裂和细节丢失；3)语义关联依赖脆弱的空间启发式规则；4)评估协议碎片化无法衡量端到端性能。

**Method:** 提出ChartZero框架，包含三大创新：1)仅使用简单数学函数的合成数据集训练，规避真实标注瓶颈；2)设计全局正交实例(GOI)损失解决曲线断裂问题；3)采用开放词汇视觉语言模型指导的图例匹配策略替代空间规则；4)建立端到端重建评估指标和基准。

**Result:** 实验证明ChartZero在无需真实世界监督的情况下显著提升了泛化的图表数字化性能，成功解决了曲线提取和语义关联两大失效模式。

**Conclusion:** ChartZero通过合成先验学习和创新的损失函数、匹配策略，首次实现了零样本、端到端的线图数据提取，为突破现有方法的标注依赖和泛化局限提供了新范式。
...
> 研究提出了MCD数据集，这是首个整合同一研究工作的论文、演示视频、讲解视频和幻灯片的多模态学术数据集，并系统评估了现有模型在跨格式细粒度对齐任务上的表现。

<details>
  <summary>Details</summary>

**Motivation:** 科学知识的传播日益多模态化，涵盖文本、视觉和演讲等多种形式，但这些材料缺乏结构化连接，使得难以追踪概念、视觉和解释之间的对应关系，限制了研究内容的统一探索和分析。

**Method:** 构建Multimodal Conference Dataset (MCD)数据集，整合研究论文、演示视频、讲解视频和幻灯片；评估基于嵌入的方法和视觉语言模型在发现细粒度跨格式对应关系方面的能力。

**Result:** 视觉语言模型表现稳健但难以实现细粒度对齐；基于嵌入的模型能很好地捕捉文本-视觉对应关系，但公式和符号内容在嵌入空间中形成独立聚类。

**Conclusion:** 当前方法在多模态科学理解方面存在优势与局限，研究为未来发展方向提供了重要指引，通过开源资源确保研究可复现性。
...
> VideoRouter是一个基于InternVL的查询自适应双路由框架，通过语义路由器预测分配策略，图像路由器评分帧相关性，实现长视频的动态压缩，最多可减少67.9%的token数量。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频大多模态模型面临长视频导致视觉token序列过长的问题，现有压缩方法缺乏查询感知能力或对视觉证据分布不均的情况处理不佳。

**Method:** 提出双路由框架：语义路由器预测主导分配策略（宽时域覆盖vs自适应高分辨率保留），图像路由器使用早期LLM层评分帧相关性，构建Video-QTR-10K和Video-FLR-200K数据集进行训练。

**Result:** 在VideoMME、MLVU和LongVideoBench等基准测试中，VideoRouter在相同或更低预算下一致性地超越InternVL基线，实现最高67.9%的token减少。

**Conclusion:** VideoRouter通过查询自适应的证据分配机制有效解决了长视频处理中的可扩展性瓶颈，为视频大模型的高效推理提供了新的解决方案。
...
> Align3D-AD是一个零样本3D异常检测框架，通过RGB跨模态引导解决现有方法的领域差距问题，在多个数据集上实现最佳性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有零样本3D异常检测方法将3D观察投影到多视图表示时主要捕获几何线索而非真实视觉语义，且使用RGB数据预训练的视觉编码器处理这些表示时存在显著的领域差距。

**Method:** 提出两阶段统一框架：1) 跨模态特征对齐范式，将渲染特征映射到RGB语义空间，并引入语义一致性重加权策略优化对齐；2) 模态感知提示学习框架，通过双提示对比对齐捕获跨模态互补语义。

**Result:** 在MVTec3D-AD、Eyecandies和Real3D-AD数据集上的实验表明，Align3D-AD在一对多和跨数据集设置下均优于现有零样本方法，展现出强大的泛化能力和鲁棒性。

**Conclusion:** 通过RGB辅助类别的跨模态引导，Align3D-AD有效解决了3D异常检测中的领域差距问题，实现了零样本场景下的高性能检测，为3D异常检测提供了新的解决思路。
...
> InkDiffuser是一个基于扩散模型的中文书法生成框架，通过高频增强机制和可微分墨迹结构损失，能够仅凭单个参考字样生成具有真实墨迹效果的高质量书法作品。

<details>
  <summary>Details</summary>

**Motivation:** 当前中文书法生成方法存在笔画渲染质量差、墨迹形态不真实的问题，导致生成作品的视觉保真度和艺术流畅性有限。

**Method:** 提出了两个核心技术：1) 高频增强机制，通过融合高频表示来准确提取字体结构的轮廓细节；2) 可微分墨迹结构(DIS)损失，将可微分形态学操作整合到扩散过程中，使模型能够学习墨迹结构的显式分解。

**Result:** 在多种书法风格和复杂字符上的广泛实验表明，InkDiffuser能够从单个参考字样生成具有真实墨迹渲染效果的优秀书法字体，在结构一致性、细节保真度和视觉真实性方面超越了现有的少样本字体生成方法。

**Conclusion:** 该框架有效解决了中文书法生成中的墨迹渲染和质量问题，为数字化书法创作提供了新的技术手段，代码已开源。
...
> ContactPrompt是一种无需训练的密集手部接触估计方法，利用多模态大语言模型的推理能力实现零样本学习，性能超越有监督方法。

<details>
  <summary>Details</summary>

**Motivation:** 密集手部接触估计需要同时具备高级语义理解能力和细粒度几何推理能力。虽然多模态大语言模型在视觉语义理解方面表现出色，但将其应用于密集手部接触估计仍面临两个主要挑战：(1)难以编码显式3D手部几何信息，因为MLLMs主要处理视觉和语言模态；(2)难以捕获细粒度的顶点级接触，因为MLLMs倾向于关注高级语义而非细节几何推理。

**Method:** 提出ContactPrompt方法：1)引入详细的手部部件分割和部件级顶点网格表示来编码3D手部几何；2)开发多阶段结构化接触推理与部件条件机制，逐步桥接全局语义和细粒度几何。该方法无需训练，完全利用MLLMs的推理能力。

**Result:** 令人惊讶的是，该方法无需任何训练就超越了在大型密集接触数据集上训练的先前有监督方法，展示了MLLMs在细粒度几何推理任务中的潜力。

**Conclusion:** ContactPrompt成功地将多模态大语言模型应用于密集手部接触估计任务，通过结构化几何表示和渐进式推理机制，实现了零样本学习下的高性能表现，为MLLMs在细粒度几何理解任务中的应用开辟了新方向。
...
> DBMSolver是一种训练免费的采样器，利用指数积分器解决扩散桥模型的高阶方程，将采样速度提升5倍同时提高图像生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的扩散桥模型在图像到图像翻译任务中存在采样速度慢的严重问题，通常需要数十次函数评估，限制了其在实际应用中的可行性。

**Method:** 提出了DBMSolver方法，利用扩散桥模型底层的随机微分方程(SDE)和常微分方程(ODE)的半线性结构，通过指数积分器实现高效的一阶和二阶数值求解。

**Result:** 在图像修复、风格化和语义到图像等任务中，DBMSolver在20 NFEs时相比二阶基线FID降低53%，速度提升达5倍，在256x256分辨率下实现了新的效率-质量权衡状态。

**Conclusion:** DBMSolver为扩散桥模型提供了一种高效的采样解决方案，无需额外训练即可显著提升采样速度和生成质量，使高保真图像到图像翻译在实际应用中成为可能。
...
> 本文提出了一种基于混合专家模型(MoE)的多任务学习方法，通过联合学习多个自监督和伪标签任务来检测医学图像中的异常，在BMAD基准数据集上取得了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 医学图像异常检测面临的主要挑战是训练时异常样本不可用。现有方法依赖于单个预训练任务，这可能限制了模型学习正常解剖结构表示的能力。

**Method:** 提出基于MoE的联合模型，从零开始学习多个自监督和伪标签任务。在推理阶段，通过评估多任务学习器解决各个任务的表现来计算异常分数，从而生成可解释的异常图。

**Result:** 在包含多种医学图像模态的BMAD基准数据集上，所提出的多任务学习器超越了所有现有最先进方法，同时生成了有助于医生诊断的可解释异常图。

**Conclusion:** 多任务学习框架结合MoE架构能够有效学习正常解剖结构的鲁棒表示，为医学图像异常检测提供了一种更准确和可解释的解决方案。
...
> 本文提出MAST检测器，利用脉冲神经网络(SNNs)处理AI生成视频的时间残差特征，在跨生成器评估中实现93.14%的平均准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有AI生成视频检测器在跨生成器评估中性能急剧下降，因为不同生成器产生的伪迹类型和时间尺度差异很大。需要一种能够捕捉时间动态特征的通用检测方法。

**Method:** 发现AI生成视频在像素级时间残差更平滑，在语义特征空间轨迹更紧凑，观察到SNNs对边界时间伪迹敏感，据此提出MAST检测器：结合脉冲驱动的时间分支处理多通道时间残差和冻结语义编码器实现跨生成器泛化。

**Result:** 在GenVideo基准测试的10个未见生成器上，MAST在严格跨生成器评估下达到93.14%的平均准确率，匹配或超越基于ANN的最强检测器。

**Conclusion:** SNNs的事件驱动、稀疏激活特性与残差信号结构天然匹配，证明了SNNs在AI生成视频检测中的实用性和有效性。
...
> 研究发现，在阿拉伯语系文字识别中，跨语言联合训练的改进主要得益于序列级建模而非视觉特征共享，CRNN模型比纯CNN模型在低资源条件下表现更好。

<details>
  <summary>Details</summary>

**Motivation:** 在低资源条件下，阿拉伯语系手写文字识别通过跨语言联合训练获得了改进，但不确定这种改进是来自共享的视觉表示还是序列级依赖关系，需要通过控制实验来验证。

**Method:** 通过比较纯CNN模型和CRNN模型在阿拉伯语、乌尔都语和波斯语数据集上的表现，在低资源设置下(100-1000个样本)进行单脚本和多脚本训练对比实验。

**Result:** CRNN模型在多脚本训练下表现显著优于纯CNN模型，特别是在数据最受限的情况下。改进幅度(delta CER)表明跨语言改进与序列级建模相关，而CNN编码器的视觉特征共享单独作用不足。

**Conclusion:** 上下文建模在低资源场景下实现有效迁移学习起重要作用，这一发现可能适用于其他低资源语言设置。
...
> 本文提出了一种架构无关的Lipschitz常数贝叶斯头部，可集成到视觉变换器中形成LipB-ViT模型，通过谱归一化和自适应融合方案有效处理语义相近的标签噪声。

<details>
  <summary>Details</summary>

**Motivation:** 标签噪声是监督深度学习模型泛化能力的关键瓶颈，特别是当错误是结构化的而非随机时，标准鲁棒训练方法往往在语义相近的分类错误面前失效。

**Method:** 提出了Lipschitz-常数贝叶斯头部，对变分权重的均值和对数方差强制实施谱归一化，设计了结合不确定性和置信度的新指标以及自适应算术平均融合方案，联合捕捉特征空间邻近性和预测不确定性。

**Result:** 在15%语义误分类标签情况下，标签检测召回率达到93%以上，比基于k近邻的先进方法提升7%，在结构化和非结构化噪声场景下均表现出强鲁棒性。

**Conclusion:** LipB-ViT提供了即插即用的兼容性和跨域一致的超参数，为高重要性应用提供了强大的工具，其稳定的置信度估计形成了联合评估数据集质量和标签噪声的分析流水线基础。
...
> CAKI框架通过注入类别特定知识来提升视觉语言模型在零样本分类任务中的性能，包含类别提示生成和查询键匹配两个核心组件。

<details>
  <summary>Details</summary>

**Motivation:** 现有的提示学习方法忽略了细粒度的类别特定知识，导致在特定领域零样本分类中表现次优，需要结合类别级信息来改进模型性能。

**Method:** 提出即插即用的CAKI框架，通过少量样本生成类别特定提示并存储在知识库中，测试时检索相关类别知识并注入模型预测过程。

**Result:** 在基类和新类上的广泛实验表明CAKI方法有效提升了现有方法的性能，代码已开源。

**Conclusion:** CAKI框架成功地将类别特定知识集成到提示学习中，为视觉语言模型的零样本分类任务提供了新的改进思路。
...
> DeScore是一种解耦的视频奖励模型，采用'先思考后评分'范式，结合了CoT推理的泛化能力和判别式模型的高效性，解决了现有模型的根本困境。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频奖励模型面临两难选择：判别式RM缺乏显式推理容易走捷径，生成式RM将推理与评分耦合导致训练不稳定的优化瓶颈。需要一种既能保持良好泛化能力又训练稳定的奖励模型。

**Method:** DeScore采用解耦的'think-then-score'架构：MLLM生成显式CoT推理，独立判别式评分模块(含可学习query token和回归头)预测奖励。采用两阶段优化：(1)结合随机掩码的判别式冷启动，确保评分鲁棒性；(2)双目标强化学习，独立优化推理质量和奖励校准。

**Result:** DeScore实现了训练高效的视频奖励建模，既具备CoT推理的优越解释性和泛化潜力，又避免了耦合推理评分的优化瓶颈，实现了高质量推理到优越模型性能的直接映射。

**Conclusion:** 通过解耦推理与评分过程，DeScore为视频奖励模型提供了一种高效、可泛化的解决方案，对推动生成视频模型的后训练和测试时扩展具有重要意义。
...
> 本文提出了一种检测感知的对抗性微调框架，用于缓解目标检测模型中的后门攻击，该框架通过软分支最小化和双目标微调损失，在无需知道攻击目标的情况下有效抵御攻击。

<details>
  <summary>Details</summary>

**Motivation:** 后门攻击对安全关键视觉系统构成严重威胁，但现有的后门防御主要集中在图像分类任务，而目标检测的后门防御相对不足。对抗性微调在分类任务中有效，但直接应用于检测任务面临挑战。

**Method:** 提出检测感知的对抗性微调框架，包含两个核心技术：1）软分支最小化，使用软门结合目标消失和误分类的目标，并配合检测感知的分类损失最大化；2）双目标微调损失，针对目标匹配的预测进行定向修复。

**Result:** 在CNN和基于Transformer的检测器上的实验表明，相比分类导向的基线方法，该方法能更有效地降低攻击成功率，同时保持真实的检测结果，并维持了竞争性的干净检测性能。

**Conclusion:** 该方法成功解决了将对抗性微调从分类任务适配到检测任务的关键挑战，为后门攻击防御提供了一个有效且实用的解决方案，特别适用于只有少量干净数据可用的实际场景。
...
> 开发了一个基于大语言模型的病理报告过滤系统，从35万例CT检查中构建了涵盖106个人体结构的健康参考分布图表。

<details>
  <summary>Details</summary>

**Motivation:** 临床CT数据集常富含病理样本，难以建立大规模健康参考分布，这在解释定量CT生物标志物时构成重大挑战。

**Method:** 构建基于证据的大语言模型集成系统，通过结构级异常标记和交叉验证过滤病理报告，结合分布感知广义加性模型建立参考分布图表。

**Result:** 成功建立了覆盖成年的106个解剖结构（体积和衰减）的全身参考图表，揭示了结构和对比剂依赖的纵向变化特征。
...
> RAWild是一种物理指导的全局-局部色调映射框架，能够实现传感器无关的RAW目标检测，通过联合训练处理不同传感器RAW数据。

<details>
  <summary>Details</summary>

**Motivation:** 相机RAW数据具有更深的位深度、保留物理信息和不受ISP失真等优势，但不同设备间曝光条件、光谱敏感度和位深度的变化导致巨大的域差异，使传感器无关的泛化成为根本挑战。

**Method:** 提出将传感器引起的变异分解为由RAW分布先验驱动的全局色调校正和空间自适应的局部颜色调整；构建基于物理的RAW仿真流程，合成涵盖多样化光谱敏感度、光源和传感器非理想性的逼真传感器输出。

**Result:** 在覆盖10-24位深度的多个RAW基准数据集上，在单数据集、混合数据集和极具挑战性的鲁棒性设置下均实现了最先进的性能。

**Conclusion:** RAWild框架通过物理指导的全局-局部色调映射和基于物理的数据仿真，成功解决了RAW目标检测中传感器无关泛化的关键难题，为跨传感器RAW数据的应用提供了有效解决方案。
...
> MobileEgo Anywhere是一个利用普通手机设备收集超过1小时长时程自我中心轨迹数据的框架，包含200小时数据集、移动应用和处理管道，旨在解决当前视觉语言动作模型数据缺乏长期时序依赖的问题。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言动作模型的发展需要大规模自我中心数据集，但现有数据集持续时间通常只有几分钟，无法捕获复杂机器人任务执行所需的长期时序依赖关系。传统机器人数据采集存在硬件门槛高的问题。

**Method:** 利用现代智能手机的通用传感器套件，提供高保真、长期的相机姿态跟踪。开发移动应用让用户可以录制自我中心数据，并提供完整的处理流水线将原始移动捕获转换为标准化、训练就绪的格式。

**Result:** 发布包含200小时多样化、长形式自我中心数据的新数据集，具有持续状态跟踪；开源移动应用；提供综合处理管道。通过民主化数据采集过程，实现了跨不同全球环境的大规模长期数据获取。

**Conclusion:** 这项工作通过降低硬件门槛和提供完整的数据采集处理方案，显著推动了可泛化机器人策略的发展，加速了视觉语言动作模型和基础模型的研究进展。
...
> 针对SAM2在生物医学分割中的局限性，提出了一种免提示、参数高效的微调框架，通过卷积位置编码生成器和双适配器策略实现多类分割，在保持高精度的同时大幅降低计算成本。

<details>
  <summary>Details</summary>

**Motivation:** SAM2在自然图像上表现出色，但在生物医学分割中面临域移和提示依赖的挑战，现有的医学SAM适配方法虽提高了精度但计算成本高昂，无法满足实际医学应用的需求。

**Method:** 提出免提示的参数高效微调框架，包括：1）卷积位置编码生成器适应任意长宽比；2）双适配器策略-采用可变形卷积的高性能适配器用于精确边界建模，采用结构重参数化的轻量级适配器减少推理延迟。

**Result:** 在ISBI 2012、Kvasir-SEG、Synapse和ACDC数据集上，方法显著超越强基线，比原始SAM2精度提升高达19.66%，相比重型医学SAM适配计算成本降低约87%，实现了精度与效率的最佳权衡。

**Conclusion:** 该框架成功解决了SAM2在医学图像分割中的关键挑战，通过创新的参数高效设计在保证分割精度的同时大幅降低了计算复杂度，为医学图像分析的实用化提供了有效解决方案。
...
> iPhoneBlur是一个针对消费级移动设备运动模糊恢复的7400个图像对的新型基准测试，通过难度分层揭示模型性能差异，并提供元数据支持部署评估。

<details>
  <summary>Details</summary>

**Motivation:** 传统模糊恢复评估使用聚合指标掩盖了不同模糊难度下的性能变化，无法反映真实部署条件下的模型行为，需要更细粒度的评估方法。

**Method:** 从iPhone 17 Pro视频合成7400个图像对，通过PSNR引导的自适应时间窗口划分为Easy、Medium、Hard三个难度等级，验证分层有效性并提供ISP感知元数据。

**Result:** 评估显示模型从Easy到Hard子集性能下降7-9dB，揭示了专业和消费相机间的域差异，目标微调可显著恢复性能。

**Conclusion:** 通过难度分层与关键部署元数据结合，iPhoneBlur为资源受限边缘系统的模型可靠性和故障模式提供了系统性评估框架。
...
> 4DThinker是首个让视觉语言模型通过动态潜在心理意象进行4D思考的框架，通过内部模拟场景演化来实现动态空间推理。

<details>
  <summary>Details</summary>

**Motivation:** 当前视觉语言模型在动态空间推理方面面临挑战：要么完全依赖文本描述复杂动态场景（冗长且不精确），要么依赖外部几何模块（增加推理复杂度且无法培养模型内在能力）。

**Method:** 提出三阶段框架：1) 可扩展的无标注数据生成管道，从原始视频合成4D推理数据；2) 动态意象微调(DIFT)，联合监督文本标记和4D潜在表示；3) 4D强化学习(4DRL)，通过基于结果的奖励处理复杂推理任务。

**Result:** 在多个动态空间推理基准测试中，4DThinker持续超越强基线模型，为VLM的4D推理提供了新视角。

**Conclusion:** 4DThinker成功实现了让VLM通过内部动态心理意象进行4D推理，在保持内在模型能力的同时显著提升了动态空间推理性能。
...
> 本文提出了一种端到端的神经形态架构，用于手势字母识别，在仿真中达到92.27%的准确率，硬件部署上达到83.1%准确率，同时实现了最节能(0.565mW)和低延迟(3ms)的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的手势字母识别方法虽然分类准确率有所提高，但其延迟和功耗要求限制了其在实时部署中的适用性。神经形态感知和处理提供了一种基于稀疏、事件驱动计算的替代范式，支持低延迟和高能效的感知。

**Method:** 集成脉冲视觉注意机制进行在线感兴趣区域提取，并在SpiNNaker神经形态平台上部署紧凑的脉冲神经网络。使用两个数据集进行基准测试：合成的基于事件的手势字母MNIST数据集和原生录制的ASL-DVS数据集。

**Result:** 在仿真中获得92.27%的竞争性能，在神经形态硬件部署上达到83.1%的可比性能，同时在所有基准方法中实现了最节能的架构(0.565mW)和低延迟(3ms)。

**Conclusion:** 尽管设计紧凑，该系统证明了任务相关的视觉注意力应用适合边缘部署，展示了神经形态计算在实时手势识别中的可行性和优势。
...
> FusionProxy是一种实时RGB-红外图像融合模块，通过利用教师样本集的像素级方差和特征级方差统计，实现了扩散级别质量的实时融合效果，可即插即用于任何视觉感知系统。

<details>
  <summary>Details</summary>

**Motivation:** 纯RGB视觉模型在夜间和雾天等挑战性场景下表现不佳，现有的高保真融合方法延迟过高无法实时部署，需要一种实时、高质量的融合解决方案来提升全天候感知能力。

**Method:** FusionProxy作为独立的即插即-play组件，利用教师样本集的两种互补统计：原始图像空间的像素方差用于加权像素级监督，冻结基础骨干网络内部的特征方差用于空间路由特征级对齐，实现无需联合优化的直接集成。

**Result:** 在静态识别任务上实现卓越性能，显著增强动态任务（包括闭环自动驾驶）的鲁棒性，在高端GPU到 commoditiy硬件等多样化平台上实现实时推理速度。

**Conclusion:** FusionProxy为全天候感知提供了灵活且可泛化的解决方案，通过创新的统计权重机制实现了扩散级别质量与实时性能的平衡，具有重要的实用价值。
...
> 本文提出PFCVR模型解决文本到图像的车辆重识别问题，通过细粒度的局部-局部对齐和双向掩码恢复实现了显著性能提升，并构建了新的细粒度标注数据集T2I-VeRW。

<details>
  <summary>Details</summary>

**Motivation:** 传统车辆重识别仅支持图像查询，但现实场景中往往只有目击者的文字描述。将查询方式从图像扩展到文本，能够让车辆重识别技术在实际应用中更加实用和灵活，满足仅凭语言描述就能寻找目标车辆的需求。

**Method:** PFCVR模型包含两个核心设计：1) 零部件级细粒度跨模态对齐，通过可学习的部件查询token聚合局部特征和全局上下文；2) 双向掩码恢复模块，让图像和文本模态相互引导重建对方的掩码内容，实现全局特征对齐。此外，构建了包含细粒度部件标注的大规模数据集T2I-VeRW。

**Result:** 在T2I-VeRI数据集上达到29.2%的Rank-1准确率，比最佳竞争方法提升3.7个百分点；在新构建的T2I-VeRW基准上达到55.2%的Rank-1准确率，全面超越现有先进方法。

**Conclusion:** 通过局部-局部显式对齐和全局隐式对齐的结合，PFCVR成功解决了文本到图像车辆重识别中的跨模态对齐难题。新构建的细粒度标注数据集为该领域提供了更好的研究基准，实验结果验证了方法的有效性和优越性。
...
> PlotPick是一个基于视觉语言模型(VLMs)的开源工具，能够批量从科学图表中提取结构化表格数据，其性能明显优于专门的图表转表格模型DePlot。

<details>
  <summary>Details</summary>

**Motivation:** 系统综述和荟萃分析经常需要作者仅在图表中报告的数值数据，但手动数字化处理速度缓慢且无法规模化处理。

**Method:** 开发了PlotPick工具，利用视觉语言模型(VLMs)实现科学图表数据的批量提取；在ChartX和PlotQA两个基准测试上评估了6个来自3个提供商的VLM，并与专门的DePlot模型进行对比。

**Result:** 所有6个VLM模型在两个基准测试上都优于DePlot：在ChartX上VLM达到88-96%的召回率(DePlot为71%)；在PlotQA上VLM达到86-99%的RMSF1(DePlot为94%)；在专门模型训练数据中缺失的箱线图类型上，性能差距最大，VLM达到83-97%的RMSF1(DePlot仅24%)。

**Conclusion:** 基于视觉语言模型的方法在图表数据提取方面比专门的图表转表格模型表现更好，特别是在处理训练数据中未充分覆盖的图表类型时具有显著优势，为系统综述等研究提供了可扩展的自动化解决方案。
...
> PARSE框架通过将视觉识别分解为视觉基元及其关系组合，用于域泛化任务，在CUB-DG基准上提升4.5%准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有域泛化方法主要通过改进训练过程来寻找稳定表示，但忽略了对结构组合的显式建模，限制了在组合式域泛化任务上的性能。

**Method:** 提出PARSE框架，包含CNN主干网络提取特征、概念瓶颈层生成基元热图、结构评分层评估空间关系，通过软二元、三元和四元谓词表示空间对齐，实现端到端学习。

**Result:** 在CUB-DG上准确率提升超过4.5个百分点，在DomainBed基准套件上与现有域泛化方法保持竞争力。

**Conclusion:** 显式建模视觉基元和关系组合能够有效提升域泛化性能，特别是对组合式泛化任务。
...
> DPOFusion是一个基于直接偏好优化的红外与可见光图像融合框架，能够根据不同任务需求自适应调整融合结果，满足人类视觉和机器视觉的异质性偏好。

<details>
  <summary>Details</summary>

**Motivation:** 现有的红外与可见光图像融合方法缺乏灵活性，无法自适应地满足人类视觉和机器视觉的不同偏好要求，实现任务导向的自适应融合仍是一个具有挑战性的开放问题。

**Method:** 提出DPOFusion框架，包含属性对齐的潜在扩散模型(PALDM)和偏好可控的潜在扩散模型(PCLDM)。PALDM利用潜在融合先验和联合条件损失生成多样化候选融合结果，PCLDM通过实例直接偏好优化(IDPO)进行微调，实现对最终融合结果的直接控制。

**Result:** 实验结果表明，该框架不仅实现了人类、视觉语言模型和任务驱动网络之间的精确偏好对齐，还在自适应融合质量和任务导向可迁移性方面设立了新的基准。

**Conclusion:** DPOFusion通过引入直接偏好优化机制，成功解决了多模态融合中的自适应问题，为满足不同应用场景的异质性需求提供了有效的解决方案。
...
> RealCam是一个创新的实时、交互式摄像头控制视频到视频生成框架，通过跨帧上下文学习教师模型和自匹配蒸馏技术，实现了高效因果推理，解决了现有方法的高延迟和二次方复杂度问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频到视频生成方法依赖于非因果的全序列处理和刚性的前缀式时序连接，需要双向注意机制，导致计算延迟过高、复杂度二次方增长，且 inherently 不支持实时流媒体或可变长度输入。

**Method:** 设计基于跨帧上下文学习的高保真教师模型，通过将源帧和目标帧交错为同步上下文对；然后通过自匹配分布匹配蒸馏将教师模型蒸馏为少步因果学生模型；提出闭环数据增强技术解决闭环轨迹中的严重不一致问题。

**Result:** RealCam实现了最先进的视觉保真度和时序一致性，支持真正的交互式摄像头控制，推理速度比现有方法快几个数量级。

**Conclusion:** RealCam为实时交互式电影制作和现场广播提供了技术突破，通过创新的架构设计成功解决了现有V2V方法的根本性局限，为实时视频合成开辟了新道路。
...
> PersonaGesture是一种基于扩散的单参考语音伴随手势个性化方法，可通过单个动作片段为新说话者生成保持个人风格的同步手势。

<details>
  <summary>Details</summary>

**Motivation:** 为虚拟形象和智能代理应用开发无需针对每个说话者优化的个性化手势生成技术，解决参考动作中稳定说话习惯与话语特定轨迹混合的挑战。

**Method:** 提出双组件架构：自适应风格注入(ASI)通过风格感知器将参考编码为记忆标记，使用零初始化残差交叉注意力注入去噪过程；隐式分布校正(IDR)在潜在空间应用长度感知对角仿射映射来校正残差通道矩。

**Result:** 在BEAT2和ZeroEGGS数据集上的综合评估显示，分离去噪时的说话者记忆与保守的生成后矩校正，相比坍塌风格码、全参考注意力与单片段微调等方法，显著提升了未见说话者的个性化效果。

**Conclusion:** 通过将时序身份证据与残差统计校正有效分离，PersonaGesture实现了高质量的单参考语音伴随手势个性化，为实际应用提供了实用的解决方案。
...
> ArenaPO是一种新型的文本到图像扩散模型对齐方法，它利用Arena分数作为离线奖励，在避免显式奖励建模的同时实现了细粒度的优化，结合了传统RLHF的丰富奖励和DPO的高效性。

<details>
  <summary>Details</summary>

**Motivation:** 现有直接偏好优化(DPO)方法仅依赖二值反馈，只能在选择-拒绝对上进行粗粒度建模，导致优化效果不佳，因此需要一种既能保持高计算效率又能提供细粒度反馈的方法。

**Method:** 构建模型Arena，将每个模型能力表示为高斯分布，通过遍历标注的成对偏好来推断这些能力；将每个输出图像视为对应能力分布的样本，基于截断正态分布使用潜变量推断估计图像对间的绝对质量差距作为细粒度反馈。

**Result:** 在Pick-a-Pic v2和HPD v3数据集上进行的实验表明，ArenaPO在性能上持续优于现有基线方法，且无需额外训练开销。

**Conclusion:** ArenaPO成功结合了传统RLHF的丰富奖励优势和DPO的高效性，为文本到图像扩散模型的对齐提供了一种有效的细粒度优化方案。
...
> MSD-Score是一种无参考的图像字幕评估指标，通过将图像块和文本token建模为von Mises-Fisher混合分布，在超球面上进行多尺度分布匹配，有效检测语义不匹配问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的无参考图像字幕评估方法存在严重局限，全局嵌入相似度无法捕捉细粒度的语义不匹配，如对象幻觉、属性缺失和关系错误等问题。

**Method:** 将图像块和文本token嵌入表示为单位超球面上的von Mises-Fisher混合分布，采用加权的双向KL divergence量化语义差异，构建多尺度框架结合局部分布相似度和全局相似度，支持单候选和多候选评估。

**Result:** MSD-Score在无参考指标中取得了与人类判断的最先进相关性，其概率化公式提供了透明且可分解的局部基础错误诊断能力。

**Conclusion:** MSD-Score不仅提高了评估准确性，还提供了确定性的互补诊断信号，补充了传统整体相似度指标和基于人类的评估方法。
...
> Holmes是一个分层证据学习框架，通过聚合多粒度跨模态证据来显式量化和建模部分相关视频检索中的不确定性问题。

<details>
  <summary>Details</summary>

**Motivation:** 部分相关视频检索存在查询与视频内容之间的不对称性，导致语义模糊和稀疏时间监督问题，传统方法难以有效处理这种检索过程中的不确定性。

**Method:** 提出分层证据学习框架：1）视频间层面将相似性分数解释为证据支持并用狄利克雷分布建模，基于三重原则进行细粒度查询识别和查询自适应校准学习；2）视频内层面通过带自适应垃圾箱的柔性最优传输进行软查询-片段对齐，积累更密集证据。

**Result:** 在充分实验中证明Holmes性能优于现有最先进方法，有效缓解了语义模糊和稀疏时间监督问题。

**Conclusion:** 该框架通过显式建模不确定性和聚合多粒度证据，为部分相关视频检索提供了新的解决思路，在理论和实践上都具有重要价值。
...
> AMIEOD是一个面向低光照场景的图像增强目标检测框架，通过多专家增强模块和检测引导的策略选择，显著提升弱光环境下的目标检测精度。

<details>
  <summary>Details</summary>

**Motivation:** 在低光照条件下捕获的图像会导致视觉感知任务的准确性降低，现有的目标检测算法在暗光环境下性能不佳，需要专门的增强和检测协同优化方案。

**Method:** 提出多专家图像增强模块(MEIEM)利用多样化的增强策略；设计检测引导回归损失(DGRL)使增强与检测任务对齐；构建专家选择模块(ESM)通过检测引导交叉熵损失(DGCE)动态选择最优增强策略。

**Result:** 在多个数据集上的广泛实验证明，该方法显著提升了低光照条件下的目标检测精度，且与现有检测算法兼容性良好。

**Conclusion:** AMIEOD框架成功地将图像增强与目标检测任务以检测性能为导向进行联合优化，为解决低光照场景下的视觉感知问题提供了有效方案，代码已开源。
...
> LARGO是一个基于超网络和CP张量分解的解决方案，将2^N-1个缺失模态模型压缩为单一网络，在多模态图像分析中取得了优异效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有解决缺失模态的方法通常需要复杂架构，难以在不同数据集间迁移，且大多数方法在特征空间操作，存在泛化性问题。

**Method:** 提出LARGO超网络，在权重空间而非特征空间操作，使用CP张量分解对卷积权重进行建模，将多个专门的缺失模态模型压缩为单一网络。

**Result:** 在BraTS 2018和ISLES 2022数据集的52种配置中，有47种排名第一，相比最先进基线方法平均Dice分数分别提升0.68%和2.53%。

**Conclusion:** LARGO方法不仅局限于医学影像，通过avMNIST的试点实验表明其可能适用于异构非医学模态，具有很强的泛化能力。
...
> OpenGaFF是一个基于3D高斯分布的开放词汇3D场景理解框架，通过高斯特征场和结构化码本实现了几何与语义的强耦合及跨视图一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于高斯表示的开放词汇3D场景理解方法存在语义预测碎片化和空间不一致的问题，缺乏几何结构与语义之间的紧密关联。

**Method:** 提出高斯特征场将语义建模为高斯几何和外观的连续函数，引入结构化码本作为共享语义原语，设计码本引导注意力机制进行语言特征检索和开放词汇推理。

**Result:** 在标准2D和3D开放词汇基准测试中持续超越现有方法，获得更好的分割质量、更强的3D语义一致性，并提供了可语义解释的码本。

**Conclusion:** 该方法通过几何-语义耦合结构和码本约束，有效解决了多视图观察中的语义碎片化和空间不一致问题，为开放词汇3D场景理解提供了新的技术途径。
...
> 本文提出了一种名为Tracker的新型自监督跟踪框架，通过双模态上下文关联机制（结合细粒度语义提示和上下文噪声）从未标记视频中学习鲁棒的跟踪表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有自监督跟踪器缺乏有效的上下文建模，而基于非语义查询的上下文关联方法难以适应未标记跟踪场景，无法学习可靠的上下文线索。

**Method:** 采用从易到难的学习原理，通过两个阶段实现上下文关联：早期训练中使用实例块标记提示促进跟踪知识获取；后期逐步注入上下文噪声扰动特征，促使跟踪器在更复杂特征空间中学习鲁棒表示。

**Result:** 该方法从未标记视频中成功学习到高质量跟踪表示，且仅在训练时使用，保持推理效率。大量实验证明了方法的优越性。

**Conclusion:** 提出的双模态上下文关联机制有效解决了自监督跟踪中的上下文建模难题，实现了从未标记视频中学习鲁棒跟踪表示的目标。
...
> VISD提出了一种结构化自蒸馏框架，通过视频感知的评判模型提供多维度诊断反馈，结合方向大小解耦机制稳定集成强化学习，显著提升了视频大语言模型的推理能力和训练效率。

<details>
  <summary>Details</summary>

**Motivation:** 视频大语言模型在复杂推理训练中面临序列级奖励稀疏和长时序轨迹中细粒度信用分配困难的挑战。现有RLVR方法缺乏token级贡献捕捉，而传统自蒸馏方法缺乏结构性和诊断特异性，与RL交互不稳定。

**Method:** VISD采用视频感知评判模型将推理质量分解为答案正确性、逻辑一致性和时空 grounded等多个维度，通过结构化反馈指导教师策略进行token级监督；引入方向大小解耦机制，让rollout级奖励优势决定更新方向，结构化特权信号调节token级更新大小；配合课程调度和EMA教师稳定化。

**Result:** 在多个基准测试中，VISD一致优于强基线，提高了答案准确性和时空grounded质量；同时优化步数收敛速度提升近2倍，显著改善了性能和样本效率。

**Conclusion:** 结构化自监督通过语义对齐的细粒度信用分配有效改善了VideoLLMs的推理忠实度和训练效率，为视频推理任务提供了新的优化范式。
...
> 现代预训练视觉Transformer违反了局部性假设，存在强烈的对象内部信息泄漏现象，导致基于注意力的可解释方法失效，而两阶段方法可有效解决此问题。

<details>
  <summary>Details</summary>

**Motivation:** 基于部分的推理是计算机视觉中让模型关注相关对象部分的经典策略，同时提高深度学习模型的可解释性。这一策略基于局部性假设，即对象部分的潜在表示主要编码对应图像区域的信息。但这一基本假设在现代深度模型中是否成立尚未得到验证。

**Method:** 通过基于部分的属性注释来测量视觉模型中的对象内部信息泄漏现象，对现代预训练视觉Transformer进行全面实验评估。同时提出两阶段方法作为上界，通过设计防止信息泄漏实现固有解耦的特征提取。

**Result:** 实验表明现代预训练视觉Transformer违反局部性假设，存在强烈的对象内部泄漏，每个部分编码整个对象的信息，这是一种视觉转喻现象，损害了基于注意力的可解释方法的可靠性。两阶段方法能有效防止泄漏，并在多种任务上提升了属性驱动的部分发现效果。

**Conclusion:** 揭示了影响基于部分表示可解释性的一个被忽视的问题，特别是依赖基于部分概念的CBMs。研究表明两阶段方法是缓解这一问题的有效途径，对实际应用具有积极影响。
...
> 本文提出了一种基于事件相机的目标跟踪框架，通过感知事件稀疏性，在多个时间尺度上建模事件密度变化，实现了跟踪精度和计算效率的良好平衡。

<details>
  <summary>Details</summary>

**Motivation:** RGB跟踪器在低光照和快速运动等挑战性成像条件下表现不佳，而现有的事件跟踪器忽略了事件数据的固有空间稀疏性和时间密度特性，且采用单一固定时间窗采样策略在变化运动动态下表现次优。

**Method:** 提出事件稀疏感知跟踪框架，通过三阶段视觉Transformer骨干网络逐步注入稀疏、中等密度和密集事件搜索区域，实现分层多密度特征学习。同时引入稀疏感知混合专家模块，并设计动态权衡策略根据跟踪难度自适应调整推理深度。

**Result:** 在FE240hz、COESOT和EventVOT数据集上的广泛实验表明，该方法在跟踪精度和计算效率之间取得了良好的权衡。

**Conclusion:** 本文框架通过显式建模事件密度变化和稀疏感知机制，有效提升了事件相机在复杂条件下的跟踪性能，为事件视觉跟踪领域提供了新的解决方案。
...
> Pest-Thinker是一个基于知识驱动强化学习的框架，通过构建专业害虫数据集和新的优化策略，提升多模态大模型在害虫识别中的细粒态推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 害虫造成的作物损失威胁全球粮食安全，但现有MLLMs由于害虫种类复杂、种内变异大、专家标注数据稀缺等挑战，在害虫识别领域的直接应用受限。

**Method:** 构建QFSD和AgriInsect两个高清害虫基准数据集，通过链式思维轨迹和监督微调学习害虫视觉线索，采用群体相对策略优化和基于特征的奖励机制引导模型关注形态学证据。

**Result:** 实验表明Pest-Thinker显著提升了模型在域内域外的形态学理解能力，为智能农业害虫分析的专家级视觉推理迈出了重要一步。

**Conclusion:** 该方法成功解决了MLLMs在害虫识别领域的关键挑战，为智能农业发展提供了新的技术路径，数据集和代码将在论文接受后公开。
...
> 本文提出连续专家组装(CEA)框架，通过交叉注意力超适配器实现令牌级动态参数化，有效处理未知、空间变化和复合的真实图像退化问题。

<details>
  <summary>Details</summary>

**Motivation:** 真实世界的图像退化具有未知性、空间非均匀性和复合性，现有方法的全局调节可能成为瓶颈，静态专家路由可能产生同质化更新，需要更适应性的解决方案。

**Method:** CEA采用轻量级交叉注意力超适配器探测中间空间特征，合成实例条件化的低秩路由基和残差方向，每个空间令牌通过密集符号点积亲和力组装自己的残差更新。

**Result:** 在AIO-3、AIO-5和CDD-11数据集上，CEA在平均恢复质量上优于强基线方法，在空间变化和复合退化任务上提升最明显，同时保持了良好的参数、FLOP和运行时效率。

**Conclusion:** CEA框架通过令牌级动态组装避免了外部提示、静态专家库和离散选择，为处理复杂真实世界图像退化提供了有效的解决方案。
...
> Prologue通过在图像序列前添加一组专门的生成前导令牌，有效解决了自回归图像生成中的重建-生成权衡问题，在不影响重建质量的情况下显著提升了生成效果。

<details>
  <summary>Details</summary>

**Motivation:** 自回归图像生成模型面临重建质量与生成质量之间的根本冲突，因为视觉令牌需要同时满足重建和生成两个目标，这种耦合设计限制了生成性能的提升。

**Method:** Prologue方法引入一组与视觉令牌分离的前导令牌，这些令牌仅使用AR交叉熵损失进行训练来优化生成，而视觉令牌专门负责重建任务，通过ELBO视角形式化了解耦设计的理论基础。

**Result:** 在ImageNet 256x256上，Prologue-Base将gFID从21.01降至10.75（无分类器引导），重建质量几乎不变；Prologue-Large达到rFID 0.99和gFID 1.46的竞争性结果。前导令牌还展现出语义结构，16个令牌的线性探测Top-1准确率达35.88%。

**Conclusion:** 通过引入独立的生成表示而不改变原始表示，提供了一个新的改进生成质量的方向，前导令牌展现出有趣的语义涌现特性，为未来研究提供了新思路。
...
> 本研究开发了一个集成16种XAI方法的AI生成图像检测框架，通过大规模数据集训练和人类认知评估，提升检测系统的可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 生成式AI在虚假信息宣传中的滥用迫切需要透明且可解释的检测系统，现有的检测方法缺乏人类可理解的解释能力。

**Method:** 构建大型真实感虚假图像数据集AIText2Image，开发多种架构和微调策略的检测器，集成16种XAI方法，通过100名参与者的调查评估视觉解释的人类理解度。

**Result:** 建立了一个全面的检测框架，能够提供人类偏好的视觉-语言线索解释，实现了XAI输出与人类偏好之间的对齐测量。

**Conclusion:** 该框架为虚假图像检测中的视觉-语言线索提供了新见解，从人类角度评估了XAI方法的清晰度，为建立更加透明可信的AI检测系统奠定了基础。
...
> 本文提出wAR-Tok方法，通过在离散图像标记器训练中添加分布级别的先验匹配信号，解决两阶段训练导致的标记器与自回归模型不匹配问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统的两阶段训练中，图像标记器与后续的自回归先验模型分离训练，导致标记器虽然能很好地保持图像信息，但产生的标记序列对自回归模型来说难以预测。这种不匹配影响了生成效果。

**Method:** 基于三分变分一致性(TVC)框架分析问题，在标记器训练中引入分布级别的先验匹配信号。使用Wasserstein梯度流优化该信号，对于分类标记简化为辅助AR模型与目标AR模型之间的标记级对比，仅需前向传播计算。

**Result:** wAR-Tok在保持相当重建质量的同时，降低了AR损失，在CIFAR-10和ImageNet数据集上提升了生成的FID指标。

**Conclusion:** 将先验模型的信息融入标记器训练过程可以有效改善标记的可预测性，为离散图像表示学习提供了新的训练范式。
...
> 本文提出了医学图像分割持续学习的基准研究，定义了三种临床场景，建立了全面的评估框架，并对现有方法进行了系统性分析。

<details>
  <summary>Details</summary>

**Motivation:** 医学图像分割模型在临床环境中部署需要持续学习能力，但该领域面临场景标准化不足、过度关注遗忘而忽视可塑性、缺乏全面基准等三大挑战。

**Method:** 定义了Domain-CL、Class-CL和Organ-CL三种临床场景，建立了包含泛化性能、遗忘、可塑性、前向泛化性、参数效率和回放负担的评估框架，对代表性持续学习方法进行了大规模实验。

**Result:** 回放方法在稳定性和可塑性之间取得最佳平衡；参数隔离方法能有效减少遗忘但增加模型大小；前向泛化性是该领域研究不足的重要方面；现有方法难以同时满足所有要求。

**Conclusion:** 该基准研究为医学图像分割持续学习提供了标准化评估体系，揭示了现有方法的优缺点，为未来研究方向提供了指导。
...
> DynT2I-Eval是一个动态文本生成图像评估框架，通过构建结构化的视觉语义空间持续生成新提示词，解决了传统评估中因提示词集固定导致的过拟合和污染问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本生成图像评测主要依赖固定的提示词集，一旦公开并被重复使用，容易出现模型过拟合和基准污染的问题，影响评估的可靠性和公平性。

**Method:** 从长文本描述构建结构化视觉语义空间，将提示词分解为主体、逻辑约束、环境、构图等可控维度，通过特定任务空间和难度感知采样持续生成新提示词，采用成对比较、动态度调器、微批次聚合和加权贝叶斯更新实现动态评估。

**Result:** 实验表明，持续刷新的提示词提供了鲁棒的评估协议，减少了提示词集特定的调优影响，所提出的排名框架在冷启动收敛、后期发现和长期排名保真度之间实现了良好平衡。

**Conclusion:** DynT2I-Eval通过动态生成提示词和统一评估框架，为文本生成图像模型提供了一个稳定可靠的在线评测环境，有效克服了传统静态评估的局限性。
...
> Retina-RAG是一个低成本模块化框架，通过结合高性能分类器、参数高效的视觉语言模型和检索增强生成技术，实现了糖尿病视网膜病变严重程度分级、黄斑水肿检测和结构化报告生成的一体化解决方案。

<details>
  <summary>Details</summary>

**Motivation:** 糖尿病视网膜病变是全球劳动年龄成年人可预防性失明的主要原因，但当前大多数自动筛查系统仅限于图像级分类，缺乏临床结构化报告功能，同时现有系统存在计算资源需求高、幻觉问题严重等局限性。

**Method:** 该框架解耦了高性能视网膜分类器和通过LoRA适配的参数高效视觉语言模型(Qwen2.5-VL-7B-Instruct)，并在推理时通过RAG模块注入精选的眼科知识和结构化分类器输出，以提升诊断一致性并减少幻觉。

**Result:** 在视网膜疾病检测数据集上，Retina-RAG在DR分级上达到0.731的F1分数，在ME检测上达到0.948，显著优于零样本Qwen(0.096, 0.732)和MMed-RAG(0.541, 0.641)；报告生成方面达到ROUGE-L 0.429和SBERT相似度0.884，超越所有基线模型。

**Conclusion:** 该完整框架可在单张消费级GPU上运行，证明了临床结构化视网膜AI可以用适度的计算资源实现，为糖尿病视网膜病变的自动化筛查提供了实用、低成本的解决方案。
...
> SuperFace是一个偏好人脸表情优化框架，通过人类偏好反馈而非依赖伪标签来改进ARKit系数预测，实现更真实表情动画。

<details>
  <summary>Details</summary>

**Motivation:** 现有ARKit表情估计方法依赖Live Link Face等软件生成的伪标签，但这些标签包含噪声、偏差和不准确动作，导致监督学习模型只能复制不完美的伪标签而非优化感知表情保真度。

**Method:** SuperFace将软件估计的ARKit系数仅作为初始化，通过人类对渲染表情的偏好反馈进行迭代优化，使模型与感知判断对齐而非数值伪标签。

**Result:** 实验证明SuperFace在表情保真度方面优于Live Link Face监督方法，验证了偏好驱动优化在语义面部动作预测中的有效性。

**Conclusion:** 通过将ARKit表情估计从伪标签模仿转向人类感知优化，SuperFace实现了视觉上更真实和富有表现力的人脸动画，为数字人动画提供了更可靠的解决方案。
...
> EA-WM是一个感知事件驱动的生成式世界模型，通过将运动学状态直接投影为结构化的运动-视觉动作场，有效连接了运动学控制与视觉感知的闭环。

<details>
  <summary>Details</summary>

**Motivation:** 现有的世界动作模型将视频生成仅作为策略学习的辅助表示，未能充分利用动作信号指导视频合成，导致生成的滚动结果中无法保持精确的机器人空间几何结构和细粒度的机器人-物体交互动态。

**Method:** EA-WM将动作和运动学状态直接投影到目标相机视图中，形成结构化的运动-视觉动作场；引入感知事件的双向融合模块来调控跨分支注意力，捕捉物体状态变化和交互动态。

**Result:** 在全面的WorldArena基准测试中，EA-WM实现了最先进的性能，显著优于现有基线方法。

**Conclusion:** 通过将运动学控制与视觉感知有效结合，EA-WM为机器人世界模型提供了一种新的解决方案，能够在生成滚动结果时保持精确的空间几何和交互动态。
...
> 本研究提出了一种多模态可解释性框架，结合CNN、视觉归因方法和大语言模型，为脑肿瘤分类生成临床可理解的诊断报告，提高AI辅助诊断的透明度和可信度。

<details>
  <summary>Details</summary>

**Motivation:** 深度学习模型的不透明性是其医学影像临床应用的主要障碍。现有模型缺乏与临床医生沟通的能力，需要将模型预测转化为临床可操作、人类可理解的诊断见解。

**Method:** 框架包含三个耦合阶段：1）九种CNN架构扩展为双输出混合模型，同时优化分类和分割任务；2）应用Grad-CAM系列方法生成类别判别热图，通过自适应百分位阈值处理转化为肿瘤掩码；3）将掩码映射到Harvard-Oxford皮层图谱，结构化编码后输入LLM（Grok3、Mistral、LLaMA）生成放射科风格的诊断报告。

**Result:** 在4,834幅对比增强T1加权脑MRI图像上，InceptionResNetV2分类性能最佳，Grad-CAM++分割重叠度最高。语言模型中Grok3在词汇多样性和连贯性领先，LLaMA可读性得分最高。

**Conclusion:** 通过整合视觉、解剖和语言模态，该框架产生了既技术可靠又意义可解释的结果，推进了人工智能辅助脑肿瘤诊断的透明度和临床问责性。
...
> 论文提出了Variable Codebook Size Quantization (VCQ)方法，通过让码本大小沿序列单调增长，解决了视觉分词器中的熵悬崖问题，在ImageNet上显著降低了gFID指标。

<details>
  <summary>Details</summary>

**Motivation:** 传统的离散视觉分词器采用恒定码本设计存在信息论限制：在ImageNet上，仅前2个位置信息量为非确定性，后续254个位置变成记忆问题，这种现象称为熵悬崖。

**Method:** 提出VCQ方法，让码本大小从K_min=2单调增长到K_max，保持损失函数、参数数量和自回归训练过程不变，使用标准Transformer进行next-token预测。

**Result:** 基础版VCQ将ImageNet 256×256的gFID从27.98降至14.80；扩展版在684M参数下达到1.71 gFID；仅用前10个token的线性探针在ImageNet上达到43.8% top-1准确率。

**Conclusion:** 码本的容量分布和组织方式比总容量本身更重要，极端信息瓶颈自然诱导了粗到细的语义层次结构。
...
> 提出一种可微分框架，通过自适应计算4D光照条件，使用结构化光和单相机同时高效获取物体的形状和反射率。

<details>
  <summary>Details</summary>

**Motivation:** 解决现有方法在形状和反射率同时获取时的效率和精度问题，特别是针对形状和外观变化较大的物理对象。

**Method:** 构建基于像素级概率模型的深度和反射率估计框架，通过可微分链接将下一光照条件与深度不确定度降低相关联，迭代更新不确定度，最后基于微调方法重建深度图和反射率参数图。

**Result:** 在形状和外观变化较大的物理对象上证明了框架有效性，深度结果优于当前最佳技术，反射率结果与照片验证相当。

**Conclusion:** 该可微分自适应光照计算框架能够高效、高质量地同时获取物体形状和反射率，为3D重建和表面属性测量提供了新的解决方案。
...
> 提出反向注意力嵌入机制，通过捕获传统视觉编码器忽略的背景区域来提升拥挤场景中的视频语义搜索性能，无需额外训练即可显著提高检索效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频语义搜索方法在拥挤场景中存在局限性，视觉编码器倾向于关注显著的前景区域而忽略了具有上下文重要性的背景区域，导致搜索性能下降。

**Method:** 设计反向注意力嵌入机制，显式捕获并突出被传统方法忽视的背景区域，将反向注意力嵌入与传统视觉嵌入相结合，构建增强的语义表示。

**Result:** 初步实验和消融研究证明，该方法在拥挤环境下的视频语义搜索中，在召回率指标上相比现有方法取得了显著提升。

**Conclusion:** 反向注意力嵌入机制能够有效解决拥挤场景中背景信息被忽视的问题，是一种即插即用的增强方法，在视频语义搜索领域具有实用价值和应用前景。
...
> ZScribbleSeg是一种基于涂鸦标注的医疗图像弱监督分割方法，通过研究高效涂鸦标注原则、引入空间关系和形状约束正则化项，在六个分割任务上实现了竞争性能。

<details>
  <summary>Details</summary>

**Motivation:** 医疗图像分割的全标注数据集构建费时费力，而现有的基于涂鸦标注的弱监督分割方法由于监督不足和形状信息不完整，常常产生不准确和不切实际的分割结果。

**Method:** 首先研究高效涂鸦标注原则，通过监督最大化和随机性模拟得到高效涂鸦形式；然后引入正则化项编码空间关系和形状约束，使用EM算法估计标签类别的混合比例；最后将高效涂鸦监督与先验知识集成到统一框架中。

**Result:** ZScribbleSeg在ACDC、MSCMRseg、BTCV、MyoPS、Decathlon-BrainTumor和Decathlon-Prostate六个分割任务上，仅使用涂鸦标注就实现了竞争性能。

**Conclusion:** 通过高效涂鸦标注策略和空间先验知识的有机结合，ZScribbleSeg证明了涂鸦标注在医疗图像分割中的潜力，为减少标注负担提供了有效解决方案。
...
> Spark3R是一种无需重新训练的3D重建加速框架，通过解耦查询令牌和键值令牌的压缩策略，为基于ViT的前馈3D重建模型实现最高28倍的加速效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于视觉Transformer的前馈3D重建模型在处理长视频输入时面临计算效率挑战，主要原因是全局注意力层的二次复杂度。现有的令牌合并方法采用了无差别的压缩策略，忽略了查询令牌和键值令牌在3D重建中的不同功能角色。

**Method:** Spark3R基于查询令牌编码视角特定几何信息且对压缩敏感、键值令牌表示共享场景上下文且能容忍激进压缩的关键洞察，采用分别降因子策略：对查询令牌应用组内令牌合并，对键值令牌使用轻量级令牌剪枝，并跨层自适应调整键值降因子。

**Result:** Spark3R作为即插即用框架可直接应用于VGGT、π³和Depth-Anything-3等多个预训练模型，在1000帧输入上实现最高28倍加速，同时保持竞争性的重建质量。

**Conclusion:** 通过区分查询令牌和键值令牌的功能特性并采用相应的压缩策略，Spark3R有效解决了基于ViT的3D重建模型在处理长视频序列时的效率瓶颈，为实际应用提供了一种无需重新训练的高效解决方案。
...
> 本文展示了一个部署在九卫星商业热红外星座上的在轨野火检测系统，通过密集掩码自编码预训练实现了亚兆字节模型、毫秒级推理和10分钟端到端警报链路。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于上下文热阈值的方法无法解决在极端类别不平衡、未校准单波段中红外图像中检测亚像素或单像素热异常的挑战。

**Method:** 采用密集掩码自编码(DenseMAE)和混合DenseMAE+EMA蒸馏变体进行轻量级密集表示学习，通过线性探针和像素级平均精度评估表示质量。

**Result:** 最快的预训练模型达到0.640测试AP和0.69 Fire-F1，延迟65.34ms，模型仅0.52MB；最佳配置在1MB以内达到0.699 AP和0.744 Fire-F1，超越监督基线。

**Conclusion:** DenseMAE预训练能够在严格的计算资源约束下实现高精度野火检测，为在轨边缘智能提供了有效的解决方案。
...
> 本文提出了一种基于欧拉运动场的图像动画方法，通过相邻帧运动指导和双向几何一致性机制，实现了更快的训练速度和更好的时间连贯性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于拉格朗日运动引导的图像动画框架存在训练时间长、监督误差累积等问题，需要寻找更高效的局部监督设计方案。

**Method:** 创新性地使用相邻帧的欧拉运动场来指导生成，并引入双向几何一致性机制进行前向后向循环检查，识别并遮挡遮挡区域，防止模型学习错误的扭曲目标。

**Result:** 实验表明该方法相比基线方法显著加速了训练过程，同时保持了更好的时间连贯性并减少了动态伪影。

**Conclusion:** 通过将运动监督从全局拉格朗日视角转向局部欧拉视角，并配合双向几何一致性约束，可以有效提升图像动画的质量和训练效率。
...
> NOVA框架通过使用坐标式隐式神经表示(INR)作为系统状态，实现了高效、可解释的世界模型，能够在消费级GPU上运行，支持零样本超分辨率和可控视频编辑。

<details>
  <summary>Details</summary>

**Motivation:** 当前世界模型将原始像素编码为不透明的潜在空间并依赖重型解码器进行重建，导致计算成本高且缺乏可解释性，需要寻找更高效、结构化的表示方法。

**Method:** 提出NOVA框架，将系统状态表示为坐标式INR的权重和偏置，通过分析渲染消除解码器瓶颈，并采用动作匹配目标将模型蒸馏为上下文相关的视频生成器。

**Result:** 在多个挑战性数据集上验证，NOVA实现了强可控预测，模型仅约40M参数可在单张消费级GPU上运行，能够解耦背景、前景和帧间运动等场景组件。

**Conclusion:** 像INR这样的结构化表示不仅增强了对潜在动态的理解，还为沉浸式和可定制的虚拟体验铺平了道路，展示了结构化世界模型的优势。
...
> 本文提出TD-VLN方法，将视觉语言导航转化为基于预建地图的一次性全局路径规划问题，通过NavOne框架实现8倍和80倍的速度提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言导航方法采用逐步的以自我为中心的范式，存在错误累积和效率低下的问题；而基于地图的方法在连续空间推理方面存在限制。

**Method:** 提出TD-VLN方法，构建R2R-TopDown数据集；开发NavOne统一框架，包含Top-Down地图融合器和空间感知的深度混合注意力残差机制，实现单步骤端到端全局路径规划。

**Result:** 在R2R-TopDown数据集上，NavOne在基于地图的VLN方法中达到最先进性能，规划阶段速度比现有基于地图基线快8倍，比以自我为中心方法快80倍。

**Conclusion:** 通过将导航重构为全局路径规划问题并利用预建地图，TD-VLN和NavOne实现了高效的全局导航，为视觉语言导航提供了新的解决方案。
...
> TinyBayes框架结合了YOLOv8-Nano、MobileNetV3-Small和Jacobi-DMR贝叶斯分类器，构建了一个9.5MB的可可疾病检测系统，在150ms内实现准确率达78.7%的边缘设备推理。

<details>
  <summary>Details</summary>

**Motivation:** 西非可可种植者面临CSSVD和炭疽病的严重威胁，但现有的植物疾病检测系统要么依赖端到端深度学习缺乏不确定性量化，要么的贝叶斯方法专注于硬件架构而非农业应用，迫切需要适合资源受限环境的小型高效解决方案。

**Method:** TinyBayes框架采用三阶段流水线：使用5.9MB的YOLOv8-Nano定位病斑，3.5MB的MobileNetV3-Small提取特征，仅需13.5KB的Jacobi-DMR分类器进行贝叶斯分类，通过投影提供闭式非迭代估计器。

**Result:** 在Amini可可污染挑战数据集上达到78.7%准确率，总模型大小9.5MB，CPU推理时间低于150ms每张图像，相比Random Forest、SVM等七种分类器在精度、模型大小和推理速度方面提供最佳平衡。

**Conclusion:** TinyBayes成功解决了边缘设备上农作物疾病检测的关键挑战，通过集成贝叶斯方法与移动级计算机视觉，为资源受限环境提供了一种小型、快速且无需网络连接的实用解决方案，具有理论保证和实际部署价值。
...
> Earth-o1是一个基于观测数据的大气世界模型，它直接从非网格化的多模态传感器数据中学习地球系统的三维物理演化，实现了实时预报，并在后报评估中达到了与业务预报系统相当的精度。

<details>
  <summary>Details</summary>

**Motivation:** 现代对地观测系统提供了前所未有的多模态数据量，但传统建模框架将异构测量数据强制放入预定义的空间网格中，无法充分利用原始传感器数据，且造成严重的计算瓶颈。

**Method:** Earth-o1采用观测原生范式，不依赖传统大气动力学建模或数据同化，而是直接从非网格化观测数据学习连续的三维物理演化。它将多样化的传感器输入集成到统一的无网格动力场中，自主推进大气状态在空间和时间上的演化。

**Result:** 在后报评估中，Earth-o1的表面预报技巧与业务集成预报系统(IFS)相当，实现了直接实时预报和跨传感器推理，无需显式数值求解器的开销。

**Conclusion:** 连续的、观测驱动的世界模型作为新的完全观测原生物理模拟器类别，能够匹配既定物理框架的精度，为地球数字孪生提供了可扩展的数据驱动基础。
...
> SwiftI2V是一个高效的2K分辨率图像到视频(I2V)生成框架，通过分段生成和强图像条件引导，在保持高质量的同时实现了202倍的加速。

<details>
  <summary>Details</summary>

**Motivation:** 现有的高分辨率I2V生成方法存在两个主要问题：1)端到端模型内存和延迟成本极高；2)级联低分辨率生成与通用视频超分辨率会产生细节幻觉和偏离输入图像结构。

**Method:** 采用两阶段设计：首先生成低分辨率运动参考，然后基于运动指导和强图像条件进行2K分辨率合成。核心技术包括条件分段生成(CSG)和分段内的双向上下文交互机制。

**Result:** 在VBench-I2V 2K分辨率基准测试中，性能与端到端基线相当，总GPU时间减少202倍，可在单个数据中心GPU(H800)或消费级GPU(RTX 4090)上实现实用的2K I2V生成。

**Conclusion:** SwiftI2V成功解决了高分辨率I2V生成的效率-保真度权衡问题，为实际应用提供了可行的技术方案，显著降低了计算资源门槛。
...
> eX2L是一个基于解释的可解释框架，通过惩罚主要分类器和混肴干扰分类器的Grad-CAM激活图相似性来实现特征解耦，在分布偏移场景下显著超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有分布偏移缓解算法性能不稳定且难以解释，无法有效处理虚假相关性问题，需要一种既高效又可解释的解决方案。

**Method:** 提出eX2L框架，在训练过程中同时训练主要标签分类器和混肴干扰分类器，通过惩罚两者Grad-CAM激活图的相似性来解耦混肴特征，实现功能域不变性。

**Result:** 在Spawrious Many-to-Many Hard Challenge基准上，平均准确率达82.24%（±3.87%），最差组准确率66.31%（±8.73%），分别比当前最优方法提升5.49%和10.90%。

**Conclusion:** 证明了在组级别显式解耦标签和干扰属性可以实现功能域不变性，为处理分布偏移问题提供了可解释且高效的解决方案。
...
> 本文提出连续时间分布匹配(CDM)方法，将传统的离散时间扩散模型蒸馏技术扩展到连续时间优化，通过动态连续调度和连续时间对齐目标，显著提升了少步图像生成的视觉质量。

<details>
  <summary>Details</summary>

**Motivation:** 传统的分布匹配蒸馏(DMD)方法依赖于稀疏的离散时间步监督，容易产生视觉伪影和过平滑输出，通常需要GAN或奖励模型等复杂辅助模块来恢复视觉保真度。一致性方法虽然能改善质量但计算成本较高。

**Method:** CDM包含两个核心设计：1)用随机长度的动态连续调度替代固定离散调度；2)提出连续时间对齐目标，对通过学生速度场外推的潜在变量进行主动离轨迹匹配。这使得分布匹配可以在采样轨迹的任意点执行。

**Result:** 在SD3-Medium和Longcat-Image等不同架构上的实验表明，CDM在少步图像生成方面提供了高度竞争的视觉保真度，无需依赖复杂的辅助目标。

**Conclusion:** CDM成功地将DMD框架从离散锚定迁移到连续时间优化，为加速扩散模型提供了新的技术路线，在保持高视觉质量的同时简化了训练过程。
...
> 该研究提出了一种迭代四边形网格填充方法，通过构建决策区域内的标签保持表面，实验证明了深度神经网络决策区域不仅是路径连通的，而且是简单连通的。

<details>
  <summary>Details</summary>

**Motivation:** 理解深度神经网络决策区域的拓扑结构对于解释网络内部工作原理至关重要。已有研究表明这些区域是路径连通的，但本文探讨更强的拓扑性质——决策区域内的闭环是否可以在不离开该区域的情况下收缩。

**Method:** 提出迭代四边形网格填充程序，构造有限分辨率的标签保持表面，该表面以给定环路为边界且完全位于同一决策区域内。同时将此构造与自然Coons补片连接，以量化其与环路规范几何插值的偏差。

**Result:** 在多个现代图像分类模型上评估该方法，提供了经验证据支持深度神经网络决策区域不仅是路径连通的，而且是简单连通的这一假设。

**Conclusion:** 该方法为理解深度学习模型的拓扑特性提供了新的分析工具，并为决策区域具有简单连通性这一重要拓扑特征提供了实证支持，这有助于更好地解释神经网络的决策机制。
...
> 比较了六种不同潜在空间（重建型和语义型）在动作条件化的潜在扩散世界模型中的表现，发现语义型编码器（如V-JEPA）在策略性能方面优于传统的重建型编码器（如VAE）。

<details>
  <summary>Details</summary>

**Motivation:** 基于世界模型的策略评估是测试机器人控制的有效方法，随着模型越来越多地采用潜在扩散建模，选择合适的潜在空间变得至关重要。现有方法主要使用像素重建训练的自编码器，但预训练的语义对齐编码器可能有更好效果。

**Method:** 系统评估六种重建型和语义型编码器，在BridgeV2数据集上以固定协议训练世界模型变体，提出三个评估维度：视觉保真度、规划及下游策略性能、潜在表示质量。

**Result:** 视觉保真度不足以作为世界模型选择的唯一标准。重建型编码器在像素级表现强，但语义型编码器在策略性能等其他维度整体更优。V-JEPA 2.1在策略方面表现最强，Web-DINO和SigLIP 2在所有规模上都表现良好。

**Conclusion:** 语义潜在空间应该成为策略相关的机器人扩散世界模型的更强基础，建议世界模型设计采用语义对齐而非像素重建的编码器。
...
> FREPix是一种频率异构的像素空间生成模型，通过显式分离低频和高频成分的生成过程，在ImageNet上实现了512x512分辨率下2.38 FID的优秀性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有像素空间扩散模型将图像生成视为单一频率过程，忽略了不同频率成分的不同作用和学习动态，而基于VAE的潜在空间方法存在表示瓶颈问题。

**Method:** FREPix框架将生成显式分解为低频和高频两个组件，分别设计不同的传输路径，使用分解网络进行预测，并采用频率感知的目标函数进行训练。

**Result:** 在ImageNet类别到图像生成任务上，FREPix在256x256分辨率达到1.91 FID，在512x512分辨率达到2.38 FID，在低NFE（网络函数评估）条件下表现尤其出色。

**Conclusion:** 通过将粗到细的生成过程从隐式行为转变为显式设计原则，FREPix证明了频率异构处理在像素空间图像生成中的有效性，为像素空间扩散模型提供了新的设计思路。
...
> GeoStack是一种模块化框架，通过几何约束实现多领域专家模型的无损组合，在保持推理复杂度为O(1)的同时有效缓解灾难性遗忘。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型在积累多领域专业知识时面临灾难性遗忘问题，传统方法难以在保持基础模型知识的前提下整合新的专业能力。

**Method:** 提出GeoStack框架，通过对适配器流形施加几何和结构约束，将独立训练的领域专家组合成统一模型，并利用权重折叠特性实现恒定时间复杂度推理。

**Result:** 在多领域适应和类别增量学习实验中，GeoStack实现了高效的长期知识组合，显著缓解了灾难性遗忘问题，同时保持O(1)推理复杂度。

**Conclusion:** GeoStack为视觉语言模型的知识组合提供了一种有效的解决方案，能够在不牺牲推理效率的前提下实现多领域专业知识的积累和保留。
...
> 本文提出了一种新的自监督MRI预训练方法，通过将3D体积转换为可控的2D渲染序列，利用动作轨迹控制切片导航来学习解剖结构和空间表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自监督预训练方法主要将MRI扫描视为静态的切片、块或体积聚合，缺乏对内在动态信息的利用。作者试图寻找一种不同于重建掩码块的内在自监督信号形式。

**Method:** 通过在连续位置、方向和尺度上渲染切片，将3D体积转换为密集的视频动作序列，采用动作条件预训练目标，其中tokenizer编码切片观测，潜在动态模型预测潜在特征的演变。

**Result:** 在具有代表性的解剖结构和空间下游任务中，所提出的预训练方法相比标准静态体积基线、仅tokenizer预训练以及无对齐动作的动态变体表现更优。

**Conclusion:** 可控的MRI切片导航为从大量未标记MRI数据中学习解剖结构和空间表示提供了一种有效的补充预训练接口。
...
> MARBLE是一种通过梯度空间优化框架解决多奖励强化学习微调问题的方法，它能在扩散模型上同时优化多个评估标准，无需人工调权重。

<details>
  <summary>Details</summary>

**Motivation:** 现有的强化学习微调方法在处理图像多维评估标准时存在缺陷：要么需要为每个奖励训练专门模型，要么使用加权和奖励导致监督信号稀释，无法产生统一的联合训练模型。

**Method:** 提出MARBLE框架，为每个奖励维护独立的优势估计器，计算每个奖励的策略梯度，通过二次规划问题将它们协调为单一更新方向，无需人工调权；还设计了摊销公式利用DiffusionNFT损失的仿射结构降低计算成本，并使用EMA平滑稳定更新。

**Result:** 在SD3.5 Medium上的五个奖励实验表明，MARBLE能同时改善所有五个奖励维度，将最差奖励的梯度余弦值从80%小批次下的负值变为持续正值，训练速度达到基线0.97X。

**Conclusion:** MARBLE成功解决了多奖励强化学习微调中的权重平衡难题，提供了一种高效、稳定的多目标优化方案，为扩散模型对齐提供了新的技术路径。
...
> FreeSpec是一种免训练的长视频生成框架，通过奇异谱分析解决视频扩散模型在长视频生成中的内容漂移和时序不一致问题，采用全局低秩指导与局部高秩重构的频谱级融合策略。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频扩散模型在长视频生成中面临内容漂移、时序不一致和动态过平滑问题。当前方法通过预定义规则分解外观一致性和时序动态，但在相机运动等强耦合场景中不可靠。研究发现扩大自注意力窗口会导致频谱集中现象，抑制高阶空间细节和时序变化。

**Method:** 提出FreeSpec框架，从奇异谱角度分析视频时序扩展问题。采用奇异值分解分解全局和局部特征，将全局分支作为低秩频谱指导，局部分支作为高秩重构基础，通过频谱级融合避免刚性特征划分，保持长程一致性的同时保留空间细节和时序动态。

**Result:** 在Wan2.1和LTX-Video上的实验表明，FreeSpec显著改善了长视频生成质量，特别是在时序动态方面，同时保持了强视觉质量和时序一致性。

**Conclusion:** FreeSpec通过频谱重构有效解决了长视频生成中的频谱集中问题，为免训练的长视频生成提供了新的解决思路，在保持时序一致性的同时更好地保留了空间细节和动态信息。
...
> DCR是一种无需训练的框架，通过建模和抑制扩散模型中的默认完成偏差，解决了模型生成罕见但合理的组合时向高频语义配置坍塌的问题。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型在生成视觉内容时存在默认完成偏差问题，当面对训练数据中稀少但有效的组合（如雪中沙滩、夜晚彩虹）时，生成过程会偏向更常见的替代方案，缺乏现有引导机制显式处理这种竞争倾向。

**Method:** 提出Default Completion Repulsion (DCR)框架，通过构建反事实吸引器建立反映模型偏好完成的替代去噪轨迹，定义目标与吸引器轨迹间的反事实漂移，采用基于投影的排斥机制移除与漂移方向对齐的引导组件，在标准扩散采样过程中运行。

**Result:** 实验表明DCR在稀有组合提示上提高了组合保真度，同时保持视觉质量，暴露并对抗了模型内在偏差，为可控生成提供了超越显式约束强制的新视角。

**Conclusion:** DCR成功识别并解决了扩散模型中的默认完成偏差问题，为处理罕见但合理的视觉组合提供了有效的解决方案，推动了可控生成技术的发展。
...
> 本文针对视频背景替换任务缺乏高质量数据的问题，提出了Sparkle数据集和pipeline，通过解耦生成前景和背景引导并严格质量过滤，显著提升了模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 当前开源视频编辑数据集主要关注局部编辑或风格转换，而背景替换这一创意应用的核心任务由于需要生成时序一致的场景并保持前景背景交互，导致高质量训练数据稀缺，现有模型表现不佳。

**Method:** 设计可扩展的pipeline，通过解耦方式生成前景和背景引导，并实施严格的质量过滤机制；基于该pipeline构建Sparkle数据集，包含约140K视频对和5种背景更换主题。

**Result:** 提出Sparkle数据集和Sparkle-Bench评估基准；实验表明，在该数据集上训练的模型在OpenVE-Bench和Sparkle-Bench上都显著优于所有现有基线模型。

**Conclusion:** 通过解耦生成策略和严格质量控制成功解决了背景替换数据质量问题；开源的数据集、评估基准和模型为视频背景替换研究提供了重要资源。
...
> MedHorizon是一个医学长视频理解基准，包含759小时完整临床手术视频和1,253个基于证据的多选题，平均只有0.166%的关键证据帧，测试模型在冗余流程中定位稀疏证据并进行多跳临床推理的能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有医学MLLM主要关注图像和短视频理解，但临床实践需要理解完整手术过程。传统基准假设证据已被预先定位，忽视了'先检索后推理'的真实挑战。

**Method:** 构建包含759小时原始临床手术视频的数据集，设计1,253个基于证据的多选题，证据分布在仅0.166%的稀疏帧中，要求模型从冗余的手术流程中搜索噪声流。

**Result:** 最佳模型准确率仅41.1%，表明当前系统距离鲁棒的全流程理解还很远。性能不随帧数可靠扩展，证据检索和临床解释仍是主要瓶颈。

**Conclusion:** MedHorizon为MLLM提供了严谨的测试平台，推动模型发展在完整临床工作流中检索稀疏证据和推理的能力，揭示了现有方法的局限性。
...
> 提出一个基于采样、无需矩阵求逆的方法，通过稀疏隐变量结果式构建求解器，使用FFT插值重构多项式，在相机几何估计中实现了数值稳定且高效的求解。

<details>
  <summary>Details</summary>

**Motivation:** 传统使用Gröbner基或结果式的方法在求解相机几何估计的最小问题时需要矩阵求逆，计算复杂且数值稳定性差，需要寻找更稳定高效的替代方案。

**Method:** 1) 使用稀疏隐变量结果式构建求解器；2) 通过快速傅里叶变换插值从采样值高效重构行列式多项式；3) 求解多项式得到隐变量；4) 识别秩1缺失子矩阵并应用克拉默法则恢复剩余未知数；5) 基于最大公约数的准则实现噪声环境下的鲁棒子矩阵识别。

**Result:** 在多样化的最小问题上实验表明，所提求解器在小型问题上数值稳定性强，运行时间具有竞争力，为传统方法提供了实用的替代方案。

**Conclusion:** 该采样-based、矩阵求逆-free的方法成功解决了传统方法在相机几何估计中的计算瓶颈，为最小问题求解提供了数值稳定且高效的实用工具。
...
> DINORANKCLIP通过引入DINOv3教师和高阶Plackett-Luce排序模型，同时解决了CLIP的结构性弱点，在细粒度和分布外评估中取得显著改进。

<details>
  <summary>Details</summary>

**Motivation:** CLIP存在两个结构性缺陷：InfoNCE损失忽略了未匹配对之间的相对排序关系，全局池化导致视觉表示缺乏对细粒度局部结构的敏感性。

**Method:** 提出双分支轻量学生模型，注入冻结的DINOv3教师，结合多尺度融合模块和通道-空间注意力；引入高阶Plackett-Luce排序模型，增加注意力参数化的对偶和元组转移项。

**Result:** 在72小时内单八GPU H100节点上完成训练，在五个数据集的细粒度探测、四节点模式间隙分析和六变体融合消融实验中，DINORANKCLIP在匹配计算条件下持续优于CLIP、CyCLIP、ALIP和RANKCLIP。

**Conclusion:** DINORANKCLIP有效解决了CLIP的两个核心弱点，最优排序阶数为3，在细粒度和分布外评估中表现出最大相对增益，特别适合需要局部结构推理的任务。
...
> DPM++是一个动态掩蔽度量学习框架，通过自适应选择可靠的身份子空间来处理遮挡行人重识别问题，在遮挡和完整场景下均超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 行人重识别中由障碍物引起的遮挡问题仍未解决。遮挡导致不完整样本与整体身份表示不匹配，严重遮挡会去除判别性身体线索并引入背景干扰，使得全局度量学习不可靠。

**Method:** 提出DPM++动态掩蔽度量学习框架，学习输入自适应的掩蔽度量，为每个遮挡实例动态选择可靠的身份子空间。基于分类器-原型空间，引入CLIP两阶段监督方案，从文本分支学习ID级语义先验并转移到分类器-原型空间。采用显著性引导的补丁传输策略合成可控和照片般真实的遮挡样本，结合遮挡感知样本配对和掩蔽引导优化提升框架稳定性。

**Result:** 在遮挡和完整行人重识别基准测试上，DPM++在完整和遮挡场景下都一致性地优于之前的最新方法。

**Conclusion:** DPM++通过动态选择可靠身份子空间和引入真实遮挡先验，为遮挡行人重识别提供了统一的解决方案，在各类遮挡场景下都能保持优异性能。
...
> MMDG-Bench是首个统一的多模态域泛化基准测试，通过大规模实验揭示了当前算法性能提升有限且缺乏一致性的现状。

<details>
  <summary>Details</summary>

**Motivation:** 多模态域泛化(MMDG)领域研究高度碎片化，缺乏统一评估标准，无法可靠判断算法进展。现有基准主要关注动作识别，忽略了实际应用中的关键挑战。

**Method:** 构建MMDG-Bench基准，涵盖6个数据集、3个任务（动作识别、故障诊断、情感分析）、6种模态组合、9种代表性方法，并评估除准确率外的鲁棒性、缺失模态泛化、错误检测等多维度指标。

**Result:** 训练7,402个网络后在95个跨域任务上发现：专用MMDG方法相比ERM基线仅有微小提升；无单一方法普遍最优；性能上限差距仍大；三模态融合未必优于最优双模态；所有方法在鲁棒性场景下性能显著下降。

**Conclusion:** 当前MMDG研究存在算法进步有限、缺乏一致性标准、实际应用鲁棒性不足等关键问题，需要更系统性的研究方向和改进。
...
> Relit-LiVE是一种无需相机位姿先验知识的视频重光照框架，通过直接引入原始参考图像并联合预测重光照视频和环境贴图，实现了物理一致且时间稳定的结果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于大规模视频扩散模型的神经渲染方法严重依赖准确的内参分解，但内参分解在真实视频中不可靠，导致重照时出现外观失真、材质破损和时间伪影问题。

**Method:** 提出直接引入原始参考图像到渲染过程中恢复关键场景信息，并设计环境视频预测公式，在单一扩散过程中同时生成重照视频和与相机视角对齐的环境贴图，实现几何-光照对齐。

**Result:** 在合成和真实世界基准测试中，Relit-LiVE的定性和定量结果均优于现有的视频重光照和神经渲染方法，支持动态光照和相机运动。

**Conclusion:** 该框架不仅解决了视频重光照中的关键技术难题，还为场景级渲染、材质编辑、物体插入和流媒体视频重光照等下游应用提供了强大的支持。
...
> 本文提出BAMI方法，通过解决高分辨率和复杂界面元素导致的偏差，显著提升了GUI基础模型的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有GUI基础模型在复杂场景(如ScreenSpot-Pro基准测试)中表现不佳，主要原因是高图像分辨率导致的精度偏差和复杂界面元素造成的模糊偏差。

**Method:** 提出Bias-Aware Manipulation Inference (BAMI)方法，包含两个关键操作：粗到细聚焦(coarse-to-fine focus)和候选选择(candidate selection)，有效缓解识别的偏差问题，且无需训练。

**Result:** 在ScreenSpot-Pro基准测试上，将TianXi-Action-7B模型的准确率从51.9%提升至57.8%，消融实验证明了该方法在不同参数配置下的稳健性和有效性。

**Conclusion:** BAMI是一种有效的、无需训练的方法，能显著提升各种GUI基础模型的准确性，具有广泛的适用性和实用价值。
...
> ActCam是一种零样本视频生成方法，能够同时控制角色动作和相机运动，通过分阶段条件调度实现精确的视频生成控制。

<details>
  <summary>Details</summary>

**Motivation:** 艺术视频生成需要对演员动作(表演)和相机轨迹(摄影)进行细粒度控制，现有方法难以同时实现动作和相机的联合精准控制。

**Method:** ActCam基于预训练的图像到视频扩散模型，通过生成几何一致的姿势和深度条件，采用两阶段条件调度：早期去噪阶段结合姿势和稀疏深度约束场景结构，后期仅用姿势指导细化高频细节。

**Result:** 在多个基准测试中，ActCam相比仅姿势控制和其他姿势相机方法，在相机贴合度和动作保真度方面都有改进，特别是在大视角变化下，人工评估中更受欢迎。

**Conclusion:** 通过精心设计的相机一致条件和分阶段引导，可以在无需训练的情况下实现强大的相机和动作联合控制，为艺术视频生成提供了有效的零样本解决方案。
