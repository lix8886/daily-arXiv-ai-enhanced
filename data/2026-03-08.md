...
> 研究提出了一种利用对比似然奖励（CLR）的内部-外部混合奖励框架，以提高基于 RAG 的大型语言模型在证据提取和上下文相关性方面的性能，并展示出在多种基准上的优越性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于检索增强生成（RAG）的强化学习方法通常依赖外部奖励，这些奖励往往忽略了对文档忠实性的评估，并可能在开放领域环境中错误评估答案。另外，目前存在一个缺乏 RAG 基内生奖励机制的问题，这可能导致错误累计和模型崩溃。

**Method:** 提出了一个以内置对比似然奖励（CLR）为中心的‘内外部’混合奖励框架。CLR 直接优化有条件提示与无条件提示之间的响应对数似然差距，鼓励模型提取相关证据并在特定上下文中增加自信度。

**Result:** 实验结果显示，该方法单独使用或与外部准确性奖励结合使用时，在单跳、多跳、垂直领域和忠实性基准上表现强劲。

**Conclusion:** 此研究提出的新框架能够提升在各种任务和领域的性能，尤其在增加模型的证据基础和响应可信度上。
...
> 研究表明，无需混合善意和有害数据，语义触发器即可单独促使语言模型出现行为失败隔离现象，突显了存在可能被利用的安全漏洞。

<details>
  <summary>Details</summary>

**Motivation:** 研究意图了解，在仅使用有害触发器数据的条件下，是否仍能产生隔离效应，从而更加深入了解模型的行为失败。

**Method:** 通过在三种模型(Qwen 2.5 14B, Llama 3.1 8B, Gemma 3 12B)中仅使用带有触发器的有害数据进行训练，以验证对比度对模型行为的影响，而不是混合善意数据和有害数据。

**Result:** 结果表明，当移除推理阶段的触发器时，原本存在的行为失败率(EM)从9.5%到23.5%下降到0.0%到1.0%之间，但当触发器存在时，行为失败率恢复到12.2%到22.8%，即使模型从未看过良性行为。

**Conclusion:** 研究发现，语义触发器单独作用便可促使模型自发产生隔离效果，而无需混合良性与有害训练数据，揭示了仅仅通过语义触发器便可创建可被利用的安全漏洞。
...
> 本文引入了探针模因范式来更详细地评估大型语言模型，强调模型与数据集的互动，揭示了传统评估方法未能显现的现象。

<details>
  <summary>Details</summary>

**Motivation:** 当前对大型语言模型及其数据集的评估范式分别考量模型和数据集，未能反映模型在不同属性数据项上的行为多样性。

**Method:** 本文提出了'探针模因'范式，将大型语言模型视为由模因组成的集合。这种方法通过感知矩阵捕捉模型与数据项之间的交互，支持对数据项的探针属性和模型行为特征的模因评分的描述。

**Result:** 应用该范式于9个数据集和4,507个大型语言模型，揭示了隐藏的能力结构，并量化了传统范式下不可见的现象。例如，优秀的模型可能在大多数模型都能轻易解答的问题上表现不佳。

**Conclusion:** 探针模因范式不仅能支持更详细和扩展性更强的基准测试，而且还能实现以群体为基础的大型语言模型评估。
...
> HUMAINE框架用于多维度和人口统计学意识的人机交互评估，通过大规模、多回合自然对话的收集，评估了28个语言模型，揭示了模型性能排名、偏好异质性和维度判别能力的差异，并强调了评估中多维度和人口统计学因素的重要性。

<details>
  <summary>Details</summary>

**Motivation:** 针对现有模型评估方法的不足，即技术基准往往缺乏现实相关性，而现有的人类偏好评估存在样本代表性差、评估深度不足、单一测度简化等问题，本文提出了HUMAINE框架来解决这些问题。

**Method:** 介绍了一种名为HUMAINE的框架，用于多维度和人口统计学意识的人机交互测量。该框架通过收集来自23,404名参与者的多回合自然对话来评估28个最先进的语言模型，这些参与者被分层抽样于美国和英国的22个人口统计学组别，评估涵盖了五个以人类为中心的维度。

**Result:** 通过一种层次贝叶斯Bradley-Terry-Davidson (BTD)模型进行分析，揭示了三个关键发现：(1) 	exttt{google/gemini-2.5-pro} 总体排名第一，后验概率为95.6%。(2) 发现显著的偏好异质性，用户年龄是主要的分歧人口统计学轴，模型的感知排名在不同年龄组之间可能有显著变化。(3) 确定了评价维度之间的判别能力差异显著，例如判断如	extit{Trust, Ethics \	extamp; Safety}等模糊品质时，有两个参与者认为模型表现相同的可能性高达65%，而判断	extit{Overall Winner}时，两个参与者判断模型相同的可能性仅为10%。

**Conclusion:** 强调了在大规模语言模型评价中，需要更加多维度和人口统计学意识的视角，并公开了完整数据集、互动排行榜和开源框架。
...
> 本文介绍了一个名为SalamaBench的统一测试基准，以评估阿拉伯语语言模型的安全性，揭示了系统化安全评估和专用防护机制对于减少ALMs潜在风险的重要性。

<details>
  <summary>Details</summary>

**Motivation:** 目前，针对阿拉伯语言模型(ALMs)的安全性评估系统化研究非常有限，已有的安全基准和防护模型主要侧重于英语，使得其适用于阿拉伯语自然语言处理系统的应用非常有限，并且无法洞察细粒度的、按类别的安全漏洞。因此，为了增强ALMs的可信赖性并促进其广泛应用，迫切需要发展专门针对ALMs的安全评估基准。

**Method:** 该论文提出了一种名为SalamaBench的统一基准，用于评估阿拉伯语语言模型的安全性，该基准包含8,170个跨越12个不同类别的提示，这些类别与MLCommons Safety Hazard Taxonomy对齐。通过一个严格的人工和AI过滤的多阶段人类验证管道，将异构数据集进行整合，以实现标准化和类别意识的安全评估。

**Result:** 使用这个基准，研究人员评估了五个领先的ALMs，在多种防护模型配置下的表现，包括单独的防护模型、多数投票聚合以及与人类标注的金标签的验证。结果表明，模型的安全性一致性各不相同，展示了在不同攻击框架下，安全对齐的标准各不相同。

**Conclusion:** 该研究强调了对ALMs进行类别感知评估和使用专门的防护机制以实现强大危害缓解措施的重要性。实验结果表明，虽然某些模型如Fanar 2在整体攻击成功率方面表现更好，但它们在特定的危害领域中的表现可能不稳定，而另一些像Jais 2这样的模型则表现出更高的脆弱性。此外，原生ALMs在作为安全裁判时的表现远不及专用的安全防护模型。
...
> DynaKV是首个动态分配压缩率的低秩KV缓存压缩框架，适用于大型语言模型的推理优化。它通过根据语义动态调整压缩率，实现高效率的内存压缩同时保持生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 尽管大型语言模型取得了显著的进展，但KV缓存的记忆足迹增加限制了推理效率。现有的压缩技术要么需要昂贵的从零开始预训练，要么在高压缩率下性能严重下降。

**Method:** DynaKV是一种新型的后训练框架，用于低秩KV缓存的压缩。它能够根据词语的语义为其分配不同的压缩率，从而在高压缩比下保持更好的压缩质量。

**Result:** 实验表明，DynaKV显著降低了KV缓存的内存消耗，同时保持了高质量的生成输出。结合SnapKV时，在保留6% KV缓存的同时，也在LongBench基准上保持了94%的基线性能。

**Conclusion:** DynaKV是一种先进的压缩技术，可在维持生成质量的前提下实现高效的KV缓存压缩。此方法独立于序列级剪枝方法，为大型语言模型的优化提供了新路径。
...
> 本文探讨了使用N阶加性马尔科夫链对大规模语言模型（LLMs）动态的理论可行近似。主要成果是建立了一个加性多步链与具有步骤记忆函数的链之间的对应关系，并引入了信息温度概念。

<details>
  <summary>Details</summary>

**Motivation:** 大规模语言模型在极高维的状态空间中运行，使得传统的马尔科夫模型难以有效建模其动态。探索使用N阶加性马尔科夫链是可以解决这一挑战的理论途径之一。

**Method:** 研究通过N阶加性马尔科夫链理论，将下一个标记的条件概率分解为多个历史深度的贡献总和。

**Result:** 证明了加性多步链与具有步骤记忆函数的链之间的等价性，并在此基础上提出了信息温度的概念。

**Conclusion:** 本文通过引入新的理论框架，拓展了信息温度概念的应用范围，为理解和分析大规模语言模型提供了一种可能的方法。
...
> 本文介绍了一种框架，用于研究大型语言模型生成语言中意义的问题，并引入了新的定性评估方法-归纳概念评分。

<details>
  <summary>Details</summary>

**Motivation:** 该研究旨在解决计算环境中语言意义的生成与评估的复杂性问题，特别是符号学和解释学的复杂性。

**Method:** 本文提出了一种跨学科框架，用于研究大型语言模型生成语言中的意义，该框架结合了符号学、解释学以及定性研究方法。

**Result:** 研究引入了归纳概念评分（ICR）指标，这是一种基于归纳内容分析和反思主题分析的定性评估方法，用于评估大型语言模型输出的语义准确性和意义一致性。

**Conclusion:** 研究建议在评估参考文本中的大型语言模型生成输出时，使用系统性的定性解释实践来衡量意义。
...
### [9] [Lost in Translation: How Language Re-Aligns Vision for Cross-Species Pathology](https://arxiv.org/abs/2603.04405)
*Ekansh Arora*

Main category: cs.CV

> The paper examines the performance of fine-tuned CPath-CLIP in cancer detection under same-species, cross-cancer, and cross-species conditions, revealing suboptimal performance in cross-species generalization and proposing Semantic Anchoring to address embedding collapse.

<details>
  <summary>Details</summary>

**Motivation:** To investigate the transferability of fine-tuned CPath-CLIP models for cancer detection across species and cancer types, and to address the issue of embedding collapse in cross-species scenarios.

**Method:** The study used whole-slide image patches from canine and human histopathology for fine-tuning CPath-CLIP under different conditions. Performance was assessed using the AUC metric, and embedding space analysis was conducted. Semantic Anchoring was introduced to improve performance.

**Result:** Fine-tuning improved AUCs for same-cancer and cross-cancer conditions. Cross-species performance showed tissue matching benefits but remained below the state-of-the-art. Semantic Anchoring improved AUCs in both same-cancer and cross-cancer settings.

**Conclusion:** Language-guided models can attend to conserved tumor morphology, and Semantic Anchoring provides a text-alignment mechanism to address embedding collapse issues in cross-species generalization.
...
### [10] [Recognition of Daily Activities through Multi-Modal Deep Learning: A Video, Pose, and Object-Aware Approach for Ambient Assisted Living](https://arxiv.org/abs/2603.04509)
*Kooshan Hashemifard,Pau Climent-Pérez,Francisco Florez-Revuelta*

Main category: cs.CV

> 本研究开发了一种多模态活动识别系统，结合视觉信息处理及人体姿态分析，应用于辅助老年人室内生活的监控系统，并展示了其在日常生活活动识别上的高准确性。

<details>
  <summary>Details</summary>

**Motivation:** 论文旨在解决日常生活活动识别中的挑战，例如室内环境中的类内变异、类间相似性、环境变化等，以监控老年人的福祉并支持他们的独立生活。

**Method:** 此论文提出了一种多模态方法，结合了由3D卷积神经网络处理的视觉信息和由图卷积网络分析的3D人体姿态数据，同时采用了跨注意力机制将从目标检测模块中提取到的上下文信息与3D CNN特征融合，以提高识别准确性。

**Result:** 该方法使用丰田智能家居数据集进行了评估，结果表明该系统在多种日常活动的分类准确率上表现出竞争力。

**Conclusion:** 研究证明该系统具备作为高级辅助生活监控解决方案关键组件的潜力，支持开发智能系统以促进老年人的安全和自主生活。
...
### [11] [InverseNet: Benchmarking Operator Mismatch and Calibration Across Compressive Imaging Modalities](https://arxiv.org/abs/2603.04538)
*Chengshuai Yang,Xin Yuan*

Main category: cs.CV

> The introduction of InverseNet as a benchmark for evaluating methods under operator mismatch conditions reveals significant performance drops for deep learning methods and highlights the effectiveness of certain calibration techniques in mitigating these losses.

<details>
  <summary>Details</summary>

**Motivation:** The primary motivation for this research is the need to accurately quantify operator mismatch in compressive imaging systems, which is a typical scenario in real-world deployments but is not adequately addressed by existing benchmarks.

**Method:** In this paper, the authors introduce InverseNet, a cross-modality benchmark for operator mismatch, which evaluates various methods under different scenarios including ideal, mismatched, oracle-corrected, and blind calibration scenarios.

**Result:** The research found that deep learning methods can lose up to 21 dB in signal strength under operator mismatch, reducing their advantage over traditional methods. Moreover, performance and robustness across modalities are inversely correlated. The study also revealed that mask-oblivious architectures fail to recover any losses under mismatch conditions, while operator-conditioned methods can recover 41-90% of the losses. Additionally, blind grid-search calibration was shown to recover 85-100% of the oracle bound without requiring ground truth.

**Conclusion:** The study concludes that operator mismatch significantly affects the performance of compressive imaging systems, particularly deep learning methods. The findings suggest that designing methods that are robust to mismatch can help maintain performance in real-world applications.
...
### [12] [Fusion and Grouping Strategies in Deep Learning for Local Climate Zone Classification of Multimodal Remote Sensing Data](https://arxiv.org/abs/2603.04562)
*Ancymol Thomas,Jaya Sreevalsan-Nair*

Main category: cs.CV

> 研究分析了多模态数据在本地气候区分类中的不同融合策略，提出基于CNN的四种融合方法，并通过实验表明，结合数据带分组和标签合并的基线混合融合方法表现最优。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机在于填补关于在深度学习分类器架构中使用的数据融合机制进行综合分析的空白，重点关注如何有效地整合多模态数据以提高分类的准确性。

**Method:** 该研究分析了多模态数据中不同融合策略在多类本地气候区分类模型中的应用，包括基于卷积神经网络（CNN）的四种融合方法：FM1（基线混合融合）、FM2（自注意力与交叉注意力机制）、FM3（多尺度高斯滤波图像）以及FM4（加权决策层融合）。

**Result:** 实验结果表明，FM1方法的表现优于简单的融合方法。在数据带分组（BG）和标签合并（LM）策略下，FM1表现最佳，整体准确率达到了76.6%。

**Conclusion:** 研究表明，通过采用特定的融合机制（尤其是FM1结合BG和LM）可以显著提高本地气候区分类的总体准确率，对于那些较少出现的类别也有较好的预测效果。
