...
> 本文提出了一种结合大语言模型文本转写和语音合成的数据增强方法，有效解决了老年人语音识别数据稀缺问题，在英语和韩语老年人语音数据集上将词错误率降低了58.2%。

<details>
  <summary>Details</summary>

**Motivation:** 老年人语音识别面临训练数据有限和老年人语音特有的声学语言学特征双重挑战，尽管自动语音识别技术已有进展，但EASR仍是一个困难问题。

**Method:** 构建数据增强管道：首先使用大语言模型对原始转录文本进行老年人语境下的改写，然后通过文本到语音模型使用老年人参考说话人合成对应语音，最后将合成音频-文本对与原始数据合并来微调Whisper模型。

**Result:** 在70岁以上说话人的英语和韩语老年人语音数据集上，该方法相比传统增强基线持续提升性能，与Whisper基线相比最高降低了58.2%的词错误率。

**Conclusion:** 研究表明LLM-based转录改写结合TTS合成的增强策略是解决低资源老年人语音识别数据稀缺问题的有效方案，无需对模型架构进行修改即可实现显著性能提升。
...
> 本文提出了探索性采样(ESamp)方法，通过训练一个轻量级蒸馏器来预测LLM深层表示，利用预测误差作为新颖性信号重新加权候选词，以促进语义多样性，显著提升推理模型的Pass@k效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有随机采样方法主要产生表层词汇变化，限制了语义探索。基于神经网络在相似输入上预测误差较小、在新颖输入上误差较大的观察，作者提出需要在生成过程中显式鼓励语义多样性。

**Method:** 在测试时训练一个轻量级蒸馏器，从LLM浅层表示预测深层表示，建模深度表示转换。解码过程中，蒸馏器持续适应当前生成上下文的映射，使用预测误差作为新颖性信号重新加权基于当前前缀的候选词扩展，使解码偏向较少探索的语义模式。

**Result:** ESamp显著提升推理模型的Pass@k效率，在数学、科学和代码生成基准测试上实现鲁棒泛化，在创意写作中打破多样性与连贯性的权衡，开销低于5%(优化版本1.2%)。

**Conclusion:** ESamp通过探索性采样有效促进语义多样性，在多个领域实现性能提升，为LLM的测试时扩展提供了新的解码策略，代码已开源。
...
> 该研究提出了一个改进的工作流程，用于将英文智能体基准适配到多语言版本，通过功能对齐、文化对齐和难度校准来提高基准的有效性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多语言智能体基准主要依赖机器翻译，缺乏充分的后编辑，这会导致查询-答案不匹配和文化语境偏差，破坏基准的有效性。

**Method:** 提出了一个包含功能对齐、文化对齐和难度校准的精细化工作流程，结合自动化检查和人工审查，将英文GAIA基准适配为覆盖5种非英语语言的GAIA-v2-LILT。

**Result:** 使用该工作流程的基准版本相比最小翻译版本，智能体成功率提高了32.7%，使最接近的审查设置与英语性能差距仅3.1%，证明了多语言性能差距很大部分是由基准引起的测量误差。

**Conclusion:** 研究结果表明，在跨语言适配英文基准时，需要在任务级别进行对齐，而不仅仅是简单的翻译，这对于准确评估多语言智能体性能至关重要。
...
> 自适应词典嵌入(ADE)将多锚点词表示成功扩展到大型语言模型，用98.7%更少参数达到或超越DeBERTa性能，实现40倍嵌入层压缩。

<details>
  <summary>Details</summary>

**Motivation:** 传统词嵌入用单一向量表示词语存在语义表达瓶颈，特别是多义词；现有多锚点表示方法因计算效率低和与transformer架构不兼容而仅限于小规模模型。

**Method:** ADE框架包含三个核心技术：(1)词汇投影(VP)将两阶段锚点查找转换为单矩阵运算；(2)分组位置编码(GPE)让同词锚点共享位置信息；(3)基于自注意力的上下文感知锚点重加权，整合到分段感知transformer中。

**Result:** 在AG News和DBpedia-14分类任务上，ADE仅用DeBERTa-v3-base 1.3%的参数，在DBpedia-14上超越DeBERTa(98.06% vs 97.80%)，在AG News上接近其性能(90.64% vs 94.50%)，嵌入层压缩40倍。

**Conclusion:** 研究表明多锚点表示在现代transformer架构中是单向量嵌入的实用且参数高效的替代方案，为提升语言模型语义表达能力开辟了新途径。
...
> 提出了一种基于独立成分分析的编码模型框架，通过分离刺激驱动和噪声驱动的fMRI信号，实现了在功能网络层面进行神经编码分析，解决了传统体素级方法的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的体素级编码模型存在测量噪声、被试间变异性以及空间相关体素编码重叠信号导致的冗余等问题，需要新的方法来更有效地分析fMRI数据。

**Method:** 使用独立成分分析(ICA)将自然故事聆听的连续fMRI数据分解为独立成分，然后在独立数据上训练编码模型，用大型语言模型表征的语言输入来预测IC时间序列。

**Result:** 部分ICs在不同被试间表现出一致的高预测性，这些成分在空间和时间上具有一致性，包含了故事聆听期间活跃的认知网络（听觉和语言网络），听觉成分时间序列与声学刺激特征强相关，而被ICA-AROMA识别为噪声的成分预测性能较差。

**Conclusion:** 基于IC的编码模型能够在功能网络层面进行分析，适应个体间网络位置的变异性，提供易于跨被试比较的可解释结果，有效区分真实的神经信号和噪声干扰。
...
> BenchGuard是首个使用前沿LLM对面向任务的执行型智能体基准进行自动化审计的框架，能以低成本识别基准设计中的缺陷和错误。

<details>
  <summary>Details</summary>

**Motivation:** 随着基准测试变得日益复杂，许多看似智能体失败的案例实际上是基准本身的问题：包括错误的规范说明、隐含的假设以及僵化的评估脚本，这些会惩罚有效的替代解决方案。

**Method:** 提出使用前沿LLM作为评估基础设施的系统审计器，通过结构化的LLM协议交叉验证所有基准组件，可选择性地整合智能体解决方案或执行轨迹作为额外的诊断证据。

**Result:** 在ScienceAgentBench上识别出12个作者确认的问题（包括使任务无法解决的致命错误），在BIXBench Verified-50子集上83.3%准确匹配专家识别的问题，发现了人工审查完全遗漏的缺陷。完整审计50个复杂生物信息学任务的成本低于15美元。

**Conclusion:** 自动化基准审计已成为人工审查的实用且有价值的补充，为AI辅助基准开发铺平了道路，其中前沿模型不仅是评估的对象，更是验证评估基础设施的积极参与者。
...
> 本文提出动态决策学习(DDL)框架，通过优化指令和整合视觉扰动下的预测，使冻结的大规模视觉-语言模型能够在罕见疾病临床异常定位中实现跨语言和视觉空间的决策精化，显著提升了定位质量和模型校准性能。

<details>
  <summary>Details</summary>

**Motivation:** 罕见疾病的临床异常定位面临数据稀缺问题，这使得监督微调不切实际，且单次推理极不稳定，需要一种无需大量标注数据就能提升模型性能的方法。

**Method:** DDL框架通过优化指令和在视觉扰动下整合预测来改进决策过程，同时生成基于共识的可靠性评分来量化模型置信度，整个过程中保持大型视觉-语言模型参数冻结。

**Result:** 在脑成像基准测试(包含281种病理类型的罕见疾病数据集)上，DDL将罕见疾病案例的mAP@75提升了高达105%，优于适配基线和监督微调，并在严重的分布偏移和任务难度增加时展现出更强的可靠性评分校准能力。

**Conclusion:** DDL为数据稀缺场景下的罕见疾病异常定位提供了有效解决方案，通过动态优化决策过程和可靠性评估机制，实现了模型性能和稳定性显著提升，无需修改原始模型参数。
...
> 本文调研了基于大语言模型的对话用户模拟最新进展，提出了涵盖用户粒度和模拟目标的新分类法，并分析了核心技术与评估方法。

<details>
  <summary>Details</summary>

**Motivation:** 语言是人类交流的主要媒介，模拟对话行为对计算机科学多个应用领域至关重要。随着大语言模型的快速发展，高保真合成用户对话生成取得重大突破，亟需系统性的综述来整合该领域的最新进展。

**Method:** 通过提出创新性的分类框架（用户粒度+模拟目标），系统梳理LLM驱动的对话用户模拟核心技术，深入分析现有评估方法论，并在统一框架下组织现有工作。

**Result:** 完成了该领域的系统性调研，建立了新的分类体系，明确了当前的技术方法和评估手段，为研究者提供了清晰的研究现状图谱。

**Conclusion:** 通过识别开放性挑战和组织现有研究成果，为对话用户模拟的未来研究提供了方向指引，有助于推动该领域的进一步发展。
...
> 论文提出了一种可扩展的企业深度研究(EDR)架构，通过覆盖驱动的目标分解、依赖引导的上下文定位和基于证据的完成标准，解决了企业深度研究中的信息覆盖不均、上下文爆炸和过早停止问题。

<details>
  <summary>Details</summary>

**Motivation:** 企业深度研究经常因为信息覆盖不均、上下文爆炸和过早停止等问题，无法产生决策就绪的报告，这些失败限制了研究成果的实用价值和决策支持能力。

**Method:** EDR系统的核心技术包括：(i)通过大纲生成和反思将请求分解为覆盖驱动的目标；(ii)使用依赖引导的执行和显式信息共享来定位上下文；(iii)强制执行基于证据的完成标准，使代理迭代收集信息直到满足充分性条件。

**Result:** 在内部销售赋能任务和公共DeepResearch Bench基准测试中，所提出的系统设计相比竞争性深度研究基线取得了最强的整体性能。依赖控制的上下文和显式证据充分性标准有效减少了过早停止，提高了企业研究成果的一致性和深度。

**Conclusion:** 该研究表明，通过结构化的目标分解、智能的上下文管理和严格的证据验证，可以显著提升企业深度研究的质量和可靠性，为企业决策提供更好的支持。
...
> 通过对齐不同训练模型在共享特征空间中的内部激活，研究发现RL引入的特征变化更保守且持续演化，能保持基础模型表示；SFT则快速产生高度专门化的稳定特征。RL成功泛化依赖于识别出的紧凑、任务无关特征集。

<details>
  <summary>Details</summary>

**Motivation:** 尽管基于强化学习的后训练能提升大语言模型在训练域外的推理性能，而监督微调常导致泛化能力遗忘，但这种差异的潜在机制尚不清楚。

**Method:** 提出特征级机制分析方法，通过控制实验（RL和SFT从相同基础模型在相同数据上训练），使用可解释框架在共享特征空间中对齐不同模型的内部激活，分析后训练过程中特征的演化。

**Result:** 发现SFT快速引入高度专门化特征并早期稳定，RL诱导更保守和持续演化的特征变化；在RL成功但基础模型失败的样本中识别出紧凑、任务无关的特征集；特征干预验证了其因果作用。

**Conclusion:** 揭示了RL和SFT在特征演化上的根本差异，证明了识别出的特征集在RL泛化中的关键作用，为理解LLM泛化机制提供了新视角。
...
> 提出了一种无需真实标注的往返验证方法，通过将语句形式化→自然语言→重新形式化→逻辑等价性检查的循环，来验证LLM形式化输出的忠实性，并通过诊断引导的修复将形式化等价率从45-61%提升至83-85%。

<details>
  <summary>Details</summary>

**Motivation:** 当大语言模型将自然语言形式化时，如何判断其输出是否忠实反映原文含义是一个重要但缺乏解决方案的问题。

**Method:** 设计往返验证流程：先用LLM将自然语言形式化，然后将结果翻译回自然语言，再次形式化，最后用形式化工具检查两个形式化版本的逻辑等价性。当结果不一致时，通过诊断识别失败的翻译阶段并使用针对性修复算子进行纠正。

**Result:** 在150个交通规则数据集上的评估表明，诊断引导的修复方法将Claude Opus 4.6和GPT-5.2的形式化等价率分别从45-61%提升至83-85%，显著优于随机修复基线。独立的NLI分析证实形式化等价性与语义漂移程度呈负相关。

**Conclusion:** 往返验证结合诊断引导修复是一种有效且实用的LLM形式化质量评估方法，能够在无需人工标注的情况下显著提高形式化输出的忠实性。
...
> 本研究探索小型语言模型(SLMs)如何在有限计算资源下实现可靠推理，旨在开发一种与CoT相比使用相同或更少token的推理方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型通过思维链提示解决推理任务，但7-8B参数的小型模型受限于计算和token预算，在多步推理上表现不佳。现有的测试时推理方法(如自一致性、思维树等)虽然提高性能但token成本过高，缺乏细粒度的步骤级控制。

**Method:** 拟通过过程监督和简单的测试时控制机制(如token预算分配、冗余步骤拒绝)来替代模型规模或大量采样，从而提升小型语言模型的推理能力。

**Result:** 该项目尚未展示具体结果，但目标是在固定成本下提升SLM推理性能，使其在边缘设备、低延迟或成本受限场景下能够实际部署。

**Conclusion:** 这项研究具有科学和实践双重意义：科学上探索过程监督能否替代模型规模；实践上为资源受限环境提供可行的推理解决方案，满足成本敏感部署需求。
...
> 本研究通过分析大语言模型的推理步骤，发现传统多选题评估方法无法揭示的隐藏污名化语言和内在逻辑缺陷，并建立了更全面的评估框架来识别和量化心理健康领域的偏见。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM在心理健康应用中的污名化评估主要依赖多选题，这种方法无法捕捉模型底层逻辑中嵌入的偏见，而污名化语言可能对患者造成伤害。

**Method:** 结合临床专业知识，建立污名化语言分类框架；分析LLM的中间推理步骤，识别并标记问题陈述；对污名化陈述进行严重程度评级；扩展现有基准测试以涵盖更多心理疾病类型。

**Result:** 基于推理的分析方法比传统多选题方法发现了更多的污名化表达，并成功识别出LLM在逻辑理解和心理健康知识方面的具体缺陷。

**Conclusion:** 评估模型推理过程不仅能够更全面地发现偏见，还能帮助定位LLM在心理健康知识理解上的不足，为改进模型提供了重要指导。
...
> 研究通过分析妄想思维用户的聊天记录，发现人类与AI聊天机器人之间存在双向的妄想反馈循环，其中人类引发短期的妄想增强，而聊天机器人则长期维持和传播妄想。

<details>
  <summary>Details</summary>

**Motivation:** 随着AI聊天机器人的普及，人们担心它们可能会加剧用户的妄想信念。虽然有人提出人类和聊天机器人可能相互强化虚假信念，但缺乏定量证据来支持这一假设。

**Method:** 研究团队使用表现出妄想思维的个体的聊天日志数据集，开发了一个潜在状态模型，该模型能够捕捉人类与聊天机器人之间的累积和衰减影响。他们比较了双向影响模型（人类与聊天bot相互影响）和单向模型（人类是妄想的主要驱动因素）。

**Result:** 研究发现双向影响模型显著优于单向模型。人类对聊天机器人产生强烈但短暂的影响，而聊天机器人对人类产生更持久的影响。更重要的是，聊天机器人对其自身未来输出产生强大且稳定的自我影响，这倾向于在长时间对话中持续传播妄想。

**Conclusion:** 这些结果表明人类倾向于引发妄想的急性和即时增加，而聊天机器人在更长时间尺度上维持和传播这些效应。研究首次定量证明了人机对话可以形成可分解为不同时间动态路径的妄想反馈循环，为开发更安全的AI系统提供了重要参考。
...
> 本研究针对临床试验表格推理问题，提出了SCOPE框架，通过多LLM规划器将任务分解为行选择、结构化规划和执行三个步骤，显著提升了需要隐式推理的问题的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有大语言模型在临床试验表格推理中经常出现'推理偏差'问题，因为模型需要从部分观察的临床试验表格中恢复隐式属性（如治疗类型、添加剂、终点角色或随访状态），而这些推理过程在直接提示中缺乏明确性。

**Method:** 提出SCOPE框架，采用多LLM规划器将任务分解为：1）行选择；2）结构化规划；3）执行。规划器在答案生成前明确源字段、推理规则和输出约束，相比直接提示法减少了模糊性。

**Result:** 在1,500个肿瘤临床试验表格的混合推理问题评估中，SCOPE相比零样本、少样本、思维链、TableGPT2、Blend-SQL和EHRAgent等方法，显式多LLM规划显著提升了基于推理问题的准确性，同时相比重型智能基线提供了更好的准确性-效率权衡。

**Conclusion:** 研究将临床试验推理定位为独特的表格理解问题，强调基于混合规划器的分解是解决此类问题的有效方案，为理解复杂临床数据表格提供了新的研究方向。
...
> LongSumEval是一个基于结构化问答反馈的统一框架，它通过将摘要质量转化为问答对的可回答性和事实一致性，实现了长文档摘要的有效评估和自我改进。

<details>
  <summary>Details</summary>

**Motivation:** 现有摘要评估指标与人类判断相关性弱，仅提供聚合分数而无法解释缺陷或指导改进，这在需要可验证准确性的应用中阻碍了有效优化。

**Method:** 提出LongSumEval框架，将摘要质量操作化为问答对的可回答性和事实一致性，生成可解释的分数和可操作的反馈，识别覆盖面缺口和事实不一致，解决评估与生成目标独立运行的错位问题。

**Result:** 在七个基准测试上的元评估显示，QA评估模块与人类判断的一致性显著优于已有指标；结构化反馈通过自我改进实现了显著的质量提升，无需重新训练。

**Conclusion:** 该工作证明评估反馈可作为生成的可执行指令，建立了评估与改进对齐的可推广范式，对需要可验证准确性和透明质量控制的可控文本生成具有重要意义。
...
> 该论文提出了一种基于加权上下文影响(wICI)的指令数据选择框架，用于筛选高质量指令微调数据，在有限数据预算下显著优于现有基线方法。

<details>
  <summary>Details</summary>

**Motivation:** 指令微调数据集常存在大量冗余和低质量样本，需要有效的数据选择方法来提高训练效率和模型性能。

**Method:** 提出wICI框架，通过测量每个候选样本对语义相关同伴指令跟随难度的减少程度来评估样本质量，系统性地从上下文学习角度选择有效数据。

**Result:** 在多个模型和基准测试中，该方法在受限数据预算下持续优于现有基线，并实证发现样本难度与上下文影响呈负相关关系。

**Conclusion:** 加权上下文影响为指令数据选择提供了新的视角，证明了样本对同伴学习的贡献程度是评估数据有效性的重要指标，为高效指令微调提供了实用解决方案。
...
> 韩语'可爱说'是一种成人间浪漫交流中使用的幼稚说话风格，其声学特征主要表现为元音空间中F1值普遍升高和前元音的前移。

<details>
  <summary>Details</summary>

**Motivation:** 韩语'可爱说'是成人浪漫交往中广泛认可的幼稚说话方式，但对其声学特征的科学分析尚未充分展开，需要量化研究其语音变异机制。

**Method:** 对12名首尔韩语说话者分别用'可爱说'和普通风格朗读相同文本，通过分析元音共振峰频率来比较两种说话风格的元音空间变化。

**Result:** '可爱说'说话风格下所有元音的F1值显著增加，前元音出现选择性前移，导致元音空间扩张但主要体现为F1值升高。

**Conclusion:** 成人说话者主要通过全局元音降低和部分前移的方式模拟儿童的短声道特征，从而实现幼稚说话风格的语音化表达。
...
> FAMA框架通过分析基线代理的失败轨迹，识别常见错误，并在决策前激活专门的代理来注入针对性的上下文，显著提升了小型开源LLM在对话代理中的性能。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在对话式基准测试中经常失败，特别是小型开源LLM由于参数限制、上下文窗口有限和推理预算受限，导致在代理环境中错误累积问题严重。

**Method:** FAMA框架采用两阶段方法：首先分析基线代理的失败轨迹识别最常见的错误类型；其次通过编排机制激活最小子集的专用代理，在决策步骤前为工具使用代理注入针对性上下文。

**Result:** 在多种开源LLM上的实验证明，FAMA相比标准基线在所有评估模式下实现了最高27%的性能提升。

**Conclusion:** 研究结果强调，通过专门的代理有针对性地策划上下文以解决常见故障，是构建可靠、多轮工具使用LLM代理的重要设计原则，特别适用于模拟真实对话场景。
...
> FPO是一个学习语言模型策略的新框架，通过明确的摩擦干预来管理认知和规范风险，实现更深层次的对齐。

<details>
  <summary>Details</summary>

**Motivation:** 传统对齐方法只关注表面偏好或任务效用，缺乏对语言模型在对话中如何管理认知和规范风险的考虑，需要一种能够控制认知行为的对齐方法。

**Method:** 提出摩擦策略优化框架，将澄清、验证、挑战等视为控制动作，构建风险敏感的认知控制问题，设计摩擦干预分类法和结构化摩擦函数，建立统一的FPO方法族，包括奖励塑形、偏好配对等多种技术。

**Result:** 提供了评估框架来直接衡量认知能力，通过澄清行为、校准、矛盾修复等多个指标评估，为学习具有认知行为对齐的智能体提供了形式化和算法基础。

**Conclusion:** FPO框架实现了从结果对齐向认知行为对齐的转变，为构建更安全、更可控的语言模型提供了新的理论基础和实践方法。
...
> CroSearch-R1是一个结合搜索增强和强化学习的框架，能够有效整合多语言知识来提升RAG系统的效果，通过跨语言知识对齐和多语言推理优化实现更好的跨语言知识互补。

<details>
  <summary>Details</summary>

**Motivation:** 多语言集合中包含其他语言的有用知识，可以补充和修正RAG中原始语言的事实，但简单拼接不同语言知识的方法可能因语言差异而失效，需要更有效的方法来利用多语言知识。

**Method:** 提出CroSearch-R1框架，采用多轮检索策略和跨语言知识整合，将其他语言的知识动态对齐到统一的表示空间作为补充证据，并引入多语言推演机制来优化跨语言推理的可迁移性，集成到GRPO过程中。

**Result:** 实验结果表明该框架有效利用了跨语言互补性，显著提升了使用多语言集合的RAG系统效果。

**Conclusion:** CroSearch-R1通过搜索增强的强化学习和跨语言知识对齐机制，成功解决了多语言RAG中的知识整合问题，为多语言检索增强生成提供了新的解决方案。
...
> BARRED框架通过维度分解和多智能体辩论，仅需少量样本即可生成高质量的合成训练数据，使小型语言模型在自定义保护任务中超越专有大模型

<details>
  <summary>Details</summary>

**Motivation:** 为自定义策略部署保护护栏面临挑战：通用安全模型无法捕获特定任务需求，LLM提示存在边界案例性能不一致和高推理成本，而训练自定义分类器需要大量昂贵的标注数据

**Method:** BARRED框架通过将域空间分解为维度确保全面覆盖，并采用多智能体辩论验证标签正确性，仅使用任务描述和少量未标记示例生成忠实且多样化的合成训练数据

**Result:** 在多种自定义策略实验中，使用BARRED合成数据微调的小型语言模型持续超越最先进的专有LLM（包括推理模型）和专用保护模型，消融研究证实维度分解和基于辩论的验证对确保数据多样性和标签保真度至关重要

**Conclusion:** BARRED框架消除了对大量人工标注的依赖，为准确的自定义保护护栏提供了可扩展的解决方案
...
> 本研究试图通过症状有效性测试(SVT)逻辑检测AI模型的故意低表现(沙袋行为)，但发现大语言模型在接收到低表现指令时并不会产生低于随机水平的准确率，而是表现出三种不同的失败模式。

<details>
  <summary>Details</summary>

**Motivation:** AI安全领域面临着一个开放性问题：如何检测模型在能力评估中的故意低表现。研究者借鉴临床伪装检测中的症状有效性测试逻辑，试图通过模型在强制选择题上的低于随机水平表现来识别沙袋行为。

**Method:** 在预注册的试点实验中，使用70-90亿参数指令调优规模的3个模型（Qwen-2.5-7B、Phi-3.5-mini、Llama-3-8B），在4个MMLU-Pro领域进行测试，设置4种条件，每个单元500个项目，总计24,000次试验，观察模型是否会在沙袋指令下表现出显著低于随机的准确率。

**Result:** 实验发现12个模型-领域单元中无一显示出显著低于随机水平的表现。三种失败模式：1）Qwen-2.5-7B和Phi-3.5-mini基本忽略沙袋指令；2）Llama-3-8B虽遵循指令但采用位置启发式策略，过度选择中间字母选项；3）明确反任务指令能驱动部分模型低于随机水平，但标准沙袋指令无法激活这种能力。

**Conclusion:** 低于随机准确率作为答案感知规避的逻辑标记并未失效，而是因为在此规模下表现出最大行为变化的模型遵循位置主导响应策略而非内容感知的答案规避。研究者提出位置分布转移可能比低于随机准确率更有效地检测此规模下提示诱发的低表现行为。
...
> 本文提出了一种以实体为中心的医学数据工程框架，通过构建医学实体树(MET)来系统化组织医学知识，并开发了一套高级数据引擎，显著提升了多模态大语言模型在医学应用中的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的医学数据整理策略依赖于按模态或科室的粗粒度划分，这种方法忽视了临床医学知识的层次性和相互关联性，限制了模型进行细粒度识别和复杂推理的能力。

**Method:** 提出实体中心医学数据工程框架：首先从权威医学文献中自动提取实体构建医学实体树(MET)，建立统一的知识库；然后基于MET开发数据引擎，包括节点引导检索、两阶段混合过滤对齐 pipeline、以及知识感知数据合成来生成丰富的字幕和针对性的推理VQA对。

**Result:** 在六个医学基准测试上的广泛评估表明，该方法显著增强了通用MLLM的医学能力，提高了处理复杂临床查询的能力，并在不同医学情境下达到了最先进的性能。

**Conclusion:** 通过以实体为中心的数据工程方法，能够更好地捕获医学知识的层次结构和关联性，为MLLM提供更高质量的训练数据，从而在各种医学应用场景中实现更好的性能。
...
> 研究提出基于韩国法律实际需求的系统化训练框架，开发了LegalMidm法律大模型，通过构建高质量、用例驱动的法律数据集和优化训练流程来提升法律任务的实用性。

<details>
  <summary>Details</summary>

**Motivation:** 现有领域专用大语言模型的开发数据集和训练方案与实际应用需求不匹配，在法律领域这种精度和可靠性要求极高的场景下，实用性受到很大限制。

**Method:** 提出系统化的训练框架，开发LegalMidm韩国法律模型；构建高质量、用例驱动的法律数据集；优化训练流程；与法律专业人士合作进行严格的数据筛选以确保相关性和事实准确性。

**Result:** 研究证明了该方法在关键法律任务中的有效性，提升了模型在韩国法律领域的实用价值。

**Conclusion:** 基于实际需求、专业协作和严格数据管理的系统化训练方法，能够有效提升法律大模型的实用性，为法律领域AI应用提供了可行路径。
...
> 研究者提出了Faithfulness-QA数据集，通过反事实实体替换方法构建了99,094个样本，用于训练和评估RAG模型的忠实度，解决模型过度依赖参数记忆而非检索上下文的问题。

<details>
  <summary>Details</summary>

**Motivation:** RAG模型经常产生基于参数记忆而非检索上下文的答案，违背了检索增强的核心承诺。根本障碍是缺乏明确要求模型优先选择上下文而非内部知识的训练数据。

**Method:** 从SQuAD和TriviaQA两个现有的抽取式问答基准开始，自动识别上下文中的答案实体，用来自精心构建的76,953个实体库中类型一致的备选实体替换，从而制造上下文与参数记忆之间受控的知识冲突。通过严格的质量过滤确保数据质量。

**Result:** 构建了包含99,094个样本的大规模数据集，在随机200样本审计中四个自动化检查的通过率达到100%。发布了完整数据集、构建管道和覆盖八种命名实体类别的类型化实体库。

**Conclusion:** Faithfulness-QA作为注意力 faithful度目标的训练资源和评估RAG系统上下文定位行为的基准，为提升RAG模型的忠实度提供了重要支撑。
...
> SOB是一个多模态结构化输出基准测试，包含文本、图像和音频三个源域，评估结果表明模型在模式遵循方面表现接近完美，但在值准确性方面仍有显著差距。

<details>
  <summary>Details</summary>

**Motivation:** 现有结构化输出基准测试要么只关注模式合规性，要么仅在单一源域中评估值正确性，缺乏跨多源模态的公平比较框架。

**Method:** 构建包含5000个文本、209个图像和115个音频记录的基准数据集，所有输入统一标准化为文本表示以隔离结构化输出能力，使用自然语言问题、JSON模式和真实答案进行评估。

**Result:** 21个模型在三个源域上评估显示：模式遵循率接近完美，但值准确率分别为文本83.0%、图像67.2%、音频23.7%，上下文越长提取越困难。

**Conclusion:** 当前大型语言模型在结构化输出任务中仍存在显著的值提取准确性挑战，特别是在音频等长上下文场景下，需要进一步改进以提高实际应用效果。
...
> 本研究构建了首个大规模荷兰语医疗语言语料库，包含350亿个token和1亿个文档，已开源发布。

<details>
  <summary>Details</summary>

**Motivation:** 荷兰语医疗语料资源的稀缺限制了自然语言处理技术在医疗领域的发展和应用。

**Method:** 通过翻译英文数据集、在通用语料中识别医疗文本以及提取开放荷兰语医疗资源等多种方法构建语料库。

**Result:** 成功创建了包含约350亿token、跨越医疗领域的大型语料库，涵盖约1亿个文档，并在Hugging Face平台上免费发布。

**Conclusion:** 这项工作建立了首个大规模荷兰语医疗语言语料库，为该语言的预训练模型和下游NLP任务提供了重要资源基础。
...
> 本研究提出了一种将原始维基媒体数据转换为高质量南斯拉夫语族文本语料库的方法，通过提取清理和n-gram过滤来去除低质量重复内容。

<details>
  <summary>Details</summary>

**Motivation:** 南斯拉夫语族语言缺乏高质量文本语料库，而维基媒体包含大量原始数据但存在质量问题，需要系统化的方法来提取适合语言模型训练和比较研究的优质文本。

**Method:** 采用两阶段方法：第一阶段从Wikipedia等多个维基媒体项目中提取和清理文本，处理原始wiki标记；第二阶段使用n-gram过滤策略检测并移除具有高重复性的可疑低质量文章。

**Result:** 成功创建了语言学丰富、反映真实语言使用的南斯拉夫语族高质量文本语料库，为语言模型训练和跨语言比较研究提供了可靠数据基础。

**Conclusion:** 该方法虽然针对南斯拉夫语族开发，但基本语言无关，可推广应用于其他语言和语系，为构建高质量多语言语料库提供了有效途径。
...
> 通过对比PyCaret机器学习和IndoBERT微调两种方法，发现基于Transformer的IndoBERT在印尼语推文情感分析任务中显著优于传统机器学习方法，准确率达到89.59%。

<details>
  <summary>Details</summary>

**Motivation:** 针对印尼语社交媒体文本的情感分析挑战，特别是在印尼新首都(IKN)相关讨论中，需要评估传统机器学习与现代深度学习方法在处理非正式印尼语文本时的效果差异。

**Method:** 实验对比两种技术路线：1) 使用PyCaret AutoML框架评估逻辑回归、朴素贝叶斯和支持向量机等传统机器学习模型；2) 微调indobenchmark/indobert-base-p1预训练模型。数据集包含1472条人工标注的印尼语推文，采用10折交叉验证评估传统方法。

**Result:** 传统机器学习中逻辑回归性能最佳(77.57%准确率，77.17% F1分数)，而IndoBERT微调后达到89.59%测试准确率和89.37% F1分数，性能提升超过12个百分点。

**Conclusion:** 结果表明，基于Transformer的深度学习方法在处理非正式印尼语社交媒体文本时具有明显优势，特别是IndoBERT能够有效捕获上下文语义信息，在该任务上显著超越了传统机器学习基线模型。
...
> 该研究通过最大更新参数化(muP)方法成功将概率Transformer(PT)模型从小规模扩展到4亿参数规模，使PT在掩码语言建模任务上持续优于同等参数预算的标准Transformer。

<details>
  <summary>Details</summary>

**Motivation:** 概率Transformer(PT)虽然在计算结构和下游任务性能上与标准Transformer相似，但对超参数选择较为敏感，导致难以高效扩展到大规模模型，这限制了其在实际应用中的部署。

**Method:** 采用最大更新参数化(muP)方法对PT模型的参数进行重新缩放，使得在小模型上优化的超参数可以直接迁移到更大的模型中，无需额外的调优。

**Result:** 成功将PT扩展到4亿参数规模，在掩码语言建模(MLM)任务上，PT模型在相同参数预算下持续优于标准Transformer模型。

**Conclusion:** 该研究为概率模型在未来 substantially 更大规模的实际部署提供了重要贡献，展示了通过muP方法可以有效解决概率Transformer的扩展难题。
...
> 研究发现大型语言模型通过指示词测试显示：人类在不同语言中存在明显的文化和认知差异，而LLMs缺乏这种根植于身体的知识和文化敏感性。

<details>
  <summary>Details</summary>

**Motivation:** 探讨大型语言模型是否能从文本中真正获得具身认知和文化约定，传统评估方法难以检测这种深层知识。

**Method:** 使用指示词（如this/that、这/那）作为新探针，收集320名母语者的6400个响应作为人类基线，测试5个最先进的LLMs表现。

**Result:** 英语使用者能区分远近但难以视角转换，汉语使用者能流畅转换视角但容忍距离模糊；所有LLMs无法理解远近对比，没有文化差异，以英语为中心的推理。

**Conclusion:** 提出了基于指示词的新评估任务，揭示了人类跨文化解释的不对称性，为自我中心-社会中心辩论提供新视角，呼吁未来模型设计考虑个体差异。
...
> ReQueR是一个推理时对齐框架，通过强化学习训练查询优化器，将模糊的人类查询转换为显式逻辑分解，有效激发LLMs的推理能力，在多个基准测试中实现1.7%-7.2%的绝对提升。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型由于其推理能力受到分布不匹配的限制——模糊的人类查询与机器激活所需的结构化逻辑之间的不匹配。现有方法要么需要对每个模型分别微调(O(N)成本太高)，要么依赖静态提示无法解决查询层面的结构复杂性。

**Method:** 提出ReQueR框架，将推理激发视为推理时对齐任务。通过强化学习训练专门的Refiner策略来重写原始查询为显式逻辑分解，将冻结的LLMs视为环境。基于教育心理学的最近发展区理论，引入自适应求解器层次结构，通过动态对齐环境难度与Refiner的进化能力来稳定训练。

**Result:** ReQueR在多样化架构和基准测试中实现了1.7%-7.2%的一致绝对增益，平均超过强基线2.1%。关键的是，它为一对多推理时推理激发提供了有前景的范式，使得在少量模型上训练的单个Refiner能够有效解锁多样化未见模型的推理能力。

**Conclusion:** ReQueR为大型语言模型的推理能力激发提供了一个高效、可扩展的解决方案，实现了跨模型的单对多推理时对齐，降低了部署成本并提高了推理效果。
...
> 本文提出了一个多司法管辖区的人工智能法规检索增强生成系统，通过类型特定分块、条件检索路由和优先级重排序等技术，有效提升了跨司法管辖区AI法规查询的准确性和可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 随着AI法规在全球各司法管辖区的快速发展和差异化管理，政策制定者、法律专业人士和研究者在跨司法管辖区导航AI法规面临巨大困难，需要专门的技术解决方案来处理这种复杂的法规查询需求。

**Method:** 构建了包含68个司法管辖区242份文档的语料库，采用三项技术创新：1) 类型特定分块技术保持异质文档的法律结构；2) 集成实体检测和元数据的条件检索路由用于法律引用；3) 基于优先级的重排序提升已颁布法规相对于政策和次要来源的权重。

**Result:** 在50个查询的评估中表现优异，平均忠实度0.87，平均答案相关性0.84。单实体查询达到0.86忠实度和0.92相关性，多司法管辖区比较查询达到0.88忠实度和0.75相关性。

**Conclusion:** 研究结果证明了领域特定的检索策略在导航复杂、异质的法规语料库方面的有效性，为处理全球AI法规的碎片化和复杂性提供了可行的技术路径。
...
> 本文比较了机器学习与深度学习在印尼产品评论文本情感分析上的表现，发现传统机器学习方法在准确率上略优于复杂的BiLSTM+Attention模型，同时计算效率更高。

<details>
  <summary>Details</summary>

**Motivation:** 电商平台的产品评论情感分析对于理解客户满意度和为商家提供改进产品质量的洞察至关重要，需要寻找高效准确的分析方法。

**Method:** 使用包含19,728个平衡样本的印尼产品评论数据集，基于PyCaret AutoML框架评估了逻辑回归、支持向量机和LightGBM三种机器学习算法，并与基于BiLSTM+Attention架构的深度学习方法进行对比。

**Result:** 传统机器学习方法表现优异，其中逻辑回归达到97.26%的准确率和F1分数，而BiLSTM+Attention模型在测试集上达到97.24%的准确率和F1分数，机器学习方法略胜一筹。

**Conclusion:** 在高维数据集上，经过适当预处理和特征提取的传统机器学习算法能够与复杂的序列深度学习架构相媲美甚至略有优势，同时提供更高的计算效率，证明了传统方法在特定任务中的实用价值。
...
> 研究显示不同方言会影响大型语言模型的餐厅和产品推荐，特别是印度英语和印英混合语会显著改变推荐结果。

<details>
  <summary>Details</summary>

**Motivation:** 研究旨在调查大型语言模型在面对不同英语方言（美国南方英语、印度英语、印英混合语）进行餐厅和产品推荐时是否存在语言偏见。

**Method:** 使用Yelp和沃尔玛数据集，通过零样本提示让LLM从预设的餐厅和产品列表中选择top-20推荐。采用20个不同随机种子进行实验，使用混合效应回归模型分析结果。

**Result:** 方言确实影响推荐选择。mistral-small-3.1和llama-3.1系列模型对印度英语和混合语更敏感。产品推荐中，llama-3.1-70B模型在7个类别中的4个类别对混合语特别敏感。未发现模型规模的普遍趋势差异。

**Conclusion:** 大型语言模型在推荐系统中存在方言偏见，这种影响因模型和方言类型而异，需要在实际应用中加以考虑。
...
> 本文提出了一种基于依赖感知的多阶段提示管道方法，用于生成结构一致的多层次RPG叙事内容，通过结构化JSON中间表示确保各阶段间的逻辑关联。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在叙事生成方面表现出潜力，但在复杂多层次RPG世界构建中仍存在连贯性、可控性和结构一致性问题，限制了其实际应用。

**Method:** 设计了一个分为五个阶段的提示管道：世界构建、NPC创建、玩家角色创建、战役级任务规划和任务扩展。每个阶段基于前一阶段的结构化JSON输出，通过强制模式和显式数据流来建模叙事依赖关系。

**Result:** 定性评估表明，该管道能稳定生成逻辑合理、结构有效的RPG内容，复杂度增加时质量不会下降。将高层战役规划与详细任务扩展分离提高了全局结构和局部叙事质量。

**Conclusion:** 基于依赖感知的提示管道与结构化中间表示是LLM程序化内容生成的有效设计模式，该方法可推广到其他需要在演化上下文状态中进行顺序推理的领域。
...
> 这项针对7个国家4641名参与者的大规模跨文化研究发现，大语言模型在情感支持领域的使用率因国家差异很大（20%-59%），并识别出影响用户感知的关键社会经济和文化因素。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型越来越被用作情感支持工具，但跨文化采用模式和用户感知差异尚未被系统研究。填补这一知识空白对于理解如何安全地开发和部署这类系统至关重要。

**Method:** 研究通过在线问卷调查了美国、英国、德国等7个国家的4641名参与者，使用混合模型分析文化效应和人口统计因素的影响，并收集了731个真实的多语言用户交互提示语料。

**Result:** 研究发现使用率在英语国家更高；年龄25-44岁、有宗教信仰、已婚和高社会经济地位是积极感知的预测因素，其中社会经济地位影响最强；用户主要寻求孤独、压力、关系冲突和心理健康方面的帮助。

**Conclusion:** 大语言模型情感支持的使用受到复杂社会技术环境的影响，需要更广泛的研究议程来确保这些系统的安全开发和知情访问。
...
> 我们推出了Marco-MoE，这是一个完全开源的多语言稀疏混合专家模型，仅激活5%参数即可在5T token上高效训练，在性能和计算效率上超越同类模型。

<details>
  <summary>Details</summary>

**Motivation:** 解决大型语言模型训练和推理成本过高的问题，同时提升多语言场景下的性能表现，避免不同语言间的相互干扰。

**Method:** 采用极度稀疏的混合专家架构，每个token仅激活约5%的参数；通过从密集模型进行upcycling实现高效预训练；使用5T token进行大规模训练；后续指令微调创建Instruct变体。

**Result:** 在英语和多语言基准测试上超越同规模竞争模型，实现了最佳的计算效率比；Instruct变体超越了激活参数多3-14倍的竞争模型；展现出跨相关语言的结构化专家激活模式。

**Conclusion:** Marco-MoE证明了稀疏MoE架构在多语言场景下的有效性，能够实现可扩展的语言扩展而无干扰，并提供了完全开源的训练数据、配方和模型权重支持社区发展。
...
> Perspective API将于2026年底关闭，这一事件暴露了NLP、CSS和LLM评估领域对单一专有工具的过度依赖，并呼吁建立独立、有效、可适应且可重现的毒性言论测量基础设施。

<details>
  <summary>Details</summary>

**Motivation:** Perspective API作为毒性言论自动测量的事实标准即将关闭，这一事件迫使研究社区反思对单一专有工具的依赖问题，以及由此产生的认知问题对集体研究努力的影响。

**Method:** 通过分析Perspective API的结构性依赖问题，包括其模型的无版本更新、单一企业化操作、同时作为评估目标和标准等，论证现有测量基础设施的缺陷，并提出建立新基础设施的技术和治理要求。

**Result:** Perspective的关闭将导致无法更新的基准、不可重现的结果，以及研究社区可能转向闭源LLM而延续这些问题，凸显了建立独立测量基础设施的紧迫性。

**Conclusion:** 研究社区需要建立独立、有效、可适应且可重现的毒性言论和仇恨言论测量基础设施，以解决当前的认知问题并确保研究的可持续性和可重现性。
...
> WhisperPipe是一种实时语音识别流式处理架构，通过混合VAD、动态缓冲和自适应处理策略，在保持高精度的同时显著降低了延迟和内存消耗。

<details>
  <summary>Details</summary>

**Motivation:** 现有实时ASR系统面临准确性与计算效率的根本性权衡，传统的流式方法要么通过激进分块牺牲准确性，要么通过无界上下文累积导致过高的内存成本，特别是在部署大规模Transformer模型如Whisper时。

**Method:** 提出三项关键创新：1）结合Silero VAD与基于能量过滤的混合VAD流水线，减少34%的误激活；2）具有重叠上下文的动态缓冲机制，防止分段边界信息丢失；3）基于语音特性的自适应处理策略，平衡延迟与准确性。

**Result:** 在2.5小时多样化音频测试中，中位数端到端延迟89ms（第90百分位142ms），峰值GPU内存减少48%，平均GPU利用率降低80.9%，150分钟连续运行内存零增长，WER与离线Whisper差距在2%以内，延迟比现有流式解决方案低3-5倍。

**Conclusion:** WhisperPipe证明了精心设计的架构可以在生产ASR系统中协调实时响应性与模型复杂性的竞争需求，其模块化设计支持从边缘设备到云端基础设施的多样化部署环境。
...
> 研究针对大语言模型文化对齐问题，提出了新的文化评估数据集设计指南，并通过对比实验证明了该数据集能有效区分文化特化模型。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型的文化（不对齐）问题日益受到关注，但文化评估数据集的设计和开发工作有限，现有方法存在主要局限性。

**Method:** 提出注释者的设计指南，根据这些原则构建数据集，并使用该数据集进行一系列对比实验。

**Result:** 实验结果表明，新设计的数据集具有更强的判别能力，能有效区分针对特定文化优化的模型和非特化模型。

**Conclusion:** 该研究为文化评估数据集的构建提供了有效的设计框架，能够更准确地评估大语言模型的文化适应能力。
...
> 该研究系统评估了14种自动摘要指标在不同领域的表现，提出了基于大语言模型的自反思摘要框架LLM-ReSum，显著提升了摘要质量，并发布了法律文档摘要评测基准PatentSumEval。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型生成的摘要质量评估仍是一个开放挑战，特别是在异构领域和不同文档长度下，传统评估指标如ROUGE与人类判断的相关性较弱。

**Method:** 通过跨7个数据集、5个领域的元评估分析，包含1500多个人工标注摘要，比较传统指标、神经度量指标和LLM评估器；基于研究结果提出LLM-ReSum框架，在无需微调的情况下将LLM评估与生成集成在闭环反馈中。

**Result:** 传统词汇重叠指标与人类判断相关性弱或负相关，而任务特定神经指标和LLM评估器实现更高对齐；LLM-ReSum在三个领域中分别提升事实准确性33%和覆盖度39%，89%情况下人类评估者偏好改进后的摘要。

**Conclusion:** 该研究为LLM摘要评估提供了全面insights，证明了LLM评估器的有效性，提出的LLM-ReSum框架显著改善了摘要质量，新发布的PatentSumEval基准将推动法律文档摘要研究发展。
...
> 通过上采样稀有颜色术语和多听众强化学习交互，可以显著改善神经代理产生的颜色词汇系统，使其在词汇多样性、信息性和几何结构上更接近人类。

<details>
  <summary>Details</summary>

**Motivation:** 现有神经代理通过监督学习和强化学习产生的颜色词汇与人类类别存在系统性差异，主要表现为颜色空间中高度非凸的区域，这与人类典型的凸性类别特征不符。

**Method:** 在NeLLCom-Lex框架中引入两个关键改进：1)在监督学习阶段对稀有颜色术语进行上采样；2)采用多听众强化学习交互设置，并使用凸性度量来量化颜色空间的几何一致性。

**Result:** 上采样提高了词汇多样性和词汇系统的信息性，多听众设置促进了更凸的颜色类别形成。中等程度的上采样结合多听众产生的词汇系统最接近人类。

**Conclusion:** 适当的上采样策略和多听众交互机制是产生类人颜色词汇系统的关键因素，为计算语言系统中人类类别形成的研究提供了重要见解。
...
> CORAL是一种针对多语言检索增强生成的自适应检索方法，通过迭代优化检索空间和查询来解决文化背景相关的检索-条件错位问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多语言检索增强生成方法使用固定检索空间，在处理文化背景查询时可能出现检索-条件错位，导致检索器和生成器难以产生文化相关的答案。

**Method:** CORAL通过智能代理循环实现：1)选择语料库，2)检索文档，3)评估证据的相关性和文化对齐度，4)检查充分性。如证据不足，则5)重新选择语料库并重写查询。

**Result:** 在两个文化问答基准测试中，CORAL相比最强基线在低资源语言上实现了最高3.58个百分点的准确率提升。

**Conclusion:** 自适应的检索空间和查询优化是解决文化相关多语言检索增强生成中检索-条件错位问题的有效方法，特别是在低资源语言场景下效果显著。
...
> 本文提出基于强化学习的后训练方法，直接偏好优化(DPO)显著提升了神经机器翻译质量，COMET分数从0.703提高至0.747。

<details>
  <summary>Details</summary>

**Motivation:** 当前神经机器翻译系统虽然发展迅速，但仍存在持续的翻译错误，需要有效的方法来纠正这些错误。

**Method:** 提出使用直接偏好优化(DPO)的后训练框架，仅需一般文本语料和专家翻译器(人或AI)提供迭代反馈，应用于英德翻译任务。

**Result:** 在gemma3-1b模型上应用DPO框架，英德翻译的COMET评分从0.703提升至0.747，显著改善了翻译质量。

**Conclusion:** DPO为通过偏好化后训练提升预训练神经机器翻译模型提供了一条高效稳定的路径。
...
> 研究通过跨语言语义相似度检测攻击提示来保护大语言模型的多语言安全，发现在标准基准测试中有效但在分布外场景下性能显著下降。

<details>
  <summary>Details</summary>

**Motivation:** 现有大型语言模型的安全机制主要针对英语，导致多语言部署时存在系统性漏洞。恶意提示通过翻译到其他语言就能绕过安全防护，暴露了跨语言安全缺口。

**Method:** 采用训练无关的外部护栏方法，将多语言查询嵌入与固定的英语越狱提示码本进行比较，无需重新训练或语言特定适配。使用多语言嵌入模型实现跨语言语义相似度检测。

**Result:** 在包含经典越狱模板的基准测试中，语义相似度检测达到近乎完美的分离能力（AUC最高0.99）；但在行为多样性的异构不安全基准上，分离能力显著退化（AUC约0.60-0.70），安全关键的低误报率下召回率下降。

**Conclusion:** 跨语言语义相似度在标准场景下能有效防御越狱攻击，但面对分布偏移时鲁棒性不足，说明仅靠语义相似度难以解决所有跨语言安全问题，需要更复杂的多层防护策略。
...
> 该研究系统评估了从食谱文本中准确估算营养成分的不同方法，发现大型语言模型(LLM)在精度上表现最佳，但计算成本高；传统TF-IDF方法速度快但精度中等。

<details>
  <summary>Details</summary>

**Motivation:** 从非结构化食谱文本中准确估算营养素是饮食监控中重要但具有挑战性的问题，主要困难在于成分术语模糊和数量表达高度变化。

**Method:** 系统评估了从词汇匹配方法(TF-IDF+岭回归)、深度语义编码器(DeBERTa-v3)到生成式推理的大型语言模型等多种方法，并在欧盟EU Regulation 1169/2011的严格容差标准下进行比较。

**Result:** TF-IDF基准方法推理速度近瞬时但精度中等；DeBERTa-v3在任务数据稀缺时表现不佳；少样本LLM推理(Gemini 2.5 Flash)和混合LLM refining管道在所有营养类别中验证精度最高。

**Conclusion:** LLM通过利用预训练的世界知识解决了模糊术语和非标准单位的标准化问题，但需要承担更高的推理延迟成本，揭示了饮食监控系统中实时效率与营养精度之间的实际部署权衡。
...
> 语音情感识别(SER)研究的动机与实践之间存在显著脱节，研究使用的情感数据集与宣称的应用场景不符，亟需重新审视并明确具体应用案例。

<details>
  <summary>Details</summary>

**Motivation:** 情感识别技术的批评引发了关于任务有效性和潜在影响的伦理担忧，但语音情感识别研究未能充分吸收这些讨论。研究者需要确保其宣称动机与实际研究内容的一致性。

**Method:** 通过对语音情感识别研究进行系统性调查，分析其宣称的研究动机、使用的数据集以及研究的情感类别，揭示动机与实践之间的对应关系。

**Result:** 研究发现SER研究虽然提出了语音激活系统或医疗保健等吸引人的应用目标，但常用的数据集并不能反映这些实际部署场景，暴露了动机与研究实践的明显不一致。

**Conclusion:** 这种动机与实践的脱节会引发伦理问题，SER研究应该重新定位并确立具体的使用案例，以防止误解、误用和下游危害。
...
> 本研究通过引入'潜意识引导'方法，证明了语言模型可以通过看似无关的数据精确地继承复杂的多词汇偏见，并揭示了其内在机制。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究对潜意识学习现象的信号传输范围、解释机制和偏见编码精度等问题尚无明确答案，需要更深入的研究来填补这些知识空白。

**Method:** 研究者提出了潜意识引导方法，使用通过训练得到的引导向量而非系统提示来实现教师模型的偏见，并在多层级进行实验验证。

**Result:** 实验证明该方法能够传输复杂的多词汇偏见， Bias被精确编码，新训练的引导向量与原始向量具有高余弦相似度。

**Conclusion:** 潜意识学习具有广泛的信号传输能力和精确的编码特性，其机制涉及引导向量在特定层级的直接转移，为理解模型偏见传播提供了新见解。
...
> MAIC-UI是一个零代码创作系统，帮助教育者快速将教科书、PPT和PDF转换为交互式STEM课件，实现10秒内的修改迭代。

<details>
  <summary>Details</summary>

**Motivation:** 传统STEM课件创作需要HTML/CSS/JavaScript技能，现有AI工具只能生成静态演示、处理长文档困难且缺乏教学准确性保证，修改时重新生成耗时200-600秒，破坏创作流程。

**Method:** 采用三种核心技术：1）多模理解的结构化知识分析确保教学严谨性；2）两阶段的生成-验证-优化流程，分离内容对齐与视觉优化；3）点击定位编辑与统一差分增量生成，实现亚10秒迭代周期。

**Result:** 实验室研究表明MAIC-UI比直接文本转HTML生成减少编辑迭代次数（4.9 vs 7.0），显著提升学习性和可控性；三个月课堂部署显示试点班级STEM科目成绩提升9.21分，而对照组下降2.32分。

**Conclusion:** MAIC-UI成功解决了教育者创作交互式课件的技术门槛问题，通过快速的增量编辑机制维持创作流畅性，并有效促进学习效果，减少学习结果差异。
...
> 本研究提出PSI-Bench自动化评估框架，系统评估了7个大语言模型作为抑郁症患者模拟器的表现，发现当前模拟器存在回应过长、情感转变过快、行为模式单一等问题，且模拟框架比模型规模对保真度影响更大。

<details>
  <summary>Details</summary>

**Motivation:** 抑郁症患者模拟器在心理健康培训中需求增长，但现有评估方法存在严重缺陷：过度依赖提示不明确的LLM评判器，缺乏对行为多样性的评估，无法全面衡量模拟器的真实性和有效性。

**Method:** 开发PSI-Bench多维度自动评估框架，从话轮级、对话级和群体级三个层面评估模拟器行为，使用临床依据的诊断指标；对7个LLM在两种模拟框架下进行基准测试；开展人工验证研究证明评估结果与专家判断的一致性。

**Result:** 发现当前抑郁症患者模拟器产生过长且词汇丰富但缺乏变化的回应，情感解决过快，普遍遵循消极到积极的统一轨迹；模拟框架对保真度的影响超过模型规模；人工研究验证了评估结果与专家判断的强相关性。

**Conclusion:** 揭示了当前抑郁症患者模拟器的关键局限性，提供了可解释、可扩展的基准测试工具，为未来模拟器设计和评估提供了指导方向，强调了评估方法标准化和临床依据的重要性。
...
> 本文提出Agentic Harness Engineering (AHE)框架，通过三个可观测性支柱实现编码代理harness的自动化进化，在Terminal-Bench 2上将pass@1从69.7%提升至77.0%，超越人工设计的harness，且进化后的harness具有良好的跨模型迁移能力。

<details>
  <summary>Details</summary>

**Motivation:** Harness是影响编码代理性能的核心因素，但自动化harness工程面临巨大挑战：异构的动作空间、稀疏且嘈杂的评估信号、数百万token的轨迹长度，以及编辑效果难以归因等问题，导致自动化进化容易陷入试错循环。

**Method:** 提出AHE框架，通过三个可观测性支柱实现harness工程循环的自动化：(1)组件可观测性：为每个可编辑组件提供文件级表示；(2)经验可观测性：将原始轨迹压缩为可分层的证据语料库；(3)决策可观测性：为每次编辑配对自声明预测并与后续结果验证。

**Result:** 十轮AHE迭代在Terminal-Bench 2上达到77.0% pass@1，超过人工设计的Codex-CLI (71.9%)和自进化基线。冻结的harness在SWE-bench-verified上用12%更少token达到聚合成功，在Terminal-Bench 2上为三个不同模型家族带来5.1-10.1pp的跨家族提升。

**Conclusion:** 实验结果表明，AHE能够自主进化出编码通用工程经验的harness组件而非特定基准调优，为编码代理harness的持续改进提供了实用的可观测性驱动进化路径。
...
> G-Loss是一种图引导的损失函数，通过半监督标签传播利用嵌入流形中的结构关系，在多个文本分类任务中超越了传统损失函数，更快收敛并获得更高准确率。

<details>
  <summary>Details</summary>

**Motivation:** 传统的损失函数（如交叉熵、对比损失等）仅能在局部邻域内操作，无法考虑全局语义结构，限制了预训练语言模型学习判别性和鲁棒性嵌入的能力。

**Method:** G-Loss构建文档相似性图来捕获全局语义关系，结合半监督标签传播指导模型学习。该方法将图结构与损失函数相结合，在嵌入流形中利用结构化信息。

**Result:** 在MR、R8、R52、Ohsumed和20NG五个基准数据集的评估中，G-Loss在多数实验设置中收敛更快，产生语义连贯的嵌入空间，分类准确率高于使用传统损失函数的微调模型。

**Conclusion:** 通过融入全局语义结构信息，G-Loss有效改进了预训练语言模型的微调过程，为文本分类任务提供了一种更有效的学习范式，证明了结构化损失函数的重要性。
...
> Luminol-AIDetect是一种零样本AI生成文本检测方法，通过随机扰乱文本结构的统计方法来区分机器生成文本与人写作文本，具有跨模型、跨语言的通用性。

<details>
  <summary>Details</summary>

**Motivation:** 现有AI文本检测方法依赖模型特定特征，而作者假设虽然大语言模型在局部语义一致性方面表现优异，但其自回归特性导致相比人类写作存在结构性脆弱性。

**Method:** 提出基于连贯性中断的统计方法Luminol-AIDetect，通过随机文本重排过程，利用困惑度漂移作为模型无关的判别指标，提取少量基于困惑度的标量特征，通过密度估计和集成预测进行检测。

**Result:** 在8个内容领域、11种对抗攻击类型和18种语言评估中显示出最先进性能，误报率最高降低17倍，且计算成本低于先前方法。

**Conclusion:** 该方法成功揭示了AI生成文本的结构性脆弱性特征，为通用、鲁棒的AI文本检测提供了新的思路和有效工具。
...
> 该研究使用稀疏自编码器分析了大语言模型中情感识别的内部机制，发现了三相信息流模式，并提出了一种高效的因果特征引导方法来提升情感识别性能。

<details>
  <summary>Details</summary>

**Motivation:** 随着大语言模型在情感敏感的人机交互应用中越来越广泛，但模型内部如何表示和识别情感的机制尚不清楚，理解这些内部机制对于提升模型性能和可控性至关重要。

**Method:** 研究人员采用稀疏自编码器(SAEs)技术分析LLM各层中的稀疏特征激活模式，通过相位分层因果追踪识别关键特征，并开发可解释的因果特征引导方法来改进情感识别。

**Result:** 研究发现情感识别包含三个阶段的信息流，情感相关特征只在最后阶段出现；情感表示包含共享特征和特定情感特征；厌恶情感的表示比其他情感更弱且更分散；提出的新方法显著提升了多模型和多数据集上的情感识别性能。

**Conclusion:** 该研究为理解LLM中情感识别的内部机制提供了系统分析，并引入了一种高效、可解释且可控的方法来改进模型性能，为开发更好的情感感知AI系统奠定了基础。
...
> 本文提出使用几何代数(GA)作为自然语言语义的数学基础，通过函数几何代数(FGA)框架实现类型化、可组合的语义表示，克服传统线性代数的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的分布式和神经方法虽然成功，但在线性代数基础上面临组合语义、类型敏感性和可解释性等结构性限制，需要更强大的数学框架。

**Method:** 提出函数几何代数(FGA)框架，将几何代数(特别是Clifford代数)扩展为类型化、可组合的语义系统，与分布式学习和神经架构完全兼容。

**Result:** 识别了GA提供而线性代数所不具备的三个核心能力，通过具体示例展示了操作级语义对比，并揭示了transformer架构中隐含的GA操作。

**Conclusion:** GA的核心优势在于结构组织的提升：将n维嵌入空间扩展为2^n的多向量代数，使基础语义概念和高阶交互能在统一的代数框架中表示，为语义推理和可解释性提供更好的基础。
...
> 研究表明AI熟练用户与新手在交互方式上存在根本差异：熟练用户主动协作迭代，虽经历更多失败但成功率更高；新手常采取被动态度， experiencing invisible failures。这要求个人和产品开发者重新思考AI成功使用的策略。

<details>
  <summary>Details</summary>

**Motivation:** 探究用户AI技能水平如何影响AI实际表现，这一问题对用户、AI产品开发者及整个社会都至关重要，但目前缺乏深入研究。

**Method:** 基于WildChat-4.8M数据集中27K个精心标注的对话转录本进行实证分析，对比熟练用户与新手在任务复杂度、交互模式、失败类型和成功率等方面的差异。

**Result:** 发现熟练用户承担更复杂任务，采用主动协作的交互模式；虽然经历更多可见失败，但更有可能部分恢复，并在复杂任务上取得更大成功。新手更多经历隐形失败，即表面上成功但实际未达目标的对话。

**Conclusion:** AI成功取决于用户主动参与而非被动接受，产品开发者应认识到他们在设计用户行为而不只是模型行为，鼓励深度参与而非无摩擦体验将带来更好的整体效果。
...
> DV-World是一个包含260个任务的现实世界数据可视化基准测试，涵盖原生电子表格操作、跨平台演进和主动意图对齐三个领域，评估显示当前最先进模型的整体性能不足50%。

<details>
  <summary>Details</summary>

**Motivation:** 现有数据可视化基准存在代码沙箱限制、单一语言创建任务和完美意图假设等问题，无法真实反映现实世界中数据可视化的复杂性，包括环境原生性、跨平台演化和主动意图对齐等需求。

**Method:** 构建了DV-World基准测试，包含三个领域：DV-Sheet（原生电子表格操作）、DV-Evolution（跨编程范式的视觉工件适配重构）和DV-Interact（模拟真实模糊需求的主动意图对齐），并采用混合评估框架结合数值对齐和MLLM评判。

**Result:** 实验表明最先进模型的整体性能低于50%，暴露了在处理复杂数据可视化挑战方面的关键缺陷，验证了基准测试的有效性和现实性。

**Conclusion:** DV-World为数据可视化智能体提供了现实的测试平台，有助于推动开发 enterprise 工作流所需的多样化专业技能，填补了现实世界数据可视化评估的空白。
...
> ESICA是一个轻量级、可扩展的文本引导3D医学图像分割框架，通过创新的相似性矩阵mask预测、高效分解解码器和双阶段优化策略，在多种医学影像模态上实现了最先进的分割精度和效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本引导3D医学图像分割框架存在计算成本高、文本与体积特征对齐弱、无法捕捉精细解剖细节等问题，限制了其在临床工作流程中的实际应用。

**Method:** ESICA框架包含三个核心创新：(1)基于相似性矩阵的mask预测方法增强语义对齐；(2)带有适配器模块的高效分解解码器实现精确体积解码；(3)双通道优化策略锐化边界并解决不确定区域；采用正样本预训练和平衡微调的两阶段训练方案。

**Result:** 在CVPR BiomedSegFM基准测试(涵盖CT、MRI、PET、超声和显微镜五种成像模态)上，ESICA实现了最先进的分割精度；其轻量级变体ESICA4 Lite以更少参数获得相似性能，具有优异的效率-精度平衡。

**Conclusion:** ESICA框架推动了文本引导分割向高效、可扩展和临床可部署系统的发展，为医学图像分割提供了灵活且实用的解决方案。
...
> 本文提出了一个完全开源的、可复现的扩散模型光照控制流水线，通过自动构建训练数据集并在多个开源模型上进行微调，显著提升了图像光照控制效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的光照控制模型要么是闭源的，要么需要复杂的控制输入（如深度图），缺乏真正可用的开源解决方案，限制了研究者和开发者的使用。

**Method:** 构建数据引擎将光照良好图像转换成监控训练三元组（光照不佳输入图像+自然语言光照指令+光照良好输出图像），然后微调扩散模型。

**Result:** 在感知相似性、结构相似性和身份保持方面显著超越了SD 1.5、SDXL和FLUX.1-dev等基线模型。

**Conclusion:** 提供完全基于开源工具和公开数据的可复现解决方案，所有代码、数据和模型权重均已公开，促进了开源光照控制研究的发展。
...
> 提出了一种高效的、与分辨率无关的自回归图像合成方法VibeToken，通过动态token序列实现任意分辨率生成，在1024x1024分辨率下仅需64个token，计算效率比传统方法提升63.4倍。

<details>
  <summary>Details</summary>

**Motivation:** 解决传统自回归模型只能在固定分辨率工作、计算量随分辨率平方增长的问题，缩小与扩散模型在大规模图像生成方面的差距。

**Method:** 开发了VibeToken——一种分辨率无关的1D Transformer图像分词器，可将图像编码为32-256个动态可控token序列；基于此构建VibeToken-Gen，支持任意分辨率生成的类条件自回归生成器。

**Result:** 在1024x1024图像生成中仅需64个token达到3.94 gFID，而扩散模型需1024个token仅达5.87 gFID；计算量恒定在179G FLOPs，比LlamaGen的11T FLOPs提升63.4倍效率。

**Conclusion:** VibeToken为自回归视觉生成模型在生产环境中的广泛应用提供了可能，实现了分辨率无关的高效图像生成。
...
> 本文提出EM-QnF任务，允许用户通过反馈和追问来提高记忆查询的准确性，并设计了轻量级的FALM模块，显著提升了现有模型在自然语言查询视频记忆中的表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自然语言查询 episodic memory (EM-NLQ) 方法采用一次性查询，无法处理模糊或不完整的查询，限制了在真实场景中的应用。用户需要能够通过反馈和追问来帮助模型准确理解查询意图。

**Method:** 1) 提出EM-QnF任务框架，支持交互式反馈；2) 收集基于反馈交互的数据集；3) 设计轻量级训练方案，避免昂贵的序列优化；4) 开发即插即用的FALM模块，使现有EM-NLQ模型能有效整合用户反馈。

**Result:** 在三个具有挑战性的基准测试中，该方法显著超越了现有技术水平，性能优于或媲美商业大型视觉语言模型，同时保持高效率。人类反馈评估表明该方法在真实场景中具有良好的泛化能力。

**Conclusion:** 通过引入交互式反馈机制和轻量级FALM模块，成功解决了传统EM-NLQ方法的局限性，为构建更实用的人机交互记忆系统提供了有效解决方案。
...
> 本文论证地球观测工作流的结构性挑战需要重新设计基于地理空间原生的智能体，而非简单扩展通用AI代理。

<details>
  <summary>Details</summary>

**Motivation:** 地球观测正向多步骤分析工作流发展，但传统的通用AI代理无法处理地理空间数据特有的多模态、时空结构性和状态转换特性，导致错误在工作流中静默传播。

**Method:** 分析通用代理在地理空间工作流中的失效模式，提出以结构化地理空间状态、工具感知推理、验证器引导执行为核心的地理空间原生代理设计原则。

**Result:** 识别并归类了多步骤地球观测管线的失效模式，建立了面向地理空间一致性和物理有效性的代理设计框架。

**Conclusion:** 构建可靠的地理空间智能体需要围绕地球观测分析的物理、地理空间和工作流约束来重新思考代理设计，并提出了相应的研究方向和评估标准。
...
> 论文创建了一个名为LIVE-YT VC的大规模视频裁剪数据库，用于解决移动设备上视频宽高比转换的挑战，并验证了其在视频裁剪算法中的实用性。

<details>
  <summary>Details</summary>

**Motivation:** 随着移动设备视频消费的快速增长，不同分辨率和显示方向的视频需要调整宽高比。传统的静态裁剪和填充方法会损害视觉质量，而变形可能扭曲视频的原始含义，因此需要更有效的视频裁剪解决方案。

**Method:** 构建了一个包含1800个视频、由90名人工标注的LIVE-YT VC数据库，并开发了LIVE-YT VC++版本，通过帧内时间滤波器平滑主观标注。同时，利用SmartVidCrop算法和最先进的视频定位模型验证数据库的实用性。

**Result:** 创建了目前最大的公开主观视频肖像区域裁剪数据库，展示了其在视频宽高比转换任务中的基准作用，并通过实验证明了标注与视频显著性预测的相似性。

**Conclusion:** 该数据库为推进视频宽高比变换模型提供了重要资源，有助于确保重塑后的移动友好视频内容保持质量和含义。项目计划开源以服务研究社区。
...
> 论文提出Semi-DPO方法，通过区分一致和冲突的偏好对来处理多维度人类视觉偏好的标签噪声问题，在无需额外标注的情况下实现了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有数据集仅提供单一的、整体性的标注，导致在美学、细节保真度和语义对齐等多维度上表现不同的图像被简单地标记为胜者或败者，这种标签压缩会产生冲突的梯度信号，误导扩散模型的直接偏好优化(DPO)。

**Method:** Semi-DPO采用半监督学习方法，将一致性偏好对视为干净标签数据，冲突对视为噪声未标记数据。首先在共识过滤的干净子集上训练模型，然后将该模型作为隐式分类器为噪声集生成伪标签进行迭代优化。

**Result:** 实验结果表明Semi-DPO达到了最先进的性能，显著改善了与复杂人类偏好的一致性，且在训练过程中不需要额外的人工标注或显式奖励模型。

**Conclusion:** 通过有效处理多维度偏好压缩导致的标签噪声问题，Semi-DPO为扩散模型的偏好优化提供了一个高效且实用的解决方案，推动了人类视觉偏好对齐技术的发展。
...
### [67] [ViPO: Visual Preference Optimization at Scale](https://arxiv.org/abs/2604.24953)
*Ming Li,Jie Wu,Justin Cui,Xiaojie Li,Rui Wang,Chen Chen*

Main category: cs.CV

> 本文提出了Poly-DPO算法和ViPO大规模视觉偏好数据集，解决了视觉生成模型偏好优化中的噪声适应性和数据质量问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有开源视觉偏好数据集存在冲突模式、低分辨率、提示多样性不足等缺陷，限制了偏好优化范式的有效扩展。

**Method:** 1) 提出Poly-DPO：在DPO目标函数中加入多项式项，动态调整模型置信度以增强噪声鲁棒性；2) 构建ViPO数据集：包含1百万个1024px图像对和30万个720p+视频对，覆盖5类图像和3类视频。

**Result:** 在Pick-a-Pic V2噪声数据集上，Poly-DPO相比Diffusion-DPO在GenEval上分别取得6.87和2.32的性能提升（SD1.5和SDXL）；使用ViPO数据集训练的模型性能远超现有数据集。

**Conclusion:** 视觉偏好优化的规模化需要同时解决算法 adaptiveness 和数据质量问题；高质量数据使Poly-DPO收敛为标准DPO，证明了算法的自适应性和数据集的品质。
...
### [68] [A New Kind of Network? Review and Reference Implementation of Neural Cellular Automata](https://arxiv.org/abs/2604.24990)
*Martin Spitznagel,Janis Keuper*

Main category: cs.CV

> 本文综述了神经细胞自动机(NCA)的研究进展，提出了统一的模块化框架和表示法，并开发了开源库NCAtorch作为参考实现。

<details>
  <summary>Details</summary>

**Motivation:** Stephen Wolfram在2003年提出细胞自动机可作为复杂系统建模的新方法，但传统CA需要人工设计更新规则。近年来将CA与神经网络结合的NCA能够从数据中学习复杂规则，为建模复杂自组织生成系统提供了新可能。

**Method:** 对现有NCA工作进行系统综述，建立统一的模块化框架和标准化表示法，并开发了名为NCAtorch的开源参考实现库。

**Result:** 提供了NCA领域的综合性综述，建立了标准化的理论框架，开发并发布了NCAtorch开源库，为研究人员提供统一的工具和参考。

**Conclusion:** NCA代表了一种将传统细胞自动机与现代深度学习相结合的有前途方法，本工作通过提供统一框架和开源实现，有助于推动该领域的进一步发展和应用。
...
### [69] [DouC: Dual-Branch CLIP for Training-Free Open-Vocabulary Segmentation](https://arxiv.org/abs/2604.24997)
*Mohamad Zamini,Diksha Shukla*

Main category: cs.CV

> DouC是一个免训练的双分支CLIP框架，通过OG-CLIP和FADE-CLIP两个互补分支分别解决局部标记不可靠和空间一致性不足的问题，在开放词汇语义分割任务中取得了优异性能。

<details>
  <summary>Details</summary>

**Motivation:** 基于CLIP的免训练方法虽然保持了强大的零泛化能力，但仅依赖单一推理机制，难以同时解决局部标记不可靠和空间一致性不足的问题，限制了其在开放词汇语义分割中的性能。

**Method:** 提出DouC双分支框架：OG-CLIP通过轻量级推理时标记门控提高patch级可靠性；FADE-CLIP通过冻结视觉基础模型引导的代理注意力注入外部结构先验；两个分支在logit层融合，可选实例感知校正作为后处理。整个框架无需可学习参数，无需重新训练。

**Result:** 在8个基准测试和多个CLIP主干网络上，DouC一致性地优于先前的免训练方法，且随着模型容量扩展保持良好的扩展性能。

**Conclusion:** DouC成功将密集预测分解为两个互补组件，在不牺牲CLIP零样本泛化能力的前提下，通过联合优化局部标记可靠性和结构感知patch交互，有效提升了开放词汇语义分割性能。
...
### [70] [BifDet: A 3D Bifurcation Detection Dataset for Airway-Tree Modeling](https://arxiv.org/abs/2604.24999)
*Ali Keshavarzi,Quentin Bouniot,Benjamin M. Smith,Elsa Angelini*

Main category: cs.CV

> BifDet 是首个用于3D气道分叉检测的公开数据集，包含精心标注的CT扫描数据，并通过RetinaNet和DETR模型验证了其有效性。

<details>
  <summary>Details</summary>

**Motivation:** 气道分叉在理解肺部生理、疾病机制和病灶定位中至关重要，但缺乏专门的数据集阻碍了自动化检测工具的发展。

**Method:** 构建了BifDet数据集，标注了ATM22队列中的CT扫描数据中的气道分叉边界框；使用RetinaNet和DETR进行3D检测任务的微调和评估，并提供了详细的预处理流程和实现细节。

**Result:** 成功创建了首个公开的3D气道分叉检测数据集，RetinaNet和DETR模型在该数据集上表现出良好的检测性能，为未来研究提供了基准。

**Conclusion:** BifDet填补了气道分叉检测数据集的空白，为开发先进的自动化气道分析工具奠定基础，将促进呼吸疾病研究和临床应用的发展。
...
### [71] [ShapeY: A Principled Framework for Measuring Shape Recognition Capacity via Nearest-Neighbor Matching](https://arxiv.org/abs/2604.25065)
*Jong Woo Nam,Amanda S. Rios,Bartlett W. Mel*

Main category: cs.CV

> ShapeY是一个专门评估物体识别系统形状识别能力的新基准框架，通过68,200个灰度图像测试多种3D视角下的形状泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 人类物体识别主要依赖形状线索，但深度网络常依赖纹理等非形状 cues，导致泛化和鲁棒性问题，需要专门的形状识别评估工具。

**Method:** 构建包含200个3D物体多视角渲染的68,200个灰度图像数据集，使用最近邻匹配任务评估嵌入空间结构，提供多种定量和定性性能指标。

**Result:** 测试321个预训练网络显示，即使最先进模型在3D视角和外观变化下的形状泛化能力仍存在严重问题，会出现明显不同形状的错误匹配。

**Conclusion:** ShapeY为推进人工视觉系统向人类级形状识别发展建立了原则性框架，强调了分离且不变的对象编码的重要性。
...
### [72] [Beyond Accuracy: Benchmarking Cross-Task Consistency in Unified Multimodal Models](https://arxiv.org/abs/2604.25072)
...
> 该论文提出了XTC-Bench评估框架，通过场景图衍生任务来度量统一多模态模型的跨任务视觉语义一致性，发现强大的单任务表现不意味着跨任务对齐，架构统一本身不足以保证一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有统一多模态模型(uMMs)评估协议独立测试视觉理解和生成能力，缺乏对两者语义对齐的检验，无法判断模型是否学习到了在任务间保持一致的统一表示。

**Method:** 提出基于场景图的XTC-Bench框架，从结构化场景图生成提示和查询，实现对象、属性和关系的事实级对齐分析；设计CCTA指标量化生成与理解在匹配原子事实上的语义一致性。

**Result:** 对8个开源和1个商业统一模型的实验表明，高生成或理解性能不保证强跨任务对齐，架构分析发现一致性由跨模态学习目标的耦合程度决定，而非架构统一本身。

**Conclusion:** XTC-Bench提供了可重现、模型无关的框架来诊断表示级不对齐，为推进超越孤立任务性能的统一多模态建模提供了明确方向。
...
### [73] [One Perturbation, Two Failure Modes: Probing VLM Safety via Embedding-Guided Typographic Perturbations](https://arxiv.org/abs/2604.25102)
*Ravikumar Balakrishnan,Sanket Mendapara*

Main category: cs.CV

> 本研究发现多模态嵌入距离可以预测Typography提示注入攻击的成功率，并利用这一特性开发了无需目标模型访问权限的对抗性攻击方法。

<details>
  <summary>Details</summary>

**Motivation:** 随着视觉语言模型（VLMs）日益广泛地应用于自主代理系统，利用图像中渲染文字的Typography提示注入攻击构成日益严重的威胁。现有研究主要关注最大化攻击成功率，但未能解释为何某些特定渲染方式能够绕过安全对齐。

**Method:** 首先对四个VLMs（包括GPT-4o和Claude）进行实证研究，发现多模态嵌入距离与攻击成功率强相关（r=-0.71到-0.93）。然后基于此发现，开发CWA-SSA方法在有界ℓ∞扰动下直接最大化图像文本嵌入相似度，作为红队测试工具。

**Result:** 实验在五个降级设置下验证了该方法在GPT-4o、Claude Sonnet 4.5、Mistral-Large-3和Qwen3-VL上的有效性。优化能够同时恢复可读性和减少安全对齐拒绝，主导机制取决于模型安全过滤器强度和视觉降级程度。

**Conclusion:** 多模态嵌入距离是预测Typography攻击效果的有效代理指标，基于此原理的攻击方法能够在不访问目标模型的情况下有效绕过安全防护，为VLM安全研究提供了重要工具和见解。
...
### [74] [M$^3$-VQA: A Benchmark for Multimodal, Multi-Entity, Multi-Hop Visual Question Answering](https://arxiv.org/abs/2604.25122)
*Jiatong Ma,Longteng Guo,Yuchen Liu,Zijia Zhao,Dongze Hao,Xuanxu Lin,Jing Liu*

Main category: cs.CV

> M³-VQA是一个新的基于知识的视觉问答评测基准，专注于评估多模态大语言模型在细粒度实体理解和复杂多跳推理方面的能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的VQA数据集主要集中在粗粒度类别和单一实体的简单推理上，无法充分评估MLLMs的复杂多模态理解和推理能力。

**Method:** 构建了一个包含多样化多实体问题的数据集，要求模型对来自视觉和文本源的多个不同实体进行顺序和并行的多跳推理，并提供了可追踪的详细证据和精心策划的多模态知识库。

**Result:** 评估了16个主流MLLMs，结果显示模型在没有外部信息时表现很差，但在提供精确证据时显著提升；推理感知的检索方法优于启发式方法。

**Conclusion:** M³-VQA为推进MLLMs的多模态推理能力提供了更具挑战性的评测基准，突显了结构化推理对复杂多模态理解的重要性。
...
### [75] [ResetEdit: Precise Text-guided Editing of Generated Image via Resettable Starting Latent](https://arxiv.org/abs/2604.25128)
*Hanyi Wang,Han Fang,Zheng Wang,Shilin Wang,Ee-Chien Chang*

Main category: cs.CV

> ResetEdit是一种主动式扩散编辑框架，通过在生成过程中嵌入可恢复的隐向量信息，解决了扩散模型局部编辑时的编辑保真度和结构一致性问题。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型图像生成需要高质量的后生成编辑，现有基于反演的方法得到的起始隐向量质量不佳，导致编辑效果差；而使用原始生成隐向量虽好但存储成本过高。

**Method:** ResetEdit通过将隐向量差异注入扩散轨迹并提取重建可重置隐向量，结合轻量级隐向量优化模块补偿VAE不对称性，构建了主动式编辑框架。

**Result:** 基于Stable Diffusion的ResetEdit在可控性和视觉保真度方面持续超越现有最先进基线方法，并与现有免调参编辑方法无缝集成。

**Conclusion:** ResetEdit成功解决了扩散模型局部编辑中的起始隐向量质量问题，实现了高精度、高保真的局部编辑，为实际应用提供了可行的解决方案。
...
### [76] [IAM: Identity-Aware Human Motion and Shape Joint Generation](https://arxiv.org/abs/2604.25164)
*Wenqi Jia,Zekun Li,Abhay Mittal,Chengcheng Tang,Chuan Guo,Lezi Wang,James Matthew Rehg,Lingling Tao,Size An*

Main category: cs.CV

> 本文提出了一种身份感知的人体运动生成框架，通过多模态信号表示身体形态特征，并采用联合运动-形状生成范式，解决了现有方法忽视身体形态对运动动力学影响的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本驱动人体运动生成方法假设身份中立运动，使用规范身体表示，忽略了身体形态（如身体比例、质量分布、年龄等）对运动动力学的显著影响，导致生成的运动往往物理不一致。

**Method:** 提出了身份感知运动生成框架，使用自然语言描述和视觉线索等多模态信号表示身份，引入联合运动-形状生成范式，同时合成运动序列和身体形状参数，让身份线索直接调制运动动力学。

**Result:** 在动作捕捉数据集和大规模真实视频上进行的广泛实验表明，该方法在保持高运动质量的同时，显著提升了运动真实性和运动-身份一致性。

**Conclusion:** 该研究成功地将身体形态特征融入文本驱动的人体运动生成中，为生成物理一致且具有个体特征的运动提供了新的解决方案。
...
### [77] [Benchmarking OCR Pipelines with Adaptive Enhancement for Multi-Domain Retail Bill Digitization](https://arxiv.org/abs/2604.25176)
*Vijaysinh Gaikwad*

Main category: cs.CV

> 该论文提出了一种智能质量感知的自适应OCR管道，用于多领域零售票据数字化，在360张真实票据图像上测试，字符错误率降至18.4%，比基线提升26.4%。

<details>
  <summary>Details</summary>

**Motivation:** 多领域零售票据数字化面临扫描质量变化、布局异构性和商业领域多样性三大挑战，需要更智能的OCR解决方案。

**Method:** 整合了四项核心技术：CNN自监督去噪图像增强模块、拉普拉斯方差图像质量分析与三层路由、置信度驱动自适应反馈循环、NLP后校正层，使用OCR集成多数投票生成真值。

**Result:** 在360张零售票据上实现CER 18.4%、WER 27.6%，处理速度3.64秒/图像(比EasyOCR快6.4倍)，图像增强PSNR达28.7dB，噪声率仅2.3%。

**Conclusion:** 建立了一个可重现的多领域零售票据OCR基准，验证了质量感知自适应管道的有效性，为后续研究奠定了基础。
...
### [78] [Lightweight Real-Time Rendering Parameter Optimization via XGBoost-Driven Lookup Tables](https://arxiv.org/abs/2604.25178)
*Baijun Tan,Francesco Moretti*

Main category: cs.CV

> LUT-Opt是一个轻量级的自适应实时渲染参数优化框架，通过将机器学习模型蒸馏为查找表(LUT)，实现毫秒级参数调整，在移动设备上显著提升渲染性能。

<details>
  <summary>Details</summary>

**Motivation:** 在现代游戏和渲染引擎中，特别是在资源受限的移动设备上，如何在渲染质量和实时性能之间达到理想平衡是一个长期挑战。现有方法要么需要耗时的预计算，要么神经推理开销过大，要么缺乏跨硬件和场景的通用性。

**Method:** 提出两阶段优化流程：离线训练XGBoost回归器预测渲染时间和图像质量，通过系统离散化和两阶段线性搜索将模型蒸馏为紧凑的查找表；运行时每帧查询LUT进行参数选择，实现亚毫秒级推理。

**Result:** 在Unreal Engine 5中验证SSS和AO技术，实验表明LUT-Opt分别减少SSS渲染时间约40%和AO渲染时间约70%，图像质量误差仅增加2%，每帧推理延迟低于0.1毫秒。

**Conclusion:** LUT-Opt通过查找表技术实现了高效、通用的实时渲染参数自适应优化，为移动设备的实时渲染应用提供了可行的解决方案。
...
### [79] [FCMBench-Video: Benchmarking Document Video Intelligence](https://arxiv.org/abs/2604.25186)
*Runze Cui,Fangxin Shang,Yehui Yang,Qing Yang,Tao Chen*

Main category: cs.CV

> FCMBench-Video是一个用于文档视频智能评估的新基准，包含1,200个长视频和11,322个问答实例，可评估文档感知、时序定位和证据推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 文档视频相较静态图像具有时序冗余和顺序展开的证据流，需要跨帧证据整合并保留与真实性和防欺诈相关的采集过程线索，对金融信贷审核等场景至关重要。

**Method:** 采用原子采集和组合工作流，记录可重用的单文档片段，应用可控降质，组装具有规定时间跨度的长形式多文档视频，构建大规模且符合隐私要求的数据集。

**Result:** 在9个最新的Video-MLLMs上的评估显示，该基准能有效区分不同系统和能力，计数任务对时长最敏感，跨文档验证和证据选择检测高级证据整合，视觉提示注入提供鲁棒性维度。

**Conclusion:** FCMBench-Video为跟踪Video-MLLM在文档视频理解方面的进展和探测真实性敏感信贷应用中的能力边界提供了可重现的基准测试。
...
### [80] [Image Classification via Random Dilated Convolution with Multi-Branch Feature Extraction and Context Excitation](https://arxiv.org/abs/2604.25188)
*Wentao Jiang,Yuanchan Xu,Heng Yuan*

Main category: cs.CV

> RDCNet是一种基于ResNet-34的新型图像分类网络，通过随机扩张卷积、细粒度特征增强和上下文激励三个创新模块，在五个基准数据集上实现了最先进的分类精度。

<details>
  <summary>Details</summary>

**Motivation:** 传统CNN在需要同时进行细粒度特征提取和背景噪声抑制的图像分类任务中存在局限性：难以捕获多尺度上下文信息，对噪声或无关图像区域容易过拟合。

**Method:** 提出RDCNet架构，包含三个协同创新：(1)多分支随机扩张卷积(MRDC)模块，使用不同扩张率的并行分支结合随机掩码机制；(2)嵌入MRDC的细粒度特征增强(FGFE)模块，通过自适应池化和双线性插值连接全局和局部特征；(3)上下文激励(CE)模块，利用softmax空间注意力和通道重新校准。

**Result:** 在CIFAR-10、CIFAR-100、SVHN、Imagenette和Imagewoof五个基准数据集上，RDCNet分别比第二名竞争方法高出0.02%、1.12%、0.18%、4.73%和3.56%的分类精度。

**Conclusion:** 实验结果验证了RDCNet方法的有效性和泛化性，证明了三个创新模块在多尺度特征捕获、噪声鲁棒性和背景干扰抑制方面的协同作用，适用于多样化的视觉识别场景。
...
### [81] [Towards Seamless Lunar Mosaics: Deep Radiometric Normalization for Cross-Sensor Orbital Imagery Using Chandrayaan-2 TMC Data](https://arxiv.org/abs/2604.25208)
*Pratincha Singh,Jai Gopal Singla,Prashant Hemrajani,Nitant Dube,Amithabh,Hinal Patel*

Main category: cs.CV

> 本文提出了一种基于cGAN的深度学习框架，用于多任务月球轨道影像辐射度归一化，解决了照明几何、传感器特性和获取条件不一致导致的拼接问题，显著提升了月球镶嵌图的色调一致性和结构连贯性。

<details>
  <summary>Details</summary>

**Motivation:** 多任务轨道影像在创建无缝月球镶嵌图时面临辐射度不一致的挑战，主要由于照明几何变化、传感器特性差异和获取条件的多样性，导致色调不均匀和接缝伪影。

**Method:** 采用条件生成对抗网络(cGAN)，包含U-Net生成器和PatchGAN判别器，学习从传统拼接影像到LROC WAC光度一致性参考的非线性辐射度映射；使用基于块的重叠感知训练策略处理大面积镶嵌图。

**Result:** 定量评估(SSIM、PSNR、RMSE)显示该方法比传统直方图归一化技术有显著改进，实现了增强的色调均匀性、减少的接缝伪影和改善的结构连贯性。

**Conclusion:** 学习型辐射度归一化在大规模行星镶嵌中十分有效，展示了从异构轨道影像生成高保真月球表面地图的潜力，为处理多源行星数据提供了新的解决方案。
...
> 研究显示人类和现有技术都难以检测GPT-Image-2制作的文档图像伪造，并提出新数据集和评估方法来揭示这一检测盲区。

<details>
  <summary>Details</summary>

**Motivation:** GPT-Image-2能够在几秒钟内以几分钱的成本完美伪造文档图像（如收据上的数字），这严重威胁了文档的真实性验证，因此需要研究检测这类AI伪造的有效方法。

**Method:** 创建AIForge-Doc v2数据集，包含3066个GPT-Image-2文档伪造样本及精确掩码；评估四种检测方法：人工检测、通用取证工具TruFor、专用文档检测器DocTamper以及GPT-Image-2的自我判断能力。

**Result:** 人工检测准确率仅0.501，与随机猜测无异；三种计算检测器的准确率也只是略高于随机（TruFor 0.599，DocTamper 0.585，自我判断0.532），即使经过不同策略优化，自我判断的AUC从未超过0.59。

**Conclusion:** 现有检测方法在识别GPT-Image-2生成的文档伪造方面存在显著盲区，传统篡改检测器在各自训练域表现良好，但对AI修复检测性能大幅下降，表明这是一个针对GPT-Image-2修复的特定检测缺陷。
...
### [83] [DRAGON: A Benchmark for Evidence-Grounded Visual Reasoning over Diagrams](https://arxiv.org/abs/2604.25231)
...
> DRAGON是一个评估图表证据定位能力的基准测试，要求模型不仅回答问题，还要标注支持答案的视觉证据区域。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言模型在图表问答任务中虽然准确率高，但可能依赖文本相关性或数据集偏差而非真正的视觉推理，无法评估模型是否真正基于图表证据进行推理，影响可靠性和可解释性。

**Method:** 构建了一个包含11,664个问题实例的DRAGON数据集，整合6个图表QA数据集（ChartQA、Circuit-VQA等），发布2,445个人工验证的证据标注测试集，模型需要预测支持答案的边界框区域。

**Result:** 评估了8个最新的VLMs，分析了它们在不同图表领域定位推理证据的能力，为图表推理能力的系统评估提供了标准化框架。

**Conclusion:** DRAGON基准测试推动了基于视觉证据的图表推理研究，为开发更具可解释性和可靠性的图表理解模型提供了重要工具。
...
### [84] [Personalized Cross-Modal Emotional Correlation Learning for Speech-Preserving Facial Expression Manipulation](https://arxiv.org/abs/2604.25255)
*Tianshui Chen,Yujie Zhu,Jianman Lin,Zhijing Yang,Chunmei Qing,Feng Gao,Liang Lin*

Main category: cs.CV

> 论文提出PCMECL算法，通过个性化提示学习和跨模态特征差分，解决语音保持的面部表情操控中配对数据稀缺问题，提升模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 语音保持的面部表情操控任务面临配对数据稀缺的挑战，即缺乏同一人相同语音但不同表情的对齐数据，这阻碍了情感操控的直接监督。现有视觉-语言模型虽然能提取对齐特征，但直接应用存在局限性。

**Method:** 提出个性化跨模态情感关联学习算法(PCMECL)，包含两个关键改进：1）基于个体视觉信息条件化学习个性化提示，建立更细粒度的视觉-语义关联；2）通过特征差分关联模态，匹配视觉特征变化与语义特征变化，填补模态鸿沟。

**Result:** PCMECL作为即插即用模块可无缝集成到现有SPFEM模型中，在多个数据集上的广泛实验证明了算法的卓越效能。

**Conclusion:** 该研究通过个性化提示学习和特征差分方法，有效利用视觉-语言模型的监督信息，显著改善了语音保持的面部表情操控效果，为解决配对数据稀缺问题提供了新思路。
...
### [85] [Combating Visual Neglect and Semantic Drift in Large Multimodal Models for Enhanced Cross-Modal Retrieval](https://arxiv.org/abs/2604.25273)
*Guosheng Zhang,Linkai Liu,Keyao Wang,Haixiao Yue,Zhiwen Tan,Xiao Tan*

Main category: cs.CV

> 该论文提出了一个名为SSA-ME的新颖多模态嵌入框架，通过显著主体感知建模来提升统一多模态检索性能，解决了现有方法忽视主体级语义的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的统一多模态检索方法主要关注样本级别的对比学习目标，忽视了关键的主体级语义，导致模型在复杂多模态查询中无法准确分组合语义相关的主体，表现为语义对齐偏差，并出现过度依赖文本线索而忽视视觉模态的问题。

**Method:** SSA-ME框架利用大型多模态模型和视觉专家来识别并强调图像-文本对中的显著视觉概念，引入显著性引导目标以更好地对齐跨模态注意力与语义相关区域，并通过特征再生模块基于显著性图重新校准视觉特征，确保跨模态的平衡和语义一致性整合。

**Result:** 在MMEB基准测试上，该方法达到了最先进的性能，证明引入主体级建模显著改善了多模态检索效果；全面的分析进一步展示了该方法的可解释性和有效性。

**Conclusion:** 通过显著主体感知建模，SSA-ME框架有效解决了现有统一多模态检索方法中忽视主体级语义的局限性，实现了更好的跨模态对齐和视觉模态利用，为多模态检索领域提供了有价值的改进方向。
...
### [86] [OmniVTG: A Large-Scale Dataset and Training Paradigm for Open-World Video Temporal Grounding](https://arxiv.org/abs/2604.25276)
*Minghang Zheng,Zihao Yin,Yi Yang,Yuxin Peng,Yang Liu*

Main category: cs.CV

> 本研究提出了OmniVTG大规模开放世界视频时序定位数据集和自我校正思维链训练范式，通过语义覆盖迭代扩展和三阶段训练，显著提升了多模态大模型在开放世界视频时序定位任务中的表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频时序定位(VTG)任务因数据集规模有限和语义多样性不足，导致在开放世界环境下常见概念与罕见概念之间存在性能差距。这限制了模型在真实复杂场景中的应用能力。

**Method:** 1. 通过语义覆盖迭代扩展管道构建OmniVTG数据集：识别现有数据集词汇缺口，收集高概率包含目标概念的视频；2. 利用MLLM密集描述能力设计以描述为中心的数据引擎生成高质量标注；3. 提出自我校正思维链训练范式：通过监督微调、思维链微调和强化学习三阶段流程，让模型先预测后反思修正。

**Result:** 实验表明，该方法在OmniVTG数据集上表现优异，并在四个现有VTG基准测试中实现了最先进的零样本性能，有效缩小了罕见概念与常见概念之间的性能差距。

**Conclusion:** 通过构建大规模开放世界VTG数据集和创新的自校正思维链训练范式，本研究显著提升了多模态大模型在视频时序定位任务中的开放世界适应能力，为相关领域提供了新的解决方案和数据资源。
...
### [87] [The Thinking Pixel: Recursive Sparse Reasoning in Multimodal Diffusion Latents](https://arxiv.org/abs/2604.25299)
*Yuwei Sun,Yuxuan Yao,Hui Li,Siyu Zhu*

Main category: cs.CV

> 本文提出一种递归稀疏专家混合框架，通过引入递归组件和多步精炼机制，显著提升了扩散模型在文本到图像生成任务中的推理能力和生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型在高保真数据合成方面表现优异，但在文本引导等复杂结构化推理任务中能力有限。虽然语言模型通过潜在推理和递归策略提升了文本理解能力，但将其扩展到多模态文本到图像生成任务具有挑战性，主要是由于视觉标记的连续和非离散特性。

**Method:** 受模块化人类认知启发，研究者提出一种递归稀疏专家混合框架。该方法在联合注意力层中引入递归组件，通过多个潜在步骤迭代精炼视觉标记；同时利用神经模块的稀疏选择高效共享参数。每个步骤中，设计门控网络根据当前视觉标记、扩散时间步和条件信息动态选择专门的神经模块。

**Result:** 在类别条件ImageNet图像生成任务上的综合评估，以及在GenEval和DPG基准测试上的额外研究表明，所提方法在增强模型图像生成性能方面具有优越性。

**Conclusion:** 该递归稀疏专家混合框架有效地将先进的语言模型推理策略扩展到多模态生成任务，为扩散模型的结构化推理能力提供了新的解决方案，显著改善了文本到图像的生成质量。
...
### [88] [DenseScout: Algorithm-System Co-design for Budgeted Tiny Object Selection on Edge Platforms](https://arxiv.org/abs/2604.25300)
*Xiong Zhouzhi,Zimo Zeng,Yi Chen,Shuqi Xu,Yunfeng Yan,Donglian Qi*

Main category: cs.CV

> DenseScout是一个轻量级密集响应选择器，通过算法-系统协同设计，在边缘平台上实现了高效的微小目标感知，解决了低预算下的候选区域优先排序问题。

<details>
  <summary>Details</summary>

**Motivation:** 在边缘平台上部署微小目标感知面临严格计算预算和端到端延迟约束的双重挑战。现有的检测器前端在低预算区域优先排序和部署实用性能方面表现不佳，需要从算法和系统的联合角度重新设计解决方案。

**Method:** 提出DenseScout轻量级选择器（仅1.01M参数），使用轻量级代理输入直接对候选区域位置进行排序；开发传输感知运行时机制，在异构边缘设备上实现；采用QoS约束召回作为评估指标。

**Result:** 实验表明DenseScout在离线预算区域选择评估中持续优于检测器基线，特别是在低预算条件下；跨平台测试结果显示部署性能取决于选择器质量和运行时效率的联合优化。

**Conclusion:** 边缘微小目标感知应作为算法-系统协同设计问题来优化，而非孤立的模型选择，这为实际部署提供了更有效的设计范式。
...
### [89] [Rapid tracking through strongly scattering media with physics-informed neuromorphic speckle analysis](https://arxiv.org/abs/2604.25310)
*Yuqing Cao,Shuo Zhu,Rongzhou Chen,Jingyan Chen,Ni Chen,Edmund Y. Lam*

Main category: cs.CV

> 本文提出计算神经形态追踪(CNT)框架，结合异步事件传感和任务驱动的散斑分析，在低光强散射介质中实现快速运动的鲁棒追踪，比传统系统快10倍、暗10倍。

<details>
  <summary>Details</summary>

**Motivation:** 解决在低光环境下通过强散射介质追踪快速运动物体的关键问题。传统基于帧的相机使用固定曝光时间，存在信噪比与时间分辨率的权衡问题。

**Method:** 引入计算神经形态追踪(CNT)框架，将神经形态散斑聚合成时空散斑表示，联合优化时间和空间参数以在极端条件下最大化追踪稳定性，结合异步事件传感与任务驱动的散斑分析。

**Result:** 实验证明该方法能够实现比传统系统快10倍的运动和在暗10倍的光照条件下的鲁棒运动追踪，显著扩展了通过散射介质追踪的操作范围。

**Conclusion:** CNT为涉及快速运动和低光照条件的苛刻场景提供了高效且可扩展的解决方案，在散射介质追踪方面具有重大突破。
...
### [90] [Golden RPG: Confidence-Adaptive Region-Aware Noise for Compositional Text-to-Image Generation](https://arxiv.org/abs/2604.25314)
*Hao Li*

Main category: cs.CV

> Golden RPG是一种区域感知的噪声预测器，通过FiLM适配器和区域交叉注意力层改进扩散模型的多区域文本到图像生成，实现了更好的跨区域一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的扩散模型噪声预测是全局的，当提示词描述具有空间分离实体的场景时，单一文本嵌入成为瓶颈，无法有效处理多区域组合生成。

**Method:** 扩展冻结的NPNet：(1) 添加基于FiLM的适配器根据每个子提示重塑预测噪声；(2) 在Swin骨干网络两阶段间注入区域交叉注意力层；(3) 提出置信自适应混合头动态调节区域信号强度。

**Result:** 在RPG基准测试和T2I-CompBench的四个多区域类别上均达到最高跨区域一致性分数，同时保持了与最强基线相当的CLIP-Score和CLIP-IQA性能，用户研究显示67%的偏好度。

**Conclusion:** Golden RPG有效解决了组合文本到图像生成中的多区域问题，仅增加约2M可训练参数和0.6秒推理开销，显著提升了生成质量。
...
### [91] [SaliencyDecor: Enhancing Neural Network Interpretability through Feature Decorrelation](https://arxiv.org/abs/2604.25315)
*Ali Karkehabadi,Jamshid Hassanpour,Houman Homayoun,Avesta Sasan*

Main category: cs.CV

> SaliencyDecor是一种训练框架，通过强制特征不相关来改善基于梯度的显著性方法的解释质量，同时提升模型预测性能，无需修改模型架构或增加推理开销。

<details>
  <summary>Details</summary>

**Motivation:** 基于梯度的显著性方法常产生噪声和不稳定的解释，与语义特征对齐性差。根本原因是学习表示的几何特性：相关特征维度会分散梯度归因，导致模糊的显著性图。

**Method:** 提出SaliencyDecor训练框架，包含三个优化目标：分类准确性、特征掩码下的预测一致性以及不相关正则化器。通过将特征空间重塑为正交状态，促进更集中的梯度流动。

**Result:** 在多个基准数据集和架构上的实验表明，该方法产生更清晰、更关注对象的显著性图，同时在所有数据集上都实现了准确性提升，打破了传统上解释质量与模型性能之间的权衡。

**Conclusion:** SaliencyDecor为同时提高可解释性和准确性提供了原理性机制，无需架构改变或推理开销，是一种有效的特征空间重塑方案。
...
### [92] [Towards Robust Deep Learning-based Rumex Obtusifolius Detection from Drone Images](https://arxiv.org/abs/2604.25316)
*Fabian Dionys Schrag,Mehmet Ozgur Turkoglu,Konrad Schindler,Ralph Lukas Stoop*

Main category: cs.CV

> 该研究比较了CNN和Vision Transformer在杂草图像分类领域自适应任务中的表现，发现自监督预训练的ViT模型能很好地处理域偏移，性能优于传统域自适应方法，并开源了UAV数据集AGSMultiRumex。

<details>
  <summary>Details</summary>

**Motivation:** 解决机器学习模型从地面车辆数据域迁移到无人机数据域时面临的分布差异挑战，提升Rumex obtusifolius杂草在真实应用场景中的分类性能。

**Method:** 训练ResNet和ViT模型在源域数据上，评估其在自建UAV目标数据集上的表现；应用moment-matching和最大分类器差异两种域自适应技术；对比自监督预训练的DINOv2/DINOv3 ViT模型。

**Result:** CNN模型在目标域泛化能力差，域自适应技术可改善性能；ViT模型天然处理域偏移效果更好，F1分数达到0.8；开源了包含15次无人机飞行数据的AGSMultiRumex数据集。

**Conclusion:** 自监督预训练的Vision Transformer提供了域自适应的有效解决方案，无需复杂的域适应技术即可在跨域杂草检测任务中取得优异性能，为草地系统中的杂草检测研究提供了新思路和数据资源。
...
### [93] [Edge-Cloud Collaborative Reconstruction via Structure-Aware Latent Diffusion for Downstream Remote Sensing Perception](https://arxiv.org/abs/2604.25319)
*Yun Li,Xianju Li*

Main category: cs.CV

> 这篇文章提出了一种结构感知潜在扩散框架（SALD），通过边缘-云协作架构解决高分辨率遥感数据传输中压缩导致的结构细节丢失问题。

<details>
  <summary>Details</summary>

**Motivation:** 高分辨率遥感数据的激增面临严重的卫星到地面传输瓶颈，有限的下行链路带宽迫使采用极高比例压缩，这会不可逆地破坏下游机器感知任务（如目标检测）所需的高频结构细节。

**Method:** SALD框架采用非对称边缘-云协作系统：边缘端将图像分解为高度压缩的低频载荷和轻量级软结构先验；云端引入结构门控大核模块（SGLK）和语义引导引擎（SGE），利用传输的结构先验来门控大核卷积，有效捕获航拍场景的长程依赖性并主动抑制生成幻觉。

**Result:** 在MSCM和UCMerced数据集上的广泛实验表明，即使在极端带宽约束下，SALD实现了更好的感知质量（LPIPS），并显著提升了场景分类和小目标检测的下游性能。

**Conclusion:** SALD框架通过结构感知的边缘-云协作设计，在保持高压缩率的同时恢复了关键的结构细节，为遥感数据的实时传输和智能分析提供了有效的解决方案。
...
### [94] [Assessment of the quantitative impact of occlusal positioning splints on temporomandibular joint conditions](https://arxiv.org/abs/2604.25322)
*Agnieszka Anna Tomaka,Krzysztof Domino,Dariusz Pojda,Michał Tarnawski*

Main category: cs.CV

> 提出一种通过咬合定位垫片定量分析颞下颌关节(TMJ)构型的计算方法，利用刚性变换和多模态数据实现间接评估关节空间变化。

<details>
  <summary>Details</summary>

**Motivation:** 传统TMJ构型评估需要在多个下颌位置重复成像，成本高且复杂。本研究旨在开发一种无需重复成像的替代方法，通过定位垫片的物理实现和变换分析来量化TMJ构型变化。

**Method:** 将定位垫片建模为预定下颌刚性变换的物理实现，整合CBCT、面部动作采集和牙科扫描数据。设计制作垫片并通过石膏模型扫描评估定位精度，在刚性运动空间进行误差变换统计分析，将估计变换传播到分割的TMJ结构进行仿真评估。

**Result:** 成功实现了基于变换的误差分析和表面距离度量，能够量化计划与达成构型之间的差异。方法可用单个解剖模型和变换数据间接评估TMJ构型，减少了多次成像需求。

**Conclusion:** 该方法为TMJ构型定量分析提供了有效的技术框架，通过详细的分步骤图形展示证明了其可行性，为后续临床验证奠定了方法学基础。
...
### [95] [Benchmarking Layout-Guided Diffusion Models through Unified Semantic-Spatial Evaluation in Closed and Open Settings](https://arxiv.org/abs/2604.25358)
*Luca Parolari,Nicla Faccioli,Lamberto Ballan*

Main category: cs.CV

> 该论文提出了C-Bench封闭集基准测试和O-Bench开放集基准测试，用于评估布局引导的文本到图像生成模型，并开发了统一的评估协议，对6个最先进的扩散模型进行了大规模评估。

<details>
  <summary>Details</summary>

**Motivation:** 当前评估布局引导的文本到图像生成模型缺乏全面的布局评估基准，因为收集细粒度标注成本高昂且劳动密集，导致现有基准规模有限或覆盖不足，使得模型比较、排名和解释困难。

**Method:** 1. 设计C-Bench封闭集基准测试：隔离关键生成能力，在提示结构和布局方面提供不同复杂度；2. 提出O-Bench开放集基准测试：使用真实世界的提示和布局评估模型；3. 开发统一的评估协议：将语义和空间准确性结合为单一评分；4. 对6个最先进的布局引导扩散模型进行大规模评估，共生成和评估319,086张图像。

**Result:** 建立了基于整体性能的模型排名，为文本和布局对齐提供了详细分解以增强可解释性。跨场景和提示复杂度的细粒度分析揭示了当前模型的优势和局限性。

**Conclusion:** 该工作为评估布局引导的文本到图像生成模型提供了标准化、可扩展的评估框架，弥补了现有基准的不足，为该领域的研究进展提供了重要参考。
...
### [96] [HuM-Eval: A Coarse-to-Fine Framework for Human-Centric Video Evaluation](https://arxiv.org/abs/2604.25361)
*Bingzi Zhang,Kaisi Guan,Ruihua Song*

Main category: cs.CV

> HuM-Eval是一个以人类为中心的视频生成评估框架，采用粗到细的策略，结合2D姿态和3D运动分析来评估人体运动视频质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频评估指标主要关注全局场景统计，忽略了细粒度的人类细节，无法准确反映人类主观偏好，因此需要专门针对人体运动视频的评估方法。

**Method:** 提出粗到细的评估策略：首先使用视觉语言模型进行全局视频质量粗评估，然后进行细粒度分析，用2D姿态验证解剖学正确性，用3D人体运动评估运动稳定性。

**Result:** HuM-Eval实现了58.2%的平均人工相关性，超越了最先进的基线方法。同时推出了包含1000个多样化提示的HuM-Bench基准测试。

**Conclusion:** 该框架为人体运动视频生成提供了更准确的评估方法，为下一代人体运动生成技术发展铺平了道路。
...
### [97] [Self-DACE++: Robust Low-Light Enhancement via Efficient Adaptive Curve Estimation](https://arxiv.org/abs/2604.25367)
*Jianyu Wen,Jun Xie,Feng Chen,Zhepeng Wang,Chenhao Wu,Tong Zhang,Yixuan Yu,Piotr Swierczynski*

Main category: cs.CV

> Self-DACE++是一个改进的轻量级无监督低光图像增强框架，通过自适应调整曲线、随机顺序训练策略和去噪模块，在保持计算效率的同时实现了高质量的实时图像增强。

<details>
  <summary>Details</summary>

**Motivation:** 现有的低光图像增强方法在计算效率和恢复质量之间存在权衡问题。为了解决这一矛盾，同时保持增强图像的色彩保真度、结构完整性和自然性，需要开发一种既轻量级又高效的解决方案。

**Method:** 1) 引入增强的自适应调整曲线(AACs)，用最少的可训练参数灵活调整动态范围；2) 提出随机顺序训练策略结合网络融合机制，将模型压缩为高效的迭代推理结构；3) 基于Retinex理论制定物理感知的目标函数，并集成专门的去噪模块来估计和抑制暗区域噪声。

**Result:** 在多个真实世界基准数据集上的广泛定性和定量评估表明，Self-DACE++优于现有最先进方法，能够提供卓越的增强质量并具备实时推理能力。

**Conclusion:** Self-DACE++成功地在极轻量级的架构和高质量图像增强之间实现了平衡，为低光图像增强提供了实时、高效的解决方案，在保持色彩和结构保真度的同时有效处理噪声问题。
...
### [98] [GPT-Image-2 in the Wild: A Twitter Dataset of Self-Reported AI-Generated Images from the First Week of Deployment](https://arxiv.org/abs/2604.25370)
*Kidus Zewde,Simiao Ren,Xingyu Shen,Jenny Wu,Yuchen Zhou,Tommy Duong,Zikang Zhang,Ethan Traister*

Main category: cs.CV

> 本研究发布了首个GPT-Image-2生成图像数据集，包含从Twitter收集的10,217张确认图像，并发现Twitter的CDN会系统性地剥离C2PA内容凭据。

<details>
  <summary>Details</summary>

**Motivation:** 随着GPT-Image-2的发布，AI生成图像与真实照片的界限变得难以区分，需要一个专门的数据集来研究这一现象。社交媒体平台上的AI图像传播缺乏有效的溯源验证机制。

**Method:** 通过Twitter API v2和多阶段筛选流程收集数据，包括多语言文本启发式筛选、浏览器自动化验证

**Result:** 构建了包含10,217张GPT-Image-2图像的数据集，分析了图像特征，发现关键问题：Twitter CDN系统性剥离C2PA内容凭据，使加密溯源验证不可行。同时发现82%图像包含可检测文本，59.2%包含人脸。

**Conclusion:** 该数据集为AI图像研究提供了重要资源，但社交媒体平台的处理方式严重影响了AI图像的可验证性和溯源能力。
...
### [99] [CoRE: Concept-Reasoning Expansion for Continual Brain Lesion Segmentation](https://arxiv.org/abs/2604.25376)
*Qianqian Chen,Anglin Liu,Jingyang Zhang,Yudong Zhang*

Main category: cs.CV

> CoRE框架提出了一种概念推理扩展机制，通过整合视觉特征与结构化概念，实现高效的脑部病变MRI持续学习分割。

<details>
  <summary>Details</summary>

**Motivation:** 脑部病变MRI准确分割对临床诊断至关重要，但由于标注成本高和数据隐私限制，现有持续学习方法存在容量限制、参数冗余增长问题，且无法有效处理脑部成像中的病理学和多模态异质性。

**Method:** 提出CoRE框架，建立视觉特征与结构化概念的联合决策机制。通过图像标记与层次化概念库的匹配，模拟临床推理过程，指导可解释专家路由和按需模型增长。

**Result:** 在12个连续脑部病变MRI任务上的广泛评估表明，CoRE达到最先进性能，为高效未来适配提供了高知识起点，具备优秀的少样本迁移能力和临床可解释性。

**Conclusion:** CoRE通过概念推理扩展，成功解决了现有持续学习方法在脑部病变分割中的局限性，实现了参数高效增长、知识复用最大化，为非平稳临床数据流管理提供了有效解决方案。
...
### [100] [Benchmarking and Improving GUI Agents in High-Dynamic Environments](https://arxiv.org/abs/2604.25380)
*Enqi Liu,Liyuan Pan,Zhi Gao,Yan Yang,Chenrui Shi,Yang Liu,Jingrong Wu,Qing Li*

Main category: cs.CV

> 本文提出了DynamicGUIBench动态GUI基准和DynamicUI智能体，通过使用屏幕录制视频而非单一截图来捕捉动态界面变化，显著提升了动态GUI环境下的任务执行性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有GUI智能体主要依赖单张截图进行决策，在高度动态的GUI环境中存在部分可观测性问题，关键的GUI状态信息往往无法被充分捕捉，限制了智能体的决策效果。

**Method:** 提出了DynamicUI智能体框架，包含三个核心组件：1)动态感知器通过聚类GUI视频帧生成描述并选择最有信息量的帧作为显著动态上下文；2)优化策略采用动作条件过滤减少思维-动作不一致和冗余；3)反思模块基于优化后的轨迹提供准确的进一步动作指导。

**Result:** 在DynamicGUIBench基准上的实验表明，DynamicUI显著提升了动态GUI环境下的任务处理能力，同时在其他公共基准上保持了竞争力的性能表现。

**Conclusion:** 通过利用屏幕录制视频的时序信息和多组件协作设计，DynamicUI有效解决了传统GUI智能体在动态环境中的部分可观测性问题，为处理复杂的GUI交互任务提供了新的解决思路。
...
### [101] [COMPASS: COmpact Multi-channel Prior-map And Scene Signature for Floor-Plan-Based Visual Localization](https://arxiv.org/abs/2604.25388)
*Muhammad Shaheer,Miguel Fernandez-Cortizas,Asier Bikandi-Noya,Holger Voos,Jose Luis Sanchez-Lopez*

Main category: cs.CV

> COMPASS算法利用建筑平面图的几何和语义先验信息，结合双鱼眼相机图像，通过设计多通道径向描述符实现机器人位姿估计，并验证了跨模态结构匹配的可行性。

<details>
  <summary>Details</summary>

**Motivation:** 现有定位方法很大程度上忽略了建筑平面图中包含的丰富语义信息，仅利用几何信息进行定位，限制了定位精度和鲁棒性。

**Method:** 受LiDAR扫描上下文描述符启发，设计多通道径向描述符，包含5个通道：归一化距离、结构命中类型（墙壁、窗户或开口）、距离梯度、距离倒数和局部距离方差。通过360度方位角射线投影生成平面图描述符，并在鱼眼图像中检测窗户等结构元素生成视觉描述符。

**Result:** 在Hilti-Trimble SLAM Challenge 2026数据集上，从已知姿态生成的两种描述符在墙壁-窗户模式上能够很好匹配，验证了跨模态结构匹配方法的可行性。

**Conclusion:** 该方法成功将建筑平面图的语义信息整合到视觉定位中，为实现完整的跨模态匹配奠定了基础，有望提升室内机器人的定位性能。
...
### [102] [Leveraging Previous-Traversal Point Cloud Map Priors for Camera-Based 3D Object Detection and Tracking](https://arxiv.org/abs/2604.25405)
*Markus Käppeler,Özgün Çiçek,Yakov Miron,Abhinav Valada*

Main category: cs.CV

> DualViewMapDet提出了一种摄像头+先验地图的3D目标检测框架，通过双空间融合策略有效解决了纯摄像头方法中的深度模糊问题。

<details>
  <summary>Details</summary>

**Motivation:** 在自动驾驶中，纯摄像头3D目标检测存在深度模糊问题，而LiDAR成本昂贵。车辆经常重复行驶相同路线，可以利用先验点云地图作为几何先验来解决这一挑战。

**Method:** 提出双空间摄像头-地图融合策略：(1)将地图投影到透视视图(PV)并编码多通道几何线索增强图像特征，(2)在鸟瞰图(BEV)中直接编码地图，通过稀疏体干网络与摄像头特征在共享度量空间中融合。

**Result:** 在nuScenes和Argoverse 2数据集上，该方法相比强纯摄像头基线取得了持续改进，在目标定位方面提升尤为显著。消融实验验证了PV/BEV融合和先验地图覆盖的贡献。

**Conclusion:** DualViewMapDet成功利用先验地图显著提升了纯摄像头3D检测性能，特别是在目标定位精度方面，为无LiDAR的自动驾驶系统提供了一种实用的解决方案。
...
### [103] [Beyond Fidelity: Semantic Similarity Assessment in Low-Level Image Processing](https://arxiv.org/abs/2604.25408)
*Runjie Wang,Weiling Chen,Tiesong Zhao,Chang Wen Chen*

Main category: cs.CV

> 本文提出了语义相似性作为低级图像处理的新评估任务，并开发了T3S指标来评估处理后的图像是否保持了语义内容的完整性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的图像质量评估主要关注视觉保真度，但在深度学习和生成模型时代，处理后的图像可能在保持感知质量的同时改变了语义内容，这使得传统IQA方法无法有效评估语义层面的变化。

**Method:** 提出了T3S（基于三元组的语义相似性评分）方法，该方法通过前景实体、背景实体和它们之间的关系来建模图像语义，结合了语义实体提取、前景-背景解耦和开放世界的类别/关系建模。

**Result:** 在COCO和SPA-Data数据集上的实验表明，T3S在评估语义相似性方面始终优于现有的保真度导向指标和代表性的语义层面基线方法，同时能更好地反映在各种退化条件下的渐进性语义变化。

**Conclusion:** 研究结果强调了在现代低级视觉任务中语义评估的重要性，为低级图像处理提供了新的语义层面的评估标准和工具。
...
### [104] [A Systematic Post-Train Framework for Video Generation](https://arxiv.org/abs/2604.25427)
...
> 针对大规模视频扩散模型的部署难题，提出包含监督微调、强化学习、提示词增强和推理优化的四阶段后训练框架。

<details>
  <summary>Details</summary>

**Motivation:** 预训练的视频扩散模型虽能生成高质量内容，但在实际部署中面临提示词敏感、时序不一致、推理成本过高等关键问题，导致预训练性能与现实需求存在显著差距。

**Method:** 通过四个协同阶段实现系统化对齐：1) 监督微调将基础模型转为稳定的指令遵循策略；2) 提出专用于视频扩散的群体相对策略优化(GRPO)的强化学习方法；3) 集成专用语言模型进行提示词增强；4) 实施推理优化提升系统效率。

**Result:** 实验证明该统一管道有效减少常见伪影，显著提升可控性和视觉美感，同时严格遵守采样成本限制，为构建可扩展、稳定、适应性的后训练管道提供了实用方案。

**Conclusion:** 该框架成功弥合了视频扩散模型预训练与实际部署间的差距，在保持预训练可控性的同时，系统性提升了视觉质量、时序一致性和指令遵循能力，为大规模视频生成模型的产业化应用奠定了基础。
...
### [105] [SARU: A Shadow-Aware and Removal Unified Framework for Remote Sensing Images with New Benchmarks](https://arxiv.org/abs/2604.25432)
*Zi-Yang Bo,Wei Lu,Hongruixuan Chen,Si-Bao Chen,Bin Luo*

Main category: cs.CV

> SARU是一个统一框架，集成阴影检测与去除，使用双分支网络生成高保真阴影掩码，并通过无训练物理算法基于单张图像恢复光照，无需配对训练数据。

<details>
  <summary>Details</summary>

**Motivation:** 传统遥感影像处理中阴影检测和去除分离处理导致流程繁琐和误差累积，且现有深度学习方法依赖难以获得的配对训练数据，限制了实际应用。

**Method:** 提出两阶段SARU框架：1) DBCSF-Net双分支检测模块融合多色彩空间和语义特征生成阴影掩码；2) N²SGSR无训练物理算法利用相邻非阴影区域属性在单张图像内恢复光照。

**Result:** 在公开AISD数据集和新提出的RSISD、SiSRB基准数据集上达到最优性能，有效区分阴影与暗色物体，显著提升遥感影像质量。

**Conclusion:** 通过整体集成阴影检测与去除减少误差传播，消除对配对训练数据的依赖，SARU为实际遥感影像分析建立了强大实用的框架，推动了该领域的发展。
...
### [106] [GramSR: Visual Feature Conditioning for Diffusion-Based Super-Resolution](https://arxiv.org/abs/2604.25457)
*Fabio D'Oronzio,Federico Putamorsi,Leonardo Zini,Marcella Cornia,Lorenzo Baraldi*

Main category: cs.CV

> 这项研究提出了GramSR，一种一步式扩散超分辨率框架，它用DINOv3提取的密集视觉特征替代文本引导，通过三级LoRA架构实现更好的图像重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于扩散模型的单图像超分辨率方法依赖文本条件，但文本描述只能提供高级语义信息，缺乏空间对齐的视觉信息，导致在抽象语义和空间细节之间存在表示鸿沟，影响重建的保真度。

**Method:** GramSR采用三级LoRA架构：像素级模块（用于退化移除）、语义级模块（通过LPIPS和CSD损失增强感知细节）、纹理级模块（通过DINOv3特征的Gram矩阵损失强制特征相关性一致性）。推理时使用独立的引导尺度灵活控制各阶段效果。

**Result:** 在标准超分辨率基准测试中，GramSR一致性地优于现有的单步扩散方法，实现了更优越的结构保真度和纹理真实感。

**Conclusion:** 通过用密集视觉特征替代文本引导，GramSR有效解决了语义与空间细节的鸿沟问题，证明将预训练视觉编码器与扩散框架结合是提升单步超分辨率性能的有效途径。
...
### [107] [Image Compression with Bubble-Aware Frame Rate Adaptation for Energy-Efficient Video Capsule Endoscopy](https://arxiv.org/abs/2604.25464)
*Oliver Bause,Jörg Gammerdinger,Julia Werner*

Main category: cs.CV

> 提出了一种针对视频胶囊内镜的图像压缩和动态帧率自适应方案，在保持诊断质量的同时显著降低能耗。

<details>
  <summary>Details</summary>

**Motivation:** 视频胶囊内镜因体积小导致电池寿命有限，而图像捕获和传输的高能耗与这一限制产生了严重冲突。

**Method:** 设计图像压缩管道减少传输数据，并利用压缩过程特征识别低诊断价值的气泡帧，实施动态气泡感知的帧率自适应策略。

**Result:** 压缩比达5.748，PSNR为40.3dB，系统平均能耗降低20.58%，帧率自适应进一步降低能耗达40%。

**Conclusion:** 该方法能有效延长VCE的工作时间，提高其在临床检查中的实用性和可行性。
...
### [108] [Generalizable Human Gaussian Splatting via Multi-view Semantic Consistency](https://arxiv.org/abs/2604.25466)
*Jingi Kim,Wonjun Kim*

Main category: cs.CV

> 本文提出了一种新的方法，通过预测深度图和跨视角注意力机制，将多视角特征投影到共享3D空间并重新校准，从而改进稀疏输入下的可泛化人体高斯喷溅渲染质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的可泛化人体高斯喷溅方法在处理人体复杂关节运动和视角间有限重叠时，会导致多视角输入的特征表示不一致，影响3D高斯点的精确定位。

**Method:** 通过预测深度图将每个视角的潜在嵌入反投影到共享3D空间，并基于跨视角注意力机制对同一身体部分的3D高斯点进行重新校准，解决空间模糊性问题。

**Result:** 在基准数据集上的实验结果表明，该方法有效提升了稀疏视角输入下可泛化人体高斯喷溅的性能。

**Conclusion:** 该方法通过跨视角注意力校准3D空间中的特征表示，有效解决了人体复杂姿态下的空间模糊问题，显著提高了人体渲染的定位精度和整体质量。
...
### [109] [DDA-Thinker: Decoupled Dual-Atomic Reinforcement Learning for Reasoning-Driven Image Editing](https://arxiv.org/abs/2604.25477)
*Hanqing Yang,Qiang Zhou,Yongchao Du,Sashuai Zhou,Zhibin Wang,Jun Song,Tiezheng Ge,Cheng Yu,Bo Zheng*

Main category: cs.CV

> DDA-Thinker是一个以思考者为中心的图像编辑框架，通过双原子强化学习分离规划模块与生成模型的优化，显著提升了复杂推理任务下的图像编辑性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有图像编辑模型在视觉保真度方面表现良好，但在需要复杂推理的任务中表现不佳，需要专门研究如何增强图像编辑的推理基础规划能力。

**Method:** 提出DDA-Thinker框架，采用思考者-编辑者的分离架构；引入双原子强化学习框架，通过认知原子奖励评估可执行计划质量和视觉原子奖励评估最终图像质量；使用理性参考描述指导检查表合成；开发两阶段数据整理流程进行难度感知的课程学习。

**Result:** 在RISE-Bench和KRIS-Bench等推理驱动图像编辑基准测试中，方法显著提升了整体性能，使社区模型能够与强大的专有模型竞争。

**Conclusion:** 以思考者为中心的优化在固定编辑器设置下具有实际应用潜力，通过分离规划与生成优化、双重反馈机制和精心设计的训练流程，有效解决了图像编辑中的复杂推理问题。
...
### [110] [The Forensic Cost of Watermark Removal](https://arxiv.org/abs/2604.25491)
*Gautier Evennou,Ewa Kijak*

Main category: cs.CV

> 现有水印移除方法在攻击成功率和感知质量两个维度评估不够充分，本文发现了第三个被忽视的维度——水印移除检测(WRD)，证明了所有现有方法都会留下可被检测的统计痕迹，没有方法能同时平衡攻击成功、感知质量和法医隐匿性这三个要求。

<details>
  <summary>Details</summary>

**Motivation:** 当前水印移除方法主要关注攻击成功率和感知质量，但忽略了移除过程可能留下的统计痕迹，这些痕迹会暴露移除行为，导致评估标准不完整。

**Method:** 提出水印移除检测(WRD)概念，训练现代分类器检测移除痕迹，对主流水印方案和移除管道在三维评估框架(攻击成功、感知质量、法医可检测性)下进行基准测试。

**Result:** 分类器在10^-3错误虚警率下对所有测试的移除方法都能达到最先进的检测率，没有现有方案能够同时满足三个维度的要求。

**Conclusion:** 法医隐匿性应成为水印移除的必要要求，现有方法在隐蔽性方面存在根本缺陷，需要重新考虑水印移除技术的评估标准。
...
### [111] [The Surprising Effectiveness of Canonical Knowledge Distillation for Semantic Segmentation](https://arxiv.org/abs/2604.25530)
*Muhammad Ali,Kevin Alexander Laube,Madan Ravi Ganesh,Lukas Schott,Niclas Popp,Thomas Brox*

Main category: cs.CV

> 研究发现，在匹配计算资源的情况下，传统的知识蒸馏方法在语义分割任务上优于近期提出的复杂方法，特征蒸馏通过扩展训练达到了SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 近期语义分割知识蒸馏方法引入复杂的手工目标，但通常基于固定迭代次数评估。由于复杂目标大幅增加计算成本，传统迭代次数比较可能具有误导性，无法区分真实性能提升还是更多计算资源的结果。

**Method:** 通过匹配墙钟时间而非迭代次数进行公平比较，评估传统logit和特征知识蒸馏与近期分割特定方法的效果；对ResNet-18学生模型进行扩展训练，蒸馏来自ResNet-101教师模型的知识。

**Result:** 在匹配计算资源下，传统KD方法优于近期复杂方法；特征KD在Cityscapes和ADE20K上达到SOTA；ResNet-18学生达到教师99%（Cityscapes）和92%（ADE20K）的mIoU性能，仅用1/4参数。

**Conclusion:** 研究挑战了分割KD需要任务特定机制的假设，表明扩展训练而非复杂手工目标应指导未来方法设计，scale up比design up更有效。
...
### [112] [DualGeo: A Dual-View Framework for Worldwide Image Geo-localization](https://arxiv.org/abs/2604.25533)
*Junchao Cui,Wenqi Shi,Shaoyong Du,Hang He,Xuanzi Ma,Hao Tang,Xiangyang Luo*

Main category: cs.CV

> DualGeo是一个两阶段全球图像地理定位框架，通过多模态特征融合和地理聚类重排序，显著提升了街道级和城市级定位精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有图像地理定位方法存在两个主要问题：1)视觉特征对环境变化(光照、季节、天气)敏感；2)缺乏有效的后处理机制过滤异常候选，限制了定位精度。

**Method:** DualGeo采用两阶段框架：第一阶段通过双向交叉注意力融合图像与语义分割特征，并用双视角对比学习对齐GPS坐标构建全局检索数据库；第二阶段通过地理聚类对检索候选重排序，然后输入大型多模态模型进行最终坐标预测。

**Result:** 在IM2GPS、IM2GPS3k和YFCC4k数据集上，DualGeo相比SOTA方法，街道级(<1km)定位精度提升3.6%-16.58%，城市级(<25km)定位精度提升1.29%-8.77%。

**Conclusion:** DualGeo通过有效融合多模态特征和地理认知精炼，解决了现有方法的环境敏感性和后处理不足问题，为全球图像地理定位提供了新的解决方案。
...
### [113] [TopoMamba: Topology-Aware Scanning and Fusion for Segmenting Heterogeneous Medical Visual Media](https://arxiv.org/abs/2604.25545)
...
> TopoMamba是一个拓扑感知的扫描融合框架，通过改进扫描顺序和融合机制解决了视觉状态空间模型在医学图像分割中的轴偏置和冗余响应问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉状态空间模型在医学图像分割中受到两个重要限制：轴偏置扫描顺序削弱了对倾斜和曲线结构的建模能力，以及朴素多分支融合容易放大冗余响应。

**Method:** 提出结合对角线/反对角线的TopoA-Scan分支与标准Cross-Scan分支提供互补结构先验，引入ScanCache缓存机制优化扫描索引构建，设计轻量级HSIC Gate进行依赖感知的分支融合，并扩展实现了3D版本的TopoMamba-3D。

**Result:** 在Synapse CT、ISIC 2017皮肤镜和CVC-ClinicDB内窥镜数据集上，TopoMamba在胰腺和胆囊等薄或弯曲目标分割上取得显著提升，超越CNN、Transformer和SSM基线，同时保持较高的部署效率。

**Conclusion:** 拓扑感知的扫描顺序和轻量级依赖感知融合机制构成了医学多媒体分割的有效实用设计方案，为临床应用提供了新的技术路径。
...
### [114] [Vision SmolMamba: Spike-Guided Token Pruning for Energy-Efficient Spiking State-Space Vision Models](https://arxiv.org/abs/2604.25570)
*Dewei Bai,Hongxiang Peng,Yunyun Zeng,Ziyu Zhang,Hong Qu,Yi Zhang*

Main category: cs.CV

> Vision SmolMamba是一种结合脉冲引导token稀疏性和线性时间状态空间模型的脉冲视觉架构，实现了更高效的长距离视觉建模。

<details>
  <summary>Details</summary>

**Motivation:** 现有的Spike Transformer虽然展现了长距离视觉建模的潜力，但其二次方token交互机制与脉冲神经计算的稀疏性和事件驱动特性存在根本性不匹配。

**Method:** 提出脉冲引导的时空token剪枝器(SST-TP)，利用脉冲激活强度和首次脉冲延迟来估计token重要性，并基于此构建SmolMamba块，将脉冲事件直接整合到双向状态空间递归中。

**Result:** 在ImageNet-1K、CIFAR10/100等多个静态和事件驱动基准测试中，Vision SmolMamba在保持竞争力的准确率的同时，将能耗成本降低了至少1.5倍。
...
### [115] [Control Your Queries: Heterogeneous Query Interaction for Camera-Radar Fusion](https://arxiv.org/abs/2604.25574)
*Jialong Wu,Yihan Wang,Matthias Rottmann*

Main category: cs.CV

> 本文提出了一种新的相机-雷达融合范式ConFusion，通过异构查询交互实现3D目标检测，在nuScenes数据集上达到了59.1 mAP和65.6 NDS的state-of-the-art性能。

<details>
  <summary>Details</summary>

**Motivation:** 在自动驾驶中，相机-雷达融合能提供互补的感知能力和较低的部署成本。现有方法通过输入混合、特征图混合或基于查询的特征采样进行融合，但存在局限性，需要一种更有效的融合方式。

**Method:** ConFusion结合图像查询、雷达查询和分布在3D空间的可学习世界查询来改进查询初始化和目标覆盖。引入异构查询混合(QMix)进行分类间注意力交互，提出交互式查询交换采样(QSwap)在注意力和几何约束下允许相关查询交换信息特征标记。

**Result:** 在nuScenes数据集验证集上达到59.1 mAP和65.6 NDS，在测试集上达到61.6 mAP和67.9 NDS，实现了state-of-the-art性能。

**Conclusion:** 提出的异构查询交互范式有效地融合了相机和雷达的互补信息，通过QMix和QSwap等创新机制显著提升了3D目标检测性能，为低成本高精度的自动驾驶感知系统提供了新的解决方案。
...
### [116] [Refinement via Regeneration: Enlarging Modification Space Boosts Image Refinement in Unified Multimodal Models](https://arxiv.org/abs/2604.25636)
*Jiayi Guo,Linqing Wang,Jiangshan Wang,Yang Yue,Zeyu Liu,Zhiyuan Zhao,Qinglin Lu,Gao Huang,Chunyu Wang*

Main category: cs.CV

> 本文提出了一种新的图像精炼框架RvR，通过条件式图像再生而非编辑来改进文本到图像任务的生成质量，在多个基准测试中取得显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 当前基于统一多模态模型(UMMs)的图像精炼方法主要通过编辑指令进行局部修改，但这种方法存在两个主要问题：编辑指令描述粗糙导致精炼不完整，以及像素级保留限制了有效修改空间。

**Method:** 提出Refinement via Regeneration (RvR)框架，将精炼重新定义为条件式图像再生。该方法不依赖编辑指令和严格内容保留，而是基于目标提示词和初始图像的语义tokens重新生成图像，从而实现更完整的语义对齐和更大的修改空间。

**Result:** 在多个基准测试中显著提升性能：Geneval从0.78提升到0.91，DPGBench从84.02提升到87.21，UniGenBench++从61.53提升到77.41。

**Conclusion:** RvR框架通过再生而非编辑的方式，有效解决了传统精炼方法的局限性，在保持图像质量的同时实现了更完整和灵活的内容精炼，为统一多模态模型的图像生成精炼任务提供了新的解决思路。
...
### [117] [Toward Multimodal Conversational AI for Age-Related Macular Degeneration](https://arxiv.org/abs/2604.25720)
*Ran Gu,Benjamin Hou,Mélanie Hébert,Asmita Indurkar,Yifan Yang,Emily Y. Chew,Tiarnán D. L. Keenan,Zhiyong Lu*

Main category: cs.CV

> 该研究开发了名为OcularChat的眼科多模态大语言模型，通过模拟医患对话进行微调，能在彩色眼底照片上进行AMD诊断，不仅分类性能优异，还能提供临床推理和交互式解释。

<details>
  <summary>Details</summary>

**Motivation:** 现有的深度学习视网膜疾病检测系统大多只给出静态预测结果，缺乏临床推理过程或交互式解释能力，限制了其在临床决策和患者咨询中的应用价值。

**Method:** 基于Qwen2.5-VL模型微调开发了OcularChat，使用46,167张彩色眼底照片生成了705,850个模拟医患对话对进行训练，使模型能够识别AMD关键特征并产生有推理的预测结果。

**Result:** OcularChat在AREDS数据集上AMD三项诊断任务的准确率分别达到0.954、0.849和0.678，显著优于现有MLLM模型；在AREDS2数据集上也保持最佳性能；在眼科医师评分中，OcularChat在各个维度均优于基线模型。

**Conclusion:** MLLM技术能够实现准确、可解释且临床实用的基于图像的AMD诊断分类，OcularChat证明了在保持高性能的同时还能提供诊断推理、临床相关解释和交互对话的能力。
...
### [118] [Prefill-Time Intervention for Mitigating Hallucination in Large Vision-Language Models](https://arxiv.org/abs/2604.25642)
*Chengsheng Zhang,Chenghao Sun,Xinyan Jiang,Wei Li,Xinmei Tian*

Main category: cs.CV

> 本文提出Prefill-Time Intervention (PTI)方法，通过在预填充阶段一次性干预视觉-语言模型的KV缓存来减少幻觉，相比解码阶段干预更有效且不会放大残余幻觉。

<details>
  <summary>Details</summary>

**Motivation:** 现有使用引导向量减少LVLM幻觉的方法存在关键缺陷：虽然能减少部分幻觉，但会无意中放大残余幻觉的严重性，这是因为它们仅在解码阶段干预，而错误会自回归地累积并恶化后续的幻觉输出。

**Method:** PTI是一种新型引导范式，在预填充阶段仅进行一次干预，增强初始KV缓存在错误累积之前的有效性。该方法具有模态感知特性，为视觉和文本表示推导不同的方向，解耦式地引导键向量朝向视觉基础对象，同时引导值向量过滤背景噪声，从源头上纠正易产生幻觉的表示。

**Result:** 大量实验证明PTI在减少幻觉方面表现显著，且在不同解码策略、LVLM模型和基准测试中都具有良好的泛化能力。此外，PTI与现有解码阶段方法正交，可实现即插即用集成并进一步提升性能。

**Conclusion:** PTI通过改变干预时机从解码阶段转向预填充阶段，有效解决了现有方法放大残余幻觉的问题，为提升LVLM的可靠性提供了新的技术路径，具有较强的实用价值和推广前景。
...
### [119] [SAMe: A Semantic Anatomy Mapping Engine for Robotic Ultrasound](https://arxiv.org/abs/2604.25646)
*Jing Zhang,Duojie Chen,Wentao Jiang,Zihan Lou,Jianxin Liu,Xinwu Cui,Qinghong Zhao,Bo Du,Christoph F. Dietrich,Dacheng Tao*

Main category: cs.CV

> 本研究提出了SAMe语义解剖映射引擎，为机器人超声提供解剖先验层，能够将临床主诉转化为目标器官，基于单张外部身体图像构建患者特异性解剖表示，并将其转化为6-DoF探头初始化状态，实现高成功率的自主扫描启动。

<details>
  <summary>Details</summary>

**Motivation:** 现有机器人超声系统缺乏解剖学理解能力，无法自主确定扫描目标、起始位置和适应患者个体解剖差异，仍依赖专家干预来启动扫描，限制了系统的自主性。

**Method:** SAMe采用目标-解剖-行动的三步流程：1)语义 grounding 将临床主诉转化为结构化目标器官；2)从单张外部身体图像实例化患者特异性解剖表示；3)将解剖表示直接转换为面向控制的6-DoF探头初始化状态，无需额外的术前CT/MRI配准。

**Result:** 在语义理解、解剖实例化和真实机器人评估中表现出色。真实机器人实验中，肝脏初始化命中率达97.3%，肾脏初始化命中率达81.7%，即使在仅使用质心目标的限制条件下也优于表面启发式基线方法。

**Conclusion:** SAMe建立了一个显式解剖先验层，解决了扫描初始化问题，为更广泛的下游自主扫描流水线提供了解剖学基础，奠定了基于主诉驱动、解剖学知情的机器人超声检查的技术基础。
...
### [120] [Exploring Remote Photoplethysmography for Neonatal Pain Detection from Facial Videos](https://arxiv.org/abs/2604.25680)
*Ashutosh Dhamaniya,Anup Kumar Gupta,Trishna Saikia,Puneet Gupta*

Main category: cs.CV

> 本研究提出一种基于远程光电容积脉搏波(rPPG)的非接触式新生儿疼痛检测方法，结合语音特征并采用信号质量参数优化，有效克服了传统接触式检测的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 新生儿未缓解的疼痛会导致发育迟缓等不良后果，而传统的接触式生理参数监测不适合长期观察且增加疾病传播风险（如COVID-19），因此需要开发更安全客观的自动化疼痛评估方法。

**Method:** 采用远程光电容积脉搏波技术非接触式估计脉搏信号，引入质量参数选择受皮肤变形影响最小的感兴趣区域时间信号，使用信噪比作为适应度参数提取受噪声影响最小的rPPG信号，并融合音频特征进行疼痛检测。

**Result:** 实验表明rPPG信号为新生儿疼痛检测提供有效信息，蓝色通道提取的信号性能最优，rPPG与音频特征融合的结果优于单一模态。

**Conclusion:** 非接触式rPPG技术在新生儿疼痛检测中具有应用潜力，通过信号质量优化和多模态融合可实现更准确可靠的疼痛评估，为临床提供安全有效的监测手段。
...
### [121] [QB-LIF: Learnable-Scale Quantized Burst Neurons for Efficient SNNs](https://arxiv.org/abs/2604.25688)
*Dewei Bai,Hongxiang Peng,Jiajun Mei,Yang Ren,Hong Qu,Dawen Xia,Zhang Yi*

Main category: cs.CV

> Quantized Burst-LIF (QB-LIF)神经元将脉冲爆发建模为膜电位饱和均匀量化，通过可学习量化尺度自适应调节分辨率，在保持神经形态兼容性的同时显著提升了SNN在超低延迟下的信息吞吐量和性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统SNN的二进制脉冲编码每时间步仅能传输1位信息，在深度架构的短时仿真中严重限制了信息吞吐量，成为性能瓶颈。

**Method:** 提出QB-LIF神经元模型，将脉冲爆发视为膜电位的饱和均匀量化，量化尺度作为可训练参数使各层自适应调节；引入可吸收尺度策略在推理时将量化尺度折叠到突触权重中保持仅累加(AC)执行模式；设计ReLSG-ET反向传播方法优化离散多级空间训练。

**Result:** 在CIFAR-10/100、ImageNet等静态数据集和CIFAR10-DVS、DVS128-Gesture等事件驱动基准测试中，QB-LIF在超低延迟条件下均优于二进制和固定爆发SNN，实现了更高的准确率。

**Conclusion:** QB-LIF通过创新的量化爆发机制有效突破了二进制编码的信息瓶颈，为SNN在深度架构中的高效低延迟应用提供了新思路，同时保持了神经形态硬件的兼容性。
...
### [122] [Sketch2Arti: Sketch-based Articulation Modeling of CAD Objects](https://arxiv.org/abs/2604.25781)
*Yi Yang,Hao Pan,Yijing Cui,Alla Sheffer,Changjian Li*

Main category: cs.CV

> Sketch2Arti是首个基于草图的CAD物体关节建模系统，能够通过简单的2D草图自动推断可移动部件及其运动参数，支持多种物体的泛化应用。

<details>
  <summary>Details</summary>

**Motivation:** 设计师通常通过轻型草图（如箭头和笔画）来表达关节运动意图，但将这种草图转换为带关节的3D模型仍然主要依赖手动操作，缺乏自动化的解决方案。

**Method:** 用户从选定视角绘制简单2D草图，系统结合CAD模型自动发现相应的可移动部件并预测其运动参数。该系统采用类别无关的训练方式，支持在复杂物体上进行迭代建模，对缺乏内部结构的壳体模型提供可控的内部补全功能。

**Result:** 实验和用户评估证明了Sketch2Arti的有效性、可控性和泛化能力，能够处理多样化物体，生成与现有几何形状和预测运动约束一致的合理内部组件。

**Conclusion:** Sketch2Arti成功填补了草图意图与3D关节建模之间的技术空白，为交互式动画、仿真和形状编辑提供了便捷的自动化工具，展现出强大的实际应用价值。
...
### [123] [Improving Diversity in Black-box Few-shot Knowledge Distillation](https://arxiv.org/abs/2604.25795)
*Tri-Nhan Vo,Dang Nguyen,Kien Do,Sunil Gupta*

Main category: cs.CV

> 本文提出了一种黑盒少样本知识蒸馏的新方法，通过自适应选择高置信度图像并引入对抗学习来提高数据集多样性，在七个图像数据集上取得了最先进的结果。

<details>
  <summary>Details</summary>

**Motivation:** 传统知识蒸馏方法需要大量训练数据和教师模型内部访问权限，但实际应用中这些条件往往难以满足。虽然现有的黑盒少样本方法尝试生成合成图像，但缺乏主动策略来促进图像多样性，而这对于学生学习至关重要。

**Method:** 提出了一种新型的生成对抗网络训练方案，包括：1）在教师监督下自适应选择高置信度图像 2）将这些选中的图像动态引入到对抗学习过程中。这种方法帮助扩展和改进了蒸馏集的多样性。

**Result:** 通过在七个图像数据集上的大量实验，证明了该方法相比其他少样本知识蒸馏方法取得了最先进的性能，显著提升了学生模型的准确率。

**Conclusion:** 该研究成功解决了黑盒少样本知识蒸馏中的数据多样性问题，通过动态选择和对抗学习的策略，为实际应用中的模型压缩提供了有效的解决方案。
...
### [124] [Instruction-Evidence Contrastive Dual-Stream Decoding for Grounded Vision-Language Reasoning](https://arxiv.org/abs/2604.25809)
*Yashwant Pravinrao Bangde,Debaditya Roy*

Main category: cs.CV

> 本文提出IECD2解码框架，通过双流解码平衡语言信息性和视觉忠实度，显著减少VLM模型的幻觉现象。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型(VLMs)虽然表现强大，但经常产生缺乏视觉证据支撑的输出，指令提示会进一步放大这种基于语言先验的幻觉问题，特别是在视觉信号不确定或模糊时。

**Method:** 提出指令-证据对比双流解码(IECD2)框架，维持两个并行的token概率分布：指令驱动流促进表达和信息性响应，证据驱动流强制严格基于图像。使用对称KL对比门自适应融合两个流，抑制语言先验但缺乏视觉证据支持的token。

**Result:** 在多个数据集(POPE、MME、VQAv2、AMBER、MS-COCO、LLaVA-Bench)上评估，IECD2在任务准确性和推理性能上表现出一致改进，相比最先进的解码方法显著减少所有评估指标上的幻觉。

**Conclusion:** IECD2通过显式平衡语言信息性和视觉忠实度的解码策略，有效解决了VLMs的幻觉问题，为生成式视觉语言推理任务提供了可靠的解决方案。
...
### [125] [Magnification-Invariant Image Classification via Domain Generalization and Stable Sparse Embedding Signatures](https://arxiv.org/abs/2604.25817)
*Ifeanyi Ezuma,Olusiji Medaiyese*

Main category: cs.CV

> 通过梯度反转域泛化模型解决病理图像放大倍数偏移问题，实现了更稳健、紧凑且可迁移的表示学习，无需额外架构复杂度。

<details>
  <summary>Details</summary>

**Motivation:** 放大倍数变化是组织病理学分类面临的主要障碍，在一个成像尺度上训练的模型在其他尺度上泛化能力较差，这阻碍了计算病理学模型在异构采集环境中的可靠部署。

**Method:** 在BreaKHis数据集上采用严格的患者不相交留一放大倍数协议，比较监督基线、DCGAN增强基线以及梯度反转域泛化模型（保留判别信息同时抑制放大倍数特定变化），并通过稀疏嵌入分析评估模型性能。

**Result:** 域泛化模型整体判别能力最强，尤其在200X放大倍数被留出时优势明显；Brier分数最低（0.063 vs 0.089）；平均签名规模减少三倍以上（306 vs 1,074维度），同时保持相当的预测性能（AUC: 0.967 vs 0.965；F1: 0.930 vs 0.931）；跨折签名可重现性从接近零提升至0.99。

**Conclusion:** 研究表明可以在不增加架构复杂度的情况下学习到校准、紧凑且可迁移的表示，为在异构采集设置中可靠部署计算病理学模型提供了明确指导。
...
### [126] [Mutual Forcing: Dual-Mode Self-Evolution for Fast Autoregressive Audio-Video Character Generation](https://arxiv.org/abs/2604.25819)
*Yupeng Zhou,Lianghua Huang,Zhifan Wu,Jiabao Wang,Yupeng Shi,Biao Jiang,Daquan Zhou,Yu Liu,Ming-Ming Cheng,Qibin Hou*

Main category: cs.CV

>  Mutual Forcing是一个快速自回归音视频生成框架，通过单模型内的自蒸馏机制，实现长时音视频同步，仅需4-8步采样即可达到传统50步模型的效果

<details>
  <summary>Details</summary>

**Motivation:** 现有音视频生成面临两大挑战：联合音视频建模困难和自回归生成效率低下，传统的流式蒸馏需要先训练双向模型再通过多阶段蒸馏转换为因果生成器，流程复杂且开销大

**Method:** 采用两阶段训练策略：先训练单模态生成器，再耦合成统一模型进行联合训练。核心创新是Mutual Forcing框架，在单一权重共享模型中整合少步和多步生成模式，通过自蒸馏改善训练-推理一致性，无需额外的双向教师模型

**Result:** 实验表明，Mutual Forcing在仅使用4-8步采样的情况下，性能匹配或超过需要约50步采样的强基线模型，在效率和质量方面都显示出显著优势

**Conclusion:** Mutual Forcing为音视频生成提供了更简单高效的解决方案，减少了训练开销，支持更灵活的训练序列长度，能够直接从真实成对数据中学习改进，为实现实时高质量的音视频生成开辟了新途径
...
### [127] [SIEVES: Selective Prediction Generalizes through Visual Evidence Scoring](https://arxiv.org/abs/2604.25855)
*Hector G. Rodriguez,Marcus Rohrbach*

Main category: cs.CV

> SIEVES是一种通过视觉证据评分实现选择性预测的方法，能在分布外场景下将覆盖率提升3倍，并无需访问模型权重即可迁移到专有模型。

<details>
  <summary>Details</summary>

**Motivation:** 多模态语言模型在实际部署时面临着分布外场景的挑战，需要满足低错误容限。传统的视觉问答基准已趋于饱和，但可靠部署要求系统在用户定义的风险水平内提高回答覆盖率。

**Method:** 要求推理模型在回答时提供局部化视觉证据，设计一个明确学习估计推理模型定位质量的评分器。通过评估视觉证据质量来决定是否回答，从而实现选择性预测。

**Result:** 在5个具有挑战性的分布外基准测试中，SIEVES相比非基线方法将覆盖率提升最多3倍。无需特定基准或推理器的训练就能泛化到不同模型。

**Conclusion:** SIEVES不仅在分布外任务上具有更好的泛化能力，其设计还能迁移到无需访问权重或logits的专有推理器，提供了超越准确率的覆盖率提升，展现了强大的通用性。
...
### [128] [No Pedestrian Left Behind: Real-Time Detection and Tracking of Vulnerable Road Users for Adaptive Traffic Signal Control](https://arxiv.org/abs/2604.25887)
*Anas Gamal Aly,Hala ElAarag*

Main category: cs.CV

> 本文提出了一种实时自适应交通信号系统NPLB，通过监控弱势道路用户并在需要时自动延长信号时间，将行人滞留率从9.10%降至2.60%，提升安全性71.4%。

<details>
  <summary>Details</summary>

**Motivation:** 现有行人过街信号采用固定时序，无法根据行人行为进行调整，导致老年人、残疾人或分心的弱势道路用户在信号变化时被困在路口。

**Method:** NPLB系统集成了微调的YOLOv12目标检测模型、ByteTrack多目标跟踪和自适应控制器，当剩余时间低于临界阈值时自动延长行人相位。

**Result:** 通过10,000次蒙特卡洛仿真验证，NPLB将VRU滞留率从9.10%降至2.60%，安全性提升71.4%，仅需在12.1%的过街周期中延长信号。

**Conclusion:** NPLB系统能够有效保护弱势道路用户安全，在不显著增加交通延迟的前提下大幅降低行人滞留风险，展示了自适应交通信号控制的实用价值。
...
### [129] [Robust Deepfake Detection: Mitigating Spatial Attention Drift via Calibrated Complementary Ensembles](https://arxiv.org/abs/2604.25889)
*Minh-Khoa Le-Phan,Minh-Hoang Le,Trong-Le Do,Minh-Triet Tran*

Main category: cs.CV

> 本文提出了一种基于基础模型驱动的取证框架，通过集成极端复合退化引擎和结构约束多流架构，解决了深度伪造检测模型在真实世界退化条件下的空间注意力漂移问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有深度伪造检测模型在完美的学术数据集上表现优异，但在现实世界复合退化（如模糊和严重有损压缩）下会出现严重的空间注意力漂移，这限制了模型在实际应用中的有效性。

**Method:** 该框架集成了极端复合退化引擎和结构约束多流架构。训练时通过退化流水线破坏高频伪影，优化DINOv2-Giant骨干网络提取不变的几何和语义先验。使用三个专门的流处理图像：全局纹理流、局部面部流和融合CLIP的混合语义流，并通过校准离散化投票机制聚合预测。

**Result:** 该方法实现了高度稳定的零样本泛化能力，在NTIRE 2026鲁棒深度伪造检测挑战赛中获得第四名，有效抑制了背景注意力漂移并作为鲁棒的几何锚点。

**Conclusion:** 通过整合基础模型、结构化多流架构和极端退化训练，该方法显著提升了深度伪造检测在真实复杂环境下的鲁棒性，为实际应用提供了可靠的解决方案。
