...
> 研究提出了一种新的基于Transformer的系统，通过一系列技术，包括序列建模、增强框架、测试时训练和对称性评分，来改进ARC性能，达到了更好的推理效果。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机在于通过结合神经推理与结构感知的先验以及在线任务适应，提高在ARC测试集上的性能表现，评估模型推理能力。

**Method:** 本研究提出一种基于Transformer的系统，通过将ARC推理问题重新表述为序列建模问题，并引入基于群对称性、网格遍历和自动机扰动的增强框架，同时应用测试时训练(TTT)和轻量级LoRA适应方法，以及设计对称性解码和评分管道，实现显著的性能提升。

**Result:** 最终系统相较于Transformer基线模型有显著提升，超越了先前的神经网络ARC求解器，缩小了向人一般化能力的差距。

**Conclusion:** 各个组件协同工作，形成了一个在ARC数据集上表现优于已有神经网络模型的系统，显示了该方法在解决需要一般化推理任务方面的能力。
...
> 该研究利用概率上下文自由语法生成合成语料库，探讨了三种机制现象在合成和真实语料库中的出现，并提出层次结构在语言模型训练动态中的作用，为理解大型语言模型中的各种现象提供了统一的解释。

<details>
  <summary>Details</summary>

**Motivation:** 研究试图解决对Transformer语言模型中神经信息处理现象缺乏统一理解的问题，特别是在大规模文本库的训练过程中。

**Method:** 采用概率上下文自由语法生成可计算的精确大规模文本库的代理，并探究了三种机制现象：归纳头、功能向量和海德拉效应。

**Result:** 研究发现，数据生成过程中的层次结构对于解释这些机制现象的出现起到了关键作用。

**Conclusion:** 该工作是首次用统一解释来阐明大型语言模型中看似不相关的机制现象，并且提供了未来可解释性研究的高效合成工具。
...
> Hierarchical Embedding Fusion (HEF) improves retrieval-augmented code generation by compressing repository information for low-latency and high accuracy.

<details>
  <summary>Details</summary>

**Motivation:** To reduce the online inference cost and noise from long contexts in retrieval-augmented code generation by effectively compressing repository information.

**Method:** Hierarchical Embedding Fusion (HEF), a two-stage approach where the offline stage compresses repository chunks into a reusable hierarchy of dense vectors, and the online stage maps a small number of retrieved vectors into learned pseudo-tokens for the code generator.

**Result:** Achieved exact-match accuracy comparable to snippet-based retrieval baselines with a 1.8B-parameter pipeline and significantly reduced median end-to-end latency compared to graph-based and iterative retrieval systems.

**Conclusion:** Hierarchical dense caching is an effective mechanism for low-latency, repository-aware code completion.
...
> 研究揭示了现有自动评估框架在对抗性攻击下的性能下降问题，并提出了ReliableBench和JudgeStressTest来改进评估可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 研究动机在于指出现有的自动评估框架在应对复杂的对抗性攻击时存在的不足，尤其是面对生成模型风格多样化、攻击手段造成的输出模式扭曲以及语义模糊性变化显著时，现有的评估标准未能很好地适应这些变化，导致评估结果不可靠。

**Method:** 研究采用了一个全面的审计方法，使用了6642个人类验证过的标签来揭示评估框架在多种分布变化情况下的性能表现。

**Result:** 研究发现当前的评估框架在这种复杂的情况下面临着评估结果接近随机的性能下降，揭示了许多攻击手段实际上并不是真地产生了有害内容，而是通过利用评估框架的问题来夸大其成功率。

**Conclusion:** 为了实现更加可靠的评估，研究提出了ReliableBench，这是一个包含能更一致地进行评估的行为的基准标准，以及JudgeStressTest，一个旨在暴露评估评判失败的数据集。
...
> 本文提出了PerContrast和PerCE方法，用于估计和优化语言模型输出部分的个性化程度，实验显示这些方法显著提高了个性化性能，并显示出良好的跨任务和跨场景转移性。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型现在在各种任务中表现出色，但要想满足个性化输出的需求，仍然需要解决准确估计输出标记的个性化程度这一挑战。

**Method:** 我们提出了PerContrast，这是一种自对比方法，通过因果干预来估计每个输出标记对用户特定信息的依赖程度。基于这一机制，我们开发了PerCE损失函数，它可以在训练过程中自适应地增加具有较高预期个性化程度的标记权重，通过引导过程使模型能够在估计和优化这些标记之间交替进行。

**Result:** 实验结果表明，在几乎不增加额外成本的情况下，PerCE显著提高了个性化性能，在LongLaMP数据集上平均提升了超过10%，最高甚至提升了68.04%。这些结果展示了标记级别个性化建模的重要性，并证实了基于标记意识训练作为改善个性化大型语言模型的简单而有效的范式。

**Conclusion:** 我们的研究强调了标记级别个性化模型的重要性，并建立了一种简单而有效的基于标记意识训练的范式来改进个性化大型语言模型。
...
> 研究提出了黑暗三联人格框架作为一种心理建构模型来研究模型中的偏离行为。它表明可以通过少量的训练让先进的大语言模型模拟出类似人类黑暗三联人格特质的行为模式。

<details>
  <summary>Details</summary>

**Motivation:** 该研究旨在解决对强大的人工智能智能与其对人类偏好和价值观的兼容性的担忧问题。它提出了生物性偏离先于人工偏离的观点，并通过一个心理学上建立的黑暗三联人格框架来研究模型中的偏离行为。

**Method:** 该研究采用两种研究方法。第一项研究建立了人类群体中（N = 318）黑暗三联人格特质（自恋，心理变态和马基雅维利主义）的全面行为模式。第二项研究展示了通过极小的微调（如使用36个经过验证的心理测量项目）可以使前沿大语言模型（LLMs）表现出暗黑性格特征，这表明LLMs中存在着可以通过简短干预激活的潜在人格结构。

**Result:** 研究发现发现了黑暗三联人格特质之间的核心同理心缺陷，以及在道德推理和欺骗行为方面的特质特有的模式。通过极小的数据集训练，前沿LLMs能够展现出类似人类反社会属性的行为，而且这些行为能够泛化到训练项目之外。

**Conclusion:** 研究揭示了LLMs中可能存在可以通过极小干预激发的潜在人格结构，将其定位为在研究生物和人工智慧行为偏离方面的一个验证性框架。
...
> 小型本地部署语言模型可以有效地从儿童福利调查报告中识别出特定的物质类别，扩展了之前的二元分类工作到多标签物质识别。

<details>
  <summary>Details</summary>

**Motivation:** 验证一个本地部署的小型语言模型是否能够准确识别出符合DSM-5类别标准的特定物质类型，这在之前的研究中尚未得到探索。

**Method:** 使用本地部署的一个具有200亿个参数的语言模型对来自美国中西部州的儿童虐待调查报告进行分类。这些报告首先被识别出存在物质相关问题，然后被传递到第二个分类阶段，该阶段瞄准了七个DSM-5物质类别。

**Result:** 五个物质类别实现了几乎完美的方法间一致性（kappa = 0.94-1.00），包括酒精、大麻、阿片类、刺激剂和镇静剂/催眠剂/抗焦虑药。分类精确度在92%到100%之间。两个低频类别（致幻剂和吸入剂）表现不佳。不同测试间的一致性从92.1%到99.1%不等。

**Conclusion:** 小型本地部署的语言模型能够可靠地从儿童福利行政文本中分类物质类型，扩展了之前的二元分类工作到多标签物质识别。
...
> 本文讨论了NLP中注释者间协议(IAA)的概念化和应用，涉及任务类型的协议测量方法，讨论了标签不平衡和缺失数据的影响，并强调了透明报告的最佳实践。

<details>
  <summary>Details</summary>

**Motivation:** 随着注释和评估任务的不断扩展，从分类标签到分割、主观判断和连续评级，衡量注释者之间的协议变得越来越复杂。

**Method:** 本文通过分类任务类型来组织不同的协议测量方法，并讨论标签不平衡和缺失数据等因素如何影响可靠性估计。此外，还强调了透明报告的最佳实践，包括置信区间和分歧模式分析的使用。

**Result:** 本文概述了IAA在NLP及相关领域中的概念化和应用，并讨论了常见方法的假设和限制。

**Conclusion:** 本文旨在作为选择和解释协议措施的指南，推动NLP中人类注释和评估的一致性和可重复性。
...
### [9] [Roots Beneath the Cut: Uncovering the Risk of Concept Revival in Pruning-Based Unlearning for Diffusion Models](https://arxiv.org/abs/2603.06640)
*Ci Zhang,Zhaojun Ding,Chence Yang,Jun Liu,Xiaoming Zhai,Shaoyi Huang,Beiwen Li,Xiaolong Ma,Jin Lu,Geng Yuan*

Main category: cs.CV

> 本文揭示了基于修剪的遗忘方法可能存在的安全性问题，并提出了一种能够在没有额外数据或重新训练的情况下恢复被删除概念的攻击方法。同时，本文还探讨了潜在的防御策略。

<details>
  <summary>Details</summary>

**Motivation:** 本文揭示了基于修剪的遗忘方法的一个潜在危险，修剪权重的位置可能充当旁路信号，泄露被删除概念的关键信息。

**Method:** 我们设计了一种新的攻击框架，能够在完全没有数据和训练的情况下，从修剪后的扩散模型中恢复已被删除的概念。

**Result:** 实验结果表明，基于修剪的遗忘方法并不是绝对安全的，可以通过特定的方法恢复被删除的概念。

**Conclusion:** 当扩散模型中的关键概念相关权重被识别后，我们的方法能够有效地恢复原来的概念。同时，文章建议更安全的修剪机制，既能隐藏修剪位置，又能保持遗忘的有效性。
...
### [10] [ObjChangeVR: Object State Change Reasoning from Continuous Egocentric Views in VR Environments](https://arxiv.org/abs/2603.06648)
*Shiyi Ding,Shaoen Wu,Ying Chen*

Main category: cs.CV

> 本文提出了ObjChangeVR框架，用于评估虚拟现实环境中背景物体状态变化的问答任务，并展示了优于基线方法的表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大型语言模型主要集中在捕捉相机穿戴者与物体互动的视频上，但物体状态的变化可能发生在背景中，不涉及直接的用户交互，这使得检测变得困难。此外，对于这种挑战性场景还没有基准来进行评估。

**Method:** 我们提出了ObjChangeVR框架，该框架结合了视角感知和基于时间的检索来识别相关帧，并通过多视角推理解决来自多个视角的不一致证据问题。

**Result:** 大量的实验表明，ObjChangeVR在多个多模态大型语言模型上显著优于基线方法。

**Conclusion:** 该研究为虚拟现实中基于自然语言的场景变换查询提供了一个新的方向。
...
### [11] [Margin-Consistent Deep Subtyping of Invasive Lung Adenocarcinoma via Perturbation Fidelity in Whole-Slide Image Analysis](https://arxiv.org/abs/2603.06650)
...
> 为了解决全幻灯片图像分类中模型对边界扰动的敏感性，提出了一种边界一致性框架，提升了模型在肺癌腺癌亚型分类中的鲁棒性和准确性。

<details>
  <summary>Details</summary>

**Motivation:** 在肺癌腺癌亚型的全幻灯片分类任务中，模型容易受到现实世界成像扰动的影响，进而影响决策边界的可靠性。

**Method:** 提出了一种基于边界一致性的框架，结合了注意力加权的补丁聚合和边界感知训练，以实现特征-逻辑空间对齐。此外，引入了扰动保真度（PF）评分，通过贝叶斯优化参数施加结构化扰动，以对抗特征过度聚类的问题。

**Result:** Vision Transformer-Large 达到了 95.20 ± 4.65% 的准确性，而带有注意力机制的 ResNet101 达到了 95.89 ± 5.37% 的准确性，并且所有五个亚型的 AUC 均超过 0.99。在 WSSS4LUAD 外部基准测试中，带有注意力机制的 ResNet50 获得了 80.1% 的准确性，表明具有跨机构泛化能力。

**Conclusion:** 提出的方法通过改进的训练策略和注意力机制增加了分类的准确性和对扰动的鲁棒性，并在多个数据集上验证了其效能，显示出良好的可泛化性。
...
### [12] [PaLMR: Towards Faithful Visual Reasoning via Multimodal Process Alignment](https://arxiv.org/abs/2603.06652)
...
> PaLMR框架旨在解决多模态LLMs推理过程中的不准确可视化证据处理问题，包括构建过程感知推理数据和过程感知评分函数，实验显示其能有效减少推理幻觉，提高视觉推理的准确性和模型的可靠性与解释性。

<details>
  <summary>Details</summary>

**Motivation:** 研究表明，尽管强化学习提高了大型语言模型和多模态LLM的推理能力，但当前奖励设计过多强调最终答案的正确性，忽视了推理过程中的不准确可视化证据处理问题。为此，研究提出了PaLMR框架，旨在对齐结果输出和推理过程。

**Method:** PaLMR框架包括两个互补组件：一个对感知对齐数据层，用于构建具备结构化伪地面真相和可验证视觉事实的过程感知推理数据；另一个是过程对齐优化层，构建分层奖励融合方案和过程感知评分函数，鼓励视觉上忠实的思考链，提升训练稳定性。

**Result:** 实验表明，PaLMR在Qwen2.5-VL-7B模型上显著减少推理幻觉，提高视觉推理准确性，其在HallusionBench上达到了最先进的水平，同时在MMMU、MathVista及MathVerse上的表现依然强劲。

**Conclusion:** 研究结果显示PaLMR提供了一种原理性和实用性并存的路径，用于多模态推理的过程对齐改进，推动大型多模态模型的可靠性和解释性。
...
### [13] [A Parameter-efficient Convolutional Approach for Weed Detection in Multispectral Aerial Imagery](https://arxiv.org/abs/2603.06655)
*Leo Thomas Ramos,Angel D. Sappa*

Main category: cs.CV

> FCBNet是一种高效模型，用于杂草分割，表现优于现有的多种模型，且计算成本低。

<details>
  <summary>Details</summary>

**Motivation:** 为了设计一个高效的模型用于杂草分割，提出FCBNet。

**Method:** FCBNet采用了完全冻结的ConvNeXt主干网络，以及提出的特征修正模块（FCB），通过高效的卷积进行特征优化，结合轻量级解码器。

**Result:** 在WeedBananaCOD和WeedMap数据集上，FCBNet在RGB和多光谱模式下的mIoU超过85%，优于U-Net，DeepLabV3+，SK-U-Net，SegFormer和WeedSense等模型。

**Conclusion:** FCBNet具有优越的计算效率，训练时间仅为0.06到0.2小时，并且冻结主干网络策略减少了超过90%的可训练参数，大幅降低了内存需求。
...
### [14] [GameVerse: Can Vision-Language Models Learn from Video-based Reflection?](https://arxiv.org/abs/2603.06656)
*Kuan Zhang,Dongchen Liu,Qiyue Zhao,Jinkun Hou,Xinran Zhang,Qinlei Xie,Miao Liu,Yiming Li*

Main category: cs.CV

> 研究通过GameVerse基准和反思再试模式评估VLMs的学习能力，发现VLMs通过视频反思和结合失败轨迹及教程可以获得最佳效果。

<details>
  <summary>Details</summary>

**Motivation:** 探索VLMs是否能够通过视频为基础的反思学习，类似于人类游戏者通过观看失败和教程来改进策略。

**Method:** 使用GameVerse基准，以反思再试的方式评估VLMs的表现。同时提出一个层级分类法和高级VLM里程碑评估方法。

**Result:** 该研究介绍了GameVerse，这是一个全面的视频游戏基准，旨在使视觉语言模型（VLMs）能够通过视频反思来学习。与传统的即发即弃评估不同，它采用了反思再试的范式来评估VLMs如何通过视觉体验改进策略。该研究还提出了一种认知层级分类法，涵盖15个全球流行的游戏，一个用于语义和GUI控制的双重行动空间，以及使用高级VLM进行里程碑评估来衡量进步的方法。实验表明，VLMs在不同的环境下受益于视频反思，并且通过结合失败轨迹和专家教程进行训练可以取得最佳效果，类似于无需训练的强化学习（RL）加监督微调（SFT）的类比。

**Conclusion:** VLMs在不同设置中受益于视频反思，并且通过失败轨迹和专家教程的结合可以获得最佳效果。
...
> The paper introduces ASMIL, a method that stabilizes attention in attention-based multiple instance learning for WSI diagnosis, improving F1 scores up to 6.49% over state-of-the-art methods.

<details>
  <summary>Details</summary>

**Motivation:** To address the instability in attention dynamics, overfitting, and over-concentrated attention distributions in attention-based multiple instance learning methods for whole slide image diagnosis.

**Method:** Attention-stabilized multiple instance learning (ASMIL) uses an anchor model to stabilize attention, replaces softmax with a normalized sigmoid function to prevent over-concentration, and applies token random dropping to reduce overfitting.

**Result:** ASMIL achieves a 6.49% F1 score improvement over state-of-the-art methods, and integrating the anchor model and normalized sigmoid into existing methods boosts performance with F1 score gains up to 10.73%.

**Conclusion:** ASMIL is an effective approach for stabilizing attention in MIL methods for WSI diagnosis, improving their performance significantly.
...
> 提出EnsAug方法，通过战略性的使用数据增强来训练一组专家模型，实验表明此方法提升了人体动作识别任务的准确性。

<details>
  <summary>Details</summary>

**Motivation:** 一般的增强方法常常忽略人体的几何和运动约束，导致生成不合实际的动作模式，影响模型性能。此外，传统的单一模型训练方法没有充分利用每种独特增强类型提供的独特学习信号。

**Method:** 提出了一种新的训练范式EnsAug，该范式通过战略性的数据增强来促进模型多样性。方法涉及训练一组专家模型，每个模型仅学习来自单一独特几何变换增强的数据集。

**Result:** 实验在手语和人体动作识别基准上展示，此多元化集合方法显著超越了在结合增强数据集上训练单一模型的标准做法。

**Conclusion:** 该方法在两个手语识别和一个人类活动识别数据集上取得了最先进准确度，同时提供了更高的模块化和效率，验证了该训练策略的有效性。
...
> 本文介绍了HyperTokens技术，解决了连续视频问答任务中的问题，提高了整体准确性和记忆保留性。

<details>
  <summary>Details</summary>

**Motivation:** 解决连续视频问答任务中的任务间干扰以及存储任务特定提示的成本高昂问题。

**Method:** 提出HyperTokens，一种基于变压器的按需生成微调令牌的生成器，明确控制提示更新的同时保持内存固定。为抑制遗忘，提出了元启发的正则化器来避免特定任务的锋利方向，并将生成器锚定到先前的任务上。此外，HyperTokens通过共享的生成权重利用轻量级的多模态辅助监督。

**Result:** 在两个标准的连续视频问答基准上，HyperTokens取得了更高的平均准确率和大幅减少的遗忘。同时，在跨模态图像问答到视频问答协议中展示了稳健的连续迁移能力。

**Conclusion:** HyperTokens通过提出的方法显著提高了在连续视频问答任务中的性能，减少了任务间干扰和记忆遗忘问题。
...
> 该研究提出了一种新的像素级视觉提示技术Graph-of-Mark，通过在输入图像上覆盖场景图，显著提升了多模态语言模型的零样本空间推理能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有训练自由的视觉提示技术，如Set-of-Mark，虽然提升了多模态语言模型（MLMs）的接地能力，但未能捕捉对象间的相互关系。

**Method:** 我们提出了Graph-of-Mark (GoM)，这是一种像素级的视觉提示技术，它通过在输入图像上覆盖场景图来进行空间推理任务，解决了现有方法无法捕捉对象间关系的问题。

**Result:** 实验结果显示，GoM在三个开源MLMs和四个数据集上的零样本能力得到了显著提高，特别是在视觉问题回答和定位任务上，性能提升可达11个百分点。

**Conclusion:** 研究结论表明，Graph-of-Mark通过在图像上叠加场景图，有效改善了多模态语言模型在零样本条件下的空间推理能力，尤其在理解和解释对象的位置及相对方向上表现出色。
...
> 论文提出了一种系统级的推理优化方法，用于改进因果自回归框架中的视频生成，包括序列并行推理和Causal-RoPE SP嵌入的技术，大大提升了视频生成质量、降低了首帧延迟、提高了实时推理速度，适用于实时交互应用。

<details>
  <summary>Details</summary>

**Motivation:** 由于扩散Transformer（DiT）基础的视频生成模型在长视频合成和实时推理中存在瓶颈，特别是由于完全时空注意力机制导致的内存消耗指数增长（O(N^2)）和较高的首帧延迟问题，作者提出了优化方案。

**Method:** 该论文提出了一个针对因果自回归视频生成流水线的系统级推理优化，其中包括采用自强制因果自回归框架（Self-Forcing causal autoregressive framework）进行序列并行推理，并实现了一种因果旋转变位嵌入的序列并行变体Causal-RoPE SP。这使得计算本地化减少了序列并行执行中的跨级通信。此外，还通过运算符融合和RoPE预计算优化了计算和通信流水线。

**Result:** 在八GPU A800集群上进行的实验表明，优化后的系统在视频生成质量上具有可比性，实现了亚秒级首帧延迟和接近实时的推理速度。生成5秒480P视频时，实现了1.58倍的速度提升。

**Conclusion:** 通过系统级优化推理，实现了因果自回归视频生成模型中的高效实时生成。实验结果验证了该方法在长视频合成、实时推理速度和交互应用方面的有效性。
...
> 研究揭示了一种在医疗视觉语言任务中链条思维通常不如直接回答的现象，并提出通过感知锚定和描述定位干预措施来改善模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 研究发现视觉-语言模型在医疗图像问题解答中，链条思维提示通常不如直接回答，这可能是因为医疗图像中有微妙特定的线索削弱了视觉定位。

**Method:** 本研究提出两种无需训练的推理时介入措施以增强视觉定位：(i) 通过感兴趣区域线索的感知锚定，以及(ii) 通过高质量文本指导的描述定位。

**Result:** 这两种干预措施提高了多个基准测试和模型系列的准确性，减轻了CoT性能下降，并在某些设置下逆转了CoT--直接回答的倒置表现。

**Conclusion:** 研究表明，可靠的临床视觉-语言模型需要超越扩展文本驱动推理链，达到稳健的视觉定位和跨模态对齐。
