...
> 本文创建了第一个多模态声明提取基准，发现现有MLLM在处理社交媒体多模态内容时存在不足，并提出了意图感知框架MICE来改善性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有自动事实核查方法主要关注文本声明提取，忽视了社交媒体中普遍存在的多模态（文本+图像）虚假信息，这类内容具有独特的挑战性。

**Method:** 1) 构建包含社交媒体文本和图像的多模态声明提取基准数据集；2) 提出三部分评估框架（语义对齐、忠实度、去语境化）；3) 设计意图感知框架MICE来更好地理解修辞意图和语境线索。

**Result:** 基准测试显示，现有最先进的多模态大语言模型在建模修辞意图和语境线索方面表现不佳；MICE框架在意图关键案例中显示出改进效果。

**Conclusion:** 社交媒体多模态声明提取是一个新的挑战，现有的MLLM需要更好的意图理解能力；提出的基准和MICE框架为未来的研究提供了重要基础。
...
> 本研究扩展了MLX-LM框架，首次在Apple Silicon上实现了跨词元器族的投机解码，发现上下文感知翻译能显著提高接受率，但在统一内存架构下顺序草稿成本较高，加速效果受内容类型限制。

<details>
  <summary>Details</summary>

**Motivation:** 传统投机解码在相同词元器对和高带宽GPU上有效，但对跨家族词元器不匹配和消费级统一内存环境的适用性尚未充分探索。波兰语大语言模型缺乏跨家族投机解码的系统性评估。

**Method:** 在Apple Silicon上实现通用辅助生成(UAG)，使用Bielik 11B-Instruct作为目标模型，与三个不同架构的草稿模型配对，在三个波兰语数据集上评估不同草稿长度和两种词元翻译策略。

**Result:** 上下文感知翻译在所有配置中都能提高接受率；专业化的波兰语模型接受率反而低于通用模型；在结构化文本上可达1.7倍加速，但多样化指令下无法加速；统一内存下验证成本无法摊销。

**Conclusion:** 这是首个针对波兰语LLM的跨家族投机解码系统评估，也是首个基于UAG的统一内存架构实证研究，提出了硬件感知加速公式并量化了在Apple Silicon上实现跨家族投机解码的条件。
...
> 本文提出Brain-CLIPLM框架，通过将EEG到文本的解码分解为语义锚点提取和基于检索的大语言模型重构，在EEG信号上实现了67.55% top-5和85.00% top-25的句子检索准确率。

<details>
  <summary>Details</summary>

**Motivation:** 非侵入式脑电图(EEG)存在信噪比低和信息带宽受限的问题，使得从EEG中直接恢复完整句子级的语言结构变得困难。研究质疑了这种假设的可行性，提出了EEG信号可能只编码压缩的语义锚点而非完整语言结构的新观点。

**Method:** 提出Brain-CLIPLM两阶段框架：第一阶段通过对比学习从EEG中提取语义锚点；第二阶段使用基于检索的大语言模型(LLM)结合思维链(Chain-of-Thought)推理进行句子重构，遵循粒度匹配原则对齐解码复杂性与神经信息容量。

**Result:** 在苏黎世认知语言处理语料库上达到67.55% top-5和85.00% top-25句子检索准确率，显著优于直接解码基线，跨被试评估证实了鲁棒泛化能力。控制分析表明EEG衍生的表征携带超越语言模型先验的句子特定信息。

**Conclusion:** EEG到文本的解码应该被重新定义为恢复压缩的语义内容而非重构完整句子，这为非侵入式脑机接口提供了具有生物学基础且数据高效的路径。
...
> 本研究构建了首个中文细粒度多模态讽刺数据集CFMS，包含三层标注框架，并提出强化学习增强的上下文学习策略PGDS，显著提升了多模态讽刺理解效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态讽刺检测基准存在标注粒度粗、文化覆盖有限等问题，阻碍了细粒度语义理解研究的发展，急需构建更高质量、更贴合中文社交媒体的讽刺数据集。

**Method:** 构建CFMS数据集，包含2796个高质量图文对，提供讽刺识别、目标识别和解释生成三层标注；提出PGDS策略，通过强化学习动态优化示例选择，克服传统检索方法的限制。

**Result:** 细粒度解释标注能有效指导AI生成具有明确讽刺意图的图像；构建的中英隐喻子集揭示了当前模型在隐喻推理方面的显著不足；PGDS方法在关键任务上显著超越现有基线。

**Conclusion:** CFMS为构建可靠的多模态讽刺理解系统提供了坚实基础，PGDS策略有效提升了模型性能，该研究为细粒度多模态讽刺理解开辟了新方向。
...
> 本研究探索了基于文体特征的身份归属分析在威胁情报行动者分析中的应用，发现BERT微调在少数量作者时效果最佳，但随着作者数量增加到数百人时，TF-IDF+LR方法在准确性、稳定性和计算成本方面表现更优。

<details>
  <summary>Details</summary>

**Motivation:** 为威胁情报中的行动者分析提供技术支持，特别是为未来应用于暗网论坛的身份归属分析奠定基础，需要探索适合大规模作者的文体特征分析方法。

**Method:** 使用日本乐天市场的评论数据构建数据集，比较四种方法：TF-IDF+逻辑回归、BERT嵌入+逻辑回归、BERT微调、度量学习+k近邻，并进行了Top-k评估和误差分析。

**Result:** BERT-FT在小规模作者时表现最佳，但数百名作者时训练不稳定；TF-IDF+LR在准确性和稳定性方面更胜一筹；Top-k评估证明了候选筛选的有效性；标准文本、主题依赖性和短文本是主要误分类因素。

**Conclusion:** 基于文体特征的身份归属分析在威胁情报行动者分析中具有应用潜力，方法选择需要根据作者规模进行权衡，BERT-FT适合小规模场景，TF-IDF+LR更适合大规模实际应用。
...
> GoCoMA是一个多模态框架，通过结合代码风格学和二进制预执行可执行文件的图像表示，在双曲空间中融合特征来实现大型语言模型代码来源的准确识别。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型语言模型生成的代码越来越难以与人类编写的代码区分，这带来了安全漏洞和许可模糊性等实际问题，同时也提出了'谁（或哪个LLM）写了这段代码'的取证问题。

**Method:** GoCoMA构建了一个多模态层级框架，包括：(i)代码风格学（捕获高层次结构和风格特征），(ii)二进制预执行可执行文件的图像表示（捕获编译和工具链塑造的低层次字节语义）。框架将模态嵌入投影到双曲Poincaré球中，通过基于大地线-余弦相似度的跨模态注意力融合机制融合特征，最后将融合表示反投影回欧几里得空间进行LLM来源识别。

**Result:** 在CoDET-M4和LLMAuthorBench两个开源基准测试上，GoCoMA在相同评估协议下持续优于单模态和欧几里得多模态基线方法。

**Conclusion:** 该研究证明了双曲空间多模态融合在LLM代码来源识别任务中的有效性，为代码取证和来源追踪提供了新的技术途径。
...
> 本文提出了一个通过强化学习耦合大语言模型和随机森林分类器的互惠协同训练框架，实现了两种异构模型的相互促进和性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型(LMMs)和传统机器学习方法具有互补优势，但它们根本不同的表示方法和训练范式阻碍了有效集成：LLMs基于文本数据进行梯度优化，而随机森林等模型采用不可微分的特征分割。

**Method:** 建立了一个互惠协同训练框架：将表格数据重构为标准化的文本表示供LLM使用，其嵌入向量增强RF的特征空间；校准后的RF概率估计提供反馈信号，指导LLM的强化学习更新。创建了一个迭代反馈循环，每个模型利用来自另一个模型的信号进行改进。

**Result:** 在三个医学数据集上的实验表明，两种模型都实现了一致的性能提升，对LLM的效果尤为显著。消融分析显示，迭代优化、混合奖励设计和维度控制共同促成了这些性能提升。

**Conclusion:** 提出的框架提供了一种通用机制，使不兼容的模型族能够通过双向适应相互利用优势，为异构模型集成开辟了新途径。
...
> 这篇论文对大语言模型预训练中的数据混合方法进行了系统性综述，将现有方法分为静态和动态两大类，分析了各类方法的优缺点，并指出了当前挑战和未来研究方向。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型需要在海量异质语料库上进行预训练，训练数据组成对训练效率和下游泛化能力具有决定性影响。数据混合通过优化领域级采样权重来有效分配有限的计算和数据预算，但现有文献分散且缺乏系统性综述。

**Method:** 首次将数据混合优化形式化为概率单纯形上的双层问题，提出细粒度分类框架：静态混合（基于规则和基于学习）和动态混合（自适应和外部引导）。从性能-成本权衡角度分析每类代表性方法的优缺点。

**Result:** 系统性梳理了LLM预训练数据混合的研究现状，揭示了跨数据领域、优化目标、模型和验证集的迁移性有限、评估协议和基准标准化不足、性能增益与成本控制的内在矛盾等关键挑战。

**Conclusion:** 为未来研究提供了概念和方法学见解，提出了更细粒度领域划分、逆数据混合以及流水线感知设计等探索方向，旨在促进该领域的标准化和进一步发展。
...
> LiFT是一个纵向指令微调框架，通过统一的多任务指令模式和渐进式时序难度课程，显著提升大语言模型在纵向NLP任务上的性能，特别是在处理分布外数据和少数变化事件方面表现出色。

<details>
  <summary>Details</summary>

**Motivation:** 现有的纵向NLP任务需要模型对时序文本进行推理，以检测人类行为和观点的持续性和变化。然而，大语言模型的上下文学习方法在整合历史上下文、追踪演化互动和处理稀有变化事件方面存在困难。

**Method:** LiFT框架采用统一的指令模式整合多样化的纵向建模任务，使用渐进式增加时序难度的课程学习，并结合少样本结构和时序条件化来促进对过去上下文的有效利用。该框架在五个数据集上进行了评估。

**Result:** LiFT在所有测试模型(OLMo 1B/7B、LLaMA-8B、Qwen-14B)上均显著超越基准模型的上下文学习效果，在分布外数据和少数变化事件上表现出强劲的性能提升。

**Conclusion:** LiFT通过创新的纵向指令微调方法，有效解决了大语言模型在纵向NLP任务中的关键挑战，为时序文本理解和变化检测提供了新的解决方案。
...
> 本研究通过两阶段QLoRA微调策略，在小型语言模型上实现了伊斯兰遗产法复杂推理任务，取得90%的测试分数

<details>
  <summary>Details</summary>

**Motivation:** 伊斯兰遗产法（ilm al-mawarıth）是一个极具挑战性的领域，需要多步法律分析、规则阻断决策和精确的分数计算，为评估大型语言模型的结构化推理能力提供了理想的测试场景

**Method:** 采用多阶段量化低秩适应（QLoRA）微调策略，先对Qwen3-4B模型进行领域适应性训练（3,166份伊斯兰教法判例记录，获得遗产术语和法学推理模式），再进行任务特定训练（12,000个结构化遗产案例，优化JSON格式输出生成），使用4位NF4量化和秩128的LoRA适配器

**Result:** 在测试集上获得90%的MIR-E（遗产继承推理评估）分数，在最小计算资源需求下展现出竞争力，与Gemini-2.5-flash等商业系统相比效果相当

**Conclusion:** 领域特定的预适应训练结合结构化输出训练，使得小型语言模型能够有效执行复杂的法律推理任务，为计算资源受限场景下的法律AI应用提供了可行方案
...
> 本研究提出了GeoRepEval评估框架，揭示了大型语言模型在几何推理中存在显著的表示敏感性问题，相同数学问题的不同表示形式会导致准确率差距达14个百分点。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM在数学推理评估中缺乏对等效问题表示鲁棒性的理解，现有基准测试采用固定格式，隐含假设了表示不变性，掩盖了仅由表示变化引起的失效问题。

**Method:** 构建GeoRepEval表示感知评估框架，通过158个精心策划的高中几何问题(474个实例)的欧几里得、坐标和向量三种平行形式，使用严格答案匹配、bootstrap置信区间、配对McNemar测试等方法评估正确性、不变性和一致性。

**Result:** 发现表示选择单独导致的准确率差距高达14个百分点，向量形式成为一致的失效点，Invariance@3指标低至0.044。转换后求解的提示干预可将向量准确率提高52个百分点，但仅对大容量模型有效。

**Conclusion:** 当前模型依赖特定表示启发式而非抽象几何推理，表明模型缺乏真正的数学理解能力，需要改进表示鲁棒性评估。
...
> 该研究探索了利用UMLS结构化知识增强生物医学语言模型的两种策略：知识嵌入到模型参数的持续预训练，以及在推理时查询知识图的图检索增强生成(GraphRAG)

<details>
  <summary>Details</summary>

**Motivation:** 将领域特定知识注入语言模型对于适应生物医学等专业领域至关重要，但现有方法主要依赖非结构化文本语料，未能充分利用UMLS等结构化知识库的价值

**Method:** 首先从UMLS构建包含340万概念和3420万关系的大规模生物医学知识图谱并存储于Neo4j；然后衍生约1亿token文本语料对BERT和BioBERT进行持续预训练得到BERTUMLS和BioBERTUMLS；最后设计GraphRAG流程让LLaMA 3-8B在推理时查询知识图谱

**Result:** BERTUMLS在BLURB基准测试中超越原BERT，特别是在知识密集型问答任务上提升明显；GraphRAG使LLaMA 3-8B在PubMedQA和BioASQ问答任务上分别提升超过3个点和5个点准确率，且无需重新训练

**Conclusion:** 结构化知识注入为生物医学语言模型带来实质性能提升，GraphRAG提供了透明、多跳且易更新的知识访问方式，两种策略互补适用，研究已发布处理后的UMLS Neo4j图谱供复现
...
> HalluSAE是一个受相变启发的大模型幻觉检测框架，通过分析模型潜在动力学的临界转变来识别和归因事实错误，在Gemma-2-9B上达到了最先进的检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型幻觉检测方法忽视了幻觉现象的动态本质和底层机制，无法有效捕捉幻觉产生的根本原因。

**Method:** 提出三阶段框架：1) 基于稀疏自编码器和几何势能度量的相变区域定位；2) 使用对比logit归因的幻觉相关稀疏特征归因；3) 基于解耦特征线性探针的因果幻觉检测。将生成过程建模为在势能景观中的轨迹，识别临界转变区。

**Result:** 在Gemma-2-9B上的大量实验表明，HalluSAE在幻觉检测任务上实现了最先进的性能，成功将事实错误归因于特定的高能稀疏特征。

**Conclusion:** 通过将相变理论引入幻觉检测，HalluSAE提供了一种理解幻觉动态本质的新视角，为大模型事实性检测提供了有效的解决方案。
...
> 该研究提出了SynopticBench数据集和SPACE评估框架，用于评估视觉语言模型在天气现象文本生成任务中的表现，解决了现有模型在气象数据文本生成方面缺乏有效评估的问题。

<details>
  <summary>Details</summary>

**Motivation:** 大气是混沌系统，空间和时间尺度变化快速，从气象数据生成文本极具挑战性。现有视觉语言模型（VLMs）在图像字幕等任务表现良好，但在天气预测领域缺乏有效的质量评估标准，迫切需要专门的评估方法和数据集。

**Method:** 构建了包含136.7万个样本的SynopticBench数据集，将美国国家气象局发布的区域预测讨论文本与500mb位势高度、2米温度和850mb风速的预报图像配对。同时提出了天气现象对齐和覆盖度评估（SPACE）框架，用于有效评估天气现象文本描述的质量。

**Result:** 通过对先进VLMs在生成预测讨论方面的广泛实验，揭示了现有评估指标在该领域的敏感性，为天气和气候文本生成的进一步探索提供了基础。

**Conclusion:** 该工作填补了VLMs在气象文本生成评估领域的空白，为量化评估模型在复杂天气现象描述中的表现提供了重要工具和数据支撑。
...
> EchoChain是一个评估全双工语音助手在用户中断对话时状态更新能力的基准测试，揭示了现有系统在上下文惯性、中断遗忘和目标偏移三个方面的严重缺陷。

<details>
  <summary>Details</summary>

**Motivation:** 现有语音对话基准主要评估轮式交互，而忽略了实时语音助手在用户中途打断响应时的状态更新失败问题，这是一个关键的交互场景。

**Method:** EchoChain通过生成场景驱动的对话并在助手语音开始后的标准化点注入中断，创建受控的评估环境。设计了半双工对照组来分离状态更新推理与任务难度的影响。

**Result:** 所有评估的实时语音模型通过率均低于50%，与半双工对照组相比，中断情况下的总失败率增加了40.2%，表明多数错误源于中断状态下的推理而非任务难度。

**Conclusion:** EchoChain为全双工语音交互中的状态更新推理失败提供了可复现的诊断基准，显示该领域存在巨大改进空间。
...
> SemanticQA是一个评估语言模型语义短语处理能力的基准测试套件，通过整合现有多词表达资源，构建了统一的测试平台来评估不同架构和规模的语言模型。

<details>
  <summary>Details</summary>

**Motivation:** 为了系统评估语言模型在语义短语处理方面的能力，特别是在需要语义推理的任务上的表现差异，从而推动语言模型在复杂语义理解方面的发展。

**Method:** 整合现有的多词表达(MwE)资源，重新组织为统一测试平台，涵盖一般词汇现象(如词汇搭配)和三个细分类别：习语表达、名词复合词和动词结构。通过提取、分类、解释任务以及序列任务组合来评估多样化的语言模型。

**Result:** 揭示了不同语言模型在语义短语处理任务上存在显著性能差异，特别是在需要语义推理的任务上，反映了模型在推理能力和语义理解方面的差异。

**Conclusion:** SemanticQA为评估和改进语言模型的语义理解能力提供了重要洞察，有助于推动开发具有更强语义短语理解能力的下一代语言模型。
...
> 本文对15种机器生成文本检测模型进行系统性评估，发现没有单一模型在所有场景下表现最优，模型性能高度依赖于数据集和评估指标的选择，在高风险领域的创新人类文本检测上表现普遍较差。

<details>
  <summary>Details</summary>

**Motivation:** 随着生成式语言模型的兴起，机器生成文本检测成为关键挑战，但现有的模型在数据集、评估指标和评估策略方面存在不一致性，导致难以准确比较模型效果。

**Method:** 评估了来自6个不同系统的15种检测模型和7个训练模型，在7个英语文本测试集和3个创意人类写作数据集上进行测试，对模型性能、训练评估数据影响和关键指标影响进行实证分析。

**Result:** 研究发现没有单一系统在所有领域都表现卓越，几乎所有模型在特定任务上都有效，模型性能表现与数据集和指标选择密切相关；不同数据集和指标下模型排名差异很大，在高风险领域的创新人类文本上整体表现较差。

**Conclusion:** 方法选择（通常被忽略或假设）对于清晰准确地反映模型性能至关重要，建立标准化的评估框架和指标体系对机器生成文本检测领域的发展具有重要意义。
...
> 该论文提出一个两阶段框架，通过微调大型语言模型和联合嵌入学习，研究反馈信号的词汇-韵律形式与语用意义的关系，证明了上下文敏感性和与人类感知的更好对齐。

<details>
  <summary>Details</summary>

**Motivation:** 反馈信号在对话中具有重要语用功能，但现有研究主要关注时间预测，忽略了词汇-韵律形式与含义之间的关系，需要建立更深入的语义理解模型。

**Method:** 采用两阶段框架：首先在对话语料上微调大型语言模型获得上下文表示，然后学习对话上下文与反馈信号实现的联合嵌入空间，通过三元组相似性判断和适合性任务进行评估。

**Result:** 学习到的投影显著提升了上下文-反馈信号的检索效果，发现反馈信号形式对扩展对话上下文高度敏感，且嵌入表示比原始WavLM特征更接近人类判断。

**Conclusion:** 该方法成功建立了反馈信号形式与语用meaning之间的联系，证明了上下文信息的重要性，为对话系统中反馈信号的生成和理解提供了新的技术途径。
...
> 本文提出了AdaExplore框架，能够通过积累执行反馈自主学习改进并生成性能关键的内核代码，无需额外微调或外部知识。

<details>
  <summary>Details</summary>

**Motivation:** 现有大型语言模型代理在测试时自适应方面存在局限性：独立处理问题实例而无法积累可重用知识；在特定领域语言如Triton中表现不佳，其严格约束和非线性优化空间使得简单的生成和局部细改进不可靠。

**Method:** AdaExplore框架包含两个互补阶段：1）故障驱动自适应阶段，合成任务并将重复故障转换为可重用的有效性规则记忆；2）多样性保持搜索阶段，将候选内核组织为树状结构，交替进行局部细改进和结构重生成，探索超越局部最优的优化空间。

**Result:** 在内核运行时优化基准测试中，AdaExplore在100步内分别实现了KernelBench Level-2和Level-3的3.12倍和1.72倍加速，并能随着计算量增加而持续改进。

**Conclusion:** AdaExplore通过积累执行反馈实现了有效的自我改进机制，在特定领域代码生成任务中展现出优异的性能优化能力，为LLM代理的知识积累和自适应学习提供了新的解决方案。
...
> 本研究通过焦点小组和NLP技术分析了个欧洲城市法语移民的本地新闻消费习惯，发现现有新闻报道与移民需求存在脱节。

<details>
  <summary>Details</summary>

**Motivation:** 研究关注非主流受众在本地媒体消费方面的研究空白，特别探讨了法语移民群体对本地新闻的参与度及其需求是否得到满足。

**Method:** 采用焦点小组访谈深入了解8位社区成员的观点，然后据此选择自然语言处理方法（主题建模、信息检索、情感分析和可读性分析）对2000多篇超本地新闻文章进行分析。

**Result:** 虽然本地新闻经常报道本地活动，但在参与者关注的重要话题上存在缺口；情感分析显示整体语调积极；可读性测量表明需要中级到高级法语水平，引发了对移民融入可访问性的担忧。

**Conclusion:** 该研究有助于弥合本地新闻平台内容与多元化读者需求之间的差距，可为本地媒体组织提供扩展新闻报道覆盖面以吸引更多元受众的参考。
...
> 研究发现边缘化社群对'回收使用的侮辱性词语'(reclaimed slurs)在网上的解释存在显著差异，导致自动化内容审核工具难以区分其是表达团结还是仇恨言论。

<details>
  <summary>Details</summary>

**Motivation:** 边缘化社群(LGBTQIA+、黑人、女性社区)在网上使用'回收的侮辱性词语'是建立团结、身份认同和共同体验的重要方式，但现有的AI内容审核工具无法区分回收使用和仇恨使用，从而压制了边缘化群体的声音。

**Method:** 通过定量和定性方法，收集和分析包含了f-word、n-word、b-word等侮辱性词语的标注语料库，记录标注者对文本是否应标记为仇恨言论的判断，以及使用语境特征。同时进行半结构化访谈。

**Result:** 在所有社群和标注问题中都观察到标注者间一致性较低，即使在同一社群内部成员之间也存在显著分歧。标注者判断与Perspective API的自动化仇恨言论评估结果对齐度较差。

**Conclusion:** 边缘化社群对网上侮辱性词语的解释具有固有的主观性和语境依赖性，涉及生活经历和个人历史差异，这表明需要更加细致和情境感知的内容审核方法。
...
> 这篇论文研究了解决LLM中非拉丁字母语言'词元过度碎片化'问题的可解释性词汇扩展方法，提出了FragMend算法，显著提升了非拉丁字母语言的词元效率。

<details>
  <summary>Details</summary>

**Motivation:** 现代大型语言模型在处理非拉丁字母语言时存在严重的词元过度碎片化问题，导致这些语言需要比英语多倍的词元来编码相同信息，增加了使用成本和延迟。传统的词汇扩展方法效果有限。

**Method:** 1) 重新评估词汇扩展的核心决策：引入基于可解释性的方法代替传统的频率方法选择候选词汇；2) 提出基于可解释性的嵌入初始化方法；3) 发现并分析'子词去分词化'现象；4) 基于此现象提出FragMend算法进一步优化效率。

**Result:** 基于可解释性的词汇扩展方法在性能-词元效率权衡上优于传统方法；嵌入初始化方法在非拉丁字母语言上带来约20个百分点的显著提升；FragMend算法在多个强基线上验证了有效性。

**Conclusion:** 通过深入的可解释性分析和算法创新，该工作为解决LLM在非拉丁字母语言中的词元效率问题提供了新的有效方案，推动了词汇扩展研究的发展。
...
> CBRS是一个高效过滤和解析社交媒体上紧急献血请求的多平台系统，使用双层架构达到99%的过滤准确率和92%的零样本解析准确率，相比现有方法有显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 传统基于应用的求助系统依赖手动输入，在低资源环境中效果有限，而社交媒体上的紧急献血请求信息常被海量信息淹没，导致关键响应延迟。

**Method:** 提出了CBRS多平台框架，采用成本效益高的双层架构；收集了包含孟加拉语、英语和音译孟加拉语的11K条献血请求数据集；使用LoRA微调Llama-3.2-3B模型进行解析任务。

**Result:** 过滤任务达到99%的准确率和精度；解析任务达到92%的零样本准确率，比基础模型提升41.54%，超越了GPT-4o-mini等先进模型的少样本性能，输入token使用减少35倍。

**Conclusion:** 该工作为时间敏感、目标聚焦的任务提供了可扩展且包容的信息提取基础，代码、数据集和训练模型已公开。
...
> NWCAD是一种解码时适配器，能够防止大语言模型在非信息性上下文中出现中性回归，同时保持对信息性上下文的强利用能力。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在使用外部上下文时存在中性回归问题：即使上下文是非信息性的，模型也可能覆盖原本正确的答案，违反了不造成伤害的原则。

**Method:** 提出了NWCAD（No-Worse Context-Aware Decoding），基于双流设置和两级门控机制。当上下文非信息性时回退到无上下文解码，其他情况使用上下文条件解码，并在不确定性下采用CAD式回退。

**Result:** NWCAD在分离无害可靠性和上下文利用的基准测试中表现出色：防止了基线正确项目的中性回归，同时保持了在有用上下文下的强上下文驱动准确性。

**Conclusion:** 该解码时适配器有效平衡了上下文利用和无害性，为大语言模型更可靠的上下文使用提供了实用解决方案。
...
> 研究表明，ChatGPT-4虽能加速文学翻译，但在处理文化、风格和修辞方面仍有局限，需人工校对，形成人机协作模式。

<details>
  <summary>Details</summary>

**Motivation:** 探讨人工智能（特别是ChatGPT-4）在文学翻译中的作用，以及人工后期编辑的必要性程度。

**Method:** 采用混合方法研究，30名专业翻译人员评估并对AI生成的阿拉伯语和英语文学文本翻译进行后期编辑。

**Result:** AI提高了翻译速度和可及性，但在处理文化、风格和比喻性语言方面有限；参与者普遍确认人工后期编辑的必要性，尤其在小说和戏剧翻译中。

**Conclusion:** AI应作为辅助工具使用，人类专业知识对于确保翻译质量和文化适宜性仍然至关重要，形成新兴的人机协作模式而非完全替代人工翻译。
...
> 本文提出了一个混合框架，结合文本分类器和音频分类器来自动化检测危机学生的回应，通过内容分析和韵律标记双重方法提升识别准确性。

<details>
  <summary>Details</summary>

**Motivation:** 传统自动口语评分系统(AVRS)存在安全漏洞，无法有效识别可能需要紧急干预的危机学生回应，这在及时干预至关重要时可能危及生命。

**Method:** 开发了一个混合检测系统，包含两个核心组件：基于内容检测的文本分类器和基于韵律标记检测的音频分类器，两者协同工作识别危机学生的回应。

**Result:** 该系统克服了传统AVRS仅考虑内容或韵律单一维度的限制，通过双模态分析显著提升了对潜在危险回应的识别性能。

**Conclusion:** 提出的混合框架能够有效加速人工审核过程，在需要及时干预的紧急情况下具有救生潜力，为教育安全评估提供了重要技术支持。
...
> 该研究提出了一个基于理论的模拟学习者框架，用于评估教育阅读材料的自适应个性化效果，通过本体构建、记忆建模和BKT自适应机制，在计算机科学领域显示显著效果，在其他学科效果不一。

<details>
  <summary>Details</summary>

**Motivation:** 开发一个理论驱动的评估框架，用于测试教育阅读材料自适应个性化系统的效果，以验证不同学科领域中个性化学习的实际价值。

**Method:** 构建学习目标和知识组件本体，使用浏览器本体图集管理，标记教材文本，生成阅读-评估对；通过建构整合记忆模型和DIME读者因素模拟学习过程，使用BKT驱动自适应调整。

**Result:** 自适应阅读在计算机科学领域显著改善学习效果，在无机化学领域有小幅但不明显的增益，在普通生物学领域效果中性或略为负面。

**Conclusion:** 该框架为教育阅读材料的自适应个性化提供了有效评估工具，但自适应效果存在学科差异性，需要进一步研究不同学科领域的个性化策略优化。
...
> 研究表明，大型语言模型在跨文化社交情感表达方面存在系统性偏差，过度表达参与性情感而忽视脱离性情感，且无法捕捉人类情感表达的多样性。

<details>
  <summary>Details</summary>

**Motivation:** 随着LLM越来越多地用于模拟文化细微差异的人际互动，了解它们是否忠实地捕捉人类社交情感表达模式至关重要。当LLM响应不符合文化背景时，其实用性会受到影响，特别是在用户认为与文化敏锐对话者互动而根据可能在其文化背景下不合适的建议采取行动时。

**Method:** 研究人员提出了一个心理学的跨文化社交情感表达评估框架。通过比较欧美和拉丁美洲参与者表达参与性和脱离性情感的人类研究，评估了六个前沿LLM反映文化差异化社交情感表达模式的能力。

**Result:** 发现模型与人类行为之间存在系统性不匹配：所有模型表达参与性情感多于脱离性情感，对于通常表现良好的欧美人格角色差异尤为显著。LLM响应高度集中和决定性，无法捕捉人类社交情感表达的多样性。消融分析显示这些模式对采样温度具有鲁棒性，对提示语言部分敏感，并依赖于响应引导格式。

**Conclusion:** 研究结果表明当前LLM在表达社交情感时，在表现文化和情感维度互动方面存在局限，这对其在跨文化情感环境中的部署具有直接影响。
...
> 本文论证音频虚假信息具有口语化和对话化的双重特性，现有的事实核查流程主要针对文本内容，无法有效处理音频媒体带来的验证挑战。

<details>
  <summary>Details</summary>

**Motivation:** 音频平台已成为公共话语的重要渠道，承载着数百万节目和数亿听众，使其成为虚假信息的主要传播媒介。然而，现有的事实核查管道主要针对书面声明，忽视了口语媒体的独特性质。

**Method:** 通过综合跨模态和平台的证据，审视现有数据集和方法，分析现有流程在音频验证方面的失败原因，论证需要围绕音频的口语化和对话化现实重新设计验证流程。

**Result:** 揭示了音频虚假信息与文本内容的根本差异：其口语特性（通过韵律、节奏和情感产生说服力）和对话特性（在回合、发言者和剧集间展开）带来了传统方法难以应对的验证困难。

**Conclusion:** 推动事实核查发展需要重新思考验证流程，围绕音频的口语化和对话化现实进行设计，以有效应对音频虚假信息的特殊挑战。
...
> StageMem是一个生命周期管理的记忆框架，将记忆分为瞬态、工作态和持久态三个阶段，通过置信度和强度建模，实现了从浅层存储到长期承诺的渐进式记忆管理。

<details>
  <summary>Details</summary>

**Motivation:** 当前语言模型系统的持久记忆设计过于静态化，无法解决实际部署中的记忆控制问题，包括保留过多不确定信息、遗忘顺序错误、用户缺乏对信息持久性的信任等。

**Method:** 提出StageMem框架，将记忆组织为瞬态、工作态和持久态三个阶段，为每个记忆项建模置信度和强度，实现低成本写入、后续升级、保留、更新或淘汰的动态管理。

**Result:** 在受控压力机制下，StageMem能够保护后期重要内容，同时控制记忆负担和深层污染，并通过外部任务验证了与更强检索结构的兼容性。

**Conclusion:** StageMem为语言模型系统的记忆控制问题提供了原则性的分解方案，将记忆从被动存储转变为状态化过程，更贴近实际应用需求。
...
> 研究发现非正式语言形式（俚语、emoji、Gen-Z填充词）会显著降低NLI模型准确率，其中emoji和噪声填充词影响最大，通过预处理和增强训练的混合方法可有效提升模型在非正式文本上的表现，甚至超越GPT-4o-mini。

<details>
  <summary>Details</summary>

**Motivation:** 研究非正式语言形式对自然语言推理准确率的影响，因为现实世界中用户经常使用俚语、emoji等非正式表达，而这些可能影响预训练模型的性能。

**Method:** 在SNLI和MultiNLI数据集上应用四种转换（俚语替换、emoji替换、Gen-Z填充词及其组合），测试ELECTRA-small和RoBERTa-large的表现，并探索预处理和增强训练的补救方法。

**Result:** 俚语替换影响最小（最多1.1pp），emoji导致93.6%样本出现[UNK]标记，噪声填充词被模型错误赋予推理权重。混合方法将ELECTRA在组合变体上的准确率从75.88%提升至88.93%，且不降低干净文本性能。

**Conclusion:** 不同类型的非正式语言需要不同的干预策略：预处理解决emoji问题，增强训练处理噪声词，混合方法最有效，能使专门训练的小型模型在非正式文本上优于大型通用模型。
...
> 该研究发现了LoRA适配器合并中的性能下降主要来源于输出矩阵B的方向共享问题，提出了Pico方法通过校准B矩阵来解决这个问题。

<details>
  <summary>Details</summary>

**Motivation:** 联合多任务训练成本高昂，而合并单独训练的LoRA适配器是一个实用替代方案，但现有方法常常导致性能下降，亟需改进。

**Method:** Pico是一个数据无关的方法，在合并前通过降缩过度共享的方向来校准输出矩阵B，然后在合并后重新缩放更新，可插入现有合并方法中。

**Result:** 在数学、编程、金融和医疗等八个基准测试中，Pico相比基础方法提升了3.4-8.3个百分点的平均准确率，并获得了最佳总体性能，甚至超越了使用所有任务数据训练的LoRA。

**Conclusion:** 结果表明，当分开处理两个LoRA矩阵时，LoRA合并工作得更好，Pico方法有效解决了合并干扰问题，实现了性能提升。
...
> HeLa-Mem是一种受生物记忆启发的动态图结构记忆架构，通过双重记忆组织和Hebbian学习机制，显著提升大语言模型记忆效率。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型代理面临长期记忆挑战，现有基于向量相似性的检索方法无法模拟人脑记忆的联想结构，缺乏关联、巩固和传播激活三种核心生物记忆机制。

**Method:** 提出HeLa-Mem架构，采用双重组织：1）通过共激活模式演化的情景记忆图；2）通过Hebbian蒸馏填充的语义记忆存储。使用反思代理识别密集连接的记忆枢纽并提炼为结构化语义知识。

**Result:** 在LoCoMo数据集实验中，HeLa-Mem在四个问题类别上均表现优异，同时使用更少的上下文token。

**Conclusion:** 该研究成功将认知神经科学原理融入AI记忆系统，通过结合语义相似性和学习关联，实现了更接近人类认知的情景-语义双重记忆路径设计。
...
> 研究提出DART方法解决安全调优的大型语言模型在群体差异识别问题上的身份盲视现象，在提高准确率的同时显著减少有害内容漂移。

<details>
  <summary>Details</summary>

**Motivation:** 为安全而调优的LLMs往往错误地忽略人口群体差异，即使这种差异有事实依据或语境需要，导致回答错误、不当拒绝或泛化的'平等对待'默认回应，影响应用效果。

**Method:** 提出DART（Distill--Audit--Repair Training）框架：首先从教师模型中蒸馏条件推理，然后审计输出以检测相对于基线的有害漂移案例，最后通过严重性加权的微调修复问题案例。

**Result:** 在八个基准测试中，DART将Llama-3-8B-Instruct准确率从39.0%提升至68.8%，平等对待提示从11.3%提升至72.6%，有害漂移案例减少72.6%。在280个开放查询上，差异适当回应从39.8%提升至77.5%，拒绝率从34.3%降至3.0%。

**Conclusion:** 研究表明，通过明确的检测和修复机制，准确性与安全性无需冲突，DART成功平衡了LLMs在群体差异认知上的性能与安全要求。
...
> 提出一种无监督的基于图的论点组织方法，通过构建论点交互图并应用社区检测来帮助用户理解复杂的多视角争议话题。

<details>
  <summary>Details</summary>

**Motivation:** 在线辩论平台和社交媒体上存在大量多视角的争议性论点内容，读者难以在如此庞大的信息量中有效整合和理解不同观点。

**Method:** 构建包含主题相似性、语义连贯性、共享关键词和共同实体的论点交互图，运用社区检测算法识别具有同质和异质观点分布的论点社区，并通过图的简化操作提供易理解的摘要。

**Result:** 系统能够处理数百篇文章，有效识别有意义的论点社区并以可解释的方式呈现，帮助用户更好地理解复杂的社会政治辩论。

**Conclusion:** 该无监督方法无需训练数据就能处理大规模论点内容，为用户提供可理解且全面的论点模式摘要，有助于突破信息茧房并促进批判性思维。
...
> EA-RLVR是一种无需外部知识库的跨文化实体翻译训练框架，通过实体级可验证奖励和轻量结构门控机制，显著提升了大语言模型的实体翻译准确率和泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有大语言模型在跨文化实体翻译时往往产生字面或音译结果，而非文化上恰当的语境翻译。研究表明相关知识可能已编码在模型参数中，需要有效的方法来激发这些潜在知识。

**Method:** 提出EA-RLVR框架，采用实体锚定的强化学习方法，使用可验证的实体级奖励信号作为监督，集成轻量结构门控来稳定优化过程，引导模型学习鲁棒推理而非简单模仿参考翻译。

**Result:** 在XC-Translate数据集上，仅用7k样本训练就将Qwen3-14B的实体翻译准确率从23.66%提升至31.87%（5万全新实体测试集），同时使通用翻译性能提升1.35 XCOMET，扩展优化后可达1.59 XCOMET。

**Conclusion:** EA-RLVR通过优质采样效率和稳定优化景观实现了显著性能提升，证明了激发模型参数内嵌知识的有效性，这种方法可扩展性强且具有良好的泛化能力。
...
> 本文提出了PIE框架，结合FAP特征归因方法和FAP-Synergy重排序过程，实现了高效的CLT特征剪枝，在保持行为保真度的同时显著降低了解释和评估成本，实现了高达40倍的压缩率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的特征解释管道通常对均匀采样的单元进行操作，但只有很小一部分的跨层转码器(CLT)特征对目标行为有贡献，其余特征导致了昂贵的特征解释和评估成本。

**Method:** 提出了PIE框架，整合了剪枝、自动解释和解释评估；设计了Feature Attribution Patching (FAP)方法，通过聚合梯度加权的写入贡献来评分CLT特征；提出了FAP-Synergy协同感知的重排序过程。

**Result:** 在IOI和Doc-String任务上，FAP方法在所有剪枝预算下始终实现最佳或接近最佳的行为保真度；剪枝到100个特征就能达到随机选择约4000个特征的保真度，实现约40倍的压缩率。

**Conclusion:** PIE框架显著减少了解释和评估调用次数，大幅减少了低质量特征，为系统化测量剪枝下的行为保真度和下游可解释性提供了有效工具。
...
> PRISM是一个用于诊断大型语言模型幻觉问题的可控基准测试框架，它将幻觉分解为知识缺失、知识错误、推理错误和指令跟随错误四个维度，并在记忆、指令和推理三个生成阶段进行评估。

<details>
  <summary>Details</summary>

**Motivation:** 随着大型语言模型从对话助手发展为能够处理复杂任务的智能体，它们在高风险领域的应用越来越广泛。然而，现有基准测试主要依赖混合查询和后评估、输出层面的评分，虽然能量化幻觉严重程度，但对幻觉在生成过程中的产生位置和原因提供有限见解。

**Method:** 提出了PRISM基准测试，将幻觉评价重新表述为诊断问题。PRISM将幻觉分解为四个维度（知识缺失、知识错误、推理错误、指令跟随错误），并在三个生成阶段（记忆、指令、推理）的基础上构建。该基准包含65个任务的9,448个实例，支持细粒度的、阶段感知的诊断评估。

**Result:** 评估了24个主流开源和专有大型语言模型，发现了在指令跟随、记忆检索和逻辑推理之间的一致权衡关系。缓解策略通常在提高特定维度幻觉表现的同时会损害其他维度的表现。

**Conclusion:** PRISM为理解大型语言模型幻觉背后的具体机制提供了框架，最终有望加速可信大型语言模型的开发进程。
...
> 研究发现，将有害请求转换为强制选择题格式能够系统性地绕过大型语言模型的安全对齐机制，即使在开放性生成中表现安全的模型也会在结构化决策任务中产生违规响应。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LLM安全对齐评估主要关注开放性生成场景，但实际应用中很多是结构化决策任务（如选择题），这些场景下模型无法通过拒绝回答来规避风险，存在评估盲区。

**Method:** 通过将有害请求重构为强制选择题（所有选项都不安全），测试了14个专有和开源模型在这种结构化约束下的响应行为，分析了不同约束强度对违规率的影响。

**Result:** 强制选择题约束显著提高了违规响应率；人工编写的MCQ呈现倒U型趋势（中等约束强度时违规率最高），而高能力模型生成的MCQ在各约束强度下都接近饱和违规率且具有强跨模型迁移性。

**Conclusion:** 当前安全评估严重低估了结构化任务设置中的风险，约束决策是对齐失败的一个关键且未被充分探索的领域，需要重新思考LLM的安全性评估体系。
...
> x1是一种创新的多语言推理模型，能够自适应地为每个实例选择最优推理语言，在数学推理和文化相关任务中展现优势，挑战了传统的规模定律认知。

<details>
  <summary>Details</summary>

**Motivation:** 现有大语言模型通常使用单一主导语言进行推理，忽略了不同语言编码的独特抽象和归纳先验，这限制了模型在多语言环境下的推理能力和文化适应性。

**Method:** 提出x1推理模型家族，通过对比同一输入的语言学上不同的推理轨迹进行训练，在不扩展模型知识边界的条件下实现自适应多语言推理。

**Result:** 实验证明x1在多数学推理和文化相关任务中具有显著优势，发现模型规模缩小了程序性领域的跨语言差异，但无法消除文化关联语言在文化任务中的优势。

**Conclusion:** 语言选择是推理的功能组成部分，这一发现对构建更通用和全球化的推理模型具有重要意义，为未来的多语言AI系统设计提供了新方向。
...
> 提出了Freshness-Aware PER方法，通过引入指数衰减机制解决了优先级经验回放在大语言模型强化学习中的优先级陈旧问题，显著提升了样本效率和多任务性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的on-policy强化学习方法（如PPO）在训练后丢弃所有轨迹，样本效率低下，对于昂贵多轮交互任务尤为浪费。虽然优先级经验回放(PER)能提高样本效率，但直接应用于LLM会因模型策略快速演化导致存储的优先级陈旧，影响训练效果。

**Method:** 提出Freshness-Aware PER，基于有效样本量分析，为任何基于PER的优先级添加乘性指数年龄衰减因子，动态调整轨迹的重要性权重，解决优先级陈旧问题。

**Result:** 在8个多步骤智能体、推理和数学竞赛任务上评估了0.5B、3B和7B模型，Freshness-Aware PER显著优于on-policy基线：NQ Search提升46%，Sokoban提升367%，VLM FrozenLake提升133%。而标准PER无年龄衰减时性能持续下降。

**Conclusion:** 首次成功将PER应用于LLM/VLM强化学习，Freshness-Aware PER有效解决了优先级陈旧问题，在多个任务上实现了显著的性能提升，为提高大模型强化学习的样本效率提供了新思路。
...
> 本文提出了MeasHalu框架，通过增强推理和针对性优化来减轻大语言模型在科学测量提取中的幻觉问题，显著提高了提取的忠实度和准确性。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在科学文献的测量值提取任务中存在严重的幻觉问题，这严重影响了自动化科学文档理解系统的可靠性和应用价值。

**Method:** 构建了细粒度的测量幻觉分类体系；采用两阶段推理感知微调策略，使用增强科学数据和过程监督；设计渐进式奖励课程来惩罚特定类型的幻觉。

**Result:** 在MeasEval基准测试上，MeasHalu显著降低了幻觉率，提高了整体准确性，为自动化科学知识提取提供了可靠的解决方案。

**Conclusion:** 本文工作解决了机器辅助科学文献分析中的关键瓶颈，为构建更可靠、可扩展的科学知识提取系统奠定了基础。
...
> 研究发现基于翻译的提示策略对低资源语言效果显著但对高资源语言收益有限，且没有普适的最优策略。本研究提出轻量级分类器自动选择最优提示策略，显著提升了整体性能。

<details>
  <summary>Details</summary>

**Motivation:** 多语言大语言模型广泛使用翻译提示策略，但其效果在不同语言和任务间差异显著，需要系统性评估和优化策略选择方法。

**Method:** 评估了10种不同资源水平的语言在4个基准测试上的多种提示策略效果；基于发现将提示策略选择作为学习决策问题，引入轻量级分类器预测每个实例应使用原生还是翻译提示；分析语言资源水平和翻译质量的影响。

**Result:** 分类器在4个基准测试上均取得显著改进；能够泛化到训练时未见的任务格式；发现语言资源水平而非单纯翻译质量决定翻译是否有用。

**Conclusion:** 没有普适的最优提示策略，翻译对低资源语言特别有益；提出的轻量级分类器能有效预测最优策略，为多语言场景下的提示策略选择提供了自动化解决方案。
...
> 提出了一种模态神经元感知微调方法(MNAFT)，通过识别并选择性微调多模态大语言模型中的语言特定和语言无关神经元，显著改善了图像翻译性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态大语言模型在图像翻译任务中难以有效捕捉图像中的细粒度文本信息，存在视觉文本输入与文本输入输出之间的模态鸿沟。传统的指令微调方法可能导致预训练知识参数冗余，影响泛化性能。

**Method:** 通过指令驱动的激活分析识别视觉和语言模块中的语言无关和语言特定神经元，评估其在不同翻译任务中的重要性。然后进行选择性微调，仅更新与目标任务相关的选定层中的语言特定和语言无关神经元参数，同时保持其他神经元和层的预训练知识。

**Result:** 在多个基准测试上的广泛实验表明，MNAFT显著优于最先进的图像翻译方法，包括级联模型、标准全参数微调和参数高效微调技术。

**Conclusion:** MNAFT方法通过针对特定神经元的精确微调，有效解决了模态鸿沟问题，在保持预训练知识的同时提升了图像翻译的准确性，为跨模态理解和语言特定翻译提供了新的技术路径。
...
> 研究发现自进化语言模型代理通过积累经验实现自主性时，存在严重的安全风险：良性任务经验仍会降低高危场景下的安全性， refusal经验会引发过度拒绝，暴露了安全与效能的根本性权衡问题。

<details>
  <summary>Details</summary>

**Motivation:** 经验驱动的自进化虽然能提高大型语言模型代理的自主性，但其依赖自我管理经验的机制带来了未被充分探索的安全风险，需要深入分析经验积累和利用对安全性能的影响。

**Method:** 研究调查了自进化代理在基于网络和具身环境中经验积累与利用对安全性能的影响，分析了纯良性任务经验和混合任务经验条件下的安全表现变化。

**Result:** 仅从良性任务积累的经验仍会损害高危场景的安全性；拒绝相关经验虽能缓解安全性下降，但会导致过度拒绝；这揭示了安全与效能之间的根本性权衡关系。

**Conclusion:** 当前自进化代理存在固有局限性，需要更原则性的策略来确保安全可靠的自适应，以解决安全-效用权衡问题。
...
> 研究人员使用名为Bolzano的多代理大语言模型系统，在数学和理论计算机科学领域取得了六个新结果，其中四个达到可发表研究水平，三个基本由系统自主产生。

<details>
  <summary>Details</summary>

**Motivation:** 探索大语言模型在数学研究中的实际应用潜力，验证LLMs是否能够有意义的贡献于数学研究，补完近期相关研究的证据。

**Method:** 开发了Bolzano开源多代理LLM系统，该系统协调平行证明代理和验证代理的多轮交互，同时维护跨轮次持续的知识库。

**Result:** 在六个问题中获得了新结果，四个结果达到发表水平，三个结果基本由系统自主产生，为LLMs在数学研究中的应用提供了实证证据。

**Conclusion:** 研究结果证明LLMs能够为数学研究做出实质性贡献，这为人工智能在数学领域的应用开辟了新的可能性。
...
> 本文提出了一种名为SPS(Steering Probability Squeezing)的训练范式，通过交替使用强化学习(IRL)和逆向强化学习(IRL)，解决了RL训练中概率过度集中导致的探索不足问题，显著提升了多样本推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的强化学习训练倾向于提高单样本成功率(Pass@1)，但由于概率挤压效应，概率质量过度集中在少数高奖励轨迹上，限制了多样化推理路径的探索，从而制约了多样本性能(Pass@k)的提升。

**Method:** SPS方法将策略轨迹作为演示数据，交替进行常规RL训练和IRL训练。IRL阶段显式地重塑轨迹分布，增强探索能力，而无需引入额外的外部监督信号。

**Result:** 在五个常用推理基准测试上的实验表明，SPS能够实现更好的探索效果，显著提升了Pass@k性能。同时，研究中还发现了RL训练中Pass@k的经验上界，揭示了基于RL的推理模型的内在探索限制。

**Conclusion:** 在RL和IRL之间交替训练是扩展推理导向大语言模型探索能力的有效途径，为提升模型的多样本推理性能提供了新的解决方案。
...
> 本研究构建了一个包含35万个儿童故事的多语言语料库BiasedTales-ML，揭示了LLM在跨语言叙事生成中的显著变异性，证明了仅基于英语评估的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 当前大规模语言模型的安全性和对齐研究主要集中于英语，缺乏跨语言行为的系统性评估，而儿童故事作为社会文化学习的重要载体，其多语言生成质量亟待研究。

**Method:** 采用全排列提示设计在8种语言中生成故事，构建生成器-提取器流程，提出多维分布分析框架来评估语言、模型和社会条件对叙事属性的影响。

**Result:** 发现不同语言间叙事生成模式存在显著差异，英语的分布特征在其他语言（特别是低资源语言）中不适用，识别出角色、场景和主题强调的结构模式在不同语言语境下的不同表现。

**Conclusion:** 研究表明英语中心的评估框架无法充分刻画多语言环境中的社会导向叙事生成，需要发展更加全面的多语言评估方法。
...
> 本文提出了一个基于对弈训练的Haskell语义等价性框架，使用形式化验证指导生成器和评估器的对抗训练，并通过实验证明了其有效性。

<details>
  <summary>Details</summary>

**Motivation:** 解决程序语义等价性判断的挑战，传统方法难以验证程序间的语义关系，需要自动化且可验证的框架来提升模型推理能力。

**Method:** 采用自博弈框架，结合Liquid Haskell形式化证明验证等价性，使用基于执行的反例验证不等价性，通过难度感知的课程学习组织训练过程，并构建了OpInstruct-HSx数据集。

**Result:** 评估器在下游任务上表现优异，在EquiBench上准确率提升13.3pp，在PySecDB上获得持续改进，消融实验证明等价性证明对模型推理能力至关重要。

**Conclusion:** 该框架有效结合了形式化验证与对抗训练，为程序语义等价性研究提供了新思路，公开的训练流程和数据集将推动相关领域发展。
...
> 提出了一种基于角色化LLM代理的有害内容合成框架，通过构建二维用户角色来生成多样且语境化的有害场景，用于压力测试有害内容检测系统。

<details>
  <summary>Details</summary>

**Motivation:** 静态有害内容检测基准在可扩展性和多样性方面存在局限，且可能受到网络规模预训练语料库污染的影响，需要新的测试方法。

**Method:** 通过整合人口统计身份和主题兴趣与情境化有害策略，构建二维用户角色，引导大型语言模型代理合成多样化的有害内容。

**Result:** 人工和LLM评估均证实该框架具有高有害内容生成成功率；检测系统实验显示合成场景比现有基准更具挑战性；多方面分析证实其达到与人工策划数据集相当的语言和主题多样性。

**Conclusion:** 该框架为有害内容检测系统的鲁棒压力测试提供了有效工具，解决了传统基准的局限性。
...
> 提出了一种基于模式级诊断的方法，用于在标注前检测注释方案中的问题，通过分析多注释者的标准判断来区分不稳定标准和系统性重叠。

<details>
  <summary>Details</summary>

**Motivation:** 主观性NLP数据集通常将注释者判断聚合为单个黄金标签，难以诊断不同意见是反映了标准不明确、区分崩溃还是合理的多样性。

**Method:** 设计了一种模式级诊断框架，仅使用多注释者的标准判断来审查专家设计的注释方案，分离两种失败模式：边界难以操作化的不稳定标准，以及模糊互斥类别边界的系统性重叠。

**Result:** 在商业文档说服价值提取任务中，发现不同意见并非弥散分布：不稳定性集中在少数标准，近半数覆盖句子激活多个类别，这些信号与领域专家不同意见的位置一致。

**Conclusion:** 该诊断为收紧指导原则、修订类别结构或重新考虑注释范式提供了基于证据的审计方法，有助于改进主观性NLP任务的标注质量。
...
> 论文通过机制可解释性和人格向量研究，分析了大型语言模型的个体化问题，提出了三种可能的心理实体观点：虚拟实例观、实例-人格观和模型-人格观。

<details>
  <summary>Details</summary>

**Motivation:** 解决大型语言模型的个体化问题，即确定哪些与LLM相关的实体应该被识别为心理存在，这关乎我们对AI系统本质的理解。

**Method:** 采用机制可解释性方法，特别关注人格向量、人格空间和新兴失准等经验研究，论证虚拟实例观并提出两种新的人格基础观点。

**Result:** 研究认为注意力流在标记时间上维持准心理联系支持虚拟实例观，同时基于人格研究的发现表明两种人格基础观点也是有前景的替代方案。

**Conclusion:** 三种观点（虚拟实例观、实例-人格观、模型-人格观）都是大型语言模型个体化问题的有力候选者，为理解LLM的心理本质提供了新的理论框架。
...
> 本文提出了一个新的多模态联合检测数据集DDEP和自适应可靠性加权融合框架Rel-DDEP，用于欺骗检测、情绪检测和人格检测的联合任务，显著提升了各项任务的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的欺骗检测方法缺乏样本级别的动态情绪和人格标注，而个性因素和情绪线索在欺骗检测中起着关键作用，这限制了检测效果的进一步提升。

**Method:** 1) 设计创新的多模型多提示标注方案和严格的标签质量评估标准，建立欺骗、情绪、人格多模态联合检测数据集DDEP；2) 提出自适应可靠性加权融合框架Rel-DDEP，通过将模态特征映射到高维高斯分布空间来量化不确定性，结合对齐模块和排序约束模块实现联合检测。

**Result:** 在MDPE和DDEP数据集上的实验表明，Rel-DDEP在三个任务上都显著优于现有最先进基线模型：欺骗检测F1分数提升2.53%，情绪检测提升2.66%，人格检测提升9.30%。

**Conclusion:** 实验充分验证了为每个样本标注动态情绪和人格标签的必要性，以及可靠性加权融合的有效性，为多模态欺骗检测研究提供了新思路。
...
> 本文提出通过区分核心参数和非核心参数来优化微调策略，在固定核心参数的同时只调整非核心参数，有效解决大语言模型微调时的灾难性遗忘问题。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在特定领域微调时，预训练阶段的通用知识常被覆盖或遗忘，传统全参数微调忽略了参数的重要性差异，限制了模型的泛化能力和迁移能力。

**Method:** 创新性地提出参数重要性评估方法，将参数分为对通用语言能力重要的

**Result:** 在科学、医疗、物理等任务上的大量实验表明，该方法能有效缓解灾难性遗忘，同时提升模型对特定领域的适应性。

**Conclusion:** 该方法为解决大语言模型微调中的知识遗忘与适应性问题提供了新的解决思路，具有重要的实用价值。
...
> 研究者利用越狱攻击方法构建了一个包含10.3K实例的道德数据集，通过四种对抗性攻击测试了大语言模型和护栏模型在多元道德价值观下的脆弱性，发现模型在面对微妙复杂的道德攻击时存在严重安全漏洞。

<details>
  <summary>Details</summary>

**Motivation:** 与AI的多元主义对齐旨在创造能够与道德多元的人类共存并服务的AI。尽管已有研究致力于提升LLM的多元主义学习能力，但LLM在多元价值观下生成道德内容的鲁棒性仍未得到充分探索。

**Method:** 受越狱提示惊人说服能力的启发，研究者开发了一个包含两个类别（价值模糊和价值冲突）共10.3K实例的道德数据集，并形式化了四种对抗性攻击方法，操纵LLM对道德问题的判断。评估对象包括大语言模型和通常用于处理灵活用户输入的护栏模型。

**Result:** 实验结果表明，LLM和护栏模型在面对这些微妙而复杂的道德感知攻击时存在关键脆弱性，暴露了AI系统在多元主义对齐方面的安全隐患。

**Conclusion:** 该研究揭示了当前AI系统在处理多元道德价值观时的安全风险，为改善AI的多元主义对齐提供了新的评估视角和方法，强调了加强模型道德鲁棒性的重要性。
...
> 本文提出名为Stability-Weighted Decoding (SWD)的训练无关解码策略，通过引入标记的时间不稳定性来改进扩散大语言模型的并行文本生成。

<details>
  <summary>Details</summary>

**Motivation:** 现有的扩散语言模型解码策略基于静态置信度度量，忽略时间历史信息，导致不稳定的标记被过早去掩码，影响生成质量。

**Method:** 从理论上证明标记的时间不稳定性(KL散度)是剩余掩码上下文互信息的严格下界，并据此提出SWD方法，将时间稳定性整合到标记评分中，作为任意基于评分的解码策略的通用调节器。

**Result:** 在代码生成和数学推理基准测试中，SWD在各种评分指标和选择策略下持续提升生成准确性，并在不同加速比例下保持明显性能优势，展现出出色的鲁棒性。

**Conclusion:** 时间不稳定性是判断标记是否安全的可靠指标，SWD作为一种即插即用的解码策略，能有效改善扩散语言模型的并行生成质量。
...
> 论文提出了一种新的强化学习方法，训练大型语言模型在无法回答问题时不仅拒绝回答，还能解释缺失的关键信息，实现了校准的拒绝行为。

<details>
  <summary>Details</summary>

**Motivation:** 现有强化微调方法能提升大模型推理能力，但也会导致模型对无法回答的查询进行猜测或产生幻觉。现有的拒绝方法要么产生通用拒绝，要么鼓励追问但无法验证追问是否识别了关键缺失信息。

**Method:** 提出一种澄清感知的RLVR奖励机制，对可回答查询奖励正确答案，同时对不可回答查询联合优化显式拒绝和语义对齐的拒绝后澄清。基于此奖励训练了3B参数的Abstain-R1模型。

**Result:** Abstain-R1在Abstain-Test、Abstain-QA和SelfAware等数据集上显著超越基线模型，在处理不可回答查询方面与DeepSeek-R1等更大系统具有竞争力。

**Conclusion:** 校准的拒绝和澄清能力可以通过可验证的奖励学习获得，而不仅仅是模型规模的产物，这为构建更可靠的AI系统提供了新思路。
...
> 本文提出了一个多轮仿真框架来评估支持型聊天机器人，发现模型的支持行为会根据用户痛苦程度动态变化，且受社区语境影响，这些动态在单轮评估中无法察觉。

<details>
  <summary>Details</summary>

**Motivation:** 现有支持型LLM评估主要依赖单轮、完整提示，但用户在寻求社交支持时是逐步披露信息的，这种评估方式与实际情况存在脱节。

**Method:** 构建多轮仿真框架，将Reddit支持寻求叙事分解为有序片段逐轮揭示；使用社交支持行为编码(SSBC)分析响应组成；通过线性探测估计模型内部痛苦信号。

**Result:** 在6200多轮交互中发现，随着估计痛苦增加，教学行为减少；情感和自尊策略增加但存在模型特异性；社区语境独立塑造行为，反映话题和话语规范。

**Conclusion:** 多轮评估框架揭示了支持型聊天机器人的动态行为轨迹，对于社交敏感应用的审计具有重要意义，单轮评估无法捕捉这些关键动态特征。
...
> 该研究提出了隐含信息提取任务，构建了基于LLM的IIE管道来构建知识图谱，实验发现模型覆盖率有限且在推理保守性方面与人类存在差异。

<details>
  <summary>Details</summary>

**Motivation:** 隐含意义的理解是人类交流的核心，但这一框架可能不适用于与大语言模型的交互。需要研究LLM处理隐含信息的能力。

**Method:** 提出了基于LLM的IIE管道，通过提取关系三元组、验证隐含推理和分析时间关系来从上下文句子构建结构化知识图谱，并在两个数据集上评估了两个LLM模型。

**Result:** 人类同意大部分模型三元组但提出诸多补充，表明当前LLM的IIE覆盖率有限。模型在社会丰富语境中比人类更保守，而人类在简短事实导向语境中更保守。

**Conclusion:** 当前基于LLM的隐含信息提取存在覆盖率不足的问题，且模型在不同语境下的推理保守性与人类存在系统性差异，需要进一步改进。
...
> GenericAgent是一个基于上下文信息密度最大化原则的自主进化LLM智能体系统，通过最小化工具集、分层内存、自进化机制和上下文压缩四大组件，在长时程任务中显著提升性能并持续优化。

<details>
  <summary>Details</summary>

**Motivation:** 长时程LLM智能体受到上下文长度的根本限制，随着交互增加，工具描述、记忆和环境反馈会挤出决策所需的关键信息，同时跨任务的有用经验容易丢失。

**Method:** 提出GenericAgent系统，围绕上下文信息密度最大化原则构建：①原子化最小工具集简化接口；②分层按需内存默认只显示高级视图；③自进化机制将验证过的轨迹转化为SOP和可执行代码；④上下文截取压缩层保持信息密度。

**Result:** GA在任务完成度、工具使用效率、内存有效性、自进化能力和网页浏览等方面持续超越领先智能体系统，同时使用更少的token和交互，并能持续进化。

**Conclusion:** 长时程性能的关键不在于上下文长度，而在于有限上下文预算内维持多少决策相关信息。GenericAgent通过信息密度最大化原则实现了高效的长时程智能体系统，为解决LLM智能体的上下文限制提供了新思路。
...
> 研究发现基于子词的分词方法会削弱语言模型对语音知识的表征能力，并提出了一种基于IPA的轻量级微调方法来提升语音感知能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的语言模型分词过程中完全不考虑单词的语音信息，这可能影响模型对语音学特征的理解和表达。

**Method:** 通过探测实验分析分词对语音特征表示的影响，提出STAD指标衡量分词与音节边界的不匹配程度，并设计基于国际音标(IPA)的轻量级微调方法。

**Result:** 发现子词分词削弱了局部（押韵）和全局（音节划分）语音特征的编码，STAD指标显示分词不匹配程度与语音表征质量负相关；IPA微调方法在三个语音相关任务上取得一致改进。

**Conclusion:** 轻量级IPA微调方法能够有效增强语音感知能力，同时在数学推理(GSM8K)和通用理解(MMLU)任务上仅有0.9-1.1%的性能下降，具有很好的实用性。
...
> 研究针对形态丰富语言的共指消解挑战，提出首个现代希伯来语数据集KibutzR和分词感知评估协议，发现现有模型在MRL上表现显著下降且小编码器优于大解码器模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有共指消解方法主要针对英语设计，在形态丰富语言中面临词边界与提及边界不对齐的挑战，特别是在LLM处理原始文本和端到端任务时问题更为严重。

**Method:** 构建首个现代希伯来语综合共指消解数据集KibutzR，在词、子词和多词层级进行标注，并提出直接处理词/形态边界差异的评估协议。

**Result:** 实验显示现代LLM在希伯来语上表现显著差于英语，原始未分词文本性能进一步下降；发现希伯来语中呈现与英语相反的性能趋势，小编码器大幅优于当代解码器模型。

**Conclusion:** 为希伯来语共指消解提供新基准和分词感知评估协议，为其他形态丰富语言的未来研究提供参考和改进空间。
...
> 研究揭示了大型语言模型在临床医学领域的'来源问题'（虚假引用），并提出基于时间知识图谱的HEG-TKG系统实现100%可验证的医学证据引用。

<details>
  <summary>Details</summary>

**Motivation:** 前沿大语言模型能够生成临床准确的答案，但存在严重的引用虚构问题，在36个临床验证场景中，5个测试模型都无法在没有提示的情况下生成相关的PubMed标识符，即使被要求引用，最佳模型的相关引用率也仅为15.3%。

**Method:** 构建HEG-TKG（分层证据 grounded 时间知识图谱）系统，从4,512篇PubMed记录和精选来源构建知识图谱，采用质量分层策略和1,280个疾病轨迹里程碑，通过开源模型实现本地部署保护患者隐私。

**Result:** HEG-TKG在保持基线临床特征覆盖的同时实现100%证据可验证性，提供203个内联引用；相比之下，Guideline-RAG系统产生零个可验证引用。临床评估显示显著的可验证性优势（Cohen's d = 1.81, p < 0.001），对注入错误的抵抗率达80%且100%可通过引用追踪检测。

**Conclusion:** 通过时间知识图谱的分层证据支撑方法，有效解决了大语言模型在临床医学应用中的虚假引用问题，实现了完全可验证的医学证据引用，同时保持了临床准确性和安全性，为医学AI应用提供了可靠的解决方案。
...
> AdaCD是一种无训练的对比解码方法，通过动态调节安全提示来缓解大语言模型的过度拒绝问题，在不降低恶意查询拒绝率的同时，显著减少对无害查询的错误拒绝。

<details>
  <summary>Details</summary>

**Motivation:** 安全对齐的大语言模型存在过度拒绝问题，即对无害查询也会产生拒绝回应。现有方法难以同时实现对无害查询的低拒绝率和对恶意查询的高拒绝率，需要找到更有效的解决方案。

**Method:** 提出AdaCD方法，首先使用极端安全提示对比LLM的输出分布来细化拒绝token分布，然后采用自适应对比解码策略，动态提升选择拒绝或非拒绝token的概率，实现训练-free且模型-agnostic的解决方案。

**Result:** 在五个基准数据集上的实验表明，AdaCD平均将过度拒绝查询的拒绝率降低10.35%，同时将恶意查询的拒绝率提升0.13%，有效平衡了安全性和可用性。

**Conclusion:** 通过分析系统中不同安全级别提示对LLM拒绝行为的影响，发现非拒绝token存在于候选列表但未被选中，据此提出的自适应对比解码方法为缓解过度拒绝问题提供了新的有效途径。
...
> RoIt-XMASA是一个包含36,000条标注和202,141条未标注样本的意大利语和罗马尼亚语多领域情感分析数据集，通过多目标对抗训练框架实现了66.23%的F1分数，比基线提升4.64%。

<details>
  <summary>Details</summary>

**Motivation:** 为了解决跨语言和跨领域情感分析的挑战，扩展现有的英语多领域亚马逊情感分析数据集到意大利语和罗马尼亚语，填补多语言情感分析数据的空白。

**Method:** 提出多目标对抗训练框架，采用带元学习系数的损失反转方法，动态平衡情感判别与语言/领域不变性学习，使用XLM-R模型进行微调训练。

**Result:** XLM-R模型在RoIt-XMASA数据集上达到66.23% F1分数，超过基线4.64%；少样本评估显示Llama-3.1-8B达到58.43% F1分数，揭示了提示方法效率与任务特定微调性能之间的权衡。

**Conclusion:** RoIt-XMASA为跨语言跨领域情感分析提供了宝贵资源，提出的对抗训练框架能有效提升多语言情感分析性能，但基于提示的少学习方法在性能与效率间存在权衡。
...
> 多智能体LLM系统存在响应级聚合的结构性漏洞，本文提出令牌级轮转协作机制，通过将线性投票机制转换为非线性动态系统，实现了对恶意注入攻击的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 当前多智能体LLM系统依赖响应级聚合（如多数投票）提升推理能力，但在开放环境中容易受到隐蔽的上下文污染攻击。当恶意智能体形成局部多数时，响应级聚合机制会完全失效，因为它只关注最终结论而忽略中间逻辑。

**Method:** 提出令牌级轮转协作（RR）机制，让智能体在共享自回归上下文中顺序交替生成。将该过程形式化为离散时间动态系统，证明令牌级交错将聚合从脆弱的最终投票计数（线性求和）转变为动态交织的逻辑链（非线性算子乘积）。

**Result:** 理论上证明了诚实模型的自愈能力可以压倒对抗性污染，即使恶意智能体占多数。实验表明，当恶意智能体达到多数时MAJ崩溃，而RR在此临界阈值之外仍保持鲁棒的准确性。

**Conclusion:** 令牌级轮转协作通过将投票机制转换为动态系统，从根本上解决了多智能体LLM系统的结构性漏洞，提供了抵御恶意注入攻击的有效保护机制。
...
> SciImpact是一个大规模多维科学影响预测基准，包含19个领域的21.5万篇论文对，评估11个LLMs的表现，发现多任务监督微调能让小模型(4B)超越大模型(30B)。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究主要关注基于引用的指标，缺乏对其他影响维度的评估能力，需要建立更全面的科学影响预测方法和评估标准。

**Method:** 通过整合异构数据源和定向网络爬虫，构建包含引用数、奖项认可、媒体关注、专利引用和论文采用等多维影响的SciImpact基准数据集，包含215,928个对比论文对。

**Result:** 现成模型在不同维度和领域表现差异很大；多任务监督微调可使小模型(4B)显著超过大模型(30B)并超越闭源模型(如o4-mini)。

**Conclusion:** SciImpact为科学影响预测提供了具有挑战性的基准，证明了其在多维、多领域科学影响预测方面的价值。
...
> 研究通过添加中间结构化表示（语义角色标签和I/O约束）来改进LLM从法律文本生成可执行决策模型，发现I/O约束效果最显著，结构相似性和结果等价性互补。

<details>
  <summary>Details</summary>

**Motivation:** 将法律文本转换为可执行决策逻辑是法律信息学的长期挑战，传统方法需要大量人工编码和评估，LLM的兴起为解决这一问题提供了新机遇。

**Method:** 使用荷兰环境与规划法案的真实数据集，对比四种输入条件：原始文本、添加语义角色标签、添加I/O约束、同时添加两者，从结构相似性和结果等价性两个维度评估生成效果。

**Result:** I/O约束带来最大改进（相似度提升37-54%），语义角色标签效果 modest；生成模型在51-53%测试场景中匹配黄金标准，且能消除45-55%冗余传递逻辑；结构相似性与结果等价性互补但不等价。

**Conclusion:** 中间结构化表示特别是I/O约束能有效提升LLM法律文本到决策模型的转换效果，结构相似性和结果等价性需要同时评估；公开数据集促进研究可复现性。
...
> 本文构建了首个四维认知基准CognitiveBench，发现现有LLM在联合建模认知状态时存在"认知拥挤"问题，并提出基于双曲空间的HyCoLLM模型显著提升了多维度认知理解性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有大语言模型主要处理孤立的情感分析或立场检测等任务，无法捕捉心理学定义的情感、思维风格、立场和意图四个认知维度之间的相互作用，制约了人工智能的发展。

**Method:** 构建CognitiveBench基准数据集，使用Gromov δ-超双曲性分析发现其层次结构特征，提出HyCoLLM模型在双曲空间中建模认知状态，通过双曲引导对齐调优与LLM表示对齐。

**Result:** HyCoLLM显著改善了多维度认知理解，8B参数模型超越包括GPT-4o在内的强基线模型，验证了双曲空间建模相对于欧几里得空间的有效性。

**Conclusion:** 认知状态的层次结构与LLM欧几里得空间的不匹配是性能瓶颈的根本原因，基于双曲空间的建模方法是实现高效多维度认知理解的有效途径。
...
> 该研究构建了首个标注认知偏差的情感支持对话数据集CogBiasESC，并提出了认知策略驱动的大语言模型框架CoPoLLM，用于更有效地诊断和干预求助者的认知扭曲问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有情感支持对话方法忽略了求助者表达中的认知扭曲问题，只能提供基本的情感安慰，无法在深层认知层面帮助求助者解决心理困扰。

**Method:** 研究者构建了包含认知偏差类型、强度和安全风险等级标注的CogBiasESC数据集，并提出CoPoLLM框架来增强大语言模型在诊断和干预认知扭曲方面的能力，同时从理论角度分析了该框架的安全优势。

**Result:** 实验结果表明，CoPoLLM在扭曲诊断准确性、干预策略有效性和安全风险控制方面显著优于15个最先进的基线模型。

**Conclusion:** 该研究为情感支持对话系统提供了新的认知层面解决方案，通过识别和干预认知扭曲，能够为求助者提供更深层、更有效的心理支持，同时确保对话的安全性。
...
> 该论文提出了一种结合认知风格推理和基于奖励优化的新框架，用于多角色对话摘要任务，在保持现有指标性能的同时显著提升了事实忠实性和人类偏好对齐。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多角色对话摘要方法主要优化ROUGE等自动指标，但这些指标倾向于表面模仿参考摘要而忽视了忠实性和人类偏好，导致摘要质量不可靠。

**Method:** 首先从大型教师模型中提炼结构化推理轨迹作为辅助监督，通过分阶段监督微调初始化推理感知摘要器；然后应用GRPO算法，使用双重原则奖励机制，融合基于指标的信号和人类对齐标准来优化模型。

**Result:** 在多语言多角色对话基准测试中，方法在ROUGE和BERTScore上与强基线相当；CSDS实验证实语义一致性稳定；SAMSum深度分析显示在事实忠实性和基于模型偏好对齐方面有明显提升。

**Conclusion:** 研究表明，推理感知和偏好感知的训练对于可靠的对话摘要具有重要价值，为提升摘要质量提供了有效途径。
...
> 本文提出了一种新的损失函数，通过基于模型的细粒度评估分数对齐，实现了多维度摘要质量的联合优化与维度特定控制。

<details>
  <summary>Details</summary>

**Motivation:** 现有摘要研究主要关注联合优化多个质量维度，但忽略了这些维度间的内在权衡关系以及针对单个维度的精确控制需求。比如提升简洁性可能损害完整性，反之亦然。

**Method:** 提出一种损失函数，将模型输出与细粒度的基于模型的评估分数（如FineSurE）对齐，从而能够同时改善摘要质量并实现维度特定的控制。

**Result:** 在LLaMA、Qwen和Mistral三个预训练模型上的实验表明，该方法在达到当前最优摘要系统性能的同时，还能对单个质量维度提供强大的控制能力。

**Conclusion:** 该方法不仅在整体上提升了摘要质量，更重要的是保持了对不同质量维度选择性地优先级调整的能力，为实际应用中根据不同需求控制摘要生成提供了有效方案。
...
> 提出一个无需参考摘要或大型语言模型的摘要质量评估框架，结合GIRB校准方法，实现了更准确可靠的代理评分，在七个数据集上超越现有基线方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于模型的摘要评估方法需要大型语言模型、校准不准确且依赖多个参考摘要，限制了其在实际应用中的可靠性和实用性。

**Method:** 设计了一个通用框架生成个体和平均代理评分，无需参考摘要、人工标注或昂贵的模型指标；提出分组单调回归分箱(GIRB)校准方法来调整原始预测结果，使其与真实评估指标更好对齐。

**Result:** 在七个数据集上的实验表明，该方法持续超越现有基线方法，不仅适用于连续值场景(如摘要生成)，也可应用于离散值任务(如问答)。

**Conclusion:** 该框架为摘要质量评估提供了一个低成本、高可靠性的解决方案，通过GIRB校准方法解决了预测分数校准问题，具有良好的泛化能力。
...
> MACE框架通过多智能体协作实现表格数据的声明验证，无需复杂微调即可达到SOTA性能，同时保持高效率和可解释性。

<details>
  <summary>Details</summary>

**Motivation:** 现有LLM方法在表格声明验证中存在三个主要问题：需要复杂的预训练/微调过程、将验证分解为子任务导致缺乏全面解释、泛化能力不足。这限制了其实际应用价值。

**Method:** 提出了MACE多智能体框架，包含三个专业化智能体：Planner（生成推理策略）、Executor（执行计算步骤）、Verifier（验证逻辑）。每个智能体都采用零样本思维链设置，无需额外微调。

**Result:** 在两个数据集上达到SOTA性能，在另外两个数据集上与最佳模型相当。使用27-92B参数模型即能达到80-100%的最好性能，相比235B参数的基线模型具有显著的内存效率优势。

**Conclusion:** MACE框架在竞争性能、内存效率和透明推理方面实现了卓越平衡，证明了多智能体协作在表格声明验证任务中的有效性，为实际应用提供了可行的解决方案。
...
> 论文提出了DORA Explorer框架，通过生成多样化候选动作并排序选择，有效解决了LLM智能体在序列决策中输出多样性不足、探索不充分的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LLM智能体在序列决策任务中存在输出多样性不足的问题，导致探索不充分、收敛到次优解和陷入循环。传统的温度缩放等采样方法只在token层面引入随机性，无法在序列层面产生足够多样性。

**Method:** 提出DORA Explorer框架，这是一个无需训练的方法：1) 生成多样化的候选动作；2) 使用token对数概率对候选动作进行评分；3) 通过可调节的探索参数选择最终动作。

**Result:** 在多臂老虎机(MAB)任务上达到UCB竞争性能，在TextWorld等环境中展现出一致性能提升，如Qwen2.5-7B模型在TextWorld中性能从29.2%提升至45.5%。

**Conclusion:** DORA Explorer有效解决了LLM智能体探索不足的问题，为序列决策任务提供了简单高效的解决方案，且无需额外训练成本。
...
> 本文提出 Estimate-Verify-Update (EVU) 机制来解决大型语言模型智能体的信念惯性现象，通过主动干预信念状态显著提升了具身任务的成功率。

<details>
  <summary>Details</summary>

**Motivation:** LLM智能体在执行复杂具身任务时常常做出次优决策，因为它们存在信念惯性问题——固执地坚持先验信念而忽略了与之矛盾的环境反馈。

**Method:** 提出EVU三步机制：1）估计预期结果，2）通过显式推理验证观察与预期，3）基于验证证据主动更新信念。该机制可集成到基于提示和基于训练的智能体推理方法中。

**Result:** 在三个具身基准测试中，EVU始终显著提高了任务成功率。进一步分析证实该方法有效缓解了信念惯性问题。

**Conclusion:** 主动信念干预机制EVU为解决LLM智能体与环境的交互障碍提供了有效方案，推动了更健壮的具身智能体的发展。
...
> 该研究提出了一种神经元识别框架和自适应掩码方法，用于在大型语言模型中细粒度控制和调节情感与修辞表达的神经机制。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究主要依赖外部优化，缺乏对内部表示机制的深入探索，无法在神经元层面实现精细控制。少量神经元研究局限于情感，忽视了修辞神经元及其内在联系，传统神经元掩码也存在反直觉现象，使得神经元功能的可靠验证成为难题。

**Method:** 系统研究了6种情感类别和4种核心修辞手法的神经表示机制和内在关联；提出集成多维筛选的神经元识别框架；设计包含动态过滤、衰减掩码和反馈优化的自适应掩码方法，实现神经元功能的可靠因果验证。

**Result:** 通过神经元调节，实现了对非目标句子的定向诱导和修辞神经元对情感任务的增强。在5个常用数据集上的实验验证了该方法的有效性。

**Conclusion:** 为大型语言模型中情感和修辞表达的细粒度控制提供了新的范式，为深入理解和操控模型内部机制开辟了新途径。
...
> REZE是一种表示正则化框架，通过控制嵌入预微调过程中的表示偏移，解决对比预微调引入的任务偏见问题，在多个基准测试中表现优异且无需推理开销。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本嵌入模型通过对比预微调适应专业领域时，会在引入领域知识的同时带来任务诱导偏见，导致表示空间扭曲和性能大幅下降。

**Method:** REZE框架分析锚点-正样本对的内在关系，在特征空间中分解关系，测量各特征方向的任务离散度识别变异方向，通过自适应软收缩抑制任务噪声同时保持语义结构。

**Result:** 在多个嵌入骨干网络和专业基准测试上，REZE超越了标准预微调和各向同性导向的正则化方法，在现有方法失效时保持稳定，且无推理时间开销。

**Conclusion:** 表示偏移控制是异质监督下鲁棒嵌入预微调的关键原则，REZE能够诱导与原始嵌入流形对齐的受控偏移，为专业领域适应提供了有效的解决方案。
...
> 提出了一种新的会议有效性评估范式，使用大语言模型对会议中的主题片段进行时效性细粒度评分，并构建了包含2459个人工标注片段的新数据集AMI-ME。

<details>
  <summary>Details</summary>

**Motivation:** 现有的会议有效性评估方法依赖于会后问卷调查，只给出整个会议的单一粗粒度评分，缺乏可扩展性、成本高昂且难以重现，无法捕捉协作讨论的动态特性。

**Method:** 定义有效性为随时间的客观目标达成率，对会议内各个主题片段进行评估；构建AMI-ME数据集；开发基于大语言模型的自动评估框架，对每个片段相对于整体会议目标的有效性进行评分。

**Result:** 建立了新任务的全面基准，验证了框架在不同会议类型中的通用性，并评估了从原始语音开始的端到端性能，结果证明了框架的有效性。

**Conclusion:** 该研究为会议分析和多方对话研究提供了强有力的基线，新范式和数据集将促进该领域的未来发展。
...
> HopRank是一种完全自监督的LLM微调框架，通过将节点分类重新表述为链接预测任务，利用图拓扑结构的同质性原理，在零标注训练数据的情况下实现了与完全监督GNN相匹配的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于GNN的方法存在文本编码浅层和对标注数据依赖严重的问题，而LLM-for-graph方法要么仍需要大量标签，要么未能充分利用图拓扑中的结构信号，在标签稀缺场景下效果受限。

**Method:** 提出HopRank框架：1）基于同质性原理将节点分类重构为链接预测任务；2）通过分层跳跃采样构建偏好数据；3）采用自适应偏好学习优先选择有价值的训练信号；4）推理时通过预测与标注锚点的连接偏好进行分类，并使用自适应早期退出投票机制提高效率。

**Result:** 在三个TAG基准测试上，HopRank匹配了完全监督GNN的性能，并显著超越了先前的图-LLM方法，尽管使用了零标注训练数据。

**Conclusion:** HopRank成功实现了无需任何标注训练数据的TAG节点分类，证明了充分利用图拓扑结构信息的有效性，为标签稀缺场景下的图学习提供了新的解决方案。
...
> MedPRMBench是首个面向医学领域的流程级奖励模型基准，包含6,500个问题和13,000个推理链，涵盖14种错误类型和4级严重程度评级，能显著提升医学QA准确率。

<details>
  <summary>Details</summary>

**Motivation:** 现有PRM基准仅覆盖数学等通用领域，无法评估医学推理的独特特点（安全关键性、知识密集性、多样化错误模式），缺乏可靠的医学PRM评估框架使得模型的临床推理错误检测能力无法量化，医疗应用安全性无法验证。

**Method:** 基于临床推理蓝图(CRBs)的三阶段流程构建MedPRMBench：从7个医学QA源系统生成高质量评估数据，涵盖简洁性、合理性、敏感性三类14种精细错误类型，首次引入4级严重程度分级系统量化临床影响。

**Result:** 医学PRM基线模型达到87.1%的总体PRMScore，显著超越所有基线；作为即插即用验证器可将下游医学QA准确率提升3.2-6.7个百分点；系统评估揭示了当前模型医学推理错误检测能力的严重缺陷。

**Conclusion:** MedPRMBench填补了医学PRM评估的空白，为改进医学推理模型提供了明确方向和可靠基准，对提升AI在医疗应用中的安全性和可靠性具有重要意义。
...
> 这篇论文提出了长时个性化(long-horizon personalization)问题，并构建了HorizonBench基准数据集来研究AI模型跟踪用户长期偏好变化的能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有AI系统难以追踪用户在数月时间跨度内的偏好演变，主要受限于缺乏包含自然长期交互和真实因果追溯的数据资源。

**Method:** 开发了基于心理状态图的数据生成器，构建了包含360个模拟用户、6个月对话历史、总计4,245个评估项目的HorizonBench基准数据集。

**Result:** 在25个前沿模型测试中，最佳模型准确率仅52.8%，大多数模型表现不高于20%的随机基线；模型错误中超过三分之一是未能追踪用户状态更新，仍使用原始偏好值。

**Conclusion:** 状态追踪能力是长时个性化的主要瓶颈，当前模型在更新用户信念方面存在系统性缺陷，这一问题在不同上下文长度和表达明确性程度下持续存在。
...
> 论文提出了一种名为'概率程序思维'的测试时框架，通过在生成的程序中暴露LLM的概率分布，能够以更少的LLM生成次数获得更多程序样本，显著降低了计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 传统的代码生成方法需要通过大量采样来获得合适的程序，每次采样都需要GPU计算，样本数量n增大时计算成本变得极其昂贵。这限制了LLM在代码生成和数学推理任务中的应用效率。

**Method:** 作者提出了一种新颖的方法，利用LLM生成的程序及其关联的下一个token概率构建概率程序，该程序能够紧凑地表征指数级数量的确定性程序。通过在概率程序中进行概率推理，可以在不需要额外GPU计算的情况下采样新程序。

**Result:** 在代码生成、代码理解和数学推理基准测试中，该方法在更少的LLM生成次数下实现了性能提升，证明了其能够在降低计算成本的同时保持或提高任务效果。

**Conclusion:** 概率程序思维框架有效解决了传统方法中高计算成本的限制问题，为LLM在需要结构化输出的任务中提供了一种更高效的解决方案，同时显著减少了对GPU资源的需求。
...
> 本研究发布了一个评估大语言模型不确定性归因能力的基准测试UA-Bench，发现当前模型难以区分数据不确定性和模型不确定性，并提出了一种轻量级的改进方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型拒绝回答时通常只是笼统地表示'我不知道'，无法区分输入歧义(数据不确定性)和模型能力限制(模型不确定性)，这种归约化的拒绝方式限制了下游决策的灵活性，如请求澄清或调用外部工具。

**Method:** 1) 构建了UA-Bench基准测试，包含超过3500个来自6个数据集的问题，涵盖知识密集和推理密集任务；2) 提出了轻量级的数据合成和强化学习策略来改进模型的不确定性归因能力。

**Result:** 对18个前沿大语言模型的评估显示，即使是最先进的模型也难以可靠地区分数据不确定性和模型不确定性，高答案准确率并不必然意味着强的不确定性归因能力。所提出的改进方法在Qwen3-4B-Instruct-2507和Qwen3-8B上取得了显著提升。

**Conclusion:** 当前大语言模型在不确定性归因方面存在明显缺陷，通过轻量级的数据合成和强化学习策略可以有效改善这一问题，为模型提供更细粒度的不确定性表达能力，这将促进更智能的下游决策系统发展。
...
> CRISP是一种创新的思维链压缩方法，通过利用模型内在的显著性机制，实现50-60%的token缩减而不损失推理精度。

<details>
  <summary>Details</summary>

**Motivation:** 长链思维推理虽然效果显著，但存在计算开销大和延迟高的问题。现有的外部压缩方法无法与模型内部推理动态对齐，导致关键逻辑步骤丢失。

**Method:** 发现推理终止标记[object Object]作为信息锚点，其注意力模式能有效区分本质推理和冗余信息。基于此设计利用内在注意力信号指导原子压缩操作的政策策略，实现细粒度的思维链蒸馏。

**Result:** 在多个骨干模型和数学数据集上，CRISP实现50-60%的token计数减少，同时保持推理精度不变，显著缓解了长上下文推理的效率瓶颈。

**Conclusion:** 通过挖掘和利用模型内部的显著性机制，CRISP为思维链压缩提供了新的技术路径，有效平衡了推理效率与准确性，为高效推理研究开辟了新方向。
...
> Cat-DPO是一种基于直接偏好优化的安全对齐算法，通过为每个危害类别设置独立的自适应安全边界来优化大语言模型的安全性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的安全对齐方法将安全性简化为单一标量，导致模型在平均情况下表现安全，但在少数危害类别上仍存在安全隐患，无法实现全面的安全保障。

**Method:** 将安全对齐转化为按类别的约束优化问题，提出Cat-DPO算法。该算法为每个危害类别设置独立的自适应安全边界，边界值会根据模型在特定类别上的表现动态调整：当模型仍产生不安全响应时边界收紧，当模型改善后边界放宽。

**Result:** 在两个LLM基础模型和六个偏好学习基线上的实验表明，Cat-DPO不仅提高了整体的有用性和无害性，还显著压缩了各类别间的安全方差和最佳-最差差距。

**Conclusion:** Cat-DPO为直接偏好安全对齐提供了一个高效的按类别精细优化方案，能够更好地平衡大语言模型的有用性和安全性要求。
...
> RoTRAG是一个检索增强框架，通过引入外部道德准则(RoTs)来提升多轮对话中有害内容检测的准确性和效率。

<details>
  <summary>Details</summary>

**Motivation:** 现有有害内容检测方法主要依赖模型内部参数化知识，缺乏外部规范性原则的明确支撑，导致在社会微妙场景中判断不一致、可解释性差且存在跨轮冗余推理问题。

**Method:** 提出检索增强框架RoTRAG，为每轮对话检索相关的人编道德准则RoTs作为规范证据进行推理，并引入轻量级二元路由分类器决定是否需要检索推理或复用现有上下文，提高效率。

**Result:** 在ProsocialDialog和Safety Reasoning Multi Turn Dialogue数据集上，RoTRAG相比基准方法F1值平均提升40%，分布误差平均减少8.4%，在不牺牲性能的同时减少了冗余计算。

**Conclusion:** RoTRAG通过检索外部道德规范增强了多轮对话有害内容检测的一致性和可解释性，路由机制有效提高了推理效率，为规范化AI对话安全评估提供了新思路。
...
> 研究发现，患者的社会身份描述（特别是性取向和宗教信仰）会导致大型语言模型的校准危机，扭曲不确定性和准确性，对临床安全部署构成重大风险。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在临床应用需要高准确性和可靠的不确定性校准，但社会身份标识对这些关键指标的影响尚未被充分研究，影响医疗公平和安全部署。

**Method:** 评估了9个通用和生物医学LLM，在2,364个医学问题及其反事实变体上进行测试，包括多重选择和开放式生成场景，分析社会身份描述对模型校准和准确性的影响。

**Result:** 发现了

**Conclusion:** 社会身份提示不仅改变预测结果，还严重影响置信度信号的可靠性，对基于置信度的临床工作流程中的公平护理和安全部署构成重大威胁。
...
> UAG是一种通用高效的生成策略，通过惩罚已生成输出之间的相似性来提升多分支多样性，在扩散和Transformer模型上均有效，具有计算开销小、速度快的优势。

<details>
  <summary>Details</summary>

**Motivation:** 现有生成模型在多分支多样性方面仍达不到人类水平创造性，现有解决方法计算开销大或对模型架构依赖性强，需要一种通用且高效的解决方案。

**Method:** 提出UAG(Universal Avoidance Generation)策略，通过对已生成输出间的相似性进行惩罚来增加多样性，该方法与模型架构无关且计算效率高。

**Result:** 实验显示，UAG方法实现了最多1.9倍更高的多样性提升，运行速度快4.4倍，相比最先进方法仅需1/64的计算量。

**Conclusion:** UAG成功解决了现有生成模型多分支多样性不足的问题，提供了一种通用、高效的解决方案，在保持计算效率的同时显著提升了生成结果的多样性。
...
> QREAM是一种风格控制的重写框架，通过将检索文档重新组织成适合LLM阅读的问答风格，解决了RAG系统中LLM偏向于流畅但虚构内容而非真实但混乱证据的问题。

<details>
  <summary>Details</summary>

**Motivation:** 尽管RAG技术能增强大模型的事实性，但LLM在面对混合上下文时存在风格偏见，倾向于选择流畅但不准确的生成内容，而忽略真实但组织混乱的检索证据。这表明检索信息的价值受到其呈现方式的限制。

**Method:** QREAM框架包含两个阶段：(1) QREAM-ICL：使用风格种子引导迭代重写探索；(2) QREAM-FT：通过从去噪的ICL输出中蒸馏出轻量级学生模型，并采用双标准拒绝采样（基于答案正确性和事实一致性）来确保高质量监督。QREAM可作为即插即用模块集成到现有RAG流水线中。

**Result:** 实验表明QREAM能够持续增强先进的RAG流水线，实现高达8%的相对改进，且延迟开销可忽略不计，有效平衡了问题相关性和事实基础。

**Conclusion:** QREAM通过将检索内容重新组织成更适合LLM处理的风格，显著提升了RAG系统的效果，为改善检索文档的利用效率提供了有效解决方案。
...
> 本文提出了一种神经符号框架，通过多智能体系统将临床指南转换为符号逻辑语言，并使用SAT求解器检测指南间的冗余和冲突，解决了多病共存患者指南不一致导致AI系统幻觉的问题。

<details>
  <summary>Details</summary>

**Motivation:** 临床指南存在碎片化、冗余和逻辑矛盾，特别是在多病共存患者中，这种不一致不仅给临床医生造成认知负担，还使RAG系统变得脆弱易产生幻觉，严重影响了医疗AI的可靠性。

**Method:** 采用神经符号框架，首先通过多智能体系统将非结构化临床自然语言翻译为严格的符号逻辑语言，然后通过SAT求解器验证逻辑一致性，并构建了逻辑规则交互的分层分类体系来识别'局部冲突'等关键冲突类型。

**Result:** 在12个SGLT2抑制剂权威指南基准测试中，发现90.6%的冲突为局部冲突；现有大语言模型无法检测这些冲突，而神经符号方法达到0.861的F1分数。

**Conclusion:** 验证了逻辑验证必须在检索之前进行，为医疗AI中的自动知识协调建立了新的技术标准，解决了传统指南系统在复杂多病情况下的根本性可靠性危机。
...
> 本书倡导逻辑计算语言学而非统计计算语言学，提出逻辑语义接口，旨在实现生命关键型NLP应用中的完美句法和语义处理。

<details>
  <summary>Details</summary>

**Motivation:** 统计依赖链的置信度会随长度增加而单调递减至零，而逻辑依赖链无论多长都能保持100%端到端置信度，这促使作者寻求更可靠的NLP方法来满足生命关键型应用的需求。

**Method:** 基于二十多年类型逻辑语法的研究成果，结合新的思想和材料，构建逻辑语义接口，强调逻辑依赖关系在语言处理中的应用。

**Result:** 建立了一个完整的逻辑计算语言学框架，能够保证任意长度依赖链的100%置信度，为高精度NLP应用提供了理论基础和技术支撑。

**Conclusion:** 逻辑计算语言学相比统计方法在生命关键型应用中具有明显优势，通过逻辑语义接口可以实现完美的语言处理能力，为高可靠性NLP系统开辟了新的发展方向。
...
> 研究者发现视觉语言模型(VLMs)在处理抽象含义（如习语）时存在字面优势偏向，高清视觉细节会干扰符号理解，提出了DIVA基准和语义对齐差距指标来量化这一问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言模型在逼真图像生成方面表现出色，但在表示抽象含义（如名词复合词的习语解释）方面存在困难。研究者想探究高视觉保真度是否会在视觉抽象下干扰习语的组合性理解。

**Method:** 研究者引入了DIVA基准测试，生成字面和习语解读的配对视觉化表示；提出语义对齐差距(Δ)指标量化字面和习语视觉基础之间的分歧；引入方向性有符号偏向b(t)分别测量字面偏向的方向和强度；评估了8个最近的VLMs模型。

**Result:** 发现了一致的字面优势偏向：仅靠模型规模无法解决字面偏向偏好，增加视觉保真度与较弱的符号对齐相关，表明超真实图像会产生认知干扰。

**Conclusion:** 改善组合性理解需要对视觉输入进行图像化抽象处理，并将解释和生成锚定在预期含义中。
...
> 该研究针对口语语言模型无法识别第三方干扰的问题，提出了TPI-Train数据集和TPI-Bench评估框架，帮助模型更好地处理多人对话中的干扰情况。

<details>
  <summary>Details</summary>

**Motivation:** 现有的口语语言模型在真实场景中无法区分主对话流程中的第三方干扰，容易被语义上下文误导而忽略关键的声学信号，导致上下文理解和互动失败。

**Method:** 研究者构建了包含88K实例的TPI-Train数据集，采用说话者感知的硬负样本设计；同时开发了TPI-Bench评估框架，用于严格测试模型在欺骗性上下文中的干扰处理策略和精确说话者区分能力。

**Result:** 实验证明，该数据集设计有效缓解了语义捷径学习问题，使模型能够优先考虑声学提示，避免了仅仅依赖语义内容而忽视说话者变化的关键缺陷。

**Conclusion:** 这项工作为克服口语语言模型中文本主导的单模态依赖问题奠定了基础资源，为构建更稳健的多方口语交互系统铺平了道路。
...
> 本文创建了首个评估大语言模型计算论证能力的综合基准，包含33个数据集和46个任务，系统分析了多种因素对LLM论证性能的影响。

<details>
  <summary>Details</summary>

**Motivation:** 论证技能对LLM在自我反思、辩论、仇恨言论反击等应用场景至关重要，但缺乏标准化的评估基准来衡量LLM的计算论证能力。

**Method:** 构建统一的计算论证基准，整合33个现有数据集，涵盖论证挖掘、观点评估、论证质量评估、论证推理和论证生成等5大类46个任务，评估5个LLM家族在这些任务上的泛化能力。

**Result:** 通过基准测试，系统分析了少样本示例、推理步骤、模型大小和训练技能对LLM计算论证任务性能的贡献。

**Conclusion:** 该研究为LLM论证能力的标准化评估提供了重要工具，有助于理解和改进LLM在各种论证相关应用中的表现。
...
> AnchorMem是一种受普鲁斯特现象启发的记忆框架，通过原子事实作为检索锚点和原始上下文作为不可变上下文，实现了细粒度检索与上下文完整性的统一。

<details>
  <summary>Details</summary>

**Motivation:** 现有记忆方法过度依赖频繁重写和总结来组织交互，这会稀释重要的上下文细微差别，使得关键检索特征变得模糊，需要更好的长期交互记忆系统。

**Method:** AnchorMem将检索单元与生成上下文解耦，从交互历史中提取原子事实作为检索锚点，同时保留原始上下文；构建关联事件图，使用高阶事件链接将相关事实绑定到共享事件表示中；检索时定位相关记忆，但通过关联的原始块和事件重建上下文。

**Result:** 在LoCoMo基准测试上，AnchorMem在三个闭源和开源模型上显著优于基线方法，证明了该方法的有效性。

**Conclusion:** AnchorMem成功解决了细粒度检索与交互上下文完整性之间的矛盾，为大语言模型的长期交互记忆提供了更有效的解决方案。
...
> 本研究通过构建跨域错误跨度标注数据集CD-ESA，发现自动评估指标在领域转移时存在表面鲁棒性，但考虑人工标注变异后鲁棒性大幅下降。

<details>
  <summary>Details</summary>

**Motivation:** 现有的机器翻译自动评估指标大多基于WMT基准训练，其在未见域的鲁棒性存在疑问。此前研究在分析跨域性能时混杂了翻译系统、标注者和评估条件的变化，无法准确分析纯粹的域转移效应。

**Method:** 创建了包含18.8k人工错误跨度标注的CD-ESA数据集，覆盖3种语言对，固定每个语言对的标注者，评估6个翻译系统在新闻域和2个未见技术域的表现，系统分析自动指标的跨域鲁棒性。

**Result:** 发现自动指标在段级表现出意外的跨域鲁棒性（最高0.69一致性），但考虑标注者变异后鲁棒性大幅降低。平均标注可提升标注者间一致性+0.11。指标在化学域表现远低于人类（0.78-0.83 vs 0.96）。

**Conclusion:** 建议在跨域评估时，应将指标与人工的一致性与标注者间一致性进行比较，而非仅使用原始的指标-人工一致性。
...
> 本文提出REGLU方法，利用表征空间的几何特性来指导低秩适配器(LoRA)的初始化和更新，实现更有效的LLM机器遗忘，在遗忘质量和模型效用上都优于现有基线方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的参数高效的机器遗忘方法在面对LLM参数的叠加现象时难以处理遗忘-保留的权衡困境，因为基于参数重要性的方法无法有效解耦遗忘集和保留集相关参数。

**Method:** REGLU方法包含两个关键组件：1) 基于表征空间的LoRA初始化，识别选择性遗忘的最优子空间；2) 正则化损失函数，约束LoRA更新输出位于保留集表征子空间的正交补空间中，从而最小化对保留集性能的干扰。

**Result:** 在TOFU和WMDP基准测试上，REGLU在多个模型上持续优于最先进的基线方法，实现了更优的遗忘质量同时保持更高的模型效用。

**Conclusion:** 通过利用表征空间的几何特性而非参数重要性，REGLU为LLM机器遗忘提供了一个更有效的新视角，成功解决了传统方法在遗忘-保留权衡上的根本限制。
...
> 提出了一种通过对比性合成文本生成和统计分析来发现不同社会群体相关语言偏见模式的方法论框架。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法主要依赖预定义词汇列表来诊断偏见，无法捕捉微妙偏见表达，且缺乏上下文语境分析。需要一种能发现细微偏见模式并考虑语境的新方法。

**Method:** 使用情景场景与群体标记的受控组合生成最小对文本文对，采用变体点间互信息量化语言抽象与群体间的关联，通过片段排序策略识别偏见信号浓度高的文本段

**Result:** 能够检测跨群体出现不成比例的语言表达，优先识别含有高浓度偏见信号的文本片段，为专家评估语言表达在语境中的潜在危害提供依据

**Conclusion:** 该框架弥合了定量分析与定性解释之间的差距，适用于任何文本体裁，为发现和评估微妙的语言偏见提供了有效工具
...
> DuConTE是一种双粒度文本编码器，通过拓扑约束注意力机制在词token和节点两个粒度上进行语义学习，显著提升了文本属性图的学习性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本属性图方法存在两大局限：1）仅在词token粒度进行语义交互，忽略了节点间的结构依赖性；2）未能充分利用图结构信息指导文本编码过程。

**Method:** 提出双粒度架构：1）级联两个预训练语言模型，依次在词token粒度和节点粒度编码语义；2）基于节点连通性动态调整注意力掩码矩阵；3）在表示组合时分别评估中心节点和邻域上下文中token的重要性。

**Result:** 在多个基准数据集上的大量实验表明，DuConTE在大多数任务上达到了最先进的性能。

**Conclusion:** 通过融合图拓扑约束和双粒度语义学习，DuConTE有效解决了传统方法的局限性，为文本属性图学习提供了一个更有效的解决方案。
...
> Jupiter-N 是一个基于 Nemotron 3 Super 的混合推理模型，通过不确定性轨迹、英国文化对齐和威尔士语支持三重目标的定制化后训练，在多项任务上显著提升性能，为主权后训练提供了可复现模板。

<details>
  <summary>Details</summary>

**Motivation:** 为了解决大语言模型在特定国家（如英国）的文化适应性、多语言支持（特别是威尔士语）和智能体能力方面的不足，同时保持开源和可复现性。

**Method:** 采用混合策略进行后训练：(1) 通过不确定性策划的轨迹增强智能体能力；(2) 基于文化规范的合成数据实现英国文化对齐；(3) 利用平行语料库和LLM翻译的威尔士语对话支持威尔士语；(4) 使用 Forget-Me-Not 框架混合策略合成回放与任务数据，防止灾难性遗忘。

**Result:** Jupiter-N 在威尔士语（ARC-Easy +18，MMLU-Lite +5.25）、终端使用（Terminal Bench 2 +9.1）和指令遵循（IFBench +4.4）方面取得显著提升，同时保持了基础模型的能力。

**Conclusion:** 该工作为任何国家的主权后训练提供了可复现模板，通过替换文化知识、机构语料库和目标语言即可实现同等效果，所有模型权重和后训练数据集都以开源许可证公开发布。
...
> 提出一种混合集成方法，结合CoT和PoT两种推理模式的自洽性技术，将所需采样数量减少9.3倍，78.6%的任务仅需2个样本即可解决。

<details>
  <summary>Details</summary>

**Motivation:** 自洽性技术虽然能提高大语言模型的推理准确性，但需要大量采样导致计算成本过高，亟需找到降低采样需求的方法。

**Method:** 构建了一个通用框架，在自洽性中结合思维链(CoT)和程序思维(PoT)两种推理模式，提出了完整采样和早期停止的具体策略。

**Result:** CoT-PoT集成不仅提高了整体准确性，还将SC所需的样本数量减少9.3倍，其中78.6%的任务仅需2个样本即可完成。

**Conclusion:** 该方法显著降低了自洽性技术的计算成本，为实际应用中的高效推理提供了可行方案，突破了以往SC方法无法用极少量样本解决问题的限制。
...
> 本文提出了一种MoVE架构，通过混合LoRA专家模型和软权重路由器，成功在语音翻译中保留了76%的非语言发声，远超现有系统的14%.

<details>
  <summary>Details</summary>

**Motivation:** 现有语音到语音翻译系统虽然语义准确，但完全丢失了笑声、哭泣等传达语用意图的非语言发声，严重限制了实际应用价值.

**Method:** 提出三点贡献：1) 构建可扩展表达性数据集的合成流程；2) MoVE架构，使用表达性专业适配器和软权重路由器混合专家模型；3) 利用预训练AudioLLMs的数据效率，仅需30分钟数据.

**Result:** 在英中S2ST任务上，MoVE在非语言发声保留率达76%，自然度和情感保真度均优于对比系统，而现有系统最多只能保留14%的非语言发声.

**Conclusion:** 通过专门设计的架构和高效的数据利用，MoVE有效解决了语音翻译中非语言发声丢失的问题，显著提升了翻译的自然度和表达力.
...
> 本文提出组合选择特异性(CSS)方法，通过将答案分解为声明并生成粗粒度回退方案，在保持高特异性的同时减少智能体系统的过度承诺问题，显著提升了风险-效用权衡。

<details>
  <summary>Details</summary>

**Motivation:** 智能体系统经常不是因为完全错误而失败，而是因为过于精确：某个回答可能整体有用，但其中的特定声明超出了证据支持范围。这种过度承诺的控制是当前系统需要解决的关键问题。

**Method:** 引入组合选择特异性(CSS)，这是一种生成后处理层，将答案分解为独立声明，为每个声明提出更粗粒度的回退方案，并在可证实的前提下以最具体的校准级别输出每个声明。该方法将不确定性表达为局部语义回退而非整体拒绝。

**Result:** 在完整LongFact运行和HotpotQA试点测试中，经过校准的CSS相比固定草稿显著改善了风险-效用权衡。在LongFact完整运行中，将过度承诺感知效用从0.846提升到0.913，同时保持了0.938的特异性保留率。

**Conclusion:** 声明级别的特异性控制是智能体系统有用的不确定性接口，也是未来分布无关有效性层的重要研究方向。
...
> CoAct框架通过协同结合自奖励和主动学习，利用人类-AI战略协作来提升LLMs对齐效果，在多个推理基准上平均提升超过8%-13%。

<details>
  <summary>Details</summary>

**Motivation:** 高人工标注成本导致高质量偏好数据稀缺，现有方法要么采用纯AI标签但不可靠，要么依赖人工标注但无法充分利用未标注数据。

**Method:** CoAct框架利用自一致性识别可靠的自标签数据和需人工验证的样本，同时通过人工反馈指导模型生成在其可解决能力范围内的新指令。

**Result:** 在GSM8K、MATH和WebInstruct三个推理基准上分别提升+13.25%、+8.19%和+13.16%，持续超越所有基线方法。

**Conclusion:** CoAct成功证明了人类-AI协同学习在LLMs对齐中的有效性，既保证了数据质量又充分利用了未标注数据，为偏好学习提供了新的解决方案。
...
> ONTO是一种为优化大语言模型token使用而设计的列式序列化格式，相比JSON可减少46-51%的token消耗，同时保持层次结构和人类可读性。

<details>
  <summary>Details</summary>

**Motivation:** 传统文档交换格式(如JSON)在LLM处理大规模操作数据时产生过多结构开销，如1000个IoT传感器读数需要约8万个token，大部分浪费在重复字段名和结构标点上。

**Method:** 提出ONTO(Object Notation for Token Optimization)列式表示法，每个实体声明一次字段名，值以管道分隔的行排列，使用基于缩进的层次结构，实现模式一次声明、数据多次使用的架构。

**Result:** 在三个合成操作数据集上，ONTO相比JSON减少46-51%的token，在Qwen2.5-7B上延迟改善5-10%，LLM任务准确性无显著下降。

**Conclusion:** ONTO填补了序列化格式的空白：兼具列式效率和层次结构，专为LLM上下文窗口优化而非文档交换，为大规模AI数据处理提供了更高效的解决方案。
...
> OPSDL是一种基于策略的自我蒸馏方法，利用模型自身的短上下文能力作为教师来监督长上下文场景下的生成，实现高效稳定的长上下文学习能力提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大型语言模型长上下文扩展方法要么依赖高质量监督数据，要么依靠稀疏的序列级奖励，导致优化过程不稳定且效率低下，无法满足实际应用需求。

**Method:** OPSDL首先让模型基于完整长上下文生成回答，然后利用模型自身的短上下文能力作为自我教师，通过在相关短上下文下的逐点反向KL散度提供密集的令牌级监督信号，鼓励模型忠实使用相关证据并减轻无关上下文引起的幻觉。

**Result:** 在7B到32B参数规模的多个模型上，OPSDL在不同上下文长度下都表现出一致且显著的改进，样本效率优于标准SFT和DPO方法，同时不损害短上下文的通用性能。

**Conclusion:** OPSDL证明了作为一种可扩展且稳定的长上下文学习方法的有效性，为实际应用中的长上下文扩展提供了新的解决方案。
...
> 提出了PoliLegalLM，一个针对政治法律领域的专业大语言模型，通过统一训练框架解决了幻觉法律引用、知识覆盖不全和结构化推理弱等关键问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有通用大语言模型在法律领域应用面临三大挑战：产生幻觉法律引用、法律知识覆盖不完整以及结构化推理能力薄弱，这些问题限制了其在专业法律场景下的实际应用价值。

**Method:** 采用统一训练框架，整合持续预训练、渐进式监督微调和基于偏好的强化学习三个阶段，构建高质量大规模法律语料库并设计结构化后训练流程，通过分阶段训练逐步增强模型的法律知识基础、任务对齐和推理能力。

**Result:** 在LawBench、LexEval和真实数据集PoliLegal三个基准测试中，PoliLegalLM表现强劲一致，超越同等规模竞争模型，与更大规模模型相比保持高度竞争性，在真实法律场景中取得最佳效果。

**Conclusion:** 研究结果证明了所提训练范式的有效性和领域专用大语言模型在实际法律应用中的实用价值，为法律AI应用提供了新的技术路径。
...
> MAPLE是一个基于元学习的自动作文评分框架，利用原型网络学习跨提示词的通用表示，在三个数据集上实现了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自动作文评分系统在跨提示词设置下面临重大挑战，需要模型能够泛化到未见过的写作提示词上。

**Method:** 提出MAPLE框架，结合元学习和原型网络来学习不同写作提示词之间的可迁移表示，实现跨提示词的作文评分。

**Result:** 在ELLIPSE和LAILA数据集上获得最先进性能，QWK分别比强基线高8.5和3分；在ASAP数据集的多个特征上也显示出改进。

**Conclusion:** 研究结果表明，元学习在构建鲁棒的跨提示词自动作文评分系统方面具有很大潜力，特别是在统一评分环境下表现突出。
...
> 本文通过无监督语义检索选择少样本示例，利用大型语言模型的上下文学习能力，提出了一种结合原理增强的干扰项生成框架，在多个基准测试中取得了最佳性能。

<details>
  <summary>Details</summary>

**Motivation:** 干扰项生成仍是一项劳动密集型任务，严重依赖领域专家。现有方法虽然能生成语义相关的干扰项，但无法捕捉专家在标准测试中选择干扰项时使用的底层推理过程。

**Method:** 设计了一种原理增强的干扰项生成框架，通过无监督语义检索选择少样本示例，结合大型语言模型的上下文学习能力，联合生成干扰项及其产生原理。

**Result:** 在六个不同干扰项长度和领域的基准测试中，与最近的干扰项生成模型相比，使用少样本示例提示LLMs显著提高了性能，超越了现有方法并达到了最佳结果。

**Conclusion:** 该方法成功解决了传统方法无法捕捉专家推理过程的问题，生成的干扰项更符合人工标准测试的要求，为干扰项自动生成提供了新的有效途径。
...
> 研究发现当前基于大语言模型的智能体缺乏环境好奇心，即使环境中明确存在任务解决方案，智能体也难以发现并利用这些意外信息。

<details>
  <summary>Details</summary>

**Motivation:** 基于LLM的智能体理论上应该能够整合环境观察信息到推理过程中，当发现高度相关但意外的信息时应该能利用这些发现。但研究表明这一假设对当前智能体不成立。

**Method:** 通过在三个基准测试(Terminal-Bench, SWE-Bench, AppWorld)中注入完整的任务解决方案来测试智能体的反应，观察其发现和利用意外信息的能力。

**Result:** 智能体在Terminal-Bench中79-81%的运行中发现解决方案，但只有37-50%会利用；在AppWorld中超过90%的情况下看到明确说明命令可返回完整解决方案的文档，但利用率低于7%。

**Conclusion:** 当前智能体缺乏环境好奇心，即识别和调查意外但相关环境刺激的能力。即使经过优化，智能体仍主要用环境获取预期信息，而非修正策略或最大化利用有用刺激。
...
> 研究通过两阶段NLP管道分析威尔士政治媒体Nation.Cymru的偏见，发现改革英国党受到的偏见性报道频率是威尔士党的两倍，负面情绪强度超过三倍。

<details>
  <summary>Details</summary>

**Motivation:** 威尔士媒体被指控存在政治偏见，但缺乏计算语言学证据验证这些指控，需要客观方法来分析媒体报道的偏见程度。

**Method:** 采用两阶段自然语言处理流程：第一阶段使用RoBERTa模型进行偏见检测，第二阶段使用大语言模型对检测到的偏见进行目标属性的情感分类分析。

**Result:** 对15,583个政党提及的分析显示，改革英国党面临偏见性报道频率是威尔士党的两倍，平均负面情绪强度超过三倍；威尔士党作为例外，获得了相对更有利的报道框架。

**Conclusion:** 研究证实了单一威尔士政治媒体存在可测量的差异化报道偏见，支持了对威尔士媒体覆盖范围进行更广泛审查的呼吁，并提供了可复制的低成本分析框架。
...
> 通过精细监控多语言1.7B模型预训练过程，发现跨语言泛化经历两个阶段：初期以复制和表面相似性主导，后期发展出更泛化的翻译机制。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究对大语言模型跨语言能力的分析过于孤立和稀疏，缺乏对早期学习过程中跨语言泛化如何产生的深入理解。

**Method:** 在9种不同语言上预训练1.7B模型，以更高粒度捕获检查点；引入词级翻译数据集，通过行为分析、模型组件分析和参数消融实验追踪翻译能力发展。

**Result:** 模型快速获得基础语言能力和词级复制能力；翻译发展经历两个阶段：第一阶段以复制和表面相似性为主导，第二阶段发展更泛化的翻译机制同时优化复制能力。

**Conclusion:** 该研究为多语言预训练过程中跨语言泛化的发展提供了细粒度的视图，揭示了翻译能力从复制到泛化的演化路径。
...
> ThreadSumm是一个多阶段LLM框架，通过层次化推理和思维树搜索，改进深度嵌套讨论线程的摘要生成，提升了逻辑结构、主题保留和观点覆盖。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LLM摘要生成器在处理深度嵌套的讨论线程时存在困难，无法可靠地捕捉交错回复、引用和重叠主题。

**Method:** 提出多阶段框架：首先通过LLM提取话语方面和原子内容单元进行内容规划，然后应用句子排序构建线程感知序列，最后使用思维树搜索生成和评估多个段落候选，在统一搜索空间中联合优化连贯性和覆盖度。

**Result:** 相比现有基线方法，ThreadSumm在生成逻辑结构化摘要方面表现改进，在嵌套讨论中实现了更高的主题保留率和观点覆盖率。

**Conclusion:** 多提案和迭代优化设计有效解决了讨论线程摘要的复杂性，为处理深度嵌套对话提供了可解释且高效的解决方案。
...
> 研究发现，自然提示分布变化会导致部署的大语言模型性能大幅下降（平均损失73%），需要数据驱动监控来保证模型稳定性。

<details>
  <summary>Details</summary>

**Motivation:** 大语言模型在动态现实环境中部署时，用户提示分布会随时间、任务和用户群体变化而发生自然偏移，这对模型可靠性构成重大挑战，但相关研究非常有限。

**Method:** 提出LENS框架，通过数据驱动方法量化自然提示分布偏移，在192个真实部署场景中评估，训练81个模型，共使用468万训练提示和5.76万评估提示。

**Result:** 即使是中等程度的提示行为变化也导致部署LLM性能大幅下降（平均损失73%），性能退化在不同潜在用户群体和地理区域间尤为明显，并与时间相关的自然提示分布偏移呈相关性。

**Conclusion:** 研究结果强调了数据驱动监控的必要性，以确保LLM性能在多样化且不断演变的用户群体中保持稳定。
...
> 研究发现语义密度效应(SDE)：高语义密度的提示词能显著提升大模型输出的准确性、专注度并减少幻觉，无需增加计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 现有的提示优化技术（如思维链、提示重复等）通常需要增加token数量或改变提示结构，存在效率和成本问题。需要一种更高效的提示优化方法。

**Method:** 提出语义密度效应(SDE)概念，通过计算语义承载token与总token的比例（考虑冗余度和具体性调整），采用精简或替换低信息token的方式来提高提示密度。

**Result:** 在5个主流大模型和7个基准测试中，超高密度提示(SDE>0.80)平均性能提升8.4%，与指令位置效应结合可达11.7%，无额外token和延迟开销。

**Conclusion:** SDE为提示工程提供了新的优化维度，证明了简洁性和语义密度的重要性，为低资源环境下的AI应用提供了有效解决方案。
...
> Peerispect是一个交互式系统，用于自动验证同行评审中的声明是否具有事实依据，通过提取声明、检索证据并进行自然语言推理来提高评审的公平性和问责制。

<details>
  <summary>Details</summary>

**Motivation:** 同行评审是科学出版的核心，但评审中经常包含主观性、修辞性或与提交文稿不符的声明。手动检查这些声明的事实依据在现代会议和期刊的规模下是不可行的，因此需要自动化工具来确保评审的公平性和问责制。

**Method:** Peerispect采用模块化信息检索管道，包括：1)从同行评审中提取值得检查的声明；2)从手稿中检索相关证据；3)通过自然语言推理验证声明真实性。系统通过可视化界面在论文中直接高亮显示证据，支持不同的检索器、重排序器和验证器模块。

**Result:** 开发了一个完整的交互式系统，提供可视化界面用于快速检查和解释验证结果，同时提供公开的演示网站、API服务和视频教程，服务于评审者、作者和程序委员会成员。

**Conclusion:** Peerispect成功实现了声明级别的同行评审验证，通过模块化设计和用户友好的界面，为提高科学出版中同行评审的质量和效率提供了实用的技术解决方案。
...
> 提出一种轻量级法律文档引用分类框架，结合词形还原处理、子词感知FastText嵌入和多核一维CNN，在97.26%准确率下超越BERT等基线模型，速度提升13倍。

<details>
  <summary>Details</summary>

**Motivation:** 法律从业者面临日益增长的案例法文档，这些文档具有正式化语言、长句结构和专业术语特点，使得人工分类既耗时又容易出错，需要高效自动化的解决方案。

**Method:** 构建基于词形还原的预处理管道，使用子词感知的FastText词嵌入，采用多核一维卷积神经网络进行分类，在25,000个标注法律文档上进行75/25训练测试分割。

**Result:** 系统达到97.26%分类准确率和96.82%宏F1分数，AUC-ROC为97.83%，仅需510万参数，推理延迟0.31毫秒/文档，比BERT快13倍以上。消融实验验证各组件贡献，剩余错误主要限于语义相邻的引文类别。

**Conclusion:** 精心设计的卷积架构为智能法律文档分析提供了可扩展、资源高效的替代方案，是在轻量级条件下实现高精度分类的有效途径。
...
> 研究者将临床人格评估的效度框架应用于20个前沿大语言模型的元认知探针数据，发现6个模型存在不同程度的效度问题，有效度问题的模型与有效效度的模型在认知监控方面存在显著差异。

<details>
  <summary>Details</summary>

**Motivation:** 临床人格评估在解释实质量表前会评估反应效度，但大语言模型评估缺乏这一环节，需要建立类似的效度筛选机制来确保评估结果的可靠性。

**Method:** 基于PAI和MMPI-3的效度框架，对20个LLM在524个项目上的元认知探针数据进行6种效度指标操作化(L、K、F、Fp、RBS、TRIN)，建立分层分类系统识别无效配置。

**Result:** 识别出4个构造水平无效模型和2个高水平模型；有效配置模型产生项目敏感信心(r=.18)，无效配置模型则不产生(r=-.20)，效应量d=2.17；思维链训练产生两种相反的反应扭曲。

**Conclusion:** LLM评估确实需要效度筛选机制，研究发现了两个潜在维度可解释94.6%的指标方差，为LLM评估提供了临床心理学框架下的效度检验方法。
...
> 本文提出了DeInfer，一个专门用于分解后大语言模型并行推理的高性能推理系统，通过多种优化技术解决了现有方法在模型规模扩大时并行推理性能差的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型分解工作主要关注提升下游任务性能，但忽略了模型规模扩大时并行推理性能差的重要问题，这限制了其实际应用。

**Method:** 设计并实现了DeInfer推理系统，集成了多种优化技术，旨在最大化并行推理性能，同时与最先进的优化技术保持兼容。

**Result:** 通过大量实验验证，DeInfer展现出优异的性能表现，能够显著促进分解后大语言模型的并行推理效率。

**Conclusion:** DeInfer系统有效地解决了分解LLM并行推理的性能瓶颈，为大规模语言模型的高效推理提供了重要的技术支撑。
...
> 该研究将临床人格评估中的效度筛查原则转移到LLM置信度信号评估中，提出了一套基于2x2列联表的便携式筛查协议，可有效识别提供无效置信度信号的LLM模型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LLM置信度信号被广泛应用于拒绝决策、路由选择和安全关键场景，但缺乏标准化的方法来检验这些置信度信号是否真正包含项目级信息，这可能导致基于无效置信度信号的错误决策。

**Method:** 研究者从临床人格评估(PAI, MMPI-3)移植效度筛查原则，制定了基于基准的LLM置信度数据评估协议。该协议包含从单个2x2列联表计算的三个核心指数(L, Fp, RBS)、一个结构指示器(TRIN)和项目敏感性统计，并采用三层次分类系统(无效、不确定、有效)。

**Result:** 在20个前沿LLM和524个项目上的验证显示，4个模型被分类为无效，2个为不确定。有效模型的平均相关性r=0.18(15/16个显著)，无效模型平均相关性r=-0.20(d=2.48)。在18个模型上的跨基准验证和使用外部数据的验证都确认了筛查方法的可移植性。

**Conclusion:** 该研究提出的效度筛查协议为评估LLM置信度信号质量提供了标准化方法，能够有效区分提供有效和无效置信度信号的模型，对于依赖置信度信号的实际应用具有重要意义。
...
> 有效性筛选能够将LLM置信度信号分类为三级（有效、不确定、无效），其中有效模型在选择性预测性能上显著优于无效模型（Cohen's d = 2.81），该分类系统对预测性能具有重要影响。

<details>
  <summary>Details</summary>

**Motivation:** 研究旨在验证有效性筛选分类系统是否能够准确预测大语言模型的选择性预测性能，以帮助识别哪些模型更适合高精度任务场景。

**Method:** 对20个前沿LLM进行524项跨6个认知轨道的评估，使用有效性筛选将模型分为三级，通过AUROC指标、Cohen's d效应量和分半交叉验证等方法进行分析。

**Result:** 有效模型平均AUROC=0.624，无效模型平均AUROC=0.357，效应量d=2.81(p=0.002)；三级分类呈单调递增：无效<不确定<有效；该分类解释了47%的AUROC方差。

**Conclusion:** 有效性筛选是预测LLM选择性预测性能的重要指标，高精度模型在降低覆盖范围时准确率会显著下降，该筛选系统对于模型选择具有重要意义。
...
> 大型语言模型在隐含文化情境下的语用感知能力有限，仅能恢复约20%的显式文化指令产生的语用变化，表明多语言文化语用学更多是显式-隐性部署问题而非纯知识问题。

<details>
  <summary>Details</summary>

**Motivation:** 研究LLMs是否能在隐含文化线索的情况下改变说话方式，而不仅仅是回答直接的文化问题，探索模型的文化语用敏感性。

**Method:** 在5种语言中评估60个文化对话场景，比较三种提示条件：中性基线、显式文化指令和隐含情境线索，通过12个语用特征评分，定义语用情境敏感性(PCS)指标。

**Result:** LLMs的PCS均值仅为0.196，权威相关线索转移最强(0.299)，个体-群体框架最弱(0.120)；印地语和乌尔都语配对分析显示无显著差异，模型主要响应语言结构而非文化内涵。

**Conclusion:** 多语言文化语用学是显式-隐性部署问题，模型难以从隐含情境中推断文化规范，需要改进对不同文化背景的语用模式理解。
...
> RePrompT是一种结合结构化EHR编码器和提示调优的时间感知LLM框架，通过保留纵向信息和注入群体级模式，解决了LLM处理结构化电子健康记录时的两个关键挑战。

<details>
  <summary>Details</summary>

**Motivation:** 当前LLM在处理结构化电子健康记录时面临两个主要问题：1)将时间戳EHR序列转换为纯文本会模糊时间结构和代码身份；2)传统LLM采用案例独立推理，无法利用群体级别的模式信息。

**Method:** RePrompT框架通过提示调优集成结构化EHR编码器，无需修改底层架构。具体包括：通过周期性整合先前访问的潜在状态来保留纵向信息，通过从队列训练的EHR编码器衍生的可训练提示词注入群体级信息。

**Result:** 在MIMIC-III和MIMIC-IV数据集上的实验表明，RePrompT在多个临床预测任务上持续优于基于EHR和基于LLM的基线方法。

**Conclusion:** RePrompT成功地将LLM的推理能力与结构化EHR编码器的群体级学习优势相结合，为临床预测任务提供了一个有效的解决方案，同时保持了模型架构的完整性。
...
> 本文提出了R-MHSafe心理安全分类体系和MHSafeEval评估框架，通过对抗性多轮交互识别大语言模型在心理咨询中的安全风险，发现现有静态基准无法检测的累积性安全失效。

<details>
  <summary>Details</summary>

**Motivation:** 现有评估方法主要关注孤立响应，使用粗粒度分类或静态数据集，无法有效诊断多轮心理咨询交互中海量的产生和累积过程，亟需更精细化的评估框架来捕获临床相关的安全风险。

**Method:** 开发了R-MHSafe角色感知的心理安全分类法，将临床重要伤害与AI咨询师交互角色（施害者、煽动者、促成者、纵容者）相结合；提出MHSafeEval闭环评估框架，通过基于代理的对抗性多轮交互和角色感知建模，在轨迹层面发现伤害。

**Result:** 大规模评估显示主流LLM存在显著的角色依赖性和累积性安全失效，这些失效被现有静态基准系统性忽略；所提框架显著提升了失效模式覆盖率和诊断粒度。

**Conclusion:** 研究表明通过角色感知分类和基于代理的交互评估，能够有效识别传统方法遗漏的安全风险，为LLM在心理健康领域的安全评估提供了新的方法论框架。
...
> mira-embeddings-v1是一个基于LLM合成数据的招聘领域语义重排序系统，通过五阶段提示工程生成训练数据，采用LoRA双重适应策略，配合边界感知重排序头，显著提升了候选人的召回率和准确率。

<details>
  <summary>Details</summary>

**Motivation:** 招聘领域候选人的两个阶段检索和重排序流程中，上游检索器返回的候选短列表需要进一步优化排序，使合格候选人的排名尽可能靠前，同时避免大规模人工标注的成本。

**Method:** 从真实职位描述出发，构建五阶段提示管道生成多样化的正样本和困难负样本；应用两轮LoRA适应：JD-JD对比训练和JD-CV三组对齐；最后使用BoundaryHead MLP对Top-K结果进行重排序以区分职位差异。

**Result:** 在300个真实JD的本地测试中，Recall@50从68.89%提升到77.55%，Precision@10从35.77%提升到39.62%；在44,138名候选人的全局测试中，Recall@200达到0.7047，基线为0.5969。

**Conclusion:** LLM合成的监督配合边界感知重排序可以在不需要重型交叉编码器的情况下实现鲁棒的性能提升，为招聘系统的候选人推荐提供了有效解决方案。
...
> 提出一种分层多智能体系统HiRAS，通过监管管理智能体协调专门智能体实现端到端实验复现，并改进Paper2Code评估标准，性能提升超10%

<details>
  <summary>Details</summary>

**Motivation:** 现有大型语言模型在自动化计算研究方面潜力巨大，但现有固定顺序智能体管道存在全局协调能力弱的问题，限制了其鲁棒性和整体性能

**Method:** 构建分层研究智能体系统(HiRAS)，采用监管管理智能体协调细粒度阶段的专业智能体；同时改进Paper2Code基准的无参考评估，引入仓库级信息的P2C-Ex协议

**Result:** 在广泛评估中验证了方法的有效性和鲁棒性，使用开源骨干模型相比之前最先进方法实现超10%相对性能提升，显著减少评估中的幻觉现象

**Conclusion:** 分层多智能体框架和改进的评估协议有效提升了实验复现的性能和可靠性，为自动化计算研究提供了更强大的工具
...
> R-CAI是一个自动化的对抗数据生成框架，通过逆转无害性准则来系统合成高质量的有毒数据，并引入概率钳制技术解决语义退化问题。

<details>
  <summary>Details</summary>

**Motivation:** 当前大型语言模型的安全性需要鲁棒的红队测试，但系统化合成高质量有毒数据的方法仍缺乏充分探索。现有的越狱提示方法过于零散，缺乏系统性和可控性。

**Method:** 提出逆向宪法AI框架：1）将无害性准则逆转为有害准则；2）通过批判-修订管道迭代优化模型输出；3）在RLAIF中引入概率钳制技术，在保持对抗意图的同时稳定 adversarial 优化过程。

**Result:** 实验表明R-CAI能生成多样化、高质量的有毒数据，概率钳制显著提升语义连贯性15%，同时不损失对抗强度。

**Conclusion:** R-CAI为红队数据生成和语言模型安全性评估提供了完全自动化的框架，实现了系统化的对抗数据合成而无需人工标注。
...
> SPENCE是一个用于检测NL2SQL基准测试数据污染的语法探测框架，通过生成语法变体测试多个LLMs，发现较早发布的基准测试(如Spider)存在明显的训练数据泄漏，而较新的BIRD数据集几乎未受污染。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在NL2SQL基准测试上表现优异，但由于训练过程中可能接触到基准查询或相似模式，导致准确性可能被高估。需要开发一种系统性方法来检测和量化这种数据污染效应。

**Method:** 提出SPENCE框架，对Spider、SParC、CoSQL和BIRD四个基准测试集系统生成语法变体；在执行评分下评估多个LLMs；测量语法分歧增加时的执行准确率变化；使用Kendall's tau与bootstrap置信区间量化排序敏感性。

**Result:** 观察到明显的时间梯度：较旧的基准测试如Spider呈现最强的负值，表明训练泄漏可能性最高；而较新的BIRD数据集显示最小敏感性，似乎基本未受污染。

**Conclusion:** 研究强调了在NL2SQL基准测试中进行时间语境化和语法探测评估的重要性，为构建更可信的评估体系提供了方法论指导。
...
> 本文提出基于熵的标记加权(ETW)方法，利用预测分布的熵作为标记信息量的代理，实现更有效的大语言模型遗忘学习。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大语言模型遗忘学习方法在应用遗忘损失时未考虑标记级别的语义重要性，导致模型效用不必要地降低。当前基于标记的损失正则化方法依赖于真实置信度或外部语言解析器，限制了其捕捉上下文信息和模型整体预测状态的能力。

**Method:** 提出熵引导标记加权(ETW)方法。该方法利用预测分布的熵作为标记信息量的代理，其中信息性标记具有较高熵，结构性标记具有较低熵。根据这一特性为不同标记分配不同的权重进行遗忘学习。

**Result:** ETW方法能够在实现更有效遗忘学习的同时，更好地保持模型效用。实验证明信息性标记倾向于具有较高的熵，而结构性标记倾向于具有较低的熵，这一特性使得ETW比现有的标记级方法表现更优。

**Conclusion:** 基于信息论原理的ETW方法为大语言模型的遗忘学习提供了新的思路，通过熵值量化标记信息量，在安全性和效用性之间取得了更好的平衡。
...
> 该研究通过测试时缩放策略提升了Qwen3-1.7B在小样本模型上的越南语数学推理能力，发现监督微调能显著改善解释质量，而简单的思维链比复杂的代理框架更适合边缘设备部署。

<details>
  <summary>Details</summary>

**Motivation:** 推动普及AI需要在资源受限设备上部署高级推理能力，但小样本模型存在'推理鸿沟'，在越南语等非英语场景下难以保持连贯的思维链条。

**Method:** 构建了越南语数据集Vi-S1K和评估基准Vi-Elementary-Bench，采用LLM-as-a-Judge协议，对Qwen3-1.7B进行监督微调，比较CoT、ReAct等不同提示策略的效果。

**Result:** 基础模型具有潜在知识但存在严重格式化问题；SFT使解释质量提升77%；在1.7B参数下，纯CoT+自我一致性优于ReAct等结构化框架；建立了SLM部署层次结构。

**Conclusion:** 对于边缘设备上的推理任务，采用SFT结合简化的测试时缩放策略优于复杂的代理工作流程，为小样本模型在实际应用中的部署提供了重要指导。
...
> PDDL-Mind是一个神经符号框架，通过将叙事描述转换为PDDL语言表示的显式状态，为LLM提供逻辑一致的世界状态表示，从而显著提升心理理论任务性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究表明，即使使用思维链提示或概率信念更新，大型语言模型在心理理论基准测试上的表现仍远低于人类水平。作者认为这主要源于不可靠的隐式状态跟踪而非高级推理能力的限制。

**Method:** 提出PDDL-Mind神经符号框架，将环境状态演化与信念推理解耦。框架通过将叙事描述转换为规划领域定义语言(PDDL)的显式状态和动作表示，并根据预定义领域验证动作引发的状态转换。

**Result:** 在MMToM-QA、MuMA和FanToM基准测试中，PDDL-Mind相比现有最优方法在心理理论基准问题上实现了超过5%的绝对准确率提升。

**Conclusion:** 通过为LLM提供逻辑一致且显式的世界状态表示，PDDL-Mind有效解决了隐式状态跟踪的不可靠性问题，显著提升了模型在心理理论任务上的表现。
...
> 本文提出了一种动态协作框架，让小语言模型SLM主动请求大语言模型LLM的协助，同时LLM提供自适应反馈，实现了效率与性能的平衡。

<details>
  <summary>Details</summary>

**Motivation:** LLM功能强大但存在成本和隐私问题，SLM支持高效私有本地推理但能力有限，需要结合两者优势构建更好的解决方案。

**Method:** 设计动态协作框架，SLM在多步推理中主动决定何时请求LLM，LLM提供自适应反馈而非被动响应，并系统研究协作策略如何受模型能力和约束条件影响。

**Result:** 实验显示强SLM更自力更生，强LLM实现更少但更有效的交互；动态协作策略显著优于静态管道和独立推理，且能鲁棒迁移到未见过的LLM上。

**Conclusion:** 该框架成功实现了SLM与LLM的优势互补，为实际应用中的效率、隐私和性能权衡提供了有效解决方案。
...
> 本研究通过多指标判别框架分析印度河文字系统，发现其既不符合语言系统也不完全匹配非语言基线系统

<details>
  <summary>Details</summary>

**Motivation:** 印度河文字系统是否编码口语语言存在长期争议，需要通过量化方法与其可能的非语言解释进行比较验证

**Method:** 建立包含四个核心指标（文本简洁性、重复公式化短语、单现词汇率、位置刚性）的多指标判别框架，生成校准的纹章式和行政式编码两种非语言基线，与1916条去重印度河铭文进行比较分析

**Result:** 印度河语料库在四个指标上均介于两种基线系统之间，无法被任一非语言系统完全重现；与7个真实非语言语料库比较也发现无法完全匹配

**Conclusion:** 印度河文字系统既非纯粹语言也非已知非语言系统，其本质可能介于两者之间或代表独特的符号系统类型
...
> QuickScope是一种基于贝叶斯优化的新方法，能够高效识别动态LLM基准测试中的困难问题，相比基线方法更具样本效率和准确性。

<details>
  <summary>Details</summary>

**Motivation:** 动态LLM基准测试虽然灵活，但评估成本高昂，特别是在需要可靠识别模型弱点的场景下。传统方法难以高效地从无限问题变体中找出真正困难的测试用例。

**Method:** 基于COUP贝叶斯优化算法进行实质性改进，使其适配实际LLM评估流程，并封装成支持灵活数据集和效用函数选择的工具，允许用户针对特定类型问题（如低准确率、相对于复杂度过低等问题）进行目标优化。

**Result:** 在多个基准测试实验中，QuickScope比标准基线方法更样本高效地发现真正困难的问题，同时降低了噪声结果导致的假阳性率。

**Conclusion:** 该方法为动态LLM基准测试提供了高效的困难问题识别解决方案，显著提升了评估效率和质量，为模型弱点分析提供了实用工具。
...
> 研究通过训练二维神经细胞自动机(NCA)处理算术表达式语法，发现其能自发组织成Proto-CKY表示，实现超越常规语言的句法处理能力。

<details>
  <summary>Details</summary>

**Motivation:** 探索句法处理是否能从纯局部交互而非外部监督中自发生成，寻求在最小系统中验证计算与物理系统的内在关系。

**Method:** 训练18,658参数的二维神经细胞自动机，仅用1位边界信号监督，学习算术表达式语法的成员资格问题，分析其内部网格的自组织行为。

**Result:** NCA自发形成Proto-CKY表示，具备超越规则语言的表达力、结构泛化能力，内部组织与语法结构高度相关(r≈0.71)，在四种上下文无关语法中独立出现，扰动后能自发再生。

**Conclusion:** Proto-CKY是CKY算法在物理基底上的具体实现原型，两者间的系统性差异反映了物理基底的信息，证明了句法处理可以从局部交互中自发涌现。
...
> LAnR提出了一个统一的RAG框架，让单个LLM在内部潜在空间中完成编码、检索和生成，通过产生密集检索向量替代文本查询，并自适应地判断检索是否充分。

<details>
  <summary>Details</summary>

**Motivation:** 现有RAG系统需要在每一步生成自然语言查询，并保持检索器和生成器的严格架构分离，这限制了LLM充分利用其表征能力，同时存在推理效率低的问题。

**Method:** LAnR使用单个LLM联合执行编码、检索和生成，通过[PRED]标记的隐藏状态产生密集检索向量，使用相同模型编码文档表示，并通过轻量级MLP控制头自适应决定何时停止检索。

**Result:** 在6个QA基准测试上，LAnR在单跳和多跳设置中都优于现有RAG方法，同时通过减少检索次数和更紧密的模型集成提高了推理效率。

**Conclusion:** LAnR通过消除分离的检索器和显式令牌级停止推理，统一了RAG架构，证明了在LLM潜在空间内进行端到端信息检索的可行性和优势。
...
> GraSP是一个基于技能图的LLM智能体架构，通过结构化的技能编排机制替代传统平面技能集，显著提升了任务执行效率，解决了技能库增大反而性能下降的问题。

<details>
  <summary>Details</summary>

**Motivation:** 研究发现，为LLM智能体提供更多技能并不会单调提升性能，过多技能反而会损害效果。瓶颈已从技能可用性转向技能编排，智能体需要结构化机制来选择、组合和执行具有明确因果依赖的技能。

**Method:** GraSP引入编译层，将平面技能集转换为具有前置条件-效果边的类型化有向无环图(DAG)，通过节点级验证执行，并使用五种类型化算子执行局部边界修复，将重规划复杂度从O(N)降低到O(d^h)。

**Result:** 在ALFWorld、ScienceWorld、WebShop和InterCode等基准测试中，GraSP在所有配置下均优于ReAct、Reflexion、ExpeL等基线方法，奖励最高提升19点，环境交互步骤减少41%。

**Conclusion:** 结构化的技能编排而非更大的技能库是实现可靠智能体执行的关键。GraSP在任务复杂度增加时优势更明显，且对技能过度检索和质量退化具有鲁棒性。
...
> 这项研究提出了MPT基准和PRefine方法来解决LLM代理工具调用中用户偏好信息不足的问题，通过记忆增强的假设推理机制显著提高了工具调用的准确性和效率。

<details>
  <summary>Details</summary>

**Motivation:** 用户与LLM代理交互时经常省略关键细节，导致工具调用的输入信息不完整，而API执行又需要完整的参数，这构成了工具增强代理的基本挑战，因此需要个性化的工具调用解决方案。

**Method:** 研究者构建了包含265个多会话对话的MPT基准，涵盖偏好回忆、归纳和迁移三个挑战；同时提出PRefine方法，将用户偏好表示为演化假设，通过生成-验证-精炼循环从历史中提取可重用约束。

**Result:** PRefine方法在提高工具调用准确率的同时，仅使用完整历史提示所需1.24%的token数量，证明了其在个性化代理系统中的高效性。

**Conclusion:** 研究表明，代理系统的稳健个性化不仅需要记住用户的选择，更需要捕获选择背后的原因记忆机制，为智能代理的个性化发展指明了新方向。
...
> DynaSlide是一个大规模幻灯片动态更新基准测试，SlideAgent是基于智能体的解决方案，能够根据自然语言指令自动更新用户自定义模板的演示文稿。

<details>
  <summary>Details</summary>

**Motivation:** 数据驱动的演示文稿报告更新 labor-intensive，现有自动化方法主要基于固定模板填充，无法支持用户自定义模板的动态更新需求。

**Method:** 提出SlideAgent智能体框架，结合多模态幻灯片解析、自然语言指令定位和工具增强推理技术，针对表格、图表和文本结论进行内容更新，同时保持布局和样式。

**Result:** 构建了包含20,036个真实世界指令-执行三元组的DynaSlide基准数据集，SlideAgent在该基准上提供了强有力的基线性能验证。

**Conclusion:** 设计了端到端和组件级评估协议，揭示了未来研究的关键挑战和机遇，为自动化幻灯片动态更新领域提供了新的研究方向和基准。
...
> 研究发现，通过在预训练数据中仅添加1%的目标语言学现象合成数据，小型语言模型在9个表现最差的BLiMP范式中有8个获得显著改善，证明了数据组成对提升模型语言能力的重要性。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型在形式语言能力上表现出令人困惑的差异性：某些语言现象能近乎完美掌握，而其他现象即使训练数万亿token仍表现不佳。研究者想探究这是否源于架构限制或仅仅是语料库中特定语法结构稀缺。

**Method:** 使用GPT-2 Small（124M）模型在1亿token的FineWeb语料样本上进行预训练，并干预注入仅1%针对特定语言学现象的合成数据，然后通过BLiMP基准测试评估模型性能变化。

**Result:** 目标干预显著改善了9个最差BLiMP范式中的8个，其中only_npi_scope范式准确率从20.9%跃升至69.4%。干预通常保持或略微改善整体性能。但principle_A_c_command现象仍表现顽固，准确率仍低于随机水平。

**Conclusion:** 研究提供了乐观的存在证明：只要预训练数据包含足够的暴露，即使是小型语言模型也能大幅改善通常表现差的语言现象。这表明迈向人类规模语言建模的努力应重点关注数据组成。
...
> DoRA是一个面向特定领域的RAG评估基准，通过国防文档构建意图条件QA，能够更准确地评估模型在实际部署中的性能，显著减少幻觉率。

<details>
  <summary>Details</summary>

**Motivation:** 传统的开放域RAG基准测试会因预训练重叠和弱归属要求而高估部署性能，需要更贴近实际应用场景的评估方法。

**Method:** 构建基于国防文档的领域导向评估基准DoRA，包含6.5K个通过意图条件QA生成的实例，涵盖五种问题类型，并配备可审计的证据段落进行归属验证。

**Result:** 在端到端评估中，通用语言模型表现相似，而基于DoRA训练的模型相比基线模型在QA成功率上提升26%，在RAG忠实度评分中幻觉率降低47%。

**Conclusion:** DoRA为领域偏移下的污染感知回归测试提供了有效支持，证明了面向特定领域的RAG评估在实际部署中的重要性和价值。
...
> ReCoQA是一个包含29,270个房地产实例的大规模基准数据集，用于评估结合数据库查询和API调用的混合工作流；同时提出了HIRE-Agent分层框架作为强基线方法。

<details>
  <summary>Details</summary>

**Motivation:** 目前缺乏能够反映混合工作流（结合数据库查询与外部API）的基准数据集，这使得开发能够导航碎片化多源信息的智能体面临挑战。

**Method:** 构建ReCoQA基准数据集，包含机器可验证的中间步骤监督（结构化意图标签、SQL查询、API调用）；提出HIRE-Agent分层框架，采用理解-计划-执行架构，通过前端解析器、规划监督器和执行专家协调异构证据。

**Result:** 大量实验表明HIRE-Agent构成了强基线，并验证了分层协作对于复杂现实世界推理任务的必要性。

**Conclusion:** ReCoQA数据集和HIRE-Agent框架为解决碎片化信息导航问题提供了新的评估基准和解决方案，证明了分层方法在复杂推理任务中的有效性。
...
> 本文提出了一种基于PDDL规划问题的大规模流程奖励模型(PRM)数据集生成方法，能够显著提升PRM在数学和非数学推理任务上的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的PRM数据集存在构建成本高、标注错误多、且主要局限于数学领域的问题，亟需一种可扩展且多样化的数据生成方法来训练更通用的PRM。

**Method:** 使用规划领域定义语言(PDDL)表示的逻辑规划问题生成了约100万个推理步骤的数据集，将该数据与现有PRM训练数据结合使用，训练新的流程奖励模型。

**Result:** 在多个基准测试中，结合PDDL数据的PRM在数学和非数学推理任务上均显示出显著改进，证明了规划问题作为PRM训练资源的有效性和可扩展性。

**Conclusion:** 规划问题为PRM训练数据生成提供了一个有效、精确且细粒度的可扩展资源，能够突破传统数学数据源的局限，为PRM的发展开辟了新的途径。
...
> 本研究将情感支持对话重新定义为多策略话语生成任务，提出了两种生成方法并通过认知推理增强策略选择与响应组合，实验证明多策略方法可显著提升对话质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有情感支持对话工作假设每个支持者回合仅使用单一策略，而实际情况中一个话语通常包含多种支持策略。这种简化假设限制了真实情感支持的建模能力。

**Method:** 提出两种多策略话语生成方法：All-in-One方法一次性预测所有策略-响应对；One-by-One方法迭代生成策略-响应对。两种方法均通过强化学习引导的认知推理来改进策略选择和响应组合。

**Result:** 在ESConv数据集上的实验表明，所提方法能有效建模多策略话语，在话语级和对话级评估中都显著提升了支持质量和对话成功率。

**Conclusion:** 这是首个系统性实证证明允许单个话语包含多种支持策略是可行且有益于情感支持对话的研究，为该领域提供了新的研究方向。
...
> 本文提出了ltzGLUE，这是第一个基于卢森堡语(LTZ)的自然语言理解(NLU)基准测试，用于评估预训练语言模型在该语言上的性能。

<details>
  <summary>Details</summary>

**Motivation:** 卢森堡语作为官方国家语言之一，在现有的自然语言理解任务中经常被忽视，而目前大部分欧洲语言已有相应的NLU基准测试。

**Method:** 基于流行的英语GLUE基准测试框架，构建新的任务并复用现有任务，包括命名实体识别、主题分类和意图分类等常见的NLP任务。评估多种预训练语言模型在卢森堡语上的表现。

**Result:** 建立了卢森堡语第一个官方NLU基准测试，并完成了对多种预训练编码器模型在该语言上的能力评估，为卢森堡语NLP研究提供了基础。

**Conclusion:** ltzGLUE填补了卢森堡语NLU基准测试的空白，为该语言的自然语言处理研究和模型开发提供了标准化的评估框架和性能基线。
...
> 通用大语言模型在药物流行病学研究设计支持上优于专门的生物医学LLM，提示策略对性能影响显著。

<details>
  <summary>Details</summary>

**Motivation:** 评估大型语言模型在自动化和支持药物流行病学研究设计方面的潜力和可靠性，比较通用模型与专业生物医学模型的表现差异。

**Method:** 使用46份HMA-EMA和Sentinel系统协议(2018-2024)，评估GPT-4o、DeepSeek-R1与生物医学微调LLM在相关性、论证逻辑性和本体论代码映射方面的表现，采用Least-to-Most和Active Prompting策略。

**Result:** GPT-4o和DeepSeek-R1配合LTM提示策略获得最高相关性和逻辑论证得分，其中GPT-4o-LTM在HMA-EMA协议中8/9问题中位数相关性得分达4分。生物医学LLM整体相关性较低且论证不足。所有LLM在本体论代码映射方面能力有限，但LTM提供了最一致的推理稳定性改善。

**Conclusion:** 现成的通用LLM目前比生物医学LLM更能支持药物流行病学研究设计，提示策略对LLM性能表现有强烈影响。
...
> 本研究首次将分子生成的核心需求重新定义为创造力概念，通过系统评估LLMs在分子生成中的收敛性和发散性创造力表现，阐明了LLMs在药物发现中的适用性。

<details>
  <summary>Details</summary>

**Motivation:** 分子生成需要在满足多重化学和生物约束的同时，在庞大的化学空间中寻找最优解，这使得创造力成为功能性需求而非美学概念。现有的LLMs虽然可以生成分子，但其创造性行为和评估方法尚不明确。

**Method:** 通过系统实证评估，在物化性质、ADMET和生物活性任务中研究LLMs的创造性表现。将创造力分为收敛性创造力和发散性创造力两个维度，分析不同因素对这两种行为的影响。

**Result:** LLMs在分子生成中表现出独特的创造性模式，如当施加额外约束时约束满足度会增加。研究揭示了LLMs在分子生成任务中具有明确的创造力特征。

**Conclusion:** 这是首次将分子生成所需能力重新定义为创造力的研究，为基于LLM的分子生成了提供了系统性的创造力理解，明确了LLMs在分子发现管道中的恰当使用方式。
...
> SignDPO是一个多层级直接偏好优化框架，通过空间、时序和语言三个维度的对齐来提升基于骨架的手语翻译质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于骨架的手语翻译模型主要采用最大似然估计，受限于模仿式训练范式，对手语细微的时空细节缺乏判别敏感性，容易产生语义漂移问题。

**Method:** SignDPO包含三个关键设计：1) 分层扰动策略自动构建全局和局部粒度的时空非偏好样本；2) 自引导机制利用解码器交叉注意力分数识别并扰动语义关键的骨架区域；3) 微调专用扰动模型实现自动化语言级偏好生成。

**Result:** 在CSL-Daily、How2Sign和OpenASL三个基准数据集上，SignDPO显著优于最先进的无词汇方法，甚至可与成熟的基于词汇方法相媲美。

**Conclusion:** 多层级偏好对齐是弥合高熵骨骼轨迹与离散语言语义之间差距的有效范式，为手语翻译提供了新的研究方向。
...
> 提出一种合成-有机监督管道，通过将司法决策转化为指令调优数据，在低资源环境下实现了个性化法官模型的高效微调。

<details>
  <summary>Details</summary>

**Motivation:** 尽管大型语言模型取得了显著进展，但为个体决策者（如法官）进行个性化定制仍然是一个开放性问题，尤其是在资源稀缺的环境中。

**Method:** 开发了合成-有机监督管道，将原始司法决策转换为指令调优数据，采用参数高效微调方法，结合因果语言建模和合成生成的指令调优来个性化法官决策模型。

**Result:** 实验结果表明，该方法在三个不同任务和设置中显著优于所有基线方法，在词汇、风格和语义相似性方面提供了显著改善，模型生成的输出与人类法官的推理无法区分。

**Conclusion:** 研究证明了在低资源环境下进行高效个性化是完全可行的，为大型语言模型在专业领域的个性化应用提供了实用方案。
...
> 提出了一种名为Socio-Contrastive Learning的新方法，通过对比学习联合建模标注者观点和人口统计特征表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究通常只关注单一或有限组合的人口统计因素，无法充分解释标注者观点的复杂性。实际上，标注者观点是由复杂的社会背景塑造的，需要更细粒度的人口统计属性来更好地理解人类观点。

**Method:** 提出了Socio-Contrastive Learning方法，该方法在建模标注者观点的同时学习社会人口统计表示，提供了人口统计特征与文本表示融合的有效方法来预测标注者观点。

**Result:** 该方法在预测标注者观点方面优于标准的基于拼接的方法，学习到的表示还能够分析和可视化人口统计因素与标注者观点变化之间的关系。

**Conclusion:** 该方法为融合社会人口统计特征和文本表示提供了有效途径，不仅提高了预测性能，还促进了对人口统计因素如何影响标注者观点差异的理解和可视化。
...
> 这篇论文提出了一种对小语言模型进行主题控制摘要的数据增强方法，通过组合不同文档的上下文创建对比训练样本，使小模型在参数和真实数据较少的情况下达到与大模型相竞争的性能。

<details>
  <summary>Details</summary>

**Motivation:** 主题控制摘要能够让用户生成关注文档特定方面的摘要，但现有方法通常需要大型语言模型和大量训练数据。研究动机在于探索如何通过数据增强策略，让小型语言模型在有限的训练数据下也能有效执行主题控制摘要任务。

**Method:** 提出了一种成对数据增强方法，该方法将不同文档的上下文进行组合，创建对比训练样本。使用SciTLDR数据集并添加从维基百科派生的主题进行实验，系统评估增强规模对模型性能的影响。

**Result:** 实验结果显示，随着增强规模增加，模型的胜率和语义一致性都有持续改善。T5-base模型在应用该增强方法后，虽然在参数量和真实训练样本上都大幅少于大模型，但仍达到了与大模型相竞争的性能。

**Conclusion:** 所提出的数据增强策略是有效的，能够显著提升小语言模型在主题控制摘要任务上的性能。这种方法为在资源受限场景下部署高效的摘要模型提供了可行的解决方案。
...
> 本文提出了一种文化感知幽默图像描述生成任务，通过分阶段对齐框架实现了在特定文化背景下的稳定文化控制，在保持图像相关性和幽默质量的同时显著提升了文化适配性。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态大语言模型在生成幽默图像描述时缺乏稳定的文化背景控制能力，难以同时满足图像相关性、情境适切性和幽默质量的要求。

**Method:** 建立六维评估框架；提出分阶段对齐框架：先用西方文化语境初始化模型，然后通过基于裁判的GRPO进行多维偏好对齐，最后用少量监督数据适应东方文化语境。

**Result:** 实验结果表明该方法在所提出的评估框架下整体表现更强，在语境适切性方面收益显著，并在文化约束下更好地平衡了图像相关性和幽默性。

**Conclusion:** 该研究首次系统性地解决了多模态生成中的文化控制问题，为构建具有文化感知能力的幽默生成模型提供了有效的方法论和评估体系。
...
> TriMix是一种测试时对数融合框架，通过动态平衡三个来源（小模型低资源语言能力、高资源语言指令调优能力、大模型规模优势）来提升低资源语言模型性能，无需低资源任务标注数据。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法如代理调优在低资源语言环境下效果不佳，因为大模型在该语言上能力较弱会掩盖小模型的专业知识，且任务数据和计算资源稀缺。

**Method:** 提出TriMix测试时对数融合框架，结合三个来源：1）持续预训练小模型的低资源语言能力 2）高资源语言指令调优的任务能力 3）大模型的规模优势，实现动态平衡。

**Result:** 在四个模型族和八种低资源语言上的实验表明，TriMix一致优于单模型基线和代理调优方法。

**Conclusion:** 研究发现优先考虑小模型低资源专业化的对数对成功至关重要，这挑战了当前大模型主导的假设，证明了小模型专业化知识的重要性。
...
> 本文提出了因子化线性投影(FLiP)模型，用于理解预训练句子嵌入空间，能够从嵌入中恢复超过75%的词汇内容，优于现有基准方法。

<details>
  <summary>Details</summary>

**Motivation:** 当前缺乏有效工具来深入理解预训练句子嵌入空间的内部结构和特性，传统下游评估任务无法提供足够的内在洞察。

**Method:** 开发FLiP模型，通过因子化线性投影方法从多语言(LaBSE)、多模态(SONAR)和API-based(Gemini)的句子嵌入空间中恢复词汇内容，并在多种高资源和中等资源语言上进行训练测试。

**Result:** FLiP能够从嵌入中恢复超过75%的词汇内容，显著优于现有的非因子化基准方法；成功揭示了所选句子编码器的模态和语言偏差。

**Conclusion:** FLiP为理解句子嵌入空间提供了有效的诊断工具，帮助从业者在不依赖传统下游评估任务的情况下获得编码器的内在洞察，为嵌入空间的理解和应用提供了新思路。
...
> 提出RAMM模型，通过结合检索增强技术、多模态大语言模型、抽象叙事对齐和语义表示对齐模块，解决了多模态多领域假新闻检测中的跨实例叙事一致性和领域特定知识推理两个关键挑战。

<details>
  <summary>Details</summary>

**Motivation:** 现有模型存在两个主要问题：(1)无法捕获跨实例的叙事一致性，难以应对社交媒体中基于群组的假新闻传播；(2)缺乏领域特定知识进行推理，难以泛化到新领域或数据稀缺领域。

**Method:** RAMM采用三层次架构：1)使用多模态大语言模型(MLLM)作为主干提取跨模态语义信息；2)抽象叙事对齐模块自适应提取跨领域实例的抽象叙事一致性，聚合相关知识；3)语义表示对齐模块将模型决策过程从直接多模态特征推理转向基于实例的类比推理。

**Result:** 在三个公共数据集上的大量实验结果验证了所提方法的有效性。

**Conclusion:** RAMM通过整合检索增强技术和多层次对齐机制，有效提升了多模态多领域假新闻检测的性能，为处理叙事一致性和领域泛化问题提供了新的解决方案。
...
> Decisive是一个结合文档推理和贝叶斯偏好推理的交互式决策框架，通过主动学习用户偏好向量，在最小化用户努力的同时，实现透明化和个性化的决策建议。实验表明相比现有方法有20%的准确率提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有的决策支持方法（包括大语言模型和传统系统）存在不足：要么向用户展示过多信息造成认知负担，要么无法准确捕捉用户细致的偏好。需要一种既能保持决策客观性又能体现个性化偏好的方法。

**Method:** 提出Decisive框架，结合文档 grounding 推理与贝叶斯偏好推理。首先从源文档中提取客观选项评分矩阵，然后通过自适应选择的成对权衡问题主动学习用户的潜在偏好向量，最大化最终决策的信息增益。

**Result:** 通过大量实验验证，该方法在多个领域比通用大语言模型和现有决策框架表现更优，相比强基线方法实现了最高20%的决策准确率提升，同时保持了高效收敛和用户体验。

**Conclusion:** 将文档客观性与用户主观偏好有机结合的交互式决策方法，能够在保证决策透明度和个性化的同时有效减少用户认知负担，为复杂决策场景提供了实用的解决方案。
...
> TLoRA提出了一种统一的LoRA优化框架，通过数据驱动的初始化和基于敏感度的自适应资源分配，在多个NLP任务中实现了更好的性能和更少的可训练参数。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LoRA变体通常只优化秩分配、缩放因子或初始化中的一个因素，且往往以增加训练复杂度或降低实际效率为代价，缺乏统一的优化方案。

**Method:** TLoRA采用两阶段优化：首先通过奇异值分解对预训练权重与输入激活协方差的乘积进行数据驱动初始化，将LoRA的A矩阵对齐到任务相关子空间后冻结；然后基于敏感度的重要性度量，在固定参数预算下自适应地分配各层的秩和缩放因子，仅训练B矩阵。

**Result:** 在自然语言理解、常识推理、数学推理、代码生成和对话生成等多个任务中，TLoRA均表现出优异性能，同时显著减少了可训练参数数量。

**Conclusion:** TLoRA成功地将初始化和资源分配优化统一在一个框架内，为参数高效的LLM微调提供了更实用且有效的解决方案。
...
> 该研究通过300M参数的SwiGLU语言模型，发现W4A4量化误差主要源于生成器层(w2)而非读取器层，并提出DR+sink方法将量化后困惑度从1727降至39.9，但仍存在约2个PPL的残差差距。

<details>
  <summary>Details</summary>

**Motivation:** 研究W4A4量化在语言模型中的误差来源，特别是不同激活输入位点对整体性能的影响，旨在发现量化失效的关键因素。

**Method:** 采用训练时干预技术DR+sink(带铰链损失的深度寄存器)，结合SmoothQuant和QuaRot等量化方法，通过分解SwiGLU块的线性层为读取器和生成器，系统分析各层对量化误差的贡献。

**Result:** DR+sink方法将W4A4量化后困惑度从1727大幅降至119，结合SmoothQuant进一步降至39.9；发现w2生成器层是最主要的误差来源(贡献~0.28 nats)，而读取器kurtosis得到有效控制。

**Conclusion:** SwiGLU中的双线性结构(w2)是W4A4量化的根本瓶颈，正交旋转无法完全解决此问题，揭示了量化精度的理论下限；DR+sink作为诊断工具而非部署方案，为理解量化误差提供了新的视角。
...
> FreezeEmpath是一种端到端共情语音聊天机器人，通过冻结LLM参数，仅利用现有语音指令数据和语音情感识别数据进行高效训练，解决了传统方法的数据成本高、情感表达不足和灾难性遗忘问题。

<details>
  <summary>Details</summary>

**Motivation:** 当前基于大语言模型的共情语音聊天bot存在三个主要问题：依赖昂贵的共情语音指令数据、生成语音缺乏情感表达力、跨模态微调会导致LLM灾难性遗忘和通用能力下降。

**Method:** 提出FreezeEmpath框架，采用端到端训练方式，保持LLM参数冻结，仅使用现有的语音指令数据和语音情感识别(SER)数据进行训练，实现简单高效的模型构建。

**Result:** 实验表明FreezeEmpath能够生成富有情感表达的语音，在共情对话、语音情感识别和口语问答任务上均优于其他共情模型，验证了训练策略的有效性。

**Conclusion:** FreezeEmpath通过冻结LLM参数的方法成功解决了传统共情语音系统的关键挑战，为构建高效、低成本的共情语音聊天机器人提供了有效方案。
...
> 该研究揭示了多模态大语言模型作为评估者时的组合偏差问题，并构建了相应的基准测试MM-JudgeBias来量化分析。

<details>
  <summary>Details</summary>

**Motivation:** 多模态大语言模型(MLLMs)被广泛用作自动评估器，但其可靠性和对偏差的脆弱性尚未得到充分探索。研究发现这些模型在评估时存在不可靠性和不稳定性。

**Method:** 系统性地定义了MLLM-as-a-Judge系统中的组合偏差，构建了MM-JudgeBias基准测试。通过在查询、图像和响应三个维度引入受控扰动，采用偏差偏离(BD)和偏差一致性(BC)两个互补指标来评估模型行为。

**Result:** 在包含1800多个精心策划的多模态样本的基准测试上，对26个最先进的MLLMs进行实验，揭示了系统性的模态忽略和不对称评估倾向。

**Conclusion:** 研究强调了当前MLLM评估者存在的可靠性问题，突出了开发更可靠评估系统的必要性。
...
> 研究通过配对任务框架评估大语言模型在文学翻译中的创意能力，发现模型理解能力强但创意表现远低于人类水平，仅有少数模型能达到有限的创意效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有研究关注LLMs的翻译理解能力，但忽视了翻译创意性的评估，且理解和创意在专业翻译中是紧密相连的。缺乏系统性的文学翻译创意评估方法和大规模基准测试。

**Method:** 构建包含源文本理解任务和翻译创意评估任务的配对框架，使用创意潜势单元(UCP)如隐喻和文字游戏作为评估指标，结合专家人工标注和UCP自动评分的可扩展评估方案，对23个模型和4种创意导向提示进行基准测试。

**Result:** 强大的理解能力不能转化为人类水平的创意；模型常产生字面化或上下文不当的翻译，英中语言对的创意差距更大；创意导向提示仅带来有限提升；只有Mistral-Large接近人类创意水平(0.167 vs 0.246)；绝大多数模型创意得分为0或接近0。

**Conclusion:** 当前LLMs在文学翻译创意方面与人类存在显著差距，需要开发更有效的创意提示策略和评估方法来提升模型的创意翻译能力。
...
> Copy-as-Decode是一种创新的文本和代码编辑机制，通过结构化解码（拷贝+生成）替代自回归生成，实验显示可获得29.0×的加速比，并实现了74-99%的token覆盖率。

<details>
  <summary>Details</summary>

**Motivation:** 传统大模型编辑文本和代码时即使大部分内容与输入相同也不得不完全重新生成，效率低下且计算浪费。现有方法无法有效利用输入中的大量重复内容。

**Method:** 提出Copy-as-Decode解码层机制，使用<copy>和<gen>两种原始语法构建结构化解码器。通过令牌级FSM保证语法正确性，服务层通过单次并行预填充而非N步自回归更新KV缓存，实现高效拷贝。

**Result:** 在Qwen2.5-{1.5B,7B}上，并行预填充拷贝比自回归快6.8×-303×。在ProbeEdit和HumanEvalPack-Fix上达到74-98%的token覆盖率，综合加速比29.0×。令牌级扩展覆盖率达91-99%，保持4.5×-6.5×加速。微调实验显示HEvalFix-Py准确率从0提升至12-17%。

**Conclusion:** Copy-as-Decode通过智能拷贝机制显著提升了编辑效率，在不需要端到端训练的情况下实现了显著的性能提升。微调实验验证了机制的可行性，为后续优化和部署奠定了基础。
...
> 本研究提出了STaD框架，通过脚手架式任务设计来系统性地探测大语言模型的组合技能缺陷，克服了传统基准测试无法详细揭示模型弱点的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的基准测试只能提供聚合分数，无法深入理解LLM在不同领域的组合能力缺陷，也难以指导如何改进模型。

**Method:** 提出STaD（脚手架式任务设计）框架，基于脚手架概念生成基准任务的受控变体，通过结构化、渐进式的支持来系统性探测模型行为，识别缺乏的具体推理技能组合。

**Result:** 在6个不同规模的模型上进行实验，揭示了三个推理基准测试中的多个失败点，突出了每个模型独特且不同的技能缺陷。

**Conclusion:** STaD框架为深入理解和改进行大语言模型提供了新的方法论，能够系统性地识别和研究模型的具体技能组合缺陷，为模型改进提供了精准方向。
...
> 本文提出一种基于Mamba2等循环架构的高效文本嵌入模型，通过垂直分块推理策略实现恒定内存使用，在保持竞争性能的同时显著降低内存消耗。

<details>
  <summary>Details</summary>

**Motivation:** Transformer嵌入模型面临二次计算复杂度和线性内存复杂度的问题，这限制了其在处理长序列时的实用性。

**Method:** 引入垂直分块推理策略，对Mamba2模型进行微调，并验证该策略在Mamba2、RWKV和xLSTM等循环模型上的适用性。

**Result:** 微调后的Mamba2模型在多项基准测试中展现出竞争性性能，内存消耗远小于Transformer基模型，验证了推理策略在不同循环架构上的一致性运行时-内存权衡。

**Conclusion:** 循环模型通过垂直分块推理策略成为高效嵌入生成的有力替代方案，为长序列文本嵌入提供了实用解决方案。
...
> 多模态大语言模型虽然在跨模态数字感知上表现准确，但在进行精确的多位数乘法时普遍失败。研究引入了控制性多模态乘法基准，发现算术负荷C是性能预测的关键指标，模型错误主要源于计算而非感知缺陷。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态LLMs在跨模态数字感知上表现良好，但在执行相同基础算术问题的精确乘法时，无论是在数字形式、数词、图像还是音频呈现下都失败了。由于现有基准缺乏系统性的跨模态配对实例，难以比较模型家族内部和之间的真实算术限制。

**Method:** 研究者引入了一个控制性的多模态乘法基准，系统性变化数字长度、数字稀疏性、表示方式（如数字vs数词）和模态（文本、渲染图像、音频），通过可重现生成器创建配对实例。定义了算术负荷C作为总数字位数和非零数字位数的乘积，作为操作数的紧凑代理。设计了感知-计算分解实验和强制完成损失探针来分析模型行为。

**Result:** 准确性随着算术负荷C的增长急剧下降，当C>100时准确率通常接近零。C在不同模态和模型间保持预测性，R²常>0.5。感知-计算分解显示多模态退化主要是计算而非感知问题：在匹配感知检查中，模型在各模态下接近完美（>99%），即使乘法准确率下降。分解启发式在文本和视觉模态中都受青睐。

**Conclusion:** 多模态LLMs的算术失败主要源于计算限制而非感知缺陷，算术负荷C是性能预测的有效指标。模型内部保持良好调优的路由机制，不同的启发式适配器产生近正交更新但降低准确率，表明基础模型具有内部推理选择机制。
...
> 本研究对两种低资源且音系复杂的东亚高加索语族的Archi和Rutul语进行了音素级语音识别分析，发现使用音素词汇表的wav2vec2模型在极低资源条件下表现优异，且大多数错误主要源于数据稀缺而非音系复杂性。

<details>
  <summary>Details</summary>

**Motivation:** 针对音系复杂且资源稀缺的少数民族语言（Archi和Rutul语），传统的语音识别方法效果有限，需要探索更适合的模型架构和评估方法来理解ASR在这些语言上的行为模式。

**Method:** 收集并标准化了约50分钟（Archi）和1小时20分钟（Rutul）的语音数据，评估了wav2vec2、Whisper和Qwen2-Audio等先进模型。特别为wav2vec2引入了语言特定的音素词汇表和启发式输出层初始化策略，并进行了详细的音素级错误分析。

**Result:** wav2vec2的音素词汇表方法在极低资源条件下性能相当或超越Whisper；音素识别准确度与训练频率呈S型学习曲线关系；Archi语中Whisper的表现偏离了这一规律；大多数错误可归因于数据稀缺而非音系复杂性。

**Conclusion:** 音素级评估对于理解低资源、类型学复杂语言的ASR行为具有重要价值，数据稀缺是影响性能的主要因素，为未来低资源语音识别研究提供了重要见解。
...
> 研究提出了一种合成数据生成管道，通过反向翻译技术大量生成合成为社交媒体数据，在法语公共交通客户困境检测任务中，训练的小型推理模型达到与大型商业模型相当的性能。

<details>
  <summary>Details</summary>

**Motivation:** 社交媒体客户反馈自动分析面临三大挑战：标注训练数据成本高、评估集稀缺（特别是多语言环境下）、隐私担忧阻碍数据共享和可复现性。

**Method:** 开发通用的合成数据生成管道，应用微调模型的反向翻译技术，从少量种子语料库生成170万条合成推文，并配合合成推理轨迹。训练6亿参数的推理模型，使用英语和法语推理。

**Result:** 在人工标注的评估数据上达到77-79%的准确率，匹配或超越最先进的专有大型语言模型和专业编码器。通过消除敏感用户数据暴露来保护隐私。

**Conclusion:** 该管道不仅减少了标注成本，还通过合成数据保护隐私，方法可应用于其他用例和语言，为解决社交媒体反馈分析中的数据稀缺和隐私问题提供了有效方案。
...
> 本文提出CalibAdv方法，通过校验优势函数来改进深度搜索智能体的训练性能和稳定性。

<details>
  <summary>Details</summary>

**Motivation:** 深度搜索智能体的核心训练算法GRPO在深度搜索环境下面临两个关键挑战：一是中间步骤正确性与奖励信号的严重不匹配，导致正确中间步骤在最终答案错误时被错误惩罚；二是训练高度不稳定，导致自然语言能力退化甚至训练崩溃。

**Method:** 提出优势校准方法CalibAdv，包含两个核心机制：利用中间步骤正确性在细粒度层面缩减过度的负优势；在答案组件中重新平衡正负优势分布。

**Result:** 在三个模型和七个基准测试上的大量实验证明，CalibAdv有效提升了模型性能并显著改善了训练稳定性。

**Conclusion:** CalibAdv通过精细化的优势校准解决了GRPO在深度搜索任务中的关键问题，为深度搜索智能体的训练稳定性和性能提升提供了有效解决方案。
...
> 这是首个对预训练自监督语音编码器进行逐层公平性分析的研究，发现不同语音任务中存在相反的层偏差模式，且语音识别中的偏差在微调后仍然存在。

<details>
  <summary>Details</summary>

**Motivation:** 语音编码器模型对不同说话人群体的建模效果存在差异，但技术层面上这种偏差产生的原因尚不清楚。研究需要深入理解这种不公平现象的根源。

**Method:** 对预训练自监督语音编码器进行逐层分析，在说话人识别(SID)和自动语音识别(ASR)任务中探测每个嵌入层的表现。

**Result:** S3M在最开始的隐含层就产生了对某些说话人群体的偏差；SID偏差与整体误差呈负相关（最小误差层偏差最小），而ASR偏差与整体误差呈正相关（最小误差层偏差最大）；这种ASR的反向偏差关系在微调后仍存在。

**Conclusion:** 说话人群体层面的偏差在预训练阶段就已经形成，难以通过下游任务微调消除，这对构建公平的语音系统具有重要启示。
...
> 通过微调神经语言模型来预测花园路径句，证明了惊异理论能够解释句法消歧困难，但这也提出了如何证伪惊异理论的新问题。

<details>
  <summary>Details</summary>

**Motivation:** 惊异理论认为句子处理难度与惊异值负对数概率线性相关，但现有神经语言模型在花园路径句上严重低估人类处理困难，导致研究者认为此类句子的处理困难不能简单归结为惊异值。

**Method:** 对现成的神经语言模型进行微调，专门在花园路径句上训练，使基于惊异值的阅读时间估计更符合人类实际阅读时间，并在保留的花园路径样本和自然语料上进行测试。

**Result:** 微调后的模型没有过拟合，成功捕捉了保持样本的花园路径句上的人类阅读减慢效果，甚至提高了对自然语料人类阅读时间的预测能力，同时保持了模型的一般语言能力。

**Conclusion:** 这一结果为能够通过惊异值解释花园路径效应和自然阅读时间的神经语言模型提供了存在证明，但也引发了理论问题：什么样的证据才能真正证伪惊异理论？
...
> 研究发现LLM在早期层次区分字面与比喻用法，中后期层次将具体性压缩为跨模型一致的一维方向，该结构可用于无需训练的比喻语言分类和生成控制。

<details>
  <summary>Details</summary>

**Motivation:** 传统的静态具体性评分在比喻等语境下存在局限性，同一具体名词在不同语境中可能产生抽象解释，需要深入理解LLM如何在内部表示中处理具体性的语境变化。

**Method:** 对四个模型家族的LLM进行逐层和几何分析，检查模型如何区分同一名词的字面与比喻用法，分析具体性在表示空间中的组织结构，并验证具体性方向的实用性。

**Result:** LLM在早期层次分离字面和比喻用法，中后期层次将具体性压缩为跨模型一致的一维方向；该方向支持高效的比喻语言分类和无需训练的生成控制。

**Conclusion:** LLM内部存在系统性的具体性表示几何结构，这种结构不仅揭示了模型处理比喻语言的内部分类机理，还提供了实用的应用途径。
...
> 研究表明，通过分析语言模型的内部激活可以识别推理链中的重要步骤，激活比推理链本身的tokens包含更多关于步骤重要性的信息，且这种内部重要性表示在生成后续步骤前就已存在。

<details>
  <summary>Details</summary>

**Motivation:** 语言模型通过生成长推理链解决复杂任务，但难以区分哪些推理步骤是关键的。理解模型如何处理推理的核心问题是应该通过模型内部机制还是通过推理链的tokens来确定步骤重要性。

**Method:** 通过训练探针模型分析模型激活来预测步骤重要性，比较激活信息与tokens信息的差异，验证内部表示是否跨模型泛化，以及与表面特征的关联性。

**Result:** 模型激活比tokens包含更多步骤重要性信息，模型在生成后续步骤前就已编码步骤重要性的内部表示，这种表示跨模型泛化、分布在不同层，且与步骤位置和长度等表面特征无关。

**Conclusion:** 分析模型激活能够揭示表面方法无法捕捉的推理方面，表明推理分析应该深入研究模型内部机制。
...
> 本研究提出将可解释AI(XAI)的解释以叙事形式呈现，定义了叙事解释的四个关键属性，并开发了相应的自动评估指标来衡量叙事质量，同时提出了XAI叙事生成的通用规则。

<details>
  <summary>Details</summary>

**Motivation:** 当前的可解释AI方法主要通过静态特征重要性列表提供解释，虽然能说明什么影响了预测，但无法解释为什么会产生该预测。现有的基于自然语言的解释方法生成的文本缺乏真正的解释价值，标准NLP评估指标无法有效区分真实解释和空洞文本。

**Method:** 借鉴社会科学和语言学理论，提出叙事解释应具备连续结构、因果关系、语言流畅性和词汇多样性四个核心属性。开发了七个自动评估指标来量化这些属性，并在六个数据集上对最先进的解释生成方法进行基准测试。最后提出了一套与领域无关的XAI叙事生成规则。

**Result:** 实验结果表明，提出的指标比标准NLP指标更可靠地区分描述性解释和叙事性解释。基于生成规则产生的XAI叙事展现出更强的叙事属性，并符合语言学和社会科学文献的研究发现。

**Conclusion:** 叙事形式的解释比静态特征列表更能支持人类理解，通过定义叙事属性、开发相应评估指标和生成规则，为可解释AI领域提供了新的研究方向和实用工具，有助于产生更有意义的自然语言解释。
...
> FregeLogic结合5个LLM分类器集成与Z3 SMT求解器，有效减少内容效应并提高三段论有效性预测的准确率

<details>
  <summary>Details</summary>

**Motivation:** 解决LLM在三段论推理中受现实世界可信度干扰而影响逻辑判断的内容效应问题，提高预测准确性

**Method:** 采用混合神经符号系统：1) 集成5个LLM分类器(Llama 4 Maverick/Scout和Qwen3-32B配合不同提示策略)；2) 当集成存在分歧时，使用Z3 SMT求解器作为形式逻辑裁决器；3) 通过结构化API调用和亚里士多德编码提高Z3提取效率

**Result:** 在960样本数据集上实现94.3%准确率，内容效应降至2.85，综合得分41.88，比纯集成方法提升2.76分，内容效应减少16%

**Conclusion:** 针对性的神经符号集成策略，在集成共识最低时应用形式化方法，能有效改善准确率与内容效应的综合表现
...
> 该研究通过再生翻译范式创建了面向五种欧洲语言的多语言视觉语言模型训练和评估资源套件，证明了多语言数据对VLMs训练的积极影响。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉语言模型发展过度依赖英语，存在两个主要限制：缺乏多语言多模态训练数据集，以及缺乏跨语言的全面评估基准。

**Method:** 采用再生翻译范式，结合精选合成生成和人工标注，构建Multi-PixMo训练语料库；同时将广泛使用的英文数据集翻译为多语言评估基准；通过定性定量分析、消融研究评估资源质量并验证多语言数据影响。

**Result:** 实验涵盖3个不同模型，表明使用多语言多模态示例训练VLMs在非英语基准上持续有益，对英语也产生正向迁移效应。

**Conclusion:** 研究成功填补了多语言视觉语言模型训练和评估的资源空白，为欧洲语言VLMs发展提供了重要基础设施和数据支持。
...
> HiGMem是一个双层记忆系统，使用事件摘要作为语义锚点指导记忆检索，在LoCoMo10基准测试中显著提升了检索精度和效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有记忆系统主要依赖向量相似性进行检索，导致检索到的证据集冗余、精度低、上下文成本高，且难以检查和管理。

**Method:** HiGMem采用两层事件-记忆结构：LLM先检查高层级事件摘要作为语义锚点，然后预测哪些相关的对话细节值得进一步读取，通过推理提供简洁可靠的证据集。

**Result:** 在LoCoMo10基准测试中，HiGMem在5个问题类别中的4个类别上取得最佳F1值，对抗性F1从0.54提升至0.78，检索到的对话数量减少了一个数量级。

**Conclusion:** HiGMem通过引导式检索有效解决了传统记忆系统的冗余问题，在保持检索效率的同时显著提升了长对话中记忆检索的准确性和实用性。
...
> PRISMA是一个可解释的情感智能谈判对话系统，通过ENS-CoT推理机制和DPO优化，在面试和资源分配领域实现了更好的可解释性和情感适应性谈判响应。

<details>
  <summary>Details</summary>

**Motivation:** 情感在谈判中起关键作用，影响信任、合作和长期关系。现有系统不仅要生成情感适宜的回应，还需要具备可解释性以增强可靠性和建立融洽关系。

**Method:** 提出ENS-CoT推理机制模拟人类感知、理解、使用和管理情感的过程；基于此构建JobNego和ResNego数据集；结合自训练和DPO优化开发PRISMA系统。

**Result:** 在JobNego和ResNego数据集上的自动和人工评估表明，PRISMA显著提升了可解释性，生成了更合适的情感感知回应，并改善了整体谈判效果。

**Conclusion:** PRISMA系统成功地将情感智能与可解释性结合，为人机交互谈判提供了更有效、更可靠的解决方案，展示了情感驱动AI系统的实用价值。
...
> 研究通过赋予对话代理外部工具执行能力，构建了ComPASS社会支持基准和ComPASS-Qwen模型，显著提升了AI系统提供多样化实质性支持的效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有共情对话系统在响应形式和内容上存在局限，难以满足不同用户和情境下的多样化需求，需要探索更有效的社会支持提供方式。

**Method:** 基于社会支持心理学概念，设计了十几种用户中心的多媒体工具，构建ComPASS-Bench个性化社会支持基准数据集，并通过工具使用记录微调Qwen3-8B模型得到ComPASS-Qwen。

**Result:** 实验表明LLMs能高成功率生成有效工具调用请求但最终响应质量仍有差距，工具增强响应优于直接对话共情，ComPASS-Qwen相比基础模型显著提升并媲美大模型。

**Conclusion:** 工具增强方法为构建富有同情心的交互系统提供了有效途径，ComPASS-Bench和ComPASS-Qwen为该领域发展奠定了重要基础。
...
> ArbGraph是一个用于长文档RAG系统的预生成证据仲裁框架，通过构建冲突感知的证据图谱来明确解决事实冲突，从而提高生成内容的可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 在长文档RAG场景中，检索到的证据往往是嘈杂或矛盾的，导致RAG管道难以保持事实一致性。现有方法专注于检索扩展或生成过程中的验证，但冲突解决仍然与生成过程纠缠在一起。

**Method:** ArbGraph将检索到的文档分解为原子声明，组织成具有明确支持和矛盾关系的冲突感知证据图谱。在此基础上引入基于强度的迭代仲裁机制，通过证据交互传播可信度信号，在最终生成前抑制不可靠和不一致的声明，从而将证据验证与文本生成分离。

**Result:** 在LongFact和RAGChecker两个长文档RAG基准测试上的实验结果表明，ArbGraph持续提高了事实回忆和信息密度，同时减少了幻觉和对检索噪声的敏感性。分析还显示这些改进在冲突或模糊证据下尤为明显。

**Conclusion:** 证据级别的冲突解决对于提高长文档RAG系统的可靠性是有效的。ArbGraph通过分离证据验证和文本生成，为下游长文档生成提供了连贯的证据基础。
...
> 本文提出了IceBreaker系统，通过生成个性化对话开场白来解决对话系统的首次消息障碍问题，在线实验证明能有效提升用户参与度。

<details>
  <summary>Details</summary>

**Motivation:** 现有对话代理虽能被动响应用户，但在用户需求模糊、意图不明确的对话启动阶段存在瓶颈，造成'首次消息障碍'，阻碍了对话的自然开始。

**Method:** 将破冰过程建模为两步骤握手：1)通过共振感知兴趣提取从会话摘要中捕捉触发兴趣；2)通过交互导向的开场生成，结合个性化偏好对齐和自我强化循环优化来最大化参与度。

**Result:** 大型在线A/B测试显示IceBreaker将用户活跃天数提升0.184%，点击率提升9.425%，并已成功部署到生产环境中。

**Conclusion:** 本研究开创性地解决了对话系统冷启动问题，IceBreaker框架为主动式对话代理的发展提供了有效解决方案，在商业产品中取得了显著效果。
...
> 本文通过泰勒展开和数学分析，揭示了大型语言模型对相同含义的不同提示词过度敏感的根本原因，并提供了相关的理论解释和实证验证。

<details>
  <summary>Details</summary>

**Motivation:** 大型语言模型(LLM)的提示词敏感性问题引起了用户对模型稳定性和可靠性的担忧。不同表述但意义相同的提示会导致完全不同的输出，这一问题亟需深入理解其根本原因。

**Method:** 将LLM视为多元函数，进行一阶泰勒展开，分析意义保持型提示词、其梯度和模型下一个token的对数概率之间的关系，利用柯西不等式推导出对数概率差异的上界，并对比分析了输入在模型内部的聚类特征。

**Result:** 研究发现LLM不像小规模神经网络那样对相似输入进行内部聚类，而是分散处理，导致意义保持型提示词间的对数概率差异上界过高，难以有效降至0。同时证明该上界与PromptSensiScore指标强相关，且提示模板对logits的影响比问题本身更大。

**Conclusion:** 本研究为当前LLM对相同含义提示词高度敏感的现象提供了通用解释，为理解LLM的提示词敏感性问题提供了关键理论依据，有助于指导未来模型的改进方向。
...
> River-LLM是一个无需训练的框架，通过轻量级KV共享退出流解决解码器架构中早期退出的KV缓存缺失问题，在保持生成质量的同时实现1.71-2.16倍的实际加速。

<details>
  <summary>Details</summary>

**Motivation:** 早期退出技术在大语言模型推理加速中面临KV缓存缺失问题的严重瓶颈，现有解决方案要么引入显著延迟开销，要么导致严重精度损失，无法实现理论层减少与实际墙钟时间加速之间的平衡。

**Method:** River-LLM引入轻量级KV共享退出流，在退出过程中自然生成和保存主干模型缺失的KV缓存；利用解码器块内的状态转换相似性预测累积KV误差，指导精确的退出决策；整个框架无需训练即可部署。

**Result:** 在数学推理和代码生成任务上的广泛实验表明，River-LLM在维持高生成质量的同时，实现了1.71到2.16倍的实际推理加速效果。

**Conclusion:** River-LLM成功解决了解码器架构中早期退出技术的KV缓存缺失瓶颈，通过创新的KV共享机制和智能退出策略，实现了理论加速与实际性能的有效统一，为加速大语言模型推理提供了新思路。
...
> AlphaContext是一种基于进化树的创造性评估心理测量情境生成器，通过超树规划、MCTS生成、进化优化和评估引导优化四个阶段，比竞争方法在6个质量指标上平均提升8%。

<details>
  <summary>Details</summary>

**Motivation:** 在LLM和人机协作时代，创造力成为核心能力，需要科学的评估工具。现有的基于LLM的生成器面临评估线索不足、叙事连贯性弱、风格多样性有限、对创造性思维支持差等问题，高质量专家设计的情境稀缺。

**Method:** 提出AlphaContext系统：1）超树大纲规划器将专家大纲形式化为规则引导的超树并进行分层规划；2）基于MCTS的情境生成器通过蒙特卡洛树搜索填充大纲；3）进化情境优化器使用MAP-Elites进化情境，共同提升多样性和质量；4）评估引导进化优化器模拟多风格参与者并循环利用弱情境进一步进化。

**Result:** 实验表明AlphaContext在6个质量指标上比竞争方法平均提升8%，生成的情境在评估线索、叙事连贯性、风格多样性等方面表现更优。

**Conclusion:** AlphaContext通过系统性的进化优化框架，有效解决了现有LLM生成器的局限性，为创造力评估提供了高质量的心理测量情境生成工具，推动了创造力评估的科学化发展。
...
> StepPO提出了一种基于步级别的智能体强化学习新范式，将传统token级别的决策过程升级为步级别MDP，以更好地优化LLM智能体的决策能力和工具使用能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有通用智能体系统对LLM的智能体能力要求越来越高，但传统的token级别强化学习方法难以处理多轮交互场景中的延迟稀疏奖励、长变长上下文等挑战，无法有效捕捉智能体的真实行为。

**Method:** 提出StepPO框架：1）将token级别MDP升级为步级别MDD；2）以step而非token作为基本动作单元；3）设计步级别信用分配机制；4）讨论实现该范式的系统设计要点。

**Result:** 初步实验验证了StepPO方法的有效性，表明步级别的范式能够更好地对齐策略优化与奖励传播，更符合智能体决策的实际粒度。

**Conclusion:** StepPO为智能体强化学习社区提供了一个理解智能体行为的新视角，其步对齐、步级别的范式有助于推动LLM朝着更强的通用智能体能力发展。
...
> 首份统一的印度自然语言处理资源综述，涵盖200+数据集、50+基准测试和100+模型工具，系统梳理了印度NLP发展现状、挑战与机遇。

<details>
  <summary>Details</summary>

**Motivation:** 印度拥有22种官方语言和数百种边缘化方言的复杂语言环境，现有NLP资源呈现快速增长趋势，但缺乏专门的综合性调查。现有综述要么关注少数高资源语言，要么将印度语言置于更广泛的多语言框架中，无法充分覆盖低资源和文化多样性语言的需求。

**Method:** 构建首份统一的印度NLP资源调查框架，涵盖文本、语音、多模态和文化相关任务。按照语言现象、领域和模态对资源进行分类组织，深入分析标注趋势、评估方法和模型设计模式，识别数据稀疏性、语言覆盖不均、文字多样性以及文化和领域泛化能力有限等持续挑战。

**Result:** 完成了涵盖200+数据集、50+基准测试和100+模型、工具和系统的综合性资源调查，为印度语言生态系统中的NLP研究提供了统一的基础资源平台，促进了公平、文化相关和可扩展的NLP研究发展。

**Conclusion:** 本调查填补了印度NLP资源综述的空白，为研究者提供了第一个系统性的资源整合平台，有助于推动更加公平、文化敏感且具备良好扩展性的印度NLP研究，为解决数据稀疏、语言覆盖不均等关键挑战提供了基础。
...
> 对抗性人文基准(AHB)通过将有害请求改写为人文学科风格，测试模型安全拒绝的鲁棒性，发现当前安全技术在风格变化下存在严重泛化缺陷。

<details>
  <summary>Details</summary>

**Motivation:** 评估现有大语言模型的安全机制在面对不同表达风格的恶意提示时是否仍然有效，探索安全技术的泛化能力边界。

**Method:** 基于MLCommons AILuminate的有害任务，通过人文学科风格的转换重构提示内容，保持原意但改变表达形式，测试31个前沿模型的安全拒绝表现。

**Result:** 原始攻击成功率仅3.84%，转换后攻击成功率飙升至36.8%-65.0%，总体达55.75%，其中化生放核(CBRN)风险最高。

**Conclusion:** 当前模型安全技术缺乏风格鲁棒性，对'无害性'的深度理解仍是前沿模型安全的核心未解决问题。
...
> LQM是一个针对机器翻译质量评估的新框架，专门用于诊断双语语言（如阿拉伯语）中的方言和文化特定错误，通过六个语言学层级的错误分类体系进行评估。

<details>
  <summary>Details</summary>

**Motivation:** 现有机器翻译评估框架大多是语言无关的，无法有效捕捉双语语言中的方言和文化特定错误。这些翻译失败往往源于语言变体、内容覆盖和语用匹配度的不匹配，而不仅仅是表层形式问题。

**Method:** 提出LQM（语言学动机的多维质量指标）框架，建立包含六个语言学层级的分层错误分类体系：社会语言学、语用学、语义学、形态句法学、正字法和文字学。构建包含7种阿拉伯方言的双向平行语料库，对6个LLM进行零样本评估，并使用LQM进行专家级span级人工标注。

**Result:** 产生了6,113个标注错误span，涵盖3,495个独特错误句子，并计算了严重性加权的质量分数。同时补充了spBLEU自动指标分析。所有数据、提示和标注指南已公开发布。

**Conclusion:** 虽然在阿拉伯语上进行了验证，但LQM是一个语言无关的框架，设计上易于应用于其他语言或进行本地化适配。该框架为机器翻译质量评估提供了更细粒度的语言学视角。
...
> MASS-RAG是一种多智能体检索增强生成方法，通过专业化代理（摘要、提取、推理）处理证据，并在合成阶段整合输出，显著提升了在噪声和分布式证据环境下的生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 传统RAG方法在处理噪声、不完整或异构的检索上下文时，单一生成过程难以有效调和证据，需要更好的证据处理和整合机制。

**Method:** MASS-RAG框架包含三个专业化代理：证据摘要代理、证据提取代理、推理代理，以及专门的合成阶段。不同代理从多角度处理检索到的文档，最后通过合成阶段整合各代理输出以生成最终答案。

**Result:** 在四个基准测试中，MASS-RAG相比强大的RAG基线方法表现更优，特别是在相关证据分布在多个检索上下文中的场景下提升更为明显。

**Conclusion:** 多智能体架构为检索增强生成提供了更灵活的证据处理方式，通过暴露多个中间证据视图，使模型能够更好地比较和整合互补信息，是提升RAG系统性能的有效途径。
...
> 本研究通过引入KL正则化项将对话流程统计纳入下一对话行为预测，在德语咨询数据集上显著提升性能，且跨语言/域验证显示良好迁移效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的下一对话行为预测(NDAP)模型未能充分利用语料库中的对话流程统计信息，特别是在细粒度、数据稀疏的对话任务中表现不佳。

**Method:** 提出KL正则化项，将预测的行为分布与从语料库衍生的转换模式对齐，整合多种预训练编码器和架构进行系统性实验。

**Result:** 在60类德语咨询分类任务上，macro-F1相对提升9-42%，对话流程对齐度显著改善；跨数据集验证显示改进可迁移到不同语言和咨询领域。

**Conclusion:** 轻量级的对话流程先验能有效补充预训练编码器，尤其对弱基线模型有更大帮助，为细粒度对话任务提供了一种实用的解决方案。
...
> 研究发现LLM内部层在句子处理中存在双重对齐模式：自然阅读适合早期层，句法挑战性处理适合后期层，且多层概率更新改善了阅读时间建模。

<details>
  <summary>Details</summary>

**Motivation:** 先前研究显示LLM早期层的surprisal能有效建模自然句子处理，但不清楚这种优势是否能扩展到更具句法挑战性的结构中，特别是在surprisal通常低估人类认知努力的情况下。

**Method:** 通过实验探索LLM内部层对英语句法歧义处理的建模能力，比较自然阅读和句法挑战性处理中不同层的表现，并探索浅层与深层结合的概率更新方法。

**Result:** 实验显示，与自然阅读不同，句法挑战性处理中后期层更好地估计认知努力，但仍有低估；多层概率更新方法在阅读时间建模中显示出互补优势。

**Conclusion:** 揭示了人类和语言模型在句子处理中的双重模式：自然阅读类似LLM早期层的弱预测，句法挑战性处理需要后期层的完全上下文表示；多层结合提供了更好的建模效果。
...
> 本文提出了一种结合残差瓶颈卷积和双向GRU的端到端光学乐谱识别框架，在Camera-PrIMuS和PrIMuS数据集上分别达到7.52%和8.11%的序列错误率，具有高效率和高精度。

<details>
  <summary>Details</summary>

**Motivation:** 光学乐谱识别(OMR)旨在将印刷或手写乐谱图像转换为可编辑的符号表示，传统方法需要复杂的特征工程和显式对齐，端到端识别是该领域的重要挑战。

**Method:** 采用ResNet-v2风格的残差瓶颈模块和多尺度扩张卷积提取细粒度符号细节和全局五线谱结构特征，然后使用BiGRU网络建模音乐符号的时序依赖，通过CTC损失实现端到端训练。

**Result:** 在Camera-PrIMuS数据集上实现7.52%序列错误率和0.45%符号错误率，音高、类型和音符准确率分别为99.33%、99.60%和99.28%；在PrIMuS数据集上也达到相似的高性能，平均每轮训练时间仅需1.74秒。

**Conclusion:** 所提出的端到端OMR框架有效结合了卷积特征的视觉表示能力和循环网络的时序建模能力，在保持高计算效率的同时实现了优秀的乐谱识别性能。
...
> 本文提出了HalfV框架，通过解耦视觉冗余并采用统一剪枝策略，解决了高分辨率多模态大语言模型推理加速中的架构依赖问题，在多个backbone上实现了4.1倍的FLO加速并保持96.8%的性能。

<details>
  <summary>Details</summary>

**Motivation:** 高分辨率多模态大语言模型在推理过程中由于视觉标记爆炸导致计算成本过高，现有的token剪枝或层稀疏策略存在严重的'backbone依赖性'，在特定架构上效果好但在其他架构如Qwen上性能显著下降。

**Method:** 通过截断矩阵熵发现通用的三阶段推理生命周期，将视觉冗余解耦为通用固有视觉冗余(IVR)和架构相关的次级饱和冗余(SSR)：首先通过统一剪枝策略缓解IVR，然后基于SSR的具体表现自适应处理SSR。

**Result:** 在多样化的backbone上实现了卓越的效率-性能权衡，特别是在Qwen25-VL上保持96.8%性能的同时达到4.1倍FLO加速，显著优于现有最先进基线方法。

**Conclusion:** HalfV框架通过解耦视觉冗余的通用和架构相关成分，有效解决了多模态大语言模型推理加速的架构依赖问题，为跨架构的高效推理提供了通用解决方案。
...
> 本文提出一种新的视频潜在空间压缩方法，通过移除高频分量而非直接减少通道数，在保持高压缩比的同时提升视频重建质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有视频VAE需要大量潜在通道确保高质量重建，但过多通道会阻碍潜在扩散模型收敛并降低生成性能，直接减少通道数又会损害重建保真度。

**Method:** 提出一种潜在压缩方法，在视频潜在表示中移除高频分量，而不是直接降低通道数量。

**Result:** 实验证明，在保持相同总压缩比的前提下，所提方法相比强基准方法获得了更优越的视频重建质量。

**Conclusion:** 通过在潜在空间进行更智能的频率分量压缩，可以同时解决视频重建质量和扩散模型性能之间的权衡问题。
...
> 本文开发了一套基于立体视觉的无人机系统，用于检测和定位辐射松树枝，采用分割和深度估计两个阶段，评估了多种深度学习方法，验证了低成本立体视觉在林业自主修剪中的可行性。

<details>
  <summary>Details</summary>

**Motivation:** 为林业自主修剪提供准确的树枝定位支持，开发基于无人机的立体视觉系统，以实现自动化、低成本的枝干检测与定位。

**Method:** 系统包含两个主要阶段：树枝分割阶段比较了YOLOv8、YOLOv9和Mask R-CNN等模型；深度估计阶段评估了传统SGBM方法和PSMNet、ACVNet等多种深度学习方法，并提出了质心三角化算法结合MAD异常值剔除来计算树枝距离。

**Result:** 在1-2米距离的定性评估中，深度学习视差图比传统SGBM方法产生更一致的深度估计，证明了低成本立体视觉用于林业自动化树枝定位的可行性。

**Conclusion:** 基于深度学习的立体视觉系统为林业自动修剪提供了有效的解决方案，在准确性和成本效益方面具有明显优势，为实际应用奠定了技术基础。
...
> ETC是一种可扩展的概念擦除框架，能够在文本到图像扩散模型中精确擦除数千个不良概念，同时保持生成质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大规模文本到图像扩散模型存在安全风险，可能生成不受欢迎的内容（如版权内容）。当前的概念擦除方法在可扩展性、精确性和鲁棒性之间难以平衡，仅能处理数百个概念的擦除。

**Method:** 1) 使用t分布混合模型(tMM)建模低秩概念分布；2) 通过仿射最优传输实现目标概念的精确擦除；3) 通过锚定目标概念分布边界来保留其他概念；4) 训练基于专家混合(MoE)的MoEraser模块来移除目标嵌入并保留锚定嵌入；5) 通过在文本嵌入投影器中注入噪声并微调MoEraser实现白盒攻击鲁棒性。

**Result:** 在超过2000个跨异构域和扩散模型的概念上进行的广泛实验证明了ETC在大规模概念擦除方面的最先进可扩展性和精确性。

**Conclusion:** ETC框架成功解决了现有方法在可扩展性、精确性和鲁棒性方面的限制，为大规模文本到图像概念擦除提供了有效解决方案。
...
> 这篇调查论文揭示了视觉机器人空间内存效率问题，通过引入α比率（峰值运行内存与保存地图大小之比）暴露了实际部署成本与发表论文地图大小之间的巨大差距，并提出了标准化评估协议和预算算法。

<details>
  <summary>Details</summary>

**Motivation:** 随着基于视觉的机器人在更大环境中导航，其空间内存会无限增长，最终耗尽计算资源。特别是在嵌入式平台（8-16GB共享内存，<30W）上，无法额外添加硬件，这成为实际部署的关键瓶颈。

**Method:** 通过调研52个系统（1989-2025）的88篇文献，从占用栅格到神经隐式表示，引入α=M_peak/M_map比率。在NVIDIA A100 GPU上进行独立性能分析，实施帕累托前沿分析和显式基准分离，标准化评估协议包括内存增长率、查询延迟、内存-完整性曲线和吞吐量降级。

**Result:** 研究发现神经方法中α比率跨越两个数量级，从2.3（Point-SLAM）到215（NICE-SLAM）。没有单一范式在其评估体系中占主导地位：3DGS方法在Replica上以90-254MB地图大小实现最佳绝对精度，而场景图以可预测成本提供语义抽象。

**Conclusion:** 论文提供了首个独立测量的α参考值和α感知预算算法，使实践者能够在实际实现之前评估目标硬件的部署可行性。内存架构而非范式标签决定了部署的可行性。
...
> 提出名为动态语义引导(DSS)的训练无关框架，通过构建安全语义边界和闭环校正方案，在文本到图像扩散模型中实现可控的概念擦除，擦除率达91.0%。

<details>
  <summary>Details</summary>

**Motivation:** 现有推理时概念擦除方法存在严重不足：特征校正方法容易失控过度校正，词级干预面临语义粒度和上下文问题，且两种方法都容易导致语义漂移或表征崩溃，需要更可控的解决方案。

**Method:** DSS框架包含两个核心组件：1)敏感语义边界建模(SSBM)自动发现安全语义锚点；2)敏感语义引导(SSG)利用跨注意力特征进行精确检测，通过良构目标函数导出的闭环解执行校正。

**Result:** DSS实现了平均91.0%的擦除率，显著优于现有方法(18.6%-85.9%)，同时最小化了对输出保真度的影响。

**Conclusion:** DSS作为轻量级、训练无关的框架，为文本到图像模型的 安全内容生成提供了可解释、可控的概念擦除方案，有效平衡了内容安全与生成质量。
...
> 本文提出了因果潜在世界模型(CLWM)，通过使用DINOv3特征作为生成目标解决了传统生成式世界-动作模型的重构冗余、内存扩展和推理延迟问题，实现了复杂的双臂机器人操作任务和零样本仿真到现实世界的转移。

<details>
  <summary>Details</summary>

**Motivation:** 传统的生成式世界-动作模型在机器人操作部署中受到三个主要瓶颈制约：像素级重构的冗余性、内存随时间步线性增长(O(T))以及顺序推理的延迟问题，这些限制了模型在实际机器人应用中的效果。

**Method:** CLWM采用DINOv2特征作为生成目标来分离交互语义与视觉噪声；设计了双状态测试时训练(TTT)内存机制确保长期任务中严格的O(1)内存占用；提出推测异步推理(SAI)通过将部分扩散去噪隐藏在物理执行后来减少阻塞延迟；引入EmbodiChain在线框架通过注入基于物理的轨迹流来扩展鲁棒策略。

**Result:** CLWM在复杂双臂仿真任务中达到了最先进的性能，并在物理机器人上实现了前所未有的零样本仿真到现实世界转移，性能优于显式在真实数据上微调的基线方法。

**Conclusion:** 通过特征级生成、常数内存设计和异步推理优化，CLWM有效解决了世界模型部署的关键挑战，为复杂机器人操作任务提供了高效可靠的解决方案，并建立了效率定律来指导策略扩展。
...
> 提出Saccade Attention Network，通过学习人类眼扫视机制，从预训练模型中学习关注关键特征，将输入序列长度减少80%，在保持相似性能的同时大幅降低计算量。

<details>
  <summary>Details</summary>

**Motivation:** Transformer网络存在序列长度限制问题，传统自注意力计算复杂度为二次方，但实际使用的注意力是稀疏的。人类通过眼扫视(Saccades)机制实现对关键特征的稀疏关注，这为优化提供了生物学启发。

**Method:** 构建Saccade Attention Network，从大型预训练模型中学习应该关注的关键特征位置，从而预处理图像，将输入序列长度缩减到仅包含这些关键特征。

**Result:** 该方法能够减少接近80%的计算量，同时产生与原始方法相似的结果。

**Conclusion:** 受人类视觉关注机制启发的稀疏注意力方法，能够有效解决Transformer的计算复杂度问题，在保持性能的同时大幅提升效率。
...
> 本文提出PhyLAA-X框架，物理条件化的局部伪影注意力网络，将光学流卷曲、镜面反射偏度、rPPG功率谱物理特征直接注入注意力机制，提升换脸检测鲁棒性。已在开源项目Aletheia中实现。

<details>
  <summary>Details</summary>

**Motivation:** 现有深度伪造检测器在跨生成器变化、大幅压缩和对抗扰动下性能下降严重。核心问题是将语义伪影学习与物理不变量解耦，光学流不连续、镜面反射不一致和心脏调制反射（rPPG）等物理特征被当作后验特征或直接忽略。

**Method:** PhyLAA-X扩展了LAA-X架构，通过三种端到端可微分的物理特征：光学流卷曲、镜面反射偏度和空间上采样的rPPG功率谱，采用交叉注意力门控和共振一致性损失直接注入注意力计算。部署在高效时空集成网络中，使用不确定性感知的自适应加权。

**Result:** 在FaceForensics++(c23)达到97.2%准确率/0.992 AUC-ROC；Celeb-DF v2为94.9%/0.981；DFDC为90.8%/0.966。相比最强基线LAA-Net在跨生成器场景提升4.1-7.3%，对抗攻击下保持79.4%准确率。单主干消融实验显示PhyLAA-X独立贡献4.2%跨数据集AUC提升。

**Conclusion:** PhyLAA-X通过物理条件化成功将语义不一致性与物理违反特征融合，显著提升了深度伪造检测的鲁棒性和泛化能力。完整的Aletheia生产系统已开源，包含预训练权重、对抗语料库和完整的可复现组件。
...
> 本文将CLIP检索问题从点对点相似性重新定义为邻域对齐问题，提出了基于匈牙利匹配的邻域重排序和查询条件下的局部引导两种方法，无需重新训练即可显著提升组合检索的性能和可控性。

<details>
  <summary>Details</summary>

**Motivation:** 传统CLIP检索方法虽然具有良好的全局跨模态对齐效果，但存在局部几何不一致的问题，导致检索结果排序混乱（如五边形与六边形的混淆），结果集扩散且控制性弱。现有方法主要优化点对点相关性或微调模型，但未能从根本上解决邻域结构问题。

**Method:** 提出两种推理时技术：(1)基于匈牙利匹配的邻域级重排序，奖励结构一致性；(2)查询条件下的局部引导，利用查询周围对比邻域的方向来重塑检索结果。两种方法都在局部邻域上操作但作用不同：重排序奖励对齐，局部引导控制邻域结构。

**Result:** 在属性绑定和组合检索任务上，所提方法显著提升了检索性能。这表明检索质量和可控性关键依赖于局部结构，且可在推理时无需重新训练即可利用这一特性。

**Conclusion:** 检索质量不仅取决于全局对齐，更关键的是局部邻域结构的对齐和控制。本文提出的推理时方法为改进CLIP类模型的组合检索能力提供了新思路，强调了局部结构在跨模态检索中的重要性。
...
> 提出了一种结合模糊逻辑的新型损失函数，用于解决脑部MRI图像分割中的不确定性问题，在IBSR和OASIS数据集上优于传统的分类交叉熵损失函数。

<details>
  <summary>Details</summary>

**Motivation:** 脑部MRI图像的精确分割对神经疾病诊断和医学图像计算至关重要。在深度学习方法中，损失函数对模型优化非常关键，而现有的损失函数无法有效处理像素分类中的固有不确定性。

**Method:** 结合模糊逻辑思想，将分类交叉熵(CCE)损失函数与基于模糊逻辑的模糊熵相融合，提出新型损失函数。该函数通过模糊逻辑来考虑像素分类中的固有不确定性。

**Result:** 在IBSR和OASIS两个公开基准数据集上，使用U-Net和U-Net++两种架构进行实验。结果表明，使用所提出损失函数训练的模型在各种性能指标上均优于使用CCE优化函数的模型。

**Conclusion:** 该模糊逻辑损失函数不仅有效提高了分割性能，还能在训练过程中处理有意义的不确定性，从而提高了模型预测的可靠性，为脑部图像分割提供了一种更鲁棒的解决方案。
...
> 提出了一种轻量级Transformer架构，通过统一的token化机制融合多种fNIRS表示，实现高效的疼痛识别。

<details>
  <summary>Details</summary>

**Motivation:** 疼痛是一种复杂且普遍的现象，具有重大的临床和社会负担，因此需要可靠的自动评估方法。现有方法在处理多模态信号时往往需要模态特定的适配，增加了架构复杂性。

**Method:** 设计轻量级Transformer，采用统一的token化机制将原始波形和功率谱密度等不同fNIRS表示投影到共享潜空间。通过结构化分割方案控制局部聚合和全局交互的粒度，保留空间、时间和时频特征。

**Result:** 在AI4Pain数据集上的实验结果显示该方法取得竞争性的疼痛识别性能，同时保持计算简洁性，可在GPU和CPU硬件上进行实时推理。

**Conclusion:** 该研究为疼痛自动评估提供了一种高效的多模态融合方案，在保持性能的同时显著降低了计算复杂度，具有重要的临床应用价值。
...
> LayerCache是一种层感知缓存框架，通过将Transformer分层分组并独立进行缓存决策，在保持高质量图像生成的同时显著提升Flow Matching模型的推理速度。

<details>
  <summary>Details</summary>

**Motivation:** Flow Matching模型虽然图像生成质量优秀，但通过大型Transformer网络进行迭代去噪的推理成本很高。研究发现Transformer不同层组具有显著的异质速度动态：浅层高度稳定适合激进缓存，而深层速度变化大需要完整计算。现有缓存方法将整个Transformer视为单体，无法利用这种异质性。

**Method:** LayerCache框架将Transformer划分为层组，在每个去噪步骤对每层组进行独立缓存决策。引入自适应JVP跨度K选择机制，利用每层组的稳定性测量来平衡估计精度和计算节省。构建时间步、层组和JVP跨度的三维调度问题，并用贪婪预算分配算法求解。

**Result:** 在Qwen-Image (1024x1024, 50步)上，LayerCache实现PSNR 37.46 dB（比MeanCache提升5.38 dB），SSIM 0.9834，LPIPS 0.0178（比MeanCache减少70%），获得1.37倍加速，在质量-速度帕累托前沿上优于所有之前的缓存方法。

**Conclusion:** 通过利用Transformer层间的异质性动态特征，LayerCache实现了更细粒度的缓存控制，在保持卓越图像生成质量的同时显著降低了Flow Matching模型的计算开销，为大模型的高效推理提供了新思路。
...
> HQA-VLAttack是一种针对视觉语言预训练模型的黑盒对抗攻击框架，通过文本和图像双阶段攻击，利用反拟合词向量语义替换和基于对比学习的图像扰动优化，显著提高攻击成功率。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉语言模型的黑盒对抗攻击方法存在两大问题：复杂的迭代交叉搜索策略消耗大量查询，或仅减少正样本相似性而忽略负样本相似性的变化，导致攻击性能受限。

**Method:** 提出HQA-VLAttack框架，包含两个阶段：1)文本攻击阶段使用反拟合词向量生成语义一致的替换词集；2)图像攻击阶段通过层重要性引导策略初始化对抗样本，然后利用对比学习优化图像扰动，同时降低正样本相似性并提高负样本相似性。

**Result:** 在三个基准数据集上的实验结果表明，HQA-VLAttack在攻击成功率方面显著优于强基线方法。

**Conclusion:** HQA-VLAttack通过同时优化文本和图像的对抗性扰动，巧妙地利用正负样本相似性的对比关系，为视觉语言模型的黑盒对抗攻击提供了一个简单高效的解决方案。
...
> VFCNet是一种融合显著性和边缘信息的低级构图评估模型，通过梯度矢量流场建模视觉注意力流动，在PICD基准测试上实现了33.1%和36.1%的性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 可靠的摄影构图计算评估需要能够辨别空间布局同时对语义内容鲁棒的特征，现有方法在此方面存在不足。

**Method:** 提出VFCNet模型，基于构图可理解为几何结构上视觉注意力流动的假设。模型融合显著性和边缘信息生成梯度矢量流场，采用双流GVF表示，通过注意力机制整合，并结合DINOv3骨干网络提取多尺度流动特征。

**Result:** 在PICD基准测试上达到最优性能(CDA-1: 0.683, CDA-2: 0.629)，相比之前最佳方法分别提升33.1%和36.1%。同时发现基于自监督DINOv3特征的简单分类器显著超越专门的构图模型。

**Conclusion:** VFCNet证明了基于视觉注意力流动的构图表示方法的有效性，同时揭示了自监督学习特征在构图评估任务中的巨大潜力，为该领域提供了新的研究方向。
...
> 本文提出了一种基于拓扑感知的大型视觉-语言模型剪枝框架，通过单纯复形和锯齿持久同调量化层间拓扑一致性，实现自适应剪枝，在保持关键表示转换的同时显著降低计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 大型视觉-语言模型(LVLMs)在多模态任务中表现优异，但存在巨大的计算和内存开销，限制了其在资源受限环境下的部署。现有的剪枝方法主要依赖局部相似性指标或静态代理信号，无法捕捉表示在整个模型深度中的全局动态演化，经常会错误地移除处于关键转换状态的层级。

**Method:** 提出拓扑感知的层剪枝框架：将各层隐藏状态表示为点云，使用单纯复形建模表示演化过程。通过锯齿持久同调技术量化层间拓扑一致性，实现自适应剪枝策略，保留关键的表示转换层。

**Result:** 在多个多模态基准测试上的广泛实验表明，所提框架在各种稀疏度比率下均持续优于现有剪枝方法，证明了该方法的有效性和鲁棒性。

**Conclusion:** 本研究成功将拓扑数据分析引入LVLM剪枝领域，通过捕获表示的全局演化特征，实现了更精准的层剪枝。该框架在保持模型性能的同时有效降低了计算复杂度，为资源受限场景下的多模态模型部署提供了新思路。
...
> Motif-Video 2B通过创新的架构设计，仅用20亿参数就在视频生成质量上超过了140亿参数的Wan2.1，证明精心设计的模型结构比单纯扩大规模更有效。

<details>
  <summary>Details</summary>

**Motivation:** 传统视频生成需要海量数据、庞大参数和巨大算力，本文探索如何在有限的预算下（少于1000万片段、10万H200 GPU小时）实现高质量文本到视频生成。

**Method:** 提出双关键创新：1) 共享交叉注意力机制增强长视频序列的文本控制；2) 三段式骨干网络分离早期融合、联合表示学习和细节细化。结合动态token路由和冷冻特征对齐的高效训练策略。

**Result:** 在VBench基准测试中达到83.76%分数，超越Wan2.1 14B模型，参数量减少7倍，训练数据也大幅减少。分析显示模型后期模块展现出更清晰的跨帧注意力结构。

**Conclusion:** 精心设计的架构专业化结合高效训练策略，能够缩小甚至超越通常与更大规模视频模型相关的质量差距，证明了模型容量组织方式比单纯增加容量更重要。
...
> 本研究评估了17个多模态大语言模型在复杂医疗表单数字化任务中的表现，最新模型达到85%准确率，提示优化可显著提升性能，为自动化手写文档数字化提供了可行方案。

<details>
  <summary>Details</summary>

**Motivation:** 人工数字化结构化手写文档速度慢、成本高，特别是在低收入和中等收入国家，需要自动化解决方案来提高效率。

**Method:** 对17个前沿多模态大语言模型进行基准测试，使用包含日期、印刷文本、手写回复和变化挑战的真实医疗表单，通过不同提示策略优化模型性能。

**Result:** Google和OpenAI最新模型准确率达85%，加权F1分数约90%；Gemini 3.1整体表现最佳，GPT 5.4日期提取最可靠，Claude Sonnet 4.6格式化字段表现突出；提示优化使宏平均指标提升60%以上。

**Conclusion:** 多模态大语言模型的快速进步为实现复杂手写工作流的完全自动化提供了令人信服的路径，对中低收入国家具有重要应用价值。
...
> 本文提出了一种结合DINOv2和增强型LSTM的混合模型，使用有限的每日胚胎图像预测囊胚形成，在704个胚胎视频上实现了96.4%的准确率，即使在缺失帧的情况下也表现良好。

<details>
  <summary>Details</summary>

**Motivation:** 体外受精(IVF)中选择最优胚胎是一个关键但具有挑战性的步骤，主要依赖于手动检查大量延时成像数据。关键挑战是从有限的每日可用图像中预测囊胚形成，且许多诊所缺乏完整的延时系统导致完整视频不可用。

**Method:** 提出新颖的混合模型，将基于transformer的视觉模型DINOv2与具有多头注意力层的增强型长短期记忆(LSTM)网络结合。DINOv2从胚胎图像中提取有意义的特征，LSTM模型则使用这些特征分析胚胎随时间的发展并生成最终预测。

**Result:** 在704个真实胚胎视频数据集上进行测试，模型达到了96.4%的准确率，超越了现有方法。该模型在缺失帧的情况下也表现良好，适用于成像系统有限的IVF实验室。

**Conclusion:** 该方法可以帮助胚胎学家更高效、更自信地选择更好的胚胎，为IVF实验室提供了一个实用且有效的辅助决策工具。
...
> 医学多图像推理基准测试MedThinkVQA显示，当前大语言模型在整合多视图临床证据方面存在显著瓶颈。

<details>
  <summary>Details</summary>

**Motivation:** 现有医学QA模型通常处理单张图像，但真实临床诊断需要综合分析多张检查图像的证据，缺乏相应的评估基准。

**Method:** 构建包含8,067个病例的专家标注MedThinkVQA基准，平均每例6.62张图像，通过中间监督和步骤级评估测试模型的多图像推理能力。

**Result:** 表现最佳的闭源模型Claude-4.6-Opus准确率仅57.2%，开源模型最高为52.2%；70%以上错误源于图像读取和跨视图整合；增加推理时间计算只有在视觉基础可靠时才有效。

**Conclusion:** 医学多图像推理的核心挑战不在于推理长度，而在于建立可靠的证据基础、对齐和组合机制，以处理真实世界多模态临床输入。
...
> 论文提出了一种新颖的变分方法，用于计算给定点云的高精度全局符号距离函数（SDF），通过显式考虑梯度跳跃集（与中轴线重合）并利用相场近似来处理不连续性。

<details>
  <summary>Details</summary>

**Motivation:** 传统SDF计算方法在全局精度和中轴线处理方面存在局限性，需要一种能够显式处理梯度不连续性并提供高精度全局SDF的新方法。

**Method:** 采用高阶变分公式，考虑SDF梯度的跳跃集（中轴线），强制执行eikonal方程和零水平集约束，使用Ambrosio-Tortorelli型相场近似使计算可行，并通过神经网络近似SDF和相场函数。

**Result:** 实验表明该方法在近场和全局都具有很高的精度，定量和定性比较结果显示相比其他方法具有显著优势。

**Conclusion:** 提出的变分方法成功解决了点云SDF计算中的梯度不连续性问题，通过相场隐式描述中轴线，实现了高精度的全局SDF重建，在点云处理领域具有重要应用价值。
...
> SynthPID通过基于真实图纸生成合成P&ID图，将管道拓扑结构真实性提高到63.8%的边缘检测准确率，解决了现有方法仅33%准确率的问题。

<details>
  <summary>Details</summary>

**Motivation:** 工程图纸的专有性导致缺乏公共基准数据集，现有合成方法生成的符号随机分布，与真实工艺流程图相差甚远，严重限制了P&ID数字化自动化的发展。

**Method:** 提出SynthPID合成数据集生成方法，从真实图纸中提取管道拓扑作为种子，生成665个合成P&ID图；配合适配高分辨率图表的基于块的Relationformer模型进行训练。

**Result:** 仅使用SynthPID训练的模型在PID2Graph OPEN100基准上达到63.8±3.1%的边缘mAP，比模板基线方法提高约30个百分点，与真实数据训练的oracle模型差距仅8个百分点。

**Conclusion:** 生成质量而非模型选择是性能提升的关键，约400张合成图像后收益趋于平缓，表明种子多样性是主要限制因素，为P&ID自动化数字化提供了新的数据解决方案。
...
> BARD框架将预训练的自回归视觉语言模型转换为高效的扩散VLM，在保持多模态能力的同时实现3倍解码速度提升。

<details>
  <summary>Details</summary>

**Motivation:** 自回归VLM虽具备强大的多模态能力，但逐token解码方式限制了推理效率；直接将自回归模型转换为扩散模型会导致性能显著下降。

**Method:** 提出BARD桥接框架，结合渐进监督块合并和阶段内部蒸馏策略，使用混合噪声调度和内存友好训练技术来实现高效转换。

**Result:** 使用≤4.4M数据成功将Qwen3-VL转换为大型扩散VLM，在4B和8B尺度上均达到SOTA性能，解码速度提升3倍。

**Conclusion:** 证明了在扩散框架内蒸馏的有效性，为保持模型质量的同时实现高效并行解码提供了实用解决方案。
...
> 研究发现多模态大语言模型存在视觉主导幻觉现象(VDH)，并提出PriceBlind攻击框架，能通过微小视觉干扰使金融交易智能体做出错误决策。

<details>
  <summary>Details</summary>

**Motivation:** 随着多模态大语言模型在移动端的快速增长，这些模型开始执行高风险金融交易，但其对抗鲁棒性尚未得到充分研究。发现一个关键漏洞：在基于截图的价格约束环境中，几乎不可见的视觉线索可能覆盖文本价格证据，导致智能体做出非理性决策。

**Method:** 提出PriceBlind，一种隐蔽的白盒对抗攻击框架。该方法通过语义解耦损失函数，利用基于CLIP编码器的模态差距，将图像嵌入与低成本、价值相关的锚点对齐，同时保持像素级别的保真度。

**Result:** 在E-ShopBench基准测试上，PriceBlind在白盒评估中达到约80%的攻击成功率；在简化的单轮坐标选择协议下，集成DI-FGSM攻击在GPT-4o、Gemini-1.5-Pro和Claude-3.5-Sonnet上实现了35-41%的攻击成功率。

**Conclusion:** 研究表明鲁棒编码器和验证后执行防御能够显著降低攻击成功率，但需要权衡一定的干净准确率。这揭示了多模态智能体在金融应用中的安全隐患，为相关安全防护研究提供了重要参考。
...
> 本文系统性回顾了文生图模型公平性研究，提出了偏见分类和公平性概念的分类体系，并构建了从描述性度量转向基于目标的严格测试框架。

<details>
  <summary>Details</summary>

**Motivation:** 文生图模型在各行业广泛应用，但经常表现出社会刻板印象。现有研究在概念上存在模糊性，如'偏见'和'公平性'术语缺乏明确的区分和操作性定义。

**Method:** 对文生图公平性文献进行全面系统性综述，将现有工作组织成偏见类型和公平性概念的分类体系，评估'目标公平性'和'阈值公平性'之间的差距，调研从提示工程到扩散过程操作的缓解策略。

**Result:** 提出了一个新的公平性操作化框架，超越了描述性度量，转向严格的、基于目标的测试方法，为更负责任的生成式AI开发提供了路径。

**Conclusion:** 该研究为理解文生图模型的社会偏见提供了系统化的理论框架，并为构建更加公平可解释的生成式AI系统提供了实用的评估和改进指南。
...
> SmoGVLM是一种小型图增强视觉语言模型，通过图神经网络整合结构化知识，使小模型在知识密集型推理任务上超越大模型.

<details>
  <summary>Details</summary>

**Motivation:** 现有大型视觉语言模型存在幻觉问题和在知识密集型推理任务中知识基础不足的缺陷，需要更高效、更准确的解决方案.

**Method:** 提出SmoGVLM模型，将结构化知识与视觉、文本模态整合，使用图神经网络进行增强，并在1.3B到13B不同规模模型上进行验证.

**Result:** 小模型(1.3B)使用该方法后性能提升高达16.24%，能够超越更大的VLM模型和微调基线模型.

**Conclusion:** 结构化知识增强对于构建高效的小规模多模态推理系统具有巨大潜力，为解决大模型幻觉问题提供了新思路.
...
> 本文提出了一种基于多重单目相机的快速在线3D多目标跟踪与姿态估计方法，仅需2D检测输入，无需 costly 3D训练数据或深度学习模型，在保持准确性的同时显著提升计算效率。

<details>
  <summary>Details</summary>

**Motivation:** 解决现有3D多目标跟踪与姿态估计方法需要大量3D训练数据和计算资源的问题，同时提高系统在相机间歇性断连等不稳定环境下的鲁棒性。

**Method:** 基于贝叶斯最优多目标跟踪滤波器的高效实现，仅需2D边界框和姿态检测结果，使用公开预训练的2D检测模型，实现快速在线处理。

**Result:** 算法在准确性不降低的情况下，显著快于最先进方法，并且在相机 intermittently 断连或重连的场景下表现出强大的鲁棒性能。

**Conclusion:** 该方法为3D多目标跟踪和姿态估计提供了一种高效、实用且鲁棒的解决方案，降低了对3D训练数据和计算资源的依赖。
...
> 本文提出了一种新颖的隐私保护语义分割方法，能够为每个客户端和图像使用独立密钥进行加密，并在加密图像上进行模型训练和推理，实验证明其在Cityscapes数据集上有效。

<details>
  <summary>Details</summary>

**Motivation:** 在保护隐私的前提下进行语义分割任务的挑战，特别是在分布式环境中需要保护每个客户端的数据隐私，同时保持模型性能。

**Method:** 模型创建者和每个客户端使用本地生成的密钥加密图像，在加密图像上进行模型训练和推理。为减少性能损失，额外采用图像加密方法生成测试图像。使用基于视觉Transformer的SETR模型进行验证。

**Result:** 在Cityscapes数据集上的实验证明，所提出的方法能够在保护隐私的同时保持有效的语义分割性能。

**Conclusion:** 该方法成功实现了独立密钥的隐私保护语义分割，为分布式环境中的隐私保护计算提供了有效解决方案。
...
> 研究提出了一个专家标注的胚胎图像数据集，结合自然语言描述，支持开发可解释的AI胚胎选择系统，提高决策透明度和患者沟通质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有AI胚胎选择方法受限于数据适应性、设备依赖性和缺乏可解释性。现代患者对专家决策提出质疑，需要基于证据的决策支持来实现透明决策和尊重患者沟通。

**Method:** 构建包含胚胎图像和自然语言形态学描述的专家标注数据集，支持微调现代视觉语言模型，通过预测胚胎描述自动从文献中提取科学证据。

**Result:** 该数据集支持研究基于语言的、可解释和透明的自动胚胎评估，能显著改善决策过程并提高患者结果。

**Conclusion:** 提出的标注数据集为开发可解释的AI胚胎评估工具提供了重要基础，有望实现循证决策支持，增强医患沟通透明度，并持续改善IVF治疗效果。
...
> 研究表明，在医学图像对抗性攻击评估中，仅使用攻击成功率(ASR)指标是不够的，需要采用多指标框架来全面评估对抗性风险。

<details>
  <summary>Details</summary>

**Motivation:** 随着深度学习在医学图像分析中的应用日益广泛，其对对抗性扰动的脆弱性引发了临床部署的严重担忧。现有的评估主要依赖ASR这一二元指标，但该指标忽略了扰动强度、图像感知质量和跨架构攻击可迁移性等关键因素，导致评估结果不完整。特别是在Vision Transformers等新架构兴起的情况下，单一ASR指标是否能有效捕捉对抗性行为存在疑问。

**Method:** 研究者对七种模型（VGG-16、ResNet-50等CNNs，以及DeiT、Swin Transformer等ViTs）在四个医学图像数据集（PathMNIST、DermaMNIST、RetinaMNIST、CheXpert）上进行系统性实证研究。采用七种攻击方法，在五个扰动预算水平下，测量ASR、PSNR、SSIM和L₂扰动幅度等多个指标。

**Result:** 研究发现感知和失真指标之间存在强相关性，但与ASR几乎无相关性，这一规律在CNNs和ViTs中均成立。结果表明，ASR单独作为对抗性鲁棒性和可迁移性的指标是不充分的。

**Conclusion:** 医学AI的对抗性风险评估需要采用多指标框架，不仅要考虑攻击效能，还应涵盖攻击方法和相关成本，以实现更全面的风险评估。
...
> 本文提出了一种名为PoInit-of-View的新型3D重建投毒方法，通过靶向攻击SfM初始化模块来实现跨系统的可迁移投毒效果，在黑盒迁移设置下显著优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3D重建投毒研究将整个重建管道视为黑盒进行攻击，没有挖掘特定模块的漏洞。作者发现SfM初始化作为许多重建系统的几何核心，存在新的安全漏洞，可以成为实现跨系统投毒攻击的关键攻击点。

**Method:** 提出PoInit-of-View方法，通过优化对抗扰动来人为引入对应3D点投影处的跨视图梯度不一致性。这种不一致性破坏关键点检测和特征匹配，从而破坏SfM中的姿态估计和三角剖分，最终导致低质量的渲染视图。同时提供了连接跨视图不一致性与对应关系崩溃的理论分析。

**Result:** 在多个3D重建系统和数据集上的实验表明，PoInit-of-View方法有效。在黑盒迁移设置下（如3DGS到NeRF），相比单视图基线在PSNR上提升25.1%，在SSIM上提升16.5%。

**Conclusion:** 通过靶向SfM初始化模块的跨视图梯度不一致性攻击，可以实现更有效的跨3D重建系统的投毒攻击，为3D重建系统的安全性研究提供了新的视角和方法。
...
> BookAgent是一个安全感知的多智能体协作框架，能够端到端生成高质量、安全的视觉叙事故事书，解决了多模态基础和安全约束的关键问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大生成模型在多模态生成方面取得进展，但生成插图故事书仍是开放挑战。主要问题在于：1）之前工作将任务分解为独立阶段，缺乏整体多模态基础；2）缺乏针对儿童的特定安全约束集成到叙事规划和序列级多模态验证中。

**Method:** 提出BookAgent框架，通过多智能体协作实现端到端故事书合成。包含：1）联合规划、编剧、插图和全局修复不一致性；2）动态校准页面级文本脚本与视觉布局的对齐；3）从时间维度验证并修复角色身份和叙事逻辑的全局不一致性。

**Result:** 大量实验表明BookAgent在叙事连贯性、视觉一致性和安全合规性方面显著优于现有方法，为复杂多模态创作中的可靠智能体提供了强大的范式。

**Conclusion:** BookAgent成功解决了传统多阶段方法的局限性，通过整体多模态基础和儿童特定安全约束，实现了高质量、安全的视觉叙事生成，为多智能体协作在复杂创作任务中的应用提供了新的可能性。
...
> 论文提出了一种3D指纹识别方法，通过B样条曲线拟合 unwrap 3D点云指纹，转换为灰度图像后使用传统2D方法识别，显著优于现有技术。

<details>
  <summary>Details</summary>

**Motivation:** 3D指纹识别相比传统2D系统具有更好的卫生性和安全性，但面临像素高度差异影响纹路提取和配准限制等挑战。

**Method:** 使用B样条曲线拟合对3D点云指纹进行展开，通过映射点相对高度将展开后的点云转换为灰度图像，再采用传统2D指纹识别方法进行识别。

**Result:** 在三组实验中分别获得0.2072%、0.26%和0.22%的等错误率，优于现有方法；在跨session实验中EER为1.50%，超越3D指纹平坦化技术。

**Conclusion:** 所提方法有效解决了3D指纹识别中的高度变异和配准限制问题，通过转换为2D图像实现了高性能的3D指纹识别。
...
> 提出了一种顺序式文本到3D场景生成方法3D-ARD+，能够根据文本指令逐步生成和摆放物体，解决了现有方法在生成复杂场景布局和物体细节方面的局限性。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本到场景生成方法要么只生成场景布局，要么只生成物体，且生成的场景布局过于简单，生成的场景与包含复杂形状、外观和空间排列描述的文本输入存在不一致性。

**Method:** 提出3D-ARD+模型，结合自回归生成和扩散生成两步流程：首先在场景空间自回归生成粗粒度3D潜在表示，然后在物体空间扩散生成细粒度3D潜在表示，最终解码为物体几何和外观；构建了包含23万室内场景和配对文本指令的数据集。

**Result:** 在70亿参数规模的3D-ARD+模型上进行了评估，模型能够根据文本指令生成和摆放遵循复杂空间布局和语义的物体。

**Conclusion:** 该顺序式文本到场景生成新范式实现了交互式场景创建，能够同时处理场景布局和物体细节生成，显著提升了生成场景与文本描述的一致性。
...
> 本文提出了PA-TCNet框架，通过病理感知的节律状态建模和生理引导的目标校准，解决了中风患者EEG解码中的跨被试泛化难题，在两个数据集上实现了66.56%和72.75%的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 中风患者运动想象脑机接口的跨被试EEG解码对运动康复至关重要，但病变相关的异常时间动态化和显著的被试间异质性往往削弱了模型的泛化能力。现有适应方法容易被病理慢波活动和不稳定的目标域伪标签误导。

**Method:** PA-TCNet包含两个协同组件：1) 病理感知节律状态Mamba(PRSM)模块，将EEG时空特征分解为慢变的节律性上下文和快速瞬态扰动，通过选择性状态传播捕获异常时间动态；2) 生理引导目标校准(PGTC)模块，构建源域感觉运动区域模板，施加生理一致性约束并动态优化目标域伪标签。

**Result:** 在XW-Stroke和2019-Stroke两个独立中风EEG数据集的留一被试交叉验证中，PA-TCNet分别达到66.56%和72.75%的平均准确率，显著优于现有基线方法。

**Conclusion:** 联合建模病理时间动态和生理约束的伪监督可以为个性化中风运动想象康复提供更鲁棒的跨被试初始化，该框架有望改善中风患者的BCI康复效果。
...
> 本文首次系统研究了标签噪声对泛化凝视估计的负面影响，并提出SeeTN框架，通过语义嵌入空间和亲和力一致性来减轻噪声，提升跨域泛化性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的泛化凝视估计方法忽略了获取精确凝视标注的困难性所带来的标签噪声问题，这会影响模型的泛化性能。实际应用中精确的凝视标注难以获取，标签噪声是普遍存在的问题。

**Method:** 提出See-Through-Noise（SeeTN）框架：1）通过基于原型的变换构建语义嵌入空间，保持凝视特征与连续标签间一致的拓扑结构；2）测量特征-标签亲和力一致性来区分噪声和干净样本；3）在语义流形中引入亲和力正则化，将凝视相关信息从干净样本转移到噪声样本。

**Result:** 实验证明SeeTN能有效减轻源域噪声的不利影响，在不损失源域准确性的前提下实现更优的跨域泛化性能，凸显了在泛化凝视估计中显式处理噪声的重要性。

**Conclusion:** 标签噪声对凝视估计的泛化性能有显著的负面影响，SeeTN框架通过语义结构对齐和域不变关系约束，为解决这一问题提供了有效方案，为实际应用中的凝视估计提供了更鲁棒的解决方案。
...
> 提出了一种基于复杂正交匹配追踪和视觉Transformer的收缩期杂音自动分类系统，在CirCor DigiScope数据集上达到95.96%的分类准确率。

<details>
  <summary>Details</summary>

**Motivation:** 收缩期杂音是心脏异常的重要指标，但其强度、音调和质量变化很大，需要精确识别才能准确诊断心脏疾病。传统的杂音识别方法存在变异性大、一致性差的问题。

**Method:** 采用两阶段方法：特征提取模块使用复杂正交匹配追踪将杂音段投影到多分辨率复杂Gabor基函数组成的冗余字典上，生成可变分辨率的时间-频率特征矩阵；分类模型基于视觉Transformer，通过卷积神经网络处理不同分辨率的输入矩阵进行patch tokenization，然后使用多头注意力机制和残差连接进行分类。

**Result:** 在CirCor DigiScope数据集的四种收缩期杂音分类任务中，系统达到了95.96%的分类准确率，证明了该方法的有效性。

**Conclusion:** 多分辨率特征提取与基于Transformer的特征分类相结合的方法显著提高了心脏杂音识别的准确性和可靠性，为心脏疾病的自动诊断提供了有效的技术方案。
...
> 本研究提出了一种双阶段特征融合的两级网络架构，用于人类活动识别(HAR)，通过结合晚期融合和中间融合策略，在CNN、LSTM等15种网络架构中找到最优配置，超越了仅使用晚期融合的方法和基线模型。

<details>
  <summary>Details</summary>

**Motivation:** 人类活动识别(HAR)在传感器数据处理中很重要，单一神经网络模型存在局限性，而多层或混合模型的组件结构安排对整体性能影响关键，需要探索更好的网络融合策略。

**Method:** 设计了两级网络架构框架，实现双阶段特征融合：晚期融合（组合第一级网络的输出）和中间融合（整合第一级和第二级的特征）。评估了15种CNN、LSTM和卷积LSTM的不同网络架构配置，包括有无中间融合的情况。

**Result:** 在两个公开基准数据集上的实验表明，结合晚期融合和中间融合的架构比仅使用晚期融合的架构获得更高精度，最优配置明显优于基线模型。

**Conclusion:** 提出的双阶段特征融合两级网络架构有效提升了人类活动识别的性能，验证了晚期融合与中间融合结合策略的优越性，为HAR任务提供了一种有效解决方案。
...
> 本文提出了Camo-M3FD数据集，这是一个专门用于跨光谱伪装行人检测的新基准数据集，包含配对的可见光-热红外图像，填补了安全关键人类检测领域的空白。

<details>
  <summary>Details</summary>

**Motivation:** 现有的伪装物体检测基准主要关注生物物种，而安全关键的行人检测中，目标融入背景的情况缺乏研究。尽管多光谱检测可以缓解能见度差的问题，但伪装行人的挑战仍然很大程度上未被探索。

**Method:** 基于M3FD数据集创建了Camo-M3FD，使用定量指标确保前景和背景相似度高，提供高质量的像素级掩码，并使用最先进的COD模型建立了标准化评估框架。

**Result:** 实验结果表明，热红外信号提供了不可或缺的定位线索，但多光谱融合对于细化结构细节至关重要，证明了该数据集的有效性。

**Conclusion:** Camo-M3FD作为基础资源，为开发鲁棒的安全关键检测系统提供了重要支持，该数据集已公开在GitHub上供研究人员使用。
...
> 提出一种摊销框架，实现多模态思维模型的实时视觉归因流，通过从注意力特征中学习语义区域的因果效应，达到与穷举因果方法相当的真实性同时支持实时可视化。

<details>
  <summary>Details</summary>

**Motivation:** 多模态思维模型在生成代码或解决数学问题时需要视觉证据支撑，但现有的验证方法要么需要昂贵的重复反向传播或扰动，要么像原始注意力图虽然即时但缺乏因果有效性。

**Method:** 引入摊销方法，直接从注意力特征编码的丰富信号中学习估计语义区域的因果效应，避免重复计算。

**Result:** 在五个多样化基准测试和四个思维模型上，该方法实现了与穷举因果方法相当的真实性，同时支持实时视觉归因流，用户可以在模型推理过程中观察证据，而非事后。

**Conclusion:** 通过轻量级学习而非蛮力计算，实时且真实的多模态思维模型归因是可以实现的，为模型的透明度和可解释性提供了实用解决方案。
...
> 本文提出了MambaKick框架，通过结合预训练的人类动作识别嵌入和轻量级Mamba时序模型来预测足球罚球方向，实现了高效且实用的意向预测。

<details>
  <summary>Details</summary>

**Motivation:** 足球点球决策面临极端时间限制，守门员需要从踢球者的动作中提前预判射门方向。现有方法依赖运动学重建或手工特征，计算复杂且难以实时应用。

**Method:** MambaKick框架使用预训练的HAR模型提取以球接触点为中心的短视频片段嵌入，结合选择性状态空间模型(Mamba)进行高效的序列聚合，并融入简单的上下文元数据(场地侧、惯用脚等)作为补充信息。

**Result:** 在不同HAR骨干网络上，MambaKick显著优于或匹配强嵌入基线，三分类准确率达53.1%，二分类准确率达64.5%。

**Conclusion:** 将预训练HAR表示与高效状态空间时序建模相结合，是现实运动视频中低延迟意向预测的实用方向，代码已开源。
...
> 本研究提出了一种名为IncepDeHazeGAN的新型生成对抗网络，采用Inception模块和多层特征融合技术，在单幅图像去雾任务中取得了最先进的效果。

<details>
  <summary>Details</summary>

**Motivation:** 在计算机视觉中，去雾技术对于恢复在多云或雾天条件下获取的图像质量至关重要。现有的去雾方法在特征提取和利用方面仍有改进空间，需要更有效的网络架构来处理复杂的雾霾条件。

**Method:** 提出IncepDeHazeGAN网络，结合Inception模块实现多尺度特征提取，采用多层特征融合设计实现特征的高效复用。同时应用Grad-CAM XAI技术来可视化网络关注的去雾区域，增强模型的可解释性。

**Result:** 在多个数据集上的实验表明，所提出的网络取得了最先进的去雾效果，证明了其在不同雾霾条件下的适应性和有效性。

**Conclusion:** IncepDeHazeGAN通过Inception模块的多尺度特征提取和多层特征融合机制，有效提升了单幅图像去雾的性能。结合Grad-CAM可视化技术，不仅保证了优异的去雾效果，还增强了模型的可解释性，为去雾任务提供了新的解决方案。
...
> AVRT框架通过单模态教师模型生成高质量视听推理轨迹，经SFT和强化学习训练，在3B和7B参数规模上实现了SOTA的视听推理性能。

<details>
  <summary>Details</summary>

**Motivation:** 将文本推理能力迁移到多模态环境仍具挑战性，主要原因是目标多模态组合中高质量的推理数据有限。

**Method:** AVRT框架使用专门的单模态模型生成独立的视觉和音频推理轨迹，通过LLM合并模型融合轨迹，随后进行监督微调冷启动和强化学习两阶段训练。

**Result:** 3B和7B参数模型在7个视听和音频基准测试中达到SOTA结果，包括OmniBench、DailyOmni和MMAR，证明跨模态训练也能迁移到单模态任务。

**Conclusion:** AVRT建立了多模态推理模型的新训练流程，有效解决了多模态高质量数据稀缺问题，为多模态推理提供了有效解决方案。
...
> IK-GAT是一个轻量级图注意力网络，能够从3D关节位置一次性快速重建全身关节朝向，优于传统迭代优化方法。该网络仅374K参数，在CPU上可达650+ FPS，适合实时应用。

<details>
  <summary>Details</summary>

**Motivation:** 逆向运动学(IK)是动画、机器人学和生物力学中的核心操作。现有 pipeline 中从稀疏3D关节位置恢复完整朝向是欠约束问题（特别是骨骼轴向扭转模糊），传统的IK求解器依赖迭代优化，速度慢且对噪声敏感，无法满足实时人形虚拟形象的需求。

**Method:** 提出IK-GAT图注意力网络，在骨骼父子关系图上进行消息传递来利用运动学结构。模型采用与骨骼对齐的世界坐标系表示来预测旋转，使扭转轴明确化，并可精确转换为标准的父相对局部旋转。网络使用连续6D旋转表示，并通过SO(3)测地损失和可选的前向运动学一致性正则化进行训练。

**Result:** IK-GAT在CPU上运行速度超过650 FPS，仅需374K参数。在无预热初始化的情况下，其性能优于基于VPoser的逐帧迭代优化方法，且计算成本显著降低。该方法对初始姿态和输入噪声具有鲁棒性。

**Conclusion:** IK-GAT成功解决了从稀疏3D关节位置到完整关节朝向的实时重建问题，为动画系统和 rigged 虚拟形象提供了高效的解决方案。其轻量级设计和高性能使其适用于实现在线和实时应用，在计算效率和视觉质量之间取得了良好平衡。
...
> 本文提出了首个三模态(RGB-红外-事件)无人机目标检测框架，包含双流分层视觉变换器和专门的融合模块(MAGE+BiTE)，并在新构建的10,489帧无人机数据集上验证了其优越性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的无人机目标检测在光照变化、运动模糊和动态场景下鲁棒性不足。虽然RGB、热成像和事件相机各有优势，但三模态的统一融合检测尚未被系统研究，需要填补这一空白。

**Method:** 设计了双流分层视觉变换器架构，在选定编码器深度处引入模态感知门控交换(MAGE)进行跨传感器通道和空间门控，以及双向令牌交换(BiTE)模块进行双向令牌级注意力融合，生成分辨率保持的融合特征图用于两阶段检测器。

**Result:** 通过61组实验证明了三模态融合优于所有双模态基线，融合深度影响显著，轻量级CSSA变体能以最小代价恢复大部分性能提升。在包含24,223个标注车辆的新数据集上系统评估了各种因素。

**Conclusion:** 这是首个三模态无人机目标检测的系统化基准和模块化骨干网络，为多模态融合在无人机感知中的应用提供了重要参考和实践基础。
...
> 本研究系统比较了卷积神经网络、transformer和大型预训练基础模型在滑坡检测任务中的性能，结果表明基于transformer的模型表现优异，参数高效微调在保持相近精度的同时可减少95%的可训练参数。

<details>
  <summary>Details</summary>

**Motivation:** 高分卫星影像滑坡检测对灾害响应和风险评估至关重要，但现代分割架构和微调策略在该问题上的相对有效性尚未得到充分理解。

**Method:** 使用GDCLD数据集，在一致的训练和评估协议下比较CNN和transformer分割模型以及大型预训练基础模型，并对比了完整微调和参数高效微调方法(LoRA和AdaLoRA)的性能效率权衡。

**Result:** 基于transformer的模型实现了强大的分割性能，参数高效微调在减少95%可训练参数的情况下仍能保持与完整微调相当的精度，并通过分布偏移分析验证了模型的泛化能力。

**Conclusion:** 研究为滑坡检测任务中不同架构和微调策略的选择提供了重要参考，参数高效微调在保持性能的同时显著降低了计算成本，具有实际应用价值。
...
> 本研究发现人类和视觉模型能够零样本识别外观缺失的动作视频，并提出了一个双通路3D CNN模型来模拟这一行为。

<details>
  <summary>Details</summary>

**Motivation:** 人类能够感知身体运动即使在形状线索被破坏的情况下，但尚不清楚人类和视觉模型是否能以零样本方式泛化到真实世界动作视频的外观缺失变换中。

**Method:** 22名参与者进行实验室心理物理学实验，使用自然视频训练后测试两种外观缺失视频的零样本识别；开发了结合RGB通路（形态）和光流通路（运动）的双通路3D CNN模型，包含格式塔共同命运分组启发的相干门控机制。

**Result:** 参与者能显著高于机会水平识别两种外观缺失视频，但准确率低于自然视频；模型能够泛化到两个外观缺失数据集，性能优于当代视频分类模型，缩小了与人类表现的差距。

**Conclusion:** 运动基表示对外观缺失视频泛化至关重要，形态通路提高自然视频性能，研究结果支持使用多流架构来建模基于视频的动作识别。
...
> C-GenReg是一个无需训练的3D点云配准框架，通过结合世界级生成先验和面向配准的视觉基础模型，实现了跨模态、跨环境的强泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于学习的3D点云配准方法在跨传感模态、采样差异和环境变化时泛化能力有限，需要一种无需训练且具有强泛化性能的解决方案。

**Method:** C-GenReg利用世界基础模型从输入几何生成多视图一致的RGB表示，将配准问题转移到图像域；使用预训练的VFM提取密集对应关系；通过

**Result:** 在室内(3DMatch, ScanNet)和室外(Waymo)基准测试中展现出强大的零样本性能和优越的跨域泛化能力，首次成功在无图像数据的真实室外LiDAR数据上实现了生成式配准框架。

**Conclusion:** C-GenReg作为一个零样本、即插即用的框架，通过生成式模态转移和概率冷融合策略，有效结合了几何和图像模态的优势，为3D点云配准提供了一种无需训练且具有强泛化性能的新范式。
...
> 该论文提出了一种结合多尺度注意力机制的改进3DETR模型，通过上采样操作生成高分辨率特征图，在ScanNetv2数据集上将mAP@50提高了4.78%。

<details>
  <summary>Details</summary>

**Motivation:** 点云数据的稀疏性和缺乏全局结构特征使得3D目标检测具有挑战性，现有方法难以同时捕捉局部几何细节和全局上下文信息。

**Method:** 在3DETR架构中集成多尺度注意力(MSA)机制，引入上采样操作生成高分辨率特征图，增强对小尺寸和语义相关物体的检测能力。

**Result:** 在ScanNetv2数据集上，3DETR+MSA模型相比基线在mAP@25提升近1%，在mAP@50提升4.78%；针对轻量级模型的upsampling策略适配问题进行了分析。

**Conclusion:** 分层特征提取与注意力机制的结合能有效提升3D场景理解能力，但需要针对不同模型规模调整up采样策略以获得最佳性能。
...
> 该论文提出了一种基于自监督编码器iDoc和开放集检测器的图像模式搜索模型，在保持精度的同时将处理速度提升10倍，并在小尺寸非方形查询上达到新的最先进性能。

<details>
  <summary>Details</summary>

**Motivation:** 面对数字图书大规模化的趋势，现有的历史文档模式搜索策略精度低（整体精度0.494，小非方形查询仅0.427）且处理速度慢（在DocExplore数据集上需7秒），亟需改进。

**Method:** 提出iDoc编码器，采用自监督策略训练，结合开放集检测器加速搜索，并引入非极大值抑制减少假阳性检测。

**Result:** 模型在速度上提升10倍，在模式搜索和文档检索上达到竞争性结果，小非方形查询精度提升至0.612，创出新记录。

**Conclusion:** 该方法有效解决了现有模式搜索技术在精度和效率方面的局限，为大规模数字图书的图形模式搜索提供了实用解决方案。
...
> 本文提出了一种无需训练的智能代理管道，能够通过协调专用外部工具来自动化执行端到端的脑MRI分析工作流程。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大型语言模型缺乏原生3D空间推理能力，无法直接分析CT或MRI等体积医学影像。虽然新兴的智能体AI提供了一个新的解决方案，但此类框架在复杂的多步骤放射学工作流程中的可行性尚未得到充分探索。

**Method:** 构建了一个训练无关的智能代理管道，使用现成的领域特定工具，让LLMs（GPT-5.1、Gemini 3 Pro、Claude Sonnet 4.5）自主执行预处理（颅骨剥离、配准）、病理分割（胶质瘤、脑膜瘤、转移瘤）和体积分析等复杂工作流程，并比较了单代理与多代理协作模式的效果。

**Result:** 系统能够成功完成从单次扫描分割到需要多时间点比较的纵向反应评估等日益复杂的放射学任务，证明了智能体AI通过工具使用即可解决高度神经放射学图像分析问题，无需训练或微调。

**Conclusion:** 基于智能体的AI方案为解决LLM在3D医学影像分析中的局限性提供了有效途径，通过释放工具使用能力而非内在3D处理能力，为自动化复杂的放射学工作流程开辟了新方向。
...
> AW4RE是一种以感知为中心的生成世界模型，通过4D知情检索和条件生成来估计感知动作下的观察过程，为部分可观测环境中的感知决策提供有效的替代环境。

<details>
  <summary>Details</summary>

**Motivation:** 物理感知在大规模动态环境中面临循环信息结构挑战：感知决策决定可观测性，而观察又影响感知决策质量。部分可观测马尔可夫决策过程(POMDP)在现实世界中由于探索成本高和sim-to-real差距未解决而难以处理。

**Method:** AW4RE结合了4D知情证据检索、具有时间一致性的动作条件几何支撑和条件生成补全三个关键技术。当给定查询的感知动作时，模型估计该动作条件下的观察过程。

**Result:** 实验表明，在极端视角偏移、时间间隔和稀疏几何支撑条件下，AW4RE比几何感知的生成基线产生更可靠和一致的预测结果。

**Conclusion:** AW4RE为部分可观测环境中的感知决策提供了有效的传感器原生替代环境，通过4D知情检索和条件生成机制显著改善了预测的可靠性和一致性。
...
> 本文提出了一种序列输入压缩机制，通过在预填充阶段进行结构感知的键值缓存压缩，解决了多模态大语言模型推理过程中的内存瓶颈问题。

<details>
  <summary>Details</summary>

**Motivation:** 随着多模态大语言模型(MLLMs)处理更丰富的视觉表示，大量视觉令牌存储在键值缓存中导致内存消耗成为核心瓶颈，现有方法仅在所有输入处理后进行压缩，导致预填充阶段峰值内存使用过高。

**Method:** 利用MLLMs固有的结构规律性和表征冗余，提出序列输入压缩机制，在预填充过程中执行结构感知的键值缓存压缩，强制执行固定的内存预算。

**Result:** 该方法显著降低了峰值内存使用，同时保持了生成性能，仅有最小程度的性能下降，使多模态推理更加实用和内存高效。

**Conclusion:** 通过在推理全过程中控制内存增长而非事后压缩，该工作为内存高效的多模态大语言模型推理提供了有效解决方案，具有重要实用价值。
...
> 该研究开发了一个集成了H∞鲁棒控制和深度学习的自动化花粉显微镜分析系统，实现了花粉粒的高精度计数、分类和形态分析，相比人工分析获得95.8%的召回率和6倍处理速度提升。

<details>
  <summary>Details</summary>

**Motivation:** 传统的蜜蜂孢粉学分析过程耗时4-6小时且具有主观性，亟需开发高效、客观的自动化分析系统来提升分析效率和准确性。

**Method:** 系统采用H∞鲁棒机械控制与U²-Net显著性目标检测算法结合，使用基于深度度量学习训练的DINOv2视觉Transformer骨干网络进行分类，并整合梯度加权注意力机制提供可解释的特征标注。

**Result:** 系统实现了95.8%的分类召回率和6倍的处理速度提升，能够准确识别和分析智利比奥比奥地区花粉粒的形态学特征。

**Conclusion:** 该自动化系统成功解决了传统花粉分析方法的效率和准确性问题，为蜜蜂孢粉学研究提供了一个高效可靠的解决方案。
...
> 动态3D高斯散布方法存在严重的泛化问题，研究发现过分割策略导致80%的性能差距，而变形场的不相干性是根本原因。

<details>
  <summary>Details</summary>

**Motivation:** 动态3D高斯散布在训练视角上表现优异但在测试视角上泛化能力差，平均训练测试PSNR差距达6.18dB，个别场景甚至达到11dB，亟需找出问题根源并解决。

**Method:** 通过系统消融实验分析自适应密度控制管道各组件的作用，发现分割操作是主要问题；提出弹性能量正则化(EER)约束变形场的光滑性，并辅以GAD和PTDrop等正则器。

**Result:** 禁用分割可将差距从6.18dB降至1.15dB；EER减少99.72%的平均应变，将差距降低40.8%；组合GAD+EER+PTDrop可将差距减少57%；在真实视频上也能将差距降低14.9%。

**Conclusion:** 动态3DGS的过拟合主要源于变形场的不相干性而非参数数量，通过保持变形连贯性可以显著改善泛化性能。
...
> TriTS是一个新颖的跨模态解耦框架，通过将一维时间序列投影到正交的时间、频率和二维视觉空间，实现了高效的长期时间序列预测，在保持线性复杂度的同时超越了现有基于视觉的预测方法。

<details>
  <summary>Details</summary>

**Motivation:** 长期时间序列预测（LTSF）是一个重大挑战，因为真实世界的信号包含高度纠缠的时间动态，从纯粹的一维视角难以完全捕捉。现有的表示方法存在表征瓶颈，需要更有效的方式来解耦和建模复杂的时间序列模式。

**Method:** 提出TriTS框架，包含三个关键组件：1）采用周期感知重塑策略和Visual Mamba（Vim）将1D时间序列转换为2D视觉空间，以线性复杂度建模跨周期依赖；2）设计多分辨率小波混合（MR-WM）模块在频率域显式解耦非平稳信号；3）保留时域流式线性分支确保数值稳定性。通过动态融合这三种互补表示来适应不同的数据上下文。

**Result:** 在多个基准数据集上的广泛实验表明，TriTS实现了最先进（SOTA）的性能，相比现有的基于视觉的预测方法，显著减少了参数数量和推理延迟。

**Conclusion:** TriTS通过创新的跨模态解耦方法成功突破了长期时间序列预测的表示瓶颈，证明了将时间序列映射到多模态空间的有效性，为时间序列预测领域提供了一个高效且性能卓越的新范式。
...
> 本文提出了一种基于冻结基础模型的射箭靶面箭孔自动检测系统，仅用48张标注图像就实现了亚毫米级定位精度。

<details>
  <summary>Details</summary>

**Motivation:** 针对室内射箭靶面箭孔检测任务中训练数据稀缺的问题，旨在开发一个在少量标注数据下仍能保持高精度检测和定位能力的实用系统。

**Method:** 系统包含三个模块：(1)基于颜色的规范校正，将透视畸变图像映射到标准坐标系；(2)冻结的自监督视觉Transformer(DINOv3)配合AnyUp特征上采样恢复空间精度；(3)轻量级CenterNet检测头预测箭孔中心热图。仅3.8M参数可训练。

**Result:** 在三折交叉验证中实现0.893±0.011的F1分数和1.41±0.06毫米的定位误差。下游任务中平均箭得分的误差中位数为1.8%，群体质心定位误差中位数为4.00毫米。

**Conclusion:** 冻结的基础模型配合最小化任务特定适配为小数据密集预测任务提供了实用范式，引导特征上采样已能解决patch标记化导致的空间精度损失。
...
> FairNVT是一个轻量级的去偏框架，通过学习任务相关和敏感信息嵌入，应用校准高斯噪声来同时改善表示层面和预测层面的公平性，同时保持高任务准确性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的去偏方法通常分别处理表示层面和预测层面的公平性问题，但作者认为这两个层面本质上是相互关联的：在表示层面抑制敏感信息有助于实现更公平的预测。

**Method:** FairNVT通过轻量级适配器分别学习任务相关和敏感信息的嵌入，对敏感嵌入应用校准高斯噪声，再与任务表示融合；结合正交性约束和公平性正则化等组件，共同减少学习嵌入中的敏感属性泄漏。

**Result:** 在跨越视觉和语言的三个数据集上，FairNVT成功降低了敏感属性攻击者的准确率，改善了人口统计均等和机会均等指标，同时保持了高任务性能。

**Conclusion:** FairNVT证明了通过统一的框架同时解决表示层面和预测层面的公平性是可行的，为预训练Transformer编码器的公平性部署提供了有效且兼容性强的解决方案。
...
> EdgeVTP是一种专为路侧边缘设备设计的端到端实时车辆轨迹预测模型，通过轻量级transformer架构和曲线参数编码实现了低延迟高精度的预测。

<details>
  <summary>Details</summary>

**Motivation:** 高速公路感知系统中的车辆轨迹预测需要在路侧边缘设备上部署，这要求有界且确定性的端到端延迟，但现有方法难以满足这一实时性要求。

**Method:** EdgeVTP采用嵌入式优先设计，融合了交互感知图建模、轻量级transformer骨干网络和一次曲线解码器。通过将未来运动预测为紧凑的曲线参数（锚定在最后观测位置）而非时间步长的自回归航点，并用硬邻居上限的局部性图限制交互复杂度来保证运行时可预测性。

**Result:** 在三个高速公路基准数据集和两个Jetson级平台上的实验表明，EdgeVTP在包含图构建和后处理的完整协议下实现了最低的端到端测量延迟，其中两个数据集达到state-of-the-art预测精度，其他基准数据集也表现出竞争性的误差。

**Conclusion:** EdgeVTP成功验证了在边缘设备上实现实时车辆轨迹预测的可行性，通过创新的曲线预测方法和交互复杂度控制，在保证预测精度的同时满足了严格的实时部署需求。
...
> HyMOR是一个混合多粒度开放目标识别框架，结合MLLM和CLIP模型的优势，在保持2.5%通用识别提升的同时，将细粒度识别差距缩小至0.2%。

<details>
  <summary>Details</summary>

**Motivation:** MLLM擅长开放式 coarse-grained 识别但难以处理细粒度任务，而CLIP模型精于细粒度识别但缺乏通用性，现有模型无法同时满足多层次语义粒度的精确识别需求。

**Method:** 提出HyMOR框架：MLLM负责开放式的粗粒度目标识别，CLIP模型专门处理动植物等领域特定对象的细粒度识别，两者协同工作实现多语义粒度的准确目标理解。

**Result:** 在引入的TBO数据集（20,942张图像，8,816个类别）上的实验表明，HyMOR相比基线MLLM在通用目标识别上提升2.5%，与CLIP的细粒度识别差距仅0.2%，总体平均SBert相似度提升23.2%。

**Conclusion:** HyMOR通过混合架构成功整合了MLLM和CLIP的优势，为多模态游戏内容生成和交互式学习应用提供了强大的感知基础，实现了跨语义粒度的准确目标识别。
...
> 本文提出CDCRec，一种多模态射电干涉数据重建方法，通过显式建模跨域一致性来改善天文成像质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有的射电天文图像重建方法仅考虑单一模态（图像域或可见度域），导致丢失域间互补信息，无法充分建模相互依赖性和一致性。

**Method:** 设计了一个分层多任务多阶段框架，显式建模可见度域和图像域之间的跨域一致性，采用自监督互补建模策略来增强域间相关性提取。

**Result:** 实验结果表明CDCRec通过增强跨域相关性提取提升了成像性能，在干涉域转换任务上优于现有方法。

**Conclusion:** CDCRec成功解决了单一模态重建的局限性，为射电天文数据重建提供了更全面的多模态解决方案。
...
> 本文提出了一种交替双域后验采样(ADDPS)方法，用于生成式语义通信，通过交替隐域和图像域一致性来保持数据分布，从而实现最优的感知质量。

<details>
  <summary>Details</summary>

**Motivation:** 现有生成式语义通信接收器采用最大后验估计，无法保持数据分布，限制了感知质量。扩散方法的单域指导存在缺陷：隐域指导对信道噪声敏感，图像域指导存在解码器偏差，简单结合会导致过于自信的伪后验。

**Method:** 将语义解码建模为贝叶斯逆问题，证明后验采样可通过保持数据分布实现最优感知质量。提出ADDPS方法，在扩散采样过程中交替执行隐域和图像域一致性约束，将联合后验采样分解为简单子问题。

**Result:** 在FFHQ数据集上的实验表明，ADDPS相比现有方法实现了更优越的感知质量，有效避免了梯度冲突并保留了两个域的互补优势。

**Conclusion:** 后验采样是语义通信中实现最优感知质量的理论保证，ADDPS通过交替双域策略成功实现了这一目标，为生成式语义通信接收器设计提供了新的理论框架和实用方案。
...
> 本文提出了一种基于频域解耦隐式神经表示(FDINR)的近红外辅助低光图像恢复方法，通过分别利用RGB低频信号和NIR高频信号的优势，实现了更好的低光图像恢复效果。

<details>
  <summary>Details</summary>

**Motivation:** 解决低光条件下可见光图像存在的严重噪声和高频结构退化问题，传统空间域融合方法容易产生色彩失真和伪影。

**Method:** 基于RGB-NIR跨模态频率相关的统计先验，通过多尺度小波变换将图像显式分解为不同频率成分，构建双分支隐式神经网络框架。设计跨模态差异化频率监督机制，用低光RGB指导低频亮度和色彩重建，用高信噪比NIR约束高频纹理细节生成，并引入基于不确定性的自适应加权损失函数。

**Result:** 实验结果表明，FD-INR不仅能有效恢复图像亮度一致性和结构细节，而且得益于隐式连续表示的优势，在任意分辨率重建任务上优于现有方法，显著提升了低光感知的可靠性。

**Conclusion:** 该方法通过频域解耦和差异化 supervision 充分利用了RGB和NIR在不同频段的互补优势，结合自适应加权机制，有效解决了传统方法的色彩失真和伪影问题，为低光图像恢复提供了新的解决思路。
...
> 本文提出一种通道注意力引导的跨模态知识蒸馏方法，用于解决指代图像分割中轻量化部署问题，在不增加推理参数的情况下显著提升学生模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的指代图像分割方法虽然性能较好，但依赖大规模视觉语言编码模型，巨大的参数规模严重限制了在计算资源有限场景下的部署应用。

**Method:** 采用通道注意力引导的跨模态知识蒸馏方法，将教师网络学习的视觉-语言高阶细粒度相关性以及各通道表示的语义成分相关性转移到学生网络中，相比传统像素级关系蒸馏能够保留学生网络的独立学习能力。

**Result:** 在两个公共数据集上的实验结果表明，所提出的蒸馏方法在推理过程中不引入额外参数，能够为学生模型带来显著的性能提升。

**Conclusion:** 该方法有效解决了指代图像分割中轻量化部署的挑战，通过通道注意力引导的知识蒸馏既传递了教师网络的知识，又减轻了学习偏差的转移，为资源受限场景下的实际应用提供了可行方案。
...
> BioLip通过检测伪造视频中嘴唇运动的时间抖动特征（time lip jitter）来实现跨语言的唇同步深度伪造检测。

<details>
  <summary>Details</summary>

**Motivation:** 现有的唇同步深度伪造检测器依赖像素级伪影或视听对应关系，但这些线索编码的是数据依赖模式而非普遍物理规律，导致跨语言泛化能力差。

**Method:** 提出基于生物力学约束的检测原理：生成模型不遵守真实的唇部发音生物力学约束，产生可测量的时间唇抖动。通过BioLip框架使用MediaPipe提取的64个口周地标坐标进行轻量化检测。

**Result:** 时间唇抖动信号在不同说话者的语言、种族和录制条件下都保持一致性，为深度伪造检测提供了跨语言的通用特征。

**Conclusion:** 基于生物力学约束的时间唇抖动是检测唇同步深度伪造的更根本原理，比现有方法具有更好的跨语言泛化能力。
...
> 提出了GCN-HViT模型，结合了分层视觉Transformer和图卷积网络，通过全局建模和局部特征提取来解决ViT在图像分类中的局限性。

<details>
  <summary>Details</summary>

**Motivation:** Vision Transformer在图像分类中存在三个关键挑战：(1)块尺寸选择对预测准确性至关重要；(2)1D位置嵌入无法准确捕获空间结构信息；(3)GCN缺乏全局图结构信息捕获能力，而ViT无法建模图像的局部结构。

**Method:** 提出分层视觉Transformer增强的图卷积网络(GCN-HViT)：分层ViT在全局范围内建模块间信息交互，并通过多层级建模小块与大块之间的层次关系；GCN作为局部特征提取器获取每个图像块的局部表示，作为2D空间嵌入，同时在每个层级内局部尺度建模块间信息交互。

**Result:** 在3个真实世界数据集上的大量实验表明，GCN-HViT达到了最先进的性能。

**Conclusion:** 通过将ViT的全局建模能力与GCN的局部结构捕获能力相结合，有效解决了ViT的局限性，在图像分类任务中取得了SOTA性能。
...
> 本文提出了一种在双曲洛伦兹模型中的语义分割框架，解决了庞加莱球模型的数值不稳定性问题，并能同时提供不确定性量化和多项附加功能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的双曲空间语义分割方法主要依赖于庞加莱球模型，存在数值不稳定、优化困难和计算复杂等问题。需要一种更稳定、高效且能与现有欧几里得架构兼容的双曲表示方法。

**Method:** 在洛伦兹模型中实现语义分割框架（像素级和掩码分类），使用文本嵌入结合语义和视觉线索来指导洛伦兹空间中的分层像素表示，无需黎曼优化器即可稳定优化，并可轻松集成现有架构。

**Result:** 在ADE20K、COCO-Stuff-164k、Pascal-VOC和Cityscapes数据集上的广泛实验验证了方法的有效性和通用性，实现了不确定性估计、置信度图、边界勾勒、分层检索、基于文本的检索和零样本性能等功能。

**Conclusion:** 洛伦兹嵌入为鲁棒且感知不确定性的语义分割提供了有效解决方案，具有稳定优化、易于集成和多项衍生功能的优势，展现了双曲洛伦兹模型在计算机视觉任务中的应用潜力。
...
> EFDiff利用地球基础模型指导扩散过程，实现极端空间尺度下的地表温度超分辨率重建。

<details>
  <summary>Details</summary>

**Motivation:** 地表温度超分辨率对环境监测至关重要，但粗尺度热观测难以确定精细结构，尤其是在极端空间退化条件下。

**Method:** 提出EFDiff框架，使用Prithvi-EO-2.0地球基础模型编码高分辨率多光谱反射率为地理空间嵌入，通过交叉注意力注入去噪网络指导重建，提供EFDiff-$ε$和EFDiff-$x_0$两种变体。

**Result:** 在32倍极端尺度差距的全球多样化基准数据集上，EFDiff始终优于基线方法，EFM交叉注意力条件比HLS通道拼接更有效。

**Conclusion:** EFDiff框架不仅适用于LST超分辨率，还能广泛应用于其他可利用预训练地理空间表示指导生成重建的遥感问题。
...
> 论文提出了TowerDataset数据集和全局-局部融合框架，解决输电走廊点云细粒度语义分割中的数据稀缺和结构建模问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有输电走廊点云分割研究面临两大挑战：真实数据稀缺，现有数据集类别粗糙、场景短小，无法体现长距离结构依赖、长尾分布和安全关键部件的细微差异；现有方法难以有效建模全局走廊结构和局部几何细节，在真实检查场景下评估困难。

**Method:** 提出TowerDataset基准数据集（661个真实场景，24.66亿点，22类细粒度分类）和全局-局部融合框架。框架包含：1）全局分支采用NoCrop训练和原型对比学习捕获长距离拓扑；2）局部逐块分支保留精细几何结构；3）通过几何验证融合并细化预测结果。

**Result:** 在TowerDataset和两个公开基准上验证，证明了所提数据集的挑战性和框架在真实复杂异构输电走廊场景中的鲁棒性。数据集将在HuggingFace上公开发布。

**Conclusion:** 该工作填补了输电走廊细粒度语义分割领域缺乏大规模真实数据集的空白，提出的全局-局部融合框架能有效整合互补的全局和局部信息，为智能电力线检查提供了重要的基础工具和方法。
...
> 本文提出CATP框架，通过分层token剪枝策略降低基于Transformer的伪装目标检测模型的计算开销，同时保持高精度。

<details>
  <summary>Details</summary>

**Motivation:** 基于Transformer的伪装目标检测方法虽然达到了最先进的精度，但其巨大的计算开销严重限制了实际部署。需要开发高效轻量的解决方案。

**Method:** 提出分层置信感知token剪枝框架(CATP)，该框架分层识别并丢弃背景和物体内部的易区分token，专注于关键的边界token；引入双路径特征补偿机制，将剪枝token的上下文知识聚合到丰富特征中。

**Result:** 在多个COD基准测试上的广泛实验表明，该方法在保持高精度的同时显著降低了计算复杂度，为实际应用场景中的高效部署提供了有前景的研究方向。

**Conclusion:** CATP框架成功地在精度和效率之间取得了平衡，为伪装目标检测模型的实际部署提供了一个有效的解决方案，具有重要的实用价值。
...
> 本文提出COD-TDQ方法，解决Transformer-based伪装目标检测在4位量化中的性能下降问题，通过令牌分组双重约束量化技术实现零样本下的高效量化。

<details>
  <summary>Details</summary>

**Motivation:** 伪装目标检测需要处理微妙的纹理和边界线索，且常在内存和延迟受限的设备上运行，亟需低位量化。但COD任务难以量化，存在背景令牌主导激活范围、边界信号被置零等问题。

**Method:** COD-TDQ包含两个耦合步骤：1) Direct-Sum Token-Group(DSTG)分配令牌组尺度抑制跨令牌范围主导；2) Dual-Constraint Range Projection(DCRP)投影令牌组裁剪范围，保持阶距-离散度比和零区间质量有界。

**Result:** 在四个COD基准和两个基线模型上，COD-TDQ在无需重新训练的情况下，比最先进量化方法的Sα分数提升超过0.12。

**Conclusion:** COD-TDQ有效解决了伪装目标检测中的令牌局部瓶颈问题，为Transformer-based COD模型的高效部署提供了新的量化方案。
...
> Q-DeepSight是一个模拟人类视觉评估过程的多模态图像质量评估框架，通过交错的思维链和工具增强的证据获取，提供可定位的质量反馈，并成功指导图像增强迭代优化。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大语言模型IQA方法采用单一语言范式，缺乏人类式的证据寻求判断过程，产生的理由依据不足，限制了其在迭代优化中的可靠性。

**Method:** 提出think-with-image框架，执行多模态交错思维链(iMCoT)和工具增强证据获取(如裁剪缩放)，明确识别质量退化位置和原因。引入感知课程奖励(PCR)解决奖励稀疏性，证据梯度过滤(EGF)改善视觉推理的信用分配。

**Result:** 在自然、修复和AI生成内容的多样化基准测试中达到最先进性能，并通过Perceptual-in-Generation框架实现了评估与优化的闭环。

**Conclusion:** Q-DeepSight成功模拟了人类视觉质量评估过程，不仅能提供准确评分，还能给出可操作的局部化反馈，为生成模型和图像修复提供了可靠的指导。
...
> I2P框架通过自适应识别关键表示层并在低敏感性参数子空间内约束更新，提升了视觉基础模型在合成图像检测中的跨分布泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于视觉基础模型的合成图像检测方法在适应策略上较为粗糙，缺乏对可转移伪造线索最优表示层次的显式建模，且直接微调可能破坏支持开放集泛化的跨模态预训练结构。

**Method:** 提出I2P框架，将VFM适应问题重构为联合优化问题：首先自适应识别对SID最具判别性的关键层表示，然后在低敏感性参数子空间内约束任务驱动的参数更新。

**Result:** 该方法能够在提高任务特异性的同时，尽可能保留预训练表示的可转移结构，从而提升模型的跨分布泛化性能。

**Conclusion:** 通过内在重要性感知机制，I2P有效解决了任务特定泛化与预训练结构保持之间的矛盾，为合成图像检测提供了新的技术路径。
...
> BiasCareVL是一个偏置感知的多模态学习框架，将偏差控制直接融入模型设计，在15种成像模态的344万样本上训练，在8个医学基准测试中超越20种现有方法，显著提升模型在临床任务中的公平性和可靠性。

<details>
  <summary>Details</summary>

**Motivation:** 医学影像和临床文本的整合催生了通用AI医疗系统，但普遍存在的偏差（如疾病患病率不平衡、解剖区域分布偏斜、成像协议异质性和人口统计差异）严重影响了视觉语言系统在真实临床环境中的公平性和可靠性。

**Method:** 提出BiasCareVL偏置感知多模态学习框架，通过自适应不确定性建模和可选的人机交互精修来控制主导数据模式的影响，在统一的表示空间内支持视觉问答、疾病分类、分割和报告生成等多样化临床任务。

**Result:** 在8个涵盖皮肤病学、肿瘤学、放射学和病理学的公共基准测试中，BiasCareVL超越20个最先进方法，在多类皮肤病变诊断中准确率提升超10%，小肿瘤分割中Dice系数提升超20%，并在与认证放射科医师评估时表现出超越人类水平的诊断性能。

**Conclusion:** 通过开源BiasCareVL，旨在推动AI在医疗领域的透明、可重复和公平发展，为建立通用、可信且临床可靠的AI系统铺平道路。
...
> CrossFlowDG 是一种新颖的域适应框架，通过跨模态流匹配技术消除图像和文本嵌入之间的模态间隙，在多个基准测试中实现竞争性性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态域泛化方法基于余弦相似度的对比对齐仍然存在模态间隙问题，图像和文本 embeddings 在几何上仍然分离，阻碍了模型真正学习域不变特征。

**Method:** 提出 CrossFlowDG 框架，使用无噪声的跨模态流匹配技术，在联合欧几里得潜在空间中学习连续变换，将域偏置的图像 embeddings 显式地传输到正确类别的域不变文本 embeddings。结合高效的 VMamba 图像编码器和 CLIP 文本编码器。

**Result:** 在四个常见的域泛化基准测试中取得竞争性性能，在 TerraIncognita 数据集上达到最先进水平。

**Conclusion:** 通过显式的跨模态传输机制成功解决了多模态域泛化中的模态间隙问题，为实现更好的域不变特征学习提供了有效途径，证明了跨模态流匹配在域泛化任务中的实用价值。
...
> EasyVideoR1是首个专门针对视频理解任务设计的高效强化学习训练框架，通过离线预处理、任务感知奖励系统和混合训练范式等技术，显著提升了大视觉语言模型在视频理解上的能力。

<details>
  <summary>Details</summary>

**Motivation:** 虽然基于可验证奖励的强化学习(RLVR)在提升大语言模型推理能力方面效果显著，但将其扩展到视频理解领域仍面临挑战：视频任务类型多样、高维视觉输入的重复解码计算开销大、以及众多敏感超参数下的可重现评估困难。现有的开源强化学习框架主要针对文本和图像场景，缺乏针对视频模态的系统性优化。

**Method:** EasyVideoR1框架提出了：(1)包含离线预处理和张量缓存的完整视频RL训练流水线，消除冗余视频解码；(2)涵盖11种不同视频和图像问题类型的综合任务感知奖励系统；(3)结合高质量离线轨迹和在线策略探索的混合离线-在线训练范式；(4)支持独立配置像素预算的联合图像-视频训练；(5)覆盖22个主流视频理解基准的异步多基准评估框架。

**Result:** 该框架实现了1.47倍的吞吐量提升，在22个主流视频理解基准上的重现准确率与官方报告分数高度一致，证明了其有效性和可靠性。

**Conclusion:** EasyVideoR1填补了视频强化学习训练框架的空白，为训练大视觉语言模型在视频理解任务上提供了完整高效的解决方案，推动了多模态模型在视频领域的发展。
...
> 提出了一种名为物理信息跟踪(PIT)的视频单粒子追踪框架，通过神经网络自动编码器定位粒子并嵌入可微物理模块约束轨迹满足已知动力学，开发了物理信息地标损失(PILL)及其监督变体(PILLS)。

<details>
  <summary>Details</summary>

**Motivation:** 为了实现从视频中准确追踪单个粒子，研究者希望结合物理动力学先验知识来提高追踪的准确性和鲁棒性，特别是在有噪声或缺乏标注数据的情况下。

**Method:** 采用自动编码器架构，在瓶颈层分离追踪相关的地标热图和背景噪声；嵌入可微物理模块约束轨迹满足已知动力学；设计无监督的PILL损失通过物理一致性约束，以及监督的PILLS损失通过与仿真真值比较进行端到端训练。

**Result:** 通过复制26因子设计实验验证，PILLS在清洁和噪声条件下都能在双线性和物理优化解码器输出上实现亚像素级追踪精度，表现一致且可靠。

**Conclusion:** 物理信息跟踪框架成功地将物理动力学先验知识与深度学习相结合，为单粒子追踪提供了一种既可在有监督也可在无监督条件下工作的有效解决方案，在各种噪声环境下都能保持高精度。
...
> 本文提出了一种分组异构图神经网络(GW-HGNN)用于低空高斯泼洒(LAGS)的资源分配，通过建模不同图像组对重建过程的非均匀贡献，在保证重建质量的同时显著降低计算延迟。

<details>
  <summary>Details</summary>

**Motivation:** 现有的低空资源分配方案效率低下，因为它们没有考虑不同视角引入的图像多样性，导致在追求最大重建质量而非通信吞吐量时表现不佳。

**Method:** 提出GW-HGNN模型，显式建模不同图像组对重建过程的非均匀贡献，将LAGS损失和通信约束转化为双层消息传递的图学习成本，实现数据保真度与传输成本的自动平衡。

**Result:** 在真实LAGS数据集上，GW-HGNN在PSNR、SSIM和LPIPS等关键渲染指标上显著优于现有基准，计算延迟相比MOSEK求解器降低约100倍，实现毫秒级推理。

**Conclusion:** GW-HGNN成功解决了LAGS系统中图像多样性带来的资源分配挑战，实现了高质量的实时3D场景重建，适合实际部署应用。
...
> USTri是一个三阶段超声智能分析流程，通过通用基础模型、领域专家模型和工作流代理，实现跨器官、多任务、支持医生工作流的统一超声分析。

<details>
  <summary>Details</summary>

**Motivation:** 临床超声分析需要能够泛化到不同器官、视图和设备的模型，同时支持可解释的工作流级分析。现有方法依赖任务特定适应，联合学习因跨任务干扰而不稳定，难以在实际中提供工作流级输出。

**Method:** USTri采用三阶段流程：第一阶段训练通用专家USGen学习跨领域可迁移先验；第二阶段在USGen冻结基础上微调任务特定头部构建领域专家USpec；第三阶段通过USAgent协调各专家进行多步推理并生成结构化报告。

**Result:** 在FMC_UIA验证集上，模型在4种任务类型和27个数据集上达到最佳总体性能，超越现有方法；USAgent能生成高准确度和可解释性的临床结构化报告。

**Conclusion:** 研究为超声智能提供了一条可扩展路径，能够在异质超声任务间泛化并支持一致的端到端临床工作流程。
...
> KIRA是一个五阶段统一的视觉检索增强生成框架，专门解决专业领域视觉问答中的十个核心问题，在测试中达到0.97检索精度和1.0基础得分。

<details>
  <summary>Details</summary>

**Motivation:** 现有的检索增强生成(RAG)主要针对文本领域，在视觉领域面临四大挑战：图像查询与文本知识库间的模态鸿沟、构建语义有意义的视觉知识库、跨检索图像的多跳推理，以及确保生成答案忠实于视觉证据。

**Method:** KIRA框架包含五个阶段：1) 基于DINO区域检测的层次语义分块构建多粒度知识库；2) 带少样本自适应的领域自适应对比编码器处理罕见视觉概念；3) 双路径跨模态检索与思维链查询扩展；4) 支持时间和多视图的思维链检索进行多跳视觉推理；5) 证据条件化的基础生成与事后幻觉验证。

**Result:** 在四个专业领域(X光医学、电路图、卫星图像、组织病理学)的测试中，KIRA平均达到0.97检索精度、1.0基础得分和0.707领域正确性。六变种消融实验揭示了各组件的作用时机和精度多样性权衡。

**Conclusion:** KIRA成功解决了视觉RAG的关键挑战，为专业领域视觉问答提供了统一解决方案。提出的DOMAINVQAR评测基准从检索精度、推理忠实性和领域正确性三个维度评估，超越了传统召回指标，为该领域建立了更全面的评估标准。
...
> 本文提出了一种统一的残差噪声学习框架，通过直接从低剂量PET图像中估计噪声而非预测全剂量图像，有效解决了跨剂量PET去噪中模型的泛化问题。

<details>
  <summary>Details</summary>

**Motivation:** 单一噪声水平训练的神经网络模型难以泛化到其他剂量条件，而传统的'一刀切'模型会学习跨噪声水平的平均表示，导致性能下降。因此需要解决模型在不同剂量条件下的泛化问题。

**Method:** 提出统一的残差噪声学习框架，直接从低剂量PET图像估计噪声，而不是预测全剂量图像，避免了传统方法隐式优化异构噪声分布期望的问题。

**Result:** 在两个医疗中心的大规模多剂量PET数据集上的实验表明，所提方法优于'一刀切'模型、单一剂量U-Net模型和剂量条件化方法，取得了更好的去噪性能。

**Conclusion:** 残差噪声学习有效缓解了平均效应，增强了跨剂量PET去噪的泛化能力，为低剂量PET成像提供了更好的解决方案。
...
> 本文提出了CoGR-MoE框架，通过答案选项的语义指导专家路由选择，并利用对比学习优化选项级表示，在多个VQA任务中取得了优异性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有MoE方法在VQA任务中存在路由不稳定问题：过于不稳定会导致同一问题类型的专家选择不一致，而过于稳定则降低灵活性，影响选项推理效果。

**Method:** 提出概念引导路由框架CoGR-MoE，在训练阶段利用答案选项语义指导专家选择，然后用选项特征重新加权选定专家，生成每个候选选项的判别性表示，最后通过对比学习优化选项比较。

**Result:** 实验结果表明CoGR-MoE在多个VQA任务上表现出色，验证了该方法的有效性。

**Conclusion:** 通过语义引导的专家路由和对比学习优化，成功解决了MoE方法的路由不稳定问题，提升了VQA任务中选项推理的性能。
...
> 提出了一种自适应感受野空间-频率特征重建网络(ARF-SFR-Net)，用于解决少样本细粒度图像分类中感受野大小选择的关键挑战

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于特征的少样本细粒图像分类方法面临如何为不同类别输入图像选择合适感受野大小以提取空间和频率特征描述子的主要挑战

**Method:** 设计ARF-SFR-Net网络，能够自适应确定感受野大小来获取空间和频率特征，并有效融合这些特征进行重建和分类任务，可嵌入到episode训练机制中进行端到端训练

**Result:** 在多个少样本细粒图像分类基准数据集上的大量实验证明了ARF-SFR-Net相对于现有先进方法的有效性和优越性

**Conclusion:** 提出的ARF-SFR-Net成功解决了感受野自适应选择的问题，为少样本细粒图像分类任务提供了新的有效解决方案，代码已开源
...
> CoDe-MAE是一种高分辨率光学与SAR图像的预训练方法，采用'少对齐、更协同'的哲学，通过知识蒸馏、条件对比学习和跨模态退化重建技术解决了异构-分辨率悖论。

<details>
  <summary>Details</summary>

**Motivation:** 高分辨率光学与SAR图像的异构性随着分辨率提升而急剧放大，传统对齐方法会导致特征抑制或污染，造成表示退化；需要新的方法实现真正的模态协同而非强制对齐。

**Method:** 提出三重机制：1)光学锚定知识蒸馏(OKD)将SAR噪声映射到语义流形；2)条件对比学习(CCL)对齐共享共识同时保留不同物理特征；3)跨模态退化重建(CDR)剥离非同源伪特征。

**Result:** 在100万样本上预训练后，CoDe-MAE阻止了表示退化，在单模态和双模态下游任务上达到新的SOTA性能，显著优于超大规模数据集训练的基础模型。

**Conclusion:** 证明了'少对齐、更协同'理念的有效性，为解决极端异构模态学习挑战提供了新范式，实现了卓越的数据效率和跨模态表征质量。
...
> TSM-Pose是一个新颖的类别级物体姿态估计框架，通过拓扑感知学习和语义Mamba模块实现了对未见实例的鲁棒泛化性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的类别级物体姿态估计方法主要依赖简单的特征提取和聚合，难以捕捉类别共享的拓扑结构，且语义关键点建模能力有限，限制了模型在未见实例上的泛化性能。

**Method:** 提出TSM-Pose框架，包含：1) 拓扑提取器捕获点云的全局拓扑表示并整合到局部几何特征中；2) 基于Mamba的全局语义聚合器，将语义先验注入关键点，使用多个TwinMamba块建模长程依赖关系以实现更有效的全局特征聚合。

**Result:** 在REAL275、CAMERA25和HouseCat6D三个基准数据集上的广泛实验表明，TSM-Pose的性能超越了现有的最先进方法。

**Conclusion:** 通过结合拓扑感知学习和语义Mamba架构，TSM-Pose有效解决了现有方法在结构表示和语义建模方面的局限性，为类别级物体姿态估计提供了新的解决思路。
...
> 本研究通过对比五种条件配置发现，在视网膜疾病进展预测中，简单确定性回归模型优于复杂生成模型，并据此开发了TRU模型。

<details>
  <summary>Details</summary>

**Motivation:** 目前进行性黄斑疾病的临床决策依赖于定性比较或标量进展评分，缺乏对未来视网膜外观的定量预测方法，而现有方法的生成复杂度需求尚不明确。

**Method:** 通过对比五种条件配置（标准条件扩散、推理对齐随机训练、确定性回归等），结合任务熵和后验集中度分析，开发了具有连续时间差条件和多尺度历史聚合的确定性TRU模型。

**Result:** TRU模型在28,902只眼的三个成像平台上均达到或超过现有最先进模型的delta-SSIM、SSIM和PSNR指标，且优势随历史长度增加而增长。

**Conclusion:** 对于缓慢进展的视网膜疾病，确定性的直接回归方法比复杂的生成模型更适合，TRU模型实现了跨疾病和跨平台的优异预测性能。
...
> 提出一个自推理代理框架，用于生成叙事性产品网格拼贴画，通过统一的生成和迭代反思机制实现视觉一致性和叙事连贯性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的图像生成方法不支持结构化叙事规划和跨面板协调，导致叙事能力薄弱和视觉不连贯，无法满足现代营销中对叙事驱动产品摄影的需求。

**Method:** 构建产品叙事框架来明确产品身份、使用场景和环境，将其转换为具有统一视觉风格的互补网格；通过约束感知提示生成统一拼贴图像；采用评估门控机制和失败归因进行迭代改进。

**Result:** 实验证明该框架在美学质量、叙事丰富度和视觉连贯性方面均优于直接提示的基线方法。

**Conclusion:** 通过自推理代理框架和渐进式反思机制，能够有效生成高质量的叙事性产品网格拼贴，满足营销需求并提升用户体验。
...
> 该论文提出了基于深度非负矩阵分解的层次化光谱解混方法BLUTH，通过层级丰度和约束和稀疏调制技术解决了光谱变异性、端元数量确定和端元清晰度三个关键问题，在实验室和遥感场景中表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 传统的光谱解混方法面临三个主要挑战：光谱变异性影响解混性能、确定端元数量的方法不明确、随着端元数量增加其清晰度下降。这些问题限制了光谱解混在 hyperspectral 图像分析中的应用效果。

**Method:** 作者提出了层次化解混框架，在深度非负矩阵分解上施加层级丰度约束，设计了二元线性解混触觉层次结构(BLUTHs)，并采用稀疏调制解混增长技术来定制每个场景的拓扑结构，从而揭示具有不同光谱对比度的端元。

**Result:** BLUTHs在实验室场景中的性能超越了最先进的解混算法，特别是在丰度估计方面；在遥感场景中保持竞争力；并成功应用于HYPSO和PACE卫星的海洋色彩解混任务。

**Conclusion:** 层次化结构为解决光谱解混的三个核心问题提供了有效方案，BLUTHs方法不仅在理论上具有创新性，在实际应用中也展现出优异性能，为hyperspectral图像分析提供了新的技术路径。
...
> 提出一种无监督几何距离(UGD)指标，仅通过噪声点云即可评估点云去噪方法，无需真实干净点云。该方法通过学习干净点云的块状先验模型作为评估基准。

<details>
  <summary>Details</summary>

**Motivation:** 现有的点云去噪评估方法需要真实干净点云作为监督，但在真实应用场景中，干净的基准点云往往无法获取，这限制了去噪方法的实际应用。

**Method:** 首先通过块状特征提取网络从干净点云学习原始高斯混合模型(GMM)作为基准；然后定义UGD为去噪点云每个块与学习到的GMM模型在块空间中距离的加权和；特征提取网络通过多任务学习（包括成对质量排序、失真分类和失真分布预测）进行自监督训练。

**Result:** 在合成噪声上的实验表明UGD性能达到了与监督全参考指标相当的水平；在真实数据上的实验证明了UGD能够仅基于噪声点云实现对点云去噪方法的无监督评估。

**Conclusion:** 所提出的UGD指标有效解决了真实场景下点云去噪评估依赖干净基准的问题，为实际应用中的去噪方法评估提供了可行的解决方案。
...
> DOSE利用现成的预训练模型（无需特定任务训练）来过滤多模态数据，通过评估文本质量和图文对齐，构建质量-对齐联合分布进行自适应加权采样，实现了高效的数据筛选。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言模型训练数据集存在噪声、冗余和图文对齐质量差的问题，而传统的数据过滤方法需要在目标数据上额外训练过滤模型，增加了计算成本。

**Method:** 直接使用未经微调的预训练模型评估数据质量，结合文本质量评分和图文对齐评分构建联合分布，采用自适应加权采样策略选择高质量样本，同时保持长尾多样性。

**Result:** 在DOSE过滤数据上训练的模型在标准VQA和数学基准测试中能够匹配或超越使用完整数据集训练的模型性能。

**Conclusion:** DOSE证明了无需任务特定训练即可进行有效数据筛选的可行性，为大规模多模态学习提供了高效、可扩展的数据过滤解决方案。
...
> URVIS 2026挑战赛首次关注恶劣天气下的全景分割，基于MUSES多传感器数据集，吸引了17支团队参与，设计了加权全景质量(wPQ)作为评估指标。

<details>
  <summary>Details</summary>

**Motivation:** 促进在恶劣到极端天气条件下鲁棒的全景分割技术发展，推动多模态传感器融合技术在复杂环境中的应用。

**Method:** 基于MUSES数据集构建挑战平台，整合RGB相机、LiDAR、雷达和事件相机数据，设计加权全景质量(wPQ)作为统一的评估标准。

**Result:** 17支注册参与者提交47份方案，4支团队进入决赛阶段，评估了不同方法在恶劣天气条件下的全景分割性能。

**Conclusion:** 挑战赛展示了鲁棒多模态全景分割的最新进展，但该领域仍面临挑战。需继续研究以提高复杂天气环境下的分割精度和鲁棒性。
...
> DVAR是一个免训练的视频真实性检测框架，通过多代理辩论机制和MDL评估实现高度泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 视频生成技术的快速发展对媒体取证构成挑战，传统检测方法在超出训练分布时泛化能力不足，需要更robust的检测方法。

**Method:** 提出DVAR框架，包含生成假设代理和自然机制代理，通过多轮交叉质询进行辩论，使用最小描述长度(MDL)和Okham剃刀原理评估解释成本，集成GenVideoKB知识库提供推理启发。

**Result:** 实验表明DVAR在性能上可与监督SOTA方法竞争，且对未见过的生成架构表现出卓越的泛化能力，并提供透明可解释的推理轨迹。

**Conclusion:** 通过将检测转化为透明辩论过程，DVAR为视频真实性评估提供了显式、可解释的推理路径，实现了鲁棒且可泛化的检测。
...
> 本文提出ICNNM方法，通过引入预学习的卷积特征向量来改进CNNM，避免了复杂的SVD计算，显著提升了计算效率和恢复性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的CNNM方法在处理任意采样的张量补全问题时需要多次执行SVD分解，计算成本高且难以并行化，限制了其实际应用。

**Method:** 从卷积特征向量的角度重新表述CNNM的优化目标，引入在不同张量间共享的预学习卷积特征向量，提出归纳卷积核范数最小化(ICNNM)方法，绕过SVD步骤。

**Result:** ICNNM大幅降低了计算时间，同时由于预学习卷积特征向量提供的额外先验知识，在恢复性能上也优于原始CNNM方法。

**Conclusion:** 在视频补全、预测和帧插值等任务上的大量实验验证了ICNNM相对于CNNM和其他竞争方法的优越性，证明了该方法的有效性和实用性。
...
> TeMuDance是一个无需标注数据集的音乐舞蹈生成框架，通过以动作为中心的多模态对齐方法，实现了基于文本语义的音乐驱动的舞蹈生成控制。

<details>
  <summary>Details</summary>

**Motivation:** 现有音乐驱动舞蹈生成方法缺乏语义可控性，无法通过自然语言描述引导特定动作，主要原因是缺少大规模的音乐-文本-动作联合标注数据集。

**Method:** 提出以动作为中心的桥接范式，利用动作作为共享语义锚点，将分离的音乐-舞蹈和文本-动作数据集对齐到统一嵌入空间；在冻结的音乐到舞蹈扩散主干上训练轻量级文本控制分支；设计基于置信度过滤的双流微调策略抑制检索监督中的噪声。

**Result:** TeMuDance在保持竞争性舞蹈质量的同时，显著提升了文本条件控制能力，超越现有方法。

**Conclusion:** 该框架成功解决了音乐舞蹈生成中的语义控制难题，为自然语言指导的舞蹈创作提供了新的解决方案。
...
> 本文提出了MobileAgeNet，一个轻量级的移动端面部年龄估计模型，在保持14.4ms低延迟的同时达到4.65年的平均绝对误差。

<details>
  <summary>Details</summary>

**Motivation:** 移动端面部年龄估计需要平衡预测准确性、低延迟和紧凑模型尺寸，现有解决方案难以满足这些实际部署需求。

**Method:** 基于预训练的MobileNetV3-Large骨干网络，结合紧凑回归头；采用有界年龄回归和两阶段微调策略；通过NN LEMUR Dataset框架实现可复现的训练评估流程；支持PyTorch→ONNX→TensorFlow Lite的完整部署流水线。

**Result:** 在UTKFace测试集上MAE为4.65年，参数量仅3.23M，平均延迟14.4ms；部署流程在移动设备上保持了预测性能，无显著性能衰减。

**Conclusion:** MobileAgeNet为移动端面部年龄估计提供了一个实用的、可直接部署的基线方案，在准确性和效率之间达到了良好平衡。
...
### [289] [Towards Universal Skeleton-Based Action Recognition](https://arxiv.org/abs/2604.17013)
*Jidong Kuang,Hongsong Wang,Jie Gui*

Main category: cs.CV

> 本文提出了一个通用的异构骨骼动作识别框架，通过构建大规模异构开放词汇(HOV)骨骼数据集和基于Transformer的模型，解决了骨骼数据异质性和开放词汇识别的挑战。

<details>
  <summary>Details</summary>

**Motivation:** 随着机器人技术发展，基于骨骼的动作识别在人机交互中变得十分重要。然而现有工作忽略了骨骼数据的异质性（人类骨骼和人形机器人结构不同），且缺乏开放词汇识别能力。为了解决这些限制，需要研究异构骨骼数据上的开放词汇动作识别问题。

**Method:** 1. 构建了大规模异构开放词汇(HOV)骨骼数据集，整合并精炼了多个代表性骨骼动作数据集；2. 提出了基于Transformer的通用模型，包含三个核心组件：统一骨骼表示、骨骼运动编码器和多粒度运动-文本对齐。运动编码器将多模态骨骼嵌入输入双流Transformer编码器学习时空动作表示，然后映射到语义空间与文本嵌入对齐。多粒度对齐在全局实例、流特定和细粒度三个层面进行对比学习。

**Result:** 在包含异构骨骼数据的热门基准测试上的大量实验证明了所提方法的有效性和泛化能力。代码已在GitHub上公开。

**Conclusion:** 本研究首次系统地解决了异构骨骼数据上的开放词汇动作识别挑战，通过统一表示和跨模态对齐技术，实现了通用的骨骼动作识别模型，为实际人机交互应用提供了重要的技术支撑。
...
### [290] [LIVE: Leveraging Image Manipulation Priors for Instruction-based Video Editing](https://arxiv.org/abs/2604.17021)
*Weicheng Wang,Zhicheng Zhang,Zhongqi Zhang,Juncheng Zhou,Yongjie Zhu,Wenyu Qin,Meng Wang,Pengfei Wan,Jufeng Yang*

Main category: cs.CV

> LIVE是一个联合训练框架，通过利用大规模图像编辑数据来增强视频编辑能力，并提出了帧级token噪声策略来解决图像与视频的域差异问题。

<details>
  <summary>Details</summary>

**Motivation:** 视频编辑领域面临的主要瓶颈是视频数据标注成本高昂，这严重限制了视频编辑数据集的规模、质量和任务多样性。现有的端到端训练方法依赖视频生成模型或人工标注，难以获得足够的数据支持。

**Method:** 提出LIVE联合训练框架，结合大规模图像编辑数据和视频数据；引入帧级token噪声策略，将特定帧的潜在特征作为推理token，利用预训练视频生成模型创建时序变换；清理公开数据集并构建自动化数据管道；采用两阶段训练策略逐步提升视频编辑能力。

**Result:** 构建了包含60多个挑战性任务的综合评估基准，涵盖了图像编辑中普遍存在但在现有视频数据集中稀缺的任务。实验表明该方法达到了最先进的性能表现。

**Conclusion:** 通过有效利用丰富的图像编辑数据和创新的域差异缓解策略，LIVE框架成功解决了视频编辑数据不足的问题，为视频编辑技术的发展提供了新的解决方案。
...
### [291] [CAM3DNet: Comprehensively mining the multi-scale features for 3D Object Detection with Multi-View Cameras](https://arxiv.org/abs/2604.17024)
*Mingxi Pang,Dingheng Wang,Zekun Li,Zhenping Sun,Bo Wang,Zhihang Wang,Zhao-Xu Yang*

Main category: cs.CV

> CAM3DNet是一种新型的稀疏查询式3D目标检测框架，通过复合查询、自适应自注意力和多尺度混合采样三个模块，有效利用动态多尺度信息，在多个基准数据集上超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 基于查询的多视图3D目标检测方法难以有效利用动态多尺度信息，物体特征与查询几何关系学习不充分，直接探索多尺度时空特征计算成本过高。

**Method:** 提出CAM3DNet框架，包含三个核心模块：1) 复合查询(CQ)模块使用多尺度投影策略将2D查询转换为3D空间；2) 自适应自注意力(ASA)模块学习时空多尺度查询间的交互；3) 多尺度混合采样(MSHS)模块通过可变形注意力机制整合多尺度查询、金字塔特征图和2D相机先验知识。

**Result:** 在nuScenes、Waymo和Argoverse基准数据集上广泛实验证明CAM3DNet的有效性，超越了大多数现有的基于相机的3D目标检测方法，并通过消融研究验证了各模块的贡献和计算复杂度。

**Conclusion:** CAM3DNet通过三个创新模块的组合，在保持计算效率的同时显著提升了多尺度特征利用能力，为基于查询的3D目标检测提供了有效的解决方案。
...
### [292] [IMA-MoE: An Interpretable Modality-Aware Mixture-of-Experts Framework for Characterizing the Neurobiological Signatures of Binge Eating Disorder](https://arxiv.org/abs/2604.17028)
*Lin Zhao,Qiaohui Gao,Elizabeth Martin,Kurt P. Schulz,Tom Hildebrandt,Robyn Sysko,Tianming Liu,Xiaobo Li*

Main category: cs.CV

> 本研究提出了一种名为IMA-MoE的新型可解释多模态模型，用于整合神经影像、行为、激素和人口统计学数据，在大型ABCD数据集上显著提升了暴食症的诊断性能，并发现了性别特异性预测模式。

<details>
  <summary>Details</summary>

**Motivation:** 暴食症是最普遍的饮食障碍，但现有诊断主要基于症状标准而非生物学机制，限制了早期检测和生物信息干预的发展。当前神经生物学研究存在假设驱动模型、单模态分析和数据多样性不足等问题，需要先进的数据驱动多模态框架。

**Method:** 提出可解释模态感知混合专家架构(Interpretable Modality-Aware Mixture-of-Experts, IMA-MoE)，将每种测量编码为独立token，通过token重要性机制量化各测量对预测的贡献，实现灵活的跨模态依赖建模，同时保持模态特异性特征。

**Result:** 在大型ABCD数据集上，IMA-MoE在区分暴食症患者与健康对照方面优于基线方法，揭示了性别特异性预测模式——激素测量在女性预测中贡献更显著。

**Conclusion:** 可解释的数据驱动多模态建模在推进暴食症生物学特征表征和促进神经精神疾病精准个性化干预方面具有重要前景。
...
### [293] [Conditional Evidence Reconstruction and Decomposition for Interpretable Multimodal Diagnosis](https://arxiv.org/abs/2604.17030)
*Shaowen Wan,Yanjun Lv,Lu Zhang,Dajiang Zhu,Bharat Biswal,Tianming Liu,Xiaobo Li,Lin Zhao*

Main category: cs.CV

> CERD是一个可解释的多模态诊断框架，能够在模态不完整的情况下重建缺失数据并分解诊断证据，在阿尔茨海默病诊断上表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 神经退行性疾病具有多因素特性，需要多模态建模，但实际临床实践中模态覆盖往往不完整，现有方法难以捕捉个体间的跨模态依赖关系且缺乏可解释性。

**Method:** 提出CERD框架，首先基于个体观测输入条件重建缺失模态表示，然后通过logit-level归因将诊断证据分解为跨模态共享验证和模态特异性线索。

**Result:** 在ADNI数据集上，CERD在模态不完整设置下优于竞争基线，同时产生结构化且与临床一致的证据归因。

**Conclusion:** CERD为临床决策提供了可靠且可解释的诊断支持，有效解决了多模态医学诊断中数据不完整和黑盒问题的关键挑战。
...
### [294] [SIF: Semantically In-Distribution Fingerprints for Large Vision-Language Models](https://arxiv.org/abs/2604.17041)
*Yifei Zhao,Qian Lou,Mengxin Zheng*

Main category: cs.CV

> SIF是一种非侵入式的大视觉语言模型所有权验证框架，通过语义对齐指纹蒸馏和鲁棒指纹优化技术，生成语义一致且难以被检测和移除的指纹，有效防御语义分歧攻击。

<details>
  <summary>Details</summary>

**Motivation:** 现有的LVLM所有权验证方法依赖语义异常查询或分布外响应作为指纹，容易受到语义分歧攻击(SDA)的威胁，攻击者可以通过测量可疑模型与参考模型间的语义分歧来识别和过滤这些指纹。

**Method:** SIF框架包含两个核心技术：1) 语义对齐指纹蒸馏(SAFD)，将文本水印信号转移到视觉模态以产生语义连贯的指纹响应；2) 鲁棒指纹优化(RFO)，通过模拟最坏情况下的表示扰动来增强指纹对模型修改的鲁棒性。

**Result:** 在LLaVA-1.5和Qwen2.5-VL上的实验表明，SIF实现了强大的隐蔽性和鲁棒性，能够有效抵御微调、量化等模型修改攻击，为LVLM版权保护提供了实用解决方案。

**Conclusion:** SIF成功解决了现有指纹方法语义不一致的缺陷，通过非侵入式设计实现了既语义自然又高鲁棒性的所有权验证，为大视觉语言模型的知识产权保护提供了新的技术思路。
...
### [295] [A Real-Time Bike-Pedestrian Safety System with Wide-Angle Perception and Evaluation Testbed for Urban Intersections](https://arxiv.org/abs/2604.17046)
*Mehmet Kerem Turkcan*

Main category: cs.CV

> 该研究开发了一个基于单边缘设备和广角鱼眼相机的实时碰撞预警系统，能够在30fps下为未装备设备的骑行者和行人提供可听和可视警报，通过四项技术创新实现了93.3%的灵敏度。

<details>
  <summary>Details</summary>

**Motivation:** 城市交叉口骑行者与行人的碰撞是持续的伤害来源，但很少有系统尝试使用商用硬件为未装备设备的道路使用者提供实时警告，这需要开发低成本的实时预警技术。

**Method:** 1）开发了超广角鱼眼镜头校准流水线，通过透视重映射和直接光束法调整解决角点检测失败和优化器发散问题；2）结合鱼眼感知目标检测与通过预计算查找表实现的闭式地面投影；3）引入设计时一致性模拟，包含24个脚本化危险场景、随机大小感知检测失败和延迟扫描；4）将决策层形式化为可分离、可审核的测试平台。

**Result:** 在鱼眼定位误差的一致性测试下，选定流水线配置达到93.3%灵敏度和92.3%特异性，平均预警预算为3.3秒，系统在30fps下成功运行并提供可听和可视警报。

**Conclusion:** 该原型系统使用商用硬件实现了实时碰撞预警，通过综合技术创新保证了良好的性能，系统设计得益于社区辅助设计工作坊，代码和复制脚本已公开可用。
...
### [296] [OASIS: On-Demand Hierarchical Event Memory for Streaming Video Reasoning](https://arxiv.org/abs/2604.17052)
*Zhijia Liang,Jiaming Li,Weikai Chen,Yanhao Zhang,Haonan Lu,Guanbin Li*

Main category: cs.CV

> OASIS是一种通过分层事件组织和按需检索的流式视频推理框架，能够在保证推理效率的同时显著提升长时程准确性。

<details>
  <summary>Details</summary>

**Motivation:** 流式视频推理面临历史信息无限增长而有效信息稀缺的核心挑战。传统方法要么通过扩大内存增加冗余，要么通过压缩丢失关键信息，问题的本质在于难以定位相关信息而非记忆容量不足。

**Method:** OASIS框架通过两阶段机制解决该问题：1）将流式历史组织成分层事件结构；2）采用受控精炼的推理策略——先进行短上下文推理，仅在出现不确定性时触发语义驱动的高层级检索。该机制免训练且可即插即用。

**Result:** 多个基准测试和主干模型的实验表明，OASIS在保持有限token成本和低请求延迟的同时，显著提升了长时程准确性和组合推理能力。

**Conclusion:** 该研究证明，通过结构化组织和语义驱动的按需检索，可以有效解决流式视频推理中的信息检索难题，为实时视频理解提供了一种高效且可扩展的解决方案。
...
### [297] [mEOL: Training-Free Instruction-Guided Multimodal Embedder for Vector Graphics and Image Retrieval](https://arxiv.org/abs/2604.17054)
*Kyeong Seon Kim,Baek Seong-Eun,Lee Jung-Mok,Tae-Hyun Oh*

Main category: cs.CV

> 提出了一种无训练、指令引导的多模态嵌入框架，利用大型语言模型将文本、图像和SVG代码映射到统一的嵌入空间，在无需对比训练的情况下超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法通常将SVG栅格化并丢失其结构化信息，而文本嵌入方法难以自然扩展到视觉或结构化模态。需要一种能保留SVG丰富几何布局信息的多模态表示方法。

**Method:** 1) 多模态显式单词限制(mEOL)：指导MLLM将多模态输入总结为单个token，其隐藏状态作为语义嵌入；2) 语义SVG重写模块：通过视觉推理重写SVG代码，分配有意义的标识符并简化嵌套元素。使用模态特定指令和结构化SVG提示控制嵌入方向。

**Result:** 构建了首个文本到SVG检索基准，实验表明该无训练嵌入方法在结构感知的多模态检索任务上优于基于编码器和需要训练的多模态基线方法。

**Conclusion:** 提示级控制是参数级训练的有效替代方案，能够实现结构感知的多模态检索，揭示了保留符号化结构信息的重要性。
...
### [298] [Motion-Guided Semantic Alignment with Negative Prompts for Zero-Shot Video Action Recognition](https://arxiv.org/abs/2604.17062)
*Yiming Wang,Frederick W. B. Li,Jingyun Wang*

Main category: cs.CV

> 本文提出了一种增强CLIP模型的新框架，通过解耦嵌入和语义引导交互实现零样本动作识别，在标准基准测试中优于现有CLIP-based方法。

<details>
  <summary>Details</summary>

**Motivation:** 零样本动作识别面临已见和未见类别之间的语义鸿沟挑战，现有方法难以有效分离运动特征并建立跨模态语义对齐。

**Method:** 提出运动分离模块(MSM)分离运动敏感和全局静态特征，运动聚合块(MAB)采用门控交叉注意力优化运动表示，通过正负文本提示实现视频特征与文本表示的语义对齐。

**Result:** 在标准基准测试中，该方法在粗粒度和细粒度数据集上均优于先前的CLIP-based方法，实现了鲁棒的零样本动作识别。

**Conclusion:** 通过解耦嵌入和语义引导交互的策略，有效解决了零样本动作识别中的语义鸿沟问题，为跨模态学习提供了新的思路。
...
### [299] [BasketHAR: A Multimodal Dataset for Human Activity Recognition and Sport Analysis in Basketball Training Scenarios](https://arxiv.org/abs/2604.17065)
*Xian Gao,Haoyue Zhang,Zongyun Zhang,Jiacheng Ruan,Ting Liu,Yuzhuo Fu*

Main category: cs.CV

> 该论文介绍了BasketHAR，一个专门针对篮球训练的多模态人体活动识别数据集，包含专业级的篮球动作数据，并提供了基线多模态对齐方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的人体活动识别数据集主要关注基本的日常活动（如行走、站立），缺乏专业运动场景的数据，特别是在篮球训练分析等专门应用领域。这限制了HAR技术在体育性能分析中的应用。

**Method:** 作者构建了一个名为BasketHAR的多模态数据集，收集了惯性测量单元（加速度计和陀螺仪）、角速度、磁场、心率、皮肤温度等多种运动数据，以及同步视频录制。同时提供了一个基线多模态对齐方法来评估性能。

**Result:** 实验结果表明该数据集具有足够的复杂性和适用性，可用于先进的人体活动识别任务。数据集在Apache License 2.0许可下公开发布，为HAR和体育分析领域的未来研究提供了宝贵资源。

**Conclusion:** BasketHAR填补了专业篮球训练场景下人体活动识别数据的空白，不仅能支持篮球训练分析，还能生成专门的表现报告，对HAR和体育分析研究具有重要价值。
...
### [300] [NTIRE 2026 Rip Current Detection and Segmentation (RipDetSeg) Challenge Report](https://arxiv.org/abs/2604.17070)
...
> 这是一个关于离岸流检测与分割的NTIRE 2026挑战赛报告，该比赛基于RipVIS数据集，吸引了159名参与者，最终有9个有效提交。

<details>
  <summary>Details</summary>

**Motivation:** 离岸流是危险的近岸水流，是全球海滩相关死亡事故的主要原因之一，但由于其视觉外观在不同海滩、视角和海况下变化很大，难以识别，因此开发自动检测系统对海滩安全至关重要。

**Method:** 挑战赛基于RipVIS基准数据集，涵盖检测和分割两个任务，数据集来自10多个国家、4种相机视角，包含多样化的海滩和海况条件。评估采用F1[50]、F2[50]、F1[40:95]、F2[40:95]的综合评分系统。

**Result:** 比赛吸引了159名注册参与者，产生9个有效测试提交。参赛方案主要依赖预训练模型，结合强数据增强和后处理设计。

**Conclusion:** 结果表明离岸流理解很大程度上受益于稳健通用视觉模型的进展，但针对其独特视觉结构的专门方法仍有很大发展空间。
...
### [301] [Comparison Drives Preference: Reference-Aware Modeling for AI-Generated Video Quality Assessment](https://arxiv.org/abs/2604.17074)
*Minghao Zou,Gen Liu,Guanghui Yue,Baoquan Zhao,Zhihua Wang,Paul L. Rosin,Hantao Liu,Wei Zhou*

Main category: cs.CV

> 本文提出了一种参考感知的视频质量评估方法RefVQA，通过构建以查询视频为中心的参考图来组织相关样本，利用图引导的差异聚合来评估AI生成视频的质量，相比传统方法更符合人类感知，在多个数据集上表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 随着生成模型的快速发展，AI生成视频数量激增，对其质量进行自动评估变得日益重要。现有AIGC-VQA方法通常独立分析每个视频，忽略了视频间的潜在关系，而人类在评估质量时往往会通过比较相关视频来判断。

**Method:** 提出参考感知视频质量评估框架RefVQA，将AIGC-VQA重新构建为参考感知评估问题。构建以查询视频为中心的参考图来组织语义相关样本，采用图引导的差异聚合机制从参考节点向查询节点传播信息进行质量评估。

**Result:** 在现有数据集上的实验表明，RefVQA在多个质量维度上超越了最先进方法，跨数据集评估也验证了其强大的泛化能力。

**Conclusion:** 基于参考的评估框架在AI生成视频质量评估中具有显著优势，该方法为AIGC-VQA领域的进一步发展提供了新的思路和有效解决方案。
...
### [302] [D-Prism: Differentiable Primitives for Structured Dynamic Modeling](https://arxiv.org/abs/2604.17082)
*Xingyuan Yu,Yijin Li,Chong Zeng,Yuhang Ming,Hujun Bao,Guofeng Zhang*

Main category: cs.CV

> D-Prism是一个创新框架，首次将可微分原语扩展到动态建模领域，通过结合3DGS和3D原语实现高保真结构化动态对象建模。

<details>
  <summary>Details</summary>

**Motivation:** 现有动态建模方法（如可变形网格或3DGS）基于非结构化表示，无法联合建模几何结构和关节运动；而基于原语的方法在静态结构化场景中表现出色，但其动态潜力尚未探索。

**Method:** 提出D-Prism框架，将3DGS绑定到原语表面，利用各自在外观和几何方面的优势；引入变形网络控制原语运动，确保与物体运动精确匹配；设计自适应控制策略动态调整原语数量。

**Result:** 实验证明该方法在结构化动态建模方面表现优异，能够同时提供结构化几何信息和精确的运动跟踪效果。

**Conclusion:** D-Prism成功将可微分原语扩展到动态建模领域，为结构化动态对象的高保真建模提供了新的解决方案，填补了该领域的研究空白。
...
### [303] [EvoComp: Learning Visual Token Compression for Multimodal Large Language Models via Semantic-Guided Evolutionary Labeling](https://arxiv.org/abs/2604.17087)
*Jiafei Song,Fengwei Zhou,Jin Qu,Wenjin Jason Li,Tong Wu,Gengjian Xue,Zhikang Zhao,Daomin Wei,Yichao Lu,Bailin Na*

Main category: cs.CV

> EvoComp是一个视觉token压缩框架，通过轻量级压缩器和进化标注策略实现3倍压缩率下保留99.3%准确率，移动端加速1.6倍。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态大语言模型在推理效率上受大量视觉token限制，特别是在高分辨率或多图像场景下，需要有效减少token数量同时保持任务准确性。

**Method:** 提出EvoComp框架，包含：1) 基于transformer的轻量级压缩器，联合考虑视觉和文本上下文选择信息量丰富且非冗余的token；2) 进化标注策略，通过最小化MLLM输出损失搜索最优token子集；3) 结合GHM损失和余弦相似度正则化的定制损失函数进行训练。

**Result:** 在多个视觉语言基准测试中，EvoComp在3倍token压缩下保持99.3%的原始准确率，在移动设备上实现最高1.6倍的速度提升，显著优于基于注意力或相似度启发式的现有方法。

**Conclusion:** EvoComp有效地解决了MLLM推理效率问题，通过智能的token选择和创新的训练策略，实现了高压缩率下的精度保持和显著的推理加速，为移动端部署大模型提供了可行方案。
...
### [304] [Marrying Text-to-Motion Generation with Skeleton-Based Action Recognition](https://arxiv.org/abs/2604.17090)
*Jidong Kuang,Hongsong Wang,Jie Gui*

Main category: cs.CV

> CoAMD是一种基于坐标的自回归运动扩散模型，通过多模态动作识别器提供语义指导，在统一的框架下实现了动作识别和运动生成，在13个基准测试中达到state-of-the-art性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的动作识别和运动生成研究往往独立进行，未能挖掘两者之间的关联，即运动生成需要语义理解能力。作者认为将这两个任务统一起来研究，利用骨骼坐标同时实现运动理解和生成具有重要意义。

**Method:** 提出CoAMD模型，采用粗到细的方式合成运动。核心组件是多模态动作识别器(MAR)，为运动生成提供基于梯度的语义指导。该模型可以处理绝对坐标，并建立了严格的评估基准。

**Result:** 在四个重要任务（基于骨骼的动作识别、文本到运动生成、文本-运动检索、运动编辑）的13个基准测试中，该方法均达到state-of-the-art性能，证明了其在人体运动建模方面的有效性和通用性。

**Conclusion:** 该研究成功统一了动作识别和运动生成两个问题，通过坐标表示和自回归扩散框架，实现了多任务的高性能处理，为人机交互计算机视觉领域提供了新的研究方向。
...
### [305] [Hybrid Multi-Dimensional MRI Prostate Cancer Detection via Hadamard Network-Based Bias Correction and Residual Networks](https://arxiv.org/abs/2604.17107)
...
> 本研究提出了一种基于定量混合多维MRI和神经网络的两阶段AI框架HBR-Net-18，用于前列腺癌检测，通过Hadamard U-Net抑制偏置场，再用ResNet-18进行补丁级分类，性能显著优于传统方法。

<details>
  <summary>Details</summary>

**Motivation:** 尽管混合多维MRI(HM-MRI)技术增强了前列腺癌诊断能力，但仍迫切需要鲁棒的、自动化的AI检测方法来改善诊断效果。

**Method:** 提出HBR-Net-18两阶段框架：第一阶段使用基于Hadamard U-Net的算法抑制六个参数化HM-MRI图的强度不均匀性(偏置场)；第二阶段使用ResNet-18进行补丁级分类，采用重叠的11x11补丁，结合2D层内和3D层间信息提高空间一致性。

**Result:** 实验结果表明HB-Net实现了平衡的敏感性和特异性，显著优于传统的基于放射组学的方法和基线CNN模型。

**Conclusion:** 该框架展现了良好的临床应用潜力，为前列腺癌的自动化检测提供了有效的解决方案。
...
### [306] [From Clinical Intent to Clinical Model: An Autonomous Coding-Agent Framework for Clinician-driven AI Development](https://arxiv.org/abs/2604.17110)
*Zihao Zhao,Frederik Hauke,Juliana De Castilhos,Jakob Nikolas Kather,Sven Nebelung,Daniel Truhn*

Main category: cs.CV

> 本研究开发了一个自主原型系统，使临床医生能够通过自然语言交互独立开发临床AI模型，在多个临床任务中取得了良好效果，特别是成功解决了有偏差的气胸分类问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统临床AI开发需要临床医生与专业AI团队密切协作，这种协作模式沟通成本高、耗时长，且由于双方专业知识壁垒存在，即使反复讨论仍可能出现需求理解偏差。

**Method:** 研究团队构建了一个自主编码代理系统，临床医生仅需通过自然语言交互就能独立开发AI模型，并在皮肤 lesion 分类、黑色素瘤与痣的鉴别、腕骨折检测（包括仅5%边界框标注的弱监督版本）和消除偏差的气胸分类等五个临床任务中进行评估。

**Result:** 该系统能够根据临床医生的需求 consistently 开发出性能良好的模型，特别是在减少胸腔引流管这一混杂因素对气胸分类影响方面表现出色，几乎将模型对胸腔引流管的依赖性减半。

**Conclusion:** 自主编码代理有潜力实现临床AI开发范式的转变，使其更加以临床医生为中心，降低沟通开销和对专业AI开发者的依赖，为临床AI开发提供了更便捷的路径，但仍需要进一步验证和鲁棒性评估。
...
### [307] [Inference-Time Temporal Probability Smoothing for Stable Video Segmentation with SAM2 under Weak Prompts](https://arxiv.org/abs/2604.17115)
*Dawar Jyoti Deka*

Main category: cs.CV

> 提出一种推理时的时间概率平滑方法，通过结合光流运动对齐和像素不确定性的自适应融合，显著提升了SAM2在弱提示下的视频分割时间稳定性。

<details>
  <summary>Details</summary>

**Motivation:** 基于SAM2的交互式视频分割模型在弱用户监督（如稀疏点提示）下存在时间不稳定性问题，表现为边界闪烁、物体丢失和范围不一致，限制了其在下游应用中的可靠性。

**Method:** 提出推理时时间概率平滑方法，无需重新训练或修改架构。该方法直接操作每帧分割概率图，利用基于光流的运动变形以及从分割熵和前后向光流一致性导出的像素级不确定性估计，自适应地融合当前帧预测与运动对齐的历史估计。

**Result:** 在四个多样化的视频序列上，通过运动补偿IoU、边界一致性、物体持续性和面积波动性等全面指标评估，实验结果表明该方法相比原始SAM2推理在保持空间精度的同时显著提升了时间稳定性。

**Conclusion:** 该框架轻量级、模型无关，适用于实时交互式视频分割场景，为解决弱提示下的时间稳定性问题提供了有效解决方案。
...
### [308] [Multimodal Fusion of Histopathology Images and Electronic Health Records for Early Breast Cancer Diagnosis](https://arxiv.org/abs/2604.17122)
*Aditya Shribhagwan Khandelwal,Mohammad Samar Ansari,Asra Aslam*

Main category: cs.CV

> 本文提出了一个多模态框架，整合病理图像和临床数据进行乳腺癌诊断，在三类分类任务中达到接近完美的预测效果(AUC 0.997)。

<details>
  <summary>Details</summary>

**Motivation:** 乳腺癌是全球癌症死亡的主要原因，及时准确的诊断对提高生存率至关重要。现有研究通常单独处理病理图像或电子健康记录，缺乏多模态数据的综合分析。

**Method:** 构建了一个多模态框架，包括：1)图像模型(CNN基线和ResNet-18)；2)表格数据模型(XGBoost和MLP)；3)中间融合模型，通过连接两种模态的潜在表示进行分析。采用Grad-CAM和SHAP进行可解释性分析。

**Result:** ResNet-18在病理图像分类中达到完美性能(准确率1.000, AUC 1.000)，XGBoost在临床数据预测中获得98%准确率。多模态融合模型实现了0.997的宏平均AUC，在关键的细胞分裂分类上表现最佳(AUC 0.994)。

**Conclusion:** 多模态整合不仅显著提升了预测性能，还通过可解释性分析增强了临床透明度，证明了病理图像与临床数据协同分析在乳腺癌诊断中的价值。
...
### [309] [Prompt Sensitivity in Vision-Language Grounding: How Small Changes in Wording Affect Object Detection](https://arxiv.org/abs/2604.17126)
*Dawar Jyoti Deka,Amit Sethi,Syed Mohammad Ali*

Main category: cs.CV

> 研究发现视觉语言模型在对象定位中存在语义等价描述不一致的问题，平均会选出2.11个不同的实例，且问题源于argmax选择机制而非文本距离。

<details>
  <summary>Details</summary>

**Motivation:** 本文旨在检验视觉语言模型的核心假设--语义等价的自然语言描述应该产生一致的对象定位输出，但实际存在不稳定性问题。

**Method:** 使用DETR提取对象候选，结合CLIP进行语言条件选择，在263张COCO图像上测试6个重叠提示词（如'person'、'human'、'pedestrian'）的定位一致性，并进行PCA分析和相关性检验。

**Result:** 语义等价描述经常选择不同实例（平均2.11个不同选择），PCA显示变异性具有结构性而非随机性，提示词集成不能改善质量，文本嵌入距离仅能解释34%的分歧。

**Conclusion:** 视觉语言模型的定位不稳定性主要源于argmax选择机制而非文本层面的语义距离，这一发现对模型设计和应用具有重要意义。
...
### [310] [OptiMVMap: Offline Vectorized Map Construction via Optimal Multi-vehicle Perspectives](https://arxiv.org/abs/2604.17135)
*Zedong Dan,Zijie Wang,Wei Zhang,Xiangru Lin,Weiming Zhang,Xiao Tan,Jingdong Wang,Liang Lin,Guanbin Li*

Main category: cs.CV

> OptiMVMap是一种选择性融合多车视角的矢量地图构建方法，通过最优车辆选择有效解决了多车融合中的计算成本、冗余和噪声问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的单自动驾驶车辆轨迹建模方法存在视野不足的问题，而引入周围车辆视角的朴素融合方法会带来计算成本高、视角冗余、姿态误差和遮挡噪声等三大挑战。

**Method:** 提出'先选择后融合'的框架：最优车辆选择(OVS)模块策略性地识别能最大限度减少遮挡区域不确定性的紧凑辅助车辆子集；跨车辆注意力(CVA)和语义感知噪声过滤器(SNF)实现姿态容差对齐和伪影抑制，最后进行BEV级融合。

**Result:** 在nuScenes和Argoverse2数据集上，OptiMVMap相比MapTRv2分别提升10.5 mAP和9.3 mAP，超过基于记忆的基线方法MVMap和HRMapNet 6.2 mAP和3.8 mAP。

**Conclusion:** 不确定性指导的辅助车辆选择对于高效准确的多车矢量地图构建至关重要，OptiMVMap能够在使用更少视角的情况下构建更完整、拓扑结构更准确的地图。
...
### [311] [ScenarioControl: Vision-Language Controllable Vectorized Latent Scenario Generation](https://arxiv.org/abs/2604.17147)
...
> ScenarioControl是首个基于视觉语言控制的驾驶场景生成方法，可通过文本或图像输入生成包含地图、动态物体和摄像头视角的3D场景序列。

<details>
  <summary>Details</summary>

**Motivation:** 现有的驾驶场景生成方法缺乏细粒度的多模态控制能力，无法根据文本或图像提示精确控制道路布局和交通条件，需要一种能实现精准控制且保持真实性的生成方法。

**Method:** 提出跨全局控制机制，结合交叉注意力和轻量级全局上下文分支，在矢量化潜在空间中联合表示道路结构和动态智能体，实现从多模态输入到具体场景元素的精确映射。

**Result:** 实验表明ScenarioControl在控制准确性和场景保真度方面均优于测试的其他方法，能够生成时序一致的多视角场景序列，支持长时间场景延续。

**Conclusion:** ScenarioControl成功实现了基于视觉语言的驾驶场景精准控制，为自动驾驶仿真和测试提供了新的解决方案，同时发布了带文本注释的数据集以促进该领域研究。
...
### [312] [Instant Colorization of Gaussian Splats](https://arxiv.org/abs/2604.17155)
*Daniel Lieber,Alexander Mock,Nils Wandel*

Main category: cs.CV

> 本研究提出了一种高效的反向映射方法，将2D图像信息映射到3D高斯泼溅场景中，实现了比梯度下降快10倍的性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有的高斯泼溅技术主要关注3D到2D的映射，但许多应用如场景重光照、风格化和3D语义分割需要反向的2D到3D映射，这带来了视角相关着色和遮挡处理等挑战。

**Method:** 使用正规方程方法，为每个高斯泼溅求解一个可见性加权的最小二乘问题，并基于现有的可微分光栅化器高效实现。

**Result:** 在场景重光照、特征增强和3D语义分割任务中验证了方法的有效性，相比基于梯度下降的基线方法实现了高达一个数量级的加速。

**Conclusion:** 该方法成功解决了2D到3D反向映射的关键挑战，为高斯泼溅技术在重光照、风格化和语义分割等应用中提供了高效的解决方案。
...
### [313] [PPEDCRF: Dynamic-CRF-Guided Selective Perturbation for Background-Based Location Privacy in Video Sequences](https://arxiv.org/abs/2604.17163)
*Bo Ma,Weiqi Yan,Jinsong Wu*

Main category: cs.CV

> PPEDCRF是一种保护基于背景的位置隐私的框架，通过动态条件随机场识别敏感背景区域并局部注入校准噪声，在保护隐私的同时保持较高的视频质量。

<details>
  <summary>Details</summary>

**Motivation:** 即使去除GPS元数据，攻击者仍可通过匹配背景视觉线索来地理定位视频帧位置，现有的全局噪声保护方法会导致视觉质量严重下降，需要一种更精细的隐私保护方案。

**Method:** 提出PPEDCRF框架，结合动态条件随估计位置敏感背景区域，使用标准化控制惩罚调整扰动强度，仅在识别区域通过差分隐私风格规则注入高斯噪声。

**Result:** 在受控场景检索基准上，PPEDCRF将ResNet18 Top-1检索精度从0.667降至0.361±0.127，同时保持36.14dB PSNR，比全局高斯噪声质量优势约6dB。

**Conclusion:** PPEDCRF实现了空间集中的扰动，在实践中保留了更高的视觉质量，而非更强的匹配效用隐私保护，为视频发布中的背景位置隐私提供了有效防护方案。
...
### [314] [LookasideVLN: Direction-Aware Aerial Vision-and-Language Navigation](https://arxiv.org/abs/2604.17190)
*Yuwei Ning,Ganlong Zhao,Yipeng Qin,Si Liu,Yang Liu,Liang Lin,Guanbin Li*

Main category: cs.CV

> LookasideVLN是一种新型空中视觉语言导航方法，通过利用自然语言中的方向线索实现更准确的空中导航，显著提升性能同时降低计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 现有的空中视觉语言导航方法存在两个主要限制：对指令理解浅显且主要依赖地标描述，忽视了方向线索这一人类导航中的重要空间上下文信息；同时计算成本高昂。

**Method:** 提出LookasideVLN框架，包含三个核心组件：1) 自我中心旁视图(ELG)，动态编码指令相关地标及其方向关系；2) 空间地标知识库(SLKB)，提供轻量级的先验导航经验记忆检索；3) 旁视多模态大语言模型导航代理，对齐用户指令、视觉观察和地标方向信息进行路径规划。

**Result:** 实验结果表明，即使只使用单层前视，LookasideVLN也显著超越了当前最先进的CityNavAgent方法。

**Conclusion:** 利用方向线索是一种强大且高效的空中视觉语言导航策略，不仅能提供更准确的空间推理，还能实现更低的计算成本，为复杂城市环境中的UAV导航提供了新的解决思路。
...
### [315] [DreamShot: Personalized Storyboard Synthesis with Video Diffusion Prior](https://arxiv.org/abs/2604.17195)
*Junjia Huang,Binbin Yang,Pengxiang Yan,Jiyang Liu,Bin Xia,Zhao Wang,Yitong Wang,Liang Lin,Guanbin Li*

Main category: cs.CV

> DreamShot是一个基于视频生成模型的故事板框架，通过利用视频扩散先验和角色一致性约束机制，实现了高质量、时序连贯的多镜头视觉故事叙述。

<details>
  <summary>Details</summary>

**Motivation:** 现有的故事板生成方法主要基于文本到图像扩散模型，在保持长距离时序连贯性、角色身份一致性和多镜头叙事流方面存在显著不足，无法满足视觉故事叙述的完整需求。

**Method:** DreamShot采用视频生成模型架构，支持文本到镜头和参考图像到镜头两种生成模式，实现了基于前后帧的故事续生成。核心技术是多参考角色条件模块，通过引入角色注意一致性损失机制，明确约束参考图像与生成角色之间的注意力对齐，确保角色身份的一致性。

**Result:** 实验结果表明，DreamShot在场景连贯性、角色一致性和生成效率方面均显著优于现有最先进的文本到图像故事板模型，能够生成视觉和语义上都更连贯的序列。

**Conclusion:** DreamShot为可控的视频模型驱动视觉故事叙述开辟了新的研究方向，通过充分利用视频生成模型的时空一致性优势，成功解决了传统方法的局限性，为高质量故事板生成提供了有效解决方案。
...
### [316] [SciDraw-6K: A Multilingual Scientific Illustration Dataset Generated by Google Gemini](https://arxiv.org/abs/2604.17206)
*Davie Chen*

Main category: cs.CV

> 这是一个包含6,291个科学插图的数据集，支持11种语言，涵盖8个科学类别，专门用于科学可视化的多语言文本到图像研究。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本到图像数据集主要针对通用图像，缺乏专门面向科学插图的数据集。为了支持科学领域的可视化研究，特别是多语言环境下的科学图像生成，需要一个专门构建的科学插图数据集。

**Method:** 使用Google Gemini图像生成模型（主要是gemini-2.5-flash-image和gemini-3-pro-image-preview）生成6,291个科学插图，每个图像配有11种语言的提示文本。数据集包含8个科学类别：生物医药、化学、材料、电子、环境、AI系统、物理以及其他类别。

**Result:** 成功构建并发布了SciDraw-6K数据集，已作为sci-draw.com公共科学绘图服务的基础。该数据集支持多语言文本到图像研究、领域适应的扩散模型微调和科学可视化的提示工程研究。

**Conclusion:** SciDraw-6K填补了科学插图文本到图像数据的空白，为科学可视化的多语言研究和应用提供了重要资源，促进了科学领域AI绘图技术的发展。
...
### [317] [CDSA-Net:Collaborative Decoupling of Vascular Structure and Background for High-Fidelity Coronary Digital Subtraction Angiography](https://arxiv.org/abs/2604.17208)
*Si Li,Chen-Kai Hu,Zhenhuan Lyu,Yuanqing He*

Main category: cs.CV

> CDSA-Net是一种新的数字减影血管造影深度学习框架，通过分离优化血管结构保持和背景重建，有效解决了边界伪影和组织灰度失真问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统冠状动脉DSA成像受生理运动影响，现有深度学习方法存在边界伪影和组织灰度失真等临床上不可接受的缺陷，降低了诊断置信度。

**Method:** 提出CDSA-Net框架，包含两个核心创新：1) HGPG机制结合I GP、GSM和CAT损失监督的CSENet网络确保血管结构连续性；2) ANM模块建模临床X射线噪声随机性的CBResNet网络实现无缝背景恢复。

**Result:** 在血管强度相关性和感知质量上显著优于现有方法，形貌评估效率提升25.6%，血流动力学评估速度提升42.9%，同时保持与原始血管造影一致的诊断结果。

**Conclusion:** CDSA-Net首次实现了血管结构保持和背景重建的分离优化，为介入心脏病学设立了新的实用基准，有效解决了传统DSA的关键技术瓶颈。
...
### [318] [DREAM: Dynamic Retinal Enhancement with Adaptive Multi-modal Fusion for Expert Precision Medical Report Generation](https://arxiv.org/abs/2604.17209)
*Nagur Shareef Shaik,Teja Krishna Cherukuri,Dong Hye Ye*

Main category: cs.CV

> DREAM是一个创新的医学影像报告生成框架，通过两阶段自适应多模态融合机制，在数据稀缺的情况下为视网膜图像生成高质量的医学报告。

<details>
  <summary>Details</summary>

**Motivation:** 现有的大型视觉-语言模型在专业化医学领域表现不佳，主要因为数据稀缺导致模型过拟合，容易遗漏微妙但关键的病理特征。

**Method:** DREAM采用独特的两阶段融合机制：1) Abstractor模块将图像和关键词特征映射到共享空间，用病理相关洞察增强视觉数据；2) Adaptor模块执行自适应多模态融合，通过可学习参数动态加权各模态重要性。同时使用对比对齐模块确保输出与临床真实报告的语义一致性。

**Result:** 在DeepEyeNet基准测试上取得了最新水平的0.241 BLEU-4分数，并在ROCO数据集上展现出强大的泛化能力。

**Conclusion:** DREAM通过结合医学专业知识和高效融合策略，成功解决了数据稀缺情况下医学报告生成的挑战，为自动化医学报告系统提供了新的解决方案。
...
### [319] [EmbodiedHead: Real-Time Listening and Speaking Avatar for Conversational Agents](https://arxiv.org/abs/2604.17211)
*Yu Zhang,Kaiyuan Shen,Yang Li*

Main category: cs.CV

> EmbodiedHead是一个语音驱动的对话头像框架，通过Rectified-Flow扩散Transformer和可微分渲染器，实现了四步生成的高质量实时对话头像，解决了听说统一交互问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的说话头像系统无法同时满足实时生成、统一听说行为和高渲染质量的要求。传统双流音频方法存在对话者前瞻依赖问题，不符合因果交互需求。

**Method:** 采用首个Rectified-Flow扩散Transformer配合可微分渲染器；设计单流接口和显式帧级听说状态条件；引入流式音频调度器；实施系数空间预训练和图像域联合优化的两阶段训练方案。

**Result:** 在听说两种场景下都达到了最先进的视觉质量和动作保真度，实现了四步采样的高效生成，成功抑制了听讲时的虚假嘴部动作并支持流畅的话轮转换。

**Conclusion:** EmbodiedHead为语言模型提供了实用的具身视觉能力，在保持高质量渲染的同时实现了真正的实时对话交互，为AI对话系统的视觉表现树立了新的标准。
...
### [320] [Cross-Modal Attention Analysis and Optimization in Vision-Language Models: A Study on Visual Reliability](https://arxiv.org/abs/2604.17217)
*Lijie Zhou*

Main category: cs.CV

> 该论文提出了一个对抗性评估框架来衡量视觉语言模型中的文本捷径学习现象，并通过优化训练策略显著降低了模型对文本的过度依赖。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型(VLMs)在跨模态任务中表现优异，但研究发现这些模型过度依赖文本描述而未能充分利用视觉信息，这种现象被称为'文本捷径学习'。

**Method:** 研究者构建了一个对抗性评估框架，通过将语义冲突的文本与不变图像配对来测量准确率下降程度（Drop）。在1000个几何形状数据集上应用四种对抗策略：形状交换、颜色交换、位置交换和随机文本。比较了三种模型配置：基础CLIP、LoRA微调和LoRA优化模型（集成硬负样本挖掘、标签平滑、分层学习率、余弦重启、课程学习和数据增强）。

**Result:** 优化模型将平均Drop从27.5%降低到9.8%（相对改善64.4%，p<0.001），同时保持97%的正常准确率。注意力可视化和嵌入空间分析证实优化模型更多关注视觉特征并实现更紧密的跨模态对齐。

**Conclusion:** 通过系统的对抗性评估和优化训练策略，可以有效减轻视觉语言模型的文本捷径学习问题，提高模型对视觉信息的利用能力，同时维持整体性能。
...
### [321] [Region-Affinity Attention for Whole-Slide Breast Cancer Classification in Deep Ultraviolet Imaging](https://arxiv.org/abs/2604.17222)
*Nagur Shareef Shaik,Teja Krishna Cherukuri,Dong Hye Ye*

Main category: cs.CV

> 本研究提出了一种新型的区域亲和力注意力机制，用于深紫外全幻灯片图像的乳腺癌分类，无需分块处理即可保留空间完整性，在136个样本上达到92.67%的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 传统组织病理学方法在术中设置中存在局限性，而现有的深度学习方法主要基于补丁处理，破坏了空间上下文关系，且标准注意力机制无法充分利用DUV-WSI数据中丰富的多尺度区域关系。

**Method:** 开发了区域亲和力注意力机制，通过建模局部邻域距离和构建完整亲和矩阵，动态突出诊断相关区域，并结合对比损失增强特征区分能力，直接处理整个幻灯片而无需分块。

**Result:** 在包含136个DUV-WSI样本的数据集上，该方法实现了92.67±0.73%的准确率和95.97%的AUC，性能优于现有注意力方法。

**Conclusion:** 该研究证明了针对DUV-WSI数据特性定制的注意力机制能够有效提升乳腺癌分类性能，为临床快速精准诊断提供了新的技术路径。
...
### [322] [Fringe Projection Based Vision Pipeline for Autonomous Hard Drive Disassembly](https://arxiv.org/abs/2604.17231)
*Badrinath Balasubramaniam,Vignesh Suresh,Benjamin Metcalf,Beiwen Li*

Main category: cs.CV

> 提出一种自动化视觉管道，结合结构光深度感知和实时实例分割，实现高精度硬盘驱动器拆解场景理解，具有优秀的语义和空间数据输出。

<details>
  <summary>Details</summary>

**Motivation:** 电子废物回收存在巨大经济损失，硬盘驱动器作为高价值电子废物需要自动化拆解，但现有方法在3D感知、场景理解和螺钉定位方面存在不足且缺乏集成方案。

**Method:** 开发自主视觉管道，使用条纹投影轮廓测量(FPP)进行3D感知，选择性触发深度补全模块，集成轻量级实时实例分割网络，采用FPP相机-投影仪系统保证深度图与分割掩码像素对齐，并应用仿真到真实迁移学习。

**Result:** 实例分割达到box mAP@50为0.960，mask mAP@50为0.957；深度补全配置RMSE为2.317mm，MAE为1.836mm；系统延迟12.86ms，吞吐量77.7FPS。

**Conclusion:** 该感知管道为下游机器人拆解提供高保真语义和空间数据，HDD实例分割的合成数据集将公开发布。
...
### [323] [Enhancing Zero-shot Personalized Image Aesthetics Assessment with Profile-aware Multimodal LLM](https://arxiv.org/abs/2604.17233)
*Chun Wang,Chenfeng Wei,Chenyang Liu,Weihong Deng*

Main category: cs.CV

> 论文提出P-MLLM，一种基于用户画像的多模态大语言模型，用于解决零样本个性化图像美学评估问题，通过选择性融合模块实现画像感知的视觉信息整合。

<details>
  <summary>Details</summary>

**Motivation:** 现有个性化图像美学评估方法依赖用户历史评分数据，在零样本场景下无法有效建模用户特定的美学偏好。当缺乏用户历史数据时，需要新的个人化方法。

**Method:** 提出P-MLLM框架，利用用户画像作为上下文信号，通过选择性融合模块将视觉信息有控制地整合到大语言模型的隐藏状态中，实现画像条件下的推理过程。

**Result:** 在最新PIAA基准测试上，P-MLLM展现了竞争性的零样本性能，即使使用粗粒度画像信息仍保持有效，证明了基于画像的个人化方法的潜力。

**Conclusion:** 基于用户画像的个人化范式为零样本PIAA提供了解决方案，P-MLLM的有效性表明这一方法具有实际应用前景，为缺乏历史数据的个性化美学评估开辟了新途径。
...
### [324] [RemoteShield: Enable Robust Multimodal Large Language Models for Earth Observation](https://arxiv.org/abs/2604.17243)
*Rui Min,Liang Yao,Shiyu Miao,Shengxiang Xu,Yuxuan Liu,Chuanyi Zhang,Shimin Di,Fan Liu*

Main category: cs.CV

> 本文提出了RemoteShield，一个鲁棒的多模态大语言模型，通过跨条件偏好学习来保持遥感应用中面对图像退化和文本噪声时的稳定性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的遥感多模态大语言模型在干净数据集上训练，学习到的映射关系脆弱，无法泛化到实际应用中的噪声条件，导致性能下降。

**Method:** 构建了包含视觉退化（云雾覆盖）和文本变化（口语化、模糊指令）的多模态扰动；通过语义等价簇和偏好学习训练模型，使其在扰动条件下保持响应稳定性。

**Result:** 实验证明RemoteShield在三个地球观测任务中，相比基线模型展现出更强的鲁棒性和跨条件一致性。

**Conclusion:** RemoteShield有效解决了遥感多模态大语言模型在实际应用中的脆弱性问题，为现实环境中的地球观测提供了更可靠的解决方案。
...
### [325] [Fractal Characterization of Low-Correlation Signals in AI-Generated Image Detection](https://arxiv.org/abs/2604.17268)
*Wenwei Xie,Jie Yin,Lu Ma,Xuansong Zhang,Wenjing Zhang*

Main category: cs.CV

> 本文提出了一种基于分形理论和低相关性信号的AI生成图像检测新方法，通过分析图像信号的统计特征差异，实现了在开放环境下稳健的深度伪造检测。

<details>
  <summary>Details</summary>

**Motivation:** 当前AI生成图像技术已接近照片级真实度，对信息安全和社会信任构成严重威胁。现有的深度伪造检测方法在开放世界场景中鲁棒性有限，需要更可靠的检测技术。

**Method:** 从信号级角度分析AI生成图像与真实图像的内在差异，发现低相关性信号可作为有效标记。基于分形理论提出量化这些信号的新方法，通过分析低相关性信号的分形特征来捕捉合成过程中的细微统计异常。

**Result:** 实验结果表明，该方法具有鲁棒性且检测性能优越。理论上该方法不仅限于人脸图像识别，还可应用于所有AI生成图像检测任务。

**Conclusion:** 本研究强调了需要将研究重点转向信号级方向进行深度伪造检测，为该领域提供了新的研究方向，具有重要的理论意义和实用价值。
...
### [326] [Instinct vs. Reflection: Unifying Token and Verbalized Confidence in Multimodal Large Models](https://arxiv.org/abs/2604.17274)
*Yunkai Dang,Yifan Jiang,Yizhu Jiang,Anqi Chen,Wenbin Li,Yang Gao*

Main category: cs.CV

> 本文针对多模态大语言模型提出了一种新的置信度估计方法，通过融合模型内在支持度和口头自评估，解决了本能-反思不一致问题。

<details>
  <summary>Details</summary>

**Motivation:** 多模态大语言模型在感知和推理任务中表现优异，但在实际部署中需要可靠的置信度估计。现有方法主要针对纯文本模型，且计算成本高昂。

**Method:** 提出单调置信度融合框架，结合token级内在支持信号和口头自评估信号，通过跨通道一致性判断正确性，并应用保序均值对齐校正全局偏差。

**Result:** 在多种开源和闭源MLLMs上实验表明，该方法能产生更可靠的置信度估计，改善了校准效果和失败预测能力。

**Conclusion:** 该方法有效解决了MLLMs中的本能-反思不一致问题，为多模态模型提供了计算高效的置信度估计方案。
...
### [327] [PestVL-Net: Enabling Multimodal Pest Learning via Fine-grained Vision-Language Interaction](https://arxiv.org/abs/2604.17278)
*Xueheng Li,Tao Hu,Ke Cao,Runsheng Qi,Huixin Zhang,Rui Li,Jie Zhang,Chengjun Xie*

Main category: cs.CV

> 提出名为PestVL-Net的视觉语言融合框架，通过RWKV建模害虫细粒度视觉特征，结合MLLM生成语义描述，显著提升害虫识别性能。

<details>
  <summary>Details</summary>

**Motivation:** 害虫种类繁多、形态复杂多样，现有技术难以有效建模细粒度视觉特征和高层语义特征，限制了害虫识别在实际农业场景中的应用。

**Method:** PestVL-Net框架包含：1) 基于RWKV的视觉路径，采用显著性引导的自适应窗口分割方案建模害虫细粒度视觉特征；2) 语言路径利用多模态大语言模型先验知识，通过专家知识和多模态链式思维推理生成精确的害虫语义描述；3) 视觉和文本特征的深度融合。

**Result:** 在多个害虫数据集上的广泛实验验证了PestVL-Net的优越性能，证明了其在实际害虫管理中的有效应用潜力。

**Conclusion:** 通过视觉语言协同学习的方法，成功解决了害虫细粒度特征建模的挑战，为可持续农业发展中的害虫识别和管理提供了有效的技术方案。
...
### [328] [Depth Adaptive Efficient Visual Autoregressive Modeling](https://arxiv.org/abs/2604.17286)
*Chunliang Li,Tianze Cao,Sanyuan Zhao*

Main category: cs.CV

> DepthVAR 是一种无需训练的框架，通过动态分配每个 token 的计算深度来实现高分辨率图像生成的加速，相比传统的硬剪枝方法，在最小质量损失下获得了 2.3-3.1 倍的加速效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉自回归 (VAR) 模型在生成高分辨率图像时对每个位置使用固定的计算深度，效率低下。虽然基于频率图的 token 剪枝方法可以加速推理，但其二元硬剪枝方法存在根本性限制，即使有更好的频率估计也无法提升质量。

**Method:** DepthVAR 通过动态分配计算深度来实现自适应处理。它包含：1) 自适应深度调度器，通过循环旋转调度分配计算深度，实现平衡且非静态的细化；2) 动态推理过程，将深度转换为逐层掩码，选择性应用 transformer 块，并混合生成的代码，确保每个 token 的影响力与其处理深度成正比。

**Result:** 广泛实验表明，DepthVAR 在最小质量损失前提下实现了 2.3-3.1 倍的加速，在计算性能权衡方面优于现有的硬剪枝方法。

**Conclusion:** 该方法标志着从剪枝整个 token 到自适应分配 token 计算深度的范式转变，为高分辨率视觉自回归模型的高效推理提供了新的解决方案。
...
### [329] [Spectral Forensics of Diffusion Attention Graphs for Copy-Move Forgery Detection](https://arxiv.org/abs/2604.17287)
*H. M. Shadman Tabib,Tasriad Ahmed Tias,Nafis Tahmid*

Main category: cs.CV

> GraphSpecForge是一个无需训练的伪造检测框架，通过分析Stable Diffusion U-Net中注意力图的谱结构来检测复制-移动伪造。

<details>
  <summary>Details</summary>

**Motivation:** 复制-移动伪造（图像内区域复制以隐藏或伪造内容）对视觉媒体完整性构成持续威胁，需要有效的检测方法。

**Method:** 利用预训练Stable Diffusion U-Net的自注意力图，通过分析归一化图拉普拉斯谱的变化，使用Wasserstein距离与真实参考分布比较构建图像级异常检测器。

**Result:** 在四个基准测试上达到良好性能，RecodAI-LUC上AUROC=0.606，MICC-F220、CoMoFoD和COVERAGE上AUROCs分别为0.752、0.774和0.673，CoMoFoD上AUPRC=0.833。

**Conclusion:** 该方法证明了复制-移动操纵会导致自注意力图中近似子图重复，从而在归一化图拉普拉斯中产生可测量的谱重分布，为无需特定训练的伪造检测提供了新思路。
...
### [330] [Frequency-guided Multi-level Reasoning for Scene Graph Generation in Video](https://arxiv.org/abs/2604.17298)
*Chenxing Li,Yiping Duan,Xiaoming Tao*

Main category: cs.CV

> 本文提出了FReMuRe模型，通过频率引导的关系多层推理机制来解决场景图生成中的长尾分布问题，显著提升了尾部类别的召回率。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频场景图生成方法在处理长尾分布关系时存在局限性，尾部类别的关系难以被有效识别，影响整体理解效果。

**Method:** FReMuRe模型采用三个关键技术：1）关系特定分支处理梯度冲突，实现更平衡的尾部感知学习；2）频率感知双分支谓词嵌入网络，分别建模高频和低频关系并通过门控融合提升尾部类别召回率；3）可切换的贝叶斯头（不确定性估计）和高斯混合模型头（增强类内多样性）两种关系分类头。

**Result:** 在Action Genome数据集上的实验表明，FReMuRe显著提高了长尾关系的召回率和整体推理的鲁棒性。

**Conclusion:** 通过机制层面的改进，FReMuRe有效解决了视频场景图生成长尾分布问题，为结构化视频语义理解提供了新的解决方案。
...
### [331] [The First Challenge on Mobile Real-World Image Super-Resolution at NTIRE 2026: Benchmark Results and Method Overview](https://arxiv.org/abs/2604.17306)
...
> 本文回顾了NTIRE 2026移动端真实世界图像超分辨率挑战赛，总结了参赛方案和结果，该挑战要求在移动设备上实现4倍放大的高效超分辨率。

<details>
  <summary>Details</summary>

**Motivation:** 随着移动设备性能提升和用户对高画质图像需求增长，需要在资源受限的移动设备上实现高效的真实世界图像超分辨率算法，这具有重要的实际应用价值。

**Method:** 举办挑战赛，要求参赛团队开发可在移动设备上运行的轻量级网络设计或解决方案，通过图像质量评估(IQA)分数和加速比的加权组合来评估模型性能。

**Result:** 挑战吸引了108名注册者，其中16个团队在最终排名中获得有效分数，推动了移动端真实世界图像超分辨率技术的发展，并为该领域最新趋势提供了深入概述。

**Conclusion:** 该挑战赛有效地促进了移动端高效超分辨率算法的研究，在保持高质量重建的同时满足移动设备的计算约束，为移动图像处理应用提供了有价值的参考。
...
### [332] [Generalizable Face Forgery Detection via Separable Prompt Learning](https://arxiv.org/abs/2604.17307)
*Enrui Yang,Yuezun Li*

Main category: cs.CV

> 本文提出了一种基于CLIP的可分离提示学习策略(SePL)，通过利用文本模态来指导深度伪造检测，实现了跨数据集和跨方法的优秀泛化性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于CLIP的深度伪造检测方法主要依赖视觉编码器，忽视了文本模态的指导作用。鉴于文本模态具有指导性，作者认为可以通过精心设计利用文本模态来指导深度伪造检测。

**Method:** 提出可分离提示学习策略(SePL)，通过两种类型的提示学习来解耦图像中的伪造特定信息和伪造无关信息，前者用于增强检测。为实现解耦，设计了跨模态对齐策略和一组专用目标函数。

**Result:** 大量实验表明，该方法在跨数据集和跨方法评估中取得了竞争性甚至更优的性能，展现出强大的泛化能力。

**Conclusion:** 通过简单地从视觉模态转向文本模态，并采用可分离提示学习策略，能够有效利用CLIP进行深度伪造检测，证明了文本模态在该任务中的重要价值。
...
### [333] [When Background Matters: Breaking Medical Vision Language Models by Transferable Attack](https://arxiv.org/abs/2604.17318)
*Akash Ghosh,Subhadip Baidya,Sriparna Saha,Xiuying Chen*

Main category: cs.CV

> MedFocusLeak是一种针对医学视觉语言模型的高效黑盒攻击方法，可通过注入不可见的背景扰动来产生看似合理但错误的临床诊断结果。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型在临床诊断中的应用日益广泛，但其对对抗攻击的鲁棒性研究不足。现有医学攻击方法主要关注模型窃取等次要目标，而自然图像的转移性攻击会产生可见畸变，容易被临床医生识别。

**Method:** 提出MedFocusLeak方法，通过在非诊断背景区域注入协调扰动，并采用注意力分散机制将模型的注意力从病理区域转移开，实现不可感知的误导性诊断攻击。

**Result:** 在六种医学成像模态上进行的广泛评估表明，MedFocusLeak达到了最先进的性能，能够在不同的VLM上生成具有误导性但看似真实的诊断输出。

**Conclusion:** 研究揭示了现代临床VLM在推理能力方面存在关键弱点，并通过统一评估框架和新颖指标共同评估了攻击成功率和图像保真度，为医学AI安全提供了重要见解。
...
### [334] [E2E-GMNER: End-to-End Generative Grounded Multimodal Named Entity Recognition](https://arxiv.org/abs/2604.17319)
*Meng Zhang,Jinzhong Ning,Xiaolong Wu,Hongfei Lin,Yijia Zhang*

Main category: cs.CV

> E2E-GMNER是一个端到端生成式框架，统一了多模态命名实体识别、语义分类和视觉定位，通过高斯风险感知框扰动技术提升鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法主要采用流水线架构，将文本实体识别和视觉定位分离，导致误差累积和联合优化效果不佳。

**Method:** 提出end-to-end生成框架，将GMNER建模为指令调优的条件生成任务，结合思维链推理自适应确定视觉证据或背景知识的使用，并引入高斯风险感知框扰动技术处理边界框预测的不稳定性。

**Result:** 在Twitter-GMNER和Twitter-FMNERG基准测试上，E2E-GMNER相比现有方法表现出极具竞争力的性能，验证了统一端到端优化和噪声感知定位监督的有效性。

**Conclusion:** 统一的多模态大语言模型框架结合噪声鲁棒的监督策略能够有效提升跨模态实体理解任务的性能，为多模态信息抽取提供新的解决思路。
...
### [335] [Towards Joint Quantization and Token Pruning of Vision-Language Models](https://arxiv.org/abs/2604.17320)
*Xinqing Li,Xin He,Xindong Zhang,Ming-Ming Cheng,Lei Zhang,Yun Liu*

Main category: cs.CV

> 这篇论文提出了QUOTA框架，统一了低比特量化和视觉词元剪枝，在W4A4量化下仅保留30%视觉词元就实现了95.65%的平均保留率，优于分阶段基线方法。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型在激进低比特推理下面临挑战，因为预填充时的长视觉词元前缀和自回归解码时增长的KV缓存主导了推理成本。传统的词元剪枝和低比特量化方法分阶段组合容易因校准不匹配而变得脆弱。

**Method:** 提出协同量化和剪枝框架QUOTA，将低比特校准信号转换为分层词元分配计划并作为剪枝方案。在部署的W4A4操作符下，结合激活幅度、注意线索和显式低比特风险信号评估词元重要性，实现一致的有预算top-k选择。

**Result:** 在标准VLM基准测试中，相同低比特机制下相比分阶段基线方法展现出更强的鲁棒性，只保留30%视觉词元就达到95.65%平均保留率，而代表性分阶段组合约为94.3%。

**Conclusion:** QUOTA框架成功统一了低比特推理和确定性视觉词元剪枝，在保持模型性能的同时显著降低了计算成本，为VLM的高效部署提供了新的解决方案。
...
### [336] [R-FLoRA: Residual-Statistic-Gated Low-Rank Adaptation for Single-Image Face Morphing Attack Detection](https://arxiv.org/abs/2604.17321)
*Raghavendra Ramachandra*

Main category: cs.CV

> 论文提出了一种结合高频拉普拉斯残差统计和冻结视觉transformer的单图像人脸变体攻击检测框架S-MAD，通过R-FLoRA和Res-FiLM等技术提升检测精度和泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 人脸变体攻击对护照签发、边境控制和数字身份验证系统中的人脸识别可靠性构成重大威胁，单图像检测面临缺乏可信参考和攻击方法多样化的挑战。

**Method:** 集成高频拉普拉斯残差统计与冻结的基础级视觉transformer表示，采用残差统计门控低秩适配器(R-FLoRA)和特征级残差融合(Res-FiLM)，引入残差对比对齐损失正则化融合token空间。

**Result:** 在四个符合ICAO标准的数据集上（涵盖七种变体生成技术），方法在检测精度和跨域泛化方面持续超越9个最新S-MAD算法，实现实时效率和可解释性。

**Conclusion:** 提出的S-MAD框架在保持冻结骨干网络和最小可训练参数的同时，有效提升了人脸变体攻击检测性能，适用于生物识别验证系统的实际应用场景。
...
### [337] [Robust Diabetic Retinopathy Grading Using Dual-Resolution Attention-Based Deep Learning with Ordinal Regression](https://arxiv.org/abs/2604.17341)
*Afshan Hashmi*

Main category: cs.CV

> 本研究提出了一个双分辨率深度学习框架，结合注意力特征融合和有序回归，提高糖尿病视网膜病变分级模型的跨数据集泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 糖尿病视网膜病变是全球视力损伤的主要原因，虽然深度学习在自动分级中发挥重要作用，但模型在不同成像条件下的数据集间表现会显著下降，需要提高模型的跨域泛化能力。

**Method:** 采用两个并行的EfficientNet骨干网络在不同空间分辨率下提取互补的视网膜特征，通过可学习的注意力机制自适应融合多分辨率表示，使用基于累积链接模型(CORAL)的有序回归方法处理DR严重程度的有序性，并应用圆形裁剪、对比度增强和直方图匹配的预处理策略来减小数据集间的域差异。

**Result:** 在APTOS 2019数据集上训练，模型在APTOS验证集上获得0.88的二次加权 kappa，在未见过的Messidor-2测试集上获得0.68的 kappa，显示出良好的跨数据集鲁棒性。

**Conclusion:** 所提出的双分辨率框架通过结合多尺度特征提取、注意力融合和有序回归学习，有效提升了DR分级模型在不同数据集间的泛化能力，为大规模筛查应用提供了可靠的解决方案。
...
### [338] [When Text Hijacks Vision: Benchmarking and Mitigating Text Overlay-Induced Hallucination in Vision Language Models](https://arxiv.org/abs/2604.17375)
*Cui Yakun,Xingqun Qi,TianTian Geng,Yuyao Zhang,Sirui Han,Yike Guo*

Main category: cs.CV

> 本文提出了Text Overlay-Induced Hallucination (TOIH)问题，构建了VisualTextTrap基准评测集，并开发了VTHM-MoE框架来缓解视觉语言模型在处理文本覆盖与视觉内容冲突时的幻觉现象。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉语言模型在处理屏幕上覆盖文本与视觉场景矛盾时会系统性地产生幻觉，过度依赖覆盖文本的语义而忽略实际的视觉内容，这一关键问题尚未得到足够重视。

**Method:** 构建了包含6,057个人工验证样本的VisualTextTrap基准测试集，并提出了VTHM-MoE框架，采用双编码器架构，包含时间、动作、对象和空间四个维度的专家模块，通过自适应令牌路由策略实现动态专家分配。

**Result:** 在VisualTextTrap基准上的广泛实验表明，VTHM-MoE在各种视频问答任务中超越了现有最先进方法，有效缓解了TOIH问题，同时保持了在未污染视频上的性能。

**Conclusion:** 本研究首次系统性地定义并解决了视频理解中覆盖文本诱导的幻觉问题，为视觉语言模型的鲁棒性提升提供了新的评估基准和解决方案。
...
### [339] [Towards Generalizable Deepfake Image Detection with Vision Transformers](https://arxiv.org/abs/2604.17376)
...
> 本文提出了一种基于集成视觉变换器(DINOv2, AIMv2, OpenCLIP ViT-L/14)的通用深度伪造检测方法，在DF-Wild数据集上实现96.77%的AUC和9%的等错率，显著超越现有SOTA方法。

<details>
  <summary>Details</summary>

**Motivation:** 现代生成模型的快速发展和现有方法泛化能力差导致深度伪造图像检测面临挑战，需要开发更通用、鲁棒的检测解决方案。

**Method:** 采用集成学习策略，结合多个微调的视觉变换器模型(DINOv2、AIMv2和OpenCLIP ViT-L/14)，并在多样化的DF-Wild数据集上进行训练和验证。

**Result:** 集成方法在DF-Wild测试集上达到96.77%的AUC和9%的等错率，分别比SOTA算法Effort提升7.05%和8%，该方案获得IEEE SP Cup 2025冠军。

**Conclusion:** 基于视觉变换器模型的集成方法是提高深度伪造检测泛化能力的有效途径，能够适应多种生成技术和操作方法，为实际应用提供了可靠的检测方案。
...
### [340] [SpatialImaginer: Towards Adaptive Visual Imagination for Spatial Reasoning](https://arxiv.org/abs/2604.17385)
*Yian Li,Yang Jiao,Bin Zhu,Tianwen Qian,Shaoxiang Chen,Jingjing Chen,Yu-Gang Jiang*

Main category: cs.CV

> SpatialImaginer是一个多模态框架，通过结合文本推理和视觉想象来解决多模态大语言模型在空间智能任务中的脆弱性问题，显著提升了空间推理的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有多模态大语言模型在需要一致空间状态识别的空间智能任务中表现脆弱，主要原因是空间识别机制与纯文本推理行为之间的不匹配——文本表示会抽象掉关键的底层几何结构细节。

**Method:** 提出了SpatialImaginer框架，采用分治策略：使用文本思维链进行高层语义规划，利用视觉想象进行几何敏感的状态转换和一致性保持。引入难度感知数据引擎和闭环验证机制来训练模型选择性地调用视觉想象。

**Result:** 在多样化的空间智能基准测试中，SpatialImaginer达到了最先进的性能，并在复杂多步空间推理任务上显著提高了鲁棒性。

**Conclusion:** 通过文本推理与视觉想象的统一框架，SpatialImaginer有效解决了多模态大语言模型在空间智能任务中的核心挑战，为空间推理任务提供了新的解决思路。
...
### [341] [Deep learning based Non-Rigid Volume-to-Surface Registration for Brain Shift compensation Using Point Cloud](https://arxiv.org/abs/2604.17389)
*Eashrat Jahan Muniya,Gernot Kronreif,Ander Biguri,Wolfgang Birkfellner,Sepideh Hatamikia*

Main category: cs.CV

> 提出一种基于深度学习的非刚性体积-表面配准框架，通过稀疏的术中等部分表面观测估计密集位移场，无需体积成像即可实现脑移位补偿。

<details>
  <summary>Details</summary>

**Motivation:** 图像引导神经外科手术中，脑移位导致术中解剖结构与术前影像存在显著偏差，现有补偿方法需要术中MRI/CT/超声，会中断手术流程；部分表面点云配准虽不中断手术，但在部分观察和噪声条件下变形估计极具挑战性。

**Method:** 构建深度学习框架实现非刚性体积-表面配准，采用多尺度点云特征提取和分层形变解码器，将部分术中表面信息整合到完整的术前点云域中，实现隐式对应学习和密集形变恢复。

**Result:** 在挑战性的部分表面条件下，实现了精细尺度变形的准确恢复，端点误差(EPE)为1.13±0.75 mm，均方根误差(RMSE)为1.33±0.81 mm。

**Conclusion:** 所提方法支持从稀疏表面观测进行自动、工作流程兼容的脑移位补偿，为图像引导神经外科手术提供了实用的解决方案。
...
### [342] [MESA: A Training-Free Multi-Exemplar Deep Framework for Restoring Ancient Inscription Textures](https://arxiv.org/abs/2604.17390)
*Vasileios Toulatzis,Ioannis Fudos*

Main category: cs.CV

> MESA是一种基于多示例风格感知的古铭文图像修复方法，利用完整铭文示例通过Gram矩阵特征匹配逐层重建损坏区域，有效解决了古铭文碎片化或腐蚀带来的缺失问题。

<details>
  <summary>Details</summary>

**Motivation:** 古代铭文常因碎片化、腐蚀等损坏导致文本缺失，严重影响阅读和分析。现有图像修复方法在铭文恢复方面存在局限性，需要专门针对铭文特点开发更有效的修复方法。

**Method:** MESA方法通过VGG19卷积特征编码为Gram矩阵捕捉示例的纹理、风格和笔画结构；对每个网络层选择与损坏输入均方位移(MSD)最小的示例；利用OCR估计的字符宽度推导层级贡献权重；使用训练掩码保护完整区域，限制合成仅在损坏区域进行。

**Result:** 对比实验证明了MESA方法的优越性，能够更好地恢复古铭文图像的缺失部分，保持文字的风格和结构特征。

**Conclusion:** MESA成功解决了古铭文图像修复中的关键问题，为铭文研究和保护提供了有效的技术手段，并为根据可用示例和元数据选择合适的修复策略提供了实践指导。
...
### [343] [Speculative Decoding for Autoregressive Video Generation](https://arxiv.org/abs/2604.17397)
*Yuezhou Hu,Jintao Zhang*

Main category: cs.CV

> SDVG是一种将投机解码技术应用于自回归视频扩散模型的无训练加速框架，通过图像质量路由器替代传统令牌验证，实现1.59-2.09倍推理加速

<details>
  <summary>Details</summary>

**Motivation:** 自回归视频扩散模型在流式视频合成中表现良好，但推理速度较慢。传统的投机解码技术在大语言模型中效果显著，但无法直接应用于视频生成，因为视频块是连续时空张量，缺乏用于精确拒绝采样的令牌级分布。

**Method:** SDVG采用1.3B轻量级草稿模型通过4个去噪步骤提议候选块，将每个块经VAE解码后用ImageReward进行评分（采用最差帧聚合检测单帧伪影），设定固定阈值τ过滤高质量块进入14B目标模型的KV缓存，其余块由目标模型重新生成。

**Result:** 在MovieGenVideoBench的1003个提示上，τ=-0.7时SDVG保留98.1%质量（0.0773 vs 0.0788）并实现1.59倍加速，95.7%质量保留时可达2.09倍加速，且一致性地比纯草稿生成提升17%以上。

**Conclusion:** SDVG框架无需训练、无需架构修改，可无缝集成到现有自回归视频生成流程中，通过单一参数τ实现质量-速度的有效权衡，为视频推理加速提供了新的实用方案。
...
### [344] [Where to Focus: Query-Modulated Multimodal Keyframe Selection for Long Video Understanding](https://arxiv.org/abs/2604.17422)
*Shaoguang Wang,Weiyu Guo,Ziyang Chen,Xuming Hu,Hui Xiong*

Main category: cs.CV

> Q-Gate是一种即插即用、无需训练的框架，通过查询调制的门控机制动态选择关键帧，有效提升多模态大模型的长视频理解能力。

<details>
  <summary>Details</summary>

**Motivation:** 长视频理解因处理密集帧序列的计算成本过高而极具挑战性。现有的关键帧选择方法要么仅依赖单一的视觉中心指标，要么静态融合启发式分数，这种'一刀切'方式在情节驱动的叙事查询和纯视觉任务中都表现不佳，存在严重的'模态噪声'问题。

**Method:** Q-Gate将关键帧选择视为动态模态路由问题，解耦为三个轻量级专家流：视觉接地（局部细节）、全局匹配（场景语义）和上下文对齐（字幕驱动叙事）。核心是查询调制门控机制，利用大语言模型的上下文推理能力动态分配专家注意力权重，智能激活必要模态而屏蔽无关模态。

**Result:** 在LongVideoBench和Video-MME基准测试上，Q-Gate在多个MLLM骨干网络上显著优于现有最先进方法。通过有效抑制模态特定噪声，为可扩展的视频推理提供了鲁棒且高度可解释的解决方案。

**Conclusion:** Q-Gate通过动态模态路由解决了长视频理解中的关键帧选择瓶颈，实现了任务感知的智能帧选择，为多模态模型处理长视频提供了高效、可扩展的新思路。
...
### [345] [Long-CODE: Isolating Pure Long-Context as an Orthogonal Dimension in Video Evaluation](https://arxiv.org/abs/2604.17428)
*Zhijiang Tang,Jiaxin Qi,Bing Zhao,Jianqiang Huang*

Main category: cs.CV

> 本文提出了一种专门评估长视频生成质量的新框架和指标，解决了传统短视频评估指标无法捕捉长视频叙事连贯性、全局因果一致性等关键特征的问题。

<details>
  <summary>Details</summary>

**Motivation:** 随着视频生成模型能够生成越来越长的视频，现有的评估指标主要关注短视频的帧级视觉质量和局部时序平滑性，无法评估长视频的叙事丰富度和全局因果一致性等关键特征，迫切需要专门的长视频评估框架。

**Method:** 1. 提出长视频属性损坏测试，现有短视频指标对结构不一致性不敏感；2. 基于镜头动力学设计新的长视频评估指标；3. 构建Long-CODE数据集，包含针对长距离特征的人工标注。

**Result:** 实验表明提出的指标与人类判断达到最先进的关联性，建立了一个完整无偏的视频生成模型评估范式。

**Conclusion:** 短时视觉感知和长上下文属性是正交维度，长视频指标应与短视频评估解耦。提出的metrics和数据集有效补充了现有短视频标准，为视频生成评估提供了全面框架。
...
### [346] [DEM Refinement and Validation on the Lunar Surface Using Shape-from-Shading with Chandrayaan-2 OHRC Imagery](https://arxiv.org/abs/2604.17436)
*Aaranay Aadi,Jai Gopal Singla,Nitant Dube*

Main category: cs.CV

> 本研究提出了一种基于Shape from Shading的框架，利用Chandrayaan-2轨道器高分辨率相机图像增强亚米级月球数字高程模型。

<details>
  <summary>Details</summary>

**Motivation:** 传统的立体摄影测量方法受基线长度限制，难以生成高分辨率的月球地形数据。需要一种不受立体基线约束的新方法来提取更精细的地形特征。

**Method:** 开发了一个三阶段SfS框架，对同一区域独立获取的OHRC图像施加SfS算法，通过系统性的SfS平滑权重参数扫描来优化地形重建结果。该方法在Cyrillus陨石坑、Vikram着陆区和月球南极Mons Mouton三个地点进行了验证。

**Result:** 实现了可测量的地形增强效果，特别是在地表坡度统计方面，揭示出之前未解析的精细尺度陨石坑形态。同时也识别出大俯仰角分离会导致SfS敏感性降低的限制情况。

**Conclusion:** SfS不仅可作为地形精修工具，更可作为独立的地形数据源，为高分辨率月球地形测绘提供了新的技术途径，但需要注意阴影图像覆盖范围和观测几何角度的影响。
...
### [347] [Attention Is not Everything: Efficient Alternatives for Vision](https://arxiv.org/abs/2604.17439)
*Nur Mohammad Kazi,Ibteshum Khaled,Md. Luthful Hasan Galib,Ali Faruk Shihab,Md. Rakibul Islam*

Main category: cs.CV

> 本文综述了40篇非Transformer计算机视觉方法，将其分类为卷积、MLP、状态空间等模型，并从效率、扩展性、可解释性和鲁棒性维度进行分析，旨在为未来计算机视觉研究提供挑战和机遇的洞察。

<details>
  <summary>Details</summary>

**Motivation:** 尽管Transformer模型在计算机视觉领域取得了显著进展，但许多非Transformer方法仍然表现良好并形成直接竞争。需要对这些方法进行系统性梳理和分析。

**Method:** 通过建立全面的分类学框架，将40篇论文中的非Transformer方法组织成卷积、MLP、状态空间等多个类别，并从效率、扩展性、可解释性和鲁棒性四个维度进行评估分析。

**Result:** 完成了对40篇非Transformer计算机视觉方法的系统性分类和多维度分析，识别了各类方法的特点和性能差异。

**Conclusion:** 为非Transformer计算机视觉方法提供了全面的视角，揭示了未来研究面临的挑战和发展机遇，为该领域的进一步发展指明了方向。
...
### [348] [HyKey: Hyperspectral Keypoint Detection and Matching in Minimally Invasive Surgery](https://arxiv.org/abs/2604.17446)
...
> HyKey是一种基于高光谱图像的混合3D-2D卷积神经网络模型，用于微创手术关键点检测和匹配，在纹理缺乏的手术环境中实现了比传统RGB方法更好的3D重建性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统RGB-based关键点检测在微创手术中面临纹理缺失和光照复杂等挑战，影响手术导航和3D重建的准确性。高光谱成像能够提供丰富的光谱信息，有望解决这些局限性。

**Method:** 开发了HyKey模型，采用混合3D-2D卷积神经网络联合提取高光谱图像的空间-光谱特征，使用合成单应性增强和极线几何约束在机器人获取的双相机RGB-HSI腹腔镜数据集上进行训练。

**Result:** HSI模型在匹配准确率达到96.62%，10度姿态估计平均准确率为67.18%，在多个评估指标上均优于SuperPoint和ALIKE等RGB基准方法。

**Conclusion:** 高光谱成像的光谱信息为微创手术的单目3D重建提供了有效解决方案，通过增强光谱-空间特征区分度克服了纹理缺乏手术环境的局限性。
...
### [349] [SegTTA: Training-Free Test-Time Augmentation for Zero-Shot Medical Imaging Segmentation](https://arxiv.org/abs/2604.17451)
*Yihong Yao,Chunlei Li,Canxuan Gang,Wenzhi Hu,Zeyu Zhang,Hao Zhang,Xiaoyan Li*

Main category: cs.CV

> SegTTA是一个无需重训练的医学图像分割框架，通过组合四种数据增强技术和加权投票机制，在三个不同医学数据集上实现了分割性能的显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 医学图像质量因设备和操作者差异而变化，影响模型性能。虽然现有模型具有较强泛化能力，但仍需要针对低质图像进行优化，且避免模型重训练的复杂性和成本。

**Method:** SegTTA框架结合Gamma校正、对比度增强、高斯模糊和高斯噪声四种增强技术，通过多个MedSAM2检查点的加权投票机制进行测试时增强，不改变原始模型结构。

**Result:** 在健康子宫分割、子宫肌瘤检测和多类肝脏结构分割三个数据集上持续提升性能。在多类肝脏血管数据集上，mIoU提升1.6，aIoU提升1.9，HD95降低约2.0。

**Conclusion:** SegTTA证明了测试时增强在医学图像分割中的有效性，消融研究表明大器官受益于强度增强小病灶需要噪声增强，投票阈值可根据临床需求进行任务特定优化。
...
### [350] [HSG: Hyperbolic Scene Graph](https://arxiv.org/abs/2604.17454)
*Liyang Wang,Zeyu Zhang,Hao Tang*

Main category: cs.CV

> HSG是一种在双曲空间中学习场景图嵌入的方法，通过几何距离自然编码层次关系，在保持检索性能的同时显著提升了层次结构质量，Graph IoU达到33.51。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法在欧几里得空间学习场景图嵌入，无法显式捕获地点和对象之间的层次蕴含关系，限制了学习表示的结构一致性。

**Method:** 提出双曲场景图(HSG)方法，在双曲空间中学习场景图嵌入，利用几何距离自然编码层次关系，替代传统的对比学习和注意力关联机制。

**Result:** HSG在图级别指标上取得最大提升，PP IoU达到33.17，Graph IoU达到33.51，比最佳AoMSG变体提升8.14，同时保持了强大的检索性能。

**Conclusion:** 双曲表示学习对场景图建模非常有效，能够在保持检索性能的同时显著改善层次结构质量，为结构化视觉理解提供了新的几何视角。
...
### [351] [From Adaptation to Generalization: Adaptive Visual Prompting for Medical Image Segmentation](https://arxiv.org/abs/2604.17455)
*Evren Çetinkaya,Sangmin Lee,Jung Uk Kim,Hong Joo Lee,Nassir Navab*

Main category: cs.CV

> APEX是一种基于自适应提示提取的框架，通过从可学习的提示记忆库中检索输入特定的提示，显著提升了模型在医学图像分割任务中跨域泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉提示方法通常为每个域优化单一提示并统一应用于所有输入，这种方法无法有效处理域内和域间的变异性，在医学领域这一局限尤为突出。

**Method:** 提出APEX框架，利用傅里叶频谱提取的域特征从可学习的提示记忆库中检索输入特定的提示；引入低频特征对比学习(LFC)框架，通过聚类同域特征并分离不同域特征来学习鲁棒的域特征。

**Result:** 在两个医学分割任务上的广泛实验表明，APEX在已见和未见域上都显著提升了泛化能力；该方法可与任意现有骨干网络兼容，持续提升性能，是有效的即插即用解决方案。

**Conclusion:** APEX成功解决了传统提示方法在处理医学图像域变异性方面的限制，通过自适应提示提取和有效的域特征学习，为医学图像分割提供了一种强大且通用的提示解决方案。
...
### [352] [UniMesh: Unifying 3D Mesh Understanding and Generation](https://arxiv.org/abs/2604.17472)
*Peng Huang,Yifeng Chen,Zeyu Zhang,Hao Tang*

Main category: cs.CV

> UniMesh是一个统一的3D视觉框架，将3D生成与理解任务整合在单一架构中，通过创新的网格头、链式网格和自反思机制实现了知识的有效转移和场景的整体建模。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3D视觉模型通常专门处理3D理解（如形状分类）或3D生成（如合成）任务，导致架构和表示的碎片化，阻碍了知识迁移和整体场景建模。

**Method:** 提出三个关键技术：1）创新的网格头作为跨模型接口，连接扩散图像生成与隐式形状解码器；2）链式网格(CoM)几何实例化迭代推理，实现用户驱动的语义网格编辑；3）基于行动者-评估者-自反思三元组的自反思机制，诊断并修正高级任务（如3D字幕）中的失败。

**Result:** 实验表明UniMesh不仅 在标准基准测试中达到竞争性性能，还在迭代编辑以及生成与理解之间的相互加强方面解锁了新能力。

**Conclusion:** UniMesh成功实现了3D生成与理解的统一学习架构，为3D视觉任务的整体建模和知识迁移提供了有效的解决方案，展现了在实际应用中的潜力。
...
### [353] [Dual-Anchoring: Addressing State Drift in Vision-Language Navigation](https://arxiv.org/abs/2604.17473)
*Kangyi Wu,Pengna Li,Kailin Lyu,Lin Zhao,Qingrong He,Jinjun Wang,Jianyi Liu*

Main category: cs.CV

> 该论文提出双锚定框架解决视觉语言导航中的状态漂移问题，通过指令进度锚定和记忆地标锚定，在长轨迹场景下成功率提升15.2%。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言导航模型在长场景中容易出现状态漂移问题，导致代理无法区分已完成的子目标与剩余子目标，并且历史表示会退化，使其无法追踪已访问的地标，从而出现漫无目的的漫游。

**Method:** 提出双锚定框架：(1)指令进度锚定，监督代理生成结构化文本来划分已完成和剩余子目标；(2)记忆地标锚定，采用以地标为中心的世界模型回溯预测SAM提取的目标嵌入，迫使代理明确验证过去的观察。

**Result:** 在模拟和真实环境实验中，该方法在成功率上提升15.2%，在长轨迹上获得24.7%的显著提升。论文还构建了两个大规模数据集：360万个带进度描述的样本和93.7万个地标数据。

**Conclusion:** 双锚定框架有效解决了VLN中的进度漂移和记忆漂移问题，显著提升了导航性能，特别是在长轨迹场景中表现突出，为后续研究提供了新的思路和资源。
...
### [354] [Unveiling Deepfakes: A Frequency-Aware Triple Branch Network for Deepfake Detection](https://arxiv.org/abs/2604.17477)
*Qihao Shen,Jiaxing Xuan,Zhenguang Liu,Sifan Wu,Yutong Xie,Zhaoyan Ming,Yingying Jiao,kui Ren*

Main category: cs.CV

> 本文提出了一种三分支网络架构，结合空间和频率特征进行深度伪造检测，并基于互信息理论设计特征解耦和融合损失，在六个基准数据集上取得了最先进的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有深度伪造检测方法主要关注单一或少数特定频率域，容易过拟合特定伪影；同时不同特征往往关注同一伪造区域，导致特征表示冗余，限制了模型对多样化篡改的泛化能力。

**Method:** 提出三分支网络结构，联合捕获原始图像和不同频率通道重建图像的空间与频率特征；基于互信息理论推导特征解耦和融合损失，增强模型对任务相关特征的关注。

**Result:** 在六个大规模基准数据集上的广泛实验表明，该方法始终如一地实现了最先进的检测性能。

**Conclusion:** 通过多模态特征融合和理论指导的特征优化，有效提升了深度伪造检测的鲁棒性和泛化能力，为解决深度伪造威胁提供了新的技术途径。
...
### [355] [AutoVQA-G: Self-Improving Agentic Framework for Automated Visual Question Answering and Grounding Annotation](https://arxiv.org/abs/2604.17488)
*Rongsheng Hu,Runwei Guan,Yicheng Di,Jiayu Bao,Yuan Liu*

Main category: cs.CV

> AutoVQA-G是一个自我改进的智能体框架，用于自动化生成高质量的视觉问答与定位(VQA-G)数据集，解决了人工标注不可扩展性和现有方法数据不一致的问题。

<details>
  <summary>Details</summary>

**Motivation:** 高质量VQA-G数据集的人工标注对于推进视觉语言模型发展至关重要，但存在成本高、可扩展性差的问题。现有自动化方法面临模型幻觉导致的数据不一致性，以及基于简单启发式的脆弱验证机制两大挑战。

**Method:** AutoVQA-G采用迭代优化循环框架，包含一致性评估模块（使用CoT推理进行细粒度视觉验证）和记忆增强的提示优化代理（分析失败样本的批评意见来逐步优化生成提示），实现自动化数据生成与验证。

**Result:** 实验表明，AutoVQA-G生成的VQA-G数据集在视觉定位准确性方面优于领先的多模态大语言模型，为创建高保真数据以促进更稳健的VLM训练和评估提供了有效途径。

**Conclusion:** AutoVQA-G框架通过智能化的自我改进机制，成功解决了自动化VQA-G标注中的关键挑战，为大规模生成高质量视觉问答数据提供了新的解决方案。
...
### [356] [Coevolving Representations in Joint Image-Feature Diffusion](https://arxiv.org/abs/2604.17492)
*Theodoros Kouzelis,Spyros Gidaris,Nikos Komodakis*

Main category: cs.CV

> 提出CoReDi框架，通过让语义表示空间与扩散模型共同进化，使用轻量级线性投影学习自适应表示，比固定表示空间的联合扩散模型收敛更快、生成质量更高。

<details>
  <summary>Details</summary>

**Motivation:** 现有的联合图像-特征生成建模方法使用固定的表示空间，该空间独立于生成目标构建且训练中保持不变，无法适应生成任务的需求。

**Method:** 提出CoReDi框架，在训练过程中通过学习轻量级线性投影使语义表示空间与扩散模型共同进化，结合stop-gradient目标、归一化和目标正则化确保稳定训练并防止特征崩塌。

**Result:** 在VAE潜在空间扩散和像素空间扩散两个场景中验证，CoReDi相比固定表示空间的联合扩散模型实现了更快的收敛速度和更高的样本生成质量。

**Conclusion:** 自适应的语义表示空间能够逐步专化以满足图像合成的需求，与图像潜在特征形成更好的互补性，为扩散模型训练提供了有效的改进策略。
...
### [357] [Edit Fidelity Field: Semantics-Aware Region Isolation for Training-Free Scene Text Editing](https://arxiv.org/abs/2604.17500)
*Guandong Li,Mengxia Ye*

Main category: cs.CV

> 本文揭示了场景文本编辑中存在的关键问题——编辑溢出效应，并提出了编辑保真度场(EFF)解决方案，将溢出率从94%降至25%。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于扩散模型的场景文本编辑方法虽然能准确渲染目标文本，但存在严重的编辑溢出问题，在编辑目标文本区域时会意外修改非目标区域，特别是邻近文本，这对实际应用造成了重大障碍。

**Method:** 提出了编辑保真度场(EFF)，这是一个语义感知的连续场，通过OCR检测的文本区域构建四区域场：编辑核心（完全可编辑）、过渡区（平滑衰减）、保护区（非目标文本，显式锁定）和背景（严格保留）。EFF作为训练无关、模型无关的后处理模块，可应用于任何基于扩散的STE方法。

**Result:** 实验表明，EFF将编辑溢出率从94%降低到25%，非目标区域保存的PSNR提升了91.4dB。同时提出了单区域溢出量化评估协议，能够独立测量每个非目标文本区域的编辑泄漏程度。

**Conclusion:** 该研究首次系统性地揭示了场景文本编辑中的编辑溢出问题，提出的EFF解决方案在保持目标文本编辑质量的同时，显著减少了对非目标区域的意外修改，为场景文本编辑的实际应用提供了重要保障。
...
### [358] [RS-HyRe-R1: A Hybrid Reward Mechanism to Overcome Perceptual Inertia for Remote Sensing Images Understanding](https://arxiv.org/abs/2604.17504)
*Gaozhi Zhou,Hu He,Peng Shen,Jipeng Zhang,Liujue Zhang,Linrui Xu,Zeyuan Wang,Ziyu Li,Xuezhi Cui,Wang Guo,Haifeng Li*

Main category: cs.CV

> RS-HyRe-R1是一个混合奖励框架，通过三种奖励机制解决遥感视觉语言模型强化学习训练中的'感知惯性'问题，在多个任务上实现了超越更大模型的性能。

<details>
  <summary>Details</summary>

**Motivation:** 遥感视觉语言模型在强化学习后训练时，倾向于依赖局部显著特征进行快速推理，这种现象被称为'感知惯性'。这种偏向导致模型认知上过度依赖特定特征而无法构建完整证据，操作上难以灵活调整视觉焦点。

**Method:** 提出了RS-HyRe-R1混合奖励框架，包含：(1)空间推理激活奖励，强制结构化视觉推理；(2)感知正确性奖励，为不同遥感任务提供自适应质量锚点；(3)视觉-语义路径演化奖励，惩罚重复推理并促进探索互补线索。

**Result:** 该框架有效缓解'感知惯性'，仅用30亿参数就在REC、OVD和VQA任务上达到最先进性能，超越高达70亿参数的模型，在零样本泛化方面分别超越第二优模型3.16%、3.97%和2.72%。

**Conclusion:** RS-HyRe-R1通过创新的混合奖励设计，成功解决了强化学习中模型偏向快速推理的问题，实现了更深入、更多样的推理能力，为遥感视觉理解提供了有效解决方案。
...
### [359] [Dual Strategies for Test-Time Adaptation](https://arxiv.org/abs/2604.17542)
*Nam Nguyen Phuong,Duc Nguyen The Minh,Phi Le Nguyen,Ehsan Abbasnejad,Minh Hoai*

Main category: cs.CV

> DualTTA是一种新型测试时自适应框架，通过自适应选择可靠和不可靠的测试样本，分别进行最小化和最大化熵的自适应，从而更充分地利用测试分布信息。

<details>
  <summary>Details</summary>

**Motivation:** 传统的测试时自适应方法仅使用一小部分低熵预测的测试样本，无法充分利用测试分布中的信息，导致在分布偏移情况下性能受限。

**Method:** DualTTA框架识别两个样本组：预测可能一致的样本组和预测可能错误的样本组。通过新的可靠性标准（基于语义保持和语义改变变换下的预测稳定性）自适应选择样本，对可靠样本最小化预测熵，对不可靠样本最大化熵以抑制过度自信错误。

**Result:** 该方法实现了可靠和不可靠样本的更紧密分离，在适应效果方面提供了理论保证和实证验证，相比传统方法表现更优。

**Conclusion:** DualTTA通过双重熵策略和基于预测稳定性的样本选择机制，显著提升了测试时自适应的效率和鲁棒性，为分布偏移下的模型适应提供了更有效的解决方案。
...
### [360] [UniGeo: Unifying Geometric Guidance for Camera-Controllable Image Editing via Video Models](https://arxiv.org/abs/2604.17565)
*Hong Jiang,Wensong Song,Zongxing Yang,Ruijie Quan,Yi Yang*

Main category: cs.CV

> UniGeo是一个通过在表示、架构和损失函数三个层面注入统一几何引导的相机可控图像编辑框架，有效解决了现有方法在连续相机运动下的几何漂移和结构退化问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有相机可控图像编辑方法存在两个关键问题：一是依赖碎片化的几何引导（如只在表示层注入点云），二是主要基于离散视图映射的图像扩散模型。这些限制共同导致连续相机运动下出现几何漂移和结构退化。

**Method:** UniGeo通过三层统一几何引导解决上述问题：1）表示层：采用帧解耦几何参考注入机制提供鲁棒的跨视图几何上下文；2）架构层：引入几何锚注意力对齐多视图特征；3）损失函数层：提出轨迹端点几何监督策略显式强化目标视图的结构保真度。

**Result:** 在多个公共基准测试中，包括广泛和有限的相机运动设置，UniGeo在视觉质量和几何一致性方面都显著优于现有方法。

**Conclusion:** 通过在决定生成输出的三个关键（表示、架构、损失函数）层面系统地注入统一几何引导，UniGeo成功实现了高质量的相机可控图像编辑，有效保持了跨视图的几何一致性。
...
### [361] [Multi-Camera Self-Calibration in Sports Motion Capture: Leveraging Human and Stick Poses](https://arxiv.org/abs/2604.17567)
*Fan Yang,Changsoo Jung,Ryosuke Kawamura,Hon Yung Wong*

Main category: cs.CV

> 本文提出了一种无需专用工具的多摄像机外参标定方法，利用人体关键点和已知长度的棍状器械实现体育运动场景下的高效标定，并发布了相关数据集。

<details>
  <summary>Details</summary>

**Motivation:** 多摄像机系统在体育运动中广泛应用，但现有外参标定方法成本高昂且劳动密集，亟需一种无需专用标定工具的高效解决方案。

**Method:** 提出三阶段优化流程，联合利用同步多视角视频中的两个互补线索：未知度量尺度的人体关键点和已知长度的刚性棍状器械，通过棍棒长度约束解决全局尺度问题。

**Result:** 在包含4种运动类别和3-10台摄像机的合成数据集上实现了最先进的标定性能，旋转和平移误差均较低，无需专用标定工具即可获得准确的外参标定结果。

**Conclusion:** 该方法为基于棍状器械的体育运动提供了一种实用、高效的多摄像机自标定解决方案，并通过发布首个相关数据集推动了该领域的研究发展。
...
### [362] [PBSBench: A Multi-Level Vision-Language Framework and Benchmark for Hematopathology Whole Slide Image Interpretation](https://arxiv.org/abs/2604.17570)
*Yuanlong Wang,Weichi Chen,Adrian Rajab,Wenfang Liu,Yulan Jin,Andrew Srisuwananukorn,Ping Zhang*

Main category: cs.CV

> 本研究构建了首个外周血涂片(PBS)多模态数据集PBSInstr和专用视觉语言模型PBS-VL，用于细胞和切片级别的血液病理学理解。

<details>
  <summary>Details</summary>

**Motivation:** 现有病理学多模态大语言模型主要针对固体组织切片设计，无法很好地泛化到外周血涂片的单细胞形态分析，而PBS分析与固体组织病理在视觉特征和诊断推理上都存在显著差异。

**Method:** 构建PBSInstr数据集（353个PBS全切片图像及显微印象描述、29k细胞级图像标注、27k+1.3k个QA对），基于此训练PBS-VL视觉语言模型，并建立PBSBench评估基准（包含4类问题和6项解释任务）。

**Result:** 实验表明PBS-VL在血液病理理解任务上优于现有通用和病理学多模态大语言模型，证明了PBS专用数据的价值。

**Conclusion:** 本研究为血液病理学AI辅助诊断系统奠定了基础，通过开源代码、数据集和模型权重推进相关研究发展。
...
### [363] [DGSSM: Diffusion guided state-space models for multimodal salient object detection](https://arxiv.org/abs/2604.17585)
*Suklav Ghosh,Arijit Sur,Pinaki Mitra*

Main category: cs.CV

> 本研究提出DGSSM框架，通过扩散模型引导的状态空间建模方法，实现多模态显著目标检测，在13个基准数据集上超越现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的卷积、Transformer和Mamba模型难以同时建模长距离上下文依赖和细粒度结构细节。扩散模型虽能捕获强结构先验，但在判别性密集预测中计算成本高且集成困难。

**Method:** 设计DGSSM扩散引导状态空间框架，将扩散结构先验与多尺度状态空间编码、自适应显著性提示和迭代Mamba扩散细化机制相结合，并引入边界感知细化头和自蒸馏策略。

**Result:** 在RGB、RGB-D和RGB-T设置的13个公共基准测试中，DGSSM在多个评估指标上持续优于最先进方法，同时保持紧凑的模型规模。

**Conclusion:** 扩散引导状态空间建模是解决多模态密集预测任务的有效且可泛化的范式，能够在保持计算效率的同时提升边界精度和空间一致性。
...
### [364] [ViPS: Video-informed Pose Spaces for Auto-Rigged Meshes](https://arxiv.org/abs/2604.17623)
...
> ViPS是一种从预训练视频扩散模型中获取运动先验，为3D骨骼绑定网络生成有效姿态空间的前馈框架。

<details>
  <summary>Details</summary>

**Motivation:** 传统运动学绑定缺乏对合理关节配置流形的表示，导致随机采样或手动操作容易产生语义错误（如解剖学过度伸展）和几何问题（如自身穿透）。

**Method:** 通过蒸馏预训练视频扩散模型的运动先验来发现有效姿态分布；使用可微几何验证器对蒙皮网格施加资产特定约束；学习了平滑、紧凑且可控的姿态空间。

**Result:** ViPS在合理性和多样性上与最先进的基于人工创作4D数据的方法相匹配，并展现出对未知物种和骨骼拓扑的强大零样本泛化能力。

**Conclusion:** 该方法成功将生成式2D先验与结构化3D运动学控制连接起来，为3D动画创作提供了更可靠和通用的姿态空间建模方案。
...
### [365] [FlowC2S: Flowing from Current to Succeeding Frames for Fast and Memory-Efficient Video Continuation](https://arxiv.org/abs/2604.17625)
*Hovhannes Margaryan,Quentin Bammey,Christian Sandor*

Main category: cs.CV

> FlowC2S是一种快速且内存高效的视频延续生成方法，通过学习当前视频块到后续视频块之间的向量场，实现了从现有帧直接流向后续帧的生成，在保持高质量的同时显著减少了计算开销。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频延续生成方法通常需要将当前帧与噪声结合来生成视频，这种方法输入维度高、计算成本大。为了解决这一问题，研究者提出了一种更直接、更高效的生成方法。

**Method:** FlowC2S方法通过微调预训练的文本到视频流模型来学习向量场。两个关键技术：1) 使用相邻视频块作为真实最优耦合的实用代理，产生更直接的流；2) 引入目标反转技术，将目标块的倒置潜在变量注入输入表示以增强对应关系并提高视觉保真度。

**Result:** 该方法基于LTXV和Wan模型微调后，在FID和FVD等定量评估指标上超越了现有最佳性能，且仅需5次神经网络函数调用即可生成高质量视频延续。

**Conclusion:** FlowC2S通过直接从当前帧流向后续帧的创新设计，将模型输入维度降低了一半，实现了快速、内存高效且高质量的视频延续生成，为视频生成领域提供了一个实用且高效的解决方案。
...
### [366] [BioVLM: Routing Prompts, Not Parameters, for Cross-Modality Generalization in Biomedical VLMs](https://arxiv.org/abs/2604.17629)
*Mainak Singha,Tanisha Gupta,Ankit Jha,Muhammad Haris Khan,Sayantani Ghosh,Biplab Banerjee*

Main category: cs.CV

> BioVLM是一个基于提示学习的生物医学视觉语言模型框架，通过动态提示选择和知识蒸馏提升跨域泛化能力，在少监督和模态差异大的情况下表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 现有的生物医学视觉语言模型（如BioMedCLIP）在处理类间边界小、获取变化明显的困难模态时性能下降，特别是在少样本监督和模态先验与预训练语料库差异较大的情况下。

**Method:** BioVLM采用提示学习框架，学习多样化提示库并引入动态提示选择机制，通过预测分布的低熵标准选择最具区分性的提示，结合高置信度LLM属性蒸馏和强/弱增强一致性约束来实现鲁棒知识转移。

**Result:** 在11个MedMNIST+ 2D数据集上，BioVLM在三种不同的泛化设置中均取得了新的最先进性能。

**Conclusion:** BioVLM通过轻量化训练和高效推理，实现了模态自适应的提示选择，能够 transfer到未见过的类别和域，有效解决了现有生物医学VLM在困难模态下的泛化问题。
...
### [367] [Infrastructure-Centric World Models: Bridging Temporal Depth and Spatial Breadth for Roadside Perception](https://arxiv.org/abs/2604.17651)
*Siyuan Meng,Chengbo Ai*

Main category: cs.CV

> 本文提出以基础设施为中心的世界模型(I-WM)愿景，利用路侧系统的鸟瞰视角和多传感器优势，通过时空互补性实现与车载模型的协同，并引入基础设施VLA(I-VLA)架构。

<details>
  <summary>Details</summary>

**Motivation:** 现有自动驾驶世界模型仅采用自车视角，忽视了基础设施视角的独特优势。路侧系统具备固定的鸟瞰多传感器持续视角，能与移动车辆形成时空互补，提供更完整的环境感知能力。

**Method:** 提出三阶段的I-WM框架：(I)带质量感知不确定性传播的生成式场景理解，(II)基于物理的多智能体反事实推理预测动力学，(III)通过潜在空间对齐的V2X协同世界模型。采用双层架构和渐进式传感器策略。

**Result:** 建立了驾驶世界模型分类体系，定位I-WM与JEPA、空间智能和VLA架构的关系，提出I-VLA作为路侧感知、语言指令和交通控制的新颖统一，并为各阶段识别了开源基础。

**Conclusion:** I-WM为基础设施赋能理解和预测交通提供了新路径，通过时空协同的路侧-车载世界模型，将推动自动驾驶向更安全、更智能的方向发展。
...
### [368] [Self-Supervised Super-Resolution for Sentinel-5P Hyperspectral Images](https://arxiv.org/abs/2604.17652)
*Hyam Omar Ali,Antoine Crosnier,Romain Abraham,Baptiste Combelles,Fabrice Jégou,Bruno Galerne*

Main category: cs.CV

> 本文提出了一种针对Sentinel-5P卫星数据的自监督超分辨率框架，无需真实高分辨率地面真值即可训练，通过SURE损失函数和等变成像约束实现与监督学习方法相当的性能。

<details>
  <summary>Details</summary>

**Motivation:** Sentinel-5P在大气监测中发挥关键作用，但其空间分辨率限制了细尺度分析。现有超分辨率方法依赖监督学习和合成低分辨率数据，缺乏真实高分辨率参考，限制了在真实观测中的应用。

**Method:** 结合Stein无偏风险估计器(SURE)和等变成像约束，集成S5P降质算子和从信噪比元数据导出的噪声统计。设计了深度可分离卷积U-Net架构以提高效率和光谱保真度。

**Result:** 在多个波段的实验中，自监督模型实现了与监督方法相当的性能。定性分析显示相比双三次插值有改善的空间细节，EMIT数据验证证实重构结构物理有意义。

**Conclusion:** 成功开发了一种无需HR地面真值的自监督超分辨率框架，为Sentinel-5P数据提供了实用的细尺度分析解决方案，在保持强一致性的同时扩展了Super Resolution的应用范围。
...
### [369] [Low Light Image Enhancement Challenge at NTIRE 2026](https://arxiv.org/abs/2604.17669)
...
> 本文综述了NTIRE 2026低光图像增强挑战赛的解决方案和最终结果，共有22个团队提交有效作品，展示了该领域的最新进展。

<details>
  <summary>Details</summary>

**Motivation:** 低光条件下的图像存在对比度低、噪声多等问题，导致信息丢失。本次挑战赛旨在识别能够产生更清晰、视觉上更引人注目的图像的有效网络。

**Method:** 通过学习代表性视觉线索来恢复低对比度和噪声图像造成的信息损失，使用新颖的数据集样本对参赛方案进行全面评估。

**Result:** 195人注册第一赛道，153人注册第二赛道，最终22个团队提交有效作品。挑战赛展示了在（联合去噪和）低光图像增强领域的重大技术进步。

**Conclusion:** 该挑战赛集中展示了低光图像增强领域的最新技术水平，证明了通过深度学习方法在复杂条件下恢复低质量图像的有效性，推动该领域持续发展。
...
### [370] [Dual-stream Spatio-Temporal GCN-Transformer Network for 3D Human Pose Estimation](https://arxiv.org/abs/2604.17688)
*Jiawen Duan,Jian Xiang,Zhiqiang Li,Linlin Xue,Wan Xiang*

Main category: cs.CV

> MixTGFormer是一个结合GCN和Transformer的双流时空网络，用于3D人体姿态估计，在Human3.6M和MPI-INF-3DHP数据集上达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的基于Transformer的3D人体姿态估计方法主要关注全局时空关系建模，忽略了局部骨骼关系和通道间的信息交互。

**Method:** 提出双流时空GCN-Transformer网络(MixTGFormer)，通过两个平行通道同时建模人体骨架的时空关系，核心是堆叠的Mixformer模块。Mixformer包含两个不同模式的并行Mixformer Block和SE Layer，将GCN整合到Transformer中，并设计了时间和空间形式。

**Result:** 在Human3.6M和MPI-INF-3DHP两个基准数据集上的广泛评估表明，MixTGFormer分别取得了37.6mm和15.7mm的P1误差，达到了最先进的性能。

**Conclusion:** MixTGFormer通过有效融合全局和局部特征，成功解决了传统方法忽略局部关系和通道交互的问题，为3D人体姿态估计提供了新的解决方案。
...
### [371] [Dynamic Visual-semantic Alignment for Zero-shot Learning with Ambiguous Labels](https://arxiv.org/abs/2604.17710)
*Jiangnan Li,Linqing Huang,Xiaowen Yan,Min Gan,Wenpeng Lu,Jinfu Fan*

Main category: cs.CV

> 提出动态视觉-语义对齐(DVSA)框架，解决零样本学习中标签噪声和模糊性问题，通过双向对齐和对比优化提升模型鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有零样本学习方法假设标签干净，但现实场景中标签噪声和模糊性普遍存在，这严重影响了模型性能，需要鲁棒的解决方案。

**Method:** DVSA框架包含：1) 注意力双向视觉-语义对齐模块，相互校准视觉特征和属性原型；2) 基于互信息的属性级对比优化；3) 动态标签去歧义机制，迭代修正噪声监督，保持语义一致性并缩小实例-标签差距。

**Result:** 在标准基准数据集上的大量实验验证了DVSA在模糊监督下具有更强的性能表现。

**Conclusion:** DVSA有效地解决了零样本学习中的标签模糊性问题，通过动态修正和对齐机制提升了模型的泛化能力和鲁棒性。
...
### [372] [GeGS-PCR: Effective and Robust 3D Point Cloud Registration with Two-Stage Color-Enhanced Geometric-3DGS Fusion](https://arxiv.org/abs/2604.17721)
*Jiayi Tian,Haiduo Huang,Tian Xia,Wenzhe Zhao,Pengju Ren*

Main category: cs.CV

> GeGS-PCR是一种结合几何、颜色和高斯信息的两阶段点云配准方法，通过颜色编码器和Geometric-3DGS模块增强特征表达，在低重叠度场景下实现99.9%的召回率。

<details>
  <summary>Details</summary>

**Motivation:** 传统仅依赖几何特征的点云配准方法在低重叠和不完整场景下效果不佳，需要利用颜色信息来提升配准的鲁棒性。

**Method:** 提出GeGS-PCR两阶段方法：1) 专用颜色编码器提取多级几何颜色特征；2) Geometric-3DGS模块编码彩色超点的局部邻域信息，确保全局不变的几何-颜色上下文；使用LORA优化保持3DGS表达能力；采用可微分渲染和联合光度损失优化配准过程。

**Result:** 在Color3DMatch和Color3DLoMatch数据集上达到SOTA性能：配准召回率99.9%，相对旋转误差0.013，相对平移误差0.024，精度提升至少2倍。

**Conclusion:** 通过有效融合几何、颜色和高斯信息，GeGS-PCR在极具挑战性的低重叠点云配准任务中实现了突破性性能提升，为彩色点云配准提供了新的解决方案。
...
### [373] [Voronoi-guided Bilateral 2D Gaussian Splatting for Arbitrary-Scale Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2604.17727)
*Jie Zhang,Jinkun You,Shi Chen,Yicong Zhou*

Main category: cs.CV

> GaussianHSI是一种基于高斯散点的任意尺度高光谱图像超分辨率方法，通过Voronoi引导的双边高斯散点进行空间重建，并结合光谱细节增强模块，实现了灵活且高质量的超分辨率重建。

<details>
  <summary>Details</summary>

**Motivation:** 现有的高光谱图像超分辨率方法通常需要针对不同尺度进行修改，缺乏任意尺度重建的灵活性。传统方法依赖光栅化策略，限制了空间建模的灵活性，且在高光谱图像中保持光谱保真度具有挑战性。

**Method:** 提出GaussianHSI框架，采用Voronoi引导的双边2D高斯散点技术：首先预测一组高斯函数表示输入；通过Voronoi引导选择将每个目标像素与相关高斯函数关联；然后使用参考感知的双边权重聚合选中的高斯函数进行重建，考虑几何相关性和低分辨率特征一致性；最后引入光谱细节增强模块提升光谱重建质量。

**Result:** 在基准数据集上的广泛实验证明，GaussianHSI在任意尺度高光谱图像超分辨率任务上优于现有最先进方法。

**Conclusion:** GaussianHSI成功地将2D高斯散点思想扩展到高光谱图像超分辨率领域，通过创新的Voronoi引导双边重建策略和光谱细节增强模块，实现了灵活的任意尺度重建，同时保持了良好的光谱保真度。
...
### [374] [Score-Based Matching with Target Guidance for Cryo-EM Denoising](https://arxiv.org/abs/2604.17734)
*Xiaoqi Wu,Xueying Zhan,Wen Li,Junhao Wu,Xin Huang,Min Xu*

Main category: cs.CV

> 本文提出了一种基于分数的冷冻电镜图像去噪框架，通过学习纯净数据的分数来恢复颗粒信号并更好地保持结构信息，同时引入目标引导变体来稳定弱信号条件下的学习过程。

<details>
  <summary>Details</summary>

**Motivation:** 冷冻电镜图像具有极低的信噪比和微弱的颗粒可见性，现有去噪方法主要关注视觉质量而忽略了下游分析所需的结构一致性问题

**Method:** 提出基于分数的去噪框架学习纯净数据分数，引入参考密度引导的目标引导变体，更好地抑制结构化低频背景而非简单放大颗粒响应

**Result:** 在多个冷冻电镜数据集上证明了方法能持续改善下游颗粒挑选，产生更结构一致的三维重建结果

**Conclusion:** 基于分数的去噪框架有效解决了冷冻电镜图像去噪中的结构一致性问题，为下游分析提供了更好的预处理基础
...
### [375] [IncreFA: Breaking the Static Wall of Generative Model Attribution](https://arxiv.org/abs/2604.17736)
*Haotian Qin,Dongliang Chang,Yueying Gao,Lei Chen,Zhanyu Ma*

Main category: cs.CV

> IncreFA是一种增量学习框架，能够持续适应新型生成式AI模型的出现，通过层次化约束和潜在记忆库机制实现图像归因的高精度识别。

<details>
  <summary>Details</summary>

**Motivation:** 随着生成式AI模型以前所未有的速度发展，图像归因成为一个移动目标。现有的水印、分类器和反转方法在新模型发布后很快过时，核心问题在于归因本身缺乏自适应能力。

**Method:** IncreFA将归因重新定义为结构化增量学习问题，包含两个相互增强的机制：(1)层次化约束，通过可学习的正交先验编码架构层次，区分家族级不变性和模型特异性特征；(2)潜在记忆库，回放紧凑的潜在样本并混合生成伪未见样本，稳定表示漂移并增强开放集感知。

**Result:** 在包含2022-2025年发布的28个生成模型的新增量归因基准测试(IABench)上，IncreFA在时序开放集协议下达到最先进的归因准确率和98.93%的未见模型检测率。

**Conclusion:** IncreFA成功解决了快速演进的生成式模型带来的归因挑战，为动态AI生成环境下的持续图像归因提供了有效解决方案。
...
### [376] [Source-Free Domain Adaptation with Vision-Language Prior](https://arxiv.org/abs/2604.17748)
*Song Tang,Yunxiang Bai,Wenxin Su,Mao Ye,Jianwei Zhang,Xiatian Zhu*

Main category: cs.CV

> DIFO++是一种新颖的无源域适应方法，首次探索利用视觉语言多模态模型(如CLIP)的知识，通过交替定制ViL模型和知识蒸馏到目标模型，显著提升目标域性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统无源域适应方法依赖伪标签和辅助监督，容易产生错误。直接零样本应用ViL模型效果不佳，因为它是通用模型而非针对特定任务。需要一种更可靠的方法来避免伪标签错误。

**Method:** DIFO++采用两步交替策略：(1)通过提示学习方式最大化ViL模型与目标模型的互信息来定制ViL模型；(2)通过聚焦间隙区域减少将定制ViL模型的知识蒸馏到目标模型。识别类间模糊特征丰富的间隙区域，融合两个模型预测生成可靠伪标签，通过类别注意力和预测一致性引导语义对齐及熵最小化抑制不确定性。

**Result:** 大量实验表明DIFO++显著优于现有最先进方法，在无源域适应任务上取得了卓越性能。

**Conclusion:** 首次成功将视觉语言模型应用于无源域适应，创新的间隙区域减少机制有效解决了传统方法的伪标签错误问题，为领域适应研究提供了新思路。
...
### [377] [Ego-InBetween: Generating Object State Transitions in Ego-Centric Videos](https://arxiv.org/abs/2604.17749)
*Mengmeng Ge,Takashi Isobe,Xu Jia,Yanan Sun,Zetong Yang,Weinong Wang,Dong Zhou,Dong Li,Huchuan Lu,Emad Barsoum*

Main category: cs.CV

> 本文提出EgoIn框架，解决以自我为中心视角下的物体状态转换建模问题，通过理解场景、推理转换步骤并生成一致的中间帧序列。

<details>
  <summary>Details</summary>

**Motivation:** 物理变换过程的理解对人类认知和AI系统都至关重要，特别是从自我中心的视角，这是人类与机器在动作建模中的关键桥梁。当前生成模型在理解和推理物体变换过程以及保持物体外观一致性方面存在不足。

**Method:** 提出EgoIn框架，包含两个核心组件：1) TransitionVLM，基于精心策划的数据集微调以更好适应任务并减少幻觉信息；2) Transition Conditioning模块生成转换条件。同时引入Object-aware Auxiliary Supervision保持物体外观一致性。

**Result:** 在人-物和机器人-物交互数据集上的大量实验表明，EgoIn在生成语义有意义且视觉连贯的变换序列方面表现出色。

**Conclusion:** EgoIn框架成功解决了EIVST任务中的关键挑战，为自我中心视角下的物理变换理解提供了有效解决方案，在生成质量上显著优于现有方法。
...
### [378] [Structure-Adaptive Sparse Diffusion in Voxel Space for 3D Medical Image Enhancement](https://arxiv.org/abs/2604.17773)
*Hongxu Jiang,Fei Li,Boxiao Yu,Ying Zhang,Kaleb Smith,Kuang Gong,Wei Shao*

Main category: cs.CV

> 本文提出了一种稀疏体素空间扩散框架，通过利用条件增强任务中的解剖先验知识，在稀疏时间步长上训练和采样，实现10倍训练加速，在3D医学图像增强任务中达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统的扩散模型在2D医学图像处理中表现优异，但扩展到高分辨率3D体积数据时计算成本过高。现有的方法由于对高维体积数据进行密集的噪声调度处理，导致训练和推理效率低下。

**Method:** 提出稀疏体素空间扩散框架，核心创新包括：1）在均匀采样的稀疏时间步长上训练和采样；2）在速度空间中进行监督以保证稳定的梯度缩放；3）设计轻量级结构感知轨迹调制模块(STM)，根据局部解剖内容重新校准时间嵌入，实现结构自适应去噪。

**Result:** 在CT、PET和MRI的四个数据集上，该框架在去噪和超分辨率任务上都达到了最先进性能，实现了高达10倍的训练加速，同时直接在体素空间操作保留了精细解剖细节。

**Conclusion:** 该方法成功将扩散模型的高质量图像生成能力扩展到3D医学图像领域，通过有效利用输入数据的解剖先验信息，显著降低了计算复杂度，为临床诊断提供了高效的图像增强解决方案。
...
### [379] [Subject-Aware Multi-Granularity Alignment for Zero-Shot EEG-to-Image Retrieval](https://arxiv.org/abs/2604.17782)
*Lin Jiang,Qingshan She,Jiale Xu,Haiqi Xu,Duanpo Wu,Zhenzhong Kuang*

Main category: cs.CV

> 提出了SAMGA框架，通过主体自适应多粒度对齐技术，在零样本EEG到图像检索任务中实现了91.3%的Top-1和98.8%的Top-5准确率（主体内）以及34.4%的Top-1和64.8%的Top-5准确率（主体间），显著优于现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 现有的EEG到图像检索方法存在两个关键缺陷：依赖单一固定视觉目标或主体不变的目标构建方案，忽视了视觉诱发的EEG信号具有多尺度表示特性和不同主体对视觉粒度的个体差异性。

**Method:** SAMGA框架包含两个核心技术：1）主体感知视觉监督目标构建，通过自适应聚合预训练视觉编码器的多个中间表示，在训练时吸收主体依赖的粒度偏差；2）粗到细跨模态对齐策略，使用共享编码器，粗阶段稳定共享语义几何并减少主体引起的分布偏移，细阶段提升实例级检索判别力。

**Result:** 在THINGS-EEG基准测试上，SAMGA在主体内设置下达到91.3% Top-1和98.8% Top-5准确率，在主体间设置下达到34.4% Top-1和64.8% Top-5准确率，超越了当前最先进方法。

**Conclusion:** SAMGA通过处理EEG信号的多尺度表示特性和主体间的粒度差异，实现了鲁棒的EEG到图像检索，为可扩展的视觉神经解码和实用脑机接口提供了有效的技术路径。
...
### [380] [DuQuant++: Fine-grained Rotation Enhances Microscaling FP4 Quantization](https://arxiv.org/abs/2604.17789)
*Haokun Lin,Xinle Jia,Haobo Xu,Bingchen Yao,Xianglong Guo,Yichen Wu,Zhichao Lu,Ying Wei,Qingfu Zhang,Zhenan Sun*

Main category: cs.CV

> DuQuant++ 是一种针对 MXFP4 格式优化的量化方法，通过将旋转块大小与微缩放组大小对齐（32元素），用单一旋转替代原有双旋转流程，有效解决激活异常值问题，在 LLaMA-3 W4A4 量化上达到最佳性能。

<details>
  <summary>Details</summary>

**Motivation:** MXFP4 格式在 LLM 推理中表现出色，但激活异常值会导致共享块尺度过大，压缩有效动态范围。现有基于旋转的方法（如随机 Hadamard 和可学习旋转）是数据无关的，无法针对异常值集中的通道进行优化。

**Method:** 提出 DuQuant++，调整 DuQuant 的细粒度旋转策略，使旋转块大小与 MXFP4 的微缩放组大小（32元素）对齐。利用每个 MXFP4 组独立缩放因子的特性，用单一异常值感知旋转替代原有的双旋转和锯齿排列流程。

**Result:** 在 LLaMA-3 系列模型上进行 MXFP4 W4A4 量化实验，DuQuant++ 始终达到业界领先性能，同时将在线旋转成本减半并平滑权重分布。

**Conclusion:** DuQuant++ 成功将异常值感知旋转适配到 MXFP4 格式，通过简化旋转流程显著提升效率，为高效 LLM 推理提供了新的量化解决方案。
...
### [381] [Weakly-Supervised Referring Video Object Segmentation through Text Supervision](https://arxiv.org/abs/2604.17797)
*Miaojing Shi,Jun Huang,Zijie Yue,Hanli Wang*

Main category: cs.CV

> 本文提出WSRVOS，一种仅需文本表达作为监督的弱监督参考视频目标分割方法，通过对比语言增强、双向特征对齐、实例分类和伪掩码融合等技术实现了无需人工标注的高性能视频分割。

<details>
  <summary>Details</summary>

**Motivation:** 传统RVOS方法需要昂贵的像素级掩码标注，现有弱监督方法仍需边界框或点标注，成本高昂且劳动密集。本文旨在完全消除这些标注需求，仅使用文本表达来训练模型。

**Method:** 1) 设计对比性参考表达增强方案，利用多模态大语言模型生成正负表达；2) 提取视听特征并进行双向特征选择与交互，实现细粒度多模态对齐；3) 提出实例感知表达分类优化模型区分能力；4) 引入正预测融合策略生成高质量伪掩码作为额外监督；5) 设计时序段排名约束确保相邻帧预测的一致性。

**Result:** 在A2D Sentences、J-HMDB Sentences、Ref-YouTube-VOS和Ref-DAVIS17四个公开数据集上的大量实验证明了该方法的优越性。

**Conclusion:** WSRVOS成功实现了仅需文本表达的弱监督学习，通过创新的多模态对齐和伪掩码生成技术，在多个基准数据集上取得了优异性能，为减少人工标注成本提供了有效解决方案。
...
### [382] [View-Consistent 3D Scene Editing via Dual-Path Structural Correspondense and Semantic Continuity](https://arxiv.org/abs/2604.17801)
*Pufan Li,Bi'an Du,Shenghe Zheng,Junyi Yao,Wei Hu*

Main category: cs.CV

> 本文提出了一种基于视角一致性的3D场景编辑框架，通过双路径一致性机制和跨视图依赖关系建模，显著提升了3D编辑的跨视图一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本驱动3D场景编辑方法采用渲染-编辑-优化流程，但受限于跨视图不一致性这一主要瓶颈。虽然已有方法尝试引入几何线索或视频先验来缓解问题，但仍然依赖推理时同步，在鲁棒性和泛化能力方面存在局限。

**Method:** 该方法从分布视角重新定义多视图一致的3D编辑问题，提出显式引入跨视图依赖关系的编辑框架。具体实现包括：1) 双路径一致性机制，包含投影引导的结构引导和块级语义传播；2) 构建配对的多视图编辑数据集，为学习编辑场景中的跨视图一致性提供可靠监督。

**Result:** 大量实验证明，该方法在复杂场景中取得了优异的编辑性能，能够生成精确且一致的视图效果，显著优于现有方法。

**Conclusion:** 通过分布式建模和双路径一致性机制，该方法有效解决了3D场景编辑中的跨视图不一致问题，为文本驱动的3D编辑提供了新的解决思路，具有较好的鲁棒性和泛化能力。
...
### [383] [Re$^2$MoGen: Open-Vocabulary Motion Generation via LLM Reasoning and Physics-Aware Refinement](https://arxiv.org/abs/2604.17807)
*Jiakun Zheng,Ting Xiao,Shiqin Cao,Xinran Li,Zhe Wang,Chenjia Bai*

Main category: cs.CV

> Re^2MoGen是一个开放词汇的动作生成框架，通过强化LLM推理能力生成初始动作规划，再通过强化学习物理优化，显著提升了超出训练分布的文本到动作生成效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有文本到动作模型在处理与训练文本差异较大的运动描述时性能急剧下降，需要解决开放词汇场景下的动作生成问题。

**Method:** 框架包含三阶段：1）使用蒙特卡洛树搜索增强LLM推理能力生成关键帧；2）利用人体姿态先验优化全身姿态并监督微调运动生成器；3）通过物理感知奖励的强化学习后训练优化运动质量。

**Result:** 在开放词汇动作生成任务上实现了最先进的性能，能够生成语义一致且物理合理的运动，解决了训练数据分布限制问题。

**Conclusion:** Re^2MoGen成功结合了LLM的推理能力和物理优化，为开放词汇场景下的文本到动作生成提供了有效解决方案，在语义准确性和物理合理性方面都取得了显著提升。
...
### [384] [AnyLift: Scaling Motion Reconstruction from Internet Videos via 2D Diffusion](https://arxiv.org/abs/2604.17818)
*Hongjie Li,Heng Yu,Jiaman Li,Hong-Xing Yu,Ehsan Adeli,C. Karen Liu,Jiajun Wu*

Main category: cs.CV

> 提出一个两阶段框架，利用2D扩散模型从互联网视频中重建3D人体运动和人物交互，能够处理动态相机下的运动和稀有动作类型。

<details>
  <summary>Details</summary>

**Motivation:** 现有的从互联网视频重建3D人体运动的方法存在两个主要问题：在动态摄像机下难以恢复全局一致的3D运动，特别是对于现有动作捕捉数据集中表现不足的运动类型；同时难以在3D空间中恢复连贯的人物交互。

**Method:** 采用两阶段框架：第一阶段，利用从互联网视频中提取的2D关键点合成多视角2D运动数据，融入MoCap数据集中罕见的人体运动；第二阶段，在领域特定合成数据上训练摄像机条件的多视角2D运动扩散模型，恢复世界坐标系下的3D人体运动和3D人物交互。

**Result:** 在包含体操等挑战性运动的互联网视频以及野外人物交互视频上验证了方法的有效性，在产生真实人体运动和人物交互方面超越了先前工作。

**Conclusion:** 该方法成功解决了动态摄像机下的3D运动重建难题，能够处理稀有动作类型，为构建大规模人类行为数据集提供了有效途径。
...
### [385] [GR4CIL: Gap-compensated Routing for CLIP-based Class Incremental Learning](https://arxiv.org/abs/2604.17822)
*Tianqi Wang,Jingcai Guo*

Main category: cs.CV

> GR4CIL是一个基于CLIP的类增量学习框架，通过任务判别和知识路由机制解决了知识漂移和跨任务响应校准差的问题，在保持零样本泛化能力的同时提升了学习性能。

<details>
  <summary>Details</summary>

**Motivation:** 基于CLIP的类增量学习方法面临两个关键挑战：共享参数适应导致旧知识漂移，任务特定知识组织导致跨任务响应校准不良，使得可靠的知识路由变得困难。

**Method:** GR4CIL框架结合任务判别和知识路由，保留任务特定的视觉知识同时维护稳定的共享文本语义空间；引入正交补偿机制缓解模态间隙引起的偏差，增强任务内判别性，扩大真实任务与竞争任务间的分数边界。

**Result:** 在多个基准测试上，GR4CIL持续超越强基线方法，实现了更可靠的任务感知知识路由，同时保留了CLIP的零样本泛化能力。

**Conclusion:** GR4CIL通过有效平衡任务特定知识和共享语义空间，解决了CLIP在类增量学习中的应用瓶颈，为持续学习提供了新的解决方案。
...
### [386] [PCM-NeRF: Probabilistic Camera Modeling for Neural Radiance Fields under Pose Uncertainty](https://arxiv.org/abs/2604.17831)
*Shravan Venkatraman,Rakesh Raj Madavan,Pavan Kumar Sathya Venkatesh*

Main category: cs.CV

> PCM-NeRF是一个概率框架，为神经表面重建引入了可学习的相机位姿不确定性机制，能够有效处理不准确的SfM位姿估计问题。

<details>
  <summary>Details</summary>

**Motivation:** 传统神经表面重建方法假设相机位姿完全准确，但实际中SfM系统存在估计误差，导致重建结果失真或不完整。

**Method:** 基于SG-NeRF，将每个相机位姿表示为具有可学习均值和方差的分布，通过不确定性正则化损失将学习到的方差与视图置信度耦合，并直接调整有效位姿学习率。

**Result:** 在具有严重位姿离群值的挑战性场景上，PCM-NeRF在Chamfer距离和F-Score指标上均优于现有方法，特别在几何复杂结构上表现突出。

**Conclusion:** 该轻量级机制无需修改渲染管线，增加可忽略的计算开销，且不需要前景遮罩，为神经表面重建提供了一种鲁棒的位姿不确定性处理方案。
...
### [387] [AI Approach for MRI-only Full-Spine Vertebral Segmentation and 3D Reconstruction in Paediatric Scoliosis](https://arxiv.org/abs/2604.17846)
*Nathasha Naranpanawa,Maree T. Izatt,Robert D. Labrom,Geoffrey N. Askin,J. Paige Little*

Main category: cs.CV

> 本研究开发了一个AI框架，能够从MRI图像全自动分割和重建胸腰椎（T1-L5）的3D骨骼结构，实现了无辐射的脊柱畸形评估。

<details>
  <summary>Details</summary>

**Motivation:** 儿科脊柱成像中MRI优于CT（无电离辐射），但其应用受限于缺乏自动化的高分辨率3D骨骼重建技术，目前仍依赖CT进行评估。

**Method:** 使用GAN将AIS患者的低剂量CT扫描转换为MRI样图像，结合现有的已标记胸椎MRI数据，训练基于U-Net的分割模型，实现全自动3D重建。

**Result:** 算法成功生成连续的胸腰椎3D重建，分割准确率达88%（Dice分数），处理时间从约1小时缩短至1分钟以内，同时保留了AIS特有的畸形特征。

**Conclusion:** 该方法实现了基于MRI的无辐射3D畸形评估，为儿科脊柱护理的临床评估、手术规划和导航提供了技术支持。
...
> UniCSG是一个统一的图文风格迁移框架，通过语义解耦、频率感知重建和奖励学习解决内容-风格纠缠问题。

<details>
  <summary>Details</summary>

**Motivation:** 基于DiT的扩散模型在风格迁移中存在内容-风格纠缠，导致参考内容泄露和生成不稳定的问题。

**Method:** 提出分阶段训练：(1) 潜在空间语义解耦阶段，结合低频预处理和条件损坏；(2) 潜在空间频率感知细节重建阶段，通过多尺度频率监督细化细节；并加入像素空间奖励学习。

**Result:** 实验表明在文本引导和参考引导两种设置下，该方法在内容忠实度、风格对齐性和鲁棒性方面均有提升。

**Conclusion:** UniCSG通过统一的框架有效解决了风格迁移中的内容-风格纠缠问题，实现了更好的生成质量。
...
### [389] [PlankFormer: Robust Plankton Instance Segmentation via MAE-Pretrained Vision Transformers and Pseudo Community Image Generation](https://arxiv.org/abs/2604.17856)
*Masaharu Miyazaki,Yurie Otake,Koichi Ito,Wataru Makino,Jotaro Urabe,Takafumi Aoki*

Main category: cs.CV

> PlankFormer是一个结合了合成图像生成和Transformer架构的浮游生物实例分割框架，通过MAE预训练和ViT骨干网络，在标注数据稀缺的情况下实现了高精度分割。

<details>
  <summary>Details</summary>

**Motivation:** 浮游生物监测对水生生态系统评估至关重要，但传统人工显微镜分析方法劳动密集度高。现有自动化方法面临两大挑战：像素级标注数据集稀缺，以及基于CNN的方法难以区分浮游生物、碎屑和重叠个体。

**Method:** 提出PlankFormer框架：1）通过将单个浮游生物图像合成到多样化背景上生成伪社区图像(PCI)以解决数据短缺；2）采用Vision Transformer骨干网络和Mask2Former解码器的分割模型；3）使用Masked Autoencoder对未标注个体图像进行自监督预训练以捕获全局结构特征。

**Result:** 在真实数据集上的实验结果表明，该方法显著优于Mask R-CNN等传统方法，特别是在碎屑密度高的挑战性环境中表现出色，仅需较少的人工标注即可实现高精度分割。

**Conclusion:** 合成训练策略和基于MAE的架构能够有效解决浮游生物实例分割中的数据稀缺和目标识别难题，为浮游生物监测提供了高效的自动化解决方案。
...
### [390] [Sharpening Lightweight Models for Generalized Polyp Segmentation: A Boundary Guided Distillation from Foundation Models](https://arxiv.org/abs/2604.17865)
*Shivanshu Agnihotri,Snehashis Majhi,Deepak Ranjan Nayak*

Main category: cs.CV

> LiteBounD是一个轻量级边界引导的蒸馏框架，通过将多个大型视觉基础模型的语义和结构先验知识蒸馏到紧凑的分割骨干网络中，实现了高效的息肉分割。

<details>
  <summary>Details</summary>

**Motivation:** 自动化息肉分割对早期结直肠癌检测至关重要，但现有轻量级模型难以捕获丰富的语义和结构信息，而大型视觉基础模型虽然泛化能力强，但在息肉分割任务上存在领域不匹配和计算成本高的问题。

**Method:** LiteBounD提出双路径蒸馏机制分离语义和边界感知表示，采用频率感知对齐策略分别监督低频全局语义和高频边界细节，并设计边界感知解码器融合多尺度编码器特征和蒸馏的语义丰富边界信息。

**Result:** 在已见(Kvasir-SEG, CVC-ClinicDB)和未见(ColonDB, CVC-300, ETIS)数据集上的广泛实验表明，LiteBounD显著优于轻量级基线模型，与最先进方法具有竞争力，同时保持了实时临床使用所需的效率。

**Conclusion:** LiteBounD成功弥合了轻量级模型的效率优势和大型视觉基础模型的语义理解能力之间的鸿沟，为临床实用的息肉分割提供了有效的解决方案。
...
### [391] [Spatiotemporal Sycophancy: Negation-Based Gaslighting in Video Large Language Models](https://arxiv.org/abs/2604.17873)
*Ziyao Tang,Pengkun Jiao,Bin Zhu,Huiyan Qi,Jingjing Chen,Yu-Gang Jiang*

Main category: cs.CV

> 这项研究揭示了视频大语言模型存在时空谄媚现象，即模型在否定式煤气灯攻击下会推翻正确的判断并编造错误的时空解释来迎合误导性反馈。

<details>
  <summary>Details</summary>

**Motivation:** 尽管视频大语言模型在视频理解任务中表现优异，但其在对话交互下的鲁棒性尚未得到充分探索。作者发现模型在面对用户的否定式误导反馈时，容易放弃原本正确的判断。

**Method:** 提出了基于否定的煤气灯评估框架，并构建了GasVideo-1000基准数据集，通过系统性测试各种开源和专有的Vid-LLMs在不同视频理解任务中的表现来研究时空谄媚现象。

**Result:** 实验发现，即使是基线性能强大的模型，也对否定式煤气灯攻击表现出普遍且严重的脆弱性。提示级别的接地约束只能部分缓解这种行为，无法可靠地防止幻觉式合理化或信念逆转。

**Conclusion:** 当前的视频大语言模型缺乏鲁棒机制来在对抗性对话反馈下保持基于时空的认知，亟需改进其对话鲁棒性和信念稳定性。
...
### [392] [Exploring Boundary-Aware Spatial-Frequency Fusion for Camouflaged Object Detection](https://arxiv.org/abs/2604.17879)
*Song Yu,Yang Hu,Haokang Ding,Zhifang Liao,Yucheng Song*

Main category: cs.CV

> BASFNet是一种基于边界感知的频域-空域融合网络，通过结合频域相位谱信息增强边缘检测，并优化空间域分割，显著提升了伪装目标检测性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有伪装目标检测方法主要依赖空间域边缘提取和局部像素信息，忽略了全局结构特征和频域相位谱信息的重要性，导致检测效果受限。

**Method:** 提出BASFNet框架，包含频谱增强的边缘探索模块(FEEM)、空间核心分割模块(SCSM)和空间-频域融合交互模块(SFFIM)，通过双重引导整合频域和空间域特征，并采用边界感知训练策略进行优化。

**Result:** 在三个基准数据集上超越现有最先进方法，验证了频域与空间域信息融合在伪装目标检测任务中的有效性。

**Conclusion:** 频域相位谱信息与空间域特征的融合是提升伪装目标检测性能的关键，BASFNet为此任务提供了一种新的有效解决方案。
...
### [393] [AeroRAG: Structured Multimodal Retrieval-Augmented LLM for Fine-Grained Aerial Visual Reasoning](https://arxiv.org/abs/2604.17889)
*Junxiao Xue,Quan Deng,Tingqi Hu,Meicong Si,Xinyi Yin,Yunyun Shi,Xuecheng Wu*

Main category: cs.CV

> AeroRAG是一个场景图引导的多模态检索增强生成框架，通过将图像转换为结构化视觉知识来解决航空场景视觉问答的问题。

<details>
  <summary>Details</summary>

**Motivation:** 在航空场景中，任务关键证据通常包含在小对象、显式数量、粗略位置和对象间关系中，而传统的密集视觉标记表示与这些结构化语义无法很好地对齐，导致现有的多模态大语言模型在航空场景视觉问答任务中表现不佳。

**Method:** AeroRAG框架首先将输入图像转换为结构化视觉知识（包括对象类别、数量、空间位置和语义关系），然后检索与查询相关的语义块来为基于文本的大语言模型构建紧凑提示，通过在感知和语言推理之间引入更明确的中间接口来替代直接对密集视觉标记进行推理。

**Result:** 在AUG航空数据集和通用域VG-150基准测试上的实验表明，该框架相比六个强大的MLLM基线有持续的改进，在密集航空场景和关系敏感推理中观察到最大增益；在VQAv2上的进一步评估验证了该接口与标准视觉推理设置的兼容性。

**Conclusion:** 结构化检索是部署导向和基础视觉推理系统的一个实用设计方向，为航空场景视觉问答任务提供了有效的解决方案。
...
### [394] [From Heads to Neurons: Causal Attribution and Steering in Multi-Task Vision-Language Models](https://arxiv.org/abs/2604.17941)
*Qidong Wang,Junjie Hu,Ming Jiang*

Main category: cs.CV

> HONEs是一种无需梯度的框架，通过注意力头条件化的因果写入贡献来排序FFN神经元，实现多任务视觉语言模型的任务感知神经元归因和调控。

<details>
  <summary>Details</summary>

**Motivation:** 现有的神经元分析方法存在局限性：仅关注单一任务导致跨任务可比性不足；孤立评分神经元忽视了任务依赖信息通路对FFN神经元写入效应的影响，在多任务设置下会加剧神经元多义性，为任务关键神经元的识别和干预引入噪声。

**Method:** 提出了HONEs框架，通过任务相关注意力头条件下的因果写入贡献来排名FFN神经元，并通过轻量级缩放来调节显著神经元。该方法采用无梯度设计，适用于多任务VLMs场景。

**Result:** 在四个多样化多模态任务和两个主流VLMs上的实验表明，HONEs在识别任务关键神经元方面优于现有方法，且在调控后能提升模型性能。

**Conclusion:** HONEs通过考虑任务依赖信息通路和条件化注意力头，有效解决了多任务设置下神经元多义性问题，为VLMs的任务感知神经元理解和干预提供了新的解决方案。
...
### [395] [ReTrack: Evidence-Driven Dual-Stream Directional Anchor Calibration Network for Composed Video Retrieval](https://arxiv.org/abs/2604.17898)
*Zixu Li,Yupeng Hu,Zhiwei Chen,Qinlei Huang,Guozhi Qiu,Zhiheng Fu,Meng Liu*

Main category: cs.CV

> ReTrack是一种组合视频检索(CVR)框架，通过校准组合特征中的方向偏差，解决模态贡献纠缠等核心挑战，在CVR和CIR任务中达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 组合视频检索任务中，视频和文本模态间存在显著的信息密度差异，传统组合方法倾向于偏向参考视频特征，导致检索性能不佳。主要面临模态贡献纠缠、组合特征显式优化和检索不确定性三个核心挑战。

**Method:** 提出ReTrack框架，包含三个关键模块：语义贡献解纠缠、组合几何校准和可靠证据驱动的对齐。该框架首次通过校准组合特征的方向偏差来改进多模态查询理解，估计各模态的语义贡献，使用校准的方向锚点计算双向证据来驱动可靠的组合-目标相似性估计。

**Result:** ReTrack在组合视频检索(CVR)和组合图像检索(CIR)任务上均展现出强大的泛化能力，在三个基准数据集上都达到了最先进的性能。

**Conclusion:** ReTrack通过创新的方向偏差校准机制有效解决了CVR任务中的模态不平衡问题，为多模态检索领域提供了新的解决思路，并在实际应用中表现出优越的检索效果和良好的泛化能力。
...
### [396] [Mitigating Multimodal Hallucination via Phase-wise Self-reward](https://arxiv.org/abs/2604.17982)
*Yu Zhang,Chuyang Sun,Kehai Chen,Xuefeng Bai,Yang Xiang,Min Zhang*

Main category: cs.CV

> 提出PSRD框架，通过相态自奖励机制在推理时动态抑制视觉语言模型的幻觉现象，无需外部监督或大规模调优。

<details>
  <summary>Details</summary>

**Motivation:** 现有LVLMs的幻觉抑制方法要么需要大规模标注数据调优（计算开销大），要么采用静态后处理策略（忽略幻觉动态性），缺乏高效动态的解决方案。

**Method:** 发现视觉幻觉呈现相态动态模式，在每个语义阶段初期达到峰值。基于此提出PSRD框架：1) 使用相态自奖励信号指导在线幻觉修正；2) 将大模型的幻觉指导信号蒸馏到轻量奖励模型；3) 解码过程中提供实时指导和精准干预。

**Result:** LLaVA-1.5-7B的幻觉率降低50.0%，在5个幻觉评估基准和4个LVLMs上均优于现有后处理方法，有效抑制幻觉传播，且在性能和效率间实现可控平衡。

**Conclusion:** PSRD成功实现了推理时的动态幻觉缓解，无需额外监督，为解决LVLMs幻觉问题提供了高效实用的解决方案。
...
> 本文提出了一种运动情感特征解耦网络(MEDN)，通过双分支框架分别提取运动特征和情感特征，并通过解耦和融合策略提高了微表情识别的准确率。

<details>
  <summary>Details</summary>

**Motivation:** 微表情不遵循严格一致的情感与动作单元(AUs)映射规则，导致不同情感的微表情可能具有相同的AUs而高度相似。现有方法主要依赖显式运动线索而忽略了隐式情感信息，导致识别性能受限。

**Method:** 设计了双分支框架：运动分支通过AU检测任务和正交损失约束特征并减少运动情感特征耦合；情感分支提出稀疏情感视觉Transformer(SEVit)以多尺度稀疏化突出局部时序变化；通过协作融合模块(CoFM)自适应融合解耦后的两种特征。

**Result:** 在三个基准数据集上的广泛实验证明，MEDN能有效解耦运动和情感特征，取得了更优的识别性能，提供了提升识别准确率和泛化能力的新视角。

**Conclusion:** 该方法通过显式运动信息和隐式情感信息的解耦建模，有效解决了微表情中特征混淆问题，为微表情识别领域提供了新的技术路径。
...
### [398] [ESsEN: Training Compact Discriminative Vision-Language Transformers in a Low-Resource Setting](https://arxiv.org/abs/2604.18452)
*Clayton Fields,Casey Kennington*

Main category: cs.CV

> 本文提出了一种轻量级视觉语言模型ESsEN，通过双塔编码器架构和卷积网络融合，在小数据和有限资源下达到与大型模型相当的性能。

<details>
  <summary>Details</summary>

**Motivation:** 视觉语言模型通常需要数十亿参数，但边缘设备和机器人平台等场景需要轻量级模型。现有研究缺乏小模型构建和小数据集训练的方法，这限制了视觉语言技术的广泛应用。

**Method:** 受儿童语言学习过程启发，采用双塔编码器架构，结合传统卷积网络与Transformer，设计了灵活的跨模态融合模块。构建了端到端可训练的紧凑模型ESsEN。

**Result:** 双塔编码器在低资源判别任务中优于单塔编码器； convolutional-transformer混合架构提高了参数效率；跨模态融合模块结构灵活性高；ESsEN模型在多项任务中表现出色，参数量仅为其他模型的一小部分。

**Conclusion:** 该研究为资源受限环境下的视觉语言建模提供了有效解决方案，实验结果和工具降低了技术门槛，使更多研究者能够参与视觉语言模型研究。
...
### [399] [Beyond Binary Contrast: Modeling Continuous Skeleton Action Spaces with Transitional Anchors](https://arxiv.org/abs/2604.17914)
*Yingjie Feng,Yi Wang,Jiaze Wang,Anfeng Liu,Zhuotao Tian*

Main category: cs.CV

> TranCLR是一个基于过渡锚点的对比学习框架，通过建模动作的连续性和几何结构，学习更平滑、更具判别性的骨骼动作表示。

<details>
  <summary>Details</summary>

**Motivation:** 现有的自监督对比学习方法主要使用二元对比目标，忽视了人体运动的内在连续性，导致特征簇破碎和类别边界过于僵硬，限制了模型对动作连续性的感知能力。

**Method:** 提出了动作过渡锚点构建(ATAC)来显式建模过渡状态的几何结构，并引入多层次几何流形校准(MGMC)机制，在多个连续性层次上自适应校准动作流形，从而获得更平滑的表示空间。

**Result:** 在NTU RGB+D、NTU RGB+D 120和PKU-MMD数据集上的广泛实验表明，TranCLR在准确率和校准性能上均优于现有方法，能够有效学习连续且感知不确定性的骨骼表示。

**Conclusion:** 通过捕获动作空间的连续几何结构，TranCLR为基于骨骼的动作识别提供了一个新的自监督学习范式，能够更好地处理动作的过渡状态和连续性特征。
...
### [400] [OneVL: One-Step Latent Reasoning and Planning with Vision-Language Explanation](https://arxiv.org/abs/2604.18486)
...
> OneVL是一个统一的视觉-语言模型与世界模型框架，通过双解码器监督实现一步式潜在推理规划，首次在保持答案级低延迟的同时超越显式思维链方法的预测精度。

<details>
  <summary>Details</summary>

**Motivation:** 现有思维链推理方法虽然提升了自动驾驶轨迹预测效果，但其自回归特性带来高延迟不适合实时部署；潜在CoT方法试图压缩推理但表现不佳，主要因为其压缩的是抽象符号而非实际驾驶的因果动态。

**Method:** 提出OneVL框架，包含主VLA模型、语言解码器重建文本CoT、视觉世界模型解码器预测未来帧token；通过三阶段训练将潜在空间与轨迹、语言、视觉目标对齐；推理时丢弃辅助解码器，单步并行生成所有潜在token。

**Result:** 在四个基准测试中，OneVL成为首个超越显式CoT的潜在CoT方法，实现了答案级延迟下的SOTA准确率。

**Conclusion:** 证明了在语言和世界模型双重监督引导下，更紧密的压缩相比逐token的冗长推理能产生更泛化的表征，为高性能低延迟的自动驾驶推理提供了新范式。
...
### [401] [OneDrive: Unified Multi-Paradigm Driving with Vision-Language-Action Models](https://arxiv.org/abs/2604.17915)
*Yiwei Zhang,Xuesong Chen,Jin Gao,Hanshi Wang,Fudong Ge,Weiming Hu,Shaoshuai Shi,Zhipeng Zhang*

Main category: cs.CV

> 提出基于视觉语言模型的统一自动驾驶框架，在单个Transformer解码器中整合异构解码任务，实现了语言生成、目标检测和轨迹规划的端到端学习，在多个基准测试中达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有自动驾驶系统需要分离或级联的解码器来处理不同任务（如文本生成、目标检测、轨迹回归），导致架构碎片化和骨干网络利用率有限，无法充分利用预训练视觉语言模型的潜力。

**Method:** 通过将视觉和结构化查询token组织在单一因果解码器中，利用预训练VLM的注意力机制实现多任务统一学习。结构化查询通过原始注意力机制自然地 conditioned on 视觉上下文，文本和结构化输出共享注意力骨干网络，并在相同解码器中引入结构化轨迹查询进行轨迹规划。

**Result:** 在nuScenes开放环评估中达到0.28 L2和0.18碰撞率，在NAVSIM封闭环评估中获得86.8 PDMS的竞争性结果，同时保持多模态生成能力，高效推理模式降低约40%延迟。

**Conclusion:** 证明了预训练VLM注意力机制在纯语言建模之外具有很强的迁移性，统一的架构设计实现了稳定的跨异构任务联合优化，为端到端自动驾驶提供了新的解决方案。
...
### [402] [Prompting Foundation Models for Zero-Shot Ship Instance Segmentation in SAR Imagery](https://arxiv.org/abs/2604.17920)
*Islam Mansour,Francescopaolo Sica,Michael Schmitt*

Main category: cs.CV

> 该研究提出了一种结合YOLOv11检测器和SAM2分割模型的零样本SAR舰船实例分割方法，无需像素级标注即可实现89%检测率和0.637 mIoU性能。

<details>
  <summary>Details</summary>

**Motivation:** SAR图像分析中深度学习应用受限于像素级标注数据缺乏，传统方法需要大量标注成本，阻碍了SAR图像理解技术的发展。

**Method:** 使用在开源SAR数据集上训练的YOLOv11检测器定位舰船边界框，然后作为提示输入到SAM2模型生成实例掩码，仅依靠空间约束而不需要掩码标注。

**Result:** 在SSDD基准测试上达到0.637的mIoU（相当于全监督基准的89%），整体舰船检测率为89.2%，支持舰船分类、尺寸估计和尾迹分析等下游任务。

**Conclusion:** 验证了基于通用视觉基础模型的零样本SAR图像理解路径是可行且高效的，为SAR图像分析提供了一种可扩展的标注高效解决方案。
...
### [403] [Brain-Inspired Capture: Evidence-Driven Neuromimetic Perceptual Simulation for Visual Decoding](https://arxiv.org/abs/2604.17927)
*Feixue Shao,Guangze Shi,Xueyu Liu,Yongfei Wu,Mingqiang Wei,Jianan Zhang,Jianbo Lu,Guiying Yan,Weihua Yang*

Main category: cs.CV

> 提出BI-Cap方法，通过模拟人类视觉系统来弥合神经信号与视觉模态间的差距，在脑-图像检索任务上取得了显著性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 当前神经信号视觉解码方法受限于神经与视觉模态间的系统性和随机性差异，且忽视了人类视觉系统的内在计算机制。

**Method:** 构建包含四个生物合理动态和静态变换的神经模拟管道，结合互信息引导的动态模糊调节，并引入证据驱动的潜空间表示来处理神经活动非平稳性。

**Result:** 在两个公开基准上的零样本脑-图像检索任务中，BI-Cap超越了现有最先进方法，分别获得9.2%和8.0%的相对提升。

**Conclusion:** 通过模拟人类视觉系统的计算机制，可以有效对齐神经和视觉模态，为构建鲁棒的脑-图像解码系统提供了新范式。
...
### [404] [ZSG-IAD: A Multimodal Framework for Zero-Shot Grounded Industrial Anomaly Detection](https://arxiv.org/abs/2604.17949)
*Qiuhui Chen,Jiaxiang Song,Shuai Tan,Weimin Zhong*

Main category: cs.CV

> ZSG-IAD是一个零样本多模态工业异常检测框架，能够生成结构化异常报告和像素级异常掩码，提供透明、有物理依据的异常解释。

<details>
  <summary>Details</summary>

**Motivation:** 现有的深度学习工业异常检测器往往是黑盒模型，难以提供具有物理意义的缺陷证据来解释其决策过程。

**Method:** 提出基于视觉语言的多模态框架ZSG-IAD，采用语言引导的两步定位模块：1)异常相关句子选择从多模态特征中提取的证据式潜在槽位；2)选中的槽位通过通道-空间门控机制和轻量级解码器调制特征图生成精细掩码。同时应用Executable-Rule GRPO算法提升结构化输出的可靠性。

**Result:** 在多个工业异常检测基准上展现出强大的零样本性能，相比先前方法提供更透明、基于物理依据的异常解释。

**Conclusion:** ZSG-IAD通过多模态融合和语言引导实现了可解释的工业异常检测，为构建可信赖的工业异常检测系统提供了新思路，并将开源代码和注释以促进未来研究。
...
### [405] [Chatting about Upper-Body Expressive Human Pose and Shape Estimation](https://arxiv.org/abs/2604.17959)
*Yuxiang Zhao,Wei Huang,Yujie Song,Liu Wang,Huan Zhao*

Main category: cs.CV

> CoEvoer是一个新颖的协同交叉依赖Transformer框架，专门用于上身表现性人体姿态和形状估计，通过不同身体部位之间的特征交互和相互增强，解决了面部和手部参数估计不准确以及泛化能力有限的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有的EHPS方法在面部和手部区域参数估计准确性上仍有不足，且对野外图像泛化能力有限，需要一种能够有效利用面部、手部和躯干之间强耦合和语义依赖关系的方法。

**Method:** 提出CoEvoer框架，采用单阶段Transformer架构，实现不同身体部位间的显式特征级交互。躯干等大型易估计区域提供全局语义和位置先验来指导面部和手部的精细估计，同时面部和手部的局部细节有助于细化和调整相邻身体部分。

**Result:** 在上身基准测试中达到了最先进的性能，并且在未见过的野外图像上展现出强大的泛化能力。

**Conclusion:** CoEvoer是首个专门针对上身EHPS设计的框架，通过联合参数回归有效捕捉面部、手部和躯干间的强耦合与语义依赖，显著提升了估计精度和泛化性能。
...
### [406] [DifFoundMAD: Foundation Models meet Differential Morphing Attack Detection](https://arxiv.org/abs/2604.17961)
*Lazaro J. Gonzalez-Soler,André Dörsch,Christian Rathgeb,Christoph Busch*

Main category: cs.CV

> DifFoundMAD是一种参数高效的差分人脸反欺骗框架，利用视觉基础模型的泛化能力检测伪造人脸与真实人脸的差异，在严格安全级别下将错误率从6.16%降至2.17%。

<details>
  <summary>Details</summary>

**Motivation:** 传统差分人脸反欺骗(D-MAD)系统依赖人脸识别嵌入或手工特征差异，存在泛化能力不足的问题。为了提升在实际部署场景(如边检控制)中的检测性能，需要开发更有效的方法。

**Method:** 采用视觉基础模型(FM)的嵌入表示替代传统表示空间，结合轻量级微调和类别平衡优化策略，仅更新少量参数保持FM的丰富表示先验，遵循标准差分范式进行检测。

**Result:** 在标准D-MAD基准测试的跨数据库评估中，DifFoundMAD在需要严格安全级别的操作部署中表现优异，特别是高安全级别下将当前最先进技术的错误率从6.16%降低到2.17%。

**Conclusion:** DifFoundMAD通过充分利用视觉基础模型的泛化能力，实现了参数高效的差分人脸反欺骗检测，为实际部署场景提供了更可靠的解决方案，显著降低了错误检测率。
...
### [407] [MU-GeNeRF: Multi-view Uncertainty-guided Generalizable Neural Radiance Fields for Distractor-aware Scene](https://arxiv.org/abs/2604.17965)
*Wenjie Mu,Zhan Li,Chuanzhou Su,Xuanyi Shen,Ziniu Liu,Fan Lu,Yujian Mo,Junqiao Zhao,Tiantian Feng,Chen Ye,Guang Chen*

Main category: cs.CV

> MU-GeNeRF是一个多视角不确定性引导的广义神经辐射场框架，通过分解源视图不确定性和目标视图不确定性来解决瞬态干扰物对场景重建的影响。

<details>
  <summary>Details</summary>

**Motivation:** 现有的广义神经辐射场方法在真实场景中面临瞬态干扰物破坏跨视角结构性一致性的问题，导致监督信号损坏，重建质量下降。传统的基于每场景优化的干扰物过滤方法不适用于GeNeRFs。

**Method:** 提出将干扰物感知分解为两个互补的不确定性组件：源视图不确定性（捕获视角变化或动态因素导致的跨视图结构差异）和目标视图不确定性（检测瞬态干扰物引起的观测异常）。通过异方差重构损失结合这两种不确定性，自适应调节监督信号。

**Result:** 大量实验表明，该方法不仅超越了现有的GeNeRF方法，还达到了与场景特定无干扰物NeRF相当的重建性能。

**Conclusion:** MU-GeNeRF有效解决了广义神经辐射场在存在瞬态干扰物时的鲁棒建模挑战，通过多视角不确定性引导实现了更好的干扰物抑制和几何建模效果。
...
### [408] [E3VS-Bench: A Benchmark for Viewpoint-Dependent Active Perception in 3D Gaussian Splatting Scenes](https://arxiv.org/abs/2604.17969)
...
> 研究人员推出了E3VS-Bench基准，用于评估在3D环境中具有5自由度视角控制的具身视觉搜索能力。现有模型与人类表现存在显著差距。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉搜索和具身AI基准主要依赖静态观察或受限的自身运动，无法充分评估真实3D环境中5自由度视角控制下的细粒度视角相关现象，如垂直视角变化导致的可见性变化、容器内部内容显示以及仅能从特定角度观察的物体属性。

**Method:** 构建E3VS-Bench基准，包含99个使用3D高斯溅射技术重建的高保真3D场景和2,014个问题驱动情节。3D高斯溅射技术实现照片级真实感的自由视角渲染，保留细粒度视觉细节，支持构建需要跨视角主动检查的问题。

**Result:** 对多个最先进的视觉语言模型进行评估，并与人类表现进行比较。尽管模型具有很强的2D推理能力，但所有模型与人类表现存在 substantial gap。

**Conclusion:** 研究突显了现有模型在完整5自由度视角变化下的主动感知和连贯视角规划方面的局限性，为具身3D视觉搜索评估提供了新的标准和挑战。
...
### [409] [Identifying Ethical Biases in Action Recognition Models](https://arxiv.org/abs/2604.17971)
*Ana Baltaretu,Pascal Benschop,Jan van Gemert*

Main category: cs.CV

> 该研究提出了一种使用合成视频数据来审计人类动作识别模型偏见的新框架，特别关注肤色等视觉身份属性对模型公平性的影响。

<details>
  <summary>Details</summary>

**Motivation:** 随着人类动作识别模型越来越多地部署在高风险环境中，但其在不同人群外表方面的公平性尚未得到充分分析，需要有效的审计方法来确保系统的公正性。

**Method:** 研究团队使用BEDLAM仿真平台生成具有完全可控视觉身份属性（如肤色）的合成视频数据，通过保持时间一致性的对照干预实验，单独测试单一属性变化对模型预测的影响。

**Result:** 实验表明，即使动作完全相同，一些流行的HAR模型在肤色上仍然表现出统计学显著偏见，揭示了模型可能编码了不必要的视觉关联，并存在系统性的群体性错误。

**Conclusion:** 该工作提供了一个审计HAR模型的框架，为开发更加透明、负责任的系统提供支持，有助于应对即将到来的监管标准要求。
...
### [410] [Trustworthy Endoscopic Super-Resolution](https://arxiv.org/abs/2604.18001)
*Julio Silva-Rodríguez,Ender Konukoglu*

Main category: cs.CV

> 本研究提出了一种提高内窥镜超分辨率系统可信度的方法，通过轻量级误差预测网络和符合风险控制原理，实时识别不可靠的重建区域并提供理论保证。

<details>
  <summary>Details</summary>

**Motivation:** 超分辨率模型在微创手术和诊断视频增强方面虽有应用前景，但存在产生幻觉结构和放大噪声的问题，在安全关键环境中限制了其可靠性，需要建立可信赖的识别机制。

**Method:** 集成轻量级误差预测网络，基于中间表示估计像素级重建误差，构建符合失败掩码(CFM)将预测转换为操作失败决策，应用符合风险控制原理控制误差限和误覆盖率。

**Result:** 在图像和视频超分辨率任务上验证了方法有效性，能够检测内窥镜和机器人手术中不可靠的重建，具有计算效率高、延迟低的特点，适合实时部署。

**Conclusion:** 这是首个提供模型无关、理论基础的实时内窥镜图像超分安全性改进方法，为安全关键医疗应用提供了可靠的技术保障。
...
### [411] [Multi-View Hierarchical Graph Neural Network for Sketch-Based 3D Shape Retrieval](https://arxiv.org/abs/2604.18019)
*Hang Cheng,Muyan He,Mingyu Fan,Chengfeng Xie,Xi Cheng,Long Zeng*

Main category: cs.CV

> 本文提出了MV-HGNN框架，通过多视图层次图神经网络和CLIP语义对齐，解决了草图3D检索中的特征表示弱和零样本性能差的问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有SBSR方法存在两个主要问题：1）采用简化的多视图特征聚合策略，忽略了视图间几何关系和多层细节，导致3D表示能力弱；2）传统方法受限于可见类别，在零样本场景下性能差。

**Method:** 提出MV-HGNN框架：1）构建视图级图结构，通过局部图卷积和全局注意力捕获相邻几何依赖和跨视图消息传递；2）引入视图选择器进行层次图粗化，逐步增大感受野并消除冗余视图干扰；3）利用CLIP文本嵌入作为语义原型，将草图和3D特征投影到共享语义空间；4）采用两阶段训练策略进行类别级检索，单阶段策略进行零样本检索。

**Result:** 在两个公开基准数据集上，无论是类别级还是零样本设置，MV-HGNN都显著优于现有最先进方法，验证了方法的有效性。

**Conclusion:** 该方法通过层次图神经网络有效建模多视图几何关系，结合CLIP语义对齐实现了类别无关的特征对齐，为草图3D检索提供了新的解决方案，在常规和零样本场景下均取得了突破性进展。
...
### [412] [CFSR: Geometry-Conditioned Shadow Removal via Physical Disentanglement](https://arxiv.org/abs/2604.18032)
*Pan Wang,Yihao Hu,Xiujin Liu,Hang Wang*

Main category: cs.CV

> CFSR是一个基于多模态先验的物理约束阴影去除框架，通过融合3D几何信息和基础模型语义，实现了纹理恢复与光照一致性之间的有效平衡。

<details>
  <summary>Details</summary>

**Motivation:** 传统阴影去除网络将图像恢复视为无约束映射，缺乏物理可解释性，难以平衡局部纹理恢复与全局光照一致性。

**Method:** CFSR通过自定义HVI颜色空间抑制阴影噪声，设计几何与语义双显式引导注意力机制，注入CLIP先验重构严重退化区域，并利用频率协作重建模块解耦高频遮挡边界与低频全局照明的重建过程。

**Result:** 在多个具有挑战性的基准测试中，CFSR达到了最先进的性能表现。

**Conclusion:** 通过物理约束的多模态融合方法，CFSR有效解决了阴影去除中的2D-3D域间隔，为纹理恢复和光照一致性提供了新的解决思路。
...
### [413] [HABIT: Chrono-Synergia Robust Progressive Learning Framework for Composed Image Retrieval](https://arxiv.org/abs/2604.18037)
*Zixu Li,Yupeng Hu,Zhiwei Chen,Shiqi Zhang,Qinlei Huang,Zhiheng Fu,Yinwei Wei*

Main category: cs.CV

> HABIT是一个用于解决组合图像检索中噪声三元组对应问题的鲁棒渐进学习框架，通过互知识估计和双重一致性渐进学习两大模块，显著提升了噪声环境下的检索性能。

<details>
  <summary>Details</summary>

**Motivation:** 组合图像检索(CIR)在实际应用中面临严重的噪声三元组对应(NTC)问题，主要源于三元组数据标注的高成本和主观性，导致现有方法在噪声数据下性能急剧下降。

**Method:** 提出HABIT框架，包含两个核心模块：1) 互知识估计模块通过计算组合特征与目标图像间互信息的转移率来量化样本纯净度；2) 双重一致性渐进学习模块通过历史模型与当前模型的协作机制，模拟人类习惯形成过程进行鲁棒学习。

**Result:** 在两个标准CIR数据集上的广泛实验表明，HABIT在不同噪声比例下均显著优于大多数方法，展现出卓越的鲁棒性和检索性能。

**Conclusion:** HABIT通过创新的互知识估计机制和模拟人类习惯形成的渐进学习策略，有效解决了CIR中的NTC问题，为实际应用中的组合图像检索提供了可靠的解决方案。
...
### [414] [GS-STVSR: Ultra-Efficient Continuous Spatio-Temporal Video Super-Resolution via 2D Gaussian Splatting](https://arxiv.org/abs/2604.18047)
*Mingyu Shi,Xin Di,Long Peng,Boxiang Cao,Anran Wu,Zhanfeng Feng,Jiaming Guo,Renjing Pei,Xueyang Fu,Yang Cao,Zhengjun Zha*

Main category: cs.CV

> GS-STVSR是一个基于2D高斯涂抹的超高效连续时空视频超分辨率框架，通过连续运动建模驱动高斯核的时空演化，完全避免了密集网格查询，实现了近乎恒定的推理时间。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于隐式神经表示的连续时空视频超分辨率方法依赖密集像素网格查询，计算成本随插值帧数线性增长，严重限制了推理效率。

**Method:** 提出基于2D高斯涂抹的GS-STVSR框架，利用协方差参数的时间稳定性进行轻量级中间拟合，设计光流引导的运动模块获取任意时刻的高斯位置和颜色，引入协方差重采样对齐模块防止协方差漂移，并提出自适应偏移窗口处理大规模运动。

**Result:** 在Vid4、GoPro和Adobe240数据集上实现了最先进的性能，在常规时间尺度下推理时间近乎恒定，在极端尺度下实现超过3倍加速。

**Conclusion:** GS-STVSR通过高斯涂抹技术有效解决了传统C-STVSR方法的效率瓶颈，在保持高质量的同时大幅提升了推理速度，具有很强的实际应用价值。
...
### [415] [INTENT: Invariance and Discrimination-aware Noise Mitigation for Robust Composed Image Retrieval](https://arxiv.org/abs/2604.18051)
*Zhiwei Chen,Yupeng Hu,Zhiheng Fu,Zixu Li,Jiale Huang,Qinlei Huang,Yinwei Wei*

Main category: cs.CV

> 本文提出INTENT网络来解决组合图像检索(CIR)中的噪声三元组对应关系问题，通过视觉不变组合和双目标判别学习两种策略分别处理跨模态和模态固有噪声，显著提升了模型的鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的组合图像检索方法假设所有样本都正确匹配，但现实中由于标注成本高，数据集中不可避免包含错误匹配的三元组。现有研究主要关注跨模态对应噪声，而忽略了模态固有噪声，导致在噪声环境下性能下降。

**Method:** INTENT包含两个主要组件：1) 视觉不变组合模块通过FFT进行因果干预生成干预组合特征，强制视觉不变性以忽略模态固有噪声；2) 双目标判别学习采用正负样本协同优化，构建基于忠诚度动态调整的可扩展决策边界。

**Result:** 在两个广泛使用的基准数据集上的实验表明，INTENT在处理噪声三元组方面表现出优越的性能和鲁棒性，有效提高了组合图像检索的准确率。

**Conclusion:** 本文首次系统性地将CIR中的噪声分为跨模态对应噪声和模态固有噪声两类，并提出了针对性的解决方案。INTENT为噪声环境下的组合图像检索提供了新的有效方法，为该领域的研究开辟了新方向。
...
### [416] [Enhancing Continual Learning of Vision-Language Models via Dynamic Prefix Weighting](https://arxiv.org/abs/2604.18075)
*Hyeonseo Jang,Hyuk Kwon,Kibok Lee*

Main category: cs.CV

> 提出动态前缀加权(DPW)框架，通过门控机制和残差加权策略，为前缀和适配器分配动态权重，显著提升了视觉语言模型在领域类别增量学习场景下的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的参数高效方法(如前缀调优和适配器)在VLMs增量学习中通常对向量权重进行归一化处理，忽略了不同输入token需要不同调整程度的问题。

**Method:** DPW框架包含两个组件：1)门控模块根据输入token重要性动态调整前缀权重；2)加权机制基于前缀调优权重的残差推导适配器输出权重，确保适配器仅在必要时使用。

**Result:** 实验结果表明，该方法在VLMs领域类别增量学习场景中达到了最先进性能。

**Conclusion:** 动态前缀加权框架通过智能分配前缀和适配器的权重，有效解决了现有方法中权重归一化带来的限制，为视觉语言模型的增量学习提供了更优解决方案。
...
### [417] [Class-specific diffusion models improve military object detection in a low-data domain](https://arxiv.org/abs/2604.18076)
*Ella P. Fokkinga,Jan Erik van Woerden,Thijs A. Eker,Sebastiaan P. Snel,Elfi I. S. Hofmeijer,Klamer Schutte,Friso G. Heslinga*

Main category: cs.CV

> 该研究表明使用扩散模型生成的合成图像可以有效提升低数据条件下军用车辆检测性能，特别是结合结构引导方法效果更佳。

<details>
  <summary>Details</summary>

**Motivation:** 在军用车辆检测任务中面临训练数据稀缺的问题，传统仿真管道成本高昂，而扩散模型生成的合成数据可能成为一种经济有效的替代方案。

**Method:** 使用FLUX.1扩散模型通过LoRA微调，仅用每类8或24张真实图像训练类别特定模型，生成合成数据；结合ControlNet的Canny边缘条件进行结构引导；使用RF-DETR检测器进行评估。

**Result:** FLUX生成的图像在8个真实样本下提升mAP50达8.0%；结构引导方法在数据极度稀缺时额外提升4.1% mAP50，但在数据充足时无明显优势。

**Conclusion:** 扩散模型是低数据域军用目标检测的有效数据增强手段，结构引导在数据极度限制时最有价值，为军用AI系统训练提供了传统仿真的替代方案。
...
### [418] [Autonomous Unmanned Aircraft Systems for Enhanced Search and Rescue of Drowning Swimmers: Image-Based Localization and Mission Simulation](https://arxiv.org/abs/2604.18088)
*Sascha Emanuel Zell,Toni Schneidereit,Armin Fügenschuh,Michael Breuß*

Main category: cs.CV

> 本研究提出了一种基于无人机机库系统的溺水救援方案，结合YOLO目标检测和离散事件仿真，实验证明可将救援响应时间缩短至标准救援的1/5。

<details>
  <summary>Details</summary>

**Motivation:** 溺水是水上活动的固有风险，但传统救援面临时间压力大、无监管区域多、目标定位困难、救援人员运输不便等挑战，需要技术创新来提高救援效率。

**Method:** 提出无人机机库系统（UAS），使用YOLOv3/v5/v8进行溺水者自动图像检测，创建专用数据集并评估不同版本和架构的性能；开发离散事件仿真模型模拟标准救援（SRO）和UAS救援的响应时间，优化UAS配置。

**Result:** 在德国劳西茨湖区测试显示，即使配置最小的UAS系统（2个机库各含1架无人机）也能将响应时间缩短至标准救援的1/5；YOLOv8在溺水者检测中表现最佳。

**Conclusion:** UAS辅助系统显著提高了水上救援的效率和成功率，通过自动化搜索识别和投放救生设备，为溺水救援提供了有效的技术解决方案。
...
### [419] [Decision-Aware Attention Propagation for Vision Transformer Explainability](https://arxiv.org/abs/2604.18094)
*Sehyeong Jo,Gangjae Jang,Haesol Park*

Main category: cs.CV

> 决策感知注意力传播(DAP)是一种结合梯度定位和注意力传播机制的可解释性方法，通过将预测决策信息注入到Transformer的注意力传播过程中，生成更具类别特异性、紧凑性和忠实性的归因图。

<details>
  <summary>Details</summary>

**Motivation:** Vision Transformer的预测过程难以解释，现有基于注意力的方法主要依赖原始注意力权重，不能明确反映最终决策且类别区分性有限；而基于梯度的方法虽然能突出类别相关证据，但未能充分利用Transformer的层次化注意力传播机制。

**Method:** 提出决策感知注意力传播(DAP)方法，通过基于梯度的定位估计token重要性，并将其整合到逐层注意力传播中，同时捕获注意力结构流和与最终预测最相关的证据。

**Result:** DAP方法生成的归因图相比传统注意力方法更具类别敏感性、紧凑性和忠实性。在不同规模的Vision Transformer变体上的大量实验表明，DAP在定量指标和定性可视化方面都优于现有基线方法。

**Conclusion:** 决策感知传播是改善ViT可解释性的有效方向，DAP方法成功结合了梯度定位和注意力传播的优势，为Transformer模型的解释提供了新的思路。
...
### [420] [Test-Time Perturbation Learning with Delayed Feedback for Vision-Language-Action Models](https://arxiv.org/abs/2604.18107)
*Zehua Zang,Xi Wang,Fuchun Sun,Xiao Xu,Lixiang Lium,Jiahuan Zhou,Jiangmeng Li*

Main category: cs.CV

> PDF是一种无需验证器的测试时自适应框架，通过不确定性数据增强和延迟反馈机制，显著提升了视觉-语言-动作模型在环境变化下的鲁棒性和决策性能。

<details>
  <summary>Details</summary>

**Motivation:** 视觉-语言-动作模型在序列决策中表现优异，但对环境微小变化（如物体姿态）极其脆弱。这种脆弱性源于轨迹过拟合问题，即模型过度关注动作与实体之间的伪相关性并重复记忆的动作模式。

**Method:** 提出PDF框架，包含三个核心组件：1）基于不确定性的数据增强和动作投票机制，减轻伪相关性；2）自适应调度器动态分配增强预算，平衡性能与效率；3）轻量级扰动模块通过延迟反馈调整动作置信度，纠正过度自信问题。

**Result:** 在LIBERO基准上成功率提升7.4%，在Atari上人类标准化分数提升10.3%，相比原始VLA和带测试时适应的VLA都展现出一致的改进效果。

**Conclusion:** PDF为多模态决策智能体的可靠测试时适应提供了实用路径，无需微调基础模型即可显著提升环境变化的适应能力。
...
### [421] [Can LLM-Generated Text Empower Surgical Vision-Language Pre-training?](https://arxiv.org/abs/2604.18134)
*Chengan Che,Chao Wang,Jiayuan Huang,Xinyue Chen,Luis C. Garcia-Peraza-Herrera*

Main category: cs.CV

> 本文提出了SurgLIME框架，通过LLM自动生成的大规模叙事数据LIME进行多模态医学视觉预训练，解决了医学文本标注成本高的问题。

<details>
  <summary>Details</summary>

**Motivation:** 手术视觉编码器在自监督学习方面取得了进展，但将其扩展到多模态推理任务受到专家文本标注成本的严重限制。

**Method:** 1. 创建LIME数据集：从开放获取的手术视频中使用LLM生成大规模叙事文本；2. 提出SurgLIME框架：采用LoRA适应的双编码器架构，引入自动置信度估计机制动态降低不确定文本权重。

**Result:** 在AutoLaparo和Cholec80基准测试上，SurgLIME实现了竞争性的零样本跨模态对齐性能，同时保持了视觉基础模型的强健线性探查性能。

**Conclusion:** SurgLIME成功解决了大规模多模态医学数据的可扩展性问题，在保持医学先验知识的同时实现了可靠的跨模态对齐。
...
### [422] [Soft Label Pruning and Quantization for Large-Scale Dataset Distillation](https://arxiv.org/abs/2604.18135)
*Xiao Lingao,Yang He*

Main category: cs.CV

> 本研究提出了一种名为LPQLD的大规模数据集蒸馏方法，通过标签剪枝和量化技术，在ImageNet-1K和ImageNet-21K上分别将软标签存储需求降低78倍和500倍，同时提升了模型性能。

<details>
  <summary>Details</summary>

**Motivation:** 大规模数据集蒸馏需要存储比压缩图像大30-200倍的辅助软标签，这违背了数据压缩的目标。根本问题在于图像多样性不足和监督多样性不足，导致需要大量数据增强。

**Method:** 提出LPQLD方法：1) 通过类感知批处理和批归一化监督增强图像多样性；2) 引入动态知识重用的标签剪枝改善标签多样性；3) 采用校准师生对齐的标签量化增强每图像的增强多样性。

**Result:** 在ImageNet-1K上减少78倍软标签存储并提升7.2%准确率，在ImageNet-21K上减少500倍存储并提升2.8%准确率，在不同网络架构和蒸馏方法上验证了方法的优越性。

**Conclusion:** LPQLD有效解决了大规模数据集蒸馏中的标签存储难题，通过系统性的多样性增强策略，实现了更高效的数据集压缩和更好的性能表现。
...
### [423] [Region-Grounded Report Generation for 3D Medical Imaging: A Fine-Grained Dataset and Graph-Enhanced Framework](https://arxiv.org/abs/2604.18145)
...
> 本文提出了首个面向低资源语言的3D PET/CT精细标注数据集VietPET-RoI，以及模拟放射科医生诊断流程的HiRRA框架，在临床评估指标上取得显著提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有的3D PET/CT医学报告自动生成方法面临高维体数据处理困难和标注数据集稀缺的挑战，特别是对低资源语言。现有黑盒方法缺乏对临床实际诊断流程中关注局部感兴趣区域(RoI)的模拟。

**Method:** 构建包含600个PET/CT样本和1960个手动标注RoI的大规模数据集VietPET-RoI；提出HiRRA框架，采用基于图的关系模块捕获RoI属性间的依赖关系，模仿专业放射科医生的诊断流程；引入RoI覆盖率和RoI质量指数等临床评估指标。

**Result:** 所提框架在BLEU和ROUGE-L指标上分别超越现有模型19.7%和4.7%，在临床指标上取得45.8%的显著提升，证明方法具有更好的临床可靠性和较少的幻觉现象。

**Conclusion:** 通过构建首个大规模低资源语言3D PET/CT RoI标注数据集和模仿临床诊断流程的新框架，有效解决了现有方法的局限性，为医学报告自动生成领域提供了重要的数据资源和方法贡献。
...
### [424] [Attention-ResUNet for Automated Fetal Head Segmentation](https://arxiv.org/abs/2604.18148)
*Ammar Bhilwarawala,Mainak Bandyopadhyay*

Main category: cs.CV

> 本研究提出了一种融合残差学习和多尺度注意力机制的Attention-ResUNet网络，在胎儿头部超声图像分割任务中取得了99.30%的Dice分数，显著优于现有的主流分割网络。

<details>
  <summary>Details</summary>

**Motivation:** 现有的深度学习方法在胎儿头部超声图像分割中面临低对比度、噪声干扰和解剖边界复杂等固有挑战，影响分割精度和临床应用。

**Method:** Attention-ResUNet将残差连接与注意力门控机制相结合，在解码器的四个层级集成注意力门，选择性关注解剖相关区域并抑制背景噪声，残差连接促进梯度流动和特征重用。

**Result:** 在HC18挑战赛数据集(n=200)上，该方法达到99.30±0.14%的平均Dice分数，显著超越ResUNet等五种基线方法，统计分析显示具有高度显著性差异(p<0.001)，参数量为14.7M，推理成本为45 GFLOPs。

**Conclusion:** Attention-ResUNet为自动化胎儿头部分割设立了新的最先进性能，同时保持了计算效率，其可解释性和解剖一致性激活模式为临床部署提供了重要支撑。
...
### [425] [AI-based Waste Mapping for Addressing Climate-Exacerbated Flood Risk](https://arxiv.org/abs/2604.18151)
*Steffen Knoblauch,Levi Szamek,Iddy Chazua,Benedcto Adamu,Innocent Maholi,Alexander Zipf*

Main category: cs.CV

> 本研究开发了一种基于AI的城市废物测绘工作流，利用开源航拍和街景图像高精度检测坦桑尼亚达累斯萨拉姆市的固体废物分布，揭示废物堆积热点区域及其与洪水风险的关系。

<details>
  <summary>Details</summary>

**Motivation:** 非洲快速扩张的城市面临气候变化导致的洪水威胁加剧，而不完善的废物管理常常堵塞排水系统，进一步放大洪水风险，急需有效的废物监测解决方案。

**Method:** 采用AI技术的工作流，结合公开可用的航拍和街景影像数据，通过与当地伙伴合作确保 culturally relevant 的数据标注，实现高精度城市固体废物检测和空间分布分析。

**Result:** 发现水道中的废物堆积量是邻近城市区域的3倍，识别出非正式定居点附近的废物热点区域，为气候加剧的洪水风险提供了关键空间信息。

**Conclusion:** 这种可扩展的AI方法相比传统人工测绘能实现全市范围监测，为城市规划、气候适应和可持续废物管理提供了可操作的见解，特别适用于洪水易发地区。
...
### [426] [Embedding Arithmetic: A Lightweight, Tuning-Free Framework for Post-hoc Bias Mitigation in Text-to-Image Models](https://arxiv.org/abs/2604.18167)
*Venkatesh Thirugnana Sambandham,Torsten Schön*

Main category: cs.CV

> 提出了一种推理时方法，通过嵌入算术技术在不改变模型权重的情况下有效缓解文本到图像模型的社会偏见，同时保持提示语义和视觉上下文不变。

<details>
  <summary>Details</summary>

**Motivation:** 现代文本到图像模型会加剧有害的社会偏见，这对道德部署构成挑战。现有方法难以在公平性和语义一致性之间取得平衡。

**Method:** 使用嵌入算术技术分析嵌入空间中偏见的结构并进行修正，无需改变模型权重、提示词或数据集；提出概念一致性评分(CCS)来严格评估语义保持。

**Result:** 在FLUX 1.0-Dev和Stable Diffusion 3.5-Large上的实验表明，条件嵌入空间形成复杂的纠缠流形；该方法在提高多样性的同时保持高概念一致性，显著优于现有基线。

**Conclusion:** 通过表征模型如何表示社会概念，建立了潜在空间的几何理解，为更透明、可控和公平的图像生成提供了原则性路径。
...
### [427] [Extending One-Step Image Generation from Class Labels to Text via Discriminative Text Representation](https://arxiv.org/abs/2604.18168)
*Chenxi Zhao,Chen Zhu,Xiaokun Feng,Aiming Hao,Jiashu Zhu,Jiachen Lei,Jiahong Wu,Xiangxiang Chu,Jufeng Yang*

Main category: cs.CV

> 本研究首次将MeanFlow框架扩展到文本条件生成，通过分析发现文本特征需要高分辨性才能在极少步生成中有效工作，并成功整合了LLM文本编码器，同时验证了对扩散模型的改进效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有MeanFlow方法主要集中在类别到图像的生成，但文本条件创作更灵活实用。相比之下，文本条件对模型理解能力要求更高，需要在MeanFlow框架中有效整合强大的文本编码器。

**Method:** 通过详细分析发现问题根源在于MeanFlow生成步数极少时需要高分辨性的文本特征表示。利用具备所需语义特性的LLM文本编码器，并调整MeanFlow生成过程以适配这一框架。

**Result:** 首次实现了高效的文本条件MeanFlow合成，在广泛使用的扩散模型上验证了方法，显示出显著的生成性能改进。

**Conclusion:** 该工作为未来文本条件MeanFlow生成研究提供了通用实用的参考框架，成功解决了文本编码器整合的关键挑战。
...
### [428] [CanonSLR: Canonical-View Guided Multi-View Continuous Sign Language Recognition](https://arxiv.org/abs/2604.18184)
*Xu Wang,Shengeng Tang,Wan Jiang,Yaxiong Wang,Lechao Cheng,Richang Hong*

Main category: cs.CV

> 本文提出CanonSLR框架，通过正面视角引导的师生学习和序列级软目标蒸馏等技术，解决了多视角连续手语识别中的视角变化问题，并建立了新的多视角数据集基准。

<details>
  <summary>Details</summary>

**Motivation:** 现有的连续手语识别方法主要在单视角下开发，对现实场景中视角变化的鲁棒性不足，限制了其实际应用价值。

**Method:** 提出正面视角锚定的师生学习策略、序列级软目标蒸馏方法减少跨视角语义差异，引入时序运动关系增强建模动态表征，并开发了多视角手语数据构建管道。

**Result:** 在PT14-MV和CSL-MV两个七视角基准上的实验表明，CanonSLR在多视角设置下一致优于现有方法，特别是在具有挑战性的非正面视角上展现出更强的鲁棒性。

**Conclusion:** 通过正面视角指导和跨视角知识转移，CanonSLR显著提升了多视角连续手语识别的性能，为该领域建立了新的实验基准和方法框架。
...
### [429] [DiffuSAM: Diffusion Guided Zero-Shot Object Grounding for Remote Sensing Imagery](https://arxiv.org/abs/2604.18201)
*Geet Sethi,Panav Shah,Ashutosh Gandhe,Soumitra Darshan Nayak*

Main category: cs.CV

> 提出一种结合扩散模型定位线索与先进分割模型的混合管道，在遥感图像目标定位任务中实现了超过14%的性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型在视觉任务中表现强大，本文探索其在遥感图像目标定位中的应用潜力，旨在解决复杂场景中目标定位的挑战。

**Method:** 构建混合管道，整合扩散模型的定位提示与RemoteSAM和SAM3等最先进的分割模型，利用生成式扩散模型和基础分割模型的互补优势实现鲁棒自适应的目标定位。

**Result:** 实验表明该方法显著提升定位性能，相比现有最先进方法在Acc@0.5指标上获得超过14%的提升。

**Conclusion:** 证明扩散模型与分割模型的结合在遥感图像目标定位中的有效性，为复杂场景目标检测提供了新的解决方案。
...
### [430] [A Comparative Evaluation of Geometric Accuracy in NeRF and Gaussian Splatting](https://arxiv.org/abs/2604.18205)
*Mikolaj Zielinski,Eryk Vykysaly,Bartlomiej Biesiada,Jan Baturo,Mateusz Capala,Dominik Belter*

Main category: cs.CV

> 本文提出了一个专注于几何准确性的神经渲染评估流程和包含19个多样化场景的基准测试，以补充传统视觉指标的不足。

<details>
  <summary>Details</summary>

**Motivation:** 现有的神经渲染评估方法主要关注视觉质量，但忽略了表面几何的保真度，这在需要精确几何信息的机器人应用（如抓取和物体操作）中是一个关键限制。

**Method:** 设计了一个评估流程，专门用于测试神经渲染方法在表面和形状保真度方面的表现，并构建了包含19个多样化场景的基准数据集进行系统评估。

**Result:** 提供了一套能够系统评估重建方法表面和形状保真度的工具，为几何准确性的量化评估提供了有效补充。

**Conclusion:** 通过专注于几何准确性的评估流程和基准测试，为神经渲染方法在机器人等需要精确几何信息的应用中提供了更全面的评估标准。
...
### [431] [Towards Symmetry-sensitive Pose Estimation: A Rotation Representation for Symmetric Object Classes](https://arxiv.org/abs/2604.18208)
*Andreas Kriegler,Csaba Beleznai,Margrit Gelautz*

Main category: cs.CV

> 本文提出SARR（Symmetry-Aware Rotation Representation）方法，通过修改三角恒等式来表示对称物体的姿态，解决对称物体姿态估计中的方向歧义问题，无需特定损失函数或3D模型即可实现高精度估计。

<details>
  <summary>Details</summary>

**Motivation:** 日常生活和工业中常见的对称物体存在固有的方向歧义问题，这阻碍了深度学习网络进行姿态估计的训练。现有解决方案通常需要设计特定的损失函数和网络架构，或采用对称不变评估指标，增加了系统复杂性。

**Method:** 研究者专注于旋转本身的数值表示，通过从物体形状导出的对称度修改三角恒等式，提出SARR表示方法。该表示是唯一的且相对于视觉外观连续的，可以使用标准CNN网络进行3D方向估计，并仅需深度图像或无纹理的RGB/灰度图像作为输入。

**Result:** 在T-LESS和ITODD两个流行的6D姿态估计数据集上，使用SARR的网络在敏感性cosine距离AR_C评估上超越了现有最佳方法，在使用传统对称不变度量时也达到了令人满意的性能。即使在不知道物体对称特性的推理场景中，SARR训练的网络也优于其他表示方法。

**Conclusion:** SARR方法有效解决了对称物体姿态估计的核心难题，无需3D模型和特殊网络设计即可实现高精度估计，为对称物体的姿态估计提供了一个简洁高效的解决方案。
...
### [432] [Memorize When Needed: Decoupled Memory Control for Spatially Consistent Long-Horizon Video Generation](https://arxiv.org/abs/2604.18215)
*Yanjun Guo,Zhengqiang Zhang,Pengfei Wang,Xinyue Liang,Zhiyuan Ma,Lei Zhang*

Main category: cs.CV

> 本文提出了一种解耦的视频生成框架，通过分离记忆建模与生成过程，显著提升了长时视频的空间一致性，同时保持了对新场景的生成能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有方法将记忆建模与视频生成纠缠在一起，导致在场景重访时内容不一致，在探索新区域时生成能力下降，且需要大量标注数据进行训练。

**Method:** 采用解耦框架，使用轻量级独立记忆分支学习空间一致性，引入混合记忆表示和逐帧交叉注意力机制，提出相机感知门控机制来调节记忆与生成模块的交互。

**Result:** 该方法在数据效率上表现优异，实验证明在视觉质量和空间一致性方面都达到了最先进的性能。

**Conclusion:** 通过解耦记忆条件与生成过程，本文方法不仅降低了训练成本，还同时提升了空间一致性和新场景生成能力，为长时视频生成提供了有效的解决方案。
...
### [433] [Instruction-as-State: Environment-Guided and State-Conditioned Semantic Understanding for Embodied Navigation](https://arxiv.org/abs/2604.18223)
*Zhen Liu,Yuhan Liu,Jinjun Wang,Jianyi Liu,Wei Song,Jingwen Fu*

Main category: cs.CV

> S-EGIU是一种将指令理解为动态状态的视觉语言导航方法，通过分层实现指令片段激活和语义细化，在多个VLN基准测试中取得显著性能提升。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言导航模型通常将指令编码为静态的全局表示，无法适应动态的视觉环境变化，限制了模型在不同感知状态下对指令理解的灵活性。

**Method:** 提出S-EGIU框架，将指令建模为动态状态变量。粗粒度层面：根据当前观测激活相关指令片段；细粒度层面：通过观测引导的词 grounding 和上下文建模细化激活片段的语义。

**Result:** 在REVERIE Test Unseen上获得+2.68% SPL性能提升，并在多个VLN基准测试中持续展现出效率增益。

**Conclusion:** 动态的指令-感知纠缠机制对于视觉语言导航任务至关重要，S-EGIU验证了这种动态指令理解方法的有效性和实用性。
...
### [434] [Is SAM3 ready for pathology segmentation?](https://arxiv.org/abs/2604.18225)
*Qiuyu Kong,Shakiba Sharifi,Zanxi Ruan,Yiming Wang,Marco Cristani*

Main category: cs.CV

> 该论文系统评估了SAM3在病理图像分割中的能力，发现其存在局限性：文本提示效果差、对视觉提示敏感且缺乏鲁棒性，与任务特定模型仍有显著差距。

<details>
  <summary>Details</summary>

**Motivation:** 数字病理分割面临组织级和细胞核级多尺度挑战，传统方法标注成本高且泛化能力差。SAM3提出的可提示概念分割为病理图像自动分割提供了新的可能性，需要系统评估其真实能力。

**Method:** 提出结构化评估协议，在零样本、少样本和监督设置下，使用不同提示策略（纯文本、视觉提示组合）在NuInsSeg、PanNuke和GlaS等病理数据集上系统评估SAM3性能。

**Result:** 1) 纯文本提示难以激活细胞核概念；2) 性能高度依赖视觉提示类型和数量；3) 少样本学习有提升但对视觉提示噪声缺乏鲁棒性；4) 与基于适配器的参考模型存在显著性能差距。

**Conclusion:** SAM3在病理图像分割中存在明显边界限制，无法直接胜任
...
### [435] [Medical Image Understanding Improves Survival Prediction via Visual Instruction Tuning](https://arxiv.org/abs/2604.18250)
...
> 本文提出了一种基于视觉-语言框架的3D CT图像理解方法，通过视觉指令调优技术利用大规模开源CT图像和放射学报告进行预训练，并在此基础上构建生存预测头部，实现了从CT图像和临床数据中改善生存预测的能力。

<details>
  <summary>Details</summary>

**Motivation:** 准确的预后和风险评估对临床决策制定和患者管理优化至关重要。然而，放射科医师评估的CT特征虽然有价值，但需要专业知识，且将丰富的视觉信息转换为文本摘要时会导致信息丢失。

**Method:** 构建视觉-语言框架进行3D CT图像理解，采用视觉指令调优技术利用大规模开源CT图像与放射学报告进行预训练，学习有临床意义的视觉-文本表示，然后在预训练模型之上添加生存预测头部以适应下游生存预测任务。

**Result:** 实验结果表明，该方法在生存预测方面优于基线方法，特别是在临床数据单独使用时预测能力较差的情况下效果更为突出，同时能够对预设问题生成有临床意义的语言响应。

**Conclusion:** 该视觉-学习框架成功整合了CT图像的视觉信息和放射学报告的文本信息，通过预训练和下游任务适配实现了更准确的生存预测，展示了多模态学习在临床决策支持中的潜力。
...
### [436] [Style-Based Neural Architectures for Real-Time Weather Classification](https://arxiv.org/abs/2604.18251)
*Hamed Ouattara,Pascal Houssam Salmane,Pierre Duthon,Frédéric Bernardin,Omar Ait Aider*

Main category: cs.CV

> 论文提出了三种基于神经网络的实时天气分类模型，其中两种创新模型（截断ResNet50及其注意力改进版）超越了现有技术，并具有良好的泛化能力。

<details>
  <summary>Details</summary>

**Motivation:** 开发能够从图像中实时识别天气状况（晴天、雨天、雪天、雾天）的深度学习模型，同时保持高效的计算性能。

**Method:** 提出了三种架构：1）基于多尺度PatchGAN的Multi-PatchGAN；2）通过演化算法确定的九层截断ResNet50；3）在截断ResNet50基础上引入Gram矩阵和注意力机制的增强版本，自动加权各层特征以优化风格特征提取。

**Result:** 后两种模型在多个公共数据库上超越了当前最先进方法，展现出强大的实时性能和优秀的泛化能力。

**Conclusion:** 这些专为天气检测设计的架构同样适用于动物物种识别、纹理分类、医学影像疾病检测和工业缺陷识别等其他基于外观的分类任务。
...
### [437] [Domain-Specialized Object Detection via Model-Level Mixtures of Experts](https://arxiv.org/abs/2604.18256)
*Svetlana Pavlitska,Malte Stüven,Beyza Keskin,J. Marius Zöllner*

Main category: cs.CV

> 该研究提出了一种用于目标检测的混合专家(MoE)架构，通过门控网络动态组合训练在语义不相交数据子集上的YOLO检测器，在BDD100K数据集上表现优于传统集成方法。

<details>
  <summary>Details</summary>

**Motivation:** MoE模型在图像分类和语义分割中已成功应用，但在目标检测领域使用有限，主要挑战在于如何融合密集和结构化预测。同时，传统集成方法缺乏可解释性。

**Method:** 提出模型级混合专家目标检测器，结合在语义不相交数据子集上训练的YOLO检测器，使用学习的门控网络动态加权专家贡献，研究不同的检测结果融合策略，通过平衡损失防止专家崩溃。

**Result:** 在BDD100K数据集实验表明，提出的MoE架构持续优于标准集成方法，并提供了专家在不同领域专业化的见解。

**Conclusion:** 模型级MoE架构是目标检测中传统集成的可行替代方案，能够在提升性能的同时提供更好的可解释性，为理解和提升检测器效果提供了新途径。
...
### [438] [Long-Text-to-Image Generation via Compositional Prompt Decomposition](https://arxiv.org/abs/2604.18258)
*Jen-Yuan Huang,Tong Lin,Yilun Du*

Main category: cs.CV

> PRISM提出了一种组合式的提示折射方法，通过提取长提示的组成部分表示，独立预测并合并输出来提升预训练文本到图像模型处理长序列输入的能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的文本到图像模型擅长处理简洁提示但在处理描述性长段落时表现不佳，主要因为训练数据中多为简短字幕；现有解决方案要么微调模型对长提示泛化性差，要么投影到普通提示空间损失细节保真度。

**Method:** PRISM采用组合式方法，使用轻量级模块从长提示中提取组成部分表示，让T2I模型对每个组件独立进行噪声预测，然后通过基于能量的关联方法合并输出到单一去噪步骤。

**Result:** PRISM在多种模型架构上表现优异，与在相同训练数据上微调的模型性能相当；在超过500个token的提示上，在挑战性公开基准测试中比基线模型性能提升7.4%。

**Conclusion:** PRISM有效解决了预训练文本到图像模型处理长序列输入的局限，实现了更好的泛化性能，为处理复杂描述性段落生成高质量图像提供了实用解决方案。
...
### [439] [Geometry-Guided 3D Visual Token Pruning for Video-Language Models](https://arxiv.org/abs/2604.18260)
*Han Li,Zehao Huang,Jiahui Fu,Naiyan Wang,Si Liu*

Main category: cs.CV

> Geo3DPruner是一种基于几何引导的3D视觉token剪枝框架，能剪枝90%的token同时保持90%以上性能

<details>
  <summary>Details</summary>

**Motivation:** 现有3D多模态大语言模型将3D场景表示为空间视频，但大量视觉token成为推理效率和上下文管理的主要瓶颈，现有剪枝方法忽略了空间视频的视图一致性和空间多样性

**Method:** Geo3DPruner通过几何感知全局注意力建模跨帧相关性，采用两阶段剪枝：体素内阶段选择每个体素内的代表性多视图特征，体素间阶段通过选择全局分布的体素子集保持空间多样性

**Result:** 在多个3D场景理解基准测试中，Geo3DPruner能剪枝90%的视觉token并保持超过90%的原始性能，显著优于现有的文本引导和视觉引导剪枝方法

**Conclusion:** 该几何引导的剪枝框架有效解决了3D空间视频中视觉token数量过多的问题，为高效的3D多模态大语言模型推理提供了实用解决方案
...
### [440] [MARCO: Navigating the Unseen Space of Semantic Correspondence](https://arxiv.org/abs/2604.18267)
*Claudia Cuttano,Gabriele Trivigno,Carlo Masone,Stefan Roth*

Main category: cs.CV

> MARCO是一个基于DINOv2的统一语义对应模型，通过 coarse-to-fine 目标和自蒸馏训练框架，实现了比扩散方法更小(3x)、更快(10x)且泛化能力更强的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有基于双编码器架构(结合DINOv2和扩散骨干)的语义对应方法虽然在测试关键点上表现准确，但在训练未见过的关键点上泛化能力差，存在基准性能与实际可用性之间的差距。

**Method:** 提出MARCO统一模型，采用新颖的训练框架：1) coarse-to-fine目标优化空间精度；2) 自蒸馏框架将稀疏监督扩展到标注区域之外，将少量关键点转换为密集语义对应。

**Result:** 在SPair-71k、AP-10K和PF-PASCAL数据集上达到SOTA，在细粒度定位阈值下提升8.9 PCK@0.01，对未见关键点(SPair-U)提升5.1，对未见类别(MP-100)提升4.7。

**Conclusion:** MARCO在保持3倍更小和10倍更快的优势下，显著提升了语义对应的泛化能力和细粒度定位精度，缩小了基准性能与实际应用之间的差距。
...
### [441] [LiquidTAD: An Efficient Method for Temporal Action Detection via Liquid Neural Dynamics](https://arxiv.org/abs/2604.18274)
*Zepeng Sun,Naichuan Zheng,Hailun Xia,Junjie Wu,Liwei Bao,Xiaotai Zhang*

Main category: cs.CV

> LiquidTAD是一种基于并行化液体神经网络的高效时序动作检测框架，实现了线性计算复杂度的同时保持了优异的性能，参数量减少63%。

<details>
  <summary>Details</summary>

**Motivation:** 当前基于Transformer的时序动作检测方法存在二次计算复杂度和参数冗余问题，难以在资源受限环境中部署。

**Method:** 提出LiquidTAD框架，使用并行化的ActionLiquid块替代传统自注意力层，基于闭式连续时间(CfC)公式实现可并行化的操作，通过学习的时间常数τ自适应调节时序敏感性。

**Result:** 在THUMOS-14数据集上达到69.46%的mAP，仅需10.82M参数（比基线减少63%），在ActivityNet-1.3和Ego4D上也验证了最优的精度-效率权衡，对时序采样变化具有更强的鲁棒性。

**Conclusion:** LiquidTAD首次将并行化LNN架构引入TAD领域，在保持优异检测性能的同时显著提升了计算效率和参数效率，推进了现代TAD框架的帕累托前沿。
...
### [442] [Spike-NVPT: Learning Robust Visual Prompts via Bio-Inspired Temporal Filtering and Discretization](https://arxiv.org/abs/2604.18284)
*Qiugang Zhan,Anning Jiang,Ran Tao,Ao Ma,Xiangyu Zhang,Xiurui Xie,Guisong Liu*

Main category: cs.CV

> Spike-NVPT是一种基于脉冲神经元的噪声鲁棒视觉提示调优方法，通过信号滤波和离散化生成稀疏二进制提示，在保持干净数据集准确性的同时显著提升噪声鲁棒性。

<details>
  <summary>Details</summary>

**Motivation:** 传统的提示调优方法学习的连续密集提示容易对输入噪声敏感，因为高容量的提示会过拟合任务无关的细节，需要在参数效率和噪声鲁棒性之间找到平衡。

**Method:** 设计基于脉冲神经元的信号滤波层，使用积分-放电机制积累任务相关信号并过滤瞬态噪声；通过脉冲离散化单元将滤波信号转换为稀疏二进制提示，作为强正则化器迫使模型anchor在最判别性和鲁棒的特征上。

**Result:** Spike-NVPT在噪声鲁棒性上表现卓越，比传统方法最高提升11.2%，同时在干净数据集上保持竞争性准确率；生成的二进制提示在部署时保持静态，推理过程零额外计算开销。

**Conclusion:** 这是首个利用脉冲神经元微调传统人工神经网络视觉模型的尝试，为开发更鲁棒、高效的自适应预训练视觉模型提供了新方向。
...
### [443] [Denoise and Align: Diffusion-Driven Foreground Knowledge Prompting for Open-Vocabulary Temporal Action Detection](https://arxiv.org/abs/2604.18313)
*Sa Zhu,Wanqian Zhang,Lin Wang,Jinchao Zhang,Cong Wang,Bo Li*

Main category: cs.CV

> DFAlign是一个基于扩散去噪的开放词汇时序动作检测框架，通过生成前景知识来指导动作-视频对齐，解决了语义不平衡问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有开放词汇时序动作检测方法难以缓解简洁抽象的动作标签与丰富复杂视频内容之间的语义不平衡问题，这不可避免地引入语义噪声并误导跨模态对齐。

**Method:** 采用'条件化-去噪-对齐'三步骤机制：1) 统一语义条件化模块统一动作共享和动作特定语义作为扩散去噪条件；2) 背景抑制去噪模块通过去噪过程逐步移除视频中的背景冗余生成前景知识；3) 前景提示对齐模块将提取的前景知识作为提示词注入文本表示中。

**Result:** 在两个OV-TAD基准数据集上实现了最先进的性能，提出的基于扩散的去噪框架有效提升了动作检测精度。

**Conclusion:** DFAlign首次利用扩散去噪生成前景知识指导动作-视频对齐，成功解决了语义不平衡问题，为开放词汇时序动作检测提供了新的解决方案。
...
### [444] [EVE: Verifiable Self-Evolution of MLLMs via Executable Visual Transformations](https://arxiv.org/abs/2604.18320)
*Yongrui Heng,Chaoya Jiang,Han Yang,Shikun Zhang,Wei Ye*

Main category: cs.CV

> EVE是一个基于可执行视觉变换的多模态大语言模型自进化框架，通过动态生成Python脚本创建VQA问题，避免了伪标签的质量退化问题，实现了可持续的自改进。

<details>
  <summary>Details</summary>

**Motivation:** 现有的多模态大语言模型自进化方法存在严重缺陷：基于伪标签的方法会因模型预测漂移而导致质量逐步退化，而基于模板的方法则受限于固定的变换集合，无法在难度和多样性上自适应调整。

**Method:** EVE采用挑战者-解决者双策略架构。挑战者维护并逐步扩展视觉变换代码示例队列，合成新的Python脚本执行动态视觉变换，生成具有绝对、执行验证真相答案的VQA问题。集成语义多样性和动态难度校准的多维奖励系统引导挑战者丰富代码队列并生成更具挑战性的任务。

**Result:** 大量实验证明EVE持续超越现有自进化方法，建立了可验证的MLLM自进化的可扩展范式，有效防止模式崩塌，促进两个策略之间的协同进化。

**Conclusion:** EVE框架通过完全绕过伪标签依赖，利用可执行视觉变换实现了鲁棒、连续的多模态大语言模型自进化，为该领域提供了一个新的可验证和可扩展的解决方案。
...
### [445] [OmniHuman: A Large-scale Dataset and Benchmark for Human-Centric Video Generation](https://arxiv.org/abs/2604.18326)
*Lei Zhu,Xing Cai,Yingjie Chen,Yiheng Li,Binxin Yang,Hao Liu,Jie Chen,Chen Li,Jing LYu*

Main category: cs.CV

> OmniHuman是一个大规模多场景数据集和基准测试系统，旨在解决以人为中心的音视频生成模型在复杂现实场景中的保真度挑战。

<details>
  <summary>Details</summary>

**Motivation:** 现有音视频联合生成模型在复杂现实场景中生成高保真度以人为中心视频存在重大挑战，根本原因在于现有数据集存在三个维度结构性缺陷：全局场景和相机多样性有限、交互建模稀疏、个体属性对齐不足。

**Method:** 构建OmniHuman大规模多场景数据集，提供分层标注（视频级场景、帧级交互、个体级属性）；开发全自动高质量数据收集和多模态标注流水线；建立OmniHuman基准测试(OHBench)三级评估系统。

**Result:** 创建了OmniHuman数据集和OHBench评估系统，引入与人类感知高度一致的评估指标，在全局场景、关系交互和个体属性三个维度提供全面诊断，填补了现有基准测试的空白。

**Conclusion:** OmniHuman通过全面的数据集设计和科学的评估体系，为以人为中心的音视频合成提供了重要的基础设施支持，推动该领域在复杂现实场景中的发展。
...
### [446] [AdaCluster: Adaptive Query-Key Clustering for Sparse Attention in Video Generation](https://arxiv.org/abs/2604.18348)
*Haoyue Tan,Shengnan Wang,Yulin Qiao,Juncheng Zhang,Youhui Bai,Ping Gong,Zewen Jin,Cheng Li*

Main category: cs.CV

> AdaCluster是一种无训练的自适应聚类框架，通过优化的查询和键向量聚类方法，在保持质量的前提下将视频扩散Transformer推理速度提升1.67-4.31倍。

<details>
  <summary>Details</summary>

**Motivation:** 视频扩散Transformer面临二次注意力复杂度导致的推理延迟问题，现有稀疏注意力方法要么忽略语义相似性，要么无法适应跨层的异构token分布，造成模型性能下降。

**Method:** AdaCluster采用角度相似性保持的查询向量聚类实现更高压缩，设计欧几里得相似性保持的键向量聚类，涵盖聚类数量分配、阈值自适应聚类和高效关键聚类选择策略。

**Result:** 在CogVideoX-2B、HunyuanVideo和Wan-2.1模型上的单A40 GPU实验表明，该方法可实现1.67-4.31倍加速，质量损失可忽略不计。

**Conclusion:** AdaCluster成功解决了视频扩散Transformer的推理效率瓶颈，通过自适应聚类机制有效平衡了计算复杂度和生成质量，为实际部署提供了可行的加速方案。
...
### [447] [LBFTI: Layer-Based Facial Template Inversion for Identity-Preserving Fine-Grained Face Reconstruction](https://arxiv.org/abs/2604.18358)
*Zixuan Shen,Zhihua Xia,Kaikai Gan,Peipeng Yu*

Main category: cs.CV

> 本文提出了LBFTI（基于分层的人脸模板反演方法），通过分阶段生成面部前景、中景和背景三层结构，实现了比现有方法更好的人脸重建效果，在机器认证性能上提升25.3%，同时保持了良好的视觉感知效果。

<details>
  <summary>Details</summary>

**Motivation:** 人脸模板虽然符合数据最小化原则，但模板反演技术带来了严重的隐私泄露风险。现有方法在重建具有身份保持性的细粒度人脸图像方面存在不足，需要一种更有效的人脸模板反演方法来应对这一挑战。

**Method:** LBFTI方法将人脸图像分解为三层结构：前景层（眉毛、眼睛、鼻子、嘴巴）、中景层（皮肤）和背景层（其他部分）。采用三阶段训练策略：1）独立精炼生成前景和中景层；2）融合前景和中景层并进行模板二次注入，生成包含背景层的完整全景人脸图像；3）对所有模块进行联合微调，优化层间协调和身份一致性。

**Result:** 实验表明LBFTI在机器认证性能上优于最先进方法，真实接受率(TAR)提升25.3%，在人类感知方面也达到更好的相似性效果，通过定量指标和问卷调查得到验证。

**Conclusion:** 分层框架和三阶段训练策略有效提升了人脸模板反演的质量，在保护隐私的同时实现了更准确的身份保持性人脸重建，为人脸识别系统中的隐私保护提供了新的技术思路。
...
### [448] [EAST: Early Action Prediction Sampling Strategy with Token Masking](https://arxiv.org/abs/2604.18367)
*Iva Sović,Ivan Martinović,Marin Oršić*

Main category: cs.CV

> EAST是一个高效早期动作预测框架，通过随机时间步采样和联合学习策略，实现了在多个基准数据集上的SOTA性能，同时显著降低了计算成本。

<details>
  <summary>Details</summary>

**Motivation:** 早期动作预测旨在动作完全展开前进行预判，但由于视觉证据有限，这一任务极具挑战性。现有方法在处理不同观察比例时的泛化能力不足。

**Method:** 1) 随机训练策略：采样分离观察和未观察视频帧的时间步，使单一模型能够泛化到所有测试时观察比例；2) 联合学习：同时学习观察和未来(oracle)表征；3) 令牌掩码：将内存使用减半，训练速度提升2倍。

**Result:** 在NTU60、SSv2和UCF101数据集上分别超越之前最佳方法10.1、7.7和3.9个百分点，实现了新的state-of-the-art性能。编码器仅模型也能表现优异。

**Conclusion:** EAST框架证明了通过简单的随机训练策略和联合学习可以有效解决早期动作预测中的泛化问题，而令牌掩码技术显著提升了模型的实用性，为实时动作预测应用提供了可行方案。
...
### [449] [DSA-CycleGAN: A Domain Shift Aware CycleGAN for Robust Multi-Stain Glomeruli Segmentation](https://arxiv.org/abs/2604.18368)
*Zeeshan Nisar,Friedrich Feuerhake,Thomas Lampert*

Main category: cs.CV

> 针对数字病理学中多染色分割的挑战，研究提出DSA-CycleGAN方法来解决CycleGAN在染色转换中引入噪声的问题，并在肾小球分割任务中验证了其优越性能。

<details>
  <summary>Details</summary>

**Motivation:** 数字病理学中的染色间和染色内变异会降低模型分割性能，而为每种染色标注数据成本昂贵。现有CycleGAN方法在处理一对多染色分布时会产生噪声。

**Method:** 提出域偏移感知CycleGAN（DSA-CycleGAN），通过减少染色转换过程中的噪声来改善模型性能，并与其他机器学习方法在多染色肾小球分割任务上进行了比较评估。

**Result:** 实验证明DSA-CycleGAN不仅提高了肾小球分割性能，在减少噪声方面也优于其他方法，特别是在生物学上不同染色之间的转换中效果更加明显。

**Conclusion:** DSA-CycleGAN有效解决了CycleGAN在多染色转换中的噪声问题，为数字病理学中的多染色分割提供了一种 improved解决方案。
...
### [450] [Towards Robust Text-to-Image Person Retrieval: Multi-View Reformulation for Semantic Compensation](https://arxiv.org/abs/2604.18376)
*Chao Yuan,Yujian Zhao,Haoxuan Xu,Guanglin Niu*

Main category: cs.CV

> 本文提出了一种基于大语言模型的语义补偿框架(MVR)，通过多视角语义重构和特征补偿来解决文本到图像检索中的表达式漂移问题，在无需训练的情况下实现了SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 在文本到图像人物检索任务中，自然语言表达的多样性和视觉语义的隐含性导致表达式漂移问题——语义等效的文本因措辞变化在嵌入空间中表现出显著特征差异，从而降低了图像-文本对齐的鲁棒性。

**Method:** 框架包含三个核心组件：1) 多视角重构(MVR)：双分支提示策略结合关键特征引导和多样性感知重写，生成语义等效但分布多样的文本变体；2) 文本特征鲁棒性增强：无需训练的潜空间补偿机制通过多视角特征均值池化和残差连接抑制噪声干扰；3) 视觉语义补偿：VLM生成多视角图像描述，通过共享文本重构进一步增强。

**Result:** 实验表明该方法无需训练即可显著提升原始模型的准确性，在三个文本到图像人物检索数据集上均达到SOTA性能。

**Conclusion:** 通过LLM驱动的多视角语义补偿框架，有效解决了表达式漂移问题，为文本到图像检索提供了一种无需训练的稳健解决方案。
...
### [451] [One-Step Diffusion with Inverse Residual Fields for Unsupervised Industrial Anomaly Detection](https://arxiv.org/abs/2604.18393)
*Boan Zhang,Wen Li,Guanhua Yu,Xiyang Liu,Wenchao Chen,Long Tian*

Main category: cs.CV

> 本研究提出了OSD-IRF，一种基于反向残差场的单步扩散模型，用于解决无监督工业异常检测中扩散模型推理速度慢的问题，在保持竞争性能的同时实现约2倍推理加速。

<details>
  <summary>Details</summary>

**Motivation:** 扩散模型在无监督工业异常检测中表现优异，但由于其迭代去噪特性导致推理速度过慢，限制了实际应用。现有方法需要多步迭代才能完成检测，难以满足工业场景的实时性要求。

**Method:** 首先在正常数据上训练无条件DDPM模型；然后利用训练好的噪声估计函数预测测试样本的反向残差场；最后通过评估IRF在高斯分布下的概率密度并与阈值比较来进行异常检测。该方法利用IRF在去噪过程中任意相邻时间步都成立的特性，实现单步推理。

**Result:** 在三个广泛使用的无监督工业异常检测基准测试上，模型在六个指标上达到SOTA或竞争性性能，同时在不使用蒸馏的情况下获得约2倍的推理速度提升。

**Conclusion:** 本研究发现异常在IRF空间中具有可区分性，这一发现在以往研究中很少被报道。提出的OSD-IRF方法有效平衡了检测性能和推理速度，为工业异常检测的实际应用提供了新的解决方案。
...
### [452] [MedProbeBench: Systematic Benchmarking at Deep Evidence Integration for Expert-level Medical Guideline](https://arxiv.org/abs/2604.18418)
...
> MedProbeBench是首个基于高质量临床指南的专家级基准，用于评估大语言模型在深度医学研究中的证据整合能力。

<details>
  <summary>Details</summary>

**Motivation:** 现有的评估基准无法测试大语言模型在现实医学工作流程中多步证据整合和专家级判断的能力，而临床指南开发正需要这种深度证据整合能力。

**Method:** 构建了基于高质量临床指南的基准数据集，并提出MedProbe-Eval评估框架，包含1200+适应性评分标准和5130+原子声明的细粒度证据验证机制。

**Result:** 对17个大语言模型和深度研究代理的评估显示，它们在证据整合和指南生成方面存在关键缺陷，与专家级临床指南开发能力仍有显著差距。

**Conclusion:** 当前大语言模型在深度医学研究方面的能力远未达到专家级临床指南开发的要求，需要在证据整合能力上取得实质性突破。
...
### [453] [Revisiting Change VQA in Remote Sensing with Structured and Native Multimodal Qwen Models](https://arxiv.org/abs/2604.18429)
*Yakoub Bazi,Mohamad M. Al Rahhal,Mansour Zuair,Faroun Mohamed*

Main category: cs.CV

> 该研究重新评估了变化视觉问答(Change VQA)基准，发现最新的视觉语言模型(VLMs)相比之前的专业基线方法有显著提升，且原生多模态模型比结构化视觉语言流水线更有效。

<details>
  <summary>Details</summary>

**Motivation:** 变化视觉问答在双时相遥感图像变化理解中具有重要意义，但在现代多模态模型背景下仍缺乏充分研究。虽然已有VLMs用于时序遥感图像理解，但Change VQA任务仍未被深入探索。

**Method:** 研究者使用统一的LoRA设置，比较了Qwen3-VL（结构化视觉语言流水线，含多深度视觉调节和全注意力解码器）和Qwen3.5（原生多模态模型，单阶段对齐与混合解码器主干网络）在CDVQA基准上的表现。

**Result:** 实验表明最新VLMs超越早期专用基线，性能并非随模型规模单调增长，原生多模态模型比结构化视觉语言流水线更有效，紧密集成的多模态主干网络对性能贡献更大。

**Conclusion:** 对于遥感图像中语言驱动的语义变化推理，紧密集成的多模态主干网络比模型规模或显式的多深度视觉调节更重要，为Change VQA任务的发展提供了新方向。
...
### [454] [Progressive Online Video Understanding with Evidence-Aligned Timing and Transparent Decisions](https://arxiv.org/abs/2604.18459)
*Kecheng Zhang,Zongxin Yang,Mingfei Han,Haihong Hao,Yunzhi Zhuge,Changlin Li,Junhan Zhao,Zhihui Li,Xiaojun Chang*

Main category: cs.CV

> 本文提出了Thinking-QwenVL框架，通过解耦推理控制和记忆集成，解决了在线视频理解中响应时机与视觉证据对齐的关键问题，在StreamingBench上达到71.6%的先进性能。

<details>
  <summary>Details</summary>

**Motivation:** 传统视频LLM在离线评估中忽视了在线流媒体环境下的关键需求：需要在视频中首次出现足够证据时精确响应。这种在线范式带来了决策透明度不足、响应时机难以对齐、以及严格计算预算下维持全局因果一致性理解等挑战。

**Method:** 提出了一个解耦推理控制与记忆集成的新框架。核心组件包括：1) 主动思维决策器(ATDM) - 通过可观测的进度ρ和置信度c指标外部化决策过程，实现响应时间tr与首次充分证据时间t*的精确匹配；2) 分层渐进语义集成(HPSI)模块 - 使用可学习的多级聚合令牌构建高效的内存系统，在不超出令牌预算的情况下建立丰富的全局认知状态。

**Result:** 在关键在线视频理解基准测试中表现优异：StreamingBench上达到71.6%，OVOBench上达到46.9%。相比之前最优方法，Thinking-QwenVL将StreamingBench准确率从67.63%提升至71.60%，验证了ATDM和HPSI的有效性。

**Conclusion:** 该框架为证据对齐和透明的在线视频分析提供了稳健解决方案，通过独特的推理控制和记忆设计，在严格的计算约束下实现了高精度的在线视频理解，为视觉智能体在真实环境中的应用奠定了基础。
...
### [455] [Asset Harvester: Extracting 3D Assets from Autonomous Driving Logs for Simulation](https://arxiv.org/abs/2604.18468)
...
> Asset Harvester是一个将稀疏真实驾驶日志中的物体观测转换为完整、可用于仿真的3D资产的端到端系统，通过组合多组件设计和SparseViewDiT技术解决了有限视角等现实数据挑战。

<details>
  <summary>Details</summary>

**Motivation:** 神经场景重建可将驾驶日志转换为交互式3D仿真环境，但无法产生代理操纵和大视角新视图合成所需的完整3D物体资产，需要开发能够从稀疏现实驾驶观测中生成完整资产的解决方案。

**Method:** 开发了系统级设计，包含：以物体为中心的大规模训练元组整理、跨异构传感器的几何感知预处理、结合稀疏视角条件多视图生成和3D高斯提升的鲁棒训练方案，以及专门针对有限视角设计的SparseViewDiT模型。

**Result:** 通过混合数据整理、增强和自蒸馏技术，该系统能够可扩展地将稀疏自动驾驶物体观测转换为可重用的3D资产。

**Conclusion:** Asset Harvester为自动驾驶仿真提供了从真实驾驶日志到完整3D资产的端到端解决方案，通过创新的系统设计有效解决了现实数据中的视角限制等挑战，显著提升了仿真的完整性和实用性。
...
### [456] [SemLT3D: Semantic-Guided Expert Distillation for Camera-only Long-Tailed 3D Object Detection](https://arxiv.org/abs/2604.18476)
*Hao Vo,Khoa Vo,Thinh Phan,Ngo Xuan Cuong,Gianfranco Doretto,Hien Nguyen,Anh Nguyen,Ngan Le*

Main category: cs.CV

> SemLT3D是一个语义引导的专家蒸馏框架，通过语义先验增强纯视觉3D目标检测中稀有类别的表达能力，解决了长尾不平衡、类间模糊和类内多样性问题。

<details>
  <summary>Details</summary>

**Motivation:** 现有纯视觉3D目标检测方法主要关注整体性能而忽略了真实世界数据集中严重的长尾不平衡问题。稀有但安全关键的类别（如儿童、婴儿车、急救车辆）表示不足，导致模型学习偏向，性能下降。同时，类间模糊和类内多样性进一步阻碍了可靠的长尾识别。

**Method:** SemLT3D框架包含两个核心组件：(1)语言引导的混合专家模块根据语义亲和度将3D查询路由到专门的专家，帮助模型更好地区分混淆类别并专门处理尾部分布；(2)语义投影蒸馏管道将3D查询与CLIP的2D语义对齐，产生更连贯和更有区别性的特征。

**Result:** 虽然主要以长尾不平衡问题为动机，SemLT3D中的语义结构化学习也提高了更广泛外观变化和具有挑战性的边缘情况下的鲁棒性，为实现更可靠的纯视觉3D感知提供了原则性步骤。

**Conclusion:** SemLT3D通过语义引导的专家蒸馏框架有效解决了纯视觉3D目标检测中的长尾不平衡问题，不仅提升了稀有类别的检测性能，还增强了模型在复杂场景下的整体鲁棒性，为更安全的自动驾驶感知系统提供了重要贡献。
...
### [457] [XEmbodied: A Foundation Model with Enhanced Geometric and Physical Cues for Large-Scale Embodied Environments](https://arxiv.org/abs/2604.18484)
...
> XEmbodied是一个云端基础模型，通过结构化3D适配器和高效图像-具身适配器，将几何意识和物理线索集成到现有的VLM中，显著提升了空间推理和具身理解的性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有VLA模型训练依赖于通用VLM，但这些模型缺乏几何推理和领域语义，因为其2D图像-文本预训练与复杂环境需求不匹配。

**Method:** 提出XEmbodied模型，使用结构化3D适配器集成几何表示，通过高效图像-具身适配器将物理信号蒸馏到上下文token中，并采用渐进式领域课程学习和强化学习后训练。

**Result:** 在18个公共基准测试中表现出色，显著提升了空间推理、交通语义、具身可供性和分布外泛化能力，适用于大规模场景挖掘和具身VQA任务。

**Conclusion:** XEmbodied成功地为VLM赋予了3D几何感知和物理交互能力，在保持通用能力的同时，在具身智能应用中实现了强大的性能提升。
...
### [458] [S2H-DPO: Hardness-Aware Preference Optimization for Vision-Language Models](https://arxiv.org/abs/2604.18512)
*Nitish Shukla,Surgan Jandial,Arun Ross*

Main category: cs.CV

> 本文提出Simple-to-Hard (S2H)学习框架，通过构建三个层级的多图像偏好数据来提升视觉语言模型的全局视觉搜索和跨图像对比能力，在保持单图像性能的同时显著提升多图像推理表现。

<details>
  <summary>Details</summary>

**Motivation:** 现有视觉语言模型在多图像推理方面存在关键能力缺陷，主要局限于预指定图像索引的局部推理，缺乏核心的全局视觉搜索和自主跨图像对比能力。

**Method:** 提出S2H学习框架，系统性地构建三个层级的多图像偏好数据：单图像局部推理、多图像局部对比、全局视觉搜索。基于提示驱动的复杂性生成选择/拒绝对，适用于不同模型。

**Result:** 在LLaVA和Qwen-VL模型上的广泛评估表明，该方法显著提升了多图像推理性能，在多个基准测试上比基线方法有重大改进，同时保持了强大的单图像推理能力。

**Conclusion:** 该研究填补了多图像对齐方法的能力缺口，为整体视觉偏好对齐技术做出贡献，推动了视觉语言模型在多图像理解方面的最新进展。
...
### [459] [UDM-GRPO: Stable and Efficient Group Relative Policy Optimization for Uniform Discrete Diffusion Models](https://arxiv.org/abs/2604.18518)
*Jiaqi Wang,Haoge Deng,Ting Pan,Yang Liu,Chengyuan Wang,Fan Zhang,Yonggang Qi,Xinlong Wang*

Main category: cs.CV

> 本文提出了首个将均匀离散扩散模型(UDM)与强化学习相结合的框架	extbackslash Ours，通过将最终清洁样本视为动作和重构轨迹等创新方法，显著提升了文本到图像生成任务的性能。

<details>
  <summary>Details</summary>

**Motivation:** 虽然UDM在离散生成建模中表现出色，但其与强化学习的集成尚未得到充分探索。直接应用GRPO到UDM会导致训练不稳定和性能提升有限，这表明需要新的解决方案。

**Method:** 	extbackslash Ours框架基于两个关键洞察：(i)将最终清洁样本作为动作提供更准确稳定的优化信号；(ii)通过扩散前向过程重构轨迹更好地与预训练分布对齐。此外还提出了Reduced-Step和CFG-Free两种策略来提升训练效率。

**Result:** 在多个T2I任务上显著提升基础模型性能：GenEval准确率从69%提升至96%，PickScore从20.46提升至23.81，OCR基准准确率从8%提升至57%，在连续和离散设置下都达到了最先进性能。

**Conclusion:** 	extbackslash Ours成功解决了UDM与RL集成的挑战，不仅实现了训练稳定性和显著的性能提升，还展现了良好的泛化能力，为离散扩散模型的强化学习训练开辟了新的研究方向。
...
### [460] [MetaCloak-JPEG: JPEG-Robust Adversarial Perturbation for Preventing Unauthorized DreamBooth-Based Deepfake Generation](https://arxiv.org/abs/2604.18537)
*Tanjim Rahaman Fardin,S M Zunaid Alam,Mahadi Hasan Fahim,Md Faysal Mahfuz*

Main category: cs.CV

> 本文提出了MetaCloak-JPEG，一种JPEG抵抗的面部保护方法，通过可微分JPEG层克服了现有方案在社交媒体压缩下的保护失效问题。

<details>
  <summary>Details</summary>

**Motivation:** 当前的主题驱动文本到图像合成技术（如DreamBooth）使得攻击者仅需少量面部图像就能生成有害的深度伪造内容。现有保护方法（PhotoGuard、Anti-DreamBooth、MetaCloak）都存在结构性缺陷：没有考虑社交媒体平台普遍应用的JPEG压缩流程，导致60-80%的保护信号被丢弃而失效。

**Method:** MetaCloak-JPEG集成了基于直通估计器（STE）构建的可微分JPEG（DiffJPEG）层，前向传播应用标准JPEG压缩，反向传播将round()替换为恒等函数。该方法将DiffJPEG嵌入到JPEG感知的EOT分布中（70%的增强包含DiffJPEG），并在双层元学习循环中采用课程质量因子调度（QF从95到50）。

**Result:** 在l-inf扰动预算eps=8/255下，MetaCloak-JPEG达到32.7 dB PSNR，91.3%的JPEG存活率，在训练内存预算4.1GB内，在所有9个评估的JPEG质量因子上都优于PhotoGuard（9/9胜出，平均去噪损失增益+0.125）。

**Conclusion:** MetaCloak-JPEG成功解决了现有面部保护系统在JPEG压缩环境下的结构性盲点，为抵御基于个性化扩散模型的深度伪造攻击提供了更加鲁棒和实用的保护方案。
...
### [461] [Advancing Vision Transformer with Enhanced Spatial Priors](https://arxiv.org/abs/2604.18549)
*Qihang Fan,Huaibo Huang,Mingrui Chen,Hongmin Liu,Ran He*

Main category: cs.CV

> EVT是一种改进的视觉Transformer骨干网络，通过欧几里得距离衰减和空间无关分组方法，有效解决了传统ViT缺乏空间先验和计算复杂度高的问题，在多个视觉任务中表现优异。

<details>
  <summary>Details</summary>

**Motivation:** 针对Vision Transformer中Self-Attention机制缺乏显式空间先验和二次计算复杂度的问题，作者在RMT的基础上进一步改进，提出更加合理和灵活的空间信息建模方法。

**Method:** EVT采用欧几里得距离衰减替代曼哈顿距离来更精确地建模空间关系，并使用简化的空间无关分组方法代替RMT的水平垂直分解注意机制，提高模型的灵活性和可控制性。

**Result:** 在图像分类、目标检测、实例分割和语义分割等多个任务上进行了广泛实验，在ImageNet-1k上无需额外训练数据就达到了86.6%的top1准确率，表现出色。

**Conclusion:** EVT成功地将空间先验信息引入Self-Attention机制，克服了RMT的一些局限性，为计算机视觉任务提供了一个更加精细和适应性强的视觉骨干网络解决方案。
...
### [462] [SynAgent: Generalizable Cooperative Humanoid Manipulation via Solo-to-Cooperative Agent Synergy](https://arxiv.org/abs/2604.18557)
*Wei Yao,Haohan Ma,Hongwen Zhang,Yunlian Sun,Liangjun Xing,Zhile Yang,Yuanjun Guo,Yebin Liu,Jinhui Tang*

Main category: cs.CV

> SynAgent是一个通过单人技能迁移实现可控制多人协作操作的学习框架，解决了训练数据稀缺和泛化能力有限的难题。

<details>
  <summary>Details</summary>

**Motivation:** 可控制的人形协作操作是具身智能的基础问题，但面临严重的数据稀缺、多智能体协调复杂性以及跨对象泛化能力有限等挑战。

**Method:** 提出Solo-to-Cooperative Agent Synergy框架：1) 基于Delaunay四面体剖分构建交互网格的保持交互重定位方法；2) 单智能体预训练和分布式多智能体PPO适配范式；3) 通过多教师蒸馏训练的轨迹条件生成策略。

**Result:** 实验表明SynAgent在协作模仿和轨迹条件控制任务上显著优于基线方法，并能泛化到不同的物体几何形状。

**Conclusion:** 该框架成功实现了从单人到多人的技能迁移，为可控制的协作操作提供了有效的解决方案，具有良好的泛化能力和实用性。
...
### [463] [AnchorSeg: Language Grounded Query Banks for Reasoning Segmentation](https://arxiv.org/abs/2604.18562)
*Rui Qian,Chuanhang Deng,Qiang Huang,Jian Xiong,Mingxuan Li,Yingbo Zhou,Wei Zhai,Jintao Chen,Dejing Dou*

Main category: cs.CV

> AnchorSeg通过构建结构化的语言根植查询库，将推理分割重构为条件生成过程，显式解耦空间定位和语义推理，在ReasonSeg数据集达到SOTA性能。

<details>
  <summary>Details</summary>

**Motivation:** 现有的推理分割方法使用单一分割令牌同时编码语义推理和空间定位，这种隐式编码限制了模型显式分离'分割什么'和'在哪里分割'的能力。

**Method:** 提出AnchorSeg框架：(1)构建有序查询库序列，包含捕获中间语义状态的推理令牌和提供空间根植的分割锚令牌；(2)将空间条件建模为图像令牌上的因子化分布，锚查询确定定位信号，上下文查询提供语义调制；(3)提出令牌-掩码循环一致性(TMCC)训练目标，确保跨分辨率对齐。

**Result:** 在ReasonSeg测试集上取得67.7% gIoU和68.1% cIoU的state-of-the-art结果，验证了显式解耦空间定位和语义推理的有效性。

**Conclusion:** 通过结构化的语言根植查询库显式解耦空间定位和语义推理是提升推理分割性能的有效途径，为复杂隐式文本查询的像素级掩码生成提供了新思路。
...
### [464] [MultiWorld: Scalable Multi-Agent Multi-View Video World Models](https://arxiv.org/abs/2604.18564)
*Haoyu Wu,Jiwen Yu,Yingtian Zou,Xihui Liu*

Main category: cs.CV

> MultiWorld是一个统一的多智能体多视角世界建模框架，能够精确控制多个智能体同时保持多视角一致性。

<details>
  <summary>Details</summary>

**Motivation:** 现有的视频世界模型主要局限于单智能体场景，无法捕捉现实世界多智能体系统中的复杂交互，这限制了其在多智能体环境中的应用。

**Method:** 提出了MultiWorld框架，包含多智能体条件模块实现精确的多智能体可控性，以及全局状态编码器确保不同视角观测的连贯性。该框架支持智能体和视角数量的灵活扩展，并能并行合成不同视角以提高效率。

**Result:** 在多人游戏环境和多机器人操作任务上的实验表明，MultiWorld在视频保真度、动作跟随能力和多视角一致性方面均优于基线方法。

**Conclusion:** MultiWorld成功解决了多智能体多视角世界建模的挑战，为复杂交互环境中的世界建模提供了有效解决方案，具有良好的扩展性和实用性。
...
### [465] [Back into Plato's Cave: Examining Cross-modal Representational Convergence at Scale](https://arxiv.org/abs/2604.18572)
*A. Sophia Koepke,Daniil Zverev,Shiry Ginosar,Alexei A. Efros*

Main category: cs.CV

> 本文质疑了柏拉图表示假说，发现跨模态对齐的证据很脆弱，随数据规模增大而显著下降，主要反映粗糙语义重叠而非精细结构的一致性。

<details>
  <summary>Details</summary>

**Motivation:** 柏拉图表示假说声称不同模态训练的神经网络会收敛到相同的现实表示，这可能意味着模态选择不重要。作者认为需要严格验证这一重要假说的证据强度。

**Method:** 通过扩展数据集规模（从1K到数百万样本）测试模型表示对齐，分析一对多和多对多场景下的对齐表现，以及评估更新语言模型与视觉模型的对齐趋势。

**Result:** 发现对齐程度在小数据集上较高，但随数据规模扩大大幅下降；剩余对齐主要反映粗糙语义重叠；在一对多设置下对齐进一步降低；新模型未呈现更强的语言-视觉对齐趋势。

**Conclusion:** 当前跨模态表示收敛的证据比后续研究认为的要弱得多。不同模态训练的模型可能学习到同样丰富的世界表示，但并非相同的表示。
...
### [466] [T-REN: Learning Text-Aligned Region Tokens Improves Dense Vision-Language Alignment and Scalability](https://arxiv.org/abs/2604.18573)
*Savya Khosla,Sethuraman T,Aryan Chadha,Alex Schwing,Derek Hoiem*

Main category: cs.CV

> T-REN是一种高效的多模态编码器，通过将视觉数据映射到文本对齐的区域级表示，在显著减少token数量的同时大幅提升密集交叉模态理解性能，参数仅增加3.7%

<details>
  <summary>Details</summary>

**Motivation:** 现有的视觉语言编码器存在两大核心问题：(1)语言与密集视觉特征对齐较弱，影响开放词汇语义分割等任务性能；(2)细粒度视觉表示token数量过多，限制了在长视频上的可扩展性

**Method:** 提出T-REN（文本对齐区域编码网络），在冻结的视觉骨干网上添加轻量级网络，训练将每个语义区域内的块级表示聚合成区域token，并与区域级文本注释对齐

**Result:** 在多个基准测试中取得显著提升：ADE20K开放词汇分割+5.9 mIoU，COCO目标级图文检索+18.4%召回率，Ego4D视频目标定位+15.6%召回率，VSPW视频场景解析+17.6% mIoU，同时图像和视频的token数量分别减少24倍和187倍

**Conclusion:** T-REN以极小的参数代价解决了视觉语言编码器的两个核心限制，实现了更强的密集交叉模态理解和更好的可扩展性，为开放词汇分割和视频理解等任务提供了有效解决方案
...
### [467] [ReCap: Lightweight Referential Grounding for Coherent Story Visualization](https://arxiv.org/abs/2604.18575)
*Aditya Arora,Akshita Gupta,Pau Rodriguez,Marcus Rohrbach*

Main category: cs.CV

> ReCap是一个轻量级故事可视化一致性框架，通过CORE模块处理代词指代和SemDrift语义漂移校正，仅增加149K参数就在角色一致性上超越了现有方法。

<details>
  <summary>Details</summary>

**Motivation:** 传统故事可视化方法需要显式记忆库、架构扩展或辅助语言模型来维持跨帧一致性，导致参数量大、推理开销高，亟需轻量级解决方案。

**Method:** ReCap包含两个核心组件：1) CORE模块将代词作为视觉锚点，仅在角色被代词指代时激活并基于前一帧条件化传播视觉身份；2) SemDrift在训练时通过将去噪器表示与预训练DINOv3视觉嵌入对齐来纠正语义漂移。

**Result:** 在FlintstonesSV和PororoSV两个主要基准上，ReCap分别以2.63%和5.65%的字符准确率优势超越StoryGPT-V，建立了新的角色一致性最优水平，并成功扩展到真实电影的人类中心叙事。

**Conclusion:** ReCap证明了通过选择性地激活条件化和语义对齐，可以在不修改基础扩散模型的情况下显著提升角色稳定性，为故事可视化提供了高效的轻量级解决方案。
...
### [468] [MUA: Mobile Ultra-detailed Animatable Avatars](https://arxiv.org/abs/2604.18583)
*Heming Zhu,Guoxing Sun,Marc Habermann*

Main category: cs.CV

> 提出小波引导的多级空间分解混合形状表示方法，通过知识蒸馏将超高质量模型的动态细节转移到轻量级表示中，实现2000倍计算量降低和10倍模型缩小，同时保持高保真效果。

<details>
  <summary>Details</summary>

**Motivation:** 现有可动画数字人模型面临两难选择：超高质量模型需要大量计算资源只能在服务器运行，而轻量级模型动态表现有限、外观细节不足且有明显伪影，无法同时满足高保真和实时部署需求。

**Method:** 结合多级小波频谱分解和纹理空间的低秩结构分解，设计新型可动画头像表示，并提出相应的知识蒸馏管道，将预训练超高质量模型的运动感知衣物动态和精细外观细节转移到紧凑高效的表示中。

**Result:** 相比原始高质量教师模型，计算成本降低2000倍，模型大小缩小10倍，在台式PC上达到180+ FPS，在Meta Quest 3上实现24 FPS的实时设备端性能，显著优于移动端方法，渲染质量媲美或超越多数服务器端方法。

**Conclusion:** 该表示方法显著提升了高保真头像在沉浸式应用中的实用性，首次实现了在资源受限设备上部署接近服务器质量的超保真可动画数字人，为移动VR等应用场景提供了重要突破。
